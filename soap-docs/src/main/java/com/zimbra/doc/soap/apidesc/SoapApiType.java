...
/**
 * Information about a "type". i.e. Associated class name, description and ID information for child
 * nodes
 *
 * @author gren
 */
public class SoapApiType {
  ...
  private ValueDescription valueType =
      null; // Value associated with field annotated with @XmlValue (if present)
  private String valueJaxb =
      null; // Class associated with field annotated with @XmlValue (if present)
  ...
  public List<SoapApiElementArtifact> getSubElements() {
    return subElements;
  }

  public List<SoapApiAttribute> getAttributes() {
    return attributes;
  }
  ...
  public ValueDescription getType() {
    return type;
  }
