...
public class ZGrant implements ACLGrant, ToZJSONObject {
  ...
  public enum GranteeType {
    ...
    guest(
        com.zimbra.soap.type.GrantGranteeType.guest,
        com.zimbra.common.mailbox.GrantGranteeType.guest),
    ...
    GranteeType(
        com.zimbra.soap.type.GrantGranteeType jaxbGT,
        com.zimbra.common.mailbox.GrantGranteeType commGT) {
  ...
  public void toElement(Element parent) {
    ...
    if (mPermissions != null) grant.addAttribute(MailConstants.A_RIGHTS, mPermissions);

    grant.addAttribute(MailConstants.A_GRANT_TYPE, mGranteeType.name());

    if (mGranteeId != null) grant.addAttribute(MailConstants.A_ZIMBRA_ID, mGranteeId);

    if (mGranteeName != null) grant.addAttribute(MailConstants.A_DISPLAY, mGranteeName);

    if (mArgs != null && mArgs.length() > 0) {
      if (mGranteeType == GranteeType.key) grant.addAttribute(MailConstants.A_ACCESSKEY, mArgs);
      else grant.addAttribute(MailConstants.A_ARGS, mArgs);
    }
  }
  ...
  /**
   * the type of grantee: "usr", "grp", "dom" (domain), "all" (all authenticated users), "pub"
   * (public authenticated and unauthenticated access), "guest" (non-Zimbra email address and
   * password) "key" (access key)
   */
