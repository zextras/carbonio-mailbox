...
public class ZSimpleRecurrence {
  ...
  public enum ZSimpleRecurrenceType {
      ...
      try {
        ...
        throw ZClientException.CLIENT_ERROR(
            "invalid type "
                + s
                + ", valid values: "
                + Arrays.asList(ZSimpleRecurrenceType.values()),
            e);
      }
    }
    ...
    public boolean isDailyWeekday() {
      return this.equals(DAILY_WEEKDAY);
    }

    public boolean isDailyInterval() {
      return this.equals(DAILY_INTERVAL);
    }
    ...
    public boolean isWeeklyByDay() {
      return this.equals(WEEKLY_BY_DAY);
    }

    public boolean isWeeklyCustom() {
      return this.equals(WEEKLY_CUSTOM);
    }
    ...
    public boolean isMonthlyByMonthDay() {
      return this.equals(MONTHLY_BY_MONTH_DAY);
    }

    public boolean isMonthlyRelative() {
      return this.equals(MONTHLY_RELATIVE);
    }
    ...
    public boolean isYearlyByDate() {
      return this.equals(YEARLY_BY_DATE);
    }

    public boolean isYearlyRelative() {
      return this.equals(YEARLY_RELATIVE);
    }
  ...
  public enum ZSimpleRecurrenceEnd {
    ...
    public static ZSimpleRecurrenceEnd fromString(String s) throws ServiceException {
      try {
        ...
        throw ZClientException.CLIENT_ERROR(
            "invalid end " + s + ", valid values: " + Arrays.asList(ZSimpleRecurrenceEnd.values()),
            e);
      }
    }
    ...
    switch (rr.getFrequency()) {
      case DAI:
        if (empty(byRules)) {
          ...
          return;
        } else if (byRules != null
            && byRules.size() == 1
            && byRules.get(0).getType() == ZByType.BY_DAY) {
          ...
          if (!empty(weekDays)) {
            ...
            for (ZByDayWeekDay d : weekDays) {
              if (d.getWeekOrd() == 0) days.add(d.getDay());
            }
            if (days.size() == 5
                && days.containsAll(
                    EnumSet.of(ZWeekDay.MO, ZWeekDay.TU, ZWeekDay.WE, ZWeekDay.TH, ZWeekDay.FR))) {
      ...
      case MON:
        if (empty(byRules) && rr.getInterval() < 2 && mEnd == ZSimpleRecurrenceEnd.NEVER) {
          ...
          return;
            ...
            && byRules.size() == 1
            && byRules.get(0).getType() == ZByType.BY_MONTHDAY) {
          ZByRule br = byRules.get(0);
  ...
  private ZByRule findType(List<ZByRule> byRules, ZByType type) {
    for (ZByRule rule : byRules) if (rule.getType() == type) return rule;
    return null;
  }
  ...
  public ZRecurrence getRecurrence() {
    ...
    if (getType().isComplex() || getType().isNone()) return recur;
    ...
    switch (getType()) {
        ...
        rule.getByRules()
            .add(
                new ZByRule(
                    ZByType.BY_DAY,
                    null,
                    ZByDayWeekDay.getList(
                        ZWeekDay.MO, ZWeekDay.TU, ZWeekDay.WE, ZWeekDay.TH, ZWeekDay.FR)));
        break;
      case WEEKLY:
      ...
      case WEEKLY_BY_DAY:
        ...
        rule.setByRules(new ArrayList<ZByRule>());
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_DAY, null, ZByDayWeekDay.getList(getWeeklyByDay())));
        break;
      case WEEKLY_CUSTOM:
        rule.setFrequency(ZFrequency.WEE);
        ...
        rule.setByRules(new ArrayList<ZByRule>());
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_DAY, null, ZByDayWeekDay.getList(getWeeklyIntervalDays())));
        break;
      case MONTHLY:
      ...
      case MONTHLY_BY_MONTH_DAY:
        ...
        rule.setByRules(new ArrayList<ZByRule>());
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_MONTHDAY, Integer.toString(getMonthlyMonthDay()), null));
        break;
      case MONTHLY_RELATIVE:
        ...
        rule.setByRules(new ArrayList<ZByRule>());
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_DAY, null, Arrays.asList(getMonthlyRelativeDay())));
        break;
      case YEARLY:
      ...
      case YEARLY_BY_DATE:
        ...
        rule.getByRules()
            .add(
                new ZByRule(
                    ZByType.BY_MONTHDAY, Integer.toString(getYearlyByDateMonthDay()), null));
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_MONTH, Integer.toString(getYearlyByDateMonth()), null));
        break;
      case YEARLY_RELATIVE:
        rule.setFrequency(ZFrequency.YEA);
        rule.setByRules(new ArrayList<ZByRule>());
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_DAY, null, Arrays.asList(getYearlyRelativeDay())));
        rule.getByRules()
            .add(new ZByRule(ZByType.BY_MONTH, Integer.toString(getYearlyRelativeMonth()), null));
        break;
    }
