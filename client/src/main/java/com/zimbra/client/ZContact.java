...
public class ZContact extends ZBaseItem implements ToZJSONObject {
  ...
  public ZContact(Element e, ZMailbox zmailbox) throws ServiceException {
    super(
        e.getAttribute(MailConstants.A_ID),
    ...
    HashMap<String, ZContactAttachmentInfo> attachments =
        new HashMap<String, ZContactAttachmentInfo>();
    ...
    for (Element memberEl : e.listElements(MailConstants.E_CONTACT_GROUP_MEMBER)) {
      ...
      if (cnEl != null) contact = new ZContact(cnEl, type.equals("G") ? true : false, zmailbox);
      else
  ...
  public String getDisplayEmail() {
    if (!StringUtil.isNullOrEmpty(getAttrs().get("email"))) return getAttrs().get("email");
    else if (!StringUtil.isNullOrEmpty(getAttrs().get("email2"))) return getAttrs().get("email2");
    else if (!StringUtil.isNullOrEmpty(getAttrs().get("email3"))) return getAttrs().get("email3");
    else return "";
  }
  ...
  public void tag(String nameOrId, boolean tagged) throws ServiceException {
    ...
    if (tag == null) throw ZClientException.CLIENT_ERROR("unknown tag: " + nameOrId, null);
    else tag(tag, tagged);
  }
