...
public class ZContactByPhoneCache extends ZEventHandler {
  ...
  private static List<String> sATTRS =
      Arrays.asList(
          ContactConstants.A_homePhone,
          ContactConstants.A_homePhone2,
          ContactConstants.A_mobilePhone,
          ContactConstants.A_otherPhone,
          ContactConstants.A_workPhone,
          ContactConstants.A_workPhone2,
          ContactConstants.A_homeFax,
          ContactConstants.A_workFax,
          ContactConstants.A_workMobile,
          ContactConstants.A_workAltPhone,
          ContactConstants.A_otherFax,
          ContactConstants.A_assistantPhone,
  ...
  public synchronized ContactPhone getByPhone(String phone, ZMailbox mailbox)
      throws ServiceException {
    if (mCleared) init(mailbox);
  ...
  private synchronized void init(ZMailbox mailbox) throws ServiceException {
    ...
    for (ZContact contact : contacts) {
      if (!contact.getFolderId().equals(ZFolder.ID_TRASH)) addContact(contact);
    }
    mCleared = false;
  }
