...
public class ZEmailAddress implements ToZJSONObject {
  ...
  /**
   * (f)rom, t(o), c(c), (s)ender, (r)eply-to, b(cc). Type is only sent when an individual message
   * message is returned. In the list of conversations, all the email addresseses returned for a
   * conversation are a subset of the participants. In the list of messages in a converstation, the
   * email addressses are the senders.
   */
  ...
  private String quoteAddress(String addr) {
    if (addr == null) return "";
    else if (addr.startsWith("<")) return addr;
    else return "<" + addr + ">";
  }
  ...
  public String getFullAddress() {
    try {
      if (personal == null) return address;
      else return new JavaMailInternetAddress(address, personal).toUnicodeString();
  ...
  public boolean isBcc() {
    return ZEmailAddress.EMAIL_TYPE_BCC.equals(getType());
  }

  public boolean isCc() {
    return ZEmailAddress.EMAIL_TYPE_CC.equals(getType());
  }

  public boolean isFrom() {
    return ZEmailAddress.EMAIL_TYPE_FROM.equals(getType());
  }

  public boolean isSender() {
    return ZEmailAddress.EMAIL_TYPE_SENDER.equals(getType());
  }

  public boolean isTo() {
    return ZEmailAddress.EMAIL_TYPE_TO.equals(getType());
  }

  public boolean isReplyTo() {
    return ZEmailAddress.EMAIL_TYPE_REPLY_TO.equals(getType());
  }
   ...
   */
  public static List<ZEmailAddress> parseAddresses(String line, String type)
      throws ServiceException {
    try {
      line = line.replace(";", ",");
      ...
      for (InternetAddress ia : inetAddrs) {
        result.add(
            new ZEmailAddress(ia.getAddress().replaceAll("\"", ""), null, ia.getPersonal(), type));
      }
      return result;
