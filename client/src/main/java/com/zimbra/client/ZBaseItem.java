...
public class ZBaseItem implements ZItem, ZimbraMailItem {
  ...
  public int getModifiedSequence() {
    ...
    try {
      ...
      ZimbraLog.mailbox.debug(
          "ZBaseItem getModifiedSequence() - getItemById failed for mailbox=%s item=%s",
          mMailbox, getId(), e);
      return 0;
    }
    ...
    modifiedSequence =
        (((ZBaseItem) zmi).modifiedSequence <= 0) ? 0 : ((ZBaseItem) zmi).modifiedSequence;
  ...
  /**
   * Returns the "external" flag bitmask, which includes {@link ZItem.Flag#BITMASK_UNREAD} when the
   * item is unread.
   */
  @Override
  ...
  public InputStream getContentStream() throws ServiceException {
    String contentPath =
        String.format("%s/get?id=%s", AccountConstants.CONTENT_SERVLET_PATH, getId());
  ...
  /**
   * @return item's ID. IDs are unique within a Mailbox and are assigned in increasing (though not
   *     necessarily gap-free) order.
   */
  @Override
  public String getId() {
    return mId;
