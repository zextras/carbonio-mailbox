...
public class ZMessage extends ZBaseItem implements ToZJSONObject {
  ...
  public ZMessage(Element e, ZMailbox zmailbox) throws ServiceException {
    ...
    List<Element.KeyValuePair> hdrs =
        e.listKeyValuePairs(MailConstants.A_HEADER, MailConstants.A_ATTRIBUTE_NAME);
    if (hdrs != null) {
    ...
    if (mp != null) mMimeStructure = new ZMimePart(null, mp);

    Element inviteEl = e.getOptionalElement(MailConstants.E_INVITE);
    if (inviteEl != null) mInvite = new ZInvite(inviteEl);
  ...
  /**
   * content of the message, if raw is specified. if message too big or not ASCII, a content servlet
   * URL is returned
   */
  ...
  public void update(String destFolderId, String tagList, String flags) throws ServiceException {
    getMailbox()
        .updateMessage(getId(), destFolderId, tagList, flags); // TODO: simplify tags/folders
  }
