...
public class ZAce implements ToZJSONObject {
  ...
  public void toElement(Element parent) {
    ...
    if (mGranteeType == GranteeType.gst) ace.addAttribute(MailConstants.A_PASSWORD, mSecret);
    else if (mGranteeType == GranteeType.key) ace.addAttribute(MailConstants.A_ACCESSKEY, mSecret);

    if (mDeny) ace.addAttribute(MailConstants.A_DENY, mDeny);
  }
  ...
  /**
   * the type of grantee: "usr", "grp", "all" (all authenticated users), "guest" (non-Zimbra email
   * address and password) "pub" (public authenticated and unauthenticated access),
   */
  ...
  public String getGranteeName() {
    if (mGranteeName == null) return "";
    else return mGranteeName;
  }
