...
class ZSearchPagerCache extends ZEventHandler {
  ...
  public synchronized ZSearchPagerResult search(
      ZMailbox mailbox, ZSearchParams params, int page, boolean useCache, boolean useCursor)
  ...
  public synchronized void clear(String type) {
    if (type == null) {
      ...
      while (mi.hasNext()) {
        ...
        if (params.getTypes() != null && params.getTypes().contains(type)) mi.remove();
      }
    }
  }

  public synchronized void handleRefresh(ZRefreshEvent refreshEvent, ZMailbox mailbox)
      throws ServiceException {
  ...
  public synchronized void handleModify(ZModifyEvent event, ZMailbox mailbox)
    ...
    if (mClearOnModifyItemFolder && event instanceof ZModifyItemFolderEvent) {
      ...
      if (mif.getFolderId(null) != null) mSearchPagerCache.clear();
    }
