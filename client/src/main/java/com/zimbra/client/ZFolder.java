...
public class ZFolder implements ZItem, FolderStore, Comparable<Object>, ToZJSONObject {
  ...
  public static final String ID_USER_ROOT =
      Integer.toString(FolderConstants.ID_FOLDER_USER_ROOT); // "1"
  ...
  public static final String ID_CONTACTS =
      Integer.toString(FolderConstants.ID_FOLDER_CONTACTS); // "7"
  public static final String ID_TAGS = Integer.toString(FolderConstants.ID_FOLDER_TAGS); // "8"
  public static final String ID_CONVERSATIONS =
      Integer.toString(FolderConstants.ID_FOLDER_CONVERSATIONS); // "9"
  ...
  public static final String ID_BRIEFCASE =
      Integer.toString(FolderConstants.ID_FOLDER_BRIEFCASE); // "16"
  ...
  public int compareTo(Object obj) {
    if (!(obj instanceof ZFolder)) return 0;
  ...
  public static class Color {
    ...
    public static final Color DEFAULTCOLOR = new Color("defaultColor", 0);
    public static final Color BLUE = new Color("blue", 1);
    public static final Color CYAN = new Color("cyan", 2);
    public static final Color GREEN = new Color("green", 3);
    public static final Color PURPLE = new Color("purple", 4);
    public static final Color RED = new Color("red", 5);
    public static final Color YELLOW = new Color("yellow", 6);
    public static final Color PINK = new Color("pink", 7);
    ...
    public static Color fromString(String s) throws ServiceException {
      ...
      if (colorMap.containsKey(s)) {
        ...
        throw ZClientException.CLIENT_ERROR(
            "invalid color: " + s + ", valid values: " + colorMap.keySet(), null);
      }
    }
  ...
  public void modifyNotification(ZModifyEvent event) throws ServiceException {
    if (event instanceof ZModifyFolderEvent) {
      ...
      // SOAP returns 'i4n' only when it is different from 'n'. Therefore, fall back to
      // mMessageCount
  ...
  public ItemIdentifier getFolderItemIdentifier() {
    ...
    try {
      ...
      ZimbraLog.mailbox.debug(
          "Problem understanding folderId '%s' - assume hidden", getFolderIdAsString(), e);
      throw new RuntimeException(
          String.format(
              "Problem understanding folderId '%s' - assume hidden", getFolderIdAsString()),
          e);
    }
  }
   ...
   * FolderConstants#ID_FOLDER_USER_ROOT}) are visible; all others are hidden.
   */
  @Override
  ...
  /**
   * Returns the folder's name. Note that this is the folder's name (e.g. <code>"foo"</code>), not
   * its absolute pathname (e.g. <code>"/baz/bar/foo"</code>).
   *
   * @see #getPath()
   */
  @Override
  ...
  /**
   * Returns the folder's absolute path, url-encoded. Paths are UNIX-style with <code>'/'</code> as
   * the path delimiter. Paths are relative to the user root folder, which has the path <code>"/"
   * </code>. So the Inbox's path is <code>"/Inbox"</code>, etc.
   */
  ...
  /** Returns the "hint" as to which view to use to display the folder's contents. */
  ...
  public void moveToTrash() throws ServiceException {
    mMailbox.trashFolder(mId);
  }

  public void sync() throws ServiceException {
    mMailbox.syncFolder(mId);
  }
  ...
  public void modifyFlags(String flags) throws ServiceException {
    mMailbox.updateFolder(mId, null, null, null, null, flags, null);
  }

  public void modifyURL(String url) throws ServiceException {
    mMailbox.modifyFolderURL(mId, url);
  }

  public void rename(String newPath) throws ServiceException {
    mMailbox.renameFolder(mId, newPath);
  }
  ...
  public void updateFolder(
      String name, String parentId, Color newColor, String rgbColor, String flags, List<ZGrant> acl)
