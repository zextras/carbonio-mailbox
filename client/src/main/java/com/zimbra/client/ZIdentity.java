...
public class ZIdentity implements ToZJSONObject {
  ...
  public boolean getIsDefault() {
    return ProvisioningConstants.DEFAULT_IDENTITY_NAME.equals(getRawName());
  }

  public String getFromAddress() {
    return get(ZAttrProvisioning.A_zimbraPrefFromAddress);
  }

  public String getFromDisplay() {
    return get(ZAttrProvisioning.A_zimbraPrefFromDisplay);
  }

  public ZEmailAddress getFromEmailAddress() {
    return new ZEmailAddress(
        getFromAddress(), null, getFromDisplay(), ZEmailAddress.EMAIL_TYPE_FROM);
  }

  public String getSignatureId() {
    return get(ZAttrProvisioning.A_zimbraPrefDefaultSignatureId);
  }

  public String getReplyToAddress() {
    return get(ZAttrProvisioning.A_zimbraPrefReplyToAddress);
  }

  public String getReplyToDisplay() {
    return get(ZAttrProvisioning.A_zimbraPrefReplyToDisplay);
  }

  public ZEmailAddress getReplyToEmailAddress() {
    return new ZEmailAddress(
        getReplyToAddress(), null, getReplyToDisplay(), ZEmailAddress.EMAIL_TYPE_REPLY_TO);
  }

  public boolean getReplyToEnabled() {
    return getBool(ZAttrProvisioning.A_zimbraPrefReplyToEnabled);
  }
  ...
  public boolean getWhenInFoldersEnabled() {
    return getBool(ZAttrProvisioning.A_zimbraPrefWhenInFoldersEnabled);
  }
  ...
  public Element toElement(Element parent) {
    ...
    identity
        .addAttribute(AccountConstants.A_NAME, getRawName())
        .addAttribute(AccountConstants.A_ID, getId());
    for (Map.Entry<String, String> entry : data.getAttrsMultimap().entries()) {
      identity.addKeyValuePair(
          entry.getKey(), entry.getValue(), AccountConstants.E_A, AccountConstants.A_NAME);
    }
    return identity;
  }
