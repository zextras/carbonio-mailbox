...
public class ZSearchParams implements ToZJSONObject, ZimbraSearchParams {
  ...
  public static String getCanonicalTypes(String list) throws ServiceException {
    ...
    for (String s : list.split(",")) {
      ...
      if (s.startsWith("conv") && TYPE_CONVERSATION.startsWith(s)) sb.append(TYPE_CONVERSATION);
      ...
      else if (s.startsWith("b") && TYPE_BRIEFCASE.startsWith(s)) sb.append(TYPE_BRIEFCASE);
  ...
  /**
   * offset is an integer specifying the 0-based offset into the results list to return as the first
   * result for this search operation.
   */
  private int mOffset;

  /**
   * dateDesc|dateAsc|subjDesc|subjAsc|nameDesc|nameAsc|idDesc|idAsc|readDesc|readAsc (default is
   * "dateDesc")
   */
  private SearchSortBy mSortBy = SearchSortBy.dateDesc;

  /**
   * comma-separated list. Legal values are: conversation|message|contact|appointment|note (default
   * is "conversation" {@link #TYPE_CONVERSATION})
   */
  ...
  /**
   * if recip="1" is specified: + returned sent messages will contain the set of "To:" recipients
   * instead of the sender + returned conversations whose first hit was sent by the user will
   * contain that hit's "To:" recipients instead of the conversation's sender list
   */
  ...
  @Override
  public int hashCode() {
    if (mConvId != null) return (mQuery + mConvId).hashCode();
    else return mQuery.hashCode();
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (this.mLimit != that.mLimit
        ...
        || this.mInDumpster != that.mInDumpster) return false;
    ...
    if (!StringUtil.equal(this.mTypes, that.mTypes)
        ...
        || !StringUtil.equal(this.mField, that.mField)) return false;

    if ((this.mTimeZone == null && that.mTimeZone != null)
        || (this.mTimeZone != null && !this.mTimeZone.equals(that.mTimeZone))) return false;

    if (this.mCursor == null || that.mCursor == null) return this.mCursor == that.mCursor;
    else return this.mCursor.equals(that.mCursor);
  }
  ...
  /**
   * by default, text without an operator searches the CONTENT field. By setting the {field} value,
   * you can control the default operator. Specify any of the text operators that are available in
   * query.txt, e.g. 'content:' [the default] or 'subject:', etc. The date operators (date, after,
   * before) and the "item:" operator should not be specified as default fields because of quirks in
   * the search grammar.
   *
   * @param field
   */
  ...
  public static class Cursor implements ToZJSONObject {
    ...
    /**
     * cursorPreviousId and cursorPreviousSortValue correspond to the last hit on the previous page
     * (assuming you're going forward -- if you're backing up then th ey should be the first hit on
     * the previous page)....the server uses those parameters to find the spot in the new results
     * that corresponds to your old position: even if some entries have been removed or added to the
     * search results (e.g. if you are searching is:unread and you read some)
     */
