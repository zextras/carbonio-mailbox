...
public class ZApptSummaryCache extends ZEventHandler {
  ...
  synchronized void add(ZApptSummaryResult result, TimeZone timezone) {
    ...
    mResults.put(
        makeKey(
            result.getStart(), result.getEnd(), result.getFolderId(), timezone, result.getQuery()),
        result);
  }

  synchronized ZApptSummaryResult get(
      long start, long end, String folderId, TimeZone timezone, String query) {
    if (query == null) query = "";
    ...
    if (result == null && (end - start) < MSECS_PER_MONTH_GRID) {
      ...
      for (ZApptSummaryResult cached : mResults.values()) {
        if (cached.getQuery().equals(query)
            && cached.getTimeZone().getID().equals(timezone.getID())
            && cached.getFolderId().equals(folderId)
            && (cached.getStart() <= start && end <= cached.getEnd())) {
          List<ZAppointmentHit> appts = new ArrayList<ZAppointmentHit>();
          for (ZAppointmentHit appt : cached.getAppointments()) {
            if (appt.isInRange(start, end)) appts.add(appt);
          }
