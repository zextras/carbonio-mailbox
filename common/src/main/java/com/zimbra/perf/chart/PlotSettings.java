...
public class PlotSettings {
  ...
  public PlotSettings(
    ...
    if (mPercentTime) mNonNegative = true;
    ...
    if (mLegend == null || mLegend.length() < 1) mLegend = mDataColumn;
    if (mLegend == null) // must be a ratio
    mLegend = "(" + mRatioTop + ")" + " / " + "(" + mRatioBottom + ")";

    if (mInfile == null || mInfile.length() < 1)
      throw new IllegalArgumentException("Missing infile attribute on plot " + mLegend);

    if (!mShowRaw && !mShowMovingAvg)
      throw new IllegalArgumentException(
          "At least one of showRaw, showSum and showMovingAvg "
              + "must be true in plot "
              + mLegend);

    if (mDivisor == 0.0)
      throw new IllegalArgumentException("Divisor of 0 is not allowed in plot " + mLegend);
  }
