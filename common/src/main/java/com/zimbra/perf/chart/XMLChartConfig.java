...
public class XMLChartConfig {
  ...
  private static String getAttr(Element elem, String name) throws DocumentException {
    ...
    if (val != null) return val;
      ...
      throw new DocumentException(
          "Missing required attribute " + name + " in element " + elem.getName());
  }
  ...
  private static int getInheritedAttrInt(Element elem, String name, int defaultValue)
  ...
  private static double getInheritedAttrDouble(Element elem, String name, double defaultValue)
    ...
    if (val != null) {
      ...
      String digits;
      if (multiplier == 1) digits = val;
      else digits = val.substring(0, len - 1);
      try {
   ...
   * @throws DocumentException if there was an error parsing the file
   * @throws XmlParseException
   * @throws IllegalArgumentException if any attribute has invalid value
   */
  public static List<ChartSettings> load(File xmlFile)
    ...
    for (Iterator iter = chartsElem.elementIterator(E_CHART); iter.hasNext(); ) {
      ...
      boolean allowLogScale =
          getInheritedAttrBoolean(
              chartElem, A_CHART_ALLOW_LOG_SCALE, ChartSettings.DEFAULT_CHART_ALLOW_LOG_SCALE);
      boolean plotZero =
          getInheritedAttrBoolean(
              chartElem, A_CHART_PLOT_ZERO, ChartSettings.DEFAULT_CHART_PLOT_ZERO);
      if (!allowLogScale) plotZero = true;
      int width = getInheritedAttrInt(chartElem, A_CHART_WIDTH, ChartSettings.DEFAULT_CHART_WIDTH);
      int height =
          getInheritedAttrInt(chartElem, A_CHART_HEIGHT, ChartSettings.DEFAULT_CHART_HEIGHT);
      String outDoc = getAttr(chartElem, A_CHART_DOCUMENT, null);
      String topPlotStr = getAttr(chartElem, A_CHART_TOP_PLOTS, null);
      int topPlots = -1;
      if (topPlotStr != null) topPlots = Integer.parseInt(topPlotStr);
      ...
      for (Iterator plotIter = chartElem.elementIterator(E_PLOT); plotIter.hasNext(); ) {
        ...
        boolean showRaw =
            getInheritedAttrBoolean(plotElem, A_PLOT_SHOW_RAW, PlotSettings.DEFAULT_PLOT_SHOW_RAW);
        boolean showMovingAvg =
            getInheritedAttrBoolean(
                plotElem, A_PLOT_SHOW_MOVING_AVG, PlotSettings.DEFAULT_PLOT_SHOW_MOVING_AVG);
        int movingAvgPoints =
            getInheritedAttrInt(
                plotElem, A_PLOT_MOVING_AVG_POINTS, PlotSettings.DEFAULT_PLOT_MOVING_AVG_POINTS);
        double multiplier =
            getInheritedAttrDouble(
                plotElem, A_PLOT_MULTIPLIER, PlotSettings.DEFAULT_PLOT_MULTIPLIER);
        double divisor =
            getInheritedAttrDouble(plotElem, A_PLOT_DIVISOR, PlotSettings.DEFAULT_PLOT_DIVISOR);

        boolean nonNegative =
            getInheritedAttrBoolean(
                plotElem, A_PLOT_NON_NEGATIVE, PlotSettings.DEFAULT_PLOT_NON_NEGATIVE);
        boolean percentTime =
            getInheritedAttrBoolean(
                plotElem, A_PLOT_PERCENT_TIME, PlotSettings.DEFAULT_PLOT_PERCENT_TIME);
        String dataFunction =
            getAttr(plotElem, A_PLOT_DATA_FUNCTION, PlotSettings.DEFAULT_PLOT_DATA_FUNCTION);
        String aggFunction =
            getAttr(
                plotElem, A_PLOT_AGGREGATE_FUNCTION, PlotSettings.DEFAULT_PLOT_AGGREGATE_FUNCTION);
        ...
        boolean optional =
            getInheritedAttrBoolean(plotElem, A_PLOT_OPTIONAL, PlotSettings.DEFAULT_PLOT_OPTIONAL);
      ...
      for (Iterator plotIter = chartElem.elementIterator(E_GROUP_PLOT); plotIter.hasNext(); ) {
        ...
        GroupPlotSettings plot =
            new GroupPlotSettings(
                groupBy,
                ignore,
                infile,
                dataCol,
