...
/**
 * For use with regex expressions where you want to place a limit on the computational resources
 * allowed to execute a match. Assumption: Pattern.matcher uses the supplied CharSequence during its
 * computation (rather than taking a copy)
 */
public class AccessBoundedRegex {
  ...
  /**
   * @param patt
   * @param maxAccesses - The maximum number of accesses to characters in the <b>target</b> when
   *     invoking <b>matches</b>
   */
  ...
  /**
   * @throws TooManyAccessesToMatchTargetException if consumed too many resources attempting to
   *     match <b>target</b>
   */
  public boolean matches(CharSequence target) throws TooManyAccessesToMatchTargetException {
    ...
    try {
      ...
      ZimbraLog.misc.trace(
          "AccessBoundedRegex matches=%s pattern='%s' target='%s' charAt() calls=%s of %s",
  ...
  public static class TooManyAccessesToMatchTargetException extends ServiceException {
    ...
    public TooManyAccessesToMatchTargetException(
        int maxAccesses, Pattern pattern, CharSequence target) {
      super(
          String.format(
              "regular expression match involved more than %d accesses for pattern '%s' against"
                  + " string '%s'",
              maxAccesses, pattern, target),
          ServiceException.INTERRUPTED,
          RECEIVERS_FAULT);
    }
  }
}
