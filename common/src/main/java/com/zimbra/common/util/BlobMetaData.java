...
/**
 * This class is used to encode meta-data stored in a hash-table that is bound for a
 * blob_metadata.metadata column (text). The map should consist of keys which are Strings (and don't
 * contain the '=' character), and values which are also Strings (and can contain any value).
 *
 * @since Apr 8, 2004
 * @author schemers
 */
  ...
  public static Map<Object, Object> decode(String metaData) throws BlobMetaDataEncodingException {
    ...
    while (offset < len && (p = metaData.indexOf('=', offset)) != -1) {
      ...
      if (p + value_len > len) {
        throw new BlobMetaDataEncodingException(
            String.format(
                "invalid value length %d for key '%s' at offset %d", value_len, name, offset));
      }
          ...
          try {
            ...
            if (associatedItemId == null) {
              ZimbraLog.mailbox.warn(
                  "Unable to decode BlobMetaData value [%s] due to exception", value, e);
            } else {
              ZimbraLog.mailbox.warn(
