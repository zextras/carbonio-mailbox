...
/**
 * Runs a <tt>Callable</tt> task either once or at a given interval. Subsequent start times for a
 * task are based on the previous completion time.
 *
...
public class TaskScheduler<V> {
  ...
  /**
   * <tt>Callable</tt> wrapper that takes care of catching exceptions and logging errors that occur
   * while running the task.
   *
   * @param <V2> the type returned by the <tt>Callable</tt> task
   */
  private class TaskRunner<V2> implements Callable<V2> {
    Object mId;
    boolean mRecurs;
    ...
    TaskRunner(
        ...
        boolean recurs,
        long intervalMillis,
        List<ScheduledTaskCallback<V2>> callbacks,
        RetryParams retry) {
      mTask = task;
      mId = id;
      mRecurs = recurs;
      ...
      try {
        ...
        if (retry == null) {
          ...
          ZimbraLog.scheduler.debug(
              "Task returned result %s after %d attempts", mLastResult, retry.getNumRetries());
        }
  ...
  public void schedule(
    ...
    TaskRunner<V> runner =
        new TaskRunner<V>(taskId, task, recurs, intervalMillis, mCallbacks, retry);
    runner.mSchedule = mThreadPool.schedule(runner, delayMillis, TimeUnit.MILLISECONDS);
    mRunnerMap.put(taskId, runner);
  }

  /**
   * Returns the <tt>Callable</tt> object for the given <tt>taskId</tt>, or <tt>null</tt> if no
