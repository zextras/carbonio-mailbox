...
public class HtmlBodyTextExtractor extends org.xml.sax.helpers.DefaultHandler {
  ...
  public void startElement(
    ...
    if ("body".equals(element)) inBody = true;
  ...
  public void characters(char[] ch, int offset, int length) {
    if (length == 0 || sb.length() >= maxLength) return;
    ...
    if (inBody) {
      ...
      if (length > 0) {
        if (sb.length() > 0 && (!inCharacters || original != offset)) sb.append(' ');
  ...
  public void endElement(String uri, String localName, String qName) {
    ...
    String element = localName.toLowerCase();
    if ("body".equals(element)) inBody = false;

    inCharacters = false;

    if ("br".equals(element)) {
      if (sb.length() < maxLength) sb.append("\r\n");
    }
  }
