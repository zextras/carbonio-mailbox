...
public class HttpRequestLine {
  ...
  public HttpRequestLine(String requestLine) throws IOException {
    ...
    if (sp1 < 0) throw new IOException("unable to parse method in request-line");
    method = requestLine.substring(0, sp1).toUpperCase();

    int sp2 = requestLine.indexOf(" ", sp1 + 1);
    if (sp2 < 0) throw new IOException("unable to parse URI in request-line");
  ...
  public Map<String, String> getUriParams() {
    if (mUriParams != null) return mUriParams;
