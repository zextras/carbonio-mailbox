...
public class CharsetUtil {
  ...
  private static final Charset WINDOWS_1252 =
      CharsetUtil.toCharset(MimeConstants.P_CHARSET_WINDOWS_1252);
  ...
  private static final Charset WINDOWS_31J =
      CharsetUtil.toCharset(MimeConstants.P_CHARSET_WINDOWS_31J);
  private static final Charset SHIFT_JIS = CharsetUtil.toCharset(MimeConstants.P_CHARSET_SHIFT_JIS);

  /**
   * Returns a {@link Charset} for the {@code name}, or {@code null} if the name is invalid or if
   * the named charset is not supported in this JRE.
   */
  ...
  public static Charset normalizeCharset(Charset charset) {
    if (charset == null) {
      ...
      // windows-1252 is a superset of iso-8859-1 and they're often confused, so use cp1252 in its
      // place
      return WINDOWS_1252;
  ...
  /**
   * Determines whether {@code data} can be encoded using {@code requestedCharset}.
   *
   * @param data the data to be encoded
   * @param requestedCharset the character set
   * @return {@code requestedCharset} if encoding is supported, "<tt>utf-8</tt>" if not, or
  ...
  public static String checkCharset(String data, String requestedCharset) {
    ...
    if (requestedCharset != null
        && !requestedCharset.isEmpty()
        && !requestedCharset.equalsIgnoreCase("utf-8")) {
      try {
