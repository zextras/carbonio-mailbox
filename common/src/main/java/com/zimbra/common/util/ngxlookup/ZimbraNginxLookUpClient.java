...
public class ZimbraNginxLookUpClient {
  ...
  public void setAttributes(
      String[] lookUpServers, String[] upstreamMailServers, int connectTimeout, int retryTimeout) {
    this.ngxLookUpServers = parseServerList(lookUpServers, DEFAULT_NGINX_HANDLER_PORT);
    ZimbraLog.misc.debug(
        "got %s lookup servers",
        this.ngxLookUpServers == null ? "null" : this.ngxLookUpServers.size());
    this.upstreamMailServers =
        parseServerList(upstreamMailServers, DEFAULT_UPSTREAM_MAIL_SERVER_PORT);
    ZimbraLog.misc.debug(
        "got %s mailstore servers",
        this.upstreamMailServers == null ? "null" : this.upstreamMailServers.size());
  ...
  public String getUpstreamMailServer(String protocol) throws ServiceException {
    ...
    if (upstreamMailServers != null && upstreamMailServers.size() > 0) {
        ...
        if (count >= upstreamMailServers.size()) {
          ...
          String url =
              (new StringBuilder(protocol)
                      .append("://")
                      .append(upstreamMailServer.ngxServerAddress.getHostName())
                      .append(":")
                      .append(upstreamMailServer.ngxServerAddress.getPort()))
                  .toString();
          if (!ping(url, 60000)) {
            continue;
          }
      ...
      return new StringBuilder(upstreamMailServer.ngxServerAddress.getHostName())
          .append(":")
          .append(upstreamMailServer.ngxServerAddress.getPort())
          .toString();
    } else {
    ...
    if (ngxLookUpServers != null && ngxLookUpServers.size() > 0) {
      ...
      do {
        if (count >= ngxLookUpServers.size()) {
          ...
          String hostPath =
              (new StringBuilder()
                      .append(ngxHandler.ngxServerAddress.getHostName())
                      .append(":")
                      .append(ngxHandler.ngxServerAddress.getPort())
                      .append(urlExtension))
                  .toString();
  ...
  /**
   * Pings a HTTP(S) URL. This effectively sends a GET request and returns <code>true</code> if the
  ...
  /**
   * Pings a series of URLs constructed by combining the supplied URL schemes with the given
   * host/path. This effectively sends a GET request and returns <code>true</code> if the response
   * code is 200. Will return <code>true</code> with the first successful request, else <code>false
   * </code> if all fail.
   *
  ...
  public NginxAuthServer getRouteforAccount(
      String userName,
      String authMethod,
      String authProtocol,
      String clientIP,
    ...
    ZimbraLog.misc.debug(
        "getting route for account %s with handler %s", userName, nginxLookUpHandler);
    if (nginxLookUpHandler != null) {
      for (String scheme : asList(ngxSchemes)) {
        HttpGet method =
            new HttpGet(
                (new StringBuilder(scheme + "://")
                        .append(nginxLookUpHandler.ngxServerAddress.getHostName())
                        .append(":")
                        .append(nginxLookUpHandler.ngxServerAddress.getPort())
                        .append(urlExtension))
                    .toString());
        ...
        // currently we use default httpclient_internal_connmgr_connection_timeout instead of
        // ngxConnectTimeout
        ...
        try {
          ...
          if (statusCode == 200 && response.getFirstHeader("Auth-Status").getValue().equals("OK")) {
            return new NginxAuthServer(
                response.getFirstHeader("Auth-Server").getValue(),
                response.getFirstHeader("Auth-Port").getValue(),
  ...
  /**
   * Parse a server list. Each server value is hostname:port or just hostname.
   *
   * @param serverList
   * @return
   */
  private List<Route> parseServerList(String[] servers, int defaultPort) {
    ...
    if (servers != null) {
      ...
      for (String server : servers) {
        ...
        // In case of nginx lookup handlers, there might be additional
        // '/service/extension/nginx-lookup' at the end.
