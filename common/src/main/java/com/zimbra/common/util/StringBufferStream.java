...
public class StringBufferStream extends BufferStream implements Appendable {
  ...
  public StringBufferStream(String cset, long sizeHint, int maxBuffer, long maxSize) {
    ...
    sbuf =
        new StringBuilder((int) Math.min(sizeHint == 0 ? DEFAULT_SIZE_HINT : sizeHint, 2 * 1024));
  }
  ...
  public Appendable append(CharSequence cs) throws IOException {
    flush(cs.length());
    if (cs.length() > sbuf.capacity() - sbuf.length()) write(cs.toString().getBytes(cset));
    else sbuf.append(cs);
    return this;
  }
