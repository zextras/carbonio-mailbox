 ...
 * <p>Purposes:
 *
 * <p>1. There should be a single common MultiThreadedHttpconnectionManager shared by nearly
 * everyone so that sockets can be reused and a configurable limit put on client connections (per
 * remote host and overall).
 *
 * <p>2. HttpClient debug logging can be turned on/off on account level.
 */
public class ZimbraHttpConnectionManager {
  ...
  private static final ZimbraHttpConnectionManager INTERNAL_CONN_MGR =
      new ZimbraHttpConnectionManager(
          "Internal http client connection manager", new InternalConnMgrParams());

  // connection manager for all external connections
  private static final ZimbraHttpConnectionManager EXTERNAL_CONN_MGR =
      new ZimbraHttpConnectionManager(
          "External http client connection manager", new ExternalConnMgrParams());
  ...
  private static class InternalConnMgrParams extends ZimbraConnMgrParams {
    ...
    private InternalConnMgrParams() {
      this.socketConfig =
          SocketConfig.custom()
              .setSoTimeout(LC.httpclient_external_connmgr_so_timeout.intValue())
              .setTcpNoDelay(LC.httpclient_external_connmgr_tcp_nodelay.booleanValue())
              .build();
      this.reqConfig =
          RequestConfig.custom()
              .setStaleConnectionCheckEnabled(
                  LC.httpclient_external_connmgr_stale_connection_check.booleanValue())
  ...
  private ZimbraHttpConnectionManager(String name, ZimbraConnMgrParams zimbraConnMgrParams) {
    ...
    this.httpConnMgr.setDefaultSocketConfig(
   ...
   *
   * <p>*** See "Important notes on using HttpClient returned by ..." above. ***
   *
   * <p>http://hc.apache.org/httpclient-3.x/performance.html says: HttpClient is fully thread-safe
   * when used with a thread-safe connection manager such as MultiThreadedHttpConnectionManager.
   * Please note that each respective thread of execution must have a local instance of HttpMethod
   * and can have a local instance of BasicCookieStore or/and HostConfiguration to represent a
   * specific host configuration and conversational state. At the same time the HttpClient instance
   * and connection manager should be shared among all threads for maximum efficiency.
   *
   * <p>For callsites obtaining the HttpClient instance from this API, if HostConfiguration and/or
   * BasicCookieStore need to be changed on an invocation of HttpClient.executeMethod(), they should
   * use the executeMethod API: executeMethod(HostConfiguration hostconfig, HttpMethod method,
   * HttpState state) where all components for the executeMethod invocation is passed in as
   * parameters.
   *
   * <p>Instead of calling HttpClient.setHostConfiguration(...), HttpClient.setState(...).
   *
   * <p>Also, callsites should *not* alter any state on the returned HttpClient instance by calling
   * any of the HttpClient.set***() methods because this singleton instance is shared by all
   * threads/callsites on the system.
   *
   ...
   * <p>*** See "Important notes on using HttpClient returned by ..." above. ***
   *
   * <p>Callsites of this API are free to alter states of the returned HttpClient instance, because
   * an new instance is created each time this API is called.
   *
   * <p>e.g. many of our callsites use the pattern: BasicCookieStore state = new BasicCookieStore();
   * Cookie cookie = new Cookie(...); state.addCookie(cookie); client.setState(state);
   * client.getParams().setCookiePolicy(CookiePolicy.BROWSER_COMPATIBILITY); executeMethod(method);
   *
   * <p>The above is fine with the HttpClient object returned by this API.
   *
  ...
  private static class IdleReaper {
    ...
    private void startReaperThread() {
      ...
      if (isReaperThreadRunning()) {
        sLog.warn(
            "Cannot start a second http client idle connection reaper thread while another one is"
                + " running.");
        return;
      }
      ...
      if (!reaperEnabled()) {
        sLog.info(
            "Not starting http client idle connection reaper thread for "
                + connMgr.getName()
      ...
      sLog.info(
          "Starting http client idle connection reaper thread for "
              + connMgr.getName()
              + " - reaper sleep interval=%d, reaper connection timeout=%d",
          getReaperSleepInterval(),
          getReaperConnectionTimeout());
    ...
    private void shutdownReaperThread() {
      if (!isReaperThreadRunning()) {
        sLog.warn(
            "shutting down http client idle connection reaper thread requested but the reaper"
