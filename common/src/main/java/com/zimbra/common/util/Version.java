 ...
 *
 * <p>{@code <major number>.<minor number>.<patch number>_<release><release number>_<buildnumber>}
 *
 * <p>e.g.
 *
 * <ul>
...
public final class Version implements Comparable<Version> {
  ...
  public Version(String version, boolean strict) throws ServiceException {
    ...
    if (underscoreAt != -1) {
      ...
      if (matcher.find()) {
        ...
        if (!Strings.isNullOrEmpty(relNum)) mRelNum = Integer.parseInt(relNum);
      }
    }
    ...
    try {
      if (parts.length == 1) mMajor = Integer.parseInt(parts[0]);
   ...
   * </ul>
   *
   * @return a negative integer, zero, or a positive integer as versionX is older than, equal to, or
   *     newer than the versionY.
   */
   ...
   *
   * @param version
   * @return a negative integer, zero, or a positive integer as this object is older than, equal to,
   *     or newer than the specified version.
   */
  ...
  public int compareTo(Version version) {
    if (mFuture) {
      if (version.mFuture) return 0;
      else return 1;
    } else if (version.mFuture) return -1;
  ...
  public boolean isLaterMajorMinorRelease(Version version) {
    return (this.mMajor > version.mMajor
        || (this.mMajor == version.mMajor && this.mMinor > version.mMinor));
  }
