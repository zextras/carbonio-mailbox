...
public class ArrayUtil {
  ...
  public static byte[] toByteArray(Collection<Byte> c) {
    ...
    for (Byte id : c) byteArray[pos++] = id;
    return byteArray;
  }
  ...
  public static int[] toIntArray(Collection<Integer> c) {
    ...
    for (Integer id : c) intArray[pos++] = id;
    return intArray;
  }
  ...
  public byte[] intersectByteArrays(byte[] a1, byte[] a2) {
    ...
    for (int i = toRet.length - 1; i >= 0; i--) {
      assert (byteArrayContains(a1, toRet[i]) && byteArrayContains(a2, toRet[i]));
    }

    return toRet;
  }

  /** Returns the first element, or <tt>null</tt> if the array is <tt>null</tt> or empty. */
