...
public class MapUtil {
  ...
  /**
   * Returns a new {@code LoadingCache} that maps a key to a {@code List} of values. When {@code
   * get()} is called on a key that does not exist in the map, the map implicitly creates a new key
   * that maps to an empty {@code List}.
   */
  public static <K, V> LoadingCache<K, List<V>> newValueListMap() {
    Function<K, List<V>> listCreator =
    ...
    LoadingCache<K, List<V>> cache = CacheBuilder.newBuilder().build(CacheLoader.from(listCreator));
    return cache;
  }

  /**
   * Returns a new {@code LoadingCache} that maps a key to a {@code Set} of values. When {@code
   * get()} is called on a key that does not exist in the map, the map implicitly creates a new key
   * that maps to an empty {@code Set}.
   */
  public static <K, V> LoadingCache<K, Set<V>> newValueSetMap() {
    Function<K, Set<V>> setCreator =
    ...
    LoadingCache<K, Set<V>> cache = CacheBuilder.newBuilder().build(CacheLoader.from(setCreator));
    return cache;
  }

  /**
   * Converts a Guava {@code Multimap} to a {@code Map} that maps a key to a {@code List} of values.
   */
