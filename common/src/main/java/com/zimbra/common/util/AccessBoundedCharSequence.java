 ...
 *
 * <p>Useful for example to guard against too much computational resource being expended on complex
 * regex expressions - see {@link AccessBoundedRegex}
 */
class AccessBoundedCharSequence implements CharSequence {
  ...
  /**
   * @throws RuntimeException if consumed too many computational resources accessing this {@link
   *     CharSequence}
   */
  @Override
  ...
  public CharSequence subSequence(int start, int end) {
    return new AccessBoundedCharSequence(
        sequence.subSequence(start, end), accessCount, maxAccesses);
  }
  ...
  public static class TooManyAccessesToUnderlyingCharSequenceException extends RuntimeException {
    ...
    public TooManyAccessesToUnderlyingCharSequenceException(int accessCount) {
      super(
          String.format(
              "Thread processing CharSequence called charAt more than maximum allowed number of"
                  + " times (%d)",
              accessCount));
    }
  }
