...
public class ListUtil {
  ...
  /** Returns the first element, or {@code null} if the collection is {@code null} or empty. */
  public static <T> T getFirstElement(Iterable<T> iterable) {
    return iterable == null ? null : Iterables.getFirst(iterable, null);
  }

  /**
   * Given two unsorted lists, return TRUE if they contain exactly the same things (regardless of
   * order)
   *
   * @param <T>
   * @param lhs
   ...
   *
   * <p>I could have used the Java Set collection here...unfortunately the only ordered set is their
   * TreeSet, and set union/subtraction therefore take NlogN with a pretty big constant overhead.
   *
   * @param dest
   * @param src
   */
  ...
  /**
   * Splits a <code>Collection</code> into <i>n</i> <code>List</code>s. Lists <i>1</i> through
   * <i>n-1</i> are of size <code>listSize</code>. List <i>n</i> contains the remaining elements.
   *
   * @return the split lists. Returns <tt>null</tt> if <tt>c</tt> is <tt>null</tt> or an empty
   *     <tt>List</tt> if <tt>c</tt> is empty.
   */
  ...
  /**
   * Returns a new {@code List} whose elements are the transformed versions of the data stored in
   * {@code fromIterable}.
   *
   * @param fromIterable source data
   * @param function transformation function
   */
  public static <F, T> List<T> newArrayList(
  ...
  /**
   * Filters the given {@code Iterable} and returns a new {@code List} whose elements match the
   * {@code Predicate}.
   */
