...
public class L10nUtil {
  ...
  /**
   * Get the message for specified key in given locale, applying variable substitutions with any
   * args. ({0}, {1}, etc.)
   *
  ...
  public static String getMessage(
      boolean shoutIfMissing, String basename, String key, Locale lc, Object... args) {
    ...
    try {
      ...
      if (rb == null) {
        if (shoutIfMissing) {
          ZimbraLog.misc.warn(
              "no resource bundle found for (basename='%s', key='%s' locale='%s')",
              basename, key, lc, e);
        } else {
          ZimbraLog.misc.info(
              "no resource bundle found for (basename='%s', key='%s' locale='%s')",
        ...
        if (shoutIfMissing) {
          ZimbraLog.misc.warn(
              "no resource string found in bundle for (basename='%s', key='%s' locale='%s')",
              basename, key, lc, e);
        } else {
          ZimbraLog.misc.info(
              "no resource string found in bundle for (basename='%s', key='%s' locale='%s')",
  ...
  /**
   * Shortned version of the getBundleKeySet method that uses the default message bundle as the base
   * name
   *
  ...
  /**
   * Returns the set of localized server messages for the given keys across all installed locales on
   * the system.
   */
  public static Set<String> getMessagesAllLocales(MsgKey... msgkeys) {
    ...
    for (File file :
        dir.listFiles(new MatchingPropertiesFilter(new String[] {MSG_FILE_BASENAME}))) {
  ...
  /**
   * Returns the locale corresponding to the given properties file. Note that a filename like
   * <tt>ZmMsg.properties</tt> will return <tt>null</tt>, <u>not</u> <tt>en_US</tt>.
   */
  ...
  /**
   * Lookup a Locale object from locale string specified in language[_country[_variant]] format,
   * e.g. en_US.
   *
   * @param name
   * @return
