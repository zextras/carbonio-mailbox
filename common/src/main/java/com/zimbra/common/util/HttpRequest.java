...
public class HttpRequest {
  ...
  public HttpRequest(InputStream input) throws java.io.IOException, java.io.EOFException {
  ...
  /**
   * read the specified number of bytes from the input stream. Throws EOFException if EOF is reached
   * before all the content is read.
   */
  private byte[] readFully(InputStream input, int len) throws IOException {
    ...
    while (n < len) {
      ...
      if (count < 0) throw new EOFException("can't read content");
      // return null;
      n += count;
    }
    return bc;
  ...
  /** read chunkled content body. Returns null if EOF is read before all the content is read. */
  private byte[] readChunkedContent(InputStream input) throws IOException {
    ByteArrayOutputStream baos = null;
    int chunkSize;

    while ((chunkSize = readChunkSize(input)) > 0) {
      if (baos == null) baos = new ByteArrayOutputStream(chunkSize);
  ...
  private void readHttpHeaders(InputStream input) throws java.io.IOException {
    String name = null;
  ...
  private void readHttpRequestLine(InputStream input) throws java.io.IOException {
    String request = readHttpLine(input);
    if (request == null) throw new java.io.EOFException("readHttpRequestLine");
  ...
  private String readHttpLine(InputStream input) throws IOException {
    ...
    while ((ch = input.read()) >= 0) {
      ...
      if (prev_ch == '\r') mLineBuff.write(prev_ch);
      if (ch != '\r') mLineBuff.write(ch);
      prev_ch = ch;
    }
    if (ch == -1) return null;
    else {
  ...
  public String toString() {
    ...
    sb.append(NL);
    for (java.util.Iterator it = mRequestHeaders.keySet().iterator(); it.hasNext(); ) {
