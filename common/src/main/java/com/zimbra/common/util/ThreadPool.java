...
public class ThreadPool implements Executor {
  ...
  public ThreadPool(String name, int poolSize, long timeout) {
    ...
    mPool =
        new ThreadPoolExecutor(
            1, poolSize, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  /**
   * Returns the number of currently active worker threads. An active worker thread is a thread that
   * is currently executing a task, as opposed to a thread that is waiting for a new task to
   * execute.
   */
  ...
  /** Gracefully shutdown the thread pool by waiting for all pending tasks to complete. */
  ...
  private void awaitTermination() {
    try {
      if (!mPool.awaitTermination(mTimeout, TimeUnit.MILLISECONDS))
        mLog.warn("Thread pool did not terminate within " + mTimeout + " milliseconds");
  ...
  private class ThreadCounter {
    ...
    public synchronized void dec() {
      mCount--;
      if (mCount <= 0) notifyAll();
    }
  }

  /**
   * Wrapper class that adds counting logic around a task. Thread pool executes all tasks using this
   * wrapper class to keep track of the number of active worker threads.
   */
