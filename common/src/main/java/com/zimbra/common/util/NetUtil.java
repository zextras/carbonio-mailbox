...
public class NetUtil {
  ...
  public static ServerSocket getSslTcpServerSocket(
      String address, int port, String[] excludeCiphers, String[] includeCiphers)
      throws ServiceException {
    return getServerSocket(
        address,
        port,
        true, /* doesn't matter, but keep it false always */
        false,
        excludeCiphers,
        includeCiphers,
        null);
  ...
  public static void bindSslTcpServerSocket(
      String address, int port, String[] excludeCiphers, String[] includeCiphers)
      throws IOException {
    bindServerSocket(
        address,
        port,
        true, /* doesn't matter, but it false always */
        false,
        excludeCiphers,
        includeCiphers,
        null);
  ...
  public static synchronized ServerSocket getServerSocket(
    ...
    try {
      serverSocket =
          newBoundServerSocket(
              address, port, ssl, useChannels, excludeCiphers, includeCiphers, sslProtocols);
    ...
    if (serverSocket == null) {
      throw ServiceException.FAILURE(
          "Server socket null after bind to port="
              + port
              + " bindaddr="
              + address
              + " ssl="
  ...
  private static ServerSocket newBoundServerSocket(
      String address,
      int port,
      boolean ssl,
      boolean useChannels,
    ...
    if (useChannels) {
      ...
      serverSocketChannel.configureBlocking(
          false); // I believe the only time we use channels is in NIO
   ...
   * @return Array of enabled cipher after excluding the unwanted ones null if either enabledCiphers
   *     or excludeCiphers are null or empty. Callers should not alter the default ciphers on the
   *     SSL socket/engine if computeEnabledCipherSuites returns null.
   */
  public static String[] computeEnabledCipherSuites(
      String[] enabledCiphers, String[] excludeCiphers) {
    if (enabledCiphers == null
        || enabledCiphers.length == 0
        || excludeCiphers == null
        || excludeCiphers.length == 0) return null;
    ...
    for (String cipher : excludedCSList) {
      if (enabledCSList.contains(cipher)) enabledCSList.remove(cipher);
    }

    return enabledCSList.toArray(new String[enabledCSList.size()]);
  }

  private static void setSSLEnabledCipherSuites(
      SSLServerSocket socket, String[] excludeCiphers, String[] includeCiphers) {
    String[] enabledCiphers =
        computeEnabledCipherSuites(
            includeCiphers != null && includeCiphers.length > 0
                ? includeCiphers
                : socket.getEnabledCipherSuites(),
            excludeCiphers);
    if (enabledCiphers != null) socket.setEnabledCipherSuites(enabledCiphers);
  }

  public static void setSSLEnabledCipherSuites(
      SSLSocket socket, String[] excludeCiphers, String[] includeCiphers) {
    String[] enabledCiphers =
        computeEnabledCipherSuites(
            includeCiphers != null && includeCiphers.length > 0
                ? includeCiphers
  ...
  public static void dumpMap() {
    for (Iterator<Map.Entry<String, ServerSocket>> iter = mBoundSockets.entrySet().iterator();
        iter.hasNext(); ) {
  ...
  public static synchronized void bindServerSocket(
      String address,
      int port,
      boolean ssl,
      boolean useChannels,
  ...
  public static synchronized void bindServerSocket(
    ...
    ServerSocket serverSocket =
        NetUtil.newBoundServerSocket(
            address, port, ssl, useChannels, excludeCiphers, includeCiphers, sslProtocols);
    // System.err.println("put table=" + mBoundSockets.hashCode() + " key=" + key + " sock=" +
    // serverSocket);
    mBoundSockets.put(key, serverSocket);
    // dumpMap();
  }

  private static ServerSocket getAlreadyBoundServerSocket(
      String address, int port, boolean ssl, boolean useChannels) {
    // dumpMap();
    String key = makeKey(address, port, ssl, useChannels);
    ServerSocket serverSocket = mBoundSockets.get(key);
    // System.err.println("get table=" + mBoundSockets.hashCode() + " key=" + key + " sock=" +
    // serverSocket);
    return serverSocket;
  }
