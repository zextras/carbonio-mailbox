...
public class ByteUtil {
  ...
  public static int countBytes(InputStream is, boolean closeStream) throws IOException {
    try {
      ...
      while ((num = is.read(buf)) != -1) count += num;
      return count;
    } finally {
      if (closeStream) {
  ...
  /**
   * Reads all data from the <code>InputStream</code> into a <tt>byte[]</tt> array. Closes the
   * stream, regardless of whether an error occurs.
   *
   * @param is The stream to read from.
   * @param sizeHint A (non-binding) hint as to the size of the resulting <tt>byte[]</tt> array, or
   *     <tt>-1</tt> for no hint.
   */
  ...
  /**
   * Reads all data from the <code>InputStream</code> into a <tt>byte[]</tt> array.
   *
   * @param is The stream to read from.
   * @param sizeHint A (non-binding) hint as to the size of the resulting <tt>byte[]</tt> array, or
   *     <tt>-1</tt> for no hint.
  ...
  /**
   * Reads all data from the <code>InputStream</code> into a <tt>byte[]</tt> array. Closes the
   * stream, regardless of whether an error occurs. If a positive <code>sizeLimit</code> is
   * specified and the stream is larger than that limit, an <code>IOException</code> is thrown.
   *
   * @param is The stream to read from.
   * @param sizeHint A (non-binding) hint as to the size of the resulting <tt>byte[]</tt> array, or
   *     <tt>-1</tt> for no hint.
   * @param sizeLimit The maximum number of bytes that can be copied from the stream before an
   *     <code>IOException</code> is thrown, or <tt>-1</tt> for no limit.
   */
  ...
  /**
   * Reads a certain quantity of data from the <code>InputStream</code> into a <tt>byte[]</tt>
   * array. Closes the stream, regardless of whether an error occurs. If a nonnegative <code>length
   * </code> is specified, the amount of data read into the array is capped by that value;
   * otherwise, the method behaves exactly as {@link #getContent(InputStream, int)}.
   *
   * @param is The stream to read from.
   * @param length The maximum number of bytes that will be copied from the stream.
   * @param sizeHint A (non-binding) hint as to the size of the resulting <tt>byte[]</tt> array, or
   *     <tt>-1</tt> for no hint.
   */
  public static byte[] getPartialContent(InputStream is, int length, int sizeHint)
      throws IOException {
    return getContent(is, length, sizeHint, -1, true);
  }

  private static byte[] getContent(
      InputStream is, int length, int sizeHint, long sizeLimit, boolean close) throws IOException {
    try {
      if (length == 0) return new byte[0];

      BufferStream bs;
      if (sizeLimit == -1) bs = new BufferStream(sizeHint, Integer.MAX_VALUE, Integer.MAX_VALUE);
      else if (sizeLimit > Integer.MAX_VALUE)
        bs = new BufferStream(sizeHint, Integer.MAX_VALUE, sizeLimit);
      else bs = new BufferStream(sizeHint, (int) sizeLimit, sizeLimit);

      bs.readFrom(is, length == -1 ? Long.MAX_VALUE : length);
      if (sizeLimit > 0 && bs.size() > sizeLimit) throw new IOException("stream too large");
  ...
  /**
   * Reads a <tt>String</tt> from the given <tt>Reader</tt>. Reads until the either end of the
   * stream is hit or until <tt>length</tt> characters are read.
   *
  ...
  public static String getContent(Reader reader, int length, boolean close) throws IOException {
    if (reader == null || length == 0) return "";
  ...
  /**
   * compress the supplied data using GZIPOutputStream and return the compressed data.
   *
  ...
  /**
   * uncompress the supplied data using GZIPInputStream and return the uncompressed data.
   *
  ...
  /**
   * Determines if the data contained in the buffer is gzipped by matching the first 2 bytes with
   * GZIP magic GZIP_MAGIC (0x8b1f).
   *
   * @param data
   * @return
   */
  ...
  /**
   * Determines if the data in the given stream is gzipped. Requires that the <tt>InputStream</tt>
   * supports mark/reset.
   */
  ...
  /**
   * Returns the length of the data returned by an <tt>InputStream</tt> Reads the stream in its
   * entirety and closes the stream when done reading.
   */
  ...
  public static String encodeFSSafeBase64(byte[] data) {
    ...
    for (int i = 0; i < encoded.length; i++) {
      if (encoded[i] == (byte) '/') encoded[i] = (byte) ',';
    }
    try {
   ...
   * @param base64 if <tt>true</tt>, return as base64 String, otherwise return as hex string.
  ...
  public static String getDigest(String algorithm, byte[] data, boolean base64) {
    try {
      ...
      if (base64) return encodeFSSafeBase64(digest);
      else return new String(Hex.encodeHex(digest));
  ...
  /**
   * Reads the given <tt>InputStream</tt> in its entirety, closes the stream, and returns the digest
   * of the read data.
   *
   * @param algorithm e.g. "SHA1"
   * @param in data to digest
   * @param base64 if <tt>true</tt>, returns as base64 String, otherwise return as hex string.
   * @return hex or base64 string
   */
  ...
  /**
   * Reads the given <tt>InputStream</tt> in its entirety, closes the stream, and returns the SHA1
   * digest of the read data.
   *
   * @param in data to digest
   * @param base64 if <tt>true</tt>, returns as base64 String, otherwise return as hex string.
   * @return hex or base64 string
  ...
  /**
   * Reads the given <tt>InputStream</tt> in its entirety, closes the stream, and returns the
   * SHA-256 digest of the read data.
   *
   * @param in data to digest
   * @param base64 if <tt>true</tt>, returns as base64 String, otherwise return as hex string.
   * @return hex or base64 string
   ...
   *
   * @param data data to digest
   * @param base64 if true, return as base64 String, otherwise return as hex string.
   * @return hex or base64 string
   */
  ...
  /**
   * Returns the SHA256 digest for the given data, encoded as base64.
   *
   * @return hex or base64 string
   */
   ...
   * @param closeIn if <tt>true</tt>, the <tt>InputStream</tt> is closed before returning, even when
   *     there is an error.
  ...
  public static long copy(
      InputStream in, boolean closeIn, OutputStream out, boolean closeOut, long maxLength)
      throws IOException {
    try {
      long transferred = 0;
      if (maxLength != 0) {
      ...
      if (closeIn) closeStream(in);
      if (closeOut) closeStream(out);
    }
  }
   ...
   * @param sizeHint used to allocate the byte array that is returned. Used for optimizing memory
   *     allocation. Does not affect the behavior of this method. If unknown, set this value to
   *     <tt>0</tt>.
  ...
  public static class SegmentInputStream extends PositionInputStream {
    ...
    public static SegmentInputStream create(InputStream is, long start, long end)
        throws IOException {
      if (start > 0) {
