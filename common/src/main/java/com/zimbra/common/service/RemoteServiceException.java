...
public class RemoteServiceException extends ServiceException {
  ...
  public static RemoteServiceException POP3_UIDL_REQUIRED() {
    return new RemoteServiceException(
        "leaving messages on server requires support for UIDL",
        POP3_UIDL_REQUIRED,
        RECEIVERS_FAULT);
  }
  ...
  public static RemoteServiceException YMAIL_INCONSISTENT_STATE() {
    return new RemoteServiceException(
        "Yahoo! mailbox inconsistent and is being rebuilt",
        YMAIL_INCONSISTENT_STATE,
        RECEIVERS_FAULT);
  }
  ...
  public static String getErrorCode(Throwable t) {
    if (t instanceof SocketTimeoutException
        ...
        || t instanceof NoHttpResponseException) return TIMEOUT;
    if (t instanceof UnknownHostException) return UNKNOWN_HOST;
    if (t instanceof NoRouteToHostException) return NOROUTE;
    if (t instanceof UnknownHostException) return UNKNOWN_HOST;
    if (t instanceof ProtocolException) return PROTOCOL_EXCEPTION;
    if (t instanceof ConnectException || t instanceof SocketException) return CONNECT_FAILURE;
    if (t instanceof CertificateException) return SSLCERT_ERROR;
    if (t instanceof SSLPeerUnverifiedException) return SSLCERT_MISMATCH;
    if (t instanceof SSLHandshakeException) return SSL_HANDSHAKE;
    if (t instanceof SSLException) return SSL_FAILURE;
    return null;
  }
  ...
  public static void doConnectionFailures(String msg, Throwable t) throws RemoteServiceException {
    if (t instanceof SocketTimeoutException
        || t instanceof ConnectTimeoutException
        || t instanceof NoHttpResponseException) throw RemoteServiceException.TIMEOUT(msg, t);
    if (t instanceof UnknownHostException) throw RemoteServiceException.UNKNOWN_HOST(msg, t);
    if (t instanceof NoRouteToHostException) throw RemoteServiceException.NOROUTE(msg, t);
    if (t instanceof UnknownHostException) throw RemoteServiceException.UNKNOWN_HOST(msg, t);
    if (t instanceof ProtocolException) throw RemoteServiceException.PROTOCOL_EXCEPTION(msg, t);
  ...
  public static void doSSLFailures(String msg, Throwable t) throws RemoteServiceException {
    if (t instanceof CertificateException) throw RemoteServiceException.SSLCERT_ERROR(msg, t);
    if (t instanceof SSLPeerUnverifiedException)
      throw RemoteServiceException.SSLCERT_MISMATCH(msg, t);
    if (t instanceof SSLHandshakeException) throw RemoteServiceException.SSL_HANDSHAKE(msg, t);
    if (t instanceof SSLException) throw RemoteServiceException.SSL_FAILURE(msg, t);
  }
}
