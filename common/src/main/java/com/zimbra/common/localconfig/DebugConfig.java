...
/**
 * Various switches to turn features on/off, mainly for measuring the performance overhead. Refer to
 * the code that uses these keys to see precisely which code paths are avoided by turning a feature
 * off.
 *
 * @since 2005. 4. 25.
 */
public final class DebugConfig {
  ...
  public static final boolean validateOutgoingICalendar =
      value("debug_validate_outgoing_icalendar", false);
  ...
  /**
   * If true, turns off message structure analysis and text extraction. Attachment extraction,
   * indexing, and objects only work when message analysis is enabled.
   */
  public static final boolean disableMessageAnalysis =
      value("debug_disable_message_analysis", false);

  /**
   * If true, turns off extracting content of attachments If extraction is disabled, indexing and
   * objects features are meaningless. When extraction is disabled, not even the text of main text
   * body part is extracted and won't be searchable. Only the message subject ends up being indexed.
   *
   * <p>Disabling extraction still performs reading the MIME body part data from JavaMail API. It
   * only skips sending the body data to the code that does type-specific text extraction. Setting
   * this key to true allows one to test the performance of JavaMail apart from performance of text
   * extraction routines.
   */
  public static final boolean disableMimePartExtraction =
      disableMessageAnalysis ? true : value("debug_disable_mime_part_extraction", false);

  /** If true, turns off object detection feature. */
  public static final boolean disableObjects =
      disableMessageAnalysis ? true : value("debug_disable_objects", false);
  ...
  /**
   * If true, turns off searching for ANDed list of terms spanning multiple attachments in a
   * message.
   */
  ...
  public static final boolean calendarAllowProcedureAlarms =
      value("debug_calendar_allow_procedure_alarms", false);

  /**
   * If true, convert AUDIO and PROCEDURE VALARMs to DISPLAY when serializing to xml, so ZWC can see
   * them as regular alarms. (true by default)
   */
  public static final boolean calendarConvertNonDisplayAlarm =
      value("debug_calendar_convert_non_display_alarms", true);

  /**
   * If true, use alarms specified by organizer in an invite email. If false (default), discard
   * organizer alarms and set one based on attendee's preferences.
   */
  ...
  public static final boolean disableMessageStoreFsync =
      value("debug_disable_message_store_fsync", false);

  /**
   * If true, convert TZ-relative times to UTC time in SOAP responses for non-recurring
   * appointments/tasks. Recurring series or instances are unaffected by this switch.
   */
  ...
  /**
   * If true, every item marked as "modified" in the Mailbox's cache is checked against the database
   * at the end of the transaction in order to ensure cache consistency (very expensive)
   */
  public static final boolean checkMailboxCacheConsistency =
      value("debug_check_mailbox_cache_consistency", false);

  /**
   * If true, the database layer assumes that every mailbox gets its own database instance and that
   * all the tables in that database do *not* have the MAILBOX_ID column. All mutable mailbox data,
   * including sizes and checkpoints as well as out-of-office reply tracking and the "mailbox
   * metadata" scratch space, are moved out of the ZIMBRA database and into this per-user database.
   */
  public static final boolean disableMailboxGroups = value("debug_disable_mailbox_group", false);

  /**
   * If true, the database layer assumes that the MailboxManager implementation has an external
   * mapping from accounts to mailbox IDs. As a result, the ZIMBRA.MAILBOX table will not be
   * populated and will not be read at startup.
   */
  public static final boolean externalMailboxDirectory =
      disableMailboxGroups && value("debug_external_mailbox_directory", false);

  /**
   * The number of {@code MBOXGROUP<N>} databases to distribute the users over. This is a middle
   * ground between One Huge Database (contention and the effects of corruption are issues) and
   * database-per-user (which most DBMSes can't deal with).
   */
  public static final int numMailboxGroups =
      disableMailboxGroups ? Integer.MAX_VALUE : Math.max(LC.zimbra_mailbox_groups.intValue(), 1);

  /**
   * If true, more than one server may be sharing the same store and database install. In that case,
   * the server must perform extra checks to ensure that mailboxes "homed" on other servers are
   * treated accordingly.
   */
  public static final boolean mockMultiserverInstall =
      value("debug_mock_multiserver_install", false);

  /**
   * If true, the GAL sync visitor mechanism is disabled. SyncGal will use the traditional way of
   * adding matches to a SearchGalResult, then add each match in the SOAP response. The GAL sync
   * visitor mechanism reduces chance of OOME when there is a huge result.
   */
  public static final boolean disableGalSyncVisitor =
      value("debug_disable_gal_sync_visitor", false);

  public static final boolean disableCalendarTZMatchByID =
      value("debug_disable_calendar_tz_match_by_id", false);
  public static final boolean disableCalendarTZMatchByRule =
      value("debug_disable_calendar_tz_match_by_rule", false);
  ...
  public static final boolean disableFoldersTagsCache =
      value("debug_disable_folders_tags_cache", false);

  public static final boolean enableContactLocalizedSort =
      value("debug_enable_contact_localized_sort", true);
  public static final boolean enableMigrateUserZimletPrefs =
      value("migrate_user_zimlet_prefs", false);
  public static final boolean disableGroupTargetForAdminRight =
      value("disable_group_target_for_admin_right", false);
  ...
  public static final boolean disableCalendarReminderEmail =
      value("debug_disable_calendar_reminder_email", false);
  ...
  public static final int imapSessionSerializerFrequency =
      value("debug_imap_session_serializer_frequency", 120);
  public static final boolean imapCacheConsistencyCheck =
      value("debug_imap_cache_consistency_check", false);
  ...
  public static final boolean imapTerminateSessionOnClose =
      value("imap_terminate_session_on_close", false);
  public static final boolean imapSerializeSessionOnClose =
      value("imap_serialize_session_on_close", true);
  ...
  public static final boolean enableThisAndFuture =
      value("debug_enable_calendar_thisandfuture", false);
  public static final boolean caldavAllowAttendeeForOrganizer =
      value("debug_caldav_allow_attendee_for_organizer", false);

  /**
   * TODO: Replace/remove when support persistence of DavName to DB instead of in memory cache. In
   * memory cache version developed to enable a test mode which is more compatible with URL:
   * http://svn.calendarserver.org/repository/calendarserver/CalDAVTester/trunk As currently
   * implemented, this is only useful for testing. Names are lost on restart which would cause
  ...
  public static boolean running_unittest = value("debug_running_unittest", false);

  public static boolean useInMemoryLdapServer = value("debug_use_in_memory_ldap_server", false);

  public static final String defangStyleUnwantedFunc =
      value("defang_style_unwanted_func", "\\w+\\s*\\(.*?\\)");
  public static final String defangValidExtUrl =
      value(
          "defang_valid_ext_url",
          "^(https?://[\\w-].*|mailto:.*|notes:.*|smb:.*|ftp:.*|gopher:.*|news:.*|tel:.*|callto:.*|webcal:.*|feed:.*:|file:.*|#.+)");
  public static final String defangValidImgFile =
      value("defang_valid_img_file", "\\.(jpg|jpeg|png|gif|bmp|aspx)((\\?)?)");
  public static final String defangValidIntImg =
      value("defang_valid_int_img", "^data\\s*:image/|^cid:");

  public static final String defangValidConvertdFile =
      value("defang_valid_convertd_file", "^index\\..*\\..*\\.(jpg|jpeg|png|gif)$");
  ...
  public static final String defangAvScriptTag = value("defang_av_script_tag", "</?script/?>");
  public static final String defangAvJavascript =
      value("defang_av_javascript", "^\\s*javascript\\s*:");
  public static final String defangAvVbscript = value("defang_av_vbscript", "^\\s*vbscript\\s*:");
  public static final String defangAvTab =
      value("defang_av_tab", "^*((&|&amp;)((Tab;)|(#[0]*9;)))|(\t)");
  public static final String defangACanAllowScripts = value("defang_a_scripts", "href,action");
  public static final String defangStyleUnwantedImport =
      value(
  ...
  public static final String xhtmlWhitelistedTags =
      value(
          "defang_xhtml_whitelisted_tags",
          "a,abbr,acronym,blockquote,div,font,h1,h2,h3,h4,h5,h6,img,li,ol,p,span,table,td,th,tr,ul");

  public static final String xhtmlWhitelistedAttributes =
      value(
          "defang_xhtml_whitelisted_attributes",
  ...
  public static boolean defang_block_form_same_host_post_req =
      value("defang_block_form_same_host_post_req", true);
  ...
  public static final boolean ldapNoopSearchSupported = value("ldap_noop_search_supported", true);
  ...
  public static final int visibileFolderRecalcInterval = value("visible_folder_recal_interval", 0);

  public static final boolean debugMailboxLock = value("debug_mailbox_lock", false);
  ...
  public static final String defangImgSkipOwaspSanitize =
      value("defang_img_skip_owasp_sanitize", "^cid:.*@");
  public static final String defangOwaspValidImgTag = value("owasp_valid_img_tag", "<\\s*img");
  public static final String defangStyleUnwantedStrgPattern =
      value("defang_style_unwanted_strg_pattern", "\\s*(('){2,})");
  public static final String defangOnloadMethod =
      value("defang_owasp_alert_tag", "onload=.*\\(.*\\)");

  /*
   * Default maximum size of convertd response. This reduces OOME in case of
   * large response
   */
  public static final long convertdMaxResponseSize =
      value("convertd_max_response_size", (long) 20 * 1024 * 1024);

  public static final boolean imapForceSpecialUse = value("imap_force_special_use", true);

  public static final boolean pushNotificationVerboseMode =
      value("push_notification_verbose_mode", false);
  ...
  public static final int profileThumbnailImageDimension =
      value("profile_thumbnail_image_dimension", 50);

  /**
   * "restricted_server_ldap_attributes" comma separated list of restricted server ldap attributes
   */
  public static final String restrictedServerLDAPAttributes =
      value("restricted_server_ldap_attributes", "zimbraSSLPrivateKey");

  /**
   * sleep time between account rename and alias creation for testing mail delivery during change of
   * primary email
   */
  public static final int sleepTimeForTestingChangePrimaryEmail =
      value("change_primary_email_sleep_time", 0);
