...
public abstract class LocalConfigUpgrade {
  ...
  private static Options getAllOptions() {
    ...
    Option bugOpt =
        new Option(O_BUG, "bug", true, "bug number this upgrade is for (multiple allowed)");
  ...
  private static class LocalConfigUpgradeSwitchToCMS extends LocalConfigUpgrade {
    ...
    private String modify(String mjo) {
      ...
      mjo =
          appendOptionIfNoOptionBeginsWith(
              mjo, "-XX:SoftRefLRUPolicyMSPerMB=", "-XX:SoftRefLRUPolicyMSPerMB=1");
      return mjo;
    }
  }
  ...
  public static void main(String[] args) throws ServiceException {
    ...
    try {
      ...
      if (cl == null) throw new ParseException("");
    ...
    for (String bug : bugs) {
      ...
      System.out.println(
          "== Running local config upgrade for bug " + lcu.mBug + " (" + lcu.mShortName + ")");
      try {
        lcu.upgrade(lc);
        System.out.println(
            "== Done local config upgrade for bug " + lcu.mBug + " (" + lcu.mShortName + ")");
