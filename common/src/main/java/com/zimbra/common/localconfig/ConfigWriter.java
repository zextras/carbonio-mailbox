...
public abstract class ConfigWriter {
  ...
  public static ConfigWriter getInstance(
      String format, boolean expandVariables, boolean hidePasswords) throws ConfigException {
    if (format == null || format.equals(FORMAT_PLAIN))
      return new PlainConfigWriter(expandVariables, hidePasswords);
    if (format.equals(FORMAT_XML)) return new XmlConfigWriter(expandVariables, hidePasswords);
    if (format.equals(FORMAT_SHELL)) return new ShellConfigWriter();
    if (format.equals(FORMAT_EXPORT)) return new ExportConfigWriter();
    if (format.equals(FORMAT_NOKEY)) return new NokeyConfigWriter(expandVariables, hidePasswords);
