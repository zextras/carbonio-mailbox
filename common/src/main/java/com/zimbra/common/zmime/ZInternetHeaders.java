...
public class ZInternetHeaders extends InternetHeaders {
  ...
  static ZInternetHeaders copyHeaders(MimePart part) throws MessagingException {
    ...
    for (@SuppressWarnings("unchecked") Enumeration<String> en = part.getAllHeaderLines();
        en.hasMoreElements(); ) {
   ...
   * no matching header exists in the block, the logic from {@link #addHeader(ZInternetHeader)}
   * applies. If the {@code header} is {@code null}, any matching existing headers are removed and
   * no new header is added.
   *
  ...
  private static final Set<String> RESENT_HEADERS =
      ImmutableSet.of(
          "resent-date",
          "resent-from",
          "resent-sender",
   ...
   *       block, this call is converted into a call to {@link #setHeader(String, ZInternetHeader)}
   *       and the existing header(s) will be replaced with a copy of the new header.
  ...
  public ZInternetHeaders addHeader(ZInternetHeader header) {
    ...
    if (hinfo.first) {
      ...
      for (index = 0; index < headers.size(); index++) {
        if (((ZInternetHeader) headers.get(index)).hinfo.position >= hinfo.position) break;
      }
    } else if (ordered) {
   ...
   * subsequent calls to {@link #addHeader(ZInternetHeader)} will maintain that ordering.
   *
   * @see ZInternetHeader.HeaderInfo
   */
  @SuppressWarnings("unchecked")
  public void appendHeader(ZInternetHeader header) {
    ...
    if (ordered
        && !isEmpty()
        && header.hinfo.position
            < ((ZInternetHeader) headers.get(headers.size() - 1)).hinfo.position) {
      this.ordered = false;
    }
    headers.add(header);
    return;
  ...
  public void removeHeader(String name) {
    if (ZPARSER) {
      ...
      for (@SuppressWarnings("unchecked") Iterator<ZInternetHeader> it = headers.iterator();
          it.hasNext(); ) {
  ...
  /**
   * Returns the length of this {@code ZInternetHeaders} when serialized as a {@code byte[]}. This
