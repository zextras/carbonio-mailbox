...
public enum ZTransferEncoding {
  ...
  private ZTransferEncoding(String value) {
    mValue = value;
  }
  ...
  static class Base64DecoderStream extends TransferEncodingStream {
    ...
    static {
      for (int i = 0; i < 128; i++) BASE64_DECODE[i] = -1;
    ...
    public int read() throws IOException {
      ...
      if (c == '=') {
        ...
        if (p1 >= 0x30 && p1 <= 0x66 && p2 >= 0x30 && p2 <= 0x66) {
          ...
          if (hi4 != -1 && lo4 != -1) return (hi4 << 4) | lo4;
        } else if (p1 == '\r' && p2 == '\n') {
          return read();
        } else if (p1 == '\n') {
          unread(p2);
  ...
  static class QuotedPrintableEncoderStream extends TransferEncodingStream {
    ...
    QuotedPrintableEncoderStream setContentType(ZContentType ctype) {
      text = ctype == null || ctype.getPrimaryType().equals("text");
      return this;
    }
    ...
    public int read() throws IOException {
      if (valid == 0) {
        ...
        if (c == ' ' || c == '\t') {
          unread(p1 = nextByte());
          if (p1 != '\r' && (force == null || !force[c])) return c;
        ...
        if (fold && column >= CHUNK_SIZE - 3) {
          ...
          out1 = QP_TABLE[(c >> 4) & 0x0F];
          out2 = QP_TABLE[c & 0x0F];
          valid = 2;
          return '=';
        }
      } else if (valid == 1) {
  ...
  private static class TransferEncodingStream extends PushbackInputStream {
    ...
    private static InputStream ensureBuffering(InputStream is) {
      return is instanceof ByteArrayInputStream || is instanceof BufferedInputStream
          ? is
    ...
    public int read(byte[] b, int off, int len) throws IOException {
      if (b == null || in == null) throw new NullPointerException();
    ...
    public long skip(long n) throws IOException {
      // can't just skip in the underlying stream, since there isn't a 1-to-1 mapping of underlying
