...
public class ZMimeBodyPart extends MimeBodyPart implements ZMimePart {
  ...
  ZMimeBodyPart(MimeBodyPart source, ZContentType ctype, ZMimeMultipart container)
  ...
  public Charset defaultCharset() {
    ...
    while (part != null) {
      ...
      if (container instanceof MimeMessage) {
        return container instanceof ZMimeMessage
            ? ((ZMimeMessage) container).defaultCharset()
            : null;
      } else {
  ...
  public String getContentType() throws MessagingException {
    ...
    if (s == null) {
      if (parent instanceof MimeMultipart
          && new ZContentType(parent.getContentType()).getSubType().equals("digest")) {
        s = "message/rfc822";
      } else {
        s = "text/plain";
      }
  ...
  static void recalculateTransferEncoding(ZMimePart part) throws MessagingException {
    ...
    if (part.getHeader("Content-Transfer-Encoding") != null) return;
  ...
  static class EncodingTester {
    /**
     * Maximum number of octets in a line before we force a content transfer encoding. Many MTAs
     * will wrap lines over 998 octets.
     */
