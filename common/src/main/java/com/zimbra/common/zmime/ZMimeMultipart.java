...
public class ZMimeMultipart extends MimeMultipart {
  ...
  ZMimeMultipart(MimeMultipart source, ZContentType ctype, ZMimePart container)
    ...
    String defaultType =
        ctype.getSubType().equals("digest") ? ZContentType.MESSAGE_RFC822 : ZContentType.TEXT_PLAIN;
    ...
    for (int i = 0, count = source.getCount(); i < count; i++) {
      ...
      parts.add(
          new ZMimeBodyPart(part, new ZContentType(part.getContentType(), defaultType), this));
    }
  ...
  String getBoundary() {
    return implicitBoundary != null
        ? implicitBoundary
        : new ZContentType(contentType).getParameter("boundary");
  }

  // JavaMail makes it impossible to set MimeMultipart.allowEmpty if you don't use their parser, and
  // that
  // means an empty multipart always throws an exception on MimeMultipart.writeTo regardless of what
  // "mail.mime.multipart.allowempty" is set to.  So we have to copy the whole method from the
  // superclass
