...
public class ZInternetHeader {
   ...
   * @param content Complete raw header line, <b>including</b> the field name and colon and with
   *     folding and trailing CRLF and 2047- encoding intact.
   * @param start The position within <code>content</code> where the header field value begins
   *     (after the ":"/": ").
   */
  ...
  /**
   * Creates a new {@code ZInternetHeader} with {@code value} as the field value. Header will be
   * serialized as <tt>{name}: {encoded-value}CRLF</tt> after appropriate RFC 2047 encoded-word
   * encoding and RFC 5322 line folding has been performed. When generating encoded-words,
   * <tt>utf-8</tt> will be used as the encoding charset. <i>Note: No line folding is done at
   * present.</i>
   */
   ...
   * Creates a new {@code ZInternetHeader} with {@code value} as the field value. Header will be
   * serialized as <tt>{name}: {encoded-value}CRLF</tt> after appropriate RFC 2047 encoded-word
   * encoding and RFC 5322 line folding has been performed. When generating encoded-words, {@code
   * charset} will be used as the encoding charset if possible, defaulting back to <tt>utf-8</tt>.
   * <i>Note: No line folding is done at present.</i>
   */
  ...
  /**
   * Creates a new {@code ZInternetHeader} serialized as "<tt>{name}: {bvalue}CRLF</tt>". {@code
   * bvalue} is copied verbatim; no charset transforms, encoded-word handling, or folding is
   * performed.
   */
  ...
  /**
   * Reserializes the {@code ZInternetHeader}, using {@code bvalue} as the field value (the bit
   * after the '<tt>:</tt>'). {@code bvalue} is copied verbatim; no charset transforms, encoded-word
   * handling, or folding is performed.
   */
  ZInternetHeader updateContent(final byte[] bvalue) {
    ...
    this.content = buf;
    this.valueStart = nlen + 2;
    return this;
  }

  /** Returns this header's field name (the bit before the '<tt>:</tt>' in the header line). */
  public String getName() {
    return name;
  }

  /**
   * Returns the entire header line (including the field name and the '<tt>:</tt>') as a raw byte
   * array.
   */
  ...
  /**
   * Returns the header's value (the bit after the '<tt>:</tt>') after all unfolding and decoding of
   * RFC 2047 encoded-words has been performed.
   */
  ...
  /**
   * Returns the header's value (the bit after the '<tt>:</tt>') as a {@code String}. No decoding is
  ...
  /**
   * Returns the header's value (the bit after the '<tt>:</tt>') as a {@code String}. If non-{@code
   * null}, the {@code charset} is used when converting the header bytes to a {@code String}. No
   * decoding is performed other than removing the trailing CRLF.
   */
  ...
  private static String createString(
      final byte[] bytes, final int offset, final int length, final Charset charset) {
  ...
  static String decode(
      final byte[] content, final int start, final int length, final Charset charset) {
    ...
    for (int i = 0; i < last; i++) {
      ...
      if (current.getSeqType() == SequenceType.LWS) {
        ...
        if (prev != null
            && prev.getSeqType() == SequenceType.EW
          ...
          if ("B".equalsIgnoreCase(current.getDecodeType())
              && endIdx > 1
              && decodeMe.charAt(endIdx) == '=') {
            // This chunk is correctly padded
            decodeAndAppend(decodeMe, text);
            setupNewNeedDecode(decodeMe, current);
          } else if ("Q".equalsIgnoreCase(current.getDecodeType())
              && endIdx > 1
              && decodeMe.charAt(endIdx) == '=') {
  ...
  public enum EncodeSequenceState {
    CHARSET,
    ENCODEMETHOD,
    TEXT,
    UNDEFINED
  ...
  public static class FieldElement {
    ...
    public void appendCharset(byte b) {
      this.charset.write(b);
    }

    public void appendDecodeType(byte b) {
      this.decodeType.write(b);
    }

    public boolean isPadding(int index) {
      return (bytes.toByteArray()[index] == '=');
    }
  ...
  static class Fields {
    ...
    public void add(FieldElement element) {
      ...
      if (end > 0
          && element.getSeqType() == SequenceType.EW
  ...
  private static List<FieldElement> parse(final byte[] content, final int start, final int length) {
    ...
    for (int pos = start; pos < end; pos++) {
      ...
      if (currStat == SequenceType.COMMENT) {
        ...
        if ((content[pos] == ' ' || content[pos] == '\t')
            && !allowInvalidEncoding(currElement.getCharset())) {
          return null;
        } else if (encodeStat == EncodeSequenceState.TEXT
            && (pos < (end - 1))
            && content[pos] == '?'
            && content[pos + 1] == '=') {
          pos++;
  ...
  public static boolean allowInvalidEncoding(String charset) {
    ...
    return ("iso-8859-1".equalsIgnoreCase(charset)
        || "us-ascii".equalsIgnoreCase(charset)
