...
public class LmtpClient {
  ...
  public LmtpClient(String host, int port, Protocol proto, boolean skipTLSCertValidation)
      throws IOException {
    if (proto != null) mProtocol = proto;
    else mProtocol = Protocol.LMTP;
  ...
  public boolean sendMessage(
      InputStream msgStream, String recipient, String sender, String logLabel)
      throws IOException, LmtpProtocolException, LmtpClientException {
    return sendMessage(msgStream, new String[] {recipient}, sender, logLabel, null);
  }

  public boolean sendMessage(
      InputStream msgStream, String recipient, String sender, String logLabel, long size)
      throws IOException, LmtpProtocolException, LmtpClientException {
    return sendMessage(msgStream, new String[] {recipient}, sender, logLabel, size);
  }

  public boolean sendMessage(
      InputStream msgStream, String[] recipients, String sender, String logLabel, Long size)
      throws IOException, LmtpProtocolException, LmtpClientException {
   ...
   */
  public boolean sendMessage(
      InputStream msgStream, Iterable<String> recipients, String sender, String logLabel, Long size)
      throws IOException, LmtpProtocolException, LmtpClientException {
    ...
    if (mNewConnection) {
      ...
      if (Protocol.SMTP.equals(mProtocol)) sendLine("EHLO " + mGreetname);
      else sendLine("LHLO " + mGreetname);
      if (!replyOk()) {
        throw new LmtpProtocolException(mResponse);
      }
    ...
    BufferedReader br =
        new BufferedReader(new InputStreamReader(msgStream, CharsetUtil.ISO_8859_1));
    String line;
    while ((line = br.readLine()) != null) {
      /**
       * http://tools.ietf.org/html/rfc2821#section-4.5.2 Transparency: Before sending a line of
       * mail text, the SMTP client checks the first character of the line. If it is a period, one
  ...
  private SSLSocket newSSLSocket(Socket sock) throws IOException, LmtpClientException {
    try {
      ...
      sslContext.init(
          null, new TrustManager[] {getTrustManager()}, new java.security.SecureRandom());
