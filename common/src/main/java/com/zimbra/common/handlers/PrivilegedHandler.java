...
/**
 * This is the class jetty will call back before setuid. We will open sockets on privileged ports if
 * configured to do so.
 *
 * @author jjzhuang
 */
public class PrivilegedHandler {
  ...
  public static void openPorts(Map<String, Object> attributes) {
    ...
    try {
      ...
      if (LC.zimbra_ssl_enabled.booleanValue()) { // default is true
        ...
        System.setProperty(
            "javax.net.ssl.keyStorePassword", getAttr(attributes, mailboxd_keystore_password));
        System.setProperty(
            "javax.net.ssl.trustStorePassword", getAttr(attributes, mailboxd_truststore_password));
      }
      ...
      if (getBooleanAttr(attributes, A_zimbraPop3SSLServerEnabled, false)) {
        ...
        if (LC.nio_pop3_enabled.booleanValue()) {
          ...
          NetUtil.bindSslTcpServerSocket(
              address, port, excludeCiphers, includeCiphers, sslProtocols);
        }
      }
  ...
  private static String getAttr(Map<String, Object> attributes, String name, String defaultValue) {
    ...
    String s = (String) v;
    if (s != null && s.equals(""))
      s = null; // null out empty string because jetty-setuid.xml may pass in ""
    return s == null ? defaultValue : s;
  }

  private static boolean getBooleanAttr(
      Map<String, Object> attributes, String name, boolean defaultValue) {
    Object v = attributes.get(name);
  ...
  private static String[] getSpaceDelimitedStringArray(
