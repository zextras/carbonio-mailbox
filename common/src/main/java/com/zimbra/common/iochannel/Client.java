...
/**
 * Client manages the outbound packets in iochannel framework. It maintains the list of available
 * peer servers in the cluster and allows the application to send a packet to designated peer
 * server.
 *
 * @author jylee
 */
public class Client implements Runnable {
  ...
  /**
   * PeerServer is an abstraction of server running on another machine in the cluster. There is a
   * separate message queue and socket channel for each peer.
   */
  public class PeerServer {
    ...
    /** Returns the backlog, or how many messages are queued for this peer server. */
    ...
    private long write() throws IOException {
      ...
      try {
        ...
        log.warn(
            "channel %s:%d is down",
            current.getDestination().hostname, current.getDestination().port);
      }
      synchronized (this) {
  ...
  /** Returns a collection of peer servers available to send a message to from this client. */
