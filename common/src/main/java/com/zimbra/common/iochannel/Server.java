...
/**
 * iochannel is a framework to send and receive data in a cluster of machines. All the machines in
 * the clusters are fully connected with each other using TCP connection. Each peer machines are
 * addressable using unique identifier by implementation of Config.
 *
 * <p>The connections are managed using non blocking {@link Channel} and {@link Selector}. Each
 * machine in the cluster runs an instance of Server and an instance of Client. Server class
 * provides a channel of unidirectional inbound packets, and Client class is used to send packets to
 * another machine. Typically Client maintains open sockets to all the peer servers for outbound
 * packets, and Server maintains a server socket bound to a well known port, and open accepted
 * sockets from all the peer machines. The connections are persistent, and there is no TCP setup and
 * teardown overhead for sending packets. When a connection breaks due to network or hardware issue,
 * then Client attempts to restore the connection in regular interval until it succeeds.
 *
 * <p>The socket io is done asynchronously. When the application requests a packet to be sent to a
 * peer, the request is queued for the peer in memory, and will be sent as soon as all the preceding
 * packets to the peer have been sent. The queues for each peers are managed independently, so when
 * one of the peer goes down, the packets to other peers can be sent unaffected. When the Server
 * receives a packet from a peer, it will send the packet to the registered callback class to be
 * consumed.
 *
 * @author jylee
 */
public class Server implements Runnable {
  ...
  /**
   * NotifyCallback is a callback class to be used to alert the application that a packet has been
   * received from peer.
   */
            ...
            if (key.isAcceptable()) {
              ...
              if (bytesRead == -1) {
                ...
                if (buffer.limit() == buffer.position()) {
                  ...
                  ByteBuffer buf =
                      ByteBuffer.allocate(Math.max(2 * buffer.limit(), Packet.maxMessageSize));
                  key.attach(buf);
