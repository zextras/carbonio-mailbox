...
/**
 * Packet is smallest unit of data transmitted among the machines in iochannel cluster. There is a
 * small header field with magic identification, id of the sender, and the payload.
 *
 * @author jylee
 */
public class Packet {
  ...
  private Packet(String clientId, PeerServer server, ByteBuffer message) {
    ...
    header
        .putInt(magic)
        .putLong(clientId.length())
        .putLong(message.remaining())
        .put(clientIdBytes)
        .flip();
  ...
  public static Packet fromBuffer(ByteBuffer buffer) throws IOChannelException {
    ...
    if (log.isDebugEnabled()) {
      try {
        log.debug(
            "msg from %s '%s'", new String(headerBuf, "UTF-8"), new String(contentBuf, "UTF-8"));
      } catch (UnsupportedEncodingException e) {
      }
    }
  ...
  private static final int minimumHeaderSize =
      (Integer.SIZE + 2 * Long.SIZE) / 8; // one integer and two longs;
