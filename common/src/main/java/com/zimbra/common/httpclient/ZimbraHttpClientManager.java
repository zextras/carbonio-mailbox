 ...
 *     <p>Wrapper class for Apache HTTP Components HttpClient 4. Server code should use this wrapper
 *     to ensure that outgoing HTTP connections are being reused.
 */
public class ZimbraHttpClientManager {
  ...
  public ZimbraHttpClientManager() {
    ...
    try {
      sslcontext =
          SSLContexts.custom()
              .loadTrustMaterial(CustomTrustManager.loadKeyStore(), new TrustSelfSignedStrategy())
              .build();
    ...
    internalConnectionMgr.setDefaultMaxPerRoute(
        LC.httpclient_internal_connmgr_max_host_connections.intValue());
    internalConnectionMgr.setMaxTotal(
        LC.httpclient_internal_connmgr_max_total_connections.intValue());
    internalRequestConfig =
        RequestConfig.custom()
            ...
            .setStaleConnectionCheckEnabled(
                LC.httpclient_internal_connmgr_stale_connection_check.booleanValue())
            .build();
    internalClient =
    ...
    externallConnectionMgr.setDefaultMaxPerRoute(
        LC.httpclient_external_connmgr_max_host_connections.intValue());
    externallConnectionMgr.setMaxTotal(
        LC.httpclient_external_connmgr_max_total_connections.intValue());
