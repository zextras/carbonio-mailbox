...
class Soap12Protocol extends SoapProtocol {
  ...
  public SoapFaultException soapFault(Element fault) {
    if (!isFault(fault)) return new SoapFaultException("not a soap fault ", fault);
    ...
    if (code != null) {
      ...
      isReceiversFault =
          RECEIVER_CODE.getQualifiedName().equals(value == null ? null : value.getTextTrim());
    }

    String reasonValue;
    Element reason = fault.getOptionalElement(REASON);
    Element reasonText = (reason == null ? null : reason.getOptionalElement(TEXT));
    if (reasonText != null) reasonValue = reasonText.getTextTrim();
    else reasonValue = "unknown reason";
  ...
  /** Given a ServiceException, wrap it in a soap fault return the soap fault document. */
  public Element soapFault(ServiceException e) {
    ...
    else error.addUniqueElement(ZimbraNamespace.E_TRACE).setText(e.getThreadName());
