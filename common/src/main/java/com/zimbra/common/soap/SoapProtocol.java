...
public abstract class SoapProtocol {
  ...
  /** Package-Private constructor to prevent anyone else from extending */
  SoapProtocol() {
  ...
  /** Given an element, wrap it in an envelope and return the envelope. */
  ...
  /** Given an element and some headers, wrap it in an envelope and return the envelope. */
  ...
  /** Given a Exception, wrap it in a soap fault and return the soap fault element. */
  public abstract Element soapFault(ServiceException e);

  /**
   * Given an element that represents a fault (i.e,. isFault returns true on it), construct a
   * SoapFaultException from it.
   *
   * @return new SoapFaultException or null if passed in document is not a soap fault.
  ...
  public static byte[] toBytes(String message) throws java.io.UnsupportedEncodingException {
    return message.getBytes(getCharset());
  }

  /** Convert a SOAP message in bytes to a String */
  public static String toString(byte[] message) throws java.io.UnsupportedEncodingException {
    if (message == null || message.length == 0) return "";
    else if (message[message.length - 1] == '\0')
      return new String(message, 0, message.length - 1, getCharset());
    else return new String(message, getCharset());
  }
  ...
  /** returns the specified element in the Header element, or null if it doesn't exist. */
  public Element getHeader(Element soapEnvelope, QName headerQName) {
    ...
    Element header;
    if (soapHeader != null) header = soapHeader.getOptionalElement(headerQName);
    else header = null;
    return header;
  }
  ...
  /** Returns true if this soap envelope has a SOAP fault as the first child of its body. */
  ...
  /**
   * Walk the passed-in Fault element, find Arguments (see ServiceException.Argument ) of type
   * ItemID and update them so that they contain the target account ID
   *
   * @param element
  ...
  /** determine if given document is Soap11 or Soap12 envelope. returns null if neither. */
  public static SoapProtocol determineProtocol(Element env) {
    if (Soap12.isEnvelope(env)) return Soap12;
    else if (Soap11.isEnvelope(env)) return Soap11;
    else if (env instanceof Element.JSONElement) return SoapJS;
    else return null;
  }
