...
public class SoapTestHarness {
  ...
  public void runTests(String args[]) throws HarnessException, IOException, ServiceException {
    ...
    if (err || cl.hasOption("help")) usage(options);

    mDebug = cl.hasOption("d");
    String file = cl.getOptionValue("f");

    if (cl.hasOption("s")) expandSystemProperties();
    ;
  ...
  private void setProperty(String name, String value) throws HarnessException {
    ...
    if (name.equals("uri")) {
      if (value == null) throw new HarnessException("need value for uri");
      mUri = value;
      mTransport = new SoapHttpTransport(mUri);
    } else if (name.equals("authToken")) {
      if (value == null) throw new HarnessException("need value for authToken");
      mAuthToken = value;
    } else if (name.equals("sessionId")) {
      if (value != null) mSessionId = value;
  ...
  private void doProperty(Element property) throws HarnessException {
    ...
    if (name == null) throw new HarnessException("<property> tag missing name");
    if (value == null) throw new HarnessException("<property> tag missing value");
  ...
  private void doNamespace(Element property) throws HarnessException {
    ...
    if (prefix == null) throw new HarnessException("<namespace> tag missing prefix");
    if (uri == null) throw new HarnessException("<namespace> tag missing uri");
  ...
  protected void doTests(Element root) throws HarnessException, IOException, ServiceException {
    ...
    for (Test t : mTests) System.out.println(t.getStatus());
  }
  ...
  private boolean doTest(Element test) throws IOException, HarnessException, ServiceException {
    ...
    if (!mCurrent.isRequired()) return true;
    else return !mCurrent.testFailed();
  ...
  private Element expandProps(Element doc) throws HarnessException {
    ...
    for (Element.Attribute attr : doc.listAttributes()) {
      ...
      if (text.indexOf("${") != -1) attr.setValue(expandAllProps(text));
    }
    String text = doc.getText();
    if (text.indexOf("${") != -1) doc.setText(expandAllProps(text));
    return doc;
  }
  ...
  private void doRequest(Element request) throws IOException, ServiceException {
    ...
    if (mTargetUser != null) SoapUtil.addTargetAccountToCtxt(ctxt, null, mTargetUser);
  ...
  private void doSelect(Element context, Element parent)
    ...
    if (match != null) {
      ...
      mCurrent.check(
          ok, "match " + (ok ? "ok" : "failed") + " (" + match + ")" + " (" + value + ")");
    }
  ...
  private void expandSystemProperties() throws HarnessException {
    ...
    if (props != null) {
      ...
      while (keys.hasMoreElements()) {
        ...
        else throw new HarnessException("Invalid Global Property file");
      }
    }
  }
}
