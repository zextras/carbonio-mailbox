...
public class SoapHttpTransport extends SoapTransport {
  private HttpClientBuilder mClientBuilder =
      ZimbraHttpConnectionManager.getInternalHttpConnMgr().getDefaultHttpClient();
  ...
  private static boolean defaultKeepAlive =
      LC.httpclient_soaphttptransport_keepalive_connections.booleanValue();
  ...
  public Map<String, String> getCustomHeaders() {
    if (mCustomHeaders == null) mCustomHeaders = new HashMap<String, String>();
  ...
  /**
   * Get the mTimeout value in milliseconds. The default is specified by the
   * <tt>httpclient_soaphttptransport_so_timeout</tt> localconfig variable.
   */
  ...
  public Element invoke(
    ...
    try {
      ...
      if (agentName != null) {
        ...
        if (agentVersion != null) agentName += " " + agentVersion;
      ...
      if (originalUserAgent != null) {
        method.addHeader(
            new BasicHeader(HeaderConstants.HTTP_HEADER_ORIG_USER_AGENT, originalUserAgent));
      }
      ...
      if (getClientIp() != null) {
        ...
        if (ZimbraLog.misc.isDebugEnabled()) {
          ZimbraLog.misc.debug(
              "set remote IP header [%s] to [%s]", RemoteIP.X_ORIGINATING_IP_HEADER, getClientIp());
        }
      }
      Element soapReq =
          generateSoapMessage(
      ...
      method.setEntity(
          new StringEntity(
              soapMessage, ContentType.create(ContentType.APPLICATION_XML.getMimeType(), "UTF-8")));

      if (getRequestProtocol().hasSOAPActionHeader()) method.addHeader("SOAPAction", mUri);
      ...
      if (trustedToken != null) {
        BasicClientCookie cookie =
            new BasicClientCookie(ZimbraCookie.COOKIE_ZM_TRUST_TOKEN, trustedToken);
      ...
      if (mHostConfig != null
          && mHostConfig.getUsername() != null
          && mHostConfig.getPassword() != null) {
      ...
      if (!mKeepAlive) ZimbraHttpConnectionManager.getInternalHttpConnMgr().closeIdleConnections();
    }
  }

  /**
   * Returns the document name. If the given document is an <tt>Envelope</tt> element, returns the
   * name of the first child of the <tt>Body</tt> subelement.
   */
  ...
  /**
   * Use SAXReader to parse large soap response. caller must provide list of handlers, which are
   * <path, handler> pairs. To reduce memory usage, a handler may call Element.detach() in
   * ElementHandler.onEnd() to prune off processed elements.
   */
  public static final class SAXResponseHandler implements ResponseHandler {
    ...
    public void process(Reader src) throws ServiceException {
      ...
      try {
        ...
        // in ZimbraOffline.  Submitted Bugzilla Bug 104175 suggesting investigating replacing use
        // of this
  ...
  public Future<HttpResponse> invokeAsync(
    ...
    if (getRequestProtocol().hasSOAPActionHeader()) post.setHeader("SOAPAction", mUri);
    ...
    CloseableHttpAsyncClient httpClient =
        ZimbraHttpClientManager.getInstance().getInternalAsyncHttpClient();
