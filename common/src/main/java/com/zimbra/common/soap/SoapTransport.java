...
public abstract class SoapTransport {
  ...
  public static enum NotificationFormat {
    DEFAULT,
    IMAP
  };
  ...
  /**
   * @return Zimbra context (&lt;context xmlns="urn:zimbra"&gt;) from last invoke, if there was one
   *     present.
   */
  ...
  /**
   * Sets the default SOAP client name and version. These global value are used when the
   * instance-level values are not specified.
   */
   ...
   *
   * <p>This is intended to be called by the SOAP engine if the current thread is a SOAP handler.
   * All subsequent SOAP requests, i.e. proxy requests will include this {@code via} header, so that
   * we can track the proxy chain. Since a SOAP handler may dispatch another SOAP handler
   * recursively in the same thread, this is internally maintained by a stack.
   *
  ...
  public void setRequestProtocol(SoapProtocol proto) {
    if (proto != null) mRequestProto = proto;
  }

  /**
   * Returns the version of SOAP used to generate requests. Defaults to {@link SoapProtocol#Soap12}.
   */
  ...
  /**
   * Sets the version of SOAP we'd like the server to reply with. <i>(Note that this only controls
   * XML/JSON serialization; requesting SOAP 1.1 responses to a SOAP 1.2 request will not be
   * honored.)</i>
   */
  public void setResponseProtocol(SoapProtocol proto) {
    if (proto != null) mResponseProto = proto;
  }

  /**
   * Returns the version of SOAP used by the remote server when it generates its responses. Defaults
   * to {@link #getRequestProtocol()} .
   */
  ...
  protected Element generateSoapMessage(
      Element document,
      boolean raw,
      boolean noSession,
      ...
      String changeToken,
      String tokenType) {
    return generateSoapMessage(
        document, raw, noSession, requestedAccountId, changeToken, tokenType, null, null);
  }
  ...
  protected Element generateSoapMessage(
    ...
    if (proto == SoapProtocol.SoapJS) {
      if (document instanceof XMLElement) proto = SoapProtocol.Soap12;
    } else {
      if (document instanceof JSONElement) proto = SoapProtocol.SoapJS;
    }
    ...
    if (generateContextHeader()) {
      ...
      if (noSession) {
        ...
        SoapUtil.addSessionToCtxt(
            context, mAuthToken == null ? null : mSessionId, mMaxNotifySeq, nFormat, curWaitSetID);
      }
  ...
  protected Element parseSoapResponse(String envelopeStr, boolean raw)
    ...
    Element env;
    try {
      if (envelopeStr.trim().startsWith("<")) env = Element.parseXML(envelopeStr);
      else env = Element.parseJSON(envelopeStr);
  ...
  public Element extractBodyElement(Element env) throws SoapParseException, SoapFaultException {
    ...
    if (proto.isFault(e)) {
      if (mTargetAcctId != null) proto.updateArgumentsForRemoteFault(e, mTargetAcctId);
      throw proto.soapFault(e);
    } else return e;
  }

  public abstract Future<HttpResponse> invokeAsync(
      Element document,
      boolean raw,
      boolean noSession,
      String requestedAccountId,
  ...
  public final Future<HttpResponse> invokeAsync(
      Element document,
      boolean raw,
      boolean noSession,
      String requestedAccountId,
      ...
      throws IOException {
    return invokeAsync(
        document, raw, noSession, requestedAccountId, changeToken, tokenType, null, null, cb);
  }

  public final Future<HttpResponse> invokeAsync(Element document, FutureCallback<HttpResponse> cb)
      throws IOException {
    return invokeAsync(document, false, false, null, null, null, cb);
  }

  public final Future<HttpResponse> invokeWithoutSessionAsync(
      Element document, FutureCallback<HttpResponse> cb) throws IOException {
    return invokeAsync(document, false, true, null, null, null, cb);
  }

  /**
   * Sends the specified document as a Soap message and parses the response as a Soap message.
   *
   * <p>Uses the <code>invoke(document, raw, noNotify)</code> method and passes <code>false</code>
   * and <code>false</code>.
   *
   * @throws IOException
  ...
  /**
   * Sends the specified document as a SOAP message without creating a session on the server.
   *
   * @param document
   * @return
   * @throws IOException
   ...
   * Sends the specified document as a Soap message and parses the response as a Soap message.
   *
   * <p>If <code>raw</code> is true, then it expects <code>document</code> to already be a
   * &lt;soap:Envelope&gt; element, otherwise it wraps it in an envelope/body.
   *
   * <p>If <tt>noSession</tt> is true, no session object is created/accessed for this request.
   *
   * @throws ServiceException
   * @throws IOException
   */
  public final Element invoke(
      Element document, boolean raw, boolean noSession, String requestedAccountId)
   ...
   * &lt;soap:Envelope&gt; element, otherwise it wraps it in an envelope/body.
   *
   * <p><tt>nFormat</tt> indicates the format for notifications in response header (IMAP or
   * DEFAULT).
   *
   * <p><tt>noSession</tt> is assumed to be false.
   *
   * @throws ServiceException
   */
  public final Element invoke(
  ...
  public abstract Element invoke(
      Element document,
      boolean raw,
      boolean noSession,
      String requestedAccountId,
   ...
   * Sends the specified document as a Soap message and parses the response as a Soap message.
   *
   * <p>If <code>changeToken</code> is non-null, it's used in the soap context to detect modify
   * conflict.
   */
  public abstract Element invoke(
      Element document,
  ...
  /**
   * Sets the number of milliseconds to wait when reading data during a invoke call.
   *
   * <p>This implementation has no effect. Subclasses may override this method.
   *
