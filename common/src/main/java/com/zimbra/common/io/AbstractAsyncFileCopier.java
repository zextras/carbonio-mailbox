...
abstract class AbstractAsyncFileCopier implements FileCopier {
  ...
  protected AbstractAsyncFileCopier(int queueCapacity) {
    int qsize = queueCapacity > 0 ? queueCapacity : FileCopierOptions.DEFAULT_ASYNC_QUEUE_CAPACITY;
    if (qsize > MAX_QUEUE_SIZE) {
      ZimbraLog.io.warn("Async queue size " + qsize + " is too big; limiting to " + MAX_QUEUE_SIZE);
  ...
  private void enqueue(FileTask task, FileCopierCallback cb, Object cbarg) throws IOException {
    if (cb != null) {
      ...
      if (!okay) throw new IOException("Operation rejected by callback");
    }
    try {
      mQueue.put(task);
    } catch (InterruptedException e) {
      if (cb != null) cb.fileCopierCallbackEnd(cbarg, e);
    }
  }
