...
class BufferedPipe {
  ...
  private synchronized int roomAfterTail() {
    if (mIsFull) return 0;
    else if (mHead <= mTail) return mSize - mTail;
    else // mTail < mHead
    return mHead - mTail;
  }
  ...
  private synchronized int roomBeforeHead() {
    if (mIsFull) return 0;
    else if (mHead <= mTail) return mHead;
    else {
      // mTail < mHead
  ...
  private synchronized int headBytes() {
    if (mIsFull) return mSize - mHead;
    else if (mHead <= mTail) return mTail - mHead;
    else // mTail < mHead
    return mSize - mHead;
  }

  /**
   * Number of available data bytes between the beginning of buffer and mTail. Return value is
   * greater than 0 only when data has wrapped around the end of the buffer.
   *
   * @return
   */
  private synchronized int tailBytes() {
    if (mIsFull || mTail < mHead) return mTail;
    else return 0;
  }

  /**
   * Advance mTail to indicate there is less room (and therefore more data) in the buffer.
   *
  ...
  /**
   * Advance mHead to indicate there is more room (and therefore less data) in the buffer.
   *
  ...
  /**
   * Write len bytes in src from the specified offset. This method blocks until all bytes are
   * written or the pipe is closed.
   *
   * @param src
   * @param offset
   * @param len
  ...
  /**
   * Read len bytes into dst at the specified offset. This method blocks until all bytes are read or
   * the pipe is closed.
   *
   * @param dst
   * @param offset
  ...
  private abstract class BaseChannel implements Closeable, Channel, InterruptibleChannel {
  ...
  public class SinkChannel extends BaseChannel
    ...
    /**
     * Write bytes in src to this channel. This method blocks until all bytes are written or the
     * channel is closed. This method is equivalent to write(src, 0, src.length).
     *
     * @param src
     * @return actual number of bytes written
     * @throws IOException
    ...
    /**
     * Callers should synchronize on this lock if multiple writes must be done without getting
     * interrupted by other writer threads.
     *
     * @return
     */
  ...
  public class SourceChannel extends BaseChannel
    ...
    /**
     * Read bytes into dst from this channel. This method blocks until all bytes are read or the
     * channel is closed. This method is equivalent to read(dst, 0, dst.length).
     *
     * @param dst
     * @return actual number of bytes read
     * @throws IOException
    ...
    /**
     * Read len bytes into dst at the specified offset. This method blocks until all bytes are read
    ...
    /**
     * Callers should synchronize on this lock if multiple reads must be done without getting
     * interrupted by other reader threads.
     *
     * @return
     */
    public Object getLock() {
