...
public class FileCopierOptions {
  ...
  /**
   * Create a FileCopier according to the options specified in an encoded string. Options is a
   * colon-separated string with one or more fields. There are three different methods and each
   * method has a different list of fields.
   *
   * <p>file-opier-options := parallel-options | pipe-options | serial-options
   *
   * <p>parallel-options := "PARALLEL" [ ":" oio-or-nio [ ":" oio-copy-buffer-size [ ":"
   * async-queue-capacity [ ":" num-worker-threads ] ] ] ]
   *
   * <p>pipe-options := "PIPE" [ ":" oio-or-nio [ ":" oio-copy-buffer-size [ ":"
   * async-queue-capacity [ ":" num-pipes [ ":" pipe-buffer-size [ ":" num-reader-threads-per-pipe [
   * ":" num-writer-threads-per-pipe ] ] ] ] ] ] ]
   *
   * <p>serial-options := "SERIAL" [ ":" oio-or-nio [ ":" oio-copy-buffer-size ] ]
   *
   * <p>oio-or-nio := "OIO" | "NIO" ; choice between OIO (stream-based "old" IO) or NIO ; Default is
   * OIO.
   *
   * <p>oio-copy-buffer-size := size ; size of byte array to use in stream read/write loop ; Default
   * is 16k.
   *
   * <p>async-queue-capacity := size ; size of incoming request queue for PARALLEL and PIPE copiers
   * ; Requesting thread will block while queue is full. ; Default is 10000.
   *
   * <p>num-worker-threads := size ; number of worker threads in PARALLEL copier that execute queued
   * ; file operations ; Each worker thread does both file read and write. ; Default is 8.
   *
   * <p>num-pipes := size ; number of pipes to use in PIPE copier ; Default is 8.
   *
   * <p>pipe-buffer-size := size ; size of buffer that connects readers and writers ; Default is 1m.
   *
   * <p>num-reader-threads-per-pipe := size ; for each pipe, number of threads that execute queued
   * file ; operations by reading file data and writing to the pipe ; Non-copy operations (link,
   * move, etc.) are executed directly ; by reader threads instead of being piped to writers. ;
   * Default is 1.
   *
   * <p>num-writer-threads-per-pipe := size ; for each pipe, number of threads that reads data from
   * pipe ; and copies to file ; Default is 1.
   *
   * <p>size := DIGIT [DIGIT*] ["k" | "K" | "m" | "M" ] ; "k/K" means *1024, "m/M" means *1024^2
   *
   * @param options
   * @return
   */
  public FileCopierOptions(String options) throws ServiceException {
    ...
    if (fields.length > 2) mOIOCopyBufferSize = parseSize(fields[2]);

    if (mMethod.equals(Method.PARALLEL) || mMethod.equals(Method.PIPE)) {
      if (fields.length > 3) mAsyncQueueCapacity = parseSize(fields[3]);

      if (mMethod.equals(Method.PARALLEL)) {
        if (fields.length > 4) mParallelWorkers = parseSize(fields[4]);
      } else {
        if (fields.length > 4) mPipes = parseSize(fields[4]);
        if (fields.length > 5) mPipeBufferSize = parseSize(fields[5]);
        if (fields.length > 6) mReadersPerPipe = parseSize(fields[6]);
        if (fields.length > 7) mWritersPerPipe = parseSize(fields[7]);
      }
    }
  }
