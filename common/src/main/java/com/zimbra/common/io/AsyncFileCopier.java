...
class AsyncFileCopier extends AbstractAsyncFileCopier implements FileCopier {
  ...
  AsyncFileCopier(boolean useNIO, int copyBufSizeOIO, int queueCapacity, int numWorkers) {
    ...
    numWorkers = numWorkers > 0 ? numWorkers : FileCopierOptions.DEFAULT_CONCURRENCY;
    if (numWorkers > MAX_WORKER_THREADS) {
      ZimbraLog.io.warn(
          numWorkers + " worker threads are too many; limiting to " + MAX_WORKER_THREADS);
  ...
  private class WorkerThread extends Thread {
    ...
    private void copy(File src, File dest) throws IOException {
      ...
      try {
        if (mUseNIO) FileUtil.copy(src, dest);
        else FileUtil.copyOIO(src, dest, mCopyBuffer);
    ...
    private void link(File file, File link) throws IOException {
      ...
      try {
        ...
        if (ZimbraLog.io.isDebugEnabled())
          ZimbraLog.io.debug(
              "Created link " + link.getAbsolutePath() + " to file " + file.getAbsolutePath());
      } catch (FileNotFoundException e) {
        if (!ignoreMissingSource()) throw e;
      }
    }
    ...
    private void move(File oldPath, File newPath) throws IOException {
      ...
      if (moved) {
        if (ZimbraLog.io.isDebugEnabled())
          ZimbraLog.io.debug(
              "Moved " + oldPath.getAbsolutePath() + " to " + newPath.getAbsolutePath());
      } else {
        if (ZimbraLog.io.isDebugEnabled())
          ZimbraLog.io.debug(
              "Failed to move " + oldPath.getAbsolutePath() + " to " + newPath.getAbsolutePath());
      }
    }
    ...
    private void delete(File file) {
      ...
      if (deleted) {
        if (ZimbraLog.io.isDebugEnabled()) ZimbraLog.io.debug("Deleted " + file.getAbsolutePath());
      } else {
        if (ZimbraLog.io.isDebugEnabled())
