...
public class ZoneInfoParser {
  ...
  private static <T extends Comparable<? super T>> int nullsCompare(T o1, T o2, boolean nullFirst) {
    ...
    else if (o1 != null && o2 != null) return o1.compareTo(o2);
    else if (o1 != null && o2 == null) return nullFirst ? 1 : -1;
  ...
  private static class Year {
    ...
    private static int parseYear(String year) throws TZDataParseException {
      try {
        ...
        if ("maximum".startsWith(lyear)) return Integer.MAX_VALUE;
        else if ("minimum".startsWith(lyear)) return Integer.MIN_VALUE;
        else throw new TZDataParseException("Invalid year \"" + year + "\"");
      }
    }
  }
  ...
  public static class Month {
    ...
    public static int parseMonth(String str) throws TZDataParseException {
      ...
      for (int i = 0; i < sMonths.length; ++i) {
        if (sMonths[i].startsWith(mon)) return ++i;
      }
      throw new TZDataParseException("Invalid month \"" + str + "\"");
    }

    public static String toString(int mon) {
      if (mon >= 1 && mon <= sAbbrevs.length) return sAbbrevs[mon - 1];
      else return null;
    }
  }
  ...
  public static enum Weekday {
    ...
    public static Weekday lookUp(String str) {
      if (str != null && str.length() >= 2) {
        ...
        for (Weekday wd : values()) {
          if (wd.toString().startsWith(s)) return wd;
        }
      }
      return null;
    }
    ...
    public static Weekday lookUp(int index) {
      if (index >= 1 && index <= sByIndex.length) return sByIndex[index - 1];
      else return null;
    }
    ...
    public static int toInt(Weekday wkday) {
      for (int i = 0; i < sByIndex.length; ++i) {
        if (sByIndex[i].equals(wkday)) return i + 1;
      }
      return -1;
    }
  }
  ...
  public static class Day implements Comparable<Day> {
    ...
    public Day(String str) throws TZDataParseException {
      ...
      if (mType == null) {
        ...
        if (val.startsWith("last")) {
          ...
          if (mWeekday == null) throw new TZDataParseException("Invalid day \"" + str + "\"");
        } else {
          String[] fields;
          if (str.contains("<=")) {
          ...
          if (fields.length < 2) throw new TZDataParseException("Invalid day \"" + str + "\"");
      ...
      if (DayType.WEEKNUM.equals(mType)) {
        if (mWeeknum >= 1 && mWeeknum <= 5) {
          ...
          for (int i = firstDate; i <= lastDate; ++i) {
            ...
            if (cal.get(Calendar.DAY_OF_WEEK) == wkday) return i;
          }
        } else if (mWeeknum == -1) {
    ...
    private int estimateWeeknum() {
      if (mWeeknum != 0) return mWeeknum;
      if (DayType.ON.equals(mType)) return (mDate - 1) / 7 + 1;
      int date = mDate;
      DayType dtype = mType;
      ...
      if (DayType.ON_OR_BEFORE.equals(dtype)) date -= 6; // go to beginning of the week
  ...
  public static class Time implements Comparable<Time> {
    ...
    public Time(boolean negative, int hour, int min, int sec, TimeType type) {
      ...
      mSec = sec;
      mType = type;
      if (mHour == 0 && mMin == 0 && mSec == 0) mNegative = false;
      normalize();
    }
    ...
    public Time(String str) throws TZDataParseException {
      ...
      if (suffix >= '0' && suffix <= '9') {
        ...
        if (suffix == 'w') mType = TimeType.WALL_TIME;
        else if (suffix == 's') mType = TimeType.STANDARD_TIME;
        else if (suffix == 'u') mType = TimeType.UTC_TIME;
        else throw new TZDataParseException("Invalid time type \"" + suffix + "\"");
      ...
      try {
        ...
        if (fields.length >= 2) mMin = Integer.parseInt(fields[1]);
        if (fields.length == 3) mSec = Integer.parseInt(fields[2]);
    ...
    public int getDuration() {
      return (mHour * 3600 + mMin * 60 + mSec) * (mNegative ? -1 : 1);
    }
    ...
    public String toString() {
      ...
      if (mSec != 0) return String.format("%s%d:%02d:%02d%s", sign, mHour, mMin, mSec, suffix);
      else return String.format("%s%d:%02d%s", sign, mHour, mMin, suffix);
    }
    ...
    public int compareTo(Time other) {
      ...
      if (duration != otherDuration) return duration - otherDuration;
  ...
  public static class Until implements Comparable<Until> {
    ...
    public Until(List<String> tokens) throws TZDataParseException {
      ...
      if (numFields < 1) throw new TZDataParseException("Missing year in UNTIL");
      mYear = Year.parseYear(tokens.get(0));
      if (numFields >= 2) mMonth = Month.parseMonth(tokens.get(1));
      else mMonth = 1;
      if (numFields >= 3) mDay = new Day(tokens.get(2));
      else mDay = new Day(1);
      if (numFields >= 4) mTime = new Time(tokens.get(3));
      else mTime = new Time(false, 0, 0, 0, Time.TimeType.WALL_TIME);
    }
    ...
    public String toString() {
      ...
      if (mTime.getDuration() != 0) return String.format("%d %s %s %s", mYear, mon, day, mTime);
      else return String.format("%d %s %s", mYear, mon, day);
    }
    ...
    public int compareTo(Until other) {
      ...
      if (mYear < otherYear) return -1;
      else if (mYear > otherYear) return 1;
  ...
  public static class RuleLine {
    ...
    public RuleLine(List<String> tokens) throws TZDataParseException {
      if (tokens.size() < 9) throw new TZDataParseException("Not enough fields in a Rule line");
      mName = tokens.get(0);
      mFromYear = Year.parseYear(tokens.get(1));
      String toYear = tokens.get(2);
      if ("only".equalsIgnoreCase(toYear)) mToYear = mFromYear;
      else mToYear = Year.parseYear(toYear);
      mType = tokens.get(3);
    ...
    public boolean isCurrent() {
      return mToYear == Integer.MAX_VALUE;
    }

    @Override
    public String toString() {
      String fromYear, toYear;
      if (mFromYear == Integer.MAX_VALUE) fromYear = "max";
      else if (mFromYear == Integer.MIN_VALUE) fromYear = "min";
      else fromYear = Integer.toString(mFromYear);
      if (mToYear == mFromYear) toYear = "only";
      else if (mToYear == Integer.MAX_VALUE) toYear = "max";
      else if (mToYear == Integer.MIN_VALUE) toYear = "min";
      else toYear = Integer.toString(mToYear);
  ...
  public static class ZoneLine implements Comparable<ZoneLine> {
    ...
    public ZoneLine(List<String> tokens) throws TZDataParseException {
      int numFields = tokens.size();
      if (numFields < 4) throw new TZDataParseException("Not enough fields in a Zone line");
      mName = tokens.get(0);
      ...
      if (numFields > 4) mUntil = new Until(tokens.subList(4, numFields));
    }
    ...
    public boolean hasRule() {
      return RuleSaveType.RULE.equals(mRuleSaveType);
    }

    public boolean hasSave() {
      return RuleSaveType.SAVE.equals(mRuleSaveType);
    }

    public String getAbbrevFormat() {
      return mAbbrevFormat;
    }
    ...
    public String toString() {
      ...
      if (RuleSaveType.RULE.equals(mRuleSaveType)) {
        ...
        if (mSave.getDuration() == 0) ruleSave = "-";
        else ruleSave = mSave.toString();
      }
      if (mUntil != null)
        return String.format("%s %s %s %s %s", mName, mGmtOff, ruleSave, mAbbrevFormat, mUntil);
      else return String.format("%s %s %s %s", mName, mGmtOff, ruleSave, mAbbrevFormat);
    }

    public long getUntilInMillis() {
      if (mUntil == null) return Long.MAX_VALUE;
  ...
  public static class Leap {
    ...
    public Leap(List<String> tokens) throws TZDataParseException {
      if (tokens.size() < 6) throw new TZDataParseException("Not enough fields in a Leap line");
      mYear = Year.parseYear(tokens.get(0));
      ...
      if (corr == '+') mCorrPositive = true;
      else if (corr == '-') mCorrPositive = false;
      else throw new TZDataParseException("Invalid CORR value \"" + tokens.get(4) + "\"");
      String rs = tokens.get(5).toLowerCase();
      if ("rolling".startsWith(rs)) mRolling = true;
      else if ("stationary".startsWith(rs)) mRolling = false;
      else throw new TZDataParseException("Invalid R/S value \"" + tokens.get(5) + "\"");
    }
        ...
        if (ttype == StreamTokenizer.TT_WORD || ttype == dquote) {
          ...
          switch (lineType) {
            ...
            case LINK:
              ...
              if (!alias.equals(real)) mLinks.put(alias, real);
              break;
            case LEAP:
  ...
  public void analyze() {
    ...
    for (Iterator<Entry<String, String>> liter = mLinks.entrySet().iterator(); liter.hasNext(); ) {
      ...
      if (!mZones.containsKey(real)) {
        ...
        while ((real = mLinks.get(real)) != null) {
          if (mZones.containsKey(real)) {
            if (!alias.equals(real)) flattenedLinks.put(alias, real);
            break;
          }
        }
      }
    ...
    for (Iterator<Entry<String, String>> liter = flattenedLinks.entrySet().iterator();
        liter.hasNext(); ) {
      Entry<String, String> lentry = liter.next();
      String alias = lentry.getKey();
      String real = lentry.getValue();
    ...
    for (Iterator<Entry<String, String>> liter = mLinks.entrySet().iterator(); liter.hasNext(); ) {
      ...
      if (zone != null) zone.addAlias(alias);
      else
        System.err.println(
            "Invalid state!  Link \""
                + alias
                + "\" points to a non-existent zone \""
                + real
                + "\".");
    }
  ...
  public Zone getZone(String zoneName) throws TZDataParseException {
    if (!mAnalyzed) throw new TZDataParseException("not alayzed yet");

    Zone zone = mZones.get(zoneName);
    if (zone != null) return zone;
    String realZoneName = mLinks.get(zoneName);
    if (realZoneName != null) return mZones.get(realZoneName);
    return null;
  }
