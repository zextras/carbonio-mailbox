...
public final class ParsedDateTime {
  ...
  /**
   * This means that "Date" events are treated as having a time of 00:00:00 in the creator's default
   * timezone, UNLESS they have the "UTC only" flag set
   */
  ...
  public static ParsedDateTime parse(
      String str, TimeZoneMap tzmap, ICalTimeZone tz, ICalTimeZone localTZ) throws ParseException {
    return parse(str, tzmap, tz, localTZ, false);
  }

  private static ParsedDateTime parse(
      String str, TimeZoneMap tzmap, ICalTimeZone tz, ICalTimeZone localTZ, boolean utcOnly)
    ...
    if (m.matches()) {
      ...
      if (hour >= 0) {
        ...
        if (cal.get(Calendar.DATE) != date) {
          // 12AM does not exist for this date and timezone. This is possible if the day light cross
          // over happens at mid night
          // and this date happens to be day light cross over date (bug 51966). Set the time to the
          // correct mid night (i.e> 1AM)
      ...
      } else throw new ParseException("Invalid date/time specified: " + str, 0);
    }
  }
  ...
  public static ParsedDateTime parse(ZProperty prop, TimeZoneMap tzmap)
    ...
    if (tzname != null) tz = tzmap.lookupAndAdd(tzname);

    if (tz == null) tz = tzmap.getLocalTimeZone();
  ...
  public static ParsedDateTime parse(String str, TimeZoneMap tzmap)
    ...
    if (propValueColonIdx != -1) {
      ...
      if (tzidIdx != -1) {
        ...
        if (valueParamIdx > tzidIdx) tzid = str.substring(tzidIdx + 5, valueParamIdx);
        else tzid = str.substring(tzidIdx + 5, propValueColonIdx);
  ...
  /**
   * Returns a new ParsedDateTime object that has the same time values (hour, minutes, seconds, and
   * millis) and time zone as this object and the same date values (year, month, date) as "date"
   * object.
   *
   * @param other
   * @return
  ...
  /**
   * Checks if the other ParsedDateTime object has the same hour, minute, second and millisecond
   * values, in the timezone of this object.
   *
   * @param other
   * @return
   */
  public boolean sameTime(ParsedDateTime other) {
    ...
    return mCal.get(java.util.Calendar.HOUR_OF_DAY) == otherCal.get(java.util.Calendar.HOUR_OF_DAY)
        && mCal.get(java.util.Calendar.MINUTE) == otherCal.get(java.util.Calendar.MINUTE)
        && mCal.get(java.util.Calendar.SECOND) == otherCal.get(java.util.Calendar.SECOND)
        && mCal.get(java.util.Calendar.MILLISECOND) == otherCal.get(java.util.Calendar.MILLISECOND);
  }
  ...
  /**
   * Return Date suitable for use as UNTIL parameter of recurrence rule. Outlook always appends
   * "T000000Z" to UNTIL, causing meeting instance on the last day of recurrence to be excluded if
   * the UNTIL value was taken at face value. (bug 5885) To combat that, ignore the time component
   * and extend the UNTIL time to the end of the day (23:59:59) in the time zone of DTSTART. The
   * same treatment is needed if UNTIL is specified as date-only.
   *
   * @return
   */
  public Date getDateForRecurUntil(ICalTimeZone dtStartTZ) {
    ...
    if (!mHasTime
      ...
      cal.clear();
      cal.set(
          mCal.get(java.util.Calendar.YEAR),
      ...
      if (until.before(untilOriginal)) until = untilOriginal;
      return until;
    } else return getDate();
  }
  ...
  public String getDateTimePartString(boolean useOutlookCompatMode) {
    ...
    if (mHasTime) {
      ...
      toRet.append(twoDigitFormat.format(mCal.get(java.util.Calendar.HOUR_OF_DAY)));

      if (mCal.isSet(java.util.Calendar.MINUTE)) {
        toRet.append(twoDigitFormat.format(mCal.get(java.util.Calendar.MINUTE)));
      } else {
        toRet.append("00");
      }
  ...
  /**
   * Returns a String representing the date/time in UTC timezone. "YYYYMMDDThhmmssZ" if time
   * component is present, or just "YYYYMMDD" if date-only.
   *
   * @return
   */
  public String getUtcString() {
    if (isUTC()) return getDateTimePartString(false);
    else {
  ...
  public String toString() {
    if (mHasTime) {
      ...
      if (tzName != null) return "TZID=" + tzName + ":" + getDateTimePartString();
      else return getDateTimePartString();
    } else {
  ...
  public int getOffset() {
    if (mICalTimeZone == null) return 0;
