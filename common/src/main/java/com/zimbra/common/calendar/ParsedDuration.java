...
public final class ParsedDuration {
  ...
  // Don't make this a public method because adding this seconds value to ParsedDateTime is not
  // DST-safe.
  private int asSeconds() {
    return (mWeeks * SECONDS_IN_WEEK
  ...
  public boolean equals(Object other) {
    if (!(other instanceof ParsedDuration)) return false;
    ...
    switch (field) {
      ...
      default:
        throw ServiceException.INVALID_REQUEST(
            "Unknown field in ParsedDuration.add(" + field + "," + amount + ")", null);
    }
    return dur;
  }
  ...
  public static ParsedDuration parse(
      boolean negative, int weeks, int days, int hours, int mins, int secs) {
    ParsedDuration toRet = new ParsedDuration();
    if (negative) {
    ...
    if (toRet.mDays != 0 || toRet.mHours != 0 || toRet.mMins != 0 || toRet.mSecs != 0) {
      if (toRet.mWeeks != 0) {
        throw ServiceException.FAILURE(
            "Weeks may not be specified with other granularity in duration element: "
                + elt.getName(),
            null);
      }
    }
  ...
  public static ParsedDuration parse(String durationStr) throws ServiceException {
    ...
    if (m.matches()) {
      String p = m.group(1);
      String wk = m.group(2);
      if (p.charAt(0) == '-') retVal.mNegative = true;
      ...
      if (m.matches()) {
        ...
        throw ServiceException.INVALID_REQUEST(
            "Could not parse DURATION string: " + durationStr, null);
      }
    }
