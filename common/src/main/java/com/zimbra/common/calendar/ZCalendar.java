...
public class ZCalendar {
  ...
  public enum ICalTok {
    ...
    // Agent property for Organizer or Attendee, see RFC66389#7.1:
    // https://www.rfc-editor.org/rfc/rfc6638.html#section-7.1
    SCHEDULE_AGENT,
    ...
    ACCEPTED, /*COMPLETED,*/
    DECLINED,
    DELEGATED, /*IN_PROCESS,*/ /*NEEDS_ACTION,*/ /*TENTATIVE,*/
    ...
    // Used with ATTACH according to
    // http://msdn.microsoft.com/en-us/library/ee124339%28v=exchg.80%29.aspx
    X_FILENAME,
    ...
    // set to TRUE in series update to tell attendee to discard all exceptions while applying new
    // series
  ...
  public static class ZVCalendar {
    ...
    public ZComponent getComponent(ICalTok tok) {
      return findComponent(mComponents, tok);
    }
    ...
    public long getPropLongVal(ICalTok tok, long defaultValue) {
      ZProperty prop = getProperty(tok);
      if (prop != null) return Long.parseLong(prop.mValue);
    ...
    public void toICalendar(Writer w, boolean needAppleICalHacks) throws IOException {
      ...
      w.write(LINE_BREAK);
      for (ZProperty prop : mProperties) prop.toICalendar(w, needAppleICalHacks);

      for (ZComponent comp : mComponents) comp.toICalendar(w, needAppleICalHacks);
        ...
        if (ICalTok.VEVENT.equals(name)
          ...
          if (desc != null && desc.length() > 0) {
            ...
            if (prop == null) {
              ...
              if (val == null || val.length() < 1) prop.setValue(desc);
            }
          }
  ...
  /**
   * @author tim
   *     <p>Component has Name Properties Components
   */
  public static class ZComponent {
    ...
    public void toICalendar(Writer w, boolean needAppleICalHacks, boolean escapeHtmlTags)
        throws IOException {
      w.write("BEGIN:");
      w.write(mName);
      w.write(LINE_BREAK);
      ...
      for (ZProperty prop : mProperties) {
        ...
        // Also, don't generate the X-MICROSOFT-CDO-INTENDEDSTATUS property as iCal can't handle
        // this
        // property and becomes stale when the event transparency is modified in iCal5.
        if (needAppleICalHacks
            && (ICalTok.X_ALT_DESC.equals(prop.getToken())
                || ICalTok.X_MICROSOFT_CDO_INTENDEDSTATUS.equals(prop.getToken()))) continue;
  ...
  /**
   * @author tim
   *     <p>Property has Name Parameters Value
   */
  public static class ZProperty {
    ...
    public ZParameter getParameter(ICalTok tok) {
      return findParameter(mParameters, tok);
    }
    ...
    public void toICalendar(Writer w, boolean needAppleICalHacks, boolean escapeHtmlTags)
      ...
      sw.write(mName);
      for (ZParameter param : mParameters) param.toICalendar(sw, needAppleICalHacks);

      sw.write(':');
      if (ICalTok.CATEGORIES.equals(mTok) || ICalTok.RESOURCES.equals(mTok)) {
        if (mValueList != null) sw.write(toCommaSepText(mValueList));
      } else if (mValue != null) {
        String value = mValue;
        ...
        if (noEscape) sw.write(value);
        else sw.write(escape(value));
      }

      // Write with folding.
      String rawval = sanitize(sw.toString());

      if (escapeHtmlTags) { // Use only escapeHtmlTags when the file isn't supposed to be downloaded
        // (ie. it's supposed to be shown in the browser)
  ...
  public static class ZParameter {
    ...
    public void toICalendar(Writer w, boolean needAppleICalHacks) throws IOException {
      ...
      if (maValue == null || maValue.length() == 0) {
        w.write("\"\""); // bug 4941: cannot put a completely blank parameter value, will confuse
        // parsers
    ...
    /**
     * Sanitize a string to make it a valid param-value. DQUOTE is changed to a single quote and CTL
     * chars are changed to question marks ('?'). String is quoted if str is already quoted or if it
     * contains ',', ':' or ';'.
     *
     * <p>To workaround a bug in Outlook's MIME parser (see bug 12008), string is quoted if any
     * non-US-ASCII chars are present, e.g. non-English names. (These characters don't require
     * quoting according to RFC2445.)
     *
     * <p>Empty string is returned if str is null or is an empty string.
     *
     * @param str
     * @return
     */
    private static String quote(String str, boolean force) {
      ...
      if (needToQuote || force) return sb.toString();
      else return sb.substring(1, sb.length() - 1);
    }
  ...
  public static class DefaultContentHandler implements ZICalendarParseHandler {
    ...
    public void propertyValue(String value) throws ParserException {
      ...
      if (ICalTok.CATEGORIES.equals(token)
          || ICalTok.RESOURCES.equals(token)
          || ICalTok.FREEBUSY.equals(token)) {
        mCurProperty.setValueList(parseCommaSepText(value));
      } else {
        mCurProperty.setValue(
            unescape(
                null == value
                    ? value
                    : value.trim())); // be forgiving for leading and trailing spaces in property
        // values
      }
      if (mComponents.size() == 0) {
    ...
    public void parameter(String name, String value) {
      ...
      if (mCurProperty != null) {
        ...
        ZimbraLog.calendar.debug(
            "ERROR: got parameter " + name + "," + value + " outside of Property");
      }
    }
  }
  ...
  public static class ZCalendarBuilder {
    ...
    public static List<ZVCalendar> buildMulti(InputStream is, String charset)
    ...
    public static void parse(InputStream is, String charset, ZICalendarParseHandler handler)
        throws ServiceException {
