...
/**
 * Mapping of time zone IDs among Olson database, Java and Windows. Also maps deprecated names to
 * new names.
 */
public class TZIDMapper {
  ...
  public static class TZ implements Comparable<TZ> {
    ...
    public TZ(String tzid, String[] aliases, boolean isPrimary, int matchScore) {
      mID = tzid;
      if (aliases.length > 0) mAliases = aliases;
  ...
  public static synchronized Iterator<TZ> iterator(boolean primary) {
    if (primary) return sPrimaryTZs.iterator();
    else return sAllTZs.iterator();
  }
          ...
          if (lineUpper.equals("END:VTIMEZONE")) {
            ...
            if (tzid != null && tzid.length() > 0 && aliases != null) {
              if (!matchScoreSpecified) {
                if (isPrimary) matchScore = DEFAULT_MATCH_SCORE_PRIMARY;
                else matchScore = DEFAULT_MATCH_SCORE_NON_PRIMARY;
              }
              TZ tz = new TZ(tzid, aliases.toArray(new String[0]), isPrimary, matchScore);
              allTZs.add(tz);
              if (isPrimary) primaryTZs.add(tz);
              map.put(tzid, tz);
              if (aliases != null) {
            ...
            if (alias != null && alias.length() > 0) aliases.add(alias);
