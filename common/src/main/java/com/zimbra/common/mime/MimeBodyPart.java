...
public class MimeBodyPart extends MimePart {
  ...
  /**
   * Maximum number of octets in a line before we force a content transfer encoding. Many MTAs will
   * wrap lines over 998 octets.
   */
  ...
  MimeBodyPart(ContentType ctype, MimePart parent, long start, long body, MimeHeaderBlock headers) {
    ...
    encoding =
        targetEncoding =
            ContentTransferEncoding.forString(getMimeHeader("Content-Transfer-Encoding"));
  }
  ...
  ContentType updateContentType(ContentType ctype) {
    if (ctype != null
        && (ctype.getPrimaryType().equals("multipart")
            || ctype.getContentType().equals(ContentType.MESSAGE_RFC822))) {
      throw new UnsupportedOperationException(
          "cannot change a part into a multipart or message: " + ctype);
    }
  ...
  public byte[] getContent() throws IOException {
    ...
    if (encoding.normalize() == ContentTransferEncoding.BINARY) {
      ...
      return ByteUtil.getContent(
          getContentStream(),
          (int) (getSize() * (encoding == ContentTransferEncoding.BASE64 ? 0.75 : 1.0)));
    }
  }
  ...
  public Reader getTextReader() throws IOException {
    ...
    // if we're here, either there was no explicit charset or it was invalid, so try the default
    // charset
    ...
    // if we're here, the default charset was also either unspecified or unavailable, so go with the
  ...
  /**
   * Changes the <tt>Content-Type</tt> of the part to <tt>text/plain</tt> and sets the part's
   * content to the given text using the default charset.
   */
  public MimeBodyPart setText(String text) throws IOException {
    return setText(text, null, null, null);
  }

  public MimeBodyPart setText(
    ...
    ctype.setContentType(
        "text/" + (subtype == null || subtype.trim().isEmpty() ? ctype.getSubType() : subtype));
  ...
  public MimeBodyPart setContent(InputStreamSource iss, ContentTransferEncoding cte)
      throws IOException {
    return setContent(iss == null ? null : new PartSource(iss), cte);
  }

  private MimeBodyPart setContent(PartSource psource, ContentTransferEncoding cte)
      throws IOException {
  ...
  ContentTransferEncoding pickEncoding() throws IOException {
    ...
    if (is != null) {
      try {
        is =
            is instanceof ByteArrayInputStream || is instanceof BufferedInputStream
                ? is
  ...
  void markDirty(Dirty dirty) {
    ContentTransferEncoding cte =
        ContentTransferEncoding.forString(getMimeHeader("Content-Transfer-Encoding"));
    ContentTransferEncoding cteCurrent =
        targetEncoding == null ? ContentTransferEncoding.BINARY : targetEncoding;
