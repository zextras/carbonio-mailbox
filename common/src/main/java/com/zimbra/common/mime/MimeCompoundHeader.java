...
public class MimeCompoundHeader extends MimeHeader {
  ...
  private void parse() {
    ...
    for (int i = valueStart, count = content.length; i < count; i++) {
      ...
      if ((b == '\r' || b == '\n')
          && rfc2231.state != RFC2231State.VALUE
          && rfc2231.state != RFC2231State.SLOP) {
        escaped = false;
        continue;
      }
  ...
  public String getParameter(String pname) {
    if (pname != null) {
      ...
      for (Map.Entry<String, String> param : params.entrySet()) {
        if (pname.equalsIgnoreCase(param.getKey())) return param.getValue();
      }
    }
    return null;
  }
  ...
  /**
   * Printable ASCII characters that must be quoted in parameter values and avoided in parameter
   * names.
   */
    ...
    for (Map.Entry<String, String> param : params.entrySet()) {
      ...
      if (!nonascii) {
        ...
        if (LC.mime_encode_compound_xwiniso2022jp_as_iso2022jp.booleanValue()
          ...
          // however, if -Dsun.nio.cs.map is used then CharsetUtil.toCharset() returns a different
          // charset
          // we need to label the MIME with universally understood charset; even though we're
          // mapping that
        ...
        if (continued >= 0) {
          ...
          Charset encoding =
              charset == null || charset.isEmpty()
                  ? DEFAULT_CHARSET
                  : decodingCharset(CharsetUtil.toCharset(charset.toString()));
          parts.put(
              continued, new ParameterContinuation(encoding.toString(), encoded, value.toString()));
    ...
    void assembleContinuations(Map<String, String> attrs) {
      ...
      for (Map.Entry<String, Map<Integer, ParameterContinuation>> entry : partials.entrySet()) {
        ...
        for (Iterator<ParameterContinuation> it = parts.values().iterator(); it.hasNext(); ) {
          ...
          if (partial.encoded) {
            // we need to concatenate consecutive encoded parts because they don't necessarily break
