...
class MimeParser {
  ...
  private static class BoundaryChecker {
     ...
     * line thus far to the number of trailing dashes after the end of the boundary. <tt>null</tt>
     * value mean that we have not yet checked the last byte in the boundary; <tt>0</tt> means that
  ...
  /**
   * The index of the first '<tt>:</tt>' character in the current header, or <tt>-1</tt> if the
   * first colon has not yet been read.
   */
  private int colon = -1;
  ...
  /**
   * Terminates message parsing and returns the {@code MimeMessage} resulting from the parse. <b>Do
  ...
  private void boundary(String bnd, boolean isEnd, long partEnd) {
    ...
    while (!bnd.equals(pcurrent.boundary) && parts.size() > 1) {
      if ("".equals(pcurrent.boundary) && (boundaries == null || !boundaries.contains(bnd))) break;
    ...
    if (isEnd) {
      ...
      MimeBodyPart epilogue =
          new MimeBodyPart(
              new ContentType(ContentType.TEXT_PLAIN), multi, position, position, null);
   ...
   * as the line count of the part body. If the part being ended was a multipart preamble or
   * epilogue and was of nonzero length, associates the part with its parent appropriately.
   */
  ...
  private String defaultContentType() {
    ...
    boolean inDigest =
        parent != null && parent.getContentType().getContentType().equals("multipart/digest");
   ...
   * MimeHeaderBlock} that was accumulating part headers.
   *
  ...
  private boolean bodyStart(long pos) {
    ...
    if (mp instanceof MimeMultipart) {
      ...
      MimeBodyPart preamble =
          new MimeBodyPart(new ContentType(ContentType.TEXT_PLAIN), mp, pos, pos, null);
