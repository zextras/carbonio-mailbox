...
public class MimeHeaderBlock implements Iterable<MimeHeader> {
  ...
  /**
   * Returns the decoded value of the last header matching the given {@code name}, or {@code null}
   * if none match. If {@code name} is {@code null}, returns the value of the last header in the
   * block.
   */
  ...
  /**
   * Returns the decoded value of the last header matching the given {@code name}, or {@code null}
   * if none match. Header content not encoded with RFC 2047 encoded-words or RFC 2231 encoding is
   * decoded using the specified default charset. If {@code name} is {@code null}, returns the value
  ...
  /**
   * Returns the value of the last header matching the given {@code name}, or {@code null} if none
   * match. No decoding is performed other than removing the trailing CRLF. Header content not
   * encoded with RFC 2047 encoded-words or RFC 2231 encoding is decoded using the specified default
   * charset. If {@code name} is {@code null}, returns the value of the last header in the block.
   */
  ...
  /**
   * Returns the raw (byte array) value of the last header matching the given {@code name}, or
   * {@code null} if none match. If {@code name} is {@code null}, returns the last header in the
   * block.
   */
  ...
  /**
   * Returns the last header matching the given {@code name}, or {@code null} if none match. If
   * {@code name} is {@code null}, returns the last header in the block.
   */
  ...
  /**
   * Returns all headers matching the given {@code name}, in order of their appearance in the header
   * block. If none match, returns {@code null}.
   */
  ...
  /** Returns whether this header block contains any headers with the given {@code name}. */
  ...
  void announce(String hdrName, MimeHeader header) {
    if (parent != null && hdrName.toLowerCase().equals("content-type")) {
      ...
      String defaultType =
          container instanceof MimeMultipart
              ? ((MimeMultipart) container).getDefaultChildContentType()
              : "text/plain";
  ...
  /**
   * Creates a new {@code MimeHeader} and adds it to the header block, replacing any other headers
   * in the block with the same name. The new header is inserted in the location of the first
   * replaced matching header. If no matching header exists in the block, the logic from {@link
   * #addHeader(String, String)} applies. If the {@code value} is {@code null}, any matching
   * existing headers are removed but no new header is added.
   *
   * @param name The new header's name.
   * @param value The new header's value. If non-ASCII, RFC 2047 encoding will be applied to the
   *     value when generating the resulting header line.
   ...
   * in the block with the same name. The new header is inserted in the location of the first
   * replaced matching header. If no matching header exists in the block, the logic from {@link
   * #addHeader(String, byte[])} applies. If the {@code bvalue} is {@code null}, any matching
   * existing headers are removed but no new header is added.
   *
   * @param name The new header's name.
   * @param bvalue The new header's raw value. The added header is formed by concatenating {@code
   *     name}, "{@code : }", and {@code bvalue} into a single {@code byte[]}.
  ...
  /**
   * Adds a copy of the header to the header block, replacing any other headers in the block with
   * the same name. The header is inserted in the location of the first replaced matching header. If
   * no matching header exists in the block, the logic from {@link #addHeader(MimeHeader)} applies.
   * If the {@code header} is {@code null}, any matching existing headers are removed and no new
   * header is added.
   *
  ...
  /**
   * Creates a new {@code MimeHeader} and adds it to the header block, using our preferred ordering
   * if possible.
   *
   * <ul>
   *   <li>If the header is tagged "unique" and there is already a header with the same name in the
   *       block, this call is converted into a call to {@link #setHeader(String, MimeHeader)} and
   *       the existing header(s) will be replaced with a copy of the new header.
   *   <li>If the header is tagged "first" (i.e. the {@code Resent-*} headers), then it will be
   *       prepended at the very beginning of the block.
   *   <li>If the header is tagged "prepend" (i.e. {@code Return-Path} and {@code Received}), it
   *       will be added *before* any other such headers.
   *   <li>If none of the above cases apply and the header block is in our preferred ordering, the
   *       header is inserted in its appropriate position, after any other headers with the same
   *       priority.
   *   <li>If none of the above apply, the header will be inserted after the last occurrence of a
   *       header with the same name.
   *   <li>If none of the above apply, the header will be inserted at the end of the block.
   * </ul>
   *
   * @param name The new header's name.
   * @param value The new header's value. If non-ASCII, RFC 2047 encoding will be applied to the
  ...
  /**
   * Adds a copy of the header to the header block, using our preferred ordering if possible.
   *
   * <ul>
   *   <li>If the header is tagged "unique" and there is already a header with the same name in the
   *       block, this call is converted into a call to {@link #setHeader(String, MimeHeader)} and
  ...
  public MimeHeaderBlock addHeader(MimeHeader header) {
    if (header == null || validateFieldName(header.getName()) == null) return this;
    ...
    if (hinfo.first) {
      ...
      for (index = 0; index < headers.size(); index++) {
        if (headers.get(index).hinfo.position >= hinfo.position) break;
      }
    } else if (ordered) {
      ...
      for (index = headers.size(); index > 0; index--) {
        ...
        if (existing.hinfo.position > hinfo.position) continue;
        if (existing.hinfo.position < hinfo.position) {
          index = Math.max(index, slot);
          break;
        }
        if (existing.getName().equalsIgnoreCase(header.getName())) break;
        slot = Math.max(index, slot);
      }
    } else {
      // put it after the last matching header, or at the end of the block if none match
      for (index = headers.size(); index > 0; index--) {
        if (headers.get(index - 1).getName().equalsIgnoreCase(header.getName())) break;
      }
      if (index == 0) {
        index = headers.size();
      }
  ...
  /**
   * Creates a new {@code MimeHeader} and adds it to the end of the header block. No other logic or
   * sanity-checking is applied. Tracks whether the resulting header block is in our preferred
   * order; if so, subsequent calls to {@link #addHeader(String, String)} will maintain that
   * ordering.
   *
   * @param name The new header's name.
   * @param value The new header's value. If non-ASCII, RFC 2047 encoding will be applied to the
   ...
   * Creates a new {@code MimeHeader} and adds it to the end of the header block. No other logic or
   * sanity-checking is applied. Tracks whether the resulting header block is in our preferred
   * order; if so, subsequent calls to {@link #addHeader(String, byte[])} will maintain that
   * ordering.
   *
   * @param name The new header's name.
   * @param bvalue The new header's raw value. The added header is formed by concatenating {@code
  ...
  /**
   * Appends a copy of the header to the end of the header block. No other logic or sanity-checking
   * is applied. Tracks whether the resulting header block is in our preferred order; if so,
   * subsequent calls to {@link #addHeader(MimeHeader)} will maintain that ordering.
   *
   * @see MimeHeader.HeaderInfo
   */
  public MimeHeaderBlock appendHeader(MimeHeader header) {
    ...
    if (ordered
        && !isEmpty()
        && header.hinfo.position < headers.get(headers.size() - 1).hinfo.position) {
      this.ordered = false;
    }
    announce(header.getName(), header);
  ...
  /**
   * Appends a copy of all headers in the given header block to the end of this block. Ordering is
   * preserved. No other logic or sanity-checking is applied.
   *
  ...
  /**
   * Returns the length of this {@code MimeHeaderBlock} when serialized as a {@code byte[]}. This
   * count includes the extra {@code CRLF} that terminates the block.
   *
   * @see #toByteArray()
   */
