...
public class MimeHeader implements Cloneable {
  ...
  /**
   * Creates a new {@code MimeHeader} with {@code value} as the field value. Header will be
  ...
  /**
   * Creates a new {@code MimeHeader} serialized as "<tt>{name}: {bvalue}CRLF</tt>". {@code bvalue}
   * is copied verbatim; no charset transforms, encoded-word handling, or folding is performed.
   */
  ...
  /**
   * Reserializes the {@code MimeHeader}, using {@code bvalue} as the field value (the bit after the
   * '<tt>:</tt>'). {@code bvalue} is copied verbatim; no charset transforms, encoded-word handling,
   * or folding is performed.
   */
  ...
  /**
   * Marks the header as "dirty" and requiring reserialization. To enforce this reserialization
   * requirement, unsets {@link #content}.
   */
  ...
  /**
   * Permits a subclass to regenerate the {@code byte[]} content of the header as a result of
   * changes. Implementations of this method should first call {@link #isDirty()} and perform a
   * no-op if it returns {@code false}.
   */
  ...
  static class EncodedWord {
    ...
    static String encode(final String value, final String requestedCharsetName) {
      ...
      Charset charset =
          CharsetUtil.checkCharset(value, CharsetUtil.toCharset(requestedCharsetName));
    ...
    private static String encode(final String value, Charset charset, String outputCharsetName) {
      ...
      sb.append("=?")
          .append(
              outputCharsetName == null
                  ? charset.name().toLowerCase()
                  : outputCharsetName.toLowerCase());
