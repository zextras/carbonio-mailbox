...
public class MimeMultipart extends MimePart implements Iterable<MimePart> {
  ...
  public MimeMultipart(String subtype) {
    super(
        new ContentType(
                "multipart/" + (subtype == null || subtype.trim().equals("") ? "mixed" : subtype))
            .setParameter("boundary", generateBoundary()));
  }

  MimeMultipart(
      ContentType ctype, MimePart parent, long start, long body, MimeHeaderBlock headers) {
  ...
  /**
   * Returns an iterator over the child parts of this multipart. Note that changes made via the
   * iterator (e.g. {@link Iterator#remove()}) will not affect the contents of the multipart.
   */
  @Override
  ...
  @Override
  ContentType updateContentType(ContentType ctypeParam) {
