...
public class InternetAddress implements Cloneable {
  ...
  public InternetAddress setCharset(String charset) {
    ...
    this.charset =
        charset == null || charset.trim().equals("")
            ? CharsetUtil.UTF_8
            : CharsetUtil.toCharset(charset.trim());
    return this;
  }
  ...
  public String toString() {
    if (display != null) {
      return MimeHeader.escape(display, charset, requestedCharset, true)
          + (email != null ? (" <" + email + '>') : "");
    } else if (email != null) {
      return email;
    } else {
  ...
  /**
   * Returns the address, formatted as a {@link String}. No MIME encoding is done, so this form
   * cannot be used in a message header.
   */
  ...
  /**
   * Parse the given comma separated sequence of addresses into a list of {@link InternetAddress}
  ...
  static List<InternetAddress> parseHeader(
      final byte[] content, final int start, final int length) {
    return parseHeader(content, start, length, null);
  }

  public static List<InternetAddress> parseHeader(
    ...
    while (pos < end) {
      ...
      if (c == '\r' || c == '\n') {
        ...
        empty = false;
          ...
          || (c == ';' && (group != null || LC.mime_split_address_at_semicolon.booleanValue()))) {
  ...
  public static class Group extends InternetAddress {
    ...
    @Override
    public String toString() {
      return MimeHeader.escape(getName(), getCharset(), getRequestedCharset(), true)
          + ": "
          + getAddress();
    }
  ...
  private void parse(byte[] content, int start, int length, boolean angle) {
    ...
    for (int pos = start, end = start + length; pos < end; pos++) {
      ...
      if (c == '\r' || c == '\n') {
        ...
        // handle comments outside of quoted strings/encoded words, even where they're not actually
        // permitted
        if (!escaped && c == '\\') {
          escaped = true;
        ...
        // we've just read the leading '<' of an angle-addr, so we now read the addr-spec and the
        // closing '>'
        if (!builder.isEmpty()) {
        ...
        while (!builder.isEmpty()
            && (builder.endsWith((byte) '/') || builder.endsWith((byte) '\\'))) {
          builder.pop();
        }
        address = builder.appendTo(address);
        slop = true;
    ...
    if (!angle && atsign) {
      // there are some tricky little bits to parsing addresses that weren't followed, so re-parse
      // as an address
    ...
    for (int i = 0, len = content.length(); i < len; i++) {
      ...
      if (c == '.' && dot) {
        return false;
          ...
          && (c < 0x00 || c >= MimeHeader.ATEXT_VALID.length || !MimeHeader.ATEXT_VALID[c])) {
        return false;
      }
      dot = c == '.';
    }
