...
public class CustomHostnameVerifier implements HostnameVerifier {
  ...
  public static void verifyHostname(String hostname, SSLSession session) throws IOException {
    ...
    if (ctm.isCertificateAcceptedForHostname(hostname, cert)) return;
  ...
  private static java.security.cert.X509Certificate certJavax2Java(
  ...
  public boolean verify(String hostname, SSLSession session) {
    try {
      ...
      ZimbraLog.security.debug("Hostname verification failed: hostname = " + hostname, e);
      return false;
    }
    return true;
  }
