...
public class ZAuthToken {
   ...
   *
   * <p>Note: The returning ZAuthToken could be "empty" if it cannot find the expected auth data,
   * this method does not throw exception . Callsites should call the isEmpty method and
   * react(returns null to callers, throw exception, etc) if it does not deal with "empty"
   * ZAuthToken.
   *
   * @param eAuthToken
   * @param isAdmin
   ...
   */
  public ZAuthToken(com.zimbra.common.soap.Element eAuthToken, boolean isAdmin)
      throws ServiceException {
  ...
  /**
   * Parse a JSON string to ZAuthToken, use by CLI tools to pass an auth token string from command
   * line
   *
   * <p>e.g. {"type": "foo", "value": "bar", "attrs": { "k1" : "v1", "k2" : "v2"}}
   *
   * <p>or should we use keys like they appear in our AuthResponse? probably not. e.g. {"_content":
   * "bar", "type": "foo", "_attrs": { "k1": "v1", "k2": "v2"}}
   */
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof ZAuthToken) {
      ...
      if (mType != null && !mType.equals(other.getType())) return false;

      if (mValue != null && !mValue.equals(other.getValue())) return false;

      if (mAttrs != null && !mAttrs.equals(other.getAttrs())) return false;
      return true;
    }
    return false;
  }
  ...
  public Map<String, String> cookieMap(boolean isAdmin) {
    if (mType == null) return toZimbraCookieMap(isAdmin);
    else if (mType.equals(YAHOO_CALENDAR_AUTHTOKEN_TYPE) || mType.equals(YAHOO_MAIL_AUTHTOKEN_TYPE))
  ...
  private void fromSoap(com.zimbra.common.soap.Element eAuthToken, boolean isAdmin)
      throws ServiceException {
    mType =
        eAuthToken.getAttribute(isAdmin ? AdminConstants.A_TYPE : AccountConstants.A_TYPE, null);
  ...
  private Element toSoap(Element parent, String authTokenElem, String attrElem, String nameAttr) {
    ...
    if (mType != null) eAuthToken.addAttribute(AccountConstants.A_TYPE, mType);
  ...
  private void fromHttpReq(HttpServletRequest request, boolean isAdmin) {
    ...
    if (fromZimbraCookies(cookieMap, isAdmin)) return;

    // no Zimbra cookies, look for Yahoo cookies
    if (fromYahooCookies(request, cookieMap, isAdmin)) return;
  ...
  private Map<String, String> toYahooCookieMap(boolean isAdmin) {
    ...
    if (mAttrs != null) {
      ...
      if (dCookie != null)
        cookieMap.put(
            YahooAuthData.attrNameToCookieName(YahooAuthData.YAHOO_DELEGATED_ATTR), dCookie);

      if (cookieMap.size() == 0) cookieMap = null;
    }
    return cookieMap;
  }

  private boolean fromYahooCookies(
      HttpServletRequest request, Map<String, String> cookieMap, boolean isAdmin) {
    ...
    if (yCookie != null
      ...
      if (yCookie != null) attrs.put(YahooAuthData.cookieNameToAttrName(YAHOO_Y_COOKIE), yCookie);
      if (tCookie != null) attrs.put(YahooAuthData.cookieNameToAttrName(YAHOO_T_COOKIE), tCookie);
      if (aCookie != null)
      ...
      if (accessKey != null) attrs.put(YAHOO_K_ATTR, accessKey);
      if (hostAccountId != null) attrs.put(YAHOO_H_ATTR, hostAccountId);

      // if there is no auth token type cookie from the cookie map, default to
      // YAHOO_MAIL_AUTHTOKEN_TYPE
      if (authTokenTypeCookie == null) authTokenTypeCookie = YAHOO_MAIL_AUTHTOKEN_TYPE;
  ...
  public static void main(String[] args) throws Exception {
    ...
    SoapHttpTransport.DebugListener dl =
        new SoapHttpTransport.DebugListener() {
    ...
    req.addElement(AccountConstants.E_ACCOUNT)
        .addAttribute(AccountConstants.A_BY, "name")
        .setText("user1");
