...
public class MimeCompoundHeaderTest {
    private void test(boolean isContentType, String description, String raw, String value, String[] params) {
        ...
        assertEquals(value, primary, description);
        assertEquals(params.length, mch.getParameterCount() * 2, description);
        for (int i = 0; i < params.length; i += 2) {
            assertEquals(params[i + 1], mch.getParameter(params[i]), description);
        }
    }
