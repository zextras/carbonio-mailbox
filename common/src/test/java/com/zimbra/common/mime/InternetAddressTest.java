...
public class InternetAddressTest {
    ...
    private void test(String msg, String raw, String display, String email) {
        ...
        assertEquals(display, iaddr.getPersonal(), msg);
        assertEquals(email, iaddr.getAddress(), msg);
    }
  ...
  void parseHeader() {
    ...
    assertTrue(iaddrs.get(0) instanceof InternetAddress.Group);
    InternetAddress.Group group = (InternetAddress.Group) iaddrs.get(0);
    assertEquals("mine", group.getName());
    List<InternetAddress> members = group.getMembers();
    assertEquals("Bob the Builder 1", members.get(0).getPersonal());
    assertEquals("bob@example.com", members.get(0).getAddress());
  ...
  void rfc2822a12() {
    ...
    assertEquals(1, iaddrs.size());
    assertEquals("Joe Q. Public", iaddrs.get(0).getPersonal());
    assertEquals("john.q.public@example.com", iaddrs.get(0).getAddress());
    ...
    assertEquals("Mary Smith", iaddrs.get(0).getPersonal());
    assertEquals("mary@x.test", iaddrs.get(0).getAddress());
    assertNull(iaddrs.get(1).getPersonal());
    assertEquals("jdoe@example.org", iaddrs.get(1).getAddress());
    assertEquals("Who?", iaddrs.get(2).getPersonal());
    assertEquals("one@y.test", iaddrs.get(2).getAddress());
  ...
  void rfc2822a13() {
    ...
    assertTrue(iaddrs.get(0) instanceof InternetAddress.Group);
    InternetAddress.Group group = (InternetAddress.Group) iaddrs.get(0);
    assertEquals("A Group", group.getName());
    List<InternetAddress> members = group.getMembers();
    assertEquals("Chris Jones", members.get(0).getPersonal());
    assertEquals("c@a.test", members.get(0).getAddress());
    assertNull(members.get(1).getPersonal());
    assertEquals("joe@where.test", members.get(1).getAddress());
    assertEquals("John", members.get(2).getPersonal());
    assertEquals("jdoe@one.test", members.get(2).getAddress());
  ...
  void rfc2822a5() {
    ...
    List<InternetAddress> iaddrs = InternetAddress.parseHeader(raw);
    assertEquals(1, iaddrs.size());
    assertEquals("Pete", iaddrs.get(0).getPersonal());
    assertEquals("pete@silly.test", iaddrs.get(0).getAddress());
    ...
    List<InternetAddress> members = group.getMembers();
    assertEquals("Chris Jones", members.get(0).getPersonal());
    assertEquals("c@public.example", members.get(0).getAddress());
    assertNull(members.get(1).getPersonal());
    assertEquals("joe@example.org", members.get(1).getAddress());
    assertEquals("John", members.get(2).getPersonal());
    assertEquals("jdoe@one.test", members.get(2).getAddress());
  ...
  void rfc2822a61() {
    ...
    assertEquals("Mary Smith", iaddrs.get(0).getPersonal()); //FIXME
    assertEquals("mary@example.net", iaddrs.get(0).getAddress());
    assertNull(iaddrs.get(1).getPersonal());
    assertEquals("jdoe@test.example", iaddrs.get(1).getAddress());
  }
  ...
  void rfc2822a63() {
    ...
    List<InternetAddress> iaddrs = InternetAddress.parseHeader(raw);
    assertEquals(1, iaddrs.size());
    assertEquals("John Doe", iaddrs.get(0).getPersonal());
    assertEquals("jdoe@machine.example", iaddrs.get(0).getAddress());
  ...
  void groups() {
    ...
    assertEquals(raw, ahdr.toString());
    assertEquals(3, ahdr.getAddresses().size());

    iaddrs = InternetAddress.parseHeader(raw);
    assertEquals(3, iaddrs.size());
    assertEquals("Mary Smith", iaddrs.get(0).getPersonal());
    assertEquals("mary@example.net", iaddrs.get(0).getAddress());
    assertTrue(iaddrs.get(1) instanceof InternetAddress.Group);
    InternetAddress.Group group = (InternetAddress.Group) iaddrs.get(1);
    assertEquals("friends", group.getName());
    List<InternetAddress> members = group.getMembers();
    assertNull(members.get(0).getPersonal());
    assertEquals("joe@where.test", members.get(0).getAddress());
    assertEquals("John", members.get(1).getPersonal());
    assertEquals("jdoe@one.test", members.get(1).getAddress());
    assertNull(iaddrs.get(2).getPersonal());
    assertEquals("john.q.public@example.com", iaddrs.get(2).getAddress());
  }
}
