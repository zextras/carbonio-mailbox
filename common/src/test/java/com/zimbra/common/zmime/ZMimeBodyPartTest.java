...
public class ZMimeBodyPartTest {
  ...
  @Test
  void setStringContent() throws Exception {
    String content = "these are the times\nthat try men's souls";
    ZMimeBodyPart mbp = new ZMimeBodyPart();

    mbp.setContent(content, "text/plain");
    assertArrayEquals(content.getBytes(), ByteUtil.getContent(mbp.getInputStream(), -1));

    mbp.setText(content);
    assertArrayEquals(content.getBytes(), ByteUtil.getContent(mbp.getInputStream(), -1));

    mbp.setContent(content, "text/plain; charset=utf-8");
    assertArrayEquals(content.getBytes("utf-8"), ByteUtil.getContent(mbp.getInputStream(), -1));
  ...
  @Test
  void shareContent() throws Exception {
    ...
    mbp.updateHeaders();
    mbp.writeTo(System.out);
    assertEquals(ZTransferEncoding.BASE64.toString(), mbp.getHeader("Content-Transfer-Encoding", null));
    assertArrayEquals(content.getBytes("ISO-2022-JP"), ByteUtil.getContent(mbp.getInputStream(), -1));
    ...
    private void testEncodingSelection(String msg, String content, ZTransferEncoding cteExpected) throws Exception {
        ...
        mbp.setText(content);
        mbp.updateHeaders();
        assertEquals(cteExpected.toString(), mbp.getHeader("Content-Transfer-Encoding", null), msg);
    }
