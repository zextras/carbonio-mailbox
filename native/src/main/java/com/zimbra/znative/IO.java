...
public class IO {
  ...
  private static native FileInfo fileInfo0(byte[] path) throws IOException;

  private static native void chmod0(byte[] path, long mode) throws IOException;

  private static native void setStdoutStderrTo0(byte[] path) throws IOException;
  ...
  /**
   * Creates a hard link "newpath" to existing "oldpath". If we do not support hard links on current
   * platform we just copy the file.
   */
