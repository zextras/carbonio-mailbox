...
final class TcpImapHandler extends ProtocolHandler {
  ...
  protected boolean processCommand() throws IOException {
    ...
    try {
      ...
      return keepGoing
          && (LC.imap_max_consecutive_error.intValue() <= 0
              || delegate.consecutiveError < LC.imap_max_consecutive_error.intValue());
      ...
      return LC.imap_max_consecutive_error.intValue() <= 0
          || delegate.consecutiveError < LC.imap_max_consecutive_error.intValue();
  ...
  /**
   * Only an IMAP handler thread may call. Don't call by other threads including IMAP session
   * sweeper thread, otherwise concurrency issues will arise.
   */
  ...
  private final class HandlerDelegate extends ImapHandler {
    ...
    protected void dropConnection(boolean sendBanner) {
      ...
      if (credentials != null && !goodbyeSent) {
        ZimbraLog.imap.info(
            "TCP:dropping connection for user %s (server-initiated)", credentials.getUsername());
      }
     ...
     *
     * <p>This is necessarily violating RFC 3501 3.4:
     *
     * <pre>
     ...
     * </pre>
     *
     * because there is no easy way to interrupt a blocking read from the socket ({@link
     * Socket#shutdownInput()} is not supported by {@link SSLSocket}) and trying to send an untagged
     * BYE not from this IMAP handler has concurrency issues.
     */
    @Override
    ...
    protected boolean doSTARTTLS(String tag) throws IOException {
      ...
      NetUtil.setSSLEnabledCipherSuites(
          tlsconn, config.getSslExcludedCiphers(), config.getSslIncludedCiphers());
