...
public class AccountUtil {
  ...
  public static InternetAddress getFriendlyEmailAddress(Account acct) {
    ...
    if (personalPart == null
        ...
        || personalPart.equals(acct.getAttr("uid"))) personalPart = null;
    ...
    try {
      ...
      ZimbraLog.misc.warn("An UnsupportedEncodingException occurred", uee);
    }

    // UTF-8 should *always* be supported (i.e. this is actually unreachable)
    try {
      // fall back to using the system's default charset (also pretty much guaranteed not to be
      // "unsupported")
  ...
  /**
   * Returns the <tt>From</tt> address used for an outgoing message from the given account. Takes
   * all account attributes into consideration, including user preferences.
   */
  public static InternetAddress getFromAddress(Account acct) {
    ...
    String personal =
        SystemUtil.coalesce(acct.getPrefFromDisplay(), acct.getDisplayName(), acct.getCn());
    try {
  ...
  /**
   * Returns the <tt>Reply-To</tt> address used for an outgoing message from the given account,
   * based on user preferences, or <tt>null</tt> if <tt>zimbraPrefReplyToEnabled</tt> is
   * <tt>FALSE</tt>.
   */
  ...
  public static boolean isDirectRecipient(Account acct, MimeMessage mm)
      throws ServiceException, MessagingException {
    return isDirectRecipient(acct, null, mm, -1);
  }

  public static boolean isDirectRecipient(
      Account acct, String[] otherAccountAddrs, MimeMessage mm, int maxToCheck)
      throws ServiceException, MessagingException {
    ...
    int numRecipientsToCheck =
        (maxToCheck <= 0 ? recipients.length : Math.min(recipients.length, maxToCheck));
    for (int i = 0; i < numRecipientsToCheck; i++) {
      String msgAddress = ((InternetAddress) recipients[i]).getAddress();
      if (acctMatcher.matches(msgAddress)) return true;
  ...
  /**
   * True if this address matches some address for this account (aliases, domain re-writes, etc) or
   * address of another account that this account may send as.
   */
   ...
   * Send-as addresses are not considered a match. A send-as address is an allow-from address that
   * corresponds to an account different from this account.
   */
   ...
   * @return Set containing all account email addresses in lower case or, empty if no email address
   *     is found
   ...
   */
  public static void addAccountToLogContext(
  ...
  public static boolean isAllowedDataSourceSendAddress(Account account, String address)
      throws ServiceException {
    if (address == null || addressHasInternalDomain(address)) {
      // only external addresses are allowed because internal addresses require the address owner to
      // grant send rights
      return false;
    }
    ...
    for (DataSource ds : dsList) {
      ...
      if (ds.isEnabled()
          && (DataSourceType.pop3.equals(dsType)
              || DataSourceType.imap.equals(dsType)
              || DataSourceType.caldav.equals(dsType))
  ...
  public static int getRootFolderIdForItem(
      MailItem item, Mailbox mbox, Set<Integer> dsRootFolderIds) throws ServiceException {
    int folderId = item.getFolderId();
    if (folderId == Mailbox.ID_FOLDER_USER_ROOT
        || folderId == Mailbox.ID_FOLDER_ROOT
  ...
  public static Set<String> parseConfig(Metadata config) throws ServiceException {
    ...
    Set<String> subscriptions = new HashSet<>(slist.size());
  ...
  public static MimeMessage generateMimeMessage(
      Account authAccount,
      Account ownerAccount,
      String subject,
      String charset,
      Collection<String> internalRecipients,
  ...
  public static MimeMultipart generateMimeMultipart(
      String mimePartText, String mimePartHtml, String mimePartXml) throws MessagingException {
    ...
    if (domain != null) {
      ...
      if (urlExpiration != 0) {
        BlobMetaData.encodeMetaData(
            AccountConstants.P_LINK_EXPIRY,
            System.currentTimeMillis() + urlExpiration,
            encodedBuff);
      }
    }
  ...
  public static String generateResetPasswordURL(Account account, long expiry, String code)
      throws ServiceException {
    StringBuilder encodedBuff = new StringBuilder();
    BlobMetaData.encodeMetaData(AccountConstants.P_ACCOUNT_ID, account.getId(), encodedBuff);
  ...
  public static void checkAliasLoginAllowed(Account acct, String loginEmailAddr)
      throws ServiceException {
    String accountName = acct.getName();
    if (!LC.alias_login_enabled.booleanValue()) {
      if (accountName.indexOf('@') != -1
          && loginEmailAddr.indexOf('@') != -1
          && !accountName.equalsIgnoreCase(loginEmailAddr)) {
        ZimbraLog.account.debug(
            "Alias login not enabled. '%s' is the alias account", loginEmailAddr);
        throw AuthFailedServiceException.AUTH_FAILED(
            loginEmailAddr, loginEmailAddr, "alias login not enabled.");
      } else {
        String acctLocalPart =
            EmailUtil.getLocalPartAndDomain(accountName) == null
                ? accountName
                : EmailUtil.getLocalPartAndDomain(accountName)[0];
        String loginEmailLocalPart =
            EmailUtil.getLocalPartAndDomain(loginEmailAddr) == null
                ? loginEmailAddr
                : EmailUtil.getLocalPartAndDomain(loginEmailAddr)[0];
   ...
   */
  public static SMTPMessage getSmtpMessageObj(Account acct)
      throws ServiceException, MessagingException {
   ...
   * @param authAcct auth account
   * @return account or null
   * @throws ServiceException service exception
   */
  public static Account getRequestedAccount(String identityId, Account authAcct)
      throws ServiceException {
    ...
    if (identity != null) {
      ...
      if (idEmailAddress != null) {
        return Provisioning.getInstance().get(AccountBy.name, idEmailAddress);
      }
    }
    return null;
  }
  ...
  public static class AccountAddressMatcher {
    ...
    private final boolean matchSendAs;
  ...
  public static class XmlPartDataSource extends MimePartDataSource {
    private static final String CONTENT_TYPE =
        MimeConstants.CT_XML_ZIMBRA_SHARE
            + "; "
            + MimeConstants.P_CHARSET
            + "="
            + MimeConstants.P_CHARSET_UTF8;
