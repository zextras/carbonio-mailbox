...
abstract class UBIDLdapOperation {
  ...
  protected void debug(
      LdapOp op, LdapUsage usage, long startTime, LDAPResponse resp, String extraInfo) {
    debugLogger.debug(
      ...
      try {
        ...
        if (debugEnabled()) {
          debug(
              ...
              startTime,
              connection,
              String.format("connPool=[%s(%d)]", getConnectionPoolLogName(pool), pool.hashCode()));
        }
      }
    }
  }
  ...
  static class CreateEntry extends UBIDLdapOperation {
    ...
    LDAPResult execute(UBIDLdapContext ctx, Entry entry) throws LDAPException {
      ...
      try {
        ...
        if (debugEnabled()) {
          debug(ctx, startTime, result, String.format("entry=[%s]", entry.toString()));
        }
      }
    }
  }
  ...
  static class DeleteEntry extends UBIDLdapOperation {
    ...
    LDAPResult execute(UBIDLdapContext ctx, String dn) throws LDAPException {
      ...
      try {
        ...
        if (debugEnabled()) {
          debug(ctx, startTime, result, String.format("dn=[%s]", dn));
        }
      }
    }
  }
        ...
        if (debugEnabled()) {
          ...
          String extraInfo =
              ctls.length() == 0
                  ? String.format(
                      "base=[%s], filter=[%s]",
                      searchRequest.getBaseDN(), searchRequest.getFilter().toString())
                  : String.format(
                      "controls=[%s], base=[%s], filter=[%s]",
                      ctls, searchRequest.getBaseDN(), searchRequest.getFilter().toString());
    ...
    CompareResult execute(UBIDLdapContext ctx, CompareRequest compareRequest) throws LDAPException {
      ...
      try {
        ...
        if (debugEnabled()) {
          ...
          String extraInfo =
              String.format(
                  "[%sDN=[%s], attribute=[%s] assertionValue=[%s]",
                  ctls.length() == 0 ? "" : String.format("controls=[%s], ", ctls),
                  compareRequest.getDN(),
    ...
    SearchResultEntry execute(UBIDLdapContext ctx, String dn, String[] attrs) throws LDAPException {
      ...
      try {
        SearchResultEntry entry =
            (attrs == null) ? ctx.getConn().getEntry(dn) : ctx.getConn().getEntry(dn, attrs);
        stat(startTime);
        return entry;
      } catch (LDAPException e) {
        if (ResultCode.SERVER_DOWN == e.getResultCode()) {
          SearchResultEntry entry =
              (attrs == null)
                  ? ctx.getConnectionPool().getEntry(dn)
        ...
        if (debugEnabled()) {
          debug(ctx, startTime, String.format("dn=[%s]", dn));
        }
      }
    }
  }
      ...
      try {
        ...
        if (debugEnabled()) {
          debug(
              ...
              startTime,
              result,
              String.format("dn=[%s] newRDN=[%s] newSuperiorDN=[%s]", dn, newRDN, newSuperiorDN));
        }
      }
    }
  }
