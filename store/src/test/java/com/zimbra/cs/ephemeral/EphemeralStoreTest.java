...
public class EphemeralStoreTest {
	...
	void testExpiry() throws Exception {
		...
		Thread.sleep(1500);
		// purging without specifying a dynamic component will purge all keys with
		// dynamic components
		...
		store.update(input2, target);
		Thread.sleep(1500);
		// purging with a dynamic component will purge only keys that match that dynamic
		// component
	...
	@Test
	void testIncorrectDataTypes() throws Exception {
		EphemeralKey key = new EphemeralKey("foo");
		EphemeralLocation target = new TestLocation();
		store.set(new EphemeralInput(key, "bar"), target);
		...
		assertEquals(Integer.valueOf(1), result.getIntValue(1));
...
//    @Test
//    void testGetAttrs() throws Exception {
...
//    }
	...
	@Test
	void testForwardingEphemeralStore() throws Exception {
