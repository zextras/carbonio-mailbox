...
public class TestContentTransferEncoding extends MailboxTestSuite {
	...
	private MimeMessage sendForwardedMessage(SendMsgRequest req, Message origMsg) throws Exception {
		...
		ZimbraSoapContext zsc = new ZimbraSoapContext(at, mbox.getAccountId(), SoapProtocol.Soap12,
				SoapProtocol.Soap12);
	...
	@Test
	void testBug98015() throws Exception {
		MimeMessage mimeMsg = new ZMimeMessage(JMSession.getSession(),
				new SharedByteArrayInputStream(getBug98015MimeString().getBytes()));
	...
	void testSimpleMimeMessage() throws Exception {
		...
		MimeMessage mimeMsg = new ZMimeMessage(JMSession.getSession(),
				new SharedByteArrayInputStream(getSimpleMimeString().getBytes()));
		ParsedMessage pm = new ParsedMessage(mimeMsg, true);
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX);
		Message msg = mbox.addMessage(null, pm, dopt, null);
		MsgToSend msgToSend = new MsgToSend();
	...
	void testMultipartMimeMessage() throws Exception {
		Mailbox mbox = MailboxManager.getInstance().getMailboxByAccountId(account.getId());
		MimeMessage mimeMsg = new ZMimeMessage(JMSession.getSession(),
				new SharedByteArrayInputStream(getMultipartMimeString().getBytes()));
		ParsedMessage pm = new ParsedMessage(mimeMsg, true);
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX);
		Message msg = mbox.addMessage(null, pm, dopt, null);
		MsgToSend msgToSend = new MsgToSend();
	...
	void testNestedMultipartMessage() throws Exception {
		MimeMessage mimeMsg = new ZMimeMessage(JMSession.getSession(),
				new SharedByteArrayInputStream(getNestedMimeString().getBytes()));
		ParsedMessage pm = new ParsedMessage(mimeMsg, true);
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX);
		Message msg = mbox.addMessage(null, pm, dopt, null);
		MsgToSend msgToSend = new MsgToSend();
