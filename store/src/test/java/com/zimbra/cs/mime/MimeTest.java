...
public class MimeTest {
  ...
  void multipartReportRfc822Headers() throws Exception {
    ...
    for (MPartInfo child : children) {
      assertTrue(types.remove(child.getContentType()), "Expected: " + child.getContentType());
    }
  ...
  void testMultipartPGP() throws Exception {
    ...
    assertEquals("application/octet-stream; name=\"encrypted.asc\"", pgpMsg.getFullContentType());
  }
  ...
  void should_not_include_part_with_filename_as_body_part() throws Exception {
    ...
    assertEquals(1, bodies.size());
    assertTrue(
        bodies.iterator().next().getMimePart().getContent().toString().contains("HTML body"));
  }
