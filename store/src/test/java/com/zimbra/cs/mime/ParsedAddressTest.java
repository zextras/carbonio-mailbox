...
public final class ParsedAddressTest {
 ...
 void getSortString() {
  ...
  assertEquals("user1, user2@zimbra.com, user3", ParsedAddress.getSortString(addrs));
 }
    ...
    private static final class Helper {
        ...
        private void verify(ParsedAddress pa) {
            if (honorific != null || pa.honorific != null) {
                assertEquals(honorific, pa.honorific, description);
            }
            if (first != null || pa.firstName != null) {
                assertEquals(first, pa.firstName, description);
            }
            if (middle != null || pa.middleName != null) {
                assertEquals(middle, pa.middleName, description);
            }
            if (last != null || pa.lastName != null) {
                assertEquals(last, pa.lastName, description);
            }
            if (suffix != null || pa.suffix != null) {
                assertEquals(suffix, pa.suffix, description);
            }
        }
    }
}
