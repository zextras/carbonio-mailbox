...
public class MailboxInfoTest extends TestCase {
  ...
  public void testStatusResponse() throws Exception {
    MailboxInfo info =
        parseResponse(" \"INBOX\" (MESSAGES 1 RECENT 2 UIDNEXT 3 UIDVALIDITY 4 UNSEEN 5)");
  ...
  public void testTolerateGarbageInStatusResponse() throws Exception {
    MailboxInfo info =
        parseResponse(
            " \"INBOX\" ( UIDNEXT 29370321 predicted next UID  UIDVALIDITY 1 UID validity status)");
    assertEquals("INBOX", info.getName());
  ...
  private static MailboxInfo parseResponse(String response) throws IOException {
    ImapInputStream is =
        new ImapInputStream(
            new ByteArrayInputStream(response.getBytes(Charsets.US_ASCII)), new ImapConfig());
