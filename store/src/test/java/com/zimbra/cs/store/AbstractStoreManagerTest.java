...
public abstract class AbstractStoreManagerTest extends MailboxTestSuite {
	...
	void store() throws Exception {
		...
		assertTrue(TestUtil.bytesEqual(mimeBytes, blob.getInputStream()),
		...
		assertTrue(TestUtil.bytesEqual(mimeBytes, mblob.getLocalBlob().getInputStream()),
	...
	void incoming() throws Exception {
		...
		MailboxBlob mblob = sm.link(staged, mbox, 0, 0);
		assertEquals(staged.getSize(), mblob.getSize(), "link size = staged size");
		assertTrue(TestUtil.bytesEqual(bytes, mblob.getLocalBlob().getInputStream()),
				"link content = mime content");
