...
public class ImapCommandThrottleTest extends MailboxTestSuite {
	...
	void select() {
		...
		SelectCommand select3 = new SelectCommand(new ImapPath(pathName + "foo", creds), (byte) 123,
				makeQri());
	...
	void examine() {
		...
		ExamineCommand examine = new ExamineCommand(new ImapPath(pathName, creds), (byte) 123,
				makeQri());
		...
		ExamineCommand examine2 = new ExamineCommand(new ImapPath(pathName, creds), (byte) 123,
				makeQri());
		assertTrue(examine.isDuplicate(examine2), "diff obj same fields");

		ExamineCommand examine3 = new ExamineCommand(new ImapPath(pathName + "foo", creds), (byte) 123,
				makeQri());
		assertFalse(examine.isDuplicate(examine3), "different path");

		ExamineCommand examine4 = new ExamineCommand(new ImapPath(pathName, creds), (byte) 101,
				makeQri());
	...
	void append() throws Exception {
		...
		List<Part> parts = new ArrayList<Part>();
		AppendMessage appendMsg2 = new AppendMessage(appendMsg.getPersistentFlagNames(),
				appendMsg.getDate(), parts);
	...
	void list() {
		...
		Set<String> mailboxNames = new HashSet<String>(
				Arrays.asList(new String[]{"mbox1", "mbox2", "mbox3"}));
		byte selectOptions = (byte) 24;
		byte returnOptions = (byte) 38;
		byte status = (byte) 67;
		AbstractListCommand list = new ListCommand(refName, mailboxNames, selectOptions, returnOptions,
				status);

		assertTrue(list.isDuplicate(list), "same obj");

		AbstractListCommand list2 = new ListCommand(refName, mailboxNames, selectOptions, returnOptions,
				status);
		...
		list2 = new ListCommand(refName,
				new HashSet<String>(Arrays.asList(new String[]{"mbox1", "mbox2"})),
