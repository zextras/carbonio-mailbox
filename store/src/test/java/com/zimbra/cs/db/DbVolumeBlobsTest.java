...
public class DbVolumeBlobsTest extends MailboxTestSuite {
	...
	public void setUp() throws Exception {
		...
		account = createAccount()
				...
				.create();
		acct2 = createAccount().
				withAttribute(ZAttrProvisioning.A_zimbraDumpsterEnabled, "TRUE").create();
	...
	void dumpsterBlobs() throws Exception {
		final Account account1 = createAccount().withAttribute(
				ZAttrProvisioning.A_zimbraDumpsterEnabled, "TRUE").create();
		Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(account1);
		DeliveryOptions opt = new DeliveryOptions();
		opt.setFolderId(Mailbox.ID_FOLDER_INBOX);
		Map<String, String> digestToPath = new HashMap<String, String>();
		Volume vol = VolumeManager.getInstance().getCurrentMessageVolume();
