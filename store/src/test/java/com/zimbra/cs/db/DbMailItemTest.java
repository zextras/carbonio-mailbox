...
public final class DbMailItemTest {
 ...
 void setIndexIds() throws Exception {
  ...
  assertEquals(100, DbUtil.executeQuery(conn,
    "SELECT index_id FROM mboxgroup1.mail_item WHERE id = ?", 100).getInt(1));
  assertEquals(200, DbUtil.executeQuery(conn,
 ...
 void getReIndexIds() throws Exception {
  ...
  assertEquals(ImmutableList.of(100, 101, 102, 300, 301, 302),
    DbMailItem.getReIndexIds(conn, mbox, EnumSet.<MailItem.Type>of(MailItem.Type.MESSAGE)));
  assertEquals(ImmutableList.of(200, 201, 400, 401),
    DbMailItem.getReIndexIds(conn, mbox, EnumSet.<MailItem.Type>of(MailItem.Type.CONTACT)));
  assertEquals(ImmutableList.of(100, 101, 102, 200, 201, 300, 301, 302, 400, 401),
 ...
 void resetIndexId() throws Exception {
  ...
  assertEquals(0, DbUtil.executeQuery(conn,
    "SELECT index_id FROM mboxgroup1.mail_item WHERE id = ?", 100).getInt(1));
  assertEquals(0, DbUtil.executeQuery(conn,
    "SELECT index_id FROM mboxgroup1.mail_item_dumpster WHERE id = ?", 200).getInt(1));
 }
