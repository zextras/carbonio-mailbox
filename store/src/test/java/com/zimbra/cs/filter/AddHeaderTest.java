...
public class AddHeaderTest extends MailboxTestSuite {
	...
	void testAddHeaderWithMultilineValue() {
		try {
			...
			assertEquals("=?UTF-8?B?bGluZTENCglsaW5lMg0KCWxpbmUz?=", value);
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	}
	 ...
	 */
	@Test
	void testAddHeaderLinebreakVariable() {
		...
		try {
			...
			assertEquals("=?UTF-8?B?bGluZSAxCmhlYWRlcjogbGluZTIKDQpsaW5lIDQK?=", headers[0]);
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	}
	...
	void testAddHeaderNonAscii() {
		try {
			...
			// the header field is inserted at the beginning of the existing message header.
			assertEquals(1, index);
			assertEquals("=?UTF-8?B?6L+95Yqg44OY44OD44OA5YCk?=", newHeader);
	 ...
	 */
	@Test
	void testAddHeaderNonAsciiVariable() {
		...
		try {
			...
			assertEquals(1, index);
			assertEquals(
					"=?UTF-8?B?5pel5pys6Kqe44Gu5Lu25ZCN44CC5pel5pys6Kqe?=\r\n =?UTF-8?B?44Gu5Lu25ZCN44CC5pel5pys6Kqe44Gu5Lu25ZCN44CC?=",
					newHeader);
	...
	@Test
	void testAddHeaderSameHeaderNameMultipleHeaderValues1() {
		...
		try {
			...
			List<ItemId> ids = RuleManager.applyRulesToIncomingMessage(
					...
					acct1.getName(), null, new DeliveryContext(),
					Mailbox.ID_FOLDER_INBOX, true);
			...
			assertEquals("Return-Path: user1@domain1.zimbra.com", (String) e.nextElement());
			assertEquals("X-Dummy-Header: new value", (String) e.nextElement());

			// The rest of the headers
			int index = 1;
			while (e.hasMoreElements() && index < sampleBaseMsg2.length) {
				String value = (String) e.nextElement();
				assertEquals(sampleBaseMsg2[index++], value);
			}
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	...
	void testEmptyHeaderName() {
		try {
			...
			for (Enumeration<Header> e = mdnMsg.getMimeMessage().getAllHeaders(); e.hasMoreElements(); ) {
				Header temp = e.nextElement();
				assertNotEquals(temp.getValue(), "my-new-header-value");
			}
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	...
	void testAddHeaderNameStartingWithSpace() {
		try {
			...
			for (Enumeration<Header> e = mdnMsg.getMimeMessage().getAllHeaders(); e.hasMoreElements(); ) {
				Header temp = e.nextElement();
				assertNotEquals(temp.getName(), "X-My-Test");
				assertNotEquals(temp.getValue(), "my-new-header-value");
			}
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
	...
	@Test
	void testAddHeaderBadContentType() {
	...
	@Test
	void testAddHeaderImmutableHeaders() {
		String sampleBaseMsg = "Subject: example\n"
				+ "to: test@zimbra.com\n";
		...
		try {
			...
			Integer itemId = mbox1.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE)
					.get(0);
			Message message = mbox1.getMessageById(null, itemId);
			assertNull(message.getMimeMessage().getHeader("Content-Type"));
			assertNull(message.getMimeMessage().getHeader("Content-Disposition"));
			assertNull(message.getMimeMessage().getHeader("Content-Transfer-Encoding"));
			assertNull(message.getMimeMessage().getHeader("MIME-Version"));
	 ...
	 */
	@Test
	void testAddHeaderImmutableHeadersWithWhiteSpaces() {
		String sampleBaseMsg = "Subject: example\n"
				+ "to: test@zimbra.com\n";
		...
		try {
			...
			for (Enumeration<Header> e = mdnMsg.getMimeMessage().getAllHeaders(); e
				...
				if (temp.getName().equals("X-New-Header")) {
					matchFound = true;
					assertEquals("my-new-header-value", temp.getValue());
				}
			}
