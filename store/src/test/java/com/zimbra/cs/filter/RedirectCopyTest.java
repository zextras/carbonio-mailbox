...
public class RedirectCopyTest extends MailboxTestSuite {
	 ...
	 */
	@Test
	void testPlainRedirectMimeMsg1() throws ServiceException {
		final Account account1 = createAccount().create();
		final Account account = createAccount().create();
		final Account accountRedirect = createAccount().create();
		final String accountRedirectName = accountRedirect.getName();
		...
		try {
			...
			final String mail = account1.getName();
			final String mail1 = account.getName();
			String rawReal = "From: " + mail + "\n" + "To: " + mail1 + "\n"
					+ "Subject: Test\n" + "\n" + body;
			RuleManager.applyRulesToIncomingMessage(new OperationContext(mbox),
					mbox, new ParsedMessage(rawReal.getBytes("Shift_JIS"), false), 0, account.getName(),
					new DeliveryContext(), Mailbox.ID_FOLDER_INBOX, true);

			// verify the redirected message
			Integer item = mboxRedirect.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE)
					.get(0);
			Message redirectMsg = mboxRedirect.getMessageById(null, item);
