...
final class EscapeSequencesTest extends MailboxTestSuite {
	...
	void setUp() throws Exception {
		acct1 = createAccount()
				.withAttribute(Provisioning.A_zimbraSieveNotifyActionRFCCompliant, "TRUE")
				.create();
		acct2 = createAccount().withAttribute(Provisioning.A_zimbraSieveNotifyActionRFCCompliant,
						"TRUE")
				.create();
	}
	...
	void testAddressEscape1() {
		String filter =
				"if address :comparator \"i;ascii-casemap\" :matches \"To\" \"user\\*@zimbra.com\" {"
						+ "tag \"list\";}";
	...
	void testAddressEscape2() {
		doTestHeaderEscapePattern(
				"if address :comparator \"i;ascii-casemap\" :matches \"To\" \"user\\123@zimbra.com\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape1() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"Subject\" \"test\\\\123\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape2() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"Subject\" \"test\\*\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape3() {
		doTestHeaderEscapePattern(
				"if not header :comparator \"i;ascii-casemap\" :matches \"Subject\" \"test\\123\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape4() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"X-Header-0BackSlash\" \"testSample\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape5() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"X-Header-0BackSlash\" \"test\\Sample\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape6() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"X-Header-1BackSlash\" \"test\\\\Sample\" {"
						+ "tag \"list\";}");
	}
	...
	public void testHeaderEscape8() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"X-Header-2BackSlash\" \"test\\\\\\\\Sample\" {"
						+ "tag \"list\";}");
	}
	...
	void testHeaderEscape9() {
		doTestHeaderEscapePattern(
				"if header :comparator \"i;ascii-casemap\" :matches \"X-Header-DoubleQuote\" \"test\\\"Sample\" {"
						+ "tag \"list\";}");
	}
	...
	public void testReplaceheaderEscape1() {
		doTestReplaceheaderEscapePattern(
				"replaceheader :newvalue \"[replaced]\" :matches \"Subject\" \"test\\\\123\";");
	}
	...
	void testNotifyEscape1() {
		final String mail1 = acct1.getName();
		final String mail2 = acct2.getName();
		String filterScript =
				"require [\"enotify\", \"variables\"];\n"
						+ "notify :message \"Sample\\Message\"\n"
						+ "  :from \"" + mail1 + "\"\n"
						+ "  \"mailto:" + mail2 + "?Importance=High&body=sample_body\";";
	...
	void testNotifyEscape4() {
		final String mail = acct1.getName();
		final String mail2 = acct2.getName();
		String filterScript =
				"require [\"enotify\", \"variables\"];\n"
						+ "notify :message \"Sample\\\\\\\\Message\"\n"
	...
	public void doTestHeaderEscapePattern(String filterScript) {
		...
		LmtpAddress sender = new LmtpAddress("<user123@zimbra.com>", new String[]{"BODY", "SIZE"},
				null);
	...
	public void doTestEnvelopeEscapePattern(String filterScript) {
		...
		LmtpEnvelope env = new LmtpEnvelope();
		LmtpAddress sender = new LmtpAddress("<user123@example.com>", new String[]{"BODY", "SIZE"},
				null);
		LmtpAddress recipient = new LmtpAddress("<test1@zimbra.com>", null, null);
		env.setSender(sender);
		env.addLocalRecipient(recipient);
	...
	private void doTestReplaceheaderEscapePattern(String filterScript) {
		try {
			...
			RuleManager.applyRulesToIncomingMessage(
					...
					new DeliveryContext(),
					Mailbox.ID_FOLDER_INBOX, true);
			Integer itemId = mbox.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE)
					.get(0);
