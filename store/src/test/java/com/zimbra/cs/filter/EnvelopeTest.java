...
public class EnvelopeTest extends MailboxTestSuite {
	...
	public void setUp() throws Exception {
		...
		final Account original = createAccount().create();
	...
	void testFrom() {
		...
		LmtpAddress sender = new LmtpAddress(getOriginal(), new String[]{"BODY", "SIZE"}, null);
		LmtpAddress recipient = new LmtpAddress(getAddress(), null, null);
	...
	void testMailFromBackslash() {
		...
		LmtpEnvelope env = new LmtpEnvelope();
		LmtpAddress sender = new LmtpAddress("<\"ti\\\\m\"@example.com>", new String[]{"BODY", "SIZE"},
				null);
		LmtpAddress recipient = new LmtpAddress("<xyz@zimbra.com>", null, null);
		env.setSender(sender);
		env.addLocalRecipient(recipient);
	...
	public void testVariable(String filterScript) {
		...
		LmtpEnvelope env = new LmtpEnvelope();
		LmtpAddress sender = new LmtpAddress("<envelope_from@example.com>",
	...
	@Test
	void testTo_Alias() throws ServiceException {
		String filterScript =
				"require [\"variables\", \"envelope\", \"relational\", \"comparator-i;ascii-numeric\"];\n"
						+ "set \"rcptto\" \"unknown\";\n"
	...
	@Test
	void testCountForEmptyFromHeader() throws ServiceException {
		Account account = createAccount().create();
		String filterScript =
		...
		LmtpAddress sender = new LmtpAddress("<>", new String[]{"BODY", "SIZE"}, null);
		final String mail = account.getName();
		LmtpAddress recipient = new LmtpAddress("<" + mail + ">", null, null);
		env.setSender(sender);
		env.addLocalRecipient(recipient);
	...
	void testNumericNegativeValueCount() {
		String filterScript =
				"require [\"envelope\", \"tag\", \"relational\", \"comparator-i;ascii-numeric\"];\n"
	...
	@Test
	void testHeaderNameWithLeadingSpace() throws ServiceException {
		Account account = createAccount().create();
		final String mail = account.getName();
		String filterScript = "require \"envelope\";\n"
	...
	@Test
	void testHeaderNameWithTrailingSpace() throws ServiceException {
		Account account = createAccount().create();
		String filterScript = "require \"envelope\";\n"
				+ "if envelope :matches \"TO \" \"*@zimbra.com\" {\n"
				+ "    tag \"t1\";\n"
	...
	@Test
	void testHeaderNameWithLeadingAndTrailingSpace() throws ServiceException {
		Account account = createAccount().create();
		String filterScript = "require \"envelope\";\n"
				+ "if envelope :matches \" TO \" \"*@zimbra.com\" {\n"
				+ "    tag \"t1\";\n"
	...
	@Test
	void testInvalidHeaderName() throws ServiceException {
		Account account = createAccount().create();
		String filterScript = "require  \"envelope\";\n"
	...
	@Test
	void testInvalidHeaderName2() throws ServiceException {
		String filterScript = "require  \"envelope\";\n"
		...
		LmtpEnvelope env = new LmtpEnvelope();
		LmtpAddress sender = new LmtpAddress("<t1@zimbra.com>", new String[]{"BODY", "SIZE"}, null);
		LmtpAddress recipient = new LmtpAddress(account.getName(), null, null);
		env.setSender(sender);
		env.addLocalRecipient(recipient);
	...
	@Test
	void testAllDomainLocalIs() throws ServiceException {
		final String username = UUID.randomUUID().toString();
		Account account = createAccount().withUsername(username).create();
		final String mail = account.getName();
		final String domain = account.getDomainName();
		String filterScript = "require  [\"envelope\", \"tag\"];\n"
				+ "if envelope :domain :is \"to\" \"" + domain + "\" {\n"
				+ "    tag \"is-domain\";\n"
				+ "}\n"
				+ "if envelope :localpart :is \"to\" \"" + username + "\" {\n"
				+ "    tag \"is-local\";\n"
				+ "}\n"
				+ "if envelope :all :is \"to\" \"" + mail + "\" {"
				+ "    tag \"is-all\";\n"
				+ "}";
		LmtpEnvelope env = new LmtpEnvelope();
		LmtpAddress sender = new LmtpAddress("<t1@" + domain + ">", new String[]{"BODY", "SIZE"}, null);
		LmtpAddress recipient = new LmtpAddress("<" + mail + ">", null, null);
		env.setSender(sender);
		env.addLocalRecipient(recipient);
