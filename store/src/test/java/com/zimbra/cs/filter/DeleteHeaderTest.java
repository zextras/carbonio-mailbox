...
public class DeleteHeaderTest extends MailboxTestSuite {
	...
	void testDeleteLastHeader() {
		try {
			...
			assertEquals(indexMatch, 2);
			assertEquals("test2", headerValue);
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	}
	...
	@SuppressWarnings("unchecked")
	@Test
	void testDeleteHeaderUsingMatchesWildcardNonASCII() {
		try {
			String filterScript = "require [\"editheader\"];\n"
	...
	void testDeleteHeaderWithHeaderNameStartingWithSpace() {
		try {
			...
			assertEquals(indexMatch, 3);
			assertEquals("test3", headerValue);
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	}
	...
	void testDeleteHeaderImmutableHeaders() throws ServiceException {
		...
		try {
			...
			Integer itemId = mbox1.getItemIds(null, Mailbox.ID_FOLDER_INBOX).getIds(MailItem.Type.MESSAGE)
					.get(0);
			Message message = mbox1.getMessageById(null, itemId);
			assertNotNull(message.getMimeMessage().getHeader("Content-Type"));
			assertNotNull(message.getMimeMessage().getHeader("Content-Disposition"));
			assertNotNull(message.getMimeMessage().getHeader("Content-Transfer-Encoding"));
			assertNotNull(message.getMimeMessage().getHeader("MIME-Version"));
			assertNotNull(message.getMimeMessage().getHeader("Auto-Submitted"));
			String[] tags = message.getTags();
			assertEquals(2, tags.length);
			assertEquals("tag-example1", tags[0]);
			assertEquals("tag-example2", tags[1]);
		} catch (Exception e) {
			fail("No exception should be thrown: " + e.getMessage());
		}
	}
	...
	void testMalencodedHeader() throws Exception {
		...
		try {
			...
			RuleManager.clearCachedRules(acct1);
			acct1.setSieveEditHeaderEnabled(true);
			acct1.setAdminSieveScriptBefore(script);
			...
			List<ItemId> ids = RuleManager.applyRulesToIncomingMessage(new OperationContext(mbox),
					...
					0,
					acct1.getName(), new DeliveryContext(), Mailbox.ID_FOLDER_INBOX, true);
			Message message = mbox.getMessageById(null, ids.get(0).getId());
			assertNull(message.getMimeMessage().getHeader("X-Mal-Encoded-Header"));
		} catch (Exception e) {
	...
	void testMissingComparatorNumericDeclaration() throws Exception {
		...
		try {
			...
			List<ItemId> ids = RuleManager.applyRulesToIncomingMessage(new OperationContext(mbox),
					...
					acct1.getName(), new DeliveryContext(), Mailbox.ID_FOLDER_INBOX, true);
			Message message = mbox.getMessageById(null, ids.get(0).getId());
			assertNotNull(message.getMimeMessage().getHeader("X-Header"));
		} catch (Exception e) {
			fail("No exception should be thrown" + e);
		}
	}
