...
public class PurgeTest {
  ...
  void folderPurgePolicy() throws Exception {
    ...
    Message older =
        TestUtil.addMessage(
            ...
            folder.getId(),
            "test1",
            System.currentTimeMillis() - (60 * Constants.MILLIS_PER_MINUTE));
    Message newer =
        TestUtil.addMessage(
            mbox,
            folder.getId(),
            "test2",
            System.currentTimeMillis() - (30 * Constants.MILLIS_PER_MINUTE));
  ...
  void tagPurgePolicy() throws Exception {
    ...
    Message notTagged =
        TestUtil.addMessage(
            ...
            inbox.getId(),
            "test3",
            System.currentTimeMillis() - (90 * Constants.MILLIS_PER_MINUTE));
  ...
  @Test
  void purgeInbox() throws Exception {
  ...
  @Test
  void purgeSent() throws Exception {
    // Set retention policy
    Account account = getAccount();
  ...
  /** Confirms that a shorter user trash lifetime setting overrides the system setting. */
  @Test
  void testTrashUser() throws Exception {
    Account account = getAccount();
  ...
  /** Confirms that a shorter system trash lifetime setting overrides the user setting. */
  @Test
  void testTrashSystem() throws Exception {
    Account account = getAccount();
    account.setMailPurgeUseChangeDateForTrash(false);
  ...
  @Test
  void purgeBatchSize() throws Exception {
    Account account = getAccount();
    account.setMailPurgeUseChangeDateForTrash(false);
    account.setPrefTrashLifetime("24h");
  ...
  /** Confirms that a shorter user Junk lifetime setting overrides the system setting. */
  @Test
  void testJunkUser() throws Exception {
    Account account = getAccount();
  ...
  /** Confirms that a shorter system Junk lifetime setting overrides the user setting. */
  @Test
  void testJunkSystem() throws Exception {
    Account account = getAccount();
    account.setMailPurgeUseChangeDateForSpam(false);
  ...
  @Test
  void shouldPurgeMessagesOlderThanLifetime() throws Exception {
    // Set retention policy
    Account account = getAccount();
    int messageDaysLifetime = 40;
    account.setMailMessageLifetime(messageDaysLifetime + "d");
    ...
    Message toPurge =
        TestUtil.addMessage(
            mbox,
            folder.getId(),
            prefix + "should be purged",
            System.currentTimeMillis() - ((messageDaysLifetime + 1) * Constants.MILLIS_PER_DAY));
    ...
    // Run purge and verify results
    mbox.purgeMessages(null);
    assertFalse(messageExists(toPurge.getId()));
    assertTrue(messageExists(toKeep.getId()));
  }

  /** Tests the safeguard (default - 31 days) for the mailbox-wide message retention policy. */
  @Test
  void shouldNeverPurgeMessagesYoungerThan31Days() throws Exception {
    // Set retention policy
    Account account = getAccount();
   ...
   */
  @Test
  void testSpamChangeDate() throws Exception {
    Account account = getAccount();
    account.setPrefJunkLifetime("24h");
    ...
    assertTrue(kept.getDate() < cutoff, "Unexpected message date: " + kept.getDate());
    assertTrue(kept.getChangeDate() > cutoff, "Unexpected change date: " + kept.getChangeDate());
   ...
   */
  @Test
  void testTrashChangeDate() throws Exception {
    Account account = getAccount();
    account.setPrefTrashLifetime("24h");
  ...
  @Test
  void testFolderInTrash() throws Exception {
    // Create a subfolder of trash with a message in it.
    Mailbox mbox = getMailbox();
    String folderPath = "/Trash/testFolderInTrash";
    Folder f =
        mbox.createFolder(
            null, folderPath, new Folder.FolderOptions().setDefaultView(MailItem.Type.MESSAGE));
    String subject = "testFolderInTrash";
    Message msg =
        TestUtil.addMessage(
            mbox, f.getId(), subject, System.currentTimeMillis() - Constants.MILLIS_PER_DAY);
  ...
  void purgeWithSystemPolicy() throws Exception {
    ...
    Config config = Provisioning.getInstance().getConfig();
    Policy system =
        RetentionPolicyManager.getInstance().createSystemPurgePolicy(config, "system", "45m");
    ...
    RetentionPolicyManager.getInstance()
        .modifySystemPolicy(config, system.getId(), system.getName(), "20m");
    mbox.purgeMessages(null);
    folder = mbox.getFolderById(folder.getId());
   ...
   */
  @Test
  void testExpiredIMAPDeletedOnChangeDate() throws Exception {
    Account account = getAccount();
    account.setMailTrashLifetime("24h");
    ...
    Message kept =
        TestUtil.addMessage(
            ...
            Mailbox.ID_FOLDER_INBOX,
            subject,
            System.currentTimeMillis() - Constants.MILLIS_PER_MONTH);
   ...
   */
  @Test
  void testNotExpiredIMAPDeletedOnChangeDate() throws Exception {
    Account account = getAccount();
    account.setMailTrashLifetime("24h");
