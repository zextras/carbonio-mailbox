...
public class MetadataTest extends MailboxTestSuite {
  ...
  void legacyCalendarItem() throws ServiceException, MessagingException {
    ...
    MimeMessage message =
        new JavaMailMimeMessage(
            JMSession.getSession(), new ZSharedFileInputStream("data/TestMailRaw/invite1"));
    defaultInv.message =
        new ParsedMessage(message, Calendar.getInstance().getTimeInMillis(), false);
    ...
    meta.put(
        Metadata.FN_TZMAP,
        "foo"); // simulate existence of FN_TZMAP with bad content. In reality the metadata versions
                // 4, 5, 6 had more subtle differences in invite encoding, but this provokes the
                // exception we need
  ...
  void standardMetadataFormat() throws MailServiceException {
    String encoded =
        "d3:prt94:d1:X5:false1:fd1:a25:roland.schemers@gmail.com1:d6:Roland1:p15:Roland Schemerse2:noi2e1:vi10ee1:vi10ee";
  ...
  void legacyBlobMetadataFormat() throws MailServiceException {
    String encoded =
        "s=43:Roland Schemers <roland.schemers@gmail.com>;rt=44:forward schemers <roland.schemers@gmail.com>;f=74:http://support.microsoft.com/default.aspx?scid=kb;en-us;177378&Product=iep;";
    Metadata md = new Metadata(encoded);
    assertNotNull(md);
  }
}
