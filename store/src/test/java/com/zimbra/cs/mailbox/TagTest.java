...
import com.zimbra.cs.db.DbTagTest;
...
public class TagTest {
  ...
  public void name() throws Exception {
    ...
    // Note: ZWC currently disallows creation of tags containing double quotes but the server allows
    // them
    checkName(
        "contains spaces and double quote",
        "Andrew \"Barney\"  Rubble",
        "Andrew \"Barney\"  Rubble");
  }
  ...
  public void rename() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "incorrect error code when creating tag",
          MailServiceException.ALREADY_EXISTS,
          e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "incorrect error code when renaming tag",
          MailServiceException.ALREADY_EXISTS,
          e.getCode());
    }
  }
  ...
  public void color() throws Exception {
    ...
    Assert.assertEquals("tag color 2", 2, tag.getColor());
    DbTagTest.debugConsistencyCheck(mbox);
  ...
  private void checkInboxCounts(
      String msg, Mailbox mbox, int count, int unread, int deleted, int deletedUnread)
      throws Exception {
    ...
    Assert.assertEquals(
        msg + " (folder deleted unread)", deletedUnread, inbox.getDeletedUnreadCount());
  ...
  private void checkTagCounts(String msg, Mailbox mbox, String tagName, int count, int unread)
      throws Exception {
    try {
  ...
  private void doubleCheckTagCounts(String msg, Mailbox mbox, String tagName, int count, int unread)
      throws Exception {
  ...
  private void checkItemTags(Mailbox mbox, int itemId, int expectedFlags, String... expectedTags)
      throws Exception {
    ...
    Assert.assertTrue(
        "tags match on item: " + TagUtil.encodeTags(item.getTags()),
        TagUtil.tagsMatch(item.getTags(), expectedTags));
  ...
  private void checkThreeTagCounts(
      String msg,
      Mailbox mbox,
      int count1,
      int unread1,
  ...
  public void itemDelete() throws Exception {
    ...
    int msgId2 =
        mbox.addMessage(null, new ParsedMessage(ThreaderTest.getSecondMessage(), false), dopt, null)
            .getId();
    ...
    mbox.emptyFolder(null, Mailbox.ID_FOLDER_INBOX, true);
    checkThreeTagCounts(
        "delete that remaining message by emptying its folder", mbox, 0, 0, 0, 0, 0, 0);
    ...
    Message msg6 =
        mbox.addMessage(
            null, new ParsedMessage(ThreaderTest.getSecondMessage(), false), dopt, null);
    checkThreeTagCounts("add the conversation reply", mbox, 2, 1, 0, 0, 2, 1);

    mbox.setTags(
        null, msg6.getId(), MailItem.Type.MESSAGE, Flag.BITMASK_UNREAD, new String[] {tag2, tag3});
  ...
  public void folder() throws Exception {
    ...
    mbox.alterTag(
        null, Mailbox.ID_FOLDER_INBOX, MailItem.Type.FOLDER, Flag.FlagInfo.SUBSCRIBED, true, null);
    mbox.alterTag(
        null, Mailbox.ID_FOLDER_INBOX, MailItem.Type.FOLDER, Flag.FlagInfo.SUBSCRIBED, false, null);
    ...
    try {
      ...
      Assert.assertEquals(
          "incorrect error code when tagging folder", MailServiceException.CANNOT_TAG, e.getCode());
    }
  }
  ...
  public void permissions() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when fetching tag by ID", ServiceException.PERM_DENIED, e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when fetching tag by name", ServiceException.PERM_DENIED, e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when fetching tag list", ServiceException.PERM_DENIED, e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when creating tag", ServiceException.PERM_DENIED, e.getCode());
    }

    // just need insert or write to implicitly create a tag
    mbox.grantAccess(
        null, Mailbox.ID_FOLDER_INBOX, acct2.getId(), ACL.GRANTEE_USER, ACL.RIGHT_INSERT, null);
    int msgid = -1;
    try {
      DeliveryOptions dopt =
          new DeliveryOptions()
    ...
    mbox.grantAccess(
        null, Mailbox.ID_FOLDER_INBOX, acct2.getId(), ACL.GRANTEE_USER, ACL.RIGHT_WRITE, null);
    try {
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when \"creating\" existing unlisted tag",
          ServiceException.PERM_DENIED,
          e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when renaming tag", ServiceException.PERM_DENIED, e.getCode());
    }
    ...
    try {
      ...
      Assert.assertEquals(
          "unexpected error when deleting tag", ServiceException.PERM_DENIED, e.getCode());
    }
  }
  ...
  public void notifications() throws Exception {
    ...
    try {
      ...
      Assert.assertFalse(
          "explicit tag create must produce notifications", ml.pms.created.isEmpty());
      Assert.assertTrue(
          "explicit tags must be notified",
          ml.pms.created.values().iterator().next() instanceof Tag);
      ...
      mbox.alterTag(null, msgId, MailItem.Type.MESSAGE, tag1, true, null);
      Assert.assertFalse(
          "explicit tag apply must produce notifications",
          ml.pms.modified == null || ml.pms.modified.isEmpty());
      boolean found = false;
      for (Change chg : ml.pms.modified.values()) {
