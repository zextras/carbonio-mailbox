...
public class TagTest extends MailboxTestSuite {
	...
	void name() {
		...
		checkName("valid tag name: only symbols", "!@#$%^&*()`~-_=+[{]}|;\"',<.>/?",
				"!@#$%^&*()`~-_=+[{]}|;\"',<.>/?");
		...
		checkName("contains spaces and double quote", "Andrew \"Barney\"  Rubble",
	...
	void rename() throws Exception {
		...
		try {
			...
			assertEquals(MailServiceException.ALREADY_EXISTS, e.getCode(),
	...
	private void checkInboxCounts(String msg, Mailbox mbox, int count, int unread, int deleted,
			int deletedUnread) throws Exception {
	...
	private void checkItemTags(Mailbox mbox, int itemId, int expectedFlags, String... expectedTags)
		...
		assertTrue(TagUtil.tagsMatch(item.getTags(), expectedTags),
	...
	void implicitCreate() throws Exception {
		...
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
				.setFlags(Flag.BITMASK_UNREAD).setTags(new String[]{tag1, tag2});
		int msgId = mbox.addMessage(null, ThreaderTest.getRootMessage(), dopt, null).getId();
		...
		mbox.setTags(null, msgId, MailItem.Type.MESSAGE, MailItem.FLAG_UNCHANGED,
				new String[]{tag1, tag3, tag4}, null);
	...
	private void checkThreeTagCounts(String msg, Mailbox mbox, int count1, int unread1, int count2,
			int unread2, int count3, int unread3) throws Exception {
	...
	void itemDelete() throws Exception {
		...
		int msgId2 = mbox.addMessage(null, new ParsedMessage(ThreaderTest.getSecondMessage(), false),
				dopt, null).getId();
		...
		int msgId4 = mbox.addMessage(null, new ParsedMessage(ThreaderTest.getSecondMessage(), false),
				dopt, null).getId();
		...
		mbox.emptyFolder(null, Mailbox.ID_FOLDER_INBOX, true);
		checkThreeTagCounts("delete that remaining message by emptying its folder", mbox, 0, 0, 0, 0, 0,
				0);
		...
		Message msg6 = mbox.addMessage(null, new ParsedMessage(ThreaderTest.getSecondMessage(), false),
				dopt, null);
		checkThreeTagCounts("add the conversation reply", mbox, 2, 1, 0, 0, 2, 1);

		mbox.setTags(null, msg6.getId(), MailItem.Type.MESSAGE, Flag.BITMASK_UNREAD,
	...
	void folder() throws Exception {
		...
		mbox.alterTag(null, Mailbox.ID_FOLDER_INBOX, MailItem.Type.FOLDER, Flag.FlagInfo.SUBSCRIBED,
				true, null);
		mbox.alterTag(null, Mailbox.ID_FOLDER_INBOX, MailItem.Type.FOLDER, Flag.FlagInfo.SUBSCRIBED,
				false, null);

		try {
			mbox.alterTag(null, Mailbox.ID_FOLDER_INBOX, MailItem.Type.FOLDER, Flag.FlagInfo.FORWARDED,
					false, null);
			fail("failed to error on invalid flag on folder");
		} catch (MailServiceException e) {
			assertEquals(MailServiceException.CANNOT_TAG, e.getCode(),
					"incorrect error code when tagging folder");
		}
	}
	...
	void alterTag() throws Exception {
		...
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
				.setFlags(Flag.BITMASK_UNREAD).setTags(new String[]{tag1});
		int msgId = mbox.addMessage(null, ThreaderTest.getRootMessage(), dopt, null).getId();
	...
	void permissions() throws Exception {
		OperationContext octxt2 = new OperationContext(account2);
		...
		try {
			...
			assertEquals(ServiceException.PERM_DENIED, e.getCode(),
					"unexpected error when fetching tag by ID");
		}
		...
		try {
			...
			assertEquals(ServiceException.PERM_DENIED, e.getCode(),
					"unexpected error when fetching tag by name");
		}
		...
		try {
			...
			assertEquals(ServiceException.PERM_DENIED, e.getCode(),
					"unexpected error when fetching tag list");
		}
		...
		mbox.grantAccess(null, Mailbox.ID_FOLDER_INBOX, account2.getId(), ACL.GRANTEE_USER,
				ACL.RIGHT_INSERT, null);
		int msgid = -1;
		try {
			DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
					.setFlags(Flag.BITMASK_UNREAD).setTags(new String[]{tag2});
	...
	void listed() throws Exception {
		...
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
				.setFlags(Flag.BITMASK_UNREAD).setTags(new String[]{tag2, tag3});
	...
	void notifications() throws Exception {
		...
		try {
			...
			assertTrue(ml.pms.created.values().iterator().next() instanceof Tag,
			...
			mbox.alterTag(null, msgId, MailItem.Type.MESSAGE, tag1, true, null);
			assertFalse(ml.pms.modified == null || ml.pms.modified.isEmpty(),
