 ...
 * @author ysasaki
 */
public final class QueryParserTest extends MailboxTestSuite {
	...
	void text() throws Exception {
		...
		assertEquals("(Q(l.content:x) || Q(l.content:y)) && Q(IN:Inbox)",
	...
	void date() throws Exception {
		String src = "date:-4d";
		assertEquals("Q(DATE:DATE," + getDate(-4) + "-" + getDate(-3) + ")",
				Query.toString(parser.parse(src)));
		...
		src = "date:\"+1d\"";
		assertEquals("Q(DATE:DATE," + getDate(1) + "-" + getDate(2) + ")",
				Query.toString(parser.parse(src)));

		src = "date:+2w";
		assertEquals("Q(DATE:DATE," + getWeek(2) + "-" + getWeek(3) + ")",
				Query.toString(parser.parse(src)));
	...
	void subject() throws Exception {
		...
		assertEquals("Q(subject:my,subject) && Q(subject:my,subject)",
				Query.toString(parser.parse(src)));
	}
	...
	void has() throws Exception {
		...
		assertEquals("Q(attachment:any) && Q(has:phone) && Q(has:url)",
				Query.toString(parser.parse(src)));
	}
	...
	void tag() throws Exception {
		...
		assertEquals("(Q(TAG:\\Unread,READ) && Q(TAG:\\Unread,UNREAD))",
				Query.toString(parser.parse(src)));
		...
		assertEquals("(Q(TAG:\\Sent,SENT) && Q(TAG:\\Sent,RECEIVED))",
				Query.toString(parser.parse(src)));
	...
	void size() throws Exception {
		...
		assertEquals(
				"(Q(SIZE:=1) && Q(SIZE:=20) && Q(SIZE:=300) && Q(SIZE:=1024) && Q(SIZE:=10240) && Q(SIZE:=102400) && Q(SIZE:=35651584))",
				Query.toString(parser.parse(src)));
	...
	void quick() throws Exception {
		...
		assertEquals("Q(l.content:all,hands,meeting[*])",
				Query.toString(parser.parse("all hands meeting")));
		assertEquals("Q(l.content:all,hands,meeting[*])",
				Query.toString(parser.parse("all hands meeting*")));
	}
