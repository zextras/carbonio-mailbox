...
public final class ZimbraQueryTest {
 ...
 void testSanitizedQuery() throws Exception {
  ...
  query = new ZimbraQuery(new OperationContext(mbox), SoapProtocol.Soap12, mbox, params);
  assertEquals("Q(ITEMID,$TEXT)Q(&&)Q(Q(UNDER:ANY_FOLDER)Q(&&)-Q(UNDER:$FOLDER)Q(&&)-Q(UNDER:$FOLDER))", query.toSanitizedtring());
 ...
 void calItemExpandRange() throws Exception {
  ...
  assertTrue(query.toQueryString().matches(
 ...
 void dumpster() throws Exception {
  ...
  ZimbraQuery query = new ZimbraQuery(new OperationContext(mbox), SoapProtocol.Soap12, mbox, params);
  assertTrue(query.toQueryString().matches("\\(\\(content:test\\) AND MDATE:\\(>\\d+\\) \\)"),
    query.toQueryString());
 }
