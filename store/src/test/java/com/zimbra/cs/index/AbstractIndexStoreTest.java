...
public abstract class AbstractIndexStoreTest extends MailboxTestSuite {
	...
	void sortedFilteredTermQuery() throws Exception {
		...
		ZimbraLog.test.debug("Result for search for 'zimbra.com', filtering by IDs 2,4 & 5\n%s",
				result.toString());
		assertEquals(2, result.getTotalHits(), "Number of hits");
		assertEquals(String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 0),
				"Match Blob ID 1");
		...
		ZimbraLog.test.debug("Result for search for 'zimbra.com' sorted reverse, filter by IDs\n%s",
				result.toString());
		assertEquals(2, result.getTotalHits(), "Number of hits");
		assertEquals(String.valueOf(con4.getId()), getBlobIdForResultDoc(srchr, result, 0),
				"Match Blob ID 1");
		assertEquals(String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 1),
				"Match Blob ID 2");
		srchr.close();
	}
	...
	void leadingWildcardQuery() throws Exception {
		...
		String match1Id = searcher.doc(result.getScoreDoc(0).getDocumentID())
	...
	void multiPhraseQuery() throws Exception {
		...
		Mailbox mbox = MailboxManager.getInstance().getMailboxByAccountId(testAcct.getId());
		createContact(mbox, "Non", "Match", "nOn.MaTchiNg@zimbra.com");
		Contact contact1 = createContact(mbox, "Paul", "AA", "aa@example.net",
				"Software Development Engineer");
		createContact(mbox, "Jane", "BB", "bb@example.net", "Software Planning Engineer");
		Contact contact2 = createContact(mbox, "Peter", "CC", "cc@example.net",
				"Software Dev Engineer");
		createContact(mbox, "Avril", "DD", "dd@example.net", "Software Architectural Engineer");
		Contact contact3 = createContact(mbox, "Leo", "EE", "ee@example.net",
				"Software Developer Engineer");
		Contact contact4 = createContact(mbox, "Wow", "DD", "dd@example.net",
				"Softly Development Engineer");
		createContact(mbox, "Given", "Surname", "GiV.SurN@zimbra.com");
		mbox.index.indexDeferredItems(); // Make sure all indexing has been done
	...
	void prefixQuery() throws Exception {
		...
		ZimbraIndexSearcher searcher = index.openSearcher();
		ZimbraTopDocs result = searcher.search(
				new PrefixQuery(new Term(LuceneFields.L_CONTACT_DATA, "ab")), 100);
	...
	private void checkNextTerm(TermFieldEnumeration fields, Term term) {
		assertTrue(fields.hasMoreElements(),
	...
	private void checkAtEnd(TermFieldEnumeration fields, String field) {
		assertFalse(fields.hasMoreElements(),
				"fields.hasMoreElements() at end of list for field:" + field);
		try {
			fields.nextElement();
	...
	private static String getBlobIdForResultDoc(ZimbraIndexSearcher searcher, ZimbraTopDocs result,
			int index)
			throws IOException {
		return searcher.doc(result.getScoreDoc(index).getDocumentID())
				.get(LuceneFields.L_MAILBOX_BLOB_ID);
	}
