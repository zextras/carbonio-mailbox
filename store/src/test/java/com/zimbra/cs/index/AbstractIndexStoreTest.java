...
public abstract class AbstractIndexStoreTest {
  ...
  void sortedFilteredTermQuery() throws Exception {
    ...
    assertEquals(2, result.getTotalHits(), "Number of hits");
    assertEquals(
        String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 0), "Match Blob ID 1");
    assertEquals(
        String.valueOf(con4.getId()), getBlobIdForResultDoc(srchr, result, 1), "Match Blob ID 2");
    ...
    assertEquals(2, result.getTotalHits(), "Number of hits");
    assertEquals(
        String.valueOf(con4.getId()), getBlobIdForResultDoc(srchr, result, 0), "Match Blob ID 1");
    assertEquals(
        String.valueOf(con2.getId()), getBlobIdForResultDoc(srchr, result, 1), "Match Blob ID 2");
    srchr.close();
  }
  ...
  void leadingWildcardQuery() throws Exception {
    ...
    createContact(mbox, "Grand", "Piano", "grant@vmware.com");
  ...
  private void checkAtEnd(TermFieldEnumeration fields, String field) {
    assertFalse(
        fields.hasMoreElements(), "fields.hasMoreElements() at end of list for field:" + field);
    try {
      fields.nextElement();
