...
public class ItemQueryTest extends MailboxTestSuite {
	...
	void testSyntax() throws ServiceException {
		...
		assertEquals(String.format("Q(ITEMID,%s:1,%s:2)", account.getId(), account.getId()),
				q.toString());
		...
		assertEquals(String.format("Q(ITEMID,%s:1--%s:10)", account.getId(), account.getId()),
				q.toString());
	}
	...
	void testOneItemQuery() throws Exception {
		...
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
				.setFlags(Flag.BITMASK_UNREAD);
		mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject"), dopt, null);
		Message msg2 = mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject2"), dopt,
				null);
	...
	void testListOfItemsQuery() throws Exception {
		...
		DeliveryOptions dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX)
				.setFlags(Flag.BITMASK_UNREAD);
		Message msg1 = mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject"), dopt,
				null);
		mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject2"), dopt, null);
		Message msg3 = mbox.addMessage(null, MailboxTestUtil.generateMessage("test subject3"), dopt,
				null);
	...
	private static SearchResponse doSearch(SearchRequest request, Account acct) throws Exception {
		Element response = new Search().handle(
