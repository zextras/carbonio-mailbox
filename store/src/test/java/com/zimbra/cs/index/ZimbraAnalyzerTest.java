 ...
 * @author ysasaki
 */
public final class ZimbraAnalyzerTest extends MailboxTestSuite {


	@Test
	void size() throws Exception {
		String src = "123 26 1000000 100000000 1,000,000,000 1,000,000,000,000,000";
		TokenStream stream = ZimbraAnalyzer.getInstance()
				.tokenStream(LuceneFields.L_SORT_SIZE, new StringReader(src));
		assertEquals(
	...
	void filename() throws Exception {
		...
		TokenStream stream = ZimbraAnalyzer.getInstance()
				.tokenStream(LuceneFields.L_FILENAME, new StringReader(src));
	...
	void phraseQuery() throws Exception {
		...
		TokenStream stream = ZimbraAnalyzer.getInstance()
				.tokenStream(LuceneFields.L_CONTENT, new StringReader(src));
