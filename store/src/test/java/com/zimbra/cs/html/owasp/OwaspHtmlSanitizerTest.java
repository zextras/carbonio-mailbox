...
public class OwaspHtmlSanitizerTest {
  ...
  private static String htmlTemplateForUrlWithInlinePNG =
      "<table><tbody><tr><td style=\"%s\"></td></tr></tbody></table>";
  /*
  ...
  void testBug97443() throws Exception {
    ...
    assertTrue(
        result.contains(
            "<body><table><tbody><tr><td><b>javascript-blocked test"
                + " </b></td></tr><tr><td>alert</td></tr></tbody></table></body>"));
    ...
    assertTrue(
        result.contains(
            "<body><table><tbody><tr><td><b>javascript-blocked"
                + " test</b></td></tr><tr><td>alert</td></tr></tbody></table></body>"));
  }
  ...
  void testBug78902() throws Exception {
    ...
    result = new OwaspHtmlSanitizer(html, true, null).sanitize();
    assertEquals(result, "<html><body>My pictures here</body></html>");

    html = "<html><head></head><body><a target=\"_blank\" href=\"Neptune.txt\"></a></body></html>";
    result = new OwaspHtmlSanitizer(html, true, null).sanitize();
    assertEquals(
        result,
        "<html><head></head><body><a href=\"Neptune.txt\" target=\"_blank\" rel=\"nofollow noopener"
            + " noreferrer\"></a></body></html>");

    html = "<html><head></head><body><a target=\"_blank\" href=\"Neptune.pptx\"></a></body></html>";
    result = new OwaspHtmlSanitizer(html, true, null).sanitize();
    assertEquals(
        result,
        "<html><head></head><body><a href=\"Neptune.pptx\" target=\"_blank\" rel=\"nofollow"
            + " noopener noreferrer\"></a></body></html>");
  ...
  /**
   * Verify that data-mce-src attributes of img tag are maintained after passing through the
   * defanger.
   *
   * @throws Exception
   */
  ...
  void testZCS7621() throws Exception {
    String html =
        "<div class=\"gmail\" style=\"display:none; width:0; overflow:hidden; float:left;"
            + " max-height:0;\" align=\"center\">";
    String result = new OwaspHtmlSanitizer(html, true, null).sanitize();
  ...
  void testZCS7784() throws Exception {
    String html =
        "<img class=\"gmail\" style=\"display:none; width:0; overflow:hidden;\""
            + " src=\"https://localhost:8443/service/home/~/?auth=co&loc=en_US&id=285&part=2.2\" >";
    String result = new OwaspHtmlSanitizer(html, true, null).sanitize();
