...
/**
 * Tired of regressions in the defang filter. Unit test based on fixes I found in bugzilla over the
 * years for different problems to make sure they still work
 *
 * @author jpowers
 */
public class DefangFilterTest {
   ...
   */
  @Test
  void testBug37098() throws Exception {
    ...
    try (InputStream htmlStream = getHtmlBody(fileName)) {
      ...
      assertTrue(result.contains("ftp://ftp.perftech.com/hidden/aaeon/cpupins.jpg"));
    }
  }
  ...
  public void defangStyleUnwantedFunc(String styleValue, String expected) throws Exception {
    ...
    assertEquals(expected, after, "StyleUnwantedFunc result");
  }
  ...
  private static String htmlTemplateForUrlWithInlinePNG = "<td style=\"%s\n\"></td>";
   ...
   */
  @Test
  void testStyleValueContainingMultiLineUrl() throws Exception {
  ...
  void testHtmlWithStyleValueContainingMultiLineUrl() throws Exception {
    defangHtmlString(
        String.format(htmlTemplateForUrlWithInlinePNG, urlWithInlinePNG),
  ...
  @Test
  void testDefangStyleUnwantedFunc() throws Exception {
    String before =
        "background: #e7e7e7; background-image:url('http://example.com/image/k-hdr.jpg');height=";
   ...
   */
  @Test
  void testBug49452() throws Exception {
    String fileName = "bug_49452.txt";
    InputStream htmlStream = getHtmlBody(fileName);

    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    // make sure the link is still there
    // There should be a bunch of data after this link, but there's a few \n that seem to break it
    // up.
    assertTrue(
        result.contains("https://www.plus1staging.net/plus1staging.net/companyAuthorization.jsp"));
  }
   ...
   */
  @Test
  void testBug11464() throws Exception {
    ...
    assertFalse(result.contains("src=\"_media/zimbra_logo.gif\""));
  }
   ...
   */
  @Test
  void testBug60769() throws Exception {
    String fileName = "bug_60769.txt";
    InputStream htmlStream = getHtmlBody(fileName);

    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);

    assertFalse(result.contains("dfsrc=\"image001.gif\""));
    assertTrue(result.contains("src=\"image001.gif\""));
  }
  ...
  /**
   * Tests to make sure we can handle inline image data embeded with a data: protocol without tying
   * up the system
   *
   * @throws Exception
   */
  @Test
  void testBug62605() throws Exception {
    ...
    assertTrue((endTime - startTime) < 2000, "Possible slowness in a regex");
    // Make sure this has been replaced
    assertTrue(result.contains("src=\"data:"));
  }
   ...
   */
  @Test
  void testBug62632() throws Exception {
    ...
    assertFalse(result.contains("dfsrc=\"data:"));
   ...
   */
  @Test
  void testBug63150() throws Exception {
    ...
    assertTrue(result.contains("BillingInfoDisplayCmd?bi_URL"));
  }
   ...
   */
  @Test
  void testBug62346() throws Exception {
    ...
    // Check to make sure the link needed is still in there.
    assertTrue(result.contains("https://secure.sslpost.com/static/images/open_document.png"));
  }
  ...
  void testBug64974() throws Exception {
    String html =
        "<html><body><![CDATA[--><a"
            + " href=\"data:text/html;base64,PHNjcmlwdD4KYWxlcnQoZG9jdW1lbnQuY29va2llKQo8L3NjcmlwdD4=\">click</a]]></body></html>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
   ...
   */
  @Test
  void testBug67021() throws Exception {
    String html =
        "<html><body>"
    ...
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertFalse(result.contains("XSS2"));
    assertFalse(result.contains("XSS3"));
    assertFalse(result.contains("XSS4"));
    assertFalse(result.contains("XSS5"));
  }
  ...
  void testBug67537() throws Exception {
    String html =
        "<html><body><span style=\"color: rgb(255, 0, 0);\">This is RED</span></body></html>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
  ...
  @Test
  void testBug74715() throws Exception {
  ...
  @Test
  void testBug78997() throws Exception {
    String fileName = "bug_78997.txt";
    InputStream htmlStream = getHtmlBody(fileName);
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    // and make sure we still have the @media link and the device width attribute..
    assertTrue(result.contains("and (max-device-width: 480px)"));

    // Some more tests with different media attributes
    String html =
        "<td style= \"@media only  @media (max-width: 480px) {.body{font-size: 0.938em;} }\"/> ";
    ...
    assertTrue(result.contains("(max-width: 480px)"));

    html =
        "<td style= \" @media only screen and (-webkit-min-device-pixel-ratio: 2) {body {font-size:"
            + " 0.938em;}  }\"/>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("and (-webkit-min-device-pixel-ratio: 2)"));

    html = "<td style= \"@media (min-width: 1200px) {.two{margin-top:8em;} }\"/>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(min-width: 1200px)"));

    html = "<td style= \"@media (max-height: 600px) {.two{margin-top:4em;} }\"/>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(max-height: 600px) {.two{margin-top:4em;} }"));

    html = "<td style= \" @media (min-height: 1020px) { .two{margin-top:9em;} }\">";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(min-height: 1020px) { .two{margin-top:9em;} }"));

    html = "@media (min-height: 750px) and (max-height: 770px) {" + ".two{margin-top:7em;} }";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(min-height: 750px) and (max-height: 770px)"));
  }
  ...
  void testBug73874() throws Exception {
    ...
    String html =
        "<html><head><base href=\"https://wiki.tomsawyer.com\"/>"
            + "</head><body>"
    ...
    html =
        "<html><head><base href=\"https://wiki.tomsawyer.com/\" /></head><body><img  width=\"100\" "
            + " src=\"download/thumbnails/27132023/Screen+Shot.pngTest.gif\"/></body></html>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertFalse(
        result.contains(
  ...
  void testBug97443() throws Exception {
    String html =
        "<html><head></head><body><table><tr><td><B>javascript-blocked test </B></td>"
            + "</tr><tr><td><a href=\"javascript:alert('Hello!');\">alert</a>"
            + "</td></tr></table></body></html>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("href=\"JAVASCRIPT-BLOCKED:alert(&#39;Hello!&#39;);\""));

    html =
        "<html><head><base href=\"http://lbpe.wikispaces.com/\" /></head><body>"
  ...
  void testBug78902() throws Exception {
    ...
    String html =
        "<html><head></head><body><a target=\"_blank\" href=\"Neptune.gif\"></a></body></html>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("<a target=\"_blank\" href=\"Neptune.gif\"></a>"));

    html =
        "<html><body>My pictures <a"
            + " href=\"javascript:document.write('%3C%61%20%68%72%65%66%3D%22%6A%61%76%"
    ...
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("JAVASCRIPT-BLOCKED"));

    html = "<html><head></head><body><a target=\"_blank\" href=\"Neptune.txt\"></a></body></html>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("<a target=\"_blank\" href=\"Neptune.txt\"></a>"));

    html = "<html><head></head><body><a target=\"_blank\" href=\"Neptune.pptx\"></a></body></html>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("<a target=\"_blank\" href=\"Neptune.pptx\"></a>"));

    html =
        "<li><a"
            + " href=\"poc.zip?view=html&archseq=0\">\"/><script>alert(1);</script>AAAAAAAAAA</a></li>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
  ...
  void testBug73037() throws Exception {
    ...
    String html =
        "<html><head></head><body><a target=\"_blank\""
            + " href=\"smb://Aurora._smb._tcp.local/untitled/folder/03 DANDIYA"
            + " MIX.mp3\"></a></body></html>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
  ...
  @Test
  void testBug81641() throws Exception {

    String html =
        "<td style=\"background: #e7e7e7; background-image:"
            + "url(\'http://www.househuntnews.com/marketing/images/keller-header.jpg');"
            + "height=\"97\" width=\"598\">";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertFalse(
        result.contains("url('http://www.househuntnews.com/marketing/images/keller-header.jpg')"));
    ...
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains(" (max-width: 699px) and (min-width: 520px)"));

    html = "<td style= \"@media (orientation:portrait)\" />";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(orientation:portrait)"));

    html =
        "<td style= \"@media (min-width: 700px), handheld and (orientation: landscape) { ... }\"/>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("(min-width: 700px), handheld and (orientation: landscape)"));

    html = "<td style= \"@media not screen and (color), print and (color)\"/>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("not screen and (color), print and (color)"));
  ...
  @Test
  void testBug82181() throws Exception {

    String html =
        "<html><head><style> sf { display: block;}@media print {.xsfnoprint{ display : none ;}}  /*"
            + " Default css layout information   for SAP Smart Forms (XSF Output)   Last modified:"
            + " 12.05.2003 */ @media screen {  body {    background-color : #EFEFEF ;  }}@media"
            + " screen {  .page {    border-style : outset ;    border-width : 2pt ;   "
            + " background-color : white ;  }}/*@media print {  .page {    overflow: hidden; "
            + " }}*//* unification browser-dependent  settings */table {    border-spacing: 0pt;   "
            + " empty-cells: show;}tr { vertical-align: top; }td { padding: 0pt; }input {    font:"
            + " inherit;    padding: 0pt;    margin: 0pt;}img {    display: block;}img.icon {   "
            + " display: inline;}/* End of default.css */.ZNOW-USER-REGISTER-STYLE  div#P1.par{   "
            + " font-family : \"Times New Roman\" ;    font-size : 12pt ;    font-weight :20normal"
            + " ;    line-height : 4.23mm ;    text-decoration : none ;    text-align : left ;   "
            + " clear : both ;}.ZNOW-USER-REGISTER-STYLE  div#P2.par{    font-family : \"Times New"
            + " Roman\" ;   font-size : 12pt ;    font-weight : bold ;    line-height : 4.23mm ;   "
            + " text-decoration : none ;    text-align : left ;    clear : both"
            + " ;}.ZNOW-USER-REGISTER-STYLE  a{    color : #000000 ;}.ZNOW-USER-REGISTER-STYLE "
            + " span#C1.char{    font-family : \"Times New Roman\" ;    font-size : 12pt"
            + " ;}.ZNOW-USER-REGISTER-STYLE  span#C2.char{    font-family : \"Times New Roman\" ;  "
            + "  font-size : 12pt ;    font-weight : bold ;}#sf--PAGE1-001.page{    position :"
            + " absolute ;    height : 210mm ;    width : 297mm ;    top : 0pt ;}@media screen"
            + " {#MAIN.win{    overflow : auto ;}}@media print {#MAIN.win{    overflow : hidden"
            + " ;}}#sf--PAGE1-001.page  #MAIN.win{    position : absolute ;    left : 1.15cm ;   "
            + " top : 1.03cm ;    width : 21.90cm ;    height : 12.15cm ;}</style></head><html>";

    String htmlWithMultiLineComment =
        "<head>\n"
            + " <style> sf {\n"
            + "    display: block;\n"
            + "}\n"
            ...
            + "  }\n"
            + "     \n"
            + "/* Default css layout information   for SAP Smart Forms (XSF Output)   Last"
            + " modified: 12.05.2003 \n"
            ...
            + "   }\n"
            + "   \n"
            + "       /*@media print {  .page {    overflow: hidden;  }}*//* unification"
    ...
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("background-color : #EFEFEF"));

    htmlStream = new ByteArrayInputStream(htmlWithMultiLineComment.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("background-color: #EFEFEF"));
    assertFalse(result.contains("Default css layout information   for SAP Smart Forms"));
  }

  @Test
  void testBug82303() throws Exception {
    String html =
        "<a href=\"http://ebobby.org/2013/05/18/"
    ...
    html =
        "<a href=\"javascript-and-function-tracing.html\" "
            + "style=\"color: #187AAB; text-decoration: none\" target=\"_blank\">";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    ...
    html =
        "<a href=\"javascriptlessDestination.html\" onclick=\"myJSFunc(); "
    ...
    html =
        "<a href=\"http://ebobby.org/2013/05/18/"
            + "javascript/Lessonsinjavascript.html\" "
            + "style=\"color: #187AAB; text-decoration: none\" target=\"_blank\">";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    ...
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertEquals(result, "<a href=\"#\">LINK</a>");
    ...
    html =
        "<a href=\"#\" onclick=\"findString(document.getElementById('t1').value); return"
            + " false;\">Click Me</a>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("<a href=\"#\">Click Me</a>"));
  ...
  void testBug83999() throws IOException {
    ...
    String html =
        "<FORM NAME=\"buy\" ENCTYPE=\"text/plain\""
            + " action=\"http://mail.zimbra.com:7070/service/soap/ModifyFilterRulesRequest\""
            + " METHOD=\"POST\">";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    ...
    html =
        "<FORM NAME=\"buy\" ENCTYPE=\"text/plain\""
            + " action=\"http://zimbra.vmware.com:7070/service/soap/ModifyFilterRulesRequest\""
            + " METHOD=\"POST\">";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    ...
    html =
        "<FORM NAME=\"buy\" ENCTYPE=\"text/plain\""
            + " action=\"http://mail.zimbra.com/service/soap/ModifyFilterRulesRequest\""
            + " METHOD=\"POST\">";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("SAMEHOSTFORMPOST-BLOCKED"));
  ...
  @Test
  void testBug84337() throws Exception {
    String html =
        "<style type=\"text/css=\">@import \"https://emailprivacytester."
            + "com/cb/55fa19d5db052ced/\";</style>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertFalse(result.contains("@import 'https://emailprivacytester"));
    ...
    html =
        "<style type=\"text/css\">  @import url(\"import3.css\");  p { color : #f00; }"
            + "</style>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("p { color : #f00; }"));

    html = "<style type=\"text/css\">  @import 'import3.css';  p { color : #f00; }" + "</style>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("p { color : #f00; }"));
    assertFalse(result.contains("import3.css"));

    html =
        "<style type=\"text/css\">  @import \" import3.css \";  p { color : #f00; }" + "</style>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("p { color : #f00; }"));
    assertFalse(result.contains("import3.css"));
    ...
    html =
        "<style type=\"text/css\">  @import \" import3.css \"   ;  p { color : #f00; }"
            + "</style>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("p { color : #f00; }"));
  ...
  void testBug85478() throws Exception {
    String html =
        "<a href=\"data:text/html;base64,PHNjcmlwdD5hbGVydCgiSGVsbG8hIik7PC9zY3JpcHQ+\" "
            + "data-mce-href=\"data:text/html;base64,PHNjcmlwdD5hbGVydCgiSGVsbG8hIik7PC9zY3JpcHQ+\">Bug</a>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("DATAURI-BLOCKED"));

    html = "<a href=\"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAErkJggg==\" />Bug</a>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(
        result.contains("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAErkJggg&#61;&#61;"));

    html =
        "<a target=_blank"
            + " href=\"data:text/html,<script>alert(opener.document.body.innerHTML)</script>\">"
    ...
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("data:image/jpeg;base64,/9j/4AAAAAxITGlubwIQAABtbnRyUkdCI"));
  ...
  void testSvg() throws Exception {
    String svgXml =
        "<?xml version='1.0' encoding='UTF-8'?><svg id=\"xss\" xmlns=\"http://www.w3.org/2000/svg\""
            ...
            + "hsb2NhdGlvbikiIC8+DQogICAgPC9mb3JlaWduT2JqZWN0Pg0KPC9zdmc+#rectangle\"/></svg>";
    ...
    assertFalse(result.contains("<use"));
    ...
    svgXml =
        "<svg id=\"xss\" xmlns=\"http://www.w3.org/2000/svg\""
            ...
            + " xmlns=\"http://www.w3.org/1999/xhtml\" src=\"javascript:alert(location)\" />"
            + "</foreignObject></svg>";
    ...
    svgXml =
        "<svg xmlns=\"http://www.w3.org/2000/svg\">"
            ...
            + "<circle r=\"400\"></circle>"
            + "<animate attributeName=\"xlink:href\" begin=\"0\" to=\"&amp;amp;\"/>"
            + "</a></svg>";
    svgStream = new ByteArrayInputStream(svgXml.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_IMAGE_SVG).defang(svgStream, true);
    assertFalse(result.contains("<animate"));
  ...
  @Test
  void testBug88360() throws Exception {
    String fileName = "bug_88360.txt";
    InputStream inputStream = new FileInputStream(EMAIL_BASE_DIR + fileName);
    try {
      String result =
          DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(inputStream, true);
      assertFalse(StringUtil.isAsciiString(result));
    } catch (Exception e) {
      fail("Should not throw exception." + e.getMessage());
    }

    String html =
        "<style type=\"text/css\">  @import 'import3.css'; p { color : #f00; }" + "灻扵楬</style>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertFalse(StringUtil.isAsciiString(result));
    assertTrue(result.contains("p { color : #f00; }"));
  ...
  void testBug98215() throws Exception {
    String html =
        "<form id=\"test\" action=\"javascript:alert(1)\"><p>test</p>"
    ...
    html =
        "<form id=\"test\" action=\"ja&amp;Tab;vascript:alert(1)\"><p>test</p>"
            + "<button form=\"test\">Test</button></form>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("JAVASCRIPT-BLOCKED"));
   ...
   */
  @Test
  void testBug100966() throws Exception {
    String inputString =
        new String(ByteUtil.getContent(new File(EMAIL_BASE_DIR + "bug_100966.txt")));
    String expectedResult =
        new String(ByteUtil.getContent(new File(EMAIL_BASE_DIR + "bug_100966_op.txt")));
  ...
  void testBug102637() throws Exception {
    String html =
        "<html><body><div style=\"font-family: arial, helvetica, sans-serif; font-size: 12pt;"
            + " color: #000000\"><div><br></div><div><a"
            + " href=\"&amp;#106&amp;#097&amp;#118&amp;#097&amp;#115&amp;#099&amp;#114&amp;#105&amp;"
            + "#112&amp;#116&amp;#058&amp;#097&amp;#108&amp;#101&amp;#114&amp;#116&amp;#040&amp;#039&amp;#088&amp;#083&amp;#083&amp;#039&amp;#041\""
            + " data-mce-href=\"&amp;#106&amp;#097&amp;#118&amp;#097&amp;#115&amp;#099&amp;"
            + "#114&amp;#105&amp;#112&amp;#116&amp;#058&amp;#097&amp;#108&amp;#101&amp;#114&amp;#116&amp;#040&amp;#039&amp;#088&amp;#083&amp;#083&amp;#039&amp;#041\">test</a><br"
            + " data-mce-bogus=\"1\"></div><div><br data-mce-bogus=\"1\"></div><div>Test message<br"
  ...
  void testBug102910() throws Exception {
    String html =
        "<div><img pnsrc=\"cid:1040f05975d4d4b8fcf8747be3eb9ae3c08e5cd4@zimbra\" "
  ...
  /**
   * Verify that a new line in a html based signature is maintained after passing through the
   * defanger.
   *
   * @throws Exception
   */
  ...
  void testBug104666() throws Exception {
    String html =
        "<div></div><div></div><div id=\"5589f382-9e9b-47cd-ab09-3ea973fd4f6a\""
            + " data-marker=\"__SIG_PRE__\"><div>LIne 1</div></div><div>Line 2</div></div>";
    InputStream htmlStream = new ByteArrayInputStream(html.getBytes());
    String result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("<div>LIne 1</div></div><div>Line 2</div>"));

    html =
        "<div>Thanks</div><div><img"
            + " src=\"/home/ews01@zdev-vm002.eng.zimbra.com/Briefcase/rupali.jpeg\""
            + " dfsrc=\"doc:Briefcase/rupali.jpeg\" "
            + "data-mce-src=\"/home/ews01@zdev-vm002.eng.zimbra.com/Briefcase/rupali.jpeg\"></div>";
    htmlStream = new ByteArrayInputStream(html.getBytes());
    result = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML).defang(htmlStream, true);
    assertTrue(result.contains("dfsrc=\"doc:Briefcase/rupali.jpeg\""));
  ...
  void testBug105001() throws Exception {
    String html =
        "<html><body><div> <a href=\"javascript\n: alert('XSS')\">XSS LINK</a>"
  ...
  /**
   * Verify that dfsrc and data-mce-src attributes of img tag are maintained after passing through
   * the defanger.
   *
   * @throws Exception
   */
  ...
  @Test
  void testemoveAnySpacesAndEncodedChars() {
    String html =
        "j\n"
            + " av\n"
            + " ascript\n"
   ...
   */
  @Test
  void testzbug184() throws Exception {
  ...
  @Test
  void testZCS5696() throws Exception {
   ...
   */
  @Test
  void testzbug736Mime1() throws Exception {
    LC.zimbra_use_owasp_html_sanitizer.setDefault(false);
   ...
   */
  @Test
  void testzcs6871OwaspDefanger() throws Exception {
    BrowserDefang defanger2 = DefangFactory.getDefanger(MimeConstants.CT_TEXT_HTML);
    assertFalse(defanger2 instanceof OwaspDefang);
    ...
    assertTrue(defanger instanceof OwaspDefang);
  }
}
