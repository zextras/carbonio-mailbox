...
class CalendarUtilsTest extends MailboxTestSuite {
  ...
  void parseInviteRaw_shouldParseInvite() throws Exception {
    ...
    final String endDate = "20170515T110000Z";
    final String startDate = "20170515T100000Z";
    content.setText(
        "BEGIN:VCALENDAR\n"
            + "BEGIN:VEVENT\n"
    ...
    final InviteParserResult inviteParserResult =
        CalendarUtils.parseInviteRaw(account, inviteElement);
    ...
    Assertions.assertEquals(startDate , inviteParserResult.mInvite.getStartTime().toString());
    Assertions.assertEquals(endDate , inviteParserResult.mInvite.getEndTime().toString());
