...
public class FeedManagerTest {
	...
	void socialcastAtomFeed() throws Exception {
		...
		BufferedInputStream content = new BufferedInputStream(
				getClass().getResourceAsStream("socialcastAtomFeed.xml"));
		RemoteDataInfo rdi = new RemoteDataInfo(HttpStatus.OK_200, 0, content, expectedCharset,
				lastModified);
		...
		for (Object obj : subs) {
			if (obj instanceof ParsedMessage) {
				...
				String msgContent = streamToString(parts.get(0).getMimePart().getInputStream(),
						Charsets.UTF_8);
				assertTrue(msgContent.indexOf("Congratulations for passing!") > 0,
						"Text from inside <div>");
	...
	void atomEnabledOrg() throws Exception {
		...
		String expectedCharset = MimeConstants.P_CHARSET_UTF8;
		BufferedInputStream content = new BufferedInputStream(
				getClass().getResourceAsStream("atomEnabledOrg.xml"));
		RemoteDataInfo rdi = new RemoteDataInfo(HttpStatus.OK_200, 0, content, expectedCharset,
				lastModified);
		SubscriptionData<?> subsData = FeedManager.retrieveRemoteDatasource(null, rdi, null);
		List<?> subs = subsData.getItems();
	...
	void testIsInRangeTestAddressesIPv4() throws Exception {
		for (int i = 0; i < 256; i++) {
			assertTrue(FeedManager.isAddressInRange(InetAddress.getByName("198.51.100." + i),
					"198.51.100.0/24"));
		}
		assertFalse(
	...
	void testIsBlockedFeedAddressPublicBlacklistedMultiple() throws Exception {
		...
		assertTrue(FeedManager.isBlockedFeedAddress(
