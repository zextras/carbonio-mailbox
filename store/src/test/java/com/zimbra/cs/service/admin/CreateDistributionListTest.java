...
public class CreateDistributionListTest extends MailboxTestSuite {
  ...
  void shouldThrowServiceExceptionWhenDelegatedAdminRequestToCreateDynamicDistributionList()
    ...
    final Account domainAdminAccount =
        provisioningSpy.createAccount(DOMAIN_ADMIN_EMAIL, DOMAIN_ADMIN_PASSWORD, attrs);
    ...
    final ZimbraSoapContext zsc =
        new ZimbraSoapContext(
            authTokenMock, domainAdminAccount.getId(), SoapProtocol.Soap12, SoapProtocol.Soap12);
    ...
    final CreateDistributionListRequest createDistributionListRequest =
        new CreateDistributionListRequest(DISTRIBUTION_LIST_NAME, attributes, true);

    // create CreateDistributionList
    final CreateDistributionList createDistributions = new CreateDistributionList();

    // stub createGroup method of provisioning since it is not supported without real provisioning
    // backend
    ...
    assertThrows(
        ...
        () ->
            createDistributions.handle(
                JaxbUtil.jaxbToElement(createDistributionListRequest), context),
