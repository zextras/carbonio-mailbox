...
public class TagActionTest extends MailboxTestSuite {
	...
	void invalid() throws Exception {
		...
		action.addAttribute(MailConstants.A_OPERATION, ItemAction.OP_MOVE)
				.addAttribute(MailConstants.A_FOLDER, Mailbox.ID_FOLDER_USER_ROOT);
		...
		try {
			...
			assertEquals(ServiceException.INVALID_REQUEST, e.getCode(),
					"expected error code: " + ServiceException.INVALID_REQUEST);
		}

		action.addAttribute(MailConstants.A_OPERATION, ItemAction.OP_RENAME)
				.addAttribute(MailConstants.A_NAME, "tag3");
		action.addAttribute(MailConstants.A_TAG_NAMES, TagUtil.encodeTags(name2));
		try {
			new TagAction().handle(request, ServiceTestUtil.getRequestContext(acct));
			fail("allowed op on nonexistent tag");
		} catch (ServiceException e) {
			assertEquals(MailServiceException.NO_SUCH_TAG, e.getCode(),
					"expected error code: " + MailServiceException.NO_SUCH_TAG);
		}
	}
	...
	void permissions() throws Exception {
		...
		try {
			...
			assertEquals(ServiceException.PERM_DENIED, e.getCode(),
					"expected error code: " + ServiceException.PERM_DENIED);
		}

		action.addAttribute(MailConstants.A_TAG_NAMES, (String) null)
				.addAttribute(MailConstants.A_ID, tag1.getId());
		try {
			new TagAction().handle(request, ServiceTestUtil.getRequestContext(acct2, acct));
			fail("colored another user's tags without permissions");
		} catch (ServiceException e) {
	...
	void delete() throws Exception {
		...
		request.addUniqueElement(MailConstants.E_TAG).addAttribute(MailConstants.A_COLOR, 4)
				.addAttribute(MailConstants.A_NAME, "test");
		...
		request.addUniqueElement(MailConstants.E_ACTION)
				.addAttribute(MailConstants.A_OPERATION, ItemAction.OP_HARD_DELETE)
				.addAttribute(MailConstants.A_ID, tagId);
		new TagAction().handle(request, ServiceTestUtil.getRequestContext(acct));
