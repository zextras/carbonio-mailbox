...
public class SyncTest extends MailboxTestSuite {
  ...
  public static void init() throws Exception {
    ...
    config.setDefaultDomainName( "zimbra.com");
    prov.createDomain("zimbra.com", new HashMap<>());
    prov.createAccount("test@zimbra.com", "secret", new HashMap<>());
    prov.createAccount("test1@zimbra.com", "secret", new HashMap<>());
    prov.createAccount("test2@zimbra.com", "secret", new HashMap<>());
  }

  static boolean verifyItemsPresentInResponse(
      Element response, int folderId, List<Integer> expectedIds, Type type)
      throws ServiceException {
    Pair<Integer, Element> pairFolderIdElement = new Pair<>(folderId, null);
    getFolderElement(response, pairFolderIdElement);
    assertNotNull(pairFolderIdElement.getSecond());
    ...
    assertEquals(expectedIds.size(), actualMsgIds.size());
    for (Integer id : expectedIds) {
      assertTrue(actualMsgIds.contains("" + id));
    }
    return true;
  }

  static boolean getFolderElement(Element response, Pair<Integer, Element> pairFolderIdElement)
      throws ServiceException {
  ...
  void tags() throws Exception {
    ...
    ParsedContact pc =
        new ParsedContact(
            ImmutableMap.of(
                ContactConstants.A_firstName, "Bob", ContactConstants.A_lastName, "Smith"));
    ...
    for (Element hidden :
        response.getElement(MailConstants.E_FOLDER).listElements(MailConstants.E_FOLDER)) {
      if (hidden.getAttribute(MailConstants.A_NAME).equals("Tags")) {
        tagFolder = hidden;
        break;
      }
    }
    assertNotNull(tagFolder, "could not find Tags folder in initial sync response");
    assertEquals(1, tagFolder.listElements(MailConstants.E_TAG).size(), "1 tag listed");
    Element t = tagFolder.listElements(MailConstants.E_TAG).get(0);
    assertEquals("foo", t.getAttribute(MailConstants.A_NAME), "listed tag named 'foo'");
    assertEquals(
        4,
        t.getAttributeLong(MailConstants.A_COLOR, MailItem.DEFAULT_COLOR),
        "correct color for tag 'foo'");

    // change tag color and re-sync
    mbox.setColor(null, tagId, MailItem.Type.TAG, (byte) 6);
    //        mbox.purge(MailItem.Type.TAG);
    request =
        new Element.XMLElement(MailConstants.SYNC_REQUEST)
            .addAttribute(MailConstants.A_TOKEN, token);

    response = new Sync().handle(request, context);
    assertNotEquals(
        token, response.getAttribute(MailConstants.A_TOKEN), "sync token change after tag color");
    ...
    assertNotNull(t, "modified tag missing");
    assertEquals(
        6,
        t.getAttributeLong(MailConstants.A_COLOR, MailItem.DEFAULT_COLOR),
        "new color on serialized tag");
  }

  @Test
  void msgConversationsCutoff() throws Exception {
    Account acct = createAccount().create();
    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(acct);
    ...
    Message msg60minbefore =
        TestUtil.addMessage(mbox, Mailbox.ID_FOLDER_INBOX, "test1 x", before60min * 1000);
    Message msg45minbefore =
        TestUtil.addMessage(mbox, Mailbox.ID_FOLDER_INBOX, "Re: test1 x", before45min * 1000);
    Message msg30minbefore =
        TestUtil.addMessage(mbox, Mailbox.ID_FOLDER_INBOX, "test2 x", before30min * 1000);
    Message msg15minbefore =
        TestUtil.addMessage(mbox, Mailbox.ID_FOLDER_INBOX, "Re: test2 x", before15min * 1000);
    int convId1 = msg60minbefore.getConversationId();
    int convId2 = msg30minbefore.getConversationId();
    List<Integer> expectedConvIds = new ArrayList<>();
    List<Integer> expectedMsgIds = new ArrayList<>();
    ...
    Pair<Integer, Element> pairFolderIdElement = new Pair<>(Mailbox.ID_FOLDER_CONVERSATIONS, null);
    getFolderElement(response, pairFolderIdElement);
    assertNotNull(pairFolderIdElement.getSecond());
    assertNull(pairFolderIdElement.getSecond().getOptionalElement(MailConstants.E_CONV));
    pairFolderIdElement = new Pair<>(Mailbox.ID_FOLDER_INBOX, null);
    getFolderElement(response, pairFolderIdElement);
    assertNotNull(pairFolderIdElement.getSecond());
    assertNull(pairFolderIdElement.getSecond().getOptionalElement(MailConstants.E_MSG));
    ...
    assertTrue(
        verifyItemsPresentInResponse(
            response, Mailbox.ID_FOLDER_CONVERSATIONS, expectedConvIds, Type.CONVERSATION));
    ...
    assertTrue(
        verifyItemsPresentInResponse(
            response, Mailbox.ID_FOLDER_INBOX, expectedMsgIds, Type.MESSAGE));
    ...
    // Delete message before 15mins no messages no conversation should be returned if msgCutOff is
    // current time - 16 mins.
    mbox.delete(null, msg15minbefore.getId(), Type.MESSAGE);
    request = new SyncRequest();
    request.setMsgCutoff(currTime - 16 * Constants.SECONDS_PER_MINUTE);
    request.setFolderId("" + Mailbox.ID_FOLDER_ROOT);
    response = new Sync().handle(JaxbUtil.jaxbToElement(request), context);
    pairFolderIdElement = new Pair<>(Mailbox.ID_FOLDER_CONVERSATIONS, null);
    getFolderElement(response, pairFolderIdElement);
    assertNotNull(pairFolderIdElement.getSecond());
    assertNull(pairFolderIdElement.getSecond().getOptionalElement(MailConstants.E_CONV));
    pairFolderIdElement = new Pair<>(Mailbox.ID_FOLDER_INBOX, null);
  ...
  void conversations() throws Exception {
    ...
    DeliveryOptions dopt =
        new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX).setConversationId(-msgId);
    Message msg =
    ...
    for (Element subFolder : subFolders) {
      if (subFolder.getAttributeInt(MailConstants.A_ID, 0) == Mailbox.ID_FOLDER_CONVERSATIONS) {
        ...
        assertEquals(1, convIds.length);
        assertEquals(convId + "", convIds[0]);
        break;
      }
    }

    assertTrue(found, "found expected converation");
    ...
    msgId =
        mbox.addMessage(
                null,
                MailboxTestUtil.generateMessage("test subject 2"),
                MailboxTest.STANDARD_DELIVERY_OPTIONS,
                null)
            .getId();
    dopt = new DeliveryOptions().setFolderId(Mailbox.ID_FOLDER_INBOX).setConversationId(-msgId);
    convId =
        mbox.addMessage(null, MailboxTestUtil.generateMessage("Re: test subject 2"), dopt, null)
            .getConversationId();
    ...
    assertEquals(convId, conv.getAttributeInt(MailConstants.A_ID));
    ...
    for (String delete : deletedIds) {
      assertTrue(expectedDeletes.remove(Integer.valueOf(delete)), "id " + delete + " deleted");
    }

    assertTrue(expectedDeletes.isEmpty(), "all expected ids deleted");
  }
   ...
   */
  @Test
  void detaSyncTest() throws Exception {
    Account acct = createAccount().create();
    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(acct);
    mbox.beginTrackingSync();
    SyncRequest request = new SyncRequest();
    ...
    assertEquals(2, syncRes.getItems().size());
    assertEquals(2, deletes.length);
    ...
    assertEquals(1, syncRes.getItems().size());
    ...
    mbox.addMessage(
        null,
        MailboxTestUtil.generateMessage("test3 x"),
        MailboxTest.STANDARD_DELIVERY_OPTIONS,
        null);
    ...
    assertEquals(3, syncRes.getItems().size());
   ...
   */
  @Test
  void detaSyncMovedMessageTest() throws Exception {
    Account acct = createAccount().create();
    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(acct);
    mbox.beginTrackingSync();
    SyncRequest request = new SyncRequest();
  ...
  /**
   * Test Paginations. 1. Modified paged , delete paged (del cutoff modseq < modified cutoff mod
   * Seq) 2. All modified , delete paged. 3. Modified paged , delete paged (del cutoff modseq >
   * modified cutoff mod Seq) 4. Modified paged , all deletes. 5. Modified and delete unpaged.
   *
   * @throws Exception
   */
  @Test
  void testPaginatedSync() throws Exception {
    Account acct = createAccount().create();
    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(acct);
    mbox.beginTrackingSync();
    SyncRequest request = new SyncRequest();
    ...
    Element response = new Sync().handle(JaxbUtil.jaxbToElement(request), context);
    SyncResponse syncRes = JaxbUtil.elementToJaxb(response);
    Set<Integer> itemsDeleted = new HashSet<>();
    Set<Integer> itemsAddedOrModified = new HashSet<>();
    ...
    assertTrue(itemsAddedOrModified.isEmpty());
    token = syncRes.getToken();
    ...
    removeItemsFromList(itemsAddedOrModified, listObj);
    assertEquals(0, itemsAddedOrModified.size()); // All synced.
    assertTrue(syncRes.getMore());
    ...
    // Test3: previous interrupted sync. modified and delete paged.(del cutoff modseq > modified
    // cutoff mod Seq)
    int msgId7 =
        mbox.addMessage(
                null,
    ...
    int msgId9 =
        mbox.addMessage(
            ...
            .getId();
    mbox.alterTag(
        null, new int[] {msgId9}, MailItem.Type.MESSAGE, Flag.FlagInfo.UNREAD, true, null);
    ...
    assertEquals(2, deletes.length);
    removeDeleteFromList(itemsDeleted, deletes);
    assertEquals(0, itemsDeleted.size()); // All synced.
    listObj = syncRes.getItems();
    assertEquals(2, listObj.size());
    removeItemsFromList(itemsAddedOrModified, listObj);
    assertEquals(1, itemsAddedOrModified.size()); // pending to sync.
    ...
    lastChange = syncToken.getChangeId();
    lastDel = syncToken.getDeleteModSeq();
    assertEquals(lastDel, mbox.getLastChangeID());
    assertTrue(syncRes.getMore());
    ...
    int msgId13 =
        mbox.addMessage(
            ...
            .getId();
    mbox.alterTag(
        null, new int[] {msgId11}, MailItem.Type.MESSAGE, Flag.FlagInfo.UNREAD, true, null);
    ...
    lastChange = syncToken.getChangeId();
    lastDel = syncToken.getDeleteModSeq();
    assertEquals(lastChange, mbox.getLastChangeID());
    assertEquals(lastDel, -1);
    assertNull(syncRes.getMore());
  }
}
