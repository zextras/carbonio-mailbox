...
public class SyncTest {
 ...
 void tags() throws Exception {
  ...
  assertEquals(4, t.getAttributeLong(MailConstants.A_COLOR, MailItem.DEFAULT_COLOR), "correct color for tag 'foo'");
...
//        mbox.purge(MailItem.Type.TAG);
  ...
  assertNotEquals(token, response.getAttribute(MailConstants.A_TOKEN), "sync token change after tag color");
  ...
  assertEquals(6, t.getAttributeLong(MailConstants.A_COLOR, MailItem.DEFAULT_COLOR), "new color on serialized tag");
 }
 ...
 void msgConversationsCutoff() throws Exception {
  ...
  assertTrue(verifyItemsPresentInResponse(response, Mailbox.ID_FOLDER_CONVERSATIONS, expectedConvIds, Type.CONVERSATION));
  ...
  assertTrue(verifyItemsPresentInResponse(response, Mailbox.ID_FOLDER_INBOX, expectedMsgIds, Type.MESSAGE));
