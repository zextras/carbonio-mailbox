...
class GetAccountInfoTest extends MailboxTestSuite {
  ...
  void shouldContainAccountStatusInReturnedAttributes() throws Exception {
    ...
    Domain domain = prov.createDomain(UUID.randomUUID() + ".com", Maps.newHashMap());
    Map<String, Object> attributes = new HashMap<>();
    attributes.put(Provisioning.A_zimbraIsAdminAccount, "TRUE");
    Account adminAccount =
        prov.createAccount(
            UUID.randomUUID() + "@" + domain.getName(), UUID.randomUUID().toString(), attributes);
    ...
    List<Attr> attrs =
        Objects.requireNonNull(response).getAttrList().stream()
