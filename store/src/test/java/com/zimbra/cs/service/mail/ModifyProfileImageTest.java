...
public class ModifyProfileImageTest {
  ...
  @Test
  void testZCS3545() throws Exception {
    ...
    Upload up = FileUploadServlet.saveUpload(is, "img.jpg", "image/jpeg", acct.getId());
    ...
    Element response =
        new ModifyProfileImage().handle(request, ServiceTestUtil.getRequestContext(acct));
    ...
    Element getResponse = new GetInfo().handle(getRequest, ServiceTestUtil.getRequestContext(acct));
    GetInfoResponse getResponseJaxb = JaxbUtil.elementToJaxb(getResponse);
    assertEquals(profileItemId, getResponseJaxb.getProfileImageId());
    Map<String, Object> attrMap = acct.getAttrs(false, false);
    String ldapThumbnailImageData =
    ...
    assertEquals(ldapThumbnailImageData, attrMap.get(ZAttrProvisioning.A_thumbnailPhoto));
  }

  @Test
  void testZCS3871() throws Exception {
    ...
    Element getResponse1 =
        new GetInfo().handle(getRequest1, ServiceTestUtil.getRequestContext(acct));
    // check that the profile image information is not present in GetInfo as well as LDAP
    assertEquals(false, getResponse1.prettyPrint().contains("profileImageId"));
    Map<String, Object> attrMap1 = acct.getAttrs(false, false);
    assertNull(attrMap1.get(ZAttrProvisioning.A_thumbnailPhoto));
  }
