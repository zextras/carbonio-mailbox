...
public class ItemActionTest extends MailboxTestSuite {
  ...
  void inherit() throws Exception {
    ...
    Account acct2 = createAccount().create();

    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(acct);

    mbox.grantAccess(
        null, Mailbox.ID_FOLDER_INBOX, acct2.getId(), ACL.GRANTEE_USER, ACL.RIGHT_READ, null);
    ...
    request
        .addElement(MailConstants.E_ACTION)
        .addAttribute(MailConstants.A_OPERATION, ItemAction.OP_INHERIT)
        .addAttribute(MailConstants.A_ID, targets);
  ...
  void moveConversationToAcctRelativePath() throws Exception {
    ...
    ItemActionHelper.MOVE(
        null, mbox, SoapProtocol.Soap12, Arrays.asList(msgId * -1), null, targetFolderName);
  ...
  void copyMessageFromDraftsToSent() throws Exception {
    ...
    assertEquals(
        ...
        copiedMessage.getFolderId(),
        "test parent folder of copied message");
  }
  ...
  void deleteIncompleteConversation() throws Exception {
    ...
    MailItem.TargetConstraint tcon =
        new MailItem.TargetConstraint(mbox, MailItem.TargetConstraint.INCLUDE_TRASH);
    ...
    ItemActionHelper.MOVE(
        ...
        mbox,
        SoapProtocol.Soap12,
        Collections.singletonList(draft.getId()),
        MailItem.Type.MESSAGE,
        tcon,
        iid);
    ...
    ItemActionHelper.HARD_DELETE(
        ...
        mbox,
        SoapProtocol.Soap12,
        Collections.singletonList(draft.getConversationId()),
  ...
  void deleteConversation() throws Exception {
    ...
    ItemActionHelper.MOVE(
        ...
        mbox,
        SoapProtocol.Soap12,
        Arrays.asList(parent.getId(), draft.getId(), draft2.getId()),
        MailItem.Type.MESSAGE,
        tcon,
        iid);
    ...
    ItemActionHelper.HARD_DELETE(
        ...
        mbox,
        SoapProtocol.Soap12,
        Collections.singletonList(parent.getConversationId()),
        MailItem.Type.CONVERSATION,
        tcon);
    Exception ex = null;
    try {
  ...
  void moveConversationPreviousFolderTest() throws Exception {
    ...
    request
        .addElement(MailConstants.E_ACTION)
        .addAttribute(MailConstants.A_OPERATION, ItemAction.OP_MOVE)
        .addAttribute(MailConstants.A_ID, msg1.getConversationId())
        .addAttribute("l", iid3.getId());
  ...
  void mute() throws Exception {
    ...
    Element action =
        request
            .addElement(MailConstants.E_ACTION)
            .addAttribute(MailConstants.A_OPERATION, ItemAction.OP_MUTE)
            .addAttribute(MailConstants.A_ID, msg.getConversationId());
    new ConvAction().handle(request, ServiceTestUtil.getRequestContext(acct));
    ...
    action
        .addAttribute(MailConstants.A_OPERATION, ItemAction.OP_MUTE)
        .addAttribute(MailConstants.A_ID, msg2.getConversationId());
    new ConvAction().handle(request, ServiceTestUtil.getRequestContext(acct));
  ...
  void deleteAllTagKeepsStatusOfFlags() throws Exception {
    ...
    assertEquals(Flag.BITMASK_UNREAD, item.getFlagBitmask(), "Verifying Unread flag is set.");
