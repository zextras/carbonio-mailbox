...
public class JWTBasedAuthTest extends MailboxTestSuite {
	...
	@Test
	void testGenerateJWTusingProvider() throws Exception {
		AuthToken at = AuthProvider.getAuthToken(acct, TokenType.JWT);
		validateJWT(at, acct.getId());
	}

	@Test
	void testAccountAndUsageJWT() throws ServiceException {
		AuthToken at = AuthProvider.getAuthToken(acct, Usage.AUTH, TokenType.JWT);
		validateJWT(at, acct.getId());
	}

	@Test
	void testAccountAndExpiresJWT() throws AuthProviderException {
	...
	private String generateJWT(Account acct, String salt)
			throws AuthFailedServiceException, AuthTokenException {
		...
		AuthTokenProperties properties = new AuthTokenProperties(acct, false, null, expires, null,
				Usage.AUTH);
		return JWTUtil.generateJWT(jwtKey, salt, issuedAt, properties, atkey.getVersion());
	}
	...
	void testGetJWToken() {
		try {
			...
			long issuedAt = System.currentTimeMillis();
			long expires = issuedAt + 3600000;
			AuthTokenProperties properties = new AuthTokenProperties(acct, true, null, expires,
	...
	void testUserServletJWTFlowWithAuthHeader()
			throws ServiceException, UserServletException, AuthTokenException, ServletException, IOException {
