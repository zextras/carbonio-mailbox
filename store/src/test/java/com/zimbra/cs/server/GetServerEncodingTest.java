...
public class GetServerEncodingTest extends MailboxTestSuite {
	...
	void test4012() throws Exception {
		...
		final Server server = Provisioning.getInstance().createServer("myServer", attrs);
		GetServer.encodeServer(response, server, true, null, null);
		assertEquals(true,
				response.prettyPrint().contains("<a n=\"zimbraSSLPrivateKey\">VALUE-BLOCKED</a>"));
	}
}