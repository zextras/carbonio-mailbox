...
public class TestGroups extends TestCase {
    ...
    public void run() {
      ...
      try {
        for (int cnt = 0; cnt < 20; cnt++) {
          ...
          ZimbraLog.test.info(
              "XXX getGroupMembership ms=%s acct=%s NUM=%s",
          ...
          ZimbraLog.test.info(
              "XXX getGroupMembershipWithRights ms=%s acct=%s NUM=%s",
              ZimbraLog.elapsedTime(before, System.currentTimeMillis()),
              acct.getName(),
              membership.groupIds().size());
        }
  ...
  public void ENABLE_FOR_PERFORMANCE_TESTStestCustomDynamicGroups() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroups %s", ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }

  private GroupMembership doGetGroupMembershipWithRights(
      Account acct, Set<Right> rights, int expected, int adminOnlyExpected)
    ...
    ZimbraLog.test.info(
        "YYY getGroupMembershipWithRights %s ms=%s acct=%s size=%s\n%s\ngroupNames=%s",
        rightsDesc,
        after - before,
        acct.getName(),
        membership.groupIds().size(),
    ...
    ZimbraLog.test.info(
        "YYY getGroupMembershipWithRights [adminOnly] %s ms=%s acct=%s size=%s\n%s\ngroupNames=%s",
        rightsDesc,
        after - before,
        acct.getName(),
        adminOnlyMembership.groupIds().size(),
        adminOnlyMembership,
        adminOnlyGroupNames);

    assertEquals(
        String.format(
            "Number of groups with %s which contain %s groups=%s",
            rightsDesc, acct.getName(), groupNames),
        expected,
        membership.groupIds().size());
    assertEquals(
        String.format(
            "Number of adminOnly groups with %s which contain %s groups=%s",
  ...
  private void doRightsTestForAccount(String acctName, int expected, int adminOnlyExpected)
      throws ServiceException {
    RightManager rightMgr = RightManager.getInstance();
    Set<Right> rights = Sets.newHashSet();
    rights.add(rightMgr.getUserRight(RightConsts.RT_createDistList));
  ...
  public void testCustomDynamicGroupsAnyRights1() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsAnyRights1 %s",
  ...
  public void testCustomDynamicGroupsCreateDistListRights1() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsCreateDistListRights1 %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  public void testCustomDynamicGroupsAnyRights2() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsAnyRights2 %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  public void testCustomDynamicGroupsCreateDistListRights2() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsCreateDistListRights2 %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  public void testCustomDynamicGroupsAnyRights3() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsCreateDistListRights3 %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  public void testCustomDynamicGroupsAnyRights4() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testCustomDynamicGroupsCreateDistListRights4 %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  private void doGetCustomDynamicGroupMembership(int acctNum) throws ServiceException {
    ...
    assertEquals(
        String.format(
            "Number of dynamic groups with custom memberURL s which contain %s groups=%s",
            acct.getName(), groupNames),
        1,
        membership.groupIds().size());
    ...
    assertEquals(
        String.format("Name of dynamic group with custom memberURL s which contains %s", acctName),
        cosName,
        groupName);
  }
  ...
  public void testGetCustomDynamicGroups() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testGetCustomDynamicGroups %s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }
  ...
  public void testDLupdateGroupMembershipWithoutViaWithAliases() throws Exception {
    ...
    DistributionList.updateGroupMembership(
        ...
        false /* directOnly */);
    ZimbraLog.test.info(
        "testDLupdateGroupMembershipWithoutVia %s size=%d",
    ...
    for (int cnt = 1; cnt <= NUM_NORMAL_DL; cnt++) {
      ...
      Assert.assertTrue(
          String.format("DL %s (id=%s) in membership", nam, dl.getId()),
          membership.groupIds().contains(dl.getId()));
    }
    DistributionList dl = ldapProv.get(DistributionListBy.name, dlWithAlias);
    Assert.assertTrue(
        String.format("DL %s (id=%s) in membership", dl.getId(), dl.getId()),
        membership.groupIds().contains(dl.getId()));
    ...
    Assert.assertTrue(
        ...
        membership.groupIds().contains(dl.getId()));
    Assert.assertEquals(
        String.format("Number of DLs User %s is a member of", acctWithAlias),
        NUM_NORMAL_DL + 2,
        membership.memberOf().size());
    ...
    Assert.assertTrue(
        String.format("DL %s (id=%s) in membership", dl.getName(), dl.getId()),
        membership.groupIds().contains(dl.getId()));
    dl = ldapProv.get(DistributionListBy.name, dlWithAlias2);
    Assert.assertTrue(
        String.format("DL %s (id=%s) in membership", dl.getName(), dl.getId()),
        membership.groupIds().contains(dl.getId()));
    Assert.assertEquals(
        String.format("Number of DLs User %s is a member of", acctWithAlias2),
        2,
        membership.memberOf().size());
  }
  ...
  public void testDLupdateGroupMembershipWithViaWithAliases() throws Exception {
    ...
    DistributionList.updateGroupMembership(
        ...
        false /* directOnly */);
    ZimbraLog.test.info(
        "testDLupdateGroupMembershipWithVia %s size=%d via size=%d via=%s",
        ZimbraLog.elapsedTime(start, System.currentTimeMillis()),
        membership.groupIds().size(),
        via.size(),
        via);
  ...
  public void testInACLGRoup() throws Exception {
    ...
    ZimbraLog.test.info(
        "ZZZ testInACLGRoup %s", ZimbraLog.elapsedTime(start, System.currentTimeMillis()));
  }

  private void doInACLGroup(String acctName, String groupName, boolean expected)
      throws ServiceException {
    Account acct = ldapProv.getAccountByName(acctName);
    Group group = ldapProv.getGroup(DistributionListBy.name, groupName);
    assertEquals(
        String.format("account %s in group %s", acct.getName(), group.getName()),
        expected,
        ldapProv.inACLGroup(acct, group.getId()));
  }
  ...
  public void setUp() throws Exception {
    ...
    for (int cnt = 1; cnt <= NUM_CUSTOM_DL; cnt++) {
      ...
      Group grp =
          createCustomDynamicGroupWhoseMembersShareCOS(
              String.format(customDLPatt, cnt), String.format(cosPatt, cnt));
    ...
    ldapProv.grantRight(
        ...
        null /* rightModifier */);
    ZimbraLog.test.info(
        "SETUP Granted %s to Group %s for dom=%s",
        RightConsts.RT_createDistList, groupName, domain.getName());
    // account4 should match this
    ...
    ZimbraLog.test.info(
        "SETUP Granted %s to Group %s for dom=%s",
        RightConsts.RT_sendToDistList, groupName, domain.getName());
  ...
  public static Group createCustomDynamicGroupWhoseMembersShareCOS(String name, String cosName)
    ...
    if (group != null) {
      ZimbraLog.test.warn(
          "createCustomDynamicGroupWhoseMembersShareCOS(%s) - already existed!!!", name);
      return group;
    }
    Map<String, Object> attrs = Maps.newHashMap();
    Cos cos = prov.get(Key.CosBy.name, cosName);
    attrs.put(
        ZAttrProvisioning.A_memberURL,
        String.format(
