...
public class TestFilterRFCCompliantNotify extends TestCase {
  ...
  public void testNotifyAction() throws Exception {
    ...
    actions.add(
        new ZFilterAction.ZRFCCompliantNotifyAction(
    ...
    List<ZMessage> msgs =
        TestUtil.waitForMessages(zMailbox, "in:inbox subject:\"" + subject + "\"", 1, 120000);
  ...
  private ZFilterRules getTestIncomingRules() throws Exception {
    ...
    actions.add(
        new ZRFCCompliantNotifyAction(
            USER_NAME,
            "",
            "",
            "Notify You've got a mail!",
            "mailto:"
                + REMOTE_USER_NAME
                + "?body=This is the notification email alert&Importance=High&X-Priority=1"));
