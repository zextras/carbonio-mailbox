...
public class TestAccess extends TestCase {
  ...
  private static class SoapUser extends SoapProvisioning {
    ...
    protected Element invokeOnTargetAccount(Element request, String targetId)
  ...
  private static class SoapAdminUser extends SoapUser {
    ...
    DataSource setup_createDataSource(
        TestAccess ta, Role role, Perm perm, String acctName, String dataSourceName)
        throws Exception {
      ...
      try {
        ...
        attrs.put(
            Provisioning.A_zimbraDataSourceFolderId, ta.createFolderAndReturnFolderId(role, perm));
        ...
        dataSource =
            createDataSource(
                get(Key.AccountBy.name, acctName), DataSourceType.pop3, dataSourceName, attrs);
  ...
  Element accessTest(Role role, Perm perm, XMLElement req) throws Exception {
    ...
    try {
      ...
      if (!expectedCode.equals(resultCode)) e.printStackTrace();
    }
  ...
  private String createFolderAndReturnFolderId(Role role, Perm perm) throws Exception {
    ...
    if (response == null) {
      ...
      return "1000";
    } else return response.getElement(MailConstants.E_FOLDER).getAttribute(MailConstants.A_ID);
  }
  ...
  public void DeleteDataSource(Role role, Perm perm) throws Exception {
    ...
    DataSource ds =
        mProvAdmin.setup_createDataSource(this, role, perm, ACCT_1_EMAIL, dateSourceName);
  ...
  public void DeleteIdentity(Role role, Perm perm) throws Exception {
    ...
    mProvAdmin.createIdentity(
        mProvAdmin.get(Key.AccountBy.id, ACCT_1_ID), identityName, new HashMap<String, Object>());
  ...
  public void DeleteSignature(Role role, Perm perm) throws Exception {
    ...
    Signature signature =
        mProvAdmin.createSignature(
            mProvAdmin.get(Key.AccountBy.id, ACCT_1_ID),
            signatureName,
  ...
  public void GetAccountInfo(Role role, Perm perm) throws Exception {
    ...
    if (role == Role.R_USER || role == Role.R_USER_TARGET_SELF) a.setText(ACCT_1_EMAIL);
    else a.setText(ACCT_2_EMAIL);
  ...
  public void testAccess() throws Exception {
    for (Role role : Role.values()) {
      ...
      mSoapUser1
          .auth(); // urg, need to re-auth after changing password, because we now invalidate auth
