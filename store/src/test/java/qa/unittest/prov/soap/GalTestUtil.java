...
public class GalTestUtil {
  ...
  public static void enableGalSyncAccount(
      Provisioning prov, String domainName, String galSyncAcctLocalpart) throws Exception {
  ...
  public static void enableGalSyncAccount(
      Provisioning prov, String domainName, String galSyncAcctLocalpart, GSAType type)
      throws Exception {
    Domain domain = prov.get(Key.DomainBy.name, domainName);
    String[] galSyncAcctIds = domain.getGalAccountId();
    if (galSyncAcctIds.length > 0) {
      ...
      String localpart =
          galSyncAcctLocalpart == null ? GAL_SYNC_ACCOUNT_NAME : galSyncAcctLocalpart;
      GalTestUtil.createAndSyncGalSyncAccount(
          TestUtil.getAddress(localpart, domainName), domainName, type);
    }
  }
  ...
  static void createAndSyncGalSyncAccount(String galSyncAcctName, String domainName, GSAType type)
    ...
    String gsaZimbraId =
        GalTestUtil.createGalSyncAccountOrDataSource(
            transport, galSyncAcctName, domainName, dataSourceName, dataSourceType, folderName);
    ...
    //
    Element eReIndex =
        Element.create(transport.getRequestProtocol(), AdminConstants.REINDEX_REQUEST);
  ...
  static String createGalSyncAccountOrDataSource(
      SoapTransport transport,
      String galSyncAcctName,
      String domainName,
      String dataSourceName,
      String dataSourceType,
      String folderName)
    ...
    Element eCreateReq =
        Element.create(
            transport.getRequestProtocol(), AdminConstants.CREATE_GAL_SYNC_ACCOUNT_REQUEST);
  ...
  static void syncGASDataSource(SoapTransport transport, String gsaZimbraId, String dataSourceName)
      throws Exception {
    Element eSyncReq =
        Element.create(transport.getRequestProtocol(), AdminConstants.SYNC_GAL_ACCOUNT_REQUEST);
