...
import qa.unittest.prov.BinaryLdapData;
...
public class TestSearchGal extends SoapTest {
  ...
  */
  ...
  private void searchWithDot(boolean ldap, String domainName, int expectedNumEntries)
    ...
    Element request =
        Element.create(transport.getRequestProtocol(), AccountConstants.SEARCH_GAL_REQUEST);
    request.addElement(AccountConstants.E_NAME).setText(".");

    Element response = transport.invoke(request);

    boolean paginationSupported =
        response.getAttributeBool(AccountConstants.A_PAGINATION_SUPPORTED);
  ...
  private void searchWithOffsetLimit(boolean ldap, String domainName) throws Exception {
    ...
    if (ldap) {
      ...
      for (int i = 0; i < limit; i++) {
        assertEquals(
            getAcctEmail(offset + i, domainName),
            result.get(i).getSingleAttr(ContactConstants.A_email));
      }
    }
  }
  ...
  private void searchByFilter(boolean ldap, String domainName) throws Exception {
    ...
    eCondResType.addAttribute(
        AccountConstants.A_ENTRY_SEARCH_FILTER_ATTR, ContactConstants.A_email);
    ...
    eCondResSite.addAttribute(
        AccountConstants.A_ENTRY_SEARCH_FILTER_ATTR, ContactConstants.A_department);
    ...
    assertEquals(
        getAcctEmail(acctToMatch, domainName),
        result.get(0).getSingleAttr(ContactConstants.A_email));
  }

  private void binaryDataInEntry(boolean ldap, String domainName, boolean wantSMIMECert)
      throws Exception {
    SoapTransport transport = authUser(TestUtil.getAddress(AUTHED_USER, domainName));
    ...
    request
        .addElement(AccountConstants.E_NAME)
        .setText(getAcctEmail(ACCOUNT_CONTAINS_BINARY_DATA, domainName));
  ...
  private static void createDomainObjects(String domainName, PredefinedSearchBase searchBase)
      throws Exception {
