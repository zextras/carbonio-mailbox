...
public class TestCheckRights extends SoapTest {
  ...
  public void basic() throws Exception {
    ...
    req.addTarget(
        new CheckRightsTargetSpec(
            TargetType.account, TargetBy.name, target1.getName(), Lists.newArrayList(right)));
    req.addTarget(
        new CheckRightsTargetSpec(
            TargetType.account, TargetBy.name, target2.getName(), Lists.newArrayList(right)));
  ...
  public void defaultPermission() throws Exception {
    ...
    for (CheckRightsTargetInfo target : targets) {
      ...
      assertEquals(
          right1.getSecond() && right2.getSecond() && right3.getSecond(), target.getAllow());
  ...
  public void delegatedSendRight() throws Exception {
    ...
    String alias1 =
        TestUtil.getAddress(genAcctNameLocalPart("target-acct-alias-1"), domain.getName());
    String alias2 =
        TestUtil.getAddress(genAcctNameLocalPart("target-acct-alias-2"), domain.getName());
    ...
    req.addTarget(
        new CheckRightsTargetSpec(
            TargetType.account, TargetBy.name, targetAcct.getName(), Lists.newArrayList(right)));
    req.addTarget(
        new CheckRightsTargetSpec(
            TargetType.account, TargetBy.name, alias1, Lists.newArrayList(right)));
    req.addTarget(
        new CheckRightsTargetSpec(
            TargetType.account, TargetBy.name, alias2, Lists.newArrayList(right)));
    CheckRightsResponse resp = invokeJaxb(transport, req);
