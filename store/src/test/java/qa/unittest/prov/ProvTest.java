...
public class ProvTest {
  ...
  public enum SkipTestReason {
    ...
    CONNECTION_POOL_HEALTH_CHECK(
        "connection pool health check does not work for InMemoryDirectoryServer"),
    DN_SUBTREE_MATCH_FILTER(
        "entryDN:dnSubtreeMatch filter is not supported by InMemoryDirectoryServer"),
    EXTERNAL_AUTH_STATUS_AUTH_FAILED(
        "external auth STATUS_AUTH_FAILED status is not supported by InMemoryDirectoryServer"),
    EXTERNAL_AUTH_STATUS_COMMUNICATION_FAILURE(
        "external auth STATUS_COMMUNICATION_FAILURE status is not supported by"
            + " InMemoryDirectoryServer"),
    EXTERNAL_AUTH_STATUS_CONNECTION_REFUSED(
        "external auth STATUS_CONNECTION_REFUSED status is not supported by"
            + " InMemoryDirectoryServer"),
    EXTERNAL_AUTH_STATUS_UNKNOWN_HOST(
        "external auth STATUS_UNKNOWN_HOST status is not supported by InMemoryDirectoryServer"),
    SUBORDINATES_OPERTIONAL_ATTRIBUTE(
        "Subordinates operational attribute is not supported by InMemoryDirectoryServer"),
  ...
  private String genName(String suffix, Sequencer seq) {
    String name = suffix == null ? getTestName() : getTestName() + "-" + suffix;
  ...
  protected String genGroupNameLocalPart(String suffix, Sequencer seq) {
    return "group-" + genName(suffix, seq);
  }

  protected String genGroupNameLocalPart(String suffix) {
    return genGroupNameLocalPart(suffix, null);
  }

  protected String genGroupNameLocalPart(Sequencer seq) {
    return genGroupNameLocalPart(null, seq);
  }
