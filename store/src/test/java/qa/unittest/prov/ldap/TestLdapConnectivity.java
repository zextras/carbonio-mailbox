...
import qa.unittest.prov.LocalconfigTestUtil;
...
public class TestLdapConnectivity {
  ...
  */
  ...
  private static enum ConnectionConfig {
    ...
    LDAPS_T_UNTRUSTED_T_MISMATCHED(
        "ldaps://localhost:636", "ldaps://localhost:636", "0", "true", "true"),
    // JNDI: OK

    LDAPS_T_UNTRUSTED_F_MISMATCHED(
        "ldaps://localhost:636", "ldaps://localhost:636", "0", "true", "false"),
    // JNDI: OK

    LDAPS_F_UNTRUSTED_T_MISMATCHED(
        "ldaps://localhost:636", "ldaps://localhost:636", "0", "false", "true"),
    // JNDI: OK

    LDAPS_F_UNTRUSTED_F_MISMATCHED(
        "ldaps://localhost:636", "ldaps://localhost:636", "0", "false", "false"),
    // JNDI: OK

    STARTTLS_T_UNTRUSTED_T_MISMATCHED(
        "ldap://localhost:389", "ldap://localhost:389", "1", "true", "true"),
    // JNDI: OK

    STARTTLS_T_UNTRUSTED_F_MISMATCHED(
        "ldap://localhost:389", "ldap://localhost:389", "1", "true", "false"),
    // JNDI: ERROR: service.FAILURE (system failure: ZimbraLdapContext) (cause:
    // javax.net.ssl.SSLPeerUnverifiedException hostname of the server 'localhost' does not match
    // the hostname in the server's certificate.)
    /*
    Caused by: java.security.cert.CertificateException: No name matching localhost found
    at sun.security.util.HostnameChecker.matchDNS(HostnameChecker.java:210)
    at sun.security.util.HostnameChecker.match(HostnameChecker.java:77)
    ...
    STARTTLS_F_UNTRUSTED_T_MISMATCHED(
        "ldap://localhost:389", "ldap://localhost:389", "1", "false", "true"),
    // JNDI: OK

    STARTTLS_F_UNTRUSTED_F_MISMATCHED(
        "ldap://localhost:389", "ldap://localhost:389", "1", "false", "false");
    // JNDI: ERROR: service.FAILURE (system failure: ZimbraLdapContext) (cause:
    // javax.net.ssl.SSLPeerUnverifiedException hostname of the server 'localhost' does not match
    // the hostname in the server's certificate.)
    ...
    void setLocalConfig() throws Exception {
      ...
      LocalconfigTestUtil.modifyLocalConfigTransient(
          LC.ldap_starttls_supported, ldap_starttls_supported);
      LocalconfigTestUtil.modifyLocalConfigTransient(
          LC.ssl_allow_untrusted_certs, ssl_allow_untrusted_certs);
      LocalconfigTestUtil.modifyLocalConfigTransient(
          LC.ssl_allow_mismatched_certs, ssl_allow_mismatched_certs);
      LC.reload();
    }
  }
  ...
  /**
   * Needs -Djava.library.path=/opt/zextras/lib (where the native lib is) when running in Eclipse A
   * simple demo server.
   *
