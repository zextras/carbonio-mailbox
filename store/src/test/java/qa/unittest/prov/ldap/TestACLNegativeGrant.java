...
import qa.unittest.prov.ldap.ACLTestUtil.AllowOrDeny;
import qa.unittest.prov.ldap.ACLTestUtil.AsAdmin;
import qa.unittest.prov.ldap.ACLTestUtil.TestViaGrant;
...
public class TestACLNegativeGrant extends LdapTest {
  ...
  protected void verify(
    ...
    result =
        accessMgr.canDo(
            grantee == null ? null : grantee.getName(), target, right, asAdmin.yes(), via);
  ...
  public void groupGranteeTest2() throws Exception {
    ...
    grantRight(
        ...
        AllowOrDeny.ALLOW);
    grantRight(
        authedAcct, TargetType.account, target, GranteeType.GT_GROUP, GG2, right, AllowOrDeny.DENY);
    grantRight(
        authedAcct,
        TargetType.account,
        target,
        ...
        AllowOrDeny.ALLOW);
    grantRight(
        authedAcct, TargetType.account, target, GranteeType.GT_GROUP, GG4, right, AllowOrDeny.DENY);
    grantRight(
        authedAcct,
        TargetType.account,
        target,
        ...
        AllowOrDeny.ALLOW);
    grantRight(
        authedAcct, TargetType.account, target, GranteeType.GT_GROUP, GG6, right, AllowOrDeny.DENY);
  ...
  */
  ...
  public void groupGranteeTest3() throws Exception {
    ...
    */
    ...
    via =
        new TestViaGrant(
            TargetType.dl, G2, GranteeType.GT_GROUP, GB.getName(), right, TestViaGrant.NEGATIVE);
    via.addCanAlsoVia(
        new TestViaGrant(
            TargetType.dl, G3, GranteeType.GT_GROUP, GA.getName(), right, TestViaGrant.NEGATIVE));
    verify(account, target, right, AsAdmin.AS_ADMIN, AllowOrDeny.DENY, via);
  }
  ...
  public void targetPrecedence() throws Exception {
    ...
    DistributionList group1 =
        provUtil.createDistributionList(genGroupNameLocalPart("group1"), domain);
    DistributionList group2 =
        provUtil.createDistributionList(genGroupNameLocalPart("group2"), domain);
    prov.addMembers(group1, new String[] {group2.getName()});
    prov.addMembers(group2, new String[] {target.getName()});

    grantRight(
        authedAcct, TargetType.dl, group2, GranteeType.GT_USER, grantee, right, AllowOrDeny.DENY);
    grantRight(
        authedAcct, TargetType.dl, group1, GranteeType.GT_USER, grantee, right, AllowOrDeny.ALLOW);
    ...
    // revoke the grant on globalgrant, then there is no grant and callsite default should be
    // honored
    revokeRight(
        authedAcct,
        TargetType.global,
