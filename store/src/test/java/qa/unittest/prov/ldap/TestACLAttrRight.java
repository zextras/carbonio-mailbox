...
import qa.unittest.prov.ldap.ACLTestUtil.GetOrSet;
...
public class TestACLAttrRight extends LdapTest {
  ...
  public static void init() throws Exception {
    ...
    Set<String> ALL_ACCOUNT_ATTRS_MINUS_SOME =
        SetUtil.subtract(ALL_ACCOUNT_ATTRS, ATTRS_SOME.keySet());
  ...
  private void someAllSameLevel(
      AllowOrDeny some, AllowOrDeny all, GetOrSet getOrSet, AllowedAttrs expected)
      throws Exception {

    String testName =
        "someAllSameLevel-" + some.name() + "-some-" + all.name() + "-all-" + getOrSet.name();
  ...
  public void someAllDiffLevel(
    ...
    String testName =
        "someAllDiffLevel-"
            ...
            + all.name()
            + "-all-"
            + (someIsCloser ? "someIsCloser" : "allIsCloser")
            + "-"
            + getOrSet.name();
