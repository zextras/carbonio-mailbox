...
public class TestLdapProvDynamicGroup extends LdapTest {
  ...
  public void memberAttrViaSlapdOverlay() throws Exception {
    ...
    Set<String> expected =
        Sets.newHashSet(
            ...
            "cn=external,"
                + ((LdapDynamicGroup) group)
                    .getDN()); // LdapProvisioning.DYNAMIC_GROUP_STATIC_UNIT_NAME
  ...
  public void addMembersBasic() throws Exception {
    ...
    prov.addGroupMembers(
        group, new String[] {acct1.getName(), acct2.getName(), extAddr1, extAddr2});
  ...
  public void addMembersInternalAddress() throws Exception {
    ...
    Group otherStaticGroup =
        provUtil.createDistributionList(genGroupNameLocalPart("other-static"), domain);
  ...
  public void externalAddrRemovedOnInternalAddrCreationCreation() throws Exception {
    ...
    members = prov.getGroupMembers(group);
    Verify.verifyEquals(
        Sets.newHashSet(externalAddr6, externalAddr7, externalAddr8, externalAddr9), members);

    // rename the group
    prov.renameGroup(newGroup.getId(), externalAddr6);
    group = prov.getGroupBasic(DistributionListBy.id, groupId);
    members = prov.getGroupMembers(group);
    Verify.verifyEquals(Sets.newHashSet(externalAddr7, externalAddr8, externalAddr9), members);
    ...
    group = prov.getGroupBasic(DistributionListBy.id, groupId);
    members = prov.getGroupMembers(group);
    Verify.verifyEquals(Sets.newHashSet(externalAddr8, externalAddr9), members);

    // add an alias to the new account
    prov.addAlias(newAcct, externalAddr8);
    group = prov.getGroupBasic(DistributionListBy.id, groupId);
    members = prov.getGroupMembers(group);
    Verify.verifyEquals(Sets.newHashSet(externalAddr9), members);

    // rename the account
    prov.renameAccount(newAcct.getId(), externalAddr9);
    group = prov.getGroupBasic(DistributionListBy.id, groupId);
    members = prov.getGroupMembers(group);
    Verify.verifyEquals(Collections.<String>emptySet(), members);
  }
  ...
  private void testGetMemberAddrs(boolean dynamic) throws Exception {
    ...
    prov.addGroupMembers(
        group, new String[] {acct1.getName(), acct2.getName(), externalAddr1, externalAddr2});
