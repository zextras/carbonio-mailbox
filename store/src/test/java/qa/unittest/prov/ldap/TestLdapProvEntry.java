...
public class TestLdapProvEntry extends LdapTest {
  private static final String DOMAIN_NAME =
      "\u4e2d\u6587" + "." + baseDomainName(); // an IDN domain name
  private static final String ACCTNAME_LOCAL_PART = "test-ldap-prov-entry";

  private static final BinaryLdapData.Content binaryData =
  ...
  public void getAttr() throws Exception {
    ...
    assertEquals(
        ZAttrProvisioning.AccountStatus.active.name(),
        entry.getAttr(Provisioning.A_zimbraAccountStatus));
    assertEquals(
        ZAttrProvisioning.MailStatus.enabled.name(),
        entry.getAttr(Provisioning.A_zimbraMailStatus));
  }
  ...
  public void getBooleanAttr() throws Exception {
    ...
    assertTrue(
        value); // becasue cos already has a value, default provided here will not be effective
  }
