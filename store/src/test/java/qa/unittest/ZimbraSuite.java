...
import qa.unittest.prov.TestChangeEphemeralStore;
import qa.unittest.prov.soap.TestGetContactsRequest;
import qa.unittest.prov.soap.TestSearchDirectory;
import qa.unittest.prov.soap.TestSoapProvisioning;
import qa.unittest.server.TestCalDavImportServer;
import qa.unittest.server.TestDataSourceServer;
import qa.unittest.server.TestDocumentServer;
import qa.unittest.server.TestNewMailNotification;
import qa.unittest.server.TestPop3ImportServer;
...
public class ZimbraSuite {
  ...
  public static TestResults runUserTests(List<String> testNames) throws ServiceException {
    ...
    for (String testNameAndMethod : testNames) {
      ...
      if (testName.indexOf('.') < 0) {
        ...
        if (!found) {
          ZimbraLog.test.warn(
              "Could not find test %s.  Make sure it's registered with %s.",
  ...
  private static TestResults runTestsInternal(
      Collection<Class> testClasses, Iterable<Request> requests) {
