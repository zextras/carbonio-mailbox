...
import qa.unittest.prov.soap.GalTestUtil;

/*
 * To run this test:
 * zmsoap -v -z RunUnitTestsRequest/test=qa.unittest.TestGalGroupMembers
 *
 */
...
public class TestGalGroupMembers extends TestCase {
  ...
  private static final String ZIMBRA_GROUP = "group";
  ...
  enum ZimbraGroupMembers {
    ...
    private String getAddress() {
      ...
      return isZimbraAccount
          ? TestUtil.getAddress(localPart, ZIMBRA_DOMAIN)
  ...
  private void doTest() throws Exception {
    ...
    Set<String> zimbraGroupMembers =
        GalGroupMembers.getGroupMembers(TestUtil.getAddress(ZIMBRA_GROUP, ZIMBRA_DOMAIN), acct);
    ZimbraGroupMembers.assertEquals(zimbraGroupMembers);

    Set<String> externalGroupMembers =
        GalGroupMembers.getGroupMembers(TestUtil.getAddress(EXTERNAL_GROUP, EXTERNAL_DOMAIN), acct);
  ...
  public void testGSASearch() throws Exception {
    GalTestUtil.enableGalSyncAccount(
        Provisioning.getInstance(), ZIMBRA_DOMAIN, GalTestUtil.GSAType.both);
    doTest();
  }
