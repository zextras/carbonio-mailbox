...
public class TestCalendarMailSender extends TestCase {
  ...
  public void testInviteAutoDeclinePrefsUnset() throws Exception {
    ...
    ZAttendee matchingAttendee =
        new ZAttendee(
            attendeeAccount.getName(),
            attendeeAccount.getCn() /* cn */,
            ...
            null);
    boolean allowed =
        CalendarMailSender.allowInviteAutoDeclinedNotification(
            mbox,
            attendeeAccount,
            senderEmail,
            senderAccount,
            true /* applyToCalendar */,
            matchingAttendee);
    assertFalse(
        String.format(
            "Should NOT be allowed to send auto-decline because default value for %s is false",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply),
        allowed);
  }

  /**
   * Tests with original invite's organizer in same domain and {@code
  ...
  public void testPrefCalendarSendInviteDeniedAutoReplyTrue() throws Exception {
    ...
    prefs.put(
        Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply, ProvisioningConstants.TRUE);
    ...
    assertTrue(
        String.format(
            "Should be allowed to send auto-decline because %s=true",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply),
        allowed);
    ...
    allowed =
        CalendarMailSender.allowInviteAutoDeclinedNotification(
            ...
            senderAccount,
            true /* applyToCalendar */,
            null /* matchingAttendee */);
    assertFalse(
        "Should NOT be allowed to send auto-decline because no matching attendee in invite",
        allowed);
    ...
    allowed =
        CalendarMailSender.allowInviteAutoDeclinedNotification(
            ...
            matchingAttendee);
    assertFalse(
        "Should NOT be allowed to send auto-decline - no account object for sender", allowed);
    ...
    allowed =
        CalendarMailSender.allowInviteAutoDeclinedNotification(
            ...
            matchingAttendee);
    assertFalse(
        "Should NOT be allowed to send auto-decline - account is a system resource", allowed);
  }
  ...
  public void testInternalPrefCalendarAllowedTargetsForInviteDeniedAutoReply() throws Exception {
    ...
    assertTrue(
        String.format(
            "Should be allowed to send auto-decline because %s=true & %s=internal",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
    senderAccount = null;
    allowed =
        CalendarMailSender.allowInviteAutoDeclinedNotification(
    ...
    assertFalse(
        String.format(
            "Should NOT be allowed to send auto-decline for non-internal because %s=true &"
                + " %s=internal",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
  ...
  public void testSameDomainPrefCalendarAllowedTargetsForInviteDeniedAutoReply() throws Exception {
    ...
    prefs.put(
        Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply, ProvisioningConstants.TRUE);
    prefs.put(
        Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply, "sameDomain");
    Provisioning.getInstance().modifyAttrs(attendeeAccount, prefs, true, null);
    ZAttendee matchingAttendee =
        new ZAttendee(
            attendeeAccount.getName(),
    ...
    assertTrue(
        String.format(
            "Should be allowed to send auto-decline because %s=true & %s=sameDomain",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
    ...
    assertFalse(
        String.format(
            "Should NOT be allowed to send auto-decline to diff domain because %s=true &"
                + " %s=sameDomain",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
    ...
    assertFalse(
        String.format(
            "Should NOT be allowed to send auto-decline to non-internal because %s=true &"
                + " %s=sameDomain",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
  ...
  public void testAllPrefCalendarAllowedTargetsForInviteDeniedAutoReply() throws Exception {
    ...
    assertTrue(
        String.format(
            "Should be allowed to send auto-decline because %s=true & %s=all",
            Provisioning.A_zimbraPrefCalendarSendInviteDeniedAutoReply,
            Provisioning.A_zimbraPrefCalendarAllowedTargetsForInviteDeniedAutoReply),
        allowed);
    Provisioning prov = Provisioning.getInstance();
