...
import qa.unittest.prov.ldap.ACLTestUtil;

public abstract class TestACL extends TestCase {

  protected static boolean CHECK_LIMIT =
      false; // todo: remove this and all limit related code after all tests pass.
  ...
  protected static final String DOMAIN_NAME =
      TestProvisioningUtil.baseDomainName("test-ACL", TEST_ID);
  ...
  String getTestName() {
    ...
    if (testName == null) return "unknownTest";
    else return getName().substring(4);
  }
  ...
  protected static String getEmailAddr(String testCaseName, String localPartSufix) {
    if (testCaseName == null) return localPartSufix + "@" + DOMAIN_NAME;
    else return testCaseName + "-" + localPartSufix + "@" + DOMAIN_NAME;
  }
  ...
  protected static enum AllowOrDeny {
    ...
    RightModifier toRightModifier() {
      if (deny()) return RightModifier.RM_DENY;
      else if (delegable()) return RightModifier.RM_CAN_DELEGATE;
      else return null;
    }
  }
  ...
  protected ZimbraACE newPubACE(Right right, AllowOrDeny allowDeny) throws ServiceException {
    return new ZimbraACE(
        GuestAccount.GUID_PUBLIC, GranteeType.GT_PUBLIC, right, allowDeny.toRightModifier(), null);
  }
  ...
  protected ZimbraACE newAllACE(Right right, AllowOrDeny allowDeny) throws ServiceException {
    return new ZimbraACE(
        GuestAccount.GUID_AUTHUSER,
        GranteeType.GT_AUTHUSER,
        right,
        allowDeny.toRightModifier(),
        null);
  }

  // construct a ACE with "usr" grantee type
  protected ZimbraACE newUsrACE(Account acct, Right right, AllowOrDeny allowDeny)
      throws ServiceException {
    return new ZimbraACE(
        acct.getId(), GranteeType.GT_USER, right, allowDeny.toRightModifier(), null);
  }

  // construct a ACE with "grp" grantee type
  protected ZimbraACE newGrpACE(DistributionList dl, Right right, AllowOrDeny allowDeny)
      throws ServiceException {
    return new ZimbraACE(
        dl.getId(), GranteeType.GT_GROUP, right, allowDeny.toRightModifier(), null);
  }

  // construct a ACE with "key" grantee type
  protected ZimbraACE newKeyACE(
      String nameOrEmail, String accessKey, Right right, AllowOrDeny allowDeny)
      throws ServiceException {
    return new ZimbraACE(
        nameOrEmail, GranteeType.GT_KEY, right, allowDeny.toRightModifier(), accessKey);
  }

  Set<ZimbraACE> makeUsrGrant(Account grantee, Right right, AllowOrDeny alloworDeny)
      throws ServiceException {
  ...
  Set<ZimbraACE> makeGrpGrant(DistributionList grantee, Right right, AllowOrDeny alloworDeny)
      throws ServiceException {
    Set<ZimbraACE> aces = new HashSet<ZimbraACE>();
  ...
  static class TestViaGrant extends ViaGrant {
    ...
    public void addCanAlsoVia(TestViaGrant canAlsoVia) {
      if (mCanAlsoVia == null) mCanAlsoVia = new HashSet<TestViaGrant>();
  ...
  protected void verify(
      Account grantee, Entry target, Right right, AllowOrDeny expected, ViaGrant expectedVia)
      throws Exception {
  ...
  protected void verifyDefinedDefault(Account grantee, Entry target, Right right) throws Exception {
    ...
    else verify(grantee, target, right, AS_USER, DENY, null);
  }
  ...
  void assertEquals(ViaGrant expected, ViaGrant actual) {
    ...
    if (!(AccessManager.getInstance() instanceof ACLAccessManager)) return;

    if (expected instanceof TodoViaGrant) return; // TODO
  ...
  protected void verify(
      ...
      Right right,
      AsAdmin asAdmin,
      AllowOrDeny expected,
      ViaGrant expectedVia)
      throws Exception {
    boolean result;
    ...
    result =
        mAM.canDo(
            grantee == null ? null : AuthProvider.getAuthToken(grantee),
            target,
            right,
            asAdmin.yes(),
            via);
    ...
    via = (expectedVia == null) ? null : new ViaGrant();
    result =
        mAM.canDo(grantee == null ? null : grantee.getName(), target, right, asAdmin.yes(), via);
  ...
  void assertEquals(Set<String> expected, Set<String> actual) {
    ...
    for (String s : expected) assertTrue(actual.contains(s));
  }
      ...
      AllowedAttrs allowedAttrs =
          getOrSet.isGet()
              ? CheckAttrRight.accessibleAttrs(
                  new Grantee(grantee), target, AdminRight.PR_GET_ATTRS, false)
              : CheckAttrRight.accessibleAttrs(
                  new Grantee(grantee), target, AdminRight.PR_SET_ATTRS, false);
  ...
  protected void verify(
      Account grantee, Entry target, Right right, Map<String, Object> attrs, AllowOrDeny expected)
      throws ServiceException {
  ...
  protected void grantRight(
    ...
    RightCommand.grantRight(
        ...
        null,
        right.getName(),
        grant.toRightModifier());
  }

  protected void grantDelegableRight(
      Account authedAcct,
      TargetType targetType,
      NamedEntry target,
      GranteeType granteeType,
  ...
  protected void revokeDelegableRight(
      Account authedAcct,
      TargetType targetType,
      NamedEntry target,
      GranteeType granteeType,
  ...
  /*
    ...
    cp /Users/pshao/p4/main/ZimbraServer/data/unittest/ldap/rights-unittest.xml /opt/zextras/conf/rights
    and
