...
public class TestCiphers extends TestCase {
  ...
  // set to true when nio is enabled on the server, to toggle between IMAP_STARTTLS_RESPONSE and
  // IMAP_STARTTLS_RESPONSE_NIO
  ...
  private static final String POP3_CLEARTEXT_FAILED_RESPONSE =
  ...
  private static final String IMAP_STARTTLS_RESPONSE =
      "2 OK Begin TLS negotiation now"; // our non-nio imap server returns Begin
  private static final String IMAP_STARTTLS_RESPONSE_NIO =
      "2 OK begin TLS negotiation now"; // our nio imap server returns begin
  private static final String IMAP_LOGOUT = "3 LOGOUT" + CRLF;
  private static final String IMAP_LOGOUT_RESPONSE1 =
  ...
  private static final String HTTP_SOAP_PING_RESPONSE_CLEARTEXT =
      "HTTP/1.1 500 Internal Server Error"; // because PingRequest is an admin command and thus is
  // only allowed on admin port
  ...
  static void init() throws Exception {
    ...
    mHttpSslPort =
        server.getIntAttr(
            ...
            7071); // use zimbraAdminPort for testing because zimbraMailSSL Port may not be enabled
  }
  ...
  public void pop3(boolean useExcludedCipher) throws Exception {
    ...
    if (useExcludedCipher) setEnabledCipherSuites(socket);
    try {
      send(socket, "", POP3_CONNECT_RESPONSE);
      send(socket, POP3_USER, POP3_USER_RESPONSE);
      send(socket, POP3_PASS, POP3_PASS_RESPONSE);
    ...
    socket =
        SocketFactories.dummySSLSocketFactory()
            .createSocket(socket, HOSTNAME, mPop3CleartextPort, true);
    if (useExcludedCipher) setEnabledCipherSuites(socket);
    try {
      send(socket, POP3_USER, POP3_USER_RESPONSE);
      send(socket, POP3_PASS, POP3_PASS_RESPONSE);
  ...
  public void imap(boolean useExcludedCipher) throws Exception {
    ...
    socket =
        SocketFactories.dummySSLSocketFactory()
            .createSocket(socket, HOSTNAME, mImapCleartextPort, true);
    if (useExcludedCipher) setEnabledCipherSuites(socket);
    try {
      send(socket, IMAP_LOGIN, IMAP_LOGIN_RESPONSE);
      send(socket, IMAP_LOGOUT, IMAP_LOGOUT_RESPONSE1);
  ...
  /**
   * Sends the given message to the socket's <code>OutputStream</code> and validates the first line
   * returned.
   *
