...
public class TestAuthentication extends TestCase {
  ...
  /**
   * Attempts to access a deleted account and confirms that the attempt fails with an auth error.
   */
  ...
  public void testAccessInactiveAccount() throws Exception {
    ...
    try {
      ...
      String msg =
          String.format("Error message '%s' does not contain '%s'", ex.getMessage(), substring);
  ...
  public void testAccountLockout() throws Exception {
    ...
    SoapHttpTransport transport = new SoapHttpTransport(TestUtil.getSoapUrl());
    AccountSelector acctSel =
        new AccountSelector(com.zimbra.soap.type.AccountBy.name, acct.getName());
    ...
    Assert.assertTrue(
        "account is not lockedout", verifyLockedoutAndReactivateAccount(acct, transport));
    ...
    // Verify lock out should not happen after 2 invalid login using same password and next login
    // with different invalid password should be locked out.
    try {
      resp = transport.invoke(JaxbUtil.jaxbToElement(req, SoapProtocol.SoapJS.getFactory()));
    } catch (ServiceException e) {
    }
    ...
    Assert.assertTrue(
        "account is not active", acct.getAccountStatus().equals(AccountStatus.active));
  ...
  static boolean verifyLockedoutAndReactivateAccount(Account acct, SoapHttpTransport transport)
      throws Exception {
