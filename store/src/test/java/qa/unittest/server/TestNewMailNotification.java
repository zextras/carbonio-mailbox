...
public class TestNewMailNotification extends TestCase {
  ...
  private static String[] ALL_TEST_USERS = {
    MESSAGE_RECIPIENT, NOTIFICATION_RECIPIENT, MESSAGE_SENDER
  };
  ...
  /**
   * Confirms that the subject and body of the out of office and new mail notification can contain
   * UTF-8 characters.
   *
   * @throws Exception
   */
  public void testUtf8() throws Exception {
    ...
    String OUT_OF_OFFICE_BODY = NAME_PREFIX + " \u0416\u0438\u0440\u0430\u0444"; // Jiraf
    ...
    attrs.put(
        Provisioning.A_zimbraPrefNewMailNotificationAddress,
        TestUtil.getAddress(NOTIFICATION_RECIPIENT));
  ...
  public void testNotificationMessageOverwrite() throws Exception {
    String notificationTemplate =
        "From: Postmaster &lt;postmaster@${RECIPIENT_DOMAIN}&gt;${NEWLINE}To:"
            ...
            + " ${RECIPIENT_ADDRESS}.${NEWLINE}Sender: ${SENDER_ADDRESS}${NEWLINE}Subject:"
            + " ${SUBJECT}";
    ...
    List<ZMessage> messages =
        TestUtil.search(
            notificationMbox,
            "in:inbox content: New message received at " + TestUtil.getAddress(MESSAGE_RECIPIENT));
    assertEquals("New mail notification body not found", 1, messages.size());
  ...
  public void testNotificationSubjBodyFrom() throws Exception {
    ...
    List<ZMessage> messages =
        TestUtil.search(notificationMbox, "in:inbox content:'" + notificationBody + "'");
