...
public class TestSoapFault extends TestCase {
    ...
    public void testSoapFaultTraceIpReveal() throws Exception {
        ...
        assertFalse("Trace contains ip address.", response.contains(batchRequestUrl));
        ...
        assertFalse("Trace contains ip address.", response.contains(createAppointmentRequestUrl));
        ...
        response = EntityUtils.toString(httpResponse.getEntity());
        assertFalse("Trace contains ip address.", response.contains(modifyContactRequestUrl));

        HttpPost noOpRequestMethod = new HttpPost(noOpRequestUrl);
        httpResponse = HttpClientUtil.executeMethod(noOpRequestMethod);
        response = EntityUtils.toString(httpResponse.getEntity());
        assertFalse("Trace contains ip address.", response.contains(noOpRequestUrl));
        ...
        response = EntityUtils.toString(httpResponse.getEntity());
        assertFalse("Trace contains ip address.", response.contains(getMiniCalRequestUrl));
    }
}
