...
public class TestGal extends TestCase {
  ...
  // set LDAP_SERVER_SIZE_LIMIT to either UNLIMITED or LIMITED and set slapd.conf accordingly, then
  // restart ldap server
  ...
  private String TOKENIZE_TEST_DOMAIN_NAME =
      TestProvisioningUtil.baseDomainName(TEST_CLASS_NAME + "-tokenize", null);
  private Provisioning mProv;

  private boolean DEBUG = true;
  private boolean SKIP_ACCT_CHECKING = false; // to save time if we are sure accounts are OK

  // too bad SyncGal does not work with SoapProvisioning, because it calls searchGal but that does
  // not return token.  :(
  ...
  static {
    if (SOAP_PROV) {
      try {
        TestUtil
            .cliSetup(); // which will invoke CliUtil.toolSetup(); and set the Provisioning instance
        // to SoapProvisioning
      } catch (ServiceException e) {
        e.printStackTrace();
        System.exit(1);
      }
    } else CliUtil.toolSetup();
  }
  ...
  class PageSizeEnum {
    ...
    PageSizeEnum() {
      mIdx = 0;
      if (LDAP_SERVER_SIZE_LIMIT == UNLIMITED) mEnum = PageSizeLdapServerUnlimited.values();
      else mEnum = PageSizeLdapServerLimited.values();
    }
  ...
  private void setupDomain(String domainName, String domainBaseDn) throws Exception {
    ...
    assertEquals(
        domain.getAttr(Provisioning.A_zimbraGalLdapBindPassword), LC.zimbra_ldap_password.value());
    ...
    assertEquals(
        domain.getAttr(Provisioning.A_zimbraGalSyncLdapBindDn), LC.zimbra_ldap_userdn.value());
    assertEquals(
        domain.getAttr(Provisioning.A_zimbraGalSyncLdapBindPassword),
  ...
  private void createAccount(String userName, String firstName, String lastName) throws Exception {
    ...
    Map<String, Object> attrs = new HashMap<String, Object>();
    if (firstName != null) attrs.put(Provisioning.A_givenName, firstName);
    if (lastName != null) attrs.put(Provisioning.A_sn, lastName);

    Account acct = mProv.get(AccountBy.name, acctName);
    if (acct == null) acct = mProv.createAccount(acctName, PASSWORD, attrs);

    assertEquals(acct.getName(), acctName);
    if (firstName != null) assertEquals(acct.getAttr(Provisioning.A_givenName), firstName);
    if (lastName != null) assertEquals(acct.getAttr(Provisioning.A_sn), lastName);
  }
  ...
  public TestGal() throws Exception {
    ...
    for (int i = 0; i < NUM_ACCOUNTS; i++) {
      ...
      if ((i + 1) % 100 == 0) System.out.println("Created/checked " + (i + 1) + " accounts");
    }
  ...
  private void autoCompleteGal(int numResultsExpected, int maxWanted) throws Exception {
    Domain domain = mProv.get(Key.DomainBy.name, DOMAIN_NAME);
    SearchGalResult galResult =
        mProv.autoCompleteGal(
            domain,
            QUERY,
            GalSearchType.account, // Provisioning.GAL_SEARCH_TYPE.ALL,
            maxWanted,
            null);
    if (numResultsExpected != galResult.getNumMatches()) dumpResult(galResult);
  ...
  private void searchGal(int numResultsExpected) throws Exception {
    Domain domain = mProv.get(Key.DomainBy.name, DOMAIN_NAME);
    SearchGalResult galResult = mProv.searchGal(domain, QUERY, GalSearchType.all, 0, null);
    assertEquals(numResultsExpected, galResult.getNumMatches());
    boolean expectedHasMore = numResultsExpected < NUM_ACCOUNTS;
    assertEquals(expectedHasMore, galResult.getHadMore());
  }
  ...
  private String syncGal(int numResultsExpected, int numTotal, String token) throws Exception {
    ...
    */
    ...
    if (!SOAP_PROV) assertEquals(expectedHasMore, galResult.getHadMore());
  ...
  private void autoCompleteGal(GalMode galMode, int pageSize) throws Exception {
    if (DEBUG) System.out.format("autoCompleteGal: %s, page size = %d\n", galMode, pageSize);
  ...
  private void searchGal(GalMode galMode, int pageSize) throws Exception {
    if (DEBUG) System.out.format("searchGal: %s, page size = %d\n", galMode, pageSize);
  ...
  private void syncGal(GalMode galMode, int pageSize) throws Exception {
    if (DEBUG) System.out.format("syncGal: %s, page size = %d\n", galMode, pageSize);
  ...
  public void autoCompleteGal(GalMode galMode) throws Exception {
    ...
    while (pse.hasNext()) autoCompleteGal(galMode, pse.next());
  }

  public void searchGal(GalMode galMode) throws Exception {
    PageSizeEnum pse = new PageSizeEnum();
    while (pse.hasNext()) searchGal(galMode, pse.next());
  }

  public void syncGal(GalMode galMode) throws Exception {
    PageSizeEnum pse = new PageSizeEnum();
    while (pse.hasNext()) syncGal(galMode, pse.next());
  }
  ...
  private void setupTokenize(String domainName, GalMode galMode, String andOr) throws Exception {
    ...
    if (galMode != null) attrs.put(Provisioning.A_zimbraGalMode, galMode.toString());
  ...
  private void tokenizeTest(GalOp galOp, String tokenizeKey, String key, String[] expectedUsers)
      throws Exception {
  ...
  */
  public void autoCompleteWithTokenizeAND(GalMode galMode) throws Exception {
    ...
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE,
        GalConstants.TOKENIZE_KEY_AND,
        "phoebe",
        new String[] {"user1", "user2", "user3"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE, GalConstants.TOKENIZE_KEY_AND, "shao", new String[] {"user1"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE,
        GalConstants.TOKENIZE_KEY_AND,
        "phoebe shao",
        new String[] {"user1"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE, GalConstants.TOKENIZE_KEY_AND, "phoebe blah", new String[0]);
  ...
  public void autoCompleteWithTokenizeOR(GalMode galMode) throws Exception {
    ...
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE,
        GalConstants.TOKENIZE_KEY_OR,
        "phoebe",
        new String[] {"user1", "user2", "user3"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE, GalConstants.TOKENIZE_KEY_OR, "shao", new String[] {"user1"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE,
        GalConstants.TOKENIZE_KEY_OR,
        "phoebe shao",
    ...
    tokenizeTest(
        ...
        new String[] {"user1", "user2", "user3"});
    tokenizeTest(
        GalOp.GOP_AUTOCOMPLETE, GalConstants.TOKENIZE_KEY_OR, "blah shao", new String[] {"user1"});
  }
  ...
  public void searchWithTokenizeAND(GalMode galMode) throws Exception {
    ...
    tokenizeTest(
        ...
        GalConstants.TOKENIZE_KEY_AND,
        "shao",
        new String[] {"user1", "user2", "user3"}); // (gn=*%s*)
    tokenizeTest(
        GalOp.GOP_SEARCH,
        GalConstants.TOKENIZE_KEY_AND,
        "phoebe shao",
