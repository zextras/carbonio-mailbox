...
public class TestServerStats extends TestCase {
  ...
  /**
   * Confirms that the server is returning the correct values for the number of protocol handler
   * threads and client connections.
   */
  public void testThreadsAndConnections() throws Exception {
    Server server = Provisioning.getInstance().getLocalServer();
    verifyThreadsAndConnections(
        ZimbraPerf.RTS_POP_THREADS, ZimbraPerf.RTS_POP_CONN, server.getPop3BindPort());
    verifyThreadsAndConnections(
        ZimbraPerf.RTS_IMAP_THREADS, ZimbraPerf.RTS_IMAP_CONN, server.getImapBindPort());
    verifyThreadsAndConnections(
        ZimbraPerf.RTS_LMTP_THREADS, ZimbraPerf.RTS_LMTP_CONN, server.getLmtpBindPort());
  }
  ...
  private void waitForValue(String statName, int expected) throws Exception {
    int val = 0;
    for (int i = 1; i <= 20; i++) {
  ...
  private int getStatValue(String statName) throws Exception {
  ...
  private Socket connect(int port) throws ServiceException, IOException {
    Server server = Provisioning.getInstance().getLocalServer();
