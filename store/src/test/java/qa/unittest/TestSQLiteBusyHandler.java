...
/**
 * Unit test to exercise busy handler Not added to ZimbraSuite as it is specific to SQLite, and not
 * used in normal ZCS operation
 */
public class TestSQLiteBusyHandler extends TestCase {

  public static final int timeout =
      120000; // 2 minutes is long enough; might not get a busy every time, but should get one most
  // of the time
  ...
  public Connection createConnect() throws Exception {
    ...
    RetryConnectionFactory factory =
        new RetryConnectionFactory("jdbc:sqlite:" + DB_PATH + "zimbra.db", null);
  ...
  public Connection connectAndAttach(String dbName)
      throws SQLException, ServiceException, Exception {
  ...
  public boolean checkSubject(String subject, String dbName, Connection conn) throws Exception {
    PreparedStatement stmt =
        conn.prepareStatement(
            "select subject from " + dbName + ".mail_item where subject='" + subject + "'");
  ...
  public void testUpdate(String dbName, Connection conn) throws Exception {
    PreparedStatement stmt =
        conn.prepareStatement(
            "update " + dbName + ".mail_item set subject='msg123' where subject='msg'");
    stmt.executeUpdate();
    stmt.close();
    checkSubject("msg123", dbName, conn);
    stmt =
        conn.prepareStatement(
            "update " + dbName + ".mail_item set subject='msg' where subject='msg123'");
    stmt.executeUpdate();
    stmt.close();
  ...
  public boolean testEntryName(String name, Connection conn) throws Exception {
    PreparedStatement stmt =
        conn.prepareStatement("select entry_name from directory where entry_id=2");
    ResultSet rs = stmt.executeQuery();
    try {
      if (!rs.next()) {
  ...
  public void testUpdateZimbra(Connection conn) throws Exception {
    PreparedStatement stmt =
        conn.prepareStatement("update directory set entry_name='blah' where entry_id=2");
    stmt.executeUpdate();
    stmt.close();
