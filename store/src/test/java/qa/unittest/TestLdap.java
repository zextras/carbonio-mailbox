...
import qa.unittest.prov.ldap.TestLdapConnection;
import qa.unittest.prov.ldap.TestLdapHelper;
import qa.unittest.prov.ldap.TestLdapProvAccount;
import qa.unittest.prov.ldap.TestLdapProvAlias;
import qa.unittest.prov.ldap.TestLdapProvAttrCallback;
import qa.unittest.prov.ldap.TestLdapProvCos;
import qa.unittest.prov.ldap.TestLdapProvDIT;
import qa.unittest.prov.ldap.TestLdapProvDataSource;
import qa.unittest.prov.ldap.TestLdapProvDistributionList;
import qa.unittest.prov.ldap.TestLdapProvDomain;
import qa.unittest.prov.ldap.TestLdapProvDynamicGroup;
import qa.unittest.prov.ldap.TestLdapProvEntry;
import qa.unittest.prov.ldap.TestLdapProvExternalLdapAuth;
import qa.unittest.prov.ldap.TestLdapProvGal;
import qa.unittest.prov.ldap.TestLdapProvGlobalConfig;
import qa.unittest.prov.ldap.TestLdapProvGlobalGrant;
import qa.unittest.prov.ldap.TestLdapProvIdentity;
import qa.unittest.prov.ldap.TestLdapProvMimeType;
import qa.unittest.prov.ldap.TestLdapProvMisc;
import qa.unittest.prov.ldap.TestLdapProvModifyAttrs;
import qa.unittest.prov.ldap.TestLdapProvRenameDomain;
import qa.unittest.prov.ldap.TestLdapProvSearchDirectory;
import qa.unittest.prov.ldap.TestLdapProvServer;
...
import qa.unittest.prov.ldap.TestLdapZMutableEntry;
import qa.unittest.prov.ldap.TestProvAlias;
import qa.unittest.prov.ldap.TestProvAttr;
import qa.unittest.prov.ldap.TestProvCallbackAvailableZimlets;
import qa.unittest.prov.ldap.TestProvIDN;
import qa.unittest.prov.ldap.TestProvValidator;
import qa.unittest.prov.ldap.TestProvZimbraId;
import qa.unittest.prov.soap.TestACLUserRights;
import qa.unittest.prov.soap.TestDelegatedDL;
import qa.unittest.prov.soap.TestSearchGal;

public class TestLdap {
  private static final String TEST_LDAP_BASE_DOMAIN =
      InMemoryLdapServer.UNITTEST_BASE_DOMAIN_SEGMENT; // "testldap";
  ...
  enum TestConfig {
    UBID(
        com.zimbra.cs.ldap.unboundid.UBIDLdapClient.class,
        com.zimbra.cs.account.ldap.LdapProvisioning.class);
    ...
    static synchronized void useConfig(TestConfig config) throws Exception {
      ...
      if (config.ldapClientClass != null) {
        modifyLocalConfig(
            LC.zimbra_class_ldap_client.key(), config.ldapClientClass.getCanonicalName());
      } else {
        // remove the key
        modifyLocalConfig(LC.zimbra_class_ldap_client.key(), "");
      }
      modifyLocalConfig(
          LC.zimbra_class_provisioning.key(), config.ldapProvClass.getCanonicalName());
      LC.reload();
    }
  ...
  /**
   * Given a name (which is to be turn into a DN), mix in chars defined in rfc2253.txt that need to
   * be escaped in RDN value.
   *
   * <p>http://www.ietf.org/rfc/rfc2253.txt?number=2253
   *
   * <p>- a space or "#" character occurring at the beginning of the string
   *
   * <p>- a space character occurring at the end of the string
   *
   * <p>- one of the characters ",", "+", """, "\", "<", ">" or ";"
   *
   * <p>Implementations MAY escape other characters.
   *
   * <p>If a character to be escaped is one of the list shown above, then it is prefixed by a
   * backslash ('\' ASCII 92).
   *
   * <p>Otherwise the character to be escaped is replaced by a backslash and two hex digits, which
   * form a single byte in the code of the character.
   *
   * @param name
   * @return
   */
    ...
    */
    if (useInMemoryLdapServerOverride != null) {
      ...
      if (DebugConfig.useInMemoryLdapServer != useInMemoryLdapServer) {
        System.out.println(
            "useInMemoryLdapServerOverride is "
                + useInMemoryLdapServerOverride
                + " but LC key debug_use_in_memory_ldap_server is "
                + key.value()
  ...
  /*
   * zmjava -ea qa.unittest.TestLdap > ~/temp/out.txt
   *
