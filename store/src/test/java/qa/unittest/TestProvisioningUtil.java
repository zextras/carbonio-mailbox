...
public class TestProvisioningUtil extends TestCase {
  ...
  public static String baseDomainName(String testName, String testId) {
    if (testId == null) return testName + "." + NAME_ROOT_DOMAIN;
    else return testName + "-" + testId + "." + NAME_ROOT_DOMAIN;
  }
  ...
  public static void verifyEntries(List<NamedEntry> list, NamedEntry[] entries, boolean checkCount)
      throws Exception {
    try {
      if (checkCount) assertEquals(list.size(), entries.length);
      ...
      if (checkCount) assertEquals(ids.size(), 0);
  ...
  public static void verifyEntries(Set<String> list, String[] names, boolean checkCount)
      throws Exception {
    try {
      if (checkCount) assertEquals(names.length, list.size());

      for (String name : names) assertTrue(list.contains(name));
  ...
  public static void verifyEquals(Set<String> expected, Set<String> actual) throws Exception {
    ...
    for (String e : expected) assertTrue(actual.contains(e));

    for (String a : actual) assertTrue(expected.contains(a));
  }
  ...
  public static SoapProvisioning getSoapProvisioning(String userName, String password)
