...
public class TestProvisioning extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    // DOMAIN_NAME_SPECIAL_CHARS = "domain-" + SPECIAL_CHARS_ALLOWED_IN_DOMAIN + "-" + TEST_ID +
    // ".ldap-test-domain";
    DOMAIN_NAME = TestProvisioningUtil.baseDomainName("domain", TEST_ID);

    // DOMAIN_NAME_SPECIAL_CHARS = TestProvisioningUtil.baseDomainName("domain-" +
    // SPECIAL_CHARS_ALLOWED_IN_DOMAIN, TEST_ID);
    DOMAIN_NAME_SPECIAL_CHARS =
        TestProvisioningUtil.baseDomainName(
            "domain-special", TEST_ID); // TODO: javamail now is more strict on domain names!!
    ...
    ACCT_ALIAS_AFTER_ACCOUNT_RENAME_TO_OTHER_DMAIN_EMAIL =
        ACCT_ALIAS_USER + "@" + OTHER_DOMAIN_NAME;
    ...
    ACCT_ALIAS_IN_OTHER_DOMAIN_AFTER_ACCOUNT_RENAME_TO_ORIG_DOMAIN_EMAIL =
        ACCT_ALIAS_IN_OTHER_DOMAIN_USER + "@" + DOMAIN_NAME;
    ACCT_FULL_NAME = "Phoebe Shao";

    ACCT_NAMING_ATTR = LC.get("ldap_dit_naming_rdn_attr_user");
    if (StringUtil.isNullOrEmpty(ACCT_NAMING_ATTR)) ACCT_NAMING_ATTR = "uid";
  ...
  static class Flag {
    ...
    static {
      ...
      sNeedLdapPaging.add(
          "searchAccounts"); // com.zimbra.cs.backup.BackupManager.getAccountsOnServer,
      // com.zimbra.cs.service.admin.FixCalendarTimeZone.getAccountsOnServer
      sNeedLdapPaging.add("searchAccounts_domain");
      sNeedLdapPaging.add("searchCalendarResources");
      sNeedLdapPaging.add(
          "searchCalendarResources_domain"); // com.zimbra.cs.service.account.SearchCalendarResources
    ...
    public void cleanup() throws Exception {
      ...
      */
      ...
      for (NamedEntry entry : list) {
        if (entry instanceof CalendarResource) mProv.deleteCalendarResource(entry.getId());
        else if (entry instanceof Alias) mProv.removeAlias((Account) null, entry.getName());
        else if (entry instanceof Account) mProv.deleteAccount(entry.getId());
        else if (entry instanceof DistributionList) mProv.deleteDistributionList(entry.getId());
        else throw new Exception("unexpected entry type: " + entry.getClass().getCanonicalName());
      }
  ...
  public static class TestCustomAuth extends ZimbraCustomAuth {
    ...
    public void authenticate(
      ...
      if (acct.getName().equals(mTheOnlyAcctThatCanAuth.getName())
          ...
          && verifyArgs(args)) return;
      else
        throw new Exception(
            "auth failed by TestCustomAuth for " + acct.getName() + " password " + password);
    }
  }
  ...
  private String[] domainTest() throws Exception {
    ...
    TestProvisioningUtil.verifyEntries(
        list, new NamedEntry[] {entry, otherEntry, specialCharEntry}, false);
  ...
  private void externalAuthTest(Account account, boolean startTLS) throws Exception {
    ...
    attrsToMod.put(
        Provisioning.A_zimbraAuthLdapURL, "ldap://" + LC.zimbra_server_hostname.value() + ":389");
    attrsToMod.put(Provisioning.A_zimbraAuthLdapSearchFilter, "(zimbraMailDeliveryAddress=%n)");
    attrsToMod.put(
        Provisioning.A_zimbraAuthLdapSearchBindPassword, LC.zimbra_ldap_password.value());
    attrsToMod.put(Provisioning.A_zimbraAuthLdapSearchBindDn, LC.zimbra_ldap_userdn.value());

    if (startTLS) attrsToMod.put(Provisioning.A_zimbraAuthLdapStartTlsEnabled, "TRUE");
  ...
  private void authTest(Account account) throws Exception {
    ...
    if (TEST_STARTTLS) externalAuthTest(account, true);
    ...
    mProv.modifyAttrs(domain, attrsToMod, true);
    // by domain realm mapping    acct-1@PHOEBE.LOCAL has to be created (sudo
    // /usr/local/sbin/kadmin.local addprinc command)
    ...
    // by specific foreignPrincipal   user1-1@PHOEBE.LOCAL has to be created (sudo
    // /usr/local/sbin/kadmin.local addprinc command)
    // mProv.authAccount(account, PASSWORD, "unittest");  uncomment after krb5 server is fixed

    // skip these tests, as there could be multiple domain with PHOEBE.LOCAL in
    // zimbraAuthKerberos5Realm  from previous test
    ...
    */
    ...
    attrsToMod.put(
        ...
        AuthMech.custom.name() + ":" + customAuthHandlerName + " " + args);
    mProv.modifyAttrs(domain, attrsToMod, true);
  ...
  private Account[] accountTest(Account adminAcct, Cos cos, Domain domain, Domain otherDomain)
    ...
    acctAttrs.put(
        ...
        new String[] {"kerberos5:" + krb5Principal1, "kerberos5:" + krb5Principal2});
    ...
    mCustomProvTester.addAttr(
        acctAttrsSpecialChars, ACCT_NAMING_ATTR, namingAttrValue(ACCT_USER_SPECIAL_CHARS));
    acctAttrs.put(Provisioning.A_zimbraCOSId, cos.getId());
    Account entrySpecialChars =
        mProv.createAccount(ACCT_EMAIL_SPECIAL_CHARS, PASSWORD, acctAttrsSpecialChars);
    String acctSpecialCharsDn =
        ACCT_NAMING_ATTR + "=" + namingAttrValue(ACCT_USER_SPECIAL_CHARS) + "," + ACCT_BASE_DN;
    ...
    try {
      ...
      if (mCustomProvTester.isCustom()) correct = true;
    }
    assertTrue(correct);
    ...
    if (mCustomProvTester.isCustom()) assertEquals(null, entryGot);
    else TestProvisioningUtil.verifySameEntry(entry, entryGot);
    ...
    acctAttrs.put(
        Provisioning.A_zimbraForeignPrincipal, new String[] {"kerberos5:" + krb5PrincipalDup});
    ...
    domainAttrs.put(
        Provisioning.A_zimbraAuthMech,
        "kerberos5"); // not necessary, put it to make sure if it is present things should still
    // work
    ...
    if (!Flag.needLdapPaging("getAllAccounts_domain")) {
      ...
      TestProvisioningUtil.verifyEntries(
          list,
          new NamedEntry[] {entry, adminAcct, acctX, acctY},
    ...
    if (!Flag.needLdapPaging("searchDirectory")) {
      ...
      assertEquals(NEW_EMAIL, entry.getAttr(Provisioning.A_zimbraPrefFromAddress));
      if (mCustomProvTester.isCustom()) {
        ...
        TestProvisioningUtil.verifyEntriesByName(
            list,
            new String[] {ACCT_ALIAS_EMAIL},
            mCustomProvTester.verifyAliasCountForDomainBasedSearch());
      } else {
        ...
        TestProvisioningUtil.verifyEntriesByName(
            list, new String[] {ACCT_ALIAS_IN_OTHER_DOMAIN_EMAIL}, true);
      }
    }
    ...
    if (!mCustomProvTester.isCustom()) {
      if (!Flag.needLdapPaging("searchDirectory")) {
        ...
        TestProvisioningUtil.verifyEntriesByName(
            list, new String[] {NEW_EMAIL_IN_OTHER_DOMAIN}, true);
        // make sure the alias is now moved to the other domain and there shouldn't be any let in
        // the old domain
        list = searchAliasesInDomain(domain);
        assertEquals(0, list.size());
        ...
        TestProvisioningUtil.verifyEntriesByName(
            list,
            new String[] {
              ACCT_ALIAS_AFTER_ACCOUNT_RENAME_TO_OTHER_DMAIN_EMAIL, ACCT_ALIAS_IN_OTHER_DOMAIN_EMAIL
            },
            true);
      }
    }
    ...
    if (!Flag.needLdapPaging("searchDirectory")) {
      ...
      if (mCustomProvTester.isCustom()) {
        ...
        TestProvisioningUtil.verifyEntriesByName(
            list,
            new String[] {
              ACCT_ALIAS_EMAIL, ACCT_ALIAS_IN_OTHER_DOMAIN_AFTER_ACCOUNT_RENAME_TO_ORIG_DOMAIN_EMAIL
            },
            true);
      }
    }
    ...
    if (!mCustomProvTester.isCustom()) {
      mProv.removeAlias(
          entry, ACCT_ALIAS_IN_OTHER_DOMAIN_AFTER_ACCOUNT_RENAME_TO_ORIG_DOMAIN_EMAIL);
    }
    if (!Flag.needLdapPaging("searchDirectory")) {
      /*
  ...
  private void localeTest() throws Exception {
    System.out.println(
        "Testing locale"); // bug 23218: entry.getLocale() is not refreshed on modifying locale
    // (zimbraPrefLocale/zimbraLocale)
  ...
  private DistributionList[] distributionListTest(Domain domain) throws Exception {
    ...
    if (mCustomProvTester.isCustom()) setDefaultDomain(DOMAIN_NAME_SPECIAL_CHARS);
    ...
    mCustomProvTester.addAttr(
        dlAttrsSpecialChars, ACCT_NAMING_ATTR, namingAttrValue(DL_USER_SPECIAL_CHARS));
    DistributionList dlSpecilaChars =
        mProv.createDistributionList(DL_EMAIL_SPECIAL_CHARS, dlAttrsSpecialChars);
    ...
    if (mCustomProvTester.isCustom()) setDefaultDomain(curDefaultDomain);
    ...
    if (!Flag.needLdapPaging("getAllDistributionLists")) {
      ...
      TestProvisioningUtil.verifyEntries(
          list,
          new NamedEntry[] {entry, dlNested},
          mCustomProvTester.verifyDLCountForDomainBasedSearch());
    }
    ...
    if (!Flag.needLdapPaging("getAllDistributionLists")) {
      ...
      TestProvisioningUtil.verifyEntriesByName(
          list,
          new String[] {NEW_EMAIL, DL_NESTED_EMAIL},
          mCustomProvTester.verifyDLCountForDomainBasedSearch());
    }
    ...
    if (!Flag.needLdapPaging("getAllDistributionLists")) {
      ...
      TestProvisioningUtil.verifyEntriesByName(
          list,
          new String[] {DL_EMAIL, DL_NESTED_EMAIL},
          mCustomProvTester.verifyDLCountForDomainBasedSearch());
    }
  ...
  private DataSource dataSourceTest(Account account) throws Exception {
    ...
    dsAttrs.put(
        Provisioning.A_zimbraPrefDefaultSignatureId,
        LdapUtil.generateUUID()); // just some random id, not used anywhere
    ...
    DataSource entry =
        mProv.createDataSource(account, DataSourceType.pop3, DATA_SOURCE_NAME, dsAttrs);
  ...
  private Identity identityTest(Account account) throws Exception {
    ...
    Identity defaultIdentity =
        mProv.get(account, Key.IdentityBy.name, ProvisioningConstants.DEFAULT_IDENTITY_NAME);
  ...
  private void signatureTest(Account account) throws Exception {
    ...
    // since this is the only signature, it should be automatically set as the default signature of
    // the account
    ...
    assertEquals(
        SIGNATURE_VALUE_MODIFIED, entryGot.getAttr(Provisioning.A_zimbraPrefMailSignature));
  ...
  private void externalGalTest(Domain domain, boolean startTLS) throws Exception {
    ...
    attrs.put(
        Provisioning.A_zimbraGalLdapURL,
        "ldap://" + LC.zimbra_server_hostname.value() + ":389"); // cannot be localhost for startTLS
    ...
    Provisioning.SearchGalResult galResult =
        mProv.searchGal(domain, ACCT_EMAIL, GalSearchType.all, 0, null);
  ...
  private void galTest(Domain domain) throws Exception {
    ...
    // auto complete Gal
    Provisioning.SearchGalResult galResult =
        mProv.autoCompleteGal(domain, query, GalSearchType.all, 100, null);
    ...
    // search gal
    galResult = mProv.searchGal(domain, query, GalSearchType.all, 0, null);
    ...
    if (TEST_STARTTLS) externalGalTest(domain, true);
  }
  ...
  private void familyTest() throws Exception {
    ...
    try {
      ...
      attrs.put(
          Provisioning.A_zimbraPrefChildVisibleAccount,
          SetUtil.union(temp, visibleCids, idsNotChild));
      parent = mProv.createAccount(parentName, PASSWORD, attrs);
      fail();
    } catch (ServiceException e) {
      if (!e.getCode().equals(ServiceException.INVALID_REQUEST)) fail();
    }
    ...
    // delete all accounts
    for (String childId : cids) mProv.deleteAccount(childId);
    for (String childId : idsNotChild) mProv.deleteAccount(childId);
  ...
  private void flushCacheTest() throws Exception {
    ...
    // flush the account
    mSoapProv.flushCache(
        CacheEntryType.account,
        new CacheEntry[] {new CacheEntry(Key.CacheEntryBy.id, acct.getId())});
    ...
    // flush the cos
    mSoapProv.flushCache(
        CacheEntryType.cos, new CacheEntry[] {new CacheEntry(Key.CacheEntryBy.id, cos.getId())});
  ...
  private Set<String> getAvailableSkins_prior_bug31596(Account acct) throws ServiceException {
    ...
    if (skins.size() > 0) return skins;

    // 2) if set on Domain, use it
    Domain domain = Provisioning.getInstance().getDomain(acct);
    if (domain == null) return skins;
  ...
  private void attributeInheritanceTestMultiValue_prior_bug31596() throws Exception {
    ...
    TestProvisioningUtil.verifyEntries(
        val, new String[] {"domain-value-1", "domain-value-2"}, true);
  }
  ...
  private String execute() throws Exception {
    ...
    Account accounts[] =
        accountTest(
            adminAccount,
            mProv.get(Key.CosBy.name, cosName),
            mProv.get(Key.DomainBy.name, domainName),
            mProv.get(Key.DomainBy.name, otherDomainName));
    ...
    CalendarResource calendarResource =
        calendarResourceTest(
            mProv.get(Key.CosBy.name, cosName), mProv.get(Key.DomainBy.name, domainName));

    DistributionList[] distributionLists =
        distributionListTest(mProv.get(Key.DomainBy.name, domainName));
    ...
    for (Account acct : accounts) mProv.deleteAccount(acct.getId());
  ...
  public static void main(String[] args) throws Exception {
    ...
    // ZimbraLog.toolSetupLog4j("DEBUG",
    // "/Users/pshao/p4/main/ZimbraServer/conf/log4j.properties.zmprov-l");
