...
public abstract class TestProv extends TestLdap {
  ...
  protected static final String BASE_DOMAIN_NAME =
      TestLdap.baseDomainName(
          TestProv.class); // TestProvisioningUtil.baseDomainName("test-prov", TEST_ID);
  ...
  protected Account createAccount(String localpart, Domain domain, Map<String, Object> attrs)
      throws Exception {
    if (domain == null) domain = createDomain();
  ...
  protected CalendarResource createCalendarResource(String localpart, Domain domain)
      throws Exception {
    Map<String, Object> attrs = new HashMap<String, Object>();
  ...
  protected DistributionList createUserDistributionList(String localpart, Domain domain)
      throws Exception {
  ...
  */
  ...
  private void deleteEntry(NamedEntry entry) throws Exception {
    if (entry instanceof Account) mProv.deleteAccount(entry.getId());
    else if (entry instanceof CalendarResource) mProv.deleteCalendarResource(entry.getId());
    else if (entry instanceof Cos) mProv.deleteCos(entry.getId());
    else if (entry instanceof Group) mProv.deleteGroup(entry.getId());
    else if (entry instanceof Domain) mProv.deleteDomain(entry.getId());
    else if (entry instanceof Server) mProv.deleteServer(entry.getId());
    else if (entry instanceof Zimlet) mProv.deleteZimlet(entry.getName());
