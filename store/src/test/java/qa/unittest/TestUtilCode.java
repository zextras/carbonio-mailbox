...
public class TestUtilCode extends TestCase {
  ...
  /** Tests {@link ListUtil#split} on lists of size 0 through 50, splitting by 10 items. */
  public void testSplit() throws Exception {
    for (int i = 0; i < 50; i++) {
      ...
      assertEquals(
          "Unexpected number of splits for list of size " + list.size(),
          expectedSize,
          listOfLists.size());
      ...
      ZimbraLog.test.debug(
          String.format("Split a list of %d items into %d lists", list.size(), listOfLists.size()));
      assertTrue(
          "Lists don't match: " + StringUtil.join(",", list), compareLists(list, listOfLists));
    }
  }
  ...
  /**
   * Makes sure that {@link ZimbraLog#addAccountNameToContext} can be called with a <tt>null</tt>
   * value. See bug 26997 for details.
   */
  ...
  private void validateRfc822(String before, int numNullBytes, String after, boolean isValid)
    ...
    Rfc822ValidationInputStream in =
        new Rfc822ValidationInputStream(new ByteArrayInputStream(content), 10240);
    while ((in.read(buf)) >= 0) {}
    in.close();
  ...
  public void testFileSegmentDataSource() throws Exception {
    ...
    assertEquals(
        "12345",
        new String(
            ByteUtil.getContent(ds.getInputStream(), 5))); // Make sure we can get multiple streams.
