...
public class TestImapOneWayImport extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    mOriginalCleartextValue =
        TestUtil.getServerAttr(Provisioning.A_zimbraImapCleartextLoginEnabled);
  ...
  public void testImapOneWayImport() throws Exception {
    ...
    TestUtil.addMessage(
        mRemoteMbox, NAME_PREFIX + " msg1", Integer.toString(Mailbox.ID_FOLDER_INBOX), "u");
  ...
  private void compare(ZMailbox mbox1, ZFolder folder1, ZMailbox mbox2, ZFolder folder2)
    ...
    for (ZFolder child1 : folder1.getSubFolders()) {
      if (isMailFolder(child1)) {
        ...
        String msg =
            String.format(
                "Could not find folder %s/%s for %s",
    ...
    assertEquals(
        "Message count doesn't match (folder1 = " + folder1 + ", folder2 = " + folder2 + ")",
        folder1.getMessageCount(),
  ...
  public void tearDown() throws Exception {
    ...
    Provisioning.getInstance()
        .getLocalServer()
        .setImapDisplayMailFoldersOnly(mDisplayMailFoldersOnly);
