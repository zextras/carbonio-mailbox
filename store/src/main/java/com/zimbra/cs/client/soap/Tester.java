...
public class Tester {
  ...
  private static void recursiveDumpFolder(LmcFolder f, int depth) {
    ...
    if (fName != null)
      if (fName.equals(FOLDER_NAME_TRASH)) trashFolderID = f.getFolderID();
      else if (fName.equals(FOLDER_NAME_INBOX)) inboxFolderID = f.getFolderID();
  ...
  private static void doSearchAndConvAction(LmcSession session, String serverURL)
    ...
    for (Iterator cit = cList.iterator(); cit.hasNext(); ) {
      ...
      if (firstConv == null) firstConv = conv;
  ...
  private static void getAndDumpContacts(LmcSession session, String serverURL)
    ...
    for (int o = 0; contacts != null && o < contacts.length; o++) System.out.println(contacts[o]);
  }
  ...
  private static void doChangePassword(
      LmcSession session, String account, String currPassword, String serverURL)
      throws IOException, LmcSoapClientException, ServiceException, SoapFaultException,
          HttpException {
  ...
  public static void main(String argv[]) {
    ...
    System.out.println(
        "connecting to " + serverURL + " as " + argv[1] + " with password " + argv[2]);
    ...
    try {
      ...
      String aid =
          smr.postAttachment(
              "http://dogfood.example.zimbra.com/service/upload",
              session,
              new File("c:/temp/ops.txt"),
              ".example.zimbra.com",
              5000);
