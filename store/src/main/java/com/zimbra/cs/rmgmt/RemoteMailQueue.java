...
public class RemoteMailQueue {
  private static Map<String, RemoteMailQueue> mMailQueueCache =
      new HashMap<String, RemoteMailQueue>();

  public static RemoteMailQueue getRemoteMailQueue(
      Server server, String queueName, boolean forceScan) throws ServiceException {
  ...
  private class QueueItemVisitor implements RemoteResultParser.Visitor {
    ...
    public void handle(int lineNo, Map<String, String> map) throws IOException {
      ...
      if (ZimbraLog.rmgmt.isDebugEnabled()) {
        ZimbraLog.rmgmt.debug("[scan id=" + mId + "] " + doc);
      }
  ...
  private class QueueHandler implements RemoteBackgroundHandler {
    ...
    public void read(InputStream stdout, InputStream stderr) {
      try {
        ...
        if (ZimbraLog.rmgmt.isDebugEnabled())
          ZimbraLog.rmgmt.debug("starting scan with visitor id=" + v.mId + " " + mDescription);
        ...
        if (ZimbraLog.rmgmt.isDebugEnabled())
          ZimbraLog.rmgmt.debug(
              "finished scan with visitor id="
                  + v.mId
                  + " total="
                  + mNumMessages
                  + " "
    ...
    public void error(Throwable t) {
      ZimbraLog.rmgmt.error(
          "error when scanning mail queue " + mQueueName + " on host " + mServerName, t);
    }
  }
  ...
  void clearIndexInternal() throws IOException {
    ...
    try {
      ...
      writer =
          new IndexWriter(
              LuceneDirectory.open(mIndexPath),
    ...
    synchronized (mScanLock) {
      if (mScanInProgress) {
        ...
        throw ServiceException.ALREADY_IN_PROGRESS(
            "scan server=" + mServerName + " queue=" + mQueueName);
      }
  ...
  public boolean waitForScan(long timeout) {
    synchronized (mScanLock) {
      ...
      if (ZimbraLog.rmgmt.isDebugEnabled())
        ZimbraLog.rmgmt.debug("scan wait returning progress=" + mScanInProgress + " " + this);
  ...
  private RemoteMailQueue(Server server, String queueName, boolean scan) throws ServiceException {
    ...
    mDescription =
        "[mail-queue: server=" + mServerName + " name=" + mQueueName + " hash=" + hashCode() + "]";
    mIndexPath =
        new File(
            LC.zimbra_tmp_directory.value() + File.separator + server.getId() + "-" + queueName);
    if (scan) {
  ...
  private void search0(
      SearchResult result, IndexReader indexReader, Query query, int offset, int limit)
      throws IOException {
    if (ZimbraLog.rmgmt.isDebugEnabled()) {
      ZimbraLog.rmgmt.debug(
          "searching query=" + query + " offset=" + offset + " limit=" + limit + " " + this);
    }
  ...
  private static void usage(String err) {
    ...
    System.err.println(
        "Usage: "
            + RemoteMailQueue.class.getName()
            + " scan|search|action host queue [query] [action-name queueids]");
    System.exit(1);
  }
