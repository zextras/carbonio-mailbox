...
public class ExchangeFreeBusyProvider extends FreeBusyProvider {
  ...
  // public static final String FORM_AUTH_PATH = "/exchweb/bin/auth/owaauth.dll";  // specified in
  // LC.calendar_exchange_form_auth_url
  ...
  public boolean handleMailboxChange(String accountId) {
    ...
    try {
      ...
      if (status != MULTI_STATUS) {
        ...
        String respStr =
            (resp == null ? "" : new String(ByteUtil.readInput(resp, 1024, 1024), "UTF-8"));
        ZimbraLog.fb.error(
            "cannot modify resource at %s : http error %d, buf (%s)", url, status, respStr);
        return false; // retry
      }
  ...
  private HttpResponse sendRequest(HttpRequestBase method, ServerInfo info)
  ...
  private boolean formAuth(HttpClientBuilder clientBuilder, ServerInfo info)
      throws IOException, HttpException {
    ...
    ByteArrayEntity re =
        new ByteArrayEntity(buf.toString().getBytes(), ContentType.create("x-www-form-urlencoded"));
    method.setEntity(re);
  ...
  public List<FreeBusy> getFreeBusyForHost(String host, ArrayList<Request> req) throws IOException {
    ...
    try {
      ...
      if (status != 200) return getEmptyList(req);
      if (ZimbraLog.fb.isDebugEnabled()) {
        ...
        if (cl != null) contentLength = Integer.valueOf(cl.getValue());
        String buf =
            new String(
                com.zimbra.common.util.ByteUtil.readInput(
                    httpResponse.getEntity().getContent(), contentLength, contentLength),
                "UTF-8");
        ZimbraLog.fb.debug(buf);
        response = Element.parseXML(buf);
      } else response = Element.parseXML(httpResponse.getEntity().getContent());
  ...
  private String constructGetUrl(ServerInfo info, ArrayList<Request> req) {
    ...
    for (Request r : req) buf.append("&u=SMTP:").append(r.email);
    return buf.toString();
  }
  ...
  public static HttpResponse checkAuth(ServerInfo info, Account requestor)
  ...
  public static class ExchangeUserFreeBusy extends FreeBusy {
    ...
    */
    ...
    protected ExchangeUserFreeBusy(
        String fb, String emailAddr, int interval, long start, long end) {
