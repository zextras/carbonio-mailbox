...
public abstract class FreeBusyProvider {
    ...
    for (Map.Entry<String, ArrayList<FreeBusy>> entry : freebusyMap.entrySet()) {
      ...
      for (FreeBusy fb : freebusyList) {
        if (freebusy == null) {
          ...
          if (!fb.getBusiest().equals(IcalXmlStrMap.FBTYPE_FREE)) freebusy = fb;
        }
      }
      ret.add(freebusy);
    }
  ...
  public static void getRemoteFreeBusy(
      Account requestor,
      Element response,
  ...
  protected FreeBusy getFreeBusy(String accountId, int folderId) throws ServiceException {
    ...
    return mbox.getFreeBusy(
        null, cachedFreeBusyStartTime(accountId), cachedFreeBusyEndTime(accountId), folderId);
  }
  ...
  public static FreeBusyProvider getProvider(String name) {
    for (FreeBusyProvider p : sPROVIDERS) if (p.getName().equals(name)) return p;
    return null;
  }
  ...
  public static class FreeBusySyncQueue extends LinkedList<String> implements Runnable {
              ...
              try {
                retryInterval =
                    Provisioning.getInstance()
                        .getLocalServer()
                        .getFreebusyPropagationRetryInterval();
              } catch (Exception e) {
              }
    ...
    public synchronized void readFromDisk() throws IOException {
      ...
      if (len > MAX_FILE_SIZE) {
        ZimbraLog.fb.error(
            "The free/busy replication queue is too large: " + mFilename + " (" + len + ")");
        return;
      }
      ...
      if (numTokens != tokens.length) {
        ZimbraLog.fb.error(
            "The free/busy replication queue is inconsistent: "
                + "numTokens="
                + numTokens
                + ", actual="
                + tokens.length);
