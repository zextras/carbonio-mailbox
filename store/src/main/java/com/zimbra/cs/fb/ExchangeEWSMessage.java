...
public class ExchangeEWSMessage extends ExchangeMessage {
  ...
  Map<PathToExtendedFieldType, NonEmptyArrayOfPropertyValuesType> GetFreeBusyProperties(
      FreeBusy fb) {
    ...
    IntervalList consolidated = new IntervalList(fb.getStartTime(), fb.getEndTime()); // TEMP
    ...
    for (FreeBusy.Interval interval : fb) {
      ...
      if (status.equals(type)) {
        ...
        if (consolidated != null)
          consolidated.addInterval(
              new Interval(interval.getStart(), interval.getEnd(), IcalXmlStrMap.FBTYPE_BUSY));
      }
    }
