...
/**
 * @author tim
 *     <p>Calculates the FreeBusy list for a mailbox.
 *     <p>The FreeBusy list is a list of Intervals each from (start, end] with a type of "Free" or
 *     "Busy" or eventually "Out of Office".
 *     <p>Calculating from the mailbox is relatively simple: start with everything free, then expand
 *     the mailbox's list of appointments and add busy intervals to the list...then combine
...
public class FreeBusy implements Iterable<FreeBusy.Interval> {
  ...
  public static class Interval {
    ...
    public Interval(long start, long end, String status, FBInstance instance) {
      ...
      if (instance != null) mInstances.add(instance);
    }
    ...
    void addInstances(LinkedHashSet<FBInstance> instances) {
      if (instances != null) mInstances.addAll(instances);
    }
  ...
  public String toVCalendar(Method m, String organizer, String attendee, String url, String uid) {
    ...
    toRet.append("PRODID:").append(ZCalendar.sCarbonioProdID).append(NL);
    ...
    for (Interval cur : this) {
      ...
      if (status.equals(IcalXmlStrMap.FBTYPE_FREE)) {
        ...
        // Treat no-data case same as free, because other apps probably won't understand a
        // non-standard fbtype value.
        continue;
      } else if (status.equals(IcalXmlStrMap.FBTYPE_BUSY)) {
        toRet.append(
            "FREEBUSY;FBTYPE=BUSY:"); // default is BUSY, but let's be explicit about it and set
        // FBTYPE to BUSY
  ...
  public String toVCalendarAsVEvents() throws ServiceException {
    ...
    writer.append("PRODID:").append(ZCalendar.sCarbonioProdID).append(NL);
  ...
  public static class FBInstance implements Comparable<FBInstance> {
    ...
    public boolean equals(Object o) {
      ...
      return (mStartTime == other.mStartTime)
          ...
          && (mApptId == other.mApptId)
          && (mRecurIdDt == other.mRecurIdDt)
    ...
    while (propIter.hasNext()) {
      ...
      switch (tok) {
        ...
        case FREEBUSY:
          ...
          if (fbType != null) fbStatus = IcalXmlStrMap.sFreeBusyMap.toXml(fbType.getValue());
          ...
          for (String fbVal : vals) {
            ...
            intervals.add(
                new Interval(
                    period.getStart().getUtcTime(), period.getEnd().getUtcTime(), fbStatus));
          }
          break;
      }
    }

    if (name == null) throw ServiceException.INVALID_REQUEST("VFREEBUSY missing ORGANIZER", null);
