...
public class RemoteFreeBusyProvider extends FreeBusyProvider {
  ...
  public RemoteFreeBusyProvider(
      HttpServletRequest httpReq, ZimbraSoapContext zsc, long start, long end, String exApptUid) {
  ...
  public void addFreeBusyRequest(Request req) {
    ...
    if (buf == null) buf = new StringBuilder(req.email);
    else buf.append(",").append(req.email);
    mRemoteAccountMap.put(hostname, buf);
    mRequestList.add(req);
  }

  public void addFreeBusyRequest(
      Account requestor, Account acct, String id, long start, long end, int folder) {
    ...
    for (Request req : mRequestList) {
      ...
      try {
        ...
        if (req.folder != FreeBusyQuery.CALENDAR_FOLDER_ALL)
          targetUrl
              .append("&")
              .append(UserServlet.QP_FREEBUSY_CALENDAR)
              .append("=")
              .append(req.folder);
        try {
          if (mExApptUid != null)
        ...
        try {
          if (mSoapCtxt != null) authToken = mSoapCtxt.getAuthToken().getEncoded();
  ...
  public void addResults(Element response) {
    ...
    for (Map.Entry<String, StringBuilder> entry : mRemoteAccountMap.entrySet()) {
      ...
      try {
        ...
        if (acct == null) acct = prov.get(AccountBy.id, idStrs[0], mSoapCtxt.getAuthToken());
        if (acct != null) {
  ...
  protected Element proxyRequest(Element request, String acctId, ZimbraSoapContext zsc)
    ...
    Element response =
        DocumentHandler.proxyWithNotification(request.detach(), proxy, zscTarget, zsc);
    return response;
  }
}
