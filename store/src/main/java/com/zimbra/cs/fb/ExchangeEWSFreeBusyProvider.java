  ...
  static {
    ...
    factory =
        new ExchangeService(
            wsdlUrl,
            new QName(
                "http://schemas.microsoft.com/exchange/services/2006/messages", "ExchangeService"));
  }
  ...
  */
  ...
  private static class BasicUserResolver implements ExchangeUserResolver {
    ...
    public ServerInfo getServerInfo(String emailAddr) {
      ...
      if (url == null || user == null || pass == null || scheme == null) return null;
      ...
      try {
        ...
        if (emailAddr != null) {
          ...
          if (acct != null) {
            ...
            if (dom != null) val = dom.getAttr(attr, null);
            if (val != null) return val;
          }
        }
  ...
  BaseFolderType bindFolder(
      DistinguishedFolderIdNameType distinguishedFolderId, DefaultShapeNamesType shapeType) {
    ...
    nonEmptyArrayOfBaseFolderIdsType
        .getFolderIdOrDistinguishedFolderId()
    ...
    service.getFolder(
        getFolderRequest, mct, serverVersion, tzct, role, gfresponseHolder, gfversionInfo);
    ...
    if (firmtResp.getFolders().getFolderOrCalendarFolderOrContactsFolder().size() > 0) {
      ...
      ZimbraLog.fb.error(
          "Could not find the folder in Exchange : " + distinguishedFolderId.toString());
    }
  ...
  List<BaseFolderType> findFolderByProp(FolderIdType id, UnindexedFieldURIType prop, String val) {
    ...
    ieq.setPath(
        new JAXBElement<PathToUnindexedFieldType>(
    ...
    rtRestriction.setSearchExpression(
        new JAXBElement<SearchExpressionType>(
            new QName("http://schemas.microsoft.com/exchange/services/2006/types", "IsEqualTo"),
    ...
    ManagementRoleType role = null;
    service.findFolder(
        findFolderRequest, mct, serverVersion, tzct, role, findFolderResponse, gfversionInfo);
  ...
  List<BaseFolderType> findFolderByPartialProp(
      FolderIdType id, UnindexedFieldURIType prop, String val) {
    FindFolderType findFolderRequest = new FindFolderType();
    ...
    rtRestriction.setSearchExpression(
        new JAXBElement<SearchExpressionType>(
            new QName("http://schemas.microsoft.com/exchange/services/2006/types", "Contains"),
            SearchExpressionType.class,
            contains));
  ...
  List<ItemType> findItemByProp(
      FolderIdType id, UnindexedFieldURIType prop, String val, DefaultShapeNamesType shapeType) {
    ...
    service.findItem(
        ...
        serverVersion,
        tzct,
        DateTimePrecisionType.MILLISECONDS,
        role,
        fiResponse,
        gfversionInfo);
  ...
  public boolean handleMailboxChange(String accountId) {
    ...
    if (null == service) {
      try {
        ...
        ZimbraLog.fb.error(
            "exception while trying to initialize exchange service object " + serverInfo.url);
        ZimbraLog.fb.debug("Exiting handleMailboxChange() for account : " + accountId);
        return true;
      }
    }
    ...
    try {
      FolderType publicFolderRoot =
          (FolderType)
              bindFolder(
                  DistinguishedFolderIdNameType.PUBLICFOLDERSROOT,
      ...
      List<BaseFolderType> resultsNonIpm =
          findFolderByProp(
              publicFolderRoot.getParentFolderId(),
      ...
      if (resultsNonIpm != null && resultsNonIpm.size() > 0) {
        ...
        if (resultSchedulePlus != null && resultSchedulePlus.size() > 0) {
          ...
          List<BaseFolderType> resultFBFolder =
              findFolderByPartialProp(
                  folderSchedulePlus.getFolderId(),
                  UnindexedFieldURIType.FOLDER_DISPLAY_NAME,
          ...
          if (resultFBFolder != null && resultFBFolder.size() > 0) {
            ...
            List<ItemType> resultMessage =
                findItemByProp(
                    folderFB.getFolderId(),
                    UnindexedFieldURIType.ITEM_SUBJECT,
                    "USER-/CN=RECIPIENTS/CN=" + getForeignPrincipal(accountId),
                    DefaultShapeNamesType.ALL_PROPERTIES);
            if (resultMessage != null && resultMessage.size() > 0) {
              ...
              for (PathToExtendedFieldType pathExProp : props.keySet()) {
                ...
                sifItem.setPath(
                    new JAXBElement<PathToExtendedFieldType>(
                        new QName(
              ...
              tzct.setTimeZoneDefinition(tzdt);
              service.updateItem(
                  updateItemRequest, mct, serverVersion, tzct, updateItemResponse, gfversionInfo);
              ...
              tzct.setTimeZoneDefinition(tzdt);
              service.createItem(
                  createItemRequest, mct, serverVersion, tzct, createItemResponse, gfversionInfo);
            ...
            ZimbraLog.fb.error(
                "Could not find the Exchange folder containing '"
                    + serverInfo.org
                    + "'. Make sure zimbraFreebusyExchangeUserOrg is configured correctly and it"
                    + " exists on Exchange");
          }
        } else {
  ...
  public List<FreeBusy> getFreeBusyForHost(String host, ArrayList<Request> req) throws IOException {
    ...
    if (!serverInfo.enabled) return ret;
    ...
    try {
      ...
      // Request for highest hierarchy view. The rest hierarchy will be
      // Detailed->FreeBusy->MergedOnly->None
      ...
      service.getUserAvailability(
          availabilityRequest, tzct, serverVersion, availabilityResponse, gfversionInfo);
    ...
    for (Request re : req) {
      ...
      for (FreeBusyResponseType attendeeAvailability : results) {
        if (attendeeAvailability.getFreeBusyView() != null) {
          String fbResponseViewType =
              attendeeAvailability.getFreeBusyView().getFreeBusyViewType().get(0);
          String emailAddress = attendees.getMailboxData().get(i).getEmail().getAddress();
          ZimbraLog.fb.debug(
              "For user :%s free busy response type received is : %s",
              emailAddress, fbResponseViewType);

          if (re.email == emailAddress) {
            if (ResponseClassType.ERROR
                == attendeeAvailability.getResponseMessage().getResponseClass()) {
              ZimbraLog.fb.debug(
                  "Unable to fetch free busy for %s  error code %s :: %s",
                  emailAddress,
              ...
              if (attendeeAvailability
                  .getResponseMessage()
                  .getResponseCode()
                  .equals(ResponseCodeType.ERROR_NO_FREE_BUSY_ACCESS)) {
                npFreeBusy.mList.getHead().hasPermission = false;
              }
              ZimbraLog.fb.info(
                  "Error in response. continuing to next one sending nodata as response");
              i++;
              continue;
            }
            String fb = attendeeAvailability.getFreeBusyView().getMergedFreeBusy();
            ZimbraLog.fb.info(
                "Merged view Free Busy info received for user:%s is %s: ", emailAddress, fb);
            ...
            if (fb == null) {
              ...
              userIntervals.add(
                  new ExchangeFreeBusyProvider.ExchangeUserFreeBusy(
                      fb, re.email, fb_interval, startTime, endTime));
              ret.addAll(userIntervals);
            }

            // Parsing Detailed fb view response
            if ("DetailedMerged".equals(fbResponseViewType)
                || "FreeBusyMerged".equals(fbResponseViewType)) {

              parseDetailedFreeBusyResponse(
                  emailAddress, startTime, endTime, attendeeAvailability, userIntervals);
              ret.addAll(userIntervals);
            } else { // No FreeBusy view information available. returning nodata freebusy in
              // response

              ZimbraLog.fb.debug(
                  "No Free Busy view info avaiable for [%s], free busy view type from response :"
                      + " %s",
                  emailAddress, fbResponseViewType);
   ...
   */
  private static void parseDetailedFreeBusyResponse(
      String name,
      long start,
      long end,
      FreeBusyResponseType freeBusyResponse,
      ArrayList<FreeBusy> ret) {
          ...
          for (CalendarEvent event : calendarEvents) {
            ...
            ZimbraLog.fb.debug(
                "For user %s FB data received is: legacyType : %s, startTime : %s, "
                    + "endTime : %s",
                name, legacyType, event.getStartTime(), event.getEndTime());
            if (event.getCalendarEventDetails() != null && interval != null) {
              ...
              ZimbraLog.fb.debug(
                  "eventID : %s, location : %s, subject : %s, isMeeting : %b, "
                  ...
                  interval.isException,
                  interval.isReminderSet,
                  interval.isPrivate);
            } else {
   ...
   * @return
   */
  private static FreeBusy.Interval getFreeBusyInterval(
      ArrayList<FreeBusy> ret, CalendarEvent event) {
  ...
  public void addFreeBusyRequest(Request req) throws FreeBusyUserNotFoundException {
    ...
    for (ExchangeUserResolver resolver : sRESOLVERS) {
      ...
      if (req.requestor != null) email = req.requestor.getName();
      info = resolver.getServerInfo(email);
      if (info != null) {
        if (!info.enabled) throw new FreeBusyUserNotFoundException();
        if (null == service) {
          try {
            initService(info);
          } catch (MalformedURLException e) {
    ...
    if (info == null) throw new FreeBusyUserNotFoundException();
    addRequest(info, req);
  }
  ...
  private long getTimeInterval(String attr, String accountId, long defaultValue)
  ...
  public long cachedFreeBusyStartTime(String accountId) {
    ...
    try {
      long dur =
          getTimeInterval(Provisioning.A_zimbraFreebusyExchangeCachedIntervalStart, accountId, 0);
    ...
    if (cal.get(Calendar.YEAR) < curYear) {
      // Exchange accepts FB info for only one calendar year. If the start date falls in the
      // previous year
  ...
  public long cachedFreeBusyEndTime(String accountId) {
    ...
    try {
      duration =
          getTimeInterval(Provisioning.A_zimbraFreebusyExchangeCachedInterval, accountId, duration);
    } catch (ServiceException se) {
    }
  ...
  protected String getForeignPrincipal(String accountId) throws ServiceException {
    String ret = null;
    Account acct = Provisioning.getInstance().get(Key.AccountBy.id, accountId);
  ...
  public boolean registerForMailboxChanges(String accountId) {
    if (sRESOLVERS.size() > 1) return true;
    String email = null;
    try {
      Account account = null;
      if (accountId != null) account = Provisioning.getInstance().getAccountById(accountId);
      if (account != null) email = account.getName();
    } catch (ServiceException se) {
