...
public class WorkingHours {
  ...
  private static String invertStatus(String status) {
    if (IcalXmlStrMap.FBTYPE_FREE.equals(status)) return IcalXmlStrMap.FBTYPE_BUSY_UNAVAILABLE;
    else return IcalXmlStrMap.FBTYPE_FREE;
  }
  ...
  private static class DayOfWeekName {
    private static ZWeekDay[] sNames =
        new ZWeekDay[] {
    ...
    public static String lookup(int dayOfWeek) throws ServiceException {
      if (dayOfWeek >= 1 && dayOfWeek <= sNames.length) return sNames[dayOfWeek - 1].toString();
  ...
  private static class HoursByDay {
    ...
    public HoursByDay(String prefStr) throws ServiceException {
      if (!prefStr.matches("[^\\s]+"))
        throw ServiceException.INVALID_REQUEST(
            "Working hours spec should not have whitespaces", null);
      if (prefStr.endsWith(","))
        throw ServiceException.INVALID_REQUEST(
            "Working hours spec should not have trailing commas", null);

      int daySpecified[] =
          new int[] {0, 0, 0, 0, 0, 0, 0}; // tracks which days of the week are specified

      String days[] = prefStr.split(",");
      if (days.length != 7)
        throw ServiceException.INVALID_REQUEST(
            "Working hours spec must specify all days of a week", null);
      for (int i = 0; i < days.length; ++i) {
        if (days[i].endsWith(":"))
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec day section \"" + days[i] + "\" should not have trailing colons",
              null);
        ...
        if (parts.length != 4)
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec day section \""
                  + days[i]
                  + "\" must have 4 colon-separated parts",
              null);
        ...
        if (dayNum < 1 || dayNum > 7)
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec day section \""
                  + days[i]
                  + "\" has invalid day number (must be 1 to 7)",
              null);

        // Don't allow specifying the same day twice.
        if (daySpecified[dayNum - 1] != 0)
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec must not specify the same day more than once; found repeated day "
                  + dayNum,
              null);
        daySpecified[dayNum - 1] = 1;

        // Second part is a flag indicating if the working hours for the day are in effect ("Y") or
        // not ("N").
        if (parts[1].length() != 1)
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec day section \""
                  + days[i]
                  + "\" has invalid on/off flag (must be Y or N)",
              null);
        ...
        // Third part is the start time of the working hours.  Format is HHMM with 24-hour hour.
        // Range is 0000 to 2359.
        ...
        if (startTime.laterThan(endTime))
          throw ServiceException.INVALID_REQUEST(
              "Working hours spec day section \""
                  + days[i]
                  + "\" has end time earlier than start time",
              null);
  ...
  private static class StartSpec {
    ...
    /**
     * Return the YYYYMMDD string for the first day matching the dayOfWeek on or after the start
     * time.
     *
     * @param dayOfWeek
     */
