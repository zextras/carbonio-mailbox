...
public class LocalFreeBusyProvider {
   ...
   * @param start
   * @param end
   * @param folder folder to run free/busy search on; FreeBusyQuery.CALENDAR_FOLDER_ALL (-1) for all
   *     folders
   * @param exAppt appointment to exclude; calculate free/busy assuming the specified appointment
   *     wasn't there
   * @return
  ...
  public static FreeBusy getFreeBusyList(
    ...
    boolean accountAceAllowed =
        accessMgr.canDo(authAcct, mbox.getAccount(), User.R_viewFreeBusy, asAdmin);
    ...
    if (folder == FreeBusyQuery.CALENDAR_FOLDER_ALL) {
      calDataResultList =
          mbox.getAllCalendarsSummaryForRange(null, MailItem.Type.APPOINTMENT, start, end);
    } else {
      calDataResultList = new ArrayList<CalendarDataResult>(1);
      calDataResultList.add(
          mbox.getCalendarSummaryForRange(null, folder, MailItem.Type.APPOINTMENT, start, end));
    }
    for (CalendarDataResult result : calDataResultList) {
      ...
      if ((f.getFlagBitmask() & Flag.BITMASK_EXCLUDE_FREEBUSY) != 0) {
        ZimbraLog.fb.debug(
            "Calendar '%s' id=%s ignored - has EXCLUDE_FREEBUSY flag set", f.getName(), folderId);
        continue;
      }
      ...
      if (folderFBAllowed) ++numAllowedFolders;
      if (!folderFBAllowed && !accountAceAllowed) {
        ZimbraLog.fb.debug(
            "Calendar '%s' id=%s ignored - folderFBAllowed=%s accountAceAllowed=%s",
      ...
      for (Iterator<CalendarItemData> iter = result.data.calendarItemIterator(); iter.hasNext(); ) {
        ...
        for (Iterator<InstanceData> instIter = appt.instanceIterator(); instIter.hasNext(); ) {
          ...
          // Skip instances that are outside the time range but were returned due to alarm being in
          // range.
          if (instStart >= end) continue;
          long dur = defaultDuration;
          if (instance.getDuration() != null) dur = instance.getDuration().longValue();
          ...
          if (instance instanceof FullInstanceData) {
            ...
            if (IcalXmlStrMap.TRANSP_TRANSPARENT.equals(transpInst)) continue;
          } else if (isTransparent) {
            continue;
          }

          String freeBusy = instance.getFreeBusyActual();
          if (freeBusy == null) freeBusy = defaultFreeBusy;
    ...
    if (!accountAceAllowed
        && numAllowedFolders == 0
        && !LC.freebusy_disable_nodata_status.booleanValue()) {
