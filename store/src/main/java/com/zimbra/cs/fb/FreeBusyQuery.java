...
public class FreeBusyQuery {
  ...
  public FreeBusyQuery(
      HttpServletRequest httpReq, Account requestor, long start, long end, String exApptUid) {
    mReq = httpReq;
  ...
  private Account getAccountFromUid(String uid) {
    ...
    Account acct = null;
    try {
      if (Provisioning.isUUID(uid)) acct = prov.get(AccountBy.id, uid);
      else acct = prov.get(AccountBy.name, uid);
  ...
  private void prepareRequests(
      ArrayList<FreeBusy> local, RemoteFreeBusyProvider remote, ArrayList<String> external) {
    for (String id : mTargets.keySet()) {
      Account acct = mTargets.get(id);
      try {
        if (acct == null
            || acct.isAccountExternal()
        ...
        if (Provisioning.getInstance().onLocalServer(acct)) {
          ...
          else if (mRequestor != null) octxt = new OperationContext(mRequestor);
          else octxt = new OperationContext(GuestAccount.ANONYMOUS_ACCT);
          Appointment exAppt = null;
          if (mExApptUid != null) {
            CalendarItem ci = mbox.getCalendarItemByUid(octxt, mExApptUid);
            if (ci instanceof Appointment) exAppt = (Appointment) ci;
          }
  ...
  public Collection<FreeBusy> getResults() {
    ...
    if (external.size() > 0)
      fbList.addAll(
          FreeBusyProvider.getRemoteFreeBusy(
              mRequestor, external, mStart, mEnd, CALENDAR_FOLDER_ALL, getHopcount()));
    return fbList;
  }
  ...
  public void getResults(Element response) {
    ...
    if (external.size() > 0)
      FreeBusyProvider.getRemoteFreeBusy(
          mRequestor, response, external, mStart, mEnd, CALENDAR_FOLDER_ALL, getHopcount());
  }
}
