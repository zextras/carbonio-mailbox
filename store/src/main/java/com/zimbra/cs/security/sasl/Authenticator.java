...
public abstract class Authenticator {
  ...
  private static final Map<String, AuthenticatorFactory> mRegisteredMechanisms =
      new LinkedHashMap<String, AuthenticatorFactory>();
  private static Collection<String> mMechanismList = Collections.emptyList();

  static {
    registerMechanism(
        PlainAuthenticator.MECHANISM,
  ...
  public static Authenticator getAuthenticator(String mechanism, AuthenticatorUser authUser) {
    ...
    if (authFactory == null) return null;
  ...
  /**
   * Whether this Authenticator is valid for the server/protocol/etc. as constrained by its
   * AuthenticatorUser.
   */
  ...
  public abstract Account authenticate(
      String username,
  ...
  protected Account authorize(Account authAccount, String username, boolean asAdmin)
      throws ServiceException {
    if (username == null || username.length() == 0) return authAccount;
    ...
    if (userAcct == null) {
      // if username not found, check username again using the domain associated with the
    ...
    if (userAcct == null) {
      ZimbraLog.account.info(
          "authorization failed for " + username + " (account not found)", username);
      return null;
    }

    // check whether the authenticated user is able to access the target
    if (!authAccount.getId().equals(userAcct.getId())
        && !AccessManager.getInstance().canAccessAccount(authAccount, userAcct, asAdmin)) {
