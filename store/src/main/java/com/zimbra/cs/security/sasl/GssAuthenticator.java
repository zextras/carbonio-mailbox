...
public class GssAuthenticator extends Authenticator {
  ...
  protected boolean isSupported() {
    ...
    if (!GSS_ENABLED && !mAuthUser.isGssapiAvailable()) return false;
    ...
    try {
      ...
      ZimbraLog.security.warn(
          "could not determine whether TLS encryption is required for GSSAPI auth; defaulting to"
              + " FALSE",
          e);
      return false;
    }
                  ...
                  new PrivilegedExceptionAction<Object>() {
                    ...
                    public Object run() throws SaslException {
                      return Sasl.createSaslServer(
                          getMechanism(), getProtocol(), host, props, new GssCallbackHandler());
                    }
                  });
  ...
  public Account authenticate(
    ...
    if (!isProtocolEnabled(authAccount, protocol)) {
      ZimbraLog.account.info(
          "Authentication failed - %s not enabled for %s", protocol, authAccount.getName());
      return null;
    }

    Account targetAcct = authorize(authAccount, username, true);
    if (targetAcct != null) prov.accountAuthed(authAccount);
    return targetAcct;
  }
  ...
  static void debug(String format, Object... args) {
    if (DEBUG) System.out.printf("[DEBUG GssAuthenticator] " + format + "\n", args);
  }
}
