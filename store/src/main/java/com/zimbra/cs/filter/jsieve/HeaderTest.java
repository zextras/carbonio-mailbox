...
public class HeaderTest extends Header {
    ...
    while (!stop && argumentsIter.hasNext()) {
      ...
      if (argument instanceof TagArgument) {
        ...
        // [MATCH-TYPE]?
          ...
          throw context
              .getCoordinate()
              .syntaxException("Found unexpected TagArgument: \"" + tag + "\"");
        }
      } else {
  ...
  protected boolean match(
    ...
    if (HeaderConstants.COUNT.equalsIgnoreCase(matchType)) {
      ...
      for (final String key : keys) {
        isMatched =
            ZimbraComparatorUtils.counts(mail, comparator, operator, headerValues, key, context);
        if (isMatched) {
          break;
        }
      }
      ...
      while (!isMatched && headerNamesIter.hasNext()) {
        ...
        if (headerValues.isEmpty()) {
          ...
          while (!isMatched && headerValuesIter.hasNext()) {
            ...
            for (final String key : keys) {
              isMatched =
                  ZimbraComparatorUtils.match(
                      mail, comparator, matchType, operator, headerValue, key, context);
              if (isMatched) {
                break;
              }
            }
  ...
  /**
   * Matches the wildcard ("?" and "*") in the Address, Envelope, Header or String test, and stores
   * one string for each wildcard in the variable ${n} (n=1,2,...,9).
   *
   * @param mailAdapter triggering mail object
   * @param sourceNames target header name or literal value to be matched
   * @param sourceType where the value should be looked up, message header, envelope, or use as
   *     literal
  ...
  public static void evaluateVarExp(
      ZimbraMailAdapter mailAdapter,
      List<String> sourceNames,
      SourceType sourceType,
      List<String> keys)
      throws SieveMailException, SievePatternException, MessagingException {
    ...
    for (Object obj : sourceNames) {
      ...
      for (String sourceStr : values) {
        for (Object key : keys) {
          ...
          Matcher matcher =
              Pattern.compile(regex, Pattern.CASE_INSENSITIVE | Pattern.DOTALL).matcher(sourceStr);
