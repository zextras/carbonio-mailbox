...
public class ZimbraAsciiNumeric extends AsciiNumeric implements ZimbraComparator {
  ...
  public boolean values(String operator, String lhs, String rhs) throws FeatureException {
    if (getNumericType(lhs) == NumericType.NEGATIVE_NUMBER
        || getNumericType(rhs) == NumericType.NEGATIVE_NUMBER) {
   ...
   * </ul>
   *
   * @param operator Operator name (HeaderConstants.GT_OP, HeaderConstants.GE_OP,
   *     HeaderConstants.LT_OP, HeaderConstants.LE_OP, HeaderConstants.EQ_OP, HeaderConstants.NE_OP)
   ...
   * org.apache.jsieve.comparators.AsciiNumeric.isPositiveInfinity(String) It accepts an empty
   * string as well.
   *
   * @param value not null
   * @return true when the value should represent positive infinity, false otherwise
   */
   ...
   * @return true when the given character is an ASCII digit, false otherwise
   */
   ...
   *   <li>If the value is empty or does not starts with number, then it is positive infinity (RFC
   *       4790 Section 9.1.1)
  ...
  /**
   * Unsupported operation. <a href='http://tools.ietf.org/html/rfc5228#section-2.7.1'>RFC5228</a>
   * limits support to comparators that support <code>:contains</code>. <a
   * href='http://tools.ietf.org/html/rfc4790#section-9.1.1'>RFC4790</a> states that substring
