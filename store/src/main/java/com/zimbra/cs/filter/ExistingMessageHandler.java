...
public final class ExistingMessageHandler implements FilterHandler {
  ...
  private void updateTagsAndFlagsIfNecessary(
      Message msg, Collection<ActionFlag> flagActions, String[] newTags) throws ServiceException {
    ...
    if (!Sets.newHashSet(existingTags).equals(Sets.newHashSet(tags))
        || msg.getFlagBitmask() != flags) {
      ZimbraLog.filter.info(
          "Updating flags to %d, tags to %s on message %d.", flags, tags, msg.getId());
  ...
  private boolean isLocalExistingFolder(String folderPath) throws ServiceException {
    Pair<Folder, String> folderAndPath =
        mailbox.getFolderByPathLongestMatch(octxt, Mailbox.ID_FOLDER_USER_ROOT, folderPath);
  ...
  public void redirect(String destinationAddress) {
    ZimbraLog.filter.debug(
        "Ignoring attempt to redirect existing message %d to %s.", messageId, destinationAddress);
  }
  ...
  public void ereject(LmtpEnvelope envelope) {
    ZimbraLog.filter.debug(
        "Ignoring attempt to perform 'ereject' command for existing message %d", messageId);
  }

  @Override
  public void notifyMailto(
    ...
    ZimbraLog.filter.debug(
        "Ignoring attempt to notify (RFC 5435/5436) for existing message %d", messageId);
  }
