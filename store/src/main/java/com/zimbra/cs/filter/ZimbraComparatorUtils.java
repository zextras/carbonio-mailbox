...
public class ZimbraComparatorUtils {
   ...
   *
   * @see org.apache.jsieve.tests.AbstractComparatorTest#executeBasic(MailAdapter mail, Arguments
   *     arguments, SieveContext context)
   * @param mail
   * @param arguments
   * @param context
   * @return TestParameters
   */
  public static TestParameters parseTestArguments(
      MailAdapter mail, Arguments arguments, SieveContext context) throws SieveException {
    ...
    TestParameters result =
        new TestParameters(mail, addressPart, comparator, matchType, operator, headerNames, keys);
    return result;
  }
   ...
   *
   * @param comparatorName comparator name (ASCII_CASEMAP_COMPARATOR, ASCII_NUMERIC_COMPARATOR,
   *     OCTET_COMPARATOR)
  ...
  public static boolean match(
      ...
      String operator,
      String matchTarget,
      String matchArgument,
      SieveContext context)
      throws SieveException {
    boolean isMatched = false;
    if (mail instanceof ZimbraMailAdapter
        && ASCII_NUMERIC_COMPARATOR.equalsIgnoreCase(comparatorName)) {
    ...
    if (IS_TAG.equals(matchType)) {
      ...
      isMatched = values(mail, comparatorName, operator, matchTarget, matchArgument, context);
    }
    return isMatched;
  }
   ...
   * @throws SyntaxException thrown when "relational" and "comparator-i;ascii-numeric" are not
   *     declared in the "require" command.
   */
  public static boolean counts(
      ...
      String matchArgument,
      SieveContext context)
      throws LookupException, FeatureException, SyntaxException {
    if (mail instanceof ZimbraMailAdapter) {
  ...
  public static class TestParameters {
    ...
    public void setMail(MailAdapter mail) {
      this.mail = mail;
    }
  ...
  public static boolean is(
      String comparatorName, String string1, String string2, SieveContext context)
      throws LookupException, FeatureException, SyntaxException {
  ...
  public static String getComparator(String comparator, String matchType) {
    if (null == comparator) {
      if (HeaderConstants.COUNT.equalsIgnoreCase(matchType)
          || HeaderConstants.VALUE.equalsIgnoreCase(matchType)) {
