...
/** Mail filtering implementation for messages that arrive via LMTP or from an external account. */
public final class IncomingMessageHandler implements FilterHandler {
  ...
  public ItemId fileInto(String folderPath, Collection<ActionFlag> flagActions, String[] tags)
    ...
    ItemId id =
        FilterUtil.addMessage(
            ...
            folderPath,
            false,
            FilterUtil.getFlagBitmask(flagActions, Flag.BITMASK_UNREAD),
            tags,
    ...
    try {
      ...
      ZimbraLog.filter.debug(
          "Unable to do spam training for message %s because folder path %s does not exist.",
          id, folderPath);
  ...
  public Message implicitKeep(Collection<ActionFlag> flagActions, String[] tags)
      throws ServiceException {
    int folderId =
        mailbox.getAccount().isFeatureAntispamEnabled() && SpamHandler.isSpam(getMimeMessage())
    ...
    try {
      DeliveryOptions dopt =
          new DeliveryOptions()
              ...
              .setNoICal(noICal)
              .setRecipientEmail(recipientAddress);
  ...
  @Override
  public void redirect(String destinationAddress) throws ServiceException {
