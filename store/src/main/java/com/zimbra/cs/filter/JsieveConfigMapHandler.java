...
/**
 * Handler class for jSieve's configuration map, such as CommandMap & TestMap. These are registered
 * to Configuration Manger to create sieve factory.
 */
public class JsieveConfigMapHandler {
  ...
  private static Map<String, String> createDefaultCommandMap() {
    ...
    Map<String, String> mCommandMap = Collections.synchronizedMap(new HashMap<String, String>());
    ...
    mCommandMap.put(
        "zimbravariablesctrl", com.zimbra.cs.filter.jsieve.ZimbraVariablesCtrl.class.getName());
  ...
  private static Map<String, String> createDefaultTestMap() {
    ...
    Map<String, String> mTestMap = Collections.synchronizedMap(new HashMap<String, String>());
    ...
    mTestMap.put(
        "current_day_of_week", com.zimbra.cs.filter.jsieve.CurrentDayOfWeekTest.class.getName());
    ...
    mTestMap.put(
        ...
        com.zimbra.cs.filter.jsieve.CommunityConnectionsTest.class.getName());
    mTestMap.put(
        "community_requests", com.zimbra.cs.filter.jsieve.CommunityRequestsTest.class.getName());
    mTestMap.put(
        "community_content", com.zimbra.cs.filter.jsieve.CommunityContentTest.class.getName());
    mTestMap.put(CAPABILITY_RELATIONAL, com.zimbra.cs.filter.jsieve.RelationalTest.class.getName());
    mTestMap.put(
        CAPABILITY_COMPARATOR_NUMERIC,
        com.zimbra.cs.filter.jsieve.ComparatorNumericTest.class.getName());
    ...
    mTestMap.put(
        "valid_notify_method", com.zimbra.cs.filter.jsieve.ValidNotifyMethodTest.class.getName());
    mTestMap.put(
  ...
  /**
   * Register action name with action class name of that. This is supposed to be invoked from the
   * init() method of ZimbraExtension.
   */
  public static void registerCommand(String actionName, String actionClassName) {
    ...
    if (registeredClassName != null) {
      ...
      ZimbraLog.filter.warn(
          "action name "
              + actionName
              + " is already registered as action. registered "
              + registeredClassName
              + " is overwritten with "
              + actionClassName);
    }
