...
public class SetVariable extends AbstractCommand {
  ...
  public static final Pattern VALID_IDENTIFIER_PATTERN =
      Pattern.compile("([\\p{Alpha}]|_)+([\\p{Alnum}|_])*", Pattern.CASE_INSENSITIVE);
  ...
  protected void validateArguments(Arguments arguments, SieveContext context)
    ...
    if (args.size() >= 2) {
          ...
          if (!isValidModifier(tagValue)) {
            ...
            if (StringUtil.isNullOrEmpty(operations[index])) {
              ...
              throw new SyntaxException(
                  "Cannot use two or more modifiers of the same"
                      + " precedence in a single \"set\" action. Modifiers used: "
                      + tagValue
                      + " and "
                      + operations[index]);
            }
      ...
      throw new SyntaxException(
          "Minimum 2 arguments are needed. Usage: set :upperfirst \"b\" \"hello\";. Arguments"
              + " found: "
              + arguments);
    }
  }
  ...
  private boolean isValidModifier(String modifier) {
    ...
    if (temp.equals(ALL_LOWER_CASE)
        || temp.equals(ALL_UPPER_CASE)
        || temp.equals(LOWERCASE_FIRST)
        || temp.equals(UPPERCASE_FIRST)
        || temp.equals(QUOTE_WILDCARD)
        || temp.equals(STRING_LENGTH)
