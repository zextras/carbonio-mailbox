...
public final class SoapToSieve {
  ...
  public static final String requireCommon =
      "\"fileinto\", \"copy\", \"reject\", \"tag\", \"flag\", "
  ...
  private void handleRule(FilterRule rule, boolean isAdminScript) throws ServiceException {
    ...
    if (filterActions == null
        && child
            == null) { // if there is no action in this rule, filterActions is supposed to be null.
  ...
  private String handleNest(String baseIndents, NestedRule currentNestedRule, boolean isAdminScript)
    ...
    if (currentNestedRule.getChild() != null) {
      nestedIfBlock.append(
          handleNest(baseIndents + "    ", currentNestedRule.getChild(), isAdminScript));
    }

    if (childActions == null
        && currentNestedRule.getChild()
            == null) { // if there is no action in this rule, childActions is supposed to be null.
      // If there is no more nested rule, there should be at least one action.
      throw ServiceException.INVALID_REQUEST("Missing action", null);
    }
  ...
  private String handleTest(FilterTest test) throws ServiceException {
    ...
    if (test instanceof FilterTest.HeaderTest) {
      ...
      Sieve.NumberComparison comp =
          Sieve.NumberComparison.fromString(sizeTest.getNumberComparison());
      ...
      String format =
          bodyTest.isCaseSensitive()
              ? "body :contains :comparator \"i;octet\" \"%s\""
              : "body :contains \"%s\"";
      ...
      Sieve.DateComparison comparison =
          Sieve.DateComparison.fromString(timeTest.getDateComparison());
      ...
      if (time.length() != timeFormat.length()) {
        throw ServiceException.INVALID_REQUEST(
            "Time string must be of length " + timeFormat.length(), null);
      }
  ...
  private static String toSieve(FilterTest.HeaderTest test) throws ServiceException {
    ...
    if (StringUtils.isNotEmpty(test.getValueComparison())) {
      ...
      Sieve.Comparator valueComparisonComparator =
          Sieve.Comparator.fromString(test.getValueComparisonComparator());
      return toSieve(
          "header",
          header,
          comp,
  ...
  private static String toSieve(
      String name, String header, Sieve.StringComparison comp, boolean caseSensitive, String value)
      throws ServiceException {
    checkValue(comp, value);
    String format =
        caseSensitive
            ? "%s :%s :comparator \"" + Sieve.Comparator.ioctet + "\" %s \"%s\""
            : "%s :%s %s \"%s\"";
  ...
  private static String toSieve(
      String name,
      String header,
      Sieve.ValueComparison comp,
      Sieve.Comparator valueComparator,
    ...
    if (valueComparator == null) {
      ...
      // for :value, if comparator value is not set in input, iasciinumeric comparator will be used
      // if value is numeric
    ...
    if (part == null) {
      ...
      String format =
          "%s " + countOrVal + " \"%s\" :%s :comparator \"" + comparator + "\" %s \"%s\"";
  ...
  private static String formatAddress(FilterTest.AddressTest test, String testName)
      throws ServiceException {
  ...
  private String handleAction(FilterAction action, boolean isAdminScript) throws ServiceException {
    if (action instanceof FilterAction.KeepAction) {
      ...
      return new StringBuilder("notify ")
          .append(StringUtil.enclose(FilterUtil.escape(emailAddr), '"'))
          .append(' ')
          .append(StringUtil.enclose(subjectTemplate, '"'))
          .append(' ')
          .append("text:\r\n")
          .append(getDotStuffed(bodyTemplate))
          .append("\r\n.\r\n")
          .append(maxBodyBytes < 0 ? "" : " " + maxBodyBytes)
          .append(origHeaders.isEmpty() ? "" : " " + getSieveMultiValue(origHeaders))
          .toString();
      ...
      if (!from.isEmpty()) {
        filter
            .append(":from ")
            .append(StringUtil.enclose(FilterUtil.escape(from), '"'))
            .append(' ');
      }
      ...
      if (StringUtil.isNullOrEmpty(addheaderAction.getHeaderName())
          || StringUtil.isNullOrEmpty(addheaderAction.getHeaderValue())) {
        throw ServiceException.PARSE_ERROR(
      ...
      FilterAction.ReplaceheaderAction replaceheaderAction =
      ...
      if (!StringUtil.isNullOrEmpty(replaceheaderAction.getNewValue())) {
        sb.append(" :newvalue ")
            .append("\"")
            .append(replaceheaderAction.getNewValue())
            .append("\"");
      }
  ...
  private static String handleRejectAction(FilterAction.RejectAction rejectAction)
      throws ServiceException {
