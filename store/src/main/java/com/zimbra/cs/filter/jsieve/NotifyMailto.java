...
/**
 * Class NotifyMailto implements the Notify Command as defined in RFC 5435 (notify action) and RFC
 * 5436 (mailto method). This 'notify' action will be turned on only when the global configuration
 * zimbraSieveNotifyActionRFCCompliant is set to TRUE.
 */
public class NotifyMailto extends Notify {
  ...
  /**
   * Parses the arguments of the 'notify' action, and adds an ActionNotifyMailto to the List of
   * Actions.
   */
  @Override
   ...
   */
  private Object execute(MailAdapter mail, Arguments arguments, SieveContext context)
      throws SyntaxException {
    if (!(mail instanceof ZimbraMailAdapter)) {
      return null;
    }
    ...
    while (!stop && argumentsIter.hasNext()) {
      ...
      if (argument instanceof TagArgument) {
        ...
        if (from == null && NOTIFY_FROM.equals(tag)) {
            ...
            if (argument instanceof StringListArgument) {
              ...
              if (!(importance == 1 || importance == 2 || importance == 3)) {
                throw new SyntaxException(
                    "Expecting an integer number (1, 2, 3) for " + NOTIFY_IMPORTANCE);
              }
            } else {
          ...
          // The next argument must be a string-list of options
    ...
    mail.addAction(
        new ActionNotifyMailto(from, options, importance, message, mailto, mailtoParams));
    return null;
  }
