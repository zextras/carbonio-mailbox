...
public class DeleteHeader extends AbstractCommand {
  ...
  protected Object executeBasic(
      MailAdapter mail, Arguments args, Block block, SieveContext sieveContext)
    ...
    if (EditHeaderExtension.isImmutableHeaderKey(ehe.getKey(), mailAdapter)) {
      ZimbraLog.filter.info(
          "deleteheader: %s is immutable header, so exiting silently.", ehe.getKey());
      return null;
    }
    ...
    try {
      ...
      throw new OperationException(
          "deleteheader: Error occured while fetching all headers from mime.", e);
    }
    ...
    if (headerCount < 1) {
      ZimbraLog.filter.info(
          "deleteheader: No headers found matching with \"%s\" in mime.", ehe.getKey());
      return null;
    }
          ...
          if (ehe.getIndex() == null || (ehe.getIndex() != null && ehe.getIndex() == matchIndex)) {
            if (ehe.getValueList() == null || ehe.getValueList().isEmpty()) {
              ...
              for (String value : ehe.getValueList()) {
                deleteCurrentHeader =
                    ehe.matchCondition(
                        mailAdapter, header, matchingeHeaderList, value, sieveContext);
