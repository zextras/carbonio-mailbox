...
/**
 * SIEVE test whether the conversation which the message is about to belong to is what the user
 * started or has participated in. Conversations the user started are for which the oldest remaining
 * message sorted by (date, id) has FROM_ME flag AND it's not a reply. Conversations the user has
 * participated are for which there is at least one message with FROM_ME flag. These are all based
 * on what the user currently has in its mailbox. Deleted messages are irrelevant.
 *
 * @author ysasaki
 */
      ...
      case STARTED:
        for (Conversation conv : convs) {
          if ((conv.getFlagBitmask() & Flag.BITMASK_FROM_ME) > 0) {
            try {
              List<Message> msgs =
                  mbox.getMessagesByConversation(null, conv.getId(), SortBy.DATE_ASC, 1);
              if (!msgs.isEmpty()) {
                Message msg = msgs.get(0);
                // the oldest message in the conversation is FROM_ME, but conversation can't be
                // started
                // by REPLY, which is likely that the user has deleted messages older than the
                // remaining
                // oldest.
