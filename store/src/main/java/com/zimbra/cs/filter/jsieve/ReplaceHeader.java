...
public class ReplaceHeader extends AbstractCommand {
   ...
   *
   * @see
   *     org.apache.jsieve.commands.AbstractCommand#executeBasic(org.apache.jsieve.mail.MailAdapter,
   *     org.apache.jsieve.Arguments, org.apache.jsieve.Block, org.apache.jsieve.SieveContext)
   */
  @SuppressWarnings("unchecked")
  @Override
  protected Object executeBasic(
      MailAdapter mail, Arguments arguments, Block block, SieveContext sieveContext)
    ...
    if (EditHeaderExtension.isImmutableHeaderKey(ehe.getKey(), mailAdapter)) {
      ZimbraLog.filter.info(
          "replaceheader: %s is immutable header, so exiting silently.", ehe.getKey());
      return null;
    }
    ...
    try {
      ...
      throw new OperationException(
          "replaceheader: Error occured while fetching all headers from mime.", e);
    }
    ...
    if (headerCount < 1) {
      ZimbraLog.filter.info(
          "replaceheader: No headers found matching with \"%s\" in mime.", ehe.getKey());
      return null;
    }
    ...
    try {
        ...
        if (header.getName().equalsIgnoreCase(ehe.getKey())) {
          ...
          if (ehe.getIndex() == null || (ehe.getIndex() != null && ehe.getIndex() == matchIndex)) {
            ZimbraLog.filter.debug(
                "replaceheader: header before processing\n%d  %s: %s",
                matchIndex, header.getName(), header.getValue());
            for (String value : ehe.getValueList()) {
              ZimbraLog.filter.debug("replaceheader: working with %s value", value);
              replace =
                  ehe.matchCondition(mailAdapter, header, matchingeHeaderList, value, sieveContext);
              if (replace) {
                ...
                if (ehe.getNewValue() != null) {
                  ...
                  newHeaderValue =
                      MimeUtility.fold(
                          newHeaderName.length() + 2,
                          MimeUtil.encodeWord(newHeaderValue, null, null, true));
                } else {
      ...
      throw new OperationException(
          "replaceheader: Error occured while encoding header value.", uee);
    }
    return null;
  }
   ...
   * (non-Javadoc)
   *
   * @see org.apache.jsieve.commands.AbstractCommand#validateArguments(org.apache.jsieve.Arguments,
  ...
  protected void validateArguments(Arguments arguments, SieveContext context)
    ...
    if (null != ehe.getValueList()) {
      ZimbraLog.filter.debug(
          "replaceheader: header values in sieve script = %s",
    ...
    if (ehe.getNewName() != null) {
      ZimbraLog.filter.debug(
          "replaceheader: new header name in sieve script = %s", ehe.getNewName());
      if (!CharsetUtil.US_ASCII.equals(
          CharsetUtil.checkCharset(ehe.getNewName(), CharsetUtil.US_ASCII))) {
        throw new SyntaxException(
            "replaceheader: newname must be printable ASCII only in replaceheader.");
      }
    }
    if (ehe.getNewValue() != null) {
      ZimbraLog.filter.debug(
          "replaceheader: new header vlaue in sieve script = %s", ehe.getNewValue());
    }
