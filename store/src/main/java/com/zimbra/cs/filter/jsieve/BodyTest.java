...
public class BodyTest extends AbstractTest {
  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (argumentsIter.hasNext()) {
      ...
      if (argument instanceof TagArgument) {
        ...
        if (tag.equals(CONTAINS)) comparison = tag;
        else throw new SyntaxException("Found unexpected TagArgument: \"" + tag + "\"");
      }
    }
    if (null == comparison) throw new SyntaxException("Expecting \"" + CONTAINS + "\"");
        ...
        if (tag.equals(COMPARATOR)) {
          if (argumentsIter.hasNext()) {
            ...
            if (argument instanceof StringListArgument) {
              ...
              if (argumentsIter.hasNext()) argument = argumentsIter.next();
            } else {
    ...
    if (null == key) throw new SyntaxException("Expecting a string");

    // There MUST NOT be any further arguments
    if (argumentsIter.hasNext()) throw new SyntaxException("Found unexpected argument(s)");
  ...
  private boolean contains(Reader reader, boolean caseSensitive, String substring)
      throws IOException {
    int matchIndex = 0;
    if (!caseSensitive) substring = substring.toLowerCase();
    ...
    while ((c = getNextChar(pb)) > 0) {
      if ((!caseSensitive && substring.charAt(matchIndex) == Character.toLowerCase(c))
        ...
        matchIndex++;
        if (matchIndex == substring.length()) return true;
