...
public class CurrentDayOfWeekTest extends AbstractTest {
  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (argumentsIter.hasNext()) {
      ...
      if (argument instanceof TagArgument) comparator = ((TagArgument) argument).getTag();
    }
    if (!":is".equals(comparator)) throw new SyntaxException("Expecting \":is\"");
    ...
    if (argumentsIter.hasNext()) {
      ...
      if (argument instanceof StringListArgument) {
        ...
        for (String val : valList) {
          ...
          if (day < 0 || day > 6) throw new SyntaxException("Expected values between 0 - 6");
          // In Java 1=Sunday, 7=Saturday
          daysToCheckAgainst.add(day + 1);
        }
      }
    }
    if (daysToCheckAgainst.isEmpty()) throw new SyntaxException("Expecting at least one value");
    ...
    try {
      accountTimeZone =
          Util.getAccountTimeZone(((ZimbraMailAdapter) mail).getMailbox().getAccount());
