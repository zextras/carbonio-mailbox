...
public class EditHeaderExtension {
  ...
  // Utility methods
  /**
   * This method sets values provided with replaceheader or deleteheader in
   * <b>EditHeaderExtension</b> object.
   *
   * @param arguments
   * @param ac
   ...
   */
  public void setupEditHeaderData(Arguments arguments, AbstractCommand ac)
      throws SyntaxException, OperationException {
        ...
        if (tag.is(HeaderConstants.INDEX)) {
            ...
            if (arg instanceof StringListArgument) {
              ...
              if (StringUtil.isNullOrEmpty(origNewName)) {
                throw new SyntaxException(
                    "New name must be present with :newname in replaceheader : " + arg);
              }
              this.newName = origNewName;
            } else {
              throw new SyntaxException(
                  "New name not provided with :newname in replaceheader : " + arg);
            }
          }
          ...
          if (itr.hasNext()) {
            ...
            if (arg instanceof StringListArgument) {
              ...
              throw new SyntaxException(
                  "New value not provided with :newValue in replaceheader : " + arg);
            }
          }
          ...
          if (itr.hasNext()) {
            ...
            if (arg instanceof StringListArgument) {
              ...
              throw new SyntaxException(
                  "Relational comparator not provided with :count in replaceheader : " + arg);
            }
          }
          ...
          if (itr.hasNext()) {
            ...
            if (arg instanceof StringListArgument) {
              ...
              throw new SyntaxException(
                  "Relational comparator not provided with :value in replaceheader : " + arg);
            }
          }
          ...
          if (itr.hasNext()) {
            ...
            if (arg instanceof StringListArgument) {
              ...
              throw new SyntaxException(
                  "Comparator not provided with :comparator in replaceheader : " + arg);
            }
          }
        ...
        if (ac instanceof ReplaceHeader) {
          ...
          if (itr.hasNext()) {
            ...
            ZimbraLog.filter.info(
                "Value for "
                    + this.key
                    + " is not provided in deleteheader. So all headers with this key will be"
                    + " deleted.");
          }
        } else {
  ...
  public boolean matchCondition(
      ZimbraMailAdapter mailAdapter,
      Header header,
      List<String> headerList,
      String value,
      SieveContext context)
      throws LookupException, SieveException, MessagingException {
    ...
    try {
      ...
      ZimbraLog.filter.debug(
          "Header value after unfolding and decoding: %s", unfoldedAndDecodedHeaderValue);
    ...
    if (this.valueTag) {
      matchFound =
          ZimbraComparatorUtils.values(
              mailAdapter,
              comparator,
              relationalComparator,
              unfoldedAndDecodedHeaderValue,
              value,
              context);
    } else if (this.countTag) {
      matchFound =
          ZimbraComparatorUtils.counts(
              mailAdapter, comparator, relationalComparator, headerList, value, context);
    } else if (this.is
        && ComparatorUtils.is(comparator, unfoldedAndDecodedHeaderValue, value, context)) {
      matchFound = true;
    } else if (this.contains
        && ComparatorUtils.contains(comparator, unfoldedAndDecodedHeaderValue, value, context)) {
      matchFound = true;
    } else if (this.matches
        && ComparatorUtils.matches(comparator, unfoldedAndDecodedHeaderValue, value, context)) {
      matchFound = true;
    } else {
      ZimbraLog.filter.debug(
          "Key: %s and Value: %s pair not matching requested criteria.", this.key, value);
    }
  ...
  public void commonValidation(String operation) throws SyntaxException {
    ...
    // comparator must be valid and if not set, then set to default i.e.
    // ComparatorNames.ASCII_CASEMAP_COMPARATOR
    if (this.comparator != null) {
      ...
      ZimbraLog.filter.info(
          "No comparator type provided, so setting to default %s",
          ComparatorNames.ASCII_CASEMAP_COMPARATOR);
    }
    // relational comparator must be available with numeric comparison
    if (this.comparator.equals(HeaderConstants.I_ASCII_NUMERIC)
        && !(this.countTag || this.valueTag || this.is)) {
      throw new SyntaxException(
          operation + ":No valid comparator (:value, :count or :is) found for numeric operation.");
    }
  ...
  public List<String> getMatchingHeaders(MimeMessage mm) throws OperationException {
    ...
    try {
      ...
      throw new OperationException(
          "Error occured while fetching " + this.key + " headers from mime.", e);
    }
    return headerList;
  }
  ...
  public static boolean isImmutableHeaderKey(String key, ZimbraMailAdapter mailAdapter) {
    List<String> immutableHeaders =
        Arrays.asList(mailAdapter.getAccount().getSieveImmutableHeaders().split(","));
