...
public class EnvelopeTest extends Envelope {
  ...
  /**
   * From RFC 5228, Section 5.4... <code>
  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (MatchTypeTags.MATCHES_TAG.equals(params.getMatchType())) {
      try {
        HeaderTest.evaluateVarExp(
            mailAdapter, params.getHeaderNames(), HeaderTest.SourceType.ENVELOPE, params.getKeys());
    ...
    if (HeaderConstants.COUNT.equals(params.getMatchType())
        ...
        || IS_TAG.equals(params.getMatchType())) {
      return match(
          mail,
    ...
    for (final String headerName : headerNames) {
      if ("to".equalsIgnoreCase(headerName)) {
        ...
        throw new SyntaxException(
            "Unexpected header name as a value for <envelope-part>: '" + headerName + "'");
      }
    }
    ...
    if (ALL_TAG.equalsIgnoreCase(addressPart)) {
      ...
      matchAddress =
          (addressPart.equals(LOCALPART_TAG)
              ? email.substring(localStart, localEnd)
              : email.substring(domainStart, domainEnd));
    }
