...
public class AddressTest extends Address {
  ...
  /**
   * From RFC 5228, Section 5.1... <code>
   * Syntax: address [ADDRESS-PART] [COMPARATOR] [MATCH-TYPE]
   *         &lt;header-list: string-list&gt; &lt;key-list: string-list&gt;
   * </code>
   *
   * @see org.apache.jsieve.tests.AbstractComparatorTest#executeBasic(MailAdapter, Arguments,
   *     SieveContext)
   */
  @Override
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    ZimbraComparatorUtils.TestParameters params =
        ZimbraComparatorUtils.parseTestArguments(mail, arguments, context);
    ...
    if (MatchTypeTags.MATCHES_TAG.equals(params.getMatchType())) {
      ...
      try {
        HeaderTest.evaluateVarExp(
            zma, params.getHeaderNames(), HeaderTest.SourceType.HEADER, params.getKeys());
    ...
    if (HeaderConstants.COUNT.equals(params.getMatchType())
        || HeaderConstants.VALUE.equals(params.getMatchType())
        || IS_TAG.equalsIgnoreCase(params.getMatchType())) {
      return match(
          mail,
      ...
      for (final String key : keys) {
        isMatched =
            ZimbraComparatorUtils.counts(
                ...
                operator,
                headerValues,
                ZimbraComparatorUtils.getMatchKey(addressPart, key),
                context);
        if (isMatched) {
          break;
        }
  ...
  private String getMatchAddress(String addressPart, String localPart, String domain) {
    ...
    if (ALL_TAG.equals(addressPart)) matchAddress = localPart + "@" + domain;
    else {
      matchAddress = (LOCALPART_TAG.equals(addressPart) ? localPart : domain.toLowerCase());
    }
    return matchAddress;
  }
}
