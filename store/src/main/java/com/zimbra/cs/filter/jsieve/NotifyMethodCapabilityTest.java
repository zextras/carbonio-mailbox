...
public class NotifyMethodCapabilityTest extends AbstractTest {
  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (HeaderConstants.COUNT.equalsIgnoreCase(matchType) && null == operator) {
      throw context.getCoordinate().syntaxException("Expecting a String of operator");
    }
  ...
  private boolean testCount(
      MailAdapter mail, List<String> keys, String comparator, String operator, SieveContext context)
      throws SieveException {
    List<String> values = Arrays.asList(CAPABILITY_MAYBE);
    boolean isMatched = false;
    for (String key : keys) {
      isMatched = ZimbraComparatorUtils.counts(mail, comparator, operator, values, key, context);
      if (isMatched) {
        break;
      }
    }
