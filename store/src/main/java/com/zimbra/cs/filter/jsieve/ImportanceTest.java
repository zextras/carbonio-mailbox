  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (argumentsIter.hasNext()) {
      ...
      if (argument instanceof StringListArgument) {
        importance =
            FilterTest.Importance.valueOf(((StringListArgument) argument).getList().get(0));
      } else {
    ...
    boolean result1 =
        match(
            mail,
            Sieve.Comparator.iasciicasemap.toString(),
            MatchTypeTags.IS_TAG,
            headers,
            values,
            context);
    ...
    // normal is when it is neither high importance nor low importance
    return importance == FilterTest.ImportanceTest.Importance.normal
        ? !(result1 || result2)
        : result1 || result2;
  }
}
