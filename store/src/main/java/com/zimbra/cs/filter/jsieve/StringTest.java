...
public class StringTest extends Header {
  ...
  protected boolean executeBasic(MailAdapter mail, Arguments arguments, SieveContext context)
    ...
    if (null == comparator) {
      if (matchType.equalsIgnoreCase(HeaderConstants.VALUE)
          || matchType.equalsIgnoreCase(HeaderConstants.COUNT)) {
      ...
      if (matchType.equals(MatchTypeTags.MATCHES_TAG)) {
        try {
          HeaderTest.evaluateVarExp(
              mailAdapter, sourceValues, HeaderTest.SourceType.LITERAL, keyValues);
      ...
      while (!isMatched && keyIter.hasNext()) {
        isMatched =
            ZimbraComparatorUtils.counts(
                mail, comparator, operator, sourceValues, keyIter.next(), context);
      }
    } else {
        ...
        while (!isMatched && keyIter.hasNext()) {
          isMatched =
              ZimbraComparatorUtils.match(
                  mail, comparator, matchType, operator, source, keyIter.next(), context);
        }
      }
    }
    return isMatched;
