...
public final class FilterUtil {
  ...
  /**
   * Returns a Sieve-escaped version of the given string. Replaces <tt>\</tt> with <tt>\\</tt> and
   * <tt>&quot;</tt> with <tt>\&quot;</tt>.
   */
  ...
  /**
   * Adds a value to the given <tt>Map</tt>. If <tt>initialKey</tt> already exists in the map, uses
   * the next available index instead. This way we guarantee that we don't lose data if the client
   * sends two elements with the same index, or doesn't specify the index at all.
   *
  ...
  public static int getIndex(Element actionElement) {
    ...
    try {
      ...
      ZimbraLog.soap.warn(
          "Unable to parse index value %s for element %s.  Ignoring order.",
  ...
  /**
   * Parses a Sieve size string and returns the integer value. The string may end with <tt>K</tt>
   * (kilobytes), <tt>M</tt> (megabytes) or <tt>G</tt> (gigabytes). If the units are not specified,
   ...
   */
  public static ItemId addMessage(
    ...
    // Do initial lookup.
    Pair<Folder, String> folderAndPath =
        mbox.getFolderByPathLongestMatch(octxt, Mailbox.ID_FOLDER_USER_ROOT, folderPath);
    ...
    if (folder instanceof Mountpoint) {
      ...
      if (remoteFolder != null) {
        if (remainingPath != null) {
          ...
          if (remoteFolder == null) {
            String msg =
                String.format(
                    "Subfolder %s of mountpoint %s does not exist.",
      ...
      if (remoteFolder != null) {
        ...
        String msgId =
            remoteMbox.addMessage(
                remoteFolder.getId(),
                com.zimbra.cs.mailbox.Flag.toString(flags),
                null,
                0,
                content,
                false);
        ...
        String msg =
            String.format(
                "Unable to find remote folder %s for mountpoint %s.",
                remainingPath, mountpoint.getName());
        throw ServiceException.FAILURE(msg, null);
      }
    } else {
  ...
  /**
   * Returns a <tt>ZMailbox</tt> for the remote mailbox referenced by the given <tt>Mountpoint</tt>.
   */
  public static ZMailbox getRemoteZMailbox(Mailbox localMbox, Mountpoint mountpoint)
    ...
    ZMailbox.Options zoptions =
        new ZMailbox.Options(authToken.toZAuthToken(), AccountUtil.getSoapUri(account));
  ...
  public static void redirect(
      OperationContext octxt, Mailbox sourceMbox, MimeMessage msg, String destinationAddress)
      throws ServiceException {
    ...
    try {
      if (!isMailLoop(sourceMbox, msg, new String[] {HEADER_FORWARDED})) {
        ...
        String error =
            String.format("Detected a mail loop for message %s.", Mime.getMessageID(msg));
      ...
      try {
        ...
        ZimbraLog.filter.info(
            "Message format error detected.  Wrapper class in use.  %s", e.toString());
  ...
  private static boolean isDeliveryStatusNotification(MimeMessage msg) throws MessagingException {
    ...
    ZimbraLog.filter.debug(
        "isDeliveryStatusNotification(): Return-Path=%s, Auto-Submitted=%s, Content-Type=%s.",
  ...
  public static void reply(
      OperationContext octxt, Mailbox mailbox, ParsedMessage parsedMessage, String bodyTemplate)
    ...
    if (isMailLoop(mailbox, mimeMessage, new String[] {HEADER_FORWARDED})) {
      String error =
          String.format("Detected a mail loop for message %s.", Mime.getMessageID(mimeMessage));
      throw ServiceException.FAILURE(error, null);
    }
    ...
    if (StringUtil.isNullOrEmpty(to)) to = Mime.getSender(mimeMessage);
    ...
    String replySubjectPrefix =
        L10nUtil.getMessage(L10nUtil.MsgKey.replySubjectPrefix, account.getLocale());
    ...
    if (!StringUtil.isNullOrEmpty(origMsgId)) replyMsg.setHeader("In-Reply-To", origMsgId);
  ...
  public static void notify(
    ...
    if (origHeaders == null || origHeaders.isEmpty()) {
      ...
      notification.setRecipient(
          javax.mail.Message.RecipientType.TO, new JavaMailInternetAddress(emailAddr));
  ...
  public static void reject(
    ...
    if (isMailLoop(mailbox, mimeMessage, new String[] {HEADER_FORWARDED})) {
      ...
      String error =
          String.format(
              "Detected a mail loop for message %s. No MDN sent.", Mime.getMessageID(mimeMessage));
    ...
    if (reportTo == null || reportTo.isEmpty()) {
      ...
      if (returnPath == null || returnPath.length == 0) {
        ...
        throw new MessagingException(
            "Neither 'envelope from' nor 'Return-Path' specified. Can't locate the address to"
                + " reject to (No MDN sent)");
      } else {
    ...
    report.setHeader(
        "Content-Type", multi.getContentType() + "; report-type=disposition-notification");
    report.saveChanges();
  ...
  public static void notifyMailto(
      LmtpEnvelope envelope,
      OperationContext octxt,
      Mailbox mailbox,
      ParsedMessage parsedMessage,
    ...
    if (isMailLoop(mailbox, mimeMessage, new String[] {HEADER_FORWARDED, HEADER_AUTO_SUBMITTED})) {
      String error =
          String.format(
              "Detected a mail loop for message %s while notifying",
    ...
    if (originalEnvelopeFrom == null) {
      // Whenever the envelope FROM of the original message is <>, set <> to the notification
      // message too
      mailSender.setEnvelopeFrom("<>");
    } else if (!StringUtil.isNullOrEmpty(from)) {
      List<com.zimbra.common.mime.InternetAddress> addr =
          com.zimbra.common.mime.InternetAddress.parseHeader(from);
      ...
      do {
        ...
        if (matches)
          escapedFrom =
              new StringBuilder(escapedFrom)
                  .replace(matcher.start(2), matcher.end(2), "\\")
                  .toString();
      } while (matches);
    ...
    notification.addRecipient(
        javax.mail.Message.RecipientType.TO, new JavaMailInternetAddress(mailto));
    ...
    if (tos != null && tos.size() > 0) {
      for (String to : tos) {
        envelopeTos.add(to);
        notification.addRecipient(
            javax.mail.Message.RecipientType.TO, new JavaMailInternetAddress(to));
      }
    }
    ...
    if (ccs != null && ccs.size() > 0) {
      for (String cc : ccs) {
        envelopeTos.add(cc);
        notification.addRecipient(
            javax.mail.Message.RecipientType.CC, new JavaMailInternetAddress(cc));
      }
    }
    ...
    StringBuilder autoSubmitted =
        new StringBuilder("auto-notified; owner-email=\"").append(account.getName()).append("\"");
  ...
  /**
   * Gets appropriate charset for the given data. The charset preference order is: ` 1. "us-ascii"
   * 2. Account's zimbraPrefMailDefaultCharset attr 3. "utf-8"
   *
   * @param account
   * @param data
   * @return
  ...
  private static String getCharset(Account account, String data) {
    if (MimeConstants.P_CHARSET_ASCII.equals(
        CharsetUtil.checkCharset(data, MimeConstants.P_CHARSET_ASCII))) {
      return MimeConstants.P_CHARSET_ASCII;
    } else {
      return CharsetUtil.checkCharset(
          data,
          account.getAttr(
  ...
  @VisibleForTesting
  static Map<String, String> getVarsMap(
      Mailbox mailbox, ParsedMessage parsedMessage, MimeMessage mimeMessage)
      throws MessagingException, ServiceException {
    ...
    if (bodyPart != null) {
      try {
        vars.put(
            "body",
            Mime.getStringContent(
                bodyPart.getMimePart(), mailbox.getAccount().getPrefMailDefaultCharset()));
      } catch (IOException e) {
  ...
  private static MimeMessage createRedirectMsgOnError(final MimeMessage originalMsg)
  ...
  /**
   * Returns <tt>true</tt> if the triggering message is an automatically generated message.
   *
   * @param sourceMbox owner's mailbox
   * @param msg triggering message
   * @param checkHeaders a list of header field names to be checked on the triggering message. For
   *     the reply and notify (legacy Zimbra version) action, checks X-Zimbra-Forwarded For the
   *     notify (RFC compliant) action, checks X-Zimbra-Forwarded and Auto-Submitted headers.
   */
  ...
  /**
   * Look-up the variable table to get the set value. If the 'sourceStr' contains a variable-ref
   * (formatted with ${variable-name}), this method looks up the variable table with its
   * variable-name and replaces the variable with the text value assigned by the 'set' command.
  ...
  public static String replaceVariables(ZimbraMailAdapter mailAdapter, String sourceStr)
    ...
    try {
      ...
      ZimbraLog.filter.info(
          "\"variables\" capability is not declared. No variables will be replaced");
      return sourceStr;
    }
   ...
   *
   * <p>The variable name matches as short as possible (non-greedy matching). Unknown variables are
   * replaced by the empty string (RFC 5229 Section 3.)
   *
   ...
   * @return Replaced text string
   */
  public static String leastGreedyReplace(
      Map<String, String> variables, String sourceStr, Map<String, String> mimeVars) {
  ...
  /**
   * Remove the extra backslash. Any undefined escape sequences specified as a string text in the
   * sieve filter are supposed to be removed before parsing the sieve filter. So, this method is
   * mainly called to verify the variable name whose label is generated at runtime via another
   * variable.
   *
   * @param varName
   * @return "${fo\o}" => ${foo} => the expansion of variable foo. <br>
  ...
  public static String sieveToJavaRegex(String pattern) {
    ...
    for (ch = 0; ch < pattern.length(); ch++) {
      ...
      switch (nextChar) {
        case '*':
          // If there are two or more wildcards,all wildcards should be non-greedy except the last
          // wildcard.
  ...
  public static void headerNameHasSpace(String headerName) throws SyntaxException {
    ...
    if (headerName.contains(" ")) {
      throw new SyntaxException(
          "ZimbraComparatorUtils : Header name must not have space(s) : \"" + headerName + "\"");
    }
  }
}
