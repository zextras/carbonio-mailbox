 ...
 * For Bug 77287
 *
 * <p>Maps command names to common implementations or custom implementation with Zimbra Extension
 */
public class ZimbraCommandManagerImpl implements CommandManager {

  private static List<String> IMPLICITLY_DECLARED =
      Arrays.asList("if", "else", "elsif", "require", "stop", "keep", "discard", "redirect");
  ...
  private Class lookup(String name) throws LookupException {
    Class cmdClass = null;
    try {
      cmdClass = getClass().getClassLoader().loadClass(getClassName(name));
    ...
    if (!ExecutableCommand.class.isAssignableFrom(cmdClass))
      throw new LookupException(
          "Class " + cmdClass.getName() + " must implement " + ExecutableCommand.class.getName());
    return cmdClass;
  }
  ...
  protected String getClassName(String name) throws LookupException {
    ...
    if (null == className) throw new LookupException("Command named '" + name + "' not mapped.");
    return className;
  }
