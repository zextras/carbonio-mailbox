...
public class ZimbraComparatorManagerImpl implements ComparatorManager {
  ...
  /**
   * Constructs a set containing the names of those comparisons for which <code>require</code> is
   * not necessary before usage, according to RFC5228. See <a
   * href='http://tools.ietf.org/html/rfc5228#section-2.7.3'>RFC5228, 2.7.3 Comparators</a>.
   */
   ...
   * @param implicitlyDeclared names of those comparisons for which <code>require</code> is not
   *     necessary before usage
   */
  public ZimbraComparatorManagerImpl(
      final ConcurrentMap<String, String> classNameMap,
      final CopyOnWriteArraySet<String> implicitlyDeclared) {
    super();
    ...
    try {
      ...
      ZimbraLog.filter.info(
          "[%s] = [%s]", ASCII_CASEMAP_COMPARATOR, getClassName(ASCII_CASEMAP_COMPARATOR));
      ...
      ZimbraLog.filter.info(
          "[%s] = [%s]", ASCII_NUMERIC_COMPARATOR, getClassName(ASCII_NUMERIC_COMPARATOR));
  ...
  /**
   * Is an explicit declaration in a <code>require</code> statement unnecessary for this comparator?
   *
   * @param comparatorName not null
   * @return true when this comparator need not be declared by <core>require</code>, false when any
   *     usage of this comparator must be declared in a <code>require</code> statement
   */
  @Override
  ...
  public Class lookup(String name) throws LookupException {
    ...
    try {
      comparatorClass = getClass().getClassLoader().loadClass(getClassName(name));
    } catch (ClassNotFoundException e) {
      throw new LookupException("Comparator named '" + name + "' not found.");
    }
    if (!Comparator.class.isAssignableFrom(comparatorClass))
      throw new LookupException(
          "Class " + comparatorClass.getName() + " must implement " + Comparator.class.getName());
  ...
  private String getClassName(String name) throws LookupException {
    ...
    if (null == className) throw new LookupException("Comparator named '" + name + "' not mapped.");
    return className;
  }
