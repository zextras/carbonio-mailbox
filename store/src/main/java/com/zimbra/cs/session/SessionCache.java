...
/**
 * A Simple Cache with timeout (based on last-accessed time) for a {@link Session}. objects
 *
 * <p>Not all Session subclasses are stored in this cache currently, though all user-available
 * sessions should eventually be stored in this cache
 */
public final class SessionCache {

  private static final String sRunIdentifier =
      (System.currentTimeMillis() / 1000) + "." + new java.util.Random().nextInt(100);
  ...
  /**
   * Adds a {@link Session} to the cache and assigns it a session ID if it doesn't already have one.
   * When a reigistered <code>Session</code> ages out of the cache due to extended idle time, its
   * {@link Session#doCleanup()} method is invoked and its session ID is unset.
   *
  ...
  protected static String registerSession(Session session) {
    if (sShutdown || session == null) return null;

    Session.Type sessionType = session.getSessionType();
    String sessionId = session.getSessionId();
    assert (sessionId != null);

    getSessionMap(sessionType)
        .putAndPrune(
            session.getAuthenticatedAccountId(),
            sessionId,
            session,
            sessionType.getMaxPerAccount());
    return sessionId;
  }

  /**
   * Fetches a {@link Session} from the cache. Checks to make sure that the provided account ID
   * matches the one on the session. Implicitly updates the LRU expiry timestamp on the session.
   *
   * @param sessionId The identifier for the requested Session.
   * @param accountId The owner of the requested Session.
   * @return The matching cached Session, or <tt>null</tt> if no session exists with the specified
   *     ID and owner.
   */
  public static Session lookup(String sessionId, String accountId) {
    if (sShutdown) return null;
    ...
    if (s == null && ZimbraLog.session.isDebugEnabled()) {
      ZimbraLog.session.debug(
          "no session with id " + sessionId + " found (accountId: " + accountId + ")");
    }
    return s;
  }
   ...
   *
   * @param accountId
   * @return Unmodifiable Collection of Sessions for the requested account, or <tt>null</tt> if no
  ...
  public static int countActiveSessionsForAccount(String accountId, Session.Type type) {
    ...
    ZimbraLog.session.trace(
        "SessionCache.countActiveSessionsForAccount(%s,%s)=%d", accountId, type, num);
    return num;
  }
  ...
  /**
   * Immediately removes the {@link Session} from the session cache and runs its {@link
   * Session#doCleanup()} method.
   *
  ...
  /**
   * Removes the <tt>Session</tt> from the cache and unsets its session ID. Unlike {@link
   * #clearSession(Session)}, the <tt>Session</tt>'s {@link Session#doCleanup()} method is not
   * automatically invoked.
   *
   * @param session The <tt>Session</tt> to remove from the cache
   * @return the <tt>Session</tt> removed from the cache, or <tt>null</tt> if the given Session was
   *     not cached
   * @see #clearSession(Session)
   * @see #registerSession(Session)
   */
  public static Session unregisterSession(Session session) {
    if (sShutdown || session == null || !session.isAddedToSessionCache()) return null;
  ...
  private static final Session.Type getSessionTypeFromId(String sessionId) {
    if (sessionId == null || sessionId.length() < 2) return Session.Type.NULL; // invalid session id
  ...
  private static void logActiveSessions() {
    ...
    for (SessionMap sessionMap : sSessionMaps) {
      synchronized (sessionMap) {
        for (SessionMap.AccountSessionMap activeAcct : sessionMap.activeAccounts()) {
          ...
          assert (count > 0);
          if (count > 0) {
            if (accountList.length() > 0) accountList.append(',');
            accountList.append(accountId).append('(').append(count).append(')');
            if (count > 9) {
              if (manySessionsList.length() > 0) manySessionsList.append(',');
    ...
    if (manySessionsList.length() > 0) {
      ZimbraLog.session.info(
          "Found accounts that have a large number of sessions: " + manySessionsList);
    }
  }
  ...
  private static final class SweepMapTimerTask extends TimerTask {
    ...
    public void run() {
      try {
        if (sLog.isDebugEnabled()) SessionCache.logActiveSessions();
        ...
        if (sLog.isInfoEnabled() && totalRemoved > 0) {
          StringBuilder sb =
              new StringBuilder("Removed ").append(totalRemoved).append(" idle sessions (");
