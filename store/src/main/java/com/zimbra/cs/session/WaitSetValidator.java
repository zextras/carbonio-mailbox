...
public class WaitSetValidator implements DebugListener {
  ...
  void run(String id, String host, String user, String pw) {
    try {
      ...
      for (WaitSetSession w : wss) {
        if (!validateSessionStatus(w)) hasFailures = true;
      }
      if (hasFailures) {
        Element qws = invokeQueryWaitSet(id);
  ...
  public static void main(String[] args) {
    ...
    if (!cl.hasOption('i')) t.usage();

    id = cl.getOptionValue('i');

    if (cl.hasOption('h')) host = cl.getOptionValue('h');
    else host = "http://localhost:7071/service/admin";

    if (cl.hasOption('u')) user = cl.getOptionValue('u');
    else user = "admin";

    if (cl.hasOption('p')) pw = cl.getOptionValue('p');
    else pw = "test123";

    if (cl.hasOption('v')) t.setVerbose(true);
    ...
    if (hosts.length != ids.length) {
      System.err.println(
          "If multiple hosts or ids are specified, the same number is required of each");
      System.exit(3);
    }
