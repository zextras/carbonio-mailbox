...
/** The base class defines shared functions, as well as any APIs which should be package-private */
public abstract class WaitSetBase implements IWaitSet {
  ...
  protected Map<String /*accountId*/, PendingModifications> currentPendingModifications =
      Maps.newHashMap();
  ...
  protected Map<String /*accountId*/, PendingModifications> sentPendingModifications =
      Maps.newHashMap();
  ...
  protected synchronized WaitSetCallback getCb() {
    return mCb;
  }
  ...
  public synchronized void doneWaiting(WaitSetCallback myCb) {
    ...
    if ((myCb == null) || (mCb == myCb)) {
      ...
      ZimbraLog.session.debug(
          "WaitSetBase.doneWaiting - saved callback NOT ours so NOT making null");
    }
  }

  protected WaitSetBase(
      String ownerAccountId, String waitSetId, Set<MailItem.Type> defaultInterest) {
  ...
  protected synchronized void trySendData() {
    ...
    if ((mCurrentSignalledAccounts.size() > 0 || mCurrentErrors.size() > 0)
        || (!cbIsCurrent
            && (mSentSignalledSessions.size() > 0
                || mSentErrors.size() > 0
                || mSentSignalledAccounts.size() > 0))) {
      ...
      mCb.dataReady(
          ...
          false,
          mSentErrors,
          mSentSignalledSessions,
          mSentSignalledAccounts,
          sentPendingModifications);
      mCb = null;
      mLastAccessedTime = System.currentTimeMillis();
    }
  ...
  public synchronized WaitSetInfo handleQuery() {
    WaitSetInfo info =
        WaitSetInfo.createForWaitSetIdOwnerInterestsLastAccessDate(
            mWaitSetId,
            mOwnerAccountId,
            WaitSetRequest.expandInterestStr(defaultInterest),
            mLastAccessedTime);
    ...
    if (mCurrentSignalledAccounts.size() > 0) {
      ...
      for (String accountId : mCurrentSignalledAccounts) {
        if (signaledStr.length() > 0) signaledStr.append(",");
  ...
  protected synchronized void addChangeFolderIds(
      Map<String, Set<Integer>> folderIdsMap, String acctId, Set<Integer> changedFolderIds) {
  ...
  protected synchronized void addMods(
      Map<String, PendingModifications> mods, String acctId, PendingModifications mod) {
    mods.put(acctId, mod);
  }
}
