...
public final class PendingLocalModifications extends PendingModifications<MailItem> {
  ...
  PendingModifications<MailItem> add(PendingModifications<MailItem> other) {
    ...
    if (other.deleted != null) {
      for (Map.Entry<PendingModifications.ModificationKey, PendingModifications.Change> entry :
          other.deleted.entrySet()) {
  ...
  public void recordModified(
      PendingModifications.ModificationKey mkey, PendingModifications.Change chg) {
  ...
  public void recordModified(MailboxStore mbox, int reason) {
    ...
    if (mbox instanceof Mailbox) {
      ...
      recordModified(
          new PendingModifications.ModificationKey(mb.getAccountId(), 0),
          mbox,
          reason,
          null,
          false);
  ...
  private void recordModified(
      PendingModifications.ModificationKey key,
      Object item,
      int reason,
      Object preModifyObj,
  ...
  private static Map<PendingModifications.ModificationKey, PendingModifications.Change> getOriginal(
      Mailbox mbox, Map<ModificationKeyMeta, ChangeMeta> map) throws ServiceException {
    if (map == null) {
      return null;
    }
    Map<PendingModifications.ModificationKey, PendingModifications.Change> ret =
        new LinkedHashMap<PendingModifications.ModificationKey, PendingModifications.Change>();
    ...
    while (iter.hasNext()) {
      ...
      PendingModifications.ModificationKey key =
          new PendingModifications.ModificationKey(entry.getKey().accountId, entry.getKey().itemId);
  ...
  public static PendingLocalModifications deserialize(Mailbox mbox, byte[] data)
    ...
    try (ObjectInputStream ois = new SecureObjectInputStream(bis, Type.class.getName())) {
      ...
      LinkedHashMap<ModificationKeyMeta, String> metaCreated =
          (LinkedHashMap<ModificationKeyMeta, String>) ois.readObject();
      if (metaCreated != null) {
        ...
        while (iter.hasNext()) {
          ...
          PendingModifications.ModificationKey key =
              new PendingModifications.ModificationKey(
                  entry.getKey().accountId, entry.getKey().itemId);
          pms.created.put(key, item);
        }
      }

      Map<ModificationKeyMeta, ChangeMeta> metaModified =
          (Map<ModificationKeyMeta, ChangeMeta>) ois.readObject();
      pms.modified = getOriginal(mbox, metaModified);

      Map<ModificationKeyMeta, ChangeMeta> metaDeleted =
          (Map<ModificationKeyMeta, ChangeMeta>) ois.readObject();
