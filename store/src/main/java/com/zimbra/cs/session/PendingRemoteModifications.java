...
public final class PendingRemoteModifications extends PendingModifications<ZBaseItem> {
  ...
  public static PendingRemoteModifications fromSOAP(
      PendingFolderModifications mods, Integer folderId, String acctId) {

    PendingRemoteModifications prms = new PendingRemoteModifications();
    for (CreateItemNotification createSpec : mods.getCreated()) {
      prms.recordCreated(
          ModificationItem.itemUpdate(createSpec.getMessageInfo(), folderId, acctId));
    }
    for (ModifyItemNotification modifyItem : mods.getModifiedMsgs()) {
      int change = modifyItem.getChangeBitmask();
      BaseItemInfo itemUpdate =
          ModificationItem.itemUpdate(modifyItem.getMessageInfo(), folderId, acctId);
    ...
    for (RenameFolderNotification renamedFolder : mods.getRenamedFolders()) {
      ...
      ModificationItem folderRename =
          ModificationItem.folderRename(renamedFolderId, newPath, acctId);
