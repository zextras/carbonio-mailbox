...
/**
 * A {@link Session} is identified by an (accountId, sessionID) pair. A single account may have
 * multiple active sessions simultaneously.
 *
 * <p>In general, Sessions are created and managed by the {@link SessionCache} but this is not
 * always the case. Session subclasses which are not stored in the SessionCache must take special
 * care to manage their own lifetimes (timeouts, etc)
 */
public abstract class Session {
  ...
  /**
   * Creates a {@code Session} of the given <tt>Type</tt> whose target {@link Account} is the same
   * as its authenticated <tt>Account</tt>.
   *
  ...
  /**
   * Creates a {@code Session} of the given <tt>Type</tt> with its owner and target specified
   * separately. In general, a {@code Session} should only be created on the server where the target
   * mailbox lives.
   *
   * @param authId The account ID of the {@code Session}'s owner
   * @param targetId The account ID of the {@link Mailbox} the {@code Session} is attached to
   * @param type The type of {@code Session} being created
   */
  ...
  /**
   * Registers the session as a listener on the target mailbox and adds it to the session cache.
   * When a session is added to the cache, its session ID is initialized.
   *
  ...
  public Session register() throws ServiceException {
    ...
    if (isMailboxListener()) {
      MailboxStore mbox =
          mailbox = MailboxManager.getInstance().getMailboxByAccountId(mTargetAccountId);
  ...
  /**
   * Unregisters the session as a listener on the target mailbox and removes it from the session
   * cache. When a session is removed from the cache, its session ID is nulled out.
   *
   * @see #isMailboxListener()
   * @see #isRegisteredInCache()
   */
  ...
  /**
   * Whether the session should be attached to the target {@link Mailbox} when its {@link
   * #register()} method is invoked.
   */
  protected abstract boolean isMailboxListener();

  /**
   * Whether the session should be added to the {@link SessionCache} when its {@link #register()}
   * method is invoked. A session ID is assigned when a session is added to the cache.
   */
  ...
  /**
   * Sets the Session's identifier. Used for unit testing only, DO NOT CALL THIS API EXCEPT FOR TEST
   * PURPOSES
   */
  ...
  /**
   * Returns the maximum idle duration (in milliseconds) for the Session. The idle lifetime must be
   * CONSTANT
   */
  ...
  /**
   * Most session types do not support MailboxStore types other than "Mailbox". Calling this will
   * ensure that an exception is thrown if the store is not a Mailbox (or null) for any reason.
   *
   * @return
   */
  public Mailbox getMailboxOrNull() {
    ...
    throw new UnsupportedOperationException(
        String.format(
            "Operation not supported for non-Mailbox MailboxStore '%s'",
            mboxStore.getClass().getName()));
  }
   ...
   * @param source The {@code Session} originating these changes, or <tt>null</tt> if none was
   *     specified.
   */
  ...
  /**
   * This API must only be called by the SessionCache, all other callers should use
   * updateAccessTime()
   */
  ...
  /**
   * Returns the account ID of the {@link Mailbox} that the {@code Session} is attached to.
   *
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    helper.add("id", mSessionId).add("authAcct", mAuthenticatedAccountId);
    ...
    helper.add(
        "lastAccessed",
        new SimpleDateFormat("yyyy-MM-dd HH:mm:ss,SSS").format(new Date(mLastAccessed)));
    if (mCleanedUp) {
