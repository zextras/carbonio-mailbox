...
/**
 * A session subclass which is used when an external entity wants to listen on a particular account
 * for changes to the mailbox, but doesn't care what exactly those changes are: presumably because
 * the caller is going to use some sort of other channel (e.g. IMAP) for synchronizing with the
 * Mailbox.
 */
public class WaitSetSession extends Session {
  ...
  /**
   * The IDs of folders we are interested in changes for. null or empty means interested in all
   * folders
   */
  ...
  void update(
      Set<MailItem.Type> interests, Set<Integer> folderInterests, SyncToken lastKnownSyncToken) {
    ...
    if (mSyncToken != null) {
      ...
      if (null == mbox) {
        throw new UnsupportedOperationException(
            String.format(
                "WaitSetSession must have an associated MailboxStore of class '%s' before calling"
                    + " update",
                Mailbox.class.getName()));
      }
      int mboxHighestChange = mbox.getLastChangeID();
      if (mboxHighestChange > mHighestChangeId) mHighestChangeId = mboxHighestChange;
      if (mSyncToken.after(mHighestChangeId)) mWs.unsignalDataReady(this);
      else mWs.signalDataReady(this);
    }
  ...
  public void notifyPendingChanges(PendingModifications pns, int changeId, Session source) {
    ...
    if (trace) {
      ZimbraLog.session.trace(
          "Notifying WaitSetSession %s: change id=%s changedFolders=%s changesTypes='%s'",
    ...
    if (mSyncToken != null && mSyncToken.after(mHighestChangeId)) {
      if (trace) {
        ZimbraLog.session.trace(
            "Not signaling waitset; sync token '%s' is later than highest change id '%s'",
    ...
    if (Sets.intersection(interest, pns.changedTypes).isEmpty()) {
      if (trace) {
        ZimbraLog.session.trace(
            "Not signaling waitset; waitset is not interested in change type. interest=[%s]"
                + " changes=[%s]",
            interest, pns.changedTypes);
      }
      return;
    }
    if ((folderInterest != null)
        && !folderInterest.isEmpty()
    ...
    if (source != null && source instanceof SoapSession) {
      ...
      if (curWaitSetID != null && curWaitSetID.equals(mWs.getWaitSetId())) {
        if (trace) {
          ZimbraLog.session.trace(
              "Not signaling waitset; changes will be returned in SOAP headers");
        }
        return;
      }
    }
