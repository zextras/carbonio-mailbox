...
/**
 * SomeAccountsWaitSet: an implementation of IWaitSet that works by listening over one or more
 * Accounts
 *
 * <p>External APIs: WaitSet.doWait() // primary wait API WaitSet.getDefaultInterest() // accessor
 */
public final class SomeAccountsWaitSet extends WaitSetBase implements MailboxManager.Listener {
  ...
  @Override
  public synchronized List<WaitSetError> doWait(
      WaitSetCallback cb,
      String lastKnownSeqNo,
      List<WaitSetAccount> addAccounts,
      List<WaitSetAccount> updateAccounts)
  ...
  private synchronized WaitSetError initializeWaitSetSession(WaitSetAccount wsa, Mailbox mbox) {
    ...
    if (session != null && session.getMailbox() != mbox) {
      ZimbraLog.session.warn(
          "SESSION BEING LEAKED? WaitSetSession points to old version of mailbox...possibly leaking"
              + " this session:",
          session);
      wsa.cleanupSession();
    ...
    for (WaitSetAccount update : updates) {
      ...
      if (existing != null) {
        ZimbraLog.session.trace(
            "SomeAccountsWaitSet updating %s with details from %s", existing, update);
        ...
        if (session != null) {
          session.update(
              update.getInterests(), update.getFolderInterests(), update.getLastKnownSyncToken());
          // update it!
        }
      } else {
        ZimbraLog.session.trace(
            "SomeAccountsWaitSet no pre-existing session when updating %s", update);
        errors.add(
            new WaitSetError(update.getAccountId(), WaitSetError.Type.NOT_IN_SET_DURING_UPDATE));
      }
    }
    return errors;
  }
    ...
    for (WaitSetAccount wsa : wsas) {
      if (!mSessions.containsKey(wsa.getAccountId())) {
        ...
        try {
          ...
          //
          // THIS CALL MIGHT REGISTER THE SESSION (via the MailboxManager notification -->
          // mailboxLoaded() callback!
          //
          Mailbox mbox =
              MailboxManager.getInstance().getMailboxByAccountId(wsa.getAccountId(), fetchMode);
          if (mbox != null) {
            WaitSetError error = initializeWaitSetSession(wsa, mbox);
            if (error != null) {
              errors.add(error);
          ...
          if (e.getCode() == AccountServiceException.NO_SUCH_ACCOUNT) {
            ...
            errors.add(
                new WaitSetError(wsa.getAccountId(), WaitSetError.Type.WRONG_HOST_FOR_ACCOUNT));
          } else {
            errors.add(
                new WaitSetError(wsa.getAccountId(), WaitSetError.Type.ERROR_LOADING_MAILBOX));
          }
        ...
        errors.add(
            new WaitSetError(wsa.getAccountId(), WaitSetError.Type.ALREADY_IN_SET_DURING_ADD));
      }
    }
    return errors;
  }
  ...
  protected synchronized HashMap<String, WaitSetAccount> destroy() {
    try {
      ...
      ZimbraLog.session.warn(
          "Caught unexpected ServiceException while destroying WaitSet: " + e, e);
    }
    cancelExistingCB();
  ...
  /**
   * Called by the WaitSetSession to revoke a signal...this happens when we get a sync token
   * <update> with a higher sync token for this account than the account's current highest
   *
   * @param session
   */
  protected synchronized void unsignalDataReady(WaitSetSession session) {
    if (mSessions.containsKey(
        session.getTargetAccountId())) { // ...false if waitset is shutting down...
  ...
  public synchronized WaitSetInfo handleQuery() {
    ...
    for (Map.Entry<String, WaitSetAccount> entry : mSessions.entrySet()) {
      ...
      SessionForWaitSet sess =
          new SessionForWaitSet(acctId, WaitSetRequest.expandInterestStr(wsa.getInterests()));
      ...
      if (wsa.getLastKnownSyncToken() != null) {
        try {
          Mailbox mbox =
              MailboxManager.getInstance().getMailboxByAccountId(acctId, FetchMode.ONLY_IF_CACHED);
          if (mbox != null) {
          ...
          ZimbraLog.session.warn(
              "Caught exception from MailboxManager in SomeAccountsWaitSet.handleQuery() for"
                  + " accountId %s",
              acctId, e);
        }
      }
      ...
      if (wss != null) {
        WaitSetSessionInfo waitSetSession =
            new WaitSetSessionInfo(
                WaitSetRequest.interestToStr(wss.interest),
                wss.mHighestChangeId,
                wss.getLastAccessTime(),
                wss.getCreationTime(),
                wss.getSessionId());
  ...
  /**
   * Keeping this for possible future use. Currently it is not reliable as WaitSets aren't
   * necessarily cleaned up immediately, resulting in false positives.
   */
