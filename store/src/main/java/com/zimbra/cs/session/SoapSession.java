...
public class SoapSession extends Session {
  public class DelegateSession extends Session {
    ...
    protected boolean skipChangeSerialization(Mailbox mbox, MailItem item) throws ServiceException {
      ...
      return (mbox != item.getMailbox()
          && item instanceof Conversation
          && ((Conversation) item).getMessageCount() > DELEGATED_CONVERSATION_SIZE_LIMIT
          && !mParentHasFullAccess);
    }

    /**
     * Fetch account level access and store it in cache to be used in skipChangeSerialization We
     * assume here that ACL will never change between this call and the call to
     * skipChangeSerialization() We also assume that it *might* change between invocations of
     * calculateVisibileFolders (i.e. check accessmgr each time)
     *
     * @throws ServiceException
     */
    protected void cacheAccountAccess(String authedAcctId, String targetAcctId)
      ...
      mParentHasFullAccess =
          AccessManager.getInstance()
              .canAccessAccount(
                  prov.get(AccountBy.id, authedAcctId),
                  prov.get(AccountBy.id, targetAcctId),
    ...
    private PendingLocalModifications filterNotifications(PendingLocalModifications pms)
      ...
      filtered.addChangedParentFolderIds(
          pms.getChangedParentFolders()); // Not 100% sure this is best but it is conservative
      ...
      if (pms.created != null && !pms.created.isEmpty()) {
        for (BaseItemInfo item : pms.created.values()) {
          if (item instanceof Conversation
              || visible.contains(
                  item instanceof Folder ? item.getIdInMailbox() : item.getFolderIdInMailbox())) {
            filtered.recordCreated(item);
          } else if (item instanceof Comment) {
            try {
              this.getMailboxOrNull()
                  .getItemById(octxt, ((Comment) item).getParentId(), MailItem.Type.UNKNOWN);
              filtered.recordCreated(item);
            } catch (ServiceException e) {
      ...
      if (pms.modified != null && !pms.modified.isEmpty()) {
        for (Change chg : pms.modified.values()) {
          if (chg.what instanceof MailItem) {
            ...
            if (skipChangeSerialization(getParentSession().getMailboxOrNull(), item)) {
              if (ZimbraLog.session.isDebugEnabled()) {
                ZimbraLog.session.debug(
                    "skipping serialization of too-large remote conversation: " + new ItemId(item));
              }
              continue;
            }
            boolean isVisible =
            ...
            if (((Mailbox) chg.what)
                .hasFullAccess(new OperationContext(getAuthenticatedAccountId()))) {
    ...
    private void forceConversationModification(
        ...
        PendingLocalModifications filtered,
        int changeMask) {
      ...
      if (pms.created != null && pms.created.containsKey(mkey)) {
        ...
        filtered.recordModified(
            (MailItem) existing.what, existing.why | changeMask, (MailItem) existing.preModifyObj);
      } else {
        try {
  ...
  class QueuedNotifications {
    ...
    /**
     * used by the Session object to ensure that notifications are reliably received by the listener
     */
    ...
    boolean hasNotifications(boolean localMailboxOnly) {
      if (localMailboxOnly
          ? mHasLocalChanges
          : mMailboxChanges != null && mMailboxChanges.hasNotifications()) return true;
      if (!localMailboxOnly && mRemoteChanges != null && mRemoteChanges.hasNotifications())
        return true;
      if (mExternalNotifications != null && !mExternalNotifications.isEmpty()) return true;
      return false;
    }
    ...
    void addNotification(PendingLocalModifications pms) {
      if (pms == null || !pms.hasNotifications()) return;
      if (mMailboxChanges == null) mMailboxChanges = new PendingLocalModifications();
      mMailboxChanges.add(pms);
      if (!mHasLocalChanges) mHasLocalChanges |= pms.overlapsWithAccount(mAuthenticatedAccountId);
    }

    void addNotification(RemoteNotifications rns) {
      if (mRemoteChanges == null) mRemoteChanges = rns;
      else mRemoteChanges.add(rns);
    }
  ...
  static final long SOAP_SESSION_TIMEOUT_MSEC =
      Math.max(5, LC.zimbra_session_timeout_soap.intValue()) * Constants.MILLIS_PER_SECOND;
  // if a keepalive request to a remote session failed, how long to wait before a new ping is
  // permitted
  ...
  private static final int MAX_QUEUED_NOTIFICATIONS =
      LC.zimbra_session_max_pending_notifications.intValue();
  ...
  private final Map<String, DelegateSession> delegateSessions =
      new HashMap<String, DelegateSession>(3);
  ...
  /**
   * Creates a <tt>SoapSession</tt> owned by the given account and listening on its {@link Mailbox}.
   *
  ...
  public Session getDelegateSession(String targetAccountId) {
    ...
    try {
      if (!Provisioning.onLocalServer(
          Provisioning.getInstance().get(Key.AccountBy.id, targetAccountId))) {
        return null;
      }
    } catch (ServiceException e) {
  ...
  void removeDelegateSession(DelegateSession ds) {
    ...
    synchronized (sentChanges) {
      ...
      ZimbraLog.session.debug(
          "removeDelegateSession: changing mForceRefresh: %d -> %d", forceRefresh, force);
      forceRefresh = force;
    }
  }
  ...
  protected void handleRemoteNotifications(
      Server server, Element context, boolean ignoreRefresh, boolean isPing) {
    if (context == null) {
      return;
    }
    ...
    boolean isSoap =
        eSession != null && eSession.getAttribute(HeaderConstants.A_TYPE, null) == null;
    ...
    if (!ignoreRefresh
        && !refreshExpected
        && context.getOptionalElement(ZimbraNamespace.E_REFRESH) != null) {
      int force = getCurrentNotificationSequence();
      ZimbraLog.session.debug(
          "handleRemoteNotifications: changing mForceRefresh: %d -> %d", forceRefresh, force);
      forceRefresh = force;
    }
  ...
  private void pingRemoteSessions(ZimbraSoapContext zsc) {
    ...
    for (RemoteSessionInfo rsi : needsPing) {
      try {
        ...
        ProxyTarget proxy =
            new ProxyTarget(server, zscProxy.getAuthToken(), URLUtil.getSoapURL(server, false));
  ...
  /**
   * Returns the number of messages added to the Mailbox since the time returned by {@link
   * #getPreviousSessionTime()}. Note that this means that messages added to the Mailbox during this
   * session are included in the count.
   */
  ...
  /**
   * Returns the time (in milliseconds) of last write op from any SOAP session <u>before</u> this
   * session was initiated. This value remains constant for the duration of this session.
   */
  ...
  /**
   * Returns the time (in milliseconds) of the last write operation initiated by this session. If
   * the session has not done any write ops yet, returns {@link #getPreviousSessionTime()}.
   */
  ...
  public synchronized RegisterNotificationResult registerNotificationConnection(
      final PushChannel sc) throws ServiceException {
    // don't have to lock the Mailbox before locking the Session to avoid deadlock because we're not
    // calling any ToXML functions
    if (pushChannel != null) {
   ...
   *
   * <p>Takes a set of new mailbox changes and caches it locally. This is currently initiated from
   * inside the Mailbox transaction commit, but we still shouldn't assume that execution of this
   * method is synchronized on the Mailbox.
   *
  ...
  public void notifyPendingChanges(PendingModifications pmsIn, int changeId, Session source) {
    ...
    if (source == this) {
      ...
      // keep track of "recent" message count: all present before the session started, plus all
      ...
      if (pms.created != null) {
        for (BaseItemInfo item : pms.created.values()) {
          if (item instanceof Message) {
            ...
            if (msg.getFolderId() == Mailbox.ID_FOLDER_SPAM
                || msg.getFolderId() == Mailbox.ID_FOLDER_TRASH) {
              isReceived = false;
  ...
  protected boolean skipNotifications(int notificationCount, boolean fromThisSession) {
    ...
    if (forceRefresh != currentSequence && MAX_QUEUED_NOTIFICATIONS > 0) {
      ...
      if (count > MAX_QUEUED_NOTIFICATIONS) {
        ...
        ZimbraLog.session.debug(
            "skipNotifications: changing mForceRefresh: %d -> %d", forceRefresh, force);
        forceRefresh = force;
      }
  ...
  private synchronized void notifyPushChannel(final PendingLocalModifications pms)
      throws ServiceException {
  ...
  private synchronized void notifyPushChannel(
      final PendingLocalModifications pms, final boolean clearChannel) throws ServiceException {
    // don't have to lock the Mailbox before locking the Session to avoid deadlock because we're not
    // calling any ToXML functions
    if (pushChannel == null) {
      return;
    ...
    if (pushChannel.localChangesOnly()
        && pms != null
        && !pms.overlapsWithAccount(mAuthenticatedAccountId)) {
      return;
    }
  ...
  public boolean requiresRefresh(final int lastSequence) {
    synchronized (sentChanges) {
      ...
      ZimbraLog.session.debug(
          "refresh required: forceRefresh=%d,lastSequence=%d,currentSequence=%d",
   ...
   *
   * <p>Adds a &lt;refresh> block to the existing &lt;context> element. This &lt;refresh> block
   * contains the basic folder, tag, and mailbox size information needed to display and update the
   * web UI's overview pane. The &lt;refresh> block is sent when a new session is created.
   *
   * <p>This API implicitly clears all cached notifications and therefore should only been used
  ...
  public void putRefresh(Element ctxt, ZimbraSoapContext zsc) throws ServiceException {
    ...
    // graft in subfolder trees from the other user's mailbox, making sure that mountpoints reflect
    // the counts (etc.) of the target folder
    if (!mountpoints.isEmpty()) {
      transferMountpointContents(
          eRefresh.getOptionalElement(MailConstants.E_FOLDER), octxt, mountpoints);
    }
  }
  ...
  private void expandLocalMountpoint(
    ...
    try {
      ...
      if (Provisioning.ACCOUNT_STATUS_MAINTENANCE.equals(owner.getAccountStatus(prov))
          || (!Provisioning.ACCOUNT_STATUS_ACTIVE.equals(owner.getAccountStatus(prov))
              && (!octxt.isUsingAdminPrivileges()
                  || !AccessManager.getInstance()
                      .canAccessAccount(octxt.getAuthenticatedUser(), owner)))) {
        mountpoints.put(iidTarget, new Pair<Boolean, Element>(true, null));
        return;
      }
      ...
      if (remote != null && remote.mFolder != null && !remote.mFolder.isHidden()) {
        ...
        Element subhierarchy =
            GetFolder.encodeFolderNode(remote, factory.createElement("ignored"), ifmt, octxt)
                .detach();
  ...
  private void expandRemoteMountpoints(
      OperationContext octxt,
    ...
    if (remoteServers != null && !remoteServers.isEmpty()) {
      ...
      for (Map.Entry<ItemId, Pair<Boolean, Element>> mptinfo : mountpoints.entrySet()) {
        ...
        Element remoteFolderElement =
            findRemoteFolder(
                iid.toString(mAuthenticatedAccountId), remoteHierarchies.get(iid.getAccountId()));
        mptinfo.setValue(
            new Pair<Boolean, Element>(remoteFolderElement == null, remoteFolderElement));
      }
    }
  }

  private Map<String, Element> fetchRemoteHierarchies(
      OperationContext octxt, ZimbraSoapContext zsc, Map<String, Server> remoteServers) {
    Map<String, Element> hierarchies = new HashMap<String, Element>();

    Element noop;
    try {
      noop =
          Element.create(zsc.getRequestProtocol(), MailConstants.GET_FOLDER_REQUEST)
  ...
  private static Element findRemoteFolder(String id, Element eFolder) {
    ...
    for (Element eSubfolder : eFolder.listElements()) {
      ...
      if (match != null) {
        // Assemble the absolute folder path in remote account. This will be used for calculating
        // REST url.
  ...
  private static void transferMountpointContents(
      Element elem, OperationContext octxt, Map<ItemId, Pair<Boolean, Element>> mountpoints)
      throws ServiceException {
    if (elem == null) {
      return;
    }
    ...
    if (elem.getName().equals(MailConstants.E_MOUNT)) {
      ItemId iidTarget =
          new ItemId(
              elem.getAttribute(MailConstants.A_ZIMBRA_ID, null),
              (int) elem.getAttributeLong(MailConstants.A_REMOTE_ID, -1));
   ...
   *
   * <p>Adds a <tt>&lt;notify></tt> block to an existing <tt>&lt;context></tt> element, creating an
   * enclosing <tt>&lt;context></tt> element if none is passed in. This <tt>&lt;notify></tt> block
   * contains information about all items deleted, created, or modified in the {@link Mailbox} since
   * the last client interaction, without regard to the client's state/views.
   *
   * <p>For deleted items, only the item IDs are returned. For created items, the entire item is
   * serialized. For modified items, only the modified attributes are included in the response.
   *
   * <p>Example:
   *
   * <pre>
   ...
   * @param lastSequence The highest notification-sequence-number that the client has received (0
   *     means none)
  ...
  public Element putNotifications(Element ctxt, ZimbraSoapContext zsc, int lastSequence) {
    ...
    // because ToXML functions can now call back into the Mailbox, don't hold any locks when calling
    // putQueuedNotifications
    ...
    synchronized (sentChanges) {
      ...
      if (sentChanges.size() > 20) {
        ZimbraLog.session.warn(
            "clearing abnormally long notification change list due to misbehaving client");
        sentChanges.clear();
      }
  ...
  /**
   * Size limit beyond which we suppress notifications on conversations belonging to other people's
   * mailboxes. We need to fetch the entire list of visible messages when serializing delegated
   * conversations. If it looks like it'd be too expensive to fetch that list, we just skip the
   * notification.
   */
  ...
  /**
   * Write a single instance of the PendingLocalModifications structure into the passed-in <ctxt>
   * block.
   */
  protected void putQueuedNotifications(
      Mailbox mbox, QueuedNotifications ntfn, Element parent, ZimbraSoapContext zsc) {
    ...
    try {
      ...
      ZimbraLog.session.warn(
          "error fetching operation context for: " + zsc.getAuthtokenAccountId(), e);
      return;
    }
    ...
    if (SoapTransport.NotificationFormat.valueOf(zsc.getNotificationFormat())
      ...
      try {
        ...
        Map<Integer, PendingFolderModifications> folderMods =
            PendingModifications.encodeIMAPFolderModifications(pms);
        ...
        for (BaseItemInfo item : pms.created.values()) {
          if (item instanceof MailItem) {
            ...
            ItemIdFormatter ifmt =
                new ItemIdFormatter(mAuthenticatedAccountId, mi.getMailbox(), false);
            try {
              ...
              if (item instanceof Mountpoint && mbox == mi.getMailbox()) {
                Map<ItemId, Pair<Boolean, Element>> mountpoints =
                    new HashMap<ItemId, Pair<Boolean, Element>>(2);
      ...
      if (hasRemoteCreates) {
        ...
        for (Element elt : rns.created) {
          if (encodingMatches(parent, elt)) {
            ...
            ZimbraLog.session.warn(
                "unable to add remote notification due to mismatched SOAP protocol");
          }
        }
      }
    }
   ...
   * callbacks as much
   */
  private void putExtraNotifications(
      QueuedNotifications ntfn, Element eNotify, ItemIdFormatter ifmt) {
  ...
  public synchronized void putQueryResults(
      String query, String groupBy, String sortBy, ZimbraQueryResults res) {
    clearCachedQueryResults();
  ...
  public synchronized ZimbraQueryResults getQueryResults(
      String query, String groupBy, String sortBy) {
