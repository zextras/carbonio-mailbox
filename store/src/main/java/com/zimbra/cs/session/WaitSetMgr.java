...
public class WaitSetMgr {
  ...
  private static final int MAX_WAITSETS_PER_NONADMIN_ACCOUNT =
      LC.zimbra_waitset_max_per_account.intValueWithinRange(1, Integer.MAX_VALUE);
      ...
      new TimerTask() {
        ...
        public void run() {
          try {
            ...
            if (e instanceof OutOfMemoryError) Zimbra.halt("Caught out of memory error", e);
  ...
  private static final HashMap<String /*AccountId*/, List<String /*WaitSetId*/>>
      sWaitSetsByAccountId = new HashMap<String, List<String>>();

  private static final int WAITSET_SWEEP_DELAY = 1000 * 60; // once every minute

  private static final int WAITSET_TIMEOUT =
      (int) (LC.zimbra_active_waitset_timeout_minutes.intValue() * Constants.MILLIS_PER_MINUTE);

  /**
   * Create a new WaitSet, optionally specifying an initial set of accounts to start listening on
   *
   * <p>WaitSets are stored in a serverwide cache and are stamped with a last-accessed-time,
   * therefore callers should *not* cache the WaitSet pointer beyond a few seconds they should
   * instead use the lookup() API to fetch WaitSets between calls
   *
   * @param ownerAccountId Account ID of the owner/creator
   * @param allowMultiple If FALSE, then the create fails if there is already a waitset for this
   *     account
   ...
   */
  public static Pair<String, List<WaitSetError>> create(
  ...
  /**
   * WaitSets that are targeted at "all accounts" can be used across server restarts (they are
   * simply re-created)
   *
   * @param ownerAccountId Account ID of the owner/creator
   */
  public static IWaitSet lookupOrCreateForAllAccts(
      String ownerAccountId, String id, Set<MailItem.Type> defaultInterests, String lastKnownSeqNo)
      throws ServiceException {
    synchronized (sWaitSets) {
      if (!id.startsWith(ALL_ACCOUNTS_ID_PREFIX)) {
        throw ServiceException.INVALID_REQUEST(
            "Called WaitSetMgr.lookupOrCreate but wasn't an 'All-' waitset ID", null);
      }

      IWaitSet toRet = lookup(id);
      if (toRet == null) {
        // oops, it's gone!  Try to re-create it given the last known sequence number
        AllAccountsWaitSet ws =
            AllAccountsWaitSet.createWithSeqNo(
                ownerAccountId, id, defaultInterests, lastKnownSeqNo);
        toRet = ws;
        ws.setLastAccessedTime(System.currentTimeMillis());
  ...
  private static void sweep() {
    ...
    if (activeSets > 0) {
      ZimbraLog.session.info(
          "WaitSet sweeper: %d active WaitSets (%d accounts) - %d sets with blocked callbacks",
  ...
  public static void checkRightForAdditionalAccount(String acctId, ZimbraSoapContext zsc)
    ...
    if (acct == null) throw ServiceException.DEFEND_ACCOUNT_HARVEST(acctId);

    if (!AccessManager.getInstance()
        .canAccessAccount(zsc.getAuthToken(), acct, zsc.isUsingAdminPrivileges()))
