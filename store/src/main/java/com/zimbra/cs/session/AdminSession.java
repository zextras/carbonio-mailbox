...
public class AdminSession extends Session {
  ...
  public List<NamedEntry> searchDirectory(
      SearchDirectoryOptions searchOpts, int offset, NamedEntry.CheckRight rightChecker)
      throws ServiceException {

    DirectorySearchParams params = new DirectorySearchParams(searchOpts, rightChecker);

    boolean needToSearch =
        (offset == 0) || (mSearchParams == null) || !mSearchParams.equals(params);
    // ZimbraLog.account.info("this="+this+" mSearchParams="+mSearchParams+"
    // equal="+!params.equals(mSearchParams));
    if (needToSearch) {
