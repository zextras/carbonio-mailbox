...
public final class AllAccountsWaitSet extends WaitSetBase {
  ...
  private static Map<AllAccountsWaitSet, String> sAllAccountsWaitSets =
      new ConcurrentHashMap<AllAccountsWaitSet, String>();
  ...
  private String
      mNextSeqNo; // set to the commitId of the most recently signalled event....we use this to
  // update mCurrentSeqNo when we send data..

  /** Callback from the Mailbox object when a transaction has completed in some Mailbox */
  public static final void mailboxChangeCommitted(
      String commitIdStr, String accountId, Set<MailItem.Type> changedTypes) {
  ...
  protected static AllAccountsWaitSet create(
      String ownerAccountId, String id, Set<MailItem.Type> defaultInterest) {
    return new AllAccountsWaitSet(ownerAccountId, id, defaultInterest, false);
  }

  /**
   * Used for creating an AllAccountsWaitSet when we've got a seqno -- basically this happens when
   * the client's WaitSet has gone away and the client needs to re-sync.
   */
  protected static AllAccountsWaitSet createWithSeqNo(
      String ownerAccountId, String id, Set<MailItem.Type> defaultInterest, String lastKnownSeqNo)
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "Caught IOException when syncing waitset to specified commit ID", e);
    } finally {
      if (!success) {
    ...
    synchronized (sAllAccountsWaitSets) {
      ...
      if (ZimbraLog.session.isDebugEnabled()) {
        ZimbraLog.session.debug(
            "added: sAllAccountsWaitSets.size() = " + sAllAccountsWaitSets.size());
      }
  ...
  private synchronized void onMailboxChangeCommitted(
      String commitIdStr, String accountId, Set<MailItem.Type> changedTypes) {
    if (!Collections.disjoint(changedTypes, defaultInterest)) {
      if (mBufferedCommits != null) {
        mBufferedCommits.add(
            new Pair<String /*acctId*/, String /*commitId*/>(accountId, commitIdStr));
      } else {
    ...
    for (Integer id : mailboxes) {
      try {
        ...
        ZimbraLog.session.warn(
            "AllAccountsWaitSet skipping notification of mailbox id %d because we could not fetch"
                + " it",
            id, e);
      }
    }
  ...
  protected Map<String, WaitSetAccount> destroy() {
    synchronized (sAllAccountsWaitSets) {
      ...
      if (ZimbraLog.session.isDebugEnabled()) {
        ZimbraLog.session.debug(
            "removed: sAllAccountsWaitSets.size() = " + sAllAccountsWaitSets.size());
      }
