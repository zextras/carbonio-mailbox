...
public class WaitSetAccount {
  ...
  public WaitSetAccount(
      String id, SyncToken sync, Set<MailItem.Type> interest, Set<Integer> folderInterests) {
  ...
  public WaitSetSession getSession() {
    if (sessionId != null) {
      try {
        ...
        if (mbox != null) return (WaitSetSession) mbox.getListener(sessionId);
      } catch (ServiceException e) {
        ZimbraLog.session.info(
            "Caught exception fetching mailbox in WaitSetAccount.getSession()", e);
      }
    }
    return null;
  }
  ...
  /**
   * The mailbox is loaded and in memory -- so create a {@link WaitSetSession} and add it as a
   * listener to the mailbox
   *
   * <p>
   *
   * @param mbox
  ...
  public WaitSetError createSession(Mailbox mbox, SomeAccountsWaitSet ws) {
    ...
    //
    WaitSetSession session =
        new WaitSetSession(ws, accountId, interests, folderInterests, lastKnownSyncToken);
    ZimbraLog.session.debug(
        "Created WaitSetSession %s for waitset %s on account %s",
        session.getSessionId(), ws.getWaitSetId(), accountId);
    mbox.lock.lock();
    try {
      session.register();
      ...
      if (e.getCode().equals(MailServiceException.MAINTENANCE)) {
        ...
        // wsa.setRef(null);
        ZimbraLog.session.debug(
            "Maintenance mode trying to initialize WaitSetSession for accountId " + accountId);
      } else {
        ZimbraLog.session.warn(
            "Error initializing WaitSetSession for accountId "
                + accountId
      ...
      sessionId = null;
      ZimbraLog.session.warn(
          "Error initializing WaitSetSession for accountId " + accountId + " -- ServiceException",
          e);
  ...
  private Mailbox getMailboxIfLoaded() throws ServiceException {
    Mailbox mbox =
        MailboxManager.getInstance()
            .getMailboxByAccountId(accountId, MailboxManager.FetchMode.ONLY_IF_CACHED);
    return mbox;
  }
