...
/**
 * MessageChannel is a service in ZCS that allows a message to be sent from one mailboxd to another.
 * There can be multiple apps registered to share the single underlying iochannel without creating
 * its own server socket. Each application can register its own type of Message it wants to send and
 * receive, and MessageChannel will alert the app when a message for its type is received.
 *
 * @author jylee
 */
public class MessageChannel {
  ...
  /**
   * Sends the message to peer server that hosts Account identified in
