...
public class MailboxNotification extends Message {
  ...
  private static final ExecutorService executor =
      newCachedThreadPool(newDaemonThreadFactory("MailboxNotification"));

  public static MailboxNotification create(String accountId, int changeId, byte[] data)
        ...
        new Runnable() {
          ...
          public void run() {
            Collection<Session> sessions =
                SessionCache.getAllSessions(message.getRecipientAccountId());
              ...
              if (pms == null) {
                try {
                  ...
                  if ((null == mboxStore) || (mboxStore instanceof Mailbox)) {
                    pms =
                        PendingLocalModifications.deserialize(
                            (Mailbox) mboxStore, message.getPayload());
                  } else {
                    log.warn(
                        "could not deserialize notification for non-Mailbox MailboxStore '%s'",
