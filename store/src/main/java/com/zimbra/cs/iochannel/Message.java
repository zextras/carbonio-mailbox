...
/**
 * Each Message contains payload and recipient. MessageChannel framework locates the home server for
 * the recipient, serializes the message, then sends the packet to the server where the recipient is
 * homed.
 *
 * @author jylee
 */
public abstract class Message {
  ...
  /**
   * appId is a unique identifier to let MessageChannel to find correct message instance for
   * unmarshalling.
   */
  ...
  /**
   * Size in number of bytes. It's a hint used during marshalling. When the exact size is unknown
   * use a number greater than actual size
   */
  ...
  /**
   * MessageHandler is an object that gets called when a Message is received from a peer server on
   * MessageChannel. Each Message subclass must have its own unique MessageHandler object that knows
