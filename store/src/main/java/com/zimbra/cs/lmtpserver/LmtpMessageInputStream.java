...
/**
 * Wraps an <tt>InputStream</tt> and reads an LMTP message from the current position in the stream
 * according to the rules specified in RFC 2821:
 *
 * <ul>
 *   <li>If <tt>CRLF.CRLF</tt> is encountered, returns everything up to the first <tt>CRLF</tt>,
 *       strips the trailing <tt>.CRLF</tt> and stops reading.
 *   <li>If <tt>CRLF.</tt> is encountered without a trailing <tt>CRLF</tt>, removes the <tt>.</tt>
 *       from the stream (transparency, section 4.5.2)
 * </ul>
 *
 * The transparency implementation allows a user-entered line that begins with a dot.
 *
 * <p>
 *
 * @throws IOException if <tt>EOF</tt> is encountered before <tt>CRLF.CRLF</tt> or there's an error
...
public class LmtpMessageInputStream extends InputStream {
  ...
  /**
   * <tt>true</tt> if the last character was <tt>CR</tt> or the last two characters were
   * <tt>CRLF</tt>.
   */
  ...
  public int read() throws IOException {
    ...
    if (c == -1) {
      ...
      throw new IOException(
          "End of stream encountered after '<CR><LF>.<CR>' when looking for '<LF>'");
    }
    if (c != LF) {
  ...
  /**
   * Does what {@link super#read(byte[], int, int)} does except that IOException thrown by {@link
   * super#read()} is never caught.
   *
   * @param b
   * @param off
   * @param len
