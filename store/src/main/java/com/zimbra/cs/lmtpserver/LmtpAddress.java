...
public class LmtpAddress {
  ...
  public LmtpAddress(String arg, String[] allowedParameters, String rcptDelim) {
    ...
    if (mLocalPart != null
        && rcptDelim != null
        && (delimIndex = mLocalPart.indexOf(rcptDelim)) > 0) {
  ...
  /**
   * 'offset' is the *index* in the array of the next char that we have not processed (other than
   * lookahead). Alternately, this is also the length of the part that we have already processed
   * (think about it whichever way works for you). For instance if the content is:
   *
   * <p><x@y.com> 012345678
   *
   * <p>After local part processing, offset should be 2. After domain parsing, offset should be 8.
   *
   * <p>Yes, the naming convention here takes a break from 'm' prefixing. It's just more readable
   * this way.
   */
  private int offset;
   ...
   *
   * <p>When checking for the presence of something optional, ie it may or may not be there, use
   * peek()
   */
  private int peek() {
    if (offset < length) {
   ...
   *
   * <p>If the optional item was there, and you noticed it via peek(), you might want to skip it.
   */
  private void skip() {
    offset++;
  }
   ...
   *
   * <p>Example of what is skipped from RFC 2821/Section 4.1.1.3:
   *
   * <p>RCPT TO:<@hosta.int,@jkl.org:userc@d.bar.org> _________^^^^^^^^^^^^^^^^^^^^________________
   */
