...
public class LmtpInject {
  ...
  public void incSuccess() {
    ...
    if (report && !mQuietMode) {
      ...
      double rate = 0.0;
      if (elapsed > 0) rate = howmany * 1000.0 / elapsed;

      long elapsedTotal = now - startTime;
      double rateAvg = 0.0;
      if (elapsedTotal > 0) rateAvg = count * 1000.0 / elapsedTotal;

      System.out.printf(
          "[progress] " + "%d msgs in %dms @ %.2fmps; " + "last %d msgs in %dms @ %.2fmps\n",
  ...
  /** Returns the next file and increments the current file index. */
    ...
    public void run() {
      ...
      while (file != null) {
        ...
        try {
          ...
          ok =
              mClient.sendMessage(
                  in, mDriver.getRecipients(), mDriver.getSender(), file.getName(), dataLength);
          if (ok) {
            mDriver.incSuccess();
  ...
  static {
    ...
    mOptions.addOption(
        OptionBuilder.withLongOpt("sender")
            .hasArg(true)
            .withDescription("envelope sender (mail from)")
            .create("s"));
    Option ropt =
        new Option(
            "r",
            "recipient",
            true,
            "envelope recipients (rcpt to).  This option accepts multiple arguments, so it can't be"
                + " last if a list of input files is used.");
    ropt.setArgs(Option.UNLIMITED_VALUES);
    mOptions.addOption(
        OptionBuilder.withLongOpt("recipient")
            .hasArgs(Option.UNLIMITED_VALUES)
            .withDescription(
                "envelope recipients (rcpt to).  This option accepts multiple arguments, so it"
                    + " can't be last if a list of input files is used.")
            .create("r"));
  ...
  public static void main(String[] args) {
    ...
    if (cl.hasOption("p")) port = Integer.valueOf(cl.getOptionValue("p")).intValue();
