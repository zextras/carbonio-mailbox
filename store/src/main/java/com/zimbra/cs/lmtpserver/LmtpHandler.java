...
public abstract class LmtpHandler extends ProtocolHandler {
  ...
  LmtpHandler(LmtpServer server, MeterRegistry meterRegistry) {
  ...
  private void doRCPT(String arg) {
    ...
    if (reply.success()) {
      if (addr.isOnLocalServer()) mEnvelope.addLocalRecipient(addr);
      else mEnvelope.addRemoteRecipient(addr);
    }
    sendReply(reply);
  }
  ...
  protected void processMessageData(LmtpMessageInputStream in) {
    ...
    try {
      ...
      ZimbraLog.lmtp.error(
          "Unrecoverable error while handling DATA command.  Dropping connection.", e);
      try {
  ...
  private void finishMessageData(long size) {
    ...
    Counter.builder(LMTP_RCVD_MSGS).register(meterRegistry).increment();
    Counter.builder(LMTP_RCVD_BYTES).register(meterRegistry).increment(size);
    Counter.builder(LMTP_RCVD_RCPT).register(meterRegistry).increment(numRecipients);
    ...
    Counter.builder(LMTP_DLVD_MSGS).register(meterRegistry).increment(numDelivered);
    Counter.builder(LMTP_DLVD_BYTES).register(meterRegistry).increment(numDelivered * size);
  ...
  protected String getAdditionalHeaders() {
    ...
    String value =
        String.format(
            "from %s (LHLO %s) (%s) by %s with LMTP; %s",
  ...
  protected abstract void doSTARTTLS(String arg) throws IOException;
