 ...
 *   <li>Add {@code X-Zimbra-DL: DL1, DL2...} header if one or more recipients are a DL. {@code
 *       List-Id} header (RFC 2919) is not supported because it requires to fork the message. Let's
 *       say, "To: listA, listB", user1 belongs to listA, user2 belongs to listB. The user1 should
 *       receive "List-Id: listA", and the user2 should receive "List-Id: listB", which requires
 *       Milter to fork the message into each DLs. However, this is not supported by Milter.
...
public final class MilterHandler implements NioHandler {
  ...
  private static final byte SMFIR_CONTINUE =
      'c'; // Accept and keep processing (accept/reject action)
  // private static final byte SMFIR_DISCARD = 'd'; // Set discard flag for entire message
  ...
  // private static final byte SMFIR_REJECT = 'r'; // Reject command/recipient with a 5xx
  // (accept/reject action)
  private static final byte SMFIR_TEMPFAIL =
      't'; // Reject command/recipient with a 4xx (accept/reject action)
  private static final byte SMFIR_REPLYCODE =
      'y'; // Send specific Nxx reply message (accept/reject action)
  ...
  public void connectionIdle() throws IOException {
    ZimbraLog.milter.info(
        "Dropping connection because inactive for more than %s seconds (milter_max_idle_time)",
  ...
  private void processCommand(MilterPacket command) throws IOException, ServiceException {
    switch ((char) command.getCommand()) {
      ...
      case 'Q':
        SMFIC_Quit();
        break;
      default: // for unimplemented commands that require responses, always return "Continue" for
        // now
        ZimbraLog.milter.debug(
            "Unimplemended command, sending SMFIR_CONTINUE: %s", command.toString());
    ...
    if (prov.isDistributionList(rcpt)) {
      ...
      if (group != null) {
        if (!accessMgr.canDo(sender, group, User.R_sendToDistList, false)) {
          ...
          SMFIR_ReplyCode(
              "571", "571 Sender is not allowed to email this distribution list: " + rcpt);
          return;
        }
        lists.add(group);
