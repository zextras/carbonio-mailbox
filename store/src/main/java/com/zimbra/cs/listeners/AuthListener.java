...
public abstract class AuthListener {
  ...
  private static Map<String, AuthListenerEntry> mListeners =
      Collections.synchronizedMap(new HashMap<String, AuthListenerEntry>());

  public static void registerListener(String listenerName, Priority priority, AuthListener listener)
      throws ServiceException {
    if (mListeners.size() == LC.zimbra_listeners_maxsize.intValue()) {
      ZimbraLog.account.warn(
          "Auth listener limit reached, registering of %s is ignored", listenerName);
    ...
    if (obj != null) {
      ZimbraLog.account.warn(
          ...
          listenerName, obj.getAuthListener().getClass().getCanonicalName());
  ...
  public static void invokeOnException(ServiceException exceptionThrown) {
    if (exceptionThrown instanceof AuthFailedServiceException) {
      ZimbraLog.account.info(
          "Error occurred during authentication: %s. Reason: %s.",
          exceptionThrown.getMessage(), ((AuthFailedServiceException) exceptionThrown).getReason());
    } else {
      ZimbraLog.account.info(
          "Error occurred during authentication: %s", exceptionThrown.getMessage());
    }
  ...
  /**
   * called after a successful authentication. should not throw any exceptions.
   *
