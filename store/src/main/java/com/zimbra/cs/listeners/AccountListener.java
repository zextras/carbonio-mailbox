...
public abstract class AccountListener {
  ...
  private static Map<String, AccountListenerEntry> mListeners =
      Collections.synchronizedMap(new HashMap<String, AccountListenerEntry>());

  public static void registerListener(
      String listenerName, Priority priority, AccountListener listener) throws ServiceException {
    if (mListeners.size() == LC.zimbra_listeners_maxsize.intValue()) {
      ZimbraLog.account.warn(
          "Account listener limit reached, registering of %s is ignored", listenerName);
      throw ServiceException.FORBIDDEN("Account listener limit reached");
    }
    AccountListenerEntry obj = mListeners.get(listenerName);
    if (obj != null) {
      ZimbraLog.account.warn(
          "Listener %s is already registered, registering of %s is ignored",
          listenerName, obj.getAccountListener().getClass().getCanonicalName());
  ...
  public static void invokeOnStatusChange(Account acct, String oldStatus, String newStatus)
    ...
    ZimbraLog.account.info(
        "Account status for %s changed from '%s' to '%s'", acct.getName(), oldStatus, newStatus);
    // invoke listeners
    Map<String, AccountListenerEntry> sortedListeners = ListenerUtil.sortByPriority(mListeners);
    for (Map.Entry<String, AccountListenerEntry> listener : sortedListeners.entrySet()) {
      AccountListenerEntry listenerInstance = listener.getValue();
  ...
  public static void invokeOnException(ServiceException exceptionThrown) {
    ZimbraLog.account.info(
        "Error occurred during account creation: %s", exceptionThrown.getMessage());
  ...
  /**
   * called after a successful account creation. should not throw any exceptions.
   *
