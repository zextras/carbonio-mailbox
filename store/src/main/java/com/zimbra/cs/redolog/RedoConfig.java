...
public class RedoConfig {
  ...
  private void reloadInstance() throws ServiceException {
    ...
    mRedoLogPath =
        Config.getPathRelativeToZimbraHome(
                config.getAttr(Provisioning.A_zimbraRedoLogLogPath, D_REDOLOG_PATH))
            .getAbsolutePath();
    mRedoLogArchiveDir =
        Config.getPathRelativeToZimbraHome(
                config.getAttr(Provisioning.A_zimbraRedoLogArchiveDir, D_REDOLOG_ARCHIVEDIR))
            .getAbsolutePath();
    mRedoLogRolloverFileSizeKB =
        config.getLongAttr(
            Provisioning.A_zimbraRedoLogRolloverFileSizeKB, D_REDOLOG_ROLLOVER_FILESIZE_KB);
    mRedoLogRolloverHardMaxFileSizeKB =
        config.getLongAttr(
            Provisioning.A_zimbraRedoLogRolloverHardMaxFileSizeKB,
            D_REDOLOG_ROLLOVER_HARDMAX_FILESIZE_KB);
    mRedoLogRolloverMinFileAge =
        config.getLongAttr(
            Provisioning.A_zimbraRedoLogRolloverMinFileAge, D_REDOLOG_ROLLOVER_MIN_FILE_AGE);
    mRedoLogDeleteOnRollover =
        config.getBooleanAttr(
            Provisioning.A_zimbraRedoLogDeleteOnRollover, D_REDOLOG_DELETE_ON_ROLLOVER);
    mRedoLogFsyncIntervalMS =
        config.getLongAttr(
            Provisioning.A_zimbraRedoLogFsyncIntervalMS, D_REDOLOG_FSYNC_INTERVAL_MS);

    mRedoLogCrashRecoveryLookbackSec =
        config.getLongAttr(
            Provisioning.A_zimbraRedoLogCrashRecoveryLookbackSec,
  ...
  /**
   * The path to the redo.log file. Relative path is resolved against ZIMBRA_HOME. Default value is
   * "$ZIMBRA_HOME/redolog/redo.log".
   *
  ...
  /**
   * Directory in which redo logs are archived. When the current redo.log file reaches a certain
   * threshold, it is rolled over and archived. That is, the current redo.log file is renamed to a
   * timestamped name, moved into the archive directory, and a new empty redo.log file is created.
   *
  ...
  /**
   * Returns the redolog rollover threshold filesize. The current redolog file is rolled over if it
   * reaches or exceeds this size and file is old enough.
   *
  ...
  /**
   * Returns the redolog rollover hard threshold filesize. The current redolog file is rolled over
   * if it reaches or exceeds this size, regardless of file age.
   *
   * @return threshold filesize in kilobytes
   */
  ...
  /**
   * Returns the redolog rollover minimum file age. The current redolog file is rolled over if it is
   * older than this and reaches the size threshold.
   *
  ...
  /**
   * If true, redo log file is deleted and discarded upon rollover. If false, redo log being rolled
   * over is archived.
   *
   * @return
   */
  ...
  /**
   * Returns the fsync interval for flush/fsync thread. Writes to the log are written securely to
   * disk by forcing an fsync. But fsyncs are very slow, so instead of each logging thread calling
   * fsync individually, they wait for a dedicated thread to fsync the accumulated changes
   * periodically. This configuration value controls the interval between the fsyncs.
   *
   * <p>With a longer interval, there will be fewer fsyncs compared to the number of logging calls.
   * This can improve throughput under heavy load but increases the latency on individual logging
   * calls.
   *
  ...
  /**
   * This parameter is also related to running mysql with innodb_flush_log_at_trx_commit=0. When
   * recovering from a crash, mysql may not have the committed changes from roughly the last second.
   * ZCS must re-execute enough past operations to bring mysql into consistent state.
   *
   * <p>This parameter controls how long to look back. Default is 10 seconds. Crash recovery
   * normally re-executes only pending changes. But with this parameter committed changes within the
   * last 10 seconds are also re-executed.
   *
   * @return
   */
