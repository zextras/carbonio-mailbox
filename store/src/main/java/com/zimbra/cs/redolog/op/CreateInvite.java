...
public class CreateInvite extends RedoableOp
  ...
  protected void serializeData(RedoLogOutput out) throws IOException {
    ...
    if (getVersion().atLeast(1, 1)) out.writeUTF(mCalendarItemPartStat);
    out.writeInt(mFolderId);
    if (getVersion().atLeast(1, 0)) out.writeShort((short) -1);
    ...
    if (getVersion().atLeast(1, 23)) out.writeBoolean(mAddRevision);
  }

  @Override
  protected void deserializeData(RedoLogInput in) throws IOException {
    mCalendarItemId = in.readInt();
    if (getVersion().atLeast(1, 1)) mCalendarItemPartStat = in.readUTF();
    mFolderId = in.readInt();
    if (getVersion().atLeast(1, 0)) in.readShort();
    ...
    if (getVersion().atLeast(1, 23)) mAddRevision = in.readBoolean();
    else mAddRevision = false;
  }
  ...
  public void redo() throws Exception {
    ...
    mbox.addInvite(
        ...
        mFolderId,
        pm,
        mPreserveExistingAlarms,
        mDiscardExistingInvites,
        mAddRevision);
  }
}
