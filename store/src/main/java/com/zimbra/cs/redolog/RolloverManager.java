...
public class RolloverManager {
  ...
  /** Recovers from a previous process crash in the middle of RolloverManager.rollover(). */
  public void crashRecovery() throws IOException {
    ...
    if (logs.length > 0) {
      ...
      if (!mRedoLogFile.exists()) {
        ...
        if (mostRecent.renameTo(mRedoLogFile)) mLog.info("Renamed " + name + " to " + currName);
        else throw new IOException("Unable to rename " + name + " to " + currName);
      ...
      for (int i = 0; i < logs.length && logs[i] != null; i++) {
        ...
        if (log.renameTo(newLog)) mLog.info("Renamed " + oldName + " to " + newName);
        else {
          numErrors++;
  ...
  /**
   * Returns the archive log files in the specified directory, sorted by the sequence number encoded
   * in the filename.
   *
   * @param archiveDir
   * @return
            ...
            new FilenameFilter() {
              public boolean accept(File dir, String name) {
                if (name.indexOf(ARCH_FILENAME_PREFIX) == 0
                    && name.lastIndexOf(FILENAME_SUFFIX)
                        == name.length() - FILENAME_SUFFIX.length()) {
                  long seq = getSeqForFile(new File(dir, name));
                  if (from <= seq && seq <= to) return true;
                }
                return false;
              }
            });
    if (logs != null && logs.length > 0) RolloverManager.sortArchiveLogFiles(logs);
    return logs;
  }

  /**
   * Sorts an array of archive log files by the timestamp encoded into the filenames.
   *
   * @param files
   */
   ...
   * comparison.
   */
  private static class ArchiveLogFilenameComparator implements Comparator<File> {
  ...
  public synchronized long incrementSequence() {
    if (mSequence < Long.MAX_VALUE) ++mSequence;
    else mSequence = 0;
    return mSequence;
  }
}
