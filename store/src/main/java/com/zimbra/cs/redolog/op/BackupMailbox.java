...
/**
 * THIS REDOLOG OPERATION IS DEPRECATED. Backing up a mailbox will no longer log this operation in
 * redolog.
 *
 * <p>This operation is a marker within a redo log file to help locate the redo log file that
 * corresponds to a particular backup. When restoring a mailbox from backup, the mailbox is
 * reinitialized first, the data from the most recent backup is restored, and finally all redos
 * since that backup should be replayed. This marker helps us determine where to start doing the
 * redos.
 */
public class BackupMailbox extends RedoableOp {

  private long
      mBackupSetTstamp; // timestamp of when backup set started (backup set = one or more mailboxes)
  ...
  public BackupMailbox(
      int mailboxId, long backupSetTstamp, long startTime, long endTime, String label) {
    this();
  ...
  protected String getPrintableData() {
    ...
    if (mLabel != null) sb.append("label=\"").append(mLabel).append("\"");
    return sb.toString();
  }
