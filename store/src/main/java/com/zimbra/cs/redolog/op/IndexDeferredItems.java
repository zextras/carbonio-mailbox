...
public class IndexDeferredItems extends RedoableOp {
  ...
  protected void serializeData(RedoLogOutput out) throws IOException {
    ...
    for (int i = 0; i < mItemIds.length; i++) out.writeInt(mItemIds[i]);
    for (int i = 0; i < mItemIds.length; i++) out.writeByte(mItemTypes[i]);
  }
  ...
  protected void deserializeData(RedoLogInput in) throws IOException {
    ...
    for (int i = 0; i < count; i++) mItemIds[i] = in.readInt();
    for (int i = 0; i < count; i++) mItemTypes[i] = in.readByte();
  }
