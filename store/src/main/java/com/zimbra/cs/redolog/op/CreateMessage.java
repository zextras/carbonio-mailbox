...
public class CreateMessage extends RedoableOp
  ...
  private List<Integer>
      mMergedConvIds; // existing conversations to merge into the message's conversation
  ...
  private int
      mCalendarItemId; // new calendar item created if this is meeting or task invite message
  ...
  public byte[] getMessageBody() throws IOException {
    if (mMsgBodyType == MSGBODY_LINK) return null;
    ...
    if (mMsgBodyType == MSGBODY_INLINE) {
      ...
      if (inMemory) {
        ...
        if (numSkipped != dataLength) {
          String msg =
              String.format(
                  "Attempted to skip %d bytes at position %d in %s, but actually skipped %d.",
    ...
    if (mMsgBodyType == MSGBODY_LINK) {
      ...
      if (blob == null)
        throw new RedoException(
            "Missing link source blob " + mPath + " (digest=" + mDigest + ")", this);
