...
public class CreateNote extends RedoableOp {
  ...
  public CreateNote(
      int mailboxId, int folderId, String content, Color color, Note.Rectangle bounds) {
    this();
  ...
  protected void deserializeData(RedoLogInput in) throws IOException {
    ...
    in.readShort();
    mContent = in.readUTF();
    if (getVersion().atLeast(1, 27)) mColor = in.readLong();
    else mColor = in.readByte();
    int x = in.readInt();
    int y = in.readInt();
    int w = in.readInt();
    int h = in.readInt();
  ...
  public void redo() throws Exception {
    ...
    try {
      mailbox.createNote(
          getOperationContext(), mContent, mBounds, Color.fromMetadata(mColor), mFolderId);
    } catch (MailServiceException e) {
      String code = e.getCode();
      if (code.equals(MailServiceException.ALREADY_EXISTS)) {
        if (mLog.isInfoEnabled()) mLog.info("Note " + mId + " already exists in mailbox " + mboxId);
      } else throw e;
    }
  }
}
