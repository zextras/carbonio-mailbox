...
public class FileLogWriter implements LogWriter {
  ...
  public synchronized void open() throws IOException {
    ...
    if (mFsyncIntervalMS > 0) startFsyncThread();
  }
  ...
  public synchronized void close() throws IOException {
    ...
    synchronized (mLock) {
      if (mRAF != null) {
        if (mLastOpTstamp != 0) mHeader.setLastOpTstamp(mLastOpTstamp);
  ...
  /**
   * Log the supplied bytes. Depending on the value of synchronous argument and the setting of fsync
   * interval, this method can do one of 3 things:
   *
   * <p>case 1: !synchronous action: write() only; no flush/fsync
   *
   * <p>case 2: synchronous && fsyncInterval > 0 action: write(), then wait() until notified by
   * fsync thread Current thread only calls write() on the RandomAccessFile, and blocks to wait for
   * the next scheduled fsync. Fsync thread periodically flushes the output stream and fsync's the
   * file. After each fsync, all waiting logger threads are notified to continue. This method
   * batches multiple log items before each fsync, and results in greater throughput than calling
   * fsync after each log item because fsync to physical disk is a high-latency operation.
   *
   * <p>case 3: synchronous && fsyncInterval <= 0 action: write(), then fsync() in the current
   * thread Fsync is required, but the sleep interval for fsync thread is 0. We special case this
   * condition to mean fsync should be done by the calling thread.
   */
  @Override
  public void log(RedoableOp op, InputStream data, boolean synchronous) throws IOException {
    ...
    synchronized (mLock) {
      if (mRAF == null) throw new IOException("Redolog file closed");
    ...
    if (RedoConfig.redoLogDeleteOnRollover()) {
      ...
      if (!mFile.renameTo(rolloverFile))
        throw new IOException(
            "Unable to rename current redo log to " + rolloverFile.getAbsolutePath());
    }
  ...
  private void fsync() throws IOException {
    ...
    synchronized (mLock) {
      if (mFsyncSeq < mLogSeq) {
        ...
        fsyncNeeded = true;
        seq = mLogSeq;
        if (!mFsyncDisabled) mFsyncCount++;
      }
    }
    if (fsyncNeeded) {
      if (!mFsyncDisabled) {
        synchronized (mLock) {
          if (mRAF != null) mRAF.getChannel().force(false);
          else throw new IOException("Redolog file closed");
    ...
    public FsyncThread(long fsyncIntervalMS) {
      ...
      if (fsyncIntervalMS < MIN_SLEEP_MILLIS) {
        ZimbraLog.redolog.warn(
            "Invalid fsync thread sleep interval %dms; using %dms instead",
            fsyncIntervalMS, MIN_SLEEP_MILLIS);
        mSleepMS = MIN_SLEEP_MILLIS;
      } else if (fsyncIntervalMS > MAX_SLEEP_MILLIS) {
        ZimbraLog.redolog.warn(
            "Fsync thread sleep interval %ms is too long; using %dms instead",
