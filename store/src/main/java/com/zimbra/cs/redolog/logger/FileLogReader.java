...
public class FileLogReader {
    ...
    while (true) {
      try {
        ...
        if (!first) {
          String msg =
              String.format(
                  "Skipped bad bytes in redolog %s; resuming at offset 0x%08x after skipping %d"
                      + " bytes",
  ...
  private static int searchByteArray(
      byte[] searchIn, int startOffset, int endOffset, byte[] pattern) {
