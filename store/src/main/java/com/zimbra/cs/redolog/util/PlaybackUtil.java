 ...
 *
 * <p>This program is intended to be run with mailboxd process stopped. Some diagnostic information
 * is logged to stdout/stderr. Most of the logging goes to /opt/zextras/log/mailbox.log, with log
 * level controlled by /opt/zextras/conf/log4j.properties file.
 *
 * <p>When it is run without options it replays all redologs found under
 * /opt/zextras/redolog/archive directory, in sequence order, followed by
 * /opt/zextras/redolog/redo.log. For each redolog all committed operations are replayed.
 *
 * <p>Specify --mailboxId <mailbox id> option to replay committed operations for that mailbox only.
 *
 * <p>Specify --logfiles <list of redolog files> option to replay only those redologs, in the order
 * specified.
 *
 * <p>Specify --fromSeq <sequence> to replay logs from that sequence only. Specify --toSeq
 * <sequence> to replay logs up to that sequence only. Specify --fromTime <timestamp> to replay
 * operations from that time only. Specify --toTime <timestamp> to replay operations up to that time
 * only.
 *
 * <p>Specify --stopOnError option to make replay stop when it encounters any error. By default
 * errors are logged and ignored.
 *
 * <p>Specify --threads <number of threads> to set the degree of parallelism. By default 50 threads
 * are used. Operations for a given mailbox is always executed by the same thread to guarantee
 * execution order within a mailbox. Thread assignment is done by taking the module of mailbox id
 * divided by number of threads. Thus in the default setting mailboxes 1 and 51 are handled by the
 * same thread. This means replay is not fully parallelized. This is a consequence of the need to
 * guarantee order of execution for each mailbox.
 *
 * <p>When only 1 thread is used or --mailboxId option is used all replay is done by the main
 * thread. Even when multiple replay threads are used operations that span multiple mailboxes
 * (StoreIncomingBlob of a multi-recipient email delivery) or system operations (volume ops) are
 * executed by the main thread. This is necessary to guarantee that latermailbox-specific operations
 * that depend on the non-mailbox-specific operation (e.g. CreateMessage that links to blob stored
 * by StoreIncomingBlob) are not started out of order.
 *
 * <p>Specify --queueCapacity <number of ops> to set the capacity of the operation queue used by
 * each replay thread. Default capacity is 100. This parameter is related to the degree of
 * parallelism. When a replay thread's queue is full and the main thread must submit another
 * operation to the thread, it will block and will be prevented from submitting operations that
 * might have been run in parallel by other replay threads that have available room in the queue.
 */
public class PlaybackUtil {
  ...
  static {
    ...
    sOptions.addOption(
        null, OPT_QUEUE_CAPACITY, true, "Queue capacity per player thread; default=100");
  ...
  private static Params initParams(CommandLine cl) throws ServiceException, IOException {
    ...
    if (cl.hasOption(OPT_FROM_TIME)) {
      ...
      if (time != null) {
        ...
        System.err.printf(
            "Invalid timestamp \"%s\" specified for --%s option\n", timeStr, OPT_FROM_TIME);
        System.err.println();
    ...
    System.out.printf(
        "Using %d as queue capacity for each redo player thread\n", params.queueCapacity);
    ...
    if (cl.hasOption(OPT_LOGFILES)) {
      ...
      for (int i = 0; i < fnames.length; i++) {
        ...
        if (f.exists()) logList.add(f);
        else throw new FileNotFoundException("No such file: " + f.getAbsolutePath());
      }
    } else {
      ...
      if (archiveDir.exists()) {
        File[] archiveLogs =
            RolloverManager.getArchiveLogs(archiveDir, params.fromSeq, params.toSeq);
      ...
      if (redoLog.exists()) {
        ...
        if (params.fromSeq <= seq && seq <= params.toSeq) logList.add(redoLog);
      }
    }
  ...
  public PlaybackUtil(Params params) {
    ...
    if (mParams.mboxId != Params.MBOX_ID_UNSET || mParams.threads == 1)
      ...
      mPlayer =
          new ParallelRedoPlayer(
              false,
              true,
              !mParams.stopOnError,
              false,
              mParams.threads,
              mParams.queueCapacity,
              true);
  }
    ...
    try {
      for (File redolog : mParams.logfiles) {
        ...
        try {
          ...
          if (mParams.stopOnError) throw t;
  ...
  public static void main(String[] cmdlineargs) throws Throwable {
    // Bug: 47051
    // for the CLI utilities we need to set the default soap http transport timeout to 0 (no
    // timeout).
  ...
  private static void setup() throws ServiceException {
    ...
    if (consoleAppender != null) rootLogger.removeAppender(consoleAppender);
