...
/**
 * @author jhahm
 *     <p>Header for a redolog file. Redolog header is exactly 512 bytes long. The fields are:
 *     <p>MAGIC 7 bytes containing "ZM_REDO" open 1 byte (1 or 0) 0 means file was closed normally 1
 *     means either file is currently open, or process died without closing file properly; there may
 *     be partially written log entries at the end of file filesize 8 bytes; for self-integrity
 *     check sequence 8 bytes; log file sequence number; 0 to Long.MAX_VALUE wraps around after
 *     Long.MAX_VALUE serverId 128 bytes; consists of the following subfields: length - 1 byte (0 to
 *     127) data - up to 127 bytes of serverId in UTF-8 padding - 0-value bytes of length = 127 -
 *     length(data) serverId is the zimbraId LDAP attribute of the server entry firstOpTstamp 4
 *     bytes; time of first op in file lastOpTstamp 4 bytes; time of last op in file version 4
 *     bytes; serialization version number (2-byte major, 2-byte minor) createTime 4 bytes; time
 *     this log file was created padding 0-value bytes to bring total header size to 512
 */
public class FileHeader {
  ...
  void read(RandomAccessFile raf) throws IOException {
    ...
    if (bytesRead < HEADER_LEN)
      throw new IOException("Redolog is smaller than header length of " + HEADER_LEN + " bytes");
    deserialize(header);
  }
  ...
  /**
   * Get byte buffer of a String that fits within given maximum length. String is trimmed at the end
   * one character at a time until the byte representation in given charset fits maxlen.
   *
   * @param str
   * @param charset
   * @param maxlen
   * @return byte array of str in charset encoding; zero-length array if any trouble
   */
  ...
  private byte[] serialize() throws IOException {
    ...
    byte[] serverIdBuf = getStringBytes(mServerId, "UTF-8", SERVER_ID_FIELD_LEN);
    ...
    if (headerBuf.length != HEADER_LEN)
      throw new IOException(
          "Wrong redolog header length of " + headerBuf.length + "; should be " + HEADER_LEN);
    return headerBuf;
  }
  ...
  private void deserialize(byte[] headerBuf) throws IOException {
    ...
    try {
      ...
      if (!mVersion.atLeast(1, 0)) mVersion = new Version(1, 0);
  ...
  public String toString() {
    ...
    if (mOpen != 0) sb.append(" (not up to date)");
