...
public class RedoLogVerify {
  ...
  static {
    ...
    sOptions.addOption(
        null, OPT_NO_OFFSET, false, "don't show file offsets and size for each redo op");
    sOptions.addOption(
        null,
        OPT_MAILBOX_IDS,
        true,
        "one or more mailbox ids separated by comma or white space.  The entire list must be quoted"
            + " if using space as separator.  If this option is given, only redo ops for the"
            + " specified mailboxes are dumped.  Omit this option to dump redo ops for all"
            + " mailboxes.");
    sOptions.addOption(
        null,
        OPT_SHOW_BLOB,
        false,
        "show blob content.  Item's blob is printed, surrounded by <START OF BLOB> and <END OF"
            + " BLOB> markers.  The last newline before end marker is not part of the blob.");
  }
        ...
        for (String val : ids) {
          if (val != null && val.length() > 0) {
            try {
              ...
              if (i > 0) params.mboxIds.add(i);
              else usage("Invalid mailbox id \"" + val + "\"");
  ...
  public RedoLogVerify(Params params, PrintStream out) {
    mOut = out;
    mParams = params;
    if (mParams == null) mParams = new Params();
  ...
  public boolean scanLog(File logfile) throws IOException {
    ...
    try {
      while ((op = logReader.getNextOp()) != null) {
        ...
        if (!mParams.quiet) {
          printOp(
              mOut, op, mParams.hideOffset, lastOpStartOffset, lastPosition - lastOpStartOffset);
          if (mParams.showBlob) {
      ...
      int linesAfter = 10;
      long startPos =
          Math.max(lastPosition - (lastPosition % bytesPerLine) - linesBefore * bytesPerLine, 0);
      int count =
          (int)
              Math.min(
                  (linesBefore + linesAfter + 1) * bytesPerLine, lastPosition - startPos + diff);
  ...
  private static void printOp(
      PrintStream out, RedoableOp op, boolean hideOffset, long beginOffset, long size) {
    if (!hideOffset)
      out.printf(
          "[%08x - %08x: %d bytes; tstamp: %s] ",
          beginOffset,
          beginOffset + size - 1,
          size,
          sDateFormatter.format(new Date(op.getTimestamp())));
  ...
  private boolean verifyDirectory(File dir) {
    if (!mParams.quiet) mOut.println("VERIFYING DIRECTORY: " + dir.getAbsolutePath());
    ...
    for (File f : all) {
      if (!f.isDirectory()) {
        ...
        if (fname.lastIndexOf(".log") == fname.length() - 4) fileList.add(f);
      }
    }
  ...
  private static void hexdump(
      PrintStream out, byte[] data, int offset, int length, long offsetOffsetBy, long badBytePos) {
    ...
    while (offset < end) {
      ...
      for (int i = 0; i < bytesPerLine; i++) {
        if (i < bytes) out.printf("%02x", ((int) data[offset + i]) & 0x000000ff);
        else out.print("  ");
        out.print(" ");
        if (i == 7) out.print(" ");
      }
      ...
      if (offsetLineStart <= badBytePos && badBytePos < offsetLineEnd) out.print(" **");
      out.println();
  ...
  public static void main(String[] cmdlineargs) {
    ...
    if (params.help) usage(null);

    String[] args = cl.getArgs();

    if (args.length < 1) usage("No redolog file/directory list specified");
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      boolean good = false;
      if (f.isDirectory()) good = verify.verifyDirectory(f);
      else good = verify.verifyFile(f);
