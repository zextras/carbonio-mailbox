...
public class RenameMailbox extends RedoableOp {
  ...
  protected void serializeData(RedoLogOutput out) throws IOException {
    ...
    if (getVersion().atLeast(1, 25)) out.writeUTF(mOldName);
  }

  @Override
  protected void deserializeData(RedoLogInput in) throws IOException {
    mNewName = in.readUTF();
    if (getVersion().atLeast(1, 25)) mOldName = in.readUTF();
  }
  ...
  public void redo() throws Exception {
    ...
    // Redo the rename only if the current account name matches mOldName.  This prevents renaming an
    // account
