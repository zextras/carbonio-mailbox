  ...
  protected void serializeData(RedoLogOutput out) throws IOException {
    ...
    if (mReplacementMap != null) {
      ...
      for (Entry<String, ICalTimeZone> entry : mReplacementMap.entrySet()) {
        ...
        if (newTZ != null) newTZMeta = Util.encodeAsMetadata(newTZ).toString();
        out.writeUTF(tzid);
    ...
    if (numReplacements > 0) {
      ...
      for (int i = 0; i < numReplacements; i++) {
        ...
        try {
          ...
          if (newTZMeta != null) newTZ = Util.decodeTimeZoneFromMetadata(new Metadata(newTZMeta));
