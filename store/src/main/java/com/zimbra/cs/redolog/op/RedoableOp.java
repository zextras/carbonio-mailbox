...
public abstract class RedoableOp {
  ...
  /**
   * Set up a callback that will be called when the transaction commits and the commit record is
   * safely on disk. This method must be called before commit() is called.
   *
   * @param callback
   */
  ...
  public void start(long timestamp) {
    ...
    if (isStartMarker()) setTransactionId(mRedoLogMgr.getNewTxnId());
  }
  ...
  public void log(boolean synchronous) {
    ...
    if (isStartMarker()) mActive = true;
  }

  public void setChangeConstraint(boolean checkCreated, int changeId) {
    mChangeConstraint = changeId;
    if (checkCreated == OperationContext.CHECK_CREATED) mChangeConstraint *= -1;
  }
  ...
  public OperationContext getOperationContext() {
    ...
    if (mChangeConstraint != 0) {
      boolean checkCreated =
          (mChangeConstraint < 0
              ? OperationContext.CHECK_CREATED
  ...
  public synchronized void commit() {
    if (mActive) {
      ...
      if (mChainedOps != null) {
        for (RedoableOp rop : mChainedOps) rop.commit();
        mChainedOps = null;
      }
    }
  ...
  public synchronized void abort() {
    if (mActive) {
      ...
      if (mChainedOps != null) {
        for (RedoableOp rop : mChainedOps) rop.abort();
        mChainedOps = null;
      }
    }
    // We don't need to hang onto the byte arrays after commit/abort.
  ...
  /**
   * Crash recovery redoes all uncommitted operations from previous crash before allowing client
   * connections. This doesn't work out well for a long-running operation such as fully reindexing a
   * large mailbox. Those operations should be excluded from initial crash recovery and run after
   * startup procedure finishes. Note this treatment is possible only for independent operations,
   * i.e. no other operation during crash recovery should depend on having this operation done
   * first.
   *
   * @return
   */
  ...
  protected void serializeHeader(RedoLogOutput out) throws IOException {
    ...
    // still writing and reading long mailbox IDs for backwards compatibility, even though they're
    // ints again
  ...
  public String toString() {
    ...
    if (mChangeId != -1) sb.append(", change=").append(mChangeId);
    if (mChangeConstraint != 0) sb.append(", constraint=").append(mChangeConstraint);
    ...
    } else if (data != null) sb.append(", ").append(data);
    return sb.toString();
  }

  /**
   * Returns the operation code of the transaction. Returns the same value as getOperation(), except
   * CommitTxn and AbortTxn return the getOperation() value from their corresponding change entry.
   *
   * @return
   */
  ...
  /** Returns any additional data that must be written after the usual header and data. */
   ...
   * @throws EOFException - if end of stream is reached while reading; EOF may happen in the middle
   *     of a log entry, indicating the server may have died unexpectedly
  ...
  public static RedoableOp deserializeOp(RedoLogInput in) throws EOFException, IOException {
    ...
    if (magic.compareTo(REDO_MAGIC) != 0) throw new IOException("Missing redo item magic marker");
  ...
  /**
   * Returns the core redoable op data as a <tt>byte[]</tt>. Does not include the result of {@link
   * #getAdditionalDataStream()}.
   */
  ...
  /**
   * Returns the entire redoable op data as an <tt>InputStream</tt>. Includes the result of {@link
   * #getAdditionalDataStream()}.
   */
  public InputStream getInputStream() throws IOException {
    synchronized (mSBAVGuard) {
      if (mSerializedByteArrayVector == null) setSerializedByteArray(serializeToByteArray());
  ...
  public synchronized void addChainedOp(RedoableOp subOp) {
    if (mChainedOps == null) mChainedOps = new LinkedList<RedoableOp>();
    ...
    for (MailboxOperation opcode : EnumSet.allOf(MailboxOperation.class)) {
      ...
      if (className == null) {
        ...
        try {
          ...
          String msg =
              "Unable to instantiate " + className + "; Check default constructor is defined.";
  ...
  public static synchronized void deregisterClassLoader(ClassLoader ldr) {
    ...
    for (Map.Entry<String, Class> entry : sOpClassMap.entrySet()) {
      ...
      if (clz.getClassLoader().equals(ldr)) toRemove.add(entry.getKey());
    }
    ...
    for (Iterator iter = sOpClassLoaders.iterator(); iter.hasNext(); ) {
      ...
      if (loader.equals(ldr)) iter.remove();
    }
  }
