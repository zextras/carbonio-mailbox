...
public class OwaspPolicy {
  ...
  /**
   * The singleton instance. This is a volatile variable, so that we can reload the policy file on
      ...
      try (FileInputStream fis = new FileInputStream(policyFile)) {
        ...
        if (!root.getName().equals(E_OWASP_POLICY)) {
          ZimbraLog.mailbox.warn(
              String.format(
                  "OWASP policy file '%s' root tag is not '%s'", mPolicyFile, E_OWASP_POLICY));
        ...
        for (Iterator<?> iter = root.elementIterator(E_ELEMENT); iter.hasNext(); ) {
          ...
          attributes =
              attributes
                  .replace("CORE", "id,class,title,style")
                  .replace("LANG", "dir,lang,xml:lang")
                  .replace("KBD", "accesshtml,tabindex");
        ...
        ZimbraLog.mailbox.warn(
            String.format("Problem parsing owasp policy file '%s'", policyFile), e);
        throw new DocumentException(
            String.format("Problem parsing owasp policy file '%s'", policyFile));
      }
      ZimbraLog.mailbox.info("OWASP policy '%s' loaded", mPolicyFile);
    } else {
      ZimbraLog.mailbox.warn(String.format("Owasp policy file '%s' is not readable", mPolicyFile));
