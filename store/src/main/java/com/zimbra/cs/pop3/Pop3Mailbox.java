...
final class Pop3Mailbox {
  ...
  Pop3Mailbox(Mailbox mbox, Account acct, String query) throws ServiceException {
    ...
    if (Strings.isNullOrEmpty(query)) {
      ...
      Date popSince =
          dateConstraint == null ? null : LdapDateUtil.parseGeneralizedTime(dateConstraint);
      ...
      try (ZimbraQueryResults results =
          mbox.index.search(opContext, query, POP3_TYPES, SortBy.DATE_DESC, 500)) {
  ...
  Pop3Message getPop3Msg(String msg) throws Pop3CmdException {
    ...
    if (p3m.isDeleted()) throw new Pop3CmdException("message is deleted");
    return p3m;
  }
  ...
  private void updateFlags(Mailbox mbox, int msgId, boolean poped, boolean read)
