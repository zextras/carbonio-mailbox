...
abstract class Pop3Handler {
  ...
  boolean processCommandInternal() throws Pop3CmdException, IOException, ServiceException {
    ...
    if (accountId != null) {
      ...
      if (throttle.isAccountThrottled(accountId, origRemoteAddress, clientAddress)) {
        ZimbraLog.pop.warn(
            "throttling POP3 connection for account %s due to too many requests", accountId);
        dropConnection = true;
        return false;
      }
    }
  ...
  void authenticate(String username, String authenticateId, String password, Authenticator auth)
    ...
    try {
      ...
      mailbox =
          new Pop3Mailbox(MailboxManager.getInstance().getMailboxByAccount(acct), acct, query);
      state = STATE_TRANSACTION;
      expire =
          (int)
              (acct.getTimeInterval(Provisioning.A_zimbraMailMessageLifetime, 0)
      ...
      if (code.equals(AccountServiceException.NO_SUCH_ACCOUNT)
        ...
        throw new Pop3CmdException(
            "your account is having maintenance peformed. please try again later");
      } else {
  ...
  private void doCAPA() throws IOException {
    // [SASL]       RFC 5034: POP3 Simple Authentication and Security Layer (SASL) Authentication
    // Mechanism
  ...
  private void doXOIP(String origIp) throws Pop3CmdException, IOException {
    if (origIp == null) throw new Pop3CmdException("please specify an ip address");
    ...
    if (curOrigRemoteIp == null) {
      ...
      if (curOrigRemoteIp.equals(origIp)) {
        ...
        ZimbraLog.pop.error(
            "POP3 XOIP is allowed only once per session, received different IP: %s, command"
                + " ignored",
            origIp);
      }
    }
