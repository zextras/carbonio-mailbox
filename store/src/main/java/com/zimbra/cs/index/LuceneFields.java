 ...
 *
 * <p>The sort fields are used for the sorting of Lucene results. They MUST be stored with all
 * documents, and their values MUST match the values in the corresponding MailItem row entry. They
 * MUST be Indexed and NOT_ANALYZED. Snippet from Javadoc for Lucene 3.5.0
 * org.apache.lucene.search.SortField: Fields must be indexed in order to sort by them.
 *
 * @since Apr 30, 2004
 * @author schemers
 */
public final class LuceneFields {
  ...
  public static final Map<String, Character> FIELD2PREFIX =
  ...
  /**
   * The "index id" this document -- maps to one or more rows in the DB's mail_item table (index_id
   * column).
   *
   * <p>This field will automatically be populated with the index ID
   */
   ...
   *
   * <p>searches with no operator will search this field
   */
   ...
   *
   * <p>For RFC/822 messages, should be dotted-number MIME part name or "top". For all other
   * mail_item types, should be "top", or should be specific to the type note that the partname is
   * used in a weird way when constructing "NOT" queries -- bare 'not's are only checked against
   * toplevel parts....see LuceneQueryOperation.java AddClause().
   */
