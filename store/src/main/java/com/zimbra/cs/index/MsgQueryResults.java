...
final class MsgQueryResults extends ZimbraQueryResultsImpl {
  ...
  /**
   * Cache of local Message IDs we've seen this iteration -- used so that in situations where we
   * have multiple {@link MessagePartHit}s we still only return a single Message.
   */
  private final Set<Integer> mSeenMsgs = new HashSet<Integer>();

  MsgQueryResults(
      ZimbraQueryResults topLevelQueryOperation,
   ...
   *
   * <p>Side effect: will Op's iterator one or more entries forward
   *
   * @return next hit
   */
  ...
  public ZimbraHit getNext() throws ServiceException {
    ...
    assert (nextHit == null
        || (!(nextHit instanceof MessagePartHit) && !(nextHit instanceof ConversationHit)));
    nextHit = null;
    return toRet;
  }
