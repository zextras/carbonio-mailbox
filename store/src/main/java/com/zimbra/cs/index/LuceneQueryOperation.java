...
public final class LuceneQueryOperation extends QueryOperation {
  ...
  private void fixMustNotOnly(BooleanQuery query) {
    ...
    if (types.contains(MailItem.Type.NOTE)) {
      query.add(
          new TermQuery(new Term(LuceneFields.L_PARTNAME, LuceneFields.L_PARTNAME_NOTE)),
          BooleanClause.Occur.SHOULD);
    }
  }
  ...
  /**
   * Extended {@link MultiPhraseQuery} that defers wildcard expansion until actual Lucene search
