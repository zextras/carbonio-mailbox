...
public final class ZimbraAnalyzer extends Analyzer {
  ...
  static {
    ANALYZERS.put(
        "StandardAnalyzer", new ForwardingAnalyzer(new StandardAnalyzer(LuceneIndex.VERSION)));
  }
  ...
  /**
   * We maintain a single global instance for our default analyzer, since it is completely thread
   * safe.
   *
   * @return singleton
   */
   ...
   *
   * <p>Accounts are configured to use a particular analyzer by setting the "zimbraTextAnalyzer" key
   * in the Account or COS setting.
   *
   * <p>The custom analyzer is assumed to be a stateless single instance (although it can and
   * probably should return a new TokenStream instance from it's APIs)
   *
   * @param name a unique name identifying the Analyzer, it is referenced by Account or COS settings
   *     in LDAP
   * @param analyzer a Lucene analyzer instance which can be used by accounts that are so
   *     configured.
   */
  public static void registerAnalyzer(String name, Analyzer analyzer) throws ServiceException {
    if (ANALYZERS.containsKey(name)) {
      throw ServiceException.FAILURE(
          "Cannot register analyzer: "
              + name
              + " because there is one already registered with that name.",
          null);
    }
