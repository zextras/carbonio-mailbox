...
/**
 * {@link IndexStore} implementation using ElasticSearch via the REST API. There is a Java API but
 * that ties to a particular version. Security might also be an issue with standard ElasticSearch
 * https://github.com/sonian/elasticsearch-jetty is a way to add SSL to the comms Elasticsearch uses
 * http. If better network security is desired, most suggestions point towards hiding behind an SSL
 * proxy. I think nginx can help with this - see http://wiki.nginx.org/HttpCoreModule#listen
 *
 * <p>Elasticsearch URL: http://localhost:9200/INDEX-NAME/INDEX-TYPE/INDEX-ID'
 */
public final class ElasticSearchIndex extends IndexStore {
      ...
      try {
        ...
        putMethod.setEntity(
            new StringEntity(
                ...
                MimeConstants.CT_APPLICATION_JSON,
                MimeConstants.P_CHARSET_UTF8));
        int statusCode = connector.executeMethod(putMethod);
        if (statusCode == HttpStatus.SC_OK) {
          haveMappingInfo = true;
          refreshIndexIfNecessary(); // Sometimes searches don't seem to honor mapping info.  Try to
          // force it
        } else {
          ZimbraLog.index.error(
              "Problem Setting mapping information for index with key=%s httpstatus=%d",
              key, statusCode);
        }
      } catch (IOException e) {
  ...
  private JSONObject createMappingInfo() throws JSONException {
    ...
    // We rely on ZimbraAnalyzer to pre-tokenize the input and then supply the tokens to
    // Elasticsearch
    ...
    properties.put(
        LuceneFields.L_SORT_SIZE, new FieldProperty("long", true).notAnalyzed().asJSON());
    properties.put(
        LuceneFields.L_SORT_ATTACH, new StringFieldProperty(true).notAnalyzed().asJSON());
    properties.put(LuceneFields.L_SORT_FLAG, new StringFieldProperty(true).notAnalyzed().asJSON());
    properties.put(
        LuceneFields.L_SORT_PRIORITY, new StringFieldProperty(true).notAnalyzed().asJSON());
    properties.put(
    ...
    properties.put(
        ...
        new StringFieldProperty(false).analyzed().analyzer("emailaddress").asJSON());
    // For Lucene ZimbraAnalyzer "private TokenStream tokenStream(String field, Reader reader,
    // Analyzer analyzer)"
    // uses KeywordTokenizer for this.  However, it is not stored and not analyzed when added
    properties.put(
        LuceneFields.L_H_MESSAGE_ID, new StringFieldProperty(false).notAnalyzed().asJSON());
    properties.put(
        LuceneFields.L_FIELD,
    ...
    properties.put(
        ...
        new StringFieldProperty(false).analyzed().analyzer("zimbrastandard").asJSON());
    properties.put(
        LuceneFields.L_SORT_SUBJECT, new StringFieldProperty(false).notAnalyzed().asJSON());
    properties.put(
        LuceneFields.L_CONTENT,
        new StringFieldProperty(false).analyzed().analyzer("zimbrastandard").asJSON());
    // Only ever used via MimeTypeTokenStream
    properties.put(
        LuceneFields.L_ATTACHMENTS,
        new StringFieldProperty(false).analyzed().analyzer("whitespace").asJSON());
    properties.put(
        LuceneFields.L_MAILBOX_BLOB_ID, new FieldProperty("integer", true).notAnalyzed().asJSON());
  ...
  /**
   * TODO: Do something similar to this from LuceneIndex? Runs a common search query + common sort
   * order (and throw away the result) to warm up the Lucene cache and OS file system cache.
   */
  @Override
  public void warmup() {
  ...
  public static final class Factory implements IndexStore.Factory {
    ...
    public List<String> getIndexes() {
      ...
      try {
        ...
        if (statusCode != HttpStatus.SC_OK) {
          ZimbraLog.index.error(
              "Problem getting list of Elastic Search indexes httpstatus=%d", statusCode);
        }
            ...
            if (reader != null) {
              ...
              if (val != null) {
                ...
                ZimbraLog.index.debug(
                    "addFieldToDocument IGNORING tokenized field=%s", field.name());
              }
            }
          }
        } else {
      ...
      for (IndexDocument doc : docs) {
        ...
        try {
          ...
          if (statusCode != HttpStatus.SC_CREATED) {
            ZimbraLog.index.error(
                "Problem indexing document with id=%d httpstatus=%d", item.getId(), statusCode);
          }
        } catch (IOException e) {
    ...
    public void deleteDocument(List<Integer> ids) {
      ...
      for (Integer id : ids) {
        ...
        StringBuilder urlBuilder =
            new StringBuilder()
                .append(ElasticSearchConnector.actualUrl(url))
                .append("q=")
                .append(query);
        ...
        try {
          ...
          if (statusCode == HttpStatus.SC_OK) {
            ...
            ZimbraLog.index.error(
                "Problem deleting documents with id=%d httpstatus=%d", id, statusCode);
          }
        } catch (IOException e) {
  ...
  private final class ElasticIndexReader implements ZimbraIndexReader {
    ...
    public int numDeletedDocs() {
      ...
      try {
        ...
        if (statusCode == HttpStatus.SC_OK) {
          int cnt =
              connector.getIntAtJsonPath(new String[] {"_all", "total", "docs", "deleted"}, 0);
          return cnt;
        }
      } catch (IOException e) {
        ZimbraLog.index.error("Problem getting stats for index %s", url, e);
    ...
    /**
     * Relies on ElasticSearch plugin elasticsearch-index-termlist being installed. Note that that
     * currently doesn't support doc frequency information TODO: Fork plugin to add doc frequency
     * information?
     */
  ...
  public final class ZimbraElasticIndexSearcher implements ZimbraIndexSearcher {
    ...
    /**
     * Returns the stored fields of document {@code docID} (an index store specific ID for the
     * document)
     */
    @Override
    public Document doc(ZimbraIndexDocumentID docID) throws IOException {
      ...
      if (docID instanceof ZimbraElasticDocumentID) {
        ...
        String url =
            String.format(
                "%s%s/%s?fields=%s",
                indexUrl, indexType, eDocID.getDocID(), Joiner.on(',').join(storedFields));
        HttpGet method = new HttpGet(ElasticSearchConnector.actualUrl(url));
        try {
          ElasticSearchConnector connector = new ElasticSearchConnector();
          int statusCode = connector.executeMethod(method);
    ...
    public ZimbraTopDocs search(Query query, ZimbraTermsFilter filter, int n) throws IOException {
      ...
      if (requestJson != null) {
        ...
        try {
          ...
          ZimbraLog.index.debug(
              "search - problem processing JSON Query response against index %s", url, e);
        }
      }
      return ZimbraTopDocs.create(scoreDocs.size(), scoreDocs);
    }
    ...
    /**
     * Search implementation with arbitrary sorting. Finds the top n hits for query, applying filter
    ...
    /**
     * { "range" : { "letter" : { "from" : "beta", "to" : "omega", "include_lower" : true,
    ...
    private JSONObject TermRangeQueryToJSON(TermRangeQuery query) throws JSONException {
      ...
      if (null != query.getCollator()) {
        ...
        ZimbraLog.index.error(
            "ElasticSearchIndex does not support TermRangeQueries with collators");
        return null;
      }
    ...
    private JSONObject MultiTermQueryToJSON(MultiTermQuery query) throws JSONException {
      if (query instanceof PrefixQuery) {
        ...
        // Should not get here given current Zimbra functionality
        ZimbraLog.index.error(
            "ElasticSearchIndex does not support search queries of type %s",
    ...
    /**
     * e.g. : "bool" : { "should" : [ { "match_phrase" : { "l.content" : "from james hunt" } }, {
     * "match_phrase" : { "l.content" : "from jimmy hunt" } }, { "match_phrase" : { "l.content" :
     * "from jim hunt" } } ], "minimum_number_should_match" : 1, }
     */
    ...
    /**
     * { "bool" : { "must" : { "term" : { "user" : "kimchy" } }, "must_not" : { "range" : { "age" :
     * { "from" : 10, "to" : 20 } } }, "should" : [ { "term" : { "tag" : "wow" } }, { "term" : {
     * "tag" : "elasticsearch" } } ], "minimum_number_should_match" : 1, "boost" : 1.0 } }
     */
    ...
    /**
     * Note: Assumption is that only simple SortFields are used which specify a field name and an
     * order of sorting.
     */
    private JSONArray SortToJSON(List<SortField> sortFields) throws JSONException {
      ...
      for (SortField sortField : sortFields) {
        ...
        sortJson.put(
            new JSONObject().put(field, new JSONObject().put("order", reverse ? "desc" : "asc")));
      }
      return sortJson;
    }

    private JSONObject searchQueryToJSON(
        Query query, ZimbraTermsFilter filter, List<SortField> sortFields) throws JSONException {
      JSONObject requestJson = null;
      ...
      if (filterJson == null) {
        ...
        */
        JSONObject filteredJson =
            new JSONObject().put("query", queryJson).put("filter", filterJson);
