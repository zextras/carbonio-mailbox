...
public final class FieldQuery extends TextQuery {
  ...
  private FieldQuery(String name, String value) {
    super(
        new FieldTokenStream(name, value),
   ...
   *
   * <p>If it's a numeric range query, {@link NumericRangeFieldQuery} is created instead.
   *
   * @param mbox mailbox
  ...
  private static final class NumericRangeFieldQuery extends Query {
    ...
    public QueryOperation compile(Mailbox mbox, boolean bool) {
      ...
      switch (range) {
        case EQ:
          query =
              new TermQuery(
                  new Term(
                      LuceneFields.L_FIELD, name + "#:" + NumericUtils.intToPrefixCoded(number)));
          break;
        case GT:
          query =
              new TermRangeQuery(
      ...
      op.addClause(
          toQueryString(LuceneFields.L_FIELD, range.toString() + number), query, evalBool(bool));
      return op;
    }
