...
public final class IndexEditor {
  ...
  public void dumpDocumentByMailItemId(int mailboxId, int mailItemId)
  ...
  public static class TwoTerms implements Comparable<TwoTerms> {
    @Override
    public int compareTo(TwoTerms other) {
  ...
  private static class IndexEditorProtocolhandler extends ProtocolHandler {
    ...
    /** Closes any input/output streams with the client. May get called multiple times. */
    @Override
    ...
    /**
     * Called when a connection has been idle for too long. Sends protocol-specific message to
     * client notifying it that the connection is being dropped due to idle timeout.
     */
    @Override
  ...
  void getSize(int mailboxId) throws ServiceException {
    ...
    outputStream.println(
        "Mailbox " + mailboxId + " has size " + size + " (" + (size / 1024) + "kb)");
  }
  ...
  void help() {
    ...
    outputStream.println(
        "sort na|nd|sa|sd|da|dd -- set sort order (name asc/desc, subj asc/desc or date asc/desc)");
    ...
    outputStream.println(
        "loglevel -- Change the default global logging level (affects all appenders!)");
