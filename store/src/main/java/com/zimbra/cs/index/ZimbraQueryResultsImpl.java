...
abstract class ZimbraQueryResultsImpl implements ZimbraQueryResults {
  ...
  protected DocumentHit getDocumentHit(
      Mailbox mbx, int id, com.zimbra.cs.mailbox.Document item, Document doc, Object sortValue) {
  ...
  ZimbraHit getZimbraHit(Mailbox mbox, DbSearch.Result sr, Document doc, DbSearch.FetchMode fetch) {
    ...
    switch (sr.getType()) {
      ...
      case WIKI:
        result =
            getDocumentHit(
                mbox, sr.getId(), (com.zimbra.cs.mailbox.Document) item, doc, sr.getSortValue());
        break;
      default:
        assert (false);
    }
