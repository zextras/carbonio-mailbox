...
public class Fragment {
  ...
  private static boolean isHeader(String text, boolean allowSpaces) {
    for (int i = 0; i < text.length(); i++) {
      ...
      else if (!allowSpaces && Character.isWhitespace(c)) return false;
    }
    return false;
  }
  ...
  private static String skipQuotedText(String content, boolean twoLineHeader) {
    ...
    for (int i = 0; i < headerLines; i++) {
      if (fragment.startsWith(">") || fragment.startsWith("|")) break;
      returnIndex = fragment.indexOf('\n', returnIndex + 1);
  ...
  // StringUtil.stripControlCharacters(line).replaceAll("\\s+", " ").replaceAll("---+",
  // "--").replaceAll("===+", "==");
  private static String compressLine(String line) {
    ...
    for (int i = 0, len = line.length(); i < len && sb.length() <= MAX_FRAGMENT_LENGTH + 5; i++) {
      ...
      if ((c == '=' || c == '-') && last == c && lastButOne == c) continue;
  ...
  public static String getFragment(String content, boolean hasCalendar) {
    ...
    if (hasCalendar) item = Source.APPOINTMENT;
  ...
  public static String getFragment(String content, Source item) {
    ...
    if (item == Source.NOTEBOOK) {
      ...
      if (item == Source.APPOINTMENT) remainder = skipFragmentHeader(remainder, true);
      ...
      while (fragment.length() <= Fragment.MAX_FRAGMENT_LENGTH) {
        // detect quoted stuff and skip it (or unquote it if we're fragmenting a message that's
        // nothing but quotes)
        if (remainder.startsWith(">") || remainder.startsWith("|")) {
          if (mainMessage) {
            remainder = skipQuotedText(remainder, false);
            elided = true;
          } else remainder = remainder.substring(1).trim();
        }
        // if we're done, we're done
        if (remainder.equals("")) break;
        // DASH-DASH-SPACE-RETURN is the standard signature delimiter
        int returnIndex = remainder.indexOf('\n');
        if ((returnIndex == 3 && remainder.startsWith("-- "))
            || (returnIndex == 4 && remainder.startsWith("-- \r"))) break;
        // check for another "On foosday, Herbie wrote:\n> blah..." and skip (unless we're
        // fragmenting a message that's nothing but quotes)
        ...
        if (line.equalsIgnoreCase(STOPWORD_REPLY_1)
            || line.equalsIgnoreCase(STOPWORD_REPLY_2)
            || line.equalsIgnoreCase(STOPWORD_REPLY_3)
            || line.equalsIgnoreCase(STOPWORD_REPLY_4)
            || line.equalsIgnoreCase(STOPWORD_FORWARD_1)
            || line.equalsIgnoreCase(STOPWORD_FORWARD_2)) {
          if (fragment.length() != 0 || returnIndex == -1) break;
          // if we're here, the message was nothing but quoted text, so leave in the "-- Original
          // Message --" and add a few more lines
    ...
    // almost done!  just make sure we haven't accidentally trimmed off everything (in which case
    // we'll just use the original content)
    if (result.equals("")) result = compressLine(backup);
    ...
    if (isTruncated) {
      ...
      if (wspIndex != -1) result = result.substring(0, wspIndex).trim();
