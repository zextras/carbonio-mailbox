...
/**
 * Base class for a search "hit". Generally one iterates over a {@link ZimbraQueryResults} to get
 * the hits for a given query.
 *
 * @since Oct 15, 2004
 */
public abstract class ZimbraHit implements ZimbraQueryHit {
  ...
  /**
   * Returns the {@link MailItem} corresponding to this hit, or NULL if one is not available (e.g.
   * for a ProxiedHit).
   */
  ...
  /**
   * Returns TRUE if our associated {@link MailItem} is already loaded (or we don't have one, ie
   * ProxiedHit).
   */
   ...
   *
   * <p>TODO: For string sort values, we compare(toUpper()) instead of compareIgnoreCase() or using
   * a collator because that's the only method that seems to give us the same results as the sorts
   * from SQL server.
   *
   * @return {@code <0} if "this" is BEFORE other, {@code 0} if EQUAL, {@code >0} if this AFTER
   *     other
    ...
    switch (sort) {
      ...
      case RCPT_ASC:
        return ((String) sortValue)
            .toUpperCase()
            .compareTo(((String) other.sortValue).toUpperCase());
      case SUBJ_DESC:
      case NAME_DESC:
      case NAME_LOCALIZED_DESC:
      case RCPT_DESC:
        return ((String) other.sortValue)
            .toUpperCase()
            .compareTo(((String) sortValue).toUpperCase());
      case ATTACHMENT_ASC:
      case FLAG_ASC:
      case PRIORITY_ASC:
  ...
  protected static final int compareByReadFlag(boolean ascending, ZimbraHit lhs, ZimbraHit rhs) {
    ...
    try {
      ...
      ZimbraLog.index.info(
          "Caught ServiceException trying to compare ZimbraHit %s to ZimbraHit %s", lhs, rhs);
      ZimbraLog.index.debug(e);
    }
    if (ascending) return -1 * retVal;
    else return retVal;
  }
      ...
      try {
        ...
        try {
          ...
          ZimbraLog.index.debug(
              "Error reading unread flag. :%s for hit:%s", e2.getMessage(), msgHit);
          return 1;
        }
      }
    } else {
