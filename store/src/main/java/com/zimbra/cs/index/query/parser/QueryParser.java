...
public final class QueryParser {
   ...
   *
   * <p>The value is set after {@link #parse(String)}.
   *
   * @return sort field
   */
  ...
  public List<Query> parse(String src) throws ServiceException {
    if (Strings.isNullOrEmpty(src)) {
      throw MailServiceException.QUERY_PARSE_ERROR(
          src, null, "", 0, "Empty query string not allowed");
    }
    ...
    try {
      ...
      if (quick) {
        ...
        throw MailServiceException.QUERY_PARSE_ERROR(
            src, e, e.currentToken.image, e.currentToken.beginColumn, e.getMessage());
      }
    }
  }
  ...
  private SizeQuery createSizeQuery(SizeQuery.Type type, Token term, String text)
  ...
  private DateQuery createDateQuery(DateQuery.Type type, Token term, String text)
      throws ParseException {
  ...
  private Query createFieldQuery(String field, Token term, String text)
