...
public final class IntersectionQueryOperation extends CombiningQueryOperation {
  ...
  /**
   * There can be multiple Hits with the same exact sort-field. This function does a complete N^2
   * intersection of all of the hits for a particular sort field.
   *
   * <p>FIXME: this function is hideous, do _something_ with it.
   */
  ...
  /**
   * Responsible for grouping sub-results with the same sort value into a chunk so that they can
   * then be combined
   *
   * <p>1) Call buffer() to buffer the next timestamp, or buffer(timestamp) to buffer a particular
   * timestamp
   *
   * <p>2) Iterate through all the messageId's in the current timestamp
   *
   * <p>3) call getNextHit() to iterate the hits within the current message OR call
   * intersectWithBuffer() to tell you if the a particular hit intersects with something within our
   * buffer
   *
   * <p>Note: this class is somewhat confusing because there are really two completely different
   * paths through it: the first Grouper which gathers a bunch of hits and then is iterated using
   * getNextHit() and then the other groupers which gather hits and then use
   * intersectWithBuffer.....this is because we're using an N^2 intersection instead of an insertion
   * intersection.....this should be fixed.
   */
  private static final class HitGrouper {
    ...
    /**
     * Returns a hit from the current message group. This is useful because the hit's SORT FIELD is
     * guaranteed to be the same as the sort field in every other hit in this group (that's what the
     * Message Grouper does, after all)
     *
     * @return current hit
     */
  ...
  /**
   * We always transform the query into DNF: a AND (b OR c) into (a AND b) OR (a AND c)
   *
   * <p>If b or c have different targets (servers they execute on) then we *must* distribute but
   * otherwise we have a choice.
   *
   * <p>Tim: setting this to ALWAYS for now. I think in most cases it will be a win, even though it
   * appears to create 4 executable terms instead of 3. It will be a win because (from the example
   * above) in the 2nd case, it is almost certain that one both terms will combine thereby reducing
   * the number of operations to 3 with no ANDs, which is always faster.
   *
   * <p>The ideal solution would be to try both ways and compare the final # of executable ops.
   *
   * <p>tim 1/2008: SortBy="none" requires this setting, and so if you want to disable it you will
   * need to pass down and check the requested Sort.
   *
   * <p>tim:1/2009 convinced this is always the best choice. The # ops is less important than the
   * number of rows evaluated. Pushing the AND down lowers the total # rows.
   */
  @Override
  QueryOperation optimize(Mailbox mbox) throws ServiceException {
    ...
    while (true) {
      for (int i = 0; i < operations.size(); i++) {
        ...
        // if one of our direct children is an AND, then promote all of its children to our level --
        // this can
    ...
    // Step 2.5: now we want to eliminate any subtrees that have query targets which aren't
    // compatible,
    ...
    for (int i = 0; i < operations.size(); i++) {
      QueryOperation lhs = operations.get(i);
      if (lhs instanceof UnionQueryOperation || lhs.getQueryTargets().size() > 1) {
        // need to distribute!
        distributeLhs = i;
        break;
      }
  ...
  protected void begin(QueryContext ctx) throws ServiceException {
    ...
    context =
        new QueryContext(
            ctx.getMailbox(), ctx.getResults(), ctx.getParams(), (ctx.getChunkSize() + 1) * 3);
    ...
    for (int i = 0; i < operations.size(); i++) {
      ...
      if (!op.hasNext()) {
        // This operation has no terms at all. Since we're an Intersection query, that means that
        // this entire
        // query has no results. Sooo, lets release all of the operations we've already prepare()d
        // and create
