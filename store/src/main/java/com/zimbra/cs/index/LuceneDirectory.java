 ...
 *
 * <p>This forwards all its method calls to the underlying {@link FSDirectory}.
 *
 * @see FSDirectory
 * @author ysasaki
 */
public final class LuceneDirectory extends Directory {
   ...
   *
   * <p>You can switch Lucene's {@link FSDirectory} implementation by {@link
   * LC#zimbra_index_lucene_io_impl}.
   *
   * <ul>
   *   <li>{@code null} -Lucene will try to pick the best {@link FSDirectory} implementation given
   *       the current environment. Currently this returns {@link MMapDirectory} for most Solaris
   *       and Windows 64-bit JREs, {@link NIOFSDirectory} for other non-Windows JREs, and {@link
   *       SimpleFSDirectory} for other JREs on Windows.
   *   <li>{@code simple} - straightforward implementation using java.io.RandomAccessFile. However,
   *       it has poor concurrent performance (multiple threads will bottleneck) as it synchronizes
   *       when multiple threads read from the same file.
   *   <li>{@code nio} - uses java.nio's FileChannel's positional io when reading to avoid
   *       synchronization when reading from the same file. Unfortunately, due to a Windows-only Sun
   *       JRE bug this is a poor choice for Windows, but on all other platforms this is the
   *       preferred choice.
   *   <li>{@code mmap} - uses memory-mapped IO when reading. This is a good choice if you have
   ...
   *       files from user code. The files are unmapped, when GC releases the byte buffers. Due to
   *       this bug in Sun's JRE, MMapDirectory's IndexInput.close() is unable to close the
   *       underlying OS file handle. Only when GC finally collects the underlying objects, which
   *       could be quite some time later, will the file handle be closed. This will consume
   *       additional transient disk usage: on Windows, attempts to delete or overwrite the files
   *       will result in an exception; on other platforms, which typically have a "delete on last
   *       close" semantics, while such operations will succeed, the bytes are still consuming space
   *       on disk. For many applications this limitation is not a problem (e.g. if you have plenty
   *       of disk space, and you don't rely on overwriting files on Windows) but it's still an
   *       important limitation to be aware of. This class supplies a (possibly dangerous)
   *       workaround mentioned in the bug report, which may fail on non-Sun JVMs.
   * </ul>
   *
