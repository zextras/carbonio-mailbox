...
/**
 * QueryResults wrapper that implements Re-Sorting. It does this by caching **ALL** hits and then
 * sorting them. It is used for the Task sorts as well as specially localized language sorts
 */
    ...
    while ((cur = results.getNext()) != null) {
      ...
      if (isReadSort()) {
        if (!(cur instanceof ConversationHit
            || cur instanceof ProxiedHit
            || cur instanceof MessageHit
            || cur instanceof MessagePartHit)) {
          throw ServiceException.FAILURE(
              "Invalid hit type, can only read sort message, " + "conversation, message part",
              null);
        }
      }
      ...
      if (params != null
          && params.getCursor() != null
          && !SearchParams.isSortByReadFlag(params.getSortBy())) {
