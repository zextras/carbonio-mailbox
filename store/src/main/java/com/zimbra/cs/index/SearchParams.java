 ...
 *
 * <p>IMPORTANT NOTE: if you add new parameters, you MUST add parsing/serialization code to the
 * {@link #encodeParams(Element)} and {@link #parse(Element, ZimbraSoapContext, String)}) APIs. This
 * IS NOT optional and will break cross-server search if you do not comply.
 */
public final class SearchParams implements Cloneable, ZimbraSearchParams {
  ...
  private static final Pattern LOCALE_PATTERN =
      Pattern.compile("([a-zA-Z]{2})(?:[-_]([a-zA-Z]{2})([-_](.+))?)?");

  private ZimbraSoapContext requestContext;

  /**
   * this parameter is intentionally NOT encoded into XML, it is encoded manually by the
   * ProxiedQueryResults proxying code.
   */
  ...
  private boolean fullConversation =
      false; // All messages in a matching conversation should be returned
  private boolean includeMemberOf =
      false; // use to include info on which contact groups a contact is in
  ...
  /**
   * Set the range of dates over which we want to expand out the instances of any returned
  ...
  /**
   * Since the results are iterator-based, the {@code limit} is really the same as the {@code chunk
   * size + offset} i.e. the limit is used to tell the system approximately how many results you
   * want and it tries to get them in a single chunk, but it isn't until you do the results
   * iteration that the limit is enforced.
   */
  ...
  /**
   * Encode the necessary parameters into a {@code <SearchRequest>} (or similar element) in cases
   * where we have to proxy a search request over to a remote server.
   *
   * <p>Note that not all parameters are encoded here -- some params (like cursor, etc) are changed
   * by the entity doing the search proxying, and so they are set at that level.
   *
   ...
   *
   * @param request {@code <SearchRequest>} itself, or similar element ({@code <SearchConvRequest>},
   *     etc)
   * @param requestedAccount account who's mailbox we should search in
   * @param zsc SoapContext of the request
   */
  public static SearchParams parse(
      SearchParameters soapParams, ZimbraSoapContext zsc, String defaultQueryStr)
    ...
    String types =
        soapParams.getSearchTypes() == null ? soapParams.getGroupBy() : soapParams.getSearchTypes();
    if (Strings.isNullOrEmpty(types)) {
      params.setTypes(
          EnumSet.of(params.isQuick() ? MailItem.Type.MESSAGE : MailItem.Type.CONVERSATION));
    } else {
      params.setTypes(types);
    }
    params.setSortBy(soapParams.getSortBy());
    if (soapParams.getSortBy() != null
        && query.toLowerCase().contains("is:unread")
        && isSortByReadFlag(SortBy.of(soapParams.getSortBy()))) {
  ...
  public static SearchParams parse(Element request, ZimbraSoapContext zsc, String defaultQueryStr)
    ...
    params.requestContext = zsc;
    params.setHopCount(zsc.getHopCount());
    params.setCalItemExpandStart(
        request.getAttributeLong(MailConstants.A_CAL_EXPAND_INST_START, -1));
    ...
    String types =
        request.getAttribute(
            MailConstants.A_SEARCH_TYPES, request.getAttribute(MailConstants.A_GROUPBY, null));
    if (Strings.isNullOrEmpty(types)) {
      params.setTypes(
          EnumSet.of(params.isQuick() ? MailItem.Type.MESSAGE : MailItem.Type.CONVERSATION));
    } else {
    ...
    if (params.getInlineRule() != ExpandResults.NONE) {
      ...
      params.setMaxInlinedLength(
          (int) request.getAttributeLong(MailConstants.A_MAX_INLINED_LENGTH, -1));
   ...
   * @param acctId requested account id
   */
  public void parseCursor(CursorInfo cursorInfo, String acctId, SearchParams params)
      throws ServiceException {
    cursor = new Cursor();
    ...
    cursor.includeOffset =
        MoreObjects.firstNonNull(cursorInfo.getIncludeOffset(), false); // optional
  }
  ...
  private static TimeZone parseTimeZonePart(Element tzElt) throws ServiceException {
    ...
    if (test == null) {
      throw ServiceException.INVALID_REQUEST(
          "Unknown TZ: \"" + id + "\" and no " + MailConstants.A_CAL_TZ_STDOFFSET + " specified",
          null);
    }
  ...
  public static final class ExpandResults {
    ...
    /**
     * For searchConv: if there are matching hits, expand them, otherwise expand the first message
     * in the conversation
     */
    public static final ExpandResults HITS_OR_FIRST_MSG = new ExpandResults("hits!");

    /**
     * For searchConv: if there are unread matching hits, expand them, otherwise expand first
     * message in the conversation This is a little ambiguous - should 1st msg be expanded if there
     * are no matching hits at all, or if there are no UNREAD matching hits? For now, expand 1st msg
     * if there are no UNREAD matching hits (or no matching hits at all, obviously)
     */
    public static final ExpandResults U_OR_FIRST_MSG = new ExpandResults("u!");

    /**
     * For searchConv: if there are unread matching hits, expand them, otherwise expand first hit
     * AND first message in conversation Somewhat ambiguous: what to do if no matching hits? For
     * now, expand the first message.
     */
    public static final ExpandResults U1_OR_FIRST_MSG = new ExpandResults("u1!");

    private static final Map<String, ExpandResults> MAP =
        ImmutableMap.<String, ExpandResults>builder()
            .put(NONE.name, NONE)
            .put("0", NONE)
            .put("false", NONE)
            ...
            .put("u", UNREAD)
            .put(UNREAD_FIRST.name, UNREAD_FIRST)
            .put("u1", UNREAD_FIRST)
            .put(HITS.name, HITS)
            .put(ALL.name, ALL)
    ...
    private static final Map<String, ExpandResults> LEGACY_MAP =
        ImmutableMap.<String, ExpandResults>builder()
            .put(FIRST.name, FIRST)
            .put("1", FIRST)
            .put(HITS.name, HITS)
      ...
      if (server != null && server.getServerVersion() == null) {
        if (LEGACY_MAP.containsKey(this.name)) {
          ...
          ZimbraLog.search.debug(
              "mapped current ExpandResults %s to %s for legacy server %s",
              this, mapped, server.getName());
          return mapped;
        }
      } else {
        // for now 8.5+ supports the same set of expands; would add code here if 8.6 or 9.0 changes
        // it
        return this;
      }
    }
  ...
  /**
   * A cursor can be specified by itemId and sortValue. These should be enough for us to find out
   * place in the previous result set, even if entries have been added or removed from the result
   * set. If the client doesn't know sortValue, e.g. changing the sort field from date to subject,
   * it may leave it null, then the server fetches the item by the specified itemId, and sets the
   * sortValue accordingly. If the item no longer exist when fetching it, the cursor gets cleared.
   */
  public static final class Cursor {
    ...
    private boolean includeOffset =
        false; // whether or not to include the cursor offset in the response
