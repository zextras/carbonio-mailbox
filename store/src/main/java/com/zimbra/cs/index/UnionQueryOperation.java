...
public final class UnionQueryOperation extends CombiningQueryOperation {
  ...
  public ZimbraHit getNext() throws ServiceException {
    ...
    // this "if" is here so we don't keep calling internalGetNext when we've reached the end of the
    // results...
  ...
  void pruneIncompatibleTargets(Set<QueryTarget> targets) {
    ...
    for (int i = operations.size() - 1; i >= 0; i--) {
      ...
      if (op instanceof UnionQueryOperation) {
        ...
        assert (set.size() <= 1);
        if (set.isEmpty()
            || (!targets.containsAll(set) && !set.contains(QueryTarget.UNSPECIFIED))) {
          operations.remove(i);
        }
      }
    }
  ...
  public QueryOperation optimize(Mailbox mbox) throws ServiceException {
    ...
    // now - check to see if we have only one child -- if so, then WE can be eliminated, so push the
    // child up
  ...
  protected void begin(QueryContext ctx) throws ServiceException {
    ...
    for (QueryOperation op : operations) {
      ...
      op.begin(
          new QueryContext(
              ctx.getMailbox(), ctx.getResults(), ctx.getParams(), ctx.getChunkSize() + 1));
    }
    internalGetNext();
  }
