...
public class LuceneViewer {
  ...
  public LuceneViewer(
      String indexName, String outfileName, TermFilters termFilters, Console console)
      throws Exception {
    mIndexDir = indexName;
  ...
  private boolean wantThisTerm(String termField, String termText) {
    ...
    for (TermFilters.TermFilter termFilter : getFilters()) {
      ...
      boolean matched =
          ((field == null || field.equalsIgnoreCase(termField))
  ...
  private void dumpFields() throws IOException {
    ...
    Collection<String> fieldNames = mIndexReader.getFieldNames(IndexReader.FieldOption.ALL);
  ...
  private static class CLI {
    ...
    private Options getTermFilterOptions() {
      ...
      for (int i = 1; i <= NUM_TERM_FILTERS; i++) {
        options.addOption(
            termFilterFieldOption(i), "field" + i, true, "field name of term filter " + i);
    ...
    private void usage(ParseException e, boolean exit) {
      ...
      System.err.println(
          "    - each f[n], t[n] pair represents a term filter, f[n], t[n] don't have to both"
              + " exist");
      ...
      System.err.println(
          "zmjava com.zimbra.cs.index.LuceneViewer -a dump -i /opt/zextras/index/0/2/index/0 -o"
              + " /tmp/user1-index-dump.txt");
      System.err.println(
          "zmjava com.zimbra.cs.index.LuceneViewer -a dump -v -f1 l.content -t1 jay -f2 subject -t2"
              + " howdy -i /opt/zextras/index/0/2/index/0 -o /tmp/user1-index-dump.txt");
      System.err.println(
          "zmjava com.zimbra.cs.index.LuceneViewer -a dump -f1 from jay@test.com -i"
              + " /opt/zextras/index/0/2/index/0 -o /tmp/user1-index-dump.txt");
