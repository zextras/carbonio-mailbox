...
public final class InQuery extends Query {
   ...
   */
  public static InQuery create(Mailbox mbox, int fid, boolean under) throws ServiceException {
  ...
  public static InQuery create(Mailbox mailbox, String folder, boolean under)
    ...
    Pair<Folder, String> result =
        mailbox.getFolderByPathLongestMatch(null, Mailbox.ID_FOLDER_USER_ROOT, folder);
    return recursiveResolve(mailbox, result.getFirst(), result.getSecond(), under);
  }
  ...
  /**
   * Resolve through local mountpoints until we get to the actual folder, or until we get to a
   * remote folder.
   */
  private static InQuery recursiveResolve(
      Mailbox mailbox, Folder folder, String path, boolean under) throws ServiceException {

    if (!(folder instanceof Mountpoint)) {
      if (path != null) {
        throw MailServiceException.NO_SUCH_FOLDER(folder.getPath() + "/" + path);
      }
      return new InQuery(folder, null, null, null, under);
    } else {
      ...
      if (mpt.isLocal()) { // local
        if (Strings.isNullOrEmpty(path)) {
          ...
          return recursiveResolve(
              mailbox, mailbox.getFolderById(null, mpt.getRemoteId()), path, under);
        }
      } else { // remote
          ...
          for (Folder sub : subFolders) {
            ...
            if (sub instanceof Mountpoint) {
              ...
              if (!mpt.isLocal()) {
                ...
                // TODO FIXME handle local mountpoints. Don't forget to check for infinite
                // recursion!
              }
            } else {
  ...
  public void dump(StringBuilder out) {
    ...
    if (specialTarget != null) {
      ...
      out.append(
          remoteId != null
              ? remoteId.toString()
              : (folder != null ? folder.getName() : "ANY_FOLDER"));
