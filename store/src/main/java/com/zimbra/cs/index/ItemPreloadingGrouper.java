 ...
 *
 * <p>This is done so that we can lower the number of SELECT calls to the DB by batch-fetching the
 * Message objects from the store.
 *
 * @author tim
 */
final class ItemPreloadingGrouper extends BufferingResultsGrouper {
  ...
  ItemPreloadingGrouper(
      ZimbraQueryResults results, int chunkSize, Mailbox mbox, boolean inDumpster) {
    super(results);
    assert (chunkSize > 0);
  ...
  private void preload(Mailbox mbox, List<ZimbraHit> hits) throws ServiceException {
    ...
    if (numToLoad > 0) {
      MailItem[] items =
          mbox.getItemById(opContext, unloadedIds, MailItem.Type.UNKNOWN, inDumpster);
      for (int i = 0; i < hits.size(); ++i) {
        if (items[i] != null) {
