...
/**
 * Represents the results of a query made on a remote server. This class takes the ServerID and the
 * query parameters and makes does smart chunking (buffering) of the results - making SOAP requests
 * to the remote Zimbra server as necessary.
 *
 * <p>TODO wish this could be a {@link QueryOperation} subclass instead of just a results subclass:
 * unfortunately right now the intersection and other operations assume you can get to the actual
 * hit objects (for doing Intersections and the like).... Long-term-fixme...
 *
 * @since Mar 28, 2005
 */
public final class ProxiedQueryResults extends ZimbraQueryResultsImpl {
   ...
   *
   * <p>The query string for this search is the string passed in the queryString parameter, *not*
   * the query string in the params.
   *
   * @param server hostname of server
   * @param queryString queryString to use for this search
   */
  public ProxiedQueryResults(
      SoapProtocol respProto,
      AuthToken authToken,
  ...
  private void setSearchParams(SearchParams params) {
    ...
    if ((searchParams.getCursor() != null)
        && (searchParams.getLimit() > 0)
        && (searchParams.getLimit() < 500)) {
      SortBy sb = getSortBy();
      ...
      if (sb != null
          && ((SortBy.NAME_LOCALIZED_ASC.equals(sb))
              ...
              || (SortBy.NAME_ASC.equals(sb))
              || (SortBy.NAME_DESC.equals(sb)))) {
  ...
  /**
   * The results of the proxied search are added to the hit array in the same order as in the
   * proxied search response - so there should be no need to sort the results again.
   */
  @Override
