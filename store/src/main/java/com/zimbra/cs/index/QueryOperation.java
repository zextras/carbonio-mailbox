...
/**
 * A {@link QueryOperation} is a part of a Search request -- there are potentially multiple query
 * operations in a search. {@link QueryOperation} return {@link ZimbraQueryResultsImpl} sets --
 * which can be iterated over to get {@link ZimbraHit} objects.
 *
 * <p>The difference between a {@link QueryOperation} and a simple {@link ZimbraQueryResultsImpl}
 * set is that a {@link QueryOperation} knows how to Optimize and Execute itself -- whereas a {@link
 * QueryResult} set is just a set of results and can only be iterated.
 *
 * @since Oct 15, 2004
 */
public abstract class QueryOperation implements Cloneable, ZimbraQueryResults {
  ...
  // TOT_VIRTUAL_CONVS = select count(*) from mail_item mi where mi.type=5 and mi.parent_id is NULL
  // and mi.mailbox_id=?;
  //
  ...
  // What level of result grouping do we want?  ConversationResult, MessageResult, or
  // DocumentResult?
   ...
   * @param chunkSize A hint to the query operation telling it what size to chunk data in. Higher
   *     numbers can be more efficient if you are using a lot of results, but have more overhead.
  ...
  final ZimbraQueryResults run(Mailbox mbox, SearchParams params, int chunkSize)
    ...
    // set me to TRUE if you're returning Conversations or something which could benefit from
    // preloading
    ...
    switch (retType) {
      ...
      case ITEM:
        if (params.getPrefetch() && usePreloadingGrouper) {
          ...
          results =
              new UngroupedQueryResults(this, types, params.getSortBy(), params.getFetchMode());
        }
        break;
      default:
        assert (false);
  ...
  /**
   * Begins query execution. It is allowed to grab and hold resources, which are then released via
   * {@link #doneWithSearchResults()}.
   *
   * <p>IMPORTANT: {@link #begin(QueryContext)} and {@link #doneWithSearchResults()} must always be
   * called in a pair. That is, if you call {@link #begin(QueryContext)}, you MUST call {@link
   ...
   * setting as necessary. Descend down the tree, when you hit an AND, then only one of the subtrees
   * must have it, when you hit an OR, then every subtree must have a setting, or else one must be
   * added.
   *
   * @param includeTrash TODO
   * @param includeSpam TODO
   */
  abstract QueryOperation ensureSpamTrashSetting(
      Mailbox mbox, boolean includeTrash, boolean includeSpam) throws ServiceException;
   ...
   * this API lets us force the "include trash/spam" part in the other query and thereby drop the
   * 1st one.
   */
  abstract void forceHasSpamTrashSetting();

  /**
   * @return TRUE if this QueryOperation definitely has no results. Note that this API might return
   *     FALSE in some cases where there really aren't any results available -- it is only
   *     guaranteed to catch "trivial-reject" cases useful during the query optimization process
   */
  abstract boolean hasNoResults();

  /**
   * @return TRUE if this QueryOperation returns *all* hits. Note that this API might return FALSE
   *     in cases where this operation returns all results -- this API is only intended to catch
   *     trivial cases and is useful during the query optimization process
   */
  ...
  /**
   * @param mbox
   * @return An Optimzed version of this query, or NULL if this query "should have no effect". If
   *     NULL is returned, it is up to the caller to decide what to do (usually replace with a
   *     NullQueryOperation -- however sometimes like within an Intersection you might just want to
   *     remove the term)
   * @throws ServiceException
   */
  ...
  /**
   * Called when optimize()ing a UNION or INTERSECTION -- see if two Ops can be expressed as one
   * (e.g. is:unread and in:inbox can both be expressed via one DBQueryOperation)
   *
   * @param other
   * @param union
   * @return the new operation that handles both us and the other constraint, or NULL if the ops
