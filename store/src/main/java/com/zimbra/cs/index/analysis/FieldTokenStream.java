 ...
 *
 * <p>{@code name:Val1 val2 val3} gets tokenized to {@code name:val1}, {@code name:val2}, {@code
 * name:val3}. If the field only consists of a single integer value, it produces an extra token of
 * which name is appended by '#' to distinguish from text search and the integer value gets encoded
 * by Lucene's {@link NumericUtils}, so that it is also searchable by numeric range query. Note that
 * numeric fields are still tokenized as text too for wildcard search.
 *
