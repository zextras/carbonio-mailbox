...
/**
 * Helper class -- deals with finding the "first result" to be returned, which is dealt with in one
 * of several ways depending on if this is an offset/limit request, a forward cursor request, or a
 * backward cursor request. (see bug 2937)
 */
public final class ResultsPager {
  ...
  /**
   * @param params if OFFSET-MODE, requires SortBy, offset, limit to be set, otherwise requires
   *     cursor to be set
   */
  private ResultsPager(
      ZimbraQueryResults results, SearchParams params, boolean useCursor, boolean skipOffset)
      throws ServiceException {
  ...
  public void reset() throws ServiceException {
    if (fixedOffset) {
      ...
      if (forward) {
        ...
        if (current != null) bufferedHits.add(current);
      } else {
    ...
    while (hit != null) {
      ...
      if (comp == 0) {
        ...
        if (params.getSortBy().getDirection() == SortBy.Direction.DESC
            || params.getSortBy().getKey() == SortBy.Key.UNREAD) {
  ...
  private List<ZimbraHit> backward() throws ServiceException {
    ...
    while (hit != null) {
      ...
      if (params.getCursor().getEndSortValue() != null
          && hit.compareTo(params.getSortBy(), dummyEndHit) <= 0) {
        break;
      }
      hit = results.getNext();
    }
  ...
  private ZimbraHit getPrevCursorHit() {
    return new CursorHit(
        results, params.getCursor().getSortValue(), params.getCursor().getItemId().getId());
  }
