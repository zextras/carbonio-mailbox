 ...
 *
 * <p>Once corruption is detected, re-indexing is the only way to recover correctness. But, since
 * re-indexing is very expensive, we do our best to repair the index data with compromising some
 * level of correctness.
 *
 * <p>Make sure there is absolutely no IndexReader or IndexWriter opening the index data during the
 * repair.
 *
 * @author ysasaki
 */
final class LuceneIndexRepair {
  ...
  int repair() throws IOException {
    ...
    try {
      ChecksumIndexOutput output =
          new ChecksumIndexOutput(directory.createOutput(nextSegsFilename));
      try {
  ...
  private int getDelCount(String name, long gen) throws IOException {
    if (gen < 0) { // no del file
      ...
      String filename =
          name + "_" + Long.toString(gen, Character.MAX_RADIX) + "." + DELETES_EXTENSION;
      if (directory.fileExists(filename)) {
        BitVector del = new BitVector(directory, filename);
        return del.count();
      } else {
