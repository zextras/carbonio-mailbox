 ...
 *
 * <p>The absolute-date (e.g. mm/dd/yyyy) pattern is locale sensitive. This implementation delegates
 * it to JDK's {@link DateFormat} class whose behavior is as follows:
 *
 * <table>
...
public final class DateQuery extends Query {
  ...
  /**
   * DATE: {@code absolute-date = mm/dd/yyyy} (locale sensitive) OR {@code relative-date =
   * [+/-]nnnn{minute,hour,day,week,month,year}}
   *
   * <p>TODO need to figure out how to represent "this week", "last week", "this month", etc.
   */
  public void parseDate(String src, TimeZone tz, Locale locale) throws ParseException {
    ...
    if (matcher.lookingAt()) {
      ...
      if (matcher.lookingAt()) {
        ...
        // special case 'day' clear all the fields that are lower than the one we're currently
        // operating on...
