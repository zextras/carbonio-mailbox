...
/**
 * An interface to a tree of ANDed and ORed search constraints for the DB-backed data in a search
 * request.
 *
 * <p>Instead of a single SearchConstraints, DB searches can be done on a tree of SearchConstraints.
 * Each node of the tree is either an AND, OR, or a leaf node.
 *
 * @author tim
 * @author ysasaki
 */
public interface DbSearchConstraints extends Cloneable {
  ...
  /**
   * Used during query optimization: the optimizer is saying "make sure this query subtree is
   * excluding spam and or trash".
   *
  ...
  /**
   * Used during query optimization: returns TRUE if we have folder settings (such as "in:foo") that
   * mean we don't need to have constraints added for the implicit spam/trash setting.
   */
  boolean hasSpamTrashSetting();

  /**
   * A bit of a hack -- when we combine a "all items including trash/spam" term with another query,
   * this lets us force the "include trash/spam" part in the other query and thereby drop the 1st
   * one.
   */
  void forceHasSpamTrashSetting();

  /**
   * AND another {@link DbSearchConstraints} into our tree: this returns the new root node (might be
   * this, might be a new top level).
   */
  DbSearchConstraints and(DbSearchConstraints other);

  /**
   * OR another {@link DbSearchConstraints} into our tree: this returns the new root node (might be
   * this, might be a new top level).
   */
  DbSearchConstraints or(DbSearchConstraints other);

  /**
   * Helper for query optimization: returns TRUE if the current set of constraints returns ZERO
   * results.
   */
  boolean hasNoResults();

  /**
   * Used during query optimization: returns TRUE if the constraints are such that we think a
   * DB-first plan will be faster.
   */
   ...
   * when we have to send a query to a remote server.
   */
  ...
  public static final class Leaf implements DbSearchConstraints, Cloneable {
    ...
    /**
     * When we COMBINE the operations during query optimization, we'll need to track some state
     * values for example "no results".
     */
    ...
    public final Set<RemoteFolderDescriptor> excludeRemoteFolders =
    ...
    // A possible result must match *ALL* the range constraints below.  It might seem strange that
    // we don't
    // just combine the ranges -- yes this would be easy for positive ranges (foo>5 AND foo >7 and
    // foo<10)
    // but it quickly gets very complicated with negative ranges such as (3<foo<100 AND NOT
    // 7<foo<20)
    ...
    /**
     * Returns TRUE if these constraints have a constraint that requires a join with the Appointment
     * table.
     */
    public boolean hasAppointmentTableConstraints() {
      Set<MailItem.Type> types = calcTypes();
      return ((ranges.containsKey(RangeType.CAL_START_DATE)
              || ranges.containsKey(RangeType.CAL_END_DATE))
          && types.contains(MailItem.Type.APPOINTMENT));
    }

    /**
     * Returns the only folder if query is for a single folder, item type list includes {@link
     * MailItem.Type#MESSAGE}, and no other conditions are specified. Otherwise returns null.
     */
    public Folder getOnlyFolder() {
      if (folders.size() == 1
          ...
          && indexIds.isEmpty()
          && ranges.isEmpty()
          && types.contains(MailItem.Type.MESSAGE)
          && !excludeTypes.contains(MailItem.Type.MESSAGE)
          && tags.isEmpty()
          && excludeTags.isEmpty()
          && convId == 0
          && prohibitedConvIds.isEmpty()
    ...
    /**
     * Returns TRUE/FALSE if the conv-count constraint is set, and if the constraint is exactly 1
     * (ie >=1 && <=1). This is a useful special case b/c if this is true then we can generate much
     * simpler SQL than for the more complicated conv-count cases. Returns NULL otherwise.
     *
     * <p>TRUE "is:solo" FLASE "-is:solo" null everything else
     */
    ...
    /**
     * If name contains spaces, quote the string using double quotes, taking care to escape any
     * existing double quotes. Introduced for bug 81940 because the query parser regards TAG:(foo
     * bar) as 2 tags, "foo" and "bar", so we need a way to preserve the integrity of tags
     * containing spaces.
     */
    private String quoteIfNecessary(String name) {
      ...
      String quoted =
          new StringBuilder("\"").append(name.replaceAll("\"", "\\\\\"")).append("\"").toString();
      return quoted;
    }
    ...
    public void and(Leaf other) {
      ...
      //  if both sets are empty going in, then an empty set means "no constraint"....on the other
      // hand if either set
        ...
        if (convId != 0) {
          if (convId != other.convId) {
            ZimbraLog.index.debug(
                "ANDING a constraint with incompatible convIds, this is a NO_RESULTS constraint"
                    + " now");
            noResults = true;
          }
        } else {
      ...
      if (other.remoteConvId != null) {
        if (remoteConvId != null) {
          if (!remoteConvId.equals(other.remoteConvId)) {
            ZimbraLog.index.debug(
                "ANDING a constraint with incompatible remoteConvIds, this is a NO_RESULTS"
                    + " constraint now");
            noResults = true;
          }
        } else {
      ...
      if (hasIndexId == null) {
        ...
        if (!hasIndexId.equals(other.hasIndexId)) {
          noResults = true;
          ZimbraLog.index.debug(
              "Adding a HAS_NO_INDEXIDS constraint to a HAS_INDEXIDS one, this is a NO_RESULTS"
                  + " result");
          return;
        }
      }
     ...
     *
     * <p>True if we have a SETTING pertaining to Spam/Trash. This doesn't necessarily mean we
     * actually have "in trash" or something, it just means that we've got something set which means
     * we shouldn't add the default "not in:trash and not in:junk" thing.
     */
    ...
    public void addCalStartDateRange(
      ...
      ranges.put(
          RangeType.CAL_START_DATE, new NumericRange(min, minInclusive, max, maxInclusive, bool));
    }
    ...
    public void addCalEndDateRange(
      ...
      ranges.put(
          RangeType.CAL_END_DATE, new NumericRange(min, minInclusive, max, maxInclusive, bool));
    }
    ...
    public void addConvCountRange(
      ...
      ranges.put(
          RangeType.CONV_COUNT, new NumericRange(min, minInclusive, max, maxInclusive, bool));
    }
    ...
    void setCursorRange(
        String min, boolean minInclusive, String max, boolean maxInclusive, SortBy sort) {
      ...
      if (truth) {
        ...
        if (convId == 0) {
          ...
          ZimbraLog.search.debug(
              "Query requested two conflicting convIDs, this is now a no-results-query");
    ...
    void addRemoteConvId(ItemId cid, boolean truth) {
      ...
      if (truth) {
        ...
        if (remoteConvId == null) {
          ...
          ZimbraLog.search.debug(
              "Query requested two conflicting Remote convIDs, this is now a no-results-query");
    ...
    void addInRemoteFolder(
        ItemId id, String subfolderPath, boolean includeSubfolders, boolean bool) {
      if (bool) {
        if ((!remoteFolders.isEmpty() && !remoteFolders.contains(id))
  ...
  static final class Union implements DbSearchConstraints {
    ...
    /**
     * Bug: 79244 This method iterates over the children of an OR node and combines leaves that have
     * only folder constraints into a single node. This improves the resulting SQL from (folder_id =
     * ?) OR (folder_id = ?) OR ... to (folder_id IN (?, ?, ?...), which is more efficient when
     * there are many folder constraints.
     *
     * @return
     */
    ...
    public DbSearchConstraints optimize() {
      ...
      for (Set<MailItem.Type> types : byValue.keySet()) {
        ...
        if (nodes.size() > 2) {
          ...
          newAnd.children.add(
              0, factoredLeaf); // need to add on the left so that types get encoded first
        }
        children.add(newAnd);
      }
      if (!optimized) {
  ...
  static final class StringRange extends Range {
    ...
    public boolean equals(Object o) {
      if (o instanceof StringRange) {
        ...
        return Objects.equal(other.min, min)
            && other.minInclusive == minInclusive
            && Objects.equal(other.max, max)
  ...
  public static final class RemoteFolderDescriptor {
    ...
    public RemoteFolderDescriptor(ItemId iidFolder, String subpath, boolean includeSubfolders) {
      ...
      if (this.subfolderPath == null) this.subfolderPath = "";
    }
