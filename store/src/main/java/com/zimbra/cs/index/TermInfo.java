...
public final class TermInfo {
   ...
   *
   * <p>if the field from the Lucene document is indexed and tokenized, for each token: a) construct
   * a key based on the field name and info about the token b) if {@code term2info} has an entry for
   * that key, get it, otherwise create an entry c) update the entry with position information for
   * this token
   *
   * @param pos is the current position
   * @return new value for {@code pos}
   */
  public static int updateMapWithDetailsForField(
      Analyzer analyzer, Fieldable field, Map<String, TermInfo> term2info, int pos)
      throws IOException {
    if (!field.isIndexed()) {
      return pos;
    }
    ...
    if (prefix == null) {
      ZimbraLog.index.info(
          "TermInfo.updateMapWithDetailsForField - skipping indexed field "
              + field.name()
              + " isTokenized="
              + field.isTokenized());
      return pos;
    ...
    if (field.isTokenized()) {
      ...
      // whole field is the only "token".  Info potentially getting stored twice - here as well as
      // where
      // the field is stored.
