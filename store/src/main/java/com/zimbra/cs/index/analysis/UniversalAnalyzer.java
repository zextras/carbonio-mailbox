 ...
 *
 * <p>This {@link Analyzer} may not be perfect compared to ones that are optimized for a specific
 * language, which requires to switch analyzers depending on the language, but does a decent job for
 * most languages and even mixed text just by this single analyzer. The implementation is based on
 * {@code StandardAnalyzer}, and applies bigram tokenization to CJK unicode blocks.
 *
 * @author ysasaki
 */
public final class UniversalAnalyzer extends Analyzer {
  ...
  @Override
  public final TokenStream reusableTokenStream(String field, Reader in) throws IOException {
    ...
    try {
      stopWords =
          Provisioning.getInstance()
              .getConfig()
              .getMultiAttrSet(Provisioning.A_zimbraDefaultAnalyzerStopWords);
