...
final class RemoteQueryOperation extends FilterQueryOperation {
  ...
  /**
   * Try to OR an operation into this one. Return FALSE if that isn't possible (incompatible query
   * targets).
   *
   * @return FALSE
   */
  ...
  protected void setup(SoapProtocol proto, AuthToken authToken, SearchParams params)
    ...
    if (ZimbraLog.search.isDebugEnabled()) {
      ZimbraLog.search.debug(
          "RemoteQuery=\"%s\" target=%s server=%s",
          queryString, queryTarget, remoteServer.getName());
    }

    results =
        new ProxiedQueryResults(
            proto,
            authToken,
            queryTarget.toString(),
