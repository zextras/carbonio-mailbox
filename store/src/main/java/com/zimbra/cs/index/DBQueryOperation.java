...
/**
 * {@link QueryOperation} which goes to the SQL DB. It might have a "child" Lucene operation
 * attached to it.
 *
 * @since Oct 29, 2004
 * @author tim
...
public class DBQueryOperation extends QueryOperation {
  ...
  private int dbOffset =
      0; // this is the offset IN THE DATABASE when we're doing a DB-FIRST iteration
  private int cursorOffset = -1; // calculated cursor offset

  /**
   * this gets set to FALSE if we have any real work to do this lets us optimize away queries that
   * might match "everything".
   */
  ...
  /**
   * TRUE if we know there are no more hits to get for mDBHitsIter, i.e. there is no need to call
   * getChunk() anymore.
   */
  ...
  /**
   * If set, then this is the AccountId of the owner of a folder we are searching. We track it at
   * the toplevel here b/c we need to make sure that we handle unions (don't combine) and
   * intersections (always empty set) correctly.
   */
  ...
  /**
   * Since Trash can be an entire folder hierarchy, when we want to exclude trash from a query, we
   * actually have to walk that hierarchy and figure out all the folders within it.
   *
   * @param mbox
  ...
  QueryOperation expandLocalRemotePart(Mailbox mbox) throws ServiceException {
    if (constraints instanceof DbSearchConstraints.Leaf) {
      ...
      throw new IllegalStateException(
          "expandLocalRemotePart must be called before optimize() is called");
    }
  }
  ...
  /**
   * A bit weird -- basically we want to AND a new constraint: but since the constraints object
   * could potentially be a tree, we need a function to find the right place in the tree to add the
   * new constraint
   */
  ...
  /**
   * In an INTERSECTION, we can gain some efficiencies by using the output of the Lucene op as
   * parameters to our SearchConstraints....we do that by taking over the lucene op (it is removed
   * from the enclosing Intersection) and handling it internally.
   *
  ...
  public void addSubjectRange(
      String min, boolean minInclusive, String max, boolean maxInclusive, boolean bool) {
    allResultsQuery = false;
    getTopLeafConstraint().addSubjectRange(min, minInclusive, max, maxInclusive, bool);
  }

  public void addSenderRange(
      String min, boolean minInclusive, String max, boolean maxIncluisve, boolean bool) {
    allResultsQuery = false;
    getTopLeafConstraint().addSenderRange(min, minInclusive, max, maxIncluisve, bool);
  }

  public void addItemIdRange(
      int min, boolean minInclusive, int max, boolean maxInclusive, boolean bool) {
    allResultsQuery = false;
  ...
  public void addConvId(Mailbox mbox, ItemId convId, boolean truth) {
    ...
    if (convId.belongsTo(mbox)) { // LOCAL
      ...
      if (queryTarget != QueryTarget.UNSPECIFIED
          && !queryTarget.toString().equals(convId.getAccountId())) {
        throw new IllegalArgumentException(
            "Cannot addConvId w/ remote target b/c DBQueryOperation already has an incompatible"
                + " remote target");
      }
  ...
  public void addIsLocalClause() {
    if (!queryTarget.isCompatibleLocal()) {
      throw new IllegalArgumentException(
          "Cannot addIsLocalFolderClause b/c DBQueryOperation already has a remote target");
    }
    queryTarget = QueryTarget.LOCAL;
    allResultsQuery = false;
  ...
  public void addIsRemoteClause() {
    ...
    if (!(queryTarget == QueryTarget.REMOTE || queryTarget == QueryTarget.UNSPECIFIED)) {
      throw new IllegalArgumentException(
          "Cannot addIsRemoteFolderClause b/c DBQueryOperation already has a remote target: "
              + queryTarget);
    }
  ...
  public void addInRemoteFolder(
      ItemId remoteFolderId, String subfolderPath, boolean includeSubfolders, boolean bool) {
    allResultsQuery = false;
    if (queryTarget != QueryTarget.UNSPECIFIED
        && !queryTarget.toString().equals(remoteFolderId.getAccountId())) {
      throw new IllegalArgumentException(
    ...
    getTopLeafConstraint()
        .addInRemoteFolder(remoteFolderId, subfolderPath, includeSubfolders, bool);
  }
  ...
  public void addAnyFolder(boolean bool) {
    ...
    if (!bool) { // if they are weird enough to say "NOT is:anywhere" then we just make it a
      // no-results-query.
      allResultsQuery = false;
    }
  }
  ...
  /**
   * Return the next hit in our search. If there are no hits buffered then calculate the next hit
   * and put it into the mNextHits list.
   *
   * <ol>
    ...
    if (nextHits.size() > 0) {
      ...
      while (toRet == null) {
        ...
        if (dbHitsIter != null && dbHitsIter.hasNext()) {
          ...
          // Sometimes, a single search result might yield more than one Lucene document -- e.g. an
          // RFC822
          // message with separately-indexed MIME parts. Each of these parts will turn into a
          // separate
          // ZimbraHit at this point, although they might be combined together at a higher level
          // (via a
          // HitGrouper).
          Collection<Document> docs =
              luceneChunk != null ? luceneChunk.getHit(sr.getIndexId()) : null;

          if (docs == null || !ZimbraQueryResultsImpl.shouldAddDuplicateHits(sr.getType())) {
            ZimbraHit toAdd =
                context.getResults().getZimbraHit(context.getMailbox(), sr, null, fetch);
            if (toAdd != null) {
            ...
            for (Document doc : docs) {
              ZimbraHit toAdd =
                  context.getResults().getZimbraHit(context.getMailbox(), sr, doc, fetch);
              if (toAdd != null) {
                // make sure we only return each hit once
                if (!mSeenHits.containsKey(toAdd)) {
                  mSeenHits.put(toAdd, toAdd);
   ...
   * for example an Appointment might have hits from multiple Exceptions (each of which has its own
   * Lucene document) and they will return the same AppointmentHit to us. This is the place where we
   * collapse those hits down to single hits.
   *
   * <p>Note that in the case of matching multiple MessageParts, the ZimbraHit that is returned is
  ...
  private void dbSearch(List<DbSearch.Result> results, SortBy sort, int offset, int size)
      throws ServiceException {
    ...
    results.addAll(
        context
            .getMailbox()
            .index
            .search(constraints, fetch, sort, offset, size, context.getParams().inDumpster()));
      ...
      if (dbResults.size() > 0) {
        ...
        try {
          ...
          for (DbSearch.Result sr : dbResults) {
            ...
            luceneOp.addFilterClause(
                new Term(LuceneFields.L_MAILBOX_BLOB_ID, String.valueOf(sr.getIndexId())));
          }
  ...
  /**
   * Use all the search parameters (including the embedded {@link LuceneQueryOperation}) to get a
   * chunk of search results and put them into dbHits
   *
   * <p>On Exit: If there are more results to be had
   *
   * <ul>
   *   <li>dbHits has entries
   *   <li>dbHitsIter is initialized
   *   <li>hitsOffset is the absolute offset (into the result set) of the last entry in dbHits +1
  ...
  protected void begin(QueryContext ctx) throws ServiceException {
    ...
    if (luceneOp != null) {
      ...
      luceneOp.begin(
          new QueryContext(ctx.getMailbox(), ctx.getResults(), ctx.getParams(), hitsPerChunk));
    }
  }
  ...
  private void addCursorConstraint() throws ServiceException {
    ...
    // bug 35039 - using cursors with conversation-coalescing leads to convs appearing on multiple
    // pages
    ...
    // - Conversation mode can use cursors to find the right location in the hits, but we *can't*
    // use a
    //   constrained-offset query to find the right place in the search results....in Conv mode we
    // need to walk
    //   through all the results so that we can guarantee that we only return each Conversation once
    // in a given
    //   results set.
    // - bug:23427 TASK sorts are incompatible with CURSORS, since cursors require real (db-visible)
    // sort fields.
    switch (sort) {
      case NONE:
      ...
      case ID_ASC:
        {
          ...
          int high =
              cursor.getEndSortValue() != null ? Integer.parseInt(cursor.getEndSortValue()) : -1;
          DbSearchConstraints.Leaf top = getTopLeafConstraint();
          if (calcOffset) {
      ...
      case DATE_ASC:
        {
          ...
          long high =
              cursor.getEndSortValue() != null ? Long.parseLong(cursor.getEndSortValue()) : -1;
          DbSearchConstraints.Leaf top = getTopLeafConstraint();
          if (calcOffset) {
            offsetConstraints = top.clone();
    ...
    if (offsetConstraints != null) {
      ...
      try {
        cursorOffset =
            new DbSearch(mbox, context.getParams().inDumpster())
                .countResults(conn, offsetConstraints);
      } finally {
        conn.closeQuietly();
      }
    }
  ...
  /**
   * Part of fix for Bug 79576 - invalid hits in shared folders being returned. Check whether this
   * is an AND search including a search for a non-existent tag. WARNING - should only be used for
   * local searches - that tag might exist for a shared folder.
   */
  public boolean isSearchForNonexistentLocalTag(Mailbox mbox) {
    ...
    if (constraints instanceof DbSearchConstraints.Intersection) {
      DbSearchConstraints.Intersection intersection =
          (DbSearchConstraints.Intersection) constraints;
  ...
  protected QueryOperation combineOps(QueryOperation other, boolean union) {
    if (union) {
      ...
      if (queryTarget != QueryTarget.UNSPECIFIED
          && dbOther.queryTarget != QueryTarget.UNSPECIFIED) {
        if (!queryTarget.equals(dbOther.queryTarget)) {
          ZimbraLog.search.debug(
              "ANDing two DBOps with different targets -- this is a no results query!");
          return new NoResultsQueryOperation();
        }
      }
  ...
  int getDbHitCount() throws ServiceException {
    if (dbHitCount < 0) {
      ...
      try {
        dbHitCount =
            new DbSearch(mbox, context.getParams().inDumpster()).countResults(conn, constraints);
      } finally {
        conn.closeQuietly();
      }
    }
