...
public class TextQuery extends Query {
  ...
  /**
   * A single search term. If text has multiple words, it is treated as a phrase (full exact match
   * required) text may end in a *, which wildcards the last term.
   */
   ...
   *
   * <p>Makes this a wildcard query and gives a query suggestion by auto-completing the last term
   * with the top term, which is the most frequent term among the wildcard-expanded terms.
   *
   * <p>TODO: The current query suggestion implementation can't auto-complete a phrase query as a
   * phrase. It simply auto-completes the last term as if it's a single term query.
   */
  ...
  public QueryOperation compile(Mailbox mbox, boolean bool) throws ServiceException {
    if (quick
      ...
      LuceneQueryOperation.LazyMultiPhraseQuery query =
          new LuceneQueryOperation.LazyMultiPhraseQuery();
      ...
      // if we have no tokens, that is usually because the analyzer removed them. The user probably
      // queried for
      ...
      LuceneQueryOperation op = new LuceneQueryOperation();
      op.addClause(
          toQueryString(field, text),
          new TermQuery(new Term(field, tokens.get(0))),
          evalBool(bool));
      return op;
    } else {
