...
public enum SortBy {
  ...
  TASK_PERCENT_COMPLETE_ASC(
      "taskPercCompletedAsc", Key.DATE, Direction.ASC, ZimbraSortBy.taskPercCompletedAsc),
  TASK_PERCENT_COMPLETE_DESC(
      "taskPercCompletedDesc", Key.DATE, Direction.DESC, ZimbraSortBy.taskPercCompletedDesc);
  ...
  private static final class NameComparator implements Comparator<ZimbraHit> {
    ...
    NameComparator(SortBy sort, Locale locale) {
      ...
      try {
        int localDecomposition =
            Provisioning.getInstance().getLocalServer().getContactSearchDecomposition();
        collator.setDecomposition(localDecomposition);
      } catch (IllegalArgumentException e) {
        collator.setDecomposition(Collator.FULL_DECOMPOSITION);
        ZimbraLog.index.info(
            "The given value is not a valid decomposition mode.  Set default value (%d)",
            Collator.FULL_DECOMPOSITION);
      } catch (ServiceException e) {
        collator.setDecomposition(Collator.FULL_DECOMPOSITION);
        ZimbraLog.index.info(
            "Failed to get a valid decomposition mode.  Set default value (%d)",
            Collator.FULL_DECOMPOSITION);
      }
    }
     ...
     * an extended comparing rule so that the a set of ascii symbols can be treated as same manner
     * as that of 3-byte symbols defined in the higher code point.
     *
     * @return Collator
     */
    private Collator getJapaneseNameRuleBaseCollator() {
      ...
      if (collator instanceof RuleBasedCollator) {
        ...
        ZimbraLog.index.debug(
            "Unexpected Collator for Japanese locale. Use the rule of [%s]",
            collator.getClass().getName());
        return collator;
      }
