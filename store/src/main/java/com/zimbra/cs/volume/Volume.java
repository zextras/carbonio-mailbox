...
/**
 * Based on default settings, there are 256 directories. We write blobs for id's 0-4095 to directory
 * 0, 4096-8191 to directory 1, etc. After filling directory 255, we wrap around and write
 * 1048576-1052671 to directory 0 and so on.
 *
 * <p>Number of dirs: 2 ^ groupBits (2 ^ 8 = 256 by default) Number of files per dir before
 * wraparound: 2 ^ bits (2 ^ 12 = 4096 by default)
 */
public final class Volume {
  ...
  public static final class Builder {
    ...
    public Builder setPath(String path, boolean normalize)
        throws VolumeServiceException, ServiceException {
      if (normalize) {
  ...
  public static String getAbsolutePath(String path) {
    return LC.zimbra_relative_volume_path.booleanValue()
        ? LC.zimbra_home.value() + File.separator + path
        : path;
  }
   ...
   *
   * <p>If there are too many ".." in the path, navigating to parent directory stops at root.
   *
   * <p>Supports UNIX absolute path, Windows absolute path with or without drive letter, and windows
   * UNC share.
   *
