...
public final class VolumeManager {
      ...
      if (current.secondaryMsgVolId != Volume.ID_NONE) {
        ...
        if (currentSecondaryMessageVolume == null) {
          ZimbraLog.store.warn(
              "Unknown current secondary message volume id=%d", current.secondaryMsgVolId);
        }
      }
  ...
  public Volume update(Volume update, boolean noRedo) throws ServiceException {
    // Don't allow changing type of a current volume. The volume must be first made non-current
    // before its type can
    // be changed. A volume can be made non-current when another volume is made current for the
    // volume type.
    ...
    synchronized (this) {
      ...
      if (vol.getType() == Volume.TYPE_MESSAGE || vol.getType() == Volume.TYPE_MESSAGE_SECONDARY) {
        ...
        if (files != null && files.length > 0) {
          // have to check DB to see if any of these files are referenced by mail_item; no FK
          // anymore
          DbConnection conn = DbPool.getConnection();
          try {
   ...
   *
   * <p>Don't cache the returned {@link Volume} object. Updates to volume information by admin will
   * create a different {@link Volume} object for the same volume ID.
   */
  ...
  public synchronized boolean isCurrent(Volume vol) {
    return currentMessageVolume == vol
  ...
  void validatePath(String path) throws ServiceException {
    ...
    // Make sure path is not a subdirectory of an existing volume. Note this check doesn't work on
    // Windows when path
