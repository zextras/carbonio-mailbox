...
public class Diff {
  ...
  private static void addString(ArrayList<Chunk> chunks, String str, Type disp) {
    ...
    if (c.disposition == disp) c.content.add(str);
    else chunks.add(new Chunk(str, disp));
  }
  ...
  public static Collection<Chunk> getResult(InputStream i1, InputStream i2) throws IOException {
    ...
    for (int i = M - 1; i >= 0; i--) {
      for (int j = N - 1; j >= 0; j--) {
        if (x[i].equals(y[j])) opt[i][j] = opt[i + 1][j + 1] + 1;
    ...
    for (Chunk c : chunks) {
      switch (c.disposition) {
        case common:
          continue;
        case first:
          for (String str : c.content) System.out.println("< " + str);
          break;
        case second:
          for (String str : c.content) System.out.println("> " + str);
          break;
