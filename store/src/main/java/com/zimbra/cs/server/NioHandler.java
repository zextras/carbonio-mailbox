...
/**
 * Protocol handler for MINA-based server connections. The protocol handler defines the action to
 * take whenever a new connection is opened, is closed, becomes idle, or a new request is received
 * on the connection.
 */
public interface NioHandler {
  ...
  /**
   * Called when any exception is thrown by user IoHandler implementation or by MINA. If cause is
   * instanceof IOException, MINA will close the connection automatically.
   *
   * @param e
   * @throws IOException
   */
  ...
  /**
   * Drop the connection and wait up to writeTimeout in the session config for last write to
