...
public abstract class TcpServer implements Runnable, Server {
  ...
  private void init(int maxThreads, ServerSocket serverSocket) {
    ...
    pooledExecutor =
        new ThreadPoolExecutor(
            ...
            new TcpThreadFactory(getName(), false, Thread.NORM_PRIORITY));
  ...
  public void stop(int forceShutdownAfterSeconds) {
    ...
    log.info(
        getName() + " waiting " + forceShutdownAfterSeconds + " seconds for thread pool shutdown");
    try {
  ...
  public void run() {
    ...
    log.info(
        "Starting accept loop: %d core threads, %d max threads.",
  ...
  private void warnIfNecessary() {
    if (log.isWarnEnabled()) {
      ...
      if (utilization >= warnPercent) {
        log.warn(
            "Thread pool is %d%% utilized.  %d out of %d threads in use.",
