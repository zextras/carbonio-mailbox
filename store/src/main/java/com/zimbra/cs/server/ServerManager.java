...
public final class ServerManager {
  ...
  private Pop3Server startPop3Server(boolean ssl) throws ServiceException {
    ...
    Pop3Server server =
        NIO_ENABLED || LC.nio_pop3_enabled.booleanValue()
            ? new NioPop3Server(config)
            : new TcpPop3Server(config);
    server.start();
    return server;
  }
  ...
  private ImapServer startImapServer(boolean ssl) throws ServiceException {
    ...
    ImapServer server =
        NIO_ENABLED || LC.nio_imap_enabled.booleanValue()
            ? new NioImapServer(config)
            : new TcpImapServer(config);
    server.start();
    return server;
  }
