...
/**
 * Protocol-agnostic abstract base class for handling each client connection to TcpServer. Each
 * derived class of this class implements a specific protocol, for example LMTP.
 */
public abstract class ProtocolHandler implements Runnable {
   ...
   *
   * @return true if expecting more commands, false if QUIT command was received and server
  ...
  /**
   * Closes any input/output streams with the client. May get called multiple times, even after
   * connection was refused.
   */
  protected abstract void dropConnection();

  /**
   * Called when a connection has been idle for too long. Sends protocol-specific message to client
   * notifying it that the connection is being dropped due to idle timeout.
   */
  ...
  public void run() {
    ...
    Preconditions.checkState(
        connection != null, "Connection can not be null when running ProtocolHandler");
    ...
    try {
      connection.setSoTimeout(
          server.getConfigMaxIdleMilliSeconds()); // must be set before any traffic
