...
/**
 * Base class for MINA-based IMAP/POP3/LMTP servers. Handles creation of new NIO request and
 * connection handler instances.
 */
public abstract class NioServer implements Server {
  private static final Multimap<Class<? extends NioServer>, IoFilter> FILTERS =
  ...
  /**
   * Extensions may add a custom {@link IoFilter} to the filter chain. Must call before the server
   * starts.
   *
  ...
  /**
   * Our cipher config attribute zimbraSSLExcludeCipherSuites specifies a list of ciphers that
   * should be disabled instead of enabled. This is because we want the same attribute to control
   * all SSL protocols running on mailbox servers. For https, Jetty configuration only supports an
   * excluded list. Therefore we adapted the same scheme for zimbraSSLExcludeCipherSuites, which is
   * written to jetty.xml by config rewrite, and will be used for protocols (imaps/pop3s) handled by
   * Zimbra code.
   *
   * <p>For MINA based servers/handlers, NioTcpServer uses SSLFilter for SSL communication.
   * SSLFilter wraps an SSLEngine that actually does all the work.
   * SSLFilter.setEnabledCipherSuites() sets the list of cipher suites to be enabled when the
   * underlying SSLEngine is initialized. Since we only have an excluded list, we need to exclude
   * those from the ciphers suites which are currently enabled for use on a engine.
   *
   * <p>Since we do not directly interact with a SSLEngine while sessions are handled, and there is
   * no SSLFilter API to alter the SSLEngine it wraps, we workaround this by doing the following: -
   * create a dummy SSLEngine from the same SSLContext instance that will be used for all SSL
   * communication. - get the enabled ciphers from SSLEngine.getEnabledCipherSuites() - exclude the
   * ciphers we need to exclude from the enabled ciphers, so we now have a net enabled ciphers list.
   ...
   * @return array of enabled ciphers, or empty array (note, not null) if all default ciphers should
   *     be enabled. (i.e. we do not need to alter the default ciphers)
   */
  private static synchronized String[] getSSLEnabledCiphers(
      SSLContext sslCtxt, ServerConfig serverConfig) {
    if (mSslEnabledCipherSuites == null) {
      try {
        ...
        if (excludeCiphers != null && excludeCiphers.length > 0) {
          ...
          mSslEnabledCipherSuites =
              NetUtil.computeEnabledCipherSuites(enabledCiphers, excludeCiphers);
        }
         ...
         */
        if (mSslEnabledCipherSuites == null) mSslEnabledCipherSuites = new String[0];
  ...
  public ZimbraSslFilter newSSLFilter() {
    ...
    if (enabledCiphers.length > 0) sslFilter.setEnabledCipherSuites(enabledCiphers);
    return sslFilter;
  }
  ...
  /**
   * Starts the server. Binds the server port and starts the connection handler. Optionally adds an
   * SSLFilter if ssl is enabled.
   *
  ...
  /**
   * Shuts down the server. Waits up to 'graceSecs' seconds for the server to stop, otherwise the
   * server is forced to shut down.
   *
  ...
  public void stop(int timeout) {
    ...
    // Would prefer to unbind first then cleanly close active connections, but mina unbind seems to
    // automatically
    // close the active sessions so we must close connections then unbind, which does expose us to a
    // potential race
    // condition.
    closeSessions();
    acceptor.unbind();
    ...
    try {
      executor.awaitTermination(
          timeout, TimeUnit.SECONDS); // Wait for handlers to cleanly terminate
  ...
  protected void registerMBean(String type) {
    ...
    try {
      mbs.registerMBean(
          new NioServerStats(this), new ObjectName("ZimbraCollaborationSuite:type=" + type));
    } catch (Exception e) {
  ...
  protected int getNumThreads() {
    ...
    if (!(ex instanceof ThreadPoolExecutor)) {
      ZimbraLog.perf.debug(
          "Unexpected Executor type %s.  NioServer.getNumThreads() returning 0.",
          ex.getClass().getName());
      return 0;
    }
