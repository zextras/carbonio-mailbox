 ...
 *
 * <p>Format : Number followed by a unit designation. Unit designation: B - bytes KB - kilobytes MB
 * - megabytes GB - gigabytes TB - terabytes
 *
 * <p>Uses a 1024 bytes per kilobyte graduation.
 */
public class MemoryUnitUtil {

  // Matches the format used by EHCache 2.5.1
  private static Pattern EHCACHE_251 =
      Pattern.compile("^(\\d+)(b?k?m?B?K?M?)$", Pattern.CASE_INSENSITIVE);

  public static Pattern MEMORYUNIT_PATTERN =
      Pattern.compile("^(\\d+)(B?[KB]?[MB]?[GB]?[TB]?)$", Pattern.CASE_INSENSITIVE);
