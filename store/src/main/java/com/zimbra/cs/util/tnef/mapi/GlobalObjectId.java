 ...
 *
 * <p>UID = EncodedGlobalId / ThirdPartyGlobalId EncodedGlobalId = Header GlobalIdData
 * ThirdPartyGlobalId = 1*UTF8-octets ; Assuming UTF-8 is the encoding Header = ByteArrayID
 * InstanceDate CreationDateTime Padding DataSize ByteArrayID = "040000008200E00074C5B7101A82E008"
 * InstanceDate = InstanceYear InstanceMonth InstanceDay InstanceYear = 4*4HEXDIG ; UInt16
 * InstanceMonth = 2*2HEXDIG ; UInt8 InstanceDay = 2*2HEXDIG ; UInt8 CreationDateTime = FileTime
 * FileTime = 16*16HEXDIG ; UInt64 Padding = 16*16HEXDIG ; "0000000000000000" recommended DataSize =
 * 8*8HEXDIG ; UInt32 little-endian GlobalIdData = 2*HEXDIG ; [Should this be = * (2*HEXDIG)?]
 *
 * <p>Effectively, an "EncodedGlobalId" is a direct encoding of the PidLidGlobalObjectId property to
 * hex. The analog PidLidGlobalObjectId property of a "ThirdPartyGlobalId" contains the full
 * "ThirdPartyGlobalId" inside it.
 *
 * <p>Information from MS-OXCOCAL on PidLidGlobalObjectId structure : byte[16] ByteArrayID = 0x04,
 * 0x00, 0x00, 0x00, 0x82, 0x00, 0xE0, 0x00, 0x74, 0xC5, 0xB7, 0x10, 0x1A, 0x82, 0xE0, 0x08 uint16
 * InstanceYear (big endian!) uint8 InstanceMonth - between 1 and 12 uint8 InstanceDay FileTime
 * (uint64) CreationDateTime byte[8] Reserved - should be zeros uint32 Size (little endian)
 * byte[Size] Data
 */
