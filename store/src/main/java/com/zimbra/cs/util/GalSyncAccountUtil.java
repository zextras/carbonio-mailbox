...
public class GalSyncAccountUtil {
  ...
  private static void usage() {
    ...
    System.out.println(
        "\taddDataSource -a {account-name} -n {datasource-name} --domain {domain-name} -t"
            + " zimbra|ldap [-f {folder-name}] [-p {polling-interval}]");
    System.out.println("\tdeleteAccount [-a {account-name} | -i {account-id}]");
    System.out.println(
        "\ttrickleSync [-a {account-name} | -i {account-id}] [-d {datasource-id}] [-n"
            + " {datasource-name}]");
    System.out.println(
        "\tfullSync [-a {account-name} | -i {account-id}] [-d {datasource-id}] [-n"
            + " {datasource-name}]");
    System.out.println(
        "\tforceSync [-a {account-name} | -i {account-id}] [-d {datasource-id}] [-n"
            + " {datasource-name}]");
    System.exit(1);
  }
  ...
  private void syncGalAccount() throws ServiceException, IOException {
    ...
    try {
      ...
      if (mDataSourceId != null) ds.addAttribute(AdminConstants.A_BY, "id").setText(mDataSourceId);
      else ds.addAttribute(AdminConstants.A_BY, "name").setText(mDataSourceName);
      if (mFullSync) ds.addAttribute(AdminConstants.A_FULLSYNC, "TRUE");
      if (mForceSync) ds.addAttribute(AdminConstants.A_RESET, "TRUE");
  ...
  private Element createGalSyncAccount(
      String accountName,
      String dsName,
      String domain,
      String type,
    ...
    try {
      ...
      if (folder != null) req.addAttribute(AdminConstants.E_FOLDER, folder);
      ...
      if (pollingInterval != null)
        req.addElement(AdminConstants.E_A)
            .addAttribute(AdminConstants.A_N, Provisioning.A_zimbraDataSourcePollingInterval)
            .setText(pollingInterval);
  ...
  private Element addGalSyncDataSource(
      String accountName,
      String dsName,
      String domain,
      String type,
  ...
  private Element deleteGalSyncAccount(String name, String id)
      throws ServiceException, IOException {
    mTransport = null;
    try {
      mTransport = new SoapHttpTransport(mAdminURL);
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (cl.hasOption('n')) cli.setDataSourceName(cl.getOptionValue('n'));
    if (cl.hasOption('d')) cli.setDataSourceId(cl.getOptionValue('d'));
    setup();
      ...
      switch (cmd) {
        ...
        case CREATE_ACCOUNT:
          ...
          for (Element account :
            ...
            System.out.println(
                account.getAttribute(AdminConstants.A_NAME)
                    + "\t"
