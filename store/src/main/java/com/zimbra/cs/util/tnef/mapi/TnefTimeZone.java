...
public class TnefTimeZone {
  ...
  /**
   * Ref: [MS-OXOCAL] - v20100729 Returns a time zone from the given index; The default TimeZone ID
   * is "tnefTimeZone"
   *
   * @param index
   * @return
  ...
  public static TimeZone getTimeZone(int index, boolean observeDaylightSaving, String tzId)
    ...
    int standardBias =
        0; // offset in minutes from bias during standard time.; has a value of 0 in most cases
    ...
    if (utcPlus12offset > 12 * 60) bias = (utcPlus12offset - 12 * 60) * -1;
    else if (utcPlus12offset < 12 * 60) bias = (12 * 60 - utcPlus12offset);
    ...
    String timeZoneId;
    if (tzId == null || tzId.length() == 0) timeZoneId = DEFAULT_TNEF_TIMEZONE_ID;
    else timeZoneId = tzId;

    TimeZoneDefinition timeZoneDefinition =
        new TimeZoneDefinition(
            timeZoneId, bias, standardBias, daylightBias, StandardDate, DaylightDate);
  ...
  /**
   * [MS-OXOCAL] - v20100729 PidLidTimeZone Type: PtypInteger32 The lower WORD specifies an index
   * into a table that contains time zone information. From the upper WORD, only the highest bit is
   * read. If that bit is set, the time zone referenced will not observe daylight saving time;
