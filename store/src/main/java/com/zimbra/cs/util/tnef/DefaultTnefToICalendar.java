...
/**
 * @author gren
 *     <p>The <code>DefaultTnefToICalendar</code> class is responsible for mining a TNEF
 *     (WINMAIL.DAT) attachment (and, potentially, the MIME of the containing message) to derive an
 *     ICALENDAR equivalent of the scheduling information if applicable.
 */
public class DefaultTnefToICalendar implements TnefToICalendar {
  ...
  public boolean convert(MimeMessage mimeMsg, InputStream tnefInput, ContentHandler icalOutput)
    ...
    try {
      ...
      if (icalType == ICALENDAR_TYPE.VTODO) {
        ...
        if (schedView == null) {
          sLog.debug(
              "Unable to map class %s to ICALENDER - no properties found for sub-msg", msgClass);
          return false;
        }
      }
      ...
      if (uid == null) {
        sLog.debug(
            "Unable to map class %s to ICALENDER - no suitable value found for UID", msgClass);
        return false;
      }
      ...
      if ((attendeeCriticalChange != null)
          && (method.equals(Method.REPLY) || method.equals(Method.COUNTER))) {
      ...
      if (method.equals(Method.COUNTER)) {
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput,
            Property.DTSTART,
            schedView.getProposedStartTime(),
            startTimeTZinfo,
            isAllDayEvent);
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput,
            Property.DTEND,
            schedView.getProposedEndTime(),
            endTimeTZinfo,
            isAllDayEvent);
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput, "X-MS-OLK-ORIGINALSTART", icalStartDate, startTimeTZinfo, isAllDayEvent);
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput, "X-MS-OLK-ORIGINALEND", icalEndDate, endTimeTZinfo, isAllDayEvent);
      } else {
        if (this.icalType == ICALENDAR_TYPE.VTODO) {
          ...
          IcalUtil.addPropertyFromUtcTimeAndZone(
              icalOutput, Property.COMPLETED, icalDateTaskCompleted, null, false);
        } else {
          IcalUtil.addPropertyFromUtcTimeAndZone(
              icalOutput, Property.DTSTART, icalStartDate, startTimeTZinfo, isAllDayEvent);
          IcalUtil.addPropertyFromUtcTimeAndZone(
              icalOutput, Property.DTEND, icalEndDate, endTimeTZinfo, isAllDayEvent);
        }
      }
      ...
      if (this.icalType == ICALENDAR_TYPE.VEVENT) {
        IcalUtil.addProperty(
            icalOutput, "X-MICROSOFT-CDO-ALLDAYEVENT", isAllDayEvent ? "TRUE" : "FALSE");
        IcalUtil.addProperty(icalOutput, "X-MICROSOFT-CDO-BUSYSTATUS", busyStatus, false);
        if (method.equals(Method.REQUEST)) {
          IcalUtil.addProperty(
              icalOutput, "X-MICROSOFT-CDO-INTENDEDSTATUS", intendedBusyStatus, false);
        }
        IcalUtil.addProperty(icalOutput, "X-MICROSOFT-CDO-OWNERAPPTID", ownerApptId, false);
        IcalUtil.addProperty(
            icalOutput, "X-MICROSOFT-CDO-REPLYTIME", schedView.getAppointmentReplyTime(), false);
        IcalUtil.addProperty(
            icalOutput, "X-MICROSOFT-CDO-OWNER-CRITICAL-CHANGE", ownerCriticalChange, false);
        Boolean disallowCounter = schedView.isDisallowCounter();
        if (disallowCounter != null) {
          IcalUtil.addProperty(
              icalOutput, "X-MICROSOFT-CDO-DISALLOW-COUNTER", disallowCounter ? "TRUE" : "FALSE");
        }
      }
      if (reminderSet) {
  ...
  private void addRecurrenceRelatedProps(
      ContentHandler icalOutput,
    ...
    if (recurDef != null) {
      ...
      for (DateTime exDate : recurDef.getExdates()) {
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput, Property.EXDATE, exDate, tzDef, isAllDayEvent);
      }
      for (DateTime rDate : recurDef.getRdates()) {
        IcalUtil.addPropertyFromUtcTimeAndZone(
            icalOutput, Property.RDATE, rDate, tzDef, isAllDayEvent);
      }
    }
  }
  ...
  private void addExceptions(
    ...
    for (ChangedInstanceInfo cInst : recurDef.getChangedInstances()) {
      ...
      IcalUtil.addPropertyFromUtcTimeAndZone(
          icalOutput, Property.DTSTART, cInst.getStartDate(), tzDef, exceptIsAllDayEvent);
      IcalUtil.addPropertyFromUtcTimeAndZone(
          icalOutput, Property.DTEND, cInst.getEndDate(), tzDef, exceptIsAllDayEvent);
      IcalUtil.addProperty(icalOutput, Property.UID, uid);
      IcalUtil.addPropertyFromUtcTimeAndZone(
          icalOutput, Property.RECURRENCE_ID, cInst.getOriginalStartDate(), tzDef, seriesIsAllDay);
      IcalUtil.addProperty(icalOutput, dtstamp);
      ...
      IcalUtil.addProperty(icalOutput, "X-MICROSOFT-CDO-OWNERAPPTID", ownerApptId, false);
      IcalUtil.addProperty(
          icalOutput, "X-MICROSOFT-CDO-ALLDAYEVENT", exceptIsAllDayEvent ? "TRUE" : "FALSE");
  ...
  private void addAttendees(
      ContentHandler icalOutput, MimeMessage mimeMsg, PartStat partstat, boolean replyWanted)
    ...
    if (method.equals(Method.REPLY) || method.equals(Method.COUNTER)) {
      ...
      if (toRecips != null) {
        for (Address a : toRecips) {
          ...
          addAttendee(
              icalOutput, ia, Role.REQ_PARTICIPANT, CuType.INDIVIDUAL, partstat, replyWanted);
        }
      }
      if (ccRecips != null) {
        for (Address a : ccRecips) {
          ...
          addAttendee(
              icalOutput, ia, Role.OPT_PARTICIPANT, CuType.INDIVIDUAL, partstat, replyWanted);
        }
      }
      if (bccRecips != null) {
  ...
  private void addStatusProperty(
      ContentHandler icalOutput, BusyStatus busyStatus, EnumSet<MeetingTypeFlag> meetingTypeFlags)
      throws ParserException, URISyntaxException, IOException, ParseException {
    ...
    if (method.equals(Method.REQUEST)) {
      ...
      if ((busyStatus == null) || (busyStatus.equals(BusyStatus.FREE))) {
        if ((meetingTypeFlags != null) && (meetingTypeFlags.contains(MeetingTypeFlag.MTG_INFO))) {
