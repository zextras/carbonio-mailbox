...
public final class JMSession {
  ...
  static {
    ...
    try {
      Security.addProvider(
          new OAuth2Provider(Provisioning.getInstance().getLocalServer().getServerVersionMajor()));
  ...
  /** Returns the shared JavaMail {@link Session} that has the latest SMTP settings from LDAP. */
  public static Session getSession() {
    return sSession;
  }

  /**
   * Returns a new JavaMail {@link Session} that has the latest SMTP settings from the local server.
   */
  ...
  /**
   * Returns the JavaMail SMTP {@link Session} with settings from the given account and its domain.
   */
  ...
  /**
   * Returns a new JavaMail {@link Session} that has the latest SMTP settings from LDAP. Settings
   * are retrieved from the local server and overridden by the domain.
   *
  ...
  /** Returns a new JavaMail {@link Session} that is configured to connect to relay MTA. */
  public static Session getRelaySession() throws MessagingException {
    ...
    try {
      ...
      useTls =
          server.getShareNotificationMtaConnectionType()
              == ShareNotificationMtaConnectionType.STARTTLS;
    } catch (ServiceException e) {
    ...
    if (useSmtpAuth) {
      ...
      if (account == null || password == null) {
        ZimbraLog.smtp.warn(
            Provisioning.A_zimbraShareNotificationMtaAuthRequired
                + " is enabled but account or password is unset");
      } else {
    ...
    Session session =
        (auth == null) ? Session.getInstance(props) : Session.getInstance(props, auth);
    setProviders(session);
    if (LC.javamail_smtp_debug.booleanValue()) {
      session.setDebug(true);
    }
  ...
  private static Properties getJavaMailSessionProperties(Server server, Domain domain)
    ...
    Boolean sendPartial =
        Boolean.parseBoolean(getValue(server, domain, Provisioning.A_zimbraSmtpSendPartial));
  ...
  public static void addOAuth2Properties(
      String oauthToken, Map<String, String> map, String protocol) {
  ...
  /**
   * Caches the set of SMTP hosts that we've failed to connect to. Only the key is used. The value
   * is ignored.
   */
  private static Map<String, Object> sBadSmtpHosts =
      Collections.synchronizedMap(
          new TimeoutMap<String, Object>(LC.smtp_host_retry_millis.intValue()));
  ...
  /**
   * Returns a random value specified for <tt>zimbraSmtpHostname</tt> on the server or domain, or
   * <tt>null</tt> if the host name cannot be determined.
   *
  ...
  /**
   * Returns a new set that contains all SMTP hosts, not including hosts that were marked as bad
   * with {@link #markSmtpHostBad}.
   */
  ...
  /**
   * Mark the given SMTP host as bad. We will not attempt to connect to this host for the interval
   * specified by {@link LC#smtp_host_retry_millis}.
   *
  ...
  public static void markSmtpHostBad(String hostName) {
    ...
    ZimbraLog.smtp.info(
        "Disallowing connections to %s for %d milliseconds.",
        hostName, LC.smtp_host_retry_millis.intValue());
  ...
  /**
   * Returns the value of <tt>zimbraSmtpHostname</tt>. If the value is not set on the domain,
   * returns the value for the local server.
   *
  ...
  private static String[] lookupSmtpHosts(Domain domain) throws ServiceException {
    ...
    if (hosts.length > 0) {
      if (ZimbraLog.smtp.isDebugEnabled()) {
        ZimbraLog.smtp.debug(
            "lookupSmtpHosts domain=%s has %s SMTP hostnames configured - %s",
    ...
    if (ZimbraLog.smtp.isDebugEnabled()) {
      ZimbraLog.smtp.debug(
          "lookupSmtpHosts domain=%s has %s SMTP hostnames configured - %s. [via server %s]",
