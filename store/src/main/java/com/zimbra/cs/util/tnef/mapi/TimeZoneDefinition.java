...
/**
 * From MS-OXCICAL this is used for PidLidAppointmentTimeZoneDefinitionRecur Specifies time zone
 * information that describes how to convert the meeting date and time on a recurring series to and
 * from UTC. If this property is set, but it has data that is inconsistent with the data that is
 * represented by PidLidTimeZoneStruct, then use PidLidTimeZoneStruct instead of this property.
 *
 * <p>PidLidAppointmentTimeZoneDefinitionStartDisplay Specifies time zone information that indicates
 * the time zone of the PidLidAppointmentStartWhole property. The value of this property is used to
 * convert the start date and time from UTC to this time zone for display purposes. The fields in
 * this BLOB are encoded exactly as specified for PidLidAppointmentTimeZoneDefinitionRecur with one
 * exception. For each TZRule specified by this property, the R flag in the TZRule flags field is
 * not set (for example, if the TZRule is the effective rule, the value of the field TZRule flags
 * MUST be 0x0002; otherwise, it MUST be 0x0000). PidLidAppointmentTimeZoneDefinitionEndDisplay
 * Specifies time zone information that indicates the time zone of the PidLidAppointmentEndWhole
 * property. The format, constraints, and computation of this property are the same as specified in
 * the PidLidAppointmentTimeZoneDefinitionStartDisplay property.
 *
 * <p>MS-OXOCAL Timezone structure definition byte MajorVersion; // 0x02 byte MinorVersion; // 0x01
 * int16_t cbHeader; // ByteCount for Reserved .. cRules int16_t Reserved; // 0x0002 int16_t
 * cchKeyName; // count of Unicode chars in KeyName int16_t KeyName[cchKeyName]; // Timezone name
 * int16_t cRules; // count of TZRules 1 <= cRules <= 1024 TZRules[cRules]; //
 */
public class TimeZoneDefinition {
  ...
  /**
   * Initialise TimeZoneDefinition object from one of the MAPI TimeZoneDefinition properties (as
   * opposed to the simpler TimeZoneStruct property)
   *
   * @param mpi is one of the known ones associated with TimeZoneDefinition - see above
   * @param ris
   * @throws IOException
   */
  public TimeZoneDefinition(MapiPropertyId mpi, RawInputStream ris) throws IOException {
    theZone = null;
    this.mpi = mpi;
  ...
  /**
   * Initialise TimeZoneDefinition object from MAPI PidLidTimeZoneStruct property. This property is
   * set on a recurring series to specify time zone information. This property specifies how to
   * convert time fields between local time and UTC.
   *
   * @param ris
   * @param tzName
   * @throws IOException
   ...
   * @param standardBias offset in minutes from bias during standard time.; has a value of 0 in most
   *     cases
  ...
  public void addVtimezone(ContentHandler icalOutput)
