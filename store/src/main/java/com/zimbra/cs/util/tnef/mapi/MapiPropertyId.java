...
public class MapiPropertyId {
  ...
  public static final MapiPropertyId PidTagImportance = new MapiPropertyId(MAPIProp.PR_IMPORTANCE);
  ...
  public static final MapiPropertyId PidTagEndDate = new MapiPropertyId(MAPIProp.PR_END_DATE);
  public static final MapiPropertyId PidTagStartDate = new MapiPropertyId(MAPIProp.PR_START_DATE);
  ...
  public static final MapiPropertyId PidNameCalendarUid =
      new MapiPropertyId(MSGUID.PS_PUBLIC_STRINGS, "urn:schemas:calendar:uid");
  ...
  public static final MapiPropertyId PidLidAppointmentTimeZoneDefinitionStartDisplay =
  ...
  public static final MapiPropertyId PidLidAppointmentTimeZoneDefinitionEndDisplay =
  ...
  public static final MapiPropertyId PidLidAppointmentTimeZoneDefinitionRecur =
  ...
  // PidLidTaskAcceptanceState 0x812a - not assigned(0)/unknown(1)/assignee accepted(2)/assignee
  // rejected(3)
  ...
  public Boolean getBooleanValue(SchedulingViewOfTnef schedView, boolean defaultValue)
      throws IOException {
  ...
  /**
   * Return the Integer value corresponding to a MAPI Property with value PT_INT or PT_ERROR
   *
   * @param schedView represents a TNEF which might contain a property with this object's ID.
   * @param defaultVal
   * @return
   * @throws IOException
   */
  public Integer getIntegerValue(SchedulingViewOfTnef schedView, int defaultVal)
      throws IOException {
  ...
  /**
   * Return the Double value corresponding to a MAPI Property with value PT_DOUBLE
   *
   * @param schedView represents a TNEF which might contain a property with this object's ID.
   * @return
   * @throws IOException
  ...
  /**
   * PT_SYSTIME properties wrap FILETIME which is a count of seconds since start of 1601 Note that
   * some PT_SYSTIME properties are actually stored such that if they are treated as UTC times, the
   * day/month/year/hour/min etc components are correct in localtime. Calling this method for such
   * properties will result in a DateTime which doesn't reflect the underlying real date and time on
   * its own - but this can still be useful.
   *
   * @param schedView
   * @return DateTime object equivalent to a PT_SYSTIME property where that property represents a
   *     time in UTC
   * @throws IOException
   */
