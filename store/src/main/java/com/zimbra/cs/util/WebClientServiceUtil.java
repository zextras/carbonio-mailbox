...
public class WebClientServiceUtil {
   ...
   *
   * @param serviceUrl the url that should be matched and handled by ServiceServlet in
   *     ZimbraWebClient reqHeaders the map of req/rsp attributes that need to be set by the UI node
   * @return response from ui node in String
   * @throws ServiceException
   */
  public static String sendServiceRequestToOneRandomUiNode(
      String serviceUrl, Map<String, String> reqHeaders) throws ServiceException {
    List<Server> servers = Provisioning.getInstance().getAllServers(Provisioning.SERVICE_WEBCLIENT);
    if (servers == null || servers.isEmpty()) {
      servers.add(Provisioning.getInstance().getLocalServer());
    }
      ...
      if (authToken != null && authToken.isRegistered()) {
        try {
          ...
          ZimbraLog.misc.debug(
              "de-registered auth token, isRegistered?%s", authToken.isRegistered());
        } catch (AuthTokenException e) {
          ZimbraLog.misc.warn("failed to de-register authToken", e);
        }
      }
  ...
  public static void sendDeployZimletRequestToUiNode(
      Server server, String zimlet, String authToken, byte[] data) throws ServiceException {
    if (isServerAtLeast8dot5(server)) {
      HttpClientBuilder clientBuilder =
          ZimbraHttpConnectionManager.getExternalHttpConnMgr().newHttpClient();
      HttpProxyUtil.configureProxy(clientBuilder);
      HttpPost method =
          new HttpPost(URLUtil.getServiceURL(server, "/fromservice/deployzimlet", false));
      method.addHeader(ZimletUtil.PARAM_ZIMLET, zimlet);
      ZimbraLog.zimlet.info(
          "connecting to ui node %s, data size %d", server.getName(), data.length);
  ...
  public static void sendUndeployZimletRequestToUiNode(
      Server server, String zimlet, String authToken) throws ServiceException {
    if (isServerAtLeast8dot5(server)) {
      HttpClientBuilder clientBuilder =
          ZimbraHttpConnectionManager.getExternalHttpConnMgr().newHttpClient();
      HttpProxyUtil.configureProxy(clientBuilder);
      HttpPost method =
          new HttpPost(URLUtil.getServiceURL(server, "/fromservice/undeployzimlet", false));
      method.addHeader(ZimletUtil.PARAM_ZIMLET, zimlet);
      postToUiNode(server, method, authToken);
    }
  }
