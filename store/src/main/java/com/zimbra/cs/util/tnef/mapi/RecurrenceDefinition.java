...
/**
 * From MS-OXCOCAL this is used for : PidLidAppointmentRecur Includes information related to
 * recurrence patterns, including summary details for modified and deleted instances. The structure
 * is an AppointmentRecurrencePattern
 */
public class RecurrenceDefinition {
  ...
  public RecurrenceDefinition(RawInputStream ris, TimeZoneDefinition tz, String oemCP)
    ...
    try {
      ...
      // This field is the interval at which the meeting pattern specified in PatternTypeSpecific
      // field
      // repeats. The Period value MUST be between 0 (zero) and the MaximumRecurrenceInterval, which
      // is
  ...
  /**
   * @return appropriate value for the ICALENDAR X-MICROSOFT-CALSCALE property or "" if not
   *     required.
   */
  ...
  public List<DateTime> getExdates() {
    ...
    for (long delSince1601 : delMidnightMinsSince1601) {
      ...
      if (canceledInstance) {
        exdateTimes.add(IcalUtil.localMinsSince1601toDate(minsSince1601, tzDef));
      }
    }
    return exdateTimes;
  }
  ...
  /**
   * Note that according to MS-OXCICAL, recurrences only support RDATEs if there is a corresponding
   * EXDATE for an item in the series.
   *
   ...
   * @param isAllDay
   * @param isFloating
   * @return The main recurrence property as ICAL - typically an RRULE but could theoretically be an
   *     X-MICROSOFT-RRULE
  ...
  private void readRecurrenceFrequency(RawInputStream ris)
      throws IOException, TNEFtoIcalendarServiceException {
    ...
    throw TNEFtoIcalendarServiceException.RECURDEF_BAD_FREQ(
        "0x" + Integer.toHexString(mapiFrequency));
  }

  private void readPatternType(RawInputStream ris)
      throws IOException, TNEFtoIcalendarServiceException {
    ...
    throw TNEFtoIcalendarServiceException.RECURDEF_BAD_PATTERN(
        "0x" + Integer.toHexString(pattType));
  }

  private void readMsCalScale(RawInputStream ris)
      throws IOException, TNEFtoIcalendarServiceException {
    ...
    throw TNEFtoIcalendarServiceException.RECURDEF_BAD_MSCALSCALE(
        "0x" + Integer.toHexString(calType));
  }
  ...
  private void readFirstDayOfWeek(RawInputStream ris)
      throws IOException, TNEFtoIcalendarServiceException {
  ...
  /**
   * Read FristDateTime value from the correct place in a stream representing the
   * PidLidAppointmentRecur property. The value depends on the RecurrenceFrequency value.
   *
   * @param ris
   * @throws IOException
   */
  ...
  public String toString() {
    ...
    if (readerVersion != 0x3004) {
      buf.append("    Unexpected ReaderVersion=").append(readerVersion).append("\n");
    }
    if (writerVersion != 0x3004) {
      buf.append("    Unexpected WriterVersion=").append(writerVersion).append("\n");
    }
    buf.append("    RecurFrequency=").append(getRecurrenceFrequency()).append("\n");
    buf.append("    PatternType=").append(getPatternType()).append("\n");
    buf.append("    MsCalScale=").append(getMsCalScale()).append("\n");
    buf.append("    FirstDateTime=").append(getFirstDateTime()).append("\n");
    buf.append("    Period=").append(period).append("\n");
    buf.append("    SlidingFlag=").append(slidingFlag).append("\n");
    if (haveDayMask) {
      dayOfWeekMask = getDayOfWeekMask();
      buf.append("    dayMask=0x")
          .append(Long.toHexString(dayMask))
          .append(" - ")
          .append(dayOfWeekMask)
          .append("\n");
    }
    if (haveDayOfMonth) {
      buf.append("    dayOfMonth=").append(dayOfMonth).append("\n");
    }
    if (haveWeekDayOccurNum) {
      buf.append("    weekDayOccurrenceNumber=").append(weekDayOccurrenceNumber).append("\n");
    }
    buf.append("    EndType=").append(getEndType()).append("\n");
    buf.append("    occurrenceCount=").append(occurrenceCount).append("\n");
    buf.append("    firstDayOfWeek=").append(getFirstDayOfWeek()).append("\n");
    buf.append("    DeletedInstanceCount=").append(deletedInstanceCount).append("\n");
    for (long since1601 : delMidnightMinsSince1601) {
      ...
      infoOnLocalTimeSince1601ValWithOffset(
          buf, "        DeletedInstance=", since1601, startTimeOffset, suffix);
    }
    buf.append("    ModifedInstanceCount=").append(modifiedInstanceCount).append("\n");
    for (long since1601 : modMidnightMinsSince1601) {
      infoOnLocalTimeSince1601ValWithOffset(
          buf, "        ModifiedInstance=", since1601, startTimeOffset, "");
    }
    infoOnLocalTimeSince1601Val(buf, "    StartDate=", startMinsSince1601);
    infoOnLocalTimeSince1601Val(buf, "    Last Instance's start=", endMinsSince1601);
    buf.append("    ReaderVersion2=0x").append(Long.toHexString(readerVersion2));
    if ((readerVersion2 != 0x3006) && (readerVersion2 != 0)) {
      buf.append(" [ODD expected 0x3006 for event or 0x0 for task]");
    }
    buf.append("\n");
    buf.append("    WriterVersion2=0x").append(Long.toHexString(writerVersion2));
  ...
  private StringBuffer infoOnLocalTimeSince1601Val(
      StringBuffer buf, String desc, long localTimeSince1601) {
    buf.append(desc);
  ...
  private StringBuffer infoOnLocalTimeSince1601ValWithOffset(
      StringBuffer buf, String desc, long localTimeSince1601, long startOffset, String suffix) {
