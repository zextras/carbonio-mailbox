...
public class TestMain {
  ...
  private static boolean doConversion(
      MimeMessage mm, Writer out, TnefToICalendar converter, File tnefFile, boolean debug)
    ...
    if (tnefPart == null) throw ServiceException.FAILURE("No TNEF part in the message!", null);
    ...
    if (converter == null) throw ServiceException.FAILURE("No converter!", null);
  ...
  private static class TNEFPartFinder extends MimeVisitor {
    ...
    protected boolean visitBodyPart(MimeBodyPart bp) throws MessagingException {
      if (mTNEFPart == null && matchingType(bp)) mTNEFPart = bp;
      return false;
    }

    @Override
    protected boolean visitMessage(MimeMessage mm, VisitPhase visitKind) throws MessagingException {
      if (mTNEFPart == null && matchingType(mm)) mTNEFPart = mm;
      return false;
    }
  ...
  private static void usage() {
    ...
    System.err.println(
        "            -i <MIME file> [-o <iCalendar file>] [-t <tnef file>] [-r <recurInfo file>]");
    ...
    System.err.println(
        "-t: TNEF output file; The main TNEF attachment will be written here if present");
    System.err.println(
        "-r: A diagnostic string representing any recurrence property will be written here");
   ...
   */
  private static boolean processMimeFile(
      File mimeFile, File icalFile, File tnefFile, File recurInfoFile, boolean verbose) {
    if (!mimeFile.exists()) {
   ...
   * @return true if successfully written data.
   */
  private static boolean writeIcalendarData(
      File mimeFile, ByteArrayOutputStream icalBaos, File icalFile) {
    String ical = null;
    Writer wout = null;
    try {
      ical = icalBaos.toString(UTF8);
      if (icalFile != null) wout = new FileWriter(icalFile);
      else wout = new OutputStreamWriter(System.out, UTF8);
      wout.write(ical);
      wout.flush();
  ...
  private static class TestContentHandler implements ContentHandler {
    ...
    public TestContentHandler(boolean debug) {
      mDebug = debug;
    }
    ...
    private void printDebug(String format, Object... objects) {
      if (mDebug) {
        ...
        for (int i = 0; i < indentChars; ++i) System.out.print(' ');
    ...
    // Value should not have any encoding/escaping.  Email address should be prefixed with "mailto:"
    // or "MAILTO:".
    @Override
