...
/**
 * For command line interface utilities that are SOAP clients and need to authenticate with the admin service using
 * credentials from local configuration.
 * <p>
 * This class takes -h,--help for displaying usage, and -s,--server for target server hostname. Subclass can provide
 * additional options. The expected use is similar to the following:
 * <pre>
...
public abstract class SoapCLI {
  ...
  public static final Option OPT_AUTHTOKEN = new Option(O_AUTHTOKEN, "authtoken", true,
      "use auth token string (has to be in JSON format) from command line");
  public static final Option OPT_AUTHTOKENFILE = new Option(O_AUTHTOKENFILE, "authtokenfile", true,
      "read auth token (has to be in JSON format) from a file");
  ...
  /**
   * Returns the command usage. Since most CLI utilities are wrapped into shell script, the name of the script should be
   * returned.
   *
  ...
  /**
   * Encapsulate the notion of a session, including auth token, session ID, and whatever else is desired...
   */
