...
/**
 * For command line interface utilities that are SOAP clients and need to authenticate with the
 * admin service using credentials from local configuration.
 *
 * <p>This class takes -h,--help for displaying usage, and -s,--server for target server hostname.
...
public abstract class SoapCLI {
  ...
  public static final Option OPT_AUTHTOKEN =
      new Option(
          O_AUTHTOKEN,
          "authtoken",
          true,
          "use auth token string (has to be in JSON format) from command line");
  public static final Option OPT_AUTHTOKENFILE =
      new Option(
          O_AUTHTOKENFILE,
          "authtokenfile",
          true,
  ...
  protected CommandLine getCommandLine(String[] args) throws ParseException {
    ...
    if (!mDisableTargetServerOption && cl.hasOption(O_S)) setServer(cl.getOptionValue(O_S));
    return cl;
  }
