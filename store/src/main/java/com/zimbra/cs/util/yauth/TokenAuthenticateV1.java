...
/**
 * An implementation of the v1 Yahoo Token Auth API. This is mostly deprecated, but still used by
 * ymsgr among others
 *
 * <p>see generic_authapi.doc on backyard for details, but basically:
 *
 * <p>https://login.yahoo.com/config/pwtoken_get?src=ymsgr&login=USERNAMNE&passwd=PLAINTEXT_PASSWORD_URLESCAPED
 * get token (ymsgr=TOKEN in HTTP response body) from response
 *
 * <p>Token is good until password is changed (ie, store the token, don't store the passwd!)
 *
 * <p>https://login.yahoo.com/config/pwtoken_login?src=ymsgr&token=TOKEN get crumb,Y,T from HTTP
 * response body
 *
 * <p>Use crumb,Y,T to login.
 */
public class TokenAuthenticateV1 {
  ...
  public static String getToken(String username, String passwd) throws IOException, HttpException {
    HttpGet method =
        new HttpGet(
            "https://login.yahoo.com/config/pwtoken_get?src=ymsgr&login="
                + username
                + "&passwd="
                + passwd);
  ...
  /**
   * @param username
   * @param token THIS IS NOT THE PASSWORD -- use the static getToken() method to get the user's
   *     token
   * @return
   */
  public static TokenAuthenticateV1 doAuth(String username, String token)
    ...
    HttpGet method =
        new HttpGet("https://login.yahoo.com/config/pwtoken_login?src=ymsgr&token=" + token);
    HttpResponse response = HttpClientUtil.executeMethod(method);
