...
public class ZipUtil {
  ...
  /**
   * Traditional java.util.zip processing either assumes archives use UTF-8 for filenames or
   * requires that you know up front what charset is used for filenames. This class uses the more
   * versatile org.apache.commons.compress.archivers.zip package combined with language information
   * to make a best guess at what the filenames might be.
   */
   ...
   * @throws IOException
   */
  public static ZipNameAndSize getZipEntryNameAndSize(
      InputStream inputStream, Locale locale, int seqNo) throws IOException {
  ...
  private static String convertBytesIfPossible(byte[] rawBytes, Charset cset) {
    ...
    try {
      ...
      ZimbraLog.misc.trace(
          "ZipUtil failed decode from Charset='%s' %s", cset.name(), ex.getMessage());
    }
    return null;
  }
  ...
  private static String getNameFromUnicodeExtraPathIfPresent(ZipArchiveEntry zae) {
    ...
    if (origCRC32 == unicodePathExtraField.getNameCRC32()) {
      String val =
          convertBytesIfPossible(unicodePathExtraField.getUnicodeName(), StandardCharsets.UTF_8);
      if (null != val) {
