...
public class ProxyPurgeUtil {
   ...
   *
   * @param servers list of memcached servers supplied, if null the function gets all the memcached
  ...
  public static void purgeAccounts(
      List<String> servers, List<String> accounts, boolean purge, String outputformat)
      throws ServiceException {
    ...
    for (String a : accounts) {
      ...
      // 2. otherwise, if memcache_entry_allow_unqualified is true, then use the bare login as the
      // route key
      // 3. else, append the IP address of the proxy interface to the login and use that as the
      // route key
      ...
      if (account == null) {
        ...
        // this logic works for for all cases account=addr@<alias domain> or alias-name@<alias
        // domain>
        if (prov instanceof LdapProvisioning) {
          ZLdapContext ldpCtx = LdapClient.getContext(LdapServerType.MASTER, LdapUsage.GET_DOMAIN);
          try {
            List<String> aliasDomainIds =
                ((LdapProvisioning) prov).getEmptyAliasDomainIds(ldpCtx, d, false);
  ...
  static void usage() {
    ...
    System.err.println(
        " Available Memcached servers are discovered by the 'zmprov gamcs' function, others can be"
            + " specified");
    System.err.println(
        " If necessary, please specify additional memcached servers in the form of server:port at"
