...
public final class Config {
  ...
  public static synchronized int getInt(String name, int defaultValue) {
    ...
    try {
      ...
      ZimbraLog.misc.warn(
          "Invalid integer value '%s' for config key '%s'.  Returning default value %d.",
          value, name, intValue);
    }
    return intValue;
  }
  ...
  public static synchronized long getLong(String name, long defaultValue) {
    ...
    try {
      ...
      ZimbraLog.misc.warn(
          "Invalid long value '%s' for config key '%s'.  Returning default value %d.",
  ...
  /**
   * Returns a File object representing the path relative to the Zimbra home directory.
   *
   * @param path
   * @return
   */
  public static File getPathRelativeToZimbraHome(String path) {
    ...
    if (first == File.separatorChar || first == '/') return new File(path);
