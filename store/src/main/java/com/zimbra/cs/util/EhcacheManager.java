 ...
 *
 * <p>As of Ehcache 2.0, disk cache only configuration is no longer supported. But, {@code
 * maxElementsInMemory = 1} is virtually disk cache only. {@code maxElementsInMemory = 0} gives an
 * infinite capacity.
 *
 * @author ysasaki
 */
public final class EhcacheManager {
  ...
  private CacheConfiguration<String, ImapFolder> createImapActiveSessionCache() {
    ...
    try {
      maxBytesOnLocalDisk =
          Provisioning.getInstance().getLocalServer().getImapActiveSessionEhcacheMaxDiskSize();
    } catch (ServiceException e) {
      ZimbraLog.imap.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraImapActiveSessionEhcacheMaxDiskSize, e);
  ...
  private CacheConfiguration<String, ImapFolder> createImapInactiveSessionCache() {
    ...
    try {
      maxBytesOnLocalDisk =
          Provisioning.getInstance().getLocalServer().getImapInactiveSessionCacheMaxDiskSize();
    } catch (ServiceException e) {
      ZimbraLog.imap.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraImapInactiveSessionCacheMaxDiskSize, e);
      maxBytesOnLocalDisk = new MemoryUnitUtil().convertToBytes("100GB");
      ;
    }
    try {
      inactiveSessionCache =
          Provisioning.getInstance().getLocalServer().getImapInactiveSessionEhcacheSize();
    } catch (ServiceException e) {
      ZimbraLog.imap.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraImapInactiveSessionEhcacheSize, e);
  ...
  private CacheConfiguration<String, String> createActiveSyncStateItemCache() {
    ...
    try {
      ...
      ZimbraLog.sync.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraActiveSyncEhcacheHeapSize, e);
    ...
    try {
      ...
      ZimbraLog.sync.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraActiveSyncEhcacheExpiration, e);
    ...
    try {
      ...
      ZimbraLog.sync.error(
          "Exception while fetching attribute %s",
          Provisioning.A_zimbraActiveSyncEhcacheMaxDiskSize, e);
