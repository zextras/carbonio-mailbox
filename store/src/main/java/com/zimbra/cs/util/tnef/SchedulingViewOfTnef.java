...
/**
 * The <code>SchedulingViewOfTnef</code> class encapsulates the Scheduling related aspects of a TNEF
 * file.
 *
 * <p>A cut down version of net.freeutils.tnef.Message which ignores attributes and attachments
 * which are not regarded as important for Zimbra scheduling.
 *
 * @author Gren Elliot
 */
public class SchedulingViewOfTnef extends Message {
  ...
  /**
   * Constructs a SchedulingView of the attributes in the supplied TNEFInputStream.
   *
        ...
        case Attr.LVL_ATTACHMENT:
          switch (attr.getID()) {
            ...
            case Attr.attAttachTransportFilename:
              ...
              String filename =
                  TNEFUtils.removeTerminatingNulls(
  ...
  /**
   * Canonical name: PidTagSensitivity - Property ID: 0x0036 MS-OXICAL recommends ICAL Class <-->
   * PidTagSensitivity mapping : CLASS PidTagSensitivity PRIVATE 0x00000002 CONFIDENTIAL 0x00000003
   * PUBLIC 0x00000000 X-PERSONAL 0x00000001
   *
  ...
  /**
   * PidLidBusyStatus Specifies the availability of a user for the event described by the object.
   * MS-OXICAL recommends PidLidBusyStatus <--> ICAL TRANSP mapping : Mapping to TRANSP : 0x00000000
   * TRANSPARENT 0x00000001 OPAQUE 0x00000002 OPAQUE 0x00000003 OPAQUE
   *
  ...
  /**
   * PidLidIntendedBusyStatus - Contains the value of the PidLidBusyStatus property on the Meeting
   * object in the organizer's calendar at the time the Meeting Request object or Meeting Update
   * object was sent.
   *
   ...
   * (asfMeeting, 0x00000001): object is a Meeting object or a meeting related object. (asfReceived,
   * 0x00000002): object was received from someone else. (asfCanceled, 0x00000004): Meeting object
   * that is represented by the object has been cancelled.
   *
  ...
  public boolean getResponseRequested() throws IOException {
    Boolean responseRequested = MapiPropertyId.PidTagResponseRequested.getBooleanValue(this);
    if (responseRequested != null) {
      return responseRequested;
    }
    Boolean replyRequested = MapiPropertyId.PidTagReplyRequested.getBooleanValue(this);
  ...
  /**
   * PidLidAppointmentCounterProposal Indicates whether a Meeting Response object is a counter
   * proposal.
   *
  ...
  /**
   * PidLidAppointmentNotAllowPropose - Indicates whether attendees are not allowed to propose a new
  ...
  /**
   * PidLidOwnerCriticalChange - Specifies the date and time at which a Meeting Request object was
  ...
  /**
   * PidLidAppointmentReplyTime - Specifies the date and time at which the attendee responded to a
   * received meeting request or Meeting Update object.
   *
  ...
  /**
   * PidLidReminderDelta is the Time before the start of an appointment in minutes at which an alarm
   * should be raised. If PidLidReminderDelta is set to 0x5AE980E1, this is a synonym for 15 minutes
   * before the start.
   *
   * @return
   * @throws IOException
  ...
  public Integer getReminderDelta() throws IOException {
    Integer reminderDelta = MapiPropertyId.PidLidReminderDelta.getIntegerValue(this);
  ...
  /**
   * @return value of PidLidReminderSet property which specifies whether a reminder is set on the
   *     object.
   * @throws IOException
   */
  ...
  public DateTime getStartTime() throws IOException {
    ...
    if (this.getIcalType() == ICALENDAR_TYPE.VTODO) {
      ...
      if (taskStartDateNum == 0x5AE980E0) {
        sLog.debug(
            "PidLidTaskStartDate as num 0x%s [means NO start date]",
  ...
  public DateTime getEndTime() throws IOException {
    DateTime timeVal = MapiPropertyId.PidLidAppointmentEndWhole.getDateTimeAsUTC(this);
    if (timeVal == null) {
   ...
   *
   * <p>PidLidExceptionReplaceTime specifies UTC date and time within a recurrence pattern that an
   * exception will replace.
   *
   * @return A suitable time for the RECURRENCE-ID property for this calendaring item if one exists.
   * @throws IOException
   */
    ...
    if (recurrenceIdTime != null) {
      ...
      try {
        ...
        if (recurrenceTZinfo == null) {
          dateString =
              String.format(
                  "%04d%02d%02dT%02d%02d%02dZ", origYear, origMonth, origDay, hrs, mins, secs);
          recurrenceIdTime = new DateTime(dateString);
        } else {
          dateString =
              String.format(
                  "%04d%02d%02dT%02d%02d%02d", origYear, origMonth, origDay, hrs, mins, secs);
  ...
  /**
   * @return PidLidAppointmentProposedStartWhole which specifies the proposed value for
   *     PidLidAppointmentStartWhole for a counter proposal.
   * @throws IOException
   */
  ...
  /**
   * PidLidAppointmentProposedEndWhole - Specifies the proposed value for PidLidAppointmentEndWhole
  ...
  /**
   * PidLidAttendeeCriticalChange - Specifies the date and time at which the meeting-related object
   * was sent.
   *
  ...
  /**
   * PidLidTimeZoneDescription Specifies a human-readable description of the time zone that is
   * represented by the data in the PidLidTimeZoneStruct property.
   *
  ...
  public RecurrenceDefinition getRecurrenceDefinition(TimeZoneDefinition tzDef)
  ...
  /**
   * Useful for X-ALT-DESC? Would need to convert to HTML, which, even for wrapped HTML which isn't
   * necessarily common place, isn't 100% straight forward.
   *
   * @return
   * @throws IOException
   */
  ...
  /**
   * PidLidGlobalObjectId is The unique identifier of the Calendar object. After it is set for a
   * Calendar object, the value of this property MUST NOT change.
   *
  ...
  /**
   * PidLidCleanGlobalObjectId The format of this property is the same as that of
   * PidLidGlobalObjectId. The value of this property MUST be equal to the value of
   * PidLidGlobalObjectId, except the YH, YL, M, and D fields MUST all be zero. All objects that
   * refer to an instance of a recurring series (including an orphan instance), as well as the
   * recurring series itself, will have the same value for this property.
   *
  ...
  private GlobalObjectId getCleanGlobalObjectId() {
    ...
    cleanGlobalObjectId = this.getGlobalObjectIdType(MapiPropertyId.PidLidCleanGlobalObjectId);
    return cleanGlobalObjectId;
  }
  ...
  /**
   * For a Task, PidLidTaskGlobalId contains a unique key. As Outlook 2007 and earlier don't send
   * assigned tasks using ICAL in the same way as meeting requests can be sent and there doesn't
   * appear to be a way to save as ICAL, it isn't totally clear what is the best source of a UID.
   * This property is a GUID, so does NOT have the ability to encode third party UIDs in the same
   * way as GlobalObjectIds do;
   *
   * @return
   * @throws IOException
   */
  ...
  private void initTaskStatusInfo() throws IOException {
    ...
    dateTaskCompleted = MapiPropertyId.PidLidTaskDateCompleted.getDateTimeAsUTC(this);
    Integer intVal;
  ...
  /**
   * Initialise timezone related fields. Replies from Outlook 2007 related to a recurrence or an
   * instance of a recurrence had : PidLidTimeZoneDescription,PidLidTimeZoneStruct,
   * PidLidAppointmentTimeZoneDefinitionStartDisplay and
   * PidLidAppointmentTimeZoneDefinitionEndDisplay present but NOT
   * PidLidAppointmentTimeZoneDefinitionRecur further, the StartDisplay/EndDisplay properties were
   * appropriate to the Outlook client replying and were NOT related to the originally sent ICAL.
   *
   * <p>The Outlook originated TimeZone names associated with StartDisplay/EndDisplay/Recur props
   * seem "nicer" than the names used in PidLidTimeZoneDescription - so, tend to prefer those.
   *
   * @throws IOException
   */
  private void initTZinfo() {
        ...
        if (tzStructInfo != null) {
          ...
          if (recurrenceTZinfo == null) {
            if ((startTimeTZinfo != null)
                && (tzsRule.equivalentRule(startTimeTZinfo.getEffectiveRule()))) {
              ...
              sLog.debug("Using %s for TZ info", "PidLidAppointmentTimeZoneDefinitionStart");
                ...
                && (tzsRule.equivalentRule(endTimeTZinfo.getEffectiveRule()))) {
    ...
    if (recurrenceTZinfo != null) {
      ...
      } else if (recurrenceTZinfo
          .getEffectiveRule()
          .equivalentRule(endTimeTZinfo.getEffectiveRule())) {
        endTimeTZinfo = recurrenceTZinfo;
