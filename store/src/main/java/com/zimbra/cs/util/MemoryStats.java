...
/** A helper class for using java.lang.management APIs to monitor JVM statistics at runtime */
public final class MemoryStats implements RealtimeStatsCallback {
  ...
  public static String dumpGarbageCollectors() {
    ...
    for (GarbageCollectorMXBean gc : gcs) {
      ...
      sb.append(
          new Formatter()
              .format(
                  "GC: %s(%s)  count=%d  time=%d(%,dms per collection)\n",
                  gc.getName(),
                  gc.isValid() ? "VALID" : "INVALID",
  ...
  public static String dumpMemoryPools() {
    ...
    for (MemoryPoolMXBean pool : pools) {
      ...
      sb.append(
          new Formatter()
              .format(
                  "\t\"%s\" memory used: %,d  reserved: %,d  max: %,d",
                  pool.getName(), usage.getUsed(), usage.getCommitted(), usage.getMax()));
    ...
    sb.append(
        new Formatter()
            .format(
                "RuntimeTotal=%,d  RuntimeMax=%,d  RuntimeFree=%,d  TotUsed=%,d  TotReserved=%,d "
                    + " TotMax=%,d  CollectUsed=%,d\n",
   ...
   *     <p>For the default (serial) hotspot settings, this means: Copy - The copy collector, Eden
   *     and Minor GCs in Survivor MarkSweepCompact - Major GCs
   */
   ...
   *     <p>For HotSpot default settings, the memory pools are: Eden (young gen) -- new allocs go
   *     here. Managed by Copy Collector. Survivor Tenured -- long-lived objects. Managed by
   *     MarkSweepCompact Collector
   */
  ...
  /**
   * Register us with the ZimbraPerf object so that we can log memory stats periodically to the
   * stats file
   */
  ...
  private MemoryStats() {
    ...
    for (String gc : getGarbageCollectorNames()) {
      gc = gc.toLowerCase();
      ZimbraPerf.addRealtimeStatName(
          getGCCountColName(gc), "Number of times that " + gc + " GC was invoked");
    ...
    for (String pool : getHeapPoolNames()) {
      ...
      ZimbraPerf.addRealtimeStatName(
          getPoolUsedSizeColName(pool), "Number of bytes used in the " + pool + " memory pool");
      ZimbraPerf.addRealtimeStatName(
          getPoolFreeSizeColName(pool), "Number of bytes free in the " + pool + " memory pool");
    }
