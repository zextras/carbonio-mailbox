...
public class ContentRange {
  ...
  /**
   * Constructs Content-Range without start/end nor instance length set.
   *
   * @param instanceLength Instance length. Must be => 0.
   * @throws RangeException Invalid range
   */
   ...
   *
   * <p>return True if instance length set, false otherwise
   */
  ...
  /**
   * Returns instance length. Value undefined if the range was not set.
   *
   * <p>return Instance length
   */
  ...
  /**
   * Checks if the range is complete, i.e. starts at 0 and the length is equal to the instance
   * length
   *
  ...
  /**
   * Creates ContentRange from supplied string. The string needs to be in form of "bytes start '-'
   * end '/' instance-length" as per HTTP 1.1 spec. instance-length can be '*'.
   *
  ...
  private void setStartAndEnd(long start, long end) throws RangeException {
    ...
    if (start < 0 || end < start) throw new RangeException(HttpServletResponse.SC_BAD_REQUEST);
  ...
  /** Returns Content-Range as header's value (i.e. starting from bytes, etc). */
  @Override
