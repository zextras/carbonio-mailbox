...
public abstract class WikiPage {
  ...
  public static WikiPage findTemplate(WikiPage.WikiContext ctxt, String accountId, String template)
    ...
    if (domain != null) {
      ...
      if (domainWiki != null) page = WikiPage.findTemplatePage(ctxt, domainWiki, template);
    }

    if (page == null) {
      String defaultWiki = prov.getConfig().getAttr(Provisioning.A_zimbraNotebookAccount, null);
      if (defaultWiki != null) page = WikiPage.findTemplatePage(ctxt, defaultWiki, template);
    }

    if (domain != null) cacheTemplate(domain, template, page);
  ...
  private static WikiPage findTemplatePage(
      WikiPage.WikiContext ctxt, String wikiAccountName, String template) throws ServiceException {
    Provisioning prov = Provisioning.getInstance();
    ...
    if (acct == null)
      throw ServiceException.FAILURE(
          "wiki account "
              ...
              + Provisioning.A_zimbraNotebookAccount
              + " on the domain or global config",
          null);
    ...
    if (Provisioning.onLocalServer(acct)) {
      try {
        ...
        if (item instanceof Document) page = WikiPage.create((Document) item);
  ...
  public static class LocalWikiPage extends WikiPage {
    ...
    public Document getWikiRevision(WikiPage.WikiContext ctxt, int version)
        throws ServiceException {
  ...
  public static class RemoteWikiPage extends WikiPage {
    ...
    public String getContents(WikiPage.WikiContext ctxt) throws ServiceException {
      ...
      AuthToken auth;
      if (ctxt != null && ctxt.auth != null) auth = ctxt.auth;
      else {
        try {
      ...
      if (status != 200) throw ServiceException.RESOURCE_UNREACHABLE("http error " + status, null);
      try {
