 ...
 *
 * <p>WikiTemplate is a parsed Wiki page. Each parsed tokens represent either a block of text, or a
 * wiklet. A wiklet can refer to another document stored in someone else's mailbox. To render a wiki
 * page, it will go through each wiklet, and get the contents denoted by each wiklet and based on
 * the context the wiklet is run (privilege of the requestor, location of the requested page,
 * location of the page referred by the wiklet).
 *
 * <p>Each parsed templates are cached in the class <code>Wiki</code>.
 */
public class WikiTemplate implements Comparable<WikiTemplate> {
  ...
  public static WikiTemplate findTemplate(Context ctxt, String name) throws ServiceException {
    ...
    if (page == null) page = WikiPage.missingPage(name);
  ...
  public void parse() {
    if (!mParsed) Token.parse(mTemplate, mTokens);
    mParsed = true;
  }
  ...
  private String apply(Context ctxt) throws ServiceException, IOException {
    if (ctxt.token.getType() == Token.TokenType.TEXT
        || ctxt.token.getType() == Token.TokenType.COMMENT) return ctxt.token.getValue();
  ...
  public static class Token {
    ...
    public static void parse(String str, List<Token> tokens) throws IllegalArgumentException {
      ...
      for (int pos = 0; pos < len; ) {
        ...
        if (tok == null) {
          ...
          if (end == lastPos) end = len;
          if (end > pos) tok = new Token(str.substring(pos, end), TokenType.TEXT);
        }
    ...
    public Map<String, String> parseParam(String text) {
      ...
      for (int start = 0, end = 0; !done; end++) {
        ...
        } else c = text.charAt(end);
        if (state == ParseState.K) {
          if (c == ' ' || c == '=') {
            key = text.substring(start, end);
            start = end + 1;
            if (c == ' ') map.put(key, key);
            else if (c == '=') state = ParseState.V;
          }
  ...
  public abstract static class Wiklet {
    ...
    protected String handleTemplates(
        Context ctxt, List<MailItem> list, String bodyTemplate, String itemTemplate)
        throws ServiceException, IOException {
      StringBuffer buf = new StringBuffer();
    ...
    public static Wiklet get(Token tok) {
      ...
      if (tok.getType() == Token.TokenType.WIKILINK) {
        ...
        if (firstTok.equals(Token.sWIKLETTAG)) {
          ...
          if (cls.equals("link")) w = sWIKLETS.get("WIKILINK");
          else w = sWIKLETS.get(cls.toUpperCase());
        } else {
  ...
  public static class TocWiklet extends Wiklet {
    ...
    public String generateList(Context ctxt, int style) throws ServiceException {
      ...
      for (Document doc :
          mbox.getDocumentList(ctxt.wctxt.octxt, folder.getId(), SortBy.NAME_NATURAL_ORDER_ASC)) {
        buf.append("<");
        buf.append(sTAGS[sINNER][style]);
        buf.append(" class='zmwiki-pageLink'>");
    ...
    public String applyTemplates(Context ctxt, Map<String, String> params)
        throws ServiceException, IOException {
      List<MailItem> list = new ArrayList<MailItem>();
      Folder folder;
      if (ctxt.item instanceof Folder) folder = (Folder) ctxt.item;
      else folder = ctxt.item.getMailbox().getFolderById(ctxt.wctxt.octxt, ctxt.item.getFolderId());
      ...
      if (ctxt.wctxt.view == null) {
        list.addAll(
            mbox.getItemList(
                ctxt.wctxt.octxt, type, folder.getId(), SortBy.NAME_NATURAL_ORDER_ASC));
      } else {
        list.addAll(
            mbox.getItemList(
                ctxt.wctxt.octxt,
                MailItem.Type.of(ctxt.wctxt.view),
                folder.getId(),
      ...
      if (bt == null) bt = sDEFAULTBODYTEMPLATE;
      if (it == null) it = sDEFAULTITEMTEMPLATE;
  ...
  public static class HistoryWiklet extends Wiklet {
    ...
    public String applyTemplates(Context ctxt, Map<String, String> params)
      ...
      Mailbox mbox = ctxt.item.getMailbox();
      List<MailItem> list =
          mbox.getAllRevisions(ctxt.wctxt.octxt, ctxt.item.getId(), ctxt.item.getType());
  ...
  public static class FragmentWiklet extends Wiklet {
    ...
    @Override
    public String apply(Context ctxt) {
      if (!(ctxt.item instanceof Document)) return "";
      ...
      if (ctxt.item instanceof Folder) {
        ...
        Document doc =
            (Document)
                ctxt.item
                    .getMailbox()
                    .getItemRevision(ctxt.wctxt.octxt, ctxt.item.getId(), ctxt.item.getType(), 1);
        if (doc != null) return doc.getCreator();
      }
  ...
  public abstract static class DateTimeWiklet extends Wiklet {
    ...
    static {
      ...
      sFORMATS.put(
          sSHORTDATETIME, DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT));
      sFORMATS.put(
          sMEDIUMDATETIME, DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.MEDIUM));
    ...
    protected String formatDate(Context ctxt, Date date) {
      ...
      Map<String, String> params = ctxt.token.parseParam();
      String format = params.get(sFORMAT);
      if (format == null || !sFORMATS.containsKey(format)) format = sSHORTDATETIME;
  ...
  public static class VersionWiklet extends Wiklet {
    ...
    @Override
    public String apply(Context ctxt) {
      if (!(ctxt.item instanceof Document)) return "1";
      Document doc = (Document) ctxt.item;
  ...
  public static class ContentWiklet extends Wiklet {
    ...
    @Override
    public String apply(Context ctxt) throws ServiceException, IOException {
      if (ctxt.content != null) return ctxt.content;
      if (!(ctxt.item instanceof WikiItem)) return "<!-- cotent wiklet on non-wiki item -->";
      ...
      for (Token t : template.getTokens()) {
        ...
        if (w == null || w.getName().equals(getName())) return template.getRaw();
      }
  ...
  public static class UrlWiklet extends Wiklet {
    ...
    @Override
    public String apply(Context ctxt) {
      if (ctxt.item == null) return "<!-- cannot resolve item for url wiklet -->";
      ...
      try {
        ...
        if (type != null && type.equals(sVERSIONURL) && ctxt.latestVersionItem != null) {
          ...
          url =
              wurl.getFullUrl(ctxt.wctxt, ctxt.latestVersionItem.getMailbox().getAccountId())
                  + "?ver="
                  + ctxt.item.getVersion();
          ...
          if (ctxt.item instanceof Folder) {
            ...
            url =
                wurl.getFullUrl(ctxt.wctxt, ctxt.item.getMailbox().getAccountId())
                    + "?view="
                    + sHISTORYURL;
          }
        } else {
          ...
          if (ctxt.item.getType() == MailItem.Type.DOCUMENT) {
            ...
            if (contentType != null
                && (contentType.indexOf(MimeConstants.CT_APPLICATION_ZIMBRA_DOC) >= 0
                    || contentType.indexOf(MimeConstants.CT_APPLICATION_ZIMBRA_SLIDES) >= 0
                    || contentType.indexOf(MimeConstants.CT_APPLICATION_ZIMBRA_SPREADSHEET) >= 0)) {
  ...
  static class WikiUrl {
    public WikiUrl(MailItem item) throws ServiceException {
      if (item instanceof Folder) mIsFolder = true;
      mUrl = item.getName();
    ...
    public Account getOwnerAccount(String referenceAccount) throws ServiceException {
      return (inAnotherMailbox())
          ...
          : (referenceAccount == null)
              ? null
              : Provisioning.getInstance().get(AccountBy.id, referenceAccount);
    }
    ...
    private String normalizePath(String path) throws ServiceException {
      ...
      if (path.endsWith("/")) tokens.add("");
      StringBuilder newPath = new StringBuilder();
      for (String token : tokens) {
        newPath.append("/").append(urlEscape(token));
      }
      if (mIsFolder) newPath.append("/");
    ...
    private String urlEscape(String str) {
      ...
      for (char c : str.toCharArray()) {
        if (c == ' ') buf.append("%20");
        else if (c == '"') buf.append("%22");
        else if (c == '\'') buf.append("%27");
        else if (c == '#') buf.append("%23");
        else if (c == '?') buf.append("%3F");
        else buf.append(c);
      }
      return buf.toString();
    }
    ...
    public boolean isAbsolute() {
      return (mTokens != null && mTokens.get(0).startsWith("/"));
    }

    public boolean inAnotherMailbox() {
      return (mTokens != null && mTokens.get(0).equals("//"));
    }
