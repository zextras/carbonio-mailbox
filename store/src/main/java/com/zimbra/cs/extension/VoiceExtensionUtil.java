...
public class VoiceExtensionUtil {
  ...
  public static void registerVoiceProvider(
      String extension, String providerName, String className, Set<String> applicableAttrs) {
    try {
      Class vsClass = ExtensionUtil.findClass("com.zimbra.cs.voice.VoiceStore");
      Method method =
          vsClass.getMethod("register", String.class, String.class, String.class, Set.class);
      method.invoke(vsClass, extension, providerName, className, applicableAttrs);
    } catch (Exception e) {
      ZimbraLog.extensions.error(
          "unable to register VoiceStore: extension=" + extension + ", className=" + className, e);
    }
  }
}
