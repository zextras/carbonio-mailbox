...
/**
 * Presents contents of the file resulting from Octopus patch application as an input stream.
 *
 * <p>The class effectively multiplexes data from existing files (blobs) on the server as referenced
 * by an Octopus patch and the data contained within the patch itself:
 *
 * <p>1) PatchReader provides access to the patch itself containing references to exiting files and
 * the embedded data
 *
 * <p>2) BlobAccess provides access to the existing blobs (files) on the server
 *
 * @author grzes
 */
public class PatchInputStream extends InputStream {
  ...
  /**
   * Factory method for convenient creation of PatchInputStream using BinaryPatchReader and
   * BlobAccessViaMailbox.
   *
   ...
   * @param patchOs Output stream for the patch. If provided a byte copy of the patch will be made
   *     and written to the output stream as the patch is being processed. Can be null
   * @param manifest Patch manifest to populate with references being made by the patch. Optional,
   *     can be null
   * @return New instance of PatchInputStream
   * @throws IOException Signals that an I/O exception has occurred.
   * @throws ServiceException the service exception
   */
  public static PatchInputStream create(
      InputStream is,
      Mailbox mbox,
      OperationContext opContext,
      int defaultFileId,
      int defaultVersion,
      OutputStream patchOs,
    ...
    if (ri.type == PatchReader.RecordType.DATA) {
      ...
      try {
        ...
        if (!Arrays.equals(patchRef.hashKey, calcHash)) {
          throw new InvalidPatchReferenceException(
              "refrenced data hash mismatch, actual hash: "
                  + new String(Hex.encodeHex(calcHash))
                  + "; "
                  + patchRef);
        }
