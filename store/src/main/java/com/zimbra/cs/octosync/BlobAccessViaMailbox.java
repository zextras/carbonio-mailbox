...
public class BlobAccessViaMailbox implements BlobAccess {
   ...
   * @param defaultFileId Default file id; 0 passed to getBlobInputStream() will be mapped to this
   *     id. This is support default references in the patch (referring to the latest revision of
   *     the file being uploaded)
   * @param defaultVersion Default version number; 0 passed to getBlobInputStream() will be mapped
   *     to this numbeer
   */
  public BlobAccessViaMailbox(
      Mailbox mbox, OperationContext opContext, int defaultFileId, int defaultVersion) {
    this.mbox = mbox;
  ...
  public InputStream getBlobInputStream(int fileId, int version)
    ...
    try {
      Document doc =
          (Document) mbox.getItemRevision(opContext, fileId, MailItem.Type.DOCUMENT, version);
      ...
      if (doc == null) {
        throw new InvalidPatchReferenceException(
            "Reference to non-existing version " + version + " of document " + fileId);
      }
      ...
      throw new InvalidPatchReferenceException(
          "Reference to non-existing document: " + fileId + " (version: " + version + ")", e);
    }
  }
}
