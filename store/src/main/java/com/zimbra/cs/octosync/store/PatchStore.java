...
public class PatchStore {
  ...
  public StoredPatch acceptPatch(IncomingPatch ip, int fileId, int version)
    ...
    StoredBlob psb =
        blobStore.store(ip.patchBlob, getStoredPatchId(ip.getAccountId(), fileId, false), version);

    StoredBlob msb = null;

    try {
      msb =
          blobStore.store(manifestBlob, getStoredPatchId(ip.getAccountId(), fileId, true), version);
    } catch (IOException e) {
