...
public class BinaryPatchReader implements PatchReader {
  ...
  /**
   * Position in the stream relative to the original position of the stream as passed to the
   * constructor. For detailed error reporting only.
   */
  private long pos;

  /**
   * For interim testing only. Validates patch format, checking the syntax, but without verifying if
   * e.g. patch references are valid.
   *
   * @param is Input stream with patch
   ...
   *
   * @param is The input stream to read patch contents from. The stream is closed once fully read
   *     from or upon encountering patch format related error.
   * @throws IOException Signals that an I/O exception has occurred.
   * @throws BadPatchFormatException Signals malformed patch format
   */
  public BinaryPatchReader(InputStream is) throws IOException, BadPatchFormatException {
   ...
   *
   * <p>The input stream must be set to the beginning of the patch. This will require composite
   * InputStream consisting of the data already stored and new data incoming from the client.
   *
   * <p>After the call succeeds, call hasMoreRecordInfos() and then appropriate getNextRecordInfo().
   * If the processing was interrupted in the midst of a data record, the returned data record will
   * point exactly to the first byte that wasn't read yet.
   *
   * <p>The important assumption is that every bit of data read from PatchReader before premature
   * end of stream has been written to the destination. If e.g. reference record was returned
   * successfully, it will not be returned again after resume.
   *
   * @param is
   */
  ...
  public boolean hasMoreRecordInfos() throws IOException, BadPatchFormatException {
    ...
    assert numBytesLeftInCurrentDataRecord == 0
        : "Not all bytes read from the current data record at " + pos;
