...
public class GalSearchConfig {
  ...
  private static class DataSourceConfig extends GalSearchConfig {
    public DataSourceConfig(DataSource ds) throws ServiceException {
      ...
      if (mGalType == GalType.zimbra) {
        ...
        if (mUrl.length == 0 || mFilter == null) loadConfig(domain, GalOp.search);
        String[] url = ds.getMultiAttr(Provisioning.A_zimbraGalSyncLdapURL);
        if (url != null && url.length > 0) mUrl = url;
        ...
        mStartTlsEnabled =
            ds.getBooleanAttr(Provisioning.A_zimbraGalSyncLdapStartTlsEnabled, mStartTlsEnabled);
        ...
        mKerberosPrincipal =
            ds.getAttr(Provisioning.A_zimbraGalSyncLdapKerberos5Principal, mKerberosPrincipal);
        mKerberosKeytab =
            ds.getAttr(Provisioning.A_zimbraGalSyncLdapKerberos5Keytab, mKerberosKeytab);
        mTimestampFormat =
            ds.getAttr(Provisioning.A_zimbraGalSyncTimestampFormat, mTimestampFormat);
      ...
      if (attrs.length > 0 || valueMap.length > 0 || groupHandlerClass != null) {
        if (attrs.length == 0) attrs = domain.getMultiAttr(Provisioning.A_zimbraGalLdapAttrMap);
  ...
  protected void loadZimbraConfig(Domain domain, GalOp op, GalSearchType stype)
      throws ServiceException {
    ...
    switch (op) {
      case sync:
        filterName =
            (stype == GalSearchType.all)
                ? NamedFilter.zimbraSync
                : (stype == GalSearchType.resource)
                    ? NamedFilter.zimbraResourceSync
                    : (stype == GalSearchType.group)
                        ? NamedFilter.zimbraGroupSync
                        : NamedFilter.zimbraAccountSync;
        break;
      case search:
        filterName =
            (stype == GalSearchType.all)
                ? NamedFilter.zimbraSearch
                : (stype == GalSearchType.resource)
                    ? NamedFilter.zimbraResources
                    : (stype == GalSearchType.group)
                        ? NamedFilter.zimbraGroups
      ...
      case autocomplete:
        filterName =
            (stype == GalSearchType.all)
                ? NamedFilter.zimbraAutoComplete
                : (stype == GalSearchType.resource)
                    ? NamedFilter.zimbraResourceAutoComplete
                    : (stype == GalSearchType.group)
                        ? NamedFilter.zimbraGroupAutoComplete
    ...
    mFilter =
        "(&"
            + filter
            + "(!(zimbraHideInGal=TRUE))(!(zimbraIsSystemResource=TRUE))"
            + dnSubtreeMatchFilter
            + ")";
  ...
  private boolean isConfigComplete() {
    return mUrl.length > 0
        ...
        && (mAuthMech.equals(Provisioning.LDAP_AM_NONE)
            || (mAuthMech.equals(Provisioning.LDAP_AM_SIMPLE)
                && mBindDn != null
                && mBindPassword != null)
            || (mAuthMech.equals(Provisioning.LDAP_AM_KERBEROS5)
                && mKerberosKeytab != null
                && mKerberosPrincipal != null));
  }
  ...
  protected void loadConfig(Domain domain, GalOp op) throws ServiceException {
    ...
    if (galMode == GalMode.zimbra) loadZimbraConfig(domain, op, null);
    ...
    switch (op) {
      case sync:
        ...
        mStartTlsEnabled =
            domain.getBooleanAttr(Provisioning.A_zimbraGalSyncLdapStartTlsEnabled, false);
        mAuthMech =
            domain.getAttr(Provisioning.A_zimbraGalSyncLdapAuthMech, Provisioning.LDAP_AM_SIMPLE);
        ...
        mTimestampFormat =
            domain.getAttr(
                Provisioning.A_zimbraGalSyncTimestampFormat,
                LdapDateUtil.ZIMBRA_LDAP_GENERALIZED_TIME_FORMAT_LEGACY);
        mPageSize = domain.getIntAttr(Provisioning.A_zimbraGalSyncLdapPageSize, 1000);
        if (isConfigComplete()) break;
      case search:
        ...
        mStartTlsEnabled =
            domain.getBooleanAttr(Provisioning.A_zimbraGalLdapStartTlsEnabled, false);
        mAuthMech =
            domain.getAttr(Provisioning.A_zimbraGalLdapAuthMech, Provisioning.LDAP_AM_SIMPLE);
