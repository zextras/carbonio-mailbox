...
public class GalExtraSearchFilter {
  ...
  public static EntrySearchFilter parseSearchFilter(Element request) throws ServiceException {
    ...
    Element termElem =
        filterElem.getOptionalElement(AccountConstants.E_ENTRY_SEARCH_FILTER_MULTICOND);
    if (termElem == null)
  ...
  private static Term parseFilterTermElem(Element termElem) throws ServiceException {
    ...
    boolean negation =
        termElem.getAttributeBool(AccountConstants.A_ENTRY_SEARCH_FILTER_NEGATION, false);
    if (elemName.equals(AccountConstants.E_ENTRY_SEARCH_FILTER_MULTICOND)) {
      ...
      if (attr == null) throw ServiceException.INVALID_REQUEST("Missing search term attr", null);
      String op = termElem.getAttribute(AccountConstants.A_ENTRY_SEARCH_FILTER_OP);
      if (op == null) throw ServiceException.INVALID_REQUEST("Missing search term op", null);
      String value = termElem.getAttribute(AccountConstants.A_ENTRY_SEARCH_FILTER_VALUE);
      if (value == null) throw ServiceException.INVALID_REQUEST("Missing search term value", null);
      term = new Single(negation, attr, op, value);
    } else {
      throw ServiceException.INVALID_REQUEST(
          "Unknown element <" + elemName + "> in search filter", null);
    }
    return term;
  }
  ...
  public abstract static class GalExtraQueryCallback implements GalSearchQueryCallback {
    ...
    public String getMailboxSearchQuery() {
      GalExtraSearchFilter.MailboxQueryVisitor visitor =
          new GalExtraSearchFilter.MailboxQueryVisitor();
    ...
    public void visitSingle(Single term) {
      ...
      if (parent != null && parent.getTerms().size() > 1) {
        ...
        if (!parent.getTerms().get(0).equals(term)) {
          if (parent.isAnd()) query.append(" AND ");
  ...
  public abstract static class FilteredGalSearchResultCallback
      extends GalSearchResultCallback.PassThruGalSearchResultCallback {
    private final Set<String> mAttrs;
    private final EntrySearchFilter mFilter;

    public FilteredGalSearchResultCallback(
        GalSearchParams params, EntrySearchFilter filter, Set<String> attrs) {
      super(params);
      mAttrs = attrs;
      mFilter = filter;
    }
  ...
  private static class FilterVisitor implements Visitor {
    ...
    private static class Result {
      ...
      private void negateResult() {
        if (mCurResult != null) mCurResult = !mCurResult;
      }
    }
    ...
    private boolean evaluate(Single term) {
      ...
      if (value instanceof String[]) {
        for (String v : (String[]) value) {
          if (shouldInclude(term, v)) return true;
        }
        return false;
      } else if (value != null) {
    ...
    private boolean shouldInclude(Single term, String value) {
      ...
      if (term.isNegation()) return !result;
      else return result;
    }
  }
}
