...
public class GalSyncToken {
  ...
  public String getLdapTimestamp(String format, String ldapTimestamp) throws ServiceException {
    ...
    if (ts == null) {
      ...
      if (format.endsWith("'Z'")) {
        ...
        // we previously used SimpleDateFormat on both ends; so we were using local timezone but
        // ignoring it
  ...
  public int getChangeId(String accountId) {
    ...
    if (cid != null) return Integer.parseInt(cid);
    return 0;
  }
  ...
  public void merge(GalSyncToken that) {
    ...
    for (String aid : that.mChangeIdMap.keySet()) {
      ...
      if (StringUtils.isNotBlank(strThatVal)
          && StringUtils.isNotBlank(strThisVal)
          && StringUtils.isNumeric(strThatVal)
          && StringUtils.isNumeric(strThisVal)) {
