...
public class GalSearchResultCallback implements GalContact.Visitor {
  ...
  public Element handleContact(Contact c) throws ServiceException {
    ...
    if (mIdOnly) {
      eContact =
          mResponse
              ...
              .addAttribute(MailConstants.A_ID, mFormatter.formatItemId(c));
    } else if (mOp == GalOp.sync) {
      eContact =
          ToXML.encodeContact(mResponse, mFormatter, null, c, true, c.getAllFields().keySet());
    ...
    if (c.isGroup()) {
      ...
      if (mNeedCanExpand) {
        boolean canExpand =
            GalSearchControl.canExpandGalGroup(
  ...
  public void handleDeleted(ItemId id) {
    mResponse
        .addNonUniqueElement(MailConstants.E_DELETED)
        .addAttribute(MailConstants.A_ID, id.toString());
  }
  ...
  public void setNewToken(GalSyncToken newToken) {
    ...
    if (oldToken != null) newToken.merge(new GalSyncToken(oldToken));
  ...
  public abstract static class PassThruGalSearchResultCallback extends GalSearchResultCallback {
    ...
    @Override
    public Element getResponse() {
      if (mProxiedResponse != null) return mProxiedResponse;
      else {
