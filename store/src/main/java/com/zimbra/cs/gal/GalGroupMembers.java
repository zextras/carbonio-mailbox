...
public class GalGroupMembers {
  ...
  public static class GalContactDLMembers extends DLMembers {
    ...
    private GalContactDLMembers(GalContact galContact) {
      ...
      if (members instanceof String) mMembers = new String[] {(String) members};
      else if (members instanceof String[]) mMembers = (String[]) members;
    }
  ...
  public static class LdapHABMembers extends DLMembers {
    ...
    public void encodeMembers(int beginIndex, int endIndex, Element resp) {
      if (endIndex <= getTotal() && getTotal() != 0) {
        ...
        for (int i = beginIndex; i < endIndex; i++) {
          Element habGroupMember =
              habGroupMembers.addNonUniqueElement(AccountConstants.E_HAB_GROUP_MEMBER);
          habGroupMember.addAttribute(AccountConstants.A_NAME, habMembers.get(i).getName());
          for (NamedValue nv : habMembers.get(i).getAttrs()) {
            habGroupMember.addKeyValuePair(
                nv.getName(), nv.getValue(), AccountConstants.E_ATTR, AccountConstants.A_NAME);
          }
        }
      }
    }
  ...
  public static DLMembersResult searchGal(
      ZimbraSoapContext zsc, Account account, String groupName, Element request)
   ...
   * @throws ServiceException
   */
  public static Set<String> getGroupMembers(String groupName, Account account)
      throws ServiceException {
    ...
    ZimbraSoapContext zsc =
        new ZimbraSoapContext(
            adminAuthToken, account.getId(), SoapProtocol.Soap12, SoapProtocol.Soap12);

    Element request =
        Element.create(SoapProtocol.Soap12, AccountConstants.GET_DISTRIBUTION_LIST_MEMBERS_REQUEST);
