...
public class GalSearchParams {
  ...
  private int mLimit;
  private Integer
      mLdapLimit; // ldap search does not support paging, allow a different limit for ldap search
  ...
  public String getSyncToken() {
    return !StringUtils.isEmpty(ldapTimeStamp)
        ? ldapTimeStamp
        : (mSyncToken != null ? mSyncToken.getLdapTimestamp() : null);
  }
  ...
  public GalSearchResultCallback getResultCallback() {
    if (mResultCallback == null) return createResultCallback();
  ...
  public String generateLdapQuery() throws ServiceException {
    ...
    String token =
        (mSyncToken != null)
            ? mSyncToken.getLdapTimestamp(mConfig.mTimestampFormat, ldapTimeStamp)
            : null;
    ...
    return GalUtil.expandFilter(
        mConfig.getTokenizeKey(), filterTemplate, mQuery, token, extraQuery, ldapHasMore);
  }
