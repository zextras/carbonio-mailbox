...
public class LdapGalMapRules {
  ...
  private Set<String>
      mBinaryLdapAttrs; // attrs need to be set in JNDI "java.naming.ldap.attributes.binary"
  ...
  private static final String CURRENT_DEFAULT_GROUPHANDLER =
  ...
  private void init(Entry entry, boolean isZimbraGal) {
    ...
    init(
        entry.getMultiAttr(Provisioning.A_zimbraGalLdapAttrMap),
  ...
  private void init(String[] rules, String[] valueMaps, String groupHandlerClass) {
    ...
    for (String rule : rules) add(rule);
    ...
    ZimbraLog.gal.debug(
        "groupHandlerClass="
            ...
            + mGroupHandler.getClass().getCanonicalName());
  }
      ...
      try {
        if (mFetchGroupMembers) {
          contactAttrs.put(
              ...
              mGroupHandler.getMembers(ldapContext, searchBase, entryDN, ldapAttrs));
        } else {
