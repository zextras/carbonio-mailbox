...
public abstract class CheckRightFallback {
  ...
  public Boolean checkRight(Account authedAcct, Entry target, boolean asAdmin) {
    try {
      ...
      ZimbraLog.acl.warn(
          "caught exception in checkRight fallback"
              + ", checkRight fallback for right ["
              + mRight.getName()
              + "] skipped",
          e);
      return null;
  ...
  protected abstract Boolean doCheckRight(Account grantee, Entry target, boolean asAdmin)
