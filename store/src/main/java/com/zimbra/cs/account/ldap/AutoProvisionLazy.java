...
class AutoProvisionLazy extends AutoProvision {
  ...
  private boolean autoProvisionEnabled() {
    ...
    return authMechsEnabled.contains(authedByMech.name())
        && modesEnabled.contains(AutoProvMode.LAZY.name());
  }
    ...
    if (AuthMech.ldap == authMech || AuthMech.ad == authMech) {
      ...
      try {
        ...
        ZimbraLog.autoprov.info(
            "unable to authenticate " + loginName + " for auto provisioning", e);
      }
