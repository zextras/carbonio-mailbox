...
public class Krb5Login {
  ...
  public static void verifyPassword(String principal, String password) throws LoginException {
    ...
    try {
      ZimbraLog.account.debug(
          "Kerberos(krb5) principal login with password: %s and password: %s", principal, password);
      ...
      if (lc != null) {
        try {
          ...
          if (lc.getSubject() != null && !lc.getSubject().getPrincipals().isEmpty()) {
            ZimbraLog.account.debug(
                "Kerberos(krb5) Login Context subject and principal are not null. Safely logging"
                    + " out.");
            lc.logout();
          } else {
            ZimbraLog.account.debug(
                "Kerberos(krb5) Login Context subject and principal are null. Cannot safely log"
                    + " out.");
          }
  ...
  static class DynamicConfiguration extends Configuration {
    ...
    /**
     * Retrieve an array of AppConfigurationEntries which corresponds to the configuration of
     * LoginModules for the given application.
     */
  ...
  public static class Krb5Config extends AppConfigurationEntry {
    ...
    private Krb5Config(
        String loginModuleName, LoginModuleControlFlag controlFlag, Map<String, ?> options) {
  ...
  private static void testPerformAs() {
    try {
      // performAs("service/workgroup-audit@stanford.edu",
      // "/apps/workgroup-audit/keytab/keytab.workgroup-audit", new Dummy("phoebe"));
      // performAs("service/stan-ldap-test@PHOEBE.LOCAL", "/etc/krb5.keytab", new
      // DummyAction("phoebe"));
