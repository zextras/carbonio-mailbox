...
public class ChildAccount extends AttributeCallback {
  ...
  public void preModify(
    ...
    MultiValueMod visibleChildrenMod =
        multiValueMod(attrsToModify, Provisioning.A_zimbraPrefChildVisibleAccount);
    MultiValueMod allChildrenMod = multiValueMod(attrsToModify, Provisioning.A_zimbraChildAccount);

    Set<String> visibleChildren =
        newValuesToBe(visibleChildrenMod, entry, Provisioning.A_zimbraPrefChildVisibleAccount);
    Set<String> allChildren =
        newValuesToBe(allChildrenMod, entry, Provisioning.A_zimbraChildAccount);
    ...
    if (allChildrenMod != null && allChildrenMod.deleting()) {
        ...
        if (!allChildren.contains(vid)) {
          ...
          if (allChildrenMod != null && allChildrenMod.removing() && visibleChildrenMod == null)
            ...
            throw ServiceException.INVALID_REQUEST(
                "visible child id " + vid + " is not one of " + Provisioning.A_zimbraChildAccount,
                null);
        }
      }
      ...
      if (vidsToRemove.size() > 0)
        attrsToModify.put(
            ...
            vidsToRemove.toArray(new String[vidsToRemove.size()]));
    }
    ...
    if (entry instanceof Account) {
      ...
      for (String childId : allChildren) {
        ...
        if (childAcct == null) throw AccountServiceException.NO_SUCH_ACCOUNT(childId);
          ...
          if (child.equals(parentId))
            throw ServiceException.INVALID_REQUEST(
                "child account "
                    ...
                    + childAcct.getName()
                    + ")"
                    + " is parent of the parent account "
                    + parentId
                    + "("
                    + parentAcct.getName()
                    + ")",
