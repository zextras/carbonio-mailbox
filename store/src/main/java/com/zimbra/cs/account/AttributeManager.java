...
public class AttributeManager {
  ...
  //
  // Note: does *not* contains attrs defined in the extensions(attrs in OCs specified in global
  // config ***ExtraObjectClass)
  //
  ...
  */
  ...
  private static AttributeCallback loadCallback(String clazz) {
    ...
    try {
      ZimbraLog.misc.info("Loading class from attributemanager " + clazz);
  ...
  private void loadAttrs(String fileName, Document doc) {
    ...
    for (Iterator iter = root.elementIterator(); iter.hasNext(); ) {
      ...
      if (id > 0
          && (optionalIn != null && optionalIn.isEmpty())
  ...
  private void checkFlag(
      ...
      AttributeClass c2,
      AttributeClass c3,
      Set<AttributeClass> required,
      Set<AttributeClass> optional) {

    if (flags != null && flags.contains(flag)) {
      boolean inC1 =
          (optional != null && optional.contains(c1))
