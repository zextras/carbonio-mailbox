...
public class DomainCache implements IDomainCache {
  ...
  private Counter mHitRate = new HitRateCounter("domain_cache");
  ...
  class NegativeCache {
    ...
    private void clean(DomainBy domainBy, String key, Domain entry) {
      ...
      for (String vh : vhost) mNegativeVirtualHostnameCache.remove(vh.toLowerCase());

      String foreignName[] = entry.getMultiAttr(Provisioning.A_zimbraForeignName);
      for (String fn : foreignName) mNegativeForeignNameCache.remove(fn.toLowerCase());

      String krb5Realm = entry.getAttr(Provisioning.A_zimbraAuthKerberos5Realm);
      if (krb5Realm != null) mNegativeKrb5RealmCache.remove(krb5Realm);
    }
  ...
  public synchronized void remove(Domain entry) {
    if (entry != null) {
      ...
      String vhost[] = entry.getMultiAttr(Provisioning.A_zimbraVirtualHostname);
      for (String vh : vhost) mVirtualHostnameCache.remove(vh.toLowerCase());

      String foreignName[] = entry.getMultiAttr(Provisioning.A_zimbraForeignName);
      for (String fn : foreignName) mForeignNameCache.remove(fn.toLowerCase());

      String krb5Realm = entry.getAttr(Provisioning.A_zimbraAuthKerberos5Realm);
      if (krb5Realm != null) mKrb5RealmCache.remove(krb5Realm);
    }
  }
  ...
  public synchronized void put(DomainBy domainBy, String key, Domain entry) {
    if (entry != null) {
      ...
      String vhost[] = entry.getMultiAttr(Provisioning.A_zimbraVirtualHostname);
      for (String vh : vhost) mVirtualHostnameCache.put(vh.toLowerCase(), cacheEntry);

      String foreignName[] = entry.getMultiAttr(Provisioning.A_zimbraForeignName);
      for (String fn : foreignName) mForeignNameCache.put(fn.toLowerCase(), cacheEntry);

      String krb5Realm = entry.getAttr(Provisioning.A_zimbraAuthKerberos5Realm);
      if (krb5Realm != null) mKrb5RealmCache.put(krb5Realm, cacheEntry);
    } else {
  ...
  public synchronized Domain getById(String key, GetFromDomainCacheOption option) {
    ...
    switch (option) {
      ...
      case BOTH:
        ...
        if (d == null) d = mNegativeCache.get(DomainBy.id, key);
        return d;
      default:
        return null;
    }
  ...
  public synchronized Domain getByName(String key, GetFromDomainCacheOption option) {
    ...
    switch (option) {
      ...
      case BOTH:
        ...
        if (d == null) d = mNegativeCache.get(DomainBy.name, key);
        return d;
      default:
        return null;
    }
  ...
  public synchronized Domain getByVirtualHostname(String key, GetFromDomainCacheOption option) {
    ...
    switch (option) {
      ...
      case BOTH:
        ...
        if (d == null) d = mNegativeCache.get(DomainBy.virtualHostname, key);
        return d;
      default:
        return null;
    }
  ...
  public synchronized Domain getByForeignName(String key, GetFromDomainCacheOption option) {
    ...
    switch (option) {
      ...
      case BOTH:
        ...
        if (d == null) d = mNegativeCache.get(DomainBy.foreignName, key);
        return d;
      default:
        return null;
    }
  ...
  public synchronized Domain getByKrb5Realm(String key, GetFromDomainCacheOption option) {
    ...
    switch (option) {
      ...
      case BOTH:
        ...
        if (d == null) d = mNegativeCache.get(DomainBy.krb5Realm, key);
        return d;
      default:
        return null;
    }
