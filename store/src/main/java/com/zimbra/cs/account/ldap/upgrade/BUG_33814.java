...
public class BUG_33814 extends UpgradeOp {
  ...
  private static final String TLSLEVEL_ENCRYPT =
      ZAttrProvisioning.MtaTlsSecurityLevel.may.toString(); // we don't support encryp yet, see
  // http://bugzilla.zimbra.com/show_bug.cgi?id=33814#c12
  ...
  private void doMtaTlsSecurityLevelOnGlobalConfig(Entry entry, Map<String, Object> attrValues) {
    ...
    if (ProvisioningConstants.TRUE.equals(zimbraMtaAuthEnabled)) {
      if (ProvisioningConstants.TRUE.equals(zimbraMtaTlsAuthOnly)) value = TLSLEVEL_ENCRYPT;
      else value = TLSLEVEL_MAY;
    } else value = TLSLEVEL_NONE;
  ...
  */
  private void doMtaTlsSecurityLevelOnServer(Entry entry, Map<String, Object> attrValues) {
    ...
    if (value != null) attrValues.put(Provisioning.A_zimbraMtaTlsSecurityLevel, value);
  }
  ...
  private void doEntry(ZLdapContext zlc, Entry entry, String entryName, AttributeClass klass)
    ...
    try {
      ...
      // new attrs
      String zimbraMtaTlsSecurityLevel =
          entry.getAttr(Provisioning.A_zimbraMtaTlsSecurityLevel, false);
      ...
      if (zimbraMtaTlsSecurityLevel == null) {
        if (entry instanceof Server) doMtaTlsSecurityLevelOnServer(entry, attrValues);
        else doMtaTlsSecurityLevelOnGlobalConfig(entry, attrValues);
      } else {
        printer.println(
            "Not updating zimbraMtaTlsSecurityLevel because there is already a value: "
      ...
      if (zimbraMtaSaslAuthEnable == null) {
        ...
        printer.println(
            "Not updating zimbraMtaSaslAuthEnable because there is already a value: "
      ...
      printer.println(
          "Caught ServiceException while modifying " + entryName + ": " + msg.toString());
