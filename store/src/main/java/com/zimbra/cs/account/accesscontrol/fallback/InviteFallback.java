...
public class InviteFallback extends CheckRightFallback {
  ...
  // (calendar folder in which the appointment is to be updated when the owner of the calendar
  // receives
  // an invite)
  //
  protected Boolean doCheckRight(Account authedAcct, Entry target, boolean asAdmin)
      throws ServiceException {
    // Don't do anything unless running inside the server
    if (!Zimbra.started()) return null;

    if (!(target instanceof Account)) return null;
    ...
    //
    // don't worry about the private right: we are checking if the authed user can
    // invite(public/private)
    // the target user, the authed user is composing the invite and he sees what's in his invite
    // anyway.
    //
    ...
    if (hasRights) return Boolean.TRUE;
    else return null;
  }
}
