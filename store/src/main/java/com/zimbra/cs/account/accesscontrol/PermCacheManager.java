...
class PermCacheManager {
  ...
  private static final long ACL_CACHE_TARGET_MAXAGE =
      LC.acl_cache_target_maxage.intValue() * Constants.MILLIS_PER_MINUTE;
  private static final int ACL_CACHE_CREDENTIAL_MAXSIZE =
      LC.acl_cache_credential_maxsize.intValue();

  private static PermCacheManager theInstance = new PermCacheManager();

  private final LruMap<String, PermCache> targetCache;

  private final Counter hitRate = new Counter("perm_cache");
   ...
   *
   * <p>Note: permission cache is invalidated only on the server on which the permission changing
   * event is executed.
   */
  ...
  /**
   * invalidate permission cache when a permission changing event happens on a target - invalidate
   * permission cache on the specified entry if it is an entry from which no right can be inherited
   * - invalidate all permission cache otherwise
   *
   * <p>possible permission changing event: - granting/revoking rights - adding/removing members on
   * dl - renaming account/dl in/out a domain - that target is deleted
   *
   * <p>Note: permission cache is invalidated only on the server on which the permission changing
   * event is executed.
   *
  ...
  void invalidateCache(Entry target) {
    ...
    try {
      ...
      ZimbraLog.acl.debug(
          "unable to determine if all permission cache should be invalidated, "
              + "invalidating permission cache on all entries",
          e);
    }
  ...
  private String getCacheKey(Entry target) {
    if (target instanceof NamedEntry) return ((NamedEntry) target).getId();
    else return target.getLabel();
  }
  ...
  */
  ...
  private static class PermCache {
    ...
    private synchronized CachedPermission get(String credential, Right right) {
      ...
      if (cachedPerms == null) return CachedPermission.NOT_CACHED;
  ...
  static class CachedPerms {
    ...
    private static final byte[] MASKS_CLEAR = {
      0x0F, 0x70 // can't do F0, would overflow.  left-most bit is not used
    };
    ...
    static CachedPermission get(byte[] cachedPerms, Right right) {
      ...
      else if ((MASKS_PERMS[mask][MASK_DENIED] & byteForRight) != 0) return CachedPermission.DENIED;
      else return CachedPermission.NOT_CACHED;
    }
