...
public class HardRules {
  ...
  public static enum HardRule {
    ...
    private static Argument getExceptionArgument(HardRule rule) {
      return new InternalArgument(rule.name(), "VIOLATED", ServiceException.Argument.Type.STR);
    }
  }
   ...
   *
   * <p>Currently, checks if {@code authedTarget} is a system admin. If not, blocks attempts at
   * delegated admin of system admins.
   *
   * @param authedTarget
   * @param target
   ...
   */
  public static Boolean checkHardRules(
      MailTarget authedTarget, boolean asAdmin, Entry target, Right right) throws ServiceException {
    if ((authedTarget instanceof Account)
        && AccessControlUtil.isGlobalAdmin((Account) authedTarget, asAdmin)) {
      return Boolean.TRUE;
    } else {
          ...
          if (target instanceof Account) {
            if (AccessControlUtil.isGlobalAdmin((Account) target, true)) {
              throw ServiceException.PERM_DENIED(
                  ...
                  HardRule.getExceptionArgument(
                      HardRule.DELEGATED_ADMIN_CANNOT_ACCESS_GLOBAL_ADMIN));
            }
          }
        } else {
          throw ServiceException.PERM_DENIED(
              "not an eligible admin account (not an account)",
              HardRule.getExceptionArgument(HardRule.NOT_EFFECTIVE_DELEGATED_ADMIN_ACCOUNT));
        }
      }
    }
