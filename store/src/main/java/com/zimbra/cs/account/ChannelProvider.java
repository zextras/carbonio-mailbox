...
public abstract class ChannelProvider {
  ...
  public static void registerChannelProvider(String channel, ChannelProvider provider)
    ...
    if (providers.get(channel) != null) {
      ZimbraLog.passwordreset.warn(
          "Channel provider %s already registered for %s, so not adding new provider",
          providers.get(channel).getClass().getName(), channel);
    } else {
      providers.put(channel, provider);
      ZimbraLog.passwordreset.info(
          "Channel provider %s registered for %s",
          providers.get(channel).getClass().getName(), channel);
    }
  }
  ...
  public Map<String, String> getResetPasswordRecoveryCodeMap(Account account)
      throws ServiceException {
  ...
  public abstract void sendAndStoreResetPasswordRecoveryCode(
  ...
  public abstract void validateSetRecoveryAccountCode(
      String recoveryAccountVerificationCode, Account account, Mailbox mbox, ZimbraSoapContext zsc)
      throws ServiceException;

  public abstract void sendAndStoreSetRecoveryAccountCode(
      Account account,
      Mailbox mbox,
