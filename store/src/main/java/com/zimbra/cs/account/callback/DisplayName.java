...
public class DisplayName extends AttributeCallback {
  ...
  public void preModify(
    ...
    // update cn only if we are not unsetting display name(cn is required for
    // ZIMBRA_DEFAULT_PERSON_OC)
    SingleValueMod mod = singleValueMod(attrName, value);
    if (mod.unsetting()) return;
    else displayName = mod.value();
    ...
    if (namingRdnAttr == null
        || // non LdapProvisioning, pass thru
