...
public class DataSource extends AccountProperty {
  ...
  private void initKnownService() {
    ...
    // Google Apps domain could be anything, but host is always imap.gmail.com or
    // imap.googlemail.com
    if (host != null && (host.endsWith(".gmail.com") || host.endsWith(".googlemail.com")))
      serviceName = "gmail.com";
    else serviceName = getDomain();

    knownService =
        serviceName == null ? null : DataSourceManager.getConfig().getService(serviceName);
  }
  ...
  /**
   * Returns true if this data source is currently being managed by DataSourceManager. Can be used
   * to check if data source or account has been removed while importing so we can abort sync.
   *
  ...
  public boolean isEnabled() {
    return getBooleanAttr(Provisioning.A_zimbraDataSourceEnabled, false);
  }
  ...
  public int getFolderId() {
    return getIntAttr(Provisioning.A_zimbraDataSourceFolderId, -1);
  }

  public String getHost() {
    return getAttr(Provisioning.A_zimbraDataSourceHost);
  }

  public String getUsername() {
    return getAttr(Provisioning.A_zimbraDataSourceUsername);
  }

  public String getAuthId() {
    return getAttr(Provisioning.A_zimbraDataSourceAuthorizationId);
  }

  public String getAuthMechanism() {
    return getAttr(Provisioning.A_zimbraDataSourceAuthMechanism);
  }

  public String getOauthRefreshTokenUrl() {
    return getAttr(Provisioning.A_zimbraDataSourceOAuthRefreshTokenUrl);
  }

  public String getOauthClientId() {
    return getAttr(Provisioning.A_zimbraDataSourceOAuthClientId);
  }

  public String getOauthRefreshToken() {
    return getAttr(Provisioning.A_zimbraDataSourceOAuthRefreshToken);
  }

  public String getDataSourceImportClassName() {
    return getAttr(Provisioning.A_zimbraDataSourceImportClassName);
  }
  ...
  /**
   * Returns the polling interval in milliseconds. If <tt>zimbraDataSourcePollingInterval</tt> is
   * not specified on the data source, uses the value set for the account. If not set on either the
   * data source or account, returns <tt>0</tt>.
   */
  ...
  private void migratePollingIntervalIfNecessary(Provisioning prov, Account account)
    ...
    if (!StringUtil.isNullOrEmpty(oldInterval)) {
      ...
      prov.modifyAttrs(
          account, attrs, true, false); // Don't run callback so we don't trigger database code.
    }
    ...
    if (!StringUtil.isNullOrEmpty(oldInterval)) {
      ...
      attrs.put(Provisioning.A_zimbraDataSourceImapPollingInterval, oldInterval);
      prov.modifyAttrs(
          cos, attrs, true, false); // Don't run callback so we don't trigger database code.
    }
  }
  ...
  /**
   * Should POP3 messages be left on the server or deleted? Default is <code>true</code> for data
   * sources created before the leave on server feature was implemented.
   */
  ...
  public boolean isDebugTraceEnabled() {
    return isRequestScopeDebugTraceOn
        || getBooleanAttr(Provisioning.A_zimbraDataSourceEnableTrace, false);
  }
  ...
  private static Cipher getCipher(String dataSourceId, byte[] salt, boolean encrypt)
  ...
  public static byte[] decryptData(String dataSourceId, byte[] data) throws ServiceException {
    try {
      ...
      return cipher.doFinal(
          encoded,
          VERSION.length + SALT_SIZE_BYTES,
          encoded.length - SALT_SIZE_BYTES - VERSION.length);
    } catch (UnsupportedEncodingException e) {
      throw ServiceException.FAILURE("caught unsupport encoding exception", e);
    } catch (GeneralSecurityException e) {
      throw ServiceException.FAILURE("caught security exception", e);
  ...
  public String getSmtpUsername() {
    ...
    return smtpUsername == null
        ? isSmtpEnabled() && isSmtpAuthRequired() ? getUsername() : null
        : smtpUsername;
  }
  ...
  public String getDecryptedSmtpPassword() throws ServiceException {
    ...
    return smtpPass == null
        ? isSmtpEnabled() && isSmtpAuthRequired() ? getDecryptedPassword() : null
  ...
  public int getImapTrashFolderId() {
    return getIntAttr(Provisioning.A_zimbraDataSourceImapTrashFolderId, -1);
  }
}
