...
public abstract class ChangePasswordListener {
  ...
  public static void registerInternal(
      InternalChangePasswordListenerId listenerEnum, ChangePasswordListener listener) {
    //  sanity check
  ...
  private static ChangePasswordListener getHandler(Account acct) throws ServiceException {
    ...
    if (domain == null) throw AccountServiceException.NO_SUCH_DOMAIN(acct.getDomainName());

    String listenerName = domain.getAttr(Provisioning.A_zimbraPasswordChangeListener);

    if (listenerName == null) return null;
  ...
  public static void invokePreModify(
      Account acct,
      String newPassword,
      ChangePasswordListenerContext ctxts,
      Map<String, Object> attrsToModify)
      throws ServiceException {

    // invoke internal listeners
    for (Map.Entry<InternalChangePasswordListenerId, ChangePasswordListener> listener :
  ...
  public static void invokePostModify(
      Account acct, String newPassword, ChangePasswordListenerContext ctxts) {
    // invoke internal listeners
    for (Map.Entry<InternalChangePasswordListenerId, ChangePasswordListener> listener :
        mInternalListeners.entrySet()) {
      InternalChangePasswordListenerId listenerEnum = listener.getKey();
  ...
  public static void invokeOnException(
      Account acct,
      String newPassword,
      ChangePasswordListenerContext ctxts,
      ServiceException exceptionThrown) {
  ...
  /**
   * Called before password(userPassword) and applicable(e.g. zimbraPasswordHistory,
   * zimbraPasswordModifiedTime) attributes are modified in LDAP. If a ServiceException is thrown,
   * no attributes will be modified.
   *
   * <p>The attrsToModify map should not be modified, other then for adding attributes defined in a
  ...
  public abstract void preModify(
      Account acct, String newPassword, Map context, Map<String, Object> attrsToModify)
      throws ServiceException;
  ...
  /**
   * called when modifyAttrs() throws ServiceException. After onException() get executed for all
   * internal listeners, onException() for external listener is called. should not throw any
   * exceptions for preventing to skip onException() for external listener. The exception instance
   * which modifyAttrs() originally throws is rethrown after onException() gets executed.
   *
   * @param USER_ACCOUNT account object being modified
   * @param newPassword Clear-text new password
   * @param context place to stash data between invocations of pre/postModify
  ...
  public void onException(
      Account acct, String newPassword, Map context, ServiceException exceptionThrown) {}
  ...
  public static void main(String[] args) throws Exception {
    ...
    ChangePasswordListener.registerInternal(
        InternalChangePasswordListenerId.CPL_SYNC, new DummyChangePasswordListener());
