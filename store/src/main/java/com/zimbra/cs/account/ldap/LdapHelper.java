...
public abstract class LdapHelper {
  ...
  /**
   * Modifies the specified entry. <code>attrs</code> is a <code>Map</code> consisting of keys that
   * are <code>String</code>s, and values that are either
   *
   * <ul>
  ...
  public abstract void modifyAttrs(
      ZLdapContext zlc, String dn, Map<String, ? extends Object> attrs, Entry entry)
      throws ServiceException;

  /**
   * like modifyAttrs, but ZLdapContext cannot be specified. Implementation should always do the
   * modification against LDAP master.
   *
   * @param dn
   * @param attrs
   * @param entry
  ...
  public abstract void modifyEntry(
      String dn, Map<String, ? extends Object> attrs, Entry entry, LdapUsage ldapUsage)
      throws ServiceException;
   ...
   * @param entry
   * @param ldapUsage
   * @returns true if the filter matches the target entry and the entry is successfully modified.
   *     false if the filter does not match the target entry
   * @throws ServiceException
   */
  public abstract boolean testAndModifyEntry(
      ZLdapContext zlc,
      String dn,
      ZLdapFilter testFilter,
      Map<String, ? extends Object> attrs,
      Entry entry)
      throws ServiceException;

  /**
   * Search for an entry by search base and query. At most one entry will be returned from the
   * search.
   *
   * @param base search base
   * @param filter search filter
   * @param initZlc initial ZLdapContext - if null, a new one will be created to be used for the
   *     search, and then closed - if not null, it will be used for the search, this API will *not*
   *     close it, it is the responsibility of callsite to close it when it i no longer needed.
   * @param useMaster if initZlc is null, whether to do the search on LDAP master.
   * @return a ZSearchResultEnumeration is an entry is found null if the search does not find any
   *     matching entry.
   * @throws LdapMultipleEntriesMatchedException if more than one entries is matched
   * @throws ServiceException all other errors
   */
  public abstract ZSearchResultEntry searchForEntry(
      String base,
      ZLdapFilter filter,
  ...
  public ZSearchResultEntry searchForEntry(
      String base, ZLdapFilter filter, ZLdapContext initZlc, boolean useMaster)
      throws LdapMultipleEntriesMatchedException, ServiceException {
    return searchForEntry(base, filter, initZlc, useMaster, null);
  }

  public ZSearchResultEntry searchForEntry(
      String base, ZLdapFilter filter, ZLdapContext initZlc, String[] returnAttrs)
      throws LdapMultipleEntriesMatchedException, ServiceException {
    if (initZlc == null) {
  ...
  public ZSearchResultEntry searchForEntry(
      String base, FilterId filterId, String filter, ZLdapContext initZlc, String[] returnAttrs)
      throws LdapMultipleEntriesMatchedException, ServiceException {
   ...
   * @param attrs
   * @return a ZAttributes objects Note: this API never returns null. If an entry is not found at
   *     the specified DN, LdapEntryNotFoundException will be thrown.
  ...
  public ZAttributes getAttributes(LdapUsage usage, String dn)
      throws LdapEntryNotFoundException, ServiceException {
    return getAttributes((ZLdapContext) null, LdapServerType.REPLICA, usage, dn, (String[]) null);
  }

  /**
   * A convenient wrapper for ZldapContext.searchDir. Saves callsites the burden of having to get
   * and close ZldapContext
   *
   * @param baseDN
   * @param filter
  ...
  public ZSearchResultEnumeration searchDir(
      String baseDN, ZLdapFilter filter, ZSearchControls searchControls) throws ServiceException {
   ...
   * @throws ServiceException
   */
  protected abstract long countEntriesByNoopSearch(
      String baseDN,
      ZLdapFilter filter,
      ZSearchControls searchControls,
      ZLdapContext initZlc,
