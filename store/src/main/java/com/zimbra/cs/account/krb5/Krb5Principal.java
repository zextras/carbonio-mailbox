...
public class Krb5Principal {
  ...
  public static Account getAccountFromKrb5Principal(String principal, boolean loadFromMaster)
    ...
    try {
      acct =
          prov.get(
              AccountBy.foreignPrincipal,
              Provisioning.FP_PREFIX_KERBEROS5 + principal,
              loadFromMaster);
  ...
  public static String getKrb5Principal(Domain domain, Account acct) {
    ...
    if (principal == null) {
      ...
      if (realm != null) {
        ...
        if (parts != null) principal = parts[0] + "@" + realm;
        else principal = acct.getName() + "@" + realm; // just use whatever in the name
      }
    }
