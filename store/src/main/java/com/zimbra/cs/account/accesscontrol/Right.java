...
public abstract class Right extends RightConsts implements Comparable<Right> {
  ...
  /**
   * - right name stored in zimbraACE. - right name appear in XML - right name displayed by CLI
   *
   * @return
   */
  ...
  void setGrantTargetType(TargetType targetType) throws ServiceException {
    if (!isUserRight()) {
      throw ServiceException.PARSE_ERROR(
          "grant target type is only supported on user rights", null);
    }

    if (!grantableOnTargetType(targetType)) {
      throw ServiceException.PARSE_ERROR(
          String.format(
              "invalid grant target type: %s, valid grant target type for right %s are: %s",
              targetType.getCode(),
              getName(),
  ...
  void completeRight() throws ServiceException {
    if (getDesc() == null) throw ServiceException.PARSE_ERROR("missing description", null);
    verifyTargetType();
  }
