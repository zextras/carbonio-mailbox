...
public class GroupACLs {
  ...
  GroupACLs(Entry target) throws ServiceException {
    if (target instanceof Account) {
      ...
      directGroupsOfAccountTarget =
          Provisioning.getInstance().getDirectDistributionLists(acctTarget);
  ...
  void collectACL(Group grantedOn, boolean skipPositiveGrants) throws ServiceException {

    Set<ZimbraACE> allowedNotDelegable = ACLHelper.getAllowedNotDelegableACEs(grantedOn);
    Set<ZimbraACE> allowedDelegable = ACLHelper.getAllowedDelegableACEs(grantedOn);
    Set<ZimbraACE> denied = ACLHelper.getDeniedACEs(grantedOn);
