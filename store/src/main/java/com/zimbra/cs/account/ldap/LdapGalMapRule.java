...
public class LdapGalMapRule {
  ...
  public LdapGalMapRule(String rule, Map<String, LdapGalValueMap> valueMaps) {
    ...
    if (matcher.matches()) {
      ...
      if (attrType == null) {
        ZimbraLog.gal.warn(
            "Unrecognized type in attr map: " + type + ", type is ignore for rule " + rule);
      } else {
    ...
    if (p != -1) {
      ...
      mContactAttrs =
          (contactAttr.indexOf(',') != -1) ? contactAttr.split(",") : new String[] {contactAttr};
  ...
  private Object mapValue(int index, Object value) {
    ...
    if (valueMap != null) return valueMap.apply(value);
    else return value;
  }
