...
public class HostedAuth extends ZimbraCustomAuth {
   ...
   *
   * <p>This custom auth module takes arguments in the following form: {URL} [GET|POST - default is
   * GET] [encryption method - defautl is plain] [auth protocol - default is imap] e.g.:
   * http://auth.customer.com:80 GET
   */
  public void authenticate(
    ...
    if (args.size() > 1) {
      if (args.get(1).equalsIgnoreCase("GET")) method = new HttpGet(targetURL);
      else method = new HttpPost(targetURL);
    } else method = new HttpGet(targetURL);

    if (context.get(AuthContext.AC_ORIGINATING_CLIENT_IP) != null)
      method.addHeader(
          HEADER_CLIENT_IP, context.get(AuthContext.AC_ORIGINATING_CLIENT_IP).toString());

    if (context.get(AuthContext.AC_REMOTE_IP) != null)
      method.addHeader(
          HEADER_X_ZIMBRA_REMOTE_ADDR, context.get(AuthContext.AC_REMOTE_IP).toString());
    ...
    if (proto != null) method.addHeader(HEADER_AUTH_PROTOCOL, proto.toString());
    ...
    try {
      ...
      throw AuthFailedServiceException.AUTH_FAILED(
          acct.getName(),
          acct.getName(),
          "HTTP request to remote authentication server failed",
          ex);
    ...
    if (response.getFirstHeader(HEADER_AUTH_STATUS) != null) {
      ...
      throw AuthFailedServiceException.AUTH_FAILED(
          acct.getName(), "Empty response from remote authentication server.");
    }
