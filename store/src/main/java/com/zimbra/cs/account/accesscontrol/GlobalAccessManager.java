...
public class GlobalAccessManager extends AccessManager implements AdminConsoleCapable {
  ...
  public GlobalAccessManager() {
    try {
      ...
      ZimbraLog.acl.warn(
          "unable to instantiate ACLAccessManager, user rights will not be honored", e);
    }
  }
  ...
  public boolean canAccessAccount(AuthToken at, Account target, boolean asAdmin)
    ...
    if (isGlobalAdmin(at, asAdmin)) return true;
  ...
  public boolean canAccessAccount(Account credentials, Account target, boolean asAdmin)
    ...
    if (AccessControlUtil.isGlobalAdmin(credentials, asAdmin)) return true;
  ...
  public boolean canDo(AuthToken grantee, Entry target, Right rightNeeded, boolean asAdmin) {
    ...
    if (granteeAcct != null) return canDo(granteeAcct, target, rightNeeded, asAdmin);
    else return false;
  }
  ...
  public AttrRightChecker getGetAttrsChecker(Account credentials, Entry target, boolean asAdmin)
    ...
    else return AllowedAttrs.DENY_ALL_ATTRS();
  }
