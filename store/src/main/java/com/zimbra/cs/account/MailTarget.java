  ...
  public String getDomainId() {
    ...
    if (domainId == null) {
      try {
        ...
        Domain domain =
            dname == null ? null : (prov == null ? null : prov.get(Key.DomainBy.name, dname));
      ...
      if (domainId == null)
        domainId =
            NO_DOMAIN_ID; // set it to a non-null string so we don't repeatedly look up the domain

      setCachedData(EntryCacheDataKey.MAILTARGET_DOMAIN_ID, domainId);
    }

    // OK and more efficient to use == instead of equal
    if (domainId == NO_DOMAIN_ID) return null;
    else return domainId;
  }
}
