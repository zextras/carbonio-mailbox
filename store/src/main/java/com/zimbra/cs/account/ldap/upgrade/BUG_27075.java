...
public class BUG_27075 extends UpgradeOp {
  ...
  private boolean needsUpgrade(AttributeManager am, String attr) throws ServiceException {
    ...
    if (!am.beforeVersion(attr, since) && !am.isFuture(attr)) return true;
    ...
    if (am.addedIn(attr, "5.0.17")) {
      boolean fromATroubledInstall =
          (mSince.compare("6.0.0_BETA1") == 0 || mSince.compare("6.0.0_BETA2") == 0);
    ...
    for (String attr : attrs) {
      ...
      if (needsUpgrade(am, attr)) {
        ...
        if (klass == AttributeClass.globalConfig) {
          ...
          if (values == null) values = ai.getGlobalConfigValues();
        } else if (klass == AttributeClass.cos) {
          values = ai.getDefaultCosValuesUpgrade();
          if (values == null) values = ai.getDefaultCosValues();
        } else {
        ...
        try {
          ...
          printer.println(
              "Caught ServiceException while modifying " + entryName + " attribute " + attr);
    ...
    if (sinceVer.startsWith("6.0.0_BETA1") && thisVer.startsWith("6.0.0_BETA2")) {
      ...
      if ("UTF-8".equalsIgnoreCase(curVal)) {
        ...
        try {
          ...
          printer.println(
              "Caught ServiceException while unsetting " + theAttr + " on " + entryName);
          printer.printStackTrace(e);
        }
      }
    }
  ...
  private void doBug79208(Config config) throws ServiceException {
    ...
    boolean stepRequired =
        mSince.compare("7.2.3") < 0
    ...
    // 7.2.2->7.2.3, 7.2.3->8.0.0 and then 8.0.0->8.0.3. The 7.2.2->7.2.3 would have already added
    // the attribute.
    HashMap<String, Object> attrs = new HashMap<String, Object>();
    ...
    if (!attrs.isEmpty()) {
      try {
        ...
        printer.println(
            "Caught ServiceException while setting " + attrs.keySet() + " on globalConfig");
        printer.printStackTrace(e);
      }
    }
  }
  ...
  private void doBug83551(Config config) throws ServiceException {
    ...
    boolean stepRequired =
        mSince.compare("7.2.5") < 0
            || (mSince.compare("8.0.0") >= 0 && mSince.compare("8.0.5") < 0);
    if (!stepRequired) {
      return;
    }
    // check if the attribute already exists. It's possible that server may have been upgraded from
    // 7.2.2->7.2.5, 7.2.5->8.0.0 and then 8.0.0->8.0.5. The 7.2.2->7.2.5 would have already added
    // the attribute.
    HashMap<String, Object> attrs = new HashMap<String, Object>();
