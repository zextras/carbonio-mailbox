...
public class NameUtil {
  ...
  private static boolean containsNonLDH(String input) {
    for (int i = 0; i < input.length(); i++) {
      ...
      //     \u002E (full stop), \u3002 (ideographic full stop), \uFF0E (fullwidth full stop), and
  ...
  public static void validNewDomainName(String domain) throws ServiceException {
    ...
    boolean allowNonLDH =
        config.getBooleanAttr(Provisioning.A_zimbraAllowNonLDHCharsInDomain, true);
    if (!allowNonLDH && containsNonLDH(domain))
      throw ServiceException.INVALID_REQUEST(
          "invalid domain name "
              ...
              + Provisioning.A_zimbraAllowNonLDHCharsInDomain
              + " is false",
          null);
  }
