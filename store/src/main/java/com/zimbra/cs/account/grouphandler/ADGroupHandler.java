...
public class ADGroupHandler extends GroupHandler {
  ...
  private static boolean domainAdminAuthMechIsAD(Domain domain, boolean asAdmin) {
    return asAdmin
        ? AuthMech.ad.name().equals(domain.getAuthMechAdmin())
   ...
   */
  private boolean legitimateDelegatedAdminAsGroupMember(
      ExternalGroup group, Account acct, boolean asAdmin) throws ServiceException {
  ...
  public boolean inDelegatedAdminGroup(ExternalGroup group, Account acct, boolean asAdmin)
    ...
    List<String> groupDNs =
        (List<String>) acct.getCachedData(EntryCacheDataKey.GROUPEDENTRY_EXTERNAL_GROUP_DNS);
  ...
  private List<String> getDelegatedAdminGroups(Account acct, boolean asAdmin)
      throws ServiceException {
    ...
    if (extDN == null) {
      throw ServiceException.FAILURE(
          "unable to get external DN for account " + acct.getName(), null);
    }
