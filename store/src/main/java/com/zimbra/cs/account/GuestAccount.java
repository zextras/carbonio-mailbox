...
public class GuestAccount extends Account {
  ...
  public GuestAccount(AuthToken auth) {
    ...
    super(
        auth.getExternalUserEmail() == null ? "" : auth.getExternalUserEmail(),
  ...
  public boolean matches(String emailAddress, String password) {
    if (getName().compareToIgnoreCase(emailAddress) != 0) return false;
    String matchDigest =
        AuthToken.generateDigest(
            getName() /* use getName() instead of emailAddress so that case doesn't affect result */,
            password);
