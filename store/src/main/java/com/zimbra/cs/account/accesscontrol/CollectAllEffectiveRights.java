...
public class CollectAllEffectiveRights {
  ...
  public static class GroupShape {
    ...
    public static void shapeMembers(
        TargetType targetType, Set<GroupShape> shapes, AllGroupMembers group)
  ...
  public static class AllGroupMembers {
    ...
    Set<String> mCalendarResources =
        new HashSet<String>(); // all calendar resource memners of the group
    Set<String> mDistributionLists =
        new HashSet<String>(); // all distribution list members of the group
  ...
  static void getAllEffectiveRights(
  ...
  public CollectAllEffectiveRights(
      RightBearer rightBearer,
      boolean expandSetAttrs,
      boolean expandGetAttrs,
      AllEffectiveRights result)
  ...
  private void collect() throws ServiceException {
    ...
    if (mRightBearer instanceof GlobalAdmin) {
      for (TargetType tt : TargetType.values()) {
        EffectiveRights er =
            new EffectiveRights(
                tt.getCode(), null, null, mRightBearer.getId(), mRightBearer.getName());
        ...
        CollectEffectiveRights.getEffectiveRights(
            mRightBearer, target, tt, mExpandSetAttrs, mExpandGetAttrs, er);
    ...
    for (Group group : groupsWithGrants) {
      ...
      if (group.isDynamic()) {
        ...
        if (dg.isMembershipDefinedByCustomURL()) {
          // cannot search members of dynamic group with custom memberURL, so do not process this
          // group.
          ZimbraLog.acl.debug(
              "Dynamic group with custom memberURL found '%s', ignoring further process on same.",
              dg.getName());
          continue;
        }
      }
      ...
      for (String nestedGoupMember : allMembers.getMembers(TargetType.dl)) {
        ...
        if (subDl != null) {
          ...
          GroupShape.shapeMembers(
              TargetType.calresource, calendarResourceShapes, allMembersOfSubDl);
  ...
  private void getAllGroupMembers(
      Group group, Set<String> allGroups, Set<String> allCalendarResources, AllGroupMembers result)
      throws ServiceException {

    Set<String> members = group.getAllMembersSet();
    Set<String> accountMembers =
        new HashSet<String>(members); // make a copy, assuming all members are account
  ...
  private Set<String> getAllGroups() throws ServiceException {
    ...
    // hack, see LDAPDIT.dnToEmail, for now we get naming rdn for both default and possible custom
    // DIT
  ...
  private void computeRightsInheritedFromGlobalGrant() throws ServiceException {
    ...
    for (TargetType tt : TargetType.values()) {
      ...
      CollectEffectiveRights.getEffectiveRights(
          mGrantee, targetEntry, mExpandSetAttrs, mExpandGetAttrs, er);
  ...
  private void computeRightsInheritedFromDomain(TargetType targetType, Domain grantedOnDomain)
    ...
    Entry pseudoTarget =
        PseudoTarget.createPseudoTarget(
            mProv, targetType, DomainBy.id, grantedOnDomain.getId(), false, null, null);

    // get effective rights on the pseudo target
    EffectiveRights er =
        new EffectiveRights(
            targetType.getCode(),
            TargetType.getId(pseudoTarget),
            pseudoTarget.getLabel(),
            mGrantee.getId(),
            mGrantee.getName());
    CollectEffectiveRights.getEffectiveRights(
        mGrantee, pseudoTarget, mExpandSetAttrs, mExpandGetAttrs, er);
  ...
  private void computeSubDomainRightsInheritedFromDomain(ZimbraACL acl, Domain grantedOnDomain)
    ...
    for (Domain subDomain : subDomains) {
      ...
      CollectEffectiveRights.getEffectiveRights(
          mGrantee, subDomain, mExpandSetAttrs, mExpandGetAttrs, er);
  ...
  private List<Domain> searchSubDomains(Domain domain) throws ServiceException {
    ...
    for (String zimbraId : zimbraIds) {
      ...
      try {
        Domain subDomain =
            (Domain) TargetType.lookupTarget(mProv, TargetType.domain, TargetBy.id, zimbraId);
   ...
   */
  private void computeRightsOnGroupShape(
      TargetType targetType, Set<GroupShape> groupShapes, Set<String> entryIdsHasGrants)
      throws ServiceException {
          ...
          if (!entryIdsHasGrants.contains(targetId)) {
            er =
                new EffectiveRights(
                    ...
                    mGrantee.getName());
            CollectEffectiveRights.getEffectiveRights(
                mGrantee, target, mExpandSetAttrs, mExpandGetAttrs, er);
            break;
  ...
  private void computeRightsOnEntry(TargetType grantedOnTargetType, Entry grantedOnEntry)
    ...
    CollectEffectiveRights.getEffectiveRights(
        mGrantee, grantedOnEntry, mExpandSetAttrs, mExpandGetAttrs, er);
  ...
  private static void setupShapeTest1() throws ServiceException {
    ...
    DistributionList groupA =
        prov.createDistributionList("groupA@" + domainName, new HashMap<String, Object>());
    DistributionList groupB =
        prov.createDistributionList("groupB@" + domainName, new HashMap<String, Object>());
    DistributionList groupC =
        prov.createDistributionList("groupC@" + domainName, new HashMap<String, Object>());
    DistributionList groupD =
        prov.createDistributionList("groupD@" + domainName, new HashMap<String, Object>());
