...
public class ACLAccessManager extends AccessManager implements AdminConsoleCapable {
  ...
  public boolean canAccessAccount(AuthToken at, Account target, boolean asAdmin)
    ...
    if (asAdmin) {
      return canDo(
          at, actualTargetForAdminLoginAs(target), actualRightForAdminLoginAs(target), asAdmin);
    } else {
  ...
  public boolean canAccessAccount(Account credentials, Account target, boolean asAdmin)
    ...
    if (asAdmin) {
      return canDo(
          credentials,
          actualTargetForAdminLoginAs(target),
          actualRightForAdminLoginAs(target),
          asAdmin);
    } else {
  ...
  @Override
  public boolean canCreateGroup(AuthToken at, String groupEmail) throws ServiceException {
  ...
  @Override
  public boolean canCreateGroup(Account credentials, String groupEmail) throws ServiceException {
    Domain domain = Provisioning.getInstance().getDomainByEmailAddr(groupEmail);
    checkDomainStatus(domain);
  ...
  public boolean canDo(
      MailTarget grantee, Entry target, Right rightNeeded, boolean asAdmin, ViaGrant via)
      throws ServiceException {
  ...
  @Override
  public boolean canDo(
      AuthToken grantee, Entry target, Right rightNeeded, boolean asAdmin, ViaGrant via)
      throws ServiceException {
    try {
  ...
  @Override
  public boolean canDo(
      String granteeEmail, Entry target, Right rightNeeded, boolean asAdmin, ViaGrant via)
      throws ServiceException {
    try {
  ...
  @Override
  public boolean canGetAttrs(
      Account grantee, Entry target, Set<String> attrsNeeded, boolean asAdmin)
      throws ServiceException {
  ...
  public AttrRightChecker getGetAttrsChecker(Account credentials, Entry target, boolean asAdmin)
    ...
    if (hardRulesResult == Boolean.TRUE) {
      ...
      AttrRightChecker rightChecker =
          CheckAttrRight.accessibleAttrs(grantee, target, AdminRight.PR_GET_ATTRS, false);
      return rightChecker;
    }
  }
  ...
  public boolean canSetAttrs(
      Account granteeAcct, Entry target, Map<String, Object> attrsNeeded, boolean asAdmin)
      throws ServiceException {
    ...
    AllowedAttrs allowedAttrs =
        CheckAttrRight.accessibleAttrs(grantee, target, AdminRight.PR_SET_ATTRS, false);
    return allowedAttrs.canSetAttrsWithinConstraints(grantee, target, attrsNeeded);
  }

  @Override
  public boolean canSetAttrs(
      AuthToken grantee, Entry target, Map<String, Object> attrs, boolean asAdmin)
      throws ServiceException {
    return canSetAttrs(grantee.getAccount(), target, attrs, asAdmin);
  }
  ...
  public boolean canPerform(
    ...
    if (rightNeeded.isPresetRight()) {
      allowed =
          checkPresetRight(grantee, target, rightNeeded, canDelegateNeeded, asAdmin, viaGrant);
  ...
  /**
   * Check for cases where the grantee has capabilities that automatically mean they should be able
   * to perform the grant. This is useful for allowing domain admins to do certain things
   *
   * @return true if this grantee should be allowed the right.
   */
  private boolean checkOverridingRules(
  ...
  /**
   * Bug 88604 This allows delegate admins to assign user rights without having to be given the
   * right they want to assign. Currently restricted to just user rights as those seem to be what a
   * delegate admin might want to assign
   */
  private boolean domainAdminAllowedToChangeUserRight(
      MailTarget grantee, boolean asAdmin, Entry target, Right rightNeeded) {
    if (!asAdmin
        || !rightNeeded.isUserRight()
        || !(grantee instanceof Account)
    ...
    try {
      ...
      if (canSetAttrs(authedAcct, target, attrsNeeded, asAdmin)) {
        ZimbraLog.acl.debug(
            "Right [%s] ALLOWED to '%s' for target '%s' because '%s' is allowed to set '%s' for"
                + " '%s'",
            rightNeeded.getName(),
            authedAcct.getName(),
            mailTarget.getName(),
  ...
  /**
   * Check whether this is a domain admin assigning an owner to a DL. If so, then having createGroup
   * / createDistributionList caps for the target DL should be sufficient as specifying an owner can
   * be considered to be part of the creation process. (Note - the equivalent modify caps are not
   * preset rights so "canDo" can't be used on them, hence sticking with the create caps)
   *
   * <p>This is needed because there is a bootstrapping problem for assigning owners to DLs created
   * by a domain admin. Without this over-ride, only a full admin or someone who already has
   * ownDistList rights could assign owners. Also, it isn't possible to assign ownDistList rights to
   * domain admins for all DLs in a domain because the only valid target for ownDistList is a DL.
   *
   * @return true if this grantee should be allowed the right.
   */
  private boolean checkForDomainAdminAssigningDLowner(
    ...
    if ((grantee instanceof Account) && (target instanceof Group)) {
      ...
      try {
        ...
        checkDomainStatus(domain);
        Right alternativeRight =
            group.isDynamic() ? Admin.R_createGroup : Admin.R_createDistributionList;
        if (canDo(authedAcct, domain, alternativeRight, true, null)) {
          ZimbraLog.acl.debug(
              "Right [%s] ALLOWED to '%s' for Group '%s' because %s is allowed right [%s] for"
                  + " domain '%s'",
              rightNeeded.getName(),
              authedAcct.getName(),
              group.getName(),
              authedAcct.getName(),
              alternativeRight.getName(),
              domain.getName());
          return true;
        }
      } catch (ServiceException e) {
  ...
  private boolean checkAttrRight(
      Account grantee,
      Entry target,
      AttrRight rightNeeded,
      boolean canDelegateNeeded,
    ...
    if (rightNeeded.getRightType() == Right.RightType.getAttrs) {
      ...
      if (attrs == null || attrs.isEmpty()) {
        // no attr/value map, just check if all attrs in the right are covered (constraints are not
        // checked)
        allowed = checkAttrRight(grantee, target, rightNeeded, canDelegateNeeded);
      } else {
  ...
  private boolean checkAttrRight(
      Account granteeAcct, Entry target, AttrRight rightNeeded, boolean canDelegateNeeded)
      throws ServiceException {
    AllowedAttrs allowedAttrs =
        CheckAttrRight.accessibleAttrs(
            Grantee.getGrantee(granteeAcct), target, rightNeeded, canDelegateNeeded);
    return allowedAttrs.canAccessAttrs(rightNeeded.getAttrs(), target);
  }

  private boolean canGetAttrsInternal(
      Account granteeAcct, Entry target, Set<String> attrsNeeded, boolean canDelegateNeeded)
      throws ServiceException {
    AllowedAttrs allowedAttrs =
        CheckAttrRight.accessibleAttrs(
            Grantee.getGrantee(granteeAcct), target, AdminRight.PR_GET_ATTRS, canDelegateNeeded);

    return allowedAttrs.canAccessAttrs(attrsNeeded, target);
  }

  private boolean canSetAttrsInternal(
      Account granteeAcct, Entry target, Set<String> attrsNeeded, boolean canDelegateNeeded)
      throws ServiceException {
    AllowedAttrs allowedAttrs =
        CheckAttrRight.accessibleAttrs(
            Grantee.getGrantee(granteeAcct), target, AdminRight.PR_SET_ATTRS, canDelegateNeeded);
    return allowedAttrs.canAccessAttrs(attrsNeeded, target);
  }
  ...
  public Set<TargetType> targetTypesForGrantSearch() {
    ...
    return new HashSet<>(Arrays.asList(TargetType.valuesWithoutXmppComponent()));
  }
}
