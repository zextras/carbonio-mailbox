...
public class PermissionCache {
   ...
   *
   * <p>In addition to just invalidating the permission cache, we also need to invalidate account,
   * aclgroup, domain, and globalgrant LDAP entry caches where the ACL is cached. On mailbox server
   * those caches are automatically updated when the modification (grant, revoke, member changes,
   * group/account/domain creation/deletion etc) happens.
   */
  ...
  static CachedPermission cacheGet(
      MailTarget grantee, Entry target, Right rightNeeded, boolean canDelegateNeeded) {
    if (!cacheEnabled) {
  ...
  static void cachePut(
    ...
    CachedPermission perm =
        (allowed == null)
            ? CachedPermission.NO_MATCHING_ACL
  ...
  static String buildCacheKey(MailTarget grantee, Right rightNeeded, boolean canDelegateNeeded) {
    ...
    if (!rightNeeded.isCacheable()) return null;
    ...
    //
    if (!rightNeeded.isUserRight() && Admin.R_adminLoginAs != rightNeeded) return null;
    ...
    if (grantee instanceof Account) {
      ...
      adminFlag =
          granteeAcct.isIsAdminAccount()
              ? '2'
              : granteeAcct.isIsDelegatedAdminAccount() ? '1' : '0';
    } else {
      adminFlag = '0';
    }
