...
public class AttributeManagerUtil {
  ...
  private String doNotModifyDisclaimer(String prefix) {
    return FileGenUtil.genDoNotModifyDisclaimer(
        prefix, AttributeManagerUtil.class.getSimpleName(), CLOptions.buildVersion());
  }
  ...
  private void sortOCsByOID(List<ObjectClassInfo> list) {
    Collections.sort(
        list,
        new Comparator<ObjectClassInfo>() {
          @Override
  ...
  private void buildAttrDef(StringBuilder ATTRIBUTE_DEFINITIONS, AttributeInfo ai) {
    ...
    ATTRIBUTE_DEFINITIONS.append(
        ML_CONT_PREFIX + "DESC '" + rfc4512Dstring(ai.getDescription()) + "'\n");
  ...
  private void buildObjectClassOIDs(
    ...
    for (Iterator<Integer> iter = getOCGroupMap().keySet().iterator(); iter.hasNext(); ) {
      ...
      for (ObjectClassInfo oci : list) {
        OC_OIDS.append(
            prefix + oci.getName() + " " + getOCGroupMap().get(i) + ':' + oci.getId() + "\n");
      }
    }
  }
  ...
  private void buildObjectClassDefs(
    ...
    for (AttributeClass cls : AttributeClass.values()) {
      ...
      if (oci == null) continue; // oc not defined in xml, skip

      // OC_DEFINITIONS:
      List<String> comment = oci.getComment();
      OC_DEFINITIONS.append("#\n");
      for (String line : comment) {
        if (line.length() > 0) OC_DEFINITIONS.append("# " + line + "\n");
      ...
      OC_DEFINITIONS.append(
          ML_CONT_PREFIX + "DESC '" + rfc4512Dstring(oci.getDescription()) + "'\n");
      OC_DEFINITIONS.append(ML_CONT_PREFIX + "SUP ");
      for (String sup : oci.getSuperOCs()) OC_DEFINITIONS.append(sup);
    ...
    for (Iterator<Integer> iter = getGroupMap().keySet().iterator(); iter.hasNext(); ) {
      ...
      for (AttributeInfo ai : list) {
        ...
        if (parentOid == null)
          ...
          ATTRIBUTE_OIDS.append(
              "objectIdentifier " + ai.getName() + " " + parentOid + "." + ai.getId() + "\n");
      }
  ...
  private void generateSchemaLdif(PrintWriter pw) {
    ...
    for (Iterator<Integer> iter = getGroupMap().keySet().iterator(); iter.hasNext(); ) {
      ...
      // GROUP_OIDS
      ATTRIBUTE_GROUP_OIDS.append(
          "olcObjectIdentifier: " + getGroupMap().get(i) + " ZimbraLDAP:" + i + "\n");
      ...
      for (AttributeInfo ai : list) {
        ...
        if (parentOid == null)
          ...
          ATTRIBUTE_OIDS.append(
              "olcObjectIdentifier: " + ai.getName() + " " + parentOid + "." + ai.getId() + "\n");
      }
  ...
  private void listAttrs(PrintWriter pw, String[] inClass, String[] notInClass, String[] printFlags)
      throws ServiceException {
    if (inClass == null) usage("no class specified");
  ...
  private static String defaultValue(AttributeInfo ai, AttributeClass ac) {
    ...
    switch (ac) {
      ...
      case cos:
        ...
        break;
      case domain:
        if (ai.hasFlag(AttributeFlag.domainInherited)) values = ai.getGlobalConfigValues();
        else return null;
        break;
      case server:
        if (ai.hasFlag(AttributeFlag.serverInherited)) values = ai.getGlobalConfigValues();
        else return null;
        break;
      case globalConfig:
    ...
    switch (type) {
      case TYPE_BOOLEAN:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "return getEphemeralAttr(Provisioning.A_%s, %s).getBoolValue(%s);",
                  name, dynamic, defaultValue);
        } else {
          javaBody =
              String.format(
                  "return getBooleanAttr(Provisioning.A_%s, %s, true);", name, defaultValue);
        }
      ...
      case TYPE_CERTIFICATE:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "String v = getEphemeralAttr(Provisioning.A_%s, %s).getValue(%s); return v =="
      ...
      case TYPE_INTEGER:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "return getEphemeralAttr(Provisioning.A_%s, %s).getIntValue(%s);",
                  name, dynamic, defaultValue);
        } else {
          javaBody =
              String.format("return getIntAttr(Provisioning.A_%s, %s, true);", name, defaultValue);
        }
        javaDocReturns = String.format(", or %s if unset", defaultValue);
        break;
      case TYPE_PORT:
      ...
      case TYPE_ENUM:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "try { String v = getEphemeralAttr(Provisioning.A_%s, %s).getValue(); return v =="
                      + " null ? %s : ZAttrProvisioning.%s.fromString(v); }"
                      + " catch(com.zimbra.common.service.ServiceException e) { return %s; }",
                  name, dynamic, defaultValue, enumName(ai), defaultValue);
        } else {
          javaBody =
              String.format(
                  "try { String v = getAttr(Provisioning.A_%s, true, true); return v == null ? %s :"
                      + " ZAttrProvisioning.%s.fromString(v); }"
                      + " catch(com.zimbra.common.service.ServiceException e) { return %s; }",
      ...
      case TYPE_LONG:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "return getEphemeralAttr(Provisioning.A_%s, %s).getLongValue(%sL);",
                  name, dynamic, defaultValue);
        } else {
          javaBody =
              String.format(
                  "return getLongAttr(Provisioning.A_%s, %sL, true);",
      ...
      case TYPE_DURATION:
        ...
        if (ai.isEphemeral()) {
          javaBody =
              String.format(
                  "return getEphemeralTimeInterval(Provisioning.A_%s, %s, %sL);",
                  name, dynamic, defaultValue);
        } else {
          javaBody =
              String.format(
                  "return getTimeInterval(Provisioning.A_%s, %sL, true);", name, defaultValue);
        }
        javaDocReturns =
            String.format(
      ...
      case TYPE_GENTIME:
        ...
        if (ai.isEphemeral()) {
          ...
          javaBody =
              String.format("return getGeneralizedTimeAttr(Provisioning.A_%s, null, true);", name);
        }
      ...
      default:
        if (ai.getCardinality() != AttributeCardinality.multi) {
          ...
          if (ai.isEphemeral()) {
            javaBody =
                String.format(
                    "return getEphemeralAttr(Provisioning.A_%s, %s).getValue(%s);",
                    name, dynamic, defaultValue);
          } else {
            javaBody =
                String.format("return getAttr(Provisioning.A_%s, %s, true);", name, defaultValue);
          }
          javaDocReturns = String.format(", or %s if unset", defaultValue);
        } else {
          if (ai.isEphemeral()) {
            ...
            if (defaultValue == null) {
              ...
              javaBody =
                  String.format(
                      "String[] value = getMultiAttr(Provisioning.A_%s, true, true); return"
  ...
  public static void generateSetter(
    ...
    switch (setterType) {
      case set:
        if (ai.isEphemeral()) {
          body =
              String.format(
                  "        modifyEphemeralAttr(Provisioning.A_%s, %s, %s, false, %s);%n",
      ...
      case add:
        if (ai.isEphemeral()) {
          body =
              String.format(
                  "        modifyEphemeralAttr(Provisioning.A_%s, %s, %s, true, %s);%n",
                  name, dynamic, putParam, expiry);
        } else {
          body =
              String.format(
                  "        StringUtil.addToMultiMap(attrs, \"+\" + Provisioning.A_%s, %s);%n",
                  name, name);
        }
      ...
      case remove:
        if (ai.isEphemeral()) {
          body =
              String.format(
                  "        deleteEphemeralAttr(Provisioning.A_%s, %s, %s);%n",
      ...
      case has:
        body =
            String.format(
                "        return hasEphemeralAttr(Provisioning.A_%s, %s);%n", name, dynamic);
        break;
      default:
        break;
    }
    ...
    if (noMap) {
      String expiryParam =
          ai.isExpirable() ? ", com.zimbra.cs.ephemeral.EphemeralInput.Expiration expiration" : "";
      if (ai.isEphemeral()) {
        switch (setterType) {
          case set:
            result.append(
      ...
      if (!ai.isEphemeral()) {
        result.append(
            String.format(
                "        HashMap<String,Object> attrs = new HashMap<String,Object>();%n"));
      }
      result.append(body);
      if (!ai.isEphemeral()) {
      ...
      result.append(
          String.format("        if (attrs == null) attrs = new HashMap<String,Object>();%n"));
      result.append(body);
  ...
  public static void main(String[] args) throws IOException, ServiceException {
    ...
    switch (action) {
      ...
      case listAttrs:
        amu.listAttrs(
            pw, cl.getOptionValues('c'), cl.getOptionValues('n'), cl.getOptionValues('f'));
        break;
    }
