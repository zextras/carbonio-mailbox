...
public abstract class AccessManager {
    ...
    if (sManager == null) {
      ...
      try {
        ...
        if (accessManager != null) {
          Provisioning.AdminAccessControlMech am =
              Provisioning.AdminAccessControlMech.fromString(accessManager);
        ...
        ZimbraLog.account.warn(
            "unable to determine access manager from global config attribute "
                + Provisioning.A_zimbraAdminAccessControlMech
                + ", fallback to use LC key "
                + LC.zimbra_class_accessmanager.key(),
            e);
      }
        ...
        if (className != null && !className.equals("")) {
          try {
            ...
            ZimbraLog.account.error(
                "could not instantiate AccessManager interface of class '"
                    + className
                    + "'; defaulting to DomainAccessManager",
                e);
          }
        }
      }
      ...
      if (Zimbra.started()) {
        ZimbraLog.account.info(
            "Initialized access manager: " + sManager.getClass().getCanonicalName());
      }
    }
  ...
  /**
   * Returns whether the specified account's credentials indicationg that it is the parent account
   * of the target account. <i>Note: This method checks only for family parent account access, and
   * passing the same account for <code>credentials</code> and <code>target</code> will not succeed
   * for non-parent accounts.</i>
   *
   * @param credentials The authenticated account performing the action.
   * @param target The target account for the proposed action.
   */
  protected boolean isParentOf(Account credentials, Account target) {
    ...
    if (childAccts.contains(targetId)) return true;
