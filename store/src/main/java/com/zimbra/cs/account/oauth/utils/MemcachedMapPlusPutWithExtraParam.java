...
/**
 * A key/value lookup map backed by memcached. This class does not implement java.util.Map because
 * memcached doesn't allow iteration. Use this class for values that can be serialized as String,
 * Integer or Long. See also the BigByteArrayMemcachedMap that supports byte array serialization and
 * chunking of values larger than 1MB.
 *
 * <p>Example:
 *
 ...
 * <p>class MyValue { ... }
 *
 * <p>class MySerializer implements Serializer<MyValue> { public Object serialize(MyValue value) {
 * return the serialized object, e.g. String or Integer } public MyValue deserialize(Object obj) {
 * return new MyValue(obj); } }
 *
 * <p>ZimbraMemcachedClient mcdClient = new ZimbraMemcachedClient(...); MySerializer serializer =
 * new MySerializer(); MemcachedMap<MyKey, MyValue> mcdMap = new MemcachedMap(mcdClient,
 * serializer);
 *
 * <p>MyKey foo = new MyKey("foo"); MyValue bar = new MyValue("bar"); mcdMap.put(foo, bar); MyValue
 * bar2 = mcdMap.get(foo); mcdMap.remove(foo);
 *
 * <p>MyKey k1 = new MyKey("k1"); MyKey k2 = new MyKey("k2"); List<MyKey> keys = new
 * ArrayList<MyKey>(); keys.add(k1); keys.add(k2); Map<MyKey, MyValue> values =
 * mcdMap.getMulti(keys);
 *
...
public class MemcachedMapPlusPutWithExtraParam<K extends MemcachedKey, V> {
   ...
   */
  public MemcachedMapPlusPutWithExtraParam(
      ZimbraMemcachedClient client, MemcachedSerializer<V> serializer, boolean ackWrites) {
    mClient = client;
  ...
  public MemcachedMapPlusPutWithExtraParam(
      ZimbraMemcachedClient client, MemcachedSerializer<V> serializer) {
  ...
  public V get(K key) throws ServiceException {
    ...
    V value = null;
    if (valobj != null) value = mSerializer.deserialize(valobj);
    return value;
  }

  /**
   * Returns values for given keys. The returned java.util.Map is never null and has an entry for
   * every key. Entry value will be null if key was not found in memcached. This operation is
   * batched and parallelized in the memcached client layer. Use this method rather than calling
   * get() in a loop.
   *
   * @param keys
   * @return
