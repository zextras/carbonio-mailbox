...
public class GalUtil {
  ...
  public static String expandFilter(
      String tokenize, String filterTemplate, String key, String token) throws ServiceException {
    return expandFilter(tokenize, filterTemplate, key, token, null);
  }

  public static String expandFilter(
      String tokenize, String filterTemplate, String key, String token, String extraQuery)
  ...
  public static String tokenizeKey(GalParams galParams, GalOp galOp) {
    if (galParams == null) return null;
    if (galOp == GalOp.autocomplete) return galParams.tokenizeAutoCompleteKey();
    else if (galOp == GalOp.search) return galParams.tokenizeSearchKey();
