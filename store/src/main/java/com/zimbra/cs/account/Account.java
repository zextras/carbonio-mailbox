 ...
 * @author schemers
 */
public class Account extends MailTarget implements GroupedEntry, AliasedEntry, ZAttrAccount<Provisioning, ServiceException> {
  ...
  public void preAuthAccount(
      ...
      throws ServiceException {
    getProvisioning()
        .preAuthAccount(this, accountName, accountBy, timestamp, expires, preAuth, authCtxt);
  }
  ...
  public void preAuthAccount(
      ...
      throws ServiceException {
    getProvisioning()
        .preAuthAccount(this, accountName, accountBy, timestamp, expires, preAuth, admin, authCtxt);
  }
  ...
  public DataSource createDataSource(
      DataSourceType type, String dataSourceName, Map<String, Object> attrs)
      throws ServiceException {
  ...
  public DataSource createDataSource(
    ...
    return getProvisioning()
        .createDataSource(this, type, dataSourceName, attrs, passwdAlreadyEncrypted);
  }
  ...
  public void modifyIdentity(String identityName, Map<String, Object> attrs)
      throws ServiceException {
  ...
  public String getAccountStatus(Provisioning prov) {
    ...
    if (domainStatus == null || domainStatus.equals(Provisioning.DOMAIN_STATUS_ACTIVE))
      ...
      if (accountStatus.equals(Provisioning.ACCOUNT_STATUS_MAINTENANCE)
          ...
          || accountStatus.equals(Provisioning.ACCOUNT_STATUS_CLOSED)) return accountStatus;
      else return Provisioning.ACCOUNT_STATUS_LOCKED;
      ...
      if (accountStatus.equals(Provisioning.ACCOUNT_STATUS_PENDING)
          || accountStatus.equals(Provisioning.ACCOUNT_STATUS_CLOSED)) return accountStatus;
      else return Provisioning.ACCOUNT_STATUS_MAINTENANCE;
    } else {
  ...
  /**
   * @param at
   * @return true if the the validity checking is OK (either disabled or passed checking) false
   *     otherwise
   */
  public boolean checkAuthTokenValidityValue(AuthToken at) throws ServiceException {
    if (!getProvisioning().getConfig().isAuthTokenValidityValueEnabled()) return true;

    int acctValue = getAuthTokenValidityValue();
    int authTokenValue = at.getValidityValue();

    if (acctValue == authTokenValue) return true;

    ZimbraLog.account.debug(
        "checkAuthTokenValidityValue: validity value on account = "
            + acctValue
            + ", validity value on auth token = "
            + authTokenValue);
    ...
    if (acctValue < authTokenValue) {
       ...
       */
      Integer highestReloadedFor =
          (Integer)
              getCachedData(EntryCacheDataKey.ACCOUNT_VALIDITY_VALUE_HIGHEST_RELOAD.getKeyName());
      boolean willReload = (highestReloadedFor == null) || (highestReloadedFor < authTokenValue);

      ZimbraLog.account.debug(
          "checkAuthTokenValidityValue: highest validity value reloaded for = "
              + highestReloadedFor
              + ", will reload = "
              + willReload);
      ...
      if (willReload) {
        ZimbraLog.account.debug(
            "checkAuthTokenValidityValue: reloading account "
                + getName()
                + " for validity value "
                + authTokenValue);
        ...
        ZimbraLog.account.debug(
            "checkAuthTokenValidityValue: validity value on account after reload = " + acctValue);
  ...
  /**
   * Updates the values of the following attributes: - userPassword - zimbraAuthTokens -
