...
public class LocalBind extends AttributeCallback {
  ...
  public void postModify(CallbackContext context, String attrName, Entry entry) {
        ...
        try {
          if ((address == null) || (address.isEmpty())) {
            ...
            if (inetAddress.isAnyLocalAddress() || inetAddress.isLoopbackAddress())
              server.setAdminLocalBind(false);
            else server.setAdminLocalBind(true);
          }
    ...
    // Update zimbraMailLocalBind if zimbraMailMode or zimbraMailBindAddress or
    // zimbraMailSSLBindAddress is changed.
    // zimbraMailMode is also in globalConfig. Make sure to update the zimbraMailLocalBind of all
    // the
    ...
    if (attrName.equals(Provisioning.A_zimbraMailMode)
      ...
      for (Server server : serverList) {
        try {
          ...
          if (mailMode == null) mailMode = Provisioning.getInstance().getConfig().getMailMode();
          if (mailMode == null || !mailMode.isHttps()) {
            ...
            if ((address == null) || (address.isEmpty())) {
              ...
              if (inetAddress.isAnyLocalAddress() || inetAddress.isLoopbackAddress())
                server.setMailLocalBind(false);
              else server.setMailLocalBind(true);
            }
          } else {
