...
public class ShareInfo {
  ...
  // when we support share info from cos/domain/all authed users, they will be added in the list
  // too.
  //
  // e.g.
  ...
  private static String granteeDisplay(Provisioning prov, ACL.Grant grant) throws ServiceException {
    ...
    if (granteeType == ACL.GRANTEE_USER) {
      Account acct = prov.get(AccountBy.id, granteeId);
      if (acct != null) granteeDisplay = acct.getDisplayName();
    } else if (granteeType == ACL.GRANTEE_GROUP) {
      Group group = prov.getGroupBasic(Key.DistributionListBy.id, granteeId);
      if (group != null) granteeDisplay = group.getDisplayName();
    } else if (granteeType == ACL.GRANTEE_COS) {
      Cos cos = prov.get(Key.CosBy.id, granteeId);
      if (cos != null) granteeDisplay = cos.getName();
    } else if (granteeType == ACL.GRANTEE_DOMAIN) {
      Domain domain = prov.get(Key.DomainBy.id, granteeId);
      if (domain != null) granteeDisplay = domain.getName();
    } else {
      // GRANTEE_AUTHUSER, GRANTEE_PUBLIC, GRANTEE_GUEST
  ...
  public static class MountedFolders {
    ...
    public Integer getLocalFolderId(String ownerAcctId, int remoteFolderId) {
      if (mMountedFolders == null) return null;
      else {
    ...
    /**
     * returns a map of: key: {owner-acct-id}:{remote-folder-id} value: {local-folder-id}
     *
     * @param octxt
     * @param acct
     * @return
    ...
    private void getLocalMountpoints(
        Folder folder, Set<Folder> visible, Map<String, Integer> mountpoints)
        throws ServiceException {
  ...
  public static class Published extends ShareInfo {
    ...
    public static void get(
      ...
      getSharesPublished(
          ...
          granteeIds,
          includePublicShares,
          includeAllAuthedShares,
          guestAcctDomainId);
    }

    public static void getPublished(
        Provisioning prov,
        DistributionList dl,
        boolean directOnly,
        Account owner,
    ...
    private static void getSharesPublished(
        Provisioning prov,
        PublishedShareInfoVisitor visitor,
        Account owner,
        List<String> granteeIds,
        boolean includePublicShares,
        boolean includeAllAuthedShares,
      ...
      searchOpts.setFilter(
          ZLdapFilterFactory.getInstance()
              .accountsByGrants(granteeIds, includePublicShares, includeAllAuthedShares));
      ...
      for (NamedEntry ne : accounts) {
        ...
        if (guestAcctDomainId != null
            && !guestAcctDomainId.equals(prov.getDomain(account).getId())) {
          continue;
        }
        ...
        for (String sharedItem : sharedItems) {
          ...
          if (!shareData.isExpired()
              && (granteeIds.contains(shareData.getGranteeId())
                  ...
                  || (includeAllAuthedShares
                      && shareData.getGranteeTypeCode() == ACL.GRANTEE_AUTHUSER))) {
  ...
  public static class NotificationSender {
    ...
    public static MimeMultipart genNotifBody(
      ...
      if (action == null && goesToExternalAddr) {
        ...
        extUserShareAcceptUrl =
            AccountUtil.getShareAcceptURL(owner, sid.getItemId(), externalGranteeName);
    ...
    private static String genPart(
      ...
      if (extUserShareAcceptUrl != null) {
        ...
        externalShareInfo =
            L10nUtil.getMessage(
                html
                    ? MsgKey.shareNotifBodyExternalShareHtml
                    : MsgKey.shareNotifBodyExternalShareText,
                locale,
                extUserShareAcceptUrl,
                extUserLoginUrl);
      }
    ...
    private static String genRevokePart(ShareInfoData sid, Locale locale, boolean html) {
      return L10nUtil.getMessage(
          html ? MsgKey.shareRevokeBodyHtml : MsgKey.shareRevokeBodyText,
          sid.getName(),
          formatFolderDesc(locale, sid),
          sid.getOwnerNotifName());
    }
    ...
    private static String genExpirePart(ShareInfoData sid, Locale locale, boolean html) {
      return L10nUtil.getMessage(
          (html ? MsgKey.shareExpireBodyHtml : MsgKey.shareExpireBodyText),
          sid.getName(),
          formatFolderDesc(locale, sid),
          sid.getOwnerNotifName());
    }
    ...
    public static String genXmlPart(ShareInfoData sid, StringBuilder sb, Action action)
      ...
      if (action == null || action == Action.edit) {
        share =
            Element.create(SoapProtocol.Soap12, ShareConstants.SHARE)
                .addAttribute(ShareConstants.A_VERSION, ShareConstants.VERSION)
                .addAttribute(
                    ShareConstants.A_ACTION,
                    action == null ? ShareConstants.ACTION_NEW : ShareConstants.ACTION_EDIT);
      } else {
        share =
            Element.create(SoapProtocol.Soap12, ShareConstants.REVOKE)
                .addAttribute(ShareConstants.A_VERSION, ShareConstants.VERSION);
      ...
      share
          .addNonUniqueElement(ShareConstants.E_GRANTEE)
      ...
      share
          .addNonUniqueElement(ShareConstants.E_GRANTOR)
    ...
    private static String getRightsText(ShareInfoData sid, Locale locale) {
      ...
      if ((rights & ACL.RIGHT_READ) != 0)
        appendCommaSeparated(r, L10nUtil.getMessage(MsgKey.shareNotifBodyActionRead, locale));
      if ((rights & ACL.RIGHT_WRITE) != 0)
        appendCommaSeparated(r, L10nUtil.getMessage(MsgKey.shareNotifBodyActionWrite, locale));
      if ((rights & ACL.RIGHT_INSERT) != 0)
        appendCommaSeparated(r, L10nUtil.getMessage(MsgKey.shareNotifBodyActionInsert, locale));
      if ((rights & ACL.RIGHT_DELETE) != 0)
        appendCommaSeparated(r, L10nUtil.getMessage(MsgKey.shareNotifBodyActionDelete, locale));
      if ((rights & ACL.RIGHT_ADMIN) != 0)
        appendCommaSeparated(r, L10nUtil.getMessage(MsgKey.shareNotifBodyActionAdmin, locale));
     ...
     *
     * <p>if all shares are put in one mail, there is no XML part.
     *
     * @return
     */
    ...
    public static void sendShareInfoMessage(
        OperationContext octxt, DistributionList dl, String[] members) {
      ...
      if (visitor.getNumShareInfo() == 0) return;
    ...
    private static Locale getLocale(Provisioning prov, Account fromAcct, String toAddr)
        throws ServiceException {
      Locale locale;
      Account rcptAcct = prov.get(AccountBy.name, toAddr);
      if (rcptAcct != null) locale = rcptAcct.getLocale();
      else if (fromAcct != null) locale = fromAcct.getLocale();
      else locale = prov.getConfig().getLocale();
     ...
     */
    private static Pair<Address, Address> getFromAndReplyToAddr(
        Account fromAcct, DistributionList dl) throws AddressException {
    ...
    private static MimeMultipart buildMailContent(
        DistributionList dl, MailSenderVisitor visitor, Locale locale, Integer idx)
        throws MessagingException, ServiceException {
    ...
    private static void buildContentAndSend(
        SMTPMessage out, DistributionList dl, MailSenderVisitor visitor, Locale locale, Integer idx)
        throws MessagingException, ServiceException {
      ...
      ZimbraLog.account.info(
          "share info notification sent rcpt='" + rcptAddr + "' Message-ID=" + out.getMessageID());
    }
