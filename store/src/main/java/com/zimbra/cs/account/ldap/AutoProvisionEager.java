...
public class AutoProvisionEager extends AutoProvision {
        ...
        try {
          ...
          if (!autoProvisionEnabled(domain)) {
            ...
            ZimbraLog.autoprov.info(
                "Domain %s is scheduled for EAGER auto provision "
                    + "but EAGER mode is not enabled on the domain.  "
                    + "Removing domain %s from %s on server %s",
                domain.getName(),
                domain.getName(),
                Provisioning.A_zimbraAutoProvScheduledDomains,
                localServer.getName());
          ...
          if (t instanceof OutOfMemoryError) {
            ...
            ZimbraLog.autoprov.warn(
                "Unable to auto provision accounts for domain %s", domainName, t);
          }
        }
      }
  ...
  private void handleBatch(ZLdapContext zlc) throws ServiceException {
    if (!autoProvisionEnabled(domain)) {
      throw ServiceException.FAILURE(
          "EAGER auto provision is not enabled on domain " + domain.getName(), null);
    }

    try {
      if (!lockDomain(zlc)) {
        ZimbraLog.autoprov.info(
            "EAGER auto provision unable to lock domain: skip domain "
                + domain.getName()
                + " on server "
                + prov.getLocalServer().getName());
        return;
      }
      createAccountBatch();
    } finally {
  ...
  private static boolean autoProvisionEnabled(Domain domain) {
    return domain
        .getMultiAttrSet(Provisioning.A_zimbraAutoProvMode)
  ...
  private void createAccountBatch() throws ServiceException {
    ...
    for (ExternalEntry entry : entries) {
      ...
      try {
        ...
        ZimbraLog.autoprov.info(
            "auto creating account in EAGER mode: "
                + acctZimbraName
                + ", dn=\""
                + entry.getDN()
                + "\"");
    ...
    // if we hit size limit and all returning items are stuck items, increase batch size to keep
    // processing batches,
    // in the last batch we won't hit size limit, then the last polled timstamp will be set, we can
    // forget about the stuck ones
    if (hitSizeLimitExceededException && entries.size() == stuckAcctNum) {
      ZimbraLog.autoprov.info(
          "search result contains unsuccessful external entries, increasing batch size by %d",
          stuckAcctNum);
    ...
    if (!hitSizeLimitExceededException) {
      ...
      ZimbraLog.autoprov.info(
          "Auto Provisioning has finished for now, setting last polled timestamp: " + lastPolledAt);
  ...
  private boolean lockDomain(ZLdapContext zlc) throws ServiceException {
    ...
    boolean gotLock =
        prov.getHelper()
            .testAndModifyEntry(zlc, ((LdapEntry) domain).getDN(), filter, attrs, domain);
  ...
  private boolean searchAccounts(final List<ExternalEntry> entries, int batchSize)
    ...
    boolean hitSizeLimitExceededException =
        AutoProvision.searchAutoProvDirectory(
            prov, domain, null, null, lastPolledAt, returnAttrs, batchSize, visitor, true);
    ZimbraLog.autoprov.debug(
        "searched external LDAP source, hit size limit ? %s", hitSizeLimitExceededException);
