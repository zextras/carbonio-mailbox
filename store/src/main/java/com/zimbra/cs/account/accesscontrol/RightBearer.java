...
public abstract class RightBearer {
  ...
  public static class Grantee extends RightBearer {
    ...
    protected Grantee(NamedEntry grantee, Set<Right> rights, boolean adminOnly)
    ...
    protected static Grantee getGrantee(NamedEntry namedEntry, boolean adminOnly)
        throws ServiceException {
      return getGrantee(namedEntry, (Set<Right>) null, adminOnly);
    }

    private static Grantee getGranteeFromCache(
        NamedEntry namedEntry, Set<Right> right, boolean adminOnly) throws ServiceException {
      Grantee grntee = null;
      ...
      if (granteeCacheSize > 0) {
        GRANTEE_CACHE =
            CacheBuilder.newBuilder()
                ...
                .expireAfterWrite(
                    granteeCacheExpireAfterMillis,
                    TimeUnit.MILLISECONDS) /* regard data as potentially stale after this time */
                .build();
        ZimbraLog.acl.trace(
            "RightBearer GRANTEE_CACHE BUILD size=%d expire=%dms",
