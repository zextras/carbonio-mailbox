...
public class IDNUtil {
  ...
  *
  */
  private static final Pattern S_DOMAIN = Pattern.compile("(.+)(@)([^>]*)(.*)");
  ...
  public static String toAscii(String name) {
    ...
    try {
      if (name.contains("@")) return toAsciiEmail(name);
      else return toAsciiDomainName(name);
  ...
  */
      ...
      try {
        ia =
            new JavaMailInternetAddress(
                unicodeAddr, ia.getPersonal(), MimeConstants.P_CHARSET_UTF8);
        ...
        ZimbraLog.account.info(
            "cannot convert to unicode, returning original addr: [" + name + "]", e);
      }
    } catch (AddressException e) {
      ZimbraLog.account.info(
          "cannot convert to unicode, returning original addr: [" + name + "]", e);
  ...
  public static String toUnicode(String name) {
    ...
    try {
      if (name.contains("@")) return toUnicodeEmail(name);
      else return toUnicodeDomainName(name);
    } catch (ServiceException e) {
      return name;
    }
  }
  ...
  private static void regexTest(String input) {
    ...
    if (m.matches()) {
      ...
      for (int i = 1; i <= groupsMatched; i++) System.out.print("[" + i + " " + m.group(i) + "]");
      System.out.println();
  ...
  private abstract static class ZimbraIDN {
    ...
    private static ZimbraIDN getInstance() {
      ...
      if (instance == null) instance = new GnuIDN(sAllowUnassigned, sUseSTD3ASCIIRules);
  ...
  private static class JavaIDN extends ZimbraIDN {
    ...
    private final Method mMethodToASCII;
    private final Method mMethodToUnicode;
    // keep constant field value defined in java.net.IDN, since we are using java.reflect
    private final int ALLOW_UNASSIGNED;
    private final int USE_STD3_ASCII_RULES;
    private int mFlags;
    ...
    private JavaIDN(boolean allowUnassigned, boolean useSTD3ASCIIRules) throws ServiceException {
      try {
        ...
        mFlags = 0;
        if (allowUnassigned) mFlags |= ALLOW_UNASSIGNED;

        if (useSTD3ASCIIRules) mFlags |= USE_STD3_ASCII_RULES;
   ...
   *
   * <p>gnu libind-1.0 methods: public static String toASCII(String input, boolean allowUnassigned,
   * boolean useSTD3ASCIIRules)
   *
   * <p>public static String toUnicode(String input, boolean allowUnassigned, boolean
   * useSTD3ASCIIRules)
   *
   * <p>expects the input is only a label, NOT the entire domain name, which is wrong according
   * their javadoc.
   *
   * <p>We copy-paste: public static String toASCII(String input) public static String
   * toUnicode(String input)
   *
   * <p>from IDNA.java, and make them sensitive to allowUnassigned/useSTD3ASCIIRules.
   *
   * <p>A bit hacky. The real fix should be using JDK1.6, which has IDN support in the java.net.IDN
   * class. In GnR, 1.6 is the JDK in production, but in dev JDK 1.5 is still around. We should get
   * rid of GNU libidn and just use the one in JDK1.6 at some point.
   */
  private static class GnuIDN extends ZimbraIDN {
    private final boolean mAllowUnassigned;
    private final boolean mUseSTD3ASCIIRules;
