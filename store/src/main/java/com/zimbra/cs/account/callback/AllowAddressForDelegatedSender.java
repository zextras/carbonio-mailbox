...
/**
 * Ensure values for zimbraPrefAllowAddressForDelegatedSender must be either the entry's primary
 * address or one of the aliases.
 */
    ...
    for (String value : values) {
      if (!StringUtil.isNullOrEmpty(value) && !allowedValues.contains(value)) {
        throw ServiceException.INVALID_REQUEST(
            "value is not one of the addresses of the entry: " + value, null);
      }
    }
  }
