...
public class AllowedAttrs implements AccessManager.AttrRightChecker {
  ...
  public boolean allowAttr(String attrName) {
    if (mResult == AllowedAttrs.Result.ALLOW_ALL) return true;
    else if (mResult == AllowedAttrs.Result.DENY_ALL) return false;
    else {
  ...
  boolean canAccessAttrs(Set<String> attrsNeeded, Entry target) throws ServiceException {
    ...
    if (sLog.isDebugEnabled()) {
      ...
      if (attrsNeeded == null) sb.append("<all attributes>");
      else {
        for (String a : attrsNeeded) sb.append(a + " ");
      }
    ...
    // need all, nope
    if (attrsNeeded == null) return false;
   ...
   *
   * <p>This method DOES check for constraints.
   *
   * @param attrsAllowed result from canAccessAttrs
   * @param attrsNeeded attrs needed to be set. Cannot be null, must specify which attrs/values to
   *     set
   * @return
   */
  boolean canSetAttrsWithinConstraints(
      Grantee grantee, Entry target, Map<String, Object> attrsNeeded) throws ServiceException {

    if (attrsNeeded == null) throw ServiceException.FAILURE("internal error", null);

    if (mResult == AllowedAttrs.Result.DENY_ALL) return false;
    ...
    if (hasConstraints) {
      ...
      if (allowedAttrsOnConstraintEntry.getResult() == AllowedAttrs.Result.ALLOW_ALL
          || (allowedAttrsOnConstraintEntry.getResult() == AllowedAttrs.Result.ALLOW_SOME
              && allowedAttrsOnConstraintEntry
                  .getAllowed()
                  .contains(Provisioning.A_zimbraConstraint))) hasConstraints = false;
    }
  ...
  private String getActualAttrName(String attr) {
    if (attr.charAt(0) == '+' || attr.charAt(0) == '-') return attr.substring(1);
    else return attr;
  }
  ...
  public String dump() {
    ...
    if (mResult == Result.ALLOW_SOME) {
      sb.append("allowed = (");
      for (String a : mAllowSome) sb.append(a + " ");
      sb.append(")");
    }
