...
public class CollectEffectiveRights {
  ...
  static void getEffectiveRights(
    ...
    CollectEffectiveRights cer =
        new CollectEffectiveRights(
            rightBearer, target, targetType, expandSetAttrs, expandGetAttrs, result);
    cer.collect();
  }
  ...
  static void getEffectiveRights(
    ...
    getEffectiveRights(rightBearer, target, targetType, expandSetAttrs, expandGetAttrs, result);
  }

  private CollectEffectiveRights(
      RightBearer rightBearer,
      Entry target,
      TargetType targetType,
      boolean expandSetAttrs,
  ...
  private void collect() throws ServiceException {
    ...
    if (isGlobalAdmin()) {
      ...
      allowSetAttrs =
          CheckAttrRight.accessibleAttrs(getGrantee(), mTarget, AdminRight.PR_SET_ATTRS, false);

      // get effective getAttrs rights
      allowGetAttrs =
          CheckAttrRight.accessibleAttrs(getGrantee(), mTarget, AdminRight.PR_GET_ATTRS, false);
    }
  ...
  private Set<Right> getEffectiveAdminPresetRights() throws ServiceException {
      ...
      if (grantedOn instanceof Group) {
        ...
        if (grantee.isAccount()) {
          skipPositiveGrants =
              !CrossDomain.crossDomainOK(
                  prov, grantee.getAccount(), grantee.getDomain(), targetDomain, (Group) grantedOn);
        }
        ...
        if (groupACLs != null) {
          ...
          if (aclsOnGroupTargets != null) {
            collectAdminPresetRightOnTarget(
                aclsOnGroupTargets, targetType, relativity, false, allowed, denied);
            relativity += 2;
          }
    ...
    if (!conflicts.isEmpty()) {
      for (Right right : conflicts) {
        if (denied.get(right) <= allowed.get(right)) allowed.remove(right);
      }
    }
  ...
  private void collectAdminPresetRightOnTarget(
      List<ZimbraACE> acl,
      TargetType targeType,
      Integer relativity,
      boolean subDomain,
      Map<Right, Integer> allowed,
      Map<Right, Integer> denied)
      throws ServiceException {
    // as an individual: user
    short granteeFlags = (short) (GranteeFlag.F_INDIVIDUAL | GranteeFlag.F_ADMIN);
    collectAdminPresetRights(acl, targeType, granteeFlags, relativity, subDomain, allowed, denied);
  ...
  private void collectAdminPresetRights(
      List<ZimbraACE> acl,
      TargetType targetType,
      short granteeFlags,
      Integer relativity,
  ...
  private SortedMap<String, RightCommand.EffectiveAttr> fillDefault(
      AllowedAttrs allowSetAttrs, AttrRight rightNeeded) throws ServiceException {
  ...
  private SortedMap<String, RightCommand.EffectiveAttr> fillDefaultAndConstratint(
    ...
    Map<String, AttributeConstraint> constraints =
        (constraintEntry == null) ? null : AttributeConstraint.getConstraint(constraintEntry);

    boolean hasConstraints = (constraints != null && !constraints.isEmpty());

    for (String attrName : attrs) {
      if (rightNeeded == AdminRight.PR_SET_ATTRS
          && !isGlobalAdmin()
