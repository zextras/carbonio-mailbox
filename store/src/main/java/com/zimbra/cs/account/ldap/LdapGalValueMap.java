...
public class LdapGalValueMap {
  ...
  Object apply(Object value) {
    if (value instanceof String) {
      ...
      for (int i = 0; i < val.length; i++) newValue[i] = replace(val[i]);

      return newValue;
    } else return value; // is really an internal error
  }

  private String replace(String value) {
    if (mRegex == null) return value;

    Matcher matcher = mRegex.matcher(value);
    if (matcher.matches()) return matcher.replaceAll(mReplacement);
    else return value;
  }
  ...
  public static void main(String[] ags) {
    ...
    for (String v : (String[]) newVal) System.out.println(v);
