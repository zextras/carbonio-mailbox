...
public class EmailChannel extends ChannelProvider {
  ...
  @Override
  public void sendAndStoreResetPasswordRecoveryCode(
      ZimbraSoapContext zsc, Account account, Map<String, String> recoveryCodeMap)
      throws ServiceException {
    DateTimeFormatter formatter =
        DateTimeFormatter.ofPattern("EEE, d MMM yyyy HH:mm:ss z").withZone(ZoneId.of("GMT"));
    ...
    String subject =
        L10nUtil.getMessage(
            MsgKey.sendPasswordRecoveryEmailSubject, locale, account.getDomainName());
    String charset =
        account.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, MimeConstants.P_CHARSET_UTF8);
    String mimePartText =
        L10nUtil.getMessage(
            MsgKey.sendPasswordRecoveryEmailBodyText,
            locale,
            displayName,
            recoveryCodeMap.get(CodeConstants.CODE.toString()),
            mailDate.format(formatter));
    String mimePartHtml =
        L10nUtil.getMessage(
            MsgKey.sendPasswordRecoveryEmailBodyHtml,
            locale,
            displayName,
            recoveryCodeMap.get(CodeConstants.CODE.toString()),
            mailDate.format(formatter));
    try {
      ...
      ZimbraLog.passwordreset.debug(
          "RecoverAccount: Error occured while sending recovery code in email to %s",
          recoveryCodeMap.get(CodeConstants.EMAIL.toString()));
      throw ServiceException.FAILURE(
          "Error occured while sending recovery code in email to "
              + recoveryCodeMap.get(CodeConstants.EMAIL.toString()),
          me);
    }
    ZimbraLog.passwordreset.debug(
        "RecoverAccount: Recovery code sent in email to %s",
   ...
   */
  @Override
  public void validateSetRecoveryAccountCode(
      String recoveryAccountVerificationCode, Account account, Mailbox mbox, ZimbraSoapContext zsc)
      throws ServiceException {
    Map<String, String> recoveryDataMap = getSetRecoveryAccountCodeMap(account);
    if (recoveryDataMap == null || recoveryDataMap.isEmpty()) {
      throw ForgetPasswordException.CODE_NOT_FOUND(
    ...
    if (ZimbraLog.passwordreset.isDebugEnabled()) {
      ...
      ZimbraLog.passwordreset.debug(
          "ValidateCode: last 3 characters of recovery code: %s", code.substring(5));
    }

    if (code.equals(recoveryAccountVerificationCode)) {
      Date now = new Date();
      if (expiryTime < now.getTime()) {
        throw ForgetPasswordException.CODE_EXPIRED(
            "The recovery email address verification code is expired.");
      }
      HashMap<String, Object> prefs = new HashMap<String, Object>();
      prefs.put(
      ...
      throw ForgetPasswordException.CODE_MISMATCH(
          "Verification of recovery email address verification code failed.");
    }
  }

  @Override
  public void sendAndStoreSetRecoveryAccountCode(
      Account account,
      Mailbox mbox,
    ...
    String subject =
        L10nUtil.getMessage(MsgKey.verifyRecoveryEmailSubject, locale, ownerAcctDisplayName);
    String charset =
        account.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, MimeConstants.P_CHARSET_UTF8);
    try {
      DateFormat format = new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss z");
      format.setTimeZone(TimeZone.getTimeZone("GMT"));
      String gmtDate =
          format.format(Long.valueOf(recoveryCodeMap.get(CodeConstants.EXPIRY_TIME.toString())));
      if (ZimbraLog.passwordreset.isDebugEnabled()) {
        ZimbraLog.passwordreset.debug(
            "sendRecoveryEmailVerificationCode: Expiry of recovery address verification code sent"
                + " to %s: %s",
      ...
      String mimePartText =
          L10nUtil.getMessage(
              MsgKey.verifyRecoveryEmailBodyText,
              locale,
              recoveryCodeMap.get(CodeConstants.CODE.toString()),
              gmtDate);
      String mimePartHtml =
          L10nUtil.getMessage(
              MsgKey.verifyRecoveryEmailBodyHtml,
              locale,
              recoveryCodeMap.get(CodeConstants.CODE.toString()),
              gmtDate);
      MimeMultipart mmp = AccountUtil.generateMimeMultipart(mimePartText, mimePartHtml, null);
      ...
      ZimbraLog.passwordreset.warn(
          "Failed to send verification code to email ID: '"
              + recoveryCodeMap.get(CodeConstants.EMAIL.toString())
              + "'",
          e);
      throw ServiceException.FAILURE(
  ...
  public static void sendAndStoreResetPasswordURL(
      ZimbraSoapContext zsc, Account account, Map<String, String> recoveryCodeMap)
      throws ServiceException {
    Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(account);
    Locale locale = account.getLocale();
    ...
    String userDisplayName =
        account.getDisplayName() != null ? String.join("", " ", account.getDisplayName()) : "";
    String subject =
        L10nUtil.getMessage(MsgKey.sendPasswordResetEmailSubject, locale, userDisplayName);
    String charset =
        account.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, MimeConstants.P_CHARSET_UTF8);
    DateTimeFormatter formatter =
        DateTimeFormatter.ofPattern("EEE, d MMM yyyy HH:mm:ss z").withZone(ZoneId.of("GMT"));
    try {
      ...
      String url =
          AccountUtil.generateResetPasswordURL(
              account, expiryTimeLong, recoveryCodeMap.get(CodeConstants.CODE.toString()));

      ZimbraLog.account.debug(
          "Expiry of Password Reset link sent to %s is %s",
          recoveryCodeMap.get(CodeConstants.EMAIL.toString()), expiryDate.format(formatter));
      ZimbraLog.account.debug(
          "Password Reset verification URL sent to %s is %s",
          recoveryCodeMap.get(CodeConstants.EMAIL.toString()), url);
      String mimePartText =
          L10nUtil.getMessage(
              MsgKey.sendPasswordResetEmailBodyText,
              locale,
              userDisplayName,
              accountName,
              url,
      ...
      String mimePartHtml =
          L10nUtil.getMessage(
              MsgKey.sendPasswordResetEmailBodyHtml,
              locale,
              userDisplayName,
              accountName,
              url,
      ...
      ZimbraLog.account.warn(
          String.format(
              "Failed to send verification link to email ID: %s'",
              recoveryCodeMap.get(CodeConstants.EMAIL.toString())),
          e);
      throw ServiceException.FAILURE(
          String.format(
              "Failed to send verification link to email ID: ",
              recoveryCodeMap.get(CodeConstants.EMAIL.toString())),
          e);
    }
  }
