...
public class XMPPComponent extends NamedEntry implements Comparable {
  ...
  public String getShortName() throws ServiceException {
    ...
    if (d == null) {
      throw ServiceException.FAILURE(
          "Invalid configuration data: XMPPComponent name, \""
              + name
              + "\" points to nonexistent domain: "
              + getDomainId(),
          null);
    }
  ...
  public List<String> getComponentFeatures() {
    ...
    if (features != null && features.length > 0) {
      ...
      for (String s : features) toRet.add(s);
    } else {
  ...
  public String toString() {
    MoreObjects.ToStringHelper helper =
        MoreObjects.toStringHelper(this)
            .add("name", getName())
            .add("category", getComponentCategory())
