...
public class DomainAccessManager extends AccessManager {
  ...
  public boolean canAccessAccount(AuthToken at, Account target, boolean asAdmin)
    ...
    if (!at.isZimbraUser()) return false;
  ...
  /**
   * Returns whether the specified account's credentials are sufficient to perform operations on the
   * target account. This occurs when the credentials belong to an admin or when the credentials are
   * for an appropriate domain admin. <i>Note: This method checks only for admin access, and passing
   * the same account for <code>credentials</code> and <code>target</code> will not succeed for
   * non-admin accounts.</i>
   *
  ...
  public boolean canAccessAccount(Account credentials, Account target, boolean asAdmin)
    ...
    if (isParentOf(credentials, target)) return true;
  ...
  public boolean canAccessCos(AuthToken at, Cos cos) throws ServiceException {
    ...
    for (String c : allowedCoses) {
      ...
      if (parts.length != 2) continue; // bad value skip
      String id = parts[0];
      if (id.equals(cosId)) return true;
    }
    return false;
  }
  ...
  public boolean canAccessEmail(AuthToken at, String email) throws ServiceException {
    ...
    if (targetAcct != null) {
      if (isParentOf(at, targetAcct)) return true;
    }
  ...
  public boolean canModifyMailQuota(AuthToken at, Account targetAccount, long mailQuota)
      throws ServiceException {
    if (!canAccessAccount(at, targetAccount)) return false;
  ...
  @Override
  public boolean canGetAttrs(Account grantee, Entry target, Set<String> attrs, boolean asAdmin)
      throws ServiceException {
    return false;
  }
  ...
  @Override
  public boolean canSetAttrs(Account grantee, Entry target, Set<String> attrs, boolean asAdmin)
      throws ServiceException {
    return false;
  }
  ...
  public boolean canSetAttrs(
      Account grantee, Entry target, Map<String, Object> attrs, boolean asAdmin)
      throws ServiceException {
    return false;
  }
