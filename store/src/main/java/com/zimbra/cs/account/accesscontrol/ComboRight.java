...
public class ComboRight extends AdminRight {
  ...
  void addRight(Right right) throws ServiceException {
    ...
    if (right.isUserRight()) throw ServiceException.FAILURE("internal error", null);
    mRights.add(right);
  }
  ...
  Set<TargetType> getGrantableTargetTypes() {
    ...
    for (Right r : getAllRights()) {
      ...
      if (targetTypes == null) targetTypes = tts;
      else targetTypes = SetUtil.intersect(targetTypes, tts);
    }
  ...
  private static void expand(ComboRight right, Set<Right> presetRights, Set<AttrRight> attrRights)
      throws ServiceException {
    for (Right r : right.getRights()) {
      if (r.isPresetRight()) presetRights.add(r);
      else if (r.isAttrRight()) attrRights.add((AttrRight) r);
      else if (r.isComboRight()) expand((ComboRight) r, presetRights, attrRights);
