...
public class ParticallyDenied {
  ...
  private static boolean isSubTarget(Provisioning prov, Entry targetSup, Entry targetSub)
    ...
    if (targetSup instanceof Domain) {
      ...
      if (targetSubInDomain == null) return false; // not a domain-ed entry
      else {
        if (domain.getId().equals(targetSubInDomain.getId())) return true;
        else {
          ...
          for (String groupId : groups.groupIds()) {
            ...
            if (groupInDomain != null
                && // hmm, log a warn if groupInDomain is null? throw internal err?
                domain.getId().equals(groupInDomain.getId())) return true;
          }
        }
      }
      return false;
    ...
    } else if (targetSup instanceof GlobalGrant) return true;
    else {
      /*
      ...
      throw ServiceException.FAILURE(
          "internal error, unexpected entry type: " + targetSup.getLabel(), null);
    }
  }
   ...
   *
   * <p>exactly one of granteeId and granteeGroups is not null, and the other is null.
   *
   * <p>if granteeGroups is not null, we check for grants granted to the groups
   *
   * @param prov
   * @param targetToGrant
   * @param rightToGrant
   ...
   * @throws ServiceException
   */
  private static void checkDenied(
      Provisioning prov,
      Entry targetToGrant,
      Right rightToGrant,
      Set<GrantsOnTarget> grantsOnTargets,
      String granteeId,
      Set<String> granteeGroups)
      throws ServiceException {
          ...
          if ((granteeId != null && granteeId.equals(ace.getGrantee()))
            ...
            if (rightToGrant.overlaps(ace.getRight()))
              throw ServiceException.PERM_DENIED(
                  String.format(
                      "insufficient right to grant. Right '%s' is denied to grp/usr '%s' on target"
                          + " %s",
  ...
  private static void getAllGrantableTargetTypes(Right right, Set<TargetType> result)
      throws ServiceException {
    ...
    if (right.isPresetRight() || right.isAttrRight()) {
      ...
      for (Right r : cr.getAllRights()) getAllGrantableTargetTypes(r, result);
    }
  }

  /**
   * Returns if rightToGrant is (partically) denied to grantor(or groups it belongs) on sub-targets
   * of targetToGrant.
   *
   ...
   * @throws ServiceException
   */
  static void checkPartiallyDenied(
      Account grantor, TargetType targetTypeToGrant, Entry targetToGrant, Right rightToGrant)
      throws ServiceException {

    if (AccessControlUtil.isGlobalAdmin(grantor, true)) return;
    ...
    if (targetTypesToSearch.isEmpty()) return;
