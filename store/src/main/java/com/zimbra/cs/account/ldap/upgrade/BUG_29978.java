...
public class BUG_29978 extends UpgradeOp {
  ...
  private static String genQuery(List<Server> servers) {
    ...
    if (numValues > 1) query = "(|" + q.toString() + ")";
    else query = q.toString();

    // zimbraPublicServiceHostname is not indexed, put objectClass=zimbraDomain in the front,
      ...
      for (Server server : servers) {
        ...
        if (!StringUtil.isNullOrEmpty(serviceName)) {
          ...
          if (sInfo != null) {
            ...
            if (proto != null)
              serverMap.put(
                  server.getAttr(Provisioning.A_zimbraServiceHostname),
      ...
      try {
        domain =
            new LdapDomain(
                ...
                upgradeOp.prov.getConfig().getDomainDefaults(),
                upgradeOp.prov);
        visit(domain);
      } catch (ServiceException e) {
        upgradeOp.printer.println(
            "entry skipped, encountered error while processing entry at:" + dn);
    ...
    private void visit(Domain domain) {
      ...
      if (StringUtil.isNullOrEmpty(domainPublicHostname)) {
        if (upgradeOp.verbose) {
          upgradeOp.printer.format(
              "Not updating domain %d: domain does not have %s\n",
      ...
      if (domainPublicProtocol != null) {
        if (upgradeOp.verbose) {
          upgradeOp.printer.format(
              "Not updating domain %s: %s already set to %s on domain\n",
      ...
      try {
        upgradeOp.printer.format(
            "Updating domain %-30s: proto => %-5s   port => %-5s\n",
            domain.getName(),
        ...
        upgradeOp.printer.println(
            "Caught ServiceException while modifying domain " + domain.getName());
        upgradeOp.printer.printStackTrace(e);
      }
    }
  ...
  /**
   * for each domain, if domain has zimbraPublicServiceHostname, and that zPSH has a corresponding
   * zimbraServer, then set public service port/protocol on domain from that zimbraServer.
   */
  @Override
  ...
  Description getDescription() {
    return new Description(
        this,
        new String[] {
          Provisioning.A_zimbraPublicServiceProtocol, Provisioning.A_zimbraPublicServicePort
        },
        new EntryType[] {EntryType.DOMAIN},
        null,
        String.format(
            "value of %s and %s on the matching server",
            Provisioning.A_zimbraPublicServiceProtocol, Provisioning.A_zimbraPublicServicePort),
  ...
  private Server createTestServer(
      String serverName, String mailMode, String mailPort, String mailSSLPort)
      throws ServiceException {
    Map<String, Object> attrs = new HashMap<String, Object>();

    attrs.put(Provisioning.A_zimbraServiceHostname, serverName + "-serviceName");
    if (mailMode != null) attrs.put(Provisioning.A_zimbraMailMode, mailMode);

    if (mailPort != null) attrs.put(Provisioning.A_zimbraMailPort, mailPort);

    if (mailSSLPort != null) attrs.put(Provisioning.A_zimbraMailSSLPort, mailSSLPort);

    printer.println("Creating server " + serverName);
    return prov.createServer(serverName, attrs);
  }

  private Domain createTestDomain(String domainName, String publicServiceHostname)
      throws ServiceException {
  ...
  private void populateTestData() throws ServiceException {
    ...
    Server http_portis0 =
        createTestServer("http_portis0.server", MailMode.http.toString(), "0", "1001");
    Server http_noport =
        createTestServer("http_noport.server", MailMode.http.toString(), null, "1001");

    Server https = createTestServer("https.server", MailMode.https.toString(), "2000", "2001");
    Server https_portis0 =
        createTestServer("https_portis0.server", MailMode.https.toString(), "2000", "0");
    Server https_noport =
        createTestServer("https_noport.server", MailMode.https.toString(), "2000", null);
    ...
    Server redirect =
        createTestServer("redirect.server", MailMode.redirect.toString(), "5000", "5001");
    // Server bogus           = createTestServer("bogus.server",            "bogus",
    //      "6000", "6001");
    ...
    for (int i = 0; i < NUM_DOMAIN_SETS; i++) {
      ...
      createTestDomain(
          "http_portis0_" + i + ".test",
          http_portis0.getAttr(Provisioning.A_zimbraServiceHostname));
      createTestDomain(
          "http_noport_" + i + ".test", http_noport.getAttr(Provisioning.A_zimbraServiceHostname));

      createTestDomain("https_" + i + ".test", https.getAttr(Provisioning.A_zimbraServiceHostname));
      createTestDomain(
          "https_portis0_" + i + ".test",
          https_portis0.getAttr(Provisioning.A_zimbraServiceHostname));
      createTestDomain(
          "https_noport_" + i + ".test",
          https_noport.getAttr(Provisioning.A_zimbraServiceHostname));
      ...
      createTestDomain(
          "redirect_" + i + ".test", redirect.getAttr(Provisioning.A_zimbraServiceHostname));
      // createTestDomain("bogus_" + i + ".test",
      // bogus.getAttr(Provisioning.A_zimbraServiceHostname));
      createTestDomain(
          "nomailmode_" + i + ".test", nomailmode.getAttr(Provisioning.A_zimbraServiceHostname));

      // domain's zimbraPublicServiceHostname does not match any server, domain should NOT be found
      // by the search
      ...
      createTestDomain(
          "proto_present_" + i + ".test",
          http.getAttr(Provisioning.A_zimbraServiceHostname),
          "https",
          null);

      // domain matching a server but already has port present, it should not be updated
      createTestDomain(
          "port_present_" + i + ".test",
          http.getAttr(Provisioning.A_zimbraServiceHostname),
          null,
          "8888");
    }
