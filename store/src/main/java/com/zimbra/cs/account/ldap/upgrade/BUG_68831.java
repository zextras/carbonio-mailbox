...
public class BUG_68831 extends UpgradeOp {
  ...
  private void upgradeDLs(ZLdapContext zlc) throws ServiceException {
    ...
    ZLdapFilter homeServerPresent =
        filterFactory.fromFilterString(
    ...
    String query =
        filterFactory
            .andWith(filterFactory.allDistributionLists(), homeServerNotPresent)
            .toFilterString();

    upgrade(zlc, bases, query);
  }

  private void upgrade(ZLdapContext modZlc, String bases[], String filter) throws ServiceException {
    ...
    for (String base : bases) {
      try {
        ...
        // log and continue
        printer.println(
            "Caught ServiceException while searching " + filter + " under base " + base);
    ...
    Bug68831Visitor(UpgradeOp upgradeOp, ZLdapContext modZlc) throws ServiceException {
      ...
      if (servers.size() == 0) {
        throw ServiceException.FAILURE(
            "no mailclient server running service for assigning home server for distribution lists",
            null);
      }
    }
