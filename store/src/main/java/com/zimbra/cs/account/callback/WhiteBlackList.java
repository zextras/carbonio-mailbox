...
public class WhiteBlackList extends AttributeCallback {
  ...
  private void check(String max, Account acct, String attrName, Map attrsToModify)
    ...
    if (replace != null) {
      ...
      if (mods.size() > numMax) throwLimitExceeded(attrName, numMax);
    } else {
      // adding/removing
      if (acct == null) {
        ...
        if (add != null) {
          mods = getMultiValueSet(add);
          for (String s : mods) {
            if (!curValues.contains(s)) numToAdd++;
          }
        }
        ...
        if (remove != null) {
          ...
          for (String s : mods) {
            if (curValues.contains(s)) numToRemove++;
          }
        }
        ...
        if (curNum > numMax) {
          ...
          if (add != null) {
            ...
            if (remove == null) throwLimitExceeded(attrName, numMax);
          ...
          if (newNum > numMax) throwLimitExceeded(attrName, numMax);
        }
      }
    }
  }
  ...
  private void throwLimitExceeded(String attrName, int numMax) throws ServiceException {
    throw ServiceException.INVALID_REQUEST(
        "exceed limit for " + attrName + ", max is " + numMax, null);
  }
}
