...
public class AutoProvisionThread extends Thread
  ...
  /**
   * Provision accounts for all domain scheduled for auto provision on this server, sleep for
   * configured amount of time between iterations.
   *
   * <p>If an iteration takes longer than the sleep interval, the next iteration will start
   * immediately.
   */
  @Override
  public void run() {
    ...
    ZimbraLog.autoprov.info(
        "Auto provision thread sleeping for %dms before doing work.", sleepTime);
    ...
    while (true) {
      ...
      if (sleepInterval == 0) {
        ...
        ZimbraLog.autoprov.debug(
            "Iteration took %d seconds, starting next iteration immediately without sleeping",
  ...
  /**
   * Stores the sleep interval, so that the auto provision thread doesn't die if there's a problem
   * talking to LDAP. See bug 32639.
   */
