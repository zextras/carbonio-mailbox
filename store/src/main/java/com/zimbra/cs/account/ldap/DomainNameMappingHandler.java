...
public abstract class DomainNameMappingHandler {
   ...
   * @return
   */
  public abstract String mapName(String foreignName, String params, String zimbraDomainName)
  ...
  private static HandlerInfo loadHandler(HandlerConfig handlerConfig) {
    ...
    try {
      handlerInfo.mClass =
          ExtensionUtil.findClass(className).asSubclass(DomainNameMappingHandler.class);
    } catch (ClassNotFoundException e) {
      // miss configuration or the extension is disabled
      sLog.warn(
          "Domain name mapping handler %s for application %s not found",
  ...
  public static HandlerConfig getHandlerConfig(Domain domain, String application) {
    Map<String, HandlerConfig> handlers =
        (Map<String, HandlerConfig>)
            domain.getCachedData(EntryCacheDataKey.DOMAIN_FOREIGN_NAME_HANDLERS.getKeyName());
  ...
  public static String mapName(
      HandlerConfig handlerConfig, String foreignName, String zimbraDomainName)
      throws ServiceException {
    DomainNameMappingHandler handler = getHandler(handlerConfig);

    if (handler instanceof UnknownDomainNameMappingHandler)
      throw ServiceException.FAILURE(
          "unable to load domain name mapping handler "
              + handlerConfig.getClassName()
              + " for application:"
              + handlerConfig.getApplicaiton(),
          null);
  ...
  public static void main(String[] args) throws Exception {
    ...
    domain.addForeignNameHandler(
        "app1:com.zimbra.cs.account.ldap.DomainNameMappingHandler$DummyHandler:p1, p2, p3");
