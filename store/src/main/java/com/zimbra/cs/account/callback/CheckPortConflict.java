...
public class CheckPortConflict extends AttributeCallback {
  ...
  public void preModify(
    ...
    if (!sPortAttrs.contains(attrName)
        || !AttributeManager.getInstance().isServerInherited(attrName)) assert (false);

    // server == null means the server entry is being created
    if (entry == null || entry instanceof Server) checkServer((Server) entry, attrsToModify);
    else checkConfig((Config) entry, attrsToModify);
  }

  private void checkServer(Server server, Map<String, Object> serverAttrsToModify)
      throws ServiceException {
    ...
    for (Map.Entry<String, Object> attrToModify : serverAttrsToModify.entrySet()) {
      ...
      if (mod.setting()) newValue = mod.value();
      else {
        // unsetting, get default, which would become the new value
        Object defValue = defaults.get(attrName);
        if (defValue != null) {
          if (defValue instanceof String) newValue = (String) defValue;
          else
            ZimbraLog.misc.info(
                "default value for "
                    + attrName
                    + " should be a single-valued attribute, invalid default value ignored");
        }
      }
      ...
      if (conflict(server, ports, newValue, attrName)) {
        ...
        throw ServiceException.INVALID_REQUEST(
            "port "
                ...
                + serverInfo,
            null);
      } else ports.put(newValue, attrName);
    }
  }

  private void checkConfig(Config config, Map<String, Object> configAttrsToModify)
      throws ServiceException {
      ...
      if (conflict(null, newDefaults, newValue, attrName)) {
        throw ServiceException.INVALID_REQUEST(
            "port "
                ...
                + attrName
                + " and "
                + newDefaults.get(newValue)
                + " on global config",
            null);
      } else newDefaults.put(newValue, attrName);
    }
  ...
  private void checkServerWithNewDefaults(
      Server server, BiMap<String, String> newDefaults, Map<String, Object> configAttrsToModify)
      throws ServiceException {
    Map<String, String> ports = new HashMap<String, String>();
    ...
    for (String attrName : sPortAttrs) {
      ...
      if (conflict(server, ports, newValue, attrName)) {
        ...
        if (configAttrsToModify.containsKey(attrName)
            || configAttrsToModify.containsKey(conflictWith))
          throw ServiceException.INVALID_REQUEST(
              "port "
                  + newValue
                  + " conflict between "
  ...
  private boolean conflict(Server server, Map<String, String> ports, String port, String attrName) {
    if (StringUtil.isNullOrEmpty(port)) return false;
    else if (port.equals("0")) return false;
    else if (server != null) {
