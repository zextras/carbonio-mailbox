...
public class ZimbraACE {
  ...
  */
  private String[] getParts(String ace) throws ServiceException {
    ...
    if (p3 == -1) throw ServiceException.PARSE_ERROR("bad ACE: " + ace, null);

    int p2 = ace.lastIndexOf(S_DELIMITER, p3 - 1);
    if (p2 == -1) throw ServiceException.PARSE_ERROR("bad ACE: " + ace, null);
  ...
  public static class ExternalGroupInfo {
    ...
    static ExternalGroupInfo parse(String grantee) throws ServiceException {
      ...
      if (parts.length != 2) {
        throw ServiceException.PARSE_ERROR("invalid external group grantee " + grantee, null);
      }
    ...
    public static String encodeIfExtGroupNameMissingDomain(
        String zimbraDomain, String extGroupName) {
   ...
   *
   * <p>We store targetType and targetName in ZimbraACE, because in some code path we've got an
   * ZimbraACE object but lost track of on which target is it granted.
   *
   * <p>targetType and targetName are *not* serialized to LDAP, they are only set in memory.
   *
   * @param ace
   * @param rm
   * @param targetType
  ...
  private ZimbraACE(ZimbraACE other) {
    ...
    if (other.mSecret != null) mSecret = new String(other.mSecret);
  ...
  /**
   * Returns whether the principal id exactly matches the grantee. <tt>principalId</tt> must be
   * {@link GuestAccount#GUID_PUBLIC} (<tt>null</tt> is also OK) if the actual grantee is {@link
   * ACL#GRANTEE_PUBLIC}. <tt>principalId</tt> must be {@link GuestAccount#GUID_AUTHUSER} if the
  ...
  /**
   * Returns whether this grant applies to the given {@link MailTarget}. If <tt>mailTarget</tt> is
  ...
  private boolean matches(MailTarget mailTarget, boolean asAdmin) throws ServiceException {
    ...
    if (mailTarget == null) return mGranteeType == GranteeType.GT_PUBLIC;
    switch (mGranteeType) {
      case GT_PUBLIC:
        return true;
      case GT_AUTHUSER:
  ...
  */
  ...
  public String getGranteeDisplayName() {
    try {
      switch (mGranteeType) {
        ...
        case GT_GROUP:
          Group group =
              Provisioning.getInstance().getGroupBasic(Key.DistributionListBy.id, mGrantee);
          if (group != null) {
  ...
  public String serialize() {
    ...
    else sb.append(mGrantee + S_DELIMITER);

    // grantee type
    sb.append(getGranteeType().getCode() + S_DELIMITER);

    // right
    if (mRightModifier != null) sb.append(mRightModifier.getModifier());
  ...
  public String dump(boolean verbose) {
    if (verbose)
      return "["
          ...
          + getGrantee()
          + ", grantee type="
          + getGranteeType().getCode()
          + ", right="
          + getRight().getName()
          + "]";
    else return "[(" + getGranteeDisplayName() + ") " + serialize() + "]";
  }
    ...
    if (ace.mGranteeType == GranteeType.GT_GUEST || ace.mGranteeType == GranteeType.GT_KEY) {
      if (ace.getGrantee().contains(S_SECRET_DELIMITER))
        throw ServiceException.INVALID_REQUEST(
            "grantee cannot contain:" + S_SECRET_DELIMITER, null);
      if (ace.getSecret() != null && ace.getSecret().contains(S_SECRET_DELIMITER))
        throw ServiceException.INVALID_REQUEST(
            "password/accesskey cannot contain:" + S_SECRET_DELIMITER, null);
    }
  }
}
