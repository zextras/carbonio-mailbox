...
public abstract class AutoProvision {
  ...
  protected Account createAccount(
      String acctZimbraName, ExternalEntry externalEntry, String password, AutoProvMode mode)
    ...
    */
    ...
    ZimbraLog.security.info(
        ZimbraLog.encodeAttrs(
            new String[] {
              "cmd", "auto provision Account", "name", acct.getName(), "id", acct.getId()
            },
            zimbraAttrs));
      ...
      if (listener != null) {
        ...
        if (mode == AutoProvMode.EAGER) {
          ZimbraLog.autoprov.warn(
              "EAGER mode should configure " + Provisioning.A_zimbraAutoProvListenerClass);
        }
      }
    } catch (ServiceException e) {
  ...
  private static class AutoProvisionCachedInfo {
    ...
    private static AutoProvisionCachedInfo getInfo(Domain domain) throws ServiceException {
      AutoProvisionCachedInfo attrMap =
          (AutoProvisionCachedInfo)
              domain.getCachedData(EntryCacheDataKey.DOMAIN_AUTO_PROVISION_DATA);
    ...
    private AutoProvisionCachedInfo(Domain domain) throws ServiceException {
      ...
      for (String rule : rules) {
        ...
        if (parts.length != 2) {
          throw ServiceException.FAILURE(
              "invalid value in " + Provisioning.A_zimbraAutoProvAttrMap + ": " + rule, null);
        }
        ...
        if (!validAccountAttrs.contains(zimbraAttr)) {
          throw ServiceException.FAILURE(
              "invalid value in "
                  + Provisioning.A_zimbraAutoProvAttrMap
                  + ": "
                  + rule
                  + ", not a valid zimbra attribute ",
              null);
        }
      ...
      if (className != null) {
        try {
          if (className != null) {
            listener =
                ExtensionUtil.findClass(className)
                    .asSubclass(AutoProvisionListener.class)
                    .newInstance();
          }
          ...
          ZimbraLog.autoprov.warn(
              "unable to instantiate auto provision listener object of class " + className, e);
  ...
  protected ZAttributes getExternalAttrsByDn(String dn) throws ServiceException {
    ...
    ExternalLdapConfig config =
        new ExternalLdapConfig(
            url, wantStartTLS, null, adminDN, adminPassword, null, "auto provision account");
  ...
  protected ExternalEntry getExternalAttrsByName(String loginName) throws ServiceException {
            ...
            prov.getHelper()
                .searchForEntry(
                    ...
                    ZLdapFilterFactory.getInstance()
                        .fromFilterString(FilterId.AUTO_PROVISION_SEARCH, searchFilter),
                    zlc,
                    attrs);
        if (entry == null) {
    ...
    throw ServiceException.FAILURE(
        "One of "
            + Provisioning.A_zimbraAutoProvLdapBindDn
            + " or "
  ...
  protected void sendNotifMessage(Account acct, String password) throws ServiceException {
    ...
    try {
      ...
      try {
        ...
        ZimbraLog.autoprov.warn(
            "invalid address in " + Provisioning.A_zimbraAutoProvNotificationFromAddress, e);
      }
      ...
      ZimbraLog.autoprov.info(
          "auto provision notification sent rcpt='"
              + rcptAddr
              + "' Message-ID="
              + out.getMessageID());
   ...
   */
  static void searchAutoProvDirectory(
      LdapProv prov,
      Domain domain,
      String filter,
      String name,
      String createTimestampLaterThan,
      String[] returnAttrs,
      int maxResults,
      final DirectoryEntryVisitor visitor)
      throws ServiceException {

    SearchLdapVisitor ldapVisitor =
    ...
    searchAutoProvDirectory(
        prov,
        domain,
        filter,
        name,
        createTimestampLaterThan,
        returnAttrs,
        maxResults,
        ldapVisitor,
        false);
   ...
   *
   * <p>Only one of filter or name can be provided. - if name is provided, the search filter will be
   * zimbraAutoProvLdapSearchFilter with place holders filled with the name.
   *
   * <p>- if filter is provided, the provided filter will be the search filter.
   *
   * <p>- if neither is provided, the search filter will be zimbraAutoProvLdapSearchFilter with
   * place holders filled with "*". If createTimestampLaterThan is provided, the search filter will
   * be ANDed with (createTimestamp >= {timestamp})
   *
   * @param prov
   * @param domain
   * @param filter
   ...
   * @param maxResults
   * @param ldapVisitor
   * @param wantPartialResult whether TOO_MANY_SEARCH_RESULTS should be thrown if the ldap search
   *     encountered LdapSizeLimitExceededException Note: regardless of this parameter, the
   *     ldapVisitor.visit is called for each entry returned from LDAP. This behavior is currently
   *     hardcoded in UBIDLdapContext.searchPaged and has been the legacy behavior. We can probably
   *     change it into a parameter in SearchLdapOptions.
   * @throws ServiceException
   * @return whether LdapSizeLimitExceededException was hit
   */
  static boolean searchAutoProvDirectory(
      LdapProv prov,
      Domain domain,
      String filter,
      String name,
    ...
    if (url == null) {
      throw ServiceException.FAILURE(
          String.format(
              "missing %s on domain %s", Provisioning.A_zimbraAutoProvLdapURL, domain.getName()),
          null);
    }
    if (searchBase == null) {
      searchBase = LdapConstants.DN_ROOT_DSE;
    ...
    ExternalLdapConfig config =
        new ExternalLdapConfig(
            ...
            adminPassword,
            null,
            "search auto provision directory");
    ...
    try {
        ...
        if (createTimestampLaterThan != null) {
          ...
          searchFilter =
              "(&"
                  + searchFilter
                  + ZLdapFilterFactory.getInstance()
                      .createdLaterOrEqual(createTimestampLaterThan)
                      .toFilterString()
                  + ")";
      ...
      try {
        ...
        if (searchFilterWithoutLastPolling != null && createTimestampLaterThan != null) {
          ...
          searchFilter =
              "(&"
                  + searchFilterWithoutLastPolling
                  + ZLdapFilterFactory.getInstance()
                      .createdLaterOrEqual(createTimestampLaterThan)
                      .toFilterString()
                  + ")";
      ...
      if (wantPartialResult) {
        ...
        ZimbraLog.autoprov.debug(
            String.format(
                "searchAutoProvDirectory encountered LdapSizeLimitExceededException: "
                    + "base=%s, filter=%s",
                searchBase, zFilter == null ? "" : zFilter.toFilterString()),
            e);
      } else {
        throw AccountServiceException.TOO_MANY_SEARCH_RESULTS(
            "too many search results returned", e);
      }
    } finally {
      LdapClient.closeContext(zlc);
    }
