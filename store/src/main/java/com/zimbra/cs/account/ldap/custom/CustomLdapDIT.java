...
public class CustomLdapDIT extends LdapDIT {
  ...
  private String getLC(KnownKey key, String defaultValue) {
    ...
    if (StringUtil.isNullOrEmpty(lcValue)) return defaultValue;
    else return lcValue;
  }
  ...
  protected void init() {
    ...
    NAMING_RDN_ATTR_DYNAMICGROUP =
        getLC(LC.ldap_dit_naming_rdn_attr_dynamicgroup, DEFAULT_NAMING_RDN_ATTR_DYNAMICGROUP);
    NAMING_RDN_ATTR_GLOBALCONFIG =
        getLC(LC.ldap_dit_naming_rdn_attr_globalconfig, DEFAULT_NAMING_RDN_ATTR_GLOBALCONFIG);
    NAMING_RDN_ATTR_GLOBALGRANT =
        getLC(LC.ldap_dit_naming_rdn_attr_globalgrant, DEFAULT_NAMING_RDN_ATTR_GLOBALGRANT);
    ...
    NAMING_RDN_ATTR_XMPPCOMPONENT =
        getLC(LC.ldap_dit_naming_rdn_attr_xmppcomponent, DEFAULT_NAMING_RDN_ATTR_XMPPCOMPONENT);
    NAMING_RDN_ATTR_ZIMLET =
        getLC(LC.ldap_dit_naming_rdn_attr_zimlet, DEFAULT_NAMING_RDN_ATTR_ZIMLET);
    ...
    BASE_DN_ADMIN =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_admin, DEFAULT_BASE_RDN_ADMIN + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_APPADMIN =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_appadmin, DEFAULT_BASE_RDN_APPADMIN + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_COS =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_cos, DEFAULT_BASE_RDN_COS + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_GLOBAL_DYNAMICGROUP =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_global_dynamicgroup,
            DEFAULT_BASE_RDN_GLOBAL_DYNAMICGROUP + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_MIME =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_mime, DEFAULT_BASE_RDN_MIME + "," + DN_GLOBALCONFIG);
    BASE_DN_SERVER =
        getLCAndValidateUnderConfigBranchDN(
    ...
    BASE_DN_SHARE_LOCATOR =
        getLCAndValidateUnderConfigBranchDN(
            ...
            DEFAULT_BASE_RDN_SHARE_LOCATOR + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_XMPPCOMPONENT =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_xmppcomponent,
            DEFAULT_BASE_RDN_XMPPCOMPONENT + "," + BASE_DN_CONFIG_BRANCH);
    BASE_DN_ZIMLET =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_zimlet, DEFAULT_BASE_RDN_ZIMLET + "," + BASE_DN_CONFIG_BRANCH);

    BASE_DN_DOMAIN =
        getLCAndValidateUnderConfigBranchDN(
            LC.ldap_dit_base_dn_domain, DEFAULT_BASE_RDN_DOMAIN + "," + BASE_DN_CONFIG_BRANCH);
  ...
  private void validateMailBranchEntryDN(String dn) throws ServiceException {
    if (!validateUnderDN(BASE_DN_MAIL_BRANCH, dn))
      throw ServiceException.INVALID_REQUEST(
          "dn " + dn + " must be under " + BASE_DN_MAIL_BRANCH, null);
  }
  ...
  private String defaultDomain() throws ServiceException {
    ...
    if (StringUtil.isNullOrEmpty(defaultDomain)) throw UNSUPPORTED("default domain is empty");
  ...
  public String aliasDN(
    ...
    if (!aliasDomain.equals(allowedDomain))
      throw UNSUPPORTED(
          "alias DN not in default domain"
              + "(alias domain="
              + aliasDomain
              + ", default domain="
              + allowedDomain
    ...
    if (!targetDomain.equals(aliasDomain))
      throw UNSUPPORTED(
          "alias DN with different target domain and alias domain"
              + "(alias domain="
              + aliasDomain
              + ", target domain="
              + targetDomain
  ...
  public String aliasDNRename(String targetNewDn, String targetNewDomain, String newAliasEmail)
    ...
    if (!targetNewDomain.equals(allowedDomain))
      throw UNSUPPORTED(
          "alias DN rename not in default domain"
              + "(alias domain="
              + targetNewDomain
              + ", default domain="
              + allowedDomain
  ...
  public ZLdapFilter filterAccountsByDomainAndServer(Domain domain, Server server) {
    return ZLdapFilterFactory.getInstance()
        .velodromeAllAccountsByDomainAndServer(domain.getName(), server.getServiceHostname());
  }

  @Override
  public ZLdapFilter filterAccountsOnlyByDomainAndServer(Domain domain, Server server) {
    return ZLdapFilterFactory.getInstance()
        .velodromeAllAccountsOnlyByDomainAndServer(domain.getName(), server.getServiceHostname());
  }

  @Override
  public ZLdapFilter filterCalendarResourceByDomainAndServer(Domain domain, Server server) {
    return ZLdapFilterFactory.getInstance()
        .velodromeAllCalendarResourcesByDomainAndServer(
   ...
   * earlier comment (when this method threw {@link UnsupportedOperationException}) we can't tell
   * whether the main email is "zimbraMailAlias" or "mail". This filter will accept the domain in
   * either "zimbraMailAlias" or "mail".
   */
  @Override
