    ...
    if (properties.getEncoded() == null) {
      try {
        ZimbraLog.account.debug(
            "auth: generating jwt token for account id: %s", properties.getAccountId());
  ...
  public void encode(BasicCookieStore state, boolean isAdminReq, String cookieDomain)
    ...
    BasicClientCookie cookie =
        new BasicClientCookie(
            ZimbraCookie.COOKIE_ZM_JWT, JWTUtil.getJWTSalt(properties.getEncoded()));
    cookie.setDomain(cookieDomain);
    cookie.setPath("/");
    cookie.setSecure(false);
    state.addCookie(cookie);
  ...
  public void encode(HttpServletRequest reqst, HttpServletResponse resp, boolean deregister)
    ...
    if (reqst != null && resp != null) {
      ...
      if (deregister) {
        ...
        finalValue =
            StringUtil.isNullOrEmpty(zmJwtCookieVal)
                ? salt
                : salt + Constants.JWT_SALT_SEPARATOR + zmJwtCookieVal;
        if (!StringUtil.isNullOrEmpty(finalValue)) {
          ...
          if (cookieLength > LC.zimbra_jwt_cookie_size_limit.intValue()) {
            ZimbraLog.security.debug(
                "JWT Cookie size limit exceeded, limit: %d",
                LC.zimbra_jwt_cookie_size_limit.intValue());
            throw AuthFailedServiceException.AUTH_FAILED("JWT Cookie size limit exceeded");
          }
        }
      }
      ZimbraCookie.addHttpOnlyCookie(
          resp, ZimbraCookie.COOKIE_ZM_JWT, finalValue, ZimbraCookie.PATH_ROOT, -1, true);
    }
  }
