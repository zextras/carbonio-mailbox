...
public class LdapEntrySearchFilter {
  ...
  /**
   * Takes a RFC 2254 filter and converts assertions value from unicode to ACE for IDN attributes.
   * IDN attributes are those storing the ACE representation of the unicode. For non-IDN attributes,
   * assertion values are just passed through.
   *
   * <p>e.g. (zimbraMailDeliveryAddress=*@test.\u4e2d\u6587.com) will be converted to
   * (zimbraMailDeliveryAddress=*@test.xn--fiq228c.com) because zimbraMailDeliveryAddress is an IDN
   * attribute.
   *
   * <p>(zimbraDomainName=*\u4e2d\u6587*) will remain the same because zimbraDomainName is not an
   * IDN attribute.
   *
  ...
  public static String toLdapIDNFilter(String filterStr) {
    ...
    try {
      ...
      ZimbraLog.account.debug(
          "original query=[" + filterStr + "], converted ascii query=[" + asciiQuery + "]");
    } catch (ServiceException e) {
      ZimbraLog.account.warn(
          "unable to convert query to ascii, using original query: " + filterStr, e);
