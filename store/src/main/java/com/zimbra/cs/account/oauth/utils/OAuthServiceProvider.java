...
/**
 * Utility methods for providers that store consumers, tokens and secrets in local cache (HashSet).
 * Consumer key is used as the name, and its credentials are stored in HashSet.
 *
 * <p>OAuth 1.0A specification is followed.
 *
...
public class OAuthServiceProvider {
  ...
  public static synchronized void setAccountPropertiesForAccessor(
      Account account, OAuthAccessor accessor) throws UnsupportedEncodingException {
    accessor.setProperty(
        "ZM_ACC_DISPLAYNAME",
        account.getAttr(Provisioning.A_displayName) == null
            ? ""
            : URLEncoder.encode(account.getAttr(Provisioning.A_displayName), "UTF-8"));
    accessor.setProperty(
        "ZM_ACC_CN",
        account.getName() == null ? "" : URLEncoder.encode(account.getName(), "UTF-8"));
    accessor.setProperty(
        "ZM_ACC_GIVENNAME",
        account.getAttr(Provisioning.A_givenName) == null
            ? ""
            : URLEncoder.encode(account.getAttr(Provisioning.A_givenName), "UTF-8"));
    accessor.setProperty(
        "ZM_ACC_SN",
        account.getAttr(Provisioning.A_sn) == null
            ? ""
            : URLEncoder.encode(account.getAttr(Provisioning.A_sn), "UTF-8"));
    accessor.setProperty(
        "ZM_ACC_EMAIL",
        account.getMail() == null ? "" : URLEncoder.encode(account.getMail(), "UTF-8"));
  }

  /** Mark OAuth consumer as authorized and update accessor properties. */
  public static synchronized void markAsAuthorized(
      OAuthAccessor accessor, String userId, String zauthtoken) throws OAuthException {
  ...
  public static void handleException(
      Exception e, HttpServletRequest request, HttpServletResponse response, boolean sendBody)
