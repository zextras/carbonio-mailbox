...
public class DomainACLAccessManager extends DomainAccessManager {
  ...
  public boolean canAccessAccount(AuthToken at, Account target, boolean asAdmin)
    ...
    if (super.canAccessAccount(at, target, asAdmin)) return true;
    else return canDo(at, target, User.R_loginAs, asAdmin, false);
  }
  ...
  public boolean canAccessAccount(Account credentials, Account target, boolean asAdmin)
      throws ServiceException {
    if (super.canAccessAccount(credentials, target, asAdmin)) return true;
    else return canDo(credentials, target, User.R_loginAs, asAdmin, false);
  }
  ...
  public boolean canDo(
      MailTarget grantee, Entry target, Right rightNeeded, boolean asAdmin, boolean defaultGrant) {
    try {
      if (grantee == null) grantee = GuestAccount.ANONYMOUS_ACCT;

      // 1. always allow self
      if (target instanceof Account) {
        if (((Account) target).getId().equals(grantee.getId())) return true;
      }
      ...
      if (rightNeeded != Rights.User.R_loginAs) {
        if (target instanceof Account) {
          if ((grantee instanceof Account)
              && canAccessAccount((Account) grantee, (Account) target, asAdmin)) return true;
        }
      }
      ...
      if (result != null) return result.booleanValue();
        ...
        if (defaultValue != null) return defaultValue.booleanValue();
  ...
  public boolean canDo(
      AuthToken grantee, Entry target, Right rightNeeded, boolean asAdmin, boolean defaultGrant) {
    try {
      Account granteeAcct;
      if (grantee == null) granteeAcct = GuestAccount.ANONYMOUS_ACCT;
      else if (grantee.isZimbraUser())
        granteeAcct = Provisioning.getInstance().get(Key.AccountBy.id, grantee.getAccountId());
      else granteeAcct = new GuestAccount(grantee);
  ...
  public boolean canDo(
      String granteeEmail, Entry target, Right rightNeeded, boolean asAdmin, boolean defaultGrant) {
    try {
      Account granteeAcct = null;

      if (granteeEmail != null)
        granteeAcct = Provisioning.getInstance().get(Key.AccountBy.name, granteeEmail);
      if (granteeAcct == null) granteeAcct = GuestAccount.ANONYMOUS_ACCT;
