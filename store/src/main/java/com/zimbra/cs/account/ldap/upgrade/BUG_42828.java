...
public class BUG_42828 extends UpgradeOp {
  ...
  void upgrade_zimbraGalLdapAttrMap() throws ServiceException {
    ...
    String oldCalResLocationDisplayName =
        "zimbraCalResLocationDisplayName=zimbraCalResLocationDisplayName";
    String newCalResLocationDisplayName =
        "zimbraCalResLocationDisplayName,displayName=zimbraCalResLocationDisplayName";
    ...
    String zimbraAccountCalendarUserType =
    ...
    for (String curValue : curValues) {
      ...
      replaceIfNeeded(
          attrs, attrName, curValue, oldCalResLocationDisplayName, newCalResLocationDisplayName);
      replaceIfNeeded(
          attrs, attrName, curValue, oldMailForwardingAddress, newMailForwardingAddress);
    }
  ...
  private void upgrade_zimbraContactHiddenAttributes(Entry entry, String entryName)
    ...
    // remove
    // zimbraCalResType,zimbraCalResLocationDisplayName,zimbraCalResCapacity,zimbraCalResContactEmail
    // add member
    ...
    if (curValue != null) {
      ...
      for (String hiddenAttr : curHiddenAttrs) {
        if (!Provisioning.A_zimbraCalResType.equalsIgnoreCase(hiddenAttr)
          ...
          if (!first) newHiddenAttrs += ",";
          else first = false;
          newHiddenAttrs += hiddenAttr;
        }

        if (ContactConstants.A_member.equalsIgnoreCase(hiddenAttr)) seenMember = true;
      }
    ...
    if (newHiddenAttrs.length() == 0)
      newHiddenAttrs =
          "dn,zimbraAccountCalendarUserType,vcardUID,vcardURL,vcardXProps"
              + ContactConstants.A_member;
