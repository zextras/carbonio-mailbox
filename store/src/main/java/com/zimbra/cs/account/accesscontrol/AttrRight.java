...
public class AttrRight extends AdminRight {
  ...
  public String dump(StringBuilder sb) {
    ...
    for (TargetType tt : mTargetTypes) sb.append(tt.name() + " ");
    sb.append("\n");

    if (mAttrs == null) sb.append("all attrs\n");
    else {
      sb.append("attrs:\n");
      for (String a : mAttrs) {
        sb.append("    " + a + "\n");
  ...
  boolean overlaps(Right other) throws ServiceException {
    if (other.isPresetRight()) {
      ...
      return false;
    } else throw ServiceException.FAILURE("internal error", null);
  }
  ...
  Set<String> getAllAttrs() throws ServiceException {
    if (!allAttrs())
      throw ServiceException.FAILURE(
          "internal error, can only be called if allAttrs is true", null);

    // this should not happen, since we've validated it in completeRight, just sanity check,
    // remove when things are stable
    if (mTargetTypes.size() != 1) throw ServiceException.FAILURE("internal error", null);
   ...
   *
   * <p>it is suitable if: - this right and the needed right type is the same get/set or - this
   * right is set and the needed right is get
   *
   * @param needed
   * @return
   */
  boolean suitableFor(RightType needed) {
    if (needed == mRightType || needed == RightType.getAttrs && mRightType == RightType.setAttrs) {
      return true;
    } else {
      return false;
    }
  ...
  void validateAttr(String attrName) throws ServiceException {
    ...
    for (TargetType tt : mTargetTypes) {
      ...
      if (!am.getAllAttrsInClass(klass).contains(attrName))
        throw ServiceException.FAILURE(
            "attribute " + attrName + " is not on " + tt.getCode(), null);
    }
  }
}
