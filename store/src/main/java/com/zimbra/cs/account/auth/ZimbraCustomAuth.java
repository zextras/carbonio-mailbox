...
public abstract class ZimbraCustomAuth {
  ...
  public static synchronized void register(String handlerName, ZimbraCustomAuth handler) {
    ...
    if (mHandlers == null) mHandlers = new HashMap<String, ZimbraCustomAuth>();
    else {
      //  sanity check
  ...
  /**
   * Returns if handler is registered. Given name must not be null + must be registered.
   *
  ...
  public static synchronized ZimbraCustomAuth getHandler(String handlerName) {
    if (mHandlers == null) return null;
    else return mHandlers.get(handlerName);
  }
