...
public class GalNamedFilter {
  ...
  public static String getZimbraCalendarResourceFilter(GalOp galOp) {
    ...
    if (galOp == GalOp.autocomplete) filter = GalNamedFilter.ZIMBRA_CALENDAR_RESOURCE_AUTO_COMPLETE;
    else if (galOp == GalOp.search) filter = GalNamedFilter.ZIMBRA_CALENDAR_RESOURCES;
    else if (galOp == GalOp.sync) filter = GalNamedFilter.ZIMBRA_CALENDAR_RESOURCE_SYNC;
  ...
  public static String getZimbraGroupFilter(GalOp galOp) {
    ...
    if (galOp == GalOp.autocomplete) filter = GalNamedFilter.ZIMBRA_GROUP_AUTO_COMPLETE;
    else if (galOp == GalOp.search) filter = GalNamedFilter.ZIMBRA_GROUPS;
    else if (galOp == GalOp.sync) filter = GalNamedFilter.ZIMBRA_GROUP_SYNC;
  ...
  public static String getZimbraAcountFilter(GalOp galOp) {
    ...
    if (galOp == GalOp.autocomplete) filter = GalNamedFilter.ZIMBRA_ACCOUNT_AUTO_COMPLETE;
    else if (galOp == GalOp.search) filter = GalNamedFilter.ZIMBRA_ACCOUNTS;
    else if (galOp == GalOp.sync) filter = GalNamedFilter.ZIMBRA_ACCOUNT_SYNC;
