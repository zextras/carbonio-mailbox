...
public class LdapLockoutPolicy {
  ...
  public LdapLockoutPolicy(Provisioning prov, Account account) throws ServiceException {
    ...
    long max2FAFailures =
        mAccount.getLongAttr(Provisioning.A_zimbraTwoFactorAuthLockoutMaxFailures, 0);
    mEnabled =
        (maxFailures > 0 || max2FAFailures > 0)
            && mAccount.getBooleanAttr(Provisioning.A_zimbraPasswordLockoutEnabled, false);
  ...
  private boolean computeIsLockedOut() throws ServiceException {
    ...
    Date locked =
        mAccount.getGeneralizedTimeAttr(Provisioning.A_zimbraPasswordLockoutLockedTime, null);
  ...
  public void successfulLogin() {
    ...
    if (twoFactorFailedLogins != null
        && twoFactorFailedLogins.mEnabled
        && twoFactorFailedLogins.mFailures.length > 0) {
        ...
        ZimbraLog.security.info(
            ZimbraLog.encodeAttrs(
                new String[] {
                  ...
                  mAccount.getName(),
                  "info",
                  "account re-activated from lockout status upon successful login"
                }));
    ...
    try {
      if (attrs.size() > 0) mProv.modifyAttrs(mAccount, attrs);
    } catch (Exception e) {
      ZimbraLog.account.warn(
          "Unable to update account password lockout attrs: " + mAccount.getName(), e);
    }
  }
  ...
  private static class PasswordLockoutCache {
    ...
    private static Cache<String, List<String>> cache =
        CacheBuilder.newBuilder()
            ...
            .expireAfterWrite(cacheExpiryInMinute, TimeUnit.MINUTES)
            .build();
              ...
              synchronized (pwds) {
                ...
                if (cacheSize == 0) {
                  ...
                  ZimbraLog.account.debug(
                      "Password lockout suppression cache size = %s (max = %s)",
      ...
      ZimbraLog.security.info(
          ZimbraLog.encodeAttrs(
              new String[] {
                ...
                mAccount.getName(),
                "error",
                "account lockout due to too many failed logins"
              }));
      attrs.put(
  ...
  private static class FailedLoginState {
    ...
    /**
     * update the failure time attr list. remove oldest if it at limit, add new entry, and return
    ...
    private int updateFailureTimes(Map<String, Object> attrs) {
      ...
      long duration =
          mAccount.getTimeInterval(Provisioning.A_zimbraPasswordLockoutFailureLifetime, 0);
      if (duration != 0) {
        String expiredTime =
            LdapDateUtil.toGeneralizedTime(new Date(System.currentTimeMillis() - duration));
      ...
      return 1
          + mFailures.length
          - (removeOldest ? 1 : 0)
          - (mFailuresToRemove == null ? 0 : mFailuresToRemove.size());
    }
  }
}
