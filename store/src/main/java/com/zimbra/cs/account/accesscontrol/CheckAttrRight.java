...
public class CheckAttrRight extends CheckRight {
  ...
  public static AllowedAttrs accessibleAttrs(
      Grantee grantee, Entry target, AttrRight rightNeeded, boolean canDelegateNeeded)
      throws ServiceException {

    CheckAttrRight checker = new CheckAttrRight(grantee, target, rightNeeded, canDelegateNeeded);

    return checker.computeAccessibleAttrs();
  }

  private CheckAttrRight(
      Grantee grantee, Entry target, AttrRight rightNeeded, boolean canDelegateNeeded)
      throws ServiceException {
  ...
  private AllowedAttrs computeAccessibleAttrs() throws ServiceException {
    ...
    List<ZimbraACE> acl = ACLHelper.getAllACEs(mTarget);
    if (acl != null) {
    ...
    if (!car.isAll()) {
      ...
      while ((grantedOn = iter.next()) != null && (!car.isAll())) {
        acl = ACLHelper.getAllACEs(grantedOn);
    ...
    if (sLog.isDebugEnabled()) {
      ...
      // bug 73188, if car is SOME but there is no attr, log NONE to be less confusing
    ...
    if (car == CollectAttrsResult.ALLOW_ALL) result = processAllowAll(allowSome, denySome, klass);
      ...
      if (!conflicts.isEmpty()) {
        for (String attr : conflicts) {
          if (denySome.get(attr) <= allowSome.get(attr)) allowSome.remove(attr);
        }
      }
  ...
  private CollectAttrsResult checkTarget(
      ...
      Integer relativity,
      boolean subDomain,
      Map<String, Integer> allowSome,
      Map<String, Integer> denySome)
      throws ServiceException {
  ...
  /**
   * expand attr rights on this collection of ACEs into attributes. for each grant(ACE): - checks if
   * grantee matches - if preset right - ignore if attrs right - expand into attributes if combo
   * right - go through all attr rights of the combo right and expand them into attributes
   *
   ...
   * @param relativity how relevant this set of grant is to the perspective target entry e.g. On the
   *     target side, if the perspective target is an account, then grants on the account entry is
   *     more relevant than grants on groups the account is a member of, and which is more relevant
   *     than the domain the account is in. On the grantee side, it is more relevant if the grantee
   *     matches the grant because of the grant is directly granted to the grantee; it is less
   *     relevant if the grantee is a member of a group to which the grant is granted. This is
   *     needed for computing the net set of attributes that should be allowed/denied after all attr
   *     rights are expanded. If an attribute is denied by a more relevant grant and allowed by a
   *     less relevant grant, it will be denied.
   * @param allowSome output param for allowed attributes, set when CollectAttrsResult.SOME is
   *     returned
   * @param denySome output param for denied attributes, set when CollectAttrsResult.SOME is
   *     returned
   * @return if the acl: allow all(CollectAttrsResult.ALLOW_ALL), or deny
   *     all(CollectAttrsResult.DENY_ALL), or allow/deny some(CollectAttrsResult.SOME) attributes to
   *     be get/set on the target.
   *     <p>If CollectAttrsResult.SOME is returned, attributes allowed are put in the allowSome
   *     param, attributes specifically denied are put in the denySome param.
   * @throws ServiceException
   */
  private CollectAttrsResult expandACLToAttrs(
    ...
    for (ZimbraACE ace : acl) {
      ...
      if (!granteeType.hasFlags(granteeFlags)) continue;
      ...
      if (rightGranted.isPresetRight()) continue;
       ...
       */
      if (mCanDelegateNeeded && ace.canExecuteOnly()) continue; // just ignore the grant

      // negative grants are always effective on sub domains
      if (!ace.deny()) {
        if (subDomain != ace.subDomain()) continue;
      }
      ...
      if (rightGranted.isAttrRight()) {
         ...
         */
        if (result != null && result.isAll()) return result;
  ...
  private CollectAttrsResult expandAttrsGrantToAttrs(
     ...
     */
    if (!attrRightGranted.suitableFor(rightTypeNeeded)) return null;
     ...
     *
     */
    if (mAttrRightNeeded != AdminRight.PR_GET_ATTRS
        && mAttrRightNeeded != AdminRight.PR_SET_ATTRS) {
      // not pseudo right, that means we are checking for a specific right
      if (SetUtil.intersect(attrRightGranted.getTargetTypes(), mAttrRightNeeded.getTargetTypes())
          .isEmpty()) return null;
    }
    ...
    if (attrRightGranted.allAttrs()) {
      ...
      if (ace.deny()) {
        ...
        else return null; // just ignore the grant
      } else {
  ...
  private AllowedAttrs processDenyAll(
      Map<String, Integer> allowSome, Map<String, Integer> denySome, AttributeClass klass)
      throws ServiceException {
    if (allowSome.isEmpty()) return AllowedAttrs.DENY_ALL_ATTRS();
    else {
  ...
  private AllowedAttrs processAllowAll(
      Map<String, Integer> allowSome, Map<String, Integer> denySome, AttributeClass klass)
      throws ServiceException {
    ...
    if (denySome.isEmpty()) {
      ...
      Set<String> allowed =
          new HashSet<String>(AttributeManager.getInstance().getAllAttrsInClass(klass));

      // remove denied from all
      for (String d : denySome.keySet()) allowed.remove(d);
      return AllowedAttrs.ALLOW_SOME_ATTRS(allowed);
    }
  }
}
