...
class SoapSignature extends Signature implements SoapEntry {
  ...
  public static void toXML(Element signature, Map<String, Object> attrs) throws ServiceException {
    for (Map.Entry entry : attrs.entrySet()) {
      ...
      if (attr.equals(Provisioning.A_zimbraSignatureId) && !StringUtil.isNullOrEmpty(value))
      ...
      else if (attr.equals(Provisioning.A_zimbraPrefMailSignatureContactId)
        ...
        if (mimeType == null)
          throw ZClientException.CLIENT_ERROR(
              "unable to determine mime type from attr " + attr, null);

        signature
            .addElement(AccountConstants.E_CONTENT)
            .addAttribute(AccountConstants.A_TYPE, mimeType)
            .addText(value);
      }
    }
  }
