...
public class AccountStatus extends AttributeCallback {
  ...
  /**
   * disable mail delivery if account status is changed to closed reset lockout attributes if
   * account status is changed to active
   */
  ...
  public void preModify(
    ...
    if (mod.unsetting())
      throw ServiceException.INVALID_REQUEST(
          Provisioning.A_zimbraAccountStatus + " is a required attribute", null);
    else status = mod.value();

    if (status.equals(Provisioning.ACCOUNT_STATUS_CLOSED)
        || status.equals(Provisioning.ACCOUNT_STATUS_PENDING)) {
      ...
      if (entry instanceof Account) {
        try {
          ...
          ZimbraLog.account.warn(
              "unable to remove account address and aliases from all DLs for closed account", se);
          return;
        }
      }
    }
  ...
  private void handleAccountStatusClosed(Account account) throws ServiceException {
    ...
    if (status.equals(Provisioning.ACCOUNT_STATUS_CLOSED)) {
      ZimbraLog.misc.info(
          "removing account address and all its aliases from all distribution lists");
