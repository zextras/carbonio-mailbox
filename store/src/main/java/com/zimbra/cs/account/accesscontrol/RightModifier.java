...
public enum RightModifier {
   ...
   *
   */
  RM_DISINHERIT_SUB_GROUPS(
      '^',
      AdminConstants.A_DISINHERIT_SUB_GROUPS,
      "the grant cannot be inherited by sub-groups on the target side");
  ...
  public static RightModifier fromChar(char c) throws ServiceException {
    ...
    if (RM_DENY.mModifier == c) return RM_DENY;
    else if (RM_CAN_DELEGATE.mModifier == c) return RM_CAN_DELEGATE;
    else if (RM_DISINHERIT_SUB_GROUPS.mModifier == c) return RM_DISINHERIT_SUB_GROUPS;
    else if (RM_SUBDOMAIN.mModifier == c) return RM_SUBDOMAIN;
    else return null;
  }
