...
/**
 * @author gsolovyev a registry of authtokens that have been deregistered (logged out) and need to
 *     be broadcasted to other servers. This class implements storage of tokens as a queue limited
 *     in size by {@link LC#zimbra_deregistered_authtoken_queue_size} and a {@link TimerTask} task
 *     responsible for broadcasting the queue to other servers. TODO: switch to using a global
 *     shared registry such as Redis or a subscription based notification mechanism such as RabbitMQ
 */
public final class AuthTokenRegistry {
  ...
  private static final ConcurrentLinkedQueue<AuthToken> deregisteredOutAuthTokens =
      new ConcurrentLinkedQueue<AuthToken>();
   ...
   * @param token
   */
  public static final void addTokenToQueue(AuthToken token) {
    while (deregisteredOutAuthTokens.size() > LC.zimbra_deregistered_authtoken_queue_size.intValue()
        && !deregisteredOutAuthTokens.isEmpty()) {
        ...
        if (copy.length > 0) {
          ...
          try {
            ...
            for (Object ztoken : copy) {
              if (!((AuthToken) ztoken)
                  .isExpired()) { // no need to broadcast expired tokens, since they will be cleaned
                // up automatically
