...
public class AvailableZimlets extends AttributeCallback {
  ...
  public void preModify(
    ...
    if (replacing != null) availZimlets = doReplacing(replacing);
    else availZimlets = doDeletingAdding(entry, attrName, deleting, adding);
  ...
  private ZimletPresence doReplacing(Object replacing) {
    ...
    // dedupe conflicts (e.g. both !foo and +foo are provided) by putting values to the
    // ZimletPresence
    ...
    if (replacing instanceof String) {
      ...
      for (String v : (String[]) replacing) availZimlets.put(v);
    }
   ...
   */
  private ZimletPresence doDeletingAdding(
      Entry entry, String attrName, Object deleting, Object adding) {
    ZimletPresence availZimlets = new ZimletPresence();
    ...
    if (deleting != null) {
      if (deleting instanceof String) availZimlets.remove((String) deleting);
      else {
        for (String v : (String[]) deleting) availZimlets.remove(v);
      }
    }
    ...
    if (adding != null) {
      if (adding instanceof String) availZimlets.put((String) adding);
      else if (adding instanceof String[]) {
        for (String v : (String[]) adding) availZimlets.put(v);
      }
    }
