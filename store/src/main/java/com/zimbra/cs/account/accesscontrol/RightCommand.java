...
public class RightCommand {
  ...
  public static class Grants {
    ...
    public Grants(Element parent) throws ServiceException {
      for (Element eGrant : parent.listElements(AdminConstants.E_GRANT)) {
        ...
        boolean disinheritSubGroups =
            eRight.getAttributeBool(AdminConstants.A_DISINHERIT_SUB_GROUPS, false);
         ...
         */
        if (deny) rightModifier = RightModifier.RM_DENY;
        else if (canDelegate) rightModifier = RightModifier.RM_CAN_DELEGATE;
        else if (disinheritSubGroups) rightModifier = RightModifier.RM_DISINHERIT_SUB_GROUPS;
        else if (subDomain) rightModifier = RightModifier.RM_SUBDOMAIN;
  ...
  public static class EffectiveAttr {
    ...
    public Set<String> getDefault() {
      if (mDefault == null) return EMPTY_SET;
  ...
  public static class EffectiveRights {
    private static final SortedMap<String, EffectiveAttr> EMPTY_MAP =
        new TreeMap<String, EffectiveAttr>();
    ...
    private String getDigest() {
      if (mDigest != null) return mDigest;
      ...
      if (mCanSetAllAttrs) rights.append("all;");
      else {
      ...
      if (mCanGetAllAttrs) rights.append("all;");
      else {
    ...
    public static EffectiveRights fromXML_CreateObjectAttrs(Element parent)
      ...
      if (eSetAttrs.getAttributeBool(AdminConstants.A_ALL, false)) er.mCanSetAllAttrs = true;
    ...
    private static EffectiveRights fromJaxb(EffectiveRightsInfo eRights) throws ServiceException {
      ...
      for (RightWithName eRight : eRights.getRights()) er.mPresetRights.add(eRight.getName());
      ...
      if (eRights instanceof EffectiveRightsTargetInfo) {
        EffectiveRightsTargetInfo eTargInfo = (EffectiveRightsTargetInfo) eRights;
    ...
    private static TreeMap<String, EffectiveAttr> from_attrs(EffectiveAttrsInfo eAttrs)
        throws ServiceException {
      ...
      for (EffectiveAttrInfo eAttr : eAttrs.getAttrs()) {
        ...
        AttributeConstraint constraint =
            AttributeConstraint.fromJaxb(am, attrName, eAttr.getConstraint());
    ...
    private void toXML(
        Element parent, String elemName, boolean allAttrs, SortedMap<String, EffectiveAttr> attrs) {
      ...
      for (EffectiveAttr ea : attrs.values()) {
        ...
        if (constraint != null) constraint.toXML(eAttr);
  ...
  public static class RightsByTargetType {
    ...
    protected static void addAggregation(
        Set<RightAggregation> entries, Set<String> names, EffectiveRights er) {
  ...
  public static class AllEffectiveRights {
    ...
    AllEffectiveRights(String granteeType, String granteeId, String granteeName) {
      ...
      for (TargetType tt : TargetType.values()) {
        if (tt.isDomained()) mRightsByTargetType.put(tt, new DomainedRightsByTargetType());
        else mRightsByTargetType.put(tt, new RightsByTargetType());
      }
    }
    ...
    public static AllEffectiveRights fromJaxb(GetAllEffectiveRightsResponse resp)
      ...
      AllEffectiveRights aer =
          new AllEffectiveRights(granteeType, grantee.getId(), grantee.getName());

      for (EffectiveRightsTarget target : resp.getTargets()) {
        TargetType targetType = TargetType.fromCode(target.getType().toString());
        ...
        if (rbtt instanceof DomainedRightsByTargetType) {
          ...
          for (InDomainInfo inDomInfo : target.getInDomainLists()) {
            ...
            for (NamedElement dom : inDomInfo.getDomains()) domains.add(dom.getName());
            EffectiveRights er = EffectiveRights.fromJaxb(inDomInfo.getRights());
        ...
        for (RightsEntriesInfo eEntries : target.getEntriesLists()) {
          ...
          for (NamedElement eEntry : eEntries.getEntries()) entries.add(eEntry.getName());
    ...
    public void toXML(Element parent) {
      ...
      //
      for (Map.Entry<TargetType, RightsByTargetType> rightsByTargetType :
          mRightsByTargetType.entrySet()) {
  ...
  private static void verifyAccessManager() throws ServiceException {
    if (!(AccessManager.getInstance() instanceof ACLAccessManager))
      throw ServiceException.FAILURE(
          "method is not supported by the current AccessManager: "
              + AccessManager.getInstance().getClass().getCanonicalName()
              + ", this method requires access manager "
              + ACLAccessManager.class.getCanonicalName(),
          null);
  }
  ...
  private static AdminConsoleCapable verifyAdminConsoleCapable() throws ServiceException {
    ...
    if (am instanceof AdminConsoleCapable) return (AdminConsoleCapable) am;
    else
      throw ServiceException.FAILURE(
          "method is not supported by the current AccessManager: "
              + AccessManager.getInstance().getClass().getCanonicalName()
              + ", this method requires an admin console capable access manager",
          null);
  }
  ...
  /**
   * return rights that can be granted on target with the specified targetType e.g. renameAccount
   * can be granted on a domain, a distribution list, or an account target
   *
   * <p>Note: this is *not* the same as "rights executable on targetType" e.g. renameAccount is
   * executable on account entries.
   *
   * @param targetType
   * @return
   * @throws ServiceException
  ...
  private static void getAllRights(
      TargetType targetType, Map<String, ? extends Right> rights, List<Right> result)
      throws ServiceException {
  ...
  public static boolean checkRight(
    ...
    boolean asAdmin =
        (grantee instanceof Account) ? am.isAdequateAdminAccount((Account) grantee) : false;
    boolean result = am.canPerform(grantee, targetEntry, r, false, attrs, asAdmin, via);
    return result;
  }

  /**
   * Particularly in the case of delegated administration, CollectAllEffectiveRights.collect()
   * Stage1 can be expensive. This cache trades accuracy for speed, so provides a short term cache
   */
  ...
  static {
    ...
    try {
      ...
      if (allEffectiveRightsCacheSize > 0) {
        allEffectiveRightsCacheExpireAfterMillis =
            server.getShortTermAllEffectiveRightsCacheExpiration();
  ...
  public static void clearAllEffectiveRightsCache() {
    if (null != ALL_EFFECTIVE_RIGHTS_CACHE) {
      ZimbraLog.acl.debug(
          "Clearing short term all effective rights cache of %d items.",
          ALL_EFFECTIVE_RIGHTS_CACHE.size());
      ALL_EFFECTIVE_RIGHTS_CACHE.invalidateAll();
    }
  }

  private static AllEffectiveRights getAllEffectiveRights(
      AdminConsoleCapable acc,
      GranteeType granteeType,
      NamedEntry granteeEntry,
      RightBearer rightBearer,
      boolean expandSetAttrs,
      boolean expandGetAttrs)
      throws ServiceException {
    AllEffectiveRights aer =
        new AllEffectiveRights(granteeType.getCode(), granteeEntry.getId(), granteeEntry.getName());
    acc.getAllEffectiveRights(rightBearer, expandSetAttrs, expandGetAttrs, aer);
    return aer;
  }

  public static AllEffectiveRights getAllEffectiveRights(
      Provisioning prov,
      String granteeType,
      GranteeBy granteeBy,
      String grantee,
      ...
      if (aer == null) {
        aer =
            getAllEffectiveRights(
                acc, gt, granteeEntry, rightBearer, expandSetAttrs, expandGetAttrs);
        ALL_EFFECTIVE_RIGHTS_CACHE.put(cacheKey, aer);
      }
    } else {
      aer =
          getAllEffectiveRights(acc, gt, granteeEntry, rightBearer, expandSetAttrs, expandGetAttrs);
    }
    return aer;
  }

  public static EffectiveRights getEffectiveRights(
      Provisioning prov,
      String targetType,
      TargetBy targetBy,
      String target,
    ...
    EffectiveRights er =
        new EffectiveRights(
            targetType,
            TargetType.getId(targetEntry),
            targetEntry.getLabel(),
            granteeAcct.getId(),
            granteeAcct.getName());
  ...
  public static EffectiveRights getCreateObjectAttrs(
    ...
    Entry targetEntry =
        PseudoTarget.createPseudoTarget(
            prov, tt, domainBy, domainStr, false, cosBy, cosStr, domainName);
  ...
  public static Grants getGrants(
    ...
    if (targetEntry != null) {
      ...
      ZimbraACL zimbraAcl = ACLHelper.getACL(targetEntry);
  ...
  private static void validateGrant(
    ...
    if (!right.isUserRight() || RightModifier.RM_CAN_DELEGATE == rightModifier) {
      ...
      if (!granteeType.allowedForAdminRights()) {
        throw ServiceException.INVALID_REQUEST(
            "grantee type " + granteeType.getCode() + " is not allowed for admin right", null);
      }
    }
    ...
    if (!right.grantableOnTargetType(targetType)) {
      throw ServiceException.INVALID_REQUEST(
          "right "
              + right.getName()
              + " cannot be granted on a "
              + targetType.getCode()
              + " entry. "
              + "It can only be granted on target types: "
              + right.reportGrantableTargetTypes(),
          null);
    }
    ...
    if (RightModifier.RM_SUBDOMAIN == rightModifier) {
      ...
      if (targetType != TargetType.dl) {
        throw ServiceException.INVALID_REQUEST(
            "right modifier "
                + RightModifier.RM_DISINHERIT_SUB_GROUPS.getModifier()
                + " can only be granted on group targets",
            null);
      }
      ...
      if (granteeType == GranteeType.GT_EXT_GROUP) {
        ...
        if (!canGrant) {
          throw ServiceException.PERM_DENIED(
              String.format(
                  "insufficient right to %s '%s' right",
    ...
    if (secret != null && !granteeType.allowSecret()) {
      throw ServiceException.PERM_DENIED(
          "password is not allowed for grantee type " + granteeType.getCode());
    }
  }
  ...
  public static void grantRight(
      Provisioning prov,
      Account authedAcct,
      EffectiveRightsTargetSelector targSel,
      GranteeSelector granteeSel,
      String right,
      RightModifier rightModifier)
      throws ServiceException {
    grantRightInternal(
        prov,
        authedAcct,
        TargetType.fromJaxb(targSel.getType()),
        targSel.getBy(),
        targSel.getValue(),
        GranteeType.fromJaxb(granteeSel.getType()),
        granteeSel.getBy(),
        granteeSel.getKey(),
        granteeSel.getSecret(),
        right,
        rightModifier,
        false);
  }
  ...
  private static void grantRightInternal(
    ...
    ACLHelper.grantRight(prov, targetEntry, aces);
  }
  ...
  public static void revokeRight(
    ...
    try {
      ...
      if (AccountServiceException.NO_SUCH_ACCOUNT.equals(code)
        ...
        if (granteeBy == GranteeBy.id) granteeId = grantee;
        else
          throw ServiceException.INVALID_REQUEST(
              "cannot find grantee by name: "
                  + grantee
                  + ", try revoke by grantee id if you want to remove the orphan grant",
              e);
      } else throw e;
    }
    ...
    List<ZimbraACE> revoked = ACLHelper.revokeRight(prov, targetEntry, aces);
    if (revoked.isEmpty()) throw AccountServiceException.NO_SUCH_GRANT(ace.dump(true));
  }
   ...
   *
   * <p>note: no verification (things done in verifyGrant) is done in this method if the authed user
   * can delete the account, it can delete all grants granted to the account.
   */
  public static void revokeAllRights(Provisioning prov, GranteeType granteeType, String granteeId)
    ...
    for (GrantsOnTarget grantsOnTarget : grantsOnTargets) {
      ...
      ACLHelper.revokeRight(prov, targetEntry, acesToRevoke);
    }
    RightBearer.Grantee
        .clearGranteeCache(); // as a precaution in case a new account is created with same name
  }

  public static Element rightToXML(
      Element parent, Right right, boolean expandAllAtrts, Locale locale) throws ServiceException {
    ...
    String desc =
        L10nUtil.getMessage(
            false /* shoutIfMissing */, L10nUtil.MSG_RIGHTS_FILE_BASENAME, right.getName(), locale);
    if (desc == null) {
      desc = right.getDesc();
    }
