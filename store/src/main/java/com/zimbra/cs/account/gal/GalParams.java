...
public abstract class GalParams {
  ...
  GalParams(Entry ldapEntry, GalOp galOp) throws ServiceException {
    ...
    if (galOp == GalOp.sync) {
      ...
      if (pageSize == null) pageSize = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapPageSize);
    } else {
  ...
  GalParams(Map attrs, GalOp galOp) {
    ...
    if (galOp == GalOp.sync) {
      ...
      if (pageSize == null) pageSize = (String) attrs.get(Provisioning.A_zimbraGalLdapPageSize);
    } else {
  ...
  public String tokenizeAutoCompleteKey() {
    return mTokenizeAutoCompleteKey;
  }

  public String tokenizeSearchKey() {
    return mTokenizeSearchKey;
  }
  ...
  public static class ExternalGalParams extends GalParams {
    ...
    public ExternalGalParams(Entry ldapEntry, GalOp galOp) throws ServiceException {
      ...
      if (galOp == GalOp.sync) {
        ...
        if (mFilter == null) mFilter = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapFilter);

        if (startTlsEnabled == null)
          startTlsEnabled = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapStartTlsEnabled);
        if (authMech == null) authMech = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapAuthMech);
        if (bindDn == null) bindDn = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapBindDn);
        ...
        else mFilter = ldapEntry.getAttr(Provisioning.A_zimbraGalLdapFilter);
      ...
      mCredential =
          new LdapGalCredential(authMech, bindDn, bindPassword, krb5Principal, krb5Keytab);
    }
    ...
    public ExternalGalParams(Map attrs, GalOp galOp) throws ServiceException {
      ...
      if (galOp == GalOp.sync) {
        ...
        if (mFilter == null) mFilter = getRequiredAttr(attrs, Provisioning.A_zimbraGalLdapFilter);

        if (startTlsEnabled == null)
          startTlsEnabled = (String) attrs.get(Provisioning.A_zimbraGalLdapStartTlsEnabled);
        if (authMech == null) authMech = (String) attrs.get(Provisioning.A_zimbraGalLdapAuthMech);
        if (bindDn == null) bindDn = (String) attrs.get(Provisioning.A_zimbraGalLdapBindDn);
        if (bindPassword == null)
        ...
        else mFilter = getRequiredAttr(attrs, Provisioning.A_zimbraGalLdapFilter);
    ...
    private static String[] getMultiAttr(Map attrs, String name, boolean required)
      ...
      if (required) throw ServiceException.INVALID_REQUEST("must specifiy: " + name, null);
      else return null;
    }

    private static String getRequiredAttr(Map attrs, String name) throws ServiceException {
      String value = (String) attrs.get(name);
      if (value == null) throw ServiceException.INVALID_REQUEST("must specifiy: " + name, null);
      return value;
    }
    ...
    public boolean requireStartTLS() {
      return mRequireStartTLS;
    }

    public String searchBase() {
      return GalSearchConfig.fixupExternalGalSearchBase(mSearchBase);
    }
