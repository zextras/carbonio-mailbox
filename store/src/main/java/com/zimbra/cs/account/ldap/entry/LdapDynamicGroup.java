...
public class LdapDynamicGroup extends DynamicGroup implements LdapEntry {
  ...
  @Override
  public String[] getAllMembers(boolean supportNonDefaultMemberURL) throws ServiceException {
  ...
  public static String getDefaultMemberURL(String zimbraId, String staticUnitZimbraId) {
    return String.format(
        "ldap:///??sub?(|(zimbraMemberOf=%s)(zimbraId=%s))", zimbraId, staticUnitZimbraId);
  }
  ...
  private static final Set<ObjectType> DYNAMIC_GROUPS_TYPE =
      Sets.newHashSet(ObjectType.dynamicgroups);
  private static final String[] BASIC_ATTRS = {
    Provisioning.A_zimbraId, Provisioning.A_zimbraIsAdminGroup, Provisioning.A_memberURL
  };

  public static GroupMembership updateGroupMembershipForCustomDynamicGroups(
      LdapProvisioning prov,
      GroupMembership membership,
      Account acct,
      Domain domain,
      boolean adminGroupsOnly)
    ...
    try {
      zlcCompare =
          LdapClient.getContext(LdapServerType.get(false /* useMaster */), LdapUsage.COMPARE);
      BySearchResultEntrySearcher searcher =
          new BySearchResultEntrySearcher(
              prov,
              (ZLdapContext) null,
              domain,
              BASIC_ATTRS,
              new GroupMembershipUpdator(
                  prov, zlcCompare, acctDN, membership, adminGroupsOnly, true, false));
  ...
  public static GroupMembership updateGroupMembershipForDynamicGroups(
      LdapProvisioning prov,
      GroupMembership membership,
      Account acct,
      Collection<String> ids,
      boolean adminGroupsOnly,
      boolean customGroupsOnly,
      boolean nonCustomGroupsOnly)
      throws ServiceException {
    if (ids.size() == 0) {
      return membership;
    }
    ...
    ZLdapFilter filter =
        ZLdapFilterFactory.getInstance().dynamicGroupByIds(ids.toArray(new String[0]));
    ZLdapContext zlcCompare = null;
    try {
      zlcCompare =
          LdapClient.getContext(LdapServerType.get(false /* useMaster */), LdapUsage.COMPARE);
  ...
  public static class GroupMembershipUpdator
    ...
    public GroupMembershipUpdator(
        LdapProvisioning prov,
        ZLdapContext zlcCompare,
        String acctDN,
        GroupMembership membership,
        boolean adminGroupsOnly,
        boolean customGroupsOnly,
    ...
    public void processSearchEntry(ZSearchResultEntry sr) {
      ...
      try {
        if (prov.getHelper()
            .compare(sr.getDN(), Provisioning.A_member, acctDN, zlcCompare, false)) {
          membership.append(new MemberOf(id, isAdmin, true));
        }
      } catch (ServiceException e) {
        ZimbraLog.search.debug(
            "Problem doing compare on group %s for member %s - ignoring", sr.getDN(), acctDN, e);
      }
    }
  }
}
