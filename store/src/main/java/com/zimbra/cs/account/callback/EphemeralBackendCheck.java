...
public class EphemeralBackendCheck extends AttributeCallback {
  ...
  public void preModify(
    ...
    if (attrName.equalsIgnoreCase(Provisioning.A_zimbraEphemeralBackendURL)) {
      ...
      if (tokens != null && tokens.length > 0) {
        ...
        if (factory == null) {
          // Probably called from zmprov in LDAP mode, so need to setup any Ephemeral Store
          // extensions
        ...
        if (factory == null) {
          throw ServiceException.FAILURE(
              String.format(
                  "unable to modify %s; no factory found for backend '%s'", attrName, backend),
              null);
        }
        checkMigration(url);
        try {
          ...
          throw ServiceException.FAILURE(
              String.format("cannot set zimbraEphemeralBackendURL to %s", url), e);
        }
      } else {
        throw ServiceException.FAILURE(
            String.format("unable to modify %s; no ephemeral backend specified", attrName), null);
      }
      savePreviousUrl(context, entry);
    }
  }
  ...
  private void resetMigrationInfo(String newURL) throws ServiceException {
    // Clear out the migration info if the backend URL we migrated to matches the URL used for
    // migration,
  ...
  public void postModify(CallbackContext context, String attrName, Entry entry) {
    ...
    try {
      ...
      ZimbraLog.ephemeral.error(
          "unable to set zimbraPreviousEphemeralBackendURL to %s", prevUrl, e);
    }
    try {
  ...
  /**
   * Class encapsulating the rules governing whether the backend URL can be changed given what is
   * known about the current state of ephemeral attribute migration
   */
  @VisibleForTesting
  ...
  public static class ZimbraMigrationStateHelper extends MigrationStateHelper {
    ...
    public void deny() throws ServiceException {
      ...
      throw ServiceException.FAILURE(
          String.format(
              "Cannot change the backend URL; attribute migration to %s is currently in progress"
                  + " (started %s)",
              info.getURL(), started),
          null);
    }
      ...
      switch (reason) {
        case MIGRATION_ERROR:
          ZimbraLog.ephemeral.warn(
              "Previous attribute migration to %s did not succeed; data may not have been migrated"
                  + " to this backend",
              URL);
          return;
        case NO_MIGRATION:
          ZimbraLog.ephemeral.warn(
              "No record of an attribute migration exists; data may not have been migrated to"
                  + " backend at %s",
              URL);
          return;
        case URL_MISMATCH:
          ZimbraLog.ephemeral.warn(
              "Provided URL %s does not match current migration URL %s; data may not have been"
                  + " migrated to this backend.\n"
                  + "Ephemeral data will be forwarded to %s until migration info is reset or the"
                  + " backend is changed to match it",
              URL, info.getURL(), info.getURL());
          return;
        default:
          return;
      }
