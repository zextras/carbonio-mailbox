...
public class CheckPresetRight extends CheckRight {
   ...
   * @param via if not null, will be populated with the grant info via which the result was
   *     determined. NOTE: cache will NOT be used if via needs to be populated.
   * @return Boolean.TRUE if allowed, Boolean.FALSE if denied, null if there is no grant applicable
   *     to the rightNeeded.
   * @throws ServiceException
   */
  public static Boolean check(
      MailTarget grantee, Entry target, Right rightNeeded, boolean canDelegateNeeded, ViaGrant via)
      throws ServiceException {
    ...
    if ((cached == null) || (cached == CachedPermission.NOT_CACHED)) {
      CheckPresetRight checker =
          new CheckPresetRight(grantee, target, rightNeeded, canDelegateNeeded, via);
    ...
    if (sLog.isDebugEnabled()) {
      sLog.debug(
          "check ACL: %s (target=%s, grantee=%s, right=%s, canDelegateNeeded=%s, wasCached=%s)",
          (allowed == null ? "no matching ACL" : allowed),
          target.getLabel(),
          grantee.getName(),
          rightNeeded.getName(),
          canDelegateNeeded,
    ...
    if (mGranteeGroups == null) {
      ...
      if (mGranteeMailTarget instanceof DistributionList) {
        ...
        if (!adminGroupsOnly || dlmt.isIsAdminGroup()) {
          mGranteeGroups.append(
              new Provisioning.MemberOf(
                  dlmt.getId(), dlmt.isIsAdminGroup(), /* is dynamic group */ false),
              dlmt.getId());
        }
      }
    }
  ...
  private Boolean checkRight() throws ServiceException {
    ...
    if (!mRightNeeded.isPresetRight()) {
      throw ServiceException.INVALID_REQUEST(
          "RightChecker.canDo can only check preset right, right "
              + mRightNeeded.getName()
              + " is a "
              + mRightNeeded.getRightType()
              + " right",
          null);
    }
    ...
    if (adminRight) {
      ...
      if (granteeDomain == null) {
        throw ServiceException.FAILURE(
            "internal error, cannot find domain for " + mGranteeMailTarget.getName(), null);
      }
    ...
    while ((grantedOn = iter.next()) != null) {
      ...
      if (grantedOn instanceof Group) {
        ...
        if (adminRight) {
          skipPositiveGrants =
              !CrossDomain.crossDomainOK(
                  mProv, mGranteeMailTarget, granteeDomain, targetDomain, (Group) grantedOn);
        }
        ...
        // didn't encounter any group grantedOn, or none of them matches, just check this grantedOn
        // entry
        if (acl == null) {
          continue;
        }
  ...
  private boolean matchesPresetRight(ZimbraACE ace, short granteeFlags, boolean subDomain)
    ...
    if ((rightGranted.isPresetRight() && rightGranted == mRightNeeded)
        || rightGranted.isComboRight()
            && ((ComboRight) rightGranted).containsPresetRight(mRightNeeded)) {
      return true;
    }
  ...
  /**
   * go through each grant in the ACL - checks if the right/target of the grant matches the
   * right/target of the grant and if the grantee type(specified by granteeFlags) is one of the
   * grantee type we are interested in this call.
   *
   * <p>- if so marks the right "seen" (so callsite default(only used by user right callsites) won't
   * be honored) - check if the Account (the grantee parameter) matches the grantee of the grant
   *
   * @param acl
   * @param granteeFlags For admin rights, because of negative grants and the more "specific"
   *     grantee takes precedence over the less "specific" grantee, we can't just do a single
   *     ZimbraACE.match to see if a grantee matches the grant. Instead, we need to check more
   *     specific grantee types first, then go on the the less specific ones. granteeFlags specifies
   *     the grantee type(s) we are checking for this call. e.g. an ACL has: adminA deny rightR -
   *     grant1 groupG allow rightR - grant2 and adminA is in groupG, we want to check grant1 before
   *     grant2.
   * @return
   * @throws ServiceException
   */
  ...
  private Boolean gotResult(ZimbraACE ace) throws ServiceException {
    if (ace.deny()) {
      if (sLog.isDebugEnabled()) {
        sLog.debug(
            "Right [%s] DENIED to %s via grant: %s on: %s=%s",
            mRightNeeded.getName(),
            mGranteeMailTarget.getName(),
            ace.dump(false),
            ace.getTargetType().getCode(),
            ace.getTargetName());
      }
      ...
      if (sLog.isDebugEnabled()) {
        sLog.debug(
            "Right [%s] ALLOWED to %s via grant: %s on: %s=%s",
            mRightNeeded.getName(),
            mGranteeMailTarget.getName(),
            ace.dump(false),
            ace.getTargetType().getCode(),
