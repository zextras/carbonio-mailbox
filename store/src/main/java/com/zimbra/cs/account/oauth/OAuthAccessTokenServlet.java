    ...
    try {
      ...
      OAuthMessage oAuthMessage =
          StringUtil.isNullOrEmpty(origUrl)
              ...
              : OAuthServlet.getMessage(request, origUrl);
      ...
      AuthToken userAuthToken =
          ZimbraAuthToken.getAuthToken((String) accessor.getProperty("ZM_AUTH_TOKEN"));
      ...
      account.addOAuthAccessor(
          accessor.accessToken + "::" + new OAuthAccessorSerializer().serialize(accessor));

      response.setContentType("text/plain");
      OutputStream out = response.getOutputStream();
      OAuth.formEncode(
          OAuth.newList(
              "oauth_token", accessor.accessToken, "oauth_token_secret", accessor.tokenSecret),
          out);
      out.close();
    } catch (Exception e) {
