...
class SoapDistributionList extends DistributionList implements SoapEntry {
  ...
  /**
   * @param dlInfo contains information about a DL that another DL is a member of.
   * @throws ServiceException
   */
  SoapDistributionList(DistributionListMembershipInfo dlInfo, Provisioning prov)
      throws ServiceException {
    // DistributionListMembershipInfo does not supply attributes
    super(dlInfo.getName(), dlInfo.getId(), new HashMap<String, Object>(), prov);
    // DistributionListMembershipInfo does not supply membership info
    addDlm(new ArrayList<String>(), getRawAttrs());
  }

  SoapDistributionList(DistributionListInfo dlInfo, Provisioning prov) throws ServiceException {
    super(dlInfo.getName(), dlInfo.getId(), Attr.collectionToMap(dlInfo.getAttrList()), prov);
    addDlm(dlInfo.getMembers(), getRawAttrs());
  }

  SoapDistributionList(DLInfo dlInfo, Provisioning prov) throws ServiceException {
    super(dlInfo.getName(), dlInfo.getId(), Attr.collectionToMap(dlInfo.getAttrList()), prov);
  ...
  private void addDlm(List<String> members, Map<String, Object> attrs) {
    attrs.put(
        Provisioning.A_zimbraMailForwardingAddress, members.toArray(new String[members.size()]));
  }
