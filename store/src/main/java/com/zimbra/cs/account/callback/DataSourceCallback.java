...
public class DataSourceCallback extends AttributeCallback {
  ...
  public void postModify(CallbackContext context, String attrName, Entry entry) {
    ...
    if (context.isCreate() && (entry instanceof Cos)) return;

    if (INTERVAL_ATTRS.contains(attrName)
        || Provisioning.A_zimbraDataSourceEnabled.equals(attrName)) {
  ...
  private void validateDataSource(DataSource ds, String newInterval) throws ServiceException {
    ...
    validateInterval(
        Provisioning.A_zimbraDataSourcePollingInterval,
        newInterval,
        account.getAttr(Provisioning.A_zimbraDataSourceMinPollingInterval));
  }
  ...
  private void validateAccount(Account account, String attrName, String newInterval)
      throws ServiceException {
    validateInterval(
        attrName, newInterval, account.getAttr(Provisioning.A_zimbraDataSourceMinPollingInterval));
  }

  private void scheduleAccount(Account account) throws ServiceException {
    ZimbraLog.datasource.info(
        "Updating schedule for all DataSources for account %s.", account.getName());
  ...
  private void validateCos(Cos cos, String attrName, String newInterval) throws ServiceException {
    validateInterval(
        newInterval, attrName, cos.getAttr(Provisioning.A_zimbraDataSourceMinPollingInterval));
  }

  /**
   * Updates data source schedules for all accounts that are on the current server and in the given
   * COS.
   */
  private void scheduleCos(Cos cos) throws ServiceException {
    ZimbraLog.datasource.info(
        "Updating schedule for all DataSources for all accounts in COS %s.", cos.getName());

    List<Account> accts;
    Provisioning prov = Provisioning.getInstance();

    // Look up all account id's for this server
    if (prov instanceof LdapProv) accts = lookupAccountsFromLDAP(prov, cos.getId());
    else accts = lookupAccountsFromDB(prov);

    // Update schedules for all data sources on this server
    for (Account account : accts) {
      if (account != null
          && Provisioning.ACCOUNT_STATUS_ACTIVE.equals(account.getAccountStatus(prov))) {
    ...
    for (String accountId : accountIds) {
      ...
      try {
        ...
        ZimbraLog.datasource.debug(
            "Unable to look up account for id %s: %s", accountId, e.toString());
      }
