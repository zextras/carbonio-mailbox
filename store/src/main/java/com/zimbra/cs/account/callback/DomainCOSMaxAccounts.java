...
public class DomainCOSMaxAccounts extends AttributeCallback {
  ...
  public void preModify(
    ...
    if (entry != null) {
      ...
      for (String v : curValues) {
        ...
        if (parsed != null) cur.put(parsed.getFirst(), parsed.getSecond());
      }
    }
    ...
    for (String v : newVals) {
      ...
      if (other != null)
        throw ServiceException.INVALID_REQUEST(
            "cannot contain multiple values for the same cos "
                + parsed.getFirst()
                + ": "
                + parsed.getSecond()
                + ", "
                + other,
            null);
      else curVals.put(parsed.getFirst(), parsed.getSecond());
    }
  }
  ...
  private Pair<String, String> parse(String value, boolean throwOnError) throws ServiceException {
    ...
    if (parts.length != 2) {
      if (throwOnError) throw ServiceException.INVALID_REQUEST("invalid format", null);
      else return null;
    }
