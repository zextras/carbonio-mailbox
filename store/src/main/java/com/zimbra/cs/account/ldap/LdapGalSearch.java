...
public class LdapGalSearch {
  ...
  public static SearchGalResult searchLdapGal(
    ...
    if (url == null || url.length == 0 || base == null || filter == null) {
      if (url == null || url.length == 0) ZimbraLog.gal.warn("searchLdapGal url is null");
      if (base == null) ZimbraLog.gal.warn("searchLdapGal base is null");
      if (filter == null) ZimbraLog.gal.warn("searchLdapGal queryExpr is null");
      return result;
    }

    if (filter.indexOf("(") == -1) {
      String queryExpr = GalSearchConfig.getFilterDef(filter);
      if (queryExpr != null) filter = queryExpr;
    }
    ...
    else searchLdapGal(galParams, galOp, query, maxResults, rules, token, result);
    return result;
  }
  ...
  private static void searchLdapGalKrb5(
    ...
    try {
      ...
      Krb5Login.performAs(
          credential.getKrb5Principal(),
          credential.getKrb5Keytab(),
  ...
  static class SearchGalAction implements PrivilegedExceptionAction {
    ...
    SearchGalAction(
        GalParams.ExternalGalParams arg_galParams,
        GalOp arg_galOp,
        String arg_query,
        int arg_maxResults,
    ...
    try {
      ...
      ExternalLdapConfig ldapConfig =
          new ExternalLdapConfig(
              galParams.url(),
              galParams.requireStartTLS(),
              credential.getAuthMech(),
              credential.getBindDn(),
              credential.getBindPassword(),
              rules.getBinaryLdapAttrs(),
              "external GAL");
  ...
  public static void galSearch(GalSearchParams params) throws ServiceException {
    String authMech = params.getConfig().getAuthMech();
    if (authMech.equals(Provisioning.LDAP_AM_KERBEROS5)) galSearchKrb5(params);
    else doGalSearch(params);
  }
    ...
    try {
      ...
      if (galType == GalSearchConfig.GalType.zimbra) {
        ...
        ExternalLdapConfig ldapConfig =
            new ExternalLdapConfig(
                cfg.getUrl(),
                cfg.getStartTlsEnabled(),
                cfg.getAuthMech(),
                cfg.getBindDn(),
                cfg.getBindPassword(),
                cfg.getRules().getBinaryLdapAttrs(),
                "external GAL");
  ...
  public static void searchGal(
    ...
    try {
      ...
      if (GalOp.sync != op || ((GalOp.sync == op) && !result.getHadMore())) {
        ...
        if (newToken == null
            || (token != null && token.equals(newToken))
            || newToken.equals(LdapConstants.EARLIEST_SYNC_TOKEN)) gotNewToken = false;
  ...
  public static void getGalEntryByDn(
      ZLdapContext zlc,
      GalSearchConfig.GalType galType,
      String dn,
      LdapGalMapRules rules,
