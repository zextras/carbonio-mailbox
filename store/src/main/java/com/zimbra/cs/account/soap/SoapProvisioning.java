...
public class SoapProvisioning extends Provisioning {
  ...
  public static class Options {
    ...
    public void setAccountBy(AccountBy accountBy) {
      mAccountBy = accountBy;
    }
  ...
  public String toString() {
    return String.format(
        "[%s %s]", getClass().getName(), mTransport == null ? "" : mTransport.getURI());
  }
  ...
  public void soapSetURI(String uri) {
    ...
    if (mTimeout >= 0) mTransport.setTimeout(mTimeout);
    if (mRetryCount > 0) mTransport.setRetryCount(mRetryCount);
    if (mAuthToken != null) mTransport.setAuthToken(mAuthToken);
    if (mDebugListener != null) mTransport.setDebugListener(mDebugListener);
  }
  ...
  public static String getLocalConfigURI() {
    ...
    return LC.zimbra_admin_service_scheme.value()
        + server
        + ":"
        + port
   ...
   * zimbra_zmprov_default_soap_server, zimbra_admin_service_port, zimbra_admin_service_scheme and
   * calling soapZimbraAdminAuthenticate.
   *
  ...
  public void soapSetTransportTimeout(int timeout) {
    mTimeout = timeout;
    if (mTransport != null && timeout >= 0) mTransport.setTimeout(timeout);
  }

  public void soapSetTransportRetryCount(int retryCount) {
    mRetryCount = retryCount;
    if (mTransport != null && retryCount >= 0) mTransport.setRetryCount(retryCount);
  }

  public void soapSetTransportDebugListener(DebugListener listener) {
    mDebugListener = listener;
    if (mTransport != null) mTransport.setDebugListener(mDebugListener);
  }
  ...
  public void soapSetHttpTransportDebugListener(HttpDebugListener listener) {
    ...
    if (mTransport != null) mTransport.setHttpDebugListener(listener);
  }
  ...
  public void setAuthToken(ZAuthToken authToken) {
    mAuthToken = authToken;
    if (mTransport != null) mTransport.setAuthToken(authToken);
  }

  /**
   * used to authenticate via admin AuthRequest. can only be called after setting the URI with
   * setURI.
   *
   * @param name
   * @param password
  ...
  public void soapAdminAuthenticate(String name, String password) throws ServiceException {
    if (mTransport == null)
      throw ZClientException.CLIENT_ERROR(
          "must call setURI before calling adminAuthenticate", null);
  ...
  public void soapLogOut() throws ServiceException {
    ...
    try {
      ...
      if (!ServiceException.AUTH_REQUIRED.equals(e.getCode())
          && !ServiceException.AUTH_EXPIRED.equals(e.getCode())) {
  ...
  private Element invokeRequest(Element request)
  ...
  private Future<HttpResponse> invokeRequestAsync(Element request, FutureCallback<HttpResponse> cb)
      throws IOException {
    if (mNeedSession) {
  ...
  public Future<HttpResponse> invokeAsync(Element request, FutureCallback<HttpResponse> cb)
      throws ServiceException {
    checkTransport();
    try {
  ...
  public Future<HttpResponse> invokeAsync(
      Element request, String serverName, FutureCallback<HttpResponse> cb) throws ServiceException {
    checkTransport();
  ...
  protected synchronized Element invokeOnTargetAccount(Element request, String targetId)
      throws ServiceException {
    checkTransport();
  ...
  public <T> T invokeJaxbAsAdminWithRetry(Object jaxbObject) throws ServiceException {
    ...
    try {
      ...
      if (ServiceException.AUTH_EXPIRED.equalsIgnoreCase(ex.getCode())
          || ServiceException.AUTH_REQUIRED.equalsIgnoreCase(ex.getCode())) {
        soapZimbraAdminAuthenticate();
        resp = invokeJaxb(jaxbObject);
      } else {
        throw ex;
  ...
  public <T> T invokeJaxbAsAdminWithRetry(Object jaxbObject, String serverName)
      throws ServiceException {
    T resp = null;
    try {
  ...
  public static void addAttrElements(Element req, Map<String, ? extends Object> attrs)
      throws ServiceException {
  ...
  public void authAccount(
      Account acct, String password, AuthContext.Protocol proto, Map<String, Object> context)
      throws ServiceException {
  ...
  public void changePassword(Account acct, String currentPassword, String newPassword)
      throws ServiceException {
    this.changePassword(acct, currentPassword, newPassword, false);
  }
  ...
  public void changePassword(
    ...
    com.zimbra.soap.type.AccountSelector jaxbAcct =
        new com.zimbra.soap.type.AccountSelector(
            com.zimbra.soap.type.AccountBy.name, acct.getName());
  ...
  public CalendarResource createCalendarResource(
    ...
    CreateCalendarResourceResponse resp =
        invokeJaxb(
            new CreateCalendarResourceRequest(emailAddress, password, Attr.mapToList(attrs)));
  ...
  public Cos copyCos(String srcCosId, String destCosName) throws ServiceException {
    CopyCosResponse resp =
        invokeJaxb(
            new CopyCosRequest(new CosSelector(CosSelector.CosBy.id, srcCosId), destCosName));
    return new SoapCos(resp.getCos(), this);
  }
  ...
  public DistributionList createDistributionList(String listAddress, Map<String, Object> listAttrs)
      throws ServiceException {
    CreateDistributionListRequest req =
        new CreateDistributionListRequest(listAddress, Attr.mapToList(listAttrs), false);
  ...
  public Group createGroup(String listAddress, Map<String, Object> listAttrs, boolean dynamic)
      throws ServiceException {
    CreateDistributionListRequest req =
        new CreateDistributionListRequest(listAddress, Attr.mapToList(listAttrs), dynamic);
    CreateDistributionListResponse resp = invokeJaxb(req);
  ...
  public DelegateAuthResponse delegateAuth(AccountBy keyType, String key, int durationSeconds)
      throws ServiceException {
  ...
  public Account getAccount(String key, boolean applyDefault) throws ServiceException {
    ...
    if (Provisioning.isUUID(key)) acct = get(AccountBy.id, key, applyDefault);
    else {
      // could be id or name, try name first, if not found then try id
      acct = get(AccountBy.name, key, applyDefault);
      if (acct == null) acct = get(AccountBy.id, key, applyDefault);
    }
      ...
      GetAccountResponse resp =
          invokeJaxb(
              new GetAccountRequest(
                  new AccountSelector(SoapProvisioning.toJaxb(keyType), key), applyDefault));
      return new SoapAccount(resp.getAccount(), this);
    } catch (ServiceException e) {
      if (e.getCode().equals(AccountServiceException.NO_SUCH_ACCOUNT)) return null;
      else throw e;
    }
  }
  ...
  public List<AccountLogger> addAccountLogger(
      Account account, String category, String level, String serverName) throws ServiceException {
    if (serverName == null) {
      serverName = account.getServerName();
    }
    LoggerInfo logger = LoggerInfo.createForCategoryAndLevelString(category, level);
    AddAccountLoggerRequest req =
        AddAccountLoggerRequest.createForAccountAndLogger(getSelector(account), logger);
    ...
    return accountLoggersFromLoggerInfos(resp.getLoggers(), account.getName());
  }

  private AccountLogger accountLoggerFromLoggerInfo(LoggerInfo loggerInfo, String accountName) {
    return new AccountLogger(
        loggerInfo.getCategory(), accountName, Level.valueOf(loggerInfo.getLevel().toString()));
  }
  ...
  /**
   * Returns all account loggers for the given server. The <tt>Map</tt>'s key is the account name,
   * and values are all the <tt>AccountLogger</tt> objects for that account.
   *
  ...
  public void removeAccountLoggers(Account account, String category, String serverName)
    ...
    RemoveAccountLoggerRequest req = new RemoveAccountLoggerRequest(getSelector(account), logger);

    RemoveAccountLoggerResponse resp =
        serverName == null
  ...
  public ReIndexInfo reIndex(Account acct, String action, ReIndexBy by, String[] values)
    ...
    if (progInfo != null) {
      progress =
          new ReIndexInfo.Progress(
              progInfo.getNumSucceeded(), progInfo.getNumFailed(), progInfo.getNumRemaining());
    }
  ...
  public String compactIndex(Account acct, String action) throws ServiceException {
    Server server = getServer(acct);
    CompactIndexRequest req =
        new CompactIndexRequest(action, new MailboxByAccountIdSelector(acct.getId()));
  ...
  public IndexStatsInfo getIndexStats(Account acct) throws ServiceException {
    Server server = getServer(acct);
    GetIndexStatsRequest req =
        new GetIndexStatsRequest(new MailboxByAccountIdSelector(acct.getId()));
  ...
  public VerifyIndexResult verifyIndex(Account account) throws ServiceException {
    VerifyIndexRequest req =
        new VerifyIndexRequest(new MailboxByAccountIdSelector(account.getId()));
  ...
  public long recalculateMailboxCounts(Account acct) throws ServiceException {
    ...
    RecalculateMailboxCountsResponse resp =
        invokeJaxb(new RecalculateMailboxCountsRequest(mbox), hostname);
    ...
    try {
      GetCalendarResourceRequest req =
          new GetCalendarResourceRequest(
              new CalendarResourceSelector(SoapProvisioning.toJaxb(keyType), key));
      GetCalendarResourceResponse resp = invokeJaxb(req);
      return new SoapCalendarResource(resp.getCalResource(), this);
    } catch (ServiceException e) {
      if (e.getCode().equals(AccountServiceException.NO_SUCH_CALENDAR_RESOURCE)) return null;
      else throw e;
    }
  }
  ...
  public Cos get(CosBy keyType, String key) throws ServiceException {
    try {
      GetCosResponse resp =
          invokeJaxb(new GetCosRequest(new CosSelector(SoapProvisioning.toJaxb(keyType), key)));
      return new SoapCos(resp.getCos(), this);
    } catch (ServiceException e) {
      if (e.getCode().equals(AccountServiceException.NO_SUCH_COS)) return null;
      else throw e;
    }
  }
  ...
  public DistributionList get(DistributionListBy keyType, String key) throws ServiceException {
    try {
      GetDistributionListResponse resp =
          invokeJaxb(
              new GetDistributionListRequest(new DistributionListSelector(toJaxb(keyType), key)));
      return new SoapDistributionList(resp.getDl(), this);
    } catch (ServiceException e) {
      if (e.getCode().equals(AccountServiceException.NO_SUCH_DISTRIBUTION_LIST)) return null;
      else throw e;
    }
  }
  ...
  public Domain getDomainInfo(DomainBy keyType, String key) throws ServiceException {
    ...
    try {
      ...
      if (e.getCode().equals(Constants.ERROR_CODE_NO_SUCH_DOMAIN)) return null;
      else throw e;
    }
  }
  ...
  public Domain get(DomainBy keyType, String key, boolean applyDefault) throws ServiceException {
    DomainSelector domSel = new DomainSelector(toJaxb(keyType), key);
    try {
      GetDomainResponse resp = invokeJaxb(new GetDomainRequest(domSel, applyDefault));
      return new SoapDomain(resp.getDomain(), this);
    } catch (ServiceException e) {
      if (e.getCode().equals(Constants.ERROR_CODE_NO_SUCH_DOMAIN)) return null;
      else throw e;
  ...
  public Server getLocalServer() throws ServiceException {
    ...
    if (hostname == null) {
      throw ServiceException.FAILURE(
          "zimbra_server_hostname not specified in localconfig.xml", null);
    }
    Server local = get(ServerBy.name, hostname);
    if (local == null) {
      throw ServiceException.FAILURE(
          "Could not find an LDAP entry for server '" + hostname + "'", null);
    }
    return local;
  }
  ...
  public Server get(ServerBy keyType, String key, boolean applyDefault) throws ServiceException {
    ServerSelector sel = new ServerSelector(SoapProvisioning.toJaxb(keyType), key);
    try {
      GetServerResponse resp = invokeJaxb(new GetServerRequest(sel, applyDefault));
      return new SoapServer(resp.getServer(), this);
    } catch (ServiceException e) {
  ...
  public void removeAlias(Account acct, String alias) throws ServiceException {
    this.invokeJaxb(new RemoveAccountAliasRequest((acct == null) ? null : acct.getId(), alias));
    if (acct != null) reload(acct);
  }

  @Override
  public void removeAlias(DistributionList dl, String alias) throws ServiceException {
    this.invokeJaxb(
        new RemoveDistributionListAliasRequest((dl == null) ? null : dl.getId(), alias));
    if (dl != null) reload(dl);
  }

  @Override
  public void removeGroupAlias(Group group, String alias) throws ServiceException {
    this.invokeJaxb(
        new RemoveDistributionListAliasRequest((group == null) ? null : group.getId(), alias));
    if (group != null) reload(group);
  }
  ...
  public SetPasswordResult setPassword(Account acct, String newPassword) throws ServiceException {
    SetPasswordResponse resp = invokeJaxb(new SetPasswordRequest(acct.getId(), newPassword));
    SetPasswordResult result = new SetPasswordResult();
    String eMsg = resp.getMessage();
    if (eMsg != null) result.setMessage(eMsg);
    return result;
  }
  ...
  public void modifyAttrs(
      com.zimbra.cs.account.Entry e, Map<String, ? extends Object> attrs, boolean checkImmutable)
      throws ServiceException {
  ...
  public List<DistributionList> getDistributionLists(
    ...
    for (DLInfo dlInfo : resp.getDlList()) {
      ...
      if (via != null && viaList != null) via.put(dl.getName(), viaList);
      result.add(dl);
    }
    return result;
  }
  ...
  private List<AccountInfo> getAllAccountsInfo(Domain d, Server s) throws ServiceException {
    GetAllAccountsResponse resp =
        invokeJaxb(new GetAllAccountsRequest(getSelector(s), getSelector(d)));
  ...
  private List<CalendarResourceInfo> getAllCalendarResourcesInfo(Domain d, Server s)
    ...
    GetAllCalendarResourcesResponse resp =
        invokeJaxb(new GetAllCalendarResourcesRequest(getSelector(s), getSelector(d)));
    return resp.getCalendarResourceList();
  }

  @Override
  public List<CalendarResource> getAllCalendarResources(Domain d) throws ServiceException {
  ...
  public SearchGalResult autoCompleteGal(
      Domain d, String query, GalSearchType type, int limit, GalContact.Visitor visitor)
      throws ServiceException {
  ...
  public SearchGalResult searchGal(
    ...
    req.addAttribute(AdminConstants.A_DOMAIN, d.getName());
    req.addAttribute(AdminConstants.A_TYPE, typeStr);
    if (limit > 0) req.addAttribute(AdminConstants.A_LIMIT, limit);
    if (offset > 0) req.addAttribute(AdminConstants.A_OFFSET, limit);
    if (sortBy != null) req.addAttribute(AdminConstants.A_SORT_BY, sortBy);
  ...
  public void addMembers(DistributionList list, String[] members) throws ServiceException {
    invokeJaxb(new AddDistributionListMemberRequest(list.getId(), Arrays.asList(members)));
    reload(list);
  }

  @Override
  public void addGroupMembers(Group group, String[] members) throws ServiceException {
    invokeJaxb(new AddDistributionListMemberRequest(group.getId(), Arrays.asList(members)));
    reload(group);
  }

  @Override
  public void removeMembers(DistributionList list, String[] members) throws ServiceException {
    invokeJaxb(new RemoveDistributionListMemberRequest(list.getId(), Arrays.asList(members)));
    reload(list);
  }

  @Override
  public void removeGroupMembers(Group group, String[] members) throws ServiceException {
    invokeJaxb(new RemoveDistributionListMemberRequest(group.getId(), Arrays.asList(members)));
    reload(group);
  }
  ...
  public Signature createSignature(Account account, String signatureName, Map<String, Object> attrs)
    ...
    if (attrs.get(Provisioning.A_zimbraSignatureName) != null)
      throw ZClientException.CLIENT_ERROR(
          "invalid attr: " + Provisioning.A_zimbraSignatureName, null);
    ...
    Element response =
        invokeOnTargetAccount(req, account.getId()).getElement(AccountConstants.E_SIGNATURE);
  ...
  public void modifySignature(Account account, String signatureId, Map<String, Object> attrs)
    ...
    if (attrs.get(Provisioning.A_zimbraSignatureId) != null)
      throw ZClientException.CLIENT_ERROR(
          "invalid attr: " + Provisioning.A_zimbraSignatureId, null);
  ...
  public DataSource createDataSource(
      Account account, DataSourceType dsType, String dsName, Map<String, Object> attrs)
      throws ServiceException {
  ...
  public DataSource get(Account account, DataSourceBy keyType, String key) throws ServiceException {
    ...
    switch (keyType) {
      ...
      case id:
        for (DataSource source : getAllDataSources(account))
          if (source.getId().equalsIgnoreCase(key)) return source;
        return null;
      default:
        return null;
    }
  ...
  public Identity get(Account account, IdentityBy keyType, String key) throws ServiceException {
    ...
    switch (keyType) {
      ...
      case id:
        for (Identity identity : getAllIdentities(account))
          if (identity.getId().equalsIgnoreCase(key)) return identity;
        return null;
      default:
        return null;
    }
  ...
  public Signature get(Account account, SignatureBy keyType, String key) throws ServiceException {
    ...
    switch (keyType) {
      ...
      case id:
        for (Signature signature : getAllSignatures(account))
          if (signature.getId().equalsIgnoreCase(key)) return signature;
        return null;
      default:
        return null;
    }
  ...
  public MailboxWithMailboxId purgeMessages(Account account) throws ServiceException {
    ...
    if (resp.getMailboxes().isEmpty()) return null;
    else return resp.getMailboxes().get(0);
  }
  ...
  // grantee
  private Element toXML(Element req, String granteeType, GranteeBy granteeBy, String grantee) {
    return toXML(req, granteeType, granteeBy, grantee, null);
  }

  private Element toXML(
      Element req, String granteeType, GranteeBy granteeBy, String grantee, String secret) {
    Element eGrantee = req.addElement(AdminConstants.E_GRANTEE);
    if (granteeType != null) eGrantee.addAttribute(AdminConstants.A_TYPE, granteeType);

    if (granteeBy != null) eGrantee.addAttribute(AdminConstants.A_BY, granteeBy.toString());

    if (secret != null) eGrantee.addAttribute(AdminConstants.A_SECRET, secret);

    if (grantee != null) eGrantee.setText(grantee);
  ...
  public Map<String, List<RightsDoc>> getRightsDoc(String[] pkgs) throws ServiceException {
    ...
    if (pkgs != null) {
      for (String pkg : pkgs) req.addPkg(new PackageSelector(pkg));
    }
    ...
    for (PackageRightsInfo ePkg : resp.getPackages()) {
      ...
      for (CmdRightsInfo eCmd : ePkg.getCmds()) docs.add(new RightsDoc(eCmd));
    }
    return allDocs;
  }
  ...
  public Right getRight(String rightName, boolean expandAllAttrs) throws ServiceException {
    GetRightResponse resp = invokeJaxb(new GetRightRequest(rightName, expandAllAttrs));
  ...
  public boolean checkRight(
    ...
    if (via != null) {
      ...
      if (eVia != null) {
        ...
        via.setImpl(
            new ViaGrantImpl(
                eTarget.getAttribute(AdminConstants.A_TYPE),
                eTarget.getText(),
  ...
  public RightCommand.Grants getGrants(
    ...
    if (targetType != null) toXML(req, targetType, targetBy, target);
  ...
  public CountAccountResult countAccount(Domain domain) throws ServiceException {
    CountAccountResponse resp = invokeJaxb(new CountAccountRequest(getSelector(domain)));
    CountAccountResult result = new CountAccountResult();
    for (CosCountInfo cosInfo : resp.getCos()) {
      result.addCountAccountByCosResult(cosInfo.getId(), cosInfo.getName(), cosInfo.getValue());
    }
    return result;
  }
  ...
  public MemcachedClientConfig getMemcachedClientConfig() throws ServiceException {
    ...
    config.serverList =
        resp.getAttribute(AdminConstants.A_MEMCACHED_CLIENT_CONFIG_SERVER_LIST, null);
    config.hashAlgorithm =
        resp.getAttribute(AdminConstants.A_MEMCACHED_CLIENT_CONFIG_HASH_ALGORITHM, null);
    config.binaryProtocol =
        resp.getAttributeBool(AdminConstants.A_MEMCACHED_CLIENT_CONFIG_BINARY_PROTOCOL, false);
    config.defaultExpirySeconds =
        (int)
            resp.getAttributeLong(
                AdminConstants.A_MEMCACHED_CLIENT_CONFIG_DEFAULT_EXPIRY_SECONDS, 0);
    config.defaultTimeoutMillis =
        resp.getAttributeLong(AdminConstants.A_MEMCACHED_CLIENT_CONFIG_DEFAULT_TIMEOUT_MILLIS, 0);
    return config;
  }
  ...
  public List<UCService> getAllUCServices() throws ServiceException {
    ArrayList<UCService> result = new ArrayList<>();
  ...
  private static CalendarResourceSelector.CalendarResourceBy toJaxb(
      Key.CalendarResourceBy provCalendarResourceBy) throws ServiceException {
  ...
  /* Convert to equivalent JAXB object */
  private static CacheEntrySelector.CacheEntryBy toJaxb(Key.CacheEntryBy provCacheEntryBy)
      throws ServiceException {
    return CacheEntrySelector.CacheEntryBy.fromString(provCacheEntryBy.toString());
  }
  ...
  public Set<String> createHabOrgUnit(Domain domain, String habOrgUnitName)
    ...
    Set<String> habOrgList = new HashSet<>();
