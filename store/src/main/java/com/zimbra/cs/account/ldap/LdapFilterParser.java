...
public class LdapFilterParser {
  ...
  private static Term parse(String filterStr, int startPos, int endPos) throws ServiceException {
    ...
    // sanity check
    if (endPos - startPos <= 0) throw ServiceException.PARSE_ERROR(dbgInfo, null);
    ...
    if (filterStr.charAt(startPos) == '(') {
      if (filterStr.charAt(endPos - 1) == ')') {
        startPos++;
        endPos--;
      } else throw ServiceException.PARSE_ERROR("mising parentheses: " + dbgInfo, null);
    }

    // see if it is a compound filter
    char c = filterStr.charAt(startPos);
    if (c == '&') return parseCompound(FilterType.AND, filterStr, startPos + 1, endPos);
    else if (c == '|') return parseCompound(FilterType.OR, filterStr, startPos + 1, endPos);
    else if (c == '!') return parseCompound(FilterType.NOT, filterStr, startPos + 1, endPos);

    // if we've gotten here, then it must be a simple filter
    return parseSimple(filterStr, startPos, endPos);
  }

  private static Multi parseCompound(
      FilterType filterType, String filterStr, int startPos, int endPos) throws ServiceException {
    String dbgInfo = "filter=" + filterStr + ", startPos=" + startPos + ", endPos=" + endPos;

    if (startPos == endPos) throw ServiceException.PARSE_ERROR(dbgInfo, null);
  ...
  private static Single parseSimple(String filterStr, int startPos, int endPos)
    ...
    if (equalPos == -1 || equalPos == startPos) throw ServiceException.PARSE_ERROR(dbgInfo, null);
