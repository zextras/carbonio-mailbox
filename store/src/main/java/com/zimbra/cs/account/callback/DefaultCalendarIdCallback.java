  ...
  public void preModify(
    ...
    try {
      ...
      throw ServiceException.INVALID_REQUEST(
          "Value for " + attrName + " must be valid integer", null);
    }
    if (value == 0) {
      throw ServiceException.INVALID_REQUEST("Invalid value received for " + attrName, null);
    }
    ...
    if (entry instanceof Account) {
      ...
      if (folder == null) {
        throw ServiceException.NOT_FOUND(
            "Folder not found for id " + value + ". Please provide valid folder id.", null);
      }
      ...
      if (folder.getType() == MailItem.Type.MOUNTPOINT) {
        ...
        if (!writeAccess) {
          throw ServiceException.PERM_DENIED(
              account.getMail()
                  + " do not have enough permissions on "
                  + folder.getName()
                  + " to set default.");
        }
      }
      ...
      throw ServiceException.INVALID_REQUEST(
          "Changing value for " + attrName + " on COS is not allowed.", null);
    } else if (entry != null) {
