...
public abstract class MigrationInfo {
  ...
  public void setStatus(MigrationInfo.Status status) {
    this.status = status;
  }

  public MigrationInfo.Status getStatus() {
    return status;
  }
  ...
  public static void setFactory(Class<? extends Factory> factoryClass) {
    ...
    try {
      ...
      ZimbraLog.ephemeral.error(
          "unable to instantiate Factory %s, using ZimbraAttributeInfo", factoryClass.getName(), e);
