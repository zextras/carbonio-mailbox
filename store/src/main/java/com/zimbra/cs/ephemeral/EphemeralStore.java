...
public abstract class EphemeralStore {
  ...
  /**
   * Get the value for a key. If key does not exist, returns an empty EphemeralResult instance.
   *
   * @param key
   * @param location
   * @return
  ...
  /**
   * Set a value for a key if the key does not exist, or overwrites otherwise. If the key is
   * multi-valued, all original values are deleted.
   *
   * @param input
   * @param location
   * @throws ServiceException
  ...
  /**
   * Set a value for a key if the key does not exist, or add another value otherwise. If this value
   * already exists, the expiration is updated if one is provided.
   *
   * @param input
   * @param location
   * @throws ServiceException
  ...
  /**
   * Delete specified value for a key. If the value does not exist, do nothing.
   *
   * @param key
   * @param value
   * @param location
   ...
   * @throws ServiceException
   */
  public abstract boolean has(EphemeralKey key, EphemeralLocation location) throws ServiceException;

  /**
   * Delete keys that have passed their expiration. If the backend natively supports key expiry,
   * this may do nothing.
   *
   * @param key
   * @param location
   ...
   * @throws ServiceException
   */
  public abstract void deleteData(EphemeralLocation location) throws ServiceException;

  public static void registerFactory(String prefix, String klass) {
    if (factories.containsKey(prefix)) {
      ZimbraLog.ephemeral.warn(
          "Replacing ephemeral factory class '%s' registered for '%s' with '%s'",
          factories.get(prefix), prefix, klass);
    } else {
      ZimbraLog.ephemeral.info(
          "Registering ephemeral factory class '%s' for prefix '%s'", klass, prefix);
    }
  ...
  private static final void setFactory(String factoryClassName, FailureMode onFailure) {
    ...
    try {
      ...
      try {
        ...
        handleFailure(
            onFailure,
            String.format("Unable to find EphemeralStore factory %s", factoryClassName),
            cnfe2);
        return;
      }
    }
  ...
  public static final void setFactory(
      Class<? extends Factory> factoryClass, FailureMode onFailure) {
    ...
    try {
      ...
      try {
        ...
        if (!Strings.isNullOrEmpty(info.getURL())
            && (curStatus == Status.COMPLETED || curStatus == Status.IN_PROGRESS)) {
          useForwarding = true;
        }
      ...
      if (useForwarding) {
        try {
          ...
          ZimbraLog.ephemeral.debug(
              "using forwarding ephemeral store; %s forwarding to %s",
              className, forwardingFactory.getClass().getName());
        } catch (ServiceException e) {
          ZimbraLog.ephemeral.warn(
              "unable to initialize forwarding migration factory, using %s", className, e);
          factory = fac;
        }
      } else {
        factory = fac;
      ...
      handleFailure(
          onFailure, String.format("unable to initialize EphemeralStore factory %s", className), e);
    }
  }
  ...
  private static String getFactoryClassName(String backendName, boolean disableAutoload)
      throws ServiceException {
    ...
    if (factoryClassName == null && !disableAutoload) {
      ...
      // cut down on noise unless enabled debug; this is called from CLI utilities so messages go to
      // the console
  ...
  public static Factory getNewFactory(BackendType backendType) throws ServiceException {
    ...
    if (url != null) {
      ...
      throw ServiceException.FAILURE(
          String.format(
              "no ephemeral backend URL specified for backend type '%s'", backendType.toString()),
          null);
    }
    if (factoryClassName == null) {
      throw ServiceException.FAILURE(
          String.format(
              "no EphemeralStore.Factory class specified for backend type '%s'",
    ...
    try {
      ...
      try {
        ...
        throw ServiceException.FAILURE(
            String.format("Unable to find EphemeralStore factory %s", factoryClassName), cnfe2);
      }
    }
    try {
      ...
      throw ServiceException.FAILURE(
          String.format(
              "unable to initialize EphemeralsStore factory %s",
  ...
  static {
    OPTIONS.addOption(
        "u", "test-url", true, "test whether can connect to this URL.  Exit non-zero if cannot.");
  ...
  private static void usage() {
    ...
    format.printHelp(
        ...
        80,
        "zmjava com.zimbra.cs.ephemeral.EphemeralStore [options]",
        null,
        OPTIONS,
        2,
        2,
  ...
  public static boolean canConnectToURL(String url) {
    ...
    if (tokens == null || tokens.length <= 0) {
      ZimbraLog.ephemeral.error(
          "'%s' is an invalid URL for %s", url, Provisioning.A_zimbraEphemeralBackendURL);
      return false;
    }
    ...
    try {
      ...
      ZimbraLog.ephemeral.error(
          "cannot set '%s' to '%s' (%s)",
          Provisioning.A_zimbraEphemeralBackendURL, url, e.getMessage());
      return false;
    }
    ZimbraLog.ephemeral.debug(
        "Successfully connected to URL '%s'.  Valid value for '%s'",
  ...
  public abstract static class Factory {
    ...
    /**
     * Validate the given URL for the EphemeralStore implementation. This method should throw an
     * ServiceException if the backend cannot function with this URL.
     *
     * @param url
     * @throws ServiceException
     */
