...
public class LdapEphemeralStore extends EphemeralStore {
  ...
  public EphemeralResult get(EphemeralKey key, EphemeralLocation location) throws ServiceException {
    ...
    DynamicResultsHelper iteratorHelper =
        new DynamicResultsHelper(key, location, encoder, callback);
  ...
  public void purgeExpired(EphemeralKey key, EphemeralLocation location) throws ServiceException {
    ...
    String[] values = helper.getMultiAttr(key.getKey());
    DynamicResultsHelper iteratorHelper =
        new DynamicResultsHelper(key, location, encoder, callback, true);
  ...
  private void deleteInternal(AbstractLdapHelper helper, String key, List<String> values)
      throws ServiceException {
  ...
  /**
   * Abstract helper class that accumulates an LDAP-style attribute map. Subclasses specify what to
   * do with this map, given an @EphemeralLocation, as well as fetching values from this location.
   */
  @VisibleForTesting
  ...
  private static class ZimbraLdapHelper extends AbstractLdapHelper {
    ...
    void executeChange() throws ServiceException {
      ...
      if (prov instanceof LdapProvisioning) {
        ...
        throw ServiceException.FAILURE(
            String.format(
                "LdapEphemeralStore can only be used with LdapProvisioning: '%s' provided",
    ...
    protected Entry locationToEntry(EphemeralLocation location) throws ServiceException {
      if (!(location instanceof LdapEntryLocation)) {
        throw ServiceException.FAILURE(
            "LdapEphemeralBackend can only be used with LdapEntryLocation", null);
      }
  ...
  private class LdapDeletionCallback implements DeletionCallback {
    ...
    @Override
    public void delete(String encodedKey, List<String> values) throws ServiceException {
