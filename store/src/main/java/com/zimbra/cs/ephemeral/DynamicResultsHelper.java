...
/**
 * Helper class used by InMemoryEphemeralStore and LdapEphemeralStore, or potentially any
 * EphemeralStore implementation that does not support native key expiration or dynamic keys,
 * requiring the expiration and dynamic key data to be encoded in the values.
 *
 * <p>This class handles the task of filtering multivalued results on whether the requested key was
 * dynamic, using the filterValue method, and builds on that to implement handling of the get(),
 * delete(), purgeExpired(), and has() methods of the EphemeralStore interface.
 *
 * @author iraykin
 */
public class DynamicResultsHelper {
  ...
  public boolean filterValue(String encodedValue) throws ServiceException {
    ...
    if (isDynamic && (dc != null && dc.equals(dynamicComponent))) {
      // If the requested key is dynamic, return true only if the value has a matching dynamic
      // component
      return true;
      ...
      // If the requested key is not dynamic but the value is, only return True if broadKeyMatch is
      // True
      ...
      try {
        if (deletionCallback != null) {
          ZimbraLog.ephemeral.debug(
              "deleting unparseable values %s for key %s",
              Joiner.on(", ").join(valuesToDelete), encodedKey);
