...
/**
 * An abstract helper class responsible for converting EphemeralBackend inputs into key/value String
 * pairs, needed for each EphemeralBackend implementation, and vice-versa. The encoding is delegated
 * to @KeyEncoder and @ValueEncoder implementations. Subclasses set the key and value encoders, and
 * implement the decode() method to convert the stored key/value strings into EphemeralKeyValuePair
 * instances.
 *
 * @author iraykin
 */
public abstract class AttributeEncoder {
  ...
  /**
   * Encode an ephemeral input into a String key to be stored in the backend. The output key may be
   * a function of the EphemeralKey and target.
   *
   * @param key
   * @param target
   * @return
  ...
  /**
   * Encode ephemeral input into a String value to be stored in the backend. The output value may be
   * a function of the EphemeralKey, value, and target.
   *
   * @param input
   * @param target
   * @return String
