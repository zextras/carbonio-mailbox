...
public class AttributeMigrationUtil {
  ...
  static {
    OPTIONS.addOption(
        "r", "dry-run", false, "Dry run: display info on what the migration would accomplish");
    OPTIONS.addOption(
        "n",
        "num-threads",
        true,
        "Number of threads to use in the migration. If not set, defaults to 1");
    ...
    OPTIONS.addOption(
        ...
        "account",
        true,
        "Comma-separated list of accounts to migrate. If not specified, all accounts will be"
            + " migrated");
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (clArgs.size() > 1) {
      ...
      attrsToMigrate =
          new ArrayList<String>(AttributeManager.getInstance().getEphemeralAttributeNames());
    }
    ...
    if (!dryRun) {
      ...
      if (Provisioning.getInstance()
          .getConfig()
          .getEphemeralBackendURL()
          .equalsIgnoreCase(destURL)) {
        ZimbraLog.ephemeral.info(
            "destination URL cannot be the same as the current ephemeral backend URL");
        return;
      }
      ...
      try {
        ...
        Zimbra.halt(
            String.format(
                "unable to connect to ephemeral backend at %s; migration cannot proceed", destURL),
            e);
        return;
      }
    } else {
    ...
    try {
      ...
      Zimbra.halt(
          String.format(
              "error encountered during migration to ephemeral backend at %s; migration cannot"
                  + " proceed",
              destURL),
          e);
      return;
  ...
  private static void clearMigrationInfo() throws ServiceException {
    ...
    if (curStatus == Status.NONE) {
      ...
      ZimbraLog.ephemeral.info(
          "resetting info for migration to %s currently marked as %s",
          info.getURL(), info.getStatus().toString());
      info.clearData();
    }
  }
  ...
  private static void usage() {
    ...
    format.printHelp(
        ...
        80,
        "zmmigrateattrs [options] [URL] [attr1 attr2 attr3 ...]",
        null,
        OPTIONS,
        2,
        2,
