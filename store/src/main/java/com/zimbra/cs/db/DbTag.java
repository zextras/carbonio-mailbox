...
public final class DbTag {
  ...
  static final String TAG_FIELDS = "id, name, color, item_count, unread, listed, sequence, policy";
  ...
  static UnderlyingData asUnderlyingData(ResultSet rs) throws SQLException, ServiceException {
    ...
    RetentionPolicy rp =
        policy == null
            ? null
            : RetentionPolicyManager.retentionPolicyFromMetadata(new Metadata(policy), true);
    long c = rs.getLong(3);
  ...
  static String tagLIKEPattern(String tagName) {
    return '%'
        + delimitTagName(tagName).replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
        + '%';
  }

  public static void createTag(Mailbox mbox, UnderlyingData data, Color color, boolean listed)
      throws ServiceException {
    createTag(mbox.getOperationConnection(), mbox, data, color, listed);
  }

  public static void createTag(
      DbConnection conn, Mailbox mbox, UnderlyingData data, Color color, boolean listed)
      throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "INSERT INTO "
                  ...
                  + " ("
                  + DbMailItem.MAILBOX_ID
                  + " id, name, color, listed, sequence)"
                  + " VALUES ("
                  + DbMailItem.MAILBOX_ID_VALUE
                  + "?, ?, ?, ?, ?)");
      int pos = 1;
      ...
      if (Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
        ...
        throw ServiceException.FAILURE(
            "Failed to create tag id=" + data.id + ",name=" + data.name, e);
      }
    } finally {
  ...
  public static List<UnderlyingData> getUnreadMessages(Tag tag) throws ServiceException {
    ...
    try {
      String mailboxesMatchAnd =
          DebugConfig.disableMailboxGroups ? "" : "mi.mailbox_id = ti.mailbox_id AND ";
      stmt =
          conn.prepareStatement(
              "SELECT "
                  + DbMailItem.DB_FIELDS
                  + " FROM "
  ...
  public static PendingDelete getImapDeleted(Mailbox mbox, Set<Folder> folders)
      throws ServiceException {
    ...
    try {
      ...
      String folderconstraint =
          folders == null ? "" : " AND " + DbUtil.whereIn("folder_id", folders.size());
      stmt =
          conn.prepareStatement(
              "SELECT "
                  + DbMailItem.LEAF_NODE_FIELDS
                  + " FROM "
                  + DbMailItem.getMailItemTableName(mbox, "mi")
                  + " INNER JOIN "
                  + getTaggedItemTableName(mbox, "ti")
  ...
  static void recalculateTagCounts(Mailbox mbox, Map<Integer, UnderlyingData> lookup)
      throws ServiceException {
    DbConnection conn = mbox.getOperationConnection();
    PreparedStatement stmt = null;
    ResultSet rs = null;
      ...
      stmt =
          conn.prepareStatement(
              "SELECT ti.tag_id, COUNT(*), "
                  ...
                  + " WHERE "
                  + inThisMailboxAnd("ti")
                  + "ti.tag_id > 0 AND type NOT IN "
                  + DbMailItem.NON_SEARCHABLE_TYPES
                  + " AND "
                  + Db.getInstance().bitAND("mi.flags", "" + Flag.BITMASK_DELETED)
                  + " = 0"
                  + " GROUP BY ti.tag_id");
      DbMailItem.setMailboxId(stmt, mbox, 1);
      ...
      while (rs.next()) {
        ...
        if (data != null) {
          ...
          ZimbraLog.mailbox.warn(
              "inconsistent DB state: items with no corresponding tag (tag id %d)", tagId);
        }
      }
    } catch (SQLException e) {
  ...
  static void updateTagReferences(
      Mailbox mbox, int itemId, MailItem.Type type, int flags, boolean unread, String[] tags)
      throws ServiceException {
    DbConnection conn = mbox.getOperationConnection();
    PreparedStatement stmt = null;
    try {
  ...
  static void storeTagReferences(
      Mailbox mbox, int itemId, MailItem.Type type, int flags, boolean unread)
      throws ServiceException {
    if (!type.isLeafNode()) return;
      ...
      stmt =
          conn.prepareStatement(
              "INSERT INTO "
                  ...
                  + " WHERE "
                  + DbMailItem.IN_THIS_MAILBOX_AND
                  + DbUtil.whereIn("id", flagIds.size()));
      int pos = 1;
      stmt.setInt(pos++, itemId);
      pos = DbMailItem.setMailboxId(stmt, mbox, pos);
  ...
  static void storeTagReferences(Mailbox mbox, int itemId, MailItem.Type type, String[] tags)
      throws ServiceException {
    if (!type.isLeafNode() || ArrayUtil.isEmpty(tags)) return;
      ...
      stmt =
          conn.prepareStatement(
              "INSERT INTO "
                  ...
                  + " WHERE "
                  + DbMailItem.IN_THIS_MAILBOX_AND
                  + DbUtil.whereIn("name", tags.length));
      int pos = 1;
      stmt.setInt(pos++, itemId);
      pos = DbMailItem.setMailboxId(stmt, mbox, pos);
      for (String tag : tags) {
  ...
  public static void alterTag(Tag tag, List<Integer> itemIds, boolean add) throws ServiceException {
    ...
    try {
      ...
      if (isFlag) {
        ...
        if (add) {
          primaryUpdate =
              "tag_names = CASE WHEN tag_names IS NULL THEN ? ELSE "
                  + Db.getInstance().concat("tag_names", "?")
                  + " END";
          sanityCheckAnd = "(tag_names IS NULL OR tag_names NOT LIKE ?) AND ";
        } else {
          primaryUpdate =
              "tag_names = CASE tag_names WHEN ? THEN NULL ELSE REPLACE(tag_names, ?, '\0') END";
          sanityCheckAnd = "";
        }
      }
      ...
      throw ServiceException.FAILURE(
          "updating tag data for "
              + itemIds.size()
              + " items: "
              + DbMailItem.getIdListForLogging(itemIds),
          e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  ...
  static void addTaggedItemEntries(Mailbox mbox, int tagId, List<Integer> itemIds)
      throws ServiceException {
    if (tagId < 0 && !Mailbox.REIFIED_FLAGS.contains(tagId)) return;
    ...
    for (int itemId : itemIds) {
      try {
        ...
        throw ServiceException.FAILURE(
            "adding TAGGED_ITEM entries for tag: " + tagId + ", item: " + itemId, e);
      } finally {
        DbPool.closeStatement(stmt);
      }
    }
  ...
  static void removeTaggedItemEntries(Mailbox mbox, int tagId, List<Integer> itemIds)
      throws ServiceException {
    if (tagId < 0 && !Mailbox.REIFIED_FLAGS.contains(tagId)) return;
    ...
    try {
      stmt =
          conn.prepareStatement(
              "DELETE FROM "
                  ...
                  + DbMailItem.IN_THIS_MAILBOX_AND
                  + "tag_id = ? AND "
                  + DbUtil.whereIn("item_id", itemIds.size()));
      int pos = 1;
      pos = DbMailItem.setMailboxId(stmt, mbox, pos);
      stmt.setInt(pos++, tagId);
      for (int itemId : itemIds) {
      ...
      throw ServiceException.FAILURE(
          "removing TAGGED_ITEM entries for tag: "
              + tagId
              + ", items: "
              + DbMailItem.getIdListForLogging(itemIds),
          e);
    ...
    try {
      stmt =
          conn.prepareStatement(
              "UPDATE "
                  + getTagTableName(mbox)
                  + " SET color = ?, policy = ?, listed = ?, sequence = ? WHERE "
                  + DbMailItem.IN_THIS_MAILBOX_AND
                  + "id = ?");
      int pos = 1;
      ...
      if (Db.supports(Db.Capability.MULTITABLE_UPDATE)) {
        ...
        stmt =
            conn.prepareStatement(
                "UPDATE "
                    ...
                    + " ON "
                    + mailboxesMatchAnd
                    + "mi.id = ti.item_id SET mi.tag_names = CASE mi.tag_names WHEN ? THEN NULL"
                    + " ELSE REPLACE(mi.tag_names, ?, '"
                    + Db.getEscapeSequence()
                    + "0') END, mod_metadata = ?, change_date = ?"
                    + " WHERE "
                    + inThisMailboxAnd("ti")
                    + "ti.tag_id = ?");
        ...
        stmt =
            conn.prepareStatement(
                "UPDATE "
                    + DbMailItem.getMailItemTableName(mbox)
                    + " SET tag_names = CASE tag_names WHEN ? THEN NULL ELSE REPLACE(tag_names, ?,"
                    + " '"
                    + Db.getEscapeSequence()
                    + "0') END, mod_metadata = ?, change_date = ?"
                    + " WHERE "
      ...
      stmt =
          conn.prepareStatement(
              "SELECT "
                  ...
                  + " WHERE "
                  + inThisMailboxAnd("ti")
                  + "ti.tag_id = ? AND date < ? AND type NOT IN "
                  + DbMailItem.NON_SEARCHABLE_TYPES
                  + orderByLimit);
      int pos = 1;
      pos = DbMailItem.setMailboxId(stmt, mbox, pos);
  ...
  private static void recalculateTagStrings(Mailbox mbox, Tag taggedWith) throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "UPDATE "
                  ...
                  + "(SELECT (CONCAT('"
                  + Db.getEscapeSequence()
                  + "0', GROUP_CONCAT(t.name SEPARATOR '"
                  + Db.getEscapeSequence()
                  + "0'), '"
                  + Db.getEscapeSequence()
                  + "0') FROM tag t INNER JOIN tagged_item ti ON t.mailbox_id = ti.mailbox_id AND"
                  + " t.id = ti.tag_id WHERE ti.item_id = mi.id AND ti.tag_id > 0)"
      ...
      throw ServiceException.FAILURE(
          "recalculating TAG_NAMES column for mailbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  /**
   * Returns the name of the table that stores {@link Tag} data. The table name is qualified with
   * the name of the database (e.g. <tt>mboxgroup1.tag</tt>).
   */
