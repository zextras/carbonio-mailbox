...
public final class DbSearch {
   ...
   * result set, and MySQL chokes on the ORDER BY when we do a UNION query (doesn't know which 2 of
   * the 4 sort columns are the "right" ones to use).
   */
  ...
  private static final byte[] APPOINTMENT_TABLE_TYPES =
      new byte[] {MailItem.Type.APPOINTMENT.toByte(), MailItem.Type.TASK.toByte()};
  ...
  /**
   * Returns true if this field is case-sensitive for search/sort, i.e. whether or not we need to do
   * an UPPER() on it in places.
   */
  ...
  private static String toSortField(SortBy sort) {
    ...
    switch (sort.getKey()) {
      ...
      case ATTACHMENT: // 0 or 1
        return db.concat(
            db.sign(db.bitAND("mi.flags", String.valueOf(Flag.BITMASK_ATTACHED))),
            db.lpad("mi.id", 10, "0"));
      case FLAG: // 0 or 1
        return db.concat(
            db.sign(db.bitAND("mi.flags", String.valueOf(Flag.BITMASK_FLAGGED))),
            db.lpad("mi.id", 10, "0"));
      case PRIORITY: // 0 or 1 or 2
        return db.concat(
            "(1 + "
                + db.sign(db.bitAND("mi.flags", String.valueOf(Flag.BITMASK_HIGH_PRIORITY)))
                + " - "
  ...
  /**
   * Generate the ORDER BY part that goes at the end of the SELECT. Note: Assumes that <b>mi.id</b>
   * is something defined in the SELECT which can be ordered by.
   */
  ...
  public int countResults(DbConnection conn, DbSearchConstraints node, boolean ignoreNoRecipients)
      throws ServiceException {
    node = node.optimize();
    sql.append("SELECT COUNT(*) FROM ")
        .append(DbMailItem.getMailItemTableName(mailbox, "mi", dumpster));
    sql.append(" WHERE ");
  ...
  private void encodeSelect(
      SortBy sort,
      FetchMode fetch,
      boolean joinAppt,
      boolean joinTaggedItem,
      DbSearchConstraints node,
      boolean validLIMIT) {
  ...
  private void encodeSelect(
      ...
      DbSearchConstraints node,
      boolean validLIMIT,
      boolean maybeExcludeHasRecipients) {
    ...
    if (sort != null
        && (sort.equals(SortBy.RCPT_ASC) || sort.equals(SortBy.RCPT_DESC))
        && maybeExcludeHasRecipients) {
  ...
  private void encodeConstraint(
      DbSearchConstraints node, byte[] calTypes, boolean inCalTable, boolean joinTaggedItem) {
    if (node instanceof DbSearchConstraints.Intersection
    ...
    for (Map.Entry<DbSearchConstraints.RangeType, DbSearchConstraints.Range> entry :
        constraint.ranges.entries()) {
      switch (entry.getKey()) {
        case ITEMID:
          needAnd =
              needAnd
                  | encodeIntRange(
                      "mi.id", (DbSearchConstraints.NumericRange) entry.getValue(), 1, needAnd);
          break;
        case DATE:
          needAnd =
              needAnd
                  | encodeDateRange(
                      "mi.date", (DbSearchConstraints.NumericRange) entry.getValue(), needAnd);
          break;
        case MDATE:
          needAnd =
              needAnd
                  | encodeDateRange(
                      "mi.change_date",
                      (DbSearchConstraints.NumericRange) entry.getValue(),
                      needAnd);
          break;
        case MODSEQ:
          needAnd =
        ...
        case SIZE:
          needAnd =
              needAnd
                  | encodeLongRange(
                      "mi.size", (DbSearchConstraints.NumericRange) entry.getValue(), 0, needAnd);
          break;
        case CAL_START_DATE:
          if (inCalTable) {
            needAnd =
                needAnd
                    | encodeTimestampRange(
                        "ap.start_time",
                        (DbSearchConstraints.NumericRange) entry.getValue(),
                        1L,
                        needAnd);
        ...
        case SENDER:
          needAnd =
              needAnd
                  | encodeStringRange(
                      "mi.sender", (DbSearchConstraints.StringRange) entry.getValue(), needAnd);
          break;
        case SUBJECT:
          needAnd =
              needAnd
                  | encodeStringRange(
                      "mi.subject", (DbSearchConstraints.StringRange) entry.getValue(), needAnd);
          break;
        case CONV_COUNT:
        default:
          break;
  ...
  /**
   * @return TRUE if some part of this query has a non-appointment select (ie 'type not in (11,15)'
   *     non-null
   */
  ...
  /**
   * @return TRUE if this constraint needs to do a join with the Appointment table in order to be
   *     evaluated
   */
  ...
  private List<Result> search(
      ...
      int limit,
      FetchMode fetch,
      boolean searchDraftsSeparately)
      throws ServiceException {
    if (!Db.supports(Db.Capability.AVOID_OR_IN_WHERE_CLAUSE)
      ...
      try {
        ...
        if (Db.errorMatches(e, Db.Error.TOO_MANY_SQL_PARAMS)) {
          ZimbraLog.sqltrace.debug(
              "Too many SQL params: %s", node, e); // fall back to splitting OR clauses
        } else {
    ...
    if (!(node instanceof DbSearchConstraints.Leaf)) {
      ...
      if (node instanceof DbSearchConstraints.Union) {
        ...
        throw ServiceException.FAILURE(
            "Reached merge/intersect block with something other than OR/AND clause", null);
      }
    } else {
      // case 2 (leaf node), we could encounter a sql clause with too many folders involved, try
      // splitting these folders
      // only deals with folders for now to avoid considering complicated situations (other
  ...
  private List<Result> searchInternal(
    ...
    // check if we need to run this as two queries: one with "mi.recipients is not NULL" and one in
    // drafts with "mi.recipients is NULL"
    if (searchingInDrafts(node)
        && searchDraftsSeparately
        && sort != null
        && (sort.equals(SortBy.RCPT_ASC) || sort.equals(SortBy.RCPT_DESC))) {
      DbSearchConstraints.Leaf draftsConstraint =
          findDraftsConstraint(node)
              .clone(); // clone the existing node containing the Drafts constraint
    ...
    if (hasMailItemOnlyConstraints) {
      ...
      encodeSelect(
          sort, fetch, false, joinTaggedItem, node, hasValidLIMIT, maybeExcludeNoRecipients);
   ...
   * @throws ServiceException
   */
  private List<Result> searchTwoConstraints(
      DbConnection conn,
      DbSearchConstraints node1,
      DbSearchConstraints node2,
      SortBy sort,
    ...
    // |---------------------------------------|--------------------------------------|
    List<Result> result1 =
        new DbSearch(mailbox, dumpster).search(conn, node1, sort, offset, limit, fetch, false);
    if (result1.size() == 0) {
      //                                              |--- somewhere here----|
      // |---------------------------------------|--------------------------------------|
      // If there are no results in the first search, we need to run the second search with some
      // possibly non-zero offset.
      // To find this offset, we need to find out how many total results there were in the first
      // query and subtract that from the given offset.
      // To accurately calculate the number of results, we need to know whether to exclude
      // mi.recipients=null or not,
      // which happens when node1 is NOT our custom draft constraint that has
      // excludeHasRecipients=true.
      boolean ignoreNoRecipients = true;
      if (node1.toLeaf() != null) {
        ignoreNoRecipients = !node1.toLeaf().excludeHasRecipients;
      }
      int offset2 =
          offset - new DbSearch(mailbox, dumpster).countResults(conn, node1, ignoreNoRecipients);
      int limit2 = limit;
      List<Result> result2 =
          new DbSearch(mailbox, dumpster).search(conn, node2, sort, offset2, limit2, fetch, false);
      result1.addAll(result2);
    } else if (result1.size() < limit) {
      //                      |------ somewhere here------|
      // |---------------------------------------|--------------------------------------|
      // If the size of the result set is less than the given limit but is nonzero, then we are
      // "straddling" the two operations.
      // This means we need to run the second query with offset=0, and limit={remainder from 1st
      // query}
      int offset2 = 0;
  ...
  private boolean encodeTag(Set<Tag> tags, boolean bool, boolean useJoin, boolean and) {
    ...
    if (dumpster) { // There is no corresponding table to TAGGED_ITEM in dumpster, hence brute-force
      // search.
      int flags = 0;
      for (Tag tag : tags) {
      ...
      sql.append("mi.id = ti.item_id AND ")
          .append(DbUtil.whereIn("ti.tag_id", tags.size()))
          .append(')');
      for (Tag tag : tags) {
        params.add(tag.getId());
      }
  ...
  private boolean encodeDateRange(
    ...
    return encodeRange(
        ...
        range,
        1L,
        (int) Math.min(range.min / 1000, Integer.MAX_VALUE),
        (int) Math.min(range.max / 1000, Integer.MAX_VALUE),
        and);
  }

  private boolean encodeTimestampRange(
      String column, DbSearchConstraints.NumericRange range, long cutoff, boolean and) {
    return encodeRange(
        column, range, cutoff, new Timestamp(range.min), new Timestamp(range.max), and);
  }
  ...
  private boolean encodeRange(
      String column,
      DbSearchConstraints.NumericRange range,
      long cutoff,
      Object min,
      Object max,
      boolean and) {
  ...
  public abstract static class Result {
    ...
    protected Result(Object sortValue) {
      assert (sortValue == null
              ...
              || sortValue instanceof Long
              || sortValue instanceof Integer)
          : sortValue;
  ...
  /**
   * {@link MailItem.UnderlyingData} to {@link MailItem} conversion should be done in {@link
   * Mailbox} layer because DB layer is not aware of mailbox transaction.
   */
  ...
  private static final class ImapResult extends IdResult {
    ...
    ImapResult(ResultSet rs, Object sortkey) throws SQLException {
      ...
      int flags =
          rs.getBoolean(offset + 2)
              ? Flag.BITMASK_UNREAD | rs.getInt(offset + 3)
              : rs.getInt(offset + 3);
      i4msg =
          new ImapMessage(
              getId(),
              ...
              rs.getInt(offset + 1),
              flags,
              DbTag.deserializeTags(rs.getString(offset + 4)));
    }
