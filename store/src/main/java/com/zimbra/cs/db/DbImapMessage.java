...
public class DbImapMessage {
  ...
  public static void storeImapMessage(
      Mailbox mbox, int localFolderId, long remoteUid, int localItemId, int flags)
      ...
      stmt =
          conn.prepareStatement(
              "INSERT INTO "
                  ...
                  + "imap_folder_id, uid, item_id, flags) "
                  + "VALUES ("
  ...
  public static void deleteImapMessages(Mailbox mbox, int localFolderId) throws ServiceException {
    ...
    ZimbraLog.datasource.debug(
        "Deleting all IMAP message trackers: mboxId=%d, localFolderId=%d",
        mbox.getId(), localFolderId);
  ...
  public static Pair<ImapMessage, Integer> getImapMessage(Mailbox mbox, DataSource ds, int itemId)
    ...
    try {
      ...
      String MBOX_JOIN =
          DebugConfig.disableMailboxGroups ? "" : "imap.mailbox_id = mi.mailbox_id AND";
      String IN_IMAP_MAILBOX_AND =
          DebugConfig.disableMailboxGroups ? "" : "imap.mailbox_id = ? AND";
      stmt =
          conn.prepareStatement(
  ...
  public static List<ImapMessage> getMovedMessages(Mailbox mbox, DataSource ds, int folderId)
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              String.format(
                  "SELECT imap.uid, imap.item_id, imap.flags as tflags, mi.unread, mi.flags  FROM"
                      + " %s imap JOIN %s mi  ON imap.mailbox_id = mi.mailbox_id AND imap.item_id ="
                      + " mi.id  WHERE imap.mailbox_id = %d AND imap.imap_folder_id = %d AND"
                      + " mi.folder_id != %d",
                  getTableName(mbox),
                  DbMailItem.getMailItemTableName(mbox),
                  mbox.getId(),
  ...
  public static ImapMessageCollection getImapMessages(
      Mailbox mbox, DataSource ds, ImapFolder imapFolder) throws ServiceException {
    ...
    ZimbraLog.datasource.debug(
        "Found %d tracked IMAP messages for %s", imapMessages.size(), imapFolder.getRemoteId());
    return imapMessages;
  }

  /**
   * Returns a list of the local message ids in the given folder for which there is no corresponding
   * IMAP message tracker. This is the list of new messages which must be appended to the remote
   * mailbox when handling a folder UIDVALIDITY change.
   *
  ...
  public static List<Integer> getNewLocalMessageIds(Mailbox mbox, int folderId)
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              "SELECT id FROM "
                  ...
                  + "WHERE "
                  + IN_MI_MAILBOX_AND
                  + " mi.folder_id = ? AND imap.item_id IS NULL "
                  + "AND mi.type IN ("
      ...
      while (rs.next()) newIds.add(rs.getInt("id"));
    } catch (SQLException e) {
