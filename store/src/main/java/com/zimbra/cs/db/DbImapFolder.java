...
public class DbImapFolder {
  ...
  public static ImapFolderCollection getImapFolders(Mailbox mbox, DataSource ds)
    ...
    try {
      ...
      while (rs.next()) {
        ...
        String remotePath = rs.getString("remote_path");
        Long uidValidity = rs.getLong("uid_validity");
        if (rs.wasNull()) uidValidity = null;
  ...
  public static ImapFolder createImapFolder(
      Mailbox mbox,
      DataSource ds,
      int itemId,
      String localPath,
      ...
      stmt =
          conn.prepareStatement(
              "INSERT INTO "
                  ...
                  + "item_id, data_source_id, local_path, remote_path, uid_validity) "
                  + "VALUES ("
    ...
    try {
      ...
      if (numRows != 1) {
        throw ServiceException.FAILURE(
            String.format("Incorrect number of rows updated (%d) for %s", numRows, imapFolder),
            null);
      }
      conn.commit();
    } catch (SQLException e) {
  ...
  public static void deleteImapData(Mailbox mbox, String dataSourceId) throws ServiceException {
    ...
    if (StringUtil.isNullOrEmpty(dataSourceId)) return;
