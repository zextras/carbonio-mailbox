...
public final class DbVolume {
  ...
  public static synchronized Volume create(DbConnection conn, Volume volume)
    ...
    try {
      stmt =
          conn.prepareStatement(
              "INSERT INTO volume (id, type, name, path, mailbox_group_bits, mailbox_bits,"
                  + " file_group_bits, file_bits, compress_blobs, compression_threshold, metadata)"
                  + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      int pos = 1;
      ...
      if (Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
        throw VolumeServiceException.ALREADY_EXISTS(
            nextId, volume.getName(), volume.getRootPath(), e);
      } else {
  ...
  public static Volume updateMetadata(
      DbConnection conn, short volumeId, VolumeMetadata volumeMetadata) throws ServiceException {
    PreparedStatement stmt = null;
    try {
  ...
  public static Volume update(DbConnection conn, Volume volume) throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "UPDATE volume SET type = ?, name = ?, path = ?, "
      ...
      if (Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
        throw VolumeServiceException.ALREADY_EXISTS(
            volume.getId(), volume.getName(), volume.getRootPath(), e);
      } else {
  ...
  public static boolean delete(DbConnection conn, short id) throws ServiceException {
    ...
    try {
      ...
      else throw ServiceException.FAILURE("deleting volume entry: " + id, e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  /**
   * Return all {@link Volume} entries in a map, where ID is the key and a {@link Volume} object is
   * the value.
   */
  ...
  public static CurrentVolumes getCurrentVolumes(DbConnection conn) throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "SELECT message_volume_id, secondary_message_volume_id, index_volume_id FROM"
                  + " current_volumes");
      rs = stmt.executeQuery();
      if (rs.next()) {
        currVols.msgVolId = rs.getShort(1);
        short s = rs.getShort(2);
        if (!rs.wasNull()) currVols.secondaryMsgVolId = s;
  ...
  public static void updateCurrentVolume(DbConnection conn, short type, short id)
      throws ServiceException {
    String col;
    switch (type) {
  ...
  private static Volume constructVolume(ResultSet rs) throws SQLException, ServiceException {
    ...
    return Volume.builder()
        .setId(rs.getShort(CN_ID))
        .setType(rs.getShort(CN_TYPE))
        .setName(rs.getString(CN_NAME))
        .setPath(Volume.getAbsolutePath(rs.getString(CN_PATH)), false)
        .setMboxGroupBits(rs.getShort(CN_MAILBOX_GROUP_BITS))
        .setMboxBit(rs.getShort(CN_MAILBOX_BITS))
        .setFileGroupBits(rs.getShort(CN_FILE_GROUP_BITS))
        .setFileBits(rs.getShort(CN_FILE_BITS))
  ...
  public static boolean isVolumeReferenced(DbConnection conn, short volumeId)
      throws ServiceException {
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
  ...
  private static boolean isVolumeReferenced(DbConnection conn, short volumeId, int groupId)
      throws ServiceException {
  ...
  private static boolean tableRefsVolume(
      DbConnection conn, short volumeId, int groupId, String table) throws ServiceException {
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
      stmt =
          conn.prepareStatement(
              "SELECT count(*) from "
                  + DbMailbox.qualifyTableName(groupId, table)
                  + " where locator=?");
