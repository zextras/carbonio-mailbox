...
public class DbPendingAclPush {
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              "SELECT mailbox_id, item_id, date FROM "
                  + TABLE_PENDING_ACL_PUSH
        ...
        stmt =
            conn.prepareStatement(
                command
                    + " INTO "
                    + TABLE_PENDING_ACL_PUSH
                    + " (mailbox_id, item_id, date) VALUES (?, ?, ?)");
        stmt.setInt(1, mbox.getId());
        stmt.setInt(2, itemId);
  ...
  public static Multimap<Integer, Integer> getEntries(Date uptoTime) throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "Unable to get entries recorded before " + uptoTime + " for ACL push", e);
    } finally {
  ...
  public static void deleteEntries(Date uptoTime) throws ServiceException {
    ...
    try {
      conn = DbPool.getConnection();
      stmt = conn.prepareStatement("DELETE FROM " + TABLE_PENDING_ACL_PUSH + " WHERE date < ?");
      stmt.setLong(1, uptoTime.getTime());
      stmt.executeUpdate();
      conn.commit();
    } catch (SQLException e) {
  ...
  public static void deleteEntry(int mailboxId, int itemId) throws ServiceException {
    ZimbraLog.misc.debug(
        "Deleting entry for ACL push for mailbox: %d, item : %d", mailboxId, itemId);
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
      conn = DbPool.getConnection();
      stmt =
          conn.prepareStatement(
              "DELETE FROM " + TABLE_PENDING_ACL_PUSH + " WHERE mailbox_id=? AND" + " item_id=?");
