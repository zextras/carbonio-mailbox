...
public class DbDataSource {
  ...
  public static void addMapping(DataSource ds, DataSourceItem item, final boolean isBatch)
    ...
    if (item.remoteId == null) item.remoteId = "";

    ZimbraLog.datasource.debug(
        "Adding mapping for dataSource %s: itemId(%d), remoteId(%s)",
        ds.getName(), item.itemId, item.remoteId);
    ...
    try {
      ...
      if (Db.supports(Db.Capability.ON_DUPLICATE_KEY)) {
        sb.append(
            " ON DUPLICATE KEY UPDATE data_source_id = ?, item_id = ?, folder_id = ?, remote_id ="
                + " ?, metadata = ?");
      }
      ...
      stmt.setString(
          i++, DbMailItem.checkMetadataLength((item.md == null) ? null : item.md.toString()));
      if (Db.supports(Db.Capability.ON_DUPLICATE_KEY)) {
        stmt.setString(i++, dataSourceId);
        stmt.setInt(i++, item.itemId);
        stmt.setInt(i++, item.folderId);
      ...
      if (!Db.supports(Db.Capability.ON_DUPLICATE_KEY)
          && Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
  ...
  public static void updateMapping(DataSource ds, DataSourceItem item, final boolean isBatch)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);

    ZimbraLog.datasource.debug(
        "Updating mapping for dataSource %s: itemId(%d), remoteId(%s)",
        ds.getName(), item.itemId, item.remoteId);
  ...
  public static void deleteMappings(DataSource ds, Collection<Integer> itemIds)
      throws ServiceException {
    deleteMappings(ds, itemIds, false);
  }

  public static void deleteMappings(DataSource ds, Collection<Integer> itemIds, boolean isBatch)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);

    ZimbraLog.datasource.debug(
        "Deleting %d mappings for dataSource %s", itemIds.size(), ds.getName());
  ...
  public static void deleteMapping(DataSource ds, int itemId, boolean isBatch)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);

    ZimbraLog.datasource.debug(
        "Deleting mappings for dataSource %s itemId %d", ds.getName(), itemId);
  ...
  public static Collection<DataSourceItem> deleteAllMappingsInFolder(DataSource ds, int folderId)
      throws ServiceException {
    return deleteAllMappingsInFolder(ds, folderId, false);
  }

  public static Collection<DataSourceItem> deleteAllMappingsInFolder(
      DataSource ds, int folderId, boolean isBatch) throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);

    ArrayList<DataSourceItem> items = new ArrayList<DataSourceItem>();

    ZimbraLog.datasource.debug(
        "Deleting all mappings for dataSource %s in folder %d", ds.getName(), folderId);
    ...
    try {
      ...
      try {
        ...
        String IN_THIS_MAILBOX_AND =
            DebugConfig.disableMailboxGroups ? "" : dataSourceTable + ".mailbox_id = ? AND ";
        StringBuilder sb = new StringBuilder();
        sb.append("DELETE FROM ");
        ...
        throw ServiceException.FAILURE(
            "Unable to delete mapping for dataSource " + ds.getName(), e);
      } finally {
        DbPool.closeStatement(stmt);
        if (!isBatch) {
          DbPool.quietClose(conn);
  ...
  public static Collection<DataSourceItem> getAllMappings(DataSource ds) throws ServiceException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (buf != null) md = new Metadata(buf);
  ...
  public static Collection<DataSourceItem> getAllMappingsInFolder(DataSource ds, int folderId)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
    ...
    try {
      ...
      String IN_THIS_MAILBOX_AND =
          DebugConfig.disableMailboxGroups ? "" : thisTable + ".mailbox_id = ? AND ";
      StringBuilder sb = new StringBuilder();
  ...
  public static Collection<DataSourceItem> getAllMappingsAndFlagsInFolder(
      DataSource ds, int folderId) throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
    ...
    try {
      ...
      String IN_THIS_MAILBOX_AND =
          DebugConfig.disableMailboxGroups ? "" : thisTable + ".mailbox_id = ? AND ";
      String MBOX_JOIN =
          DebugConfig.disableMailboxGroups ? " " : thisTable + ".mailbox_id = mi.mailbox_id AND ";
      conn = DbPool.getConnection(mbox);
      StringBuilder sb = new StringBuilder();
      sb.append("SELECT item_id, remote_id, ")
          .append(thisTable)
          .append(".metadata, mi.unread, mi.flags FROM ");
      sb.append(thisTable);
  ...
  public static Collection<DataSourceItem> getAllMappingsForRemoteIdPrefix(
      DataSource ds, int folderId, String prefix) throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
  ...
  public static DataSourceItem getMapping(DataSource ds, int itemId, boolean isBatch)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
  ...
  public static DataSourceItem getReverseMapping(DataSource ds, String remoteId)
      throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "Unable to get reverse mapping for dataSource " + ds.getName(), e);
    } finally {
      DbPool.closeResults(rs);
      DbPool.closeStatement(stmt);
      DbPool.quietClose(conn);
  ...
  public static Collection<DataSourceItem> getReverseMappings(
      DataSource ds, Collection<String> remoteIds) throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
    int folderId = 0;
    int itemId = 0;
    String remoteId;
  ...
  public static List<PurgeableConv> getOldestConversationsUpToSize(DataSource ds, long targetSize)
      throws ServiceException {
    return getOldestConversationsUpToSize(Collections.singletonList(ds), targetSize, 0);
  }

  public static List<PurgeableConv> getOldestConversationsUpToSize(
      List<DataSource> dataSources, long targetSize) throws ServiceException {
    return getOldestConversationsUpToSize(dataSources, targetSize, 0);
  }

  public static List<PurgeableConv> getOldestConversationsUpToSize(
      DataSource ds, long targetSize, long startDate) throws ServiceException {
    return getOldestConversationsUpToSize(Collections.singletonList(ds), targetSize, 0);
  }

  public static List<PurgeableConv> getOldestConversationsUpToSize(
      List<DataSource> dataSources, long targetSize, long startDate) throws ServiceException {
    ...
    try {
      ...
      StringBuilder sb = new StringBuilder();
      sb.append(
          "SELECT DISTINCT COALESCE(msgs.parent_id, msgs.msg_id) AS item_id,"
      ...
      sb.append(" FROM ");
      sb.append(
          "(SELECT di.data_source_id AS data_source_id, "
              + " mi.id AS msg_id,"
      ...
      sb.append(" LEFT JOIN ");
      sb.append(
          "(SELECT parent_id, max(date) AS latest_date, count(date) AS num_msgs, sum(size) AS"
              + " conv_size");
  ...
  public static Set<Integer> getConvMessageIdsInDataSource(
      Mailbox mbox, Integer convId, String dsId) throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
      ...
      throw ServiceException.FAILURE(
          "Unable to get message IDs for conversation in data source", e);
    } finally {
      DbPool.closeResults(rs);
      DbPool.closeStatement(stmt);
      DbPool.quietClose(conn);
  ...
  public static void purgeMessage(Mailbox mbox, Message msg, String dsId) throws ServiceException {
    ...
    try {
      ...
      sb.append(
          " (mailbox_id, data_source_id, item_id, parent_id, remote_id, remote_folder_id,"
              + " purge_date) ");
      sb.append(
          "SELECT dsi.mailbox_id, dsi.data_source_id, dsi.item_id, ?, dsi.remote_id,"
              + " dsi_inner.remote_id, ?");
      sb.append(" FROM ");
  ...
  public static void moveToPurgedConversations(Mailbox mbox, MailItem item, String dsId)
      throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
      ...
      if (Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
        ZimbraLog.datasource.warn(
            String.format(
                "moving item %d to purged conversations table more than once", item.getId()));
        return;
      }
      throw ServiceException.FAILURE(
          "Unable to move conversation to purged_conversations table", e);
    } finally {
      DbPool.closeStatement(stmt);
      DbPool.quietClose(conn);
    }
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (count > 0) {
          ...
          if (count > 1) {
            ZimbraLog.datasource.warn(
                "remote id should not show up in the purged messages table more than once");
          }
        }
      }
      rs.close();
  ...
  private static void removeFromPurgedMessageTable(Mailbox mbox, DataSource ds, PurgedMessage msg)
      throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
  ...
  private static void removeFromPurgedConversationTable(
      Mailbox mbox, DataSource ds, PurgedMessage msg) throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
      conn = DbPool.getConnection();
  ...
  public static List<PurgedConversation> lookupPurgedConversationsByHash(
      DataSource ds, List<String> hashes) throws ServiceException {
    Mailbox mbox = DataSourceManager.getInstance().getMailbox(ds);
    DbConnection conn = null;
    PreparedStatement stmt = null;
    ResultSet rs;
    ...
    try {
      ...
      sb.append(" (SELECT item_id, parent_id, remote_id, remote_folder_id FROM ")
          .append(getPurgedMessagesTableName(mbox));
  ...
  private static void deletePurgeDataByDataSource(
      Mailbox mbox, String dataSourceId, String tableName) throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
      conn = DbPool.getConnection();
  ...
  public static void deletePurgedDataForDataSource(Mailbox mbox, String dataSourceId)
      throws ServiceException {
  ...
  public static void storePurgedConversationHash(
      Mailbox mbox, String dataSourceId, Integer convId, String hash) throws ServiceException {
    DbConnection conn = null;
    PreparedStatement stmt = null;
    try {
      conn = DbPool.getConnection();
  ...
  public static class PurgedMessage {
    ...
    public String getUid() {
      return remoteId.split("_")[1];
    }

    public Integer getLocalFolderId() {
      return Integer.valueOf(remoteId.split("_")[0]);
    }
