...
public final class DbMailbox {
  ...
  public interface DbTableCallback {
    void create(DbConnection conn, int mailboxId, int groupId) throws ServiceException;
   ...
   * than the current <tt>next_mailbox_id</tt> value in the <tt>current_volumes</tt> table,
   * <tt>next_mailbox_id</tt>.
   */
  public static synchronized MailboxIdentifier getNextMailboxId(DbConnection conn, int mailboxId)
    ...
    try {
      ...
      MailboxIdentifier newId =
          new MailboxIdentifier(mailboxId, calculateMailboxGroupId(mailboxId));
  ...
  public static synchronized Mailbox.MailboxData createMailbox(
      DbConnection conn, int requestedMailboxId, String accountId, String comment, int lastBackupAt)
      throws ServiceException {
    String limitClause =
        Db.supports(Db.Capability.LIMIT_CLAUSE)
            ? " ORDER BY index_volume_id " + Db.getInstance().limit(1)
            : "";
    ...
    try {
      ...
      if (!rs.next()) {
        throw ServiceException.FAILURE(
            "cannot create mailbox: no rows in database table ZIMBRA.CURRENT_VOLUME", null);
      }
      ...
      if (DebugConfig.disableMailboxGroups) {
        ...
        if (!DebugConfig.externalMailboxDirectory) {
          ...
          stmt =
              conn.prepareStatement(
                  "INSERT INTO mailbox (account_id, id, last_backup_at, comment)"
                      + " VALUES (?, ?, ?, ?)");
        ...
        stmt =
            conn.prepareStatement(
                "INSERT INTO "
                    + qualifyTableName(groupId, TABLE_MAILBOX)
                    + "(id, account_id, index_volume_id, item_id_checkpoint, version)"
                    + " VALUES (?, ?, ?, "
                    + (Mailbox.FIRST_USER_ID - 1)
                    + ", ?)");
        ...
        stmt =
            conn.prepareStatement(
                "INSERT INTO mailbox(account_id, id, group_id, index_volume_id, item_id_checkpoint,"
                    + " last_backup_at, comment, version) VALUES (?, ?, ?, ?, "
                    + (Mailbox.FIRST_USER_ID - 1)
                    + ", ?, ?, ?)");
        stmt.setString(1, accountId.toLowerCase());
        stmt.setInt(2, mailboxId);
  ...
  public static void createMailboxDatabase(DbConnection conn, int mailboxId, int groupId)
    ...
    try {
      ...
      if (Db.getInstance().databaseExists(conn, dbname)) {
        // If database didn't exist we would end up doing CREATE DATABASE which does implicit
        // commit.
        // Let's do an explicit here so pending transactions always committed on exit from this
        // method
  ...
  private static void dropMailboxFromGroup(DbConnection conn, Mailbox mbox)
    ...
    try {
      ...
      ArrayList<String> tables = new ArrayList<>(sTables);
      for (DbTableCallback callback : callbacks) {
      ...
      for (String tableName : tables) {
        ...
        try {
          stmt =
              conn.prepareStatement(
                  "DELETE FROM "
                      + qualifyTableName(mbox, tableName)
      ...
      try {
        ...
        ZimbraLog.mailbox.error(
            "error enabling foreign key constraints during mailbox deletion", e);
  ...
  public static void renameMailbox(Mailbox mbox, String newName) throws ServiceException {
    ...
    try {
      ...
      try {
        stmt =
            conn.prepareStatement(
                "UPDATE mailbox SET comment = ?, last_backup_at = NULL WHERE id = ?");
  ...
  public static void clearMailboxContactCount(Mailbox mbox) throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "UPDATE "
                  + qualifyZimbraTableName(mbox, TABLE_MAILBOX)
  ...
  public static void recordLastSoapAccess(Mailbox mbox) throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "updating last SOAP access time for mailbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  public static String getConfig(Mailbox mbox, String section) throws ServiceException {
    ...
    try {
      stmt =
          conn.prepareStatement(
              "SELECT metadata FROM "
                  + qualifyZimbraTableName(mbox, TABLE_METADATA)
                  + " WHERE "
      ...
      throw ServiceException.FAILURE(
          "getting metadata section '" + section + "' in mailbox " + mbox.getId(), e);
    } finally {
      DbPool.closeResults(rs);
      DbPool.closeStatement(stmt);
    }
  ...
  public static void updateConfig(Mailbox mbox, String section, Metadata config)
      throws ServiceException {
    DbConnection conn = mbox.getOperationConnection();
    PreparedStatement stmt = null;
    try {
      ...
      throw ServiceException.FAILURE(
          "setting metadata section '" + section + "' in mailbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  public static void updateVersion(Mailbox mbox, MailboxVersion version) throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "setting mailbox version to '" + version + "' in mailbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  public static void updateLastPurgeAt(Mailbox mbox, long lastPurgeAt) throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "setting mailbox last_purge_at to '"
              + (int) (lastPurgeAt / 1000)
              + "' in mailbox "
              + mbox.getId(),
          e);
    } finally {
  ...
  /**
   * Returns the zimbra IDs and mailbox IDs for all mailboxes on the system. Note that mailboxes are
   * created lazily, so there may be accounts homed on this system for whom there is is not yet a
   * mailbox and hence are not included in the returned <code>Map</code>.
   *
   * @param conn An open database connection.
   * @return A <code>Map</code> whose keys are zimbra IDs and whose values are the corresponding
   *     numeric mailbox IDs.
   ...
   * is not yet a mailbox and hence are not included in the returned <code>Map</code>. Sizes are
   * checkpointed frequently, but there is no guarantee that the approximate sizes are currently
   * accurate.
   *
   * @param conn An open database connection.
   * @return A <code>Map</code> whose keys are zimbra IDs and whose values are the corresponding
   *     approximate mailbox sizes.
   * @throws ServiceException The following error codes are possible:
   *     <ul>
   *       <li><code>service.FAILURE</code> - an error occurred while accessing the database; a
   *           SQLException is encapsulated
    ...
    try {
      if (!DebugConfig.disableMailboxGroups) {
        ...
        for (int mailboxId : mailboxIds) {
          ...
          stmt =
              conn.prepareStatement(
                  "SELECT account_id, size_checkpoint FROM "
                      + qualifyZimbraTableName(mailboxId, TABLE_MAILBOX));
          rs = stmt.executeQuery();
          while (rs.next()) {
            sizes.put(rs.getString(1).toLowerCase(), rs.getLong(2));
          }
          ...
          // XXX: have to avoid having too many attached databases referenced in the same
          // transaction?
          conn.commit();
        }
      }
  ...
  public static final int CHANGE_CHECKPOINT_INCREMENT =
      Math.max(1, LC.zimbra_mailbox_change_checkpoint_frequency.intValue());
  public static final int ITEM_CHECKPOINT_INCREMENT = 20;

  public static Mailbox.MailboxData getMailboxStats(DbConnection conn, int mailboxId)
      throws ServiceException {
    // no locking check because it's a mailbox-level op done before the Mailbox object is
    // instantiated...
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              "SELECT account_id,"
                  + (DebugConfig.disableMailboxGroups ? mailboxId : " group_id")
                  + ", size_checkpoint, contact_count, item_id_checkpoint, change_checkpoint,"
                  + " tracking_sync, tracking_imap, index_volume_id, last_soap_access,"
                  + " new_messages, version, itemcache_checkpoint FROM "
                  + qualifyZimbraTableName(mailboxId, TABLE_MAILBOX)
                  + " WHERE id = ?");
      stmt.setInt(1, mailboxId);
      ...
      stmt =
          conn.prepareStatement(
              "SELECT section FROM "
                  + qualifyZimbraTableName(mailboxId, TABLE_METADATA)
  ...
  /**
   * Returns the name of the database that contains tables for the specified <code>mailboxId</code>.
   * As a side effect, also registers interest on that database with the specified {@link
   * DbConnection} (if not <tt>null</tt>).
   */
  ...
  /**
   * Qualifies the name of a database table that may be in the per-user database if {@link
   * DebugConfig.disableMailboxGroups} is set and in the <tt>ZIMBRA</tt> database otherwise.
   */
  ...
  public static void removeFromDeletedAccount(DbConnection conn, String email)
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "deleting row for " + email + " from deleted_account table", e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
  ...
  private static void addToDeletedAccount(DbConnection conn, Mailbox mbox) throws ServiceException {
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              command
                  + " INTO deleted_account (email, account_id, mailbox_id, deleted_at) "
  ...
  /**
   * Returns a DeletedAccount object for the given email, if the account for the email address was
   * previously deleted. Returns null if account for the email was not deleted.
   *
   * @param conn
   * @param email
   * @return
  ...
  public static void deleteMailbox(DbConnection conn, Mailbox mbox) throws ServiceException {
    ...
    try {
      if (DebugConfig.disableMailboxGroups) {
        ...
        stmt =
            conn.prepareStatement(
                "DELETE FROM " + qualifyTableName(mbox, TABLE_MAILBOX) + " WHERE id = ?");
        stmt.setInt(1, mbox.getId());
        stmt.executeUpdate();
        stmt.close();
      }
  ...
  public static List<Mailbox.MailboxData> getMailboxRawData(DbConnection conn)
      throws ServiceException {
    List<Mailbox.MailboxData> results = new ArrayList<>();
    if (DebugConfig.externalMailboxDirectory) {
      return results;
    }
    ...
    try {
      if (!DebugConfig.disableMailboxGroups) {
        stmt =
            conn.prepareStatement(
                "SELECT id, group_id, account_id, index_volume_id, item_id_checkpoint,"
                    ...
                    + " tracking_imap, last_backup_at, last_soap_access, new_messages FROM"
                    + " mailbox");
        rs = stmt.executeQuery();
        ...
        for (int mailboxId : mailboxIds) {
          ...
          stmt =
              conn.prepareStatement(
                  "SELECT id, id, account_id, index_volume_id, item_id_checkpoint, contact_count,"
                      + " size_checkpoint, change_checkpoint, tracking_sync, tracking_imap, -1,"
                      + " last_soap_access, new_messages FROM "
                      + qualifyZimbraTableName(mailboxId, TABLE_MAILBOX));
          rs = stmt.executeQuery();
          readMailboxRawData(results, rs);
          rs.close();
