...
public final class Versions {
  ...
  /**
   * The DB_VERSION is stored into the config table of the DB when the DB is created. If the
   * DB_VERSION does not match our server's version, we will not run.
   *
   * <p>UPDATE THESE TO REQUIRE RESET-WORLD TO BE RUN
   */
  public static final int DB_VERSION = 111;

  /**
   * The INDEX_VERSION is stored into the config table of the DB when the DB is created. If the
   * INDEX_VERSION does not match our server's version, we will not run.
   *
   * <p>UPDATE THESE TO REQUIRE RESET-WORLD TO BE RUN
   */
  ...
  static CommandLine parseCmdlineArgs(String args[], Options options) {
    ...
    if (err || cl.hasOption("h")) usage(options);
