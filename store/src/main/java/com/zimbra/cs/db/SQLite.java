...
public final class SQLite extends Db {
  ...
  void startup(org.apache.commons.dbcp.PoolingDataSource pool, int poolSize) throws SQLException {
    ...
    if (cacheSize.equals("0")) cacheSize = null;
    ...
    ZimbraLog.dbconn.info(
        "sqlite driver running with "
            + (cacheSize == null ? "default" : cacheSize)
            + " cache cache, "
            + (pageSize == null ? "default" : pageSize)
            + " page size, "
            + journalMode
            + " journal mode, "
            + syncMode
  ...
  private void pragma(Connection conn, String dbname, String key, String value)
      throws SQLException {
    ...
    try {
      ...
      (stmt =
              conn.prepareStatement(
                  "PRAGMA " + prefix + key + (value == null ? "" : " = " + value)))
          .execute();
    } finally {
  ...
  void pragmas(Connection conn, String dbname) throws SQLException {
    ...
    if (cacheSize != null) pragma(conn, dbname, "cache_size", cacheSize);
    if (pageSize != null) pragma(conn, dbname, "page_size", pageSize);
  }

  private static final int DEFAULT_CONNECTION_POOL_SIZE = 6;

  private static final int MAX_ATTACHED_DATABASES =
      readConfigInt("sqlite_max_attached_databases", "max # of attached databases", 7);
  ...
  private Connection getInnermostConnection(Connection conn) {
    ...
    if (conn instanceof DebugConnection) retVal = ((DebugConnection) conn).getConnection();
  ...
  public void optimize(DbConnection conn, String dbname, int level) throws ServiceException {
    try {
      ...
      try {
        if (!autocommit) conn.getConnection().setAutoCommit(true);
        if (dbname == null) dbname = "zimbra";
        registerDatabaseInterest(conn, dbname);
        if (level > 0 && dbname.endsWith("zimbra")) {
          if (level == 2) (stmt = conn.prepareStatement("VACUUM")).execute();
          else pragma(conn.getConnection(), dbname, "incremental_vacuum", null);
        }
      ...
      throw ServiceException.FAILURE(
          "sqlite " + (level > 0 ? "vacuum" : "analyze") + ' ' + dbname + " error", e);
    }
  }

  @Override
  public void registerDatabaseInterest(DbConnection conn, String dbname)
      throws SQLException, ServiceException {
    LinkedHashMap<String, String> attachedDBs = getAttachedDatabases(conn);
    if (attachedDBs != null && attachedDBs.containsKey(dbname)) return;
    ...
    if (attachedDBs != null && attachedDBs.size() >= MAX_ATTACHED_DATABASES) {
      for (Iterator<String> it = attachedDBs.keySet().iterator();
          attachedDBs.size() >= MAX_ATTACHED_DATABASES && it.hasNext(); ) {
        String name = it.next();

        if (!name.equals("zimbra") && detachDatabase(conn, name)) it.remove();
      }
    }
  ...
  void attachDatabase(DbConnection conn, String dbname) throws SQLException, ServiceException {
    ...
    try {
      ...
      (stmt =
              conn.prepareStatement(
                  "ATTACH DATABASE \"" + getDatabaseFilename(dbname) + "\" AS " + dbname))
          .execute();
      pragmas(conn.getConnection(), dbname);
    } catch (SQLException e) {
      ZimbraLog.dbconn.error("database " + dbname + " attach failed", e);
      if (!"database is already attached".equals(e.getMessage())) throw e;
    } finally {
      if (!autocommit) {
        try {
          conn.getConnection().setAutoCommit(autocommit);
          ...
          ZimbraLog.dbconn.warn(
              "failed to reset autocommit to false. probably caused by prior errors " + dbname);
          DbPool.quietClose(conn);
          throw ServiceException.FAILURE("failed to reset autocommit to false", sqle);
        }
      }
    ...
    if (attachedDBs != null) {
      ...
      attachedDBs =
          new LinkedHashMap<String, String>(MAX_ATTACHED_DATABASES * 3 / 2, (float) 0.75, true);
      attachedDBs.put(dbname, null);
  ...
  void preOpen(Integer mboxId) {
    ...
    // (e.g. one site locks some object before opening connection; another incorrectly locks same
    // object after opening connection)
  ...
  public boolean databaseExists(DbConnection conn, String dbname) throws ServiceException {
    if (!new File(getDatabaseFilename(dbname)).exists()) return false;
  ...
  final class SQLiteConfig extends DbPool.PoolConfig {
    SQLiteConfig() {
      ...
      whenExhaustedAction =
          GenericObjectPool
              .WHEN_EXHAUSTED_GROW; // we use a small pool. we can easily starve when any code
      // requires more than one connection to complete a single
      // operation

      // override pool size if specified in prefs
      mPoolSize =
          readConfigInt("sqlite_pool_size", "connection pool size", DEFAULT_CONNECTION_POOL_SIZE);
    }

    private Properties getSQLiteProperties() {
      Properties props = new Properties();
      if (LC.sqlite_shared_cache_enabled.booleanValue()) props.setProperty("shared_cache", "true");
      return props;
    }
  }
  ...
  static int readConfigInt(final String keyname, final String description, final int defaultvalue) {
    ...
    try {
      ...
      ZimbraLog.dbconn.warn(
          "exception parsing '" + keyname + "' config; defaulting limit to " + defaultvalue, nfe);
    }
  ...
  public void checkParamLimit(int numParams) throws ServiceException {
    if (numParams > getParamLimit()) {
      throw ServiceException.FAILURE(
          "SQLite parameter limit will be exceeded",
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      String outStr =
          "-- AUTO-GENERATED .SQL FILE - Generated by the SQLite versions tool\n"
  ...
  public String lpad(String field, int padSize, String padString) {
    return "SUBSTR('"
        + Strings.repeat(padString, padSize)
        + "' || "
        + field
        + ", -"
        + padSize
