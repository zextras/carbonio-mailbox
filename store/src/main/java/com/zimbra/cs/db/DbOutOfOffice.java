...
public class DbOutOfOffice {
  ...
  /**
   * Determines whether an out-of-office reply has already been sent to the specified address within
   * the last <code>numDays</code> days.
   *
  ...
  public static boolean alreadySent(
      DbConnection conn, Mailbox mbox, String sentTo, long cacheDurationMillis)
    ...
    try {
      stmt =
          conn.prepareStatement(
              "SELECT COUNT(*) FROM "
                  + DbMailbox.qualifyZimbraTableName(mbox, TABLE_NAME)
                  + " WHERE "
    ...
    if (mLog.isDebugEnabled()) {
      mLog.debug(
          "DbOutOfOffice.alreadySent() returning "
              + result
              + ".  mailbox_id="
              + mbox.getId()
              + ", sent_to='"
  ...
  /**
   * Stores a row in the out_of_office table, indicating that we sent an out-of-office reply to the
   * specified address.
   *
   * @param conn the database connection
   ...
   * @throws ServiceException if a database error occurred
   */
  public static void setSentTime(DbConnection conn, Mailbox mbox, String sentTo, long sentOn)
      throws ServiceException {
    ...
    try {
      ...
      if (num > 0) {
        if (mLog.isDebugEnabled()) {
          mLog.debug(
              "DbOutOfOffice.setSentTime: ok (mailbox_id="
                  + mbox.getId()
                  + " sent_to="
                  + sentTo
                  + " sent_on="
        ...
        mLog.error(
            "DbOutOfOffice.setSentTime: no rows updated (mailbox_id="
                + mbox.getId()
                + " sent_to="
                + sentTo
                + " sent_on="
      ...
      if (Db.errorMatches(e, Db.Error.DUPLICATE_ROW)) {
        try {
          ...
          throw ServiceException.FAILURE(
              "DbOutOfOffice.setSentTime: sql exception "
                  + "(mailbox_id="
                  + mbox.getId()
                  + " sent_to"
                  + sentTo
  ...
  /**
   * Clears entries from the <code>out_of_office</code> table for the specified mailbox.
   *
  ...
  public static void prune(DbConnection conn, long cacheDurationMillis) throws ServiceException {
    // there's no centralized OoO table to prune in the DB-per-user case
    if (DebugConfig.disableMailboxGroups) return;
    ...
    try {
      ...
      if (mLog.isDebugEnabled())
        mLog.debug(
            "DbOutOfOffice.prune() deleted " + num + " rows with cutoff <= " + cutoff.getTime());
    } catch (SQLException e) {
