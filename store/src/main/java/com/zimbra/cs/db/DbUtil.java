...
/**
 * <code>DbUtil</code> contains some database utility methods and automates the task of running SQL
 * statements. It wraps the JDBC <code>Connection</code>and <code>PreparedStatement</code> classes
 * and frees the caller from having to handle <code>SQLException</code> and allocate and deallocate
 * database resources.
 *
 * <p>Query results are read entirely into memory and returned in the form of a {@link
 * com.zimbra.cs.db.DbResults} object. This improves concurrency, but potentially increases memory
 * consumption. Code that deals with large result sets should use the JDBC API's directly.
 *
 * @author bburtin
 */
public final class DbUtil {
  ...
  /**
   * Converts a <tt>java.util.Date</tt> to a <tt>java.sql.Timestamp</tt>. If the date is
   * <tt>null</tt>, returns <tt>null</tt>.
   */
  ...
  /**
   * Converts a <tt>java.sql.Timestamp</tt> to a <tt>java.util.Date</tt>. If the timestamp is
   * <tt>null</tt>, returns <tt>null</tt>.
   */
  ...
  /**
   * Converts the given <code>SQL</code> string to a normalized version. Strips out any numbers,
   * string constants and extra whitespace. Flattens the contents of <code>IN</code> clauses.
   */
   ...
   *
   * @throws ServiceException if the query cannot be executed or an error occurs while retrieving
   *     results
   */
   ...
   */
  public static int executeUpdate(DbConnection conn, String sql, Object... params)
      throws ServiceException {
    PreparedStatement stmt = null;
    int numRows = 0;
  ...
  /**
   * Returns a string with the form <code>(?, ?, ?, ...)</code>, which contains as many question
   * marks as the number of elements in the given array. Used for generating SQL IN clauses.
   */
  ...
  /**
   * Returns a string with the form <code>(?, ?, ?, ...)</code>. Used for generating SQL IN clauses.
   *
  ...
  /**
   * Executes all SQL statements in the specified SQL script. Statements are separated by
   * semicolons.
   *
   * @param conn the database connection to use for executing the SQL statements
   * @param scriptReader the source of the SQL script file. The reader is closed when this method
   *     returns.
   */
  ...
  /**
   * Parses a SQL script into separate SQL statements, separated by semicolons. Removes comments
   * that begin with <code>--</code> or <code>#</code>.
   */
  public static String[] parseScript(Reader scriptReader) throws IOException {
    ...
    if (buf.length() != 0 || ret == null)
      ret =
          addScript(
              ret,
              buf.toString().split("\\s*" + Db.getInstance().scriptCommandDelimiter() + "\\s*"));
  ...
  /**
   * Returns a string that optimizes SQL WHERE IN clauses to an operator when the parm list is
   * small. A single IN can prevent some DBs like sqlite from choosing proper indexes and a small #
