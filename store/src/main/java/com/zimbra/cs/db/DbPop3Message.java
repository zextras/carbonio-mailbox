...
public class DbPop3Message {
  ...
  public static void storeUid(Mailbox mbox, String dataSourceId, String uid, int itemId)
    ...
    if (StringUtil.isNullOrEmpty(uid)) return;
  ...
  public static Map<Integer, String> getMappings(Mailbox mbox, String dataSourceId)
    ...
    try {
      ...
      while (rs.next()) mappings.put(rs.getInt(1), rs.getString(2));
    } catch (SQLException e) {
  ...
  /** Returns the set of persisted UID's that are also in the <code>uids</code> collection. */
        ...
        stmt =
            conn.prepareStatement(
                "SELECT uid FROM "
                    ...
                    + "data_source_id = ?"
                    + " AND "
                    + DbUtil.whereIn("uid", curIds.size()));
        int pos = 1;
        pos = DbMailItem.setMailboxId(stmt, mbox, pos);
