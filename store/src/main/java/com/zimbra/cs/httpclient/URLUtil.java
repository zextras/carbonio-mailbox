...
public class URLUtil {
  ...
  public static String getSoapPublicURL(Server server, Domain domain, boolean preferSSL)
    ...
    return URLUtil.getPublicURLForDomain(
        server, domain, AccountConstants.USER_SERVICE_URI, preferSSL);
  }
  ...
  public static String getAdminURL(Server server, String path) {
    ...
    sb.append(LC.zimbra_admin_service_scheme.value())
        .append(hostname)
        .append(":")
        .append(port)
        .append(path);
  ...
  public static String getMtaAuthURL(Server server) {
    ...
    try {
      ...
      port = server.getMtaAuthPort();
      ZimbraLog.misc.warn(
          "cannot determine server version; defaulting to port %d for MTA auth", port);
    }
    StringBuffer sb = new StringBuffer(128);
   ...
   *
   * <p>Not the best place for this method, but do not want to pollute Provisioning with utility
   * methods either.
   */
  public static String getMtaAuthURL(String authHost) throws ServiceException {
    ...
    throw ServiceException.INVALID_REQUEST(
        "specified "
            + Provisioning.A_zimbraMtaAuthHost
            + " does not correspond to a valid service hostname: "
            + authHost,
        null);
  }
  ...
  public static String getPublicURLForDomain(
      Server server, Domain domain, String path, boolean preferSSL) throws ServiceException {
    String publicURLForDomain = getPublicURLForDomain(domain, path);
    if (publicURLForDomain != null) return publicURLForDomain;
  ...
  private static String getAdminConsoleProxyUrl(
      Server server, Domain domain, boolean isAdminSoapURL) throws ServiceException {
    if (domain == null) {
      return null;
    }
    ...
    String portString =
        Provisioning.getInstance().getConfig().getAttr(Provisioning.A_zimbraAdminProxyPort, null);
    if (portString == null) {
      return null;
    }
    int port = 9071;
  ...
  public static String getServiceURL(Server server, String path, boolean useSSL)
    ...
    if (hostname == null)
      throw ServiceException.INVALID_REQUEST(
          "server " + server.getName() + " does not have " + Provisioning.A_zimbraServiceHostname,
          null);
    ...
    if (modeString == null)
      throw ServiceException.INVALID_REQUEST(
          "server "
              + server.getName()
              + " does not have "
              + Provisioning.A_zimbraMailMode
              + " set, maybe it is not a store server?",
          null);
