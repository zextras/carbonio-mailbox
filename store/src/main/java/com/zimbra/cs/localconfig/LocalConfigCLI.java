...
public final class LocalConfigCLI {
  ...
  private LocalConfigCLI() {
    ...
    mOptions.addOption(
        ...
        "edit",
        false,
        "Edit configuration file, changing keys and values specified. [args] is in key=value"
            + " form.");
    mOptions.addOption(
        "r",
        "random",
    ...
    mOptions.addOption(
        ...
        "changed",
        false,
        "Show values for only those keys listed in [args] that have been changed from their"
            + " defaults.");
    mOptions.addOption("i", "info", false, "Show documentation for keys listed in [args].");
    mOptions.addOption(
        null, "all", false, "Shows documentation for all keys, including unsupported ones");
    ...
    mOptions.addOption(
        ...
        "unset",
        false,
        "Remove a configuration key.  If this is a key with compiled in defaults, set its value to"
            + " the empty string.");
    mOptions.addOption(
        "l", "reload", false, "Send a SOAP request to the server to reload its local config.");
  ...
  private void usage() {
    ...
    formatter.printHelp(
        "zmlocalconfig [options] [args]", "where [options] are:", displayOptions, "");
    System.exit(0);
  }
      ...
      for (Field field : fields) {
        try {
          if (field.getType() == KnownKey.class) {
            ...
            // process annotations
            if (field.isAnnotationPresent(Supported.class)) key.setSupported(true);
            if (field.isAnnotationPresent(Reloadable.class)) key.setReloadable(true);
          }
  ...
  private void reload() throws ServiceException {
    ...
    SoapHttpTransport transport =
        new SoapHttpTransport("https://" + host + ":" + port + AdminConstants.ADMIN_SERVICE_URI);
