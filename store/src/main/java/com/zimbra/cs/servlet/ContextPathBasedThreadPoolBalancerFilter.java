...
/**
 * This Servlet {@link Filter} prevents requests for any given context path like /service, /soap,
 * /zimbra, or /zimbraAdmin from monopolizing all threads in the current pool.
 */
public class ContextPathBasedThreadPoolBalancerFilter implements Filter {
  ...
  protected boolean shouldSuspend(ServletRequest request) {
    ...
    // Disable this servlet filter if the current thread pool is not known (we're probably not
    // running in Jetty)
    ...
    if (rules != null) {
      ...
      ZimbraLog.misc.debug(
          ...
          getLoggableContextPath(contextPath),
          activeRequestsCount,
          threads,
          idleThreads,
          busyThreads,
      ...
      if (rules.maxPercent != null) {
        if (100 * activeRequestsCount / queuedThreadPool.getMaxThreads() > rules.maxPercent) {
          ZimbraLog.misc.info(
              "Suspending for %dms because context path %s is at %d threads (%d configured max"
                  + " percentage of thread pool size)",
              suspendMs,
              getLoggableContextPath(contextPath),
              activeRequestsCount,
              rules.maxPercent);
