 ...
 * /service, /soap, /zimbra, or /zimbraAdmin from monopolizing all threads in
 * the current pool.
 */
public class ContextPathBasedThreadPoolBalancerFilter implements Filter {
    ...
    final ConcurrentHashMap<String, AtomicInteger> activeRequestsByContextPath = new ConcurrentHashMap<>();
    final ConcurrentHashMap<String, Rules> rulesByContextPath = new ConcurrentHashMap<>();
    ...
    protected boolean shouldSuspend(ServletRequest request) {
        ...
        // Disable this servlet filter if the current thread pool is not known (we're
        // probably not running in Jetty)
        if (queuedThreadPool == null) {
            return false;
        }

        // Determine whether request is for one of the context paths this filter is
        // managing QoS for
        String contextPath = getContextPath(request);
        ...
        if (rules != null) {
            ...
            if (rules.max != null) {
                if (activeRequestsCount > rules.max) {
                    ZimbraLog.misc.info("Suspending for %dms because context path %s is at %d configured max threads",
    ...
    static class Rules {
        ...
        public static Rules parse(String input) throws ServletException {
            ...
            for (String str : new StrTokenizer(input, ";").getTokenArray()) {
                ...
                switch (key) {
                    case "min":
                        rule.min = Integer.valueOf(value);
                        break;
                    case "max":
                        if (value.endsWith("%")) {
                            rule.maxPercent = Integer.valueOf(value.substring(0, value.length() - 1));
                        } else {
                            rule.max = Integer.valueOf(value);
                        }
                        break;
                    default:
