...
public abstract class DiskCacheServlet extends ZimbraServlet {
  ...
  /**
   * Returns the name of the cache dir. This is <em>not</em> the full path of the cache dir; it is
   * name of the dir where resources for this servlet should be stored. The cache dir is the
   * sub-directory of the base temp directory.
   */
  ...
  protected void createCache() {
    ...
    cache =
        new LruMap<String, File>(cacheSize) {
          @Override
        ...
        if (LC.zimbra_disk_cache_servlet_flush.booleanValue()) {
          ...
          for (String file : cacheDir.list()) {
            ...
            if (idx != -1) {
              ...
              cache.put(cacheKey, new File(cacheDir.getAbsolutePath() + File.separatorChar + file));
            }
          }
        }
      }
  ...
  protected File getTempDir() {
    ...
    if (cacheDirname != null) {
      ...
      cacheDirname =
          cacheDirname
              .replaceAll("\\$\\{webapp\\}", webappDirname)
  ...
  /**
   * Compress a file and store the content in a new file in the same location as the original file,
   * appending ".gz" extension.
   */
