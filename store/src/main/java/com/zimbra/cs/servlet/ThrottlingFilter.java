...
/**
 * This Servlet {@link Filter} limits the number of concurrent HTTP requests per HTTP session.
 *
 * <p>It returns 503 HTTP error if exceeding the limit. The limit is configurable by {@link
 * LC#servlet_max_concurrent_requests_per_session}. To disable, set the LC key to 0 or remove this
 * servlet filter from web.xml.
 *
 * @author ysasaki
 */
public final class ThrottlingFilter implements Filter {
  ...
  public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
    ...
    if (tracker.tryAcquire()) {
      ...
      hresp.sendError(
          HttpServletResponse.SC_SERVICE_UNAVAILABLE, "too many concurrent HTTP requests");
    }
  }
