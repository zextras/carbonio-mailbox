...
public class ZimbraInvalidLoginFilter extends DoSFilter {
  ...
  public int DEFAULT_SIZE_OF_FAILED_IP_DB =
      7000; // Considering we do not want to exceed 1MB of data
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    try {
      this.reinstateIpTaskIntervalInMin =
          prov.getLocalServer().getInvalidLoginFilterReinstateIpTaskIntervalInMin();
    } catch (ServiceException e) {
      this.reinstateIpTaskIntervalInMin = DEFAULT_REINSTATE_IP_TASK_INTERVAL_IN_MIN;
    }

    try {
      this.delayInMinBetwnReqBeforeReinstating =
          prov.getLocalServer().getInvalidLoginFilterDelayInMinBetwnReqBeforeReinstating();
    } catch (ServiceException e) {
      this.delayInMinBetwnReqBeforeReinstating =
          DEFAULT_DELAY_IN_MIN_BETWEEN_REQ_BEFORE_REINSTATING;
    }
    ...
    this.numberOfFailedOccurence =
        new ConcurrentLinkedHashMap.Builder<String, AtomicInteger>()
            .maximumWeightedCapacity(maxSizeOfFailedIpDb)
            .build();

    this.suspiciousIpAddrLastAttempt =
        new ConcurrentLinkedHashMap.Builder<String, Long>()
            .maximumWeightedCapacity(maxSizeOfFailedIpDb)
            .build();

    Zimbra.sTimer.schedule(
        new ReInStateIpTask(), 1000, this.reinstateIpTaskIntervalInMin * MIN_TO_MS);
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (this.suspiciousIpAddrLastAttempt.containsKey(clientIp)) {
      ...
      if (req.getAttribute(AUTH_FAILED) != null) {
        ZimbraLog.misc.info(
            "Invalid login filter, checking if this was an auth req and authentication failed.");
        ...
        if (ZimbraLog.misc.isDebugEnabled()) {
          ZimbraLog.misc.debug("Login failed " + clientIP + ", " + loginFailed);
        }
      }
    }
  }
