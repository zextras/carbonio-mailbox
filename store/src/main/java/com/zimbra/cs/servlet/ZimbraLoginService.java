...
public class ZimbraLoginService implements LoginService {
      ...
      if (account != null) {
        if (!(credentials instanceof String)) {
          ZimbraLog.security.warn(
              "passed credentials are not a String? [%s]",
  ...
  private void tryLogin(Account account, String credentials, boolean first)
      throws ServiceException {
    // try login, or pass along exception if failed
    Provisioning prov = Provisioning.getInstance();
    if (account != null) {
      try {
        prov.authAccount(account, credentials, AuthContext.Protocol.http_basic);
      } catch (AuthFailedServiceException e) {
        if (!first
            || !(prov instanceof CacheAwareProvisioning)
            || !((CacheAwareProvisioning) prov).isCacheEnabled()) {
          throw e;
        } else {
          // we may have the old password cached; so reload and try one more time
          ZimbraLog.security.debug(
              "auth failed, refreshing Account object and trying again in case of recent password"
                  + " change");
          prov.reload(account);
  ...
  UserIdentity makeUserIdentity(String userName) {
    ...
    UserIdentity identity =
        identityService.newUserIdentity(subject, userPrincipal, new String[] {roleName});
    return identity;
  }
}
