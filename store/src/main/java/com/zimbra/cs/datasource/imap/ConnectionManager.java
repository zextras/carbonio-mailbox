...
final class ConnectionManager {
  ...
  /**
   * Opens a new IMAP connection or reuses an existing one if available for specified data source.
   * If a new connection is required the optional authenticator, if specified, will be used with
   * AUTHENTICATE, otherwise the LOGIN command will be used. If a suspended connection is reused and
   * has been idling, then IDLE will be automatically terminated.
   *
  ...
  /**
   * Releases existing connection so that it can be reused again. If the IMAP server supports IDLE
   * then the connection will go into IDLE state. Otherwise, the connection will be kept active for
   * as long as possible.
   *
   * @param ds the data source for the connection
  ...
  /**
   * Closes any suspended connection associated with specified data source. This must be called
   * whenever data source is modified or deleted in order to force a reconnect upon next use.
   *
      ...
      try {
        if (config.getMechanism() != null) {
          if (SaslAuthenticator.XOAUTH2.equalsIgnoreCase(config.getMechanism())) {
            auth =
                AuthenticatorFactory.getDefault()
                    .newAuthenticator(config, ds.getDecryptedOAuthToken());
          } else {
            auth =
                AuthenticatorFactory.getDefault()
                    .newAuthenticator(config, ds.getDecryptedPassword());
          }
        }
        if (auth == null) {
    ...
    if (!ds.isOffline() && ic.hasCapability(ImapCapabilities.ID)) {
      try {
        ...
        if ("Zimbra".equalsIgnoreCase(id.get(IDInfo.NAME)) && ds.getAccount() != null) {
          ...
          return user != null
              && user.equals(ds.getAccount().getName())
              && server != null
