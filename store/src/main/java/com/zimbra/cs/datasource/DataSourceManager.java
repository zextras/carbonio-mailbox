...
public class DataSourceManager {
        ...
        try {
          ...
          if (className != null && className.length() > 0) {
            ...
            ZimbraLog.datasource.warn(
                "Could not find DataImport class: %s for xsync dataSource %s Check your classpath.",
                className, ds.getName());
            return null;
          }
        } catch (Exception x) {
      ...
      default:
        ...
        if (className != null && className.length() > 0) {
          try {
            ...
            ZimbraLog.datasource.warn(
                "Could not find DataImport class: %s for dataSource %s Check your classpath.",
                className, ds.getName());
            return null;
          } catch (Exception x) {
            ZimbraLog.datasource.warn(
                "Caught an exception while instantiating DataImport class: %s", ds, x);
            return null;
          }
        } else {
          throw new IllegalArgumentException(
              String.format(
                  "Cannot create datasource %s with unknown data import type: %s and undefined"
                      + " zimbraDataSourceImportClassName",
                  ds.getName(), ds.getType()));
        }
    }
  }
  ...
  /**
   * Executes the data source's {@link MailItemImport} implementation to import data in the current
   * thread.
   */
    ...
    synchronized (importStatus) {
      if (importStatus.isRunning()) {
        ZimbraLog.datasource.info(
            "Attempted to start import while "
  ...
  private static void setErrorStatus(DataSource ds, String error) {
    ...
    if (ds.getAttr(Provisioning.A_zimbraDataSourceFailingSince) == null) {
      attrs.put(
          Provisioning.A_zimbraDataSourceFailingSince, LdapDateUtil.toGeneralizedTime(new Date()));
    }
    try {
  ...
  public static DataSourceTask getTask(Mailbox mbox, String dsId) {
    return (DataSourceTask)
        ScheduledTaskManager.getTask(DataSourceTask.class.getName(), dsId, mbox.getId());
  }
   ...
   * @throws ServiceException
   */
  public static void cancelSchedule(Account account, String dsId) throws ServiceException {
   ...
   * @throws ServiceException
   */
  public static void updateSchedule(Account account, DataSource ds) throws ServiceException {
  ...
  private static void updateSchedule(
      Account account, DataSource ds, String dsId, boolean cancelSchedule) throws ServiceException {
  ...
  public static void refreshOAuthToken(DataSource ds) {
    ...
    try {
      ...
      if (status == HttpStatus.SC_OK) {
        ...
        Map<String, Object> attrs = new HashMap<String, Object>();
        attrs.put(
            Provisioning.A_zimbraDataSourceOAuthToken,
