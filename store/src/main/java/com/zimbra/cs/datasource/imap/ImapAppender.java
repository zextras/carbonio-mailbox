...
public class ImapAppender {
  ...
  private long appendSlow(MessageInfo mi, Literal lit) throws IOException, MessagingException {
    ...
    try {
      connection.select(
          mailbox); // exchange doesn't give accurate UIDNEXT unless mbox is selected again.
      ...
      try {
        ...
        // bug 64062 : let's try 2 minutes. hopefully search shouldn't take too long now that we use
        // msg id rather than subject
      ...
      while (it.hasNext()) {
        ...
        if (found.size() > 0) {
          if (found.size() > 1) {
            ZimbraLog.imap_client.warn(
                "found more than one (%d)"
                    + "matching UID during appendSlow. Probably a leftover dupe from earlier bugs?",
                found.size());
            if (ZimbraLog.imap_client.isDebugEnabled()) {
              ZimbraLog.imap_client.debug(
                  "potential duplicate ids = %s", Joiner.on(',').join(found));
            }
          }
          return found.get(0);
        }
    ...
    // this usually is OK, and actually the way Exchange has been working due to size check in
    // matches()
  ...
  private boolean matches(MessageInfo mi, MessageData md) throws MessagingException {
    // bug 64062 Exchange misreports RFC 822 size unless configured with: Set-ImapSettings
    // -EnableExactRFC822Size:$true
