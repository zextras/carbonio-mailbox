...
/**
 * IOException handler Default implementation does nothing; may override to perform special handling
 * of IOExceptions
 */
      ...
      if (className != null && !className.equals("")) {
        try {
          ...
          ZimbraLog.account.error(
              "could not instantiate IOExceptionHandler child class '"
                  + className
                  + "'; defaulting to IOExceptionHandler",
              e);
        }
      }
      if (instance == null) instance = new IOExceptionHandler();
    }
    return instance;
  }

  public boolean isRecoverable(Mailbox mbox, long itemId, String message, Exception exception)
