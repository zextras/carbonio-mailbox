...
public abstract class ZimletMeta {
  ...
  public ZimletMeta(String meta) throws ZimletException {
    ...
    try {
      ...
      throw ZimletException.INVALID_ZIMLET_DESCRIPTION(
          "Cannot parse Zimlet description: " + de.getMessage());
    }
  ...
  private static String readFile(File f) throws ZimletException {
    try {
      ...
      throw ZimletException.INVALID_ZIMLET_DESCRIPTION(
          "Cannot find Zimlet description file: " + f.getName());
    }
  }
  ...
  protected void validate() throws ZimletException {
    ...
    if (!name.equals(ZimletConstants.ZIMLET_TAG_ZIMLET)
        && !name.equals(ZimletConstants.ZIMLET_TAG_CONFIG)) {
    ...
    mZimbraXZimletCompatibleSemVer =
        mTopElement.getAttribute(ZimletConstants.ZIMLET_ATTR_ZIMBRAX_SEMVER, "");
    try {
