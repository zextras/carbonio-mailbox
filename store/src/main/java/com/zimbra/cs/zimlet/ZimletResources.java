...
public class ZimletResources extends DiskCacheServlet {
    ...
    if (!pathInfo.startsWith(RESOURCE_PATH)) {
      ...
      if (targetContext == null) {
        throw new ServletException(
            "Could not forward the request to zimlet webapp, possible misconfiguration.");
      }
    ...
    if (file == null || !file.exists()) {
      ...
      try {
        ...
        if (!debug) {
          ...
          String uncompressedCacheId =
              compress ? cacheId.substring(0, cacheId.length() - COMPRESSED_EXT.length()) : cacheId;
          ...
          if (compress) {
            ...
            File gzfile =
                createCacheFile(compressedCacheId, type + DiskCacheServlet.EXT_COMPRESSED);
            if (ZimbraLog.zimlet.isDebugEnabled()) {
    ...
    for (String zimlet : zimletNames) {
      ...
      try {
        ...
        for (String f : files) {
          ...
          printFile(
              out,
              new BufferedReader(new InputStreamReader(entry.getContentStream())),
              zimlet,
              isCSS);
          out.println();
        }
  ...
  private String getCacheId(
      HttpServletRequest req, String type, Set<String> zimletNames, Set<String> allZimletNames) {
