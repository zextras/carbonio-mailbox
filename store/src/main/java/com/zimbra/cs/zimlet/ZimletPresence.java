...
public class ZimletPresence {
  ...
  public static enum Presence {
    ...
    public static Presence fromPrefix(char prefix) {
      ...
      } else return null; // leave it to the caller to decide that action
    }
  ...
  public void put(String zimletWithPrefix) {
    ...
    if (zimletWithPrefix.length() == 0) return;
    ...
    if (presence != null) {
      if (zimletWithPrefix.length() <= 1) return;
  ...
  public void remove(String zimletNameOrWithPrefix) {
    ...
    if (presence == null) {
      ...
      if (p == presence) mZimlets.remove(zimletName);
    }
  }
