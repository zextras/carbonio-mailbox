...
/**
 * @author schemers
 *     <p>Generic object handler that gets its regex from the handler config.
 */
  ...
  public String[] match(String text, ZimletConf config) throws ZimletException {
    if (mPattern == null) {
      ...
      ZimbraLog.zimlet.debug(
          "RegexHandler %s=%s (for config=%s)",
          ZimletConfig.CONFIG_REGEX_VALUE, handlerConfig, config.getClass().getName());
    }
    Matcher m = mPattern.matcher(text);
    List<String> l = Lists.newArrayList();
    while (m.find()) {
      l.add(text.substring(m.start(), m.end()));
      ZimbraLog.zimlet.trace(
          "RegexHandler matcher found match=[%s] for pattern=[%s]",
