...
/**
 * The equivalent of Props2JsServlet in ZimbraWebClient, handles .properties within deployed zimlets
 * only.
 */
public class ZimletProps2JsServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    ZimbraLog.zimlet.debug(
        "ZimletProps2JsServlet base context path: %s",
        this.getServletConfig().getServletContext().getContextPath());

    synchronized (buffers) {
      localeBuffers = buffers.get(locale);
      if (localeBuffers == null) {
        localeBuffers = Collections.synchronizedMap(new HashMap<String, byte[]>());
  ...
  protected void load(
    ...
    for (List<String> basenames : basenamePatterns) {
      try {
        ...
        PropsLoader loader = new PropsLoader(parentLoader, basenames, basedir, dirname, classname);
            ...
            new ResourceBundle.Control() {
              ...
              public List<Locale> getCandidateLocales(String baseName, Locale locale) {
                ...
                if (locale.equals(new Locale("zh", "HK"))
                    || locale.equals(new Locale("zh", "CN"))
        ...
        out.writeBytes("// properties error for " + classname + " - see server log\n");
        ZimbraLog.zimlet.error(e.getMessage());
      }
    }
  }
