...
public class ProxyServlet extends ZimbraServlet {
  ...
  private void doProxy(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    boolean asUpload =
        uploadParam != null && (uploadParam.equals("1") || uploadParam.equalsIgnoreCase("true"));
    ...
    try {
      ...
      if (reqMethod.equalsIgnoreCase("GET")) {
        ...
        if (body != null)
          post.setEntity(
              new ByteArrayEntity(
                  body, org.apache.http.entity.ContentType.create(req.getContentType())));
        method = post;
      ...
      int status =
          httpResp.getStatusLine() == null
              ? HttpServletResponse.SC_INTERNAL_SERVER_ERROR
              : httpResp.getStatusLine().getStatusCode();

      // workaround for Alexa Thumbnails paid web service, which doesn't bother to return a
      // content-type line
      Header ctHeader = httpResp.getFirstHeader("Content-Type");
      String contentType =
          ctHeader == null || ctHeader.getValue() == null ? DEFAULT_CTYPE : ctHeader.getValue();
      ...
      if (asUpload) {
        ...
        if ((filename == null || filename.equals(""))
            && httpResp.getFirstHeader("Content-Disposition") != null)
          filename =
              new ContentDisposition(httpResp.getFirstHeader("Content-Disposition").getValue())
          ...
          try {
            Upload up =
                FileUploadServlet.saveUpload(
                    targetResponseBody, filename, contentType, authToken.getAccountId());
            uploads = Arrays.asList(up);
          } catch (ServiceException e) {
            if (e.getCode().equals(MailServiceException.UPLOAD_REJECTED))
              status = HttpServletResponse.SC_REQUEST_ENTITY_TOO_LARGE;
            else status = HttpServletResponse.SC_INTERNAL_SERVER_ERROR;
          }
        }

        resp.setStatus(status);
        FileUploadServlet.sendResponse(
            resp, status, req.getParameter(FORMAT_PARAM), null, uploads, null);
      } else {
        resp.setStatus(status);
        resp.setContentType(contentType);
        for (Header h : httpResp.getAllHeaders())
          if (canProxyHeader(h.getName())) resp.addHeader(h.getName(), h.getValue());
        if (targetResponseBody != null)
