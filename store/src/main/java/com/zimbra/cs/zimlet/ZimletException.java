...
public class ZimletException extends Exception {
  ...
  public static ZimletException INVALID_ZIMLET_NAME() {
    return new ZimletException(
        "Zimlet name may contain only letters, numbers and the following symbols: '.', '-' and"
            + " '_'");
  }
  ...
  public static ZimletException INVALID_ABSOLUTE_PATH(String entry) {
    return new ZimletException(
        String.format(
            "Invalid entry in Zimlet archive: %s. Zimlet entries with absolute paths are not"
                + " allowed.",
            entry));
  }
  ...
  public static ZimletException CANNOT_DEPLOY(String zimlet, String msg, Throwable cause) {
    return new ZimletException(
        String.format("Cannot deploy Zimlet %s. Error message: %s", zimlet, msg), cause);
  }
