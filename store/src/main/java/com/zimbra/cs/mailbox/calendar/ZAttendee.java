...
public class ZAttendee extends CalendarUser {
  ...
  public boolean hasCUType() {
    return !StringUtil.isNullOrEmpty(mCUType);
  }

  public String getCUType() {
    return mCUType != null ? mCUType : "";
  }
  ...
  public boolean hasRole() {
    return !StringUtil.isNullOrEmpty(mRole);
  }

  public String getRole() {
    return mRole != null ? mRole : "";
  }
  ...
  public boolean hasPartStat() {
    return !StringUtil.isNullOrEmpty(mPartStat);
  }

  public String getPartStat() {
    return mPartStat != null ? mPartStat : "";
  }
  ...
  public boolean hasMember() {
    return !StringUtil.isNullOrEmpty(mMember);
  }
  ...
  public void setMember(String member) {
    mMember = getMailToAddress(member);
  }

  public boolean hasDelegatedTo() {
    return !StringUtil.isNullOrEmpty(mDelegatedTo);
  }
  ...
  public void setDelegatedTo(String delTo) {
    mDelegatedTo = getMailToAddress(delTo);
  }

  public boolean hasDelegatedFrom() {
    return !StringUtil.isNullOrEmpty(mDelegatedFrom);
  }
  ...
  public void setDelegatedFrom(String delFrom) {
    mDelegatedFrom = getMailToAddress(delFrom);
  }
  ...
  public ZAttendee(Metadata meta) throws ServiceException {
    ...
    if (meta.containsKey(FN_RSVP_BOOL)) {
      if (meta.getBool(FN_RSVP_BOOL)) rsvp = Boolean.TRUE;
      else rsvp = Boolean.FALSE;
    }
    setRsvp(rsvp);
  ...
  public Metadata encodeAsMetadata() {
    ...
    if (hasCUType()) meta.put(FN_CUTYPE, getCUType());
    if (hasRole()) meta.put(FN_ROLE, getRole());
    if (hasPartStat()) meta.put(FN_PARTSTAT, getPartStat());
    if (hasRsvp() && getRsvp().booleanValue()) meta.put(FN_RSVP_BOOL, "1");
    if (hasMember()) meta.put(FN_MEMBER, getMember());
    if (hasDelegatedTo()) meta.put(FN_DELEGATED_TO, getDelegatedTo());
    if (hasDelegatedFrom()) meta.put(FN_DELEGATED_FROM, getDelegatedFrom());
  ...
  protected void setProperty(ZProperty prop) throws ServiceException {
    ...
    if (hasCUType())
      prop.addParameter(
          new ZParameter(ICalTok.CUTYPE, IcalXmlStrMap.sCUTypeMap.toIcal(getCUType())));
    if (hasRole())
      prop.addParameter(new ZParameter(ICalTok.ROLE, IcalXmlStrMap.sRoleMap.toIcal(getRole())));
    if (hasPartStat())
      prop.addParameter(
          new ZParameter(ICalTok.PARTSTAT, IcalXmlStrMap.sPartStatMap.toIcal(getPartStat())));
    if (hasRsvp()) {
      // Apple Mac Calendar thinks a reply is still required if RSVP is set.  Suppress this if the
      // PARTSTAT
      // isn't NEEDS-ACTION
      if (IcalXmlStrMap.PARTSTAT_NEEDS_ACTION.equals(getPartStat())
          || IcalXmlStrMap.CUTYPE_RESOURCE.equals(getCUType())) {
        prop.addParameter(new ZParameter(ICalTok.RSVP, getRsvp()));
      }
    }
    if (hasMember()) prop.addParameter(new ZParameter(ICalTok.MEMBER, "MAILTO:" + getMember()));
    if (hasDelegatedTo())
  ...
  protected StringBuilder addToStringBuilder(StringBuilder sb) {
    ...
    if (hasRsvp()) {
      if (sb.length() > 0) sb.append(';');
      sb.append("RSVP=");
      if (getRsvp().booleanValue()) sb.append("TRUE");
  ...
  public boolean addressMatches(Address address) {
    if (address == null) {
      ZimbraLog.calendar.debug(
          "addressMatches called with 'null' - can never match Zattendee '%s'", this);
      return false;
    }
    ...
    try {
      ...
      ZimbraLog.calendar.debug(
          "problem matching address '%s' against Zattendee '%s'", address, this, e);
      return false;
    }
  }
  ...
  public CalendarAttendee toJaxb() {
    ...
    // CN
    if (hasCn()) attendee.setDisplayName(getCn());
    // SENT-BY
    if (hasSentBy()) attendee.setSentBy(getSentBy());
    // DIR
    if (hasDir()) attendee.setDir(getDir());
    // LANGUAGE
    if (hasLanguage()) attendee.setLanguage(getLanguage());
    // CUTYPE
    if (hasCUType()) attendee.setCuType(getCUType());
    // ROLE
    if (hasRole()) attendee.setRole(getRole());
    // PARTSTAT
    if (hasPartStat()) attendee.setPartStat(getPartStat());
    // RSVP
    if (hasRsvp()) attendee.setRsvp(getRsvp());
    // MEMBER
    if (hasMember()) attendee.setMember(getMember());
    // DELEGATED-TO
    if (hasDelegatedTo()) attendee.setDelegatedTo(getDelegatedTo());
    // DELEGATED-FROM
    if (hasDelegatedFrom()) attendee.setDelegatedFrom(getDelegatedFrom());
  ...
  public Element toXml(Element parent) {
    ...
    // CN
    if (hasCn()) atElt.addAttribute(MailConstants.A_DISPLAY, getCn());
    // SENT-BY
    if (hasSentBy()) atElt.addAttribute(MailConstants.A_CAL_SENTBY, getSentBy());
    // DIR
    if (hasDir()) atElt.addAttribute(MailConstants.A_CAL_DIR, getDir());
    // LANGUAGE
    if (hasLanguage()) atElt.addAttribute(MailConstants.A_CAL_LANGUAGE, getLanguage());
    // CUTYPE
    if (hasCUType()) atElt.addAttribute(MailConstants.A_CAL_CUTYPE, getCUType());
    // ROLE
    if (hasRole()) atElt.addAttribute(MailConstants.A_CAL_ROLE, getRole());
    // PARTSTAT
    if (hasPartStat()) atElt.addAttribute(MailConstants.A_CAL_PARTSTAT, getPartStat());
    // RSVP
    if (hasRsvp()) atElt.addAttribute(MailConstants.A_CAL_RSVP, getRsvp().booleanValue());
    // MEMBER
    if (hasMember()) atElt.addAttribute(MailConstants.A_CAL_MEMBER, getMember());
    // DELEGATED-TO
    if (hasDelegatedTo()) atElt.addAttribute(MailConstants.A_CAL_DELEGATED_TO, getDelegatedTo());
    // DELEGATED-FROM
    if (hasDelegatedFrom())
  ...
  public static ZAttendee parse(Element element) throws ServiceException {
    ...
    if (cutype != null) validateAttr(IcalXmlStrMap.sCUTypeMap, MailConstants.A_CAL_CUTYPE, cutype);

    String role = element.getAttribute(MailConstants.A_CAL_ROLE, null);
    if (role != null) validateAttr(IcalXmlStrMap.sRoleMap, MailConstants.A_CAL_ROLE, role);

    String partStat = element.getAttribute(MailConstants.A_CAL_PARTSTAT, null);
    if (partStat != null)
      validateAttr(IcalXmlStrMap.sPartStatMap, MailConstants.A_CAL_PARTSTAT, partStat);
