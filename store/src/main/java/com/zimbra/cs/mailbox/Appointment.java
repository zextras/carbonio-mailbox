 ...
 * From the appointment you can get the INSTANCES which are the start/end times of each occurence.
 *
...
public class Appointment extends CalendarItem {
  ...
  /**
   * Return this accounts "effective" FBA data -- ie the FBA that is the result of the most recent
   * and most specific (specific b/c some replies might be for just one instance, some might be for
   * recurrence-id=0, etc) given the requested Invite and Instance to check against.
   *
   * <p>For example, imagine an appt with no exceptions, but two replies: RECUR=0, REPLY=accept
   * (reply to the default invite, accept it) RECUR=20051010 REPLY=decline (reply to DECLINE the
   * instance on 10/10/2005
   *
   * <p>The FBA for the 10/10 instance will obviously be different than the one for any other
   * instance. If you add Exceptions into the mix, then there are even more permutations.
   *
   * @param inv
   * @param inst
   * @return
  ...
  public String getEffectiveFreeBusyActual(Invite inv, Instance inst) throws ServiceException {
    if (!inv.isOrganizer()) {
      ...
      if (at != null) {
        if (at.hasPartStat()) return inv.partStatToFreeBusyActual(at.getPartStat());
        else return inv.getFreeBusyActual();
      }
    }
    ...
    for (Conflict avail : list) {
      ...
      for (Iterator<Interval> iter = fb.iterator(); iter.hasNext(); ) {
        ...
        if (Conflict.isBusy(interval.getStatus())) {
          ...
          for (FBInstance fbinst : interval.getInstances()) {
            if (Conflict.isBusy(fbinst.getFreeBusy())) instances.add(fbinst);
          }
        }
      }
      for (FBInstance instance : instances) {
        ...
        if (inv != null && inv.hasOrganizer()) {
          ...
          else orgDispName = organizer.getAddress();
          sb.append(
              L10nUtil.getMessage(MsgKey.calendarResourceConflictScheduledBy, lc, orgDispName));
        }
        sb.append("\r\n");
        conflictCount++;
      }
    }
    if (hasMoreConflicts) sb.append(" * ...\r\n");
    return sb.toString();
  }
  ...
  private static class ConflictCheckResult {
    ...
    public ConflictCheckResult(
        List<Conflict> conflicts, boolean tooManyConflicts, boolean hasMoreConflicts) {
    ...
    public boolean tooManyConflicts() {
      return mTooManyConflicts;
    }

    public boolean hasMoreConflicts() {
      return mHasMoreConflicts;
    }
  }

  private static final int MIN_CONFLICT_LIST_SIZE = 5;

  private ConflictCheckResult checkAvailability(
      long now, Invite invite, int maxNumConflicts, int maxPctConflicts) throws ServiceException {
  ...
  protected String processPartStat(
    ...
    if (attendee != null) {
      ...
      if (rsvp != null) rsvpRequested = rsvp.booleanValue();
    }
    ...
    if (invite.isOrganizer()) {
      ...
      else lc = resource.getLocale();
      if (resource.autoAcceptDecline()
          || resource.autoDeclineIfBusy()
          || resource.autoDeclineRecurring()) {
        ...
        if (resource.autoAcceptDecline()) partStat = IcalXmlStrMap.PARTSTAT_ACCEPTED;
        if (isRecurring() && resource.autoDeclineRecurring()) {
          ...
          ConflictCheckResult checkResult =
              checkAvailability(opTime, invite, maxNumConflicts, maxPctConflicts);
          if (checkResult != null) {
              ...
              if (invite.isRecurrence() && !checkResult.tooManyConflicts()) {
                ...
                if (resource.autoAcceptDecline()) {
                  ...
                  if (needReplyEmail) {
                    ...
                    String declinedInstances =
                        getDeclinedTimesString(
                    ...
                    CalendarMailSender.sendResourceAutoReply(
                        ...
                        this,
                        invite,
                        new Invite[] {acceptInv},
                        mmInv);
                    // Send another email to decline instances, all in one email.
                    String conflictingTimes =
                        getBusyTimesString(octxt, mbox, conflicts, tz, lc, false);
                    msg =
                        L10nUtil.getMessage(MsgKey.calendarResourceDeclinedInstances, lc)
                            + "\r\n\r\n"
                            + declinedInstances
                    ...
                    CalendarMailSender.sendResourceAutoReply(
                        ...
                        this,
                        invite,
                        replyInvites.toArray(new Invite[0]),
                        mmInv);
                    replySent = true;
                  }
                } else {
                ...
                if (needReplyEmail) {
                  ...
                  String msg =
                      L10nUtil.getMessage(MsgKey.calendarResourceDeclineReasonConflict, lc)
                          + "\r\n\r\n"
                          + getBusyTimesString(
                              octxt, mbox, conflicts, tz, lc, checkResult.hasMoreConflicts());
                  Invite replyInv =
                      makeReplyInvite(
                          account,
                          authAcct,
        ...
        if (forCreate && replyListUpdated) saveMetadata();
      }
    }
  ...
  private ICalTimeZone chooseReplyTZ(Invite invite) throws ServiceException {
    ...
    if (tz == null && invite.isAllDayEvent()) {
      ...
      if (tz == null) ICalTimeZone.getUTC();
    } else {
      // tz != null || !allday
  ...
  private Invite makeReplyInvite(
      Account account,
      Account authAccount,
      Locale lc,
      boolean onBehalfOf,
    ...
    Invite replyInv =
        CalendarMailSender.replyToInvite(
