...
public class AclPushTask extends TimerTask {
    ...
    try {
      ...
      for (int mboxId : mboxIdToItemIds.keySet()) {
        ...
        try {
          ...
          ZimbraLog.misc.info(
              "Exception occurred while getting mailbox for id %s during ACL push", mboxId, e);
          continue;
        }
   ...
   */
  private static void deleteDbAclEntryForProcessedItems(
      Multimap<Integer, List<Integer>> currentItemIdsProcessed) throws ServiceException {
    if (currentItemIdsProcessed.size() != 0) {
      ...
      for (Entry<Integer, List<Integer>> entry : mailboxIds) {
        ...
        for (int itemId : itemIds) DbPendingAclPush.deleteEntry(mboxId, itemId);
      }
    }
  }
}
