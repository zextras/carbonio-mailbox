...
public class RdateExdate implements Cloneable {
  ...
  public RdateExdate(ICalTok propName, ICalTimeZone tz) throws ServiceException {
    this(propName, ICalTok.DATE_TIME, tz);
  }

  RdateExdate(ICalTok propName, ICalTok valueType, ICalTimeZone tz) throws ServiceException {
    if (ICalTok.RDATE.equals(propName)) mIsRdate = true;
  ...
  public void setValueType(ICalTok valueType) throws ServiceException {
    switch (valueType) {
      ...
      default:
        throw ServiceException.INVALID_REQUEST(
            "Invalid value type " + valueType.toString() + " in " + mPropertyName.toString(), null);
    }
  ...
  public String toString() {
    ...
    if (!ICalTok.DATE_TIME.equals(mValueType)) sb.append(";VALUE=").append(mValueType.toString());
    if (mTimeZone != null) sb.append(";TZID=").append(mTimeZone.getID());
    sb.append(":");
  ...
  private String getDatesCSV() {
    ...
    for (Object value : mValues) {
      ...
      } else if (value instanceof Period) sb.append(value.toString());
    }
    return sb.toString();
  }
  ...
  public Element toXml(Element parent) {
    ...
    if (mTimeZone != null) dateElem.addAttribute(MailConstants.A_CAL_TIMEZONE, mTimeZone.getID());
    for (Object val : mValues) {
      ...
      if (val instanceof ParsedDateTime) {
        ...
        Period p = (Period) val;
        Element startElem = dtvalElem.addElement(MailConstants.E_CAL_START_TIME);
        startElem.addAttribute(
            MailConstants.A_CAL_DATETIME, p.getStart().getDateTimePartString(false));
        if (p.hasEnd()) {
          Element endElem = dtvalElem.addElement(MailConstants.E_CAL_END_TIME);
          endElem.addAttribute(
              MailConstants.A_CAL_DATETIME, p.getEnd().getDateTimePartString(false));
        } else {
  ...
  public static RdateExdate parse(ZProperty prop, TimeZoneMap tzmap) throws ServiceException {
    ...
    if (valueParam != null) {
      ...
      if (typeStr != null) {
        ...
        if (valueType == null)
          throw ServiceException.INVALID_REQUEST(
              "Invalid " + propName.toString() + " value type " + typeStr, null);
      }
    }

    String tzid = prop.getParameterVal(ICalTok.TZID, null);
    ICalTimeZone tz = null;
    if (tzid != null) tz = tzmap.lookupAndAdd(tzid);
    ...
    if (csv == null || csv.length() == 0)
      throw ServiceException.INVALID_REQUEST(
          "Empty value not allowed for " + propName.toString() + " property", null);
    for (String value : csv.split(",")) {
      try {
        ...
        throw ServiceException.INVALID_REQUEST(
            "Unable to parse " + propName.toString() + " value \"" + value + "\"", e);
      }
    }
  ...
  public Metadata encodeMetadata() {
    ...
    if (mTimeZone != null) meta.put(FN_TZID, mTimeZone.getID());
  ...
  public static RdateExdate decodeMetadata(Metadata meta, TimeZoneMap tzmap)
    ...
    ICalTok valueType;
    if (vt.equals(VT_DATE_TIME)) valueType = ICalTok.DATE_TIME;
    else if (vt.equals(VT_DATE)) valueType = ICalTok.DATE;
    else valueType = ICalTok.PERIOD;
    ...
    for (int i = 0; i < numValues; i++) {
      ...
      if (valueType.equals(ICalTok.DATE_TIME) || valueType.equals(ICalTok.DATE)) {
        ...
        try {
          ...
          throw ServiceException.INVALID_REQUEST(
              "Invalid " + propName.toString() + " date/time in metadata: " + meta.toString(), e);
        }
        rexdate.addValue(dt);
  ...
  // bug 59333: Hack for Apple iCal.  It doesn't like comma-separate values in a single EXDATE
  // property.
  ...
  void addAsSeparateProperties(ZComponent comp) {
    ...
    ZParameter valType =
        !ICalTok.DATE_TIME.equals(mValueType)
            ? new ZParameter(ICalTok.VALUE, mValueType.toString())
            : null;
    for (Object value : mValues) {
