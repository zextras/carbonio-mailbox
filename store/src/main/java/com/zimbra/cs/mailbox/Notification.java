...
public class Notification implements LmtpCallback {
  ...
  public static final long DEFAULT_OUT_OF_OFFICE_CACHE_DURATION_MILLIS =
      7 * Constants.MILLIS_PER_DAY;

  /**
   * We have to check all of a user's addresses against all of the to,cc addresses in the message.
   * This is an M x N problem. We just check the first few addresses in to,cc against all of the
   * user's addresses to guard against messages with a large number of addresses in to,cc.
   */
  ...
  private void outOfOfficeIfNecessary(
      Account account, Mailbox mbox, ParsedMessage pm, String rcpt, String envSenderString)
      throws ServiceException, MessagingException {
    outOfOfficeIfNecessary(account, mbox, pm.getMimeMessage(), null, rcpt, envSenderString);
  }

  private void outOfOfficeIfNecessary(
      Account account, Mailbox mbox, Message msg, String rcpt, String envSenderString)
      throws ServiceException, MessagingException {
  ...
  /**
   * If the recipient's account requires out of office notification, send it out. We send these out
   * based on users' setting, and the incoming message meeting certain criteria.
   */
  private void outOfOfficeIfNecessary(
      Account account,
      Mailbox mbox,
    ...
    Date now = new Date();
    Date fromDate =
        account.getGeneralizedTimeAttr(Provisioning.A_zimbraPrefOutOfOfficeFromDate, null);
    if (fromDate != null && now.before(fromDate)) {
      ofailed("from date not reached", null, rcpt, msgId);
      return;
    }
    Date untilDate =
        account.getGeneralizedTimeAttr(Provisioning.A_zimbraPrefOutOfOfficeUntilDate, null);
    ...
    String[] otherAccountAddrs =
        account.getMultiAttr(Provisioning.A_zimbraPrefOutOfOfficeDirectAddress);
    if (!AccountUtil.isDirectRecipient(
        account, otherAccountAddrs, mm, OUT_OF_OFFICE_DIRECT_CHECK_NUM_RECIPIENTS)) {
    ...
    try {
      ...
      if (skipOutOfOfficeMsg(destination, account, mbox)) {
        ZimbraLog.mailbox.info(
            "%s is external user and no external reply option is set, so no OOO will be sent.",
            destination);
        return;
      }
      ...
      String body =
          account.getAttr(
              sendExternalReply
                  ...
                  : Provisioning.A_zimbraPrefOutOfOfficeReply,
              "");
      ...
      if (Provisioning.getInstance()
          .getConfig()
          .getBooleanAttr(Provisioning.A_zimbraAutoSubmittedNullReturnPath, true)) {
        out.setEnvelopeFrom("<>");
      } else {
      ...
      ZimbraLog.mailbox.info(
          "outofoffice sent dest='" + destination + "' rcpt='" + rcpt + "' mid=" + msgId);
   ...
   *
   * @return true - message should not be sent false - message should be sent
   */
  public static boolean skipOutOfOfficeMsg(String senderAddr, Account account, Mailbox mbox) {
    return account.isPrefOutOfOfficeSuppressExternalReply()
        && isOfExternalSenderType(senderAddr, account, mbox)
        && !isInternalSender(senderAddr, account)
        && !isOfSpecificDomainSenderType(senderAddr, account);
  }

  /**
   * standard Out of Office standard message should be sent or custom message
   *
   * @return true - custom message should be sent false - standard message should be sent
   */
  public static boolean sendOutOfOfficeExternalReply(
      String senderAddr, Account account, Mailbox mbox) {
    boolean sendExternalReply =
  ...
  private String getCharset(Account account, String data) {
    String requestedCharset =
        account.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, MimeConstants.P_CHARSET_UTF8);
    return CharsetUtil.checkCharset(data, requestedCharset);
  }

  /**
   * If the recipient's account is set up for email notification, sends a notification message to
  ...
  private void notifyIfNecessary(Account account, Message msg, String rcpt)
    ...
    boolean notify =
        account.getBooleanAttr(Provisioning.A_zimbraPrefNewMailNotificationEnabled, false);
    if (!notify) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        ZimbraLog.mailbox.warn(
            "error encoutered looking up return path configuration, using null return path instead",
            se);
      }
      ...
      MailSender.logMessage(
          out,
          out.getAllRecipients(),
          envFrom,
  ...
  private MimeMessage assembleNotificationMessage(
      Account account, Message msg, String rcpt, String destination, Session smtpSession)
  ...
  /**
   * If <tt>zimbraInterceptAddress</tt> is specified, sends a message to that address with the given
   * message attached.
   *
  ...
  void interceptIfNecessary(Mailbox mbox, MimeMessage msg, String operation, Folder folder)
    ...
    for (String interceptAddress : interceptAddresses) {
      try {
        ZimbraLog.mailbox.info(
            "Sending intercept of message %s to %s.", msg.getMessageID(), interceptAddress);
        ...
        String from =
            StringUtil.fillTemplate(account.getAttr(Provisioning.A_zimbraInterceptFrom), vars);
        String subject =
            StringUtil.fillTemplate(account.getAttr(Provisioning.A_zimbraInterceptSubject), vars);
        String bodyText =
            StringUtil.fillTemplate(account.getAttr(Provisioning.A_zimbraInterceptBody), vars);

        // Assemble outgoing message
        MimeMessage attached = msg;
        boolean headersOnly =
            account.getBooleanAttr(Provisioning.A_zimbraInterceptSendHeadersOnly, false);
        if (headersOnly) {
  ...
  /**
   * Returns <code>true</code> if the array is not <code>null</code> and contains the specified
   * value.
   */
  ...
  private static void failed(
      String op, String why, String destAddr, String rcptAddr, Message msg, Exception e) {
    failed(op, why, destAddr, rcptAddr, msg.getId(), e);
  }

  private static void failed(
      String op, String why, String destAddr, String rcptAddr, Integer msgId, Exception e) {
  ...
  private static void nfailed(
      String why, String destAddr, String rcptAddr, Message msg, Exception e) {
  ...
  private static void ofailed(
      String why, String destAddr, String rcptAddr, Integer msgId, Exception e) {
