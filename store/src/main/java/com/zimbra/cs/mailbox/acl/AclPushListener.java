...
/**
 * Listens to folder changes and schedules immediate run of {@link AclPushTask} in a separate thread
 * if required.
 */
    ...
    if (!runAclPushTask && notification.mods.modified != null) {
      for (Change change : notification.mods.modified.values()) {
        ...
        if (change.what instanceof Folder
            && (change.why & (Change.ACL | Change.NAME | Change.PARENT)) != 0) {
          runAclPushTask = true;
          break;
        }
      }
      ...
      for (Change change : notification.mods.deleted.values()) {
        if (change.preModifyObj == null) {
          if (change.what == MailItem.Type.FOLDER) {
            ...
            && ((Folder) change.preModifyObj).getACL() != null) {
          runAclPushTask = true;
          break;
        }
      }
