...
public class TimeZoneFixupRules {
  ...
  public static class Matcher {
    ...
    public Matcher(
      ...
      mStandardOnset = new SimpleOnset(stdWeek, stdDayOfWeek, stdMonth, 0, 2, 0, 0, true);
      mDaylightOffset = dstOffsetMins * 60000;
      mDaylightOnset = new SimpleOnset(dstWeek, dstDayOfWeek, dstMonth, 0, 2, 0, 0, true);
      mReplacementTZ = replacementTZ;
    }
    ...
    public Matcher(
      ...
      mStandardOffset = stdOffsetMins * 60000;
      // hour/minute/second don't matter.  Just use 02:00:00.
      mStandardOnset = new SimpleOnset(0, 0, stdMonth, stdDayOfMonth, 2, 0, 0, true);
      mDaylightOffset = dstOffsetMins * 60000;
      mDaylightOnset = new SimpleOnset(0, 0, dstMonth, dstDayOfMonth, 2, 0, 0, true);
      mReplacementTZ = replacementTZ;
    }
    ...
    private static boolean sameOnset(SimpleOnset os1, SimpleOnset os2) {
      ...
      if (os1.getMonth() != os2.getMonth()) return false;
    ...
    public boolean isTouchOnly() {
      return mReplacementTZ == null;
    }

    public ICalTimeZone getReplacementTZ() {
      return mReplacementTZ;
    }
    ...
    public boolean matches(ICalTimeZone tz) {
      switch (mMatchBy) {
        ...
        case RULES:
        case DATES:
          if (!tz.useDaylightTime()) return false;
          if (tz.getStandardOffset() != mStandardOffset
              || tz.getDaylightOffset() != mDaylightOffset) return false;
          if (!sameOnset(mStandardOnset, tz.getStandardOnset())
              || !sameOnset(mDaylightOnset, tz.getDaylightOnset())) return false;
          return true;
        default:
          return false;
      }
  ...
  /**
   * Replace the definition of a timezone if it matches any of the rules. Replacement timezone is
   * returned, or null if no rule matched. On match, an entry is added to the "replaced" map linking
   * old timezone ID to replacement timezone.
   *
   * @param oldTZ
