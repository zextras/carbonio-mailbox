...
/**
 * Captures differences between Before and After Invites for an Organizer calendar Builds on {@link
 * InviteChanges} but allows for Before or After invites being null and offers other utility methods
 * to aid CalDAV auto-scheduling
 */
public class OrganizerInviteChanges {
  ...
  public boolean isChanged() {
    ...
    if (!getAttendeesOnlyInNew().isEmpty()) {
      ZimbraLog.calendar.trace(
          "isChanged=true (new ATTENDEEs) for invite %s", newInvite.toString());
      return true;
    }
    if (!getAttendeesOnlyInOld().isEmpty()) {
      ZimbraLog.calendar.trace(
          "isChanged=true (old ATTENDEEs) for invite %s", newInvite.toString());
      return true;
    }
  ...
  /**
   * Just interested in the attendee names. Things like differing PARTSTAT are not taken into
   * account
   */
