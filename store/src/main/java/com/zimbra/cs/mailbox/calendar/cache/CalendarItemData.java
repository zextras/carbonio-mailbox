...
public class CalendarItemData {
  ...
  private int
      mModMetadata; // mod_metadata db column; this serves the function of last-modified-time
  ...
  public long getActualRangeStart() {
    return mActualRangeStart;
  }

  public long getActualRangeEnd() {
    return mActualRangeEnd;
  }
  ...
  public CalendarItemData getSubRange(long rangeStart, long rangeEnd) {
    if (rangeStart <= mActualRangeStart && rangeEnd >= mActualRangeEnd) return this;
    ...
    for (InstanceData inst : mInstances) {
      ...
      if (rangeStart <= alarmAt && alarmAt < rangeEnd) {
        ...
        if ((instStart == 0 && instEnd == 0) || (instStart < rangeEnd && instEnd > rangeStart)) {
          calItemData.addInstance(inst);
        }
      } else {
    ...
    if (calItemData.getNumInstances() > 0) return calItemData;
    else return null;
  }
  ...
  CalendarItemData(Metadata meta) throws ServiceException {
    ...
    if (metaAlarm != null) mAlarm = new AlarmData(metaAlarm);
    Metadata metaDefInst = meta.getMap(FN_DEFAULT_INST, true);
    if (metaDefInst != null) mDefaultData = new FullInstanceData(metaDefInst);
    ...
    if (numInst > 0) {
      ...
      for (int i = 0; i < numInst; i++) {
        ...
        if (metaInst != null) {
          ...
          if (FullInstanceData.isFullInstanceMeta(metaInst)) inst = new FullInstanceData(metaInst);
          else inst = new InstanceData(metaInst);
          instances.add(inst);
        }
      }
  ...
  Metadata encodeMetadata() {
    ...
    if (mAlarm != null) meta.put(FN_ALARM, mAlarm.encodeMetadata());
    if (mDefaultData != null) meta.put(FN_DEFAULT_INST, mDefaultData.encodeMetadata());
    if (mInstances != null) {
