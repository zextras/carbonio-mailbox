...
public class Tag extends MailItem implements ZimbraTag {
  public static class NormalizedTags {
    ...
    NormalizedTags(Mailbox mbox, String[] tagsFromClient, boolean create, boolean imapVisible)
        throws ServiceException {
      assert mbox.isTransactionActive()
          : "cannot instantiate NormalizedTags outside of a transaction";
      ...
      if (ArrayUtil.isEmpty(tagsFromClient)) {
        ...
        for (String tag : tagsFromClient) {
          ...
          if (create) {
            try {
              Tag newTag =
                  mbox.createTagInternal(
                      Mailbox.ID_AUTO_INCREMENT, tag, new Color(DEFAULT_COLOR), false);
              if (imapVisible) {
  ...
  // This is used for a pseudo non-existent tag.  Normally one defined for a mailbox of a shared
  // folder, but
  ...
  // If this is true, an a newly-created unlisted tag will still be returned as a pending remote
  // modification
  ...
  /**
   * Updates the number of items with the tag and their total size. <i>(Total size not currently
   * tracked.)</i>
   *
  ...
  static Tag create(Mailbox mbox, int id, String requestedName, Color color, boolean listed)
      throws ServiceException {
  ...
  /**
   * Create a minimal Tag object but don't persist any equivalent of it to the mail store. This is
   * useful when searching for remote tags (e.g. By clicking on a remote tag on an item in a shared
   * folder - see Bug 77646)
   */
  public static Tag createPseudoRemoteTag(Mailbox mbox, String requestedName)
    ...
    data.setSubject(name);
    data.metadata = null;
    // Need to keep this out of the cache so that the user is still able to add a local tag with the
    // same name after
  ...
  /**
   * Updates the unread state of all items with the tag. Persists the change to the database and
   * cache, and also updates the unread counts for the tag and the affected items' parents and
   * {@link Folder}s appropriately. <i>Note: Tags may only be marked read, not unread.</i>
   *
  ...
  static String validateItemName(String name) throws ServiceException {
    ...
    if (name == null
        ...
        || INVALID_TAG_CHARS.matchesAnyOf(name)) {
    ...
    if (trimmed.isEmpty()
        ...
        || trimmed.startsWith(FLAG_NAME_PREFIX)) {
      throw MailServiceException.INVALID_NAME(name);
    }
    return trimmed;
  }
  ...
  /**
   * Updates the change highwater mark for all non-empty folders. Renaming or deleting a tag may or
   * may not touch items in any or all folders. Since we can't easily tell which folders would be
   * affected, we just update the highwater mark for *all* folders.
   */
  ...
  protected void saveTagCounts() throws ServiceException {
    ...
    ZimbraLog.mailbox.debug(
        "\"%s\": updating tag counts (s%d/u%d)", getName(), (int) mData.size, mData.unreadCount);
  }
  ...
  public static String encodeMetadata(
      Color color, int metaVersion, int version, RetentionPolicy rp, boolean listed) {
