...
public class Mountpoint extends Folder implements MountpointStore {
  ...
  /**
   * Returns the <code>zimbraId</code> of the remote shared item's mailbox's owner.
   *
  ...
  /**
   * Returns the numeric item id of the remote shared item pointed to by this <code>Mountpoint
   * </code>.
   *
   * @see MailItem#getId()
   */
  ...
  /** Returns the UUID of the remote shared item pointed to by this <code>Mountpoint</code>. */
  public String getRemoteUuid() {
    return mRemoteUuid;
  }

  /**
   * Returns the {@link ItemId} of the remote shared item referenced by this <code>Mountpoint</code>
   * .
   *
  ...
  /**
   * Grants the specified set of rights to the target and persists them to the database. <i>Note:
   * This function will always throw the <code>service.PERM_DENIED</code> {@link ServiceException}
   * because <code>Mountpoint</code>s may not be re-shared.</i>
   */
   ...
   * accordingly. <i>Note: This function does nothing, as you cannot re-share a <code>Mountpoint
   * </code> in the first place.</i>
   *
  ...
  /**
   * Creates a new Mountpoint pointing at a remote item and persists it to the database. A real
   ...
   *     <ul>
   *       <li><code>mail.CANNOT_CONTAIN</code> - if the target folder can't contain mountpoints
   ...
   *       <li><code>mail.INVALID_REQUEST</code> - if the <code>ownerId</code> or <code>remoteId
   *           </code> are invalid
  ...
  static Mountpoint create(
    ...
    data.metadata =
        encodeMetadata(color, 1, 1, custom, view, ownerId, remoteId, remoteUuid, reminderEnabled);
    data.contentChanged(mbox);
    ZimbraLog.mailop.info(
        "Adding Mountpoint %s: id=%d, parentId=%d, parentName=%s.",
  ...
  static Metadata encodeMetadata(
    ...
    if (reminderEnabled) meta.put(Metadata.FN_REMINDER_ENABLED, reminderEnabled);
    return Folder.encodeMetadata(
        meta,
        color,
        metaVersion,
        version,
