...
public class Util {
   ...
   *
   * <p>start time = midnight UTC of current month + N months end time = midnight UTC of current
   * month + (N + M) months
   *
   * <p>where N = monthFrom (can be negative) and M = numMonths.
   *
   * @param now
   * @param monthFrom
   * @param numMonths
  ...
  static Pair<Long, Long> getMonthsRange(long now, int monthFrom, int numMonths) {
    ...
    calEnd.add(
        Calendar.DAY_OF_YEAR, -1); // beginning (midnight) of last day of the last month in range
  ...
  /**
   * Returns the TZ offset in millis for the given instance, using the timezone in inv. Returns null
   * if inv is not an all-day appointment/task.
   *
   * @param inv
   * @param instanceStart
   * @return
  ...
  static Long getTZOffsetForInvite(Invite inv, long instanceStart) {
    if (inv.isAllDayEvent()) {
      ...
      if (dtStart != null) {
        ...
        if (tz != null) return Long.valueOf(tz.getOffset(instanceStart));
      }
    }
    return null;
  }
