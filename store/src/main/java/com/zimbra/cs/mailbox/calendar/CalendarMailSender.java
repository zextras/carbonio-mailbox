...
public class CalendarMailSender {
  ...
  public static final Verb VERB_ACCEPT = new Verb("ACCEPT", IcalXmlStrMap.PARTSTAT_ACCEPTED);
  public static final Verb VERB_DECLINE = new Verb("DECLINE", IcalXmlStrMap.PARTSTAT_DECLINED);
  public static final Verb VERB_TENTATIVE = new Verb("TENTATIVE", IcalXmlStrMap.PARTSTAT_TENTATIVE);
  ...
  private static final Map<Verb, MsgKey> sVerbMsgKeys;
  ...
  public static String getCancelSubject(String subject, Locale lc) {
    ...
    if (subject != null && subject.startsWith(prefix)) return subject;
    else return prefix + subject;
  }

  public static MimeMessage createOrganizerChangeMessage(
      Account fromAccount,
      Account authAccount,
      boolean asAdmin,
    ...
    return createCalendarMessage(
        authAccount, from, sender, rcpts, subject, sb.toString(), null, inv.getUid(), iCal);
  }

  public static MimeMessage createDefaultReply(
      Account fromAccount,
  ...
  private static MimeMessage createDefaultReply(
      Account fromAccount,
      String fromIdentityId,
      Account authAccount,
      String authIdentityId,
    ...
    if (fromIdentityId != null) {
      ...
      if (fromIdentity == null) {
        ZimbraLog.calendar.warn(
            "No such identity " + fromIdentityId + " for account " + fromAccount.getName());
    ...
    if (authIdentityId != null) {
      ...
      if (authIdentity == null) {
        ZimbraLog.calendar.warn(
            "No such identity " + authIdentityId + " for account " + authAccount.getName());
        if (authAccount.equals(fromAccount)) authIdentity = fromIdentity;
        else authIdentity = getTargetedIdentity(authAccount, inv);
      }
    } else {
      if (authAccount.equals(fromAccount)) authIdentity = fromIdentity;
      else authIdentity = getTargetedIdentity(authAccount, inv);
    ...
    if (inv.hasOrganizer()) {
      ...
      Account organizer =
          Provisioning.getInstance().get(AccountBy.name, organizerAddress.getAddress());
    ...
    if (VERB_ACCEPT.equals(verb)) {
      if (isResourceAccount) {
        if (partialAccept) statusMsgKey = MsgKey.calendarResourceDefaultReplyPartiallyAccept;
        else statusMsgKey = MsgKey.calendarResourceDefaultReplyAccept;
      } else {
        statusMsgKey = MsgKey.calendarDefaultReplyAccept;
      }
    } else if (VERB_DECLINE.equals(verb)) {
      if (isResourceAccount) {
        if (partialAccept) statusMsgKey = MsgKey.calendarResourceDefaultReplyPartiallyDecline;
        else statusMsgKey = MsgKey.calendarResourceDefaultReplyDecline;
      } else {
        statusMsgKey = MsgKey.calendarDefaultReplyDecline;
      }
    } else if (VERB_TENTATIVE.equals(verb)) {
      if (isResourceAccount) statusMsgKey = MsgKey.calendarResourceDefaultReplyTentativelyAccept;
      else statusMsgKey = MsgKey.calendarDefaultReplyTentativelyAccept;
    } else {
    ...
    else statusMsg = L10nUtil.getMessage(statusMsgKey, lc, fromDisplayName, verb.toString());
    ...
    if (addSignature) {
      ...
      else sigKey = Provisioning.A_zimbraPrefCalendarAutoAcceptSignatureId;
    ...
    boolean allowPrivateAccess =
        calItem == null || calItem.allowPrivateAccess(authAccount, asAdmin);
    ...
    return createCalendarMessage(
        authAccount,
        fromIdentity.getFriendlyEmailAddress(),
        senderAddr,
        toList,
        replySubject,
        replyText.toString(),
  ...
  private static void attachInviteSummary(
      StringBuilder sb, Invite inv, MimeMessage mmInv, Locale lc) throws ServiceException {
    ...
    if (notes != null && notes.length() > 0) {
      ...
      sb.append("\r\n-----");
      sb.append(L10nUtil.getMessage(MsgKey.calendarResourceReplyOriginalInviteSeparatorLabel, lc));
      sb.append("-----\r\n\r\n");
      sb.append(notes);
      sb.append("\r\n\r\n");
    }
  ...
  public static String formatDateTime(Date d, TimeZone tz, Locale lc) {
    String dateTimeFmt = L10nUtil.getMessage(MsgKey.calendarResourceConflictDateTimeFormat, lc);
  ...
  public static String formatTime(Date t, TimeZone tz, Locale lc) {
    String timeOnlyFmt = L10nUtil.getMessage(MsgKey.calendarResourceConflictTimeOnlyFormat, lc);
  ...
  public static String formatDate(Date t, TimeZone tz, Locale lc) {
    String dateOnlyFmt = L10nUtil.getMessage(MsgKey.calendarResourceConflictDateOnlyFormat, lc);
  ...
  /**
   * Builds the TO: list for calendar item updates by iterating over the list of ATTENDEEs
   *
   * @param iter
   * @return
   */
  ...
  public static MimeMessage createCancelMessage(
      Account fromAccount,
    ...
    boolean hidePrivate =
        !calItem.isPublic() && !calItem.allowPrivateAccess(senderAccount, asAdmin);
    String invSubject;
    if (hidePrivate) invSubject = L10nUtil.getMessage(MsgKey.calendarSubjectWithheld, locale);
    else invSubject = inv != null ? inv.getName() : "";
    ...
    if (!hidePrivate) {
      MimeMessage mmInv = inv.getMimeMessage();
      if (mmInv == null && defaultInv != null) mmInv = defaultInv.getMimeMessage();
      if (mmInv != null) attachInviteSummary(sb, inv, mmInv, locale);
    }

    Address from = AccountUtil.getFriendlyEmailAddress(fromAccount);
    Address sender = null;
    if (onBehalfOf) sender = AccountUtil.getFriendlyEmailAddress(senderAccount);
    ...
    return createCalendarMessage(
        ...
        sb.toString(),
        null,
        defaultInv != null ? defaultInv.getUid() : "unknown",
        iCal);
  }
  ...
  public static MimeMessage createCalendarMessage(
      ...
      throws ServiceException {
    return createCalendarMessage(
        account, fromAddr, senderAddr, toAddrs, subject, desc, descHtml, uid, cal, true);
  }
  ...
  private static class CalendarPartReplacingVisitor extends MimeVisitor {
    ...
    protected boolean visitBodyPart(MimeBodyPart bp) throws MessagingException {
      ...
      if (mCalendarPart == null && isCalendarPart(bp)) mCalendarPart = bp;
      return false;
    }
  ...
  public static MimeMessage createCalendarMessage(
    ...
    try {
      ...
      if (srcMm != null) {
        ...
        if (senderAddr != null) {
          mm.setSender(senderAddr);
          if (replyToSender) mm.setReplyTo(new Address[] {senderAddr});
        }
        ...
        String descHtml = inv.getDescriptionHtml();
        return createCalendarMessage(
            account, fromAddr, senderAddr, toAddrs, subject, desc, descHtml, uid, cal, false);
      }
    } catch (MessagingException e) {
      throw ServiceException.FAILURE(
  ...
  private static MimeMessage createCalendarInviteDeniedMessage(
    ...
    String sigText =
        getSignatureText(
            fromAccount, fromIdentity, Provisioning.A_zimbraPrefCalendarAutoDenySignatureId);
    if (sigText == null || sigText.length() < 1) sigText = L10nUtil.getMessage(bodyTextKey, locale);
    if (sigText != null && sigText.length() > 0) replyText.append(sigText).append("\r\n");
    attachInviteSummary(replyText, inv, null, locale);

    String subject =
        L10nUtil.getMessage(MsgKey.calendarReplySubjectDecline, locale) + ": " + inv.getName();
    String uid = inv.getUid();
    ParsedDateTime exceptDt = null;
    if (inv.hasRecurId()) exceptDt = inv.getRecurId().getDt();
    Invite replyInv =
        replyToInvite(
            fromAccount,
            senderAccount,
  ...
  public static boolean allowInviteAutoDeclinedNotification(
    ...
    if (!applyToCalendar) {
      ZimbraLog.calendar.info(
          "Suppressed Invite Auto Decline to %s - no auto-reply when processed as message-only",
          senderEmail);
      return false;
    }
    ...
    switch (validTargetSetting) {
      ...
      case internal:
        if (senderAccount == null) {
          ZimbraLog.calendar.info(
              "Suppressed Invite Auto Decline to=%s - not internal", senderEmail);
          return false;
        }
        return true;
      case sameDomain:
      default:
        ...
        // if the senderEmail has the same domain but we weren't able to create a senderAccount for
        // it
        // then still don't allow it - assume it is an invalid address.
        if (senderAccount == null) {
          ZimbraLog.calendar.info(
              "Suppressed Invite Auto Decline to=%s - no account for sender so not in attendee"
                  + " domain=%s",
        ...
        ZimbraLog.calendar.info(
            "Suppressed Invite Auto Decline to=%s - not in attendee domain=%s",
            senderEmail, declinerAcct.getDomainName());
        return false;
    }
  }

  public static void handleInviteAutoDeclinedNotification(
      ...
      Invite invite)
      throws ServiceException {
    if (allowInviteAutoDeclinedNotification(
        mbox,
        fromAccount,
        senderEmail,
        senderAccount,
        applyToCalendar,
        invite.getMatchingAttendee(fromAccount))) {
  ...
  public static void sendInviteDeniedMessage(
    ...
    else bodyTextKey = MsgKey.calendarUserReplyPermissionDenied;
    final MimeMessage mm =
        createCalendarInviteDeniedMessage(
  ...
  public static Invite replyToInvite(
      Account acct,
      Account authAcct,
      boolean onBehalfOf,
      boolean allowPrivateAccess,
      ...
      throws ServiceException {
    return replyToInvite(
        acct, null, authAcct, onBehalfOf, allowPrivateAccess, oldInv, verb, replySubject, exceptDt);
  }

  /**
   * RFC2446 4.2.2:
   *
   * <p>BEGIN:VCALENDAR PRODID:-//ACME/DesktopCalendar//EN METHOD:REPLY VERSION:2.0 BEGIN:VEVENT
   * ATTENDEE;PARTSTAT=ACCEPTED:Mailto:B@example.com ORGANIZER:MAILTO:A@example.com
   * UID:calsrv.example.com-873970198738777@example.com SEQUENCE:0 REQUEST-STATUS:2.0;Success
   * DTSTAMP:19970612T190000Z END:VEVENT END:VCALENDAR
   *
  ...
  public static Invite replyToInvite(
    ...
    Invite reply =
        new Invite(
            oldInv.getItemType(),
            ICalTok.REPLY.toString(),
            new TimeZoneMap(Util.getAccountTimeZone(onBehalfOf ? authAcct : acct)),
            oldInv.isOrganizer());
    ...
    if (me != null) {
      ...
      if (identityAddr.equalsIgnoreCase(atAddr)) {
        ...
        if (identityCn != null) meReply.setCn(identityCn);
      } else {
        meReply = new ZAttendee(atAddr);
        if (me.hasCn()) meReply.setCn(me.getCn());
      }
      meReply.setPartStat(verb.getXmlPartStat());
      if (me.hasRole()) meReply.setRole(me.getRole());
      if (me.hasCUType()) meReply.setCUType(me.getCUType());
    } else {
      meReply = new ZAttendee(identityAddr);
      meReply.setPartStat(verb.getXmlPartStat());
      if (identityCn != null) meReply.setCn(identityCn);
    }
    if (onBehalfOf) meReply.setSentBy(authAcct.getName());
    ...
    if (!hidePrivate) reply.setLocation(oldInv.getLocation());
    else reply.setLocation("");

    // ORGANIZER
    if (oldInv.hasOrganizer()) reply.setOrganizer(oldInv.getOrganizer());
    ...
    if (exceptDt != null) {
      ...
      reply.setRecurrence(
          (IRecurrence) oldInv.getRecurrence().clone()); // Must use a cloned object!
    }
  ...
  public static MimeMessage createResourceAutoReply(
      OperationContext octxt,
      String fromIdentityId,
      String authIdentityId,
      Mailbox mbox,
    ...
    if (octxt != null) {
      ...
      if (authuser != null) {
        ...
        if (onBehalfOf) authAcct = authuser;
      }
    }
    Locale lc;
    Account organizer = inv.getOrganizerAccount();
    if (organizer != null) lc = organizer.getLocale();
    else lc = authAcct.getLocale();
    ...
    String subject;
    if (hidePrivate) subject = L10nUtil.getMessage(MsgKey.calendarSubjectWithheld, lc);
    else subject = inv.getName();
  ...
  public static MimeMessage createForwardNotifyMessage(
      Account senderAcct, Account toAcct, String to, Address[] rcpts, Invite inv)
      throws MessagingException, ServiceException {
    MimeMessage mm = new Mime.FixedMimeMessage(JMSession.getSession());
    Locale lc = toAcct.getLocale();
    mm.setSubject(
        L10nUtil.getMessage(MsgKey.calendarForwardNotificationSubject, lc, inv.getName()),
    ...
    String time =
        FriendlyCalendaringDescription.getTimeDisplayString(
            inv.getStartTime(),
            inv.getEndTime(),
            inv.isRecurrence(),
            inv.isAllDayEvent(),
    ...
    if (inv.isRecurrence()) {
      ...
      time +=
          " ("
              + FriendlyCalendaringDescription.getRecurrenceDisplayString(
                  zr, inv.getStartTime().getCalendarCopy(), lc)
              + ")";
    }
    String text =
        L10nUtil.getMessage(
            MsgKey.calendarForwardNotificationBody, lc, sender, inv.getName(), time, recipients);
    MimeBodyPart textPart = new ZMimeBodyPart();
    ...
    String html =
        L10nUtil.getMessage(
            MsgKey.calendarForwardNotificationBodyHtml,
            lc,
            sender,
            inv.getName(),
            time,
  ...
  public static void sendResourceAutoReply(
      final OperationContext octxt,
      final Mailbox mbox,
  ...
  private static String getSignatureText(Account acct, Identity identity, String signatureKey)
      throws ServiceException {
    String sigId = identity.getAttr(signatureKey);
    if (sigId == null) return null;
