...
public class FullInstanceData extends InstanceData {
  ...
  public boolean isPublic(FullInstanceData defaultInstance) {
    if (mClassProp != null) return IcalXmlStrMap.CLASS_PUBLIC.equals(mClassProp);
    if (defaultInstance != null) return defaultInstance.isPublic(null);
    return true;
  }
  ...
  public FullInstanceData(
      ...
      String priority,
      String classProp,
      String freeBusyIntended,
      String transparency,
      List<String> categories,
      Geo geo) {
    super(
        recurIdZ, dtStart, duration, alarmAt, tzOffset, partStat, freeBusyActual, percentComplete);
    init(
        invId,
        compNum,
        recurrenceId,
  ...
  private void init(
    ...
    mDraft = draft;
    mNeverSent = neverSent;
    mSummary = summary;
    mLocation = location;
    mFragment = fragment;
    mDescInMeta = descInMeta;
    mDesc = desc;
    mDescHtml = descHtml;
    mIsAllDay = isAllDay;
    mStatus = status;
    mPriority = priority;
    mClassProp = classProp;
    mFreeBusyIntended = freeBusyIntended;
  ...
  public FullInstanceData(
    ...
    if (inv.hasOtherAttendees()) attendees = inv.getAttendees();
  ...
  FullInstanceData(Metadata meta) throws ServiceException {
    ...
    if (metaOrg != null) org = new ZOrganizer(metaOrg);
    Boolean isOrg = null;
    if (meta.containsKey(FN_IS_ORGANIZER)) isOrg = new Boolean(meta.getBool(FN_IS_ORGANIZER));
    ...
    if (meta.containsKey(FN_NUM_ATTENDEES)) {
      ...
      if (num > 0) {
        ...
        for (int i = 0; i < num; i++) {
          ...
          if (metaAt != null) attendees.add(new ZAttendee(metaAt));
        }
      }
    }

    Boolean hasAlarm = null;
    if (meta.containsKey(FN_HAS_ALARM)) hasAlarm = new Boolean(meta.getBool(FN_HAS_ALARM));
    Boolean hasAttachment = null;
    if (meta.containsKey(FN_HAS_ATTACHMENT))
      hasAttachment = new Boolean(meta.getBool(FN_HAS_ATTACHMENT));
    Boolean draft = null;
    if (meta.containsKey(FN_DRAFT)) draft = new Boolean(meta.getBool(FN_DRAFT));
    Boolean neverSent = null;
    if (meta.containsKey(FN_NEVER_SENT)) neverSent = new Boolean(meta.getBool(FN_NEVER_SENT));
    ...
    if (meta.containsKey(FN_DESC_IN_META)) descInMeta = new Boolean(meta.getBool(FN_DESC_IN_META));
    String desc = meta.get(FN_DESC, null);
    String descHtml = meta.get(FN_DESC_HTML, null);

    Boolean isAllDay = null;
    if (meta.containsKey(FN_IS_ALLDAY)) isAllDay = new Boolean(meta.getBool(FN_IS_ALLDAY));
    ...
    if (metaGeo != null) geo = com.zimbra.cs.mailbox.calendar.Util.decodeGeoFromMetadata(metaGeo);
  ...
  Metadata encodeMetadata() {
    ...
    if (mOrganizer != null) meta.put(FN_ORGANIZER, mOrganizer.encodeMetadata());
    if (mIsOrganizer != null) meta.put(FN_IS_ORGANIZER, mIsOrganizer.booleanValue());
    if (mAttendees != null) {
    ...
    if (mHasAlarm != null) meta.put(FN_HAS_ALARM, mHasAlarm.booleanValue());
    if (mHasAttachment != null) meta.put(FN_HAS_ATTACHMENT, mHasAttachment.booleanValue());
    if (mDraft != null) meta.put(FN_DRAFT, mDraft.booleanValue());
    if (mNeverSent != null) meta.put(FN_NEVER_SENT, mNeverSent.booleanValue());

    meta.put(FN_SUMMARY, mSummary);
    meta.put(FN_LOCATION, mLocation);
    meta.put(FN_FRAGMENT, mFragment);
    if (mDescInMeta != null) meta.put(FN_DESC_IN_META, mDescInMeta.booleanValue());
    meta.put(FN_DESC, mDesc);
    meta.put(FN_DESC_HTML, mDescHtml);

    if (mIsAllDay != null) meta.put(FN_IS_ALLDAY, mIsAllDay.booleanValue());
