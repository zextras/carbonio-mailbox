...
/**
 * Listens to ACL changes on folder and document items and schedules an {@link ExpireGrantsTask} for
 * the item if required.
 */
public class ShareExpirationListener extends MailboxListener {

  private static final ImmutableSet<MailItem.Type> registeredTypes =
      ImmutableSet.of(MailItem.Type.FOLDER, MailItem.Type.DOCUMENT);
  ...
  static void scheduleExpireAccessOpIfReq(MailItem item) {
    ...
    try {
      ...
      ZimbraLog.scheduler.warn(
          "Error in canceling existing ExpireGrantsTask for (id=%s,mailboxId=%s)",
