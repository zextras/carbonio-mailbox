...
/**
 * Class FriendlyCalendaringDescription represents the text used in text/plain and text/html
 * alternative descriptions of a calendaring message
 *
 * @author gren
 */
public class FriendlyCalendaringDescription {
  ...
  public FriendlyCalendaringDescription(List<Invite> inviteComponents, Account account)
  ...
  private void generateDescriptions() {
    if (mDoneGenerate) return;
    mDoneGenerate = true;
    if ((mInviteComponents == null) || (mInviteComponents.size() == 0)) return;
    ...
    if (invite.isEvent()) addEventDetails(invite);
  ...
  private void addEventDetails(Invite invite) {
    ...
    try {
      ...
      if (method.equals("REQUEST")) {
        if (invite.getRecurId() != null) hdrKey = L10nUtil.MsgKey.zsApptInstanceModified;
        else if (mDefinitelyModified) hdrKey = L10nUtil.MsgKey.zsApptModified;
        else hdrKey = L10nUtil.MsgKey.zsApptNew;
      } else if (method.equals("CANCEL")) {
        if (invite.getRecurId() != null) hdrKey = L10nUtil.MsgKey.calendarCancelAppointmentInstance;
        else hdrKey = L10nUtil.MsgKey.calendarCancelAppointment;
        ...
        if ((attendees != null) && !attendees.isEmpty()) {
          ...
          if (hdrKey != null) {
            ...
            mHtml
                .append("<h3>")
                .append(L10nUtil.getMessage(hdrKey, mLc, replierName))
                .append("</h3>");
            hdrKey = null;
          }
        }
      ...
      if (invite.hasOrganizer())
        addSimpleRow(
            L10nUtil.MsgKey.zsOrganizer, invite.getOrganizer().getFriendlyAddress().toString());
      mHtml.append("</table>\n<p>\n<table border=\"0\">\n");
      ...
      if (startTime != null) {
        ...
        addSimpleRow(
            L10nUtil.MsgKey.zsTime,
            getTimeDisplayString(
                startTime, endTime, invite.isRecurrence(), invite.isAllDayEvent()));
        ZRecur zr = getRecur(invite);
        if (zr != null) {
          addSimpleRow(
              L10nUtil.MsgKey.zsRecurrence,
  ...
  /**
   * If <code>value</code> has a non-zero length value, append a line of the form: <code>msgKey
   * </code>:<code>value</code> to <code>mPlainText</code> and <code>mHtml</code>
   */
  ...
  public String getTimeDisplayString(
      ParsedDateTime start, ParsedDateTime end, boolean isRecurrence, boolean isAllDayEvent)
      throws ServiceException {
    return getTimeDisplayString(start, end, isRecurrence, isAllDayEvent, mLc, mAccount);
  }

  public static String getTimeDisplayString(
    ...
    TimeZone timeZone;
    if (start.getTimeZone() != null) timeZone = start.getTimeZone();
    else timeZone = Util.getAccountTimeZone(account);
    if (!isRecurrence) {
  ...
  public static String getRecurrenceDisplayString(ZRecur zr, Calendar dtStart, Locale lc) {
    ...
    if (zr.getFrequency().equals(ZRecur.Frequency.DAILY)) {
      ...
      if (dayList != null && !dayList.isEmpty()) { // we assume this is every weekday
        ...
        repeat =
            L10nUtil.getMessage(L10nUtil.MsgKey.zsRecurDailyEveryNumDays, lc, zr.getInterval());
      } else {
      ...
      if (zr.getInterval() == 1) {
        ...
        repeat =
            L10nUtil.getMessage(
                L10nUtil.MsgKey.zsRecurWeeklyEveryNumWeeksDate,
                lc,
                zr.getInterval(),
                cal.getTime());
      }
      ...
      if (monthdayList != null && !monthdayList.isEmpty()) {
        repeat =
            L10nUtil.getMessage(
                L10nUtil.MsgKey.zsRecurMonthlyEveryNumMonthsDate,
                lc,
                monthdayList.get(0),
                zr.getInterval());
      } else {
        ...
        if (weekdayList != null && !weekdayList.isEmpty()) {
          ...
          repeat =
              L10nUtil.getMessage(
                  L10nUtil.MsgKey.zsRecurMonthlyEveryNumMonthsNumDay,
                  lc,
                  weekOfMonth(zr),
                  cal.getTime(),
                  zr.getInterval());
      ...
      if (monthdayList != null && !monthdayList.isEmpty()) {
        repeat =
            L10nUtil.getMessage(
                L10nUtil.MsgKey.zsRecurYearlyEveryDate, lc, cal.getTime(), monthdayList.get(0));
        ...
        if (weekdayList != null && !weekdayList.isEmpty()) {
          ...
          repeat =
              L10nUtil.getMessage(
                  L10nUtil.MsgKey.zsRecurYearlyEveryMonthNumDay,
                  lc,
                  weekOfMonth(zr),
                  cal.getTime(),
                  cal.getTime());
          ...
          repeat =
              L10nUtil.getMessage(
                  L10nUtil.MsgKey.zsRecurYearlyEveryDate,
                  lc,
                  cal.getTime(),
                  dtStart.get(Calendar.DAY_OF_MONTH));
        }
  ...
  public static ZRecur getRecur(Invite invite) {
    ...
    if (r != null) {
      ...
      for (Iterator<IRecurrence> iter = masterRule.addRulesIterator();
