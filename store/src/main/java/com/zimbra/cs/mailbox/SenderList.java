...
public class SenderList {
  ...
  public SenderList(List<Message> msgs) {
    ...
    for (ListIterator<Message> lit = msgs.listIterator(mSize); lit.hasPrevious() && !mIsElided; ) {
      ...
      if (sender == null || sender.trim().equals("")) continue;
      ParsedAddress pa = new ParsedAddress(sender).parse();
      if (mParticipants != null && mParticipants.contains(pa)) continue;
      else if (mParticipants == null)
        (mParticipants = new ArrayList<ParsedAddress>(MAX_PARTICIPANT_COUNT)).add(pa);
      else if (mParticipants.size() >= MAX_PARTICIPANT_COUNT) mIsElided = true;
      else mParticipants.add(pa);
    }
  }
  ...
  public List<ParsedAddress> getLastAddresses() {
    if (mParticipants == null || mParticipants.isEmpty()) return Collections.emptyList();
    List<ParsedAddress> addrs = new ArrayList<ParsedAddress>(mParticipants);
    if (addrs.size() > 1) Collections.reverse(addrs);
    return addrs;
  }
  ...
  public static SenderList parse(Metadata meta) throws ServiceException, RefreshException {
    if (meta.getMap(Metadata.FN_FIRST, true) != null) return parseDeprecated(meta);
  ...
  private static SenderList parseDeprecated(Metadata meta)
      throws ServiceException, RefreshException {
    ...
    if (entries != null && entries.size() != size - 1)
      throw new RefreshException(
          "switching from old SenderList to new SenderList requires refresh");
