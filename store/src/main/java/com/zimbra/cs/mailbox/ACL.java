...
public final class ACL {
  ...
  /**
   * Bitmask of all rights that can be explicitly granted. <i>Note: CAN_CREATE_FOLDER is calculated
   * and hence cannot be granted.
   */
  private static final short GRANTABLE_RIGHTS =
      RIGHT_READ
          | RIGHT_WRITE
          | RIGHT_INSERT
          | RIGHT_DELETE
  ...
  public static final short ROLE_MANAGER =
      ACL.RIGHT_READ | ACL.RIGHT_WRITE | ACL.RIGHT_INSERT | ACL.RIGHT_DELETE | ACL.RIGHT_ACTION;
  ...
  public static class Grant implements ACLGrant {
    ...
    /** The type of object the grantee's ID refers to. For instance, {@link ACL#GRANTEE_USER}. */
    private final byte mType;
    /**
     * A bitmask of the rights being granted. For instance, <tt>{@link ACL#RIGHT_INSERT} | {@link
     * ACL#RIGHT_READ}</tt>.
     */
    private short mRights;
    /**
     * The password for guest accounts, or hex ascii string version of the accesskey for "key"
     * grantees.
     */
    private String mSecret;
    /**
     * Time when this grant expires. Value of 0 indicates that expiry is derived from ACL, except
     * when {@link #mType} is {@link ACL.GRANTEE_PUBLIC}.
     */
    private long mExpiry = 0;

    /**
     * Creates a new Grant object granting access to a user or class of users. <tt>zimbraId</tt> may
     * be <tt>null</tt> if the <tt>type</tt> is {@link ACL#GRANTEE_PUBLIC}.
     *
    ...
    Grant(String zimbraId, byte type, short rights, String secret, long expiry) {
      ...
      if (mType == GRANTEE_GUEST || mType == GRANTEE_KEY) mSecret = secret;
      mExpiry = expiry;
    }
    ...
    public Grant(Metadata meta) throws ServiceException {
      ...
      if (hasGrantee()) mGrantee = meta.get(FN_GRANTEE);
      if (mType == ACL.GRANTEE_GUEST) mSecret = meta.get(FN_PASSWORD, null);
      else if (mType == ACL.GRANTEE_KEY) mSecret = meta.get(FN_ACCESSKEY);
    ...
    /**
     * Returns the rights granted to the given {@link Account} by this <tt>Grant</tt>. If the grant
     * does not apply to the Account, returns <tt>0</tt>.
     */
    public short getGrantedRights(Account acct, ACL acl) throws ServiceException {
      ...
      if (ZimbraLog.acl.isTraceEnabled()) {
        ZimbraLog.acl.trace(
            "ACL.GrantedRights 0 for acl=%s (does not match %s)",
    ...
    /**
     * Returns whether this grant applies to the given {@link Account}. If <tt>acct</tt> is
     * <tt>null</tt>, only return <tt>true</tt> if the grantee is {@link ACL#GRANTEE_PUBLIC}.
     */
    public boolean matches(Account acct) throws ServiceException {
      Provisioning prov = Provisioning.getInstance();
      if (acct == null) return mType == ACL.GRANTEE_PUBLIC;
      switch (mType) {
    ...
    private boolean matchesAccessKey(Account acct) {
      if (!(acct instanceof GuestAccount)) return false;
      return ((GuestAccount) acct).matchesAccessKey(mGrantee, mSecret);
    }

    /** Utility function: Returns the zimbraId for a null-checked LDAP entry. */
    private static final String getId(NamedEntry entry) {
      return (entry == null ? null : entry.getId());
    }

    /**
     * Returns whether the principal id exactly matches the grantee. <tt>zimbraId</tt> must be
     * {@link GuestAccount#GUID_PUBLIC} (<tt>null</tt> is also OK) if the actual grantee is {@link
     * ACL#GRANTEE_PUBLIC}. <tt>zimbraId</tt> must be {@link GuestAccount#GUID_AUTHUSER} if the
    ...
    public boolean isGrantee(String zimbraId) {
      ...
      else if (zimbraId.equals(GuestAccount.GUID_AUTHUSER)) return (mType == GRANTEE_AUTHUSER);
      return mType == GRANTEE_GUEST || mType == GRANTEE_KEY
          ? zimbraId.equalsIgnoreCase(mGrantee)
          : zimbraId.equals(mGrantee);
    }

    /**
     * Updates the granted rights in the <tt>Grant</tt>. The old set of rights is discarded.
     *
     * @param rights A bitmask of the rights being granted.
    ...
    /**
     * For grants to external users, sets the password/accesskey required to access the resource.
     */
    void setPassword(String password) {
      if ((mType == GRANTEE_GUEST || mType == GRANTEE_KEY) && password != null) mSecret = password;
    }
    ...
    /** Encapsulates this <tt>Grant</tt> as a {@link Metadata} object for serialization. */
    public Metadata encode() {
      ...
      if (mType == GRANTEE_KEY) meta.put(FN_ACCESSKEY, mSecret);
      else meta.put(FN_PASSWORD, mSecret);
  ...
  /**
   * Time when all grants to internal users or groups expire. Value of 0 indicates that they never
   * expire.
   */
  private long mInternalGrantExpiry = 0;
  /**
   * Time when all grants to guest/external users expire. Value of 0 indicates that they never
   * expire.
   */
  ...
  /**
   * Returns the bitmask of rights granted to the user by the ACL, or <tt>null</tt> if there are no
   * rights granted to anyone. (Note that if rights are granted to <i>other</i> accounts but not to
   * the specified user, returns <tt>0</tt>.)
   *
   * @param authuser The user to gather rights for.
   * @return A <tt>Short</tt> containing the OR'ed-together rights granted to the user, or
   *     <tt>null</tt>.
   */
  public Short getGrantedRights(Account authuser) throws ServiceException {
    ...
    if ((rightsGranted & SUBFOLDER_RIGHTS) == SUBFOLDER_RIGHTS) rightsGranted |= RIGHT_SUBFOLDER;
  ...
  /**
   * Grants the specified set of rights to the target. If another set of rights has already been
   * granted to the exact given (id, type) pair, the previous set is revoked and the new set is
   * granted.
   *
   * @param zimbraId The zimbraId of the entry being granted rights.
   * @param type The type of object the grantee's ID refers to.
  ...
  public ACL.Grant grantAccess(String zimbraId, byte type, short rights, String secret, long expiry)
    ...
    if (type == GRANTEE_AUTHUSER) zimbraId = GuestAccount.GUID_AUTHUSER;
    else if (type == GRANTEE_PUBLIC) zimbraId = GuestAccount.GUID_PUBLIC;
    else if (zimbraId == null) throw ServiceException.INVALID_REQUEST("missing grantee id", null);

    // always generate a new key (if not provided) for updating or new key grants
    if (type == GRANTEE_KEY && secret == null) secret = generateAccessKey();
    ...
    if (!mGrants.isEmpty()) {
      for (Grant grant : mGrants) {
        if (grant.isGrantee(zimbraId)) {
          ...
          if (type == GRANTEE_GUEST || type == GRANTEE_KEY) grant.setPassword(secret);
  ...
  /**
   * Removes the set of rights granted to the specified id. If no rights were previously granted to
   * the target, no error is thrown and <tt>false</tt> is returned.
   *
  ...
  public boolean revokeAccess(String zimbraId) {
    if (mGrants == null || mGrants.isEmpty()) return false;
  ...
  public Metadata encode() {
    ...
    for (Grant grant : mGrants) mlist.add(grant.encode());
  ...
  /** Returns the list of this <tt>ACL</tt>'s set of encapsulated {@link ACL.Grant} objects. */
