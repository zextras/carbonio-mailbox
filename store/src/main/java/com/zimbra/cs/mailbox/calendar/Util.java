...
public class Util {
  ...
  public static void encodeXParamsAsMetadata(Metadata meta, Iterator<ZParameter> xparamsIter) {
    ...
    for (; xparamsIter.hasNext(); ) {
      ...
      if (paramValue != null) paramMeta.put(FN_VALUE, paramValue);
      meta.put(FN_XPROP_OR_XPARAM + xparamCount, paramMeta);
      xparamCount++;
    }
    if (xparamCount > 0) meta.put(FN_NUM_XPROPS_OR_XPARAMS, xparamCount);
  }
  ...
  public static void encodeXPropsAsMetadata(Metadata meta, Iterator<ZProperty> xpropsIter) {
    ...
    for (; xpropsIter.hasNext(); ) {
      ...
      if (propValue != null) propMeta.put(FN_VALUE, propValue);
    ...
    if (xpropCount > 0) meta.put(FN_NUM_XPROPS_OR_XPARAMS, xpropCount);
  }
  ...
  public static List<ZProperty> decodeXPropsFromMetadata(Metadata meta) throws ServiceException {
    ...
    if (xpropCount > 0) {
      ...
      for (int propNum = 0; propNum < xpropCount; propNum++) {
        ...
        if (propValue != null) xprop.setValue(propValue);
  ...
  public static Metadata encodeAsMetadata(ICalTimeZone tz) {
    ...
    if (ICalTimeZone.lookupByTZID(tzid) != null) return meta;
      ...
      if (!DebugConfig.disableCalendarTZMatchByID || !hasDef) {
        ...
        if (tz != null) {
          ...
          ZimbraLog.calendar.debug(
              "Unknown time zone \"" + tzid + "\" in metadata; using UTC instead");
          return ICalTimeZone.getUTC().cloneWithNewTZID(tzid);
        }
      }
    } else tzid = "unknown time zone";
  ...
  public static Metadata encodeAsMetadata(TimeZoneMap tzmap) {
    ...
    int nextIndex = 0;
    for (Iterator<Entry<String, ICalTimeZone>> iter = tzmap.getMap().entrySet().iterator();
        iter.hasNext(); ) {
    ...
    for (Iterator<Entry<String, String>> iter = tzmap.getAliasMap().entrySet().iterator();
        iter.hasNext(); ) {
  ...
  public static TimeZoneMap decodeFromMetadata(Metadata meta, ICalTimeZone localTZ)
    ...
    for (ICalTimeZone tz : tzlist) {
      if (tz != null) tzmap.put(tz.getID(), tz);
    }
        ...
        if (tzid.charAt(0) != '#') {
          ...
          if (idx >= 0 && idx < tzlist.length) {
            ...
            if (tz != null) {
              ...
              if (!realId.equals(tzid)) aliasMap.put(tzid, realId);
            }
          }
        }
      }
