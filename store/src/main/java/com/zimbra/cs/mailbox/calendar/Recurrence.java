...
/**
 * @author tim
 *     <p>We use this class to represent a full iCal recurrence -- as such, it is really used in two
 *     distinct ways:
 *     <p>As a member of an Invite, this class (well, a tree of this) represents a combination of
 *     RRULE, EXRULE, RDATE, EXDATE which all generate instances of the same Invite. All generated
 *     instances point to that same Invite.
 *     <p>As a member of an CalendarItem, this tree represents the overall recurrence-rule for the
 *     calendar item. IE there is a "default" Invite which generates a bunch of instances, and then
 *     there are Exceptions which each have a RecurrenceId (which corresponds to an instance
 *     generated by the default) and which override or cancel that instance. The instances returned
 *     from an CalendarItem's Recurrence tree can point to many Invites.
 */
public class Recurrence {
  /**
   * RecurrenceRule -- from the default invite.
   *
   * <p>Can have add/subtract sub-parts as well as Exception parts
   */
   ...
   *
   * <p>Has a RecurId and can also have add/subtract sub-parts
   */
  ...
  /**
   * Always stored as part of a Recurrence, Exception, or Cancellation -- a rule for generating
   * dates
   *
   * <p>Use the getZRecur() API to access the rule itself
   */
  public static final int TYPE_REPEATING = 5;

  /**
   * Always stored as part of a Recurrence, Exception, or Cancellation -- a list of one or more
   * SingleDates.DateValue objects
   */
  public static final int TYPE_SINGLE_DATES = 6;

  /**
   * @author tim
   *     <p>Represents a full iCal recurrence ruleset -- including exceptions, etc.
   *     <p>The concrete subclasses are: SingleDates: a list of 1-time events
   *     <p>SimpleRepeatingRule: anything that can be expressed in a single rule...RECUR in the iCal
   *     RFC
   *     <p>RecurrenceRule: Rule which fully expresses iCal grammar (support Exceptions, EXRULE,
   *     etc)
   *     <p>ExceptionRule: rule that has a RECURRENCE_ID which is used to determine when it applies
   */
  public interface IRecurrence extends Cloneable {
    ...
    /**
     * @return TYPE_RECURRENCE, TYPE_EXCEPTION, TYPE_CANCELLATION, TYPE_SINGLE_DATES, or
     *     TYPE_REPEATING
     */
    public int getType();

    /**
     * @return an Iterator over all child rules that add instances to this current node, or NULL if
     *     none
     */
    ...
    /**
     * WARNING WARNING: this is a special call only used when initializing an INVITE, basically it
     * walks the entire Recurrence chain and sets the InvId to the passed-in value....while this is
     * OK in the case of a single-Invite's recurrence, if this is called on an CalendarItem's
     * Recurrence, it will cause Bad Things To Happen!
     *
     * @param invId
     */
  ...
  /**
   * @author tim
   *     <p>Superset of IRecurrence for exception rules: they have a RecurId
   *     <p>Real implementations are ExceptionRule and CancellationRule
   */
  ...
  /**
   * @author tim
   *     <p>A subnode which generates instances -- ie right now either a SingleDates or
   *     SimpleRepeatingRule
   */
  ...
  /**
   * @author tim
   *     <p>Internal Node -- basically just wraps an ArrayList of subrules and does some convienence
   *     stuff (e.g. getStartTime calculates the earliest starting time for all the rules we're
   *     sorting, etc)
   */
  public static class MultiRuleSorter {
    ...
    public Metadata encodeMetadata() {
      ...
      for (int i = 0; i < mRules.size(); i++) meta.put(FN_RULE + i, mRules.get(i).encodeMetadata());
      return meta;
    }
    ...
    public MultiRuleSorter(List<IRecurrence> rules) {
      assert ((rules == null)
          || (rules.size() == 0)
          || (rules.get(0) instanceof IInstanceGeneratingRule));
      mRules = rules;
    }
    ...
    List<Instance> expandInstances(int calItemId, long start, long end) throws ServiceException {
  ...
  public static class SingleDates implements IInstanceGeneratingRule {
    ...
    public String toString() {
      ...
      if (mRdateExdate != null) {
        if (mRdateExdate.isRDATE()) toRet.append("rdate=[");
        else toRet.append("exdate=[");
      ...
      if (mInvId != null) toRet.append(", InvId=").append(mInvId.toString());
      toRet.append("]");
    ...
    public ParsedDateTime getStartTime() {
      ...
      for (DateValue val : mDates) {
        ...
        if (start == null || start.compareTo(valStart) > 0) start = valStart;
      }
      return start;
    }
    ...
    public ParsedDateTime getEndTime() {
      ...
      for (DateValue val : mDates) {
        ...
        if (end == null || end.compareTo(valEnd) < 0) end = valEnd;
      }
      return end;
    }
    ...
    public List<Instance> expandInstances(int calItemId) {
      ...
      if (mRdateExdate.isEXDATE() || DebugConfig.enableRdate) {
        for (DateValue val : mDates) {
          ...
          boolean allDay =
              !valStart.hasTime()
                  || (valStart.hasZeroTime()
                      && mDefaultDuration != null
                      && mDefaultDuration.isMultipleOfDays());
          list.add(
              new Instance(
                  calItemId,
                  mInvId,
    ...
    public Metadata encodeMetadata() {
      ...
      if (mInvId != null) meta.put(FN_INVID, mInvId.encodeMetadata());
      return meta;
    }
    ...
    SingleDates(Metadata meta, TimeZoneMap tzmap) throws ServiceException, ParseException {
      ...
      if (metaInvId != null) mInvId = InviteInfo.fromMetadata(metaInvId, tzmap);
      setDates();
    }
    ...
    private void setDates() {
      List<DateValue> list = new ArrayList<DateValue>(mRdateExdate.numValues());
    ...
    public Object clone() {
      return new SingleDates(
          mRdateExdate == null ? null : (RdateExdate) mRdateExdate.clone(),
  ...
  /**
   * @author tim
   *     <p>The output of an RRULE or EXRULE rule -- corresponds to a RECUR value in an iCal
   *     specification
   */
  public static class SimpleRepeatingRule implements IInstanceGeneratingRule {
    ...
    public SimpleRepeatingRule(
        ParsedDateTime dtstart, ParsedDuration duration, ZRecur recur, InviteInfo invId) {
      mDtStart = dtstart;
      mRecur = recur;
      fixupRecurUntil();
      mInvId = invId;
      mDuration = duration;
      if (mDuration == null) {
        if (mDtStart != null && !mDtStart.hasTime()) mDuration = ParsedDuration.ONE_DAY;
        else mDuration = ParsedDuration.ONE_SECOND;
      }
    }
    ...
    public Element toXml(Element parent) {
      ...
      if (untilDate != null) {
        ...
        untilElt.addAttribute(MailConstants.A_CAL_DATETIME, untilDate.getDateTimePartString(false));
      } else {
        int count = mRecur.getCount();
        if (count > 0) {
          rule.addElement(MailConstants.E_CAL_RULE_COUNT)
              .addAttribute(MailConstants.A_CAL_RULE_COUNT_NUM, count);
        }
      }
      ...
      if (!byMonthDay.isEmpty()) {
        rule.addElement(MailConstants.E_CAL_RULE_BYMONTHDAY)
            .addAttribute(
                MailConstants.A_CAL_RULE_BYMONTHDAY_MODAYLIST, ZRecur.listAsStr(byMonthDay));
      }
      ...
      if (!byYearDay.isEmpty()) {
        rule.addElement(MailConstants.E_CAL_RULE_BYYEARDAY)
            .addAttribute(
                MailConstants.A_CAL_RULE_BYYEARDAY_YRDAYLIST, ZRecur.listAsStr(byYearDay));
      }
      ...
      //                xElt.addAttribute(MailService.A_CAL_RULE_XNAME_NAME, (String)
      // entry.getKey());
      //                xElt.addAttribute(MailService.A_CAL_RULE_XNAME_VALUE, (String)
      // entry.getValue());
      //            }
    ...
    public List<Instance> expandInstances(int calItemId, long start, long end) {
      ...
      if (tz == null) tz = ICalTimeZone.getUTC();
      try {
        long duration = 0;
        if (mDuration != null) {
        ...
        List<java.util.Date> dateList =
            mRecur.expandRecurrenceOverRange(mDtStart, start - duration, end);
        ...
        for (Iterator iter = dateList.iterator(); iter.hasNext(); ) {
          ...
          if (instStart < end && instEnd > start) {
            ...
            boolean allDay =
                !mDtStart.hasTime()
                    || (mDtStart.hasZeroTime()
                        && mDuration != null
                        && mDuration.isMultipleOfDays());
            toRet.add(
                num++,
                new Instance(
                    calItemId, mInvId, true, true, instStart, instEnd, allDay, startTzo, endTzo,
                    false, false));
          }
        }
        ...
        ZimbraLog.calendar.warn(
            "ServiceException expanding recurrence rule: " + mRecur.toString(), se);
    ...
    public ParsedDateTime getEndTime() throws ServiceException {
      if (mDtStart == null) {
        ...
        if (mRecur.getCount() > 0) {
          ...
          List<Date> dates =
              mRecur.expandRecurrenceOverRange(mDtStart, mDtStart.getUtcTime(), endMillis);
          if (!dates.isEmpty()) endMillis = dates.get(dates.size() - 1).getTime();
        }
        ParsedDateTime end = ParsedDateTime.fromUTCTime(endMillis, mDtStart.getTimeZone());
        if (mDuration != null) end = end.add(mDuration);
        return end;
      } else {
        // non-recurring or invalid rule; assume single instance, end = start + duration
        ParsedDuration dur =
            mDuration != null ? mDuration : ParsedDuration.parse(false, 0, 0, 0, 0, 1);
    ...
    public Object clone() {
      return new SimpleRepeatingRule(
          mDtStart == null ? null : (ParsedDateTime) mDtStart.clone(),
  ...
  /**
   * @author tim
   *     <p>Base class: models a set of rules as DTSTART + (RRULEs + RDATEs - EXRULEs - EXDATEs)
   *     <p>Not instantiated directly -- used to build either a Rule (which can have Exceptions) or
  ...
  public abstract static class CompoundRuleBase implements IRecurrence {
    ...
    public List<Instance> expandInstances(int calItemId, long start, long end)
      ...
      // RRULEs + RDATEs
      List<Instance> toAdd;
      if (mAddRules != null) toAdd = mAddRules.expandInstances(calItemId, start, end);
      else toAdd = new ArrayList<Instance>(1);
      ...
      if (firstStart < end && firstEnd > start) {
        ...
        boolean allDay =
            !mDtStart.hasTime()
                || (mDtStart.hasZeroTime() && mDuration != null && mDuration.isMultipleOfDays());
        CalendarItem.Instance dtstartInst =
            new CalendarItem.Instance(
                ...
                allDay,
                mDtStart.getOffset(),
                dtFirstEnd != null ? dtFirstEnd.getOffset() : 0,
                false,
                true);
        if (first == null || first.compareTo(dtstartInst) != 0) toAdd.add(0, dtstartInst);
      }

      // -(EXRULEs + EXDATEs)
      if (mSubtractRules == null) return toAdd;
      List<Instance> toExclude = mSubtractRules.expandInstances(calItemId, start, end);
      return ListUtil.subtractSortedLists(toAdd, toExclude, new Instance.StartTimeComparator());
    }
    ...
    public Metadata encodeMetadata() {
      ...
      meta.put(FN_DTSTART, mDtStart);
      meta.put(FN_DURATION, mDuration);
      if (mAddRules != null) meta.put(FN_ADDRULES, mAddRules.encodeMetadata());
      if (mSubtractRules != null) meta.put(FN_SUBRULES, mSubtractRules.encodeMetadata());
      if (mInvId != null) meta.put(FN_INVID, mInvId.encodeMetadata());
    ...
    protected CompoundRuleBase(
        ParsedDateTime dtstart,
        ParsedDuration duration,
        MultiRuleSorter addRules,
        MultiRuleSorter subtractRules,
        InviteInfo invID) {
      mDtStart = dtstart;
      mDuration = duration;
      if (mDuration == null) {
  ...
  //
  // This is commented-out for compatability (not supported by many CUAs) --but leave the code here
  // hopefully
  ...
  //            assert(rangeType == NOT_RANGE || rangeType == THIS_AND_FUTURE || rangeType ==
  // THIS_AND_PRIOR);
  ...
  //        }
  //
  //        protected RecurrenceRange(Metadata meta, TimeZoneMap tzMap) throws ServiceException,
  // ParseException
  //        {
  ...
  /**
   * @author tim
   *     <p>an RFC2446 CANCEL request to cancel an instance or range of instances
   *     <p>basically, this intercepts one or more Recurrence instances, and replaces them with
   *     nothing
   */
  public static class CancellationRule implements IException {
    ...
    public String toString() {
      return String.format(
          "CANCELLATION(%s)", mRecurRange == null ? "<null>" : mRecurRange.toString());
    }
  }

  /**
   * @author tim
   *     <p>A recurrence-rule that has a RECURRENCE-ID and can tell you if it applies or not
   */
  public static class ExceptionRule extends CompoundRuleBase implements IException {
    ...
    public ExceptionRule(
        RecurId recurrenceId, ParsedDateTime dtstart, ParsedDuration duration, InviteInfo invId) {
      super(dtstart, duration, invId);
      mRecurRange = recurrenceId;
    }

    protected ExceptionRule(Metadata meta, TimeZoneMap tzmap)
    ...
    private ExceptionRule(ExceptionRule other) {
      super(
          other.mDtStart == null ? null : (ParsedDateTime) other.mDtStart.clone(),
  ...
  /**
   * @author tim
   *     <p>Models the initial "master" rule in a ruleset -- ie the one and only component for a
   *     particular UID that has no RECURRENCE-ID setting
   *     <p>To get the output of this rule you basically: For each Instance: ( DTSTART +
   *     (RRULEs+RDATES-EXRULEs-EXDATEs) ) Check the RECURRENCE-ID of each Exception if it matches,
   *     then use that Exceptions Instances else return the Instance
   */
            ...
            if (dt != null) {
              ...
              if (st != null) {
                ...
                if (stTime >= start && stTime < end) {
                  if (recurIdTime < startAdjusted) startAdjusted = recurIdTime;
                  else if (recurIdTime > endAdjusted) endAdjusted = recurIdTime + 1;
                }
              }
              ...
              if (except != null) {
                ...
                if (inst.isAllDay()) {
                  ...
                  if (eRid != null && eRid.getDt() != null) {
                    ...
                    if (iOffset != eOffset) instStart += iOffset - eOffset;
                  }
                }
        ...
        if (except != null) {
          ...
          for (Iterator<Instance> iter = instances.iterator(); iter.hasNext(); ) {
            ...
            if (inst != null) {
              ...
              long et = inst.getEnd();
              if (et < start || st >= end) iter.remove();
            }
          }
          if (!instances.isEmpty()) exceptionInstancesList.add(instances);
        }
      }
  ...
  public static List<Instance> expandInstances(
      IRecurrence recur, int calItemId, long start, long end) throws ServiceException {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //        List<Instance> instances = recurrence.expandInstances(-1, startCal.getTimeInMillis(),
    // endCal.getTimeInMillis());
    List<Instance> instances =
        recurrence.expandInstances(-1, startCal.getTimeInMillis(), Long.MAX_VALUE);
