...
public class WindowsSystemTime {
  ...
  public SimpleOnset toSimpleOnset() {
    ...
    int week = mDay;
    if (week == 5) week = -1;
    return new SimpleOnset(week, mDayOfWeek + 1, mMonth, mDay, mHour, mMinute, mSecond);
  }
  ...
  public static WindowsSystemTime fromSimpleOnset(SimpleOnset onset) {
    ...
    if (week != 0) {
      ...
      cal.set(
          year, onset.getMonth() - 1, onset.getDayOfMonth(), onset.getHour(), onset.getMinute());
      return new WindowsSystemTime(
          year,
          onset.getMonth(),
