...
public class Alarm {
  ...
  public Alarm(
      ...
      ParsedDuration triggerRelative,
      ParsedDateTime triggerAbsolute,
      ParsedDuration repeatDuration,
      int repeatCount,
      String description,
      String summary,
      Attach attach,
      List<ZAttendee> attendees,
      List<ZProperty> xProperties)
      throws ServiceException {
    if (action == null) throw ServiceException.INVALID_REQUEST("Missing ACTION in VALARM", null);
    mAction = action;
  ...
  public String toString() {
    ...
    if (TriggerType.ABSOLUTE.equals(mTriggerType)) {
      sb.append(", triggerAbsolute=")
          .append(mTriggerAbsolute != null ? mTriggerAbsolute.toString() : "<none>");
    } else {
      sb.append(", triggerRelated")
          .append(mTriggerRelated != null ? mTriggerRelated.toString() : "<default>");
      sb.append(", triggerRelative=")
          .append(mTriggerRelative != null ? mTriggerRelative.toString() : "<none>");
    }
    if (mRepeatDuration != null) {
      sb.append(", repeatDuration=")
          .append(mRepeatDuration != null ? mRepeatDuration.toString() : "<none>");
    ...
    if (mAttach != null) sb.append(", attach=").append(mAttach.toString());
    if (mAttendees != null) {
      sb.append(", attendees=[");
      boolean first = true;
  ...
  /**
   * Return a copy of the X properties, excluding X-WR-ALARMUID because for Outlook and ZWC, we
   * usually convert AUDIO and PROCEDURE alarms to be DISPLAY alarms and iCal.app does not like
   * X-WR-ALARMUID set for DISPLAY alarms.
   *
  ...
  public AlarmInfo toJaxb() {
    ...
    if ((Action.AUDIO.equals(mAction) || Action.PROCEDURE.equals(mAction))
    ...
    if (TriggerType.ABSOLUTE.equals(mTriggerType)) {
      ...
      if (mTriggerRelated != null) relative.setRelated(mTriggerRelated.toString());
    }
    if (mRepeatDuration != null) {
  ...
  public Element toXml(Element parent) {
    ...
    if (!Action.AUDIO.equals(action)) {
      ...
      if (mDescription != null) desc.setText(mDescription);
    }
    if (!Action.DISPLAY.equals(action) && mAttach != null) mAttach.toXml(alarm);
    if (Action.EMAIL.equals(mAction)
        || Action.X_YAHOO_CALENDAR_ACTION_IM.equals(mAction)
        || Action.X_YAHOO_CALENDAR_ACTION_MOBILE.equals(mAction)) {
  ...
  public static Alarm parse(Element alarmElem) throws ServiceException {
    ...
    while (attendeesIter.hasNext()) {
      ...
      if (attendees == null) attendees = new ArrayList<ZAttendee>();
      attendees.add(at);
    }
  ...
  public ZComponent toZComponent() throws ServiceException {
    ...
    if (!Action.AUDIO.equals(mAction)) {
      ...
      if (d == null && !Action.PROCEDURE.equals(mAction)) d = "Reminder";
      ZProperty desc = new ZProperty(ICalTok.DESCRIPTION, d);
      comp.addProperty(desc);
    }

    if (mAttach != null) comp.addProperty(mAttach.toZProperty());

    if (Action.EMAIL.equals(mAction)
        || Action.X_YAHOO_CALENDAR_ACTION_IM.equals(mAction)
        || Action.X_YAHOO_CALENDAR_ACTION_MOBILE.equals(mAction)) {
      String s = mSummary;
      if (s == null) s = "Reminder";
      ...
      switch (tok) {
        ...
        case TRIGGER:
          ...
          if (valueType != null) {
            ...
            if (ICalTok.DATE_TIME.toString().equals(vt)) triggerType = TriggerType.ABSOLUTE;
          }
          if (TriggerType.RELATIVE.equals(triggerType)) {
            ...
            try {
              if (val != null) triggerAbsolute = ParsedDateTime.parseUtcOnly(val);
            } catch (ParseException e) {
          ...
          break;
        case DURATION:
          if (val != null) repeatDuration = ParsedDuration.parse(val);
          break;
        case REPEAT:
          if (val != null) {
            try {
  ...
  private Iterator<ZProperty> xpropsIterator() {
    return xProps.iterator();
  }
  ...
  public static Alarm fromSimpleReminder(int minBeforeStart) throws ServiceException {
    return new Alarm(
        ...
        ParsedDuration.parse(true, 0, 0, 0, minBeforeStart, 0),
        null,
        null,
        0,
        null,
  ...
  private static Action expandAction(String abbrev) {
    if (abbrev == null || abbrev.length() == 0) return Action.DISPLAY;
    Action action;
    ...
    switch (ch) {
      ...
      default:
        ...
        if (action == null) action = Action.DISPLAY;
    }
    return action;
  }

  private static String abbrevTriggerType(TriggerType tt) {
    if (tt == null || TriggerType.RELATIVE.equals(tt)) return "r";
    else return "a";
  }

  private static TriggerType expandTriggerType(String abbrev) {
    if (abbrev == null || abbrev.length() == 0) return TriggerType.RELATIVE;
    char ch = abbrev.charAt(0);
    if (ch == 'a') return TriggerType.ABSOLUTE;
    else return TriggerType.RELATIVE;
  }

  private static String abbrevTriggerRelated(TriggerRelated tr) {
    if (tr == null) return null;
    else if (TriggerRelated.END.equals(tr)) return "e";
    else return "s";
  }

  private static TriggerRelated expandTriggerRelated(String abbrev) {
    if (abbrev == null || abbrev.length() == 0) return null;
    char ch = abbrev.charAt(0);
    if (ch == 'e') return TriggerRelated.END;
    else return TriggerRelated.START;
  }
  ...
  public Metadata encodeMetadata() {
    ...
    if (mAttach != null) meta.put(FN_ATTACH, Util.encodeMetadata(mAttach));
    if (mAttendees != null) {
    ...
    if (xProps.size() > 0) Util.encodeXPropsAsMetadata(meta, xpropsIterator());
  ...
  public static Alarm decodeMetadata(Metadata meta) throws ServiceException {
    ...
    if (metaAttach != null) attach = Util.decodeAttachFromMetadata(metaAttach);
  ...
  /**
   * Returns the alarm trigger time in millis. Both start and end times of the appointment/task
   * instance are required because the alarm may be specified relative to either start or end time.
   *
  ...
  public long getTriggerTime(long instStart, long instEnd) {
    ...
    if (TriggerRelated.END.equals(mTriggerRelated)) return mTriggerRelative.addToTime(instEnd);
    else return mTriggerRelative.addToTime(instStart);
  }
