...
// TODO: high cache stickiness (or dedicated non-LRU cache) for heavily shared, almost read-only
// calendars
...
public class CalSummaryCache {
  ...
  public static final String END_TIME_MUST_BE_AFTER_START_TIME =
  ...
  //     CalendarItemData, ...
  //   ]
  private static final int S_RANGE_MONTH_FROM;
  private static final int S_RANGE_NUM_MONTHS;
  private static final int S_MAX_STALE_ITEMS;
  ...
  static {
    ...
    S_RANGE_NUM_MONTHS = LC.calendar_cache_range_months.intValue();
    S_MAX_STALE_ITEMS = LC.calendar_cache_max_stale_items.intValue();
    S_MAX_STALE_ITEMS_BEFORE_INVALIDATING_CALENDAR = 100;
    S_MAX_SEARCH_DAYS = LC.calendar_search_max_days.intValueWithinRange(0, 3660);
  }
  ...
  public static CalendarItemData reloadCalendarItemOverRange(
      CalendarItem calItem, long rangeStart, long rangeEnd) throws ServiceException {
    CalendarItemData calItemData = null;
    try {
      boolean rangeValid =
          (rangeStart >= CalendarUtils.MICROSOFT_EPOC_START_MS_SINCE_EPOC
              && rangeEnd > CalendarUtils.MICROSOFT_EPOC_START_MS_SINCE_EPOC
              && rangeStart < rangeEnd);
      if (!rangeValid) {
        return null;
      }
      ...
      if (calItemAlarmData != null) {
        ...
        alarm =
            new AlarmData(
                calItemAlarmData.getNextAt(),
                calItemAlarmData.getNextInstanceStart(),
                alarmInvId,
                alarmCompNum,
                summary,
                location,
                calItemAlarmData.getAlarm());
      }
      ...
      calItemData =
          new CalendarItemData(
              ...
              calItem.getFolderId(),
              calItem.getId(),
              calItem.getFlagString(),
              calItem.getTags(),
              TagUtil.getTagIdString(calItem),
              calItem.getModifiedSequence(),
              calItem.getSavedSequence(),
              calItem.getDate(),
              calItem.getChangeDate(),
              calItem.getSize(),
              defaultInvite.getUid(),
              defaultInvite.isRecurrence(),
              calItem.hasExceptions(),
              calItem.isPublic(),
              alarm,
              defaultData);
      ...
      Collection<CalendarItem.Instance> instances =
          calItem.expandInstances(rangeStart, rangeEnd, true);
      for (CalendarItem.Instance inst : instances) {
        try {
          ...
          Long instStartLong = instStart != 0 ? instStart : null;
          Long durationLong = duration > 0 ? duration : null;
          ...
          if (hasTimes) {
            ...
            if (inst.getEnd() > actualRangeEnd) actualRangeEnd = inst.getEnd();
          }

          InviteInfo invId = inst.getInviteInfo();
          Invite inv = calItem.getInvite(invId.getMsgId(), invId.getComponentId());
          Long alarmAt = instStart == alarmInst ? alarmTime : null;
          ...
          if (!inst.isException()) {
            ...
            Long tzOffset =
                instStartLong != null && inst.isAllDay() ? (long) inst.getStartTzOffset() : null;
            instData =
                new InstanceData(
                    ridZ,
                    instStartLong,
            ...
            String ridZ = null;
            if (inv.hasRecurId()) ridZ = inv.getRecurId().getDtZ();
            instData =
                new FullInstanceData(
                    inv, ridZ, instStartLong, durationLong, effectivePartStat, fba, alarmAt);
          }
          calItemData.addInstance(instData);
        } catch (MailServiceException.NoSuchItemException e) {
          ZimbraLog.calendar.info(
              "Error could not get instance "
                  + inst.getMailItemId()
                  + "-"
                  + inst.getComponentNum()
                  + " for appt "
                  + calItem.getId(),
              e);
        }
      ...
      ZimbraLog.calendar.info(
          "Error could not get default invite for calendar item: " + calItem.getId(), e);
  ...
  private static CalendarData reloadCalendarOverRange(
    ...
    if (rangeEnd < rangeStart)
      throw ServiceException.INVALID_REQUEST(END_TIME_MUST_BE_AFTER_START_TIME, null);
    long days = (rangeEnd - rangeStart) / MSEC_PER_DAY;
    if (days > S_MAX_SEARCH_DAYS)
      throw ServiceException.INVALID_REQUEST(
          "Requested range is too large (Maximum " + S_MAX_SEARCH_DAYS + " days)", null);
    ...
    if (prevCalData == null
      ...
      return reloadCalendarOverRangeWithFolderScan(
          octxt, mbox, folderId, type, rangeStart, rangeEnd, null);
    }
    if (!incrementalUpdate || prevCalData.getNumStaleItems() > S_MAX_STALE_ITEMS) {
      ...
      return reloadCalendarOverRangeWithFolderScan(
          octxt, mbox, folderId, type, rangeStart, rangeEnd, prevCalData);
    }

    Set<Integer> staleItemIds = new HashSet<>();
    ...
    for (Iterator<CalendarItemData> iter = prevCalData.calendarItemIterator(); iter.hasNext(); ) {
      ...
      if (!staleItemIds.contains(calItemId)) {
        ...
        // Our item is either modified or deleted.  Re-fetch from backend to see which case it is.
        ...
        // list.
        if (calItemData != null && calItemData.getCalItemId() == calItemId) {
  ...
  private static CalendarItemData fetchCalendarItemData(
      OperationContext octxt,
      Mailbox mbox,
      int folderId,
      int calItemId,
    ...
    try {
      ...
      if (!e.getCode().equals(MailServiceException.NO_SUCH_ITEM)
          ...
          && !e.getCode().equals(MailServiceException.NO_SUCH_TASK)) throw e;
      // NO_SUCH_ITEM/APPT/TASK exception is an expected condition.  This happens when a calendar
      // item
      // was hard deleted from a sync client.  Calendar cache doesn't process the delete
      // notification
  ...
  // Reload by first calling Mailbox.getCalendarItemsForRange(), which runs a query on appointment
  // table.
  private static CalendarData reloadCalendarOverRangeWithFolderScan(
    ...
    Collection<CalendarItem> calItems =
        mbox.getCalendarItemsForRange(octxt, type, rangeStart, rangeEnd, folderId, null);
    for (CalendarItem calItem : calItems) {
      ...
      if (calItemData != null) calData.addCalendarItem(calItemData);
    }
    return calData; // return a non-null object even if there are no items in the range
  }

  // get summary for all appts/tasks in a calendar folder
  public CalendarDataResult getCalendarSummary(
      OperationContext octxt,
      String targetAcctId,
      int folderId,
      MailItem.Type type,
    ...
    if (!LC.calendar_cache_enabled.booleanValue()) {
      ...
      result.data =
          reloadCalendarOverRangeWithFolderScan(
              octxt, mbox, folderId, type, rangeStart, rangeEnd, null);
      if (ZimbraLog.calendar.isDebugEnabled()) {
        ZimbraLog.calendar.debug(
            "Calendar Summary for %s reloaded (no cache) - %s items private=%s",
    ...
    try {
      ...
      ZimbraLog.calendar.warn(
          "Problem discovering ACLs for folder %s:%s", targetAcctId, folderId, se);
    ...
    if (calData != null) {
      ...
      if (ZimbraLog.calendar.isDebugEnabled()) {
        ZimbraLog.calendar.debug(
            "Calendar Summary for %s:%s reloaded (memcached) - %s items private=%s",
    ...
    if (!targetAcctOnLocalServer) {
      if (ZimbraLog.calendar.isDebugEnabled()) {
        ZimbraLog.calendar.debug(
            "Calendar Summary - ignoring non-local %s:%s", targetAcctId, folderId);
      }
      return null;
    }
    ...
    boolean incrementalUpdate = S_MAX_STALE_ITEMS > 0;
      ...
      try {
        ...
        if (calData != null) {
          ...
          if (calData.getModSeq() == currentModSeq) {
            ...
            // Data loaded from file doesn't have stale items list.  It can't be refreshed
            // incrementally.
    ...
    if (calData != null) {
      if (calData.getModSeq() != currentModSeq || calData.getNumStaleItems() > 0) {
        ...
        defaultRange =
            Util.getMonthsRange(System.currentTimeMillis(), S_RANGE_MONTH_FROM, S_RANGE_NUM_MONTHS);
    ...
    if (calData == null) {
      if (defaultRange == null)
        defaultRange =
            Util.getMonthsRange(System.currentTimeMillis(), S_RANGE_MONTH_FROM, S_RANGE_NUM_MONTHS);
      calData =
          reloadCalendarOverRange(
              ownerOctxt,
              mbox,
              folderId,
              type,
              defaultRange.getFirst(),
              defaultRange.getSecond(),
              reusableCalData,
              incrementalUpdate);
      synchronized (mSummaryCache) {
        if (mLRUCapacity > 0) {
    ...
    if (ZimbraLog.calendar.isDebugEnabled()) {
      ZimbraLog.calendar.debug(
          "Calendar Summary for %s:%s reloaded (dataFrom=%s) - %s items private=%s",
          targetAcctId,
          folderId,
          dataFrom,
          (result.data == null) ? 0 : result.data.getNumItems(),
  ...
  private void invalidateSummary(Mailbox mbox, int folderId) {
    if (!LC.calendar_cache_enabled.booleanValue()) return;
  ...
  private void invalidateItem(Mailbox mbox, int folderId, int calItemId) {
    ...
    if (calData != null) {
      ...
      if (numStaleItems > S_MAX_STALE_ITEMS_BEFORE_INVALIDATING_CALENDAR)
            ...
            try {
              mbox =
                  MailboxManager.getInstance()
                      .getMailboxByAccountId(acctId, FetchMode.DO_NOT_AUTOCREATE);
            } catch (ServiceException e) {
              ZimbraLog.calendar.error(
                  "Error looking up the mailbox of account in delete notification: account="
                      + acctId,
                  e);
              continue;
            }
  ...
  private static class SummaryLRU extends LinkedHashMap<CalSummaryKey, CalendarData> {
    ...
    public CalendarData put(CalSummaryKey key, CalendarData value) {
      ...
      if (prevVal == null) registerWithAccount(key);
      return prevVal;
    }
    ...
    protected boolean removeEldestEntry(Map.Entry<CalSummaryKey, CalendarData> eldest) {
      ...
      if (remove) deregisterFromAccount(eldest.getKey());
      return remove;
    }
    ...
    private void registerWithAccount(CalSummaryKey key) {
      ...
      Set<Integer> folders = mAccountFolders.computeIfAbsent(accountId, k -> new HashSet<>());
    ...
    private void deregisterFromAccount(CalSummaryKey key) {
      ...
      if (folders != null) {
        ...
        // If no folders are cached for the account, drop the account entry from the map to save
        // memory.
        if (folders.isEmpty()) mAccountFolders.remove(accountId);
      }
    }
  ...
  public static class CalendarDataResult {
