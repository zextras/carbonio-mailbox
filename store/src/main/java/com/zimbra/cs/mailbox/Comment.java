...
public class Comment extends MailItem {
  ...
  public static Comment create(
    ...
    if (!(parent instanceof Document)) throw MailServiceException.CANNOT_PARENT();
  ...
  protected boolean hasFullPermission(Account authuser) {
    if (authuser != null && authuser.getId().equals(mCreatorId)) return true;
  ...
  private static String encodeMetadata(
      Color color, int metaVersion, int version, String accountId, CustomMetadata custom) {
    CustomMetadataList extended = (custom == null ? null : custom.asList());
    return encodeMetadata(new Metadata(), color, metaVersion, version, accountId, extended)
        .toString();
  }
