...
public abstract class CalendarUser {
  ...
  public void setAddress(String a) {
    mAddress = getMailToAddress(a);
  }

  public boolean hasCn() {
    return !StringUtil.isNullOrEmpty(mCn);
  }
  ...
  public boolean hasSentBy() {
    return !StringUtil.isNullOrEmpty(mSentBy);
  }
  ...
  public void setSentBy(String sb) {
    mSentBy = getMailToAddress(sb);
  }

  public boolean hasDir() {
    return !StringUtil.isNullOrEmpty(mDir);
  }
  ...
  public boolean hasLanguage() {
    return !StringUtil.isNullOrEmpty(mLanguage);
  }
  ...
  public CalendarUser(
    ...
    if (xparams != null) mXParams = xparams;
  }
  ...
  public CalendarUser(ZProperty prop) {
    ...
    for (Iterator<ZParameter> paramIter = prop.parameterIterator(); paramIter.hasNext(); ) {
      ...
      if (param.getToken() == null) {
        ...
        if (name.startsWith("X-") || name.startsWith("x-")) addXParam(param);
      }
    }
  }
  ...
  public Metadata encodeMetadata() {
    ...
    if (mXParams.size() > 0) Util.encodeXParamsAsMetadata(meta, xparamsIterator());
  ...
  /**
   * Reply-to address is either the address of the user, or the sent-by address if it is set.
   *
   * @return
  ...
  protected void setProperty(ZProperty prop) throws ServiceException {
    if (hasCn()) prop.addParameter(new ZParameter(ICalTok.CN, getCn()));
    if (hasSentBy()) prop.addParameter(new ZParameter(ICalTok.SENT_BY, "mailto:" + getSentBy()));
    if (hasDir()) prop.addParameter(new ZParameter(ICalTok.DIR, getDir()));
    if (hasLanguage()) prop.addParameter(new ZParameter(ICalTok.LANGUAGE, getLanguage()));
  }

  public ZProperty toProperty() throws ServiceException {
    String addr = getAddress();
    if (addr != null && addr.indexOf(':') < 0) addr = "mailto:" + addr;
  ...
  public List<ZParameter> getXParams() {
    return mXParams;
  }
  ...
  public ZParameter getXParam(String xparamName) {
    for (ZParameter param : mXParams) {
      if (param.getName().equalsIgnoreCase(xparamName)) return param;
    }
    return null;
  }
  ...
  /**
   * Returns the string after "mailto:" if a mailto address. Returns the entire string otherwise.
   *
   * @param address
   * @return
   */
  ...
  public boolean equals(Object o) {
    ...
    if (!sameValues(mAddress, other.mAddress)) return false;
    if (!sameValues(mCn, other.mCn)) return false;
