...
public final class MailboxUpgrade {
    ...
    try {
      ...
      stmt =
          conn.prepareStatement(
              "SELECT highest_indexed FROM "
                  + DbMailbox.qualifyZimbraTableName(mbox, "mailbox")
                  + " WHERE id = ?");
    ...
    try {
      if (alreadyUpgradedTo2_1(conn, mbox)) {
        ZimbraLog.mailbox.warn(
            "detected already-migrated mailbox %d during migration to version 2.1; skipping.",
            mbox.getId());
      } else {
  ...
  private static boolean alreadyUpgradedTo2_1(DbConnection conn, Mailbox mbox)
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "checking for repeated 2.1 upgrade for mbox " + mbox.getId(), e);
    } finally {
  ...
  public static void upgradeTo2_3(Mailbox mbox) throws ServiceException {
    ...
    ResultSet rs = null;
    try {
      if (flagExists(conn, mbox, Flag.FlagInfo.PRIORITY)) return;
      ...
      migrateColumnToTaggedItem(
          conn,
          mbox,
          "flags",
          Flag.ID_PRIORITY,
          matchTagsets(flagsets, Flag.BITMASK_PRIORITY),
          false);
      conn.commit();
    } catch (ServiceException e) {
      conn.rollback();
  ...
  public static void upgradeTo2_4(Mailbox mbox) throws ServiceException {
    DbConnection conn = DbPool.getConnection(mbox);
    try {
      if (flagExists(conn, mbox, Flag.FlagInfo.POST)) return;
  ...
  public static void upgradeTo2_7(Mailbox mbox) throws ServiceException {
    DbConnection conn = DbPool.getConnection(mbox);
    try {
      if (flagExists(conn, mbox, Flag.FlagInfo.MUTED)) return;
  ...
  private static boolean flagExists(DbConnection conn, Mailbox mbox, Flag.FlagInfo finfo)
      throws ServiceException {
  ...
  private static void migrateFlagColumn(DbConnection conn, Mailbox mbox, boolean checkDuplicates)
      throws ServiceException {
    // get all the different FLAGS column values for the mailbox
    List<Long> flagsets = getTagsets(conn, mbox, "flags");
    ...
    for (int tagId : Mailbox.REIFIED_FLAGS) {
      ...
      migrateColumnToTaggedItem(
          conn, mbox, "flags", tagId, matchTagsets(flagsets, bitmask), checkDuplicates);
    }

    // create rows in the new TAGGED_ITEM table for unread items
    migrateColumnToTaggedItem(
        conn, mbox, "unread", Flag.ID_UNREAD, ImmutableList.of(1L), checkDuplicates);
  }

  private static void migrateTagColumn(DbConnection conn, Mailbox mbox, boolean checkDuplicates)
      throws ServiceException {
    ...
    for (int tagId : tagNames.keySet()) {
      ...
      migrateColumnToTaggedItem(
          conn, mbox, "tags", tagId, matchTagsets(tagsets, bitmask), checkDuplicates);
    }
  ...
  private static void migrateColumnToTaggedItem(
      DbConnection conn,
      Mailbox mbox,
      String column,
      int tagId,
    ...
    try {
      String sql =
          "INSERT INTO "
              + DbTag.getTaggedItemTableName(mbox)
              + "(mailbox_id, tag_id, item_id)"
              + " SELECT "
              ...
              + " AND "
              + DbUtil.whereIn(column, matches.size());
      if (checkDuplicates) {
        sql =
            sql
                + " AND NOT EXISTS(SELECT mailbox_id, tag_id, item_id FROM "
                + DbTag.getTaggedItemTableName(mbox)
      ...
      throw ServiceException.FAILURE(
          "creating rows in TAGGED_ITEM for tag/flag " + tagId + " in mbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }

  private static List<Long> getTagsets(DbConnection conn, Mailbox mbox, String column)
      throws ServiceException {
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
      ...
      throw ServiceException.FAILURE(
          "getting distinct values for column '" + column + "' in mbox " + mbox.getId(), e);
    } finally {
      DbPool.closeResults(rs);
      DbPool.closeStatement(stmt);
    }
  ...
  private static Map<Integer, String> createTagRows(
      DbConnection conn, Mailbox mbox, boolean checkDuplicates) throws ServiceException {
    PreparedStatement stmt = null;
    ResultSet rs = null;
    try {
      stmt =
          conn.prepareStatement(
              "SELECT id, name, mod_metadata, metadata FROM "
                  + DbMailItem.getMailItemTableName(mbox)
                  + " WHERE "
                  + DbMailItem.IN_THIS_MAILBOX_AND
                  + "type = "
      ...
      while (rs.next()) {
        ...
        Color color =
            Color.fromMetadata(
                new Metadata(rs.getString(4)).getLong(Metadata.FN_COLOR, MailItem.DEFAULT_COLOR));
        ...
        try {
          DbTag.createTag(
              ...
              mbox,
              data,
              color.getMappedColor() == MailItem.DEFAULT_COLOR ? null : color,
              true);
      ...
      stmt =
          conn.prepareStatement(
              "UPDATE "
                  + DbTag.getTagTableName(mbox)
                  + ", "
                  + "(SELECT ti.tag_id tid, COUNT(ti.item_id) count, "
                  + Db.clauseIFNULL("SUM(mi.unread)", "0")
                  + " unread_count"
                  + "  FROM "
                  + DbTag.getTaggedItemTableName(mbox, "ti")
                  + "  INNER JOIN "
  ...
  private static void populateTagNameColumn(
      DbConnection conn, Mailbox mbox, Map<Integer, String> tagNames, List<Long> tagsets)
      throws ServiceException {
    PreparedStatement stmt = null;
    try {
      ...
      throw ServiceException.FAILURE(
          "populating MAIL_ITEM.TAG_NAMES values in mbox " + mbox.getId(), e);
    } finally {
      DbPool.closeStatement(stmt);
    }
  }
