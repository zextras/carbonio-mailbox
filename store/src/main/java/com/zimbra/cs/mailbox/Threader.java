...
/**
 * Manages message threading into {@link Conversation}s based on the {@link Account} attribute
 * {@link Provisioning#A_zimbraMailThreadingAlgorithm}.
 *
 * <p>If their threading mode is {@link MailThreadingAlgorithm#none}, no conversation threading is
 * performed.
 *
 * <p>If their threading mode is {@link MailThreadingAlgorithm#subject}, the message will be
 * threaded based solely on its normalized subject.
 *
 * <p>If their threading mode is {@link MailThreadingAlgorithm#strict}, only the threading message
 * headers ({@code References}, {@code In-Reply-To}, {@code Message-ID}, and {@code
 * Resent-Message-ID}) are used to correlate messages. No checking of normalized subjects is
 * performed.
 *
 * <p>If their threading mode is {@link MailThreadingAlgorithm#references}, the same logic as {@link
 * MailThreadingAlgorithm#strict} applies with the constraints slightly loosened so that the
 * non-standard {@code Thread-Index} header is considered when threading messages and that a reply
 * message lacking {@code References} and {@code In-Reply-To} headers will fall back to using
 * subject-based threading.
 *
 * <p>If their threading mode is {@link MailThreadingAlgorithm#subjrefs}, the same logic as {@link
 * MailThreadingAlgorithm#references} applies with the caveat that modifications to the subject will
 * break a thread in two.
 */
public final class Threader {
  ...
  /**
   * Retrieves the current algorithm used for threading new messages. Threading can be disabled on a
   * system-wide basis via the localconfig key {@code debug_disable_conversation}. If threading has
   * not been disabled, the threading mode is retrieved from the {@link Account} attribute {@link
   * Provisioning#A_zimbraMailThreadingAlgorithm}. If this attribute is unset, we default to {@link
  ...
  /**
   * Returns the SHA-1 hashes for all of the {@code Threader}'s message's threading header values.
   * This always includes the contents of its standard threading headers (the message-ids from
   * {@code Messsage-ID}, {@code In-Reply-To}, and {@code References}). If the threading mode is not
   * {@link MailThreadingAlgorithm#strict}, we also include hashes based on the non-standard {@code
   * Thread-Index} header, useful for interoperability with Microsoft Outlook and siblings. We
   * prepend two control characters to these values before generating the hashes to avoid
   * OPEN_CONVERSATION conflicts with hashed normalized subjects. If cross-account threading is
   * disabled, we also prepend the data source ID of the message.
   *
   * @param includeParents If {@code true}, includes the hashes for all threading headers in the
   *     returned {@code List}. If {@code false}, hashes only the {@code Message-ID} header and
   *     (optionally) {@code Thread-Index}.
  ...
  private List<String> getReferenceHashes(boolean includeParents) throws ServiceException {
    ...
    Collection<String> references =
        includeParents
            ? pm.getAllReferences()
            : Mime.getReferences(pm.getMimeMessage(), "Message-ID");
    for (String reference : references) {
      hashes.add(
          Mailbox.getHash("\u0001\u0002" + prependDataSource(Strings.nullToEmpty(reference))));
    }
  ...
  /**
   * Returns the SHA-1 hashes for the {@code Threader}'s message's {@code Thread-Index} header and
   * those derived from it for all its parents. We prepend two control characters to these values
   * before generating the hashes to avoid OPEN_CONVERSATION conflicts with hashed normalized
   * subjects.
   *
   * @param includeParents If {@code true}, includes the hashes for parent messages in the returned
   *     {@code List}. If {@code false}, returns only this message's hash.
   * @throws ServiceException
   * @see ThreadIndex
   */
  private List<String> getThreadIndexHashes(boolean includeParents) throws ServiceException {
    ...
    if (tindex == null) return Collections.emptyList();
  ...
  public static class ThreadIndex {
    /**
     * Generates a new {@code Thread-Index} value suitable for adding as a header for a message that
     * is not a reply to a parent containing a {@code Thread-Index} header. This value is a
     * base64-encoded 22-byte random value and has no relation to the message.
     */
    ...
    /**
     * Generates a new {@code Thread-Index} value suitable for adding as a header for a reply to a
     * parent containing a {@code Thread-Index} header. This value is created by appending 5 random
     * bytes to the end of the parent's decoded {@code Thread-Index} value.
     */
    ...
    /**
     * Parses the contents of a {@code Thread-Index} header and returns the base64-decoded
     * thread-index {@code byte} array. Such arrays must be of length 22, 27, 32, 37, 42, etc.; if
     * the decoded array does not match the pattern, {@code null} is returned instead.
     */
    ...
    /**
     * Generates a new {@code Thread-Topic} value suitable for adding as a header for a message that
     * is not a reply to a parent containing a {@code Thread-Topic} header. This value is the
     * normalized version of the message's {@code Subject} header (i.e. with all prefixes and
     * trailers stripped).
     *
  ...
  /**
   * Returns whether the regular old-message mailbox purge task should also trim old entries from
   * the OPEN_CONVERSATION table for the given {@code Mailbox}. In general, when we thread using
   * references as a criterion, we don't want to remove older entries.
   */
  ...
  /**
   * Searches for an existing {@code Conversation} matching the message being threaded. If the
   * user's threading mode is {@code subject}, this method always returns an empty list. If the
   * user's threading mode is {@code subject}, only the normalized subject is used for threading and
   * only one {@code Conversation} will ever be returned. For all other threading modes, 0, 1 or
   * multiple {@code Conversation}s may be returned; in the multi-{@code Conversation} case, the
   * caller is responsible for choosing one or merging them all.
   */
  List<Conversation> lookupConversation() throws ServiceException {
    if (matchedConversations == null) {
      ...
      ZimbraLog.mailbox.debug(
          "  threading message \"%s\" (%s)", pm.getSubject(), pm.getMessageID());
      ...
      if (matches.isEmpty()
          && (mode.isSubject() || (!mode.isStrict() && isReplyWithoutReferences()))) {
  ...
  /**
   * Clears cached threader results. Matching conversations will be recalculated the next time
   * {@link #lookupConversation()} is called.
   */
  void reset() {
  ...
  /**
   * Returns whether the message being threaded has a subject that looks like a reply but has no
   * {@code In-Reply-To} or {@code References} header. Several popular mailers do this, including at
   * least some versions of Outlook and Lotus Notes.
   */
  private boolean isReplyWithoutReferences() {
    ...
    return Mime.getReferences(mm, "In-Reply-To").isEmpty()
        && Mime.getReferences(mm, "References").isEmpty();
  }

  /**
   * Searches the {@code OPEN_CONVERSATION} table for existing {@code Conversation}s matching this
   * new message's threading headers. We thread on {@code References}, {@code In-Reply-To}, {@code
   * Message-ID} (which also correlates mailing list duplicates), and {@code Resent-Message-ID}. If
   * the user's threading algorithm is not {@link MailThreadingAlgorithm#strict}, we also thread on
   * {@code Thread-Index}.
   *
    ...
    if (mode.isSubjrefs()) {
      ...
      for (int i = matches.size() - 1; i >= 0; i--) {
        ...
        if (!pm.getNormalizedSubject().equals(hit.getNormalizedSubject())) {
          ...
          matches.remove(i);
          // need to trim refHashes so as not to overwrite the hashes that mapped to the other
          // subject's thread
          // FIXME: this trimming may be a little too aggressive, may miss some out-of-order
          // delivery cases
  ...
  /**
   * The number of milliseconds of inactivity (i.e. time since last message receipt) after which a
  ...
  /**
   * The number of milliseconds of inactivity (i.e. time since last message receipt) after which a
   * non-reply is not grouped with an existing conversation with the same subject.
   */
  private static final long CONVERSATION_NONREPLY_WINDOW = 2 * Constants.MILLIS_PER_DAY;

  /**
   * The maximum size for a conversation beyond which non-reply messages are not grouped with it,
   * even if their delivery time is within {@link #CONVERSATION_NONREPLY_WINDOW}.
   */
  private static final int CONVERSATION_NONREPLY_SIZE_LIMIT = 50;

  /**
   * Searches the {@code OPEN_CONVERSATION} table for existing {@code Conversation}s matching this
   * new message's normalized subject. A potential match may be discarded as stale if its latest
   * message was added too long ago, or may be discarded if the new message is not a reply and the
   * match is already too large.
   *
  ...
  private List<Conversation> lookupBySubject() throws ServiceException {
    ...
    // the caller can specify the received date via ParsedMessge constructor or X-Zimbra-Received
    // header
  ...
  /**
   * Updates the {@code OPEN_CONVERSATION} database table to force all of the new message's hashes
   * to point at its {@code Conversation}. This includes the subject hash (if the user's threading
   * algorithm is not {@link MailThreadingAlgorithm#strict}) and the reference hashes (if the
   * threading algorithm isn't {@link MailThreadingAlgorithm#subject}). If threading is disabled for
   * the {@code Mailbox}, this method does nothing.
   *
  ...
  /**
   * Updates the {@code OPEN_CONVERSATION} database table to redirect any threading hashes currently
   * pointing at the {@code Message} to instead refer to the given {@code Conversation}.
   */
  void changeThreadingTargets(Message msg, Conversation conv) throws ServiceException {
    if (conv != null && msg != null && isEnabled() && !mode.isSubject()) {
      ...
      ZimbraLog.mailbox.debug(
          "  transferred hashes from message %d to conv %d", msg.getId(), conv.getId());
    }
  }
