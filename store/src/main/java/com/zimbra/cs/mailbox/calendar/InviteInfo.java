 ...
 * ZCS-3055 - The latter restriction could be violated for pseudo exceptions created as a result of
 * replies to individual instances.
 */
public class InviteInfo implements Comparable<InviteInfo>, Cloneable {
  ...
  /**
   * ID of the MESSAGE which this invite was originally encoded in or a unique pseudo item ID
   * associated with a component. (ZWC expects each exception to have a different mMsgId)
   */
  private int mMsgId;
  ...
  public int compareTo(InviteInfo other) {
    ...
    if (toRet == 0) {
      ...
      if (toRet == 0) {
        ...
        if (toRet == 0) toRet = StringUtil.compareTo(mMethod, other.mMethod);
      }
    }
    return toRet;
  }
