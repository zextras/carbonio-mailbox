...
public class Document extends MailItem {
    ...
    try {
      ...
      if (mblob == null) {
        ZimbraLog.index.warn(
            "Unable to fetch blob for Document id=%d,ver=%d,vol=%s", mId, mVersion, getLocator());
      ...
      ZimbraLog.index.warn(
          "Error generating index data for Wiki Document " + getId() + ". Item will not be indexed",
          e);
      return new ArrayList<IndexDocument>(0);
  ...
  public void reanalyze(Object obj, long newSize) throws ServiceException {
    ...
    if (!LC.documents_disable_instant_parsing.booleanValue()) fragment = pd.getFragment();
  ...
  protected static UnderlyingData prepareCreate(
      MailItem.Type type,
      int id,
      String uuid,
      Folder folder,
    ...
    return prepareCreate(
        ...
        custom,
        flags,
        LC.documents_disable_instant_parsing.booleanValue());
  }
  ...
  protected static UnderlyingData prepareCreate(
    ...
    data.metadata =
        encodeMetadata(
                ...
                mimeType,
                pd.getCreator(),
                skipParsing ? null : pd.getFragment(),
                null,
                0,
                pd.getDescription(),
                pd.isDescriptionEnabled(),
                null)
            .toString();
    data.setFlags(flags);
    return data;
  ...
  static Document create(
    ...
    Mailbox mbox = folder.getMailbox();
    UnderlyingData data =
        prepareCreate(Type.DOCUMENT, id, uuid, folder, filename, type, pd, null, custom, flags);
    if (parent != null) {
    ...
    ZimbraLog.mailop.info(
        "Adding Document %s: id=%d, folderId=%d, folderName=%s",
  ...
  void rename(String name, Folder target) throws ServiceException {
    ...
    if (!oldName.equalsIgnoreCase(name)) mMailbox.index.add(this);
  }
  ...
  void unlock(Account authuser) throws ServiceException {
    ...
    if (!lockOwner.equalsIgnoreCase(authuser.getId())
