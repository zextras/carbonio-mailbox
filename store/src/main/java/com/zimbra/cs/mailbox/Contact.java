...
public class Contact extends MailItem {
  ...
  public static class Attachment implements DataSource {
    ...
    /**
     * Returns an <tt>InputStream</tt> to this attachment's content, or <tt>null</tt> if there is no
     * content.
     */
    @Override
    ...
    Metadata asMetadata() {
      return new Metadata()
          .put(FN_SIZE, mSize)
          .put(FN_NAME, getFilename())
          .put(FN_PART, mPartName)
          .put(FN_CTYPE, getContentType())
          .put(FN_FIELD, mFieldName);
    }

    @Override
    public String toString() {
      return new StringBuilder(mFieldName)
          .append(" [")
  ...
  // IMPORTANT NOTE - does not include the Contact Group 'dlist' entry, which is a multi-value entry
  // (comma-separated)
   ...
   *
   * <p>This gets called whenever a Contact object is constructed. With the high call rate, we cache
   * it on the account to avoid repeated computing from the attr value.
   *
   * @param acct
   * @return
   */
  public static String[] getEmailFields(Account acct) {
    ...
    if (emailFields == null) {
      ...
      if (emailFieldsStr != null) fields = emailFieldsStr.split(",");

      if (fields != null) {
        // remove dlist if it is there
        List<String> temp = new ArrayList<String>(fields.length);
        for (String field : fields) {
          if (!ContactConstants.A_dlist.equals(field)) temp.add(field);
        }
        if (temp.size() > 0) emailFields = temp.toArray(new String[0]);
      }

      if (emailFields == null) emailFields = EMAIL_FIELDS;
  ...
  public Map<String, String> getFields() {
    ...
    try {
      String hiddenAttrList =
          Provisioning.getInstance().getLocalServer().getContactHiddenAttributes();
   ...
   *
   * <p>If the contact has no attachments in its blob, an empty list is returned.
   */
   ...
   *
   * @throws ServiceException if no <tt>MimeMessage</tt> exists or there was an error retrieving it
   */
   ...
   *
   * <p>This value is derived by using the {@code fileAs} contact field value to select a standard
   * or custom formatting to apply to the contact's fields. Supported {@code fileAs} values are:
   *
   * <ul>
   ...
   * </ul>
   *
   * When a {@code fileAs} value is not specified, {@code 1} is used as the default.
   */
  ...
  private static String getFileAsString(Map<String, String> fields, boolean phonetic)
    ...
    switch (fileAsInt) {
      ...
      case ContactConstants.FA_LAST_C_FIRST:
        result.append(last);
        if (first.length() > 0 && last.length() > 0) result.append(", ");
        result.append(first);
        break;
      ...
      case ContactConstants.FA_LAST_C_FIRST_COMPANY:
        ...
        if (first.length() > 0 && last.length() > 0) result.append(", ");
        result.append(first);
        if (company.length() > 0) result.append(" (").append(company).append(')');
        break;
  ...
  public static void normalizeFileAs(Map<String, String> attrs) {
    ...
    if (fileAs == null || fileAs.trim().length() == 0) return;
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ContactConstants.A_fileAs, Integer.valueOf(ContactConstants.FA_LAST_C_FIRST).toString());
      return;
    }
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ContactConstants.A_fileAs, Integer.valueOf(ContactConstants.FA_FIRST_LAST).toString());
      return;
    }
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ...
          Integer.valueOf(ContactConstants.FA_LAST_C_FIRST_COMPANY).toString());
      return;
    }
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ContactConstants.A_fileAs,
          Integer.valueOf(ContactConstants.FA_FIRST_LAST_COMPANY).toString());
      return;
    }
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ContactConstants.A_fileAs,
          Integer.valueOf(ContactConstants.FA_COMPANY_LAST_C_FIRST).toString());
      return;
    }
    ...
    if (sb.toString().equals(fileAs)) {
      attrs.put(
          ContactConstants.A_fileAs,
          Integer.valueOf(ContactConstants.FA_COMPANY_FIRST_LAST).toString());
      return;
    }
    ...
    attrs.put(
        ContactConstants.A_fileAs,
        Integer.valueOf(ContactConstants.FA_EXPLICIT).toString() + ':' + fileAs);
  }

  /** Returns a list of all email address fields for this contact. */
  public List<String> getEmailAddresses() {
    return getEmailAddresses(
        emailFields, fields, DerefGroupMembersOption.INLINE_ONLY, getMailbox().getAccountId());
  }
  ...
  public static final boolean isEmailField(String[] emailFields, String fieldName) {
    ...
    for (String e : emailFields) {
      if (lcField.equals(e)) return true;
    }
    if (lcField.equals(ContactConstants.A_dlist)) return true;
    return false;
  }

  public static final List<String> getEmailAddresses(
      String[] fieldNames,
      Map<String, String> fields,
      DerefGroupMembersOption derefGroupMemberOpt,
      String ownerAcctId) {
   ...
   *
   * <p>A real nonnegative item ID must be supplied from a previous call to {@link
   ...
   *     <ul>
   *       <li><tt>mail.CANNOT_CONTAIN</tt> - if the target folder can't contain contacts
   *       <li><tt>service.INVALID_REQUEST</tt> - if no fields are specified for the contact
  ...
  static Contact create(
    ...
    data.setTags(ntags);
    data.metadata =
        encodeMetadata(DEFAULT_COLOR_RGB, 1, 1, custom, pc.getFields(), pc.getAttachments());
    ...
    if (ZimbraLog.mailop.isInfoEnabled()) {
      ...
      ZimbraLog.mailop.info(
          "adding contact %s: id=%d, folderId=%d, folderName=%s.",
  ...
  MailboxBlob setContent(StagedBlob staged, Object content) throws ServiceException, IOException {
    ZimbraLog.mailop.info(
        "modifying contact %s: id=%d, folderId=%d, folderName=%s.",
  ...
  void reanalyze(Object data, long newSize) throws ServiceException {
    ...
    if (attachments != null && attachments.size() > 0) {
      for (Attachment attach : attachments) {
        // refresh the data handler. the content rev changes which can change the blob
        // filename/locator.
  ...
  Metadata encodeMetadata(Metadata meta) {
    return encodeMetadata(
        meta, mRGBColor, mMetaVersion, mVersion, mExtendedData, fields, attachments);
  }
  ...
  private static String encodeMetadata(
    ...
    return encodeMetadata(
            new Metadata(), color, metaVersion, version, extended, fields, attachments)
        .toString();
  }
  ...
  private static Set<String> SMIME_FIELDS =
      ImmutableSet.of(ContactConstants.A_userCertificate, ContactConstants.A_userSMIMECertificate);
