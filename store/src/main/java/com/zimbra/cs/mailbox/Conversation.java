...
public class Conversation extends MailItem {
  ...
  Conversation(Mailbox mbox, UnderlyingData data, boolean skipCache) throws ServiceException {
    ...
    if (mData.type != Type.CONVERSATION.toByte()
        && mData.type != Type.VIRTUAL_CONVERSATION.toByte()) {
      throw new IllegalArgumentException();
    }
  }

  /**
   * Returns the normalized subject of the conversation. This is done by taking the {@code Subject:}
   * header of the first message and removing prefixes (e.g. {@code "Re:"}) and suffixes (e.g.
   * {@code "(fwd)"}) and the like.
   *
  ...
  /**
   * Returns the number of messages in the conversation, as calculated from its list of children as
   * fetched from the database. This <u>should</u> always be equal to {@link MailItem#getSize()}; if
   * it isn't, an error has occurred and been persisted to the database.
   */
  ...
  void instantiateSenderList() {
    if (mSenderList != null && mSenderList.size() == mData.size) return;
  ...
  /**
   * Makes certain the Conversation's {@link SenderList} is loaded and valid. If it's not, first
   * tries to instantiate it from the loaded metadata. If that's either missing or invalid,
   * recalculates the Conversation's metadata from its constituent messages and rewrites the
   * database row.
   *
   * @return <tt>true</tt> if the metadata was recalculated, <tt>false</tt> if the SenderList was
   *     generated from existing data
   */
  boolean loadSenderList() throws ServiceException {
    instantiateSenderList();
    if (mSenderList != null) return false;
  ...
  private static final int RECALCULATE_CHANGE_MASK =
      Change.TAGS | Change.FLAGS | Change.SENDERS | Change.SIZE | Change.UNREAD | Change.METADATA;
  ...
  SenderList recalculateMetadata(List<Message> msgs) throws ServiceException {
    ...
    ZimbraLog.mailbox.debug(
        "resetting metadata: cid=%d,size was=%d is=%d", mId, mData.size, mSenderList.size());
    saveData(new DbMailItem(mMailbox));
    return mSenderList;
  }

  /**
   * Returns all the {@link Message}s in this conversation. The messages are fetched from the {@link
   * Mailbox}'s cache, if possible; if not, they're fetched from the database. The returned messages
   * are not guaranteed to be sorted in any way.
   */
  ...
  /**
   * Returns all the {@link Message}s in this conversation. The messages are fetched from the {@link
   * Mailbox}'s cache, if possible; if not, they're fetched from the database.
   *
  ...
  void addChild(MailItem child) throws ServiceException {
    ...
    if (msg.isUnread()) {
      ...
      updateUnread(
          ...
          child.isTagged(Flag.FlagInfo.DELETED) ? child.mData.unreadCount : 0);
    }
  ...
  void merge(Conversation other) throws ServiceException {
    ...
    // delete the old conversation (must do this after moving the messages because of cascading
    // delete)
  ...
  void decodeMetadata(String metadata) {
    // when a folder is deleted, DbMailItem.markDeletionTargets() nulls out metadata for all
  ...
  static String encodeMetadata(
      Color color, int metaVersion, int version, CustomMetadataList extended, SenderList senders) {
    return encodeMetadata(new Metadata(), color, metaVersion, version, extended, senders.toString())
        .toString();
  }
