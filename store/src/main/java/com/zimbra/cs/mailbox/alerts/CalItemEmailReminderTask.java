...
public class CalItemEmailReminderTask extends CalItemReminderTaskBase {
  ...
  protected void sendReminder(CalendarItem calItem, Invite invite) throws Exception {
    ...
    if (to == null) {
      ZimbraLog.scheduler.info(
          "Unable to send calendar reminder email since %s is not set",
    ...
    mm.setSubject(
        L10nUtil.getMessage(
            calItem.getType() == MailItem.Type.APPOINTMENT
                ? L10nUtil.MsgKey.apptReminderEmailSubject
    ...
    if (invite.getDescriptionHtml() == null) {
      ...
      htmlPart.setContent(
          getBody(calItem, invite, true, locale, tz),
  ...
  private String getBody(
      CalendarItem calItem, Invite invite, boolean html, Locale locale, TimeZone tz)
    ...
    if (calItem.getType() == MailItem.Type.APPOINTMENT) {
      ...
      formattedEnd =
          onlyDateFormat.format(start).equals(onlyDateFormat.format(end))
              ? onlyTimeFormat.format(end)
    ...
    return html
        ? L10nUtil.getMessage(
            calItem.getType() == MailItem.Type.APPOINTMENT
                ? L10nUtil.MsgKey.apptReminderEmailBodyHtml
        ...
        : L10nUtil.getMessage(
            calItem.getType() == MailItem.Type.APPOINTMENT
                ? L10nUtil.MsgKey.apptReminderEmailBody
