...
public class ZOrganizer extends CalendarUser {
  ...
  public CalOrganizer toJaxb() {
    ...
    if (hasCn()) org.setDisplayName(getCn());
    if (hasSentBy()) org.setSentBy(getSentBy());
    if (hasDir()) org.setDir(getDir());
    if (hasLanguage()) org.setLanguage(getLanguage());
  ...
  public Element toXml(Element parent) {
    ...
    if (hasCn()) orgElt.addAttribute(MailConstants.A_DISPLAY, getCn());
    if (hasSentBy()) orgElt.addAttribute(MailConstants.A_CAL_SENTBY, getSentBy());
    if (hasDir()) orgElt.addAttribute(MailConstants.A_CAL_DIR, getDir());
    if (hasLanguage()) orgElt.addAttribute(MailConstants.A_CAL_LANGUAGE, getLanguage());
