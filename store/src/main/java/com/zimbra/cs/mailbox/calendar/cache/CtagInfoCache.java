...
public class CtagInfoCache {
  ...
  private CtagInfo get(CalendarKey key, boolean wasMountpoint) throws ServiceException {
    ...
    if (info != null && !info.isMountpoint()) return info;
      ...
      if (info.isMountpoint() && !wasMountpoint) {
        ...
        if (infoTarget != null) {
          ...
          ZimbraLog.calendar.warn(
              "Mounpoint target %s:%d not found during cache lookup",
              keyTarget.getAccountId(), keyTarget.getFolderId());
        }
      }
      if (needToPut) mMemcachedLookup.put(key, info);
    }
    return info;
  }
  ...
  private CtagInfo getFolder(CalendarKey key) throws ServiceException {
    ...
    if (Provisioning.getInstance().onLocalServer(acct)) {
      ...
      if (folder != null) calInfo = new CtagInfo(folder);
        ...
        ZimbraLog.calendar.warn(
            "Invalid folder %d in account %s during cache lookup", folderId, accountId);
    } else {
      ...
      if (zfolder != null) calInfo = new CtagInfo(zfolder);
      else
        ZimbraLog.calendar.warn(
            "Invalid folder %d in account %s during cache lookup", folderId, accountId);
    }
