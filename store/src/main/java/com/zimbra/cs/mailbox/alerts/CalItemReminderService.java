...
public class CalItemReminderService extends MailboxListener {
    ...
    if (notification.mods.created != null) {
      for (Map.Entry<ModificationKey, BaseItemInfo> entry : notification.mods.created.entrySet()) {
        ...
        if (item instanceof CalendarItem) {
          ...
          ZimbraLog.scheduler.debug(
              "Handling creation of calendar item (id=%s,mailboxId=%s)",
    ...
    if (notification.mods.modified != null) {
      for (Map.Entry<ModificationKey, Change> entry : notification.mods.modified.entrySet()) {
        ...
        if (change.what instanceof CalendarItem) {
          ...
          ZimbraLog.scheduler.debug(
              "Handling modification of calendar item (id=%s,mailboxId=%s)",
              calItem.getId(), calItem.getMailboxId());
          ...
          try {
            mbox =
                MailboxManager.getInstance()
                    .getMailboxByAccount(account, MailboxManager.FetchMode.DO_NOT_AUTOCREATE);
          } catch (ServiceException e) {
            ZimbraLog.scheduler.error(
                "Error looking up the mailbox of account %s", account.getId(), e);
          }
          if (mbox != null) {
  ...
  static boolean cancelExistingReminders(int calItemId, int mailboxId) {
    try {
      ScheduledTaskManager.cancel(
          CalItemEmailReminderTask.class.getName(),
          CalItemEmailReminderTask.TASK_NAME_PREFIX + Integer.toString(calItemId),
          mailboxId,
          false);
      ScheduledTaskManager.cancel(
          CalItemSmsReminderTask.class.getName(),
  ...
  private static void scheduleReminder(
    ...
    reminderTask.setProperty(
        CalItemReminderTaskBase.CAL_ITEM_ID_PROP_NAME, Integer.toString(calItem.getId()));
    reminderTask.setProperty(
        CalItemReminderTaskBase.INV_ID_PROP_NAME, Integer.toString(alarmData.getInvId()));
    reminderTask.setProperty(
        CalItemReminderTaskBase.COMP_NUM_PROP_NAME, Integer.toString(alarmData.getCompNum()));
    reminderTask.setProperty(
        CalItemReminderTaskBase.NEXT_INST_START_PROP_NAME,
        Long.toString(alarmData.getNextInstanceStart()));
