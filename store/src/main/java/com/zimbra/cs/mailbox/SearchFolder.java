...
public final class SearchFolder extends Folder implements SearchFolderStore {
  ...
  public SearchFolder(Mailbox mbox, UnderlyingData data, boolean skipCache)
  ...
  /**
   * Returns the set of item types returned by this search, or <code>""</code> if none were
   * specified.
   */
  @Override
  ...
  /**
   * Returns whether this search folder is visible through the IMAP interface. At present, this is
   * completely controlled by the COS attribute <code>zimbraPrefImapSearchFoldersEnabled</code>.
   */
  public boolean isImapVisible() {
    try {
      return mMailbox
          .getAccount()
          .getBooleanAttr(Provisioning.A_zimbraPrefImapSearchFoldersEnabled, true);
  ...
  /**
   * Returns whether the folder can contain objects of the given type. Search folders may only
   * contain other search folders.
   */
  @Override
  ...
  /**
   * Creates a new SearchFolder and persists it to the database. A real nonnegative item ID must be
   * supplied from a previous call to {@link Mailbox#getNextItemId(int)}.
   *
   ...
   *     <ul>
   *       <li><code>mail.CANNOT_CONTAIN</code> - if the target folder can't contain search folders
   *       <li><code>mail.ALREADY_EXISTS</code> - if a folder by that name already exists in the
   *           parent folder
   *       <li><code>mail.INVALID_NAME</code> - if the new folder's name is invalid
   *       <li><code>mail.INVALID_REQUEST</code> - if the supplied query string is blank or missing
   *       <li><code>service.FAILURE</code> - if there's a database failure
   *       <li><code>service.PERM_DENIED</code> - if you don't have sufficient permissions
   *     </ul>
   *
  ...
  static SearchFolder create(
    ...
    if (!parent.canAccess(ACL.RIGHT_INSERT)) {
      throw ServiceException.PERM_DENIED(
          "you do not have sufficient permissions on the parent folder");
    }
    ...
    ZimbraLog.mailop.info(
        "Adding SearchFolder %s: id=%d, parentId=%d, parentName=%s.",
  ...
  /**
   * Replaces the search folder's query and attributes with a new set. Persists the updated version
   * to the cache and to the database. Omitting the query is not permitted; omitting attributes
   * causes the search to use the default <code>types</code> and <code>sort</code>.
   *
   ...
   * @throws ServiceException The following error codes are possible:
   *     <ul>
   *       <li><code>mail.IMMUTABLE_OBJECT</code> - if the search folder cannot be modified
   *       <li><code>mail.INVALID_REQUEST</code> - if the supplied query string is blank or missing
   *       <li><code>service.FAILURE</code> - if there's a database failure
   *       <li><code>service.PERM_DENIED</code> - if you don't have sufficient permissions
   *     </ul>
  ...
  void changeQuery(String query, String types, String sort) throws ServiceException {
    ...
    if (!canAccess(ACL.RIGHT_WRITE)) {
      throw ServiceException.PERM_DENIED(
          "you do not have sufficient permissions on the search folder");
    }
    query = validateQuery(query);
  ...
  /**
   * Cleans up the provided query string and verifies that it's not blank. Removes all non-XML-safe
   * control characters and trims leading and trailing whitespace.
   *
   * @param query The query string.
   * @return The cleaned-up query string.
   * @throws ServiceException The following error codes are possible:
   *     <ul>
   *       <li><code>mail.INVALID_REQUEST</code> - if the cleaned-up query string is blank or
   *           missing
   *     </ul>
   *
  ...
  protected static String validateQuery(String query) throws ServiceException {
    if (query != null) query = StringUtil.stripControlCharacters(query).trim();
  ...
  Metadata encodeMetadata(Metadata meta) {
    return encodeMetadata(
        meta, mRGBColor, mMetaVersion, mVersion, mExtendedData, mQuery, mTypes, mSort);
  }
  ...
  private static String encodeMetadata(
    ...
    return encodeMetadata(new Metadata(), color, metaVersion, version, extended, query, types, sort)
        .toString();
  }
