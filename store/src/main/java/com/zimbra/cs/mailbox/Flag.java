...
public final class Flag extends Tag {
  ...
  public enum FlagInfo {
    ...
    /**
     * Callers of {@link Mailbox} methods treat {@code UNREAD} like the other flags. Internally, we
     * break it out into a separate variable, {@code unreadCount}. It's also persisted in a separate
     * indexed column for fast lookups of unread {@link MailItem}s.
     */
    UNREAD(-10, "\\Unread", 'u'),
  ...
  @Deprecated public static final int ID_INDEXING_DEFERRED = FlagInfo.INDEXING_DEFERRED.id;
  ...
  @Deprecated public static final int ID_ARCHIVED = FlagInfo.ARCHIVED.id;
  ...
  @Deprecated public static final int BITMASK_ARCHIVED = FlagInfo.ARCHIVED.bitmask;
  ...
  public static final int FLAGS_SYSTEM =
      BITMASK_FROM_ME
          ...
          | BITMASK_COPIED
          | BITMASK_DRAFT
          | BITMASK_HIGH_PRIORITY
          | BITMASK_LOW_PRIORITY
          | BITMASK_VERSIONED
          | BITMASK_INDEXING_DEFERRED
          | BITMASK_INVITE
          | BITMASK_ARCHIVED
          | BITMASK_GLOBAL
          | BITMASK_IN_DUMPSTER
  ...
  public static final int FLAGS_FOLDER =
      BITMASK_SUBSCRIBED
          | BITMASK_EXCLUDE_FREEBUSY
          | BITMASK_CHECKED
          | BITMASK_NO_INHERIT
          | BITMASK_SYNCFOLDER
          | BITMASK_SYNC
          | BITMASK_NO_INFERIORS
          | BITMASK_GLOBAL;
  ...
  public static final int FLAGS_ALL =
      (FLAGS_FOLDER | FLAGS_MESSAGE | FLAGS_CALITEM | FLAGS_GENERIC) & ~BITMASK_UNREAD;
  ...
  /**
   * Returns the "external" flag bitmask for the given flag string, which includes {@link
   * Flag#BITMASK_UNREAD}.
   */
  ...
  /**
   * Returns the {@link Flag} with the given id, or {@code null} if there is no {@link Flag} with
   * that id.
   */
  ...
  /**
   * Returns the {@link Flag} with the given name, or {@code null} if there is no {@link Flag} with
   * that name.
   *
   * @param mbox mailbox
