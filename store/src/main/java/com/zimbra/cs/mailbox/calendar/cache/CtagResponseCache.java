...
public class CtagResponseCache {
  ...
  public static class CtagResponseCacheValue {
    ...
    private Map<Integer /* folder id */, String /* ctag */>
        mCtags; // snapshot of ctags at response cache time

    public CtagResponseCacheValue(
        byte[] respBody,
        int rawLen,
        boolean gzipped,
        String calListVer,
        Map<Integer, String> ctags) {
      mRespBody = respBody;
      if (gzipped) {
        mRawLen = rawLen;
        mGzipped = gzipped;
    ...
    Metadata encodeMetadata() throws ServiceException {
      ...
      if (mGzipped) meta.put(FN_IS_GZIPPED, true);
    ...
    CtagResponseCacheValue(Metadata meta) throws ServiceException {
      ...
      if (body.length() != bodyLen)
        throw ServiceException.FAILURE(
            "Ctag response body has wrong length: " + body.length() + " when expecting " + bodyLen,
            null);
      try {
      ...
      if (numCtags > 0) {
        for (int i = 0; i < numCtags; ++i) {
          ...
          if (calId != -1 && ctag != null) mCtags.put(calId, ctag);
          else break;
        }
      }
    }
