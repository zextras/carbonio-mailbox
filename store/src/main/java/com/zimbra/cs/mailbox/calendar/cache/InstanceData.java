...
public class InstanceData {
  ...
  protected void clearUnchangedFields(InstanceData other) {
    if (other != null) {
      ...
      if (Util.sameValues(mDuration, other.getDuration())) mDuration = null;
      if (Util.sameValues(mAlarmAt, other.getAlarmAt())) mAlarmAt = null;
      if (Util.sameValues(mTZOffset, other.getTZOffset())) mTZOffset = null;
      if (Util.sameValues(mPartStat, other.getPartStat())) mPartStat = null;
      if (Util.sameValues(mFreeBusyActual, other.getFreeBusyActual())) mFreeBusyActual = null;
      if (Util.sameValues(mPercentComplete, other.getPercentComplete())) mPercentComplete = null;
    }
  }
  ...
  InstanceData(Metadata meta) throws ServiceException {
    ...
    if (meta.containsKey(FN_DTSTART)) dtStart = Long.valueOf(meta.getLong(FN_DTSTART));
    if (meta.containsKey(FN_DURATION)) duration = Long.valueOf(meta.getLong(FN_DURATION));
    if (meta.containsKey(FN_ALARM_AT)) alarmAt = Long.valueOf(meta.getLong(FN_ALARM_AT));
    if (meta.containsKey(FN_TZOFFSET)) tzOffset = Long.valueOf(meta.getLong(FN_TZOFFSET));
  ...
  Metadata encodeMetadata() {
    ...
    if (mDtStart != null) meta.put(FN_DTSTART, mDtStart.longValue());
    if (mDuration != null) meta.put(FN_DURATION, mDuration.longValue());
    if (mAlarmAt != null) meta.put(FN_ALARM_AT, mAlarmAt.longValue());
    if (mTZOffset != null) meta.put(FN_TZOFFSET, mTZOffset.longValue());
