...
public class ShareStartStopListener extends MailboxListener {
  ...
  private static final Set<MailItem.Type> registeredTypes =
      Collections.singleton(MailItem.Type.FOLDER);
    ...
    if (notification.mods.modified != null) {
      ...
      for (Change change : notification.mods.modified.values()) {
        if ((change.why & Change.ACL) != 0
            && change.preModifyObj instanceof Folder
            && change.what instanceof Folder) {
  ...
  private void startShare(Folder folder) {
          ...
          public void run() {
            try {
              ...
              if (shloc == null) {
                ...
                // Get the latest value from ldap master to avoid race condition during share
                // relocation.
    ...
    Zimbra.sTimer.schedule(
        t, 0); // run in separate thread to avoid ldap communication inside mailbox lock
  }
