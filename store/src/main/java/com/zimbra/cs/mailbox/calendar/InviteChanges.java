...
public class InviteChanges {
  ...
  private static void map(int mask, String label) {
    ...
    if (!labelLower.equals(label)) sLabelToMask.put(labelLower, mask);
  }
  ...
  private boolean changed(int bitmask) {
    return (mChanges & bitmask) != 0;
  }
  ...
  /**
   * Returns true if the change is significant enough to invalidate earlier replies. Changes to
   * meeting location, time, and/or recurrence warrant a new reply. See Section 2.1.4 Component
   * Revisions in RFC5546 (iTIP) for more info.
   *
   * @return
   */
  ...
  /**
   * Returns true if the change is for a recurrence series and it causes exception instances to be
   * removed. Includes changes to time or recurrence.
   *
   * @return
   */
  ...
  private void diffInvites(Invite inv1, Invite inv2) {
    // Subject
    if (!StringUtil.equal(inv1.getName(), inv2.getName())) mChanges |= SUBJECT;

    // LOCATION
    if (!StringUtil.equal(inv1.getLocation(), inv2.getLocation())) mChanges |= LOCATION;

    // DTSTART
    boolean dtStartChanged;
    ParsedDateTime dtStart1 = inv1.getStartTime();
    ParsedDateTime dtStart2 = inv2.getStartTime();
    if (dtStart1 == null || dtStart2 == null) dtStartChanged = dtStart1 != dtStart2;
    else
      dtStartChanged =
          !StringUtil.equal(
              dtStart1.getDateTimePartString(false), dtStart2.getDateTimePartString(false));
    if (dtStartChanged) {
      mChanges |= TIME;
    } else {
      // DTEND
      ...
      if (dtEnd1 == null || dtEnd2 == null) dtEndChanged = dtEnd1 != dtEnd2;
      else
        dtEndChanged =
            !StringUtil.equal(
                dtEnd1.getDateTimePartString(false), dtEnd2.getDateTimePartString(false));
      if (dtEndChanged) {
        mChanges |= TIME;
      } else {
        ...
        if (dur1 == null || dur2 == null) durationChanged = dur1 != dur2;
        else durationChanged = !dur1.equals(dur2);
        if (durationChanged) mChanges |= TIME;
      }
    }
    ...
    if (recurChanged) mChanges |= RECURRENCE;
  }
  ...
  private static int parse(String changesCSV) {
    ...
    for (String value : values) {
      ...
      if (i != null) changeMask |= i.intValue();
    }
    return changeMask;
  }

  public String toString() {
    if (mChanges == 0) return "";
