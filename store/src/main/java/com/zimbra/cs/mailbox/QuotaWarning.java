...
/**
 * LMTP callback that sends the user a warning if he is getting close to exceeding his mailbox
 * quota.
 *
 * @author bburtin
 */
public class QuotaWarning implements LmtpCallback {
  ...
  public void afterDelivery(
    ...
    try {
      ...
      long warnInterval =
          account.getTimeInterval(Provisioning.A_zimbraQuotaWarnInterval, Constants.MILLIS_PER_DAY);
      String template = account.getAttr(Provisioning.A_zimbraQuotaWarnMessage, null);

      Date lastWarnTime =
          account.getGeneralizedTimeAttr(Provisioning.A_zimbraQuotaLastWarnTime, null);
      Date now = new Date();

      ZimbraLog.lmtp.debug(
          "Checking quota warning: mbox size=%d, quota=%d, lastWarnTime=%s, warnInterval=%d,"
              + " warnPercent=%d",
      ...
      if (template == null) {
        ZimbraLog.lmtp.warn(
            "%s not specified.  Unable to send quota warning message.",
            Provisioning.A_zimbraQuotaWarnMessage);
      }

      // Send the quota warning
      ZimbraLog.lmtp.info(
          "Sending quota warning: mbox size=%d, quota=%d, lastWarnTime=%s, warnInterval=%dms,"
              + " warnPercent=%d",
          mbox.getSize(), quota, lastWarnTime, warnInterval, warnPercent);
