...
// This is memory efficient compared to the default parse handler which builds a list of parsed
// objects
// and iterates them to add.  Downside: This handler can't deal with TZID references which occur
// before
...
public class IcsImportParseHandler implements ZICalendarParseHandler {
  ...
  public void endComponent(String name) throws ParserException {
    if (mComponents.isEmpty()) throw new ParserException("Found END:" + name + " without BEGIN");
  ...
  public void propertyValue(String value) throws ParserException {
    ...
    else mCurProperty.setValue(ZCalendar.unescape(value));
    if (mComponents.size() == 0) {
      if (ICalTok.METHOD.equals(mCurProperty.getToken())) mMethod = value;
  ...
  private void doComp(ZComponent comp) throws ServiceException {
    ...
    for (String tzid : mTZIDsSeen) {
      ...
      if (tz != null) {
        if (tzmap == null) tzmap = new TimeZoneMap(tz);
        tzmap.add(tz);
      } else {
  ...
  public static class ImportInviteVisitor implements InviteVisitor {
    ...
    public void visit(Invite inv) throws ServiceException {
      ...
      try {
        mFolder
            .getMailbox()
            .addInvite(mCtxt, inv, mFolder.getId(), mPreserveExistingAlarms, addRevision);
        if (ZimbraLog.calendar.isDebugEnabled()) {
          if (inv.isEvent()) ZimbraLog.calendar.debug("Appointment imported: UID=" + inv.getUid());
          else if (inv.isTodo()) ZimbraLog.calendar.debug("Task imported: UID=" + inv.getUid());
        }
