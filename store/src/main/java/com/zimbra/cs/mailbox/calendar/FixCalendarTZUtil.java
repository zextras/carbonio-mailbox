...
public class FixCalendarTZUtil extends SoapCLI {
  ...
  protected void setupCommandLineOptions() {
    ...
    options.addOption(
        new Option(null, O_SYNC, false, "run synchronously; default is asynchronous"));
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      String after = null;
      if (cl.hasOption(O_AFTER)) after = cl.getOptionValue(O_AFTER);

      util.doit(
          getZAuthToken(cl),
          cl.getOptionValue(O_RULEFILE),
          cl.getOptionValues(O_ACCOUNT),
          after,
          cl.hasOption(O_SYNC));
      System.exit(0);
