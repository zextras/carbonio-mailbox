...
/**
 * Manages persistent scheduled tasks. Properties of recurring tasks are stored in a database table,
 * to allow the tasks to be rescheduled on server startup.
 */
public class ScheduledTaskManager {
  ...
  public static void startup() throws ServiceException {
    ...
    int numThreads =
        prov.getLocalServer().getIntAttr(Provisioning.A_zimbraScheduledTaskNumThreads, 20);
  ...
  /**
   * Schedules a task. If the task is a recurring task, the first execution time is delayed a random
   * amount of time between <tt>0</tt> and the recurrence interval.
   *
   * @param conn a database connection used for persisting the task or <tt>null</tt> if the task is
   *     not persistent
   * @param task the task
   * @see ScheduledTask#getIntervalMillis()
   */
  public static void schedule(DbConnection conn, ScheduledTask task) throws ServiceException {
    if (conn != null) {
    ...
    if (task.isRecurring()) {
      ...
      sScheduler.schedule(
          getKey(task), task, true, task.getIntervalMillis(), delay, getRetryParams());
    } else {
  ...
  public static ScheduledTask cancel(
      String className, String taskName, int mailboxId, boolean mayInterruptIfRunning)
   ...
   * Cancels a task.
   *
   * @param conn a database connection used for deleting a persistent task, or <tt>null</tt> if the
  ...
  public static ScheduledTask cancel(
    ...
    return (ScheduledTask)
        sScheduler.cancel(getKey(className, taskName, mailboxId), mayInterruptIfRunning);
  }
  ...
  /**
   * Deletes a scheduled task from the database after the task it has run. Only deletes single-run
   * tasks, not recurring tasks.
   */
  private static class TaskCleanup implements ScheduledTaskCallback<ScheduledTaskResult> {
    TaskCleanup() {}
