...
public class ZRecur implements Cloneable {
  ...
  public static class ZWeekDayNum {
    ...
    public String toString() {
      if (mOrdinal != 0) return Integer.toString(mOrdinal) + mDay;
  ...
  public int getEstimatedCount(ParsedDateTime dtStart) throws ServiceException {
    if (dtStart == null) return 0;
    return estimateMaxCountByCountAndHardLimits();
  }

  // deduct 1 from all values for mUntil calculation when appointment is not all day because it
  // picks the next X occurrences
  private Date estimateEndTimeByUntilAndHardLimits(ParsedDateTime dtStart, boolean allDay)
      throws ServiceException {
    boolean forever = false;
    Calendar hardEnd = dtStart.getCalendarCopy();
    Frequency freq = mFreq;
    if (freq == null) freq = Frequency.WEEKLY;
    switch (freq) {
      case WEEKLY:
        int weeks = sExpansionLimits.maxWeeks;
        if (weeks <= 0) {
    ...
    Date d;
    if (forever) d = new Date(MAX_DATE_MILLIS);
    else d = hardEnd.getTime();
    if (mUntil != null) {
      Date until = mUntil.getDateForRecurUntil(dtStart.getTimeZone());
      if (until.before(d)) d = until;
    }
    return d;
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      System.out.println(
          "\n\n" + test.toString() + "\n-------------------------------------------------");
      List<Date> dateList =
          test.expandRecurrenceOverRange(dtStart, rangeStart.getTime(), rangeEnd.getTime());
      for (Date d : dateList) {
        cal.setTime(d);
        cal.setTimeZone(tzUTC);
    ...
    try {
      ZRecur test =
          new ZRecur(
              "FREQ=DAILY;BYMONTH=5,6;BYMONTHDAY=1,3,5,7,9,31;BYDAY=SU,SA;BYHOUR=21,0", tzmap);
      System.out.println(
          "\n\n"
              + test.toString()
              + "\n--------------------------------------------------------------");
    ...
    try {
      ZRecur test =
          new ZRecur(
              "FREQ=DAILY;BYMONTH=5,6;BYMONTHDAY=1,3,5,7,9,31;BYDAY=SU;BYHOUR=21,0;BYMINUTE=23",
              tzmap);
      System.out.println(
          "\n\n"
              + test.toString()
    ...
    try {
      ZRecur test =
          new ZRecur(
              "FREQ=DAILY;BYMONTH=5,6;BYMONTHDAY=1,3,5,7,9,31;BYDAY=SU;BYHOUR=1,21,0;BYSECOND=0,59",
              tzmap);
      System.out.println(
          "\n\n"
              + test.toString()
    ...
    try {
      ZRecur test =
          new ZRecur("FREQ=HOURLY;BIYMONTH=6;BYMONTHDAY=1;;BYMINUTE=10;BYSECOND=11,12", tzmap);
      System.out.println(
          "\n\n"
              + test.toString()
              + "\n--------------------------------------------------------------");
    ...
    try {
      ...
      System.out.println(
          "\n\n"
              ...
              + "\n--------------------------------------------------------------");
      List<Date> dateList =
          test.expandRecurrenceOverRange(myDtStart, rangeStart.getTime(), rangeEnd.getTime());
      for (Date d : dateList) {
        cal.setTime(d);
        System.out.printf("%tc\n", cal);
      }
  ...
  public List<Date> expandRecurrenceOverRange(
      ParsedDateTime dtStart, long rangeStart, long rangeEnd) throws ServiceException {
    ...
    Date earliestDate;
    if (dtStartDate.after(rangeStartDate)) earliestDate = dtStartDate;
    else earliestDate = rangeStartDate;

    if (mUntil != null) {
      Date until = mUntil.getDateForRecurUntil(dtStart.getTimeZone());
      if (until.before(rangeEndDate)) rangeEndDate = until;
    }

    // Set limit of expansion count.
    int maxInstancesFromConfig = sExpansionLimits.maxInstances;
    int maxInstancesExpanded;
    if (maxInstancesFromConfig <= 0) maxInstancesExpanded = mCount;
    else if (mCount <= 0) maxInstancesExpanded = maxInstancesFromConfig;
    else maxInstancesExpanded = Math.min(mCount, maxInstancesFromConfig);
    ...
    ParsedDateTime earliestDateTime =
        ParsedDateTime.fromUTCTime(earliestDate.getTime(), dtStart.getTimeZone());
    Date hardEndDate = getEstimatedEndTime(earliestDateTime);
    if (hardEndDate.before(rangeEndDate)) rangeEndDate = hardEndDate;
    ...
    boolean baseIsLeapDay =
        ((baseMonthDay == 29) && (cur.get(Calendar.MONTH) == Calendar.FEBRUARY));
    ...
    while (!pastHardEndTime
        && (maxInstancesExpanded <= 0 || numInstancesExpanded < maxInstancesExpanded)) {
      numIterations++;
      ...
      switch (mFreq) {
        case HOURLY:
           ...
           */
          if (!checkMonthList(cur)) continue;

          if (!checkYearDayList(cur)) continue;

          if (!checkMonthDayList(cur)) continue;

          if (!checkDayList(cur)) continue;

          if (!checkHourList(cur)) continue;
      ...
      for (Calendar addCal : addList) {
        ...
        // current date window
        if (toAdd.after(dtStartDate)) numInstancesExpanded++;
        ...
        if (maxInstancesExpanded > 0 && numInstancesExpanded >= maxInstancesExpanded) break;
      }

      // Detect invalid rule.  If the rule was invalid the current iteration, which is for the
      // current
      // frequency interval, would have found no matching instance.  The next iteration will also
      // find
      // no matching instance, and there is no need to keep iterating until we go past the hard end
      // time or COUNT/UNTIL limit.
      //
      // However, we have to make an exception for leap year.  An yearly rule looking for February
      // 29th
      // will find no instance in up to 3 consecutive years before finding Feb 29th in the fourth
      // year.
      //
      // So the invalid rule detection must look for at least 4 consecutive failed iterations.
      if (curIsAtOrAfterEarliestDate) {
        if (noInstanceFound) numConsecutiveIterationsWithoutMatchingInstance++;
        else numConsecutiveIterationsWithoutMatchingInstance = 0;
  ...
  public String toString() {
    ...
    if (mCount > 0) toRet.append(';').append("COUNT=").append(mCount);
    if (mInterval > 0) toRet.append(';').append("INTERVAL=").append(mInterval);
    ...
    if (mByHourList.size() > 0) toRet.append(';').append("BYHOUR=").append(listAsStr(mByHourList));
    if (mByDayList.size() > 0) toRet.append(';').append("BYDAY=").append(listAsStr(mByDayList));
  ...
  /**
   * This version is for HOURLY/DAILY frequencies: it does NOT check the ordinal at all, it only
   * verifies that the day-of-the-week matches
   *
   * @param cal
   * @return
   */
  private boolean checkDayList(GregorianCalendar cal) {
    ...
    if (mByDayList.size() > 0) {
      for (ZWeekDayNum listCur : mByDayList) {
        ...
        if (listCur.mDay.getCalendarDay() == curDayOfWeek) return true;
  ...
  private boolean checkHourList(GregorianCalendar cal) {
    if (mByHourList.size() > 0) {
      for (Integer cur : mByHourList) {
        ...
        if (curHour == cur.intValue()) return true;
  ...
  private boolean checkMonthDayList(GregorianCalendar cal) {
    if (mByMonthDayList.size() > 0) {
      for (Integer cur : mByMonthDayList) {
        ...
        if (cur == curMonthDay) return true;
  ...
  private List<Calendar> expandDayListForMonthlyYearly(List<Calendar> list) {
    ...
    if (mByDayList.size() <= 0) return list;
    ...
    // we unfortunately have to sort here because, for example, the "-1FR" could happen before the
    // "-1TH"
  ...
  /**
   * rfc5545 says : Recurrence rules may generate recurrence instances with an invalid date (e.g.,
   * February 30) or nonexistent local time (e.g., 1:30 AM on a day where the local time is moved
   * forward by an hour at 1:00 AM). Such recurrence instances MUST be ignored and MUST NOT be
   * counted as part of the recurrence set. Therefore, ignore this year if the last day isn't 29th
   * for patterns: RRULE:FREQ=YEARLY;BYMONTHDAY=29;BYMONTH=2 and "RRULE:FREQ=YEARLY" where DTSTART
   * is a leap day
   *
   * @param cur
   * @param baseIsLeapDay
  ...
  private List<Calendar> expandHourList(List<Calendar> list) {
    // this func ONLY works for expanding, NOT for contracting
    assert (mFreq == Frequency.DAILY
        || mFreq == Frequency.WEEKLY
        || mFreq == Frequency.MONTHLY
        || mFreq == Frequency.YEARLY);

    if (mByHourList.size() <= 0) return list;
  ...
  private List<Calendar> expandMinuteList(List<Calendar> list) {
    ...
    if (mByMinuteList.size() <= 0) return list;
  ...
  private List<Calendar> expandMonthDayList(List<Calendar> list) {
    ...
    if (mByMonthDayList.size() <= 0) return list;
  ...
  private List<Calendar> expandMonthList(List<Calendar> list) {
    ...
    if (mByMonthList.size() <= 0) return list;
  ...
  private List<Calendar> expandSecondList(List<Calendar> list) {
    ...
    if (mBySecondList.size() <= 0) return list;
  ...
  private List<Calendar> expandYearDayList(List<Calendar> list) {
    ...
    if (mByYearDayList.size() <= 0) return list;
    ...
    for (Calendar cur : list) {
      ...
      if (!years.contains(curYear)) {
        ...
        for (Integer yearDay : mByYearDayList) {
          ...
          if (yearDay > 0) cur.set(Calendar.DAY_OF_YEAR, yearDay);
          else {
  ...
  private List<Calendar> handleSetPos(List<Calendar> list) {
    if (mBySetPosList.size() <= 0) return list;
    ...
    for (Integer cur : mBySetPosList) {
      ...
      if (idx >= -366 && idx <= 366 && idx != 0) {
        if (idx > 0) idx--; // 1-indexed!
        else idx = array.length + idx;

        if (idx >= 0 && idx < array.length)
          if (!idxsToInclude.contains(idx)) idxsToInclude.add(idx);
      }
    }
  ...
  private void parse(String str, TimeZoneMap tzmap) throws ServiceException {
    try {
      ...
      for (String tok : str.split("\\s*;\\s*")) {
        ...
        String rhs = s[1];
        // MS Exchange can add invalid spaces in RRULE part values.  Get rid of them.  (see bug
        // 25169)
        ...
        try {
          ...
          ZimbraLog.calendar.warn(
              "Skipping RECUR token: \"%s\" in Recur \"%s\" due to parse error", s[0], str, e);
        }
      }
      ...
      if (numByParts == 2
          && Frequency.MONTHLY.equals(mFreq)
          && mByDayList.size() == 1
          && mByDayList.get(0).mOrdinal == 0
          && mBySetPosList.size() == 1) {
  ...
  private static int parseSignedInt(String str) {
    if (str == null) throw new NumberFormatException("null is not a number");
    int len = str.length();
    if (len == 0) throw new NumberFormatException("empty string is not a number");
    int num = 0;
    if (str.charAt(0) == '+') {
      if (len == 1) throw new NumberFormatException("+ is not a number");
  ...
  private static int parseUnsignedInt(String str) {
    ...
    if (sign == '+' || sign == '-') throw new NumberFormatException("sign not allowed: " + str);
  ...
  private void parseIntList(String str, List<Integer> list, int min, int max, boolean signed) {
    for (String s : str.split("\\s*,\\s*"))
      try {
        int readInt;
        if (signed) readInt = parseSignedInt(s);
        else readInt = parseUnsignedInt(s);
        ...
        ZimbraLog.calendar.debug(
            new Formatter()
                .format(
                    "Skipping unparsable Recur int list entry: \"%s\" in parameter list: \"%s\"",
                    s, str));
      }
