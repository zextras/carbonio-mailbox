...
public abstract class MailboxListener {
  ...
  /**
   * Listeners will be notified at the end of each <code>Mailbox</code> transaction. The listener
   * must not throw any Exception in this method. The listener must refrain from making synchronous
   * network operation or other long latency operation within notify method.
   *
   * @param notification
   */
  ...
  /**
   * Listener can indicate specific item types it is interested in, which will reduce the number of
  ...
  static void reset() {
    ...
    if (application.supports(CalItemReminderService.class)
        && !DebugConfig.disableCalendarReminderEmail) {
    ...
    if (application.supports(ShareExpirationListener.class)
        && !DebugConfig.disableShareExpirationListener) {
