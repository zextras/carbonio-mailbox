...
public final class TypedIdList
    implements Iterable<Map.Entry<MailItem.Type, List<TypedIdList.ItemInfo>>> {
  ...
  private final Map<MailItem.Type, List<ItemInfo>> type2ids =
      new EnumMap<MailItem.Type, List<ItemInfo>>(MailItem.Type.class);
  ...
  public void add(
      MailItem.Type type, Integer id, Integer folderId, String uuid, Integer modSequence) {
    if (id == null) {
      return;
    }
    ...
    items.add(
        new ItemInfo(
            id, (null != folderId) ? folderId : Mailbox.ID_AUTO_INCREMENT, uuid, modSequence));
  }

  public void add(
      MailItem.Type type, Integer id, String uuid, int modSequence, String prevFolders) {
    if (id == null) return;
  ...
  /**
   * Removes an entry from the TypedIdList, if present. Items are specified by id.
   *
