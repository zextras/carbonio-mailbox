...
public class MessageCache {
  ...
  private static Map<String, CacheNode> sCache =
      new LinkedHashMap<String, CacheNode>(150, (float) 0.75, true);
  /** Maximum number of items in {@link #sCache}. */
  private static int sMaxCacheSize;
  /**
   * Number of bytes of message data stored in the cache. This value includes only messages that are
   * read into memory, not streamed from disk.
   */
  ...
  /**
   * Uncaches any data associated with the given item. This must be done before you change the
   * item's content; otherwise, the cache will return stale data.
   */
  ...
  static MimeMessage getMimeMessage(MailItem item, boolean expand) throws ServiceException {
    ...
    try {
      ...
      if (expand) {
        ...
        try {
          ...
          if (item instanceof Message) {
            ...
            if (isEncrypted) {
              if (isSmimeFeatureToggled(item.getMailbox(), cnode)) {
                sLog.debug(
                    "Smime feature is toggled. So remove old entry from smimeAccessInfo for"
                        + " mailboxId=%d and itemDigest=%s",
          ...
          // expand if the message has not yet been expanded or if the message is decrypted
          // successfully
          if (cnode.expanded == null
              || (decryptedMimeMessage != null && cnode.expanded != decryptedMimeMessage)) {
            cacheHit = false;
          ...
          sLog.warn(
              "MIME converter failed for message %d.  Reverting to original.", item.getId(), e);
      ...
      throw ServiceException.FAILURE(
          "IOException while retrieving content for item " + item.getId(), e);
    } catch (MessagingException e) {
      throw ServiceException.FAILURE(
          "MessagingException while creating MimeMessage for item " + item.getId(), e);
    } finally {
      ByteUtil.closeStream(in);
    }
    ...
    if (cacheHit) {
      sLog.debug(
          "Cache hit for item %d: digest=%s, expand=%b.", item.getId(), item.getDigest(), expand);
      ZimbraPerf.COUNTER_MBOX_MSG_CACHE.increment(100);
    } else {
      sLog.debug(
          "Cache miss for item %d: digest=%s, expand=%b.", item.getId(), item.getDigest(), expand);
  ...
  private static void expandMessage(
      MailItem item, CacheNode cnode, MimeMessage decryptedMimeMessage)
    ...
    if (Mime.isPKCS7Signed(mimeToExpand.getContentType())) {
      if (SmimeHandler.getHandler() != null) {
        ...
        decodedMimeMessage =
            SmimeHandler.getHandler().decodePKCS7Message(item.getAccount(), mimeToExpand);
      }
    }
    ExpandMimeMessage expander =
        new ExpandMimeMessage(decodedMimeMessage != null ? decodedMimeMessage : mimeToExpand);
    expander.expand();
      ...
      try {
        decryptedMimeMessage =
            SmimeHandler.getHandler()
                .decryptMessage(((Message) item).getMailbox(), cnode.message, item.getId());
        if (decryptedMimeMessage == null) {
  ...
  /**
   * For remote stores, we allow JavaMail to stream message content to a memory buffer when the
   * messages are sufficiently small. (For local stores, all cached messages are backed by disk.)
   */
  ...
  static InputStream fetchFromStore(MailItem item) throws ServiceException, IOException {
    ...
    if (mblob == null)
      throw ServiceException.FAILURE(
          "missing blob for id: " + item.getId() + ", change: " + item.getModifiedSequence(), null);
    ...
    synchronized (sCache) {
      ...
      while (it.hasNext()) {
        ...
        try {
          ...
          sLog.warn(
              "MessagingException while checking content type for cache node with digest = %s",
              entry.getKey(), e);
        }
      }
    }
