...
public class Note extends MailItem {
    ...
    public Rectangle(String bounds) {
      ...
      if (dimensions.length == 2 || dimensions.length == 4) {
        ...
        if (dimensions.length == 4) {
          width = Integer.parseInt(dimensions[2]);
          height = Integer.parseInt(dimensions[3]);
        }
      }
    }
  ...
  /**
   * Returns the <code>Note</code>'s bounding box. When dimensions were not specified, the returned
   * {@link Note.Rectangle} will have <code>0</code> for each omitted dimension.
   */
  ...
  /**
   * Creates a new Note and persists it to the database. A real nonnegative item ID must be supplied
   ...
   *     <ul>
   *       <li><code>mail.CANNOT_CONTAIN</code> - if the target folder can't hold notes
   ...
   */
  static Note create(
      int id, Folder folder, String content, Rectangle location, Color color, CustomMetadata custom)
    ...
    ZimbraLog.mailop.info(
        "Adding Note: id=%d, folderId=%d, folderName=%s.",
  ...
  private static String encodeMetadata(
      Color color, int metaVersion, int version, CustomMetadata custom, Rectangle bounds) {
