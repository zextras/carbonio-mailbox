...
public class CacheToXML {
  ...
  private static void encodeInstanceData(
      Element parent,
    ...
    if (instance.getDuration() != null) {
      String attribute =
          !legacyFormat ? MailConstants.A_CAL_NEW_DURATION : MailConstants.A_CAL_DURATION;
    ...
    //            parent.addAttribute(MailConstants.A_TASK_DUE_DATE,
    // instance.getDtEnd().longValue());
    ...
    else parent.addAttribute(MailConstants.A_TASK_PERCENT_COMPLETE, instance.getPercentComplete());
    ...
    if (!isException) parent.addAttribute(MailConstants.A_ID, ifmt.formatItemId(calItemId));
    parent.addAttribute(
        MailConstants.A_CAL_INV_ID, ifmt.formatItemId(calItemId, fullInstance.getInvId()));
    ...
    if (fullInstance.isNeverSent() != null)
      parent.addAttribute(
          MailConstants.A_CAL_NEVER_SENT, fullInstance.isNeverSent().booleanValue());
  }
  ...
  private static void encodeAlarmData(Element parent, AlarmData alarmData, boolean showAll) {
    ...
    if (showAll) {
      ...
      if (alarmObj != null) alarmObj.toXml(alarmElem);
    }
  }

  public static Element encodeCalendarItemData(
    ...
    Element calItemElem =
        zsc.createElement(isAppointment ? MailConstants.E_APPOINTMENT : MailConstants.E_TASK);
    ...
    calItemElem.addAttribute(
        MailConstants.A_CHANGE_DATE, calItemData.getChangeDate() / 1000); // as seconds
    ...
    if (alarmData != null) encodeAlarmData(calItemElem, alarmData, showAll);

    return calItemElem;
  }

  public static void encodeCalendarData(
      ZimbraSoapContext zsc,
      ItemIdFormatter ifmt,
      Element parent,
    ...
    for (Iterator<CalendarItemData> iter = calData.calendarItemIterator(); iter.hasNext(); ) {
      ...
      if (calItemData.getNumInstances() > 0) {
        Element calItemElem =
            encodeCalendarItemData(zsc, ifmt, calItemData, allowPrivateAccess, legacyFormat);
