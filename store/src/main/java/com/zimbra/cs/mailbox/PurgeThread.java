...
/**
 * Iterates all the mailboxes in the system, purges them one at a time and sleeps between purges for
 * a time interval specified by {@link Provisioning#A_zimbraMailPurgeSleepInterval}.
 *
 * @author bburtin
 */
public class PurgeThread extends Thread {
    ...
    synchronized (THREAD_CONTROL_LOCK) {
      ...
      if (getSleepInterval() == 0) {
        ZimbraLog.purge.info(
            "Not starting purge thread because %s is 0.",
  ...
  /** Iterates all mailboxes, purging one at a time and sleeping between purges. */
  @Override
  public void run() {
    ...
    while (true) {
      ...
      for (int i = 0; i < mailboxIds.size(); i++) {
        ...
        try {
          ...
          if (mm.isMailboxLoadedAndAvailable(mailboxId)
              || purgePendingMailboxes.contains(mailboxId)) {
            attemptedPurge = true;
            ...
            if (!Provisioning.ACCOUNT_STATUS_MAINTENANCE.equals(account.getAccountStatus(prov))
              ...
              if (!purgedAll) {
                ZimbraLog.purge.info(
                    "Not all messages were purged.  Scheduling mailbox to be purged again.");
                mailboxIds.add(mailboxId);
              }
              Config.setInt(Config.KEY_PURGE_LAST_MAILBOX_ID, mbox.getId());
            } else {
              ZimbraLog.purge.debug(
                  "Skipping mailbox %d because the account is in maintenance status or is an"
                      + " external virtual account.",
                  mailboxId);
            }
          } else {
            ZimbraLog.purge.debug(
                "Skipping mailbox %d because it is not loaded into memory.", mailboxId);
          }
          ...
          if (ServiceException.WRONG_HOST.equals(se.getCode())) {
            if (ZimbraLog.purge.isDebugEnabled()) {
              ...
              ZimbraLog.purge.info(
                  "not purging mailbox %d; account moved to another host", mailboxId);
            }
          } else {
      ...
      try {
        long lastPurgeMaxDuration =
            Provisioning.getInstance().getLocalServer().getLastPurgeMaxDuration();
        purgePendingMailboxes =
            MailboxManager.getInstance()
                .getPurgePendingMailboxes(System.currentTimeMillis() - lastPurgeMaxDuration);
  ...
  /**
   * Stores the sleep interval, so that the purge thread doesn't die if there's a problem talking to
   * LDAP. See bug 32639.
   */
  private static long sSleepInterval = 0;

  /**
   * Returns the current value of {@link Provisioning#A_zimbraMailPurgeSleepInterval}, or <tt>0</tt>
   * if it cannot be determined.
   */
  private static long getSleepInterval() {
    sSleepInterval =
        CallbackUtil.getTimeInterval(Provisioning.A_zimbraMailPurgeSleepInterval, sSleepInterval);
    return sSleepInterval;
  }

  /**
   * Returns all the mailbox id's in purge order, starting with the one after {@link
   * Config#KEY_PURGE_LAST_MAILBOX_ID}.
   */
