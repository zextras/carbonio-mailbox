...
public class FolderACL {
  ...
  // caches whether the authed account can access the target account as an admin or via the loginAs
  // right
  // only looked at if the authed account is not the owner of the target account
  // callsites of FolderACL can construct FolderACL with a non-null canAccessOwnerAccount value to
  // bypass
  ...
  private static class ShareTarget {
    ...
    private ShareTarget(String ownerAcctId, int folderId) throws ServiceException {
      ...
      if (mOwnerAcct == null) throw AccountServiceException.NO_SUCH_ACCOUNT(ownerAcctId);
    ...
    boolean onLocalServer() throws ServiceException {
      return Provisioning.getInstance().onLocalServer(mOwnerAcct);
      // return false;
    }
  }
  ...
  public FolderACL(
      OperationContext octxt, Account ownerAcct, int folderId, Boolean canAccessOwnerAccount)
      throws ServiceException {
    mOctxt = octxt;
  ...
  private Account getAuthenticatedAccount() throws ServiceException {
    ...
    if (mOctxt != null) authuser = mOctxt.getAuthenticatedUser();
    ...
    // the access.
    if (authuser != null && authuser.getId().equals(mShareTarget.getAccountId())) authuser = null;
    return authuser;
  }
  ...
  private short checkRights(short rightsNeeded, Account authuser, boolean asAdmin)
    ...
    if (canAccessOwnerAccount(authuser, asAdmin)) return rightsNeeded;
  ...
  private Short getEffectivePermissionsFromServer() throws ServiceException {
    if (mShareTarget.onLocalServer()) return getEffectivePermissionsLocal();
    else return getEffectivePermissionsRemote();
  }

  private Short getEffectivePermissionsLocal() throws ServiceException {
    Mailbox ownerMbx =
        MailboxManager.getInstance().getMailboxByAccountId(mShareTarget.getAccountId());
  ...
  public static Short getEffectivePermissionsLocal(
      OperationContext octxt, Mailbox ownerMbx, Folder folder) throws ServiceException {
    ...
    return ownerMbx.getEffectivePermissions(
  ...
  private Short getEffectivePermissionsRemote() throws ServiceException {
    ...
    request
        .addElement(MailConstants.E_FOLDER)
        .addAttribute(MailConstants.A_FOLDER, iid.toString((Account) null));
    ...
    if (mOctxt != null) authToken = AuthToken.getCsrfUnsecuredAuthToken(mOctxt.getAuthToken());
    if (authToken == null) authToken = AuthProvider.getAuthToken(GuestAccount.ANONYMOUS_ACCT);
