...
public class MigrateToDocuments {
  ...
  public void handleMailbox(Mailbox mbox) throws ServiceException {
    ...
    if (destRoot == null)
      destRoot =
          mbox.createFolder(
              octxt, path, new Folder.FolderOptions().setDefaultView(MailItem.Type.DOCUMENT));
    if (destRoot == null) {
  ...
  private void moveToBackupFolder(Folder from, Folder to) throws ServiceException {
    for (Folder source : from.getSubfolders(octxt)) {
      if (source.getDefaultView() != MailItem.Type.WIKI) continue;
    ...
    for (MailItem item : mbox.getItemList(octxt, MailItem.Type.WIKI, from.getId())) {
      try {
        ...
        if (e.getCode().equals(MailServiceException.ALREADY_EXISTS)) {
          ...
          ZimbraLog.misc.warn(
              "Can't move item %s to backup folder %s", item.getName(), to.getPath());
        }
      }
    }
  }

  private void migrateFromBackupFolder(OperationContext octxt, Folder from, Folder to)
    ...
    for (MailItem item : mbox.getItemList(octxt, MailItem.Type.WIKI, from.getId())) {
      ...
      for (int rev = 1; rev < doc.getVersion(); rev++) {
        ...
        try {
          revision =
              (Document) mbox.getItemRevision(octxt, item.getId(), MailItem.Type.DOCUMENT, rev);
        } catch (Exception e) {
  ...
  private Document addRevision(String name, Document main, Document revision, Folder to) {
    ...
    try {
      ...
      if (main == null) {
        main =
            mbox.createDocument(
                octxt, to.getId(), pd, MailItem.Type.DOCUMENT, revision.getFlagBitmask());
      } else {
        mbox.addDocumentRevision(octxt, main.getId(), pd);
      }
    } catch (Exception e) {
      ZimbraLog.misc.warn(
          "Can't add new revision for " + name + " revision " + revision.getVersion(), e);
    } finally {
      if (in != null)
        try {
          in.close();
  ...
  private InputStream getContentStream(Document item) throws IOException, ServiceException {
    if (item.getType() == MailItem.Type.DOCUMENT) return item.getContentStream();
    String contents = new String(item.getContent(), "UTF-8");
    WikiTemplate wt =
        new WikiTemplate(
            contents, mbox.getAccountId(), Integer.toString(item.getFolderId()), item.getName());
    WikiTemplate.Context wctxt =
        new WikiTemplate.Context(new WikiPage.WikiContext(octxt, null), item, wt);
