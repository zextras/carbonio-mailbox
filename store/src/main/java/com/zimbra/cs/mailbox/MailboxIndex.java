...
public final class MailboxIndex {
  ...
  private static final long FAILURE_DELAY =
      LC.zimbra_index_deferred_items_failure_delay.intValue() * 1000;
  ...
  // current compact-indexing operation for this mailbox, or NULL if a compact-index is not in
  // progress.
   ...
   *
   * <p>In order to avoid deadlock, callers MUST NOT be holding the Mailbox lock when calling this
   * API.
   *
   * <p>You MUST call {@link ZimbraQueryResults#doneWithSearchResults()} when you are done with the
   * search results, otherwise resources will be leaked.
   *
  ...
  public ZimbraQueryResults search(
      ...
      for (InternetAddress addr : addrs) {
        if (!Strings.isNullOrEmpty(addr.getAddress())) {
          ...
          try (TermFieldEnumeration values =
              searcher.getIndexReader().getTermsForField(LuceneFields.L_CONTACT_DATA, lcAddr)) {
  ...
  /**
   * Returns the maximum number of items to be batched in a single indexing pass. If a search comes
   * in that requires use of the index, all deferred unindexed items are immediately indexed
   * regardless of batch size. If this number is {@code 0}, all items are indexed immediately when
   * they are added.
   */
  ...
  void setIndexingSuspended(boolean suspended) {
    ZimbraLog.index.info(
        "indexSuspended set to %s.  Current deferred count %s",
        suspended, getDeferredCount(EnumSet.noneOf(MailItem.Type.class)));
    ...
    if (indexingSuspended) {
      if ((numMaybeIndexDeferredItemsCalls % 1000) == 0) {
        ZimbraLog.index.debug(
            "Indexing suspended. maybeIndexDeferredItems called %s times whilst suspended",
  ...
  void resumeIndexingAndDrainDeferred() {
    ...
    ZimbraLog.index.info(
        "resumeIndexingAndDrainDeferred deferred count=%s",
   ...
   * @param wait if an indexing is in progress by other threads, true to wait for them to complete,
   *     false to skip indexing
   */
  private void indexDeferredItems(Set<MailItem.Type> types, BatchStatus status, boolean wait)
    ...
    try {
      ...
      ZimbraLog.index.info(
          "Batch complete processed=%d,failed=%d,elapsed=%d (%.2f items/sec)",
          status.getProcessed(),
          status.getFailed(),
          elapsed,
  ...
  /**
   * Kick off the requested re-index in a background thread. The re-index is run on a best-effort
   * basis, if it fails a WARN message is logged, but it won't be retried.
   */
  ...
  private synchronized void startReIndex(ReIndexTask task) throws ServiceException {
    if ((indexStore != null) && indexStore.isPendingDelete()) {
      throw ServiceException.FAILURE(
          "Unable to submit reindex request. Index is pending delete", null);
    }
    try {
      if (reIndex != null) {
  ...
  private synchronized void startCompactIndex(CompactIndexTask task) throws ServiceException {
    if ((indexStore != null) && indexStore.isPendingDelete()) {
      throw ServiceException.FAILURE(
          "Unable to submit compact index request. Index is pending delete", null);
    }
    try {
  ...
  private class ReIndexTask extends IndexTask {
    ...
    public void exec() {
      try {
        ...
        ZimbraLog.index.info(
            "Re-index completed items=%d,failed=%d,elapsed=%d (avg %d ms/item, %d items/sec)",
    ...
    /**
     * Re-Index some or all items in this mailbox. This can be a *very* expensive operation (upwards
     * of an hour to run on a large mailbox on slow hardware). We are careful to unlock the mailbox
     * periodically so that the mailbox can still be accessed while the re-index is running, albeit
     * at a slower rate.
     */
    void reIndex() throws ServiceException {
      if (ids == null) { // full re-index
        ...
        // it is horribly inefficient and very rarely justified. Please check the API doc for more
        // details.
  ...
  void indexAllDeferredFlagItems() throws ServiceException {
    ...
    try {
      ...
      List<DbSearch.Result> list =
          new DbSearch(mailbox)
              .search(
                  mailbox.getOperationConnection(), c, SortBy.NONE, -1, -1, DbSearch.FetchMode.ID);
        ...
        new ReIndexTask(mailbox, ids) {
          ...
          protected void onCompletion() {
            try {
                ...
                if (!mailbox.getVersion().atLeast(1, 5)) {
                  try {
                    ...
                    ZimbraLog.mailbox.warn(
                        "Failed to update mbox version after "
                            + "reindex all deferred items during mailbox upgrade initialization.",
                        se);
                  }
                }
              } finally {
              ...
              ZimbraLog.mailbox.warn(
                  "Failed to clear deferred flag after "
                      + "reindex all deferred items during mailbox upgrade initialization.",
                  se);
            }
          }
    ...
    try {
      ...
      ZimbraLog.mailbox.warn(
          "Failed to reindex deferred items on mailbox upgrade initialization."
              + " Skipping (you will have to manually reindex this mailbox)");
    }
  }

  /**
   * Index a potentially very large list of {@link MailItem}s. Iterate through the list of items,
   * fetch each one and call generateIndexData(). Buffer the items, IndexData into a chunk and when
   * the chunk gets sufficiently large, run a Mailbox transaction to actually do the indexing
   *
   * @param ids item IDs to index
   * @param status progress will be written to the status
   * @throws ServiceException {@link ServiceException#INTERRUPTED} if {@link #cancelReIndex()} is
   *     called
   */
  private void indexItemList(Collection<Integer> ids, BatchStatus status) throws ServiceException {
    ...
    for (int id : ids) {
      ...
      // Fetch the item and generate the list of Lucene documents to index. Do this without holding
      // the Mailbox
      // lock. Once we've accumulated a "chunk" of items, do a mailbox transaction to actually add
      // them to the
      // index.
      ...
      try {
        ...
        try {
          item = mailbox.getItemById(id, MailItem.Type.UNKNOWN, true);
            ...
            MailServiceException.NoSuchItemException again) { // The item has just been deleted.
  ...
  void upgradeMailboxTo1_2() throws ServiceException {
    ...
    try {
      List<Integer> ids =
          DbMailItem.getReIndexIds(conn, mailbox, EnumSet.of(MailItem.Type.CONTACT));
      if (ids.isEmpty()) {
        return;
      }
      ReIndexTask task =
              ...
              try {
                if (!mailbox.getVersion().atLeast(1, 2)) {
                  try {
                    ...
                    ZimbraLog.mailbox.warn(
                        "Failed to update mbox version after "
                            + "reindex contacts on mailbox upgrade initialization.",
                        e);
                  }
                }
              } finally {
      ...
      ZimbraLog.mailbox.warn(
          "Failed to reindex contacts on mailbox upgrade initialization."
  ...
  /**
   * Entry point for Redo-logging system only. Everybody else should use queueItemForIndexing inside
   * a transaction.
   */
  ...
  synchronized void add(List<IndexItemEntry> entries) throws ServiceException {
    ...
    try {
      for (IndexItemEntry entry : entries) {
        if ((indexStore != null) && indexStore.isPendingDelete()) {
          ZimbraLog.index.debug(
              "add of list of entries to index aborted as index is pending delete");
          lastFailedTime = System.currentTimeMillis();
          return; // No point in indexing if we are going to delete the index
        }
  ...
  /**
   * Primes the index for the fastest available search if useful to the underlying IndexStore. This
   * is a very expensive operation especially on large index.
   */
  public void optimize() throws ServiceException {
    indexDeferredItems(
        EnumSet.noneOf(MailItem.Type.class), new BatchStatus(), true); // index all pending items
    indexStore.optimize();
  }
  ...
  public List<DbSearch.Result> search(
      DbSearchConstraints constraints,
      DbSearch.FetchMode fetch,
      SortBy sort,
      int offset,
      int size,
      boolean inDumpster)
    ...
    try {
      ...
      result =
          new DbSearch(mailbox, inDumpster)
              .search(mailbox.getOperationConnection(), constraints, sort, offset, size, fetch);
      if (fetch == DbSearch.FetchMode.MAIL_ITEM) {
        ...
        while (itr.hasNext()) {
          ...
          try {
            ...
            ZimbraLog.index.info(
                String.format(
                    "Problem constructing Result for folder=%s item=%s from UnderlyingData -"
                        + " dropping item",
                    sr.getItemData().folderId, sr.getItemData().id, sr.getId()),
                se);
            itr.remove();
          }
        }
   ...
   */
  public List<BrowseTerm> getDomains(String field, String regex)
    ...
    try (ZimbraIndexSearcher searcher = indexStore.openSearcher()) {
      try (TermFieldEnumeration values =
        ...
        while (values.hasMoreElements()) {
          BrowseTerm term = values.nextElement();
          if (pattern == null
              || AccessBoundedRegex.matches(term.getText(), pattern, MAX_REGEX_ACCESSES)) {
            result.add(term);
          }
        }
      }
  ...
  private synchronized Collection<Integer> getDeferredIds(Set<MailItem.Type> types)
      throws ServiceException {
  ...
  /**
   * Converts conversation type to message type if the type set contains it. We need to index
   * message items when a conversation search is requested.
   */
