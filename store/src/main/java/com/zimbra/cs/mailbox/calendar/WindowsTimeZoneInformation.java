...
public class WindowsTimeZoneInformation {
  ...
  public WindowsTimeZoneInformation(
      String name,
      int biasMins,
  ...
  /**
   * Windows TIME_ZONE_INFORMATION structure has separate standard and daylight time zone names,
   * both limited to 31 characters. We don't do that in this Java object. Instead we provide a
   * single time zone name that is independent of DST. Also note the name is not limited to 31
   * characters. In fact, most names are longer than that.
   *
   * @return
   */
  ...
  public int getStandardBiasMins() {
    return mStandardBiasMins;
  }

  public int getStandardOffset() {
    return mStandardOffsetMillis;
  }
  ...
  public int getDaylightBiasMins() {
    return mDaylightBiasMins;
  }

  public int getDaylightOffset() {
    return mDaylightOffsetMillis;
  }
  ...
  public ICalTimeZone toICal() {
    ...
    if (mStandardDate != null) standardOnset = mStandardDate.toSimpleOnset();
    SimpleOnset daylightOnset = null;
    if (mDaylightDate != null) daylightOnset = mDaylightDate.toSimpleOnset();
  ...
  public static WindowsTimeZoneInformation fromICal(ICalTimeZone icalTz) {
    ...
    return new WindowsTimeZoneInformation(
        ...
        standardDate,
        0,
        icalTz.getStandardTzname(),
        daylightDate,
        daylightBias / 60 / 1000,
        icalTz.getDaylightTzname());
  }
}
