...
public final class MailboxMaintenance {
  ...
  synchronized void registerOuterAllowedThread(Thread t) throws MailServiceException {
    if (inner) {
      throw MailServiceException.MAINTENANCE(
          mailboxId, "cannot add new maintenance thread when inner maintenance is already started");
    } else if (!nestedAllowed) {
      throw MailServiceException.MAINTENANCE(
          mailboxId, "cannot add outer maintenance thread when nested is not enabled");
    } else if (allowedThreads.size() > 0) {
      throw MailServiceException.MAINTENANCE(
          mailboxId, "cannot add multiple outer maintenance threads");
    }
  ...
  synchronized void startInnerMaintenance() throws MailServiceException {
    if (inner) {
      throw MailServiceException.MAINTENANCE(
          mailboxId, "attempted to nest maintenance when already nested");
    } else if (!nestedAllowed || !canAccess()) {
      throw MailServiceException.MAINTENANCE(
          mailboxId, "attempted to nest maintenance when not allowed");
    }
    inner = true;
  }
