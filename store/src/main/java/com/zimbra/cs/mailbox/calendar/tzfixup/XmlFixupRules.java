...
public class XmlFixupRules {
  ...
  private static void parseMatchers(
      Element matchElem, ICalTimeZone replacementTZ, List<Matcher> matchers)
    ...
    for (Iterator<Element> elemIter = matchElem.elementIterator(); elemIter.hasNext(); ) {
      ...
      if (elemName.equals(AdminConstants.E_ANY)) {
        ...
        Matcher m =
            new Matcher(stdOffset, stdMon, stdMday, dstOffset, dstMon, dstMday, replacementTZ);
        matchers.add(m);
      }
    }
  }
  ...
  public static List<Matcher> parseTzFixup(Element tzFixupElem) throws ServiceException {
    List<Matcher> matchers = new ArrayList<Matcher>();
    for (Iterator<Element> elemIter = tzFixupElem.elementIterator(AdminConstants.E_FIXUP_RULE);
        elemIter.hasNext(); ) {
