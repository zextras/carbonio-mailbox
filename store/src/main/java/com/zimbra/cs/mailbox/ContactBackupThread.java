...
public class ContactBackupThread extends Thread {
  ...
  private Folder getContactBackupFolder(
      OperationContext octxt, Mailbox mbox, boolean createIfDontExist) {
    Folder folder = null;
    try {
      folder =
      ...
      if (se.getCode().equals(MailServiceException.NO_SUCH_FOLDER) && createIfDontExist) {
        ZimbraLog.contactbackup.debug(
            "contact backup folder does not exist, trying to create new one");
