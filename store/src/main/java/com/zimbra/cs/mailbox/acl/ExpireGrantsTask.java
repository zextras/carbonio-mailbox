...
public class ExpireGrantsTask extends ScheduledTask {
   ...
   *
   * @return returns the item for which this task was run if we may want to schedule another
   *     instance of this task (at the next grant expiry); o/w null
  ...
  private static String getGranteeAddress(ACL.Grant grant) throws ServiceException {
    switch (grant.getGranteeType()) {
      case ACL.GRANTEE_USER:
        Account granteeAcct =
            Provisioning.getInstance().get(Key.AccountBy.id, grant.getGranteeId());
        if (granteeAcct != null) {
  ...
  static void cancel(int mailboxId, int itemId) throws ServiceException {
    ScheduledTaskManager.cancel(
        ExpireGrantsTask.class.getName(), getTaskName(itemId), mailboxId, false);
  }
}
