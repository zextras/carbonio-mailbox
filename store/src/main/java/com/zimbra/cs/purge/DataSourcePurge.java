...
public abstract class DataSourcePurge {
  ...
  protected Map<String, ConversationPurgeQueue> purgeQueues =
      new HashMap<String, ConversationPurgeQueue>();
  ...
  public void purgeConversations(
      OperationContext octxt, DataSource incoming, long bytesToPurge, Integer convId)
      throws ServiceException {
    long bytesLeft = bytesToPurge;
    while (bytesLeft > 0L) {
      long toPurge = Math.min(bytesLeft, PURGE_BATCH_SIZE);
      PurgeableConvs convsToPurge =
          getOldestConversations(getPurgeableDataSources(incoming), toPurge, convId);
      ...
      if (convsToPurge != null && convsToPurge.getTotalSize() > 0) {
        ...
        ZimbraLog.datasource.info(
            "purging %d conversations to free up %d bytes; %d bytes left",
            convsToPurge.getNumConvs(), convsToPurge.getTotalSize(), bytesLeftAfterThis);
  ...
  private void purgeConversation(OperationContext octxt, PurgeableConv conv)
      throws ServiceException {
    if (conv.isMsg()) {
  ...
  private void partiallyPurgeConversation(
      OperationContext octxt, String dataSourceId, Conversation conversation, Set<Integer> ids)
      throws ServiceException {
  ...
  private void purgeMailItem(
      OperationContext octxt, String dataSourceId, MailItem item, MailItem.Type type)
      throws ServiceException {
    if (type == Type.MESSAGE) {
      ...
      Set<Integer> msgIdsInThisDataSource =
          DbDataSource.getConvMessageIdsInDataSource(mbox, conv.getId(), dataSourceId);
  ...
  protected PurgeableConvs getOldestConversations(
      List<DataSource> dataSources, long size, Integer convId) throws ServiceException {
    ZimbraLog.datasource.info(
        String.format("finding %d bytes to purge from %d data sources", size, dataSources.size()));
    ...
    while (accumulated <= size) {
      if (purgeQueue.isEmpty()) {
        ...
        long convSizeToBuffer =
            Provisioning.getInstance().getConfig().getPurgedConversationsQueueSize();
        boolean hasData = false;
        for (PurgeableConv conv :
            DbDataSource.getOldestConversationsUpToSize(dataSources, convSizeToBuffer, startDate)) {
          hasData = true;
          purgeQueue.enqueue(conv);
        }
        if (!hasData) {
          // edge case: we ran out of purgeable conversations.
          // this can really only happen if the only conversation left is the one that's being
          // appended to,
          // or if the incoming message is larger than the data source quota
          ZimbraLog.datasource.warn(
              String.format(
                  "cannot purge sufficient data for data source %s (%s)", cacheKey, cacheKey));
          return convs;
        }
      }
  ...
  public static class ConversationPurgeQueue {
    // map that is used to look up all nodes in all queues containing OldestConvs with the same ID
    // as the key
