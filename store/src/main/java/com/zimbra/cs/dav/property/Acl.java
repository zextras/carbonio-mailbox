...
public class Acl extends ResourceProperty {
    ...
    for (Grant g : mAcl.getGrants()) {
      try {
        if (ownerAccount != null
            && authAccount.compareTo(ownerAccount) != 0
            && g.getGrantedRights(authAccount, mAcl) == 0) continue;
  ...
  protected Element addGrantDeny(Element ace, Grant g, boolean isGrant) {
    Element grant =
        isGrant ? ace.addElement(DavElements.E_GRANT) : ace.addElement(DavElements.E_DENY);
  ...
  protected Element addPrivileges(Element grant, short rights) {
    grant
        .addElement(DavElements.E_PRIVILEGE)
        .addElement(DavElements.E_READ_CURRENT_USER_PRIVILEGE_SET);
  ...
  private static class SupportedPrivilegeSet extends Acl {
    ...
    public Element toElement(DavContext ctxt, Element parent, boolean nameOnly) {
      ...
      if (nameOnly) return sps;

      if (mAcl == null) return sps;
  ...
  private static class CurrentUserPrivilegeSet extends Acl {
    ...
    public Element toElement(DavContext ctxt, Element parent, boolean nameOnly) {
      ...
      if (ctxt.getAuthAccount().getId().equals(mOwnerId)) {
        addPrivileges(
            cups, (short) (ACL.RIGHT_READ | ACL.RIGHT_WRITE | ACL.RIGHT_DELETE | ACL.RIGHT_INSERT));
        return cups;
      }
  ...
  private static class CurrentUserPrincipal extends Acl {
    ...
    public Element toElement(DavContext ctxt, Element parent, boolean nameOnly) {
      ...
      cup.addElement(DavElements.E_HREF)
          .setText(UrlNamespace.getPrincipalUrl(ctxt.getAuthAccount()));
      return cup;
    }
  }
  ...
  public static class Ace {
    ...
    public Ace(Element a) throws DavException {
      ...
      for (Element p : principal) {
        ...
        if (name.equals(DavElements.E_HREF)) {
          ...
          try {
            ...
            if (acc == null)
              throw new DavException(
                  "invalid principal: " + mPrincipalUrl, HttpServletResponse.SC_BAD_REQUEST);
            mId = acc.getId();
          } catch (ServiceException se) {
            throw new DavException(
                "can't find principal: " + mPrincipalUrl,
          ...
          throw new DavException(
              "unsupported principal: " + name.getName(), HttpServletResponse.SC_NOT_IMPLEMENTED);
        }
      }
      if (elem != null) elem = elem.element(DavElements.E_HREF);
      ...
      for (Element p : priv) {
        ...
        if (right.size() != 1)
          throw new DavException(
              "number of right elements contained in privilege element is not one",
    ...
    public String getPrincipalUrl() {
      if (mPrincipalUrl != null) return mPrincipalUrl;
