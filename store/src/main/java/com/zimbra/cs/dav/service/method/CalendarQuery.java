...
public class CalendarQuery extends Report {
  ...
  public void handle(DavContext ctxt) throws DavException, ServiceException {
    ...
    if (!query.getQName().equals(DavElements.E_CALENDAR_QUERY)) {
      throw new DavException(
          "msg " + query.getName() + " is not calendar-query",
    ...
    if (qctxt.componentFilter == null) {
      throw new DavException(
          "missing filter element in the request", HttpServletResponse.SC_BAD_REQUEST, null);
    }
     ...
     * http://tools.ietf.org/html/rfc4791#section-7.8 CALDAV:calendar-query REPORT The response body
     * for a successful request MUST be a DAV:multistatus XML element (i.e., the response uses the
     * same format as the response for PROPFIND). In the case where there are no response elements,
     * the returned DAV:multistatus XML element is empty.
     */
  ...
  private void handleCalendarItem(QueryContext ctxt, DavResource calItem) {
    if (!(calItem instanceof CalendarObject)) return;
    try {
  ...
  private static class QueryContext {
          ...
          if (name.equals(DavElements.E_FILTER)) {
            ...
            for (Object obj : elem.elements())
              if (obj instanceof Element) {
                elem = (Element) obj;
                if (elem.getQName().equals(DavElements.E_CALENDAR_DATA)) parseCalendarData(elem);
              }
          }
        }
      }
    ...
    private void parseCalendarData(Element cd) {
      ...
      if (comp != null) requestedComponent = new RequestedComponent(comp);
      Element expand = cd.element(DavElements.E_EXPAND);
      if (expand != null) expandRange = new ExpandRange(expand);
    }
    ...
    private void parseFilter(Element filter) {
      for (Object o : filter.elements()) {
        if (o instanceof Element) {
          Element e = (Element) o;
          if (e.getQName().equals(DavElements.E_COMP_FILTER)) componentFilter = new CompFilter(e);
        }
      }
    }
  }
    ...
    void parse(Element elem) {
      ...
      for (Object o : elem.elements()) {
        if (o instanceof Element) {
          ...
          if (qname.equals(DavElements.E_PROP)) props.add(e.attributeValue(DavElements.P_NAME));
          else if (qname.equals(DavElements.E_COMP)) comps.add(new RequestedComponent(e));
        }
      }
    }
  }
