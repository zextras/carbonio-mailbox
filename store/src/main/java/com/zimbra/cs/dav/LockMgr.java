 ...
 *
 * <p>We don't support locking with depth infinity. All the locks are implemented as advisory, with
 * relatively short timeout of 10 mins. The server keeps track of the most recent 100 locks only.
 *
 * @author jylee
 */
public class LockMgr {
  ...
  public static class Lock {
    ...
    public String getTimeoutStr() {
      ...
      if (timeoutInSec < 0) return sTIMEOUTINFINITE;
  ...
  private synchronized Lock hasLock(String owner, String path, LockType type, LockScope scope)
    ...
    for (Lock l : getLocks(path)) {
      if (l == null) continue;
      else if (l.owner.compareTo(owner) == 0) return l;
  ...
  public synchronized Lock createLock(
      DavContext ctxt, String owner, String path, LockType type, LockScope scope, String depth)
      throws DavException {
  ...
  public synchronized void deleteLock(DavContext ctxt, String path, String token) {
    ...
    if (!locks.contains(token)) return;
