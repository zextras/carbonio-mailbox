...
public class CalendarCollection extends Collection {
  ...
  public CalendarCollection(DavContext ctxt, Folder f) throws DavException, ServiceException {
    ...
    addProperty(
        ResourceProperty.AddMember.create(UrlNamespace.getFolderUrl(ctxt.getUser(), f.getName())));

    if (f.getDefaultView() == MailItem.Type.APPOINTMENT
        || f.getDefaultView() == MailItem.Type.TASK) {
  ...
  public java.util.Collection<DavResource> getChildren(DavContext ctxt, TimeRange range)
      throws DavException {
    ...
    if (fetchAppts) {
      try {
        ...
        if (range == null && needCalendarData(ctxt)) mAppts = requestedAppts;
  ...
  protected boolean needCalendarData(DavContext ctxt) throws DavException {
    ...
    if (method.equals(Get.GET) || method.equals(Delete.DELETE)) return true;
    for (QName prop : ctxt.getRequestProp().getProps()) if (!sMetaProps.contains(prop)) return true;
    return false;
  }
  ...
  protected Map<String, DavResource> getAppointmentMap(DavContext ctxt, TimeRange range)
    ...
    if (!needCalendarData(ctxt)) {
      ...
      for (CalendarItem.CalendarMetadata item : mbox.getCalendarItemMetadata(getId(), start, end)) {
        appts.put(
            item.uid, new CalendarObject.LightWeightCalendarObject(getUri(), getOwner(), item));
      }
    } else {
      for (CalendarItem calItem :
          mbox.getCalendarItemsForRange(ctxt.getOperationContext(), start, end, getId(), null))
  ...
  public java.util.Collection<DavResource> getAppointmentsByUids(
    ...
    for (String uid : calItems.keySet()) {
      ...
      else appts.add(new CalendarObject.LocalCalendarObject(ctxt, calItem));
    }
    return appts;
  }
      ...
      if (mItemType == MailItem.Type.APPOINTMENT || mItemType == MailItem.Type.TASK) {
        if (uid != null && uid.compareTo(i.getUid()) != 0)
          throw new DavException.InvalidData(
              DavElements.E_VALID_CALENDAR_OBJECT_RESOURCE, "too many components");
        uid = i.getUid();
      }

      if (itemType != null && itemType != mItemType)
        throw new DavException.InvalidData(
            DavElements.E_VALID_CALENDAR_OBJECT_RESOURCE,
            "different types of components in the same resource");
      else itemType = mItemType;

      if (i.isRecurrence()) inviteList.addFirst(i);
      else inviteList.addLast(i);
    }
    if (uid == null)
      throw new DavException.InvalidData(
          DavElements.E_SUPPORTED_CALENDAR_COMPONENT, "no event in the request");

    if ((getDefaultView() == MailItem.Type.APPOINTMENT || getDefaultView() == MailItem.Type.TASK)
        && (itemType != getDefaultView()))
      throw new DavException.InvalidData(
  ...
  /**
   * Check that we have a valid organizer field and if not, make adjustments if appropriate. For
   * Vanilla CalDAV access where Apple style delegation has not been enabled, attempts by the
    ...
    try {
      ...
      if (e.getCode().equals(ServiceException.FORBIDDEN)) {
        ...
        throw new DavException(
            "cannot create icalendar item", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
      }
    }
  }
   ...
   *     collection are not.
   */
  public DavResource createItemFromInvites(
      DavContext ctxt, Account account, String name, List<Invite> invites, boolean allowUpdate)
    ...
    try {
      ...
      if (origCalItem != null && !allowUpdate) {
        throw new DavException.UidConflict(
            "An item with the same UID already exists in the calendar",
      ...
      if (useEtag) {
        ...
        if (!itemEtag.equals(etag)) {
          throw new DavException(
              String.format(
                  "CalDAV client has stale event: event has different etag (%s) vs %s",
                  itemEtag, etag),
      ...
      AutoScheduler autoScheduler =
          AutoScheduler.getAutoScheduler(
              mbox,
              this.getCalendarMailbox(ctxt),
              origInvites,
              mId,
              flags,
              tags,
      ...
      if (newCalItem == null) {
        throw new DavException(
            "cannot create icalendar item - corrupt ICAL?",
            HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
      }
      if (!uid.equals(name)) {
        if (acceptableClientChosenBasename) {
          DavNames.put(
              DavNames.DavName.create(this.mMailboxId, newCalItem.getFolderId(), baseName),
              newCalItem.getId());
        }
      }
      ...
      // Some clients will keep trying with the same data if get INTERNAL_SERVER_ERROR.  Better to
      // use
  ...
  private List<Invite> uploadToInvites(DavContext ctxt, Account account)
    ...
    if (!contentType.startsWith(MimeConstants.CT_TEXT_CALENDAR)) {
      throw new DavException.InvalidData(
          DavElements.E_SUPPORTED_CALENDAR_DATA,
          String.format(
    ...
    try (InputStream is = ctxt.getUpload().getInputStream()) {
      ZCalendar.ZVCalendar vcalendar =
          ZCalendar.ZCalendarBuilder.build(is, MimeConstants.P_CHARSET_UTF8);
      ...
      throw new DavException.InvalidData(
          DavElements.E_VALID_CALENDAR_DATA,
          String.format(
              "Problem parsing %s data - %s", MimeConstants.CT_TEXT_CALENDAR, se.getMessage()));
    }
    return invites;
  }

  private String defaultUrlForCalendarItem(String user, String uid)
      throws DavException, ServiceException {
  ...
  public String getFreeBusyReport(DavContext ctxt, TimeRange range)
      throws ServiceException, DavException {
    Mailbox mbox = getCalendarMailbox(ctxt);
    FreeBusy fb =
        mbox.getFreeBusy(
