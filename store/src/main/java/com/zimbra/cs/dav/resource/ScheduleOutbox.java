...
public class ScheduleOutbox extends CalendarCollection {
  ...
  public void handlePost(DavContext ctxt) throws DavException, IOException, ServiceException {
    if (ctxt.getRequest().getHeader(DavProtocol.HEADER_SCHEDULE_AGENT).equalsIgnoreCase("client")) {
      return;
    }
    ...
    ZCalendar.ZVCalendar vcalendar =
        ZCalendar.ZCalendarBuilder.build(in, MimeConstants.P_CHARSET_UTF8);
    Closeables.closeQuietly(in);
    Iterator<ZComponent> iter = vcalendar.getComponentIterator();
    ZComponent req = null;
    while (iter.hasNext()) {
      req = iter.next();
      if (req.getTok() != ICalTok.VTIMEZONE) break;
      req = null;
    }
    if (req == null) {
    ...
    boolean isVEventOrVTodo =
        ICalTok.VEVENT.equals(req.getTok()) || ICalTok.VTODO.equals(req.getTok());
    ...
    if (isVEventOrVTodo
        && delegationInfo.getOriginator() != null
        && ctxt.getAuthAccount() != null) {
        ...
        if (isOrganizerMethod) {
          ...
          for (String at : attendees) {
            ...
            if (delegationInfo.getOriginatorEmail().equalsIgnoreCase(atEmail)) {
              ...
              ZimbraLog.dav.debug(
                  "changing originator to %s to match address/alias used in ATTENDEE",
                  delegationInfo.getOriginator());
              break;
            }
          }
        ...
        if (rcpt != null) {
          ...
          if (rcpt.length() != 0) {
            ...
            if (isVEventOrVTodo) {
              ...
              // iCal sends cancel notice to all original attendees when some attendees are removed
              // from the
              // appointment.  As a result the appointment is cancelled from the calendars of all
              // original
              // attendees.  Counter this bad behavior by filtering out any recipients who aren't
              // listed
              // as ATTENDEE in the CANCEL component being sent.  (iCal does that part correctly, at
              // least.)
              if (isCancel) {
    ...
    for (String rcpt : rcptArray) {
      ...
      switch (req.getTok()) {
        case VFREEBUSY:
          ...
          break;
        case VEVENT:
          // adjustOrganizer works around issues where we don't have delegation enabled but clients
          // write
          // to the shared calendar as if it was the delegate's calendar instead of the owner.
          // Note assumption that clients won't generate replies on behalf of owner as they won't be
          // aware
          // that they should be working on behalf of the owner and will only look for their own
          // attendee
          // records.
          if (isOrganizerMethod) {
        ...
        default:
          throw new DavException(
              "unrecognized request: " + req.getTok(), HttpServletResponse.SC_BAD_REQUEST);
      }
    }
  }

  /**
   * Check for illegal requests like trying to CANCEL a meeting when not the organizer or a delegate
   * for the organizer e.g. Bug 85875 Mac OS X/10.8.5 Calendar sometimes sending CANCEL when
   * ATTENDEE deletes an instance, resulting in other attendees getting invalid CANCELs
   */
  private void validateRequest(
      boolean isOrganizerMethod,
      DelegationInfo delegationInfo,
      String organizer,
      DavContext ctxt,
      ZComponent req)
    ...
    if ((!isOrganizerMethod)
        || (organizer != null && organizer.equals(delegationInfo.getOriginator()))) {
      return;
    }
    ...
    if (!acctMatcher.matches(delegationInfo.getOriginatorEmail())) {
      throw new DavException(
          String.format(
              "invalid POST to scheduling outbox '%s'. originator '%s' is not authorized account or"
                  + " ORGANIZER",
  ...
  private void handleFreebusyRequest(
      DavContext ctxt, ZComponent vfreebusy, String originator, String rcpt, Element resp)
    ...
    if (dtstartProp == null || dtendProp == null && durationProp == null)
      throw new DavException(
          "missing dtstart or dtend/duration in the schedule request",
          HttpServletResponse.SC_BAD_REQUEST,
          null);
    long start, end;
    try {
    ...
    ZimbraLog.dav.debug(
        "rcpt: " + rcpt + ", start: " + new Date(start) + ", end: " + new Date(end));

    FreeBusy fb = null;
    if (ctxt.isFreebusyEnabled()) {
      FreeBusyQuery fbQuery =
          new FreeBusyQuery(ctxt.getRequest(), ctxt.getAuthAccount(), start, end, null);
      ...
      if (fbResult.size() > 0) fb = fbResult.iterator().next();
    }
    if (fb != null) {
  ...
  private void handleEventRequest(
    ...
    if (method.equals("REQUEST")) {
      ...
      if (organizerProp != null) {
        String organizerStr =
            this.getAddressFromPrincipalURL(new ZOrganizer(organizerProp).getAddress());
    ...
    if (status.equals("CANCELLED")) subject = "Cancelled: ";
    ...
    try {
      ...
      FriendlyCalendaringDescription friendlyDesc =
          new FriendlyCalendaringDescription(components, ctxt.getAuthAccount());
      String desc = friendlyDesc.getAsPlainText();
      String descHtml = req.getDescriptionHtml();
      if ((descHtml == null) || (descHtml.length() == 0)) descHtml = friendlyDesc.getAsHtml();
      Mailbox mbox = MailboxManager.getInstance().getMailboxByAccount(ctxt.getAuthAccount());
      MimeMessage mm =
          CalendarMailSender.createCalendarMessage(
              target, from, sender, recipients, subject, desc, descHtml, uid, cal);
      mbox.getMailSender()
          .setSendPartial(true)
          .sendMimeMessage(
  ...
  protected String getAddressFromPrincipalURL(String url) throws ServiceException, DavException {
    ...
    if (url.startsWith("http://")) {
      ...
      if (pos != -1) {
        ...
        if (organizer == null)
          throw new DavException(
              "user not found: " + userId, HttpServletResponse.SC_BAD_REQUEST, null);
  ...
  /**
   * For Vanilla CalDAV access where Apple style delegation has not been enabled, attempts by the
   * delegate to use a shared calendar acting as themselves are translated to appear as if acting as
   * a delegate, otherwise the experience can be very poor.
   *
   * @throws ServiceException
   */
  private void adjustOrganizer(
      DavContext ctxt, ZCalendar.ZVCalendar cal, ZComponent req, DelegationInfo delegationInfo)
      throws ServiceException {
    // BusyCal 2.5.3 seems to post with the wrong organizer even if ical delegation is switched on -
    // even though
    ...
    for (ZComponent component : cal.getComponents()) {
      ...
      if (organizerProp != null) {
        ...
        if (sentBy == null) {
          organizerProp.addParameter(
              new ZParameter(ICalTok.SENT_BY, delegationInfo.getOriginator()));
        } else {
