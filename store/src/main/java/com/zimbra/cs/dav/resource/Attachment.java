 ...
 *
 * <p>Using WebDAV client, email attachments in the mailbox can be browsed in file manager like
 * interface. There are predefined phantom folders like /attachment/by-type/image/today whose
 * contents are dynamically updated just like smart folders. In those phantom folders the email
 * attachments are listed as individual files.
 *
 * @author jylee
 */
  ...
  public Attachment(String uri, String owner, List<String> tokens, DavContext ctxt)
    ...
    try {
      ...
      try (ZimbraQueryResults zqr =
          mbox.index.search(
              ctxt.getOperationContext(), query.toString(), SEARCH_TYPES, SortBy.NAME_ASC, 10)) {
        if (zqr.hasNext()) {
