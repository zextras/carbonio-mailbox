...
public class WebDavClient {
  ...
  public Collection<DavObject> listObjects(String path, Collection<QName> extraProps)
      throws IOException, DavException, HttpException {
    ...
    if (extraProps == null) {
      ...
      for (QName p : extraProps) propfind.addRequestProp(p);
    }
    return sendMultiResponseRequest(propfind);
  }

  public Collection<DavObject> sendMultiResponseRequest(DavRequest req)
      throws IOException, DavException, HttpException {
    ...
    try {
      ...
      Document doc =
          W3cDomUtil.parseXMLToDom4jDocUsingSecureProcessing(response.getEntity().getContent());
  ...
  public HttpResponse sendCustomRequest(DavRequest req)
      throws IOException, DavException, HttpException {
    HttpResponse response = executeFollowRedirect(req);
    return response;
  }

  public HttpInputStream sendRequest(DavRequest req)
      throws IOException, DavException, HttpException {
    HttpResponse response = executeFollowRedirect(req);
  ...
  public HttpInputStream sendPut(
      String href,
      byte[] buf,
      String contentType,
      String etag,
    ...
    while (!done) {
      ...
      if (etag != null) put.addHeader(DavProtocol.HEADER_IF_MATCH, etag);
      if (headers != null)
  ...
  protected void logRequestInfo(HttpRequestBase method, String body) throws IOException {
    ...
    reqLog
        .append("WebDAV request:\n")
        .append(method.getMethod())
        .append(" ")
        .append(method.getURI().toString());
    reqLog.append('\n');
  ...
  protected HttpResponse executeMethod(HttpRequestBase m, Depth d, String bodyForLogging)
      throws IOException, HttpException {
  ...
  protected HttpResponse executeMethod(HttpRequestBase m, Depth d)
      throws IOException, HttpException {
