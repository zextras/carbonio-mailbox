...
public class DavObject {
  ...
  public DavObject(Element objElem) {
    ...
    if (!objElem.getQName().equals(DavElements.E_RESPONSE)) return;
    Element href = objElem.element(DavElements.E_HREF);
    if (href != null) mHref = href.getText();
    for (Object obj : objElem.elements(DavElements.E_PROPSTAT)) {
      ...
      if (statusElem != null) status = parseStatusCode(statusElem.getText());
  ...
  private int parseStatusCode(String str) {
    ...
    try {
      if (tokens.length > 2) status = Integer.parseInt(tokens[1]);
