...
public interface CalendarObject {
  ...
  public abstract static class LocalCalendarObjectBase extends MailItemResource {
    ...
    protected static Invite getFixedUpCopy(
        DavContext ctxt, Invite inv, Account acct, boolean delegated, boolean isScheduleMsg)
        throws ServiceException {
      if (ctxt.isIcalClient() && !inv.isOrganizer()) {
        ...
        if (!delegated) {
          if (attendee != null) {
            ...
            // iCal supports accept/decline action only when user is listed as attendee.  (bug
            // 58513)
  ...
  public static class CalendarPath {
     ...
     * @param extra if greater than zero, ",<extra val>" is added before the .ics extension This is
     *     needed in places like the scheduling inbox where UID is not necessarily unique. For
    ...
    /**
     * @param extra if greater than zero, ",<extra val>" is added before the .ics extension. Needed
     *     where UID is not unique
     */
  ...
  public static class ScheduleMessage extends LocalCalendarObjectBase implements CalendarObject {
    ...
    public String getVcalendar(DavContext ctxt, Filter filter) throws IOException, DavException {
      ...
      try {
        ...
        try {
          ...
          allowPrivateAccess =
              CalendarItem.allowPrivateAccess(
    ...
    public void delete(DavContext ctxt) throws DavException {
      try {
        ...
        int resCode =
            se instanceof MailServiceException.NoSuchItemException
  ...
  public static class LightWeightCalendarObject extends DavResource implements CalendarObject {
    ...
    public LightWeightCalendarObject(
        String path, String owner, CalendarItem.CalendarMetadata data) {
      ...
      mEnd = data.end_time;
      mEtag =
          MailItemResource.getEtag(
              Integer.toString(data.mod_metadata), Integer.toString(data.mod_content));
    ...
    private CalendarObject getFullResource(DavContext ctxt) throws DavException {
      ...
      if (acct != null) user = acct.getName();
      try {
        DavResource rs = UrlNamespace.getResourceByItemId(ctxt, user, mId);
        if (rs instanceof LocalCalendarObject) return (LocalCalendarObject) rs;
        else throw new DavException("not a calendar item", HttpServletResponse.SC_BAD_REQUEST);
      } catch (ServiceException se) {
  ...
  public static class LocalCalendarObject extends LocalCalendarObjectBase
    ...
    public LocalCalendarObject(DavContext ctxt, CalendarItem calItem, boolean newItem)
        throws ServiceException {
      this(
          ctxt,
          CalendarPath.generate(
              ctxt,
              calItem.getPath(),
              calItem.getUid(),
              calItem.getMailboxId(),
              calItem.getId(),
              -1),
          calItem);
    ...
    public LocalCalendarObject(DavContext ctxt, String path, CalendarItem calItem)
        throws ServiceException {
      ...
      if (defInv != null) setProperty(DavElements.P_DISPLAYNAME, defInv.getName());
    ...
    public boolean match(Filter filter) {
      TimeRange range = filter.getTimeRange();
      if (range != null && !range.matches(mMailboxId, mId, mStart, mEnd)) return false;
    ...
    private ZVCalendar createZVcalendar(
        List<ZComponent> components, Map<String, ICalTimeZone> oldIdsToNewTZsMap) {
    ...
    /**
     * Returns iCalendar representation of events that matches the supplied filter. Unused timezones
     * are removed and well known timezones are used in preference to original timezones.
     */
    public ZVCalendar getZVcalendar(DavContext ctxt, Filter filter)
      ...
      if (!LC.calendar_apple_ical_compatible_canceled_instances.booleanValue()) {
        ...
        ZComponent[] vcomps =
            Invite.toVComponents(fixedInvs, allowPrivateAccess, false, appleICalExdateHack);
        if (vcomps != null) {
      ...
      try {
        for (CalendarItem.Instance instance :
          ...
          if (instance.isException() == false) {
            ...
            if (defInvite.isAllDayEvent()) datetime.forceDateOnly();
            else {
        ...
        ZimbraLog.dav.warn(
            "error getting calendar item " + mUid + " from mailbox " + mMailboxId, se);
      }
    }
  }
}
