 ...
 *
 * <p>Collections map to mailbox folders.
 */
public class Collection extends MailItemResource {
  ...
  public String getContentType(DavContext ctxt) {
    if (ctxt.isWebRequest()) return MimeConstants.CT_TEXT_PLAIN;
  ...
  private List<MailItem> getChildrenMailItem(DavContext ctxt)
  ...
  public Collection mkCol(DavContext ctxt, String name, MailItem.Type view) throws DavException {
    try {
      ...
      else throw new DavException("can't create", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
    }
  }
  ...
  public DavResource createItem(DavContext ctxt, String name) throws DavException, IOException {
    ...
    try {
      ...
      throw new DavException(
          "cannot get mailbox", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, null);
    }
    ...
    try {
      ...
      if (item.getType() != MailItem.Type.DOCUMENT && item.getType() != MailItem.Type.WIKI) {
        throw new DavException(
            "no DAV resource for " + item.getType(), HttpServletResponse.SC_NOT_ACCEPTABLE, null);
      }
      Document doc =
      ...
      if (!(e instanceof NoSuchItemException))
        throw new DavException(
            "cannot get item ", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, null);
    }
    ...
    try {
      Document doc =
          mbox.createDocument(
              ctxt.getOperationContext(), mId, name, ctype, author, null, upload.getInputStream());
  ...
  protected String fullUrlForChild(String user, String basename)
      throws DavException, ServiceException {
