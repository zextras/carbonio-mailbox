 ...
 * CALDAV:supported-calendar-component-sets -
 * http://tools.ietf.org/html/draft-daboo-caldav-extensions-01#section-4
 * CALDAV:supported-calendar-data - draft-dusseault-caldav section 5.2.4
 * CALDAV:supported-collation-set - draft-dusseault-caldav section 7.5.1 CALDAV:calendar-data -
 * draft-dusseault-caldav section 9.6 CALDAV:calendar-home-set - draft-dusseault-caldav section
 * 6.2.1
 *
 * @author jylee
 */
public class CalDavProperty extends ResourceProperty {
  ...
  /**
   * From : http://tools.ietf.org/html/draft-daboo-caldav-extensions-01#section-4.1 Purpose:
   * Enumerates the sets of component restrictions the server is willing to allow the client to
   * specify in MKCALENDAR or extended MKCOL requests. Description: If servers apply restrictions on
   * the allowed calendar component sets used when creating a calendar, then those servers SHOULD
   * advertise this property on each calendar home collection within which the restrictions apply.
   * In the absence of this property, clients cannot assume anything about whether the server will
   * enforce a set of restrictions or not - in that case clients need to handle the server rejecting
   * certain combinations of restricted component sets. If this property is present, but contains no
   * child XML elements, then clients can assume that the server imposes no restrictions on the
   * combinations of component types it is willing to accept. If present, each
   * CALDAV:supported-calendar-component-set element represents a valid restriction the client can
   * use in an MKCALENDAR or extended MKCOL request when creating a calendar.
   *
   * <p><C:supported-calendar-component-sets xmlns:C="urn:ietf:params:xml:ns:caldav">
   * <C:supported-calendar-component-set> <C:comp name="VEVENT" />
   * </C:supported-calendar-component-set> <C:supported-calendar-component-set> <C:comp name="VTODO"
   * /> </C:supported-calendar-component-set> </C:supported-calendar-component-sets>
   */
  private static class SupportedCalendarComponentSets extends CalDavProperty {
    public SupportedCalendarComponentSets() {
      ...
      mChildren.add(
          new SupportedCalendarComponentSet(MailItem.Type.APPOINTMENT, false).toElement(false));
  ...
  private static class CalendarUserType extends CalDavProperty {
    public CalendarUserType(Principal p) {
      ...
      if (resType == null) setStringValue(INDIVIDUAL);
      else if (resType.compareTo("Equipment") == 0) setStringValue(RESOURCE);
      else if (resType.compareTo("Location") == 0) setStringValue(ROOM);
      else setStringValue(UNKNOWN);
    }
  }
