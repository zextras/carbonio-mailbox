...
public class ScheduleInbox extends CalendarCollection {
  ...
  @Override
  public java.util.Collection<DavResource> getChildren(DavContext ctxt, TimeRange tr)
      throws DavException {
    try {
  ...
  public java.util.Collection<DavResource> getAppointmentsByUids(
    ...
    try (ZimbraQueryResults zqr =
        mbox.index.search(ctxt.getOperationContext(), query, SEARCH_TYPES, SortBy.DATE_ASC, 100)) {
      while (zqr.hasNext()) {
        ...
        if (hit instanceof MessageHit) {
          ...
          if (rs != null) {
            ...
            if (hrefs == null) result.add(rs);
              ...
              if (!found) result.add(new DavResource.InvalidResource(href, getOwner()));
            }
          }
        }
      }
  ...
  public void patchProperties(
      ...
      if (el.getQName().equals(DavElements.E_CALENDAR_FREE_BUSY_SET)) {
        ...
        try {
          ...
          ctxt.getResponseProp()
              .addPropError(
                  DavElements.E_CALENDAR_FREE_BUSY_SET,
    ...
    if (newSet != null) set = newSet;
    super.patchProperties(ctxt, set, remove);
  }

  private void updateCalendarFreeBusySet(DavContext ctxt, ArrayList<String> urls)
  ...
  private java.util.Collection<Folder> getCalendarFolders(DavContext ctxt, final int folderId)
    ...
    for (Folder f : mbox.getFolderList(ctxt.getOperationContext(), SortBy.NONE, folderId)) {
      if (!(f instanceof Mountpoint)
          && (f.getDefaultView() == MailItem.Type.APPOINTMENT
