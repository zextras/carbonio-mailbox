...
public abstract class Filter {
  ...
  private static final class AttrMapping {
    ...
    public void addSubType(String k, String... vals) {
      if (type == MappingType.simple) return;
      String[] v = new String[vals.length];
      int i = 0;
      for (String val : vals) v[i++] = val;
      structures.put(k, v);
    }

    public String[] getAttrs() {
      if (type == MappingType.simple) return new String[] {contactAttr};
      HashSet<String> ret = new HashSet<String>();
      for (String[] val : structures.values()) Collections.addAll(ret, val);
  ...
  static {
    ...
    AttrMapping.createStructured(
        "ADR", "home", ContactConstants.A_homeStreet, "work", ContactConstants.A_workStreet);
    AttrMapping.createStructured(
        "URL", "home", ContactConstants.A_homeURL, "work", ContactConstants.A_workURL);

    m =
        AttrMapping.createStructured(
            "TEL",
            "car",
            ContactConstants.A_carPhone,
            "cell",
            ContactConstants.A_mobilePhone,
            "pager",
            ContactConstants.A_pager,
            "other",
            ContactConstants.A_otherPhone);
    ...
    m.addSubType(
        "internet", ContactConstants.A_email, ContactConstants.A_email2, ContactConstants.A_email3);
  ...
  protected void parse(Element elem) {
    for (Object o : elem.elements()) {
      if (o instanceof Element) {
        ...
        else if (name.equals(DavElements.CardDav.E_TEXT_MATCH)) mTextMatch = new TextMatch(e, this);
        else if (name.equals(DavElements.CardDav.E_IS_NOT_DEFINED)) mIsNotDefinedSet = true;
        else
          ZimbraLog.dav.info(
              "unrecognized filter " + name.getNamespaceURI() + ":" + name.getName());
      }
    }
  }
  ...
  public enum MatchType {
    ...
    public static MatchType fromString(String s) {
      try {
        if (s != null) return MatchType.valueOf(s.replace('-', '_'));
  ...
  public static class TextMatch extends Filter {
    ...
    public TextMatch(Element elem, Filter parent) {
      ...
      mNegate =
          elem.attributeValue(DavElements.P_NEGATE_CONDITION, DavElements.NO)
              .equals(DavElements.YES);
      mText = elem.getText();
  ...
  public static class PropFilter extends Filter {
    ...
    public boolean match(Contact contact) {
      ...
      if (val == null) return mIsNotDefinedSet;
      boolean matched = true;
      for (ParamFilter pf : mParams) matched &= pf.match(val);
    ...
    public Collection<AddressObject> match(DavContext ctxt, AddressbookCollection folder) {
      if (mIsNotDefinedSet) {
        ...
        ArrayList<AddressObject> result = new ArrayList<AddressObject>();
        for (ParamFilter f : mParams) result.addAll(f.match(ctxt, folder));
        return result;
      }
      return null;
    }
