...
public class AddressObject extends MailItemResource {
  ...
  private static void constructContactGroupFromAppleXProps(
      DavContext ctxt, Account ownerAccount, VCard vcard, Contact existingContact, int folderId) {
    ...
    if (kind != null && kind.compareTo("group") == 0) {
      ...
      try {
        if (existingContact == null) { // create
          ...
          // remove all the contacts of type CONTACT_REF that belong to the collection same as the
          // group
          ...
          for (Member member : contactGroup.getMembers()) {
            if (Member.Type.CONTACT_REF.equals(member.getType())) {
              ...
              if (itemId.belongsTo(existingContact.getAccount())) {
                ...
                Contact c =
                    getContactByUID(
                        ctxt, itemId.toString(), existingContact.getAccount(), folderId);
                if (c != null) {
  ...
  private static void populateContactGroupAppleXProps(DavContext ctxt, Contact contact) {
    ...
    try {
      contactGroup =
          ContactGroup.init(
              contact.get(ContactConstants.A_groupMember), contact.getMailbox().getAccountId());
    } catch (ServiceException e) {
        ...
        for (Member member : contactGroup.getMembers()) {
          if (member.getType().equals(Member.Type.CONTACT_REF)) {
            ...
            if (itemId.belongsTo(contact.getAccount())) {
              ...
              Contact c =
                  getContactByUID(
                      ctxt, itemId.toString(), contact.getAccount(), contact.getFolderId());
              if (c != null) {
  ...
  public String toVCard(DavContext ctxt, java.util.Collection<String> attrs)
      throws ServiceException, DavException {
    if (attrs == null || attrs.isEmpty()) return toVCard(ctxt);
    Contact contact = (Contact) getMailItem(ctxt);
    populateContactGroupAppleXProps(ctxt, contact);
  ...
  public static VCard uploadToVCard(DavContext ctxt) throws DavException, IOException {
    ...
    if (!acceptableVCardContentType(contentType, true)) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_SUPPORTED_ADDRESS_DATA,
          String.format(
              "Incorrect Content-Type '%s', expected '%s'",
    ...
    try (InputStream is = ctxt.getUpload().getInputStream()) {
      String buf =
          new String(ByteUtil.getContent(is, (int) uploadSize), MimeConstants.P_CHARSET_UTF8);
      vcards = VCard.parseVCard(buf);
    } catch (ServiceException se) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA,
          String.format(
              "Problem parsing %s data - %s", DavProtocol.VCARD_CONTENT_TYPE, se.getMessage()));
    }
    if (vcards == null) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA,
          String.format(
              "Problem parsing %s data - no cards produced.", DavProtocol.VCARD_CONTENT_TYPE));
    }
    if (vcards.size() != 1) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA,
          String.format(
              "Problem parsing %s data - %d cards produced, only 1 allowed.",
   ...
   * @param allowUpdate - PUTs are allowed to update a pre-existing item. POSTs to the containing
   *     collection are not.
   */
  public static DavResource create(
      DavContext ctxt, String name, Collection where, boolean allowUpdate)
   ...
   */
  public static DavResource create(
      DavContext ctxt, String name, Collection where, VCard vcard, boolean allowUpdate)
      throws DavException, IOException {
    if (vcard.fields.isEmpty()) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA,
          String.format(
              "Problem parsing %s data - no fields found.", DavProtocol.VCARD_CONTENT_TYPE));
    }
    ...
    try {
      ...
      if (uid != null) {
        ...
        AddressObject ao =
            getAddressObjectByUID(
                ctxt,
                uid,
                ctxt.getTargetMailbox().getAccount(),
                ctxt.getTargetMailbox().getFolderById(ctxt.getOperationContext(), where.mId),
                baseName);
        if (ao != null) {
          if (!allowUpdate) {
      ...
      if (res == null) {
        ...
        c =
            mbox.createContact(
                ctxt.getOperationContext(), vcard.asParsedContact(), where.mId, null);
        ...
        if (useEtag && (etag != null) && !etag.equals(itemEtag)) {
          throw new DavException(
              "item etag does not match", HttpServletResponse.SC_PRECONDITION_FAILED);
        }
      ...
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA,
          String.format("cannot parse vcard - %s", e.getMessage()),
          e);
    }
    if (res == null) {
      throw new DavException.InvalidData(
          DavElements.CardDav.E_VALID_ADDRESS_DATA, "cannot parse vcard");
    }
    return res;
  }

  public static AddressObject getAddressObjectByUID(
      DavContext ctxt, String uid, Account account, Folder f) throws ServiceException {
    return getAddressObjectByUID(ctxt, uid, account, f, null /* preferredBaseName */);
  }

  /**
   * @param preferredBaseName If more than one item matches the UID, prefer one matching this name -
   *     can be null
   */
  public static AddressObject getAddressObjectByUID(
      DavContext ctxt, String uid, Account account, Folder f, String preferredBaseName)
      throws ServiceException {
    Contact c =
        getContactByUID(ctxt, uid, account, (f != null) ? f.getId() : -1, preferredBaseName);
   ...
   *     can be null
   */
  private static Contact getContactByUID(
      DavContext ctxt, String uid, Account account, int folderId, String preferredBaseName)
      throws ServiceException {
    Contact item = null;
    ...
    if (index > 0) {
      ...
      try {
        if (accountId.equals(account.getId())) id = Integer.parseInt(uid.substring(index + 1));
    ...
    if (id > 0) {
      item =
          mbox.getContactById(
              ctxt.getOperationContext(), Integer.parseInt(uid.substring(index + 1)));
    } else {
    ...
    if ((item != null) && (folderId >= 0) && (item.getFolderId() != folderId)) item = null;

    return item;
  }

  /**
   * There could be multiple contacts with the same UID from different collections. Due to an old
   * CardDAV bug, it is also possible there may be more than one contact with the same UID in the
   * same collection. Using "preferredBaseName" ensures that CardDAV clients can still do an update
   * in that case.
   *
   * @param zqr
   * @param folderId
   * @param preferredBaseName If more than one item matches, prefer one matching this name - can be
   *     null Ignored if folderId < 0
   * @return
   * @throws ServiceException
   */
  private static Contact getMatchingHit(
      ZimbraQueryResults zqr, int folderId, String preferredBaseName) throws ServiceException {
    Contact item = null;
