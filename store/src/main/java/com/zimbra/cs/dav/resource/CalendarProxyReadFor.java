 ...
 * 5.3.1. CS:calendar-proxy-read-for Property Name: calendar-proxy-read-for Namespace:
 * http://calendarserver.org/ns/ Purpose: Lists principals for whom the current principal is a read-
 * only proxy for. Description: This property allows a client to quickly determine the principal for
 * whom the current principal is a read-only proxy for. The server MUST account for any group
 * memberships of the current principal that are either direct or indirect members of a proxy group.
 * e.g., if principal "A" assigns a group "G" as a read-only proxy, and principal "B" is a member of
 * group "G", then principal "B" will see principal "A" listed in the CS:calendar-proxy-read-for
 * property on their principal resource. Definition: <!ELEMENT calendar-proxy-read-for (DAV:href*)>
 */
