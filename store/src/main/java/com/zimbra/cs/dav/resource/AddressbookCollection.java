...
public class AddressbookCollection extends Collection {
  ...
  protected static void setupAddressbookCollection(Collection coll, DavContext ctxt, Folder f)
    ...
    String description =
        L10nUtil.getMessage(
            MsgKey.carddavAddressbookDescription,
            lc,
    ...
    long maxSize =
        Provisioning.getInstance()
            .getLocalServer()
            .getLongAttr(Provisioning.A_zimbraFileUploadMaxSize, -1);
    if (maxSize > 0) {
  ...
  public void handlePost(DavContext ctxt) throws DavException, IOException, ServiceException {
    ...
    try {
      ...
      String baseName =
          new StringBuilder(vcard.uid).append(AddressObject.VCARD_EXTENSION).toString();
      rs =
          UrlNamespace.getResourceAt(
              ctxt, ctxt.getUser(), relativeUrlForChild(ctxt.getUser(), baseName));
      if (rs != null) {
        // name based on uid already taken - choose another.
        baseName =
            new StringBuilder(UUIDUtil.generateUUID())
                .append(AddressObject.VCARD_EXTENSION)
                .toString();
      }
      ...
      if (e.getCode().equals(ServiceException.FORBIDDEN)) {
        ...
        throw new DavException(
            "cannot create vcard item", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
      }
    }
  }

  /**
   * Filter out non-contact related children. Apple Mac OS X Mavericks Contacts doesn't cope well
   * with them.
   */
  @Override
  public java.util.Collection<DavResource> getChildren(DavContext ctxt) throws DavException {
    ...
    for (DavResource child : allChildren) {
      if (child.isCollection() || child instanceof AddressObject) {
        ...
        ZimbraLog.dav.debug(
            "Ignoring non-address resource %s (class %s) AddressbookCollection child",
