...
/**
 * SearchWrapper is a phantom resource that resembles a Collection (folder) with the contents
 * dynamically generated from email contents in the mailbox.
 *
 * @author jylee
 */
public class SearchWrapper extends PhantomResource {
  ...
  private void buildQuery(String prevTerm, String term) {
    if (term.equals(TODAY)) mQuery.append("after:\"-1day\" ");
    else if (term.equals(WEEK)) mQuery.append("after:\"-1week\" ");
    else if (term.equals(MONTH)) mQuery.append("after:\"-1month\" ");
    else if (term.equals(YEAR)) mQuery.append("after:\"-1year\" ");
    else if (BrowseWrapper.BY_SENDER.equals(prevTerm)) mQuery.append("from:(@" + term + ") ");
    else if (BrowseWrapper.BY_TYPE.equals(prevTerm)) {
      mQuery.append("attachment:\"" + term + "\" ");
      if (!term.equals("any")) mContentType = getActualContentType(term);
    }
  }
  ...
  public Collection<DavResource> getChildren(DavContext ctxt) {
    ...
    try {
      ...
      try (ZimbraQueryResults zqr =
          mbox.index.search(SoapProtocol.Soap12, ctxt.getOperationContext(), params)) {
        while (zqr.hasNext()) {
          ZimbraHit hit = zqr.getNext();
          if (hit instanceof MessageHit) addAttachmentResources((MessageHit) hit, children);
        }
      }
    } catch (Exception e) {
  ...
  private void addAttachmentResources(MessageHit hit, List<DavResource> children) {
    try {
      ...
      for (MPartInfo p : parts) {
        ...
        if (mContentType != null && ct != null && !ct.startsWith(mContentType)) continue;
