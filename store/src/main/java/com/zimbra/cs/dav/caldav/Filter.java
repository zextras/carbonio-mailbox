...
public abstract class Filter {
  ...
  protected void parse(Element elem) {
    for (Object o : elem.elements()) {
      if (o instanceof Element) {
        ...
        else if (name.equals(DavElements.E_TEXT_MATCH)) mTextMatches.add(new TextMatch(e));
        else if (name.equals(DavElements.E_TIME_RANGE)) mTimeRange = new TimeRange(e);
        else if (name.equals(DavElements.E_IS_NOT_DEFINED)) mIsNotDefinedSet = true;
        else
          ZimbraLog.dav.info(
  ...
  protected boolean runTextMatch(String text) {
    ...
    for (TextMatch tm : mTextMatches) matched &= tm.match(text);
    return matched;
  }

  public TimeRange getTimeRange() {
    if (mTimeRange != null) return mTimeRange;
  ...
  public static class TextMatch {
    ...
    public boolean match(String val) {
      ...
      if (ignoreCase) ret = val.equalsIgnoreCase(mText);
      else ret = val.equals(mText);
      if (mNegate) return !ret;
      else return ret;
    }
  }
  ...
  public static class ParamFilter extends Filter {
    ...
    public boolean match(ZCalendar.ZProperty prop) {
      ...
      if (param == null) return mIsNotDefinedSet;
  ...
  public static class PropFilter extends Filter {
    ...
    public boolean match(ZCalendar.ZComponent comp) {
      ...
      if (prop == null) return mIsNotDefinedSet;
      for (ParamFilter pf : mParams) matched &= pf.match(prop);
  ...
  public static class CompFilter extends Filter {
    ...
    public boolean match(ZCalendar.ZComponent comp) {
      ...
      if (mName.equals("VCALENDAR")) {
        if (mComps.size() > 0) {
          matched = false;
          for (CompFilter cf : mComps) matched |= cf.match(comp);
        }
        return matched;
      }
      ...
      if (matched && mComps.size() > 0) {
        ...
        for (CompFilter cf : mComps) {
          ...
          if (tok != null) {
            ...
            if (sub != null) matched |= cf.match(sub);
          }
        }
      }
