...
public class ExpandProperty extends Report {
  ...
  public void handle(DavContext ctxt) throws ServiceException, DavException {
    ...
    if (!query.getQName().equals(DavElements.E_EXPAND_PROPERTY))
      throw new DavException(
          "msg " + query.getName() + " is not expand-property",
    ...
    Element resp =
        ctxt.getDavResponse().getTop(DavElements.E_MULTISTATUS).addElement(DavElements.E_RESPONSE);
    expandProperties(ctxt, rs, query, resp);
  }

  /**
   * @param rs - the requested resource
   * @param elem - specification of what should be expanded - either the top level {@code
   *     <DAV:expand-property>} element or a descendant {@code <DAV:property>} element
  ...
  private void expandProperties(DavContext ctxt, DavResource rs, Element elem, Element resp) {
    ...
    while (iter.hasNext()) {
      ...
      if (rp == null) {
        if (!ctxt.isBrief()) propstat.add(p.getQName(), null, HttpServletResponse.SC_NOT_FOUND);
      } else {
        @SuppressWarnings("rawtypes")
  ...
  private static class Prop {
    ...
    public Prop(Element propElement) {
      ...
      mNamespace =
          propElement.attributeValue(DavElements.P_NAMESPACE, DavElements.WEBDAV_NS_STRING);
