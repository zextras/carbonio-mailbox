...
public class CalDavClient extends WebDavClient {
  ...
  public void login(String defaultPrincipalUrl) throws IOException, DavException, HttpException {
    ...
    if (mCalendarHomeSet.isEmpty()) {
      throw new DavException(
          "dav response from principal url does not contain calendar-home-set", null);
    }
  ...
  public Collection<Appointment> getEtags(String calendarUri)
    ...
    for (DavObject obj : response) {
      ...
      if (!obj.isFolder() && etag != null && href != null) etags.add(new Appointment(href, etag));
    }
    return etags;
  }
  ...
  public String sendCalendarData(Appointment appt) throws IOException, DavException, HttpException {
    HttpInputStream resp =
        sendPut(
            appt.href,
            appt.data.getBytes("UTF-8"),
    ...
    if (mSchedulingEnabled) sendSchedulingMessage(appt);
    return etag;
  }
  ...
  public Collection<Appointment> getCalendarData(String url, Collection<Appointment> hrefs)
    ...
    for (Appointment appt : hrefs) multiget.addHref(appt.href);
    ...
    for (DavObject obj : response) {
      ...
      if (href != null && calData != null) appts.add(new Appointment(href, etag, calData));
    }
    return appts;
  }

  public Collection<Appointment> getAllCalendarData(String url)
      throws IOException, DavException, HttpException {
    ArrayList<Appointment> appts = new ArrayList<Appointment>();
