...
public abstract class DavMethod {
  ...
  protected void sendResponse(DavContext ctxt) throws IOException {
    if (ctxt.isResponseSent()) return;
    HttpServletResponse resp = ctxt.getResponse();
    resp.setStatus(ctxt.getStatus());
    String compliance = ctxt.getDavCompliance();
    if (compliance != null) setResponseHeader(resp, DavProtocol.HEADER_DAV, compliance);
  ...
  public HttpRequestBase toHttpMethod(DavContext ctxt, String targetUrl)
      throws IOException, DavException {
    if (ctxt.getUpload() != null && ctxt.getUpload().getSize() > 0) {
      HttpPost method =
          new HttpPost(targetUrl) {
            @Override
      ...
      if (ctxt.hasRequestMessage()) {
        ...
        reqEntry =
            new InputStreamEntity(ctxt.getUpload().getInputStream(), ctxt.getUpload().getSize());
      }
