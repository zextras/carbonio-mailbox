...
public abstract class Range {
  ...
  public Range(String name) {
    ...
    if (mStart == 0) mStart = Long.MIN_VALUE;
    else mStart = now - mStart;
    if (mEnd == 0) mEnd = Long.MAX_VALUE;
    else mEnd = now + mEnd;
  }
  ...
  public Range(Element elem) {
    ...
    if (s != null) mStart = parseDateWithUTCTime(s);

    s = elem.attributeValue(DavElements.P_END);
    if (s != null) mEnd = parseDateWithUTCTime(s);
  ...
  private static long parseDateWithUTCTime(String time) {
    if (time.length() != 8 && time.length() != 16) return 0;
    if (!time.endsWith("Z")) return 0;
    TimeZone tz = TimeZone.getTimeZone("GMT");
    int year, month, date, hour, min, sec;
    int index = 0;
    year = Integer.parseInt(time.substring(index, index + 4));
    index += 4;
    month = Integer.parseInt(time.substring(index, index + 2)) - 1;
    index += 2;
    date = Integer.parseInt(time.substring(index, index + 2));
    index += 2;
    hour = min = sec = 0;
    if (time.length() == 16) {
      ...
      index += 2;
      min = Integer.parseInt(time.substring(index, index + 2));
      index += 2;
      sec = Integer.parseInt(time.substring(index, index + 2));
      index += 2;
    }
  ...
  public void intersection(Range another) {
    if (mStart < another.mStart) mStart = another.mStart;
    if (mEnd > another.mEnd) mEnd = another.mEnd;
  }
  ...
  public static class TimeRange extends Range {
    ...
    public boolean matches(int mboxId, int itemId, long apptRangeStart, long apptRangeEnd) {
      ...
      if (apptRangeStart >= mStart && apptRangeEnd <= mEnd) return true;
      if (mboxId == 0 || itemId == 0) return true;
      try {
