...
public class DavRequest {
  ...
  public static class DocumentRequestEntity implements HttpEntity {
    ...
    public Header getContentType() {
      return new BasicHeader(HttpHeaders.CONTENT_TYPE, "text/xml");
    }
     ...
     *
     * @deprecated (4.1) Either use {@link #getContent()} and call {@link
     *     java.io.InputStream#close()} on that; otherwise call {@link #writeTo(OutputStream)} which
     *     is required to free the resources.
     */
    @Override
    public void consumeContent() throws IOException {}

    /**
     * Specifies the Content-Encoding header. The default implementation sets the value of the
     ...
     *
     * <p>Note that the chunked setting is a hint only. If using HTTP/1.0, chunking is never
     * performed. Otherwise, even if chunked is false, HttpClient must use chunk coding if the
     * entity content length is unknown (-1).
     *
     * <p>The default implementation sets the value of the {@link #chunked chunked} attribute.
     *
  ...
  public HttpRequestBase getHttpMethod(String baseUrl) {
    ...
    if (url == null) {
      ...
      // For instance iCloud currently specifies a full URL instead of a relative one for
      // calendar-home-set
  ...
  public static DavRequest CALENDARQUERY(String uri) {
    ...
    root.addElement(DavElements.E_PROP);
    root.addElement(DavElements.E_FILTER)
        .addElement(DavElements.E_COMP_FILTER)
        .addAttribute(DavElements.P_NAME, "VCALENDAR");
    req.setRequestMessage(root);
    return req;
  }
