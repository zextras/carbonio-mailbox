...
public abstract class AutoScheduler {
  ...
  private static void addSchedulingMsg(List<AutoScheduleMsg> msgs, AutoScheduleMsg msg) {
    ...
    if (ZimbraLog.dav.isDebugEnabled()) {
      ...
      if (inv != null) {
        ...
        ZimbraLog.dav.debug(
            "Will schedule msg with method %s for UID %s Recurrence ID %s\n%s",
  ...
  public static AutoScheduler getAutoScheduler(
    ...
    try {
      ...
      if (calendarMailbox.getAccount().sameAccount(organizerAcct)) {
        return new OrganizerAutoScheduler(
            userMailbox,
            calendarMailbox,
            origInvites,
            calendarMailItemId,
        ...
        return new AttendeeAutoScheduler(
            userMailbox,
            calendarMailbox,
            origInvites,
            calendarMailItemId,
  ...
  protected CalendarItem persistToCalendar() throws ServiceException {
    ...
    return userMailbox.setCalendarItem(
  ...
  protected CalendarItem processSchedulingMessages(List<AutoScheduleMsg> msgs)
      throws ServiceException {
  ...
  protected class AutoScheduleMsg {
    ...
    protected AutoScheduleMsg(
        Invite calendarInvite, ICalTok msgMethod, List<Address> recipients, String subject) {
    ...
    public void updateCalendarInfo() {
      ...
      if (ICalTok.REQUEST.equals(msgMethod)) {
        ...
        calendarInvite.setMethod(
            msgMethod.toString()); // Also needed to force it to be treated as invite?
        for (ZAttendee attendee : calendarInvite.getAttendees()) {
          if (!attendee.hasRsvp() || !attendee.getRsvp()) {
            for (Address recip : recipients) {
              try {
                ...
                ZimbraLog.dav.debug(
                    "Problem getting friendly address for attendee %s", attendee, e);
              }
          ...
          if (Strings.isNullOrEmpty(attendee.getPartStat())) {
            attendee.setPartStat(
                IcalXmlStrMap.PARTSTAT_NEEDS_ACTION); // ios8 doesn't set a partstat
          }
        }
      }
    }
    ...
    public void sendSchedulingMsg() {
      if (recipients == null || recipients.isEmpty()) {
        ZimbraLog.dav.trace(
            "Not generating a scheduling message for UID=%s start=%s - no recipients",
      ...
      try {
        ...
        FriendlyCalendaringDescription friendlyDesc =
            new FriendlyCalendaringDescription(msgInvite, ctxt.getAuthAccount());
        ...
        MimeMessage mm =
            CalendarMailSender.createCalendarMessage(
                ...
                uid,
                cal,
                msgInvite.getIcalendarAttaches(),
                true);
        mbox.getMailSender()
            .setSendPartial(true)
            .sendMimeMessage(
  ...
  public static class OrganizerAutoScheduler extends AutoScheduler {
      ...
      for (Invite inv : newInvites) {
        if (inv != seriesOrSingletonInvite) {
          ...
          addSchedulingMsg(
              msgs,
              new AutoScheduleMsg(
                  inv, ICalTok.REQUEST, exceptAttendees, inv.getName() /*subject*/));
    ...
    private CalendarItem doSchedulingActionsForUpdate() throws ServiceException {
      ...
      Invite newInvite =
          Invite.matchingInvite(
              newInvites,
              (oldSeriesOrSingletonInvite == null)
                  ? null
                  : oldSeriesOrSingletonInvite.getRecurId());
      OrganizerInviteChanges changeInfo =
          new OrganizerInviteChanges(oldSeriesOrSingletonInvite, newInvite);
      if (changeInfo.inviteCanceled()) {
        addSchedulingMsg(
            msgs,
            new AutoScheduleMsg(
                oldSeriesOrSingletonInvite,
                ICalTok.CANCEL,
        ...
        if (changeInfo.isChanged()) {
          List<Address> recips =
              changeInfo.isReplyInvalidatingChange()
        ...
        // handle EXDATEs.  Note:Mac OSX Mavericks Calendar single instance delete doesn't increment
        // sequence
      ...
      for (Invite inv : newInvites) {
        ...
        addSchedulingMsg(
            msgs, new AutoScheduleMsg(inv, ICalTok.REQUEST, recips, inv.getName() /*subject*/));
        List<Address> cancelRecips = getRecipientsForAttendees(newSeriesOrSingletonInvite);
        // Send a CANCEL to any ATTENDEES for the series that have been removed for this NEW
        // instance
        ...
        if (!cancelRecips.isEmpty()) {
          addSchedulingMsg(
              msgs,
              new AutoScheduleMsg(
                  inv, ICalTok.CANCEL, cancelRecips, CANCEL_PREFIX + inv.getName()));
        }
      }
      return processSchedulingMessages(msgs);
    }
    ...
    /**
     * Apple Mac OS X Yosemite Calendar client doesn't seem to like incrementing the sequence
     * number, even for changes like moving the start date. This can cause an update not too look
     * like it needs a response and for the calendar of an attendee to be left unchanged. This is a
     * horrible hack and should be removed if possible in the future.
     *
     * <p>Only call this if we are aware of a major change done by or on the Organizer's behalf
     *
     * @param oldInvite
     * @param newInvite
     */
    private void bumpSequenceNumberIfNecessary(Invite oldInvite, Invite newInvite) {
      ...
      if (newSeqNum == oldInvite.getSeqNo()) {
        ZimbraLog.dav.debug(
            "Update has same SEQUENCE %d as previous rev. Incrementing on behalf of client",
            newSeqNum);
        newInvite.setSeqNo(newSeqNum + 1);
      }
    }

    private void addCancelsForNewExdates(
        List<AutoScheduleMsg> msgs, OrganizerInviteChanges changeInfo) {
      for (Instance exdateOnlyInNew : changeInfo.getExdatesOnlyInNew()) {
        ...
        addSchedulingMsg(
            ...
            new AutoScheduleMsg(
                cancel,
                ICalTok.CANCEL,
                getRecipientsForAttendees(cancel),
    ...
    public List<Address> getRecipientsForAttendees(List<ZAttendee> attendees) {
      ...
      for (ZAttendee attendee : attendees) {
        if (Objects.equals(attendee.getScheduleAgent(), ScheduleAgent.CLIENT)
            || Objects.equals(attendee.getScheduleAgent(), ScheduleAgent.NONE)) {
          continue;
        }
        try {
        ...
        try {
          ...
          ZimbraLog.dav.info(
              "Problem adding attendee %s to recipient list - ignoring", attendee, mse);
        }
      }
      return recipients;
    }
  ...
  public static class AttendeeAutoScheduler extends AutoScheduler {
    ...
    public CalendarItem doSchedulingActions() throws ServiceException {
      ...
      if (!scheduleReplyWanted) {
        ZimbraLog.dav.debug(
            "Skipping scheduling actions because HTTP header '%s=F'",
      ...
      for (Invite inv : newInvites) {
        ...
        addSchedulingMsg(
            msgs,
            new AutoScheduleMsg(
                inv, ICalTok.REPLY, Lists.newArrayList(organizerAddress), subject.toString()));
      }
      // Process deletions
      for (Invite inv : origInvites) {
        ...
        if (newInvite == null) {
          ...
          String subject =
              new StringBuilder(REPLY_PARTSTAT_SUBJECT_MAP.get(IcalXmlStrMap.PARTSTAT_DECLINED))
                  .append(decline.getName())
                  .toString();
          decline.setName(subject);
          addSchedulingMsg(
              msgs,
              new AutoScheduleMsg(
                  decline, ICalTok.REPLY, Lists.newArrayList(organizerAddress), subject));
        }
      }
      return processSchedulingMessages(msgs);
    }
    ...
    private ZAttendee getMatchingAttendee(List<ZAttendee> attendees) {
      ...
      for (ZAttendee attendee : attendees) {
        try {
          ...
          ZimbraLog.dav.debug(
              "problem matching attendee '%s' to account '%s'",
    ...
    static final Map<String, String> REPLY_PARTSTAT_SUBJECT_MAP =
        ImmutableMap.of(
