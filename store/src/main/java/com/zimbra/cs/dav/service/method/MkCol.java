...
/**
 * https://tools.ietf.org/html/rfc5689 Extended MKCOL for Web Distributed Authoring and Versioning
 * (WebDAV)
 */
  ...
  public void handle(DavContext ctxt) throws DavException, IOException {
    ...
    if (ctxt.hasRequestMessage()) {
      ...
      if (!top.getName().equals(DavElements.P_MKCOL)) {
        throw new DavException(
            "body " + top.getName() + " not allowed in MKCOL",
    ...
    if (elemPair == null) {
      ...
      while (setIter.hasNext()) {
        ...
        if (propName.equals(DavElements.P_RESOURCETYPE)) {
          ...
          if (prop.isAddressBook()) {
            ...
            throw new DavException(
                "resourcetype must include collection for MKCOL",
                HttpServletResponse.SC_BAD_REQUEST,
                null);
          }
      ...
      Pair<List<Element>, List<QName>> pair =
          PropPatch.processSetsAndRemoves(
              ctxt, col, elemPair.getFirst(), elemPair.getSecond(), true);
