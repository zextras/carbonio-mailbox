...
public abstract class AbstractCalendarProxy extends Principal {
   ...
   * have the appropriate perms on the default Calendar folder Need to grant access to new members
   * of the set (with sending of invites) and revoke access to deleted members as appropriate.
   *
   * <p>Note that un-checking a box next to a name in the Yosemite Calendar UI would result in 2
   * PROPPATCHes, one to add an entry to the read group and one to remove that entry from the write
   * group (or vice versa)
   *
   * <p>Note that there is a mismatch between the Apple model and ours in that in the Zimbra model
   * we send out a share notification email, and the recipient actively accepts the share -
   * something the Apple model doesn't require.
   */
  @Override
  ...
  /**
   * Reflect changes in membership, i.e. invite new members and revoke access to people no longer in
   * the group. Note that we are only dealing with the default Calendar - the Apple model doesn't
   * talk about things at the calendar level - so for our purposes we shall assume it only applies
   * to that calendar.
   */
  private void changeGroupMembership(
      DavContext ctxt, Set<Account> invitees, Set<Account> revokees, short role) {
    try {
      ...
      if (!emailAddresses.isEmpty()) {
        ssnreq =
            SendShareNotificationRequest.create(
                Mailbox.ID_FOLDER_CALENDAR,
                null /* SendShareNotificationRequest.Action */,
                null /* notes */,
                emailAddresses);
      ...
      if (!emailAddresses.isEmpty()) {
        ssnreq =
            SendShareNotificationRequest.create(
                Mailbox.ID_FOLDER_CALENDAR,
                SendShareNotificationRequest.Action.revoke,
                null /* notes */,
                emailAddresses);
        zmbox.invokeJaxb(ssnreq);
      }
