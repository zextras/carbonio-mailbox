...
public class DavContext {
  ...
  static {
    synchronized (DavContext.class) {
      if (sExtensions == null) sExtensions = new HashSet<>();
    }
  }
  ...
  /**
   * "actingAsDelegateFor" is used to form part of the scheduling inbox or scheduling outbox URL
   * used for scheduling on behalf of another user (the principal we are acting as a delegate for).
   * e.g. when actingAsDelegateFor = "user3@example.com", the associated inbox would be at path
   * /Inbox/user3@example.com/
   */
  ...
  public static class RequestProp {
    ...
    public RequestProp(Element top) {
      ...
      for (Object obj : top.elements()) {
        ...
        if (name.equals(DavElements.P_ALLPROP)) allProp = true;
        else if (name.equals(DavElements.P_PROPNAME)) nameOnly = true;
        else if (name.equals(DavElements.P_PROP)) {
          @SuppressWarnings("unchecked")
          List<Element> propElems = e.elements();
          for (Element prop : propElems) props.put(prop.getQName(), prop);
        }
      }
    }
    ...
    public RequestProp(java.util.Collection<Element> set, java.util.Collection<QName> remove) {
      this(false);
      allProp = false;
      for (Element e : set) props.put(e.getQName(), e);
      for (QName q : remove) props.put(q, DocumentHelper.createElement(q));
    }
    ...
    public void addPropError(QName prop, DavException ex) {
      allProp = false;
      if (!props.containsKey(prop)) props.put(prop, DocumentHelper.createElement(prop));
  ...
  public DavContext(HttpServletRequest req, HttpServletResponse resp, Account authUser) {
    ...
    if (mUri != null && mUri.length() > 1) {
      ...
      if (mUri.toLowerCase().endsWith(CalendarObject.CAL_EXTENSION)
          || mUri.toLowerCase().endsWith(AddressObject.VCARD_EXTENSION)) {
        String rawUri = req.getRequestURI();
        String[] fragments = HttpUtil.getPathFragments(URI.create(rawUri));

        if (!req.getContextPath().isEmpty()
            && fragments.length > 0
            && HttpUtil.urlUnescape(req.getContextPath()).equals("/" + fragments[0])) {
      ...
      if (index > 0) {
        ...
        if (index < mPath.length() - 1 && index > 0) mCollectionPath = mPath.substring(0, index);
      }
    }
  ...
  public String getFullUrlBeforePath() {
    return new StringBuilder(mReq.getScheme())
        .append("://")
        .append(mReq.getServerName())
        .append(':')
        .append(mReq.getServerPort())
        .toString();
  }
  ...
  public Depth getDepth() {
    ...
    if (hdr == null) return Depth.zero;
    if (hdr.equals("0")) return Depth.zero;
    if (hdr.equals("1")) return Depth.one;
    if (hdr.equalsIgnoreCase("infinity")) return Depth.infinity;
  ...
  public boolean hasRequestMessage() {
    try {
      ...
      return getUpload().getSize() > 0
          && ct != null
          && (ct.startsWith(DavProtocol.XML_CONTENT_TYPE)
              || ct.startsWith(DavProtocol.XML_CONTENT_TYPE2));
    } catch (Exception e) {
    }
    return false;
  }

  public Upload getUpload() throws DavException, IOException {
    if (mUpload == null) {
      String name = null;
      String ctype = getRequest().getContentType();
      if (ctype == null) name = getItem();
      try {
        mUpload =
            FileUploadServlet.saveUpload(
                mReq.getInputStream(), name, ctype, mAuthAccount.getId(), true);
        ZimbraLog.dav.debug(
            "Request: requested content-type: %s, actual content-type: %s",
            ctype, mUpload.getContentType());
      } catch (ServiceException se) {
        throw new DavException(
            "can't save upload", HttpServletResponse.SC_REQUEST_ENTITY_TOO_LARGE, se);
      }
    }
    return mUpload;
  }
  ...
  public void cleanup() {
    if (!sExtensions.isEmpty()) for (Extension ex : sExtensions) ex.callback(this);
    if (mUpload != null) FileUploadServlet.deleteUpload(mUpload);
    mUpload = null;
  }

  /* Returns XML Document containing the request. */
  public Document getRequestMessage() throws DavException {
    if (mRequestMsg != null) return mRequestMsg;
    try {
      if (hasRequestMessage()) {
        mRequestMsg =
            W3cDomUtil.parseXMLToDom4jDocUsingSecureProcessing(getUpload().getInputStream());
        return mRequestMsg;
      }
    } catch (XmlParseException e) {
      throw new DavException(
          "unable to parse request message", HttpServletResponse.SC_BAD_REQUEST, e);
    } catch (IOException e) {
      throw new DavException(
          "can't read uploaded file", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
    }
  ...
  public DavResponse getDavResponse() {
    if (mResponse == null) mResponse = new DavResponse();
    return mResponse;
  }
  ...
  public DavResource getRequestedResource() throws DavException, ServiceException {
    if (mRequestedResource == null) {
      ...
      else mRequestedResource = UrlNamespace.getPrincipalAtUrl(this, mUri);
      if (mRequestedResource != null)
  ...
  public java.util.Collection<DavResource> getAllRequestedResources()
      throws DavException, ServiceException {
    ArrayList<DavResource> rss = new ArrayList<DavResource>();
    if (mRequestType == RequestType.RESOURCE)
      rss =
          (ArrayList<DavResource>)
              UrlNamespace.getResources(this, mUser, mPath, getDepth() == Depth.one);
    else {
      DavResource rs = UrlNamespace.getPrincipalAtUrl(this, mUri);
      if (rs != null) {
        ZimbraLog.addToContext(ZimbraLog.C_NAME, rs.getOwner());
        rss.add(rs);
        if (getDepth() == Depth.one) rss.addAll(rs.getChildren(this));
      }
    }
    if (rss.isEmpty())
      throw new DavException("no DAV resource at " + mUri, HttpServletResponse.SC_NOT_FOUND, null);
  ...
  public static enum KnownUserAgent {
    ...
    static KnownUserAgent lookup(String userAgent) {
      if (userAgent != null) {
        ...
        else if (userAgent.indexOf(ICAL) >= 0) return iCal;
        else if (userAgent.indexOf(EVOLUTION) >= 0) return Evolution;
      }
      return null;
    }
  }
  ...
  public boolean isFreebusyEnabled() {
    try {
      return !Provisioning.getInstance()
          .getConfig()
          .getBooleanAttr(Provisioning.A_zimbraCalendarCalDavDisableFreebusy, false);
    } catch (ServiceException se) {
      return false;
    }
  }
  ...
  public boolean useIcalDelegation() {
    if (mAuthAccount != null) return mAuthAccount.isPrefAppleIcalDelegationEnabled();
    return false;
  }
  ...
  public Collection getDestinationCollection() throws DavException {
    ...
    try {
      ...
      if (r instanceof Collection) return ((Collection) r);
      return UrlNamespace.getCollectionAtUrl(this, destinationUrl);
    } catch (Exception e) {
      throw new DavException(
          "can't get destination collection", DavProtocol.STATUS_FAILED_DEPENDENCY);
    }
  }
  ...
  private String getInternalDestinationUrl(String destinationUrl)
    ...
    if (targetAcct == null) return destinationUrl;
  ...
  public ZMailbox getZMailbox(Account acct) throws ServiceException {
    ...
    ZMailbox.Options zoptions =
        new ZMailbox.Options(authToken.toZAuthToken(), AccountUtil.getSoapUri(acct));
  ...
  public String getNewName() throws DavException {
    ...
    if (oldName.equals(newName) == false) return newName;
    else return null;
  }
}
