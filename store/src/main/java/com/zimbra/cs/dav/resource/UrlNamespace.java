...
public class UrlNamespace {
  ...
  public static Collection getCollectionAtUrl(DavContext ctxt, String url) throws DavException {
    ...
    if (uc.path.endsWith("/")) lastPos--;
    int index = uc.path.lastIndexOf('/', lastPos);
    String path;
    if (index == -1) path = "/";
    else path = uc.path.substring(0, index);
    String user = uc.user;
    if (user == null) user = ctxt.getUser();
    DavResource rsc = getResourceAt(new DavContext(ctxt, path), user, path);
    if (rsc instanceof Collection) return (Collection) rsc;
  ...
  public static DavResource getPrincipalAtUrl(DavContext ctxt, String url) throws DavException {
    ...
    ZimbraLog.dav.debug(
        "getPrincipalAtUrl name='%s' url='%s' proxyPrincipal='%s'", name, url, proxyPrincipal);
      ...
      if (CalendarProxyRead.CALENDAR_PROXY_READ.equals(proxyPrincipal)) {
        if (!ctxt.useIcalDelegation()) {
          throw new DavException(
              "Not available because zimbraPrefAppleIcalDelegationEnabled=FALSE",
  ...
  public static DavResource getResourceAt(DavContext ctxt, String user, String path)
      throws DavException {
  ...
  public static java.util.Collection<DavResource> getResources(
      DavContext ctxt, String user, String path, boolean includeChildren) throws DavException {
    ...
    if (target.startsWith(ATTACHMENTS_PREFIX)) {
      ...
      try {
        ...
        if (path.length() == 1
            && path.charAt(0) == '/'
            && se.getCode().equals(ServiceException.PERM_DENIED)) {
          // iCal makes this request for delegated calendars. iCal5 doesn't display the contents of
          // delegated calendars if the delegator's home collection is not present in the PROPFIND
          // response.
          // Since the user does not have permissions on delegator's home collection, return an
          // empty
          ...
          ZimbraLog.dav.warn(
              "can't get mail item resource for user='%s' path='%s'", user, path, se);
        }
      }
    }
  ...
  public static DavResource getResourceByItemId(DavContext ctxt, String user, int id)
  ...
  public static String getAclUrl(String principal, String type) throws DavException {
    ...
    try {
      ...
      buf.append(type);
      if (account != null) buf.append(account.getName());
      else buf.append(principal);
      return getAbsoluteUrl(null, buf.toString().replaceAll("@", "%40"));
    } catch (ServiceException e) {
      throw new DavException(
          "cannot create ACL URL for principal " + principal,
  ...
  private static boolean onSameServer(Account thisOne, Account thatOne) {
    if (thisOne.getId().equals(thatOne.getId())) return true;
    try {
      Provisioning prov = Provisioning.getInstance();
      Server mine = prov.getServer(thisOne);
      Server theirs = prov.getServer(thatOne);
      if (mine != null && theirs != null) return mine.getId().equals(theirs.getId());
    } catch (Exception e) {
      ZimbraLog.dav.warn(
          "can't get domain or server for %s %s", thisOne.getId(), thatOne.getId(), e);
    }
    return true;
  }
  ...
  public static String getFolderUrl(String authUser, String folder) {
    StringBuilder url = new StringBuilder();
    url.append(DavServlet.DAV_PATH)
        .append("/")
        .append(authUser)
        .append("/")
        .append(folder)
  ...
  public static void addToRenamedResource(String user, String path, DavResource rsc) {
    synchronized (sRenamedResourceMap) {
      sRenamedResourceMap.put(
          new Pair<String, String>(user, path.toLowerCase()),
  ...
  public static DavResource checkRenamedResource(String user, String path) {
    ...
    synchronized (sRenamedResourceMap) {
      ...
      if (item != null) {
        ...
        if (age > 15 * Constants.MILLIS_PER_MINUTE) sRenamedResourceMap.remove(key);
        else rsc = item.getFirst();
      }
    }
    return rsc;
  }
  ...
  private static DavResource getMailItemResource(DavContext ctxt, String user, String path)
    ...
    if (account == null) {
      ...
      ZimbraLog.dav.info(
          "Failing GET of mail item resource - no such account '%s' path '%s'", user, path);
    ...
    if (f != null) {
      ...
      if (baseName.toLowerCase().endsWith(CalendarObject.CAL_EXTENSION)) {
        String uid =
            baseName.substring(0, baseName.length() - CalendarObject.CAL_EXTENSION.length());
  ...
  private static java.util.Collection<DavResource> getFolders(DavContext ctxt, String user)
      throws ServiceException, DavException {
    Provisioning prov = Provisioning.getInstance();
    Account account = prov.get(AccountBy.name, user);
    if (account == null) {
  ...
  private static DavResource getCalendarItemForMessage(DavContext ctxt, Message msg)
    ...
    if (msg.isInvite() && msg.hasCalendarItemInfos()) {
      ...
      try {
        ...
        if (invite == null && calItemInfo.calItemCreated()) {
          // Pre-6.0 data
          CalendarItem item =
              mbox.getCalendarItemById(ctxt.getOperationContext(), calItemInfo.getCalendarItemId());
        ...
        if (invite != null) {
          String path =
              CalendarObject.CalendarPath.generate(
                  ctxt, msg.getPath(), invite.getUid(), mbox.getId(), msg.getId(), msg.getId());
  ...
  public static RemoteCalendarCollection getRemoteCalendarCollection(
      DavContext ctx, Mountpoint mountpoint) throws ServiceException, DavException {
    return new RemoteCalendarCollection(ctx, mountpoint);
  }

  /* Returns DavResource for the MailItem. */
  public static DavResource getResourceFromMailItem(DavContext ctxt, MailItem item)
      throws DavException {
    DavResource resource = null;
    if (item == null) {
      return resource;
  ...
  private static MailItemResource getCalendarCollection(DavContext ctxt, Folder f)
      throws ServiceException, DavException {
    String[] homeSets =
        Provisioning.getInstance()
            .getConfig()
            .getMultiAttr(Provisioning.A_zimbraCalendarCalDavAlternateCalendarHomeSet);
    ...
    if (homeSets.length > 0
        && (f.getId() == Mailbox.ID_FOLDER_CALENDAR || f.getId() == Mailbox.ID_FOLDER_TASKS)) {
      return new Collection(ctxt, f);
    }
    if (f.getDefaultView() == MailItem.Type.APPOINTMENT
        && !ctxt.getAuthAccount().isFeatureCalendarEnabled()) {
      return new Collection(ctxt, f);
    }
    if (f.getDefaultView() == MailItem.Type.TASK
        && !ctxt.getAuthAccount().isFeatureTasksEnabled()) {
      return new Collection(ctxt, f);
    }
  ...
  private static DavResource getPhantomResource(DavContext ctxt, String user) throws DavException {
    ...
    switch (numTokens) {
      ...
      case 3:
        ...
        else resource = new BrowseWrapper(target, user, tokens);
        break;
      case 4:
        if (tokens.get(1).equals(PhantomResource.BY_DATE))
          resource = new Attachment(target, user, tokens, ctxt);
        else resource = new SearchWrapper(target, user, tokens);
        break;
      case 5:
        resource = new Attachment(target, user, tokens, ctxt);
        break;
  ...
  private static MailItem getMailItemById(DavContext ctxt, String user, int id)
