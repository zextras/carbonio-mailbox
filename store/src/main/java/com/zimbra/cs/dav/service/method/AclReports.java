...
public class AclReports extends Report {
  ...
  public void handle(DavContext ctxt) throws DavException, ServiceException {
    ...
    if (query.getQName().equals(DavElements.E_PRINCIPAL_PROPERTY_SEARCH))
      ...
      throw new DavException(
          "msg " + query.getName() + " is not an ACL report", HttpServletResponse.SC_BAD_REQUEST);
  }

  /**
   * http://tools.ietf.org/html/rfc3744#section-9.4. DAV:principal-property-search REPORT
   * Preconditions: None Postconditions: DAV:number-of-matches-within-limits
   */
  private void handlePrincipalPropertySearch(DavContext ctxt, Element query)
    ...
    // The response body for a successful request MUST be a DAV:multistatus XML element.  In the
    // case where there
  ...
  private ArrayList<DavResource> getMatchingResources(DavContext ctxt, Element query)
      throws DavException, ServiceException {
    ...
    boolean applyToPrincipalCollection =
        query.element(DavElements.E_APPLY_TO_PRINCIPAL_COLLECTION_SET) != null;
    String path = ctxt.getUri();
    if (!applyToPrincipalCollection && !path.startsWith(UrlNamespace.PRINCIPALS_PATH)) return ret;

    // apple hack to do user / resource search
    GalSearchType type = GalSearchType.all;
    String queryType = query.attributeValue("type");
    if (queryType != null) {
      if (queryType.compareToIgnoreCase("INDIVIDUAL") == 0) type = GalSearchType.account;
      else if (queryType.compareToIgnoreCase("RESOURCE") == 0) type = GalSearchType.resource;
    }
  ...
  private ArrayList<DavResource> getMatchingPrincipals(
      DavContext ctxt, QName prop, String match, GalSearchType type)
      throws DavException, ServiceException {
    ...
    if (prop.equals(DavElements.E_DISPLAYNAME)) {
      ...
      SearchGalResult result =
          prov.searchGal(
              prov.getDomain(authAccount), match, type, Provisioning.GalMode.zimbra, null);
      for (GalContact ct : result.getMatches()) {
        ...
        if (email != null) {
          ...
          if (acct != null) ret.add(UrlNamespace.getPrincipal(ctxt, acct));
        }
      }
    } else if (prop.equals(DavElements.E_CALENDAR_HOME_SET)) {
      int index = match.lastIndexOf('/');
      if (index > 0) match = match.substring(index + 1);
   ...
   * Postconditions: (DAV:number-of-matches-within-limits): The number of matching principals must
   * fall within server-specific, predefined limits. For example, this condition might be triggered
   * if a search specification would cause the return of an extremely large number of responses.
   */
  private void handleAclPrincipalPropSet(DavContext ctxt, Element query)
    ...
    if (ctxt.getDepth() != Depth.zero) {
      throw new DavException.REPORTwithDisallowedDepthException(
          query.getQName().getName(), ctxt.getDepth());
    }
    RequestProp reqProp = ctxt.getRequestProp();
    DavResponse resp = ctxt.getDavResponse();
    // The response body for a successful request MUST be a DAV:multistatus XML element.  In the
  ...
  private ArrayList<DavResource> getAclPrincipals(DavContext ctxt)
      throws DavException, ServiceException {
    ArrayList<DavResource> ret = new ArrayList<DavResource>();
    DavResource res = ctxt.getRequestedResource();
    if (!(res instanceof MailItemResource)) return ret;
  ...
  /**
   * http://tools.ietf.org/html/rfc3744#section-9.3. DAV:principal-match REPORT This report is only
   * defined when the Depth header has value "0"; other values result in a 400 (Bad Request) error
   * response.
   */
  private void handlePrincipalMatch(DavContext ctxt, Element query)
    ...
    if (principalProp == null) {
      ...
      if (owner != null) {
        ...
        if (rs.isCollection()) ret.addAll(rs.getChildren(ctxt));
      }
    }
   ...
   * report is only defined when the Depth header has value "0"; other values result in a 400 (Bad
