...
public abstract class ZAttributes extends ZLdapElement implements IAttributes {
   ...
   *
   * <p>- If a method does not have a CheckBinary parameter, it will *not* check for binary data and
   * binary transfer based on AttributeManager. It will assume all attributes are *not* binary.
   *
   * <p>- If a method has a CheckBinary parameter, it will check for binary data and binary transfer
   * based on AttributeManager if CheckBinary is CHECK. It will assume all attributes are *not*
   * binary if CheckBinary is NOCHECK.
   */
  @Override
  ...
  @Override
  public String[] getMultiAttrString(
      String attrName, boolean containsBinaryData, boolean isBinaryTransfer) throws LdapException {
  ...
  /**
   * Enumerates over the specified attributes and populates the specified map. The key in the map is
   * the attribute ID. For attrs with a single value, the value is a String, and for attrs with
   * multiple values the value is an array of Strings.
   *
   * <p>Note: this method always *check* binary.
   */
  ...
  /**
   * Retrieves the value for this attribute as a string. If this attribute has multiple values, then
  ...
  protected abstract String[] getMultiAttrString(
      String transferAttrName, boolean containsBinaryData) throws LdapException;
}
