...
public class InMemoryLdapServer {
  ...
  private static final String UNITTEST_DATA_PATH =
      LC.zimbra_home.value() + "/unittest/ldap/InMemoryLdapServer";
  ...
  private static class Server {
    ...
    private LDAPConnectionPool getConnectionPool(LdapServerConfig config) throws LdapException {
      try {
        /*
        ...
        return server.getConnectionPool(null, null, initPoolSize, config.getConnPoolMaxSize());
      } catch (LDAPException e) {
        throw UBIDLdapException.mapToLdapException(e);
      }
    }
  ...
  private static class Exporter {
    ...
    /**
     * Generate LDIF files for Zimbra schema and the entire DIT from the current image of LDAP
     * database.
     */
