...
public class UBIDLogger {
  ...
  static void beforeOp(LdapOp ldapOp, LDAPConnection conn) {
    ...
    UBIDLogger.debugLogger.debug("%s - conn=[%d]", ldapOp, conn.getConnectionID());
  }

  static void afterTimedOp(
      LdapOp ldapOp, Timer timer, LdapUsage usage, LDAPConnection conn, String dn) {
    if (!debugLogger.isDebugEnabled()) {
      return;
    }

    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d] dn=[%s]",
        ldapOp, timer.elapsedMillis(), usage.name(), conn.getConnectionID(), dn);
  }

  static void afterTimedOp(
      LdapOp ldapOp, Timer timer, LdapUsage usage, LDAPConnection conn, ZMutableEntry entry) {
    if (!debugLogger.isDebugEnabled()) {
      return;
    }
    ...
    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d] entry=[%s]",
        ldapOp,
        timer.elapsedMillis(),
        usage.name(),
  ...
  static void afterTimedOp(
    ...
    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d] dn=[%s], mod=[%s]",
        ldapOp,
        timer.elapsedMillis(),
        usage.name(),
        conn.getConnectionID(),
  ...
  static void afterTimedOp(LdapOp ldapOp, Timer timer, LdapUsage usage, LDAPConnection conn) {
    ...
    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d]",
        ldapOp, timer.elapsedMillis(), usage.name(), conn.getConnectionID());
  }
  ...
  static void afterTimedOp(
    ...
    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d], base=[%s], filter=[%s]",
        ldapOp, timer.elapsedMillis(), usage.name(), conn.getConnectionID(), baseDN, filter);
  }

  static void afterTimedOp(
      LdapOp ldapOp, Timer timer, LdapUsage usage, LDAPConnection conn, DN oldDN, DN newDN) {
    if (!debugLogger.isDebugEnabled()) {
      return;
    }

    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d], oldDN=[%s], newDN=[%s]",
        ldapOp,
        timer.elapsedMillis(),
        usage.name(),
        conn.getConnectionID(),
  ...
  static void afterTimedOp(
    ...
    UBIDLogger.debugLogger.debug(
        "%s - millis=[%d], usage=[%s], conn=[%d], connPool=[%s(%d)]",
        ldapOp,
        timer.elapsedMillis(),
        usage.name(),
        conn.getConnectionID(),
