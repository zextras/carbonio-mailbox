...
/**
 * Represent a list of LDAP servers. ZCS will attempt to establish connections in the order they are
 * provided for failover. If the first server is unavailable, then it will attempt to connect to the
...
public class LdapServerPool {
  ...
  private static final int DUMMY_LDAPI_PORT =
      1; // SingleServerSet rquires port to be (port > 0) && (port < 65536)
    ...
    if (urls.size() == 1) {
      ...
      if (socketFactory == null) {
        ...
        FailoverServerSet serverSet =
            new FailoverServerSet(hostsStrs, portsStrs, socketFactory, connOpts);
        serverSet.setReOrderOnFailover(true);
        return serverSet;
      }
    }
