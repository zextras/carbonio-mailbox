...
public final class NoopSearchControl extends Control implements DecodeableControl {
  ...
  /** The OID (2.16.840.1.113730.3.4.4) for the OpenLDAP No-op Search Control. */
  ...
  enum ControlMessages {
    ERR_NOOP_SEARCH_NO_VALUE(
        "No control value was provided, so it could not be decoded as a noop search response"
            + " control."),
    ERR_NOOP_SEARCH_NOT_SEQUENCE(
        "Unable to decode the value of the provided control as an noop search control sequence: "
            + " %s"),
    ERR_NOOP_SEARCH_INVALID_ELEMENT_COUNT(
        "Unexpected number of elements in the noop search value sequence (expected 3, got %d)."),
    ERR_NOOP_SEARCH_ELEMENT_NOT_INTEGER(
        "Unable to decode the first element of the simple paged results value sequence as an"
            + " integer: %s"),
  ...
  public NoopSearchControl(final String oid, final boolean isCritical, final ASN1OctetString value)
      throws LDAPException {
    super(oid, isCritical, value);

    if (value == null) {
      throw new LDAPException(
          ResultCode.DECODING_ERROR, ControlMessages.ERR_NOOP_SEARCH_NO_VALUE.get());
    }
    ...
    try {
      ...
      debugException(ae);
      throw new LDAPException(
          ResultCode.DECODING_ERROR, ControlMessages.ERR_NOOP_SEARCH_NOT_SEQUENCE.get(ae), ae);
    }
    ...
    if (valueElements.length != 3) {
      throw new LDAPException(
          ...
          ControlMessages.ERR_NOOP_SEARCH_INVALID_ELEMENT_COUNT.get(valueElements.length));
    }
    ...
    try {
      ...
      throw new LDAPException(
          ResultCode.DECODING_ERROR,
          ControlMessages.ERR_NOOP_SEARCH_ELEMENT_NOT_INTEGER.get(ae),
          ae);
    }
  ...
  public static NoopSearchControl get(final SearchResult result) throws LDAPException {
