...
public abstract class LdapServerConfig {
  ...
  //           next health check interval (specified by
  // LC.ldap_connect_pool_health_check_background_interval_millis).
  ...
  //
  //  This option was available before the option to automatically retry operations that fail in a
  // way that
  //  indicates that the connection is no longer valid, and in many cases it is better to use the
  // retry
  //  feature because there's no performance penalty unless there is an operation failure, and then
  // it's
  ...
  // If the provided value is less than or equal to zero, then the default value of 30000
  // milliseconds
  ...
  private LdapServerConfig() {
    ...
    //
    // System.setProperty("com.sun.jndi.ldap.connect.pool.debug",
    // LC.ldap_connect_pool_debug.value());
    this.connPoolMaxSize = LC.ldap_connect_pool_maxsize.intValue();
    // System.setProperty("com.sun.jndi.ldap.connect.pool.prefsize",
    // LC.ldap_connect_pool_prefsize.value());
    ...
    this.isAbandonOnTimeout =
        Strings.isNullOrEmpty(LC.get("ldap_abandon_on_timeout")) ? true : false;
  }
  ...
  public static class ZimbraLdapConfig extends LdapServerConfig {
    ...
    public ZimbraLdapConfig(LdapServerType serverType) {
      ...
      // Whether starttls is required for java ldap client when it establishes connections to the
      // Zimbra ldap server
      boolean ldap_starttls_required = LC.ldap_starttls_required.booleanValue();
      boolean zimbra_require_interprocess_security =
          "1".equals(LC.zimbra_require_interprocess_security.value());
  ...
  public static class GenericLdapConfig extends ExternalLdapConfig {
    public GenericLdapConfig(
        String[] urls, boolean wantStartTLS, String bindDn, String bindPassword) {
  ...
  public static class ExternalLdapConfig extends LdapServerConfig {
     ...
     * @param authMech // TODO: cleanup. For now, if null: simple bind if binDN/password is not
     *     null, anon bind otherwise
     * @param bindDn
     * @param bindPassword
     * @param binaryAttrs
     * @param note
     */
    public ExternalLdapConfig(
        String urls,
        boolean wantStartTLS,
        String authMech,
        String bindDn,
        String bindPassword,
        Set<String> binaryAttrs,
        String note) {
      super();
    ...
    public ExternalLdapConfig(
        ...
        String note) {
      this(
          LdapServerConfig.joinURLS(urls),
          wantStartTLS,
          authMech,
          bindDn,
          bindPassword,
