...
public abstract class LdapClient {
      ...
      if (className != null && !className.equals("")) {
        try {
          ...
          ZimbraLog.system.error(
              "could not instantiate LDAP client '" + className + "'; defaulting to JNDI LDAP SDK",
              e);
        }
      }
      if (ldapClient == null) {
  ...
  public static ZLdapContext toZLdapContext(
    ...
    if (!(getInstance() instanceof UBIDLdapClient)) {
      Zimbra.halt(
          "LdapClient instance is not UBIDLdapClient",
    ...
    if (ldapContext != null && !(ldapContext instanceof ZLdapContext)) {
      Zimbra.halt(
          "ILdapContext instance is not ZLdapContext",
          ServiceException.FAILURE("internal error, wrong ldap context instance", null));
    }
  ...
  public static ZLdapContext getContext(
      LdapServerType serverType, boolean useConnPool, LdapUsage usage) throws ServiceException {
   ...
   * For zmconfigd only.
   *
   * <p>zmconfigd uses ldapi connection with root LDAP bind DN/password to bind to Zimbra OpenLdap,
   * whereas LdapClient.getContext() methods use LC keys for the URL/bind DN/password.
   *
   * <p>Changing LC keys in zmconfigd is not an option, because it also uses LdapProvisioing, which
   * uses the LC settings.
   *
   * <p>We could have zmconfigd call getExternalContext with a ExternalLdapConfig, which takes
   * URL/bind DN/password from parameters. But the name "external" is misleading.
   *
   * <p>This method id just a wrapper around LdapClient.getExternalContext, the sole purpose is
   * masking out the "external" in method/parameter names.
   *
   * @param ldapConfig
   * @param usage
   * @return
   * @throws ServiceException
   */
  public static ZLdapContext getContext(GenericLdapConfig ldapConfig, LdapUsage usage)
      throws ServiceException {
    return getInstance().getExternalContextImpl(ldapConfig, usage);
  }

  public static ZLdapContext getExternalContext(ExternalLdapConfig ldapConfig, LdapUsage usage)
      throws ServiceException {
    return getInstance().getExternalContextImpl(ldapConfig, usage);
  }
  ...
  public static void externalLdapAuthenticate(
      String urls[], boolean wantStartTLS, String bindDN, String password, String note)
      throws ServiceException {
    getInstance().externalLdapAuthenticateImpl(urls, wantStartTLS, bindDN, password, note);
  }

  /**
   * LDAP authenticate to the Zimbra LDAP server. Used when stored password is not SSHA.
   *
   * @param principal
   * @param password
   * @param note
  ...
  protected abstract ZLdapContext getContextImpl(
      LdapServerType serverType, boolean useConnPool, LdapUsage usage) throws ServiceException;

  protected abstract ZLdapContext getExternalContextImpl(
      ExternalLdapConfig ldapConfig, LdapUsage usage) throws ServiceException;
  ...
  protected abstract void externalLdapAuthenticateImpl(
      String urls[], boolean wantStartTLS, String bindDN, String password, String note)
      throws ServiceException;
