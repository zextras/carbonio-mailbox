...
public class SmtpTransport extends Transport {
  ...
  protected boolean protocolConnect(String host, int port, String user, String passwd)
    ...
    if (port < 0) {
      port =
          PropUtil.getIntSessionProperty(
              session,
              "mail." + protocol + ".port",
    ...
    config.setSecurity(
        ssl
            ? SmtpConfig.Security.SSL
            : PropUtil.getBooleanSessionProperty(session, "mail.smtp.starttls.enable", false)
                ? SmtpConfig.Security.TLS_IF_AVAILABLE
                : SmtpConfig.Security.NONE);
    config.setAllowPartialSend(
        PropUtil.getBooleanSessionProperty(session, "mail." + protocol + ".sendpartial", false));
    config.setConnectTimeout(
        PropUtil.getIntSessionProperty(session, "mail." + protocol + ".connectiontimeout", 0)
            / 1000); // msec to sec
    config.setReadTimeout(
        PropUtil.getIntSessionProperty(session, "mail." + protocol + ".timeout", 0)
            / 1000); // msec to sec
  ...
  /**
   * Sends the message to the recipients. This implementation immediately closes the SMTP connection
   * after sending a message, which might be incompatible with JavaMail.
   *
    ...
    switch (notify) {
      ...
      case TransportEvent.MESSAGE_PARTIALLY_DELIVERED:
        throw new SendFailedException(
            "MESSAGE_PARTIALLY_DELIVERED", ex, validSent, validUnsent, invalid);
    }
  }
}
