...
public final class SmtpConnection extends MailConnection {
   ...
   *
   * <ul>
   *   <li>Bare CR or LF characters are converted to {@code <CRLF>}. See RFC 2821 2.3.7 Lines.
   *   <li>If the first character of the line is a period, one additional period is inserted at the
   *       beginning of the line. See RFC 2821 4.5.2 Transparency.
   * </ul>
   */
  private static final class SmtpDataOutputStream extends FilterOutputStream {
     ...
     * End of data.
     *
     * <p>RFC 2822 4.1.1.4 DATA (DATA)
     *
     * <p>The mail data is terminated by a line containing only a period, that is, the character
     * sequence {@code <CRLF>.<CRLF>}. This is the end of mail data indication. Note that the first
     * {@code <CRLF>} of this terminating sequence is also the {@code <CRLF>} that ends the final
     * line of the data (message text) or, if there was no data, ends the DATA command itself. An
     * extra {@code <CRLF>} MUST NOT be added, as that would cause an empty line to be added to the
     * message. The only exception to this rule would arise if the message body were passed to the
     * originating SMTP-sender with a final "line" that did not end in {@code <CRLF>}; in that case,
     * the originating SMTP system MUST either reject the message as invalid or add {@code <CRLF>}
     * in order to have the receiving SMTP server recognize the "end of data" condition.
     */
   ...
   *
   * <p>Returns the reply code. Handles multiple {@code 250} responses.
   *
    ...
    while (true) {
      if (reply.text == null) {
        throw new CommandFailedException(
            command, "Invalid server response at line " + line + ": " + reply);
      }
      if (reply.code != 250) {
        return reply.code;
      }
  ...
  protected void sendAuthenticate(boolean ir) throws IOException {
    ...
    while (true) {
      switch (reply.code) {
        ...
        case 334: // continue
          byte[] challenge =
              Strings.isNullOrEmpty(reply.text) ? new byte[0] : Base64.decodeBase64(reply.text);
  ...
  /**
   * Overrides the superclass implementation, in order to send {@code EHLO} and reread the server
   * extension list.
   */
  @Override
   ...
   * Sends the message.
   *
   * <p>Uses the sender and recipients from the headers in the {@link MimeMessage}.
   *
   ...
   * Sends the message.
   *
   * <p>Uses the sender from the headers in the {@link MimeMessage}.
   *
   * @see #sendMessage(String, String[], MimeMessage)
   */
   ...
   * Sends the message.
   *
   * <p>Implicitly connects to the MTA, if necessary, and disconnects.
   *
  ...
  private String getDSNNotify(SMTPMessage message) {
    if (message.getNotifyOptions() == 0) return null;
    if (message.getNotifyOptions() == SMTPMessage.NOTIFY_NEVER) return "NEVER";
    StringBuffer sb = new StringBuffer();
    if ((message.getNotifyOptions() & SMTPMessage.NOTIFY_SUCCESS) != 0) sb.append("SUCCESS");
  ...
  private void sendInternal(
      String sender, String[] recipients, MimeMessage javaMailMessage, String messageString)
      throws IOException, MessagingException {
    ...
    if (serverExtensions.contains("DSN")) {
      ...
      if (notify == null) notify = getSmtpConfig().getDsn();
    }
