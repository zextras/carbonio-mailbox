...
public final class ImapInputStream extends MailInputStream {
    ...
    while (!isEOF() && delims.indexOf(c = peekChar()) < 0) {
      if (!Chars.isTextChar(c)) {
        throw new ParseException(
            "Unexpected character '" + Ascii.pp((byte) c) + "' while reading TEXT string");
      }
      sbuf.append((char) read());
    }
    if (sbuf.length() == 0) {
  ...
  public void skipCRLF() throws IOException {
    try {
      ...
      // do nothing, just advancing to EOL. If we never find a \n then stream is prematurely closed
      // or server is noncompliant
      while (readChar() != '\n') {}
    }
  }

  /**
   * If next character in stream matches specified character then read it and return true.
