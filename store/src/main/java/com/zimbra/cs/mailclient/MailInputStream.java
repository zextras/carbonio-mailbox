...
/**
 * Input stream for reading line-oriented mail protocol data. Also supports a single character look
 * ahead.
 */
public class MailInputStream extends InputStream {
  ...
  /**
   * Reads the next line of ASCII input data. A input line can be terminated with either CRLF or LF,
   * which is excluded from the returned string.
   *
  ...
  /**
   * Returns the next byte of data from the input stream without actually reading it. This provides
   * a one byte lookahead.
   *
   * @return the next byte of input data, or <tt>-1</tt> if end of stream would be reached
   * @throws IOException if an I/O error occurs
   */
  ...
  /**
   * Reads an ASCII character from the input stream. An <tt>EOFException</tt> is thrown if the end
  ...
  /**
   * Returns the next ASCII character from the input stream without actually reading it. An
   * <tt>EOFException</tt> is thrown if the end of the stream would be reached.
   *
