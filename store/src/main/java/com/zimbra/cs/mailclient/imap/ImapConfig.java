...
public class ImapConfig extends MailConfig {
  ...
  /**
   * Returns the IMAP server port number. If not set, the default is {@link #DEFAULT_PORT} for a
  ...
  /**
   * Returns the maximum literal size to cache in memory. If a literal is received that exceeds the
   * specified number of bytes, then it will be cached to disk. The default value is {@link
   * #DEFAULT_MAX_LITERAL_MEM_SIZE}.
   *
  ...
  /**
   * Returns the maximum literal size to output to the protocol trace log. If tracing is enabled and
   * literal is received exceeding this size in bytes, then it will be abbreviated in the trace log.
   * The default value is {@link #DEFAULT_MAX_LITERAL_TRACE_SIZE}
   *
  ...
  /**
   * Returns the directory to use for temporary literal storage. If a literal is received that
   * exceeds {@link @getMaxLiteralMemSize} then it will be temporarily stored in this directory. The
   * default directory is the value of the system property <tt>java.io.tmpdir</tt>.
   *
