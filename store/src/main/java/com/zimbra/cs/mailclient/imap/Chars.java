 ...
 *
 * <p>ATOM-CHAR = <any CHAR except atom-specials> atom-specials = "(" / ")" / "{" / SP / CTL /
 * list-wildcards / quoted-specials / resp-specials list-wildcards = "%" / "*" quoted-specials =
 * DQUOTE / "\" resp-specials = "]" ASTRING-CHAR = ATOM-CHAR / resp-specials tag = 1*<any
 * ASTRING-CHAR except "+">
 */
public final class Chars {
  ...
  static {
    for (int i = 0x21; i < 0x7f; i++) {
      ATOM_CHARS[i] = ASTRING_CHARS[i] = TAG_CHARS[i] = FETCH_CHARS[i] = CAPABILITY_CHARS[i] = true;
    }
