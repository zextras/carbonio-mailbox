...
/**
 * Represents a blob in blob store incoming directory. An incoming blob does not belong to any
 * mailbox. When a message is delivered to a mailbox, message is saved in the incoming directory and
 * a link to it is created in the mailbox's directory. The linked blob in mailbox directory is
 * represented by a MailboxBlob object.
 */
public class Blob {
  private File file;
  private String path;
  ...
  /** Returns the SHA-256 digest of this blob's uncompressed data, encoded in base64. */
  ...
  /**
   * Returns the size of the blob's data. If the blob is compressed, returns the uncompressed size.
   */
