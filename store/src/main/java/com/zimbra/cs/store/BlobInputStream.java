...
public class BlobInputStream extends InputStream implements SharedInputStream, InputStreamSource {

  private static final Log sLog = LogFactory.getLog(BlobInputStream.class);

  /**
   * The file that stores the content of this stream. Only the parent stream stores the file. All
   * child objects get the path from the top-level parent.
   */
  private File mFile;
  ...
  private static int BUFFER_SIZE =
      Math.max(LC.zimbra_blob_input_stream_buffer_size_kb.intValue(), 1) * 1024;
  ...
  /**
   * Read buffer size (may be less than <tt>mBuf.length</tt>). A value of 0 means that the buffer is
   * not initialized.
   */
   ...
   * @param start starting index, or <tt>null</tt> for beginning of file
   * @param end ending index (exclusive), or <tt>null</tt> for end of file
   * @param parent the parent stream, or <tt>null</tt> if this is the first stream. If non-null, the
   *     file from the parent is used.
   */
  protected BlobInputStream(File file, long rawSize, Long start, Long end, BlobInputStream parent)
    ...
    if (start != null && end != null && start > end) {
      String msg =
          String.format(
              "Start index %d for file %s is larger than end index %d", start, file.getPath(), end);
    ...
    if (end == null) {
      ...
      if (end > mRawSize) {
        String msg =
            String.format(
                "End index %d for %s exceeded file size %d", end, file.getPath(), mRawSize);
        throw new IOException(msg);
      }
      mEnd = end;
    }
    ...
    sLog.debug(
        "Created %s: file=%s, length=%d, uncompressed length=%d, start=%d, end=%d, parent=%s,"
            + " mStart=%d, mEnd=%d.",
  ...
  private int fillBuffer(long pos) throws IOException {
    ...
    int numRead =
        getFileDescriptorCache().read(getRootFile().getPath(), mRawSize, pos, mBuf, 0, numToRead);
    if (numRead > 0) {
      mBufPos = pos;
      mBufSize = numRead;
    } else {
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    if (mPos >= mBufPos && mPos < (mBufPos + mBufSize)) {
      ...
      numRead =
          (int) Math.min(len, mBufPos + mBufSize - mPos); // Don't read past the end of the buffer.
      ...
      if (len > mBuf.length) {
        ...
        numRead =
            getFileDescriptorCache().read(getRootFile().getPath(), mRawSize, mPos, b, off, len);
      } else {
  ...
  public synchronized void reset() throws IOException {
    if (mMarkPos == Long.MIN_VALUE) {
      throw new IOException(
          String.format("reset() called before mark() for blob '%s'", getRootFilePath()));
    }
  ...
  @Override
  /**
   * Ensures that the file descriptor gets closed when this object is garbage collected. We
   * generally don't like finalizers, but we make an exception in this case because we have no
   * control over how JavaMail uses BlobInputStream.
   */
  ...
  public InputStream newStream(long start, long end) {
    if (start < 0) {
      throw new IllegalArgumentException(
          String.format(
              "Problem creating newStream from blob '%s' with start %s, end %s.  Start cannot be"
                  + " less than 0",
