...
/**
 * Abstract framework for StoreManager implementations which require content hash or other
 * content-based locator The base implementation here handles the more common cases where blob is
 * cached locally by storeIncoming and then pushed to remote store during stage operation
 */
public abstract class ContentAddressableStoreManager extends ExternalStoreManager {
  ...
  /**
   * Return the locator string for the content hash by hex encoding or other similar encoding
   ...
   * @param actualSize: size of data in stream, or -1 if size is unknown. To be used by implementor
   ...
   * @throws ServiceException
   */
  protected abstract void writeStreamToStore(
      InputStream in, long actualSize, Mailbox mbox, String locator)
      throws IOException, ServiceException;

  @Override
  public StagedBlob stage(Blob blob, Mailbox mbox) throws IOException, ServiceException {
    if (supports(StoreFeature.RESUMABLE_UPLOAD)
        && blob instanceof ExternalUploadedBlob
        && blob.getRawSize() > 0) {
  ...
  protected StagedBlob stage(InputStream in, long actualSize, Mailbox mbox, String locator)
      throws ServiceException {
