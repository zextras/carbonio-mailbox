...
/**
 * Consistency checker for ExternalStoreManager implementations. The StoreManager may optionally
 * override getAllBlobPaths(Mailbox mbox) which will be used to locate unexpected blobs/orphaned
 * blobs. In cases where the StoreManager has no direct associated between Mailbox and stored blobs
 * a list of all used blobs can be obtained and used for manual/scripted consistency checking
 */
public class ExternalBlobConsistencyChecker extends BlobConsistencyChecker {
  ...
  private void checkExternalBlob(
      ...
      throws ServiceException {
    MailboxBlob mblob =
        sm.getMailboxBlob(mbox, blobInfo.itemId, blobInfo.version, blobInfo.path, false);
    if (mblob == null) {
      ...
      try {
        ...
        blobInfo.fetchException =
            e instanceof IOException
                ? (IOException) e
                : new IOException("Exception fetching blob", e);
        results.missingBlobs.put(blobInfo.itemId, blobInfo);
      }
    }
  }
