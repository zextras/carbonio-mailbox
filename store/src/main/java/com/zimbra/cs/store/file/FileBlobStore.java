...
public final class FileBlobStore extends StoreManager {
  ...
  public void startup() throws IOException, ServiceException {
    ...
    FileCache<String> ufCache =
        FileCache.Builder.createWithStringKey(ufCacheDir, false)
            .minLifetime(LC.uncompressed_cache_min_lifetime.longValue())
            .build();
  ...
  private Blob getUniqueIncomingBlob() throws IOException, ServiceException {
    ...
    if (incdir == null) {
      throw ServiceException.FAILURE(
          "storing blob to volume without incoming directory: " + volume.getName(), null);
    }
  ...
  public VolumeStagedBlob stage(InputStream in, long actualSize, Mailbox mbox)
      throws IOException, ServiceException {
    // mailbox store is on the same volume as incoming directory, so just storeIncoming() and wrap
    // it
  ...
  /**
   * Create a copy of a blob in volume/path specified by dest* parameters. Note this method is not
   * part of the StoreManager interface It is only to be used for FileBlobStore specific code such
   * as BlobMover
   *
   * @param src
   * @param destMbox
   ...
   */
  public VolumeMailboxBlob copy(
      Blob src, Mailbox destMbox, int destItemId, int destRevision, short destVolumeId)
      throws IOException, ServiceException {
    Volume volume = MANAGER.getVolume(destVolumeId);
    return copy(src, destMbox, destItemId, destRevision, volume);
  }

  private VolumeMailboxBlob copy(
      Blob src, Mailbox destMbox, int destItemId, int destRevision, Volume destVolume)
      throws IOException, ServiceException {
    ...
    if (ZimbraLog.store.isDebugEnabled()) {
      ...
      ZimbraLog.store.debug(
          "Copying %s (size=%d, raw size=%d) to %s for mailbox %d, id %d.",
    ...
    VolumeBlob newBlob =
        (VolumeBlob)
            new VolumeBlob(dest, destVolume.getId())
                .copyCachedDataFrom(src)
                .setCompressed(destCompressed);
    return new VolumeMailboxBlob(
        destMbox, destItemId, destRevision, destVolume.getLocator(), newBlob);
  }
  ...
  public VolumeMailboxBlob link(
    ...
    if (ZimbraLog.store.isDebugEnabled()) {
      ...
      long srcRawSize = src.getRawSize();
      ZimbraLog.store.debug(
          "Linking %s (size=%d, raw size=%d) to %s for mailbox %d, id %d.",
          srcPath, srcSize, srcRawSize, destPath, destMbox.getId(), destItemId);
    }
      ...
      try {
        ...
        if (dest.exists()) {
          ...
          ZimbraLog.store.warn(
              "Destination file exists.  Backing up to " + destBak.getAbsolutePath());
          if (destBak.exists()) {
            String bak = destBak.getAbsolutePath();
            ZimbraLog.store.warn(
                bak + " already exists.  Deleting to make room for new backup file");
            if (!destBak.delete()) {
  ...
  @Override
  public VolumeMailboxBlob renameTo(
      StagedBlob src, Mailbox destMbox, int destItemId, int destRevision)
      throws IOException, ServiceException {
    Volume volume = MANAGER.getCurrentMessageVolume();
    VolumeBlob blob = ((VolumeStagedBlob) src).getLocalBlob();
    ...
    if (ZimbraLog.store.isDebugEnabled()) {
      ...
      ZimbraLog.store.debug(
          "Renaming %s (size=%d, raw size=%d) to %s for mailbox %d, id %d.",
          srcPath, srcSize, srcRawSize, destPath, destMbox.getId(), destItemId);
    }
    ...
    if (srcVolumeId == volume.getId()) {
      ...
      if (!renamed) throw new IOException("Unable to rename " + srcPath + " to " + destPath);
    } else {
    ...
    VolumeBlob vblob =
        (VolumeBlob) new VolumeBlob(destFile, volume.getId()).copyCachedDataFrom(blob);
  ...
  private File getMailboxBlobFile(
      Mailbox mbox, int itemId, int revision, short volumeId, boolean check)
      throws ServiceException {
  ...
  public static String getBlobPath(Mailbox mbox, int itemId, int revision, short volumeId)
      throws ServiceException {
    return getBlobPath(mbox.getId(), itemId, revision, volumeId);
  }

  public static String getBlobPath(int mboxId, int itemId, int revision, short volumeId)
      throws ServiceException {
