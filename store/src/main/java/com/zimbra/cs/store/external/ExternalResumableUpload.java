...
public interface ExternalResumableUpload {
  ...
  /**
   * Create a new ExternalResumableIncomingBlob instance to handle the upload of a single object.
   * The implementation should compute all remote metadata such as remote id, size, and content hash
   * inline with the upload process so that finishUpload() does not need to traverse the data again
   *
  ...
  /**
   * Finalize an upload. Depending on store semantics this may involve a commit, checksum, or other
   * similar operation.
   *
