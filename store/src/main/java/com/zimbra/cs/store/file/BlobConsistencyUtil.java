...
public class BlobConsistencyUtil {
  ...
  private BlobConsistencyUtil() {
    ...
    options.addOption(
        new Option(
            "v", LO_VERBOSE, false, "Display verbose output.  Display stack trace on error."));
    options.addOption(new Option(null, LO_SKIP_SIZE_CHECK, false, "Skip blob size check."));
    options.addOption(
        new Option(
            null,
            LO_OUTPUT_USED_BLOBS,
            false,
    ...
    o =
        new Option(
            null,
            LO_UNEXPECTED_BLOB_LIST,
            true,
            "Write the paths of any unexpected blobs to a file.");
    o.setArgName("path");
    options.addOption(o);
    ...
    options.addOption(
        null, LO_MISSING_BLOB_DELETE_ITEM, false, "Delete any items that have a missing blob.");
    ...
    options.addOption(
        new Option(
            null,
            LO_INCORRECT_REVISION_RENAME_FILE,
            false,
  ...
  private void checkMailbox(int mboxId, SoapProvisioning prov) throws ServiceException {
    ...
    request.addAttribute(
        AdminConstants.A_REPORT_USED_BLOBS, outputUsedBlobs || usedBlobWriter != null);
    ...
    for (Element mboxEl : response.listElements(AdminConstants.E_MAILBOX)) {
      ...
      for (BlobInfo blob : results.missingBlobs.values()) {
        System.out.format(
            "Mailbox %d, item %d, rev %d, %s: blob not found.\n",
            results.mboxId, blob.itemId, blob.modContent, locatorText(blob));
      }
      for (BlobInfo blob : results.incorrectSize.values()) {
        System.out.format(
            "Mailbox %d, item %d, rev %d, %s: incorrect data size.  Expected %d, was %d.  File size"
                + " is %d.\n",
            results.mboxId,
            blob.itemId,
            blob.modContent,
      ...
      if (incorrectRevisionRenameFile) {
        for (BlobInfo blob : results.incorrectModContent.values()) {
          ...
          if (dir != null) {
            ...
            System.err.format(
                "Could not determine parent directory of %s.\n", file.getAbsolutePath());
          }
        }
      }
    }
  ...
  private void exportAndDelete(SoapProvisioning prov, BlobConsistencyChecker.Results results)
    ...
    System.out.format(
        "Deleting %d items from mailbox %d.\n", results.missingBlobs.size(), results.mboxId);

    ExportAndDeleteMailboxSpec mailbox = new ExportAndDeleteMailboxSpec(results.mboxId);
    ExportAndDeleteItemsRequest jaxbRequest =
        new ExportAndDeleteItemsRequest(exportDir, "mbox" + results.mboxId + "_", mailbox);
    for (BlobInfo blob : results.missingBlobs.values()) {
