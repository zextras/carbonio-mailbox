...
public class BlobConsistencyChecker {
  ...
  public static class Results {
    public int mboxId;
    public Multimap<Integer, BlobInfo> missingBlobs =
        TreeMultimap.create(new IntegerComparator(), new BlobInfoComparator());
    public Multimap<Integer, BlobInfo> incorrectSize =
        TreeMultimap.create(new IntegerComparator(), new BlobInfoComparator());
    public Multimap<Integer, BlobInfo> unexpectedBlobs =
        TreeMultimap.create(new IntegerComparator(), new BlobInfoComparator());
    public Multimap<Integer, BlobInfo> incorrectModContent =
        TreeMultimap.create(new IntegerComparator(), new BlobInfoComparator());
    public Multimap<Integer, BlobInfo> usedBlobs =
        TreeMultimap.create(new IntegerComparator(), new BlobInfoComparator());
    ...
    public Results(Element mboxElement) throws ServiceException {
      if (!mboxElement.getName().equals(AdminConstants.E_MAILBOX)) {
        throw ServiceException.INVALID_REQUEST(
            "Unexpected element: " + mboxElement.getName(), null);
      }
      mboxId = (int) mboxElement.getAttributeLong(AdminConstants.A_ID);
      for (Element item :
          mboxElement
              .getElement(AdminConstants.E_MISSING_BLOBS)
              .listElements(AdminConstants.E_ITEM)) {
      ...
      for (Element itemEl :
          mboxElement
              .getElement(AdminConstants.E_INCORRECT_SIZE)
              .listElements(AdminConstants.E_ITEM)) {
        BlobInfo blob = new BlobInfo();
      ...
      for (Element blobEl :
          mboxElement
              .getElement(AdminConstants.E_UNEXPECTED_BLOBS)
              .listElements(AdminConstants.E_BLOB)) {
        BlobInfo blob = new BlobInfo();
      ...
      for (Element itemEl :
          mboxElement
              .getElement(AdminConstants.E_INCORRECT_REVISION)
              .listElements(AdminConstants.E_ITEM)) {
        BlobInfo blob = new BlobInfo();
        blob.itemId = (int) itemEl.getAttributeLong(AdminConstants.A_ID);
        blob.modContent = (int) itemEl.getAttributeLong(AdminConstants.A_REVISION);
      ...
      for (Element itemEl :
          mboxElement.getElement(AdminConstants.E_USED_BLOBS).listElements(AdminConstants.E_ITEM)) {
        BlobInfo blob = new BlobInfo();
        blob.itemId = (int) itemEl.getAttributeLong(AdminConstants.A_ID);
    ...
    public boolean hasInconsistency() {
      return !(missingBlobs.isEmpty()
          && incorrectSize.isEmpty()
          && unexpectedBlobs.isEmpty()
          && incorrectModContent.isEmpty());
    }
    ...
    public void toElement(Element parent) {
      ...
      for (BlobInfo blob : missingBlobs.values()) {
        missingEl
            .addElement(AdminConstants.E_ITEM)
      ...
      for (BlobInfo blob : incorrectSize.values()) {
        Element itemEl =
            incorrectSizeEl
                ...
                .addAttribute(AdminConstants.A_VOLUME_ID, blob.volumeId);
        itemEl
            .addElement(AdminConstants.E_BLOB)
  ...
  public Results check(
    ...
    if (!(sm instanceof FileBlobStore)) {
      throw ServiceException.INVALID_REQUEST(
          sm.getClass().getSimpleName() + " is not supported", null);
    }
    ...
    try {
      ...
      for (short volumeId : volumeIds) {
        ...
        int mailboxMaxId =
            DbBlobConsistency.getMaxId(conn, mbox); // Maximum id for the entire mailbox
        ...
        while (minId <= mailboxMaxId && group < numGroups) {
          ...
          // multiple BlobInfo objects are created when there are multiple revisions of the same
          // file
          ...
          while (minId <= mailboxMaxId) {
            ...
            for (BlobInfo blob :
                DbBlobConsistency.getBlobInfo(conn, mbox, minId, maxId, volumeId)) {
  ...
  /**
   * Reconciles blobs against the files in the given directory and adds any inconsistencies to the
   * current result set.
   */
  private void check(short volumeId, String blobDirPath, Multimap<Integer, BlobInfo> blobsById)
    ...
    // and if there aren't any items with same itemId in revisions then it is categorised as
    // incorrect revision
    ...
    while (keyIterator.hasNext()) {
      ...
      if (missingBlobs.size() == 1
          && unexpectedBlobs.size() == 1
          && revisions.get(itemId).size() == 0) {
