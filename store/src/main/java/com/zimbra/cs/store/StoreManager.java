...
public abstract class StoreManager {
  ...
  public static int getDiskStreamingThreshold() throws ServiceException {
    if (diskStreamingThreshold == null) loadSettings();
  ...
  public enum StoreFeature {
    /**
     * The store has the ability to delete all {@code MailboxBlob}s associated with a given {@code
     * Mailbox}, <u>without</u> having a list of those blobs provided to it.
     */
    BULK_DELETE,
    /**
     * The store is reachable from any {@code mailboxd} host. When moving mailboxes between hosts,
     * the store should be left untouched, as there is no need to move the blobs along with the
     * metadata.
     */
    CENTRALIZED,
    ...
    RESUMABLE_UPLOAD,
    /**
     * The store supports deduping based on blob content; aka SIS create. If two users upload the
     * same file, only one copy is stored. The remote store must track reference count internally
     * and delete the actual file only when ref-count reaches 0
     */
    SINGLE_INSTANCE_SERVER_CREATE,
    /**
     * The store is a custom store that does not support standard Zimbra actions. It requires to be
  ...
  /**
   * Returns a 'BlobBuilder' which can be used to store a blob in incoming directory asynchronously
   * one chunk at a time. Blob will be compressed if volume supports compression and blob size is
  ...
  /**
   * Stage an incoming <code>InputStream</code> to an appropriate place for subsequent storage in a
  ...
  /**
   * Stage an incoming <code>Blob</code> (see {@link #storeIncoming}) to an appropriate place for
   * subsequent storage in a <code>Mailbox</code> via {@link #link(StagedBlob, Mailbox, int, int)}
   * or {@link #renameTo}.
   *
   * @param blob
   * @param mbox
  ...
  public abstract MailboxBlob copy(
      MailboxBlob src, Mailbox destMbox, int destMsgId, int destRevision)
      throws IOException, ServiceException;

  /**
   * Create a link in destMbox mailbox with message ID of destMsgId that points to srcBlob. If
   * staging creates permanent blobs, this just needs to return mailbox blob with pointer to
   * location of staged blob If staging is done to temporary area such as our incoming directory
   * this operation finishes it
   *
   * @param src
   * @param destMbox
   * @param destMsgId mail_item.id value for message in destMbox
  ...
  /**
   * Rename a blob to a blob in mailbox directory. This effectively makes the StagedBlob permanent,
   * implementations may not need to do anything if the stage operation creates permanent items
   *
   * @param src
   * @param destMbox
   * @param destMsgId mail_item.id value for message in destMbox
  ...
  /**
   * Deletes a blob from store. If blob doesn't exist, no exception is thrown and false is returned.
   *
   * @param mblob
   * @return true if blob was actually deleted
   * @throws IOException
   ...
   * @return the <code>MailboxBlob</code>, or <code>null</code> if the file does not exist
   * @throws ServiceException
   */
   ...
   * @throws ServiceException
   */
  public abstract MailboxBlob getMailboxBlob(
      Mailbox mbox, int itemId, int revision, String locator, boolean validate)
      throws ServiceException;
  ...
  public MailboxBlob getMailboxBlob(MailItem item) throws ServiceException {
    MailboxBlob mblob =
        getMailboxBlob(item.getMailbox(), item.getId(), item.getSavedSequence(), item.getLocator());
    if (mblob != null) {
  ...
  /**
   * Return an InputStream of blob content. Caller should close the stream when done.
   *
   * @param mboxBlob
   * @return
   * @throws IOException
   ...
   *
   * @param mbox
   * @param blobs If the store returned {@code true} to {@link #supports(StoreFeature)} when passed
   *     {@link StoreFeature#BULK_DELETE}, {@code blobs} will be {@code null}. If it returned {@code
   *     false}, {@code blobs} will be an {@code Iterable} that contains every blob in the {@code
   *     Mailbox}.
