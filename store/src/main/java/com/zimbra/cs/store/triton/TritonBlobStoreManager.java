...
public class TritonBlobStoreManager extends SisStore implements ExternalResumableUpload {
  ...
  private MessageDigest newDigest() throws ServiceException {
    ...
    switch (hashType) {
      ...
      case SHA0:
        try {
          // SHA0 is not implemented in base Java classes since it was withdrawn in 1993 before Java
          // was released
          // we use cryptix here for demo, but we aren't bothering with legal approval since we're
          // requiring TDS to switch to SHA-256
          // so need to drop cryptix32.jar into /opt/zextras/jetty/webapps/service/WEB-INF/lib
          // during install
  ...
  protected void writeStreamToStore(InputStream in, long actualSize, Mailbox mbox, String locator)
    ...
    if (actualSize < 0) {
      throw ServiceException.FAILURE(
          "Must use resumable upload (i.e. StoreManager.newIncomingBlob()) if size is unknown",
          null);
    ...
    try {
      InputStreamEntity entity =
          new InputStreamEntity(in, actualSize, ContentType.APPLICATION_OCTET_STREAM);
      ...
      if (statusCode == HttpStatus.SC_CREATED) {
        ...
        throw ServiceException.FAILURE(
            "unable to store blob " + statusCode + ":" + httpResp.getStatusLine().getReasonPhrase(),
            null);
      }
    } catch (HttpException e) {
      throw new IOException(
          "unexpected error during write stream to store operation: " + e.getMessage());
    } finally {
    ...
    try {
      ...
      if (statusCode == HttpStatus.SC_OK) {
        ...
        if (statusCode == HttpStatus.SC_NOT_FOUND && emptyLocator.equals(locator)) {
          // empty file edge case. could compare hash before this, but that hurts perf. for normal
          // case
          ZimbraLog.store.info("returning input stream for empty blob");
          return new ByteArrayInputStream(new byte[0]);
        } else {
          throw new IOException(
              "unexpected return code during blob GET: "
  ...
  public boolean deleteFromStore(String locator, Mailbox mbox) throws IOException {
    ...
    try {
      ...
      if (statusCode == HttpStatus.SC_OK) {
        ...
        throw new IOException(
            "unexpected return code during blob DELETE: "
                + httpResp.getStatusLine().getReasonPhrase());
      }
    } catch (HttpException e) {
      throw new IOException(
          "unexpected error during delete from store operation: " + e.getMessage());
    } finally {
  ...
  /**
   * Run SIS operation against remote server. If a blob already exists for the locator the remote
    ...
    try {
      ...
      if (statusCode == HttpStatus.SC_CREATED) {
        ...
        ZimbraLog.store.error(
            ...
            statusCode, EntityUtils.toString(httpResp.getEntity()));
        throw new IOException(
            "unable to SIS create " + statusCode + ":" + httpResp.getStatusLine().getReasonPhrase(),
            null);
      }
    } catch (HttpException e) {
