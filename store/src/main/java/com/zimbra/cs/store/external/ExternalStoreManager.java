...
/**
 * Abstract base class for external store integration. Uses local incoming directory during blob
 * creation and maintains local file cache of retrieved blobs to minimize remote round-trips
 */
public abstract class ExternalStoreManager extends StoreManager implements ExternalBlobIO {

  private final IncomingDirectory incoming =
      new IncomingDirectory(LC.zimbra_tmp_directory.value() + File.separator + "incoming");
  ...
  public void startup() throws IOException, ServiceException {
    ...
    localCache =
        FileCache.Builder.createWithStringKey(localCacheDir, false)
            ...
            .removeCallback(new MessageCacheChecker())
            .build();
    ...
    for (MailboxBlob.MailboxBlobInfo mbinfo : blobs) {
      try {
        ...
        ZimbraLog.store.warn(
            "IOException during deleteStore() for mbox [%d] item [%d] revision [%d] locator [%s]",
  ...
  protected Blob getLocalBlob(Mailbox mbox, String locator, boolean fromCache) throws IOException {
    ...
    if (is == null) {
      throw new IOException(
          "Store " + this.getClass().getName() + " returned null for locator " + locator);
    } else {
  ...
  public MailboxBlob renameTo(StagedBlob src, Mailbox destMbox, int destMsgId, int destRevision)
    ...
    MailboxBlob mblob =
        new ExternalMailboxBlob(destMbox, destMsgId, destRevision, staged.getLocator());
  ...
  @Override
  public StagedBlob stage(InputStream in, long actualSize, Mailbox mbox)
    ...
    try {
      ...
      return new ExternalStagedBlob(
          mbox, ByteUtil.encodeFSSafeBase64(digest.digest()), pin.getPosition(), locator);
    } catch (IOException e) {
  ...
  /**
   * Get a set of all blobs which exist in the store associated with a mailbox Optional operation
   * used to find orphaned blobs If the blob store does not partition based on mailbox this method
