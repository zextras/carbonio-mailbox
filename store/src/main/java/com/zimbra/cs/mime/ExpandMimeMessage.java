...
public class ExpandMimeMessage {
  ...
  /**
   * Copies the <tt>MimeMessage</tt> if a converter would want to make a change, but doesn't alter
  ...
  /** Returns the expanded message, or the original message if no expansion was done. */
  ...
  /**
   * Applies all registered on-the-fly MIME converters to a the encapsulated message. The original
   * message is not modified.
   *
   * @return a new <tt>MimeMessage</tt> if converters were applied, or <tt>null</tt> if not.
  ...
  public boolean expand() throws MessagingException {
    ...
    try {
      for (Class<? extends MimeVisitor> vclass : MimeVisitor.getConverters()) {
        ...
        if (mExpandedMessage != mMimeMessage) vclass.newInstance().accept(mExpandedMessage);
      }
