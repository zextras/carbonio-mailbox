...
public final class ParsedDocument {
  ...
  public ParsedDocument(
    ...
    if (LC.documents_disable_instant_parsing.booleanValue() == false) performExtraction();
  }
    ...
    try {
      ...
      try {
        ...
        if (ConversionException.isTemporaryCauseOf(e)) {
          ZimbraLog.doc.warn(
              "Temporary failure extracting from the document.  (is convertd down?)", e);
          temporaryAnalysisFailure = true;
        } else {
          ZimbraLog.index.warn(
              "Failure indexing wiki document " + filename + ".  Item will be partially indexed",
              e);
        }
      }
      ...
      appendToContent(
          content, ZimbraAnalyzer.getAllTokensConcatenated(LuceneFields.L_FILENAME, filename));
      ...
      if (ConversionException.isTemporaryCauseOf(mhe)) {
        ZimbraLog.doc.warn(
            "Temporary failure extracting from the document.  (is convertd down?)", mhe);
        temporaryAnalysisFailure = true;
      } else {
      ...
      ZimbraLog.index.warn(
          "Failure indexing wiki document " + filename + ".  Item will be partially indexed", e);
    } finally {
      parsed = true;
    }
  }
  ...
  public List<IndexDocument> getDocumentList() {
    ...
    return document == null
        ? Collections.<IndexDocument>emptyList()
