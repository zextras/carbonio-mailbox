...
/**
 * Converts each TNEF MimeBodyPart to a multipart/alternative that contains the original TNEF file
 * and its MIME counterpart.
 *
 * <p>For example, the following structure:
 *
 * <ul>
 *   <li>MimeMessage + MimeMultipart (multipart/mixed)
 *       <ul>
 *         <li>MimeBodyPart (text/plain)
 *         <li><b>MimeBodyPart (application/ms-tnef)</b>
 *       </ul>
 * </ul>
 *
 ...
 *       <ul>
 *         <li>MimeBodyPart (text/plain)
 *         <li><b>MimeMultipart (multipart/alternative)</b>
 *             <ul>
 *               <li><b>MimeBodyPart (application/ms-tnef)</b>
 *               <li><b>MimeMessage + MimeMultipart (multipart/mixed)</b>
 *             </ul>
 *       </ul>
 * </ul>
 *
...
public class TnefConverter extends MimeVisitor {
  ...
  protected boolean visitMessage(MimeMessage msg, VisitPhase visitKind) throws MessagingException {
    ...
    try {
      ...
      if (!TNEFUtils.isTNEFMimeType(msg.getContentType())) return false;
    ...
    try {
      ...
      if (!(content instanceof MimeBodyPart)) return false;
  ...
  protected boolean visitMultipart(MimeMultipart mmp, VisitPhase visitKind)
    ...
    // do the decode in the exit phase
    if (visitKind != VisitPhase.VISIT_END) return false;
    // proactively ignore already-converted TNEF attachments
    if (MimeConstants.CT_MULTIPART_ALTERNATIVE.equals(mmp.getContentType())) return false;
  ...
  /**
   * Performs the TNEF->MIME conversion on any TNEF body parts that make up the given message.
   *
   * @throws ServiceException
   */
  private MimeMultipart expandTNEF(MimeBodyPart bp) throws MessagingException, IOException {
    if (!TNEFUtils.isTNEFMimeType(bp.getContentType())) return null;
        ...
        if (calConverter.convert(mMimeMessage, bp.getInputStream(), icalHandler)) {
          if (icalHandler.getNumCals() > 0) {
            ...
            for (ZVCalendar cal : cals) {
              ...
              if (method == null) method = cal.getMethod();
            }
            writer.close();
            ...
            if (method != null) ct.setParameter("method", method.toString());
    ...
    if (icalPart != null) altMulti.addBodyPart(icalPart);
