...
public class Mime {
  ...
  private static final Set<String> TRANSFER_ENCODINGS =
      ImmutableSet.of(
          MimeConstants.ET_7BIT,
          MimeConstants.ET_8BIT,
          MimeConstants.ET_BINARY,
          MimeConstants.ET_QUOTED_PRINTABLE,
          MimeConstants.ET_BASE64);

  private static final Set<String> INLINEABLE_TYPES =
      ImmutableSet.of("image/jpeg", "image/png", "image/gif");

  private static Set<String> TEXT_ALTERNATES =
      ImmutableSet.of(MimeConstants.CT_TEXT_ENRICHED, MimeConstants.CT_TEXT_HTML);

  private static Set<String> HTML_ALTERNATES =
      ImmutableSet.of(MimeConstants.CT_TEXT_ENRICHED, MimeConstants.CT_TEXT_PLAIN);
  ...
  /**
   * Max length (in bytes) that a MIME multipart preamble can be before we give up and wrap the
   * whole multipart in a text/plain.
   */
  ...
  public static List<MPartInfo> getParts(MimeMessage mm, String defaultCharset)
    ...
    for (MPartInfo mpi : parts) {
      ...
      if (mpi.mIsFilterableAttachment)
        mpi.mIsToplevelAttachment =
            bodies == null || !bodies.contains(mpi) || !INLINEABLE_TYPES.contains(mpi.mContentType);
    }
    return parts;
  }
  ...
  private static List<MPartInfo> listParts(MimePart root, String defaultCharset)
        ...
        if (multi != null) {
          if (multi.getCount() == 0 && LC.mime_promote_empty_multipart.booleanValue()) {
            ...
            if (MimeConstants.CT_MULTIPART_APPLEDOUBLE.equalsIgnoreCase(getContentType(mp))) {
              ...
              mpart.mContentType =
                  mpart.mContentType.replace(
                      MimeConstants.CT_MULTIPART_APPLEDOUBLE, MimeConstants.CT_APPLEFILE);
            }
          }
          mpart.mChildren = new ArrayList<MPartInfo>(multi.getCount());
          for (int i = 1; i <= multi.getCount(); i++) {
            mpart.mChildren.add(
                generateMPartInfo((MimePart) multi.getBodyPart(i - 1), mpart, prefix + i, i));
          }
    ...
    if (emptyMultipart != null && parts.size() == 1) {
      ...
      if (!StringUtil.isNullOrEmpty(text)) {
        ZimbraLog.misc.debug(
            "single multipart with no children. promoting the preamble into a single text part");
        parts.remove(0);
  ...
  /**
   * Some devices send wide base64 encoded message body i.e. without line folding. As per RFC
   * https://www.ietf.org/rfc/rfc2045.txt see 6.8. Base64 Content-Transfer-Encoding "The encoded
   * output stream must be represented in lines of no more than 76 characters each."
   *
   * <p>To fix the issue here, re-writing the same content to message part.
   *
   * @param mm
   * @throws MessagingException
   * @throws IOException
  ...
  private static MPartInfo generateMPartInfo(
      MimePart mp, MPartInfo parent, String prefix, int partNum) {
    boolean inDigest =
        parent != null && parent.mContentType.equals(MimeConstants.CT_MULTIPART_DIGEST);
  ...
  private static boolean isZimbraJavaMailShim(Object o) {
    return ZMimeMessage.usingZimbraParser()
        && (o instanceof ZMimePart || o instanceof ZMimeMultipart);
  }
  ...
  private static MimeMultipart validateMultipart(MimeMultipart multi, MimePart mp)
    ...
    try {
      if (!ctype.containsParameter("generated")
          && !findStartBoundary(mp, ctype.getParameter("boundary"))) {
  ...
  /**
   * Returns whether the given "boundary" string occurs within the first {@link
   * #MAX_PREAMBLE_LENGTH} bytes of the {@link MimePart}'s content.
   */
    ...
    try {
      for (int i = 0; i < MAX_PREAMBLE_LENGTH; i++) {
        ...
        if (c == -1) {
          ...
          if (!failed && (boundary == null ? bindex > 0 : bindex == blength)) return true;
          bindex = dashes = 0;
          failed = false;
        } else if (failed) {
          continue;
          ...
          if (bindex >= blength || c != boundary.charAt(bindex++)) failed = true;
        }
      }
    } finally {
  ...
  static InputStream getRawInputStream(MimePart mp) throws MessagingException {
    if (mp instanceof MimeBodyPart) return ((MimeBodyPart) mp).getRawInputStream();
    if (mp instanceof MimeMessage) return ((MimeMessage) mp).getRawInputStream();
  ...
  private static class RawContentMultipartDataSource extends FixedMultipartDataSource {
    ...
    private class RawContentInputStream extends FilterInputStream {
      ...
      RawContentInputStream(InputStream is) {
        ...
        mBoundary =
            explicitBoundary == null ? "_-_" + UUID.randomUUID().toString() : explicitBoundary;
  ...
  /**
   * Returns the MimeMessage object encapsulating a MIME part with content-type "message/rfc822".
   * Use this method instead of {@link Part#getContent()} to work around JavaMail's fascism about
   * proper MIME format and failure to support RFC 2184.
   */
  public static MimeMessage getMessageContent(MimePart message822Part)
      throws IOException, MessagingException {
    String ctype = getContentType(message822Part);
    if (MimeConstants.CT_MESSAGE_RFC822.equals(ctype)) {
      // JavaMail will only return a correct MimeMessage if the Content-Type header was set
      // correctly
      Object content = message822Part.getContent();
      if (content instanceof MimeMessage) return (MimeMessage) content;
    }
  ...
  /**
   * Returns the MimeMultipart object encapsulating the body of a MIME part with content-type
   * "multipart/*". Use this method instead of {@link Part#getContent()} to work around JavaMail's
   * fascism about proper MIME format and failure to support RFC 2184.
   */
  ...
  /**
   * Returns a String containing the text content of the MimePart. If the part's specified charset
   * is unknown, defaults first to the user's preferred charset and then to the to the system's
   * default charset. Use this method instead of {@link Part#getContent()} to work around JavaMail's
   * fascism about proper MIME format and failure to support RFC 2184.
   */
  ...
  /**
   * Returns a <tt>Reader</tt> for the text content of the <tt>MimePart</tt>. If the part's
   * specified charset is unknown, defaults first to the user's preferred charset and then to the to
   * the system's default charset. Use this method instead of {@link Part#getContent()} to work
   * around JavaMail's fascism about proper MIME format and failure to support RFC 2184.
   *
   * @deprecated Use getTextReader() directly after calling repairTransferEncoding() as it's almost
   *     always necessary to drain and close the mimepart's input stream.
   */
  @Deprecated
  ...
  public static void recursiveRepairTransferEncoding(MimeMessage mm)
      throws MessagingException, IOException {
  ...
  public static MimePart getMimePart(MimePart mp, String part)
      throws IOException, MessagingException {
    if (mp == null) {
      return null;
    }
    ...
    for (int i = 0; i < subpart.length; i++) {
      ...
      String ct =
          getContentType(
              mp, digestParent ? MimeConstants.CT_MESSAGE_RFC822 : MimeConstants.CT_DEFAULT);
      if (ct == null) {
        return null;
      }
   ...
   *
   * <p>Note: Zimbra folder sharing notifications are not considered attachments for this purpose.
   *
   * @param mpi
   * @return
   */
  private static boolean isFilterableAttachment(MPartInfo mpi, Set<MPartInfo> bodies) {
    ...
    if (mpi.isMultipart()) return false;
    ...
    if (ctype.startsWith(MimeConstants.CT_TEXT_PREFIX)) {
      ...
      if (parent == null || (mpi.getPartNum() == 1 && parent.isMessage())) return false;

      // inlined text parts are not filterable attachments
      if (bodies != null && bodies.contains(mpi)) return false;

      // ignore body parts with a parent of multipart/alternative
      if (parent.getContentType().equals(MimeConstants.CT_MULTIPART_ALTERNATIVE)) return false;
      ...
      if (mpi.getPartNum() == 1) {
        if (parent.isMultipart()) {
          ...
          if (pp == null || pp.isMessage()) return false;
        }
      }
    }
  ...
  /**
   * Given a list of <code>MPartInfo</code>s (as returned from {@link #getParts}), returns a <code>
   * Set</code> of unique content-type strings, or an empty set if there are no attachments.
   */
  ...
  /**
   * Returns true if any of the given message parts qualify as top-level "attachments" for the
   * purpose of displaying the little paperclip icon in the web UI. Note that Zimbra folder sharing
   * notifications are expressly *not* considered attachments for this purpose.
   */
  public static boolean hasAttachment(List<MPartInfo> parts) {
    for (MPartInfo mpi : parts) {
      if (mpi.mIsToplevelAttachment) return true;
    }
    return false;
  }

  /** Returns true if any of the given message parts has a content-type of text/calendar */
  public static boolean hasTextCalenndar(List<MPartInfo> parts) {
    for (MPartInfo mpi : parts) {
      if (MimeConstants.CT_TEXT_CALENDAR.equals(mpi.getContentType())) return true;
    }
    return false;
  }
  ...
  public static InternetAddress[] parseAddressHeader(
      MimeMessage mm, String headerName, boolean expandGroups) {
    try {
  ...
  public static InternetAddress[] parseAddressHeader(String header, boolean expandGroups) {
    if (header == null || header.trim().isEmpty()) return NO_ADDRESSES;
    header = header.trim();
    ...
    if (!expandGroups) return addresses;
    ...
    for (InternetAddress addr : addresses) {
      if (!addr.isGroup()) {
        ...
        try {
          ...
          if (members == null) expanded.add(addr);
          else for (InternetAddress member : members) expanded.add(member);
  ...
  static RecipientType[] sRcptTypes =
      new RecipientType[] {RecipientType.TO, RecipientType.CC, RecipientType.BCC};

  /**
   * Remove all email addresses in rcpts from To/Cc/Bcc headers of a MimeMessage.
   *
   * @param mm
   * @param rcpts
   * @throws MessagingException
  ...
  public static void removeRecipients(MimeMessage mm, String[] rcpts) throws MessagingException {
    for (RecipientType rcptType : sRcptTypes) {
      ...
      for (int j = 0; j < addrs.length; j++) {
        ...
        boolean match = false;
        for (int k = 0; k < rcpts.length; k++) if (addr.equalsIgnoreCase(rcpts[k])) match = true;
        if (!match) list.add(inetAddr);
      }
  ...
  /**
   * Determines the "primary/subtype" part of a Multipart's Content-Type header. Uses a permissive,
   * RFC2231-capable parser, and defaults when appropriate.
   */
  ...
  /**
   * Determines the "primary/subtype" part of a Part's Content-Type header. Uses a permissive,
   * RFC2231-capable parser, and defaults when appropriate.
   */
  ...
  /**
   * Determines the "primary/subtype" part of a Part's Content-Type header. Uses a permissive,
   * RFC2231-capable parser, and defaults as indicated.
   */
  public static final String getContentType(MimePart mp, String ctdefault) {
    try {
      String cthdr = mp.getHeader("Content-Type", null);
      if (cthdr == null || cthdr.trim().isEmpty()) return ctdefault;
      return getContentType(cthdr);
    } catch (MessagingException e) {
      ZimbraLog.extensions.warn(
          "could not fetch part's content-type; defaulting to " + MimeConstants.CT_DEFAULT, e);
      return MimeConstants.CT_DEFAULT;
    }
  }

  /**
   * Determines the "primary/subtype" part of a Content-Type header string. Uses a permissive,
   * RFC2231-capable parser, and defaults when appropriate.
   */
  ...
  /**
   * Reads the specified <code>InputStream</code> into a <code>String</code>. <code>contentType
   * </code> must of type "text/*". If a valid charset parameter is present in the Content-Type
   * string, it is used as the charset for decoding the text. If not, we fall back to the user's
   * default charset preference. If both of those options fail, the platform default is used.
   *
  ...
  public static String decodeText(InputStream input, String contentType, String defaultCharset)
    ...
    try {
      ...
      int num;
      while ((num = reader.read(cbuff, 0, cbuff.length)) != -1) buffer.append(cbuff, 0, num);
    } finally {
   ...
   *
   * <p>{@code contentType} must of type "text/*". This method tries to detect a charset in the
   * following order.
   *
   * <ol>
  ...
  /**
   * Returns the decoded and unfolded value for the given header name. If multiple headers with the
   * same name exist, returns the first one. If the header does not exist, returns <tt>null</tt>.
   */
  ...
  /**
   * Returns the decoded and unfolded values for the given header name, or an empty array if no
   * headers with the given name exist.
   */
  public static String[] getHeaders(MimePart part, String headerName) {
    try {
      String[] values = part.getHeader(headerName);
      if (values == null || values.length == 0) return NO_HEADERS;
  ...
  /**
   * Returns the value of the <tt>Message-ID</tt> header, or <tt>null</tt> if the header does not
  ...
  /**
   * Returns the decoded value of the <tt>Subject</tt> header, or <tt>null</tt> if the header does
   * not exist.
   */
  ...
  /**
   * Returns the value of the <tt>From</tt> header. If not available, returns the value of the
   * <tt>Sender</tt> header. Returns an empty {@code String} if neither header is available.
   */
  ...
  private static Set<MPartInfo> getBodySubparts(MPartInfo base, boolean preferHtml) {
    ...
    if (!base.hasChildren() || base.isMessage()) return null;
    ...
    if (ctype.equals(MimeConstants.CT_MULTIPART_ALTERNATIVE)) {
      ...
      return getRelatedBodySubpart(
          base.getChildren(), preferHtml, base.getContentTypeParameter("start"));
    } else if (ctype.equals(MimeConstants.CT_MULTIPART_REPORT)) {
      return getReportBodySubpart(base.getChildren(), preferHtml);
    } else if (ctype.equals(MimeConstants.CT_MULTIPART_MIXED)
        || !KNOWN_MULTIPART_TYPES.contains(ctype)) {
    ...
    for (MPartInfo mpi : children) {
      ...
      } else if (!mpi.getDisposition().equals(Part.ATTACHMENT)
          && !mpi.isMessage()
          && (mpi.getContentID() == null
              || mpi.getContentType().matches(MimeConstants.CT_TEXT_WILD))) {
  ...
  private static Set<MPartInfo> getAlternativeBodySubpart(
      List<MPartInfo> children, boolean preferHtml) {
    ...
    for (MPartInfo mpi : children) {
      ...
      if (!isAttachment && ctype.equals(wantType)) {
        ...
        Set<MPartInfo> body;
        if ((body = getBodySubparts(mpi, preferHtml)) != null) return body;
      }
    }
  ...
  private static Set<MPartInfo> getRelatedBodySubpart(
      List<MPartInfo> children, boolean preferHtml, String parentCID) {
    // if the multipart/related part had a "parent" param, that names the body subpart by Content-ID
    if (parentCID != null) {
      for (MPartInfo mpi : children) {
        if (!parentCID.equals(mpi.getContentID())) continue;
  ...
  private static Set<MPartInfo> getReportBodySubpart(List<MPartInfo> children, boolean preferHtml) {
    ...
    for (MPartInfo mpi : children) {
      ...
      if (!isAttachment && ctype.equals(wantType)) {
        ...
        Set<MPartInfo> body;
        if ((body = getBodySubparts(mpi, preferHtml)) != null) subparts.addAll(body);
      }
    }
    if (subparts.size() == 0) {
  ...
  public static void main(String[] args) throws MessagingException, IOException {
    String s =
        URLDecoder.decode(
            "Zimbra%20&#26085;&#26412;&#35486;&#21270;&#12398;&#32771;&#24942;&#28857;.txt",
            "utf-8");
    System.out.println(s);
    System.out.println(
        expandNumericCharacterReferences(
            "Zimbra%20&#26085;&#26412;&#35486;&#21270;&#12398;&#32771;&#24942;&#28857;.txt&#x40;&;&#;&#x;&#&#3876;&#55"));

    MimeMessage mm =
        new FixedMimeMessage(
            JMSession.getSession(), new SharedFileInputStream("C:\\Temp\\mail\\24245"));
    InputStream is =
        new RawContentMultipartDataSource(mm, new ContentType(mm.getContentType()))
            .getInputStream();
    int num;
  ...
  /**
   * Returns an {@code InputStream} to the content of a {@code MimeMessage} by starting a thread
   * that serves up its content to a {@code PipedOutputStream}. This workaround is necessary because
   * JavaMail does not provide {@code InputStream} access to the content.
   */
  ...
  /**
   * Returns the size of this <tt>MimePart</tt>'s content. If the content is encoded, returns the
  ...
  /**
   * Returns {@code true} if the {@code Auto-Submitted} header is set to a value other than {@code
   * no}.
   */
   ...
   * embedded comments and quoted-strings are stripped. No duplicate elimination is performed.
   */
  ...
  private static List<String> getReferences(String hvalue) {
    ...
    while (pos < end) {
      ...
      if (c == '\r' || c == '\n') {
        ...
        if (!empty) {
          refs.add(
              new com.zimbra.common.mime.InternetAddress(value.substring(astart, pos))
                  .getAddress());
        }
        empty = true;
        astart = pos;
    ...
    if (!empty) {
      refs.add(
          new com.zimbra.common.mime.InternetAddress(value.substring(astart, pos)).getAddress());
    }
  ...
  public static boolean isMultipartSigned(String contentType) {
    if (contentType.contains(MimeConstants.CT_MULTIPART_SIGNED)) return true;
    else return false;
  }
  ...
  public static boolean isPKCS7Signed(String contentType) {
    if ((contentType.contains(MimeConstants.CT_APPLICATION_SMIME)
        ...
        && contentType.contains(MimeConstants.CT_SMIME_TYPE_SIGNED_DATA)) return true;
    else return false;
  }

  public static boolean isEncrypted(String contentType) {
    if (contentType.contains(MimeConstants.CT_SMIME_TYPE_ENVELOPED_DATA)) return true;
    else return false;
  }
}
