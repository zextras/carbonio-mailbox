...
public class ParsedMessageOptions {
  ...
  public ParsedMessageOptions(Blob blob, byte buffer[], Long receivedDate, Boolean indexAttachments)
      throws IOException {
    if (buffer == null) setContent(blob.getFile());
    else setContent(buffer);
    setDigest(blob.getDigest()).setSize(blob.getRawSize());
    if (receivedDate != null) setReceivedDate(receivedDate);
    if (indexAttachments != null) setAttachmentIndexing(indexAttachments);
  }
