...
public abstract class MimeHandler {
  /**
   * The name of the HTTP request parameter used to identify an individual file within an archive.
   * Its value is unspecified; it may be the full path within the archive, or a sequence number. It
   * should be understood between the archive handler and the servlet.
   */
  ...
  /**
   * Returns <tt>true</tt> if a request for the handler to perform text extraction or HTML
   * conversion will result in an RPC to an external process. <tt>false</tt> indicates what all
   * processing is done in-process.
   */
  ...
  /**
   * Adds the indexed fields to the Lucene document for search. Each handler determines a set of
   * fields that it deems important for the type of documents it handles.
   */
  ...
  /**
   * Returns the content for this MIME part. Length of content returned cannot exceed the value
   * returned by {@link MimeHandlerManager#getMaxContentLength}.
   */
   ...
   */
  public abstract String convert(AttachmentInfo doc, String urlPart)
  ...
  /**
   * Determines if this handler can convert the document into HTML/images. If this method returns
   * false, then the original document will be returned to the browser.
   *
   * @return
   */
