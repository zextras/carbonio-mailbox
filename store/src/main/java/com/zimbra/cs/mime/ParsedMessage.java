...
/**
 * Instantiates a JavaMail {@link MimeMessage} from a <tt>byte[]</tt> or file on disk. Converts or
 * mutates the message as necessary by calling registered instances of {@link MimeVisitor}.
 * Conversion modifies the in- memory message without affecting the raw version. Mutation modifies
 * the raw version and affects results returned by the <tt>getRawXXX</tt> methods.
 *
 * @since 2004. 6. 30.
 * @author jhahm
 */
public final class ParsedMessage {
  ...
  /**
   * if TRUE then there was a _temporary_ failure analyzing the message. We should attempt to
   * re-index this message at a later time
   */
  ...
  /**
   * Runs MIME mutators and converters, initializes {@link #mMimeMessage}, {@link
   * #mExpandedMessage}, {@link #mFileInputStream} and {@link #mReceivedDate} based on message
   * content.
   */
  private void init(Long receivedDate, boolean indexAttachments)
    ...
    if (mimeMessage == null) {
      ...
      mimeMessage =
          expandedMessage = new Mime.FixedMimeMessage(JMSession.getSession(), sharedStream);
    }
    ...
    try {
      ...
      mimeMessage =
          expandedMessage = new Mime.FixedMimeMessage(JMSession.getSession(), getRawInputStream());
    }
  ...
  /**
   * Applies all registered on-the-fly MIME converters to a copy of the encapsulated message
   * (leaving the original message intact), then generates the list of message parts.
   *
  ...
  /**
   * Applies all registered MIME mutators to the encapsulated message. The message is not forked, so
   * both {@link #mMimeMessage} and {@link #mExpandedMessage} are affected by the changes.
   *
   * @return <tt>true</tt> if a mutator altered the content or <tt>false</tt> if the encapsulated
  ...
  private static final Set<String> ENCRYPTED_PART_TYPES =
      ImmutableSet.of(
          MimeConstants.CT_APPLICATION_SMIME,
          MimeConstants.CT_APPLICATION_PGP,
          MimeConstants.CT_MULTIPART_ENCRYPTED);

  /**
   * Analyze and extract text from all the "body" (non-attachment) parts of the message. This step
   * is required to properly generate the message fragment.
   */
  ...
  private void analyzeNonBodyParts() throws ServiceException {
    ...
    try {
      ...
      if (numParseErrors > 0) {
        LOG.warn(
            "Message had analysis errors in %d parts (Message-Id: %s, Subject: %s)",
  ...
  /**
   * Extract all indexable text from this message. This API should *only* be called if you are sure
   * you're going to add the message to the index. The only callsites of this API should be Mailbox
   * and test utilities. Don't call it unless you absolutely are sure you need to do so.
   */
  ...
  /**
   * Returns a stream to the raw MIME message. Affected by mutation but not conversion.
   *
   * <p><b>Important</b>: when the content comes from a <tt>MimeMessage</tt>, JavaMail requires us
   * to start a {@link MimeMessageOutputThread} to serve up the content. As a result, all calls to
   * this method must be wrapped in a try/finally with a call to {@link
   * ByteUtil#closeStream(InputStream)} in the finally block. This guarantees that the stream is
   * drained and the <tt>MimeMessageOutputThread</tt> exits.
   */
  ...
  /**
   * Returns the {@link BlobInputStream} if this message is being streamed from a file, otherwise
   * returns {@code null}.
   */
      ...
      try {
        ...
        if (envSender.getAddress() != null) {
          ...
          if (envSenderAddrParts != null) {
            ...
            if (sender.startsWith("owner-")
                || sender.endsWith("-owner")
                || sender.indexOf("-request") != -1
                || sender.equals("mailer-daemon")
                || sender.equals("majordomo")
                || sender.equals("listserv")) {
              return true;
            }
          }
        }
  ...
  /**
   * Returns all message-ids referenced by this message's headers. This includes those in the
   * message's {@code Message-ID} header, its {@code In-Reply-To} header, and its {@code References}
   * header. The enclosing angle brackets and any embedded comments and quoted-strings are stripped
   * from the message-ids.
   *
   * @return a non-{@code null}, mutable {@code Set} containing the message's references.
   */
  ...
  public List<ParsedAddress> getParsedRecipients() {
    if (parsedRecipients == null) {
      List<com.zimbra.common.mime.InternetAddress> addrs =
  ...
  /** Returns the email address of the first <tt>From</tt> or <tt>Sender</tt> header. */
  public String getSenderEmail() {
    return getSenderEmail(true);
  }

  /**
   * If fromFirst is true, returns the email address of the first <tt>From</tt> or <tt>Sender</tt>
   * header. If fromFirst is false, returns the email address of the first <tt>Sender</tt> or
   * <tt>From</tt> header.
   */
  ...
  /**
   * Returns CalenarPartInfo object containing a ZVCalendar representing an iCalendar part and some
   * additional information about the calendar part such as whether the part was found inside a
   * forwarded rfc822 message and if the part had "method" parameter indicating it was an actual
   * invite email as opposed to a regular email that happens to carry an ics attachment.
   */
  ...
  /**
   * @return TRUE if there was a _temporary_ failure detected while analyzing the message. In the
   *     case of a temporary failure, the message should be flagged and indexing re-tried at some
   *     point in the future
   */
  ...
  private IndexDocument getMainBodyLuceneDocument(StringBuilder fullContent)
    ...
    try {
      doc.addEnvFrom(
          new RFC822AddressTokenStream(getMimeMessage().getHeader("X-Envelope-From", ",")));
    } catch (MessagingException ignore) {
    }
    try {
    ...
    for (String fn : filenames) {
      appendToContent(
          contentPrepend, ZimbraAnalyzer.getAllTokensConcatenated(LuceneFields.L_FILENAME, fn));
      appendToContent(
          contentPrepend, fn); // also add the non-tokenized form, so full-filename searches match
    }
    ...
    try {
      ...
      ZimbraLog.index.warn(
          "Unable to recognize searchable objects in message: msgid=%s,subject=%s",
  ...
  /**
   * For every attachment, many of the lucene indexed fields from the top level message are also
   * indexed as part of the attachment: this is done so that the attachment will show up if you do
   * things like "type:pdf and from:foo"
   *
   * <p>"this" --> top level doc
   *
  ...
  private static boolean isBouncedCalendar(MPartInfo mpi) {
    if (MimeConstants.CT_TEXT_CALENDAR.equals(mpi.getContentType())) {
      ...
      while ((parent = parent.getParent()) != null) {
        ...
        if (MimeConstants.CT_MULTIPART_REPORT.equals(
            ct)) // Assume multipart/report == bounced message.
        return true;
      }
    }
    return false;
  ...
  private String analyzePart(boolean isMainBody, MPartInfo mpi)
    ...
    String methodParam =
        (new ContentType(mpi.getMimePart().getContentType())).getParameter("method");
    ...
    try {
      ...
      if (!ignoreCalendar
          && calendarPartInfo == null
          && ctype.equals(MimeConstants.CT_TEXT_CALENDAR)) {
        if (handler.isIndexingEnabled()) {
  ...
  private void handleParseError(MPartInfo mpi, Throwable error) {
    ...
    LOG.warn(
        "Unable to parse part=%s filename=%s content-type=%s message-id=%s",
  ...
  static {
    ...
    for (String localized :
        L10nUtil.getMessagesAllLocales(
            L10nUtil.MsgKey.calendarSubjectCancelled,
  ...
  static Pair<String, Boolean> trimPrefixes(String subject) {
    ...
    while (true) {
      ...
      if (subject.length() == 0) return new Pair<String, Boolean>(subject, trimmed);

      // first, strip off any "(fwd)" at the end
      int tstart = subject.length() - 5;
      char c;
      if (tstart >= 0
          && subject.charAt(tstart) == '('
      ...
      if (LC.conversation_ignore_maillist_prefix.booleanValue()) {
        int bclose;
        if (braced
            && (bclose = subject.indexOf(']')) > 0
            && subject.lastIndexOf('[', bclose) == 0) {
  ...
  private void normalizeSubject() {
    ...
    try {
      normalizedSubject =
          subject = StringUtil.stripControlCharacters(Mime.getSubject(getMimeMessage()));
    } catch (MessagingException e) {
    }
  ...
  public static String normalize(String subject) {
    String trimmed =
        compressWhitespace(trimPrefixes(StringUtil.stripControlCharacters(subject)).getFirst());
  ...
  /**
   * {@link Threader} is cached per mailbox as {@link ParsedMessage} is shared by multiple mailboxes
   * in shared delivery.
   */
