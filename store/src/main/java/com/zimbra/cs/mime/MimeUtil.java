...
public class MimeUtil {
  /**
   * (non-Javadoc) Encode <tt>string</tt> no matter what characters are contained in it.
   *
   ...
   * @param encodingWord available when encoding is "quoted-printable". If it's set to
   *     <tt>false</tt>, only "=_?" characters are encoded, otherwise
   *     "=_?\"#$%&'(),.:;<>@[\\]^`{|}~" will be encoded.
   * @return encoded text
   * @throws UnsupportedEncodingException
   * @see javax.mail.internet.MimeUtility.encodeWord(String string, String charset, String encoding,
   *     boolean encodingWord)
   */
  public static String encodeWord(
      String string, String charset, String encoding, boolean encodingWord)
      throws UnsupportedEncodingException {
   ...
   * (non-Javadoc)
   *
   * @see javax.mail.internet.MimeUtility.doEncode(String string, boolean b64, String jcharset, int
   *     avail, String prefix, boolean first, boolean encodingWord, StringBuffer buf)
   */
  private static void doEncode(
    ...
    if ((len > avail) && ((size = string.length()) > 1)) {
      ...
      doEncode(
          string.substring(0, size / 2), b64, jcharset, avail, prefix, first, encodingWord, buf);
      doEncode(
          string.substring(size / 2, size), b64, jcharset, avail, prefix, false, encodingWord, buf);
    } else {
