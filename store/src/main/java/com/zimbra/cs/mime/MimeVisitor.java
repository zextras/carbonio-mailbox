...
public abstract class MimeVisitor {
  ...
  /** The list of registered MimeVistor classes that convert stored messages on the fly. */
  private static List<Class<? extends MimeVisitor>> sMimeConverters =
      new ArrayList<Class<? extends MimeVisitor>>();
  /**
   * The list of registered MimeVistor classes that convert new messages before storing them to
   * disk.
   */
  private static List<Class<? extends MimeVisitor>> sMimeMutators =
      new ArrayList<Class<? extends MimeVisitor>>();
  ...
  static {
    ...
    try {
      if (LC.zimbra_converter_enabled_tnef.booleanValue()) registerConverter(TnefConverter.class);
    } catch (Exception e) {
  ...
  /**
   * Adds a MimeVisitor class to the list of converters invoked on the fly when a message is fetched
   * from the store or prepared for indexing. Note that changes made by these MimeVisitors are not
   * persisted to disk but instead are executed every time the message is accessed.
   */
  ...
  /**
   * Adds a MimeVisitor class to the list of mutators invoked before a message is saved to disk or
   * sent via SMTP.
   */
  ...
  /**
   * This inner interface permits the {@link Mime#accept} caller to be notified immediately before
   * any changes to the MimeMessage are performed by a <code>MimeVistor</code>. Note that when a
   * call to {@link Mime#accept} results in multiple modifications, the callback will be invoked
   * multiple times.
   */
  public static interface ModificationCallback {
    /**
     * A callback function invoked immediately prior to any modification of the message. If the
     * callback returns <code>false</code>, the modification is not performed.
     */
  ...
  /**
   * Sets the MimeVisitor's pre-modification callback. The callback can be unset by passing <code>
   * null</code> as the argument.
   *
  ...
  /**
   * Returns the pre-modification callback currently associated with the MimeVisitor, or <code>null
   * </code> if there is no such callback.
   */
  ...
  /**
   * The flags passed to the <code>visitXXX</code> methods before and after a node's children are
  ...
  /**
   * Visitor callback for traversing a MimeMessage, either standalone or as an attachment to another
   * MimeMessage.
   *
   * @return whether any changes were performed during the visit
   * @see VisitPhase
   */
  protected abstract boolean visitMessage(MimeMessage mm, VisitPhase visitKind)
   ...
   * @see VisitPhase
   */
  protected abstract boolean visitMultipart(MimeMultipart mp, VisitPhase visitKind)
      throws MessagingException;
   ...
   * Invokes the various <code>MimeVisitor</code> methods in for each visited node.
   *
  ...
  private final synchronized boolean accept(MimePart mp, int depth) throws MessagingException {
    ...
    if (depth >= MAX_VISITOR_DEPTH) return false;
    ...
    if (isMultipart) {
      ...
      if (content instanceof MimeMultipart) {
        ...
        if (visitMultipart(multi, VisitPhase.VISIT_BEGIN)) modified = multiModified = true;

        try {
          for (int i = 0; i < multi.getCount(); i++) {
            BodyPart bp = multi.getBodyPart(i);
            if (bp instanceof MimeBodyPart) {
              if (accept((MimeBodyPart) bp, depth + 1)) modified = multiModified = true;
            } else {
        ...
        if (visitMultipart(multi, VisitPhase.VISIT_END)) modified = multiModified = true;
        if (multiModified) mp.setContent(multi);
      }
    } else if (isMessage) {
      ...
      if (content != null) modified |= accept(content, depth + 1);
      ...
      ZimbraLog.extensions.info(
          "unexpected MimePart subclass: " + mp.getClass().getName() + " (ctype='" + ctype + "')");
    }
