...
public final class ParsedContact {
   ...
   * @param fields maps field names to either a <tt>String</tt> or <tt>String[]</tt> value.
   */
  public ParsedContact(Map<String, ? extends Object> fields, List<Attachment> attachments)
      throws ServiceException {
    init(fields, null);
  ...
  private static void validateImageAttachments(List<Attachment> attachments)
    ...
    for (Map.Entry<String, ? extends Object> entry : fields.entrySet()) {
      ...
      if (key != null && !key.trim().isEmpty() && !Strings.isNullOrEmpty(value)) {
        if (key.length() > ContactConstants.MAX_FIELD_NAME_LENGTH) {
          ...
          throw MailServiceException.CONTACT_TOO_BIG(
              ContactConstants.MAX_FIELD_VALUE_LENGTH, value.length());
        }
        map.put(key, value);
      }
    }
  ...
  private static MimeMessage generateMimeMessage(List<Attachment> attachments)
    ...
    for (Attachment attach : attachments) {
      ...
      // MimeBodyPart.setDataHandler() invalidates Content-Type and CTE if there is any, so make
      // sure
      // it gets called before setting Content-Type and CTE headers.
      try {
        bp.setDataHandler(
            new DataHandler(new ByteArrayDataSource(attach.getContent(), attach.getContentType())));
      } catch (IOException e) {
  ...
  private static List<Attachment> parseBlob(InputStream in)
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "MimeMultipart content expected but got " + mm.getContent().toString(), x);
    }
  ...
  public static class FieldDeltaList {
    ...
    public void addGroupMemberDelta(
        ContactGroup.Member.Type memberType, String value, FieldDelta.Op op) {
  ...
  public ParsedContact modifyField(String name, String newValue) {
    if (Strings.isNullOrEmpty(newValue)) contactFields.remove(name);
    else contactFields.put(name, newValue);
    return this;
  }

  // convert legacy API to the new API
  public ParsedContact modify(
      Map<String, String> fieldDelta, List<Attachment> attachDelta, String ownerAcctId)
      throws ServiceException {
  ...
  public ParsedContact modify(
      FieldDeltaList fieldDeltaList, List<Attachment> attachDelta, String ownerAcctId)
      throws ServiceException {
    return modify(fieldDeltaList, attachDelta, false, ownerAcctId);
  }

  public ParsedContact modify(
      FieldDeltaList fieldDeltaList,
      List<Attachment> attachDelta,
    ...
    contactGroup =
        encodedContactGroup == null
            ? ContactGroup.init()
            : ContactGroup.init(encodedContactGroup, ownerAcctId);
  ...
  private void processAttrDelta(AttrDelta delta) throws ServiceException {
    ...
    if (op == null) {
      ...
      if (newValue == null || newValue.equals("")) contactFields.remove(name);
      else contactFields.put(name, newValue);
  ...
  public ParsedContact analyze(Mailbox mbox) throws ServiceException {
    try {
      ...
      ZimbraLog.index.warn(
          "exception while analyzing contact; attachments will be partially indexed", e);
    }
    return this;
  }
  ...
  public boolean hasTemporaryAnalysisFailure() {
    return mHasTemporaryAnalysisFailure;
  }
  ...
  private void analyzeAttachment(
      Attachment attach, StringBuilder contentText, boolean indexAttachments)
  ...
  private static void appendContactField(
      StringBuilder sb, ParsedContact contact, String fieldName) {
  ...
  private IndexDocument getPrimaryDocument(Account acct, String contentStrIn)
      throws ServiceException {
    ...
    for (Map.Entry<String, String> entry : getFields().entrySet()) {
      ...
      if (!Contact.isEmailField(
          emailFields, fieldName)) { // skip email addrs, they're added to CONTENT below
    ...
    for (String email :
        Contact.getEmailAddresses(
            emailFields, getFields(), DerefGroupMembersOption.NONE, acct.getId())) {
    ...
    // text
    contentText =
        new StringBuilder(emailStrTokens)
            .append(' ')
            .append(contentText)
            .append(' ')
            .append(contentStrIn);
