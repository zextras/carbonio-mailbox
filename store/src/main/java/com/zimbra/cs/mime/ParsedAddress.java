...
public final class ParsedAddress implements Comparable<ParsedAddress> {
  ...
  /**
   * Used in parsing the display name of InternetAddress, specially in case of Japanese locale where
   * the display name is in "LastName FirstName" format. Bug #91944
   */
   ...
   * @param nameFormatLastFirst is True if the display name in InternetAddress is in "LastName
   *     FirstName" format otherwise false
   */
  ...
  private void initialize(String email, String personal) {
    ...
    if ("".equals(emailPart)) emailPart = null;
    if (personalPart != null) personalPart = personalPart.trim().replaceAll("\\s+", " ");
  }
  ...
  public ParsedAddress parse() {
    ...
    if (personalPart != null
        && personalPart.startsWith("\"")
        && personalPart.indexOf('"', 1) == personalPart.length() - 1)
      personalPart =
          personalPart.substring(1, personalPart.length() - 1).trim().replaceAll("\\s+", " ");

    if (personalPart != null && personalPart.equals("")) personalPart = null;

    if (emailPart != null && emailPart.equals(personalPart)) personalPart = null;

    if (emailPart != null && personalPart == null) {
      int p = emailPart.indexOf('@');
      String possible =
          (p == -1 ? emailPart : emailPart.substring(0, p)).replace('.', ' ').replace('_', ' ');
      if (possible.indexOf(' ') != -1) personalPart = possible;
      else firstName = possible;
    }
    ...
    if (personalPart != null) {
      ...
      if (tokens.size() == 1) {
        firstName = tokens.get(0).toString();
        parsed = true;
      }

      if (!parsed) parsed = parseLastFirst(tokens);
      if (!parsed) parsed = parseFirstLast(tokens);

      if (!parsed) {
        for (NameToken token : tokens) {
          NameTokenType type = token.getType();
          if (type == NameTokenType.ABBREVIATION
              || type == NameTokenType.INITIAL
              || type == NameTokenType.PREFIX
              || type == NameTokenType.TOKEN) {
  ...
  // PREFIX* (PREFIX | TOKEN) COMMA HONORIFIC* (ABBREVIATION | (INITIAL? (INITIAL | PREFIX |
  // TOKEN))) (INITIAL | ABBREVIATION | PREFIX | TOKEN)* .*
  private boolean parseLastFirst(List<NameToken> tokens) {
    ...
    personalPart =
        personalPart.substring(0, suffixOffset)
            + personalPart.substring(suffixOffset).replaceAll("\\\\", "");

    return true;
  }

  // HONORIFIC* (ABBREVIATION | (INITIAL? (INITIAL | PREFIX | TOKEN))) (INITIAL | ABBREVIATION |
  // PREFIX | TOKEN)* PREFIX* (PREFIX | TOKEN) .*
  private boolean parseFirstLast(List<NameToken> tokens) {
    ...
    for (; index < total; index++) {
      if ((type = tokens.get(index).getType()) != NameTokenType.INITIAL
          && type != NameTokenType.ABBREVIATION
          && type != NameTokenType.PREFIX
          && type != NameTokenType.TOKEN) break;
    }
    for (; index > start; index--) {
      if (tokens.get(index - 1).getType() != NameTokenType.ABBREVIATION) break;
    }
    if (start == index) return false;
    int lstart = index - 1;
    if (tokens.get(lstart).getType() == NameTokenType.INITIAL) return false;
    for (; lstart > start; lstart--) {
      if (tokens.get(lstart - 1).getType() != NameTokenType.PREFIX) break;
    }
    if (lstart > start) mtokens = tokens.subList(start, lstart);
  ...
  private int readHonorific(List<NameToken> tokens, int index) {
    // honorific: HONORIFIC*
    while (index < tokens.size() && tokens.get(index).getType() == NameTokenType.HONORIFIC) index++;
    return index;
  }
  ...
  private int readFirstName(List<NameToken> tokens, int index, boolean greedy) {
    ...
    else if ((type = tokens.get(index).getType()) == NameTokenType.ABBREVIATION
        || type == NameTokenType.TOKEN
        || type == NameTokenType.PREFIX) return index + 1;
    else if (type != NameTokenType.INITIAL) return index;
    ...
    //  INITIAL | PREFIX X*
    index++;
    if (index >= (greedy ? total : total - 1)) return index;
    switch (tokens.get(index).getType()) {
      case INITIAL:
        while (index < total && tokens.get(index).getType() == NameTokenType.INITIAL) index++;
        return index;
      case TOKEN:
        return (greedy
                || (type3 = tokens.get(index + 1).getType()) == NameTokenType.TOKEN
                || type3 == NameTokenType.PREFIX)
            ? index + 1
            : index;
      case PREFIX:
        return (greedy
  ...
  private int readLastName(List<NameToken> tokens, int index) {
    ...
    for (; index < tokens.size(); index++) {
      if ((type = tokens.get(index).getType()) != NameTokenType.PREFIX) {
        if (type == NameTokenType.TOKEN) index++;
        break;
      }
    }
    return index;
  ...
  private String readSuffix(List<NameToken> tokens, int index) {
    ...
    while (index < end
        && ((type = tokens.get(index).getType()) == NameTokenType.COMMA
            || type == NameTokenType.DELIMITER)) index++;
    while (index < end
        && ((type = tokens.get(end - 1).getType()) == NameTokenType.COMMA
            || type == NameTokenType.DELIMITER)) end--;
    // if the whole suffix is parenthesized, trim the parentheses
    if (index < end
        && tokens.get(index).getType() == NameTokenType.OPEN
        && tokens.get(end - 1).getType() == NameTokenType.CLOSE) {
    ...
    return index >= end
        ? null
        : personalPart
            .substring(tokens.get(index - 1).getEndOffset(), tokens.get(end - 1).getEndOffset())
            .trim();
  }

  @Override
  public int hashCode() {
    if (emailPart != null) return emailPart.hashCode();
    else if (personalPart != null) return personalPart.hashCode();
    else return 0;
  }
  ...
  public boolean equals(Object obj) {
    ...
    else if (obj == null || !(obj instanceof ParsedAddress)) return false;
    else return compareTo((ParsedAddress) obj) == 0;
  }
  ...
  public int compareTo(ParsedAddress pa) {
    ...
    else if (emailPart != null && pa.emailPart == null) return 1;
    else if (emailPart == null && pa.emailPart != null) return -1;
    else if (personalPart != null && pa.personalPart != null)
      return personalPart.compareToIgnoreCase(pa.personalPart);
    else if (personalPart != null && pa.personalPart == null) return 1;
    else if (personalPart == null && pa.personalPart != null) return -1;
    else return 0;
  }
  ...
  public String toString() {
    if (emailPart == null) return personalPart;
    else if (personalPart == null) return emailPart;
    else return '"' + personalPart + "\" <" + emailPart + '>';
  }
  ...
  private static class NameToken {
    ...
    private NameToken(String token, int start) {
      ...
      for (char c;
          tstart < tend
              && (!Character.isLetterOrDigit(c = token.charAt(tstart)) || c == '-')
              && c != '\'';
          tstart++)
        ;
      ...
      for (char c;
          tstart < tend
              && (!Character.isLetterOrDigit(c = token.charAt(tend - 1)) || c == '-')
              && c != '.'
              && c != '\''
              && (tstart > 0 || c != '!');
          tend--)
      ...
      if (delimiter) mType = NameTokenType.DELIMITER;
      else if (SURNAME_PREFIXES.contains(token.toLowerCase())) mType = NameTokenType.PREFIX;
      else if (length <= 2
          && (length == 1 || token.charAt(1) == '.')
          && Character.isLetter(token.charAt(0))) mType = NameTokenType.INITIAL;
      else if (ROMAN_NUMERALS.contains(token)) mType = NameTokenType.NUMERAL;
      else if (isAbbreviation()) mType = NameTokenType.ABBREVIATION;
      else if (length > 2 && token.charAt(length - 1) == '.') mType = NameTokenType.HONORIFIC;
      else mType = NameTokenType.TOKEN;
    }
    ...
    private boolean isAbbreviation() {
      if (mValue.length() % 2 == 1) return false;

      for (int i = 0; i < mValue.length(); i += 2) {
        if (mValue.charAt(i + 1) != '.' || !Character.isLetter(mValue.charAt(i))) return false;
      }
      return true;
    }
    ...
    private static final Set<String> SURNAME_PREFIXES =
        new HashSet<String>(
            Arrays.asList(
                "ab", "abu", "af", "al", "am", "an", "ap", "auf", "av", "bar", "ben", "bin", "da",
                "de", "degli", "dei", "del", "della", "de'", "delle", "dello", "den", "der", "di",
                "dos", "du", "el", "groot", "groote", "het", "ibn", "la", "las", "le", "lo", "los",
                "mac", "n\u00cd", "nic", "op", "ten", "ter", "toe", "van", "vande", "vander", "vom",
                "von", "zu", "zum", "zur", "'t", "\u00d3"));

    private static final Set<String> ROMAN_NUMERALS =
        new HashSet<String>(
            Arrays.asList("II", "III", "IIII", "IV", "V", "VI", "VII", "VIII", "Jr"));
    ...
    private static final Set<Character> TOKEN_DELIMITERS =
        new HashSet<Character>(
            Arrays.asList(',', ';', '/', ':', '(', ')', '[', ']', '\u3010', '\u3011'));
    private static final Set<Character> TOKEN_SYMBOLS =
        new HashSet<Character>(
            Arrays.asList(
                '!', '\"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ',', '-', '.', '/', ':',
                ';', '<', '=', '>', '?', '@', '[', '\\', ']', '^', '_', '`', '{', '|', '}', '~'));
    private static final int TYPICAL_NUM_SYMBOLS_IN_SMILEY =
        DebugConfig.numberOfConsecutiveSymbolsInSenderName;
