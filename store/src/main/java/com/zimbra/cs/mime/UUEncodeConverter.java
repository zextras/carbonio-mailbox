...
public class UUEncodeConverter extends MimeVisitor {
  ...
  protected boolean visitMessage(MimeMessage mm, VisitPhase visitKind) throws MessagingException {
    ...
    try {
      ...
      if (!mm.isMimeType(MimeConstants.CT_TEXT_PLAIN)) return false;
      ...
      if (cte != null) {
        ...
        if (!cte.equals(MimeConstants.ET_7BIT)
            && !cte.equals(MimeConstants.ET_8BIT)
            && !cte.equals(MimeConstants.ET_BINARY)) return false;
      }
        ...
        for (int c = is.read(); c != -1; ) {
          ...
          if (c == 'b'
            ...
            if (c != -1 && filename.length() > 0) {
              if (uufiles == null) uufiles = new ArrayList<UUDecodedFile>(3);
              try {
      ...
      if (uufiles == null || uufiles.isEmpty()) return false;
      ...
      for (UUDecodedFile uu : uufiles) {
        ...
        mbp.setHeader(
            ...
            new ContentDisposition(Part.ATTACHMENT)
                .setParameter("filename", uu.getFilename())
                .toString());
      ...
      try {
        ...
        for (UUDecodedFile uu : uufiles) {
          ...
          while (count > 0
              && (numRead = isOrig.read(buffer, 0, (int) Math.min(count, 8192))) >= 0) {
        ...
        mbp.setDataHandler(
            new DataHandler(
                new ByteArrayDataSource(baos.toByteArray(), MimeConstants.CT_TEXT_PLAIN)));
