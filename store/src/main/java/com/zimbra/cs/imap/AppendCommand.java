      ...
      if (appends != null) {
        ...
        for (int i = 0; i < appends.size(); i++) {
          ...
          if ((myMsg.getDate() != null && !myMsg.getDate().equals(otherMsg.getDate()))
            ...
            return false;
            // date mismatch
          } else if ((myMsg.getPersistentFlagNames() != null
                  && !myMsg.getPersistentFlagNames().equals(otherMsg.getPersistentFlagNames()))
              || (myMsg.getPersistentFlagNames() == null
                  && otherMsg.getPersistentFlagNames() != null)) {
            return false;
            // flag name mismatch
          }
          List<Part> myParts = myMsg.getParts();
          List<Part> otherParts = otherMsg.getParts();
          if ((myParts == null && otherParts != null)
              || (myParts != null && otherParts == null)
          ...
          for (int j = 0; j < myParts.size(); j++) {
            ...
            if ((myPart.getLiteral() != null
                    && otherPart.getLiteral() != null
                    && myPart.getLiteral().size() != otherPart.getLiteral().size())
