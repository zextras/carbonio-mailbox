...
public final class ImapProxy {
  private static final Set<String> UNSTRUCTURED_CODES =
      ImmutableSet.of("OK", "NO", "BAD", "PREAUTH", "BYE");
  ...
  protected ImapProxy(final ImapHandler handler, final ImapPath path) throws ServiceException {
    ...
    if (server.isImapServerEnabled()) {
      ...
      config.setPort(
          server.getIntAttr(Provisioning.A_zimbraRemoteImapBindPort, ImapConfig.DEFAULT_PORT));
    } else if (server.isRemoteImapSSLServerEnabled()) {
      config.setPort(
          server.getIntAttr(
              Provisioning.A_zimbraRemoteImapSSLBindPort, ImapConfig.DEFAULT_SSL_PORT));
      config.setSecurity(MailConfig.Security.SSL);
    } else {
      throw ServiceException.PROXY_ERROR(
    ...
    ZimbraLog.imap.info(
        "opening proxy connection (user=%s, host=%s, path=%s)",
  ...
  /** For testing. */
  protected ImapProxy(
      InetSocketAddress remote, String username, String password, ImapHandler handler)
  ...
  /**
   * Performs a {@code SELECT} on the remote folder passed into the constructor. Writes all tagged
   * and untagged responses back to the handler's output stream.
   *
  ...
  protected boolean select(final String tag, final byte params, final QResyncInfo qri)
    ...
    if ((params & ImapFolder.SELECT_CONDSTORE) != 0) {
      ...
      if (qri == null) {
        ...
        if (qri.knownUIDs != null) select.append(' ').append(qri.knownUIDs);
        if (qri.seqMilestones != null)
          select
              .append(" (")
              .append(qri.seqMilestones)
              .append(' ')
              .append(qri.uidMilestones)
              .append(')');
        select.append(')');
      }
      select.append(')');
   ...
   */
  protected boolean idle(final ImapRequest req, final boolean begin)
  ...
  public static final class ZimbraClientAuthenticator extends Authenticator {
    ...
    public void init(MailConfig config, String password) {
      username = config.getAuthenticationId();
      authtoken = password;
    }
