...
public class ImapPath implements Comparable<ImapPath> {
  ...
  /**
   * Takes a user-supplied IMAP mailbox path and converts it to a Zimbra folder pathname. Applies
   * all special, hack-specific folder mappings. Does <b>not</b> do IMAP-UTF-7 decoding; this is
   * assumed to have been already done by the appropriate method in {@link ImapRequest}.
   *
  ...
  ImapPath(String imapPath, ImapCredentials creds, Scope scope) {
    ...
    Pair<String, String> homeOwnerAndPath =
        getHomeOwnerAndPath(
            imapPath,
    ...
    if (creds != null
        && creds.isHackEnabled(ImapCredentials.EnabledHack.WM5)
        && lcname.startsWith("sent items")
        && (lcname.length() == 10 || lcname.charAt(10) == '/')) {
  ...
  protected static ImapPath get(
      String owner, String zimbraPath, ImapCredentials creds, ImapMailboxStore imapMailboxStore)
  ...
  public boolean isEquivalent(ImapPath other) {
    if (!mPath.equalsIgnoreCase(other.mPath)) {
      return false;
    } else if (mOwner == other.mOwner
        || (mOwner != null && mOwner.equalsIgnoreCase(other.mOwner))) {
      return true;
    }
    try {
      Account acct = getOwnerAccount();
      Account otheracct = other.getOwnerAccount();
      return (acct == null || otheracct == null
          ? false
          : acct.getId().equalsIgnoreCase(otheracct.getId()));
  ...
  @Override
  public int hashCode() {
    return (mOwner == null ? 0 : mOwner.toUpperCase().hashCode())
        ^ mPath.toUpperCase().hashCode()
        ^ (mCredentials == null ? 0 : mCredentials.hashCode());
  }
  ...
  public ImapPath canonicalize() throws ServiceException {
    ...
    if (mReferent == this || excess == 0) {
      ...
      mPath =
          path
              + "/"
              + mReferent.canonicalize().mPath.substring(mReferent.mPath.length() - excess + 1);
    }
    return this;
  }
  ...
  protected boolean onLocalServer() throws ServiceException {
    ...
    Account acct = getOwnerAccount();
    return acct != null && Provisioning.getInstance().onLocalServer(acct);
  }
  ...
  protected ImapMailboxStore getOwnerImapMailboxStore(boolean forceRemote) throws ServiceException {
    ...
    if (forceRemote
        || (Provisioning.canUseLocalIMAP(target)
            && !Provisioning.getInstance().onLocalServer(target))) {
      imapMboxStore = getRemoteImapMailboxStoreForOwner();
    }
    if (imapMboxStore == null) {
      if (target == null) {
        imapMboxStore = null;
      } else if (Provisioning.getInstance().onLocalServer(target)) {
  ...
  protected boolean useReferent() throws ServiceException {
    ...
    // if we're here, we should be at NAME scope -- return whether the original path pointed at a
    // mountpoint
  ...
  /**
   * @return If the folder is a mountpoint (i.e. an accepted share), may return an ImapPath
   *     representing that, otherwise, the value is this.
   */
  @VisibleForTesting
  public ImapPath getReferent() throws ServiceException {
    ...
    if (folder == null) {
      try {
        ExistingParentFolderStoreAndUnmatchedPart info =
            ownerImapMailboxStore
                .getMailboxStore()
                .getParentFolderStoreAndUnmatchedPart(getContext(), asZimbraPath());
        subpathRemote = info.unmatchedPart;
        if (info.parentFolderStore instanceof MountpointStore
            || Strings.isNullOrEmpty(subpathRemote)) {
          folder = info.parentFolderStore;
          mItemId =
              new ItemId(ItemIdentifier.fromOwnerAndFolder(accountIdFromCredentials(), folder));
        }
        if (!(info.parentFolderStore instanceof MountpointStore)) {
          return mReferent;
        }
    ...
    FolderStore fldr =
        imapMailboxStore
            .getMailboxStore()
            .getFolderById(getContext(), Integer.toString(iidRemote.getId()));
    if (fldr == null) {
      return mReferent;
    }
    ...
    if (Strings.isNullOrEmpty(subpathRemote)) {
      ...
      mReferent =
          ImapPath.get(
              owner,
              fldr.getPath() + (fldr.getPath().equals("/") ? "" : "/") + subpathRemote,
              mCredentials,
              imapMailboxStore);
    }
  ...
  private String getOwner(Account target) {
    return mCredentials != null && mCredentials.getAccountId().equalsIgnoreCase(target.getId())
        ? null
        : target.getName();
  }
  ...
  private ImapMailboxStore setupMailboxStoreForTarget(Account target, ItemId iidRemote)
    ...
    if (Provisioning.getInstance().onLocalServer(target) && onLocalServer()) {
      try {
  ...
  /**
   * Returns whether the server can return the <tt>READ-WRITE</tt> response code when the folder
   * referenced by this path is <tt>SELECT</tt>ed.
   */
  protected boolean isWritable() throws ServiceException {
    ...
    return isWritable(ACL.RIGHT_DELETE)
        || isWritable(ACL.RIGHT_INSERT)
        || isWritable(ACL.RIGHT_WRITE);
  }

  /**
   * Returns <tt>true</tt> if all of the specified rights have been granted on the folder referenced
   * by this path to the authenticated user.
   */
  ...
  protected boolean isSelectable() throws ServiceException {
    ...
    if (imapFolderStore == null
        || !isVisible()
        || imapFolderStore.isUserRootFolder()
        || imapFolderStore.isIMAPDeleted()) {
      return false;
    }
  ...
  protected boolean isVisible() throws ServiceException {
    ...
    /**
     * "folder" CAN be null when this is called - relying on getFolder() to fill in the details
     * later e.g. in the case of ". RENAME nonExistentA nonExistentB"
     */
  ...
  protected boolean isValidImapPath() throws ServiceException {
    ...
    try {
      ...
      if (folder != null) {
        ...
        if (asItemId().getId() == Mailbox.ID_FOLDER_SPAM
            && !getOwnerAccount().isFeatureAntispamEnabled()) {
          return false;
        }
        boolean isMailFolders =
            Provisioning.getInstance().getLocalServer().isImapDisplayMailFoldersOnly();
      ...
      if (ServiceException.PERM_DENIED.equals(se.getCode())) {
        // Path probably OK.  For subscriptions, don't disallow path for possibly temporary
        // permissions issue
        return true;
      }
      throw se;
   ...
   * @return If imapPath represents a path in the /home namespace, then return the owner and path
   *     sub-components otherwise return null
   */
  protected static Pair<String, String> getHomeOwnerAndPath(
      String imapPath, boolean allowWildcardOwner) {
    String owner = null;
    String path = null;
  ...
  protected String asImapPath() {
    ...
    if (lcpath.startsWith("inbox") && (lcpath.length() == 5 || lcpath.charAt(5) == '/')) {
      path = "INBOX" + path.substring(5);
        ...
        && mCredentials.isHackEnabled(ImapCredentials.EnabledHack.WM5)
        && lcpath.startsWith("sent")
        && (lcpath.length() == 4 || lcpath.charAt(4) == '/')) {
  ...
  /**
   * Formats a folder path as an IMAP-UTF-7 quoted-string. Applies all special hack-specific path
   * transforms.
   */
