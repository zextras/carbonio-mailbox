...
public class ImapMessage implements Comparable<ImapMessage>, java.io.Serializable {
  ...
  public static final Set<MailItem.Type> SUPPORTED_TYPES =
      EnumSet.of(MailItem.Type.MESSAGE, MailItem.Type.CHAT, MailItem.Type.CONTACT);

  static final int IMAP_FLAGS =
      Flag.BITMASK_UNREAD
          | Flag.BITMASK_FLAGGED
          | Flag.BITMASK_DELETED
          | Flag.BITMASK_DRAFT
          | Flag.BITMASK_REPLIED
          | Flag.BITMASK_FORWARDED
  ...
  static final short SESSION_FLAGS =
      FLAG_ADDED | FLAG_EXPUNGED | FLAG_IS_CONTACT | FLAG_RECENT | MUTABLE_SESSION_FLAGS;
  ...
  ImapMessage(ImapMessageInfo msgInfo) {
    ...
    this.sflags =
        msgInfo.getType().equalsIgnoreCase(MailItem.Type.CONTACT.name()) ? FLAG_IS_CONTACT : 0;
  }
  ...
  static InputStreamWithSize getContent(ZimbraMailItem item) throws ServiceException {
    if (item instanceof Message) {
      ...
      try {
        ...
        header
            .append("Subject: ")
            .append(qcodec.encode(fn, MimeConstants.P_CHARSET_UTF8))
            .append(ImapHandler.LINE_SEPARATOR);
        synchronized (GMT_DATE_FORMAT) {
          header
              .append("Date: ")
              .append(GMT_DATE_FORMAT.format(new Date(item.getDate())))
              .append(ImapHandler.LINE_SEPARATOR);
        }
        header
            .append("Content-Type: text/x-vcard; charset=\"utf-8\"")
            .append(ImapHandler.LINE_SEPARATOR);
        ...
        if (item instanceof Contact) {
          ...
          ZimbraLog.test.debug(
              "zcontact vcard: %s", IOUtils.toString(((ZContact) item).getContentStream()));
        }

        return new InputStreamWithSize(
            new SharedByteArrayInputStream(baos.toByteArray()), (long) baos.size());
      } catch (Exception e) {
  ...
  static MimeMessage getMimeMessage(ZimbraMailItem item) throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          String.format(
              "error creating MimeMessage for %s %s",
              ((MailItem) item).getType(), item.getIdInMailbox()),
          e);
    } finally {
  ...
  void setPermanentFlags(int f, String[] t, int changeId, ImapFolder parent) {
    if (TagUtil.tagsMatch(t, tags) && (f & IMAP_FLAGS) == (flags & IMAP_FLAGS)) return;
  ...
  void setSessionFlags(short s, ImapFolder parent) {
    if ((s & MUTABLE_SESSION_FLAGS) == (sflags & MUTABLE_SESSION_FLAGS)) return;
  ...
  private static void astring(PrintStream ps, String value, boolean upcase) {
    ...
    String content =
        nonulls == null ? value : nonulls.append(value.substring(lastNull + 1, i)).toString();
    if (upcase) {
  ...
  private static void nparams(PrintStream ps, MimeCompoundHeader header) {
    boolean first = true;
    for (Iterator<Map.Entry<String, String>> it = header.parameterIterator();
        it.hasNext();
        first = false) {
      Map.Entry<String, String> param = it.next();
      ps.print(first ? '(' : ' ');
      aSTRING(ps, param.getKey());
      ps.write(' ');
  ...
  static void serializeStructure(PrintStream ps, MimeMessage root, boolean extensions)
    ...
    while (!queue.isEmpty()) {
      ...
      // we used to force unset charsets on text/plain parts to US-ASCII, but that always seemed
      // unwise...
      ContentType ctype =
          new ContentType(mp.getHeader("Content-Type", null)).setContentType(mpi.getContentType());
  ...
  public String toString() {
    return MoreObjects.toStringHelper("").add("m", msgId).add("u", imapUid).toString();
  }
