...
public abstract class ImapRequest {
  ...
  static {
    for (int i = 0x21; i < 0x7F; i++) {
      if (i != '(' && i != ')' && i != '{' && i != '%' && i != '*' && i != '"' && i != '\\') {
        SEARCH_CHARS[i] =
            FETCH_CHARS[i] =
                PATTERN_CHARS[i] = ASTRING_CHARS[i] = ATOM_CHARS[i] = TAG_CHARS[i] = true;
      }
    }
    ATOM_CHARS[']'] = false;
    TAG_CHARS['+'] = false;
  ...
  private static final Set<String> SYSTEM_FLAGS =
      ImmutableSet.of("ANSWERED", "FLAGGED", "DELETED", "SEEN", "DRAFT");
  ...
  /**
   * Records the "tag" for the request. This tag will later be used to indicate that the server has
   * finished processing the request. It may also be used when generating a parse exception.
   */
  ...
  /**
   * Returns the character at the read position, or -1 if we're at the end of a literal or of a
   * line.
   */
  ...
  protected void skipChar(char c) throws ImapParseException {
    ...
    if (got == c) {
      ...
      throw new ImapParseException(
          tag, "wrong character; expected '" + c + "' but got '" + got + "'");
    }
  }
  ...
  protected String readQuoted(Charset charset) throws ImapParseException {
    ...
    if (charset == null
        || Charsets.ISO_8859_1.equals(charset)
        || Charsets.US_ASCII.equals(charset)) {
      return result;
    } else {
  ...
  private String readAstring(Charset charset, boolean[] acceptable)
    ...
    if (peekChar() != ')') {
      do {
        try {
          ...
          skipSpace();
          CacheEntrySelector cacheEntry =
              new CacheEntrySelector(cacheBy, readAstring(Charsets.UTF_8));
  ...
  private String readFolder(boolean isPattern) throws IOException, ImapParseException {
    ...
    if (raw == null || raw.indexOf("&") == -1) return raw;
    try {
      return ImapPath.FOLDER_ENCODING_CHARSET
          .decode(ByteBuffer.wrap(raw.getBytes(Charsets.US_ASCII)))
          .toString();
    } catch (Exception e) {
  ...
  private int validateDigits(String str, int pos, int count, Calendar cal, int field)
  ...
  protected int readFetch(List<ImapPartSpecifier> parts) throws IOException, ImapParseException {
    ...
    do {
      ...
      } else if (item.equals("BODY")
          || item.equals("BODY.PEEK")
          || ((item.equals("BINARY") || item.equals("BINARY.PEEK"))
              && extensionEnabled("BINARY"))) {
  ...
  protected ImapPartSpecifier readPartSpecifier(boolean binary, boolean literals)
    ...
    ImapPartSpecifier pspec =
        new ImapPartSpecifier(binary ? "BINARY" : "BODY", sectionPart, sectionText);
    pspec.setHeaders(headers);
    return pspec;
  }

  private ImapSearch readSearchClause(
      Charset charset, boolean single, LogicalOperation parent, int depth)
      throws IOException, ImapParseException {
    depth++;
    if (depth > maxNestingInSearchRequest) {
      ZimbraLog.imap.debug(
          "search nesting too deep (depth=%s) Max allowed=%s", depth, maxNestingInSearchRequest);
    ...
    do {
      ...
      if (key.equals("ALL")) {
        ...
        child =
            new AndOperation(
                new FlagSearch("\\Recent"), new NotOperation(new FlagSearch("\\Seen")));
        ...
        skipSpace();
        child = new HeaderSearch(HeaderSearch.Header.BCC, readAstring(charset));
  ...
  protected Charset readCharset() throws IOException, ImapParseException {
    ...
    throw new ImapParseException(
        tag,
        "BADCHARSET",
        "unknown charset: " + charset.replace('\r', ' ').replace('\n', ' '),
        true);
  }
}
