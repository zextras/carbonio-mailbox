...
final class AppendMessage {
  ...
  private List<String>
      persistentFlagNames; // flagNames is nulled out in checkFlags; this holds the original
  // constructed list of names
  ...
  private int store(ImapMailboxStore mboxStore, FolderStore folderStore)
    ...
    if (mboxStore instanceof LocalImapMailboxStore) {
      ...
      Message msg =
          ((LocalImapMailboxStore) mboxStore)
              .getMailbox()
              .addMessage(handler.getContext(), pm, dopt, null);
    ...
    if (mboxStore instanceof RemoteImapMailboxStore) {
      ...
      try (InputStream is = content.getInputStream()) {
        TagSpecifier tagSpec =
            tags.isEmpty() ? null : TagSpecifier.tagByName(Joiner.on(",").join(tags));
        long receivedDate = date != null ? date.getTime() : 0;
        id =
            ((RemoteImapMailboxStore) mboxStore)
                .getZMailbox()
                .addMessage(
                    folderStore.getFolderIdAsString(),
                    Flag.toString(flags),
                    tagSpec,
                    receivedDate,
                    is,
  ...
  protected void checkContent() throws IOException, ImapException, ServiceException {
    ...
    if ((maxMsgSize != 0 /* 0 means unlimited */)
        && (size > handler.getConfig().getMaxMessageSize())) {
      cleanup();
      if (catenate) {
