...
final class ImapURL {
  ...
  ImapURL(String tag, ImapHandler handler, String url) throws ImapParseException {
    ...
    if (mPath == null || mPath.asZimbraPath().length() == 0) {
      if (handler.getState() != ImapHandler.State.SELECTED)
        throw new ImapUrlException(
            tag, url, "IMAP URL must specify folder if session not SELECTED");
    ...
    if (mHostname == null || mHostname.length() == 0) {
      try {
        ...
        if (acct == null) throw new ImapUrlException(tag, url, "unknown user: " + mUsername);
  ...
  private void parse(String tag, ImapCredentials creds, String url) throws ImapParseException {
    ...
    if (url.length() > 7 && lcurl.startsWith("imap://")) {
      ...
      if (ampersand != -1 && ampersand < slash) {
        ...
        mUsername =
            urlDecode(
                url.substring(
                    pos, semicolon != -1 && semicolon < ampersand ? semicolon : ampersand));
    ...
    mPath =
        new ImapPath(urlDecode(url.substring(pos, uvv != -1 && uvv < iuid ? uvv : iuid)), creds);
    pos = iuid + 6;
  ...
  public InputStreamWithSize getContentAsStream(
      ImapHandler handler, ImapCredentials creds, String tag) throws ImapException {
    ...
    try {
      ...
      if (content == null) {
        ...
        content =
            mbox.getByImapId(
                octxt, mUid, mPath.getFolder().getFolderIdAsString(), mPath.asResolvedPath());
        if (null == content) {
          throw new ImapUrlException(tag, mURL, "no such message");
        }
      }
  ...
  public String toString() {
    try {
      return "imap://"
          ...
          + (mPort > 0 ? ":" + mPort : "")
          + '/'
          + URLEncoder.encode(mPath.asImapPath(), "utf-8")
          + "/;UID="
          + mUid
          + (mPart != null
              ? "/;SECTION=" + URLEncoder.encode(mPart.getSectionSpec(), "utf-8")
              : "");
      ...
      return "imap://"
          ...
          + "/;UID="
          + mUid
          + (mPart != null ? "/;SECTION=" + mPart.getSectionSpec() : "");
    }
  }
  ...
  public static void main(String[] args) throws ImapParseException, ServiceException, IOException {
    ...
    System.out.println(
        new ImapURL("tag", handler, "/Drafts;UIDVALIDITY=385759045/;UID=20/;section=HEADER"));
    System.out.println(new ImapURL("tag", handler, "/;UID=20/;section=1.mime"));
    System.out.println(
        new ImapURL("tag", handler, "/INBOX;UIDVALIDITY=785799047/;UID=113330/;section=1.5.9"));
    System.out.println(
        new ImapURL(
            "tag",
            handler,
            "imap://minbari.org:7143/\\\"gray-council\\\";UIDVALIDITY=385759045/;UID=20"));
    System.out.println(
        new ImapURL(
            "tag",
            handler,
            "imap://bester;auth=gssapi@psicorp.org/~peter/%E6%97%A5%E6%9C%AC%E8%AA%9E/%E5%8F%B0%E5%8C%97/;UID=11916"));
    System.out.println(
        new ImapURL("tag", handler, "imap://;AUTH=*@minbari.org/gray-council/;uid=20/;section="));
