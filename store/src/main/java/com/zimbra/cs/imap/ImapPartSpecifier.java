...
class ImapPartSpecifier {
  ...
  /**
   * Maximum size of byte[] buffer held in memory while processing a <tt>FETCH BINARY</tt> or
   * <tt>FETCH BINARY.SIZE</tt>.
   */
  ...
  private InputStreamWithSize getContentOctetRangeFromFullContents(InputStreamWithSize contents)
    ...
    if (octetStart >= 0 && contents != null) {
      ...
      long realLength =
          Math.max(
              0,
              Math.min(statedLength < 0 ? Integer.MAX_VALUE : statedLength, octetEnd) - octetStart);
      try {
        int start = octetStart;
        // the JavaMail implementations of the content-transfer decoders
        // don't do skip() correctly
        if (is instanceof com.sun.mail.util.BASE64DecoderStream
            || is instanceof com.sun.mail.util.QPDecoderStream) {
  ...
  private InputStreamWithSize getContent(MimeMessage msg) throws BinaryDecodingException {
    ...
    try {
      ...
      if ((modifier.equals("TEXT") || modifier.startsWith("HEADER"))
          && !(mp instanceof MimeMessage)) {
  ...
  public boolean isIgnoredExchangeHeader() {
    return requestedHeaders != null
        && requestedHeaders.size() == 1
        && StringUtil.equalIgnoreCase(requestedHeaders.get(0), "CONTENT-CLASS")
        && StringUtil.equalIgnoreCase("BODY", command)
        && StringUtil.equalIgnoreCase("HEADER.FIELDS", modifier);
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (requestedHeaders == null) {
      ...
      if (other.requestedHeaders == null
          || requestedHeaders.size() != other.requestedHeaders.size()) {
        return false;
      } else {
