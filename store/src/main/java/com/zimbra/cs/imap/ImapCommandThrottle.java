...
public class ImapCommandThrottle {
  ...
  private static final Map<String, ReentrantLock> commandLock =
      MapUtil.newTimeoutMap(1 * Constants.MILLIS_PER_HOUR);
  ...
  /**
   * Obtain a lock for serialization of expensive commands such as FETCH. Each authenticated account
   * may only perform one expensive command at a time regardless of number of sessions/connections
   *
   * @param accountId - the authenticated account ID
   * @return ReentrantLock instance. Caller *must* unlock it when finished with the expensive
   *     operation
  ...
  public ReentrantLock lock(String accountId) throws ImapThrottledException {
    ...
    if (!locked) {
      throw new ImapThrottledException(
          "Unable to obtain command lock " + lock.toString() + " aborting operation");
    } else {
      return lock;
    }
  }
