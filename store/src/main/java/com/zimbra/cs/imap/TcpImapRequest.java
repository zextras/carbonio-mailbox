...
final class TcpImapRequest extends ImapRequest {
  ...
  private void checkSize(long size) throws ImapParseException {
    ...
    if (isAppend()) {
      try {
        ...
        if ((msgLimit != 0 /* 0 means unlimited */)
            && (msgLimit < maxLiteralSize)
            && (size > msgLimit)) {
  ...
  protected void continuation() throws IOException, ImapParseException {
    ...
    if (line == null) throw new ImapTerminatedException();
  ...
  private void continueLiteral() throws IOException, ImapParseException {
    if (maxRequestSizeExceeded) {
      ...
      if (read == -1) throw new ImapTerminatedException();
  ...
  /**
   * This implementation doesn't add any more parts if we have exceeded the maximum request size.
   * The exception is if this is the first part (request line) so we can recover the tag when
