...
public class RemoteImapMailboxStore extends ImapMailboxStore {
  ...
  @Override
  public ImapListener createListener(ImapFolder i4folder, ImapHandler handler)
  ...
  public List<ImapListener> getListeners(ItemIdentifier ident) {
    ...
    try {
      ...
      ZimbraLog.imap.debug(
          "Problem getting listeners for folder=%s acct=%s from ImapServerListener",
          ident, acctId, se);
   ...
   */
  @Override
  public List<Integer> imapCopy(
      OperationContext octxt, int[] itemIds, MailItemType type, int folderId)
      throws IOException, ServiceException {
    return zMailbox.imapCopy(itemIds, type, folderId);
  }

  @Override
  public InputStreamWithSize getByImapId(
      OperationContext octxt, int imapId, String folderId, String resolvedPath)
      throws ServiceException {
    AuthToken auth;
    try {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          String.format("Failed to get content stream for item id %d", imapId), e);
    }
  }

  @Override
  public void checkAppendMessageFlags(OperationContext octxt, List<AppendMessage> appends)
      throws ServiceException {
  ...
  @Override
  public Collection<FolderStore> getVisibleFolders(
      OperationContext octxt, ImapCredentials credentials, String owner, ImapPath relativeTo)
      throws ServiceException {
    ...
    for (ZFolder zfolder : zfolders) {
      ...
      path =
          (relativeTo == null)
              ? new ImapPath(owner, zfolder, credentials)
  ...
  @Override
  public List<ImapMessage> openImapFolder(OperationContext octxt, ItemIdentifier folderId)
      throws ServiceException {
    List<ImapMessage> msgs = new ArrayList<ImapMessage>();
    Integer chunkSize =
        Provisioning.getInstance().getLocalServer().getOpenImapFolderRequestChunkSize();
