...
/**
 * Protocol Decoder for IMAP. This decodes a text line terminated by LF or CRLF into a string, and
 * an IMAP literal into a byte array.
 *
 * @author ysasaki
 */
final class NioImapDecoder extends CumulativeProtocolDecoder {
  ...
  protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out)
    ...
    /**
     * the allowed maximum size of a literal to be decoded. If the size of the literal to be decoded
     * exceeds this value, the decoder will throw a {@link TooBigLiteralException}.
     */
    long maxLiteralSize = config.getMaxMessageSize();
    int maxChunkSize = config.getWriteChunkSize();
    /**
     * the allowed maximum size of a line to be decoded. If the size of the line to be decoded
     * exceeds this value, the decoder will throw a {@link TooLongLineException}.
     */
      ...
      if (ctx.literal >= 0) {
        ...
        if (ctx.overflow) {
          ...
          if (b == '\n') {
            ...
            // The bytes between in.position() and in.limit() now contain a full CRLF terminated
            // line.
            String line = in.getString(Charsets.ISO_8859_1.newDecoder());
            // Set the position to point right after the detected line and set the limit to the old
            // one.
            in.limit(limit);
            in.position(pos);
            LiteralInfo li;
