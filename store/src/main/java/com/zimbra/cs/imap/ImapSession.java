...
public class ImapSession extends ImapListener {
  ...
  protected class PagedLocalFolderData extends ImapListener.PagedFolderData {
    ...
    protected synchronized void queueModify(int changeId, Change chg) {
      getQueuedLocalNotifications(changeId)
          .recordModified((MailItem) chg.what, chg.why, (MailItem) chg.preModifyObj);
    }
  }

  ImapSession(ImapMailboxStore imapStore, ImapFolder i4folder, ImapHandler handler)
  ...
  /**
   * If the folder is selected READ-WRITE, updates its high-water RECENT change ID so that
   * subsequent IMAP sessions do not see the loaded messages as \Recent.
   */
  @Override
    ...
    try {
      ...
      if (ZimbraLog.session.isDebugEnabled()) {
        ZimbraLog.session.info(
            "Mailbox in maintenance detected recording unloaded session's RECENT limit %s",
            this, miMe);
      } else {
        ZimbraLog.session.info(
  ...
  private void handleCreate(int changeId, BaseItemInfo item, AddedItems added)
    ...
    if (item == null || item.getIdInMailbox() <= 0) {
      return;
        ...
        && (item instanceof Message || item instanceof Contact)) {
