...
public class ImapFlagCache implements Iterable<ImapFlagCache.ImapFlag>, java.io.Serializable {
  ...
  public static final class ImapFlag implements java.io.Serializable {
    ...
    ImapFlag(String name, ZimbraTag ztag, boolean positive) {
      mId = Integer.valueOf(ztag.getTagId());
      mBitmask = 0;
      mName = ztag.getTagName();
      mImapName = normalize(name, mId);
      mPositive = positive;
      mPermanent = true;
      mListed = VISIBLE;
    }
    ...
    ImapFlag(String name, ZTag ztag, boolean positive) {
      mId = Integer.valueOf(ztag.getId());
      mBitmask = 0;
      mName = ztag.getName();
      mImapName = normalize(name, mId);
      mPositive = positive;
    ...
    ImapFlag(String name, Tag ltag, boolean positive) {
      mId = ltag.getId();
      mBitmask = ltag instanceof Flag ? ((Flag) ltag).toBitmask() : 0;
      mName = ltag.getName();
      mImapName = normalize(name, mId);
      mPositive = positive;
      mPermanent = true;
    ...
    ImapFlag(String name, FlagInfo flagInfo, boolean positive) {
      mId = flagInfo.toId();
      mBitmask = flagInfo.toBitmask();
      mName = flagInfo.toString();
      mImapName = normalize(name, mId);
      mPositive = positive;
      mPermanent = true;
      mListed = VISIBLE;
