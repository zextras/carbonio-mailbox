...
public class LocalImapMailboxStore extends ImapMailboxStore {
  ...
  public void saveSubscriptions(OperationContext octxt, Set<String> subs) throws ServiceException {
    ...
    mailbox.setConfig(
        octxt, AccountUtil.SN_IMAP, new Metadata().put(AccountUtil.FN_SUBSCRIPTIONS, slist));
  }
  ...
  public List<ImapListener> getListeners(ItemIdentifier ident) {
    ...
    if (ident == null) {
      ZimbraLog.imap.warnQuietlyFmt(
          "Attempted to getListeners for null item ID on mailbox %s", this);
      return listeners;
    }
  ...
  public int getCurrentMODSEQ(ItemIdentifier folderId) throws ServiceException {
    if ((folderId.accountId != null) && !folderId.accountId.equals(mailbox.getAccountId())) {
      ZimbraLog.imap.debug(
          "Unexpected call 'getCurrentMODSEQ(%s)' when local mailbox is %s", folderId, mailbox);
    }
    ...
    if (folders == null) {
      folders =
          mailbox
              .getFolderById(
                  octxt,
                  relativeTo == null ? Mailbox.ID_FOLDER_USER_ROOT : relativeTo.asItemId().getId())
              .getSubfolderHierarchy();
    }
