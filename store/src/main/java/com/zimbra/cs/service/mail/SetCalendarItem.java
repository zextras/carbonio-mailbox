...
public class SetCalendarItem extends CalendarRequest {
  ...
  static class SetCalendarItemInviteParser extends ParseMimeMessage.InviteParser {
    ...
    SetCalendarItemInviteParser(
        boolean exceptOk, boolean forCancel, Folder folder, MailItem.Type type) {
      mExceptOk = exceptOk;
  ...
  static SetCalendarItemData getSetCalendarItemData(
    ...
    if (inv == null || inv.getDTStamp() == -1) { // zdsync if -1 for 4.5 back compat
      ...
      Invite iCalInv =
          Invite.createFromCalendar(acct, pm.getFragment(acct.getLocale()), cal, sentByMe).get(0);
