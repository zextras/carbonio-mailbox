  ...
  public void formatCallback(UserServletContext context)
    ...
    if (rangeEnd < rangeStart)
      throw new UserServletException(
          HttpServletResponse.SC_BAD_REQUEST, "End time must be after Start time");
    ...
    if (days > maxDays)
      throw new UserServletException(
          ...
          "Requested range is too large (Maximum " + maxDays + " days)");
    ...
    if (context.targetMailbox != null) {
      ...
      fb =
          context.targetMailbox.getFreeBusy(
              context.opContext,
              acctName,
              rangeStart,
              rangeEnd,
              context.getFreeBusyCalendar(),
              exAppt);
    } else {
    ...
    if (index == -1) {
      ...
      domain =
          Provisioning.getInstance()
              .getConfig()
              .getAttr(Provisioning.A_zimbraDefaultDomainName, null);
      if (domain != null) emailAddress = emailAddress + "@" + domain;
