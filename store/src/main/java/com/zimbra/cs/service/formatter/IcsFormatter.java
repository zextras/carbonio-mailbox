...
public class IcsFormatter extends Formatter {
  ...
  public void formatCallback(UserServletContext context) throws IOException, ServiceException {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (item instanceof CalendarItem) {
          ...
          if (hasTimeRange) {
            ...
            if (!instances.isEmpty()) calItems.add(calItem);
          } else {
      ...
      if (iterator instanceof QueryResultIterator) ((QueryResultIterator) iterator).finished();
    }
    ...
    if (mayAttach(context)) {
      if (filename == null || filename.length() == 0) filename = "contacts";

      String requestFilename =
          context.req.getParameter("filename"); // Let the client specify the filename to save as
      if (requestFilename != null) filename = requestFilename;
      else
        filename =
            filename.replaceAll(
                "^\\W", ""); // Trim off leading non-word characters (e.g. forward slash)

      if (filename.toLowerCase().endsWith(".ics") == false) filename = filename + ".ics";
    ...
    boolean htmlFormat =
        !mayAttach(context)
            && Browser.IE.equals(
                browser); // Use only htmlFormat when the file isn't supposed to be downloaded (ie.
    // it's supposed to be shown in the browser). Mangles the code so it can
    // be displayed correctly, especially by IE
    ...
    FileBufferedWriter fileBufferedWriter =
        new FileBufferedWriter(
            context.resp.getWriter(),
            LC.calendar_ics_export_buffer_size.intValueWithinRange(
                0, FileBufferedWriter.MAX_BUFFER_SIZE));
    try {
      if (htmlFormat) fileBufferedWriter.write("<html><body><pre>");
      context.targetMailbox.writeICalendarForCalendarItems(
          fileBufferedWriter,
          octxt,
          calItems,
          (context.target != null && context.target instanceof Folder)
          ...
          true,
          htmlFormat,
          includeInlineAttaches(context));
      if (htmlFormat) fileBufferedWriter.write("</pre></body></html>");
    } finally {
  ...
  public void saveCallback(
    ...
    try {
      if (context.req.getContentLength() <= LC.calendar_ics_import_full_parse_max_size.intValue()) {
        ...
        // and add them one by one.  Memory hungry if there are very many events/tasks, but it
        // allows
        ...
        ImportInviteVisitor visitor =
            new ImportInviteVisitor(context.opContext, folder, preserveExistingAlarms);
