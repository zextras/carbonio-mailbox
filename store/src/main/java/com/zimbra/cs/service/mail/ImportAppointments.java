...
public class ImportAppointments extends MailDocumentHandler {
  ...
  /**
   * Return false - need to mine "content" for pointers to data in the owner mailbox and resolve
   * these before proxying.
   */
  @Override
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    try {
      ...
      if (iidTarget != null) {
        ZimbraLog.misc.info(
            "Proxying ImportAppointments - folder="
                + folder
                + ", acct="
        ...
        if (SourceSpecMethod.MSG_PART.equals(sourceSpecMethod)) {
          ...
          Upload up =
              FileUploadServlet.saveUpload(
                  is, "upload.ics", MimeConstants.CT_TEXT_CALENDAR, zsc.getAuthtokenAccountId());
      ...
      List<Invite> invites =
          Invite.createFromCalendar(mbox.getAccount(), null, icals, true, true, null);
      ...
      for (Invite inv : invites) {
        ...
        try {
          ...
          ZimbraLog.calendar.warn(
              "Skipping bad iCalendar object during import: uid=" + inv.getUid(), e);
        }
      }
      ...
      if (attachment != null) FileUploadServlet.deleteUploads(uploads);
    }
  }

  private static InputStream parseUploadedContent(
      ZimbraSoapContext lc, String attachId, List<Upload> uploads) throws ServiceException {
    Upload up =
        FileUploadServlet.fetchUpload(lc.getAuthtokenAccountId(), attachId, lc.getAuthToken());
    if (up == null) throw MailServiceException.NO_SUCH_UPLOAD(attachId);
    uploads.add(up);
    try {
