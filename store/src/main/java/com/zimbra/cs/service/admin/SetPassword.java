...
public class SetPassword extends AdminDocumentHandler {
    ...
    if (account == null) {
      try {
        defendAgainstAccountOrCalendarResourceHarvestingWhenAbsent(
            AccountBy.id, id, zsc, Admin.R_setAccountPassword, Admin.R_setCalendarResourcePassword);
      } catch (ServiceException se1) {
        defendAgainstAccountOrCalendarResourceHarvestingWhenAbsent(
            AccountBy.id,
            id,
            zsc,
            Admin.R_changeAccountPassword,
            Admin.R_changeCalendarResourcePassword);
      }
    } else {
      try {
        defendAgainstAccountOrCalendarResourceHarvesting(
            ...
            id,
            zsc,
            Admin.R_setAccountPassword,
            Admin.R_setCalendarResourcePassword);
      } catch (ServiceException se1) {
        defendAgainstAccountOrCalendarResourceHarvesting(
            account,
            AccountBy.id,
      ...
      ZimbraLog.security.info(
          ZimbraLog.encodeAttrs(new String[] {"cmd", "SetPassword", "name", account.getName()}));
   ...
   */
  private boolean checkAccountRights(ZimbraSoapContext zsc, Account acct) throws ServiceException {
    try {
