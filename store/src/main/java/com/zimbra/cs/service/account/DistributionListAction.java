...
public class DistributionListAction extends DistributionListDocumentHandler {
  ...
  private static class SubscriptionResponseSender extends NotificationSender {
    ...
    private SubscriptionResponseSender(
        Provisioning prov,
        Group group,
        Account ownerAcct,
    ...
    private void sendMessage() throws ServiceException {
      try {
        ...
        if (bccOwners) {
          ...
          if (!addrs.isEmpty()) {
            out.addRecipients(
                javax.mail.Message.RecipientType.BCC, addrs.toArray(new Address[addrs.size()]));
          }
        }
        ...
        ZimbraLog.account.warn(
            "send share info notification failed, rcpt='" + requestingAcct.getName() + "'", e);
      }
    }
  ...
  private static class AcceptSubsReqHandler extends DLActionHandler {
    ...
    protected AcceptSubsReqHandler(
        Element eAction, Group group, Provisioning prov, Account authedAcct) {
      super(eAction, group, prov, authedAcct);
    }
    ...
    void handle() throws ServiceException {
      ...
      DistributionListSubscribeOp subsOp =
          DistributionListSubscribeOp.fromString(eSubsReq.getAttribute(AccountConstants.A_OP));
      ...
      if (isMember) {
        if (subsOp == DistributionListSubscribeOp.subscribe) {
          ...
          ZimbraLog.account.debug(
              "AcceptSubsReqHandler: "
                  + memberEmail
                  + " is currently a member in list "
                  + group.getName()
        ...
        if (subsOp == DistributionListSubscribeOp.subscribe) {
          ...
          ZimbraLog.account.debug(
              "AcceptSubsReqHandler: "
                  + memberEmail
                  + " is currently not a member in list "
                  + group.getName()
      ...
      if (processed) {
        ...
        SubscriptionResponseSender notifSender =
            new SubscriptionResponseSender(
                prov, group, authedAcct, memberAcct, subsOp, bccOwners, true);
  ...
  private static class RejectSubsReqHandler extends DLActionHandler {
    ...
    protected RejectSubsReqHandler(
        Element eAction, Group group, Provisioning prov, Account authedAcct) {
      super(eAction, group, prov, authedAcct);
    }
    ...
    void handle() throws ServiceException {
      ...
      if (processed) {
        ...
        SubscriptionResponseSender notifSender =
            new SubscriptionResponseSender(
                prov, group, authedAcct, memberAcct, subsOp, bccOwners, false);
        notifSender.sendMessage();
      }
    }
  }
