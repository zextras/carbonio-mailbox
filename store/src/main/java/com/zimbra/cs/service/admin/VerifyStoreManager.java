...
/**
 * Utility for verifying StoreManager functionality. Intended to be available for 3rd party
 * integrators Does not (and must never) depend on Junit since we do not ship that with the product
 */
public class VerifyStoreManager extends AdminDocumentHandler {
  ...
  private void testStore() throws Exception {
    ...
    Mailbox mbox =
        MailboxManager.getInstance().getMailboxByAccount(AccountTestUtil.getAccount(USER_NAME));
    ...
    assertTrue(
        "blob content = mime content",
        bytesEqual(mimeBytes, readInputStream(blob.getInputStream())));
    ...
    assertTrue(
        ...
        bytesEqual(mimeBytes, readInputStream(mblob.getLocalBlob().getInputStream())));
  ...
  private void checkBlob(
    ...
    assertTrue(
        "link content = mime content",
        bytesEqual(mimeBytes, readInputStream(linked.getLocalBlob().getInputStream())));

    assertEquals("mblob size = staged size", staged.getSize(), fetched.getSize());
    assertTrue(
        "mailboxblob content = mime content",
        bytesEqual(mimeBytes, readInputStream(fetched.getLocalBlob().getInputStream())));
  }
  ...
  private boolean createAccountIfNeeded() throws ServiceException {
    ...
    Provisioning.getInstance()
        .createAccount(AccountTestUtil.getAddress(USER_NAME), "test123", null);
    return true;
  }
  ...
  @Override
  public synchronized Element handle(Element request, Map<String, Object> context)
    ...
    try {
      ...
      try {
        ...
        Stats stats =
            basicPerfTest(
                request.getAttributeInt(AdminConstants.A_NUM, 1000),
                blobSize,
                request.getAttributeBool(AdminConstants.A_CHECK_BLOBS, true));
        response
            .addAttribute("storeManagerClass", StoreManager.getInstance().getClass().getName())
            .addAttribute("incomingTime", stats.incomingTime)
            .addAttribute("stageTime", stats.stageTime)
            .addAttribute("linkTime", stats.linkTime)
            .addAttribute("fetchTime", stats.fetchTime)
        ...
        throw ServiceException.FAILURE(
            "OOME during perf test, with blob size "
                + blobSize
                + ". May need to use a smaller blob size or increase heap space",
            oome);
      }
    } finally {
  ...
  private class Stats {
    ...
    public String toString() {
      return MoreObjects.toStringHelper(this)
          .add("num blobs", numBlobs)
          .add("storeIncoming", incomingTime)
          .add("stage", stageTime)
          .add("link", linkTime)
          .add("fetch", fetchTime)
          .add("delete", deleteTime)
          .toString();
    }
  }
}
