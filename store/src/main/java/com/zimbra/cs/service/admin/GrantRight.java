...
public class GrantRight extends RightDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (modifierInfo == null) {
      throw ServiceException.INVALID_REQUEST(
          "No information specified on what right to assign", null);
    }
    ...
    RightCommand.grantRight(
        ...
        erTargSel,
        grReq.getGrantee(),
        modifierInfo.getValue(),
        rightModifier);
    // Bug 100965 Avoid Cross server delegate admin being broken after initial creation due to stale
    // caches
    if (com.zimbra.soap.type.TargetType.domain == erTargSel.getType()) {
      ...
      if ((TargetBy.id == by) || (TargetBy.name == by)) {
        CacheSelector cacheSel =
            new CacheSelector(true /* allServers */, CacheEntryType.domain.toString());
        CacheEntrySelector ceSel =
            new CacheEntrySelector(
                (TargetBy.id == erTargSel.getBy()) ? CacheEntryBy.id : CacheEntryBy.name,
                erTargSel.getValue());
        ...
        try {
          ...
          ZimbraLog.acl.info(
              "Problem flushing acl cache for domain %s/%s after granting rights",
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    notes.add(
        "Grantor must have the same or more rights on the same target or "
