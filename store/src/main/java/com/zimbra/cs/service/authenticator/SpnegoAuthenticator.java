...
public class SpnegoAuthenticator extends SSOAuthenticator {
  ...
  public SpnegoAuthenticator(
      HttpServletRequest req, HttpServletResponse resp, SpnegoLoginService spnegoUserRealm) {
    super(req, resp);
  ...
  public SpnegoAuthenticator(
      ...
      SpnegoLoginService spnegoUserRealm,
      String error401Page) {
   ...
   * =========================================================
   */
  private ZimbraPrincipal authenticate(
      LoginService realm, Request request, HttpServletResponse response)
    ...
    if (header == null) {
      ...
      if (identity == null) {
        throw AuthFailedServiceException.AUTH_FAILED(
            "SpnegoAuthenticator: unable to login", (Throwable) null);
      }
      ...
      if (user != null) {
        ...
        DefaultUserIdentity defaultUserIdentity =
            new DefaultUserIdentity(identity.getSubject(), zimbraPrincipal, roles);
        SpnegoUserIdentity spnegoUserIdentity =
            new SpnegoUserIdentity(identity.getSubject(), zimbraPrincipal, defaultUserIdentity);
        Authentication authentication = new UserAuthentication(getAuthType(), spnegoUserIdentity);
        request.setAuthentication(authentication);
        response.addHeader(
            HttpHeader.WWW_AUTHENTICATE.toString(),
            HttpHeader.NEGOTIATE.toString() + " " + ((SpnegoUserPrincipal) user).getToken());
        ...
        throw AuthFailedServiceException.AUTH_FAILED(
            "SpnegoAuthenticator: no user found, authentication failed", (Throwable) null);
      }
    } else {
      /*
       ...
       */
      throw AuthFailedServiceException.AUTH_FAILED(
          "SpnegoAuthenticator: authentication failed, unknown header (browser is likely"
              + " misconfigured for SPNEGO)",
          (Throwable) null);
    }
  }
