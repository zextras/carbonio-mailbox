...
public class SyncFormatter extends Formatter {
  ...
  private static List<Pair<String, String>> getXZimbraHeaders(MailItem item) {
    ...
    if (item instanceof Message)
      hdrs.add(
          new Pair<String, String>("X-Zimbra-Conv", ((Message) item).getConversationId() + ""));
    return hdrs;
  }
  ...
  private static void addXZimbraHeaders(UserServletContext context, MailItem item, long size)
      throws IOException {
    ...
    // also explicitly set the Content-Length header, as it's only done implicitly for short
    // payloads
    if (context.params.get(QP_NOHDR) == null) {
      byte[] inline = getXZimbraHeadersBytes(hdrs);
      if (size > 0) context.resp.setContentLength(inline.length + (int) size);
  ...
  private void handleCalendarItem(UserServletContext context, CalendarItem calItem)
    ...
    if (context.itemId.hasSubpart()) {
      Pair<MimeMessage, Integer> calItemMsgData =
          calItem.getSubpartMessageData(context.itemId.getSubpartId());
  ...
  private void handleMessagePart(UserServletContext context, MailItem item)
      throws IOException, ServiceException, MessagingException, UserServletException {
    if (!(item instanceof Message))
