...
public class ToXML {
  ...
  static Element encodeCalendarResource(
      Element parent, CalendarResource resource, boolean applyCos) {
  ...
  public static Element encodePasswordRules(Element parent, Account account) {
    ...
    ToXML.encodeAttr(
        parent, Provisioning.A_zimbraPasswordLocked, Boolean.toString(account.isPasswordLocked()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinLength,
        Integer.toString(account.getPasswordMinLength()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMaxLength,
        Integer.toString(account.getPasswordMaxLength()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinUpperCaseChars,
        Integer.toString(account.getPasswordMinUpperCaseChars()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinLowerCaseChars,
        Integer.toString(account.getPasswordMinLowerCaseChars()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinPunctuationChars,
        Integer.toString(account.getPasswordMinPunctuationChars()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinNumericChars,
        Integer.toString(account.getPasswordMinNumericChars()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordMinDigitsOrPuncs,
        Integer.toString(account.getPasswordMinDigitsOrPuncs()));
    ToXML.encodeAttr(
        parent, Provisioning.A_zimbraPasswordMinAge, Integer.toString(account.getPasswordMinAge()));
    ToXML.encodeAttr(
        parent, Provisioning.A_zimbraPasswordMaxAge, Integer.toString(account.getPasswordMaxAge()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordEnforceHistory,
        Integer.toString(account.getPasswordEnforceHistory()));
    ToXML.encodeAttr(
        parent,
        Provisioning.A_zimbraPasswordBlockCommonEnabled,
        Boolean.toString(account.isPasswordBlockCommonEnabled()));
    return parent;
  }

  static void encodeAttrs(
      Element e, Map attrs, Set<String> reqAttrs, AttrRightChecker attrRightChecker) {
    encodeAttrs(e, attrs, AccountConstants.A_N, reqAttrs, attrRightChecker);
  }

  private static void encodeAttrs(
      Element e, Map attrs, String key, Set<String> reqAttrs, AttrRightChecker attrRightChecker) {
    ...
    for (Iterator iter = attrs.entrySet().iterator(); iter.hasNext(); ) {
      ...
      if (name.equalsIgnoreCase(Provisioning.A_zimbraDataSourcePassword)) continue;
  ...
  private static class EntrySearchFilterXmlVisitor implements Visitor {
    ...
    public void enterMulti(Multi term) {
      ...
      if (term.isNegation())
        elem.addAttribute(AccountConstants.A_ENTRY_SEARCH_FILTER_NEGATION, true);
      if (!term.isAnd()) elem.addAttribute(AccountConstants.A_ENTRY_SEARCH_FILTER_OR, true);
  ...
  public static Element encodeLocale(Element parent, Locale locale, Locale inLocale) {
    ...
    String name =
        L10nUtil.getMessage(
            false /* shoutIfMissing */, L10nUtil.L10N_MSG_FILE_BASENAME, id, Locale.getDefault());
    if (name == null) {
  ...
  public static Element encodeSignature(Element parent, Signature signature) {
    ...
    for (SignatureContent c : contents) {
      e.addNonUniqueElement(AccountConstants.E_CONTENT)
          .addAttribute(AccountConstants.A_TYPE, c.getMimeType())
          .addText(c.getContent());
    }

    String contactId = signature.getAttr(Provisioning.A_zimbraPrefMailSignatureContactId);
    if (contactId != null) e.addNonUniqueElement(AccountConstants.E_CONTACT_ID).setText(contactId);
    ...
    if (value instanceof String[]) {
      ...
      if (!Strings.isNullOrEmpty((String) value)) {
        response.addKeyValuePair(
            key, (String) value, AccountConstants.E_ATTR, AccountConstants.A_NAME);
      }
    }
  }
  ...
  public static Element encodeACE(Element parent, ZimbraACE ace) {
    Element eACE =
        parent
            .addNonUniqueElement(AccountConstants.E_ACE)
