...
public class ModifyCalendarItem extends CalendarRequest {
  ...
  protected class ModifyCalendarItemParser extends ParseMimeMessage.InviteParser {
    ...
    public List<ZAttendee> getAttendeesAdded() {
      return mAttendeesAdded;
    }

    public List<ZAttendee> getAttendeesCanceled() {
      return mAttendeesCanceled;
    }
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (isInterMboxMove) {
      ...
      ItemActionHelper.MOVE(
          octxt, mbox, zsc.getResponseProtocol(), ids, calItem.getType(), null, iidFolder);
    }
  ...
  private Element modifyCalendarItem(
    ...
    if (!dat.mInvite.hasRecurId())
      ZimbraLog.calendar.info(
          "<ModifyCalendarItem> id=%d, folderId=%d, subject=\"%s\", UID=%s",
          calItem.getId(),
          folderId,
          dat.mInvite.isPublic() ? dat.mInvite.getName() : "(private)",
          dat.mInvite.getUid());
    else
      ZimbraLog.calendar.info(
          "<ModifyCalendarItem> id=%d, folderId=%d, subject=\"%s\", UID=%s, recurId=%s",
          calItem.getId(),
          folderId,
          dat.mInvite.isPublic() ? dat.mInvite.getName() : "(private)",
          dat.mInvite.getUid(),
          dat.mInvite.getRecurId().getDtZ());
    ...
    if (!dat.mInvite.isOrganizer()) {
      ...
      // This is the case of organizer saving an invite with attendees, but without sending the
      // notification.
      assert (dat.mInvite.hasOtherAttendees() && !hasRecipients);
      if (!inv.hasOtherAttendees()) {
        // Special case of going from a personal appointment (no attendees) to a draft appointment
        // with
        // attendees.  neverSent was false for being a personal appointment, so we need to
        // explicitly set it to true.
        // This case is essentially identical to creating a new appointment with attendees without
        // notification.
    ...
    if (inv.isOrganizer()) {
      ...
      if (!inv.isNeverSent()) { // No need to notify for a draft appointment.
        if (!atsCanceled.isEmpty()) {
          notifyRemovedAttendees(
              zsc, octxt, acct, mbox, inv.getCalendarItem(), inv, atsCanceled, sendQueue);
        }
      }

      List<ZAttendee> atsAdded = parser.getAttendeesAdded();
      // Figure out if we're notifying all attendees.  Must do this before clearing recipients from
      // dat.mMm.
      boolean notifyAllAttendees = isNotifyingAll(dat.mMm, atsAdded);
      // If notifying all the attendees update the last sequence number otherwise retain the
      // existing value.
      ...
      if (inv.isRecurrence()) {
        // Clear to/cc/bcc from the MimeMessage, so that the sendCalendarMessage call only updates
        // the organizer's
        // own appointment without notifying any attendees.  Notifications will be sent later,
        removeAllRecipients(dat.mMm);

        // If this is a change that removes exceptions, send cancel notification to attendees who
        // are only
        // on the exception instances.  They will be removed from the appointment entirely as a
        // result.
        ...
        if (!acct.isCalendarKeepExceptionsOnSeriesTimeChange()) {
          ...
          if (ic.isExceptionRemovingChange()) {
            ...
            for (Invite except : invites) {
              if (!except.isNeverSent()
                  && except.hasRecurId()
                  && !except.isCancel()
                  && inviteIsAfterTime(except, now)) {
                List<ZAttendee> toNotify =
                    CalendarUtils.getRemovedAttendees(
                        except.getAttendees(), seriesInv.getAttendees(), false, acct);
                if (!toNotify.isEmpty()) {
                  notifyRemovedAttendees(
                      zsc, octxt, acct, mbox, calItem, except, toNotify, sendQueue);
                }
              }
            }
          }
        ...
        sendCalendarMessage(
            zsc, octxt, folderId, acct, mbox, dat, response, true, forceSend, sendQueue);
        ...
        if (!atsAdded.isEmpty() || !atsCanceled.isEmpty()) {
          addRemoveAttendeesInExceptions(
              octxt, mbox, inv.getCalendarItem(), atsAdded, atsCanceled, ignorePastExceptions);
        }
  ...
  // Find out if we're notifying all attendees or only those who were added.  Let's assume we're
  // notifying
