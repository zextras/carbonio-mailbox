...
public class ExpandRecur extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (days > maxDays)
      throw ServiceException.INVALID_REQUEST(
          "Requested range is too large (Maximum " + maxDays + " days)", null);
  ...
  protected static ParsedRecurrence parseRecur(Element request, TimeZoneMap tzmap)
    ...
    for (Iterator<Element> compIter = request.elementIterator(); compIter.hasNext(); ) {
      ...
      if (MailConstants.E_CAL_CANCEL.equals(elemName)) {
        isCancel = true;
      } else if (!MailConstants.E_INVITE_COMPONENT.equals(elemName)
          && !MailConstants.E_CAL_EXCEPT.equals(elemName)) {
        continue;
      }

      RecurId recurId = null;
      Element recurIdElem = elem.getOptionalElement(MailConstants.E_CAL_EXCEPTION_ID);
      if (recurIdElem != null) recurId = CalendarUtils.parseRecurId(recurIdElem, tzmap);
      if (!isCancel) {
        ...
        if (dtEndElem != null) dtEnd = CalendarUtils.parseDateTime(dtEndElem, tzmap);
        ParsedDuration dur = null;
        Element durElem = elem.getOptionalElement(MailConstants.E_CAL_DURATION);
        if (durElem != null) dur = ParsedDuration.parse(durElem);
        if (dtEnd == null && dur == null)
          throw ServiceException.INVALID_REQUEST(
              "Must specify either "
                  + MailConstants.E_CAL_END_TIME
                  + " or "
                  + MailConstants.E_CAL_DURATION
                  + " in "
                  + elemName,
              null);
        ...
        if (recurElem != null) {
          ...
          if (recurId == null) recurId = new RecurId(dtStart, RecurId.RANGE_NONE);
  ...
  protected static List<Instance> getInstances(
      ParsedRecurrence parsed, long rangeStart, long rangeEnd) throws ServiceException {
    ...
    if (parsed.rrule != null) {
      ...
      for (IException except : parsed.exceptions) {
        if (except instanceof CancellationRule) continue; // Skip canceled instances.
