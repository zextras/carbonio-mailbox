...
public class ExportAndDeleteItems extends AdminDocumentHandler {
  ...
  public Element handle(Element requst, Map<String, Object> context) throws ServiceException {
    ...
    // Lock the mailbox, to make sure that another thread doesn't modify the items we're
    // exporting/deleting.
    mbox.lock.lock();
    try {
          ...
          for (int rev : revs) {
            if (rev == 0) {
              ...
              try {
                ...
                // exception happens when we try to delete a mail_item which is already in
                // mail_item_dumpster
                continue;
              }
              ...
              try {
                ...
                // exception happens when we try to delete a revision which is already in
                // revision_dumpster
                continue;
              }
            }
