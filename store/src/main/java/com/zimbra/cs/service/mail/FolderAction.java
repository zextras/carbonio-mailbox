...
public class FolderAction extends ItemAction {
  ...
  protected String[] getProxiedIdPath(Element request) {
    ...
    if (operation != null && FOLDER_OPS.contains(operation.toLowerCase())) return TARGET_ITEM_PATH;
  ...
  protected boolean checkMountpointProxy(Element request) {
    ...
    if (OP_GRANT.equalsIgnoreCase(operation)
        || OP_REVOKE.equalsIgnoreCase(operation)
        || OP_REVOKEORPHANGRANTS.equalsIgnoreCase(operation)) return true;
  ...
  private static final Set<String> FOLDER_OPS =
      ImmutableSet.of(
          ...
          OP_GRANT,
          OP_REVOKE,
          OP_REVOKEORPHANGRANTS,
          OP_UPDATE,
          OP_SYNCON,
          OP_SYNCOFF,
          OP_RETENTIONPOLICY,
          OP_DISABLE_ACTIVESYNC,
          OP_ENABLE_ACTIVESYNC,
          OP_WEBOFFLINESYNCDAYS);
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (operation.equals(OP_TAG)
        || operation.equals(OP_FLAG)
        || operation.equals(OP_UNTAG)
        || operation.equals(OP_UNFLAG)) {
    ...
    if (FOLDER_OPS.contains(operation)) {
      ...
      successes =
          Joiner.on(",").join(handleCommon(context, request, MailItem.Type.FOLDER).getSuccessIds());
    }
  ...
  private String handleFolder(
    ...
    if (operation.equals(OP_EMPTY)) {
      ...
      if (iid.getId() == Mailbox.ID_FOLDER_TRASH) mbox.purgeImapDeleted(octxt);
      ...
      if (gtype == ACL.GRANTEE_AUTHUSER) {
        ...
        // unlike guest, we do not fixup grantee type for key grantees if they specify an internal
        // user
      ...
      if (nentry != null) result.addAttribute(MailConstants.A_DISPLAY, nentry.getName());
      else if (gtype == ACL.GRANTEE_GUEST || gtype == ACL.GRANTEE_KEY)
        result.addAttribute(MailConstants.A_DISPLAY, zid);
      if (gtype == ACL.GRANTEE_KEY) result.addAttribute(MailConstants.A_ACCESSKEY, g.getPassword());
      ...
      mbox.alterTag(
          ...
          MailItem.Type.FOLDER,
          Flag.FlagInfo.SYNC,
          operation.equals(OP_SYNCON),
          null);
    } else if (operation.equals(OP_RETENTIONPOLICY)) {
      mbox.setRetentionPolicy(
          octxt,
          iid.getId(),
          MailItem.Type.FOLDER,
      ...
      mbox.setFolderWebOfflineSyncDays(
          octxt, iid.getId(), action.getAttributeInt(MailConstants.A_NUM_DAYS));
    } else {
  ...
  static ACL parseACL(Element eAcl, MailItem.Type folderType, Account account)
      throws ServiceException {
    if (eAcl == null) return null;
  ...
  static NamedEntry lookupGranteeByName(String name, byte type, ZimbraSoapContext zsc)
      throws ServiceException {
  ...
  private void revokeOrphanGrants(
      OperationContext octxt, Mailbox mbox, ItemId iid, String granteeId, byte gtype)
      throws ServiceException {
  ...
  private void revokeOrphanGrants(
      OperationContext octxt, Mailbox mbox, FolderNode node, String granteeId, byte gtype)
      throws ServiceException {
    if (node.mFolder != null) {
      ...
      boolean canAdmin =
          (mbox.getEffectivePermissions(octxt, node.mFolder.getId(), MailItem.Type.FOLDER)
                  & ACL.RIGHT_ADMIN)
              != 0;
        ...
        if (acl != null) {
          for (ACL.Grant grant : acl.getGrants()) {
            if (granteeId.equals(grant.getGranteeId()) && gtype == grant.getGranteeType()) {
              ...
              // break out of the loop since there can be only one grant for the same grantee on a
              // folder
              break;
            }
          }
