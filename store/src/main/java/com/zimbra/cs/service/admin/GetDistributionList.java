...
public class GetDistributionList extends DistributionListDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (group == null) {
        ...
        if (null != pseudoTarget) {
          AdminAccessControl aac =
              checkDistributionListRight(
                  zsc, (DistributionList) pseudoTarget, AdminRight.PR_ALWAYS_ALLOW);
      ...
      if (arc != null) {
        ...
        defendAgainstGroupHarvestingWhenAbsent(
            dlBy, dlSel.getKey(), zsc, Admin.R_getDistributionList);
      }
    } else if (group.isDynamic()) {
      AdminAccessControl aac =
          checkDynamicGroupRight(zsc, (DynamicGroup) group, AdminRight.PR_ALWAYS_ALLOW);
      arc = aac.getAttrRightChecker(group);
    } else {
      AdminAccessControl aac =
          checkDistributionListRight(zsc, (DistributionList) group, AdminRight.PR_ALWAYS_ALLOW);
      arc = aac.getAttrRightChecker(group);
    }
  ...
  private void encodeMembers(
    ...
    if (offset > 0 && offset >= members.length) {
      throw ServiceException.INVALID_REQUEST(
          "offset " + offset + " greater than size " + members.length, null);
    }
  ...
  public static Element encodeDistributionList(
      Element e,
      Group group,
      boolean hideMembers,
      boolean hideOwners,
      ...
      throws ServiceException {
    return encodeDistributionList(
        e, group, hideMembers, hideOwners, true, reqAttrs, attrRightChecker);
  }
  ...
  public static Element encodeDistributionList(
    ...
    if (encodeAttrs) {
      ...
      ToXML.encodeAttrs(
          eDL, group.getUnicodeAttrs(), AdminConstants.A_N, reqAttrs, hideAttrs, attrRightChecker);
    }
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        String.format(AdminRightCheckPoint.Notes.GET_ENTRY, Admin.R_getDistributionList.getName()));
