...
public class SendInviteReply extends CalendarRequest {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (identityId == null) {
      ...
      if (msgElem != null) identityId = msgElem.getAttribute(MailConstants.A_IDENTITY_ID, null);
    }
    ...
    if (iid.hasSubpart()) {
      ...
      if (intendedFor != null) {
        try {
          ...
          intendedAcct =
              Provisioning.getInstance().get(AccountBy.name, intendedForAddr.getAddress());
        } catch (AddressException e) {
          throw ServiceException.INVALID_REQUEST(
              "The intended account " + intendedFor + " is invalid", e);
        }
        if (intendedAcct == null) {
          throw ServiceException.INVALID_REQUEST(
              "The intended account " + intendedFor + " was not found", null);
        }
        // Special case: intended account = me.
        if (intendedAcct.equals(mbox.getAccount())) intendedAcct = null;
        else intendedForMe = false;
      }

      if (intendedAcct != null) {
        // trace logging: let's just indicate we're replying to a remote appointment
        ZimbraLog.calendar.info(
            "<SendInviteReply> (remote mbox) id=%s, verb=%s, notifyOrg=%s",
            new ItemIdFormatter(zsc).formatItemId(iid),
            verb.toString(),
            Boolean.toString(updateOrg));
        ...
        if (addInviteResult == null) throw MailServiceException.INVITE_OUT_OF_DATE(iid.toString());
        ...
        if (info.getInvite() != null) {
          ...
          if (calItem != null || !isDecline) {
            ...
            if (calItem == null)
              throw ServiceException.FAILURE(
                  "Could not refetch created/updated calendar item", null);
          } else {
            calItemId = 0;
          }
          oldInv = inv;
          ...
          if (calItem == null) throw ServiceException.FAILURE("Missing invite data", null);
    ...
    if (intendedForMe) {
      if (oldInv == null) throw MailServiceException.INVITE_OUT_OF_DATE(iid.toString());

      if (calItem != null
          && (mbox.getEffectivePermissions(octxt, calItemId, MailItem.Type.UNKNOWN)
                  & ACL.RIGHT_ACTION)
              == 0) {
        throw ServiceException.PERM_DENIED(
            "You do not have ACTION rights for CalendarItem " + calItemId);
      }
      ...
      boolean allowPrivateAccess =
          calItem != null ? calItem.allowPrivateAccess(authAcct, isAdmin) : true;
      boolean isCalendarResource = acct instanceof CalendarResource;
      if (!allowPrivateAccess && !oldInv.isPublic() && !isCalendarResource)
        throw ServiceException.PERM_DENIED(
            "Cannot reply to a private appointment/task on behalf of another user");
      ...
      if (exceptDt == null)
        ZimbraLog.calendar.info(
            "<SendInviteReply> id=%s, folderId=%s, verb=%s, notifyOrg=%s, subject=\"%s\", UID=%s",
            calItemIdStr,
            folderIdStr,
            verb.toString(),
            Boolean.toString(updateOrg),
            oldInv.isPublic() ? oldInv.getName() : "(private)",
            oldInv.getUid());
      else
        ZimbraLog.calendar.info(
            "<SendInviteReply> id=%s, folderId=%s, verb=%s, notifyOrg=%s, subject=\"%s\", UID=%s,"
      ...
      if (calItem != null && oldInv.isRecurrence() && exceptDt != null) {
        ...
        if (at != null) at.setPartStat(partStat);
        ...
        if (calItem == null)
          throw MailServiceException.NO_SUCH_CALITEM(
              iid.toString(), "Could not find calendar item");
      ...
      if (updateOrg && oldInv.hasOrganizer()) {
        ...
        if (organizer != null) locale = organizer.getLocale();
        else locale = !onBehalfOf ? acct.getLocale() : authAcct.getLocale();
        String subject;
        if (!allowPrivateAccess && !oldInv.isPublic())
          subject = L10nUtil.getMessage(MsgKey.calendarSubjectWithheld, locale);
        else subject = oldInv.getName();
        ...
        if (msgElem != null) {
          ...
          if (!(acct instanceof CalendarResource)) {
            csd.mMm =
                CalendarMailSender.createDefaultReply(
                    acct,
                    identityId,
                    authAcct,
                    identityId,
            ...
            MimeMessage mmInv = calItem.getSubpartMessage(oldInv.getMailItemId());
            csd.mMm =
                CalendarMailSender.createResourceAutoReply(
                    octxt,
                    identityId,
                    identityId,
                    mbox,
        ...
        int apptFolderId;
        if (calItem != null) apptFolderId = calItem.getFolderId();
        else apptFolderId = oldInv.isTodo() ? Mailbox.ID_FOLDER_TASKS : Mailbox.ID_FOLDER_CALENDAR;
  ...
  private static AddInviteResult sendAddInvite(ZMailbox zmbx, OperationContext octxt, Message msg)
    ...
    Element addInvite =
        Element.create(SoapProtocol.SoapJS, MailConstants.ADD_APPOINTMENT_INVITE_REQUEST);
    ToXML.encodeMessageAsMIME(
        addInvite,
        ifmt,
        octxt,
        msg,
    ...
    if (calItemId != 0) return new AddInviteResult(calItemId, invId, compNum);
    else return null;
  }
  ...
  private static CalendarItem safeGetCalendarItemById(
      Mailbox mbox, OperationContext octxt, ItemId calItemId) throws ServiceException {
    try {
      ...
      if (MailServiceException.NO_SUCH_ITEM.equals(code)
          ...
          || MailServiceException.NO_SUCH_TASK.equals(code)) return null;
      else throw e;
    }
  }

  private static CalendarItem safeGetCalendarItemById(
      Mailbox mbox, OperationContext octxt, int calItemId) throws ServiceException {
    try {
      return mbox.getCalendarItemById(octxt, calItemId);
    } catch (MailServiceException e) {
      String code = e.getCode();
