...
public class ZimbraAuthProviderForOAuth extends AuthProvider {
  ...
  protected AuthToken authToken(HttpServletRequest req, boolean isAdminReq)
    ...
    ZimbraLog.extensions.debug(
        "authToken(HttpServletRequest req, boolean isAdminReq) is requested.");
    if (isAdminReq) {
    ...
    try {
      account =
          Provisioning.getInstance()
              .getAccountByForeignPrincipal("oAuthAccessToken:" + accessToken);
    ...
    if (account == null) {
      throw AuthProviderException.FAILURE(
          "Could not identify account corresponding to the OAuth request");
    }
    ...
    for (String val : accessors) {
      if (val.startsWith(accessToken)) {
        try {
          accessor =
              new OAuthAccessorSerializer().deserialize(val.substring(accessToken.length() + 2));
        } catch (ServiceException e) {
    ...
    if (accessor == null) throw new AuthTokenException("invalid OAuth token");
    try {
  ...
  private AuthToken genAuthToken(String encodedAuthToken)
      throws AuthProviderException, AuthTokenException {
    if (StringUtil.isNullOrEmpty(encodedAuthToken)) throw AuthProviderException.NO_AUTH_DATA();
