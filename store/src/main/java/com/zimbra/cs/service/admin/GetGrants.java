...
public class GetGrants extends RightDocumentHandler {
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        "Needs a get attr right of zimbraACE on each the target entry.  "
            + "Granting the "
            + Admin.R_viewGrants.getName()
            + " is one way to do it, "
            + "which will give the right on all target types.   Use inline right "
            + "if more granularity is needed.   See doc for the "
            + Admin.R_viewGrants.getName()
            + " right in rights.xml for more details.");
  }
}
