...
public class AddCalendarItemInvite extends CalendarRequest {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    SetCalendarItemData scid =
        SetCalendarItem.getSetCalendarItemData(zsc, octxt, acct, mbox, request, parser);
    ...
    if (calItem != null) {
      ...
      if (f != Mailbox.ID_FOLDER_TRASH && f != Mailbox.ID_FOLDER_SPAM) folderId = f;
    }
        ...
        if (fromEmail != null) {
          ...
          try {
            intendedForAddress =
                scid.message
                    .getMimeMessage()
                    .getHeader(CalendarMailSender.X_ZIMBRA_CALENDAR_INTENDED_FOR, null);
    ...
    if (!inv.hasRecurId())
      ZimbraLog.calendar.info(
          "<AddCalendarItemInvite> id=%s, folderId=%d, subject=\"%s\", UID=%s",
          calItemIdStr, folderId, inv.isPublic() ? inv.getName() : "(private)", inv.getUid());
    else
      ZimbraLog.calendar.info(
          "<AddCalendarItemInvite> id=%s, folderId=%d, subject=\"%s\", UID=%s, recurId=%s",
          calItemIdStr,
          folderId,
          inv.isPublic() ? inv.getName() : "(private)",
          inv.getUid(),
          inv.getRecurId().getDtZ());
