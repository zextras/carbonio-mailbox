...
public class JWTUtil {
  ...
  private static final Cache<String, Claims> CLAIMS_CACHE =
      CacheBuilder.newBuilder().maximumSize(LC.zimbra_authtoken_cache_size.intValue()).build();
   ...
   */
  public static final String generateJWT(
      byte[] jwtKey, String salt, long issuedAt, AuthTokenProperties properties, long keyVersion)
      throws AuthFailedServiceException {
    if (properties != null) {
      ...
      if (properties.getValidityValue() != -1) {
        builder.claim(
            AuthTokenProperties.C_VALIDITY_VALUE, String.valueOf(properties.getValidityValue()));
      }
  ...
  /**
   * Look for the salt in request:soap context/soapEngine context/ZM_JWT cookie and return the value
   * if found.
   *
   * @param soapCtxt
   * @param engineCtxt
    ...
    if (salt == null && engineCtxt != null) {
      ...
      if (engineCtxt.get(SoapEngine.JWT_SALT) != null) {
        ...
        ZimbraLog.account.debug(
            "salt not found in soap context or engine context, looking in cookie");
