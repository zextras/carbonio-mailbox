...
public class GetDistributionListMembers extends GalDocumentHandler {
  ...
  protected Element proxyIfNecessary(Element request, Map<String, Object> context)
    ...
    boolean proxiedToHomeOfInternalGroup =
        request.getAttributeBool(A_PROXIED_TO_HOME_OF_GROUP, false);

    if (proxiedToHomeOfInternalGroup) {
      // the request was proxied here because this is the home server of the group, always execute
      // locally
      return null;
    } else {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (group != null) {
       ...
       */
      dlMembersResult =
          getMembersFromLdap(context, request, account, group, proxiedToHomeOfInternalGroup);
    }
    ...
    if (dlMembersResult instanceof ProxiedDLMembers) {
      ...
      throw ServiceException.FAILURE(
          "unsopported DLMembersResult class: " + dlMembersResult.getClass().getCanonicalName(),
          null);
    }
  }
   ...
   * into a proxy loop.
   */
  private DLMembersResult getMembersFromLdap(
    ...
    if (local) {
      ...
      if (server == null) {
        ...
        ZimbraLog.account.warn(
            "unable to find home server (%s) for group %s, getting members from LDAP on local"
                + " server",
        ...
        ZimbraLog.account.info(
            "Proxying request: dl.name=%s reason: target=%s", group.getName(), server.getName());
  ...
  private DLMembersResult getMembersFromLdap(Account account, Group group) throws ServiceException {
    if (group.isHABGroup()) {
      ZimbraLog.account.debug(
          "Retrieving hab group members from LDAP for group %s", group.getName());
      ...
      if (allow) {
        ...
        ZimbraLog.account.debug(
            "account %s is not allowed to get members from ldap for group %s",
  ...
  protected Element processDLMembers(
    ...
    if (!GalSearchControl.canExpandGalGroup(dlName, dlMembers.getDLZimbraId(), account)) {
      ZimbraLog.misc.warn(
          "dlName: %s, dlMembers(%s) size %d, id %s",
          dlName, dlMembers.getClass(), dlMembers.getTotal(), dlMembers.getDLZimbraId());
    ...
    if (dlMembers != null) {
      ...
      if (offset > 0 && offset >= numMembers) {
        throw ServiceException.INVALID_REQUEST(
            "offset " + offset + " greater than size " + numMembers, null);
      }
