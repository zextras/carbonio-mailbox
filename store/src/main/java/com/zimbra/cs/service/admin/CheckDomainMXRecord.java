...
public class CheckDomainMXRecord extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (SMTPHost == null || SMTPHost.length() < 1) SMTPHost = domain.getName();
    ...
    String message =
        String.format(
            "Domain is configured to use SMTP host: %s. None of the MX records match this name.",
            SMTPHost);
        ...
        if (attr.size() == 1) {
          ...
          for (int i = 0; i < attr.size(); i++) {
            ...
            if (o instanceof String) {
              String rec = o.toString();
              ZimbraLog.soap.info(
                  "found MX record " + attr.getID() + "-" + Integer.toString(i) + " = " + rec);
              if (rec.matches(SMTPHostMatch)) {
                found = true;
                break;
              }
              ...
              String rec = new String((byte[]) o);
              ZimbraLog.soap.info(
                  "found MX attribute " + attr.getID() + "-" + Integer.toString(i) + " = " + rec);
              if (rec.matches(SMTPHostMatch)) {
                found = true;
                break;
              }
