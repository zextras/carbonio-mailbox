...
public class ItemData {
  ...
  public ItemData(final String encoded) throws IOException {
    try {
      ...
      if (uuid != null
          && uuid.length() > 0) { // because optString returns an empty string rather than null
        ud.uuid = uuid;
      }
      ...
      // indexId and volumeId changed to optional strings from mandatory ints which breaks sync with
      // 5.0 clients
  ...
  private void getTagsFromJson(JSONObject json) {
    tags =
        json.optString(
            Keys.TagNames.toString()); // 6.0.7+ format (TagNames="<tag-name>[:<tag-name>]")
    if (tags.length() == 0) {
      // either 6.0.6 format (TagStr=":<tag-name>[:<tag-name>]"), or pre-6.0.6 format (TagStr="<tag
      // int>[,<tag int>]")
  ...
  public boolean tagsEqual(MailItem mi) {
    ...
    return isOldTags()
        ? tags.equals(TagUtil.getTagIdString(mi))
        : tags.equals(getTagString(mi.getTags()));
  }
}
