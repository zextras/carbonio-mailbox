...
public class NetscapeLdifFormatter extends LdifFormatter {
  ...
  @Override
  protected boolean toLDIFContact(
      Map<String, String> contact, StringBuilder sb, String[] galLdapAttrMap) {
  ...
  private void addNetscapeAttributes(StringBuilder sb, Map<String, String> contact) {
    ...
    if (!StringUtil.isNullOrEmpty(birthday)) {
      ...
      try {
        ...
        c.setTime(parse);
        addLDIFEntry(MOZILLA_BIRTH_YEAR, String.valueOf(c.get(Calendar.YEAR)), sb, false);
        addLDIFEntry(MOZILLA_BIRTH_MONTH, String.valueOf(c.get(Calendar.MONTH) + 1), sb, false);
        addLDIFEntry(MOZILLA_BIRTH_DAY, String.valueOf(c.get(Calendar.DATE)), sb, false);
    ...
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_email2), contact.get(ContactConstants.A_email2), sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_nickname),
        contact.get(ContactConstants.A_nickname),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homeCity),
        contact.get(ContactConstants.A_homeCity),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homeCountry),
        contact.get(ContactConstants.A_homeCountry),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homePostalCode),
        contact.get(ContactConstants.A_homePostalCode),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homeState),
        contact.get(ContactConstants.A_homeState),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homeStreet),
        contact.get(ContactConstants.A_homeStreet),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_homeURL),
        contact.get(ContactConstants.A_homeURL),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_workURL),
        contact.get(ContactConstants.A_workURL),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_custom1),
        contact.get(ContactConstants.A_custom1),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_custom2),
        contact.get(ContactConstants.A_custom2),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_custom3),
        contact.get(ContactConstants.A_custom3),
        sb);
    encodeAndAddLDIFEntry(
        netscapeAttrMap.get(ContactConstants.A_custom4),
        contact.get(ContactConstants.A_custom4),
        sb);
  }
  ...
  protected void addObjectClass(StringBuilder sb) {
    ...
    addLDIFEntry(
        LdapConstants.ATTR_objectClass, LdapObjectClass.ZIMBRA_DEFAULT_PERSON_OC, sb, false);
  }
}
