...
public class GetRightsDoc extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Element ePackage : request.listElements(AdminConstants.E_PACKAGE)) {
      if (specificPackages == null) specificPackages = new HashSet<String>();
  ...
  private void doGetRightsDoc(
      Map<String, Object> context, HashSet<String> specificPackages, Element response)
    ...
    Map<String, TreeMap<String, AdminRightCheckPoint>> handlersWithRightsDoc =
        new TreeMap<String, TreeMap<String, AdminRightCheckPoint>>();
    ...
    for (Map.Entry<String, TreeMap<String, AdminRightCheckPoint>> entry :
        handlersWithRightsDoc.entrySet()) {
  ...
  private void genDomainAdminRights(Map<String, Object> context, Element response)
      throws ServiceException {
    Element eDomainAdmin =
        response.addElement("domainAdmin-copypaste-to-rights-domainadmin-xml-template");
    ...
    rights
        .get(TargetType.account)
        .add(Admin.R_setDomainAdminAccountAndCalendarResourceAttrs.getName());
    rights
        .get(TargetType.calresource)
        .add(Admin.R_setDomainAdminAccountAndCalendarResourceAttrs.getName());
    ...
    for (Map.Entry<TargetType, TreeSet<String>> entry : rights.entrySet()) {
      ...
      if (entry.getValue().size() > 0) {
        Element eRight =
            eDomainAdmin
                .addElement("right")
                .addAttribute("name", "domainAdmin" + tt.getPrettyName() + "Rights")
                .addAttribute("type", "combo");
