...
public class GrantPermission extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    List<ZimbraACE> granted = ACLHelper.grantRight(Provisioning.getInstance(), account, aces);
  ...
  private static NamedEntry lookupEmailAddress(String name) throws ServiceException {
    ...
    if (nentry == null) nentry = prov.get(Key.DistributionListBy.name, name);
    return nentry;
  }

  // orig: FolderAction.lookupGranteeByName
  private static NamedEntry lookupGranteeByName(
      String name, GranteeType type, ZimbraSoapContext zsc) throws ServiceException {
    if (type == GranteeType.GT_AUTHUSER
        || type == GranteeType.GT_PUBLIC
        || type == GranteeType.GT_GUEST
        || type == GranteeType.GT_KEY) return null;
    ...
    try {
      switch (type) {
        case GT_USER:
          ...
          if (nentry == null && granting) throw AccountServiceException.NO_SUCH_ACCOUNT(zid);
          else return nentry;
        case GT_GROUP:
        ...
        case GT_DOMAIN:
          ...
          if (nentry == null && granting) throw AccountServiceException.NO_SUCH_DOMAIN(zid);
          else return nentry;
        case GT_GUEST:
        case GT_KEY:
        case GT_AUTHUSER:
