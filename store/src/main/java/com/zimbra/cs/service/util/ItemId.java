...
public class ItemId implements java.io.Serializable {
  ...
  public static ItemId createFromEncoded(String encoded, String defaultAccountId)
  ...
  public boolean isLocal() throws ServiceException {
    if (mAccountId == null) return true;
    Account acctTarget = Provisioning.getInstance().get(AccountBy.id, mAccountId);
    if (acctTarget == null) throw AccountServiceException.NO_SUCH_ACCOUNT(mAccountId);
    return DocumentHandler.getLocalHost()
        .equalsIgnoreCase(acctTarget.getAttr(Provisioning.A_zimbraMailHost));
  }

  /**
   * Returns whether the item belongs to the mailbox owned by the passed-in {@link Account}. If
   * <code>acct</code> is <tt>null</tt> and/or this ItemId was unqualified, returns <tt>true</tt>.
   */
  ...
  /**
   * Returns whether the item belongs to the mailbox owned by the passed-in account. If <code>acctId
   * </code> is <tt>null</tt> and/or this ItemId was unqualified, returns <tt>true</tt>.
   */
  ...
  /**
   * Returns whether the item belongs to the specified {@link MailboxStore}. If <code>mbox</code> is
   * <tt>null</tt> and/or this ItemId was unqualified, returns <tt>true</tt>.
   */
  ...
  public boolean equals(Object that) {
    ...
    else if (!(that instanceof ItemId)) return false;

    ItemId other = (ItemId) that;
    if (Objects.equals(mAccountId, other.mAccountId)
        || (mAccountId != null && mAccountId.equalsIgnoreCase(other.mAccountId)))
  ...
  public static Map<String, List<Integer>> groupFoldersByAccount(
      OperationContext octxt, Mailbox mbox, List<ItemId> folderIids) throws ServiceException {
    ...
    for (ItemId iidFolder : folderIids) {
      ...
      try {
        ...
        List<Integer> folderList =
            foldersMap.computeIfAbsent(targetAccountId, k -> new ArrayList<>());
