...
public class FeedManager {
  ...
  public static final class SubscriptionData<T> {
    ...
    // returns the timestamp of the most recently modified item, or the last modified time of the
    // feed itself,
  ...
  public static final String HTTP_USER_AGENT =
      "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; en-US; rv:1.9.1.7)" + BROWSER_TAG;
  ...
  protected static boolean isAddressInRange(InetAddress targetAddress, String prefix) {
    ...
    if (!targetAddress.getClass().equals(firstIp.getClass())) {
      ZimbraLog.misc.debug(
          "cannot compare across ipv4 and ipv6 address. target: %s, first ip: %s",
  ...
  /**
   * Returns true if target address is not whitelisted, and is link-local, loopback, or blacklisted.
   *
  ...
  protected static boolean isBlockedFeedAddress(URIBuilder url) {
    ...
    return targetAddress.isAnyLocalAddress()
        ...
        || blacklist.stream().anyMatch(ip -> isAddressInRange(targetAddress, ip));
  }
  ...
  private static RemoteDataInfo retrieveRemoteData(String url, Folder.SyncData fsd)
    ...
    // cannot set connection timeout because it'll affect all HttpClients associated with the conn
    // mgr.
    ...
    ConnectionConfig connConfig =
        ConnectionConfig.custom().setCharset(Charset.forName(MimeConstants.P_CHARSET_UTF8)).build();
    ...
    try {
      do {
        ...
        if (isBlockedFeedAddress(httpurl)) {
          ZimbraLog.misc.info(
              "Feed ip address blocked: %s. See localconfig for comma-separated ip list"
                  + " configuration: zimbra_feed_manager_blacklist, zimbra_feed_manager_whitelist",
              url);
          throw ServiceException.INVALID_REQUEST(
              String.format("Address for feed is blocked: %s. See mailbox logs for details.", url),
              null);
        }
        ...
        if (locationHeader != null) {
          ...
          if (statusCode == HttpServletResponse.SC_OK) {
            ...
            org.apache.http.entity.ContentType contentType =
                org.apache.http.entity.ContentType.getOrDefault(response.getEntity());
    ...
    RemoteDataInfo rdi =
        new RemoteDataInfo(statusCode, redirects, content, expectedCharset, lastModified);
    rdi.setGetMethod(get);
    return rdi;
  }

  @VisibleForTesting
  protected static SubscriptionData<?> retrieveRemoteDatasource(
      Account acct, RemoteDataInfo rdi, Folder.SyncData fsd) throws ServiceException, IOException {
    StringBuilder charset = new StringBuilder(rdi.expectedCharset);
    switch (getLeadingChar(rdi.content, charset)) {
      case -1:
        throw ServiceException.PARSE_ERROR(
            "empty body in response when fetching remote subscription", null);
      case '<':
  ...
  public static SubscriptionData<?> retrieveRemoteDatasource(
      Account acct, String url, Folder.SyncData fsd) throws ServiceException {
    assert !Strings.isNullOrEmpty(url);
  ...
  private static int getLeadingChar(BufferedInputStream is, StringBuilder charset)
      throws IOException {
    is.mark(128);
  ...
  //    private static org.dom4j.QName QN_CONTENT_ENCODED = org.dom4j.QName.get("encoded",
  ...
  private static SubscriptionData<ParsedMessage> parseRssFeed(
      Element root, Folder.SyncData fsd, long lastModified) throws ServiceException {
    try {
      ...
      Date dateChannel =
          DateUtil.parseRFC2822Date(channel.getAttribute("lastBuildDate", null), new Date());
      ...
      if (rname.equals("rss")) {
        ...
        throw ServiceException.PARSE_ERROR(
            "unknown top-level rss element name: " + root.getQualifiedName(), null);
      }
      ...
      for (Element item : root.listElements("item")) {
        ...
        if (fsd != null
            && fsd.alreadySeen(
                guid == hrefChannel ? null : guid, date == dateChannel ? null : date)) continue;
  ...
  private static SubscriptionData<ParsedMessage> parseAtomFeed(
      Element feed, Folder.SyncData fsd, long lastModified) throws ServiceException {
    try {
      ...
      for (Element item : feed.listElements("entry")) {
        ...
        if (fsd != null
            && fsd.alreadySeen(
                guid == null || guid.equals("") ? null : guid, date == dateFeed ? null : date))
          continue;
