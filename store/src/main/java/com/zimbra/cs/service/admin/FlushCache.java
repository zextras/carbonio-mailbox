...
public class FlushCache extends AdminDocumentHandler {
  ...
  private static void flushCacheOnImapDaemons(FlushCacheRequest req, ZimbraSoapContext zsc)
      throws ServiceException {
    ...
    Account acct =
        Provisioning.getInstance()
            .get(AccountBy.id, zsc.getAuthtokenAccountId(), zsc.getAuthToken());
    flushCacheOnImapDaemons(cacheTypes, cacheEntries, acct.getName(), zsc.getAuthToken());
  }

  public static void flushCacheOnImapDaemons(
      String cacheTypes, CacheEntry[] entries, AuthToken authToken) {
    flushCacheOnImapDaemons(cacheTypes, entries, LC.zimbra_ldap_user.value(), authToken);
  }

  public static void flushCacheOnImapDaemons(
      String cacheTypes, CacheEntry[] entries, String acctName, AuthToken authToken) {
  ...
  public static void flushCacheOnImapDaemon(
      Server server, String cacheTypes, CacheEntry[] entries) {
    try {
      flushCacheOnImapDaemon(
          server,
          cacheTypes,
          entries,
      ...
      ZimbraLog.imap.warn(
          "unable to generate admin auth token to issue X-ZIMBRA-FLUSHCACHE request to imapd server"
              + " '%s'",
  ...
  public static void flushCacheOnImapDaemon(
    ...
    try {
      ...
      ZimbraLog.imap.warn(
          "unable to connect to imapd server '%s' to issue X-ZIMBRA-FLUSHCACHE request",
          server.getServiceHostname(), e);
      return;
    }
    try {
      ...
      if (imapTypes != null) {
        ZimbraLog.imap.debug(
            "issuing X-ZIMBRA-FLUSHCACHE request to imapd server '%s' for cache types '%s'",
            server.getServiceHostname(), imapTypes);
      ...
      ZimbraLog.imap.warn(
          "unable to issue X-ZIMBRA-FLUSHCACHE request to imapd server '%s'",
          server.getServiceHostname(), e);
    } finally {
      connection.close();
    }
