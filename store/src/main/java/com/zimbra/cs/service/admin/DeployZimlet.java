...
public class DeployZimlet extends AdminDocumentHandler {
  ...
  private void deploy(
    ...
    if (!synchronous) {
      try {
        t.join(
            TimeUnit.MILLISECONDS.convert(LC.zimlet_deploy_timeout.intValue(), TimeUnit.SECONDS));
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    } else if (AdminConstants.A_DEPLOYALL.equals(action)
        || AdminConstants.A_DEPLOYLOCAL.equals(action)) {
      ...
      try {
        ...
        if (zf.getZimletDescription().isExtension() && !zsc.getAuthToken().isAdmin()) {
          throw ServiceException.PERM_DENIED(
              "Only global admin is allowed to deploy extensions for Zimbra Admin UI");
        }
        ...
        for (Server server : servers) {
          try {
            ...
            if (flushCache) {
              ...
              checkRight(
                  zsc, context, Provisioning.getInstance().getLocalServer(), Admin.R_flushCache);
            ...
            ZimbraLog.zimlet.warn(
                "deploy zimlet failed for node %s, coutdown latch %d",
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        "If deploying on all servers, need the "
            + Admin.R_deployZimlet.getName()
            + " right on all servers or on global grant.  If deploying on local server, need "
            + "the "
            + Admin.R_deployZimlet.getName()
            + " on the local server. Only global admin is allowed to deploy extensions to Zimbra"
            + " Admin UI.");
  }
}
