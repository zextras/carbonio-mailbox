...
public class CancelCalendarItem extends CalendarRequest {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (!iid.hasSubpart())
      throw ServiceException.INVALID_REQUEST(
          "missing invId subpart: id should be specified as \"item-inv\"", null);
    ...
    if (calItem == null)
      throw MailServiceException.NO_SUCH_CALITEM(
          iid.getId(), " for CancelCalendarItemRequest(" + iid + "," + compNum + ")");
    if (calItem.inTrash())
    ...
    try {
      ...
      if (recurElt != null) {
        ...
        ZimbraLog.calendar.info(
            "<CancelCalendarItem> id=%d, folderId=%d, subject=\"%s\", UID=%s, recurId=%s",
            calItem.getId(),
            calItem.getFolderId(),
            inv.isPublic() ? inv.getName() : "(private)",
            calItem.getUid(),
            recurId.getDtZ());

        Element msgElem = request.getOptionalElement(MailConstants.E_MSG);
        cancelInstance(
            zsc, octxt, msgElem, acct, mbox, calItem, inv, recurId, inv.getAttendees(), sendQueue);
      } else {
        // if recur is not set, then we're canceling the entire calendar item...

        // trace logging
        ZimbraLog.calendar.info(
            "<CancelCalendarItem> id=%d, folderId=%d, subject=\"%s\", UID=%s",
            calItem.getId(),
            calItem.getFolderId(),
            inv.isPublic() ? inv.getName() : "(private)",
            calItem.getUid());
        ...
        if (seriesInv != null) {
          if (seriesInv.getMethod().equals(ICalTok.REQUEST.toString())
            ...
            if (seriesInv.isOrganizer()) {
              ...
              // These attendees need to be notified separately because they aren't included in the
              // series
              // cancel notice.
              ...
              for (Invite exceptInv : invs) {
                if (exceptInv != seriesInv) {
                  ...
                  if ((mthd.equals(ICalTok.REQUEST.toString())
                          || mthd.equals(ICalTok.PUBLISH.toString()))
                    ...
                    List<ZAttendee> ats =
                        CalendarUtils.getRemovedAttendees(atsExcept, atsSeries, false, acct);
                    if (!ats.isEmpty()) {
                      // notify ats
                      cancelInstance(
                          zsc,
  ...
  void cancelInstance(
    ...
    Invite cancelInvite =
        CalendarUtils.buildCancelInstanceCalendar(
            acct, authAcct, zsc.isUsingAdminPrivileges(), onBehalfOf, calItem, inv, text, recurId);
    ...
    if (msgElem != null) {
      ...
      dat.mMm =
          ParseMimeMessage.parseMimeMsgSoap(
              zsc, octxt, mbox, msgElem, mbps, ParseMimeMessage.NO_INV_ALLOWED_PARSER, dat);
  ...
  protected void cancelInvite(
    ...
    csd.mInvite =
        CalendarUtils.buildCancelInviteCalendar(
            acct, authAcct, zsc.isUsingAdminPrivileges(), onBehalfOf, calItem, inv, text);
    ...
    if (msgElem != null) {
      ...
      csd.mMm =
          ParseMimeMessage.parseMimeMsgSoap(
              zsc, octxt, mbox, msgElem, mbps, ParseMimeMessage.NO_INV_ALLOWED_PARSER, csd);

    } else {
      List<Address> rcpts;
      if (inv.isOrganizer()) rcpts = CalendarMailSender.toListFromAttendees(inv.getAttendees());
      else rcpts = new ArrayList<Address>(0);
      csd.mMm =
          CalendarMailSender.createCancelMessage(
              acct,
              authAcct,
  ...
  private void doRecipientsCheck(CalSendData dat, boolean isOrganizer, Mailbox mbox)
      throws ServiceException {
