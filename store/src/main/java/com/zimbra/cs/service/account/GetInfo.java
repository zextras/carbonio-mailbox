...
public class GetInfo extends AccountDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    response.addAttribute(
        AccountConstants.E_NAME, account.getUnicodeName(), Element.Disposition.CONTENT);
    int profileId = getProfileId(mbox, octxt);
    if (profileId != 0) {
      response.addAttribute(
          AccountConstants.E_PROFILE_IMAGE_ID, profileId, Element.Disposition.CONTENT);
    }
    try {
      response.addAttribute(
          AccountConstants.E_CRUMB, zsc.getAuthToken().getCrumb(), Element.Disposition.CONTENT);
    ...
    if (authToken.isDelegatedAuth()) {
      ...
      if (admin != null) {
        ...
        if (isAdmin) {
          response.addAttribute(
              AccountConstants.E_ADMIN_DELEGATED, true, Element.Disposition.CONTENT);
        }
      }
    }
    ...
    try {
      ...
      if (server != null) {
        response.addAttribute(
            AccountConstants.A_DOCUMENT_SIZE_LIMIT, server.getFileUploadMaxSize());
      }
    ...
    if (sections.contains(Section.MBOX) && Provisioning.onLocalServer(account)) {
      ...
      if (s instanceof SoapSession) {
        ...
        response.addAttribute(
            AccountConstants.E_PREVIOUS_SESSION,
            ((SoapSession) s).getPreviousSessionTime(),
            Element.Disposition.CONTENT);
        response.addAttribute(
            AccountConstants.E_LAST_ACCESS,
            ((SoapSession) s).getLastWriteAccessTime(),
            Element.Disposition.CONTENT);
        response.addAttribute(
            AccountConstants.E_RECENT_MSGS,
            ((SoapSession) s).getRecentMessageCount(),
            Element.Disposition.CONTENT);
      } else {
        ...
        response.addAttribute(
            AccountConstants.E_PREVIOUS_SESSION, lastAccess, Element.Disposition.CONTENT);
        response.addAttribute(
            AccountConstants.E_LAST_ACCESS, lastAccess, Element.Disposition.CONTENT);
        response.addAttribute(
            AccountConstants.E_RECENT_MSGS,
    ...
    // we do not have any ldap attrs to define whether pasteitcleaned service is installed and
    // running
    // so check if pasteitcleaned service is installed and running by checking the installed
    // directory and connectivity
    // if yes return pasteitcleanedEnabled = true else return pasteitcleanedEnabled = false
    response.addAttribute(
        "pasteitcleanedEnabled", checkIfPasteitcleanedInstalled(), Element.Disposition.CONTENT);
    ...
    try {
      ...
      for (MailItem item : itemList) {
        CustomMetadata customData =
            item.getCustomData(ModifyProfileImage.IMAGE_CUSTOM_DATA_SECTION);
  ...
  static void doAttrs(Account acct, String locale, Element response, Map<String, Object> attrsMap)
    ...
    for (String key : attrList) {
      ...
      if (Provisioning.A_zimbraLocale.equals(key)) {
        ...
        value =
            config.isAttachmentsBlocked() || acct.isAttachmentsBlocked()
  ...
  protected void doChildAccounts(Element response, Account acct, AuthToken authToken)
      throws ServiceException {
  ...
  private void doDiscoverRights(Element eRights, Account account, Set<Right> rights)
      throws ServiceException {
