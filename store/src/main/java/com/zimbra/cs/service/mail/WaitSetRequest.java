...
public class WaitSetRequest extends MailDocumentHandler {
  ...
  static {
    DEFAULT_TIMEOUT =
        LC.zimbra_waitset_default_request_timeout.longValueWithinRange(
            1, Constants.SECONDS_PER_DAY);
    MIN_TIMEOUT =
        LC.zimbra_waitset_min_request_timeout.longValueWithinRange(1, Constants.SECONDS_PER_DAY);
    MAX_TIMEOUT =
        LC.zimbra_waitset_max_request_timeout.longValueWithinRange(1, Constants.SECONDS_PER_DAY);

    DEFAULT_ADMIN_TIMEOUT =
        LC.zimbra_admin_waitset_default_request_timeout.longValueWithinRange(
            1, Constants.SECONDS_PER_DAY);
    MIN_ADMIN_TIMEOUT =
        LC.zimbra_admin_waitset_min_request_timeout.longValueWithinRange(
            1, Constants.SECONDS_PER_DAY);
    MAX_ADMIN_TIMEOUT =
        LC.zimbra_admin_waitset_max_request_timeout.longValueWithinRange(
            1, Constants.SECONDS_PER_DAY);

    INITIAL_SLEEP_TIME_MILLIS =
        LC.zimbra_waitset_initial_sleep_time.longValueWithinRange(
            1, 5 * Constants.SECONDS_PER_MINUTE * 1000);
    NODATA_SLEEP_TIME_MILLIS =
        LC.zimbra_waitset_nodata_sleep_time.longValueWithinRange(
            1, 5 * Constants.SECONDS_PER_MINUTE * 1000);
  }

  public static long getTimeoutMillis(Element request, boolean isAdminRequest)
  ...
  public static long getTimeoutMillis(Long timeout, boolean isAdminRequest)
    ...
    if (!isAdminRequest) {
      ...
      if (to < MIN_TIMEOUT) to = MIN_TIMEOUT;
      if (to > MAX_TIMEOUT) to = MAX_TIMEOUT;
    } else {
      to = (timeout != null) ? timeout : DEFAULT_ADMIN_TIMEOUT;
      if (to < MIN_ADMIN_TIMEOUT) to = MIN_ADMIN_TIMEOUT;
      if (to > MAX_ADMIN_TIMEOUT) to = MAX_ADMIN_TIMEOUT;
    }
    return to * 1000;
  }
  ...
  public static void staticHandle(
      WaitSetReq req, Map<String, Object> context, WaitSetResp resp, boolean adminAllowed)
      throws ServiceException {
    ZimbraSoapContext zsc = getZimbraSoapContext(context);
    ...
    if (cb == null) { // Initial
      ...
      if (waitSetId.startsWith(WaitSetMgr.ALL_ACCOUNTS_ID_PREFIX)) {
        ...
        Set<MailItem.Type> defaultInterests =
            WaitSetRequest.parseInterestStr(defInterestStr, EnumSet.noneOf(MailItem.Type.class));
        cb.ws =
            WaitSetMgr.lookupOrCreateForAllAccts(
                zsc.getRequestedAccountId(), waitSetId, defaultInterests, lastKnownSeqNo);
      } else {
        cb.ws = WaitSetMgr.lookup(waitSetId);
      }

      if (cb.ws == null) throw AdminServiceException.NO_SUCH_WAITSET(waitSetId);

      WaitSetMgr.checkRightForOwnerAccount(cb.ws, zsc.getRequestedAccountId());

      List<WaitSetAccount> add =
          parseAddUpdateAccounts(zsc, req.getAddAccounts(), cb.ws.getDefaultInterest());
      ...
      // Force the client to wait briefly before processing -- this will stop 'bad' clients from
      // polling
  ...
  /**
   * No data after initial check...wait up to a few extra seconds before going into the notification
   * wait...basically we're just trying to let the server coalesce notification data a little bit.
   */
  ...
  private static void processCallback(
      WaitSetResp resp, WaitSetCallback cb, String waitSetId, String lastKnownSeqNo, boolean expand)
      throws ServiceException {
    ...
    if (cb.canceled) {
      ...
      for (String signalledAccount : cb.signalledAccounts) {
        ...
        if (signalledSession != null) {
          ...
          Mailbox mbox =
              MailboxManager.getInstance().getMailboxByAccountId(signalledAccount, false);
          if (mbox != null) {
            lastChangeId = mbox.getLastChangeID();
          }
        }
        AccountWithModifications info =
            new AccountWithModifications(signalledAccount, lastChangeId);
    ...
    for (WaitSetAccount acct : accts) {
      try {
        MailboxManager.getInstance()
            .getMailboxByAccountId(acct.getAccountId(), MailboxManager.FetchMode.AUTOCREATE);
  ...
  protected static List<WaitSetAccount> parseAddUpdateAccounts(
    ...
    if (accountDetails != null) {
      for (WaitSetAddSpec accountDetail : accountDetails) {
        ...
        Set<MailItem.Type> interests =
            parseInterestStr(accountDetail.getInterests(), defaultInterest);
  ...
  public static final Set<MailItem.Type> parseInterestStr(
      String typesList, Set<MailItem.Type> defaultInterest) {
    if (typesList == null) {
