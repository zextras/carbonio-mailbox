...
public class CreateWaitSet extends MailDocumentHandler {
  ...
  */
  ...
  public static void staticHandle(
      ...
      CreateWaitSetReq request,
      Map<String, Object> context,
      CreateWaitSetResp response)
    ...
    List<WaitSetAccount> add =
        WaitSetRequest.parseAddUpdateAccounts(zsc, request.getAccounts(), defaultInterests);
    ...
    for (WaitSetAccount acct : add) {
      try {
        ...
        Mailbox mbox =
            MailboxManager.getInstance().getMailboxByAccountId(acct.getAccountId(), fetchMode);
    ...
    Pair<String, List<WaitSetError>> result =
        WaitSetMgr.create(
            zsc.getRequestedAccountId(), adminAllowed, defaultInterests, allAccts, add);
