...
public class GetFreeBusy extends MailDocumentHandler {
  ...
  /**
   * FreeBusy requests need to always return valid looking output in able to provide consistent
   * behavior between requests against non-existent accounts and those which don't allow access to
   * the requested data. Returning true here flags GetFreeBusy as accepting responsibility for
   * measures to prevent account harvesting
   */
  @Override
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    String uidParam =
        request.getAttribute(
            MailConstants.A_UID,
            null); // comma-separated list of account emails or zimbraId GUIDs that *must* match
    // UUID format
    String idParam =
        request.getAttribute(
            MailConstants.A_ID, null); // comma-separated list of account zimbraId GUIDs
    String nameParam =
        request.getAttribute(MailConstants.A_NAME, null); // comma-separated list of account emails
    String exApptUid = request.getAttribute(MailConstants.A_APPT_FREEBUSY_EXCLUDE_UID, null);

    Account requestor =
        Provisioning.getInstance().get(Key.AccountBy.id, zc.getAuthtokenAccountId());
    ...
    for (Iterator<Element> usrIter = request.elementIterator(MailConstants.E_FREEBUSY_USER);
        usrIter.hasNext(); ) {
      Element usrElem = usrIter.next();
      int folderId =
          (int) usrElem.getAttributeLong(MailConstants.A_FOLDER, FreeBusyQuery.CALENDAR_FOLDER_ALL);
      ...
      if (id != null) fbQuery.addAccountId(id, folderId);
      String name = usrElem.getAttribute(MailConstants.A_NAME, null);
      if (name != null) fbQuery.addEmailAddress(name, folderId);
    }
