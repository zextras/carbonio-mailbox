  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
      ...
      for (Element mboxEl : mboxElementList) {
        Mailbox mbox =
            MailboxManager.getInstance()
                .getMailboxById((int) mboxEl.getAttributeLong(AdminConstants.A_ID));
    ...
    if (sm instanceof ExternalStoreManager) {
      ...
      for (int mboxId : mailboxIds) {
        ...
        BlobConsistencyChecker.Results results =
            checker.check(null, mboxId, checkSize, reportUsedBlobs);
        if (results.hasInconsistency() || reportUsedBlobs) { // or checking used blobs
          Element mboxEl =
              response
                  .addElement(AdminConstants.E_MAILBOX)
                  .addAttribute(AdminConstants.A_ID, mboxId);
      ...
      for (int mboxId : mailboxIds) {
        ...
        BlobConsistencyChecker.Results results =
            checker.check(volumeIds, mboxId, checkSize, reportUsedBlobs);
