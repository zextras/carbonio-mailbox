...
public class GetAccountMembership extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    defendAgainstAccountHarvesting(
        account, acctBy, accountSelectorKey, zsc, Admin.R_getAccountMembership);
      ...
      try {
        ...
        if (isAdminGroup != null) {
          eDL.addNonUniqueElement(AdminConstants.E_A)
              .addAttribute(AdminConstants.A_N, Provisioning.A_zimbraIsAdminGroup)
              .setText(isAdminGroup);
        }
      } catch (ServiceException e) {
        if (ServiceException.PERM_DENIED.equals(e.getCode())) {
          ZimbraLog.acl.warn(
              "no permission to view %s of dl %s",
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        "If the authed admin has get attr right on  distribution list attr "
            ...
            + ", it is returned in the response if set.");
  }
}
