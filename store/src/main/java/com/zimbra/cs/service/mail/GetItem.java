...
public class GetItem extends MailDocumentHandler {
  ...
  private static final String[] TARGET_FOLDER_PATH =
      new String[] {MailConstants.E_ITEM, MailConstants.A_FOLDER};
  ...
  protected boolean checkMountpointProxy(Element request) {
    return getXPath(request, TARGET_ITEM_PATH) == null;
  }
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    String path =
        target.getAttribute(MailConstants.A_PATH, target.getAttribute(MailConstants.A_NAME, null));

    if (folderStr != null && path == null)
      throw ServiceException.INVALID_REQUEST(
          "missing required attribute: " + MailConstants.A_PATH, null);
    ...
    if (id != null) {
      ...
      int folderId =
          folderStr == null ? Mailbox.ID_FOLDER_USER_ROOT : new ItemId(folderStr, zsc).getId();
      try {
        ...
        if (match.getFirst() instanceof Mountpoint) {
          ...
          target
              .addAttribute(MailConstants.A_FOLDER, mpt.getRemoteId())
              .addAttribute(MailConstants.A_PATH, match.getSecond());
