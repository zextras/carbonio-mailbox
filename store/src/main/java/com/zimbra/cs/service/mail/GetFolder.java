...
public class GetFolder extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (eFolder != null) {
      ...
      if (path != null) {
        ...
        if (overflow == null) {
          ...
          eFolder
              .addAttribute(MailConstants.A_FOLDER, iidTarget.toString())
              .addAttribute(MailConstants.A_PATH, overflow);
    ...
    if (rootnode != null) {
      ...
      Element folderRoot =
          encodeFolderNode(rootnode, response, ifmt, octxt, true, depth, view, traverse, mounts);
  ...
  public static Element encodeFolderNode(
      FolderNode node, Element parent, ItemIdFormatter ifmt, OperationContext octxt)
      throws ServiceException {
    return encodeFolderNode(node, parent, ifmt, octxt, false, -1, null, false, null);
  }

  private static Element encodeFolderNode(
      FolderNode node,
      Element parent,
      ...
      MailItem.Type view,
      boolean traverse,
      List<ExpandableMountpoint> mounts)
      throws ServiceException {
    Element eFolder;
    Folder folder = node.mFolder;
    if (folder != null) {
      eFolder =
          ToXML.encodeFolder(parent, ifmt, octxt, folder, ToXML.NOTIFY_FIELDS, exposeAclAccessKey);
      // if requested, fetch contents of mountpoints
      if (traverse
          && mounts != null
          && folder instanceof Mountpoint
          && folder.getMailbox().hasFullAccess(octxt)) {
        mounts.add(new ExpandableMountpoint(eFolder, (Mountpoint) folder, depth));
      }
    } else {
      eFolder =
          parent
              .addNonUniqueElement(MailConstants.E_FOLDER)
              .addAttribute(MailConstants.A_ID, ifmt.formatItemId(node.mId))
              .addAttribute(MailConstants.A_NAME, node.mName);
    }
  ...
  private void expandMountpoint(
      ExpandableMountpoint empt, Element origRequest, Map<String, Object> context)
      throws ServiceException {
    ...
    request
        .addNonUniqueElement(MailConstants.E_FOLDER)
        .addAttribute(MailConstants.A_FOLDER, iidRemote.toString());
  ...
  private FolderNode filterByView(FolderNode node, MailItem.Type viewFilter) {
    ...
    if (node.mFolder != null
        && node.mId != Mailbox.ID_FOLDER_TRASH
        && node.mFolder.getDefaultView() != viewFilter) {
      node.mFolder = null;
    }
