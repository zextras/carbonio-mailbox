...
public class ModifyAdminSavedSearches extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Iterator it = request.elementIterator(AdminConstants.E_SEARCH); it.hasNext(); ) {
      ...
      if (name != null && name.length() != 0) searches.put(name, query);
      else ZimbraLog.account.warn("ModifyAdminSavedSearches: empty search name ignored");
    }
  ...
  public void handle(Account acct, Element response, HashMap<String, String> modSearches)
      throws ServiceException {
    String[] searches = acct.getMultiAttr(Provisioning.A_zimbraAdminSavedSearches);
    Map<String, GetAdminSavedSearches.AdminSearch> curSearches =
        new HashMap<String, GetAdminSavedSearches.AdminSearch>();
    ...
    for (Iterator it = modSearches.entrySet().iterator(); it.hasNext(); ) {
      ...
      if (curSearches.containsKey(name)) {
        if (query.length() == 0) curSearches.remove(name);
        else curSearches.put(name, mod);
      } else {
        if (query.length() == 0)
