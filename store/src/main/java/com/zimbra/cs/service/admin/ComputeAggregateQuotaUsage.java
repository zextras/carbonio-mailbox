...
public class ComputeAggregateQuotaUsage extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    ExecutorService executor =
        Executors.newFixedThreadPool(LC.compute_aggregate_quota_threads.intValue());
    List<Future<Map<String, Long>>> futures = new LinkedList<Future<Map<String, Long>>>();
    for (final Server server : servers) {
      futures.add(
          executor.submit(
              new Callable<Map<String, Long>>() {
                ...
                public Map<String, Long> call() throws Exception {
                  ...
                  Element req =
                      new Element.XMLElement(AdminConstants.GET_AGGR_QUOTA_USAGE_ON_SERVER_REQUEST);
                  ...
                  List<Element> domainElts =
                      resp.getPathElementList(new String[] {AdminConstants.E_DOMAIN});
                  Map<String, Long> retMap = new HashMap<String, Long>();
                  for (Element domainElt : domainElts) {
                    retMap.put(
                        domainElt.getAttribute(AdminConstants.A_ID),
        ...
        new Runnable() {
          ...
          public void run() {
            try {
              ...
              out.setFrom(
                  new JavaMailInternetAddress("Postmaster <postmaster@" + domain.getName() + ">"));

              for (String recipient : recipients) {
                out.setRecipient(
                    javax.mail.Message.RecipientType.TO, new JavaMailInternetAddress(recipient));
              }
              ...
              out.setSubject(
                  L10nUtil.getMessage(L10nUtil.MsgKey.domainAggrQuotaWarnMsgSubject, locale));

              out.setText(
                  L10nUtil.getMessage(
                      L10nUtil.MsgKey.domainAggrQuotaWarnMsgBody,
                      locale,
                      domain.getName(),
              ...
              ZimbraLog.misc.warn(
                  "Error in sending aggregate quota warning msg for domain %s",
                  domain.getName(), e);
            }
          }
        });
