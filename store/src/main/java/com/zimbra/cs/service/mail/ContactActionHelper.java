...
public class ContactActionHelper extends ItemActionHelper {
  ...
  public static ContactActionHelper UPDATE(
    ...
    ContactActionHelper ca =
        new ContactActionHelper(octxt, mbox, zsc.getResponseProtocol(), ids, Op.UPDATE);
  ...
  ContactActionHelper(
      OperationContext octxt, Mailbox mbox, SoapProtocol responseProto, List<Integer> ids, Op op)
      throws ServiceException {
    ...
    switch (mOperation) {
      case UPDATE:
        ...
        if (mTags != null || mFlags != null) {
          getMailbox()
              .setTags(
                  getOpCtxt(), itemIds, type, Flag.toBitmask(mFlags), mTags, mTargetConstraint);
        }
        if (mColor != null) {
