 ...
 *
 * <p>After successfully reloading a new local config file, subsequent {@link LC#get(String)} calls
 * should receive new value. However, if you store/ cache those values (e.g. keep them as class
 * member or instance member), new values are of course not reflected.
 *
 * @author ysasaki
 */
  ...
  private void reloadImapDaemonLC(Server server) {
    ...
    try {
      ZimbraLog.imap.debug(
          "issuing RELOADLC request to imapd server '%s'", server.getServiceHostname());
      connection.reloadLocalConfig();
    } catch (IOException e) {
      ZimbraLog.imap.warn(
          "unable to issue RELOADLC request to IMAP server '%s'", server.getServiceHostname(), e);
    } finally {
      connection.close();
    }
  }
