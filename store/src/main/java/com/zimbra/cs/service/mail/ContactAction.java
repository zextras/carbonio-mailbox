...
public class ContactAction extends ItemAction {
  ...
  private ItemActionResult handleContact(
      Map<String, Object> context, Element request, String operation)
      throws ServiceException, SoapFaultException {
    Element action = request.getElement(MailConstants.E_ACTION);
    ...
    if (!local.isEmpty()) {
      ...
      if (operation.equals(OP_UPDATE)) {
        ...
        if (!action.listElements(MailConstants.E_ATTRIBUTE).isEmpty()) {
          ...
          Pair<Map<String, Object>, List<Attachment>> cdata =
              CreateContact.parseContact(action, zsc, octxt, cn);
          pc = new ParsedContact(cdata.getFirst(), cdata.getSecond());
        }

        localResults =
            ContactActionHelper.UPDATE(zsc, octxt, mbox, local, iidFolder, flags, tags, color, pc)
                .getResult();
      } else {
