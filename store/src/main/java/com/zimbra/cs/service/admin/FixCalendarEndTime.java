...
public class FixCalendarEndTime extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (sync) {
      ...
      CalendarEndTimeFixupThread thread = new CalendarEndTimeFixupThread(acctNames);
      thread.start();
    }
    ...
    for (NamedEntry entry : accts) {
      ...
      try {
        ...
        ZimbraLog.calendar.error(
            "Error fixing calendar item end times in mailbox "
                + mbox.getId()
                + ": "
                + e.getMessage(),
            e);
      ...
      if (numFixedAccts % every == 0) {
        ZimbraLog.calendar.info(
            "Progress: fixed calendar item end times in "
                + numFixedAccts
                + "/"
                + numAccts
                + " accounts");
  ...
  private static class CalendarEndTimeFixupThread extends Thread {
    ...
    public void run() {
      try {
        ...
        ZimbraLog.calendar.error("Error while fixing up calendar end times: " + e.getMessage(), e);
      }
    }
  }
