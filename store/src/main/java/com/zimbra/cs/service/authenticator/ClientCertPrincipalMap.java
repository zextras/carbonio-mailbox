...
public class ClientCertPrincipalMap {
  ...
  static class SubjectCertField extends CertField {
    ...
    private static final SubjectCertField EMAILADDRESS =
        new SubjectCertField(CertUtil.ATTR_EMAILADDRESS);
  ...
  static class LdapFilterRule extends Rule {
    ...
    ZimbraPrincipal apply(X509Certificate cert) throws ServiceException {
      ...
      ZimbraLog.account.debug(
          LOG_PREFIX
              + "search account by expanded filter(prepended with account objectClass filter): "
              + filter);
  ...
  static class FieldMapRule extends Rule {
    ...
    private Account getZimbraAccount(
        ZimbraKey zimbraKey, CertField certField, String certFieldValue) {
      ZimbraLog.account.debug(
          LOG_PREFIX
              + "get account by "
              + zimbraKey.name()
  ...
  private Rule parseFieldMapRule(String rawRule) throws ServiceException {
    ...
    if (parts.length != 2) {
      throw ServiceException.FAILURE(
          "Invalid config:" + rawRule + " in " + Provisioning.A_zimbraMailSSLClientCertPrincipalMap,
          null);
    }
