...
public class AutoComplete extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    GalSearchType type =
        GalSearchType.fromString(request.getAttribute(MailConstants.A_TYPE, "account"));
    ...
    AutoCompleteResult result =
        query(request, zsc, account, false, name, limit, type, needCanExpand, octxt);
  ...
  protected AutoCompleteResult query(
      Element request,
  ...
  protected AutoCompleteResult query(
    ...
    boolean includeGal =
        !excludeGal
            && request.getAttributeBool(MailConstants.A_INCLUDE_GAL, autoComplete.includeGal());
  ...
  private String getType(ContactEntry entry) {
    if (entry.getFolderId() == ContactAutoComplete.FOLDER_ID_GAL) return "gal";
    else if (entry.getFolderId() == ContactAutoComplete.FOLDER_ID_UNKNOWN) return "rankingTable";
    else return "contact";
  }
}
