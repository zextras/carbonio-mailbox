...
public class GetMiniCal extends CalendarRequest {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Iterator<Element> foldersIter = request.elementIterator(MailConstants.E_FOLDER);
        foldersIter.hasNext(); ) {
    ...
    Map<ItemId /* resolved iid */, ItemId /* requested iid */> reverseMap =
        new HashMap<ItemId, ItemId>();
    for (Map.Entry<ItemId, Resolved> entry : resolved.entrySet()) {
      ...
      if (res.error == null) {
        ...
        addError(
            response, ifmt.formatItemId(requestedIid), res.error.getCode(), res.error.getMessage());
      }
    }
        ...
        for (Iterator<Map.Entry<String, List<Integer>>> acctIter =
          ...
          for (Iterator<Integer> iterFolderId = folderIds.iterator(); iterFolderId.hasNext(); ) {
            ...
            try {
              ...
              ItemId reqIid =
                  reverseMap.get(iid); // Error must mention folder id requested by client.
              if (reqIid != null) {
                ZimbraLog.calendar.warn(
                    "Error accessing calendar folder " + ifmt.formatItemId(reqIid), e);
                addError(response, ifmt.formatItemId(reqIid), e.getCode(), e.getMessage());
              } else {
                ZimbraLog.calendar.warn(
                    "Error accessing calendar folder; resolved id="
                        + ifmt.formatItemId(iid)
    ...
    for (Map.Entry<Server, Map<String, List<Integer>>> serverMapEntry :
      ...
      if (server.equals(localServer)) { // local server
        for (Map.Entry<String, List<Integer>> entry : accountFolders.entrySet()) {
          ...
          if (targetAcct == null) {
            ZimbraLog.calendar.warn(
                "Skipping unknown account " + acctId + " during minical search");
            continue;
          }
        ...
        String nominalTargetAcctId =
            null; // mail service soap requests want to see a target account
  ...
  private static void addBusyDates(
      Calendar cal, CalendarData calData, long rangeStart, long rangeEnd, Set<String> busyDates)
      throws ServiceException {
    for (Iterator<CalendarItemData> itemIter = calData.calendarItemIterator();
        itemIter.hasNext(); ) {
      ...
      for (Iterator<InstanceData> instIter = item.instanceIterator(); instIter.hasNext(); ) {
        ...
        if (partStat == null) partStat = item.getDefaultData().getPartStat();
        if (IcalXmlStrMap.PARTSTAT_DECLINED.equals(partStat)) continue;
  ...
  private static void doLocalFolder(
      ...
      long rangeStart,
      long rangeEnd,
      Set<String> busyDates)
      throws ServiceException {
    Calendar cal = new GregorianCalendar(tz);
    CalendarDataResult result =
        mbox.getCalendarSummaryForRange(
  ...
  private static void doRemoteFolders(
      ...
      Set<String> busyDates,
      Element response,
      Map<ItemId, ItemId> reverseIidMap,
      ItemIdFormatter ifmt) {
    try {
      Account target = Provisioning.getInstance().get(Key.AccountBy.id, remoteAccountId);
      if (target == null) throw AccountServiceException.NO_SUCH_ACCOUNT(remoteAccountId);
      AuthToken authToken = zsc.getAuthToken();
      ZMailbox.Options zoptions =
          new ZMailbox.Options(authToken.toZAuthToken(), AccountUtil.getSoapUri(target));
      ...
      if (errors != null) {
        for (ZMiniCalError error : errors) {
          try {
            ...
            ItemId reqIid =
                reverseIidMap.get(iid); // Error must mention folder id requested by client.
  ...
  private static ICalTimeZone parseTimeZone(Element request) throws ServiceException {
    ...
    if (tzElem != null) {
      ...
      if (tzid != null) {
        ...
        if (knownTZ != null) return knownTZ;
      }
      ...
      if (stdOffset == null)
        throw ServiceException.INVALID_REQUEST(
            "Unknown TZ: \""
                + tzid
                + "\" and no "
                + MailConstants.A_CAL_TZ_STDOFFSET
                + " specified",
            null);
  ...
  // Resolve mountpoints for each requested folder.  Resolution can result in error per folder.
  // Typical errors
  // include PERM_DENIED (if sharer revoked permission), NO_SUCH_FOLDER (if sharer deleted shared
  // folder), and
  // NO_SUCH_ACCOUNT (if sharer account has been deleted).
  private static Map<ItemId, Resolved> resolveMountpoints(
    ...
    for (ItemId iidFolder : folderIids) {
      ...
      try {
        ...
        ZimbraLog.calendar.warn(
            "Error resolving calendar folder " + ifmt.formatItemId(iidFolder), e);
  ...
  private static Map<String /* account id */, List<Integer> /* folder ids */> groupFoldersByAccount(
      Map<ItemId, Resolved> map) {
