...
public class CreateCalendarItem extends CalendarRequest {
  ...
  private static final String[] TARGET_FOLDER_PATH =
      new String[] {MailConstants.E_MSG, MailConstants.A_FOLDER};
  ...
  protected class CreateCalendarItemInviteParser extends ParseMimeMessage.InviteParser {
    public ParseMimeMessage.InviteParserResult parseInviteElement(
      ...
      return CalendarUtils.parseInviteForCreate(
          account, getItemType(), inviteElem, null, null, false, CalendarUtils.RECUR_ALLOWED);
    }
  }
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    // Don't allow creating in Trash folder/subfolder.  We don't want to invite attendees to an
    // appointment in trash.
    ...
    if (!dat.mInvite.hasRecurId())
      ZimbraLog.calendar.info(
          "<CreateCalendarItem> folderId=%d, subject=\"%s\", UID=%s",
          iidFolder.getId(),
          dat.mInvite.isPublic() ? dat.mInvite.getName() : "(private)",
          dat.mInvite.getUid());
    else
      ZimbraLog.calendar.info(
          "<CreateCalendarItem> folderId=%d, subject=\"%s\", UID=%s, recurId=%s",
          iidFolder.getId(),
          dat.mInvite.isPublic() ? dat.mInvite.getName() : "(private)",
          dat.mInvite.getUid(),
