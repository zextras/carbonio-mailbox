...
public class FixCalendarPriority extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (sync) {
      ...
      CalendarPriorityFixupThread thread = new CalendarPriorityFixupThread(acctNames);
      thread.start();
    }
    ...
    for (NamedEntry entry : accts) {
      ...
      try {
        ...
        ZimbraLog.calendar.error(
            "Error fixing calendar item priority in mailbox "
                + mbox.getId()
                + ": "
                + e.getMessage(),
            e);
      ...
      if (numFixedAccts % every == 0) {
        ZimbraLog.calendar.info(
            "Progress: fixed calendar item priority in "
                + numFixedAccts
                + "/"
                + numAccts
                + " accounts");
  ...
  private static class CalendarPriorityFixupThread extends Thread {
    ...
    public void run() {
      try {
        ...
        ZimbraLog.calendar.error("Error while fixing up calendar priority: " + e.getMessage(), e);
      }
    }
  }
