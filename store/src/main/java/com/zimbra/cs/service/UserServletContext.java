...
public class UserServletContext {
  ...
  public UserServletContext(
      HttpServletRequest request, HttpServletResponse response, UserServlet srvlt)
  ...
  protected void parseParams(HttpServletRequest request, AuthToken authToken)
    ...
    if (pathInfo == null
      ...
      throw new UserServletException(
          HttpServletResponse.SC_BAD_REQUEST, L10nUtil.getMessage(MsgKey.errInvalidPath, request));
    ...
    try {
      ...
      throw new UserServletException(
          HttpServletResponse.SC_BAD_REQUEST, L10nUtil.getMessage(MsgKey.errInvalidId, request));
    }
  ...
  public long getStartTime() {
    if (mStartTime == -2) {
      ...
      mStartTime =
          (st != null) ? DateUtil.parseDateSpecifier(st, defaultStartTime) : defaultStartTime;
    }
    return mStartTime;
  }
  ...
  /**
   * Returns <tt>true</tt> if {@link UserServlet#QP_BODY} is not specified or is set to a non-zero
   * value.
   */
  public boolean shouldReturnBody() {
    String bodyVal = params.get(UserServlet.QP_BODY);
    if (bodyVal != null && bodyVal.equals("0")) return false;
    return true;
  }
  ...
  /**
   * Returns the maximum width of the image returned by this request, or {@code null} if the max
   * with is not specified or invalid.
   */
  ...
  /**
   * Returns the maximum height of the image returned by this request, or {@code null} if the max
   * height is not specified or invalid.
   */
  ...
  public InputStream getRequestInputStream(long limit)
    ...
    if (ServletFileUpload.isMultipartContent(req)) {
      ...
      is =
          new UploadInputStream(
              contentEncoding != null && contentEncoding.indexOf("gzip") != -1
                  ? new GZIPInputStream(req.getInputStream())
                  : req.getInputStream(),
              limit);
    }
    if (filename == null || filename.trim().equals("")) filename = "unknown";
    else params.put(UserServlet.UPLOAD_NAME, filename);
    params.put(UserServlet.UPLOAD_TYPE, contentType);
    ZimbraLog.mailbox.info(
        "UserServlet received file %s - %d request bytes", filename, req.getContentLength());
    return is;
  }
