...
public class GetQuotaUsage extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (isDomainAdminOnly(zsc)) {
      ...
      if (domain == null) domain = getAuthTokenAccountDomain(zsc).getName();

      // sanity check
      if (domain == null) throw AccountServiceException.INVALID_REQUEST("no domain", null);
    }
    ...
    if (d != null) checkDomainRight(zsc, d, Admin.R_getDomainQuotaUsage);
    else checkRight(zsc, null, AdminRight.PR_SYSTEM_ADMIN_ONLY);
    ...
    if (quotas == null) {
      if (allServers) {
        quotas =
            delegateRequestToAllServers(
                request.clone(), zsc.getRawAuthToken(), sortBy, sortAscending, prov);
  ...
  private List<AccountQuota> delegateRequestToAllServers(
    ...
    for (final Server server : servers) {
      futures.add(
          executor.submit(
              new Callable<List<AccountQuota>>() {
                @Override
                public List<AccountQuota> call() throws Exception {
                  ...
                  for (Element accountElt : accountElts) {
                    ...
                    quota.percentQuotaUsed =
                        quota.quotaLimit > 0 ? (quota.quotaUsed / (float) quota.quotaLimit) : 0;
                    retList.add(quota);
                  }
                  return retList;
                }
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        "If a domain is specified, need the the domain right "
            + Admin.R_getDomainQuotaUsage.getName()
