...
public class DeleteCalendarResource extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    defendAgainstCalResourceHarvesting(
        resource, CalendarResourceBy.id, id, zsc, Admin.R_deleteCalendarResource);

    if (!Provisioning.getInstance().onLocalServer(resource)) {
    ...
    ZimbraLog.security.info(
        ZimbraLog.encodeAttrs(
            new String[] {
              "cmd", "DeleteCalendarResource", "name", resource.getName(), "id", resource.getId()
            }));
