...
public class OctopusPatchFormatter extends Formatter {
  ...
  OctopusPatchFormatter() {
    ...
    BlobStore blobStore =
        new StoreManagerBasedTempBlobStore(
            StoreManager.getInstance(), incomingPatchExpiration, storedPatchExpiration, true);
  ...
  public void saveCallback(
    ...
    try {
      ...
      log.debug(
          "Creating parsed document; filename="
              + filename
              + ", contentType="
              + contentType
              + ", creator="
      ...
      if (item == null) {
        ...
        item =
            mbox.createDocument(context.opContext, folder.getId(), pd, MailItem.Type.DOCUMENT, 0);
      } else {
        log.debug(
            "Creating new version of the document "
                + filename
                + ", current version: "
                + item.getVersion());
      ...
      throw new UserServletException(
          HttpServletResponse.SC_CONFLICT, "patch cannot be applied, try uploading whole file", e);
    } finally {
      try {
        pis.close();
      } catch (Exception e) {
  ...
  public void formatCallback(UserServletContext context)
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Request received for patch for "
              + doc.getName()
              + ", id: "
              + doc.getId()
    ...
    if (sp != null) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "Patch not available for "
                + doc.getName()
                + ", id: "
                + doc.getId()
                + ", version: "
                + version
                + "; will return the entire file");
      }
      sendFullFile(context, doc, version);
    }
  }

  private void sendPatch(UserServletContext context, Document doc, int version, StoredPatch sp)
    ...
    final boolean sendManifest =
        (manifestParam != null) ? (Integer.parseInt(manifestParam) > 0) : true;
    ...
    try {
      ...
      log.warn(
          "Cannot access patch for "
              + doc.getName()
              + ", id: "
              + doc.getId()
              + ", version: "
              + version
              + "; will return the entire file (failure: "
              + e
              + ")");
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Sending patch for " + doc.getName() + ", id: " + doc.getId() + ", version: " + version);
    }
