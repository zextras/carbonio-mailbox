...
public class CheckRecurConflicts extends ExpandRecur {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (rangeStart == 0) rangeStart = System.currentTimeMillis();
    long rangeEnd = request.getAttributeLong(MailConstants.A_CAL_END_TIME, 0);
    if (rangeEnd == 0) rangeEnd = Long.MAX_VALUE;
    ...
    if (instances == null || instances.isEmpty()) return response;
    ...
    if (rangeStartActual >= rangeEndActual) return response;
    ...
    for (Instance inst : instances) {
      ...
      if (numConflicts == 0 && !allInstances) instElem.detach();
    }
    return response;
  }
  ...
  private static UserConflicts getConflicts(FreeBusy fb, List<Instance> instances)
    ...
    // f/b status of the previous overlap between instance and interval, if previous and current
    // overlaps
    ...
    while (ival != null && inst != null) {
      ...
      // We have an overlap!
      if (prevStatus != null) status = FreeBusy.chooseBusier(status, prevStatus);
