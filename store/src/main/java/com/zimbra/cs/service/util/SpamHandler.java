...
public class SpamHandler {
  ...
  private void sendReport(SpamReport sr) throws ServiceException, MessagingException {
    ...
    String isSpamString =
        sr.isSpam ? config.getSpamReportTypeSpam() : config.getSpamReportTypeHam();
    ...
    String body =
        String.format(
            ...
            Strings.nullToEmpty(sr.accountName),
            isSpamString,
            Strings.nullToEmpty(sr.action),
            Strings.nullToEmpty(sr.sourceFolder),
            Strings.nullToEmpty(sr.destFolder),
    ...
    out.setSubject(
        config.getSpamTrainingSubjectPrefix() + " " + sr.accountName + ": " + isSpamString);
    Transport.send(out);
  ...
  public static final class SpamReport {
    ...
    public String toString() {
      return MoreObjects.toStringHelper(this)
          ...
          .add("reportRecipient", reportRecipient)
          .toString();
    }
  }
    ...
    synchronized (spamReportQueueLock) {
      for (SpamReport report : reports) {
        if (spamReportQueue.size() > spamReportQueueSize) {
          ZimbraLog.misc.warn(
              "SpamHandler queue size " + spamReportQueue.size() + " too large, ignored " + report);
          continue;
        }
  ...
  public void handle(
      OperationContext octxt, Mailbox mbox, int itemId, MailItem.Type type, SpamReport report)
    ...
    switch (type) {
      ...
      default:
        ZimbraLog.misc.warn(
            "SpamHandler called on unhandled item type="
                + type
                + " account="
                + report.accountName
                + " id="
  ...
  /**
   * Stores the last known value of <tt>zimbraSpamHeaderValue</tt>. Used for determining whether
   * {@link #spamPattern} needs to be recompiled.
   */
  ...
  /**
   * Stores the last known value of <tt>zimbraSpamWhitelistHeaderValue</tt>. Used for determining
   * whether {@link #whitelistPattern} needs to be recompiled.
   */
   ...
   *
   * <p>If <tt>zimbraSpamWhitelistHeader</tt> does not match, returns <tt>true</tt> if the value of
   * the header named <tt>zimbraSpamHeader</tt> matches the pattern specified by
   * <tt>zimbraSpamHeaderValue</tt>.
   */
