...
public class GetAllAccounts extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (s != null) {
      ...
      if (key.equals(BY_NAME)) server = prov.get(Key.ServerBy.name, value);
      else if (key.equals(BY_ID)) server = prov.get(Key.ServerBy.id, value);
      else throw ServiceException.INVALID_REQUEST("unknown value for server by: " + key, null);

      if (server == null) throw AccountServiceException.NO_SUCH_SERVER(value);
    }
    ...
    if (d != null || isDomainAdminOnly(zsc)) {
      ...
      if (domain == null) throw AccountServiceException.NO_SUCH_DOMAIN(value);
