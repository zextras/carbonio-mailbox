...
public class RssFormatter extends Formatter {
  ...
  public void formatCallback(UserServletContext context) throws IOException, ServiceException {
    ...
    try {
      iterator =
          getMailItems(context, context.getStartTime(), context.getEndTime(), limit - offset);
      ...
      channel
          ...
          .setText("Zimbra item " + context.itemPath + " in RSS format.");
  ...
  private void addCalendarItem(CalendarItem calItem, Element channel, UserServletContext context)
      throws ServiceException {
    Collection<Instance> instances =
        calItem.expandInstances(context.getStartTime(), context.getEndTime(), false);
    for (Iterator<Instance> instIt = instances.iterator(); instIt.hasNext(); ) {
      ...
      */
      ...
      if (inv.hasOrganizer()) rssItem.addElement("author").setText(inv.getOrganizer().getAddress());
    }
  }
  ...
  private void addDocument(Document doc, Element channel, UserServletContext context) {
    ...
    item.addElement("link")
        .setText(context.req.getRequestURL().append("?id=" + doc.getId()).toString());
  }
