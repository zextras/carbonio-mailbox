...
public class ItemAction extends MailDocumentHandler {
  ...
  protected static final String[] OPERATION_PATH =
      new String[] {MailConstants.E_ACTION, MailConstants.A_OPERATION};
  protected static final String[] TARGET_ITEM_PATH =
      new String[] {MailConstants.E_ACTION, MailConstants.A_ID};
  ...
  public static final String OP_RECOVER =
      MailConstants.OP_RECOVER; // recover by copying then deleting from dumpster
  public static final String OP_DUMPSTER_DELETE =
      MailConstants.OP_DUMPSTER_DELETE; // delete from dumpster
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    boolean nonExistentIdsRequested =
        action.getAttributeBool(MailConstants.A_NON_EXISTENT_IDS, false);
    boolean newlyCreatedIdsRequested =
        action.getAttributeBool(MailConstants.A_NEWLY_CREATED_IDS, false);
    ...
    if (opStr.equals(MailConstants.OP_HARD_DELETE) && nonExistentIdsRequested) {
      ...
      act.addAttribute(
          MailConstants.A_NON_EXISTENT_IDS, Joiner.on(",").join(daResult.getNonExistentIds()));
    }
    if (newlyCreatedIdsRequested && opStr.equals(MailConstants.OP_COPY)) {
      ...
      if (!Strings.isNullOrEmpty(createdIds)) {
        act.addAttribute(
            MailConstants.A_NEWLY_CREATED_IDS, Joiner.on(",").join(caResult.getCreatedIds()));
      }
    }
    return response;
  }
  ...
  protected ItemActionResult handleCommon(
      Map<String, Object> context, Element request, MailItem.Type type) throws ServiceException {
    Element action = request.getElement(MailConstants.E_ACTION);
    ZimbraSoapContext zsc = getZimbraSoapContext(context);
    ...
    if (!local.isEmpty()) {
      ...
      if (opStr.equals(MailConstants.OP_TAG)) {
        ...
        if (tagName == null) {
          if (action.getAttribute(MailConstants.A_TAG) == null) {
            throw ServiceException.INVALID_REQUEST(
                "missing required attribute: " + MailConstants.A_TAG_NAMES, null);
          }
          tagName =
              TagUtil.tagIdToName(mbox, octxt, (int) action.getAttributeLong(MailConstants.A_TAG));
        }
        localResults =
            ItemActionHelper.TAG(octxt, mbox, responseProto, local, type, tagName, flagValue, tcon)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_FLAG)) {
        localResults =
            ItemActionHelper.FLAG(octxt, mbox, responseProto, local, type, flagValue, tcon)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_PRIORITY)) {
        localResults =
            ItemActionHelper.PRIORITY(octxt, mbox, responseProto, local, type, flagValue, tcon)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_READ)) {
        localResults =
            ItemActionHelper.READ(octxt, mbox, responseProto, local, type, flagValue, tcon)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_COLOR)) {
        Color color = getColor(action);
        localResults =
            ItemActionHelper.COLOR(octxt, mbox, responseProto, local, type, tcon, color)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_HARD_DELETE)) {
        localResults =
            ItemActionHelper.HARD_DELETE(octxt, mbox, responseProto, local, type, tcon).getResult();
      } else if (opStr.equals(MailConstants.OP_RECOVER)) {
        ItemId iidFolder = new ItemId(action.getAttribute(MailConstants.A_FOLDER), zsc);
        localResults =
            ItemActionHelper.RECOVER(octxt, mbox, responseProto, local, type, tcon, iidFolder)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_DUMPSTER_DELETE)) {
        localResults =
            ItemActionHelper.DUMPSTER_DELETE(octxt, mbox, responseProto, local, type, tcon)
                .getResult();
        ...
        localResults =
            handleMoveOperation(
                zsc, octxt, request, action, mbox, responseProto, local, type, tcon);
      } else if (opStr.equals(MailConstants.OP_COPY)) {
        ItemId iidFolder = new ItemId(action.getAttribute(MailConstants.A_FOLDER), zsc);
        localResults =
            ItemActionHelper.COPY(octxt, mbox, responseProto, local, type, tcon, iidFolder)
                .getResult();
      } else if (opStr.equals(MailConstants.OP_SPAM)) {
        String defaultFolder = (flagValue ? Mailbox.ID_FOLDER_SPAM : Mailbox.ID_FOLDER_INBOX) + "";
        ItemId iidFolder =
            new ItemId(action.getAttribute(MailConstants.A_FOLDER, defaultFolder), zsc);
        localResults =
            ItemActionHelper.SPAM(
                    octxt, mbox, responseProto, local, type, flagValue, tcon, iidFolder)
                .getResult();
        ...
        localResults =
            ItemActionHelper.RENAME(octxt, mbox, responseProto, local, type, tcon, name, iidFolder)
                .getResult();
        ...
        localResults =
            ItemActionHelper.LOCK(octxt, mbox, responseProto, local, type, tcon).getResult();
      } else if (opStr.equals(MailConstants.OP_UNLOCK)) {
        localResults =
            ItemActionHelper.UNLOCK(octxt, mbox, responseProto, local, type, tcon).getResult();
      } else if (opStr.equals(MailConstants.OP_INHERIT)) {
        mbox.alterTag(
            octxt, ArrayUtil.toIntArray(local), type, Flag.FlagInfo.NO_INHERIT, false, tcon);
        ...
        localResults =
            ItemActionHelper.TAG(
                    octxt,
                    mbox,
                    responseProto,
                    local,
                    type,
                    Flag.FlagInfo.MUTED.toString(),
                    flagValue,
                    null)
                .getResult();
        if (flagValue) {
          // when marking muted, items are also marked read
          ItemActionHelper.READ(octxt, mbox, responseProto, local, type, flagValue, null)
              .getResult();
        }
  ...
  protected ItemActionResult handleTrashOperation(
    ...
    ItemActionResult localTrashResults =
        ItemActionHelper.MOVE(octxt, mbox, responseProto, localTrashIds, type, tcon, iidTrash)
            .getResult();
    ...
    for (String dataSourceId : remoteTrashIds.keySet()) {
      ...
      ItemActionResult imapTrashResults =
          ItemActionHelper.MOVE(octxt, mbox, responseProto, imapTrashIds, type, tcon, iidImapTrash)
              .getResult();
  ...
  protected ItemActionResult handleMoveOperation(
    ...
    if (acctRelativePath == null) {
      ItemId iidFolder = new ItemId(action.getAttribute(MailConstants.A_FOLDER), zsc);
      localResults =
          ItemActionHelper.MOVE(octxt, mbox, responseProto, local, type, tcon, iidFolder)
              .getResult();
      ...
      throw ServiceException.INVALID_REQUEST(
          MailConstants.A_ACCT_RELATIVE_PATH
              + " is supported only for "
              + MailConstants.E_CONV_ACTION_REQUEST,
          null);
    }
    return localResults;
  }

  private String getDataSourceOfItem(OperationContext octxt, Mailbox mbox, MailItem item)
      throws ServiceException {
    ...
    if (ds.getType() == DataSourceType.imap) {
      ...
      if (folderId < 0) {
        ...
        for (ImapFolder folder : folders) {
          ...
          if (name.equalsIgnoreCase("trash") || name.toLowerCase().endsWith("]/trash")) {
  ...
  private Account forceRemoteSession(
      ZimbraSoapContext zsc,
    ...
    // only have to worry about operations where things can get created in other mailboxes (regular
    // notification works for all other cases)
    if (!op.equals(MailConstants.OP_MOVE)
        && !op.equals(MailConstants.OP_COPY)
        && !op.equals(MailConstants.OP_SPAM)
        && !op.equals(MailConstants.OP_RENAME)
        && !op.equals(MailConstants.OP_UPDATE)) {
      return null;
    }
    ...
    while (hopCount < ZimbraSoapContext.MAX_HOP_COUNT) {
      ...
      if (Provisioning.getInstance().onLocalServer(owner)) {
        ...
        iidFolder =
            new ItemId(((ZMountpoint) zfolder).getCanonicalRemoteId(), zsc.getAuthtokenAccountId());
      }
      hopCount++;
    }
  ...
  static void partitionItems(
      ZimbraSoapContext zsc, String ids, List<Integer> local, Map<String, StringBuilder> remote)
      throws ServiceException {
  ...
  protected ItemActionResult proxyRemoteItems(
      Element action,
      Map<String, StringBuilder> remote,
      Element request,
      Map<String, Object> context)
      throws ServiceException {
    boolean nonExistentIdsRequested =
    ...
    for (Map.Entry<String, StringBuilder> entry : remote.entrySet()) {
      // update the <action> element to reference the subset of target items belonging to this
      // user...
      ...
      try {
        ...
        if (opStr.equals(MailConstants.OP_HARD_DELETE) && nonExistentIdsRequested) {
          for (String nonExistentId :
              actionE.getAttribute(MailConstants.A_NON_EXISTENT_IDS).split(",")) {
        ...
        if (opStr.equals(MailConstants.OP_COPY) && newlyCreatedIdsRequested) {
          for (String newlyCreated :
              actionE.getAttribute(MailConstants.A_NEWLY_CREATED_IDS).split(",")) {
