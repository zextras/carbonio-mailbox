...
public class Sync extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    // last fully synced change id-last item synced in next change id and last fully synced delete
    // change id" (e.g. "32425-99213:d1231232") or
    // last fully synced change id-last item synced in next change id and
    // last fully synced delete id-last item synced in next delete id (e.g.
    // "32425-99213:d12312-82134")
    ...
    // client specify more than DebugConfig.syncMaximumChangeCount It will use
    // DebugConfig.syncMaximumChangeCount
    ...
    // permit the caller to restrict initial sync only to calendar items with a recurrence after a
    // given date
    long calendarStart =
        (syncRequest.getCalendarCutoff() != null) ? syncRequest.getCalendarCutoff() : -1;
    ...
    try {
      iidFolder =
          new ItemId(request.getAttribute(MailConstants.A_FOLDER, DEFAULT_FOLDER_ID + ""), zsc);
   ...
   *     time unconstrained.
   */
  private static boolean folderSync(
      Element response,
    ...
    if (!isVisible && subfolders.isEmpty()) return false;
    ...
    if (isVisible && visible != null && visible.isEmpty()) return true;
    ...
    for (Folder subfolder : subfolders) {
      if (subfolder != null) {
        isVisible |=
            folderSync(
                f, octxt, ifmt, mbox, subfolder, visible, calendarStart, messageSyncStart, phase);
      }
    }
  ...
  private static void initialMsgSync(
      Element f,
      TypedIdList idlist,
      OperationContext octxt,
      Mailbox mbox,
    ...
    if (messageSyncStart > 0
        && !Collections.disjoint(idlist.types(), EnumSet.of(MailItem.Type.MESSAGE))) {
      idlist =
          mbox.listItemsForSync(octxt, folder.getId(), MailItem.Type.MESSAGE, messageSyncStart);
    }

    initialItemSync(f, MailConstants.E_MSG, idlist.getIds(MailItem.Type.MESSAGE));
  }

  private static void initialCovSync(
      Element f,
      TypedIdList idlist,
      OperationContext octxt,
      Mailbox mbox,
      ...
      throws ServiceException {
    if (messageSyncStart > 0
        && !Collections.disjoint(idlist.types(), EnumSet.of(MailItem.Type.CONVERSATION))) {
      idlist =
          mbox.listConvItemsForSync(
              octxt, folder.getId(), MailItem.Type.CONVERSATION, messageSyncStart);
    }
    initialItemSync(f, MailConstants.E_CONV, idlist.getIds(MailItem.Type.CONVERSATION));
  }

  private static void initialTagSync(
      Element f, OperationContext octxt, ItemIdFormatter ifmt, Mailbox mbox)
      throws ServiceException {
   ...
   *     time unconstrained.
   */
  private static void initialCalendarSync(
      Element f,
      TypedIdList idlist,
      OperationContext octxt,
      Mailbox mbox,
    ...
    if (calendarStart > 0 && !Collections.disjoint(idlist.types(), CALENDAR_TYPES)) {
      idlist =
          mbox.listCalendarItemsForRange(
  ...
  private static final int MUTABLE_FIELDS =
      Change.FLAGS
          | Change.TAGS
          | Change.FOLDER
          | Change.PARENT
  ...
  private static String deltaSync(
      Element response,
      OperationContext octxt,
      ItemIdFormatter ifmt,
      Mailbox mbox,
      ...
      int deleteLimit,
      int changeLimit,
      boolean typedDeletes,
      Folder root,
      Set<Folder> visible,
      int messageSyncStart)
      throws ServiceException {
    ...
    if (octxt.isDelegatedRequest(mbox)) {
      ...
      if (!mbox.getModifiedFolders(begin).isEmpty()
          || !Collections.disjoint(tombstones.types(), FOLDER_TYPES)) {
        // special-case the folder hierarchy for delegated delta sync
        boolean anyFolders =
            folderSync(
                response, octxt, ifmt, mbox, root, visible, -1, messageSyncStart, SyncPhase.DELTA);
        // if no folders are visible, add an empty "<folder/>" as a hint
        if (!anyFolders) {
          response.addElement(MailConstants.E_FOLDER);
        }
      ...
      for (Folder folder : mbox.getModifiedFolders(begin)) {
        ...
        if (targetIds == null || targetIds.contains(folder.getId())) {
          ...
          tombstones.add(
              folder.getType(), folder.getId(), folder.getUuid(), folder.getModifiedSequence());
        }
      }
    }
    ...
    Pair<List<Integer>, TypedIdList> changed =
        mbox.getModifiedItems(
            octxt,
            Math.min(begin, deleteModSeqCutoff),
            messageSyncStart,
    ...
    // items that have been altered in non-visible folders will be returned as "deleted" in order to
    // handle moves
      ...
      for (MailItem item : mbox.getItemById(octxt, batch, MailItem.Type.UNKNOWN)) {
        ...
        if ((item.getModifiedSequence() == begin + 1 && item.getId() < changeItemIdCutoff)
            || item.getModifiedSequence()
                <= begin) { // if interrupted delete and un-interrupted modifications.
          continue;
        }

        // if we've overflowed this sync response, set things up so that a subsequent sync starts
        // from where we're cutting off
        ...
        ToXML.encodeItem(
            ...
            item,
            created
                ? Change.FOLDER | Change.CONFLICT | Change.DATE | Change.PARENT
                : MUTABLE_FIELDS);
        itemCount++;
      }
      batch.clear();
  ...
  private static void encodeUnpagedDelete(
      Element eDeleted, TypedIdList tombstones, boolean typedDeletes) {
  ...
  private static void encodePagedDelete(
      Element eDeleted,
      PagedDelete pgDelete,
