...
public class DiscoverRights extends AccountDocumentHandler {
  ...
  */
  ...
  public static void discoverRights(
      Account account, Set<Right> rights, Element eParent, boolean onMaster)
      throws ServiceException {
    AccessManager accessMgr = AccessManager.getInstance();
    Map<Right, Set<Entry>> discoveredRights =
        accessMgr.discoverUserRights(account, rights, onMaster);
        ...
        if (isDelegatedSendRight) {
          if (target instanceof Account || target instanceof Group) {
            ...
            throw ServiceException.FAILURE(
                "internal error, target for " + " delegated send rights must be account or group",
                null);
          }
        } else {
