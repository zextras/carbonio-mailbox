...
public class AddMsg extends MailDocumentHandler {
  ...
  protected String[] getProxiedIdPath(Element request) {
    ...
    return folder != null && (folder.indexOf(':') > 0 || sNumeric.matcher(folder).matches())
        ? TARGET_FOLDER_PATH
        : null;
  }
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    try {
      ...
      if (filterSent
          && !DebugConfig.disableOutgoingFilter
          && folderId == MailSender.getSentFolderId(mbox)
        ...
        msg =
            addedItemIds.isEmpty() ? null : mbox.getMessageById(octxt, addedItemIds.get(0).getId());
      } else {
        DeliveryOptions dopt =
            new DeliveryOptions()
                .setFolderId(folderId)
                .setNoICal(noICal)
                .setFlags(flagsBitMask)
                .setTags(tags);
