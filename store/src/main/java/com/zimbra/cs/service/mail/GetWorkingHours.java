...
public class GetWorkingHours extends GetFreeBusy {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    Map<String /* zimbraId or name */, String /* zimbraId */> idMap =
        new LinkedHashMap<String, String>(); // preserve iteration order
    if (idParam != null) {
    ...
    for (Iterator<Map.Entry<String, String>> entryIter = idMap.entrySet().iterator();
        entryIter.hasNext(); ) {
      ...
      if (acct != null) {
        String name;
        if (!idOrName.equalsIgnoreCase(
            acctId)) // requested by name; use the same name in the response
        name = idOrName;
        else name = acct.getName();
        workHours =
            WorkingHours.getWorkingHours(authAcct, asAdmin, acct, name, rangeStart, rangeEnd);
      } else {
