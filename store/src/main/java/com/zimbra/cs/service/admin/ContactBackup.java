...
public class ContactBackup extends AdminDocumentHandler {
  ...
  protected List<ContactBackupServer> stopContactBackup(
      List<ServerSelector> selectors, Map<String, Object> context, ZimbraSoapContext zsc)
    ...
    for (ServerSelector serverSelector : selectorsToIterate) {
      ...
      try {
        ...
        ZimbraLog.contactbackup.debug(
            "Could not find server or no permission on %s", serverSelector.getKey());
      ...
      if (local) {
        if (!ContactBackupThread.isRunning()) {
          ZimbraLog.contactbackup.debug(
              "ContactBackup is not running on %s", server.getServiceHostname());
  ...
  protected List<ContactBackupServer> startContactBackup(
      List<ServerSelector> selectors, Map<String, Object> context, ZimbraSoapContext zsc)
      throws ServiceException {
    List<ServerSelector> selectorsToIterate = setEffectiveSelectors(selectors);
    List<ContactBackupServer> servers = new ArrayList<ContactBackupServer>();
  ...
  private List<ServerSelector> setEffectiveSelectors(List<ServerSelector> selectors)
      throws ServiceException {
    if (selectors == null || selectors.isEmpty()) {
