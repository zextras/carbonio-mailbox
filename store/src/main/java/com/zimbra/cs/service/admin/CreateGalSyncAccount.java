...
public class CreateGalSyncAccount extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (account == null) {
      ...
      StringUtil.addToMultiMap(
          accountAttrs, Provisioning.A_zimbraIsSystemAccount, LdapConstants.LDAP_TRUE);
      StringUtil.addToMultiMap(
          accountAttrs, Provisioning.A_zimbraHideInGal, LdapConstants.LDAP_TRUE);
  ...
  static void addDataSource(
    ...
    try {
      ...
      contactFolder =
          mbox.createFolder(
              null, folder, new Folder.FolderOptions().setDefaultView(MailItem.Type.CONTACT));
    }
    ...
    for (DataSource ds : account.getAllDataSources()) {
      if (ds.getFolderId() == folderId) {
        throw MailServiceException.ALREADY_EXISTS(
            "data source " + ds.getName() + " already contains folder " + folder);
      }
    }

    mbox.grantAccess(
        null, folderId, domain.getId(), ACL.GRANTEE_DOMAIN, ACL.stringToRights("r"), null);
    ...
    ZimbraLog.security.info(
        ZimbraLog.encodeAttrs(new String[] {"cmd", "CreateGalSyncAccount", "name", acctName}));
  }
