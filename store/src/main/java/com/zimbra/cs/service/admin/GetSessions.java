...
public class GetSessions extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    SortBy sortBy =
        SortBy.fromString(request.getAttribute(AdminConstants.A_SORT_BY, SortBy.nameAsc.name()));
    ...
    public static SortBy fromString(String s) throws ServiceException {
      try {
        ...
        throw ServiceException.INVALID_REQUEST(
            "invalid sortBy: " + s + ", valid values: " + Arrays.asList(SortBy.values()), e);
      }
    }
  }
  ...
  public CachedResult getResult(
      AdminSession adminSession, Type type, boolean refresh, final SortBy sortBy) {

    List<Session> sessions = SessionCache.getActiveSessions(type);
    CachedResult result =
        (adminSession == null || refresh) ? null : (CachedResult) adminSession.getData(SESSION_KEY);

    if (result != null && result.type == type && result.sortBy == sortBy) return result;
    ...
    // SORT
    Comparator<SessionInfo> comparator =
        new Comparator<SessionInfo>() {
    ...
    if (adminSession != null) adminSession.setData(SESSION_KEY, result);
