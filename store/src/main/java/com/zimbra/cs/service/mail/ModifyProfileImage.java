...
// FIXME can this be removed too? Check SaveDocument could be deprecated I think
public class ModifyProfileImage extends SaveDocument {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    try {
      if (!StringUtil.isNullOrEmpty(base64)) {
        ...
        contentType = MimeDetect.getMimeDetect().detect(new ByteArrayInputStream(decodedBytes));
      } else if (uploadId != null) {
        Upload up =
            FileUploadServlet.fetchUpload(
                zsc.getAuthtokenAccountId(), uploadId, zsc.getAuthToken());
      ...
      if (!removeProfileImage) {
        ...
        boolean imageResized =
            updateLDAP(inputSize, thumbnailIn, contentType, imageFileName, mbox, zsc, true);
        if (!imageResized) {
        ...
        imageItemID =
            updateDatabase(mbox, octxt, imageFolderName, contentType, in, imageFileName, zsc);
  ...
  private static boolean updateLDAP(
      long inputSize,
      InputStream thumbnailIn,
      String contentType,
    ...
    if (resize) {
      if (inputSize <= LC.max_image_size_to_resize.intValue()) {
        ...
        byte[] data =
            NativeFormatter.getResizedImageData(
                thumbnailIn,
                contentType,
                imageFileName,
        ...
        ZimbraLog.mailbox.warn(
            "Profile image is larger than maximum size allowed for resizing"
                + " (max_image_size_to_resize): %d > %d",
  ...
  private int updateDatabase(
    ...
    try {
      ...
      if (MailServiceException.NO_SUCH_FOLDER.equals(exception.getCode())) {
        ...
        fopt.setAttributes(hidden)
            .setDefaultView(MailItem.Type.DOCUMENT)
            .setFlags(0)
            .setColor(MailItem.DEFAULT_COLOR_RGB);
        Folder profileFolder =
            mbox.createFolder(octxt, imageFolderName, Mailbox.ID_FOLDER_ROOT, fopt);
    ...
    Doc imageDoc =
        new Doc(
            in,
            new ContentType(contentType, "image/jpeg"),
            imageFileName,
            null,
