...
public class Auth extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (name == null && acctEl == null) {
      ...
      if (at.isExpired()) throw ServiceException.AUTH_EXPIRED();

      if (!at.isRegistered()) throw ServiceException.AUTH_EXPIRED("authtoken is invalid");

      // make sure that the authenticated account is active and has not been deleted/disabled since
      // the last request
      ...
      if (name != null && acctEl != null)
        throw ServiceException.INVALID_REQUEST(
            "only one of <name> or <account> can be specified", null);
      if (name == null && acctEl == null)
      ...
      try {
        ...
        if (by == AccountBy.name && value.indexOf("@") == -1) {
          ...
          if (acct == null) {
            if (virtualHost != null) {
              ...
              if (d != null) value = value + "@" + d.getName();
            }
          }
        }

        if (acct == null) acct = prov.get(by, value);
        ...
        AccountUtil.addAccountToLogContext(
            prov, acct.getId(), ZimbraLog.C_NAME, ZimbraLog.C_ID, null);

        ZimbraLog.security.info(
            ZimbraLog.encodeAttrs(new String[] {"cmd", "AdminAuth", "account", value}));
        ...
        if (acct != null && !acct.isIsSystemResource() && !acct.isIsSystemAccount()) {
          Element el =
              check2FA(request, password, authCtxt, acctEl, acct, context, zsc, prov, csrfSupport);
          if (el != null) {
            return el;
          }
        } else {
  ...
  private Element check2FA(
    ...
    boolean generateDeviceId =
        request.getAttributeBool(AccountConstants.A_GENERATE_DEVICE_ID, false);
    ...
    if (twoFactorManager.twoFactorAuthEnabled()) {
      registerTrustedDevice =
          trustedToken == null
              && request.getAttributeBool(AccountConstants.A_TRUSTED_DEVICE, false);
    }
    ...
    boolean usingTwoFactorAuth = twoFactorManager.twoFactorAuthRequired() && !trustedDeviceOverride;
    ...
    if (password != null || recoveryCode != null || twoFactorAuthWithToken) {
      // authentication logic can be reached with either a password, or a 2FA auth
      // token
      if (usingTwoFactorAuth
          && twoFactorCode == null
          && (password != null || recoveryCode != null)) {
        int mtaAuthPort = acct.getServer().getMtaAuthPort();
        boolean supportsAppSpecificPaswords =
            acct.isFeatureAppSpecificPasswordsEnabled() && zsc.getPort() == mtaAuthPort;
        if (supportsAppSpecificPaswords && password != null) {
          ...
          AppSpecificPasswords appPasswords =
              TwoFactorAuth.getFactory().getAppSpecificPasswords(acct, acctValuePassedIn);
        ...
        if (usingTwoFactorAuth) {
          // check that 2FA has been enabled, in case the client is passing in a
          // twoFactorCode prior to setting up 2FA
          if (!twoFactorManager.twoFactorAuthEnabled()) {
          ...
          if (password == null) {
            try {
              ...
              Account twoFactorTokenAcct =
                  AuthProvider.validateAuthToken(
                      prov, twoFactorToken, false, Usage.TWO_FACTOR_AUTH);
              ...
              if (verifyAccount && !twoFactorTokenAcct.getId().equalsIgnoreCase(acct.getId())) {
                throw new AuthTokenException(
                    "two-factor auth token doesn't match the named account");
              }
            } catch (AuthTokenException e) {
              AuthFailedServiceException exception =
                  AuthFailedServiceException.AUTH_FAILED("bad auth token");
          ...
          if (twoFactorCode != null) {
            ...
            AuthFailedServiceException e =
                AuthFailedServiceException.AUTH_FAILED("no two-factor code provided");
            AuthListener.invokeOnException(e);
            throw e;
          }
    ...
    if (registerTrustedDevice && (trustedToken == null || trustedToken.isExpired())) {
      // generate a new trusted device token if there is no existing one or if the
      // current one is no longer valid
      Map<String, Object> attrs =
          getTrustedDeviceAttrs(zsc, newDeviceId == null ? deviceId : newDeviceId);
      if (trustedDeviceManager != null) {
  ...
  private void verifyTrustedDevice(
      Account account, TrustedDeviceToken td, Map<String, Object> attrs) throws ServiceException {
  ...
  private Element needTwoFactorAuth(
      Account account,
      TwoFactorAuth auth,
      ZimbraSoapContext zsc,
      TokenType tokenType,
    ...
    if (!auth.twoFactorAuthEnabled()) {
      ...
      AuthToken twoFactorToken =
          AuthProvider.getAuthToken(account, Usage.TWO_FACTOR_AUTH, tokenType);
      response.addUniqueElement(AccountConstants.E_TWO_FACTOR_AUTH_REQUIRED).setText("true");
      response.addAttribute(
          AccountConstants.E_LIFETIME,
          twoFactorToken.getExpires() - System.currentTimeMillis(),
          Element.Disposition.CONTENT);
      twoFactorToken.encodeAuthResp(response, false);
      response
          .addUniqueElement(AccountConstants.E_TRUSTED_DEVICES_ENABLED)
          .setText(account.isFeatureTrustedDevicesEnabled() ? "true" : "false");
  ...
  private void checkAdmin(Account acct) throws ServiceException {
    ...
    if (!ok) throw ServiceException.PERM_DENIED("not an admin account");
  }
  ...
  private Element doResponse(
    ...
    boolean rememberMe =
        request.getAttributeBool(AdminConstants.A_PERSIST_AUTH_TOKEN_COOKIE, false);
  ...
  private void setCSRFToken(
    ...
    if (csrfSupport && csrfCheckEnabled) {
      ...
      String token =
          CsrfUtil.generateCsrfToken(accountId, authTokenExpiration, tokenSalt, authToken);
