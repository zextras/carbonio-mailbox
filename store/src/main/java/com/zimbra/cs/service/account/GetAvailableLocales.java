...
public class GetAvailableLocales extends AccountDocumentHandler {
  ...
  private Locale getDisplayLocale(Account acct, Map<String, Object> context)
    ...
    if (StringUtil.isNullOrEmpty(locale)) {
      ...
      if (req != null) {
        ...
        // TODO need to handle multiple languages with quality value and use the one with the
  ...
  private Locale[] computeAvailLocales(Locale[] installedLocales, Set<String> allowedLocales) {
    ...
    for (Locale locale : installedLocales) {
      ...
      if (!StringUtil.isNullOrEmpty(variant)) {
        if (allowedLocales.contains(language)
            || allowedLocales.contains(language + "_" + country)
            || allowedLocales.contains(localeId)) {
