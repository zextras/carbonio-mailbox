...
public class GetServerNIFs extends AdminDocumentHandler {
  private static final Pattern ADDR_PATTERN =
      Pattern.compile("(addr):([0-9a-z\\.:]+)", Pattern.CASE_INSENSITIVE);
  private static final Pattern MASK_PATTERN =
      Pattern.compile("(mask):([0-9\\.xabcdef]+)", Pattern.CASE_INSENSITIVE);
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (server == null) {
      throw ServiceException.INVALID_REQUEST(
          "Cannot find server record for the host: " + serverName, null);
    }

    RemoteManager rmgr = RemoteManager.getRemoteManager(server);
    RemoteResult rr = rmgr.execute(RemoteCommands.ZM_SERVER_IPS);
    BufferedReader in =
        new BufferedReader(new InputStreamReader(new ByteArrayInputStream(rr.getMStdout())));
    String line;
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (IPmatcher.find() && maskMatcher.find()) {
          ...
          elNIF
              ...
              .addAttribute(AdminConstants.A_N, IPmatcher.group(KEY_GROUP).toLowerCase())
              .addAttribute(AdminConstants.A_TYPE, type)
              .setText(ipAddress);
          elNIF
              .addElement(AdminConstants.E_A)
              .addAttribute(AdminConstants.A_N, maskMatcher.group(KEY_GROUP).toLowerCase())
              .setText(maskMatcher.group(VALUE_GROUP));
        }
      }
    } catch (IOException e) {
