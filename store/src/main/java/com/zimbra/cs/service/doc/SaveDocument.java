...
public class SaveDocument extends DocDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    try {
      ...
      ItemId fid =
          new ItemId(docElem.getAttribute(MailConstants.A_FOLDER, DEFAULT_DOCUMENT_FOLDER), zsc);
      folderId = fid.getId();
      ...
      if (attElem != null) {
        ...
        doc =
            fetchMimePart(
                octxt, zsc.getAuthToken(), iid, part, explicitName, explicitCtype, description);
        ...
        if (sourceAccount.getId().equals(zsc.getRequestedAccountId())) {
          ...
          if (revSource == 0) {
            ...
            docRev =
                (Document)
                    mbox.getItemRevision(octxt, iid.getId(), MailItem.Type.DOCUMENT, revSource);
          }
          doc = new Doc(docRev);
        } else {
      ...
      if (itemId == 0) {
        // create a new page
        docItem =
            createDocument(doc, zsc, octxt, mbox, docElem, is, folderId, MailItem.Type.DOCUMENT);
      } else {
        // add a new revision
        ...
        if (docItem.getVersion() != ver) {
          throw MailServiceException.MODIFY_CONFLICT(
              ...
              new Argument(MailConstants.A_ID, itemId, Argument.Type.IID),
              new Argument(MailConstants.A_VERSION, docItem.getVersion(), Argument.Type.NUM));
        }
        ...
        boolean descEnabled =
            docElem.getAttributeBool(MailConstants.A_DESC_ENABLED, docItem.isDescriptionEnabled());
        docItem =
            mbox.addDocumentRevision(
                octxt, itemId, getAuthor(zsc), name, doc.description, descEnabled, is);
      }
      ...
      if (e.getCode().equals(MailServiceException.ALREADY_EXISTS)) {
        ...
        if (item != null && item instanceof Document) {
          ...
          throw MailServiceException.ALREADY_EXISTS(
              "name " + doc.name + " in folder " + folderId, doc.name, item.getId());
        }
      }
      throw e;
    } finally {
  ...
  protected Doc getUploadedDoc(
      String uploadId, ZimbraSoapContext zsc, String name, String ct, String description)
      throws ServiceException {
    ZimbraLog.mailbox.info("uploadId=%s", uploadId);
    Upload up =
        FileUploadServlet.fetchUpload(zsc.getAuthtokenAccountId(), uploadId, zsc.getAuthToken());
    // scan upload for viruses
    // StringBuffer is used here rather than a StringBuilder as UploadScanner could potentially be
    // multi-threaded.
    ...
    if (result == UploadScanner.ERROR) throw MailServiceException.SCAN_ERROR(up.getName());
  ...
  protected void setDocContentType(Doc doc) {
    ...
    if (doc.name != null) {
      ...
      if (guess != null) doc.contentType = guess;
    }
  }
  ...
  protected Document createDocument(
    ...
    if (doc.name == null || doc.name.trim().equals("")) {
      throw ServiceException.INVALID_REQUEST(
          "missing required attribute: " + MailConstants.A_NAME, null);
    } else if (doc.contentType == null || doc.contentType.trim().equals("")) {
      throw ServiceException.INVALID_REQUEST(
          "missing required attribute: " + MailConstants.A_CONTENT_TYPE, null);
    }
    ...
    try {
      ...
      docItem =
          mbox.createDocument(
              octxt, folderId, pd, type, Flag.toBitmask(flags), parent, custom, index);
    } catch (IOException e) {
    ...
    if (Provisioning.onLocalServer(acct)) {
      ...
      try {
        ...
        throw ServiceException.RESOURCE_UNREACHABLE(
            "can't fetch mime part msgId=" + itemId + ", partId=" + partId, e);
      } catch (IOException e) {
        throw ServiceException.RESOURCE_UNREACHABLE(
            "can't fetch mime part msgId=" + itemId + ", partId=" + partId, e);
      }
