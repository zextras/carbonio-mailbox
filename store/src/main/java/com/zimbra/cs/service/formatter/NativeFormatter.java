...
public final class NativeFormatter extends Formatter {
  ...
  private static final Set<String> SCRIPTABLE_CONTENT_TYPES =
      ImmutableSet.of(
  ...
  public void formatCallback(UserServletContext context)
    ...
    try {
      ...
      if (context.target instanceof Message) {
        ...
        throw UserServletException.notImplemented(
  ...
  private void handleMessage(UserServletContext context, Message msg)
      throws IOException, ServiceException, MessagingException, ServletException {
    if (context.hasBody()) {
      ...
      if (size > 0) context.resp.setContentLength((int) size);
      InputStream is = msg.getContentStream();
      ByteUtil.copy(is, true, context.resp.getOutputStream(), false);
    }
  }

  private void handleCalendarItem(UserServletContext context, CalendarItem calItem)
      throws IOException, ServiceException, MessagingException, ServletException {
    if (context.hasPart()) {
      MimePart mp;
      ...
      if (is != null) ByteUtil.copy(is, true, context.resp.getOutputStream(), false);
    }
  }

  private void handleContact(UserServletContext context, Contact con)
      throws IOException, ServiceException, MessagingException, ServletException {
  ...
  private void handleMessagePart(UserServletContext context, MimePart mp, MailItem item)
    ...
    if (mp == null) {
      ...
      if (contentType == null) {
        ...
        if ((contentType =
                MimeDetect.getMimeDetect().detect(Mime.getFilename(mp), mp.getInputStream()))
            == null) contentType = MimeConstants.CT_APPLICATION_OCTET_STREAM;
        else shortContentType = contentType;
      }
      ...
      boolean simpleText =
          (context.hasView()
              && context.getView().equals(TEXT_VIEW)
      ...
      boolean html =
          checkGlobalOverride(
                  Provisioning.A_zimbraAttachmentsViewInHtmlOnly, context.getAuthAccount())
              || (context.hasView() && context.getView().equals(HTML_VIEW));
      InputStream in = null;
      try {
        if (!html
            || ExtensionUtil.getExtension("convertd") == null
            || contentType.startsWith(MimeConstants.CT_TEXT_HTML)
            || contentType.matches(MimeConstants.CT_IMAGE_WILD)) {
          byte[] data = null;
          ...
          if (data != null && !NativeFormatter.RETURN_CODE_NO_RESIZE.equals(returnCode)) {
            ...
            size =
                enc == null || enc.equals("7bit") || enc.equals("8bit") || enc.equals("binary")
                    ? mp.getSize()
                    : 0;
          }
          String defaultCharset =
              context.targetAccount.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, null);
          if (simpleText) {
            sendbackBinaryData(
                context.req,
                context.resp,
          ...
          in = mp.getInputStream();
          handleConversion(
              context, in, Mime.getFilename(mp), contentType, item.getDigest(), mp.getSize());
        }
      } finally {
  ...
  /**
   * If the image stored in the {@code MimePart} exceeds the given width, shrinks the image and
   * returns the shrunk data. If the image width is smaller than {@code maxWidth} or resizing is not
   * supported, returns {@code null}.
   */
  public static byte[] getResizedImageData(
      InputStream in, String contentType, String fileName, Integer maxWidth, Integer maxHeight)
      throws IOException {
    ImageReader reader = null;
    ImageWriter writer = null;

    if (maxWidth == null) maxWidth = LC.max_image_size_to_resize.intValue();

    if (maxHeight == null) maxHeight = LC.max_image_size_to_resize.intValue();
    ...
    try {
      ...
      if (width <= maxWidth && height <= maxHeight) {
        log.debug(
            "Image %dx%d is less than max %dx%d.  Not resizing.",
  ...
  private void handleDocument(UserServletContext context, Document doc)
    ...
    doc =
        (version > 0
            ? (Document)
                doc.getMailbox()
                    .getItemRevision(context.opContext, doc.getId(), doc.getType(), version)
            : doc);
    ...
    if (HTML_VIEW.equals(context.getView())
        && !(contentType != null && contentType.startsWith(MimeConstants.CT_TEXT_HTML))) {
      if (ExtensionUtil.getExtension("convertd") != null) {
        // If the requested view is html, but the requested content is not, use convertd extension
        // when deployed
        handleConversion(
            context, is, doc.getName(), doc.getContentType(), doc.getDigest(), doc.getSize());
      } else {
        // If the requested view is html, but the content is not, respond with a conversion error,
        // so that
        // either an error page, or page invoking an error callback handler can be shown
        try {
          updateClient(
              context,
              new ConversionUnsupportedException(
                  String.format("Native format cannot be displayed inline: %s", contentType)));
      ...
      String defaultCharset =
          context.targetAccount.getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, null);
      boolean neuter =
          doc.getAccount().getBooleanAttr(Provisioning.A_zimbraNotebookSanitizeHtml, true);
      if (neuter)
        sendbackOriginalDoc(
            is,
            contentType,
        ...
        sendbackBinaryData(
            context.req, context.resp, is, contentType, null, doc.getName(), doc.getSize());
    }
  }

  private void handleConversion(
      UserServletContext ctxt,
      InputStream is,
      String filename,
      String ct,
      String digest,
  ...
  private static void sendbackOriginalDoc(
      InputStream is,
      String contentType,
      String defaultCharset,
      String filename,
  ...
  public void saveCallback(
    ...
    if (filename == null) {
      ...
      try {
        ParsedMessage pm =
            new ParsedMessage(context.getPostBody(), mbox.attachmentsIndexingEnabled());
  ...
  private void saveDocument(
    ...
    try {
      if (contentType == null) {
        ...
        if (contentType == null) contentType = MimeConstants.CT_APPLICATION_OCTET_STREAM;
      }
      ...
      if (!(item instanceof Document))
        throw new UserServletException(
            HttpServletResponse.SC_BAD_REQUEST, "cannot overwrite existing object at that path");
      ...
      if (result == UploadScanner.ERROR) throw MailServiceException.SCAN_ERROR(filename);
  ...
  public static void sendZimbraHeaders(
      UserServletContext context, HttpServletResponse resp, MailItem item) {
    if (resp == null || item == null) return;
  ...
  public static void sendbackBinaryData(
      HttpServletRequest req,
      HttpServletResponse resp,
      InputStream in,
      String contentType,
  ...
  public static void sendbackBinaryData(
      ...
      String filename,
      long size,
      boolean ignoreContentDisposition)
      throws IOException {
    resp.setContentType(contentType);
    if (disposition == null) {
      String disp = req.getParameter(UserServlet.QP_DISP);
      disposition =
          (disp == null || disp.toLowerCase().startsWith("i")) ? Part.INLINE : Part.ATTACHMENT;
    }
    ...
    if (!isSafe) {
      ...
      if (bytesRead > 0) pis.unread(buf, 0, bytesRead);
    }
    if (!ignoreContentDisposition) {
      String cd =
          HttpUtil.createContentDisposition(
              req, disposition, filename == null ? "unknown" : filename);
