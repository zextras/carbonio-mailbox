...
public class SetRecoveryAccount extends DocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (channel == null) {
      ZimbraLog.passwordreset.debug(
          "Channel not received in request, setting default channel \"email\"");
    ...
    switch (op) {
      ...
      case validateCode:
        ...
        provider.validateSetRecoveryAccountCode(
            recoveryAccountVerificationCode, account, mbox, zsc);
        break;
      case resendCode:
  ...
  protected void sendCode(
    ...
    if (recoveryDataMap != null && !recoveryDataMap.isEmpty()) {
      ...
      if (resendCount == 0 && recoveryEmail.equals(email)) {
        throw ForgetPasswordException.CODE_ALREADY_SENT(
            "Verification code already sent to this recovery email.");
      }
    }
    ...
    provider.sendAndStoreSetRecoveryAccountCode(
        authAccount, mbox, recoveryCodeMap, zsc, octxt, prefs);
  }
  ...
  protected void resendCode(
    ...
    if (ZimbraLog.passwordreset.isDebugEnabled()) {
      ...
      ZimbraLog.passwordreset.debug(
          "resendCode: Last 3 characters of resent code: %s", code.substring(5));
    }
    if (resendCount < account.getPasswordRecoveryMaxAttempts()) {
      ...
      throw ForgetPasswordException.MAX_ATTEMPTS_REACHED(
          "Re-send code request has reached maximum limit.");
    }
  }
  ...
  private static void validateEmail(String email, Account account) throws ServiceException {
    ...
    if (aliases.contains(email) || accountName.equals(email)) {
      throw ForgetPasswordException.RECOVERY_EMAIL_SAME_AS_PRIMARY_OR_ALIAS(
          "Recovery address should not be same as primary/alias email address.");
    }
  }
}
