  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (eLogger != null) {
      ...
      if (category.equalsIgnoreCase(AddAccountLogger.CATEGORY_ALL)) {
        category = null;
      } else if (!LogManager.logExists(category)) {
        throw ServiceException.INVALID_REQUEST(
            "Log category " + category + " does not exist.", null);
      }
    }
    ...
    for (Log log : LogManager.getAllLoggers()) {
      if (category == null || log.getCategory().equals(category)) {
        if (accountName != null) {
          ...
          if (removed) {
            ZimbraLog.misc.info(
                "Removed logger for account %s from category %s.", accountName, log.getCategory());
          }
        } else {
          int count = log.removeAccountLoggers();
          if (count > 0) {
            ZimbraLog.misc.info(
                "Removed %d custom loggers from category %s.", count, log.getCategory());
          }
        }
      }
    }
