...
public class SearchCalendarResources extends GalDocumentHandler {
  ...
  private static class CalendarResourceExtraQueryCallback extends GalExtraQueryCallback
      implements GalSearchQueryCallback {
  ...
  private static class CalendarResourceGalSearchResultCallback
      extends FilteredGalSearchResultCallback {

    private CalendarResourceGalSearchResultCallback(
    ...
    public Element handleContact(Contact contact) throws ServiceException {
      com.zimbra.cs.service.account.ToXML.encodeCalendarResource(
          getResponse(),
          mFormatter.formatItemId(contact),
          contact.get(ContactConstants.A_email),
    ...
    public void handleContact(GalContact galContact) throws ServiceException {
      ...
      if (galContact.isZimbraGal() || matched(galContact)) {
        com.zimbra.cs.service.account.ToXML.encodeCalendarResource(
            getResponse(),
            galContact.getId(),
            galContact.getSingleAttr(ContactConstants.A_email),
            galContact.getAttrs(),
            neededAttrs(),
            null);
      }
