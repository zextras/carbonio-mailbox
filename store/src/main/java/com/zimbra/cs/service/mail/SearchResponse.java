...
final class SearchResponse {
  ...
  void add(ZimbraHit hit, boolean expandMsg) throws ServiceException {
    ...
    if (el != null) {
      ...
      if (includeMailbox) {
        el.addAttribute(
            MailConstants.A_ID, new ItemId(hit.getAcctIdStr(), hit.getItemId()).toString());
      }
    }
  }
    ...
    if (params.getFetchMode() == SearchParams.Fetch.IDS) {
      ...
      for (MessageHit mhit : hit.getMessageHits()) {
        ConversationMsgHitInfo cMsgHit =
            new ConversationMsgHitInfo(ifmt.formatItemId(mhit.getItemId()));
      ...
      Element el =
          ToXML.encodeConversationSummary(
              element,
              ifmt,
              octxt,
              conv,
              mhit == null ? null : mhit.getMessage(),
  ...
  private Element doConvMsgHit(Element el, Message msg, long numMsgsInConv) {
    // Folder ID useful when undoing a move to different folder, also determining whether in
    // junk/trash
    ConversationMsgHitInfo cMsgHit =
        ConversationMsgHitInfo.fromIdAndFolderId(
  ...
  private Element add(MessageHit hit, boolean expandMsg) throws ServiceException {
    ...
    if (expandMsg && msg.isUnread() && params.getMarkRead()) {
      ...
      try {
        msg.getMailbox()
            .alterTag(octxt, msg.getId(), msg.getType(), Flag.FlagInfo.UNREAD, false, null);
    ...
    if (expandMsg) {
      ...
      el =
          ToXML.encodeMessageSummary(element, ifmt, octxt, msg, params.getWantRecipients(), fields);
    }
    ...
    if (parts != null) {
      for (MessagePartHit mph : parts) {
        ...
        if (partNameStr.length() > 0) {
          el.addElement(MailConstants.E_HIT_MIMEPART)
              .addAttribute(MailConstants.A_PART, partNameStr);
        }
      }
    }
  ...
  private Element add(ContactHit hit) throws ServiceException {
    return ToXML.encodeContact(
        ...
        true /* returnCertInfo */,
        ContactMemberOfMap.setOfMemberOf(
            zsc.getRequestedAccountId(), hit.getItemId(), memberOfMap));
  }
  ...
  private Element add(CalendarItemHit hit) throws ServiceException {
    ...
    EncodeCalendarItemResult encoded =
        GetCalendarItemSummaries.encodeCalendarItemInstances(
            zsc, octxt, item, acct, rangeStart, rangeEnd, true);

    Element el = encoded.element;
    if (el != null) {
      element.addElement(el);
      ToXML.setCalendarItemFields(
          el,
          ifmt,
          octxt,
          item,
          PendingModifications.Change.ALL_FIELDS,
          false,
          params.getNeuterImages());
      el.addAttribute(MailConstants.A_CONTENTMATCHED, true);
    }
