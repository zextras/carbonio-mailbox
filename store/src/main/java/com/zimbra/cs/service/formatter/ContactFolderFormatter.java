...
public class ContactFolderFormatter extends Formatter {
  ...
  public void formatCallback(UserServletContext context)
    ...
    if (v != null && v.equals("2")) d = Delimiter.Contact;
    v = context.params.get("all");
    boolean allContacts = false;
    if (v != null) allContacts = true;

    ItemIdFormatter ifmt =
        new ItemIdFormatter(context.getAuthAccount(), context.targetAccount, false);
    ...
    if (allContacts) {
      for (Folder folder : context.targetMailbox.getFolderList(context.opContext, SortBy.NONE)) {
        ...
        if (folder == context.target
            || folder.getType() == MailItem.Type.MOUNTPOINT
  ...
  private void printContact(MailItem item, OutputStream out, ItemIdFormatter ifmt, Delimiter d)
      throws IOException {
  ...
  private void printContactGroup(String encodedContactGroup, OutputStream out, String ownerAcctId)
      throws IOException {
