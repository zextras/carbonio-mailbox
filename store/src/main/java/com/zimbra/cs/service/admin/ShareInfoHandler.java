...
public abstract class ShareInfoHandler extends AdminDocumentHandler {
   ...
   * DL only for now
   */
  protected NamedEntry getPublishableTargetEntry(
      ZimbraSoapContext zsc, Element request, Provisioning prov) throws ServiceException {
    ...
    if (dl == null) throw AccountServiceException.NO_SUCH_DISTRIBUTION_LIST(value);
  ...
  protected Account getOwner(
      ZimbraSoapContext zsc, Element eShare, Provisioning prov, boolean required)
      throws ServiceException {
    Element eOwner = null;
    if (required) eOwner = eShare.getElement(AdminConstants.E_OWNER);
    else eOwner = eShare.getOptionalElement(AdminConstants.E_OWNER);
