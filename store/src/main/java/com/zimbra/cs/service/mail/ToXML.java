...
public final class ToXML {
  ...
  // we usually don't want to return conflict info.  modseq/imapuid excluded for backwards
  // compatibility.
  public static final int NOTIFY_FIELDS =
      Change.ALL_FIELDS & ~Change.CONFLICT & ~Change.MODSEQ & ~Change.IMAP_UID;

  // no construction
  private ToXML() {}

  public static Element encodeItem(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, MailItem item, int fields)
    ...
    if (item instanceof Folder) {
      ...
      return encodeConversationSummary(
          ...
          (Conversation) item,
          null,
          OutputParticipants.PUT_SENDERS,
          fields,
          false);
      ...
      OutputParticipants output =
          fields == NOTIFY_FIELDS ? OutputParticipants.PUT_BOTH : OutputParticipants.PUT_SENDERS;
  ...
  public static boolean hasFullAccess(Mailbox mbox, OperationContext octxt)
      throws ServiceException {
    if (octxt == null || !octxt.isDelegatedRequest(mbox)) {
      ...
      return AccessManager.getInstance()
          .canAccessAccount(
              octxt.getAuthenticatedUser(), mbox.getAccount(), octxt.isUsingAdminPrivileges());
    }
  }
  ...
  public static Element encodeMailbox(
      Element parent, OperationContext octxt, Mailbox mbox, int fields) {
  ...
  public static Element encodeFolder(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Folder folder)
      throws ServiceException {
    return encodeFolder(parent, ifmt, octxt, folder, NOTIFY_FIELDS);
  }

  public static Element encodeFolder(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Folder folder, int fields)
      throws ServiceException {
        ...
        if (fields != NOTIFY_FIELDS || rp.isSet()) {
          ...
          encodeRetentionPolicy(
              ...
              RetentionPolicyManager.getInstance()
                  .getCompleteRetentionPolicy(folder.getAccount(), rp));
        }
      }
    }
    return elem;
  ...
  public static String encodeEffectivePermissions(Folder folder, OperationContext octxt) {
    try {
      short perms =
          folder.getMailbox().getEffectivePermissions(octxt, folder.getId(), MailItem.Type.FOLDER);
      return ACL.rightsToString(perms);
    } catch (ServiceException e) {
      LOG.warn(
          "ignoring exception while fetching effective permissions for folder %d",
          folder.getId(), e);
      return null;
    }
  }
  ...
  // encode mailbox ACL
  public static Element encodeACL(
      OperationContext octxt, Element parent, ACL acl, boolean exposeAclAccessKey) {
    ...
    for (ACL.Grant grant : acl.getGrants()) {
      ...
      if (needDispName) {
        ...
        if (name == null) {
          // 2. (for bug 35079), see if the name is already resolved in the in the
          // OperationContextData
          OperationContextData.GranteeNames granteeNames =
              OperationContextData.getGranteeNames(octxt);
          ...
          if (name == null) {
            NamedEntry nentry =
                FolderAction.lookupGranteeByZimbraId(grant.getGranteeId(), granteeType);
            if (nentry != null) {
              name = nentry.getName();
            }
          }
      ...
      if (needDispName) {
        ...
        if (OperationContextData.GranteeNames.INVALID_GRANT == name) {
          ...
          eGrant.addAttribute(
              MailConstants.A_DISPLAY, OperationContextData.GranteeNames.EMPTY_NAME);
        } else {
  ...
  public static Element encodeACE(Element parent, ZimbraACE ace) {
    Element eACE =
        parent
            .addNonUniqueElement(MailConstants.E_ACE)
  ...
  private static Element encodeFolderCommon(
      Element elem, ItemIdFormatter ifmt, Folder folder, int fields) throws ServiceException {
    ...
    if (folderId != Mailbox.ID_FOLDER_ROOT) {
      ...
      if (needToOutput(fields, Change.FOLDER)) {
        elem.addAttribute(
            ...
            ifmt.formatItemId(new ItemId(folder.getMailbox(), folder.getFolderId())));
    ...
    if (needToOutput(fields, Change.FLAGS)) {
      String flags =
          folder
              .getMailbox()
              .getItemFlagString(folder); // so that offline override can filter flags
  ...
  public static Element encodeSearchFolder(
      Element parent, ItemIdFormatter ifmt, SearchFolder search) throws ServiceException {
    return encodeSearchFolder(parent, ifmt, search, NOTIFY_FIELDS);
  }

  public static Element encodeSearchFolder(
      Element parent, ItemIdFormatter ifmt, SearchFolder search, int fields)
      throws ServiceException {
  ...
  public static Element encodeMountpoint(
      Element parent, ItemIdFormatter ifmt, OperationContext octx, Mountpoint mpt)
      throws ServiceException {
    return encodeMountpoint(parent, ifmt, octx, mpt, NOTIFY_FIELDS);
  }

  public static Element encodeMountpoint(
      Element parent, ItemIdFormatter ifmt, OperationContext octx, Mountpoint mpt, int fields)
      throws ServiceException {
    ...
    try {
      ...
      return URLUtil.getPublicURLForDomain(
          ...
          UserServlet.SERVLET_PATH
              + HttpUtil.urlEscape(UserServlet.getAccountPath(targetAccount) + folderPath),
          true);
    } catch (ServiceException e) {
  ...
  public static void transferMountpointContents(Element elem, Element mptTarget) {
    ...
    elem.addAttribute(
        MailConstants.A_OWNER_FOLDER_NAME, mptTarget.getAttribute(MailConstants.A_NAME, null));
    ...
    for (Element child : mptTarget.listElements()) {
      ...
      // See ZimbraSoap JAXB classes Mountpoint / Folder etc to determine which elements are
      // unique/non-unique
      if (name.equals(MailConstants.E_FOLDER)
          || name.equals(MailConstants.E_SEARCH)
          || name.equals(MailConstants.E_MOUNT)
          || name.equals(MailConstants.E_RETENTION_POLICY)
  ...
  private static void transferLongAttribute(Element to, Element from, String attrName) {
    try {
      ...
      ZimbraLog.session.warn(
          "exception adding remote folder attr to serialized mountpoint: %s", attrName, e);
    }
  }
  ...
  public static void recordItemTags(Element elem, MailItem item, OperationContext octxt)
      throws ServiceException {
    recordItemTags(elem, item, octxt, NOTIFY_FIELDS);
  }

  public static void recordItemTags(Element elem, MailItem item, OperationContext octxt, int fields)
      throws ServiceException {
  ...
  public static void encodeAllCustomMetadata(Element elem, MailItem item, int fields) {
    ...
    for (String section : sections) {
      try {
        ...
        ZimbraLog.soap.warn(
            "could not deserialize custom metadata; skipping (section '%s', item %s)",
  ...
  public static Element encodeContact(
      ...
      Collection<String> memberAttrFilter,
      boolean summary,
      Collection<String> attrFilter,
      int fields,
      String migratedDlist,
      boolean returnHiddenAttrs,
      long maxMembers,
  ...
  public static Element encodeContact(
    ...
    if (needToOutput(fields, Change.FOLDER)) {
      el.addAttribute(
          MailConstants.A_FOLDER,
          ifmt.formatItemId(
              new ItemId(contact.getMailbox().getAccountId(), contact.getFolderId())));
    }
    ...
    if (attrFilter != null) {
      for (String name : attrFilter) {
        ...
        if (!Strings.isNullOrEmpty(value)) {
          encodeContactAttr(
              el, name, value, contact, encodeContactGroupMembersBasic, octxt, returnCertInfo);
      ...
      Map<String, String> contactFields =
          returnHiddenAttrs ? contact.getAllFields() : contact.getFields();
    ...
    if (migratedDlist != null) {
      encodeContactAttr(
          el, ContactConstants.A_dlist, migratedDlist, contact, false, octxt, returnCertInfo);
    ...
    if (memberOfIds != null && !memberOfIds.isEmpty()) {
      ...
      el.addAttribute(
          MailConstants.E_CONTACT_MEMBER_OF,
          Joiner.on(',').join(memIds),
  ...
  public static boolean hasDLViewRight(String email, Account mRequestedAcct, Account mAuthedAcct) {
    boolean canExpand = true;
    if (mRequestedAcct != null && mAuthedAcct != null && email != null) {
      GalGroup.GroupInfo groupInfo =
          getGalGroupInfoProvider().getGroupInfo(email, true, mRequestedAcct, mAuthedAcct);
      if (groupInfo != null) {
  ...
  private static void encodeContactAttr(
      Element elem,
      String name,
      String value,
      Contact contact,
  ...
  private static void encodeCertificate(
      OperationContext octxt,
      Element elem,
      String name,
      String value,
    ...
    if (SmimeHandler.getHandler() != null && returnCertInfo) {
      SmimeHandler.getHandler()
          .encodeCertificate(account, elem, value, octxt.getmResponseProtocol(), emailAddresses);
    }
  }

  private static void encodeContactGroupMemberBasic(Element eMember, ContactGroup.Member member) {
    eMember.addAttribute(
        MailConstants.A_CONTACT_GROUP_MEMBER_TYPE, member.getType().getSoapEncoded());
    eMember.addAttribute(MailConstants.A_CONTACT_GROUP_MEMBER_VALUE, member.getValue());
  }

  private static void encodeContactGroup(
      Element elem,
      ContactGroup contactGroup,
      Collection<String> memberAttrFilter,
      ItemIdFormatter ifmt,
    ...
    for (ContactGroup.Member member : contactGroup.getMembers(true)) {
      ...
      if (derefedMember != null) {
        if (derefedMember instanceof String) {
          ...
          // If this member is a group, do not create/pass down a ContactGroup object from the
          // member.
          encodeContact(
              eMember, ifmt, octxt, (Contact) derefedMember, summary, memberAttrFilter, fields);
  ...
  private static void encodeContactAttachment(Element elem, Attachment attach) {
    ...
    kvp.addAttribute(MailConstants.A_PART, attach.getPartName())
  ...
  public static Element encodeTag(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Tag tag)
      throws ServiceException {
    return encodeTag(parent, ifmt, octxt, tag, NOTIFY_FIELDS);
  }

  public static Element encodeTag(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Tag tag, int fields)
      throws ServiceException {
      ...
      if (needToOutput(fields, Change.RETENTION_POLICY)) {
        ...
        if (fields != NOTIFY_FIELDS || rp.isSet()) {
          ...
          encodeRetentionPolicy(
              el,
              RetentionPolicyManager.getInstance()
                  .getCompleteRetentionPolicy(tag.getAccount(), rp));
        }
      }
    }
    return el;
  ...
  public static Element encodeConversation(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Conversation conv,
  ...
  public static Element encodeConversation(
    ...
    Element c =
        encodeConversationSummary(
            parent, ifmt, octxt, conv, msgs, null, OutputParticipants.PUT_BOTH, fields, true);
    if (msgs.isEmpty()) {
      return c;
    }
  ...
  private static class ToRecipsList {
    ...
    private final List<ParsedAddress> aggregatedRecipients =
        Lists.newArrayListWithCapacity(MAX_COUNT);
  ...
  /**
   * This version lets you specify the Date and Fragment -- we use this when sending Query Results
   * back to the client, the conversation date returned and fragment correspond to those of the
   * matched message.
   */
  public static Element encodeConversationSummary(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Conversation conv, int fields)
      throws ServiceException {
    return encodeConversationSummary(
        parent, ifmt, octxt, conv, null, OutputParticipants.PUT_SENDERS, fields, true);
  }
  ...
  public static Element encodeConversationSummary(
      ...
      throws ServiceException {
    return encodeConversationSummary(
        parent, ifmt, octxt, conv, msgHit, output, NOTIFY_FIELDS, true);
  }

  private static Element encodeConversationSummary(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Conversation conv,
    ...
    if (octxt != null && octxt.isDelegatedRequest(mbox)) {
      isDelegatedNonAccessible =
          !AccessManager.getInstance()
              .canAccessAccount(
                  octxt.getAuthenticatedUser(), conv.getAccount(), octxt.isUsingAdminPrivileges());
    }
    ...
    return encodeConversationSummary(
        parent, ifmt, octxt, conv, msgs, msgHit, output, fields, alwaysSerialize);
  }
  ...
  private static Element encodeConversationSummary(
    ...
    boolean addSenders =
        (output == OutputParticipants.PUT_BOTH || output == OutputParticipants.PUT_SENDERS)
    ...
    Element c =
        noneVisible && !alwaysSerialize
            ? null
            : encodeConversationCommon(parent, ifmt, octxt, conv, msgs, fields);
    ...
    if (needToOutput(fields, Change.SUBJECT)) {
      c.addAttribute(
          ...
          msgHit != null ? msgHit.getSubject() : conv.getSubject(),
          Element.Disposition.CONTENT);
    }
    if (needToOutput(fields, Change.FLAGS | Change.UNREAD)) {
    ...
    if (fields == NOTIFY_FIELDS && msgHit != null) {
      ...
      if (msgHit.inTrash() || msgHit.inSpam()) {
        ...
        msgsByConv =
            mbox.getMessagesByConversation(octxt, conv.getId(), SortBy.DATE_DESC, -1, true);
      }
      c.addAttribute(
          MailConstants.E_FRAG,
          msgsByConv.isEmpty() == false ? msgsByConv.get(0).getFragment() : msgHit.getFragment(),
          Element.Disposition.CONTENT);
    }
  ...
  private static Element encodeConversationCommon(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Conversation conv,
  ...
  public static Element encodeMessageAsMP(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Message msg,
      ...
      boolean wantExpandGroupInfo,
      boolean encodeMissingBlobs)
      throws ServiceException {
    return encodeMessageAsMP(
        parent,
        ifmt,
        octxt,
        msg,
  ...
  /**
   * Encodes a Message object into <m> element with <mp> elements for message body.
   *
   * @param parent The Element to add the new <tt>&lt;m></tt> to.
   * @param ifmt The formatter to sue when serializing item ids.
   * @param msg The Message to serialize.
   * @param part If non-null, serialize this message/rfc822 subpart of the Message instead of the
   *     Message itself.
   * @param maxSize TODO
   * @param wantHTML <tt>true</tt> to prefer HTML parts as the "body", <tt>false</tt> to prefer
   *     text/plain parts.
   ...
   * @param serializeType If <tt>false</tt>, always serializes as an <tt>&lt;m></tt> element.
   * @param fields Bitmask of fields to output in the response.
   * @return The newly-created <tt>&lt;m></tt> Element, which has already been added as a child to
   *     the passed-in <tt>parent</tt>.
   * @throws ServiceException
   */
  public static Element encodeMessageAsMP(
      Element parent,
    ...
    while (true) {
      try {
        return encodeMessageAsMPHelper(
            false /* bestEffort */,
            parent,
            ifmt,
            octxt,
        ...
        //   (this case generally means that the blob backing the MimeMessage disappeared halfway
        // through)
        try {
   ...
   * @param headers Extra message headers to include in the returned element.
   * @param serializeType If <tt>false</tt>, always serializes as an <tt>&lt;m></tt> element.
   * @param bestEffort If <tt>true</tt>, errors serializing part content are swallowed.
   * @return The newly-created <tt>&lt;m></tt> Element, which has already been added as a child to
   *     the passed-in <tt>parent</tt>.
   * @throws ServiceException
   */
  private static Element encodeMessageAsMPHelper(
      boolean bestEffort,
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
  ...
  public static void setCalendarItemFields(
      Element calItemElem,
      ItemIdFormatter ifmt,
      OperationContext octxt,
  ...
  public static Element encodeInvite(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      CalendarItem cal,
   ...
   *
   * <p>The content for the calendar item is a big multipart/digest containing each invite in the
   * calendar item as a sub-mimepart -- it can be retreived from the content servlet:
   * http://servername/service/content/get?id=<calItemId>
   *
   * <p>The client can ALSO request just the content for each individual invite using a compound
   * item-id request: http://servername/service/content/get?id=<calItemId>-<invite-mail-item-id>
   *
   * <p>DO NOT use the raw invite-mail-item-id to fetch the content: since the invite is a standard
   * mail-message it can be deleted by the user at any time!
   */
  public static Element encodeCalendarItemSummary(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      CalendarItem calItem,
    ...
    while (numTries < MAX_RETRIES) {
      ...
      try {
        setCalendarItemFields(
            elem, ifmt, octxt, calItem, fields, includeInvites, includeContent, true);
        ...
        //   (this case generally means that the blob backing the Calendar item disappeared halfway
        // through)
        elem.detach();
        elem = null;
        try {
          ...
          if (calItem.getSavedSequence() != changeId) {
            ...
            ZimbraLog.soap.info(
                "caught calendar item content change while serializing; will retry");
            continue;
          }
        } catch (NoSuchItemException nsie) {
        ...
        // Were unable to write calendar item structure & not clear what went wrong. Try again a few
        // times
  ...
  public static void encodeCalendarReplies(
      Element parent, CalendarItem calItem, Invite inv, String recurIdZ) {
  ...
  /**
   * Encodes an Invite stored within a calendar item object into <m> element with <mp> elements.
   *
   * @param parent The Element to add the new <tt>&lt;m></tt> to.
   * @param ifmt The SOAP request's context.
   * @param calItem The calendar item to serialize.
   * @param iid The requested item; the contained subpart will be used to pick the Invite out of the
   *     calendar item's blob & metadata.
   * @param part If non-null, we'll serialuize this message/rfc822 subpart of the specified Message
   ...
   * @throws ServiceException
   */
  public static Element encodeInviteAsMP(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      CalendarItem calItem,
  ...
  public static Element encodeMessageAsMIME(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Message msg,
  ...
  public static Element encodeMessageSummary(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Message msg,
    ...
    el.addAttribute(
        MailConstants.E_SUBJECT,
        StringUtil.stripControlCharacters(msg.getSubject()),
        Element.Disposition.CONTENT);
  ...
  private static Element encodeMessageCommon(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      MailItem item,
  ...
  public static Element encodeInviteComponent(
    ...
    if (allFields) {
      if (invite.isPublic() || allowPrivateAccess) {
        ...
        if (invite.isEvent()) {
          if (calItem != null && calItem instanceof Appointment) {
            ...
            e.addAttribute(
                MailConstants.A_APPT_FREEBUSY_ACTUAL,
      ...
      if ((itemId != null) && !("0".equals(itemId))) {
        ...
        if (invite.isEvent()) {
          e.addAttribute(
              MailConstants.A_APPT_ID_DEPRECATE_ME /* apptId */, itemId); // for backward compat
        }
        if (calItem != null) {
          ItemId ciFolderId = new ItemId(calItem.getMailbox(), calItem.getFolderId());
          e.addAttribute(
              MailConstants.A_CAL_ITEM_FOLDER /* ciFolder */, ifmt.formatItemId(ciFolderId));
        }
      }
  ...
  private static Element encodeInvitesForMessage(
      Element parent,
      ItemIdFormatter ifmt,
      OperationContext octxt,
      Message msg,
    ...
    for (Iterator<Message.CalendarItemInfo> iter = msg.getCalendarItemInfoIterator();
        iter.hasNext(); ) {
      ...
      if (info.calItemCreated()) {
        try {
          ...
          // Calendar item has been deleted. Bug 84877 - don't include stale references to it in
          // SOAP response
          calendarItemId = null;
        } catch (ServiceException e) {
          // eat PERM_DENIED
        ...
        if (invCi != null) {
          if (!Invite.isOrganizerMethod(invCi.getMethod())
              || ICalTok.DECLINECOUNTER.equals(method)) {
            invite = invCi;
          } else {
            try {
  ...
  private static void addParts(
      ...
      boolean swallowContentExceptions)
      throws ServiceException {
    addParts(
        root,
        mpiRoot,
        ...
        maxSize,
        neuter,
        excludeCalendarParts,
        defaultCharset,
        swallowContentExceptions,
        MsgContent.full);
  }
  ...
  private static Element addPart(
      VisitPhase phase,
      Element parent,
      Element root,
      MPartInfo mpi,
    ...
    if (excludeCalendarParts && MimeConstants.CT_TEXT_CALENDAR.equalsIgnoreCase(ctype)) {
      // A true calendar part has "method" parameter in the content type.  Otherwise it's just an
      // attachment
  ...
  /**
   * Adds the decoded text content of a message part to the {@link Element}. The content is added as
   * the value of a new <tt>&lt;content></tt> sub-element. <i>Note: This method will only extract
   * the content of a <b>text/*</b> or <b>xml/*</b> message part.</i>
   *
  ...
  private static void addContent(
      Element elt, MPartInfo mpi, int maxSize, boolean neuter, String defaultCharset)
      throws IOException, MessagingException {
    ...
    if (ctype.equals(MimeConstants.CT_TEXT_HTML)) {
      ...
      try {
        ...
        if (charset != null && !charset.trim().isEmpty()) {
          // make sure to feed getTextReader() a full Content-Type header, not just the
          // primary/subtype portion
  ...
  public static Element encodeWiki(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, WikiItem wiki)
      throws ServiceException {
    return encodeWiki(parent, ifmt, octxt, wiki, NOTIFY_FIELDS);
  }

  public static Element encodeWiki(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, WikiItem wiki, int fields)
      throws ServiceException {
  ...
  public static Element encodeDocument(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Document doc)
      throws ServiceException {
    return encodeDocument(parent, ifmt, octxt, doc, Change.ALL_FIELDS);
  }

  public static Element encodeDocument(
      Element parent, ItemIdFormatter ifmt, OperationContext octxt, Document doc, int fields)
      throws ServiceException {
  ...
  public static Element encodeDocumentCommon(
      Element m, ItemIdFormatter ifmt, OperationContext octxt, Document doc, int fields)
      throws ServiceException {
      ...
      try {
        ...
        while (revision == null && v <= doc.getVersion()) {
          revision =
              (Document)
                  doc.getMailbox()
                      .getItemRevision(octxt, doc.getId(), doc.getType(), v++, doc.inDumpster());
        }
        if (revision != null) {
  ...
  public static Element encodeDataSource(Element parent, DataSource ds) {
    ...
    if (ds.getAttr(Provisioning.A_zimbraDataSourceSmtpConnectionType) != null) {
      ...
      m.addAttribute(
          MailConstants.A_DS_SMTP_CONNECTION_TYPE,
          ds.isSmtpConnectionSecure() ? "ssl" : "cleartext");
    }
  ...
  private static void setCalendarItemType(Element elem, MailItem.Type type) {
    elem.addAttribute(
        MailConstants.A_CAL_ITEM_TYPE, type == MailItem.Type.APPOINTMENT ? "appt" : "task");
  }
  ...
  private static Element encodeDtStart(
      Element parent, ParsedDateTime dtStart, boolean allDay, boolean forceUtc) {
    if (forceUtc) {
  ...
  private static Element encodeDtEnd(
      Element parent, ParsedDateTime dtEnd, boolean allDay, boolean isTodo, boolean forceUtc) {
    if (forceUtc) {
  ...
  public static void encodeCalendarItemRecur(Element parent, CalendarItem calItem) {
    ...
    for (Invite inv : invites) {
      ...
      // RECURRENCE-ID
      if (inv.hasRecurId()) encodeRecurId(compElem, inv.getRecurId(), allDay);
      if (!inv.isCancel()) {
        // DTSTART
        ParsedDateTime dtStart = inv.getStartTime();
        if (dtStart != null) encodeDtStart(compElem, dtStart, allDay, false);
        // DTEND or DURATION
        ...
        if (dtEnd != null) {
          ...
          if (dur != null) dur.toXml(compElem);
        }
  ...
  public static Element encodeGalContact(
      Element response, GalContact contact, Collection<String> returnAttrs) {
    return encodeGalContact(response, contact, returnAttrs, true);
  }

  public static Element encodeGalContact(
      Element response, GalContact contact, Collection<String> returnAttrs, boolean canExpand) {
  ...
  private static void encodeAddrsWithGroupInfo(
      Element eMsg, Account requestedAcct, Account authedAcct) {
  ...
  private static void encodeAddrsWithGroupInfo(
      Element eParent, String emailElem, Account requestedAcct, Account authedAcct) {
    if (requestedAcct.isFeatureGalEnabled()
      ...
      for (Element eEmail : eParent.listElements(emailElem)) {
        ...
        if (addr != null) {
          // shortcut the check if the email address is the authed or requested account - it cannot
          // be a group
          if (addr.equalsIgnoreCase(requestedAcct.getName())
              || addr.equalsIgnoreCase(authedAcct.getName())) continue;

          GroupInfo groupInfo =
              groupInfoProvider.getGroupInfo(addr, true, requestedAcct, authedAcct);
      ...
      ZimbraLog.gal.trace(
          "group info bypassed; account [%s] does not have required attributes isFeatureGalEnabled"
              + " [%s] isFeatureGalAutoCompleteEnabled [%s] isPrefGalAutoCompleteEnabled [%s]",
          requestedAcct,
          requestedAcct.isFeatureGalEnabled(),
          requestedAcct.isFeatureGalAutoCompleteEnabled(),
          requestedAcct.isPrefGalAutoCompleteEnabled());
    }
  }

  public static Element encodeComment(
      Element response, ItemIdFormatter ifmt, OperationContext octxt, Comment comment)
      throws ServiceException {
    return encodeComment(response, ifmt, octxt, comment, NOTIFY_FIELDS);
  }

  public static Element encodeComment(
      Element response, ItemIdFormatter ifmt, OperationContext octxt, Comment comment, int fields)
      throws ServiceException {
  ...
  public interface ToXMLExtension {
    Element encodeDocumentAdditionalAttribute(
        Element elem, ItemIdFormatter ifmt, OperationContext octxt, Document doc, int fields);
  }
