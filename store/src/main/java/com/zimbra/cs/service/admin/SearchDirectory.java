...
public class SearchDirectory extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    int maxResults =
        (int) request.getAttributeLong(AdminConstants.A_MAX_RESULTS, MAX_SEARCH_RESULTS);
    ...
    Set<SearchDirectoryOptions.ObjectType> objTypes =
        SearchDirectoryOptions.ObjectType.fromCSVString(types);
    ...
    if (objTypes.contains(SearchDirectoryOptions.ObjectType.accounts)
        && attrsStr != null
        && !attrsStr.contains(Provisioning.A_zimbraMailTransport)) {
    ...
    if (((objTypes.contains(SearchDirectoryOptions.ObjectType.distributionlists))
        ...
        && (attrsStr != null)
        && !attrsStr.contains(Provisioning.A_memberURL)) {
    ...
    if (isCountOnly) {
      ...
      Set<String> origReqAttrs =
          origAttrs == null ? null : Sets.newHashSet(Arrays.asList(origAttrs));

      long start = System.currentTimeMillis();
      int numEntries;
      for (numEntries = offset;
          numEntries < limitMax && numEntries < accounts.size();
          numEntries++) {
      ...
      if (ZimbraLog.search.isTraceEnabled()) {
        ZimbraLog.search.trace(
            "SearchDirectory - encoding entries %s i=%s",
  ...
  static void encodeEntry(
    ...
    if (entry instanceof CalendarResource) {
      ...
      GetDistributionList.encodeDistributionList(
          parent, (DistributionList) entry, false, false, reqAttrs, aac.getAttrRightChecker(entry));
      ...
      GetDistributionList.encodeDistributionList(
          ...
          false,
          reqAttrs,
          null); // TODO: FIXME (aac.getAttrRightChecker)!!!
    } else if (entry instanceof Alias) {
      encodeAlias(parent, prov, (Alias) entry, reqAttrs);
    } else if (entry instanceof Domain) {
      GetDomain.encodeDomain(
          parent, (Domain) entry, applyDefault, reqAttrs, aac.getAttrRightChecker(entry));
  ...
  private static void encodeAlias(Element e, Provisioning prov, Alias a, Set<String> reqAttrs)
    ...
    ToXML.encodeAttrs(
        ealias, attrs, reqAttrs, null); // don't have/need an AttrRightChecker for alias
  }
