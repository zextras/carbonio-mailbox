...
public class CreateMountpoint extends MailDocumentHandler {
  ...
  private static final String[] TARGET_FOLDER_PATH =
      new String[] {MailConstants.E_MOUNT, MailConstants.A_FOLDER};
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    Element remote =
        fetchRemoteFolder(
            ...
            context,
            ownerId,
            (int) t.getAttributeLong(MailConstants.A_REMOTE_ID, -1),
            t.getAttribute(MailConstants.A_PATH, null));
    ...
    if (view == null) view = remote.getAttribute(MailConstants.A_DEFAULT_VIEW, null);
    ...
    try {
      ...
      if (se.getCode() == MailServiceException.ALREADY_EXISTS && fetchIfExists) {
        ...
        if (folder instanceof Mountpoint) mpt = (Mountpoint) folder;
        else throw se;
      } else {
        throw se;
      }
    ...
    if (mpt != null) {
      ...
      // transfer folder counts and subfolders to the serialized mountpoint from the serialized
      // target folder
  ...
  private Element fetchRemoteFolder(
    ...
    else throw ServiceException.INVALID_REQUEST("must specify one of rid/path", null);
