...
public class FormatterServiceException extends ServiceException {
  ...
  FormatterServiceException(
      String message, String code, boolean isReceiversFault, Throwable cause, List<Argument> args) {
    super(message, code, isReceiversFault, cause, args);
  }
  ...
  public static FormatterServiceException INVALID_FORMAT(String filename) {
    return new FormatterServiceException(
        "invalid file format", INVALID_FORMAT, false, arg(FILE_NAME, filename));
  }

  public static FormatterServiceException INVALID_TYPE(String view, String path) {
    return new FormatterServiceException(
        "folder cannot contain item type " + view,
        INVALID_TYPE,
        false,
        arg(ITEM_TYPE, view),
        arg(ITEM_PATH, path));
  ...
  public static FormatterServiceException MISMATCHED_META(String path) {
    return new FormatterServiceException(
        "mismatched item content and meta", MISMATCHED_META, false, arg(ITEM_PATH, path));
  }

  public static FormatterServiceException MISMATCHED_SIZE(String path) {
    return new FormatterServiceException(
        "ignored item data size mismatch", MISMATCHED_SIZE, false, arg(ITEM_PATH, path));
  }

  public static FormatterServiceException MISMATCHED_TYPE(String path) {
    return new FormatterServiceException(
        "cannot overwrite non matching data", MISMATCHED_TYPE, false, arg(ITEM_PATH, path));
  }

  public static FormatterServiceException MISSING_BLOB(String path) {
    return new FormatterServiceException(
        "missing item blob for meta", MISSING_BLOB, false, arg(ITEM_PATH, path));
  }

  public static FormatterServiceException MISSING_META(String path) {
    return new FormatterServiceException(
        "item content missing meta information", MISSING_META, false, arg(ITEM_PATH, path));
  }

  public static FormatterServiceException MISSING_VCARD_FIELDS(String path) {
    return new FormatterServiceException(
        "no contact fields found in vcard", MISSING_VCARD_FIELDS, false, arg(ITEM_PATH, path));
  }
  ...
  private static FormatterServiceException WRAPPED_EXCEPTION(ServiceException e) {
    return new FormatterServiceException(
        e.getMessage(), e.getCode(), e.isReceiversFault(), e.getCause(), e.getArgs());
  }
