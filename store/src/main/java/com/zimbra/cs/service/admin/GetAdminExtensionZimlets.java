...
public class GetAdminExtensionZimlets extends AdminDocumentHandler {
  ...
  private void doExtensionZimlets(
      ZimbraSoapContext zsc, Map<String, Object> context, Element response)
    ...
    try {
      ...
      ZimbraLog.mailbox.warn(
          "Exception while getting zimbraNetworkModulesNG related attributes.", e);
    }
    ...
    while (zimlets.hasNext()) {
      ...
      if (!hasRightsToList(zsc, z, Admin.R_listZimlet, Admin.R_getZimlet)) continue;
      ...
      if (z.isExtension()) {
        ...
        if ("com_zimbra_mobilesync".equals(z.getName()) && mobileNGEnabled && networkNGEnabled) {
          ...
          if (!include) {
            ZimbraLog.mailbox.info(
                "Disabled '%s' as zimbraNetworkMobileNGEnabled is true.", z.getName());
          }
        }
        ...
        if ("com_zimbra_delegatedadmin".equals(z.getName())
          ...
          if (!include) {
            ZimbraLog.mailbox.info(
                "Disabled '%s' as zimbraNetworkAdminNGEnabled is true.", z.getName());
          }
