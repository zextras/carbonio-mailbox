...
public class ModifyPrefs extends AccountDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (prefs.containsKey(Provisioning.A_zimbraPrefMailForwardingAddress)) {
      if (!account.getBooleanAttr(Provisioning.A_zimbraFeatureMailForwardingEnabled, false)) {
        ...
        if (account.getBooleanAttr(Provisioning.A_zimbraFeatureAddressVerificationEnabled, false)) {
          /*
           ...
           */
          String emailIdToVerify =
              (String) prefs.get(Provisioning.A_zimbraPrefMailForwardingAddress);
          if (!Strings.isNullOrEmpty(emailIdToVerify)) {
            prefs.remove(Provisioning.A_zimbraPrefMailForwardingAddress);
            prefs.put(Provisioning.A_zimbraFeatureAddressUnderVerification, emailIdToVerify);
  ...
  public static void sendEmailVerificationLink(
      Account authAccount,
    ...
    String subject = L10nUtil.getMessage(MsgKey.verifyEmailSubject, locale, ownerAcctDisplayName);
    String charset =
        authAccount.getAttr(
    ...
    try {
      ...
      if (ZimbraLog.account.isDebugEnabled()) {
        ...
        ZimbraLog.account.debug(
            "Forwarding address verification URL sent to %s is %s", emailIdToVerify, url);
      }
      String mimePartText =
          L10nUtil.getMessage(
              MsgKey.verifyEmailBodyText, locale, ownerAcctDisplayName, url, gmtDate);
      String mimePartHtml =
          L10nUtil.getMessage(
              MsgKey.verifyEmailBodyHtml, locale, ownerAcctDisplayName, url, gmtDate);
      MimeMultipart mmp = AccountUtil.generateMimeMultipart(mimePartText, mimePartHtml, null);
      MimeMessage mm =
          AccountUtil.generateMimeMessage(
              authAccount, ownerAccount, subject, charset, null, null, emailIdToVerify, mmp);
      mbox.getMailSender().sendMimeMessage(octxt, mbox, false, mm, null, null, null, null, false);
    } catch (MessagingException e) {
      ZimbraLog.account.warn(
          "Failed to send verification link to email ID: '" + emailIdToVerify + "'", e);
      throw ServiceException.FAILURE(
          "Failed to send verification link to email ID: " + emailIdToVerify, e);
    }
  }

  private static String generateAddressVerificationURL(
      Account account, long expiry, String externalUserEmail) throws ServiceException {
