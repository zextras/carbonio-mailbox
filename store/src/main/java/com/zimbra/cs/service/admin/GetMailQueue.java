...
public class GetMailQueue extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (server == null) {
      throw ServiceException.INVALID_REQUEST(
          "server with name " + serverName + " could not be found", null);
    }
    ...
    long waitMillis =
        (queueElem.getAttributeLong(AdminConstants.A_WAIT, MAIL_QUEUE_SCAN_DEFUALT_WAIT_SECONDS))
            * 1000;
    ...
    int limit =
        (int) queryElem.getAttributeLong(AdminConstants.A_LIMIT, MAIL_QUEUE_QUERY_DEFAULT_LIMIT);
  ...
  public static Query buildLuceneQuery(Element queryElem) throws ServiceException {
    ...
    for (Iterator fieldIter = queryElem.elementIterator(AdminConstants.E_FIELD);
        fieldIter.hasNext(); ) {
      emptyQuery = false;
      ...
      for (Iterator matchIter = fieldElement.elementIterator(AdminConstants.E_MATCH);
          matchIter.hasNext(); ) {
