...
public class SubscribeDistributionList extends DistributionListDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    SubscribeDistributionListHandler handler =
        new SubscribeDistributionListHandler(group, request, response, prov, acct);
    handler.handle();
  ...
  private static class SubscribeDistributionListHandler extends SynchronizedGroupHandler {
    ...
    protected SubscribeDistributionListHandler(
        Group group, Element request, Element response, Provisioning prov, Account acct) {
      super(group);
      ...
      if (op == DistributionListSubscribeOp.subscribe) {
        ...
        if (policy == DistributionListSubscriptionPolicy.ACCEPT) {
          ...
          throw ServiceException.PERM_DENIED(
              "subscription policy for group " + group.getName() + " is reject");
        ...
        if (policy == DistributionListUnsubscriptionPolicy.ACCEPT) {
          ...
          throw ServiceException.PERM_DENIED(
              "un-subscription policy for group " + group.getName() + " is reject");
        } else { // REQUEST APPROAVAL
          ApprovalRequestSender sender = new ApprovalRequestSender(prov, group, acct, op);
          sender.composeAndSend();
          status = DistributionListSubscribeStatus.awaiting_approval;
  ...
  private static class ApprovalRequestSender extends NotificationSender {
    ...
    private ApprovalRequestSender(
        Provisioning prov, Group group, Account requestingAcct, DistributionListSubscribeOp op) {
    ...
    private void sendMessage(String[] owners) throws ServiceException {
      try {
        ...
        out.addRecipients(
            javax.mail.Message.RecipientType.TO, addrs.toArray(new Address[addrs.size()]));
        ...
        ZimbraLog.account.warn(
            "send share info notification failed, rcpt='" + Arrays.deepToString(owners) + "'", e);
      }
    }
    ...
    private String textPart(Locale locale) {
      ...
      MsgKey msgKey =
          DistributionListSubscribeOp.subscribe == op
              ? MsgKey.dlSubscribeRequestText
              : MsgKey.dlUnsubscribeRequestText;

      sb.append("\n");
      sb.append(L10nUtil.getMessage(msgKey, locale, requestingAcct.getName(), group.getName()));
      sb.append("\n\n");
      return sb.toString();
    }
