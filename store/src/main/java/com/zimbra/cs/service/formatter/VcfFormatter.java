...
public class VcfFormatter extends Formatter {
  ...
  public void formatCallback(UserServletContext context) throws IOException, ServiceException {
    ...
    try {
      iterator =
          getMailItems(context, getDefaultStartTime(), getDefaultEndTime(), Integer.MAX_VALUE);

      String filename =
          context.target instanceof Contact
              ? ((Contact) context.target).getFileAsString()
              : "contacts";
      String cd =
          HttpUtil.createContentDisposition(context.req, Part.ATTACHMENT, filename + ".vcf");
  ...
  public void saveCallback(
    ...
    if (cards == null
      ...
      throw new UserServletException(
          HttpServletResponse.SC_BAD_REQUEST, "no contact fields found in vcard");
    }
    ...
    for (VCard vcf : cards) {
      ...
      folder
          .getMailbox()
          .createContact(context.opContext, vcf.asParsedContact(), folder.getId(), null);
    }
  }
}
