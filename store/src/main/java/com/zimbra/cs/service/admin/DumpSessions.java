...
public class DumpSessions extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Session.Type type : Session.Type.values()) {
      ...
      if (!includeAccounts) {
        ...
        if (stats[1] == 0) continue; // no active sessions, skip this type!
        ...
        if (sessions.size() == 0) continue; // no active sessions, skip this type!
  ...
  private static void encodeSession(
      Element parent, Session s, boolean includeAcct, Provisioning prov) {
