...
public class SearchAction extends MailDocumentHandler {
  ...
  private static void performMoveAction(
      BulkAction action,
      SearchRequest searchRequest,
      List<SearchHit> searchHits,
      Mailbox mbox,
    ...
    for (SearchHit searchHit : searchHits) {
      ...
      if ("message".equalsIgnoreCase(searchRequest.getSearchTypes())
        ...
        throw ServiceException.INVALID_REQUEST(
            "Target folder type does not match search item type", null);
      }
    }
  }
  ...
  private static void performReadUnreadAction(
      SearchRequest searchRequest, List<SearchHit> searchHits, Mailbox mbox, String op)
      throws ServiceException, AuthTokenException, IOException, HttpException {
    Account acct = mbox.getAccount();
    Server server = Provisioning.getInstance().getServer(acct);
    String url = URLUtil.getSoapURL(server, false);
