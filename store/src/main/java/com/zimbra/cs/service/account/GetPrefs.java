...
public class GetPrefs extends AccountDocumentHandler {
  ...
  /**
   * Pass in a request that optional has &lt;pref&gt; items as a filter, and fills in the response
   * document with gathered prefs.
   *
   * @param request
   * @param acct
   * @param response
   ...
   */
  public static void handle(Element request, Element response, Account acct)
      throws ServiceException {
    HashSet<String> specificPrefs = null;
    for (Element epref : request.listElements(AccountConstants.E_PREF)) {
      if (specificPrefs == null) specificPrefs = new HashSet<String>();
  ...
  public static void doPrefs(
      Account acct, Element prefs, Map<String, Object> attrsMap, HashSet<String> specificPrefs) {
    for (Map.Entry<String, Object> entry : attrsMap.entrySet()) {
      String key = entry.getKey();

      if (specificPrefs != null && !specificPrefs.contains(key)) continue;
      if (!key.startsWith("zimbraPref")) continue;
      ...
      if (value instanceof String[]) {
        ...
        // ZCS-10113
        if (key.equals(Provisioning.A_zimbraPrefImapEnabled) && !acct.isImapEnabled()) continue;
        if (key.equals(Provisioning.A_zimbraPrefPop3Enabled) && !acct.isPop3Enabled()) continue;

        prefs.addKeyValuePair(
            key, (String) value, AccountConstants.E_PREF, AccountConstants.A_NAME);
      }
    }
    ...
    prefs.addKeyValuePair(
        LC.zimbraPrefCalenderScaling.key(),
        calenderScalingMinutes,
        AccountConstants.E_PREF,
