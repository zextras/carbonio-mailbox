...
public final class BounceMsg extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    MailSender msender =
        mbox.getMailSender().setSaveToSent(false).setRedirectMode(true).setSkipHeaderUpdate(true);
    ...
    try {
      ...
      if (iid.belongsTo(mbox)) {
        ...
        upload =
            UserServlet.getRemoteResourceAsUpload(
                zsc.getAuthToken(), iid, Maps.<String, String>newHashMap());
  ...
  /**
   * Adds a full set of {@code Resent-*} headers to the {@code MimeMessage}. Addressees are
   * retrieved from the {@code <m>} element; note that "to" {@code <e>} elements map to {@code
   * Resent-To} addresses, "from" {@code <e>} elements map to {@code Resent-From} addresses, etc.
   * Validates {@code Resent-From} and {@code Resent-Sender} in the same manner as {@code From} and
   * {@code Sender} are treated in normal mail send. Updates the {@link MailSender}'s envelope with
   * the sender and recipient {@code Resent-*} addresses.
   */
  MailSender addResentHeaders(
      Element msgElem,
      ZMimeMessage mm,
    ...
    recipients.addAll(
        addResentRecipientHeader(mm, "Resent-Bcc", maddrs.get(EmailType.BCC.toString())));
    recipients.addAll(
        addResentRecipientHeader(mm, "Resent-Cc", maddrs.get(EmailType.CC.toString())));
    recipients.addAll(
        addResentRecipientHeader(mm, "Resent-To", maddrs.get(EmailType.TO.toString())));
  ...
  /**
   * Retrieves the sender and recipient addresses from the {@code <m>} element. Note that "to"
   * {@code <e>} elements map to {@code Resent-To} addresses, "from" {@code <e>} elements map to
   * {@code Resent-From} addresses, etc. The authenticated user's default charset is used to
   * 2047-encode the display names when needed.
   *
   ...
   * @return a {@link MessageAddresses} element encapsulating all the addresses specified by {@code
   *     <e>} subelements.
   */
  ...
  /**
   * Adds a new header to the {@link MimeMessage} consisting of the given the {@link
   * InternetAddress}es. (Note that this is an add and not a set, as you can have multiple sets of
   * {@code Resent-*} headers on a message if the message has been resent repeatedly.)
   *
   * @param mm The message to add headers to.
   * @param name The name of the header being added.
   * @param addrs The addrs being serialized to the header.
   * @return a non-{@code null} {@code List} of the email parts of the added addresses.
   */
  List<String> addResentRecipientHeader(MimeMessage mm, String name, InternetAddress[] addrs)
      throws MessagingException {
    if (addrs == null || addrs.length == 0) return Collections.emptyList();
