...
public interface AdminRightCheckPoint {
  ...
  static class Notes {
    ...
    public static final String GET_ENTRY =
        GET_ENTRY_1 + "To allow an admin to get all attributes, grant the %s right";

    public static final String MODIFY_ENTRY =
        "All attrs provided in the attribute list have to be settable by the authed admin. "
    ...
    public static final String LIST_ENTRY =
        "If the authenticated admin does not have the corresponding list{Entry} right for an entry,"
            + " the entry is skipped in the getAllXXX/searchXXX/searchDirectoryResponse,  no"
            + " PERM_DENIED exception will be thrown. "
            + GET_ENTRY_1;

    public static final String ADMIN_LOGIN_AS =
        "If the target is an account, need the adminLoginAs right; If the target is a calendar"
            + " resource, need the R_adminLoginCalendarResourceAs right.";
  }
