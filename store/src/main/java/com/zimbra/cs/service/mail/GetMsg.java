...
public class GetMsg extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    boolean alwaysUseContentUrl =
        msgSpec.getUseContentUrl() != null ? msgSpec.getUseContentUrl() : false;
    ...
    if (iid.hasSubpart()) {
      ...
      if (raw) {
        throw ServiceException.INVALID_REQUEST(
            "Cannot request RAW formatted subpart message", null);
      } else {
        ...
        if (recurIdZ == null) {
          ...
          if (invs.length > 0) {
            ...
            if (rid != null) recurIdZ = rid.getDtZ();
          }
        }
  ...
  public static CalendarItem getCalendarItem(OperationContext octxt, Mailbox mbox, ItemId iid)
      throws ServiceException {
    assert (iid.hasSubpart());
    return mbox.getCalendarItemById(octxt, iid.getId());
  }

  public static Message getMsg(OperationContext octxt, Mailbox mbox, ItemId iid, boolean read)
      throws ServiceException {
    assert (!iid.hasSubpart());
      ...
      try {
        ...
        if (e.getCode().equals(ServiceException.PERM_DENIED)) {
          ZimbraLog.mailbox.info(
              "no permissions to mark message as read (ignored): %d", msg.getId());
        } else {
  ...
  public static List<Message> getMsgs(
      OperationContext octxt, Mailbox mbox, List<Integer> iids, boolean read)
      throws ServiceException {
    int i = 0;
    ...
    for (MailItem item : mbox.getItemById(octxt, msgIdArray, MailItem.Type.MESSAGE)) {
      ...
      if (read && item.isUnread() && !RedoLogProvider.getInstance().isSlave()) {
        mbox.alterTag(
            octxt, item.getId(), MailItem.Type.MESSAGE, Flag.FlagInfo.UNREAD, false, null);
      }
    }
    return toRet;
  }
