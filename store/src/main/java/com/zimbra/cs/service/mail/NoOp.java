...
/**
 * Do nothing. The main intent of this Soap call is for the client to fetch new notifications (which
 * are automatically added onto the empty response).
 *
 * <p>The caller may set wait=1 for this request in which case the request will block until there
...
public class NoOp extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (zsc.hasCreatedSession()) wait = false;

    if (wait) {
      if (!zsc.hasSession()) {
        throw ServiceException.INVALID_REQUEST(
            "Cannot execute a NoOpRequest with wait=\"1\" without a session."
                + "  Set the <session> flag in the <context> of your request",
            null);
      }
      ZimbraSoapContext origContext =
          (ZimbraSoapContext) (servletRequest.getAttribute("nop_origcontext"));
      if (origContext == null) { // Initial
        ...
        if (zsc.beginWaitForNotifications(continuation, includeDelegates)) {
          ...
          synchronized (zsc) {
            if (zsc.waitingForNotifications()) {
              // assert (!(continuation instanceof WaitingContinuation) || ((WaitingContinuation)
              // continuation).getMutex() == zsc);
            ...
            // bug 63230: Commenting out the below assertion.  continuation can be a
            // RetryContinuation object, apparently.
            // assert(continuation instanceof WaitingContinuation); // this part of code only
            // reached if we're using WaitingContinuations

            if (zsc.isCanceledWaitForNotifications()) blockingUnsupported = true;
          }
        }
        if (enforceLimit) {
        ...
        if (origContext.isCanceledWaitForNotifications()) blockingUnsupported = true;
        if (enforceLimit) {
          // remove this soap context from the blocked-conext hash, but only
          // if it hasn't already been removed by someone else...
