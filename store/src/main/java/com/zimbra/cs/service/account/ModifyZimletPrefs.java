...
public class ModifyZimletPrefs extends AccountDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Element eZimlet : request.listElements(AccountConstants.E_ZIMLET)) {
       ...
       */
      if (defaultPresence == null
          || // zimlet not available to the user
          defaultPresence == Presence.mandatory
          || // zimlet is mandatory
  ...
  private void doResponse(Provisioning prov, Element response, Account acct)
    ...
    for (String zimletName : userZimlets.getZimletNames()) {
      ...
      if (zimlet != null && zimlet.isEnabled() && !zimlet.isExtension()) {
        ...
        eZimlet.addAttribute(
            AccountConstants.A_ZIMLET_PRESENCE, userZimlets.getPresence(zimletName).toString());
      }
    }
  }
}
