...
public class CreateContact extends MailDocumentHandler {
  ...
  private static final String[] TARGET_FOLDER_PATH =
      new String[] {MailConstants.E_CONTACT, MailConstants.A_FOLDER};
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    Contact con = null;
    if (contacts.size() > 0) con = contacts.get(0);
  ...
  static Pair<Map<String, Object>, List<Attachment>> parseContact(
      Element cn, ZimbraSoapContext zsc, OperationContext octxt) throws ServiceException {
    ...
    for (Element elt : cn.listElements(MailConstants.E_CONTACT_GROUP_MEMBER)) {
      if (!isContactGroup) {
        ...
        throw ServiceException.INVALID_REQUEST(
            MailConstants.E_CONTACT_GROUP_MEMBER + " is only allowed for contact group", null);
      }
  ...
  static void disallowOperation(Element elt) throws ServiceException {
    ...
    if (opStr != null) {
      throw ServiceException.INVALID_REQUEST(MailConstants.A_OPERATION + " is not allowed", null);
    }
  }
  ...
  static void disallowGroupMemberAttr(String attrName) throws ServiceException {
    if (attrName.trim().equals(ContactConstants.A_groupMember)) {
      throw ServiceException.INVALID_REQUEST(
          ContactConstants.A_groupMember + " cannot be specified as an attribute", null);
    }
  }
  ...
  static ParsedContact parseContactMergeMode(
    ...
    for (Element elt : cn.listElements(MailConstants.E_ATTRIBUTE)) {
      ...
      if (attach == null) {
        ...
        if (ContactConstants.A_type.equals(name)
            && ContactConstants.TYPE_GROUP.equals(value)
    ...
    for (Element elt : cn.listElements(MailConstants.E_CONTACT_GROUP_MEMBER)) {
      ...
      if (ModifyGroupMemberOperation.RESET.equals(groupMemberOp)) {
        ...
        ContactGroup.Member.Type memberType =
            ContactGroup.Member.Type.fromSoap(
                elt.getAttribute(MailConstants.A_CONTACT_GROUP_MEMBER_TYPE, null));
    ...
    return new ParsedContact(existing)
        .modify(deltaList, attachments, discardExistingMembers, mbox.getAccountId());
  }

  private static String parseCertificate(
      Element elt, String name, ZimbraSoapContext zsc, OperationContext octxt, Contact existing)
      throws ServiceException {
    ...
    if (!Strings.isNullOrEmpty(attachId)) {
      ...
      try {
        ZimbraLog.contact.debug(
            "start processing contact certificate with aid=%s for account=%s",
        ...
        ZimbraLog.contact.error(
            "Exception in adding user certificate with aid=%s for account %s",
            attachId, zsc.getRequestedAccountId());
  ...
  private static Attachment parseAttachment(
      Element elt, String name, ZimbraSoapContext zsc, OperationContext octxt, Contact existing)
      throws ServiceException {
      ...
      if (Contact.isSMIMECertField(name)) {
        ...
        Upload up =
            FileUploadServlet.fetchUpload(
                zsc.getAuthtokenAccountId(), attachId, zsc.getAuthToken());
        ...
        if (item instanceof Contact) {
          ...
          if (part != null && !part.equals("")) {
            ...
            return new Attachment(
                vcf.getFormatted().getBytes("utf-8"),
  ...
  private static List<ParsedContact> parseAttachedVCard(
      ZimbraSoapContext zsc, OperationContext octxt, Mailbox mbox, Element vcard)
      throws ServiceException {
    String text = null;
    ...
    if (attachId != null) {
      ...
      String charsetWanted =
          mbox.getAccount().getAttr(Provisioning.A_zimbraPrefMailDefaultCharset, null);
  ...
  public static List<Contact> createContacts(
      OperationContext oc, Mailbox mbox, ItemId iidFolder, List<ParsedContact> list, String[] tags)
      throws ServiceException {
  ...
  static String fetchItemPart(
      ZimbraSoapContext zsc,
      OperationContext octxt,
      Mailbox mbox,
      ItemId iid,
      String part,
      String[] acceptableMimeTypes,
      String charsetWanted)
      throws ServiceException {
    String text = null;
    try {
      if (iid.isLocal()) {
        ...
        if (!typeAcceptable) throw MailServiceException.INVALID_CONTENT_TYPE(ctype);
      ...
      throw ServiceException.FAILURE(
          "error fetching message part: iid=" + iid + ", part=" + part, e);
    } catch (MessagingException e) {
      throw ServiceException.FAILURE(
          "error fetching message part: iid=" + iid + ", part=" + part, e);
    }
  ...
  static boolean needToMigrateDlist(ZimbraSoapContext zsc) throws ServiceException {
    ...
    // This is only for the *old* migration client, the *new* migration client will eventually use
    // new API.
