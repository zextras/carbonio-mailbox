...
public class FixCalendarTZ extends AdminDocumentHandler {
  ...
  private static void fixAccounts(
      List<String> acctNames, long after, TimeZoneFixupRules tzfixupRules) throws ServiceException {
    ...
    for (NamedEntry entry : accts) {
      ...
      try {
        ...
        ZimbraLog.calendar.error(
            "Error fixing timezones in mailbox " + mbox.getId() + ": " + e.getMessage(), e);
      }
      numFixedAccts++;
      if (numFixedAccts % every == 0) {
        ZimbraLog.calendar.info(
            "Progress: fixed calendar timezones in "
                + numFixedAccts
                + "/"
                + numAccts
                + " accounts");
  ...
  private static class CalendarTimeZoneFixupThread extends Thread {
    ...
    public CalendarTimeZoneFixupThread(
        List<String> acctNames, long after, TimeZoneFixupRules tzfixupRules) {
