...
public final class ParseMimeMessage {
  ...
  public static MimeMessage importMsgSoap(Element msgElem) throws ServiceException {
    ...
    long maxSize =
        Provisioning.getInstance()
            .getConfig()
            .getLongAttr(Provisioning.A_zimbraMtaMaxMessageSize, DEFAULT_MAX_SIZE);
    ...
    try {
      ...
      if (hasContent) {
        setContent(
            mm,
            mmp,
            partElem != null ? partElem : inviteElem,
            alternatives,
            ctxt,
            isQPencodeRequired);
      }
      ...
      for (Element elem : msgElem.listElements()) {
        ...
        if (eName.equals(MailConstants.E_ATTACH)) {
          ...
          ZimbraLog.soap.warn(
              "unsupported child element '%s' under parent %s", elem.getName(), msgElem.getName());
        }
      }
  ...
  private static void attachDocument(
    ...
    if (!(item instanceof Document)) throw MailServiceException.NO_SUCH_DOC(path);
