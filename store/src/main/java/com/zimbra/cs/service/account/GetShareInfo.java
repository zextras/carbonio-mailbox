...
public class GetShareInfo extends AccountDocumentHandler {
  ...
  private void doGetShareInfo(
    ...
    ShareInfoVisitor visitor =
        new ShareInfoVisitor(prov, response, mountedFolders, resultFilter, resultFilter2);
    if (owner == null) {
  ...
  private void fetchRemoteShareInfo(
      Map<String, Object> context, Element request, String ownerId, ShareInfoVisitor visitor)
      throws ServiceException {
  ...
  public static class ResultFilterByTarget implements ResultFilter {
    ...
    @Override
    public boolean check(ShareInfoData sid) {
      if (mGranteeId != null && !mGranteeId.equals(sid.getGranteeId())) return false;

      if (mGranteeName != null && !mGranteeName.equals(sid.getGranteeName())) return false;
  ...
  public static class ResultFilterByTargetExcludeSelf extends ResultFilterByTarget {
    ...
    @Override
    public boolean check(ShareInfoData sid) {
      if (mSelf.getId().equals(sid.getOwnerAcctId())) return false;
      ...
      try {
        ...
        if (ownerDomainFilter.equalsIgnoreCase(ownerDomain)) {
          ...
          ZimbraLog.misc.debug(
              "Skipping public share '%s' - owner domain '%s' is not the same as '%s'",
              sid.getName(), ownerDomain, ownerDomainFilter);
        }
      } catch (ServiceException e) {
        ZimbraLog.misc.debug(
            "Problem checking domain of share owner '%s'", sid.getOwnerAcctEmail(), e);
      }
      return false;
    }
  }
  ...
  public static class ShareInfoVisitor implements PublishedShareInfoVisitor {
    ...
    public ShareInfoVisitor(
        Provisioning prov,
        GetShareInfoResponseInterface resp,
    ...
    public void visit(ShareInfoData sid) throws ServiceException {
      ...
      if (mResultFilter == null || mResultFilter.check(sid)) {
        ...
        if ((resultFilterForUnmounted == null)
            || (getMountpointId(sid) != null)
