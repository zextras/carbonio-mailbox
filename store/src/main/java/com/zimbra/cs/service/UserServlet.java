...
public class UserServlet extends ZimbraServlet {
  ...
  public static final String QP_MANIFEST =
      "manifest"; // selects whether server returns patch manifest or not
  ...
  protected boolean proxyIfMountpoint(
    ...
    try {
      proxyServletRequest(
          req, resp, prov.getServer(targetAccount), uri, getProxyAuthToken(context));
    } catch (HttpException e) {
      throw new IOException("Unknown error", e);
    }
    return true;
