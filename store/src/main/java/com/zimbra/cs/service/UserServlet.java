...
public class UserServlet extends ZimbraServlet {
  ...
  public static final String QP_FREEBUSY_CALENDAR =
      "fbcal"; // calendar folder to run free/busy search on

  public static final String QP_IGNORE_ERROR =
      "ignore"; // ignore and continue on error during ics import

  public static final String QP_PRESERVE_ALARMS =
      "preserveAlarms"; // preserve existing alarms during ics import
  ...
  /**
   * Used by {@link TarFormatter} to specify whether the <tt>.meta</tt> files should be added to the
   * tarball (<tt>meta=1</tt>) or not (<tt>meta=0</tt>). The default is <tt>1</tt>.
   */
  public static final String QP_META = "meta";

  /**
   * Used by {@link IfbFormatter} to specify the UID of calendar item to exclude when computing
   * free/busy.
   */
  ...
  public static final String AUTH_NO_SET_COOKIE =
      "nsc"; // don't set auth token cookie after basic auth
  ...
  private void sendError(
      UserServletContext ctxt, HttpServletRequest req, HttpServletResponse resp, String message)
      throws IOException {
    if (resp.isCommitted()) {
    ...
    if (ctxt != null
        ...
        && ctxt.basicAuthAllowed()
        && !ctxt.basicAuthHappened) {
      ...
      resp.sendError(
        ...
        && ctxt.getAuthAccount() instanceof GuestAccount
        && ctxt.basicAuthAllowed()) {
      resp.addHeader(AuthUtil.WWW_AUTHENTICATE_HEADER, getRealmHeader(req, null));
      resp.sendError(
          HttpServletResponse.SC_UNAUTHORIZED,
  ...
  protected UserServletContext createContext(
      HttpServletRequest req, HttpServletResponse resp, UserServlet servlet)
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      else throw new ServletException(se);
  ...
  private boolean checkAuthentication(UserServletContext context)
    ...
    if (context.targetAccount == null
        && context.accountPath != null
        && context.accountPath.equals("~")) {
  ...
  protected boolean proxyIfRemoteTargetAccount(
      HttpServletRequest req, HttpServletResponse resp, UserServletContext context)
  ...
  public static String getExternalRestUrl(OperationContext octxt, Mountpoint mpt)
    ...
    if (Provisioning.onLocalServer(targetAccount)) {
      ...
      ZMailbox.Options zoptions =
          new ZMailbox.Options(authToken.toZAuthToken(), AccountUtil.getSoapUri(targetAccount));
    ...
    return URLUtil.getServiceURL(
        targetServer,
        SERVLET_PATH + HttpUtil.urlEscape(getAccountPath(targetAccount) + folderPath),
        true);
  }
  ...
  public static String getExternalRestUrl(Folder folder) throws ServiceException {
    ...
    return URLUtil.getServiceURL(
        targetServer,
        SERVLET_PATH + HttpUtil.urlEscape(getAccountPath(targetAccount) + folder.getPath()),
        true);
  }
  ...
  private void doAuthGet(
    ...
    if (FormatType.FREE_BUSY.equals(context.format)) {
       ...
       */
      context.fakeTarget =
          new UserServletContext.FakeFolder(context.accountPath, "/Calendar", "Calendar");
    }
  ...
  /**
   * Adds an item to a folder specified in the URI. The item content is provided in the PUT
   * request's body.
   */
  @Override
  ...
  /**
   * Adds an item to a folder specified in the URI. The item content is provided in the POST
   * request's body.
   */
  @Override
  public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      checkTargetAccountStatus(context);
      if (proxyIfRemoteTargetAccount(req, resp, context)) return;
      ...
      if (mbox != null) {
        ...
        try {
          ...
          // perhaps it's a POST to "Notebook/new-file-name" -- find the parent folder and proceed
          // from there
          if (context.itemPath == null) throw nsie;
          int separator = context.itemPath.lastIndexOf('/');
          if (separator <= 0) throw nsie;
        ...
        folder =
            (context.target instanceof Folder
                ? (Folder) context.target
                : mbox.getFolderById(context.opContext, context.target.getFolderId()));

        if (context.target != folder) {
          if (filename == null) filename = context.target.getName();
          else
      ...
      if (context.format == null && filename != null) context.format = FormatType.HTML_CONVERTED;
      ...
      if (context.format == null && ctype != null) {
        ...
        if (fmt != null) context.format = fmt.getType();
      }
  ...
  protected boolean proxyIfMountpoint(
    ...
    StringBuilder uri =
        new StringBuilder(
            SERVLET_PATH
                ...
                + QP_ID
                + '='
                + HttpUtil.urlEscape(mpt.getOwnerId())
                + "%3A"
                + mpt.getRemoteId());
    if (context.format != null)
    ...
    if (targetAccount == null)
      throw new UserServletException(
          HttpServletResponse.SC_BAD_REQUEST, L10nUtil.getMessage(MsgKey.errNoSuchAccount, req));
    try {
      proxyServletRequest(
          req, resp, prov.getServer(targetAccount), uri.toString(), getProxyAuthToken(context));
    } catch (HttpException e) {
      throw new IOException("Unknown error", e);
    }
    return true;
  ...
  static {
    ...
    ZIMBRA_DOC_CONTENT_TYPE.add("application/x-slides");
    ZIMBRA_DOC_CONTENT_TYPE.add("application/x-xls");
  }
  ...
  private FormatType defaultFormat(UserServletContext context) {
    ...
    if (context.target instanceof Folder) type = ((Folder) context.target).getDefaultView();
    else if (context.target != null) type = context.target.getType();
    ...
    switch (type) {
      ...
      case DOCUMENT:
        ...
        if (context.target instanceof Folder) return FormatType.HTML;
        ...
        if (ZIMBRA_DOC_CONTENT_TYPE.contains(contentType)) return FormatType.HTML;
        return FormatType.HTML_CONVERTED;
      default:
        return FormatType.HTML_CONVERTED;
    }
  ...
  public static byte[] getRemoteContent(
      AuthToken authToken, Account target, String folder, Map<String, String> params)
      throws ServiceException {
  ...
  public static HttpInputStream getRemoteContentAsStream(
      AuthToken authToken, Account target, String folder, Map<String, String> params)
  ...
  private static String getRemoteUrl(Account target, String folder, Map<String, String> params)
    ...
    StringBuffer url =
        new StringBuffer(getProxyUrl(null, server, SERVLET_PATH + getAccountPath(target)));
    if (folder.length() > 0) url.append("/").append(folder);
    url.append("/?").append(QP_AUTH).append('=').append(AUTH_COOKIE);
    if (params != null) {
      for (Map.Entry<String, String> param : params.entrySet())
        url.append('&')
            .append(HttpUtil.urlEscape(param.getKey()))
            .append('=')
  ...
  public static Upload getRemoteResourceAsUpload(
    ...
    for (Header hdr : response.getFirst()) {
      ...
      if (hname.equals("content-type")) ctype = hdr.getValue();
    ...
    if (filename == null || filename.equals("")) filename = "unknown";
  ...
  public static class HttpInputStream extends FilterInputStream {
    ...
    public int getContentLength() {
      ...
      if (cl != null) return Integer.parseInt(cl);
      return -1;
    }

    public String getHeader(String headerName) {
      Header cl = response.getFirstHeader(headerName);
      if (cl != null) return cl.getValue();
      return null;
    }
  ...
  public static Pair<Header[], HttpInputStream> putMailItem(
      ZAuthToken authToken, String url, MailItem item) throws ServiceException, IOException {
    if (item instanceof Document) {
      ...
      method.setEntity(
          new InputStreamEntity(
              doc.getContentStream(), doc.getSize(), ContentType.create(contentType)));
  ...
  public static Pair<Header[], HttpInputStream> putRemoteResource(
      ZAuthToken authToken, String url, InputStream req, Header[] headers)
      throws ServiceException, IOException {
    StringBuilder u = new StringBuilder(url);
    u.append("?").append(QP_AUTH).append('=').append(AUTH_COOKIE);
    HttpPut method = new HttpPut(u.toString());
    ...
    if (headers != null) {
      for (Header hdr : headers) {
        ...
        if (name.equals("Content-Type")) contentType = hdr.getValue();
      }
    }
      ...
      if (statusCode == HttpStatus.SC_NOT_FOUND || statusCode == HttpStatus.SC_FORBIDDEN)
        ...
        throw ServiceException.RESOURCE_UNREACHABLE(
            ...
            new ServiceException.InternalArgument(
                HTTP_URL, url, ServiceException.Argument.Type.STR),
            new ServiceException.InternalArgument(
