...
public final class SearchConv extends Search {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (cid.belongsTo(mbox)) { // local
      try (ZimbraQueryResults results =
          mbox.index.search(zsc.getResponseProtocol(), octxt, params)) {
        ...
        Element container =
            nest
                ? ToXML.encodeConversationSummary(
                    response, ifmt, octxt, conv, CONVERSATION_FIELD_MASK)
                : response;
      ...
      try {
        ...
        Account target =
            Provisioning.getInstance().get(AccountBy.id, cid.getAccountId(), zsc.getAuthToken());
        ...
        proxyRequest.addAttribute(
            MailConstants.E_QUERY, rewrittenQueryString, Element.Disposition.CONTENT);
  ...
  /**
   * This will only work for messages. That's OK since we force GROUP_BY_MESSAGE here.
   *
   ...
   * @param results set of HITS for messages in this conversation which matches the search
   * @param offset offset in conversation to start at
   * @param limit number to return
   * @return whether there are more messages in the conversation past the specified limit
   * @throws ServiceException
   */
  private boolean putHits(
    ...
    if (size > 0) {
      ...
      // we might return from conv. NULL means no ZimbraHit presumably b/c the message didn't match
      // the search.
      ...
      // Okay, we've built the matched[] array. Now iterate through all the messages, and put the
      // message or
  ...
  /**
   * Determine which messages in a conversation need to be expanded. This combines logic that used
   * to be in SearchResults.isInlineExpand and SearchConv.putHits. Returns a boolean array with
   * true/false corresponding to whether each message should be expanded.
   */
  private boolean[] determineExpandedMessages(
  ...
  private Element addMessageMiss(
      Message msg,
      Element response,
      OperationContext octxt,
      ItemIdFormatter ifmt,
      ...
      SearchParams params)
      throws ServiceException {
    // ZimbraLog.search.debug("query "+params.getQueryString()+" didn't match message
    ...
    if (inline) {
      ...
      el =
          ToXML.encodeMessageSummary(
              response, ifmt, octxt, msg, params.getWantRecipients(), ToXML.NOTIFY_FIELDS);
    }
    return el;
  }
}
