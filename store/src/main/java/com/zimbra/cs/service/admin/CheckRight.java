...
public class CheckRight extends RightDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    NamedEntry ne =
        GranteeType.lookupGrantee(Provisioning.getInstance(), granteeType, granteeBy, granteeVal);
    ...
    if (!granteeVal.equals(zsc.getAuthtokenAccountId())) {
       ...
       */
      checkCheckRightRight(
          zsc,
          (grantee instanceof Account) ? GranteeType.GT_USER : GranteeType.GT_GROUP,
          granteeBy,
          granteeVal,
          true);
    }
    ...
    boolean result =
        RightCommand.checkRight(
            Provisioning.getInstance(), targetType, targetBy, target, grantee, right, attrs, via);
