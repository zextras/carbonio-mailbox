 ...
 *
 * <p>MAILBOXID -- integer for the mailbox id foo@bar.com -- email address for the account
 * 1230231-ac231-.. -- UID for account /SERVER/MAILBOXID -- Specific mailbox on a specific server
 * /SERVER/* -- ALL mailboxes on a particular server * -- ALL mailboxes in the system!
 *
 * <p>NOTENOTENOTE -- If server ID is *, then mailbox ID must be * (for simplicity sake)
 *
 * @since Mar 29, 2005
 * @author tim
 */
public final class ParseMailboxID {
  ...
  /**
   * Create an ID which represents ALL mailboxes on a specified server (same as the string
   * "/serverid/*")
   */
  ...
  /**
   * @return the specified mailbox identifier - useful if after checking isLocal() you decide you
   *     need to pass this request on to another server.
   */
  ...
  /**
   * @return the integer ID part of the mailbox, if we have one. Note that if the mailbox is
   *     nonlocal, then we may not have this value...
   */
  ...
  private void init(Account account, String idStr)
      ...
      if (substrs[2].startsWith("*")) {
        ...
        if (allServers) {
          throw new IllegalArgumentException(
              "Invalid mailboxID (\"*/number is not allowed): " + idStr);
        }
