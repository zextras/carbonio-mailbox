...
public class DeleteDataSource extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context)
    ...
    for (Element eDsrc : request.listElements()) {
      ...
      if (id != null) dsrc = prov.get(account, Key.DataSourceBy.id, id);
      else if ((name = eDsrc.getAttribute(MailConstants.A_NAME, null)) != null)
        dsrc = prov.get(account, Key.DataSourceBy.name, name);
      else throw ServiceException.INVALID_REQUEST("must specify either 'id' or 'name'", null);
      ...
      if (dstype == DataSourceType.pop3) DbPop3Message.deleteUids(mbox, dataSourceId);
