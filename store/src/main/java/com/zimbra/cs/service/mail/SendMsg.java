...
public class SendMsg extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context, QName respQname)
    ...
    try {
      ...
      boolean needCalendarSentByFixup =
          request.getAttributeBool(MailConstants.A_NEED_CALENDAR_SENTBY_FIXUP, false);
      boolean isCalendarForward =
          request.getAttributeBool(MailConstants.A_IS_CALENDAR_FORWARD, false);
        ...
        if (acctReq != null) {
          ...
          if (accessMgr.canDo(authAcct, acctReq, Rights.User.R_sendAs, false)
            ...
            if (Provisioning.onLocalServer(delegatedAccount)) {
              ...
              Element response =
                  proxyRequest(request, context, authToken, delegatedAccount.getId());
              if (iidDraft != null) {
  ...
  public static ItemId doSendMessage(
    ...
    if (needCalendarSentByFixup || isCalendarForward) {
      mv =
          new OutlookICalendarFixupMimeVisitor(mbox.getAccount(), mbox)
    ...
    if (dataSourceId == null) {
      sender =
          isCalendarMessage ? CalendarMailSender.getCalendarMailSender(mbox) : mbox.getMailSender();
      if (noSaveToSent) {
        id =
            sender.sendMimeMessage(
                oc, mbox, false, mm, uploads, origMsgId, replyType, null, false, processor);
      } else {
        id =
            sender.sendMimeMessage(
                oc, mbox, mm, uploads, origMsgId, replyType, identityId, false, processor);
      }
    } else {
      com.zimbra.cs.account.DataSource dataSource =
          mbox.getAccount().getDataSourceById(dataSourceId);
      if (dataSource == null) {
    ...
    try {
      if (isCalendarMessage && isCalendarForward && mv.getOriginalInvite() != null) {
        ...
        if (org != null) {
          ...
          if (orgAccount != null && !orgAccount.getId().equals(mbox.getAccount().getId())) {
            MimeMessage notifyMimeMsg =
                CalendarMailSender.createForwardNotifyMessage(
                    mbox.getAccount(),
                    orgAccount,
                    orgAddress,
                    mm.getAllRecipients(),
      ...
      ZimbraLog.soap.warn(
          "Ignoring error while sending Calendar Invitation Forward Notification", e);
    }

    return id;
  }

  static MimeMessage parseUploadedMessage(
      ZimbraSoapContext zsc, String attachId, MimeMessageData mimeData) throws ServiceException {
  ...
  static MimeMessage parseUploadedMessage(
      ...
      String attachId,
      MimeMessageData mimeData,
      boolean needCalendarSentByFixup)
      throws ServiceException {
    boolean anySystemMutators = MimeVisitor.anyMutatorsRegistered();

    Upload up =
        FileUploadServlet.fetchUpload(zsc.getAuthtokenAccountId(), attachId, zsc.getAuthToken());
    if (up == null) {
    ...
    try {
      ...
      if (anySystemMutators || needCalendarSentByFixup) {
        ...
        OutlookICalendarFixupMimeVisitor.ICalendarModificationCallback callback =
            new OutlookICalendarFixupMimeVisitor.ICalendarModificationCallback();
        MimeVisitor mv =
            new OutlookICalendarFixupMimeVisitor(getRequestedAccount(zsc), getRequestedMailbox(zsc))
                .needFixup(true)
  ...
  private static Pair<SendState, Pair<String, ItemId>> findPendingSend(
      Integer mailboxId, String sendUid) {
    SendState state = SendState.NEW;
    Pair<String, ItemId> sendRecord = null;
  ...
  private void deleteDraft(
      ItemId iidDraft, OperationContext octxt, Mailbox localMbox, ZimbraSoapContext zsc) {
    try {
      if (iidDraft.belongsTo(localMbox)) {
        localMbox.delete(octxt, iidDraft.getId(), MailItem.Type.MESSAGE);
      } else if (iidDraft.isLocal()) {
        Mailbox ownerMbox =
            MailboxManager.getInstance().getMailboxByAccountId(iidDraft.getAccountId());
        ...
        ZAuthToken zat =
            at.getProxyAuthToken() == null
                ? at.toZAuthToken()
                : new ZAuthToken(at.getProxyAuthToken());
  ...
  /**
   * When Outlook/ZCO sends a calendar invitation or reply message on behalf of another user, the
   * From and Sender message headers are set correctly but the iCalendar object doesn't set SENT-BY
   * parameter on ORGANIZER or ATTENDEE property of VEVENT/VTODO components. These need to be fixed
   * up.
   *
   * @author jhahm
   */
    ...
    protected boolean visitBodyPart(MimeBodyPart bp) throws MessagingException {
      ...
      if (!isReply) {
        ...
        try {
          ...
          LOG.warn(
              "Unable perform fixup of calendar reply from Outlook for mailbox "
                  + mMailbox.getId()
                  + "; ignoring error",
              e);
        }
                ...
                if (addressMatchesFrom(addr)) {
                  ...
                  if (sentBy == null) {
                    ...
                    ZimbraLog.calendar.debug(
                        "Fixed up %s ( %s) by adding SENT-BY=%s", token, addr, mSentBy);
                  }
                }
              }
              break;
