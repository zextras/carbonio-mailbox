 ...
 *
 * <p>1) "INTEGER" -- this is the highest change ID the client knows about (see
 * Mailbox.getLastChangeID)
 *
 * <p>OR
 *
 * <p>2) "INTEGER-INTEGER" -- the first integer is the highest change ID that the client has *all*
 * the data for -- the second integer is the highest item id in the NEXT CHANGE ID that the client
 * has data for
 *
 * <p>e.g. "4-32" means "I have all of change 4, AND I have up to item 32 in change 5" 3)
 * "INTEGER-INTEGER:dINTEGER-INTEGER" -- the first integer is the highest change ID that the client
 * has *all* the data for -- the second integer is the highest item id in the NEXT CHANGE ID that
 * the client has data for -- :d for Deleted Cut off. as there is delete pagination introduced.
 * :dINTEGER-INTEGER -- the first integer is the highest delete ID that the client has *all* the
 * data for -- the second integer is the highest item id in the NEXT DELETE ID that the client has
 * data for e.g. "4-32:d3-30" means "I have all of change 4, AND I have up to item 32 in change 5
 * AND I have all of delete 3, AND I have up to item 30 in delete" Similar format : a) 4-32:d3-30 b)
 * 4-32:d6 c) 4:d3-30 d) 4-32:d3-30
 */
public class SyncToken implements Cloneable, Comparable {
  ...
  public boolean hasOffsetInNext() {
    return mChangeOffsetInNext > 0;
  }

  public int getOffsetInNext() {
    return mChangeOffsetInNext;
  }

  public int getDeleteModSeq() {
    return mDeleteModSeq;
  }

  public boolean hasDeleteOffsetInNext() {
    return mDeleteOffsetInNext > 0;
  }

  public int getDeleteOffsetInNext() {
    return mDeleteOffsetInNext;
  }
  ...
  public boolean after(int changeId, int offset) {
    if (mChangeId < changeId) return false;
    if (mChangeId > changeId) return true;
      ...
      if (delDiff == 0) {
        if ((this.mChangeOffsetInNext == -1 && other.mChangeOffsetInNext == -1)
          ...
          return 0;
            ...
            && other.mChangeOffsetInNext == -1
            && other.mDeleteOffsetInNext == -1) {
          return 1;
        } else if (this.mChangeOffsetInNext == -1
