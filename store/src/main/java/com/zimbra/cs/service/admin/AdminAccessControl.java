 ...
 *
 * <p>1. Compatibility layer between legacy domain based access manager and pure ACL based access
 * manager. This is so we can go back and forth between the two, by a LC
 * key(zimbra_class_accessmanager) change:
 *
 * <p>DomainAccessControl should be deprecated at some point.
 *
 * <p>2. A utility layer between service handlers(SOAP and servlet handlers) and access manger. We
 * want to keep the AccessManager API simple and use this class to provide convenient methods, for
 * right checking callsites.
 */
public abstract class AdminAccessControl {
  ...
  public static AdminAccessControl getAdminAccessControl(AuthToken authToken)
    ...
    if (authedAcct == null) throw ServiceException.AUTH_REQUIRED();
    return newAdminAccessControl(null, authToken, authedAcct);
  }

  /**
   * Returns if the auth token is sufficient for a SOAP request that requires some level of admin
   * privileges.
   *
   * <p>Called from SoapEngine as a gate keeping checking before dispatching to the handler. If this
   * method returns true, the request is eligible to be dispatched to the handler, the actual right
   * checking for the request is handled in the handler. If this method returns false, the request
   ...
   * @throws ServiceException
   */
  public abstract boolean isSufficientAdminForSoap(
      Map<String, Object> soapCtxt, DocumentHandler handler);
   ...
   *
   * <p>Note: this method is static and it checks the specified account.
   *
   * @param acct
  ...
  public abstract void checkModifyAttrs(AttributeClass attrClass, Map<String, Object> attrs)
      throws ServiceException;
   ...
   */
  public abstract void checkSetAttrsOnCreate(
      TargetType targetType, String entryName, Map<String, Object> attrs) throws ServiceException;

  /**
   * for an entry to be listed in the Search*** and GetAll*** response, the authed admin needs to
   * have the "list***" right.
   *
   * <p>Note: if the AccessManager is a domain based AccessManager, it always returns true.
   * Callsites of the this method must have either already checked the domain right (passing a
   * pseudo "always allow" AdminRight to the checker), or the handler is not for domain admins
   * anyway so domain admins would have been blocked at SoapEngine and won't even get into the soap
   * handler.
   */
  public abstract boolean hasRightsToList(
      NamedEntry target, AdminRight listRight, Object getAttrRight) throws ServiceException;

  public abstract boolean hasRightsToListCos(Cos target, AdminRight listRight, Object getAttrRight)
      throws ServiceException;

  /**
   * For non-domained rights (i.e. not: account, calendar resource, distribution list, domain)
   *
   * <p>For checking ACL rights only, domain based access manager will always return OK. This should
   * be called only when domain based permission checking has passed.
   */
  ...
  public abstract void checkAccountRight(
      AdminDocumentHandler handler, Account account, Object needed) throws ServiceException;

  /** calendar resource right (SOAP only) */
  public abstract void checkCalendarResourceRight(
      AdminDocumentHandler handler, CalendarResource cr, Object needed) throws ServiceException;

  /** DL right (SOAP only) */
  public abstract void checkDistributionListRight(
      AdminDocumentHandler handler, DistributionList dl, Object needed) throws ServiceException;

  /** Dynamic group right (SOAP only) */
  public abstract void checkDynamicGroupRight(
      AdminDocumentHandler handler, DynamicGroup group, Object needed) throws ServiceException;

  /**
   * domain right (SOAP only)
   *
   * <p>called by handlers that need to check right on a domain for domain-ed objects: account,
   * alias, cr, dl.
   *
   * <p>Note: this method *does* check domain status.
   */
  public abstract void checkDomainRightByEmail(
      AdminDocumentHandler handler, String email, AdminRight needed) throws ServiceException;

  /** domain right (SOAP only) Note: this method does *not* check domain status. */
  public abstract void checkDomainRight(
      AdminDocumentHandler handler, String domainName, Object needed) throws ServiceException;

  /** domain right (SOAP only) Note: this method does *not* check domain status. */
  public abstract void checkDomainRight(AdminDocumentHandler handler, Domain domain, Object needed)
      throws ServiceException;
  ...
  /**
   * @param target
   * @param ignoreHardRules Set of hard rules that should be ignored. If a specified hard rule is
   *     violated, return an AttrRightChecker that allows no attribute
   * @return
   * @throws ServiceException
   */
  public abstract AccessManager.AttrRightChecker getAttrRightChecker(
      Entry target, Set<HardRule> ignoreHardRules) throws ServiceException;

  /* ================
   * internal methods
   * ================
   */
  private AdminAccessControl(
      AccessManager accessMgr, ZimbraSoapContext zsc, AuthToken authToken, Account authedAcct) {
  ...
  private static AdminAccessControl newAdminAccessControl(
      ZimbraSoapContext zsc, AuthToken authToken, Account authedAcct) {
   ...
   *
   * <p>We can't put this check in ACLAccessManager because some rights, like listAccount,
   * listCalendarResource, listDistributionList that takes a account/cr/dl target object, but should
   * be allowed even when domain status is suspended/shutdown.
   *
   * <p>Note: if target *is* a domain, domain status is *not* checked here. - if domain status is
   * "shutdown": Modify/DeleteDomain would've been already blocked in the SOAP handlers - if domain
   * status is "suspended": Modify/DeleteDomain are allowed/denied by our regular ACL checking:
   * (i.e. system admin or whoever has the right) - for both "shutdown" and "suspended" status:
  ...
  private static class DomainAccessControl extends AdminAccessControl {
    ...
    public void checkModifyAttrs(AttributeClass attrClass, Map<String, Object> attrs)
      ...
      for (String attrName : attrs.keySet()) {
        ...
        try {
          final boolean isDomainAdminModifiable = StoreAttributeManager.getInstance()
              .isDomainAdminModifiable(attrName, attrClass);
          if (!isDomainAdminModifiable) {
    ...
    @Override
    public void checkSetAttrsOnCreate(
        TargetType targetType, String entryName, Map<String, Object> attrs)
        throws ServiceException {
    ...
    @Override
    public void checkCalendarResourceRight(
        AdminDocumentHandler handler, CalendarResource cr, Object needed) throws ServiceException {
      soapOnly();
    ...
    @Override
    public void checkDistributionListRight(
        AdminDocumentHandler handler, DistributionList dl, Object needed) throws ServiceException {
      soapOnly();
    ...
    @Override
    public void checkDynamicGroupRight(
        AdminDocumentHandler handler, DynamicGroup group, Object needed) throws ServiceException {
      soapOnly();
    ...
    @Override
    public void checkDomainRightByEmail(
        AdminDocumentHandler handler, String email, AdminRight needed) throws ServiceException {
      soapOnly();
    ...
    @Override
    public AttrRightChecker getAttrRightChecker(Entry target, Set<HardRule> ignoreHardRules)
        throws ServiceException {
      return null;
    }
  }
  ...
  private static class GlobalAccessControl extends AdminAccessControl {
    ...
    public void checkDomainRight(AdminDocumentHandler handler, String domainName, Object needed)
      ...
      if (domain == null) throw ServiceException.PERM_DENIED("no such domain: " + domainName);
    ...
    @Override
    public AttrRightChecker getAttrRightChecker(Entry target) throws ServiceException {
  ...
  private static class ACLAccessControl extends AdminAccessControl {
    ...
    public void checkSetAttrsOnCreate(
      ...
      boolean hasRight =
          mAccessMgr.canSetAttrsOnCreate(mAuthedAcct, targetType, entryName, attrs, true);
    ...
    /**
     * for an entry to be listed in the Search*** and GetAll*** response, the authed admin needs to
     * have both the "list" right and enough "get attrs" rights.
     */
    @Override
    public boolean hasRightsToList(NamedEntry target, AdminRight listRight, Object getAttrRight)
        throws ServiceException {
        ...
        if (((Set) getAttrRight).isEmpty()) {
          ZimbraLog.acl.warn(
              getClass().getName()
                  ...
                  + target.getName()
                  + ": "
                  + "non of the requested attrs is valid on the entry");
          return false;
        }
      }
    ...
    /**
     * @param target
     * @param needed if instanceof AttrRight : a preset or attr AdminRight Set<String> : attrs to
     *     get Map<String, Object> : attrs to set
     * @return
     * @throws ServiceException
     */
      ...
      if (needed instanceof AdminRight) {
        ...
        if (adminRight.isPresetRight())
          ...
          if (adminRight.getRightType() == Right.RightType.getAttrs) {
            return mAccessMgr.canGetAttrs(
                mAuthedAcct, target, ((AttrRight) needed).getAttrs(), true);
          } else if (adminRight.getRightType() == Right.RightType.setAttrs) {
            // note: this does not check for constraints
            return mAccessMgr.canSetAttrs(
                mAuthedAcct, target, ((AttrRight) needed).getAttrs(), true);
          }
        }
        throw ServiceException.FAILURE("internal error", null);
  ...
  /**
   * A right checker for checking rights for large number of objects. e.g. SearchDirectory,
   * SearchMultiMailbox (all mailboxes on server)
   *
   * <p>We can't use the regular way to check right for those because of bad response time. bug
   * 39514.
   *
   * <p>The regular way will lead to a LDAP search (if not already loaded) to find the groups/domain
   * the target belongs, yuck!
   *
   * <p>Note: this is the only place where permission is computed from an AllEffectiveRights object
    ...
    */
    ...
    private boolean hasRightImplBulk(NamedEntry target, AdminRight rightNeeded)
      ...
      if (mAllEffRights == null) {
        mAllEffRights =
            mProv.getAllEffectiveRights(
                GranteeType.GT_USER.getCode(), GranteeBy.id, mAC.mAuthedAcct.getId(), false, false);
      }
      ...
      if (rbtt == null || rbtt.hasNoRight()) return false;
    ...
    private boolean hasRightBulk(
        RightCommand.EffectiveRights effRights, NamedEntry target, AdminRight rightNeeded) {
      if (effRights == null) return false;
  ...
  public static class SearchDirectoryRightChecker extends BulkRightChecker {
    ...
    public SearchDirectoryRightChecker(
        AdminAccessControl accessControl, Provisioning prov, Set<String> reqAttrs)
        throws ServiceException {
      ...
      ignoreHardRules.put(
          Admin.R_listAccount, EnumSet.of(HardRule.DELEGATED_ADMIN_CANNOT_ACCESS_GLOBAL_ADMIN));
      ignoreHardRules.put(
          Admin.R_listCalendarResource,
    ...
    private boolean hasRightsToListAlias(Alias alias) throws ServiceException {
      ...
      else return hasRight(domain, Admin.R_listAlias);
    }
    ...
    /**
     * returns a new list that contains only allowed entries from the input list. Deprecated for 8.7
     * left here in case 3rd party code still uses it.
     */
    @Deprecated
  ...
  static class AttributeRightChecker implements AccessManager.AttrRightChecker {
    ...
    private AttributeRightChecker(AdminAccessControl accessControl, Entry target)
        throws ServiceException {
      mRightChecker =
          accessControl.mAccessMgr.getGetAttrsChecker(accessControl.mAuthedAcct, target, true);
    }
   ...
   *
   * <p>TODO: migrate check*Right callsites that are calling the Set/Map interface to use this
   * class, instead of buidling the Set/Map at the call site.
   *
   * <p>e.g. in a handler
   *
   * <p>checkAccountRight(zsc, acct, needRight());
   *
   * <p>private SetAttrsRight needRight() { SetAttrsRight sar = new SetAttrsRight();
   * sar.addAttr(Provisioning.A_zimbraAdminSavedSearches); return sar; }
   */
