...
public class ModifyZimlet extends AdminDocumentHandler {
  ...
  void doAcl(ZimbraSoapContext zsc, Map<String, Object> context, Element z)
    ...
    if (cos == null) throw AccountServiceException.NO_SUCH_COS(cosName);
    checkRight(zsc, context, cos, Admin.R_manageZimlet);

    String acl = a.getAttribute(AdminConstants.A_ACL, null);
    if (acl == null) throw ServiceException.INVALID_REQUEST("missing acl attribute", null);
    acl = acl.toLowerCase();
    try {
      if (acl.equals("grant")) {
  ...
  void doStatus(ZimbraSoapContext zsc, Map<String, Object> context, Element z)
      throws ServiceException {
    String name = z.getAttribute(AdminConstants.A_NAME);

    Zimlet zimlet = Provisioning.getInstance().getZimlet(name);
    if (z == null) throw AccountServiceException.NO_SUCH_ZIMLET(name);
    ...
    attrRightNeeded.put(
        ...
        status ? ProvisioningConstants.TRUE : ProvisioningConstants.FALSE);
  ...
  void doPriority(ZimbraSoapContext zsc, Map<String, Object> context, Element z)
      throws ServiceException {
    String name = z.getAttribute(AdminConstants.A_NAME);

    Zimlet zimlet = Provisioning.getInstance().getZimlet(name);
    if (zimlet == null) throw AccountServiceException.NO_SUCH_ZIMLET(name);
  ...
  public void docRights(List<AdminRight> relatedRights, List<String> notes) {
    ...
    notes.add(
        "For status: needs right to set " + Provisioning.A_zimbraZimletEnabled + " on the zimlet");
    notes.add(
        "For priority: needs right to set "
            + Provisioning.A_zimbraZimletPriority
            + " on *all* zimlets, "
