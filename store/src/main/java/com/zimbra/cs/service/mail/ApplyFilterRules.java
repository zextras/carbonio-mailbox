...
public class ApplyFilterRules extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (msgEl != null && query != null) {
      String msg =
          String.format(
              "Cannot specify both %s and %s elements.",
    ...
    if (msgEl != null) {
      ...
      try (ZimbraQueryResults results =
          mbox.index.search(
              octxt, query, EnumSet.of(MailItem.Type.MESSAGE), SortBy.NONE, Integer.MAX_VALUE)) {
      ...
      String msg =
          String.format(
              "Must specify either the %s or %s element.",
              MailConstants.E_MSG, MailConstants.E_QUERY);
      throw ServiceException.INVALID_REQUEST(msg, null);
    }
    ...
    if (messageIds.size() > max) {
      throw ServiceException.INVALID_REQUEST(
          "Attempted to apply filter rules to "
              + messageIds.size()
              + " messages, which exceeded the limit of "
              + max,
          null);
    }
