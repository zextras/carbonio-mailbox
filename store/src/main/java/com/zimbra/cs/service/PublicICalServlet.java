...
public class PublicICalServlet extends ZimbraServlet {
  ...
  public final void doGetFreeBusy(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (checkBlankOrNull(resp, QP_ACCOUNT, acctName)) return;
    ...
    if (startStr != null) rangeStart = Long.parseLong(startStr);

    if (endStr != null) rangeEnd = Long.parseLong(endStr);
    ...
    if (days > maxDays) {
      resp.sendError(
          ...
          "Requested range is too large (max " + maxDays + " days)");
      return;
    }
    ...
    try {
      ...
      if (targetAccount != null) targetAccountId = targetAccount.getId();
    ...
    if (at != null) {
      ...
      try {
        ...
        resp.sendError(
            HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error initializing request context");
        return;
      }
    } else {
    ...
    FreeBusy fb = null;
    if (result.size() > 0) fb = result.iterator().next();
    else fb = FreeBusy.emptyFreeBusy(acctName, rangeStart, rangeEnd);
  ...
  public final void doReply(HttpServletRequest req, HttpServletResponse resp) {
    ...
    //            if (acct == null) {
    //                 resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Account "+org+" not
    // found");
    //                return;
    //            }
    //
    ...
    //            } else {
    //                resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid invId:
    // "+oldInvId);
    //                return;
    //            }
    //
    ...
    //            Invite reply = new Invite(Method.REPLY, new
    // TimeZoneMap(oldInv.getTimeZoneMap().getLocalTimeZone()));
    ...
    //            } else {
    //                resp.sendError(HttpServletResponse.SC_BAD_REQUEST, at+" not on Attendee
    // list");
    //                return;
    //            }
    //
    ...
    //
    //            // DTSTAMP
    //            // we should pick "now" -- but the dtstamp MUST be >= the one sent by the
    // organizer,
    ...
    //            try {
    //                MimeMessage mm = SendInviteReply.createDefaultReply(at, oldInv, replySubject,
    // verb, iCal);
    //
