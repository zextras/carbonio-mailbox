...
public final class GetContacts extends MailDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    if (folderIdStr != null) {
      ...
      if (iidFolder.belongsTo(mbox)) folderId = iidFolder.getId();
      else
        throw ServiceException.FAILURE(
            "Got remote folderId: " + folderIdStr + " but did not proxy", null);
    }
    ...
    if (derefContactGroupMember) {
      if (ids == null) {
        throw ServiceException.INVALID_REQUEST(
            MailConstants.A_DEREF_CONTACT_GROUP_MEMBER
                + " is supported only when specific contact ids are specified",
            null);
      }
    }
    ...
    if (ids != null) {
      ...
      if (remote.size() > 0) {
        if (folderId > 0)
          throw ServiceException.INVALID_REQUEST(
              "Cannot specify a folder with mixed-mailbox items", null);
        ...
        if (con != null) {
          ToXML.encodeContact(
              ...
              ContactMemberOfMap.setOfMemberOf(
                  zsc.getRequestedAccountId(), con.getId(), memberOfMap));
        }
      }
    }
    return response;
  ...
  static void partitionItems(
      ...
      ArrayList<ItemId> ids,
      ArrayList<Integer> local,
      Map<String, StringBuffer> remote)
      throws ServiceException {
    Account acct = getRequestedAccount(lc);
    for (ItemId iid : ids) {
      if (iid.belongsTo(acct)) local.add(iid.getId());
      else {
        StringBuffer sb = remote.get(iid.getAccountId());
        if (sb == null) remote.put(iid.getAccountId(), new StringBuffer(iid.toString()));
        else sb.append(',').append(iid.toString());
      }
    }
  }

  List<Element> proxyRemote(
      Element request, Map<String, StringBuffer> remote, Map<String, Object> context)
      throws ServiceException {
