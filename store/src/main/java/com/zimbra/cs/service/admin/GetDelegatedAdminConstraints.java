...
public class GetDelegatedAdminConstraints extends AdminDocumentHandler {
  ...
  public Element handle(Element request, Map<String, Object> context) throws ServiceException {
    ...
    for (Element a : request.listElements(AdminConstants.E_A)) {
      ...
      if (ac != null) {
        Element eAttr =
            response.addElement(AdminConstants.E_A).addAttribute(AdminConstants.A_NAME, attrName);
        ac.toXML(eAttr);
      }
    }
  ...
  static Entry getEntry(Element request) throws ServiceException {
    ...
    if (type == TargetType.config) {
      ...
      if (id != null) {
        ...
        if (cos == null) throw AccountServiceException.NO_SUCH_COS(id);

        if (name != null) {
          if (!name.equals(cos.getName()))
            throw ServiceException.INVALID_REQUEST(
                "Specified name " + name + " does not match entry name for the specified id " + id,
                null);
        }
        ...
        if (cos == null) throw AccountServiceException.NO_SUCH_COS(name);

        if (id != null) {
          if (!id.equals(cos.getId()))
            throw ServiceException.INVALID_REQUEST(
                "Specified id " + id + " does not match id for the specified name " + name, null);
        }

        return cos;
      } else throw ServiceException.INVALID_REQUEST("neither id or name is specified", null);

    } else throw ServiceException.INVALID_REQUEST("invalid type " + typeStr, null);
  }
