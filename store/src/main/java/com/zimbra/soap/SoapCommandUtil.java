...
public class SoapCommandUtil implements SoapTransport.DebugListener {
  ...
  private static final Map<String, Namespace> sTypeToNamespace = new TreeMap<String, Namespace>();

  private static final String DEFAULT_ADMIN_URL =
      String.format(
          "https://%s:%d/service/admin/soap",
          LC.zimbra_zmprov_default_soap_server.value(), LC.zimbra_admin_service_port.intValue());
  ...
  SoapCommandUtil() {
    ...
    Option opt =
        new Option(
            ...
            LO_MAILBOX,
            true,
            "Send mail and account requests to this account.  "
    ...
    opt =
        new Option(
            null, LO_AUTH, true, "Account name to authenticate as.  Defaults to account in -m.");
    opt.setArgName("account-name");
    mOptions.addOption(opt);

    mOptions.addOption(
        new Option("A", LO_ADMIN_PRIV, false, "Execute requests with admin privileges."));
    ...
    mOptions.addOption(
        new Option("n", LO_NO_OP, false, "Print the SOAP request only.  Don't send it."));
    ...
    mOptions.addOption(
        new Option(
            null, LO_JAXB, false, "Force use of JAXB to aid building request from command line."));
    ...
    opt =
        new Option(
            ...
            LO_FILE,
            true,
            "Read request from file.  For JSON, the request pair should be the child of the root"
                + " object.");
    opt.setArgName("path");
    mOptions.addOption(opt);
  ...
  private void usage(String errorMsg) {
    ...
    format.printHelp(
        ...
        80,
        "zmsoap [options] xpath1 [xpath2 xpath3 ...]",
        null,
        mOptions,
        2,
        2,
  ...
  private void parseCommandLine(String[] args) throws ParseException {
    ...
    if (CliUtil.hasOption(cl, LO_JAXB)) {
      ...
      // zmsoap's command line fits reasonably well with how Zimbra XML SOAP works, so there
      // shouldn't be much
      // value in using JAXB to validate the request before sending it in that case.  However the
      // fit is NOT
      // that good with how Zimbra JSON SOAP works - for instance there isn't an obvious way to
      // specify
      // key value pairs.  passing the XML equivalent of the comand line spec into JAXB and getting
      // the JSON
  ...
  private static final String[] XPATH_PASSWORD =
      new String[] {"Body", AdminConstants.AUTH_REQUEST.getName(), AdminConstants.E_PASSWORD};
  ...
  private void adminAuth() throws ServiceException, IOException, HttpException {
    // Create auth element
    ...
    if (!mType.equals(TYPE_ADMIN) && !StringUtil.isNullOrEmpty(mAuthAccountName)) {
      ...
      account.addAttribute(
          AdminConstants.A_BY, nameIsUUID ? AdminConstants.BY_ID : AdminConstants.BY_NAME);
      if (mVeryVerbose) {
  ...
  private void mailboxAuth() throws ServiceException, IOException, HttpException {
    if (mVeryVerbose) {
    ...
    account.addAttribute(
        AdminConstants.A_BY,
        StringUtil.isUUID(mAuthAccountName) ? AdminConstants.BY_ID : AdminConstants.BY_NAME);
  ...
  private void run() throws ServiceException, IOException, HttpException {
   ...
   *
   * <p><tt>element1/element2[/@attr][=value]</tt> If a value is specified, it sets the text of the
   * last element or attribute in the path.
   *
