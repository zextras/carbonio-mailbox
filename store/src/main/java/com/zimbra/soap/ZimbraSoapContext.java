...
public final class ZimbraSoapContext {
  ...
  public ZimbraSoapContext(
      AuthToken authToken, String accountId, SoapProtocol reqProtocol, SoapProtocol respProtocol)
   ...
   *
   * <p>Hop count is not checked for TOO_MANY_HOPS in this route.
   */
  public ZimbraSoapContext(
      AuthToken authToken,
      String accountId,
      SoapProtocol reqProtocol,
      SoapProtocol respProtocol,
      int hopCount)
      throws ServiceException {
  ...
  /**
   * Creates a {@link ZimbraSoapContext} from another existing {@link ZimbraSoapContext} for use in
   * proxying.
   *
  ...
  /**
   * Creates a <code>ZimbraSoapContext</code> from another existing <code>ZimbraSoapContext</code>
   * for use in proxying. If session is non-null, it will be used for proxy notifications. If
   * authToken is not null, the auth token in the clone will be replaced by authToken.
   */
  public ZimbraSoapContext(
      ZimbraSoapContext zsc, AuthToken authToken, String targetAccountId, Session session)
      throws ServiceException {
    ...
    if (session != null) {
      ...
      mSessionInfo =
          new SessionInfo(
              ...
              (session instanceof SoapSession
                  ? ((SoapSession) session).getCurrentNotificationSequence()
                  : 0),
              false);
    }
  }
  ...
  /**
   * Creates a {@link ZimbraSoapContext} from the {@code <context>} {@link Element} from the SOAP
   * header.
   *
  ...
  public ZimbraSoapContext(
    ...
    try {
      ...
      if (mAuthToken != null) {
        ...
        if (isExpired || !isRegistered) {
          ...
          if (ctxt != null) {
            Element eAuthTokenControl =
                ctxt.getOptionalElement(HeaderConstants.E_AUTH_TOKEN_CONTROL);
            if (eAuthTokenControl != null) {
              voidOnExpired =
                  eAuthTokenControl.getAttributeBool(HeaderConstants.A_VOID_ON_EXPIRED, false);
            }
          }
      ...
      mAuthToken = null;
      if (sLog.isDebugEnabled()) sLog.debug("ZimbraContext AuthToken error: " + e.getMessage(), e);
    }
    ...
    if (change != null) {
      try {
        ...
        if (change
            .getAttribute(HeaderConstants.A_TYPE, HeaderConstants.CHANGE_MODIFIED)
            .equals(HeaderConstants.CHANGE_MODIFIED))
          mChangeConstraintType = OperationContext.CHECK_MODIFIED;
        else mChangeConstraintType = OperationContext.CHECK_CREATED;
    ...
    String targetServerId =
        ctxt == null ? null : ctxt.getAttribute(HeaderConstants.E_TARGET_SERVER, null);
    ...
    if (notify != null) seqNo = (int) notify.getAttributeLong(HeaderConstants.A_SEQNO, 0);
    ...
    if (ctxt != null) {
      ...
      // if sessions are enabled, create a SessionInfo to encapsulate (will fetch the Session object
      // during the request)
      if (!suppress) {
        ...
        if (session != null) {
          ...
          if (sessionId != null) {
            mSessionInfo =
                new SessionInfo(
                    sessionId,
                    (int) session.getAttributeLong(HeaderConstants.A_SEQNO, seqNo),
                    false);
          }
          mNotificationFormat =
              session.getAttribute(HeaderConstants.E_FORMAT, DEFAULT_NOTIFICATION_FORMAT);
    ...
    mUnqualifiedItemIds =
        (ctxt != null && ctxt.getOptionalElement(HeaderConstants.E_NO_QUALIFY) != null);

    // Handle user agent if specified by the client.  The user agent string is formatted
    // as "name/version".
    Element userAgent =
        (ctxt == null ? null : ctxt.getOptionalElement(HeaderConstants.E_USER_AGENT));
    if (userAgent != null) {
  ...
  /**
   * tells SoapSession how to format notification elements in SOAP headers. Remote IMAP server uses
   * IMAP format, whereas default SOAP and JSON clients use default format.
   *
   * @return
   */
  ...
  /**
   * WaitSetSession will NOT trigger a response for this Waitset, since the notifications will be
   * returned in SOAP headers instead. This ensures that the the remote server isn't notified of the
   * same notifications twice using different mechanisms.
   */
  ...
  /**
   * Validate delegation rights. Request for delegated access requires a grant on at least one
   * object in the target account or admin login rights.
   *
   * @param targetAccount - Account which requested is targeted for
   * @param requestedKey - The key sent in request which mapped to target account. Passed in so
   *     error only reports back what was requested (i.e. can't harvest accountId if you only know
   *     the email or vice-versa)
   * @param requestName - The SOAP request name - may be null
   * @throws ServiceException
   */
  private void validateDelegatedAccess(
      Account targetAccount, DocumentHandler handler, QName requestName, String requestedKey)
      throws ServiceException {
    ...
    // 3. target account has shared at least one item with authed account or enclosing
    // group/cos/domain
    ...
    if (!GuestAccount.GUID_PUBLIC.equals(mAuthToken.getAccountId())) {
      ...
      if (isAdmin
          && AccessManager.getInstance().canAccessAccount(mAuthToken, targetAccount, true)) {
        // case 1 - admin
        return;
      }
    ...
    if (authAccount != null
        && authAccount.getBooleanAttr(Provisioning.A_zimbraIsExternalVirtualAccount, false)) {
      externalEmail =
          authAccount.getAttr(Provisioning.A_zimbraExternalUserMailAddress, externalEmail);
    }
    ...
    do {
      ...
      if (needRecheck) {
        break;
      } else if (!Provisioning.getInstance().onLocalServer(targetAccount)) {
  ...
  /**
   * Records in this context that we've traversed a mountpoint to get here. Enforces the "cannot
   * mount a mountpoint" rule forbidding one link from pointing to another link. The rationale for
   * this limitation is that we want to avoid having the indirectly addressed resource completely
   * change without the user knowing, and we want to know that the target of the link is also the
   * owner of the items found therein. Basically, things are just simpler this way.
   *
   * @throws ServiceException The following error codes are possible:
   *     <ul>
   *       <li><tt>service.PERM_DENIED</tt> - if you try to traverse two mountpoints in the course
   *           of a single proxy
   *     </ul>
   */
  public void recordMountpointTraversal() throws ServiceException {
    if (mMountpointTraversed) throw ServiceException.PERM_DENIED("cannot mount a mountpoint");
  ...
  /**
   * Returns the account id the request is supposed to operate on. This can be explicitly specified
   * in the supplied context; it defaults to the account id in the auth token.
   */
  ...
  /**
   * Returns the id of the account in the auth token. Operations should normally use {@link
   * #getRequestedAccountId}, as that's the context that the operations is executing in.
   */
  ...
  /**
   * Sets the AuthToken of the authenticated account. This should only be called in the course of
   * some flavor of <tt>Auth</tt> request.
   */
  ...
  /**
   * Returns whether the authenticated user is the same as the user whose context the operation is
   * set to execute in.
   */
  public boolean isDelegatedRequest() {
    return mAuthTokenAccountId != null
        && !mAuthTokenAccountId.equalsIgnoreCase(getRequestedAccountId());
  }
  ...
  /**
   * Returns the {@link SessionInfo} item associated with this SOAP request. SessionInfo objects
   * correspond to either:
   *
   * <ul>
   *   <li>sessions specified in a <tt>&lt;sessionId></tt> element in the <tt>&lt;context></tt> SOAP
   *       header block, or
   *   <li>new sessions created during the course of the SOAP call
   * </ul>
   */
  ...
  SessionInfo recordNewSession(String sessionId) {
    if (!mSessionEnabled || sessionId == null) {
      ...
      mSessionInfo.sessionId = sessionId;
      mSessionInfo.created = true;
    }
    return mSessionInfo;
  }
   ...
   *
   * <p>This special-case API is used to short-circuit blocking handlers so that they return
   * immediately to send a {@code <refresh>} block if one is needed.
   */
   ...
   *
   * <p>The attributes encapsulated by the {@link ZimbraSoapContext} -- the response protocol, the
   * auth token, etc. are carried forward, except that if {@code excludeAccountDetails} is set, any
   * account details are omitted.
   */
  Element toProxyContext(SoapProtocol proto, boolean excludeAccountDetails) {
    ...
    if (mResponseProtocol != mRequestProtocol) {
      ctxt.addNonUniqueElement(HeaderConstants.E_FORMAT)
          .addAttribute(
    ...
    if (!mSessionEnabled) {
      ...
      encodeSession(ctxt, mSessionInfo.sessionId, null)
          .addAttribute(HeaderConstants.A_PROXIED, true);
    } else {
    ...
    if (!excludeAccountDetails) {
      Element eAcct =
          ctxt.addNonUniqueElement(HeaderConstants.E_ACCOUNT)
              .addAttribute(HeaderConstants.A_MOUNTPOINT, mMountpointTraversed);
      if (mRequestedAccountId != null
          && !mRequestedAccountId.equalsIgnoreCase(mAuthTokenAccountId)) {
        eAcct
            .addAttribute(HeaderConstants.A_BY, HeaderConstants.BY_ID)
            .setText(mRequestedAccountId);
      }
    }
   ...
   *
   * <p>The attributes encapsulated by the {@link ZimbraSoapContext} -- the response protocol, the
   * auth token, etc. -- are carried forward.
   */
   ...
   *
   * <p>The serialized XML representation of a Session is: {@code <sessionId [type="admin"]
   * id="12">12</sessionId>}
   *
  ...
  public static Element encodeSession(Element parent, String sessionId, Session.Type sessionType) {
    ...
    if (oldSession != null) oldSession.detach();
    ...
    eSession
        .addAttribute(HeaderConstants.A_TYPE, typeStr)
        .addAttribute(HeaderConstants.A_ID, sessionId)
        .setText(sessionId);
    return eSession;
  }
  ...
  /**
   * Only use this for response objects (or requests) {@link jaxbToNamedElement} should be used for
   * all other cases.
   */
  ...
  /**
   * Use this rather than {@link jaxbToElement} when dealing with a class that is not in the JAXB
   * context. This will be true if the class is not for a top level Soap request or response.
   */
   ...
   *
   * <p>This can come either from an HTTP cookie attached to the SOAP request or from an {@code
   * <authToken>} element in the SOAP Header.
   */
   ...
   *
   * <p>This can come either from an HTTP cookies attached to the SOAP request or from an {@code
   * <authToken>} element in the SOAP Header.
   */
  ...
  /**
   * Returns the name and version of the client that's making the current request, in the format
   * "name/version".
   */
  ...
  private void setHopCount(int hopCount) throws ServiceException {
    if (hopCount > MAX_HOP_COUNT) throw ServiceException.TOO_MANY_HOPS(mRequestedAccountId);
    mHopCount = hopCount;
  }
  ...
  public boolean isAuthUserOnLocalhost() {
    if (mAuthTokenAccountId != null) {
      try {
        ...
        if (a != null) {
          return Provisioning.getInstance().onLocalServer(a);
        }
