...
public class SoapEngine {
   ...
   *
   * <p>It can be the IP of the SOAP client, or in the presence of a real origin IP address http
   * header(X-ORIGINATING-IP) the IP of the real origin client if the SOAP client is in a trusted
   * network.
   */
  ...
  private void logRequest(Map<String, Object> context, Element envelope) {
    if (ZimbraLog.soap.isTraceEnabled() && !context.containsKey(SoapEngine.SOAP_REQUEST_LOGGED)) {
      ...
      boolean isResumed = servletRequest.getAttribute("com.zimbra.request.asyncResumed") != null;
      ZimbraLog.soap.trace(!isResumed ? "C:\n%s" : "C: (resumed)\n%s", envelope.prettyPrint(true));
      context.put(SOAP_REQUEST_LOGGED, Boolean.TRUE);
    }
  }

  private void logUnparsableRequest(
      Map<String, Object> context, byte[] soapMessage, String parseError) {
  ...
  /**
   * Bug 77304 - If the XML for a Soap Request was bad, look at it to see if enough of it is valid
   * to be able to determine the desired response protocol. Use StAX parsing so that we can stop
   * looking at the XML once we have got past the Envelope Header context.
   */
  ...
  public Element dispatch(String path, byte[] soapMessage, Map<String, Object> context)
    ...
    if (soapMessage == null || soapMessage.length == 0) {
      ...
      return soapFaultEnv(
          soapProto, "SOAP exception", ServiceException.PARSE_ERROR("empty request payload", null));
    }
    ...
    try {
      ...
      return soapFaultEnv(
          soapProto, "SOAP exception", ServiceException.PARSE_ERROR(e.getMessage(), e));
  ...
  /**
   * dispatch to the given serviceName the specified document, which should be a soap envelope
   * containing a document to execute.
   *
  ...
  Element dispatchRequest(
    ...
    if (RedoLogProvider.getInstance().isSlave() && !handler.isReadOnly()) {
      return soapFault(
          soapProto, "cannot dispatch request", ServiceException.NON_READONLY_OPERATION_DENIED());
    }
    AuthToken at = zsc.getAuthToken();
    ...
    try {
      ...
      if (!prov.getLocalServer().getBooleanAttr(Provisioning.A_zimbraUserServicesEnabled, true)
        ...
        return soapFault(
            soapProto, "cannot dispatch request", ServiceException.TEMPORARILY_UNAVAILABLE());
      }
      if (needsAdminAuth) {
      ...
      if (!isGuestAccount) {
        ...
        if (needsAuth || needsAdminAuth) {
          ...
          if (zsc.isDelegatedRequest() && !handler.isAdminCommand()) {
            ...
            // treat the account as inactive if (a) it doesn't exist, (b) it's in maintenance mode,
            // or (c) we're non-admins and it's not "active"
            boolean inactive =
                target == null
                    || Provisioning.ACCOUNT_STATUS_MAINTENANCE.equals(
                        target.getAccountStatus(prov));
            if (!inactive
                && (!at.isAdmin() || !AccessManager.getInstance().canAccessAccount(at, target))) {
            ...
            if (inactive) {
              return soapFault(
                  ...
                  AccountServiceException.ACCOUNT_INACTIVE(
                      target == null ? zsc.getRequestedAccountId() : target.getName()));
            }
          }
        }
      ...
      if (response == null) {
        ...
        if (LC.zimbra_slow_logging_enabled.booleanValue()
          ...
          ZimbraLog.soap.warn(
              "Slow SOAP request (start=" + startTime + "):\n" + soapReqElem.prettyPrint(true));
          ZimbraLog.soap.warn(
              "Slow SOAP response (time=" + duration + "):\n" + response.prettyPrint());
        }
      }
    } catch (SoapFaultException e) {
      response =
          e.getFault() != null
              ? e.getFault().detach()
              : soapProto.soapFault(ServiceException.FAILURE(e.toString(), e));
      if (!e.isSourceLocal()) {
  ...
  /**
   * Returns the SOAP command name. In most cases this is just the name of the given request
   * element. If this request is an <tt>XXXActionRequest</tt>, appends the operation to the request
   * name.
   */
  ...
  /**
   * Creates a <tt>&lt;context></tt> element for the SOAP Header containing session information and
   * change notifications.
   *
   * <p>Sessions -- those passed in the SOAP request <tt>&lt;context></tt> block and those created
   * during the course of processing the request -- are listed:
   *
   * <p><tt>&lt;sessionId [type="admin"] id="12">12&lt;/sessionId></tt>
   *
   * @return A new <tt>&lt;context></tt> {@link Element}, or <tt>null</tt> if there is no relevant
   *     information to encapsulate.
   */
  private Element generateResponseHeader(ZimbraSoapContext zsc) {
    ...
    try {
        ...
        if (session instanceof SoapSession) {
          ...
          if (sinfo.created || soap.requiresRefresh(sinfo.sequence)) {
            ZimbraLog.session.debug(
                "returning refresh block; reason=%s",
      ...
      // we still want to encode the <change> element at least for the directly-requested
      // accountId...
      // so encode it here as a last resort
      if (requiresChangeHeader) {
        try {
          String explicitAcct =
              requestedAccountId.equals(authAccountId) ? null : requestedAccountId;
