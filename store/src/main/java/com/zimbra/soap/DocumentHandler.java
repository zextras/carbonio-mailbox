...
public abstract class DocumentHandler {
  ...
  public void postProxy(Element request, Element response, Map<String, Object> context)
      throws ServiceException {}

  public abstract Element handle(Element request, Map<String, Object> context)
      throws ServiceException;

  /**
   * Returns the {@link ZimbraSoapContext} object encapsulating the containing SOAP request's
   *
   * <pre>&lt;context></pre>
   *
   * header element.
  ...
  /**
   * Generates a new {@link com.zimbra.cs.mailbox.OperationContext} object reflecting the
   * constraints serialized in the <tt>&lt;context></tt> element in the SOAP header.
   *
   * <p>These optional constraints include:
   *
   * <ul>
   *   <li>the account ID from the auth token
   *   <li>the highest change number the client knows about
   *   <li>how stringently to check accessed items against the known change highwater mark
   * </ul>
   *
   * @return A new OperationContext object
   */
  public static OperationContext getOperationContext(
      ZimbraSoapContext zsc, Map<String, Object> context) throws ServiceException {
    return getOperationContext(
        zsc, context == null ? null : (Session) context.get(SoapEngine.ZIMBRA_SESSION));
  }

  public static OperationContext getOperationContext(ZimbraSoapContext zsc, Session session)
      throws ServiceException {
  ...
  /**
   * Returns the {@link Account} corresponding to the authenticated user. The authenticated user is
   * determined from the serialized {@link com.zimbra.cs.account.AuthToken} in the SOAP request's
   *
   * <pre>&lt;context></pre>
   *
   * header element.
  ...
  public static Mailbox getRequestedMailbox(ZimbraSoapContext zsc, boolean autoCreate)
      throws ServiceException {
    String id = zsc.getRequestedAccountId();
  ...
  /**
   * Returns whether this is an administrative command (and thus requires a valid admin auth token).
   */
  ...
  /**
   * Some requests need to always return valid looking output in able to provide consistent behavior
   * between requests against non-existent accounts and those which don't allow access to the
   * requested data. For instance FreeBusy requests. Only handlers which accept responsibility for
   * measures to prevent account harvesting should return true
   */
  ...
  /**
   * Only handlers which accept responsibility for measures to prevent account harvesting by
   * delegate admins should return true
   */
  ...
  public boolean canModifyOptions(ZimbraSoapContext zsc, Account acct) throws ServiceException {
    if (zsc.isDelegatedRequest()) {
      ...
      //   *and* we need to be able to change our own options (this is a standin for finer-grained
      // access control)
      return canAccessAccount(zsc, acct)
          && getAuthenticatedAccount(zsc)
              .getBooleanAttr(Provisioning.A_zimbraFeatureOptionsEnabled, true);
    } else {
  ...
  /**
   * Returns whether domain admin auth is sufficient to run this command. This should be overriden
   * only on admin commands that can be run in a restricted "domain admin" mode.
   */
  ...
  /**
   * Returns <tt>true</tt> if the operation is read-only, or <tt>false</tt> if the operation causes
  ...
  /**
   * Updates the {@link ZimbraSoapContext} to treat the specified account as the request's
   * authenticated account. If the new account differs from the previously authenticated account, we
   * forget about all other {@link Session}s. (Those sessions are not deleted from the cache, though
   * perhaps that's the right thing to do...) If requested, also creates a new Session object
  ...
  public Session updateAuthenticatedAccount(
      ZimbraSoapContext zsc, AuthToken authToken, Map<String, Object> context, boolean getSession) {
  ...
  /**
   * Fetches the in-memory {@link Session} object appropriate for this request. If none already
   * exists, one is created if possible.
   *
   * @param zsc The encapsulation of the SOAP request's <tt>&lt;context</tt> element.
  ...
  /**
   * Fetches a {@link Session} object to persist and manage state between SOAP requests. If no
   * appropriate session already exists, a new one is created if possible.
   *
   * @param zsc The encapsulation of the SOAP request's <tt>&lt;context</tt> element.
   * @param stype The type of session needed.
   * @return An in-memory {@link Session} object of the specified type, referenced by the request's
   *     {@link ZimbraSoapContext} object, or <tt>null</tt>.
  ...
  protected Session getSession(ZimbraSoapContext zsc, Session.Type stype) {
    ...
    // if they asked for a SOAP session on a remote host and it's a non-proxied request, we don't
    // notify
  ...
  protected static void setXPath(Element request, String[] xpath, String value)
      throws ServiceException {
  ...
  protected Element proxyIfNecessary(Element request, Map<String, Object> context)
    ...
    if (acctId != null
      ...
      if (authAcct == null) {
        ZimbraLog.soap.info(
            "Proxying request: requestedAccountId=%s authAcct <null> reason: %s",
            acctId, reasons.getReason());
      } else {
        ZimbraLog.soap.info(
            "Proxying request: requestedAccountId=%s authAcct name=%s id=%s reason: %s",
  ...
  protected Element proxyRequest(Element request, Map<String, Object> context, String acctId)
      throws ServiceException {
    ZimbraSoapContext zsc = getZimbraSoapContext(context);
  ...
  protected Element proxyRequest(
      Element request, Map<String, Object> context, AuthToken authToken, String acctId)
      throws ServiceException {
    ZimbraSoapContext zsc = getZimbraSoapContext(context);
  ...
  protected String getProxyAuthToken(String requestedAccountId, Map<String, Object> context)
      throws ServiceException {
    return Provisioning.getInstance().getProxyAuthToken(requestedAccountId, context);
  }

  protected Element proxyRequest(
      Element request, Map<String, Object> context, Server server, ZimbraSoapContext zsc)
      throws ServiceException {
    ...
    if (zsc.getRequestedAccountId() != null) {
      try {
        ...
        if (at != null
            && (at.getProxyAuthToken() == null || !at.getProxyAuthToken().equals(proxyToken))) {
    ...
    if (isLocal && engine != null) {
      ...
      if (ZimbraLog.soap.isDebugEnabled()) {
        ZimbraLog.soap.debug(
            "Proxying request locally: targetServer=%s (id=%s) localHost=%s (id=%s)",
      ...
      if (zsc.getResponseProtocol().isFault(response)) {
        zsc.getResponseProtocol()
            .updateArgumentsForRemoteFault(response, zsc.getRequestedAccountId());
      ...
      response =
          proxyWithNotification(
              request, proxy, zsc, (Session) context.get(SoapEngine.ZIMBRA_SESSION));
  ...
  public static Element proxyWithNotification(
      Element request, ProxyTarget proxy, ZimbraSoapContext zscProxy, ZimbraSoapContext zscInbound)
      throws ServiceException {
    return proxyWithNotification(request, proxy, zscProxy, getReferencedSession(zscInbound));
  }

  public static Element proxyWithNotification(
      Element request, ProxyTarget proxy, ZimbraSoapContext zscProxy, Session localSession)
      throws ServiceException {
    ...
    if (zscProxy.isNotificationEnabled()) {
      ...
      // note that requests proxied to *this same host* shouldn't request notification, as the
      // existing local session collects the notifications
  ...
  /**
   * This is for logging usage only: returns the account name if the account can be found by acctId,
   * otherwise returns the acctId if not null, otherwise returns empty string.
   *
   * @param prov
   * @param acctId
   * @return
  ...
  public void logAuditAccess(String delegatingAcctId, String authedAcctId, String targetAcctId) {
    if (!ZimbraLog.misc.isInfoEnabled()) return;

    // 8 => "Response".length()
    String reqName =
        responseQName
            .getQualifiedName()
            .substring(0, responseQName.getQualifiedName().length() - 8);
  ...
  protected static Pair<String, Version> zimbraConnectorClientVersion(ZimbraSoapContext zsc) {
    ...
    if (ua != null) {
      ...
      if (parts.length == 2) {
        ...
        if (UA_ZCO.equalsIgnoreCase(app)
