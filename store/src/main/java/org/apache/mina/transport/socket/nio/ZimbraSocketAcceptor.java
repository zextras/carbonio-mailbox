 ...
 *
 * <p>The original {@code NioSocketAcceptor} does not accept a pre-bound server socket when starting
 * a new listener. This is necessary in order for us to pre-bind server socket channel to privileged
 * port as {@code root} using Jetty's {@code setuid} extension before starting the server. To
 * address this, this implementation accepts a pre-bound {@link ServerSocketChannel}.
 *
 * <p>Since this implementation needs to access package private classes of {@code
 * org.apache.mina.transport.socket.nio}, this class is located in the same package. To avoid
 * security check by JRE, this class needs to be loaded by the same class loader as {@code
 * mina-*.jar}, i.e. do not put {@code mina-*.jar} in {@code jetty/common/}.
 *
 * @author ysasaki
 */
public final class ZimbraSocketAcceptor
    extends AbstractPollingIoAcceptor<NioSession, ServerSocketChannel> implements SocketAcceptor {
  ...
  public ZimbraSocketAcceptor(
      ServerSocketChannel channel, Executor executor, IoProcessor<NioSession> processor) {
