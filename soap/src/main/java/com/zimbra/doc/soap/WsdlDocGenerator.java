...
public class WsdlDocGenerator {
  ...
  static {
    ...
    serviceDescriptions.put(
        ...
        "The Account Service includes commands for retrieving, storing and managing user account"
            + " information.");
    serviceDescriptions.put(
    ...
    serviceDescriptions.put(
        ...
        "The zimbraSync Service includes commands for managing devices using Synchronization.");
    serviceDescriptions.put(
        VoiceConstants.NAMESPACE_STR,
        "The zimbraVoice Service includes commands related to Unified Communications.");
  }
  ...
  private static void populateWithJavadocInfo(
      DescriptionNode node, Map<String, ApiClassDocumentation> javadocInfo) {
    if (node instanceof XmlElementDescription) {
      ...
      while ((jaxbClass != null) && (jaxbClass.getName().startsWith("com.zimbra"))) {
        ApiClassDocumentation doc =
            (jaxbClass != null) ? javadocInfo.get(jaxbClass.getName()) : null;
  ...
  private static String badCmdError(String cmdName, String subtype) {
    String fmt =
        "No JAXB Class associated with %s for command %s.  Is JaxbUtil.MESSAGE_CLASSES up to"
            + " date?\n"
  ...
  private static void populateWithJavadocInfo(
      Root root, Map<String, ApiClassDocumentation> javadocInfo) {
  ...
  /**
   * Traverse description tree, creating pointers from duplicate elements to an original, so that we
   * can reduce the amount of repetition in document
   */
    ...
    for (Class<?> jaxbClass : classes) {
      ...
      if (className.endsWith("Request")) {
        ...
        throw new RuntimeException(
            String.format(
                "Cannot handle top level class %s.  Expecting Request or Response", className));
      }
    }
