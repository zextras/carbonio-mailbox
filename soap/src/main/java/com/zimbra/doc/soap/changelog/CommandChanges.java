...
/**
 * Encodes information about changes to a command between a baseline revision and the current
 * revision
 *
 * @author gren
 */
public class CommandChanges {
  ...
  public boolean hasChanges() {
    return (newAttrs.size() > 0)
        || (deletedAttrs.size() > 0)
        || (modifiedAttrs.size() > 0)
        || (newElems.size() > 0)
        || (deletedElems.size() > 0)
  ...
  public class NamedAttr {
    ...
    public SoapApiAttribute getInfo() {
      return info;
    }
  ...
  public class NamedElem {
    ...
    public SoapApiNamedElement getInfo() {
      return info;
    }
  ...
  public CommandChanges(
      SoapApiCommand baselineCmd,
      SoapApiCommand currentCmd,
      List<SoapApiType> baselineTypesList,
      List<SoapApiType> currentTypesList) {
  ...
  private void findChanges() {
    ...
    Ancestry reqAncestry =
        new Ancestry(baselineCmd.getRequest().getName(), baselineCmd.getRequest().getJaxb());
    Ancestry respAncestry =
        new Ancestry(baselineCmd.getResponse().getName(), baselineCmd.getResponse().getJaxb());
  ...
  private void findAttributeChanges(String xpath, SoapApiType baseType, SoapApiType currType) {
    ...
    for (Entry<String, SoapApiAttribute> entry : currentAttrs.entrySet()) {
      ...
      if (baselineAttr != null) {
        AttributeChanges attrInfo =
            new AttributeChanges(
                xpath, baselineAttr, entry.getValue(), baselineTypes, currentTypes);
        if (!attrInfo.isSame()) {
  ...
  private void findSubElementChanges(
      Ancestry ancestry, SoapApiType baseType, SoapApiType currType) {
  ...
  private void findSubElementChanges(
      Ancestry ancestry,
      Map<String, SoapApiNamedElement> baseElems,
      Map<String, SoapApiNamedElement> currElems) {
  ...
  private void checkElemValue(
      Ancestry ancestry, SoapApiNamedElement baseElem, SoapApiNamedElement currElem) {
    ElementChanges info =
        new ElementChanges(ancestry.getXpath(), baseElem, currElem, baselineTypes, currentTypes);
    if (!info.isSame()) {
      modifiedElements.add(info);
    }
  }

  private void findElementChanges(
      Ancestry ancestry, SoapApiNamedElement baseElem, SoapApiNamedElement currElem) {
    ...
    if (baseElem instanceof SoapApiWrapperElement) {
      ...
      if (currElem instanceof SoapApiWrapperElement) {
        ...
        Map<String, SoapApiNamedElement> currSubElems =
            makeSubElemMap(currWrapper.getSubElements());
  ...
  private void elementChangesWhereBaseIsWrapper(
    ...
    if (currElem instanceof SoapApiWrapperElement) {
      ...
      String currJaxbClass = simpleCurr.getJaxb();
      if (currJaxbClass == null) {
        // New class doesn't have children, so, have deleted elements which used to come under
        // wrapper
  ...
  /**
   * Convenience method. Creating the map is slightly complicated by SoapApiElementChoiceInfo pseudo
   * nodes which we can ignore for the purposes of the changelog, but we need to promote their
   * children
   */
  private Map<String, SoapApiNamedElement> makeSubElemMap(
      Collection<SoapApiElementArtifact> subElems) {
  ...
  public String getName() {
    return baselineCmd.getName();
  }

  public String getNamespace() {
    return baselineCmd.getNamespace();
  }

  public String getDocApiLinkFragment() {
    return baselineCmd.getDocApiLinkFragment();
  }

  public List<NamedAttr> getNewAttrs() {
    return newAttrs;
  }

  public List<NamedAttr> getDeletedAttrs() {
    return deletedAttrs;
  }

  public List<NamedElem> getNewElems() {
    return newElems;
  }

  public List<NamedElem> getDeletedElems() {
    return deletedElems;
  }

  public List<AttributeChanges> getModifiedAttrs() {
    return modifiedAttrs;
  }

  public List<ElementChanges> getModifiedElements() {
