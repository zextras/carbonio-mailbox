...
public class SoapApiChangeLog {
  ...
  public void makeChangeLogDataModel()
    ...
    for (Entry<String, SoapApiCommand> entry : currentCmds.entrySet()) {
      ...
      if (baselineCmd != null) {
        CommandChanges cmd =
            new CommandChanges(
                baselineCmd, entry.getValue(), baselineDesc.getTypes(), currentDesc.getTypes());
        if (cmd.hasChanges()) {
  ...
  public SoapApiDescription getBaselineDesc() {
    return baselineDesc;
  }

  public void setBaselineDesc(SoapApiDescription baselineDesc) {
    this.baselineDesc = baselineDesc;
  }

  public SoapApiDescription getCurrentDesc() {
    return currentDesc;
  }

  public void setCurrentDesc(SoapApiDescription currentDesc) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    opt =
        new Option(
            "b", ARG_APIDESC_BASELINE_JSON, true, "JSON file - description of baseline SOAP API");
    opt.setRequired(true);
    options.addOption(opt);
    opt =
        new Option(
            "c", ARG_APIDESC_CURRENT_JSON, true, "JSON file - description of current SOAP API");
    opt.setRequired(true);
    options.addOption(opt);
    ...
    clog.setBaselineDesc(
        SoapApiDescription.deserializeFromJson(new File(baselineApiDescriptionJson)));
    clog.setCurrentDesc(
        SoapApiDescription.deserializeFromJson(new File(currentApiDescriptionJson)));
    clog.makeChangeLogDataModel();
    clog.writeChangelog();
  }

  public List<SoapApiCommand> getNewCommands() {
    return newCommands;
  }

  public List<SoapApiCommand> getDeletedCommands() {
    return deletedCommands;
  }

  public List<CommandChanges> getModifiedCommands() {
