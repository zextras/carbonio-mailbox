...
public class XmlElementDescription implements DescriptionNode, XmlUnit {
  ...
  private String
      valueFieldDescription; // Content of JavaDoc annotation associated with @XmlValue field
  private ValueDescription valueType = null; // Description for type associated with @XmlValue field
  private String
      elementText; // Text to use between the start and end tags in the XML representation
  private String description;
  private String fieldTag;
  // If there is another description we have already seen for the XML being described for the same
  // class,
  ...
  public static XmlElementDescription createTopLevel(
      JaxbInfo jaxbInfo, String namespace, String name) {
  ...
  private static XmlElementDescription createForElement(
      DescriptionNode parent, JaxbElementInfo nodeInfo) {
    XmlElementDescription desc =
        new XmlElementDescription(!nodeInfo.isMultiElement(), false, false);
    desc.parent = parent;
  ...
  private static XmlElementDescription createForWrappedElement(
      DescriptionNode parent, WrappedElementInfo nodeInfo) {
  ...
  /** Populate {@link #description} with value, attribute and sub-element information */
  private static void addXmlInfo(XmlElementDescription desc, JaxbInfo jaxbInfo) {
    ...
    for (JaxbNodeInfo nodeInfo : jaxbInfo.getJaxbNodeInfos()) {
      if (nodeInfo instanceof WrappedElementInfo) {
        desc.children.add(
            XmlElementDescription.createForWrappedElement(desc, (WrappedElementInfo) nodeInfo));
  ...
  public void writeDescription(StringBuilder desc, int depth) {
    ...
    String startElemString =
        (parent == null)
            ? name
            : String.format("<a href=\"#%s\">%s</a>", tableLinkTargetName(), name);
  ...
  /**
   * @param primaryDescriptions Map of primary Description elements for nodes that have already been
   *     visited.
   */
  ...
  public String getTargetNamespace() {
    return Strings.nullToEmpty(targetNamespace);
  }
  ...
  public StringBuilder appendTableTextForValueFieldDescription(StringBuilder desc) {
    if (!Strings.isNullOrEmpty(this.valueFieldDescription)) {
      desc.append("<b>Description for element text content</b>:")
