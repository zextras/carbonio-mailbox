...
public final class JaxbUtil {
  ...
  static {
    MESSAGE_CLASSES =
        new Class<?>[] {
          ...
          com.zimbra.soap.mail.message.FullAutocompleteRequest.class,
          com.zimbra.soap.mail.message.FullAutocompleteResponse.class,
          ...
          com.zimbra.soap.account.message.SearchEnabledUsersRequest.class,
          com.zimbra.soap.account.message.SearchEnabledUsersResponse.class,
   ...
   */
  public static Element jaxbToElement(
      Object o, Element.ElementFactory factory, boolean removePrefixes) throws ServiceException {
    return jaxbToElement(o, factory, removePrefixes, true /* useContextMarshaller */);
  }

  /**
   * JAXB marshaling creates XML which makes heavy use of namespace prefixes. Historical Zimbra SOAP
   * XML has generally not used them inside the SOAP body. JAXB uses randomly assigned prefixes such
   * as "ns2" which makes the XML ugly and verbose. If {@link removePrefixes} is set then all
   * namespace prefix usage is expunged from the XML.
   *
   * @param o - associated JAXB class. <b>MUST</b> have an @XmlRootElement annotation
   * @param factory - e.g. XmlElement.mFactory or JSONElement.mFactory
   * @param removePrefixes - If true then remove namespace prefixes from unmarshalled XML.
   * @param useContextMarshaller - Set true if Object is a JAXB Request or Response listed in {@code
   *     MESSAGE_CLASSES}
   */
  public static Element jaxbToElement(
      Object o,
      Element.ElementFactory factory,
      boolean removePrefixes,
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "Unable to convert " + o.getClass().getName() + " to Element", e);
    }
  }
  ...
  public static Element jaxbToNamedElement(
      String name, String namespace, Object o, Element.ElementFactory factory)
      throws ServiceException {
    if (Element.JSONElement.mFactory.equals(factory)) {
  ...
  public static Element addChildElementFromJaxb(
      Element parent, String name, String namespace, Object o) {
    Element.ElementFactory factory;
  ...
  /**
   * This appears to be safe but is fairly slow. Note that this method does NOT support Zimbra's
   * greater flexibility for Xml structure. Something similar to {@link fixupStructureForJaxb} will
   * be needed to add such support.
   */
  @Deprecated
    ...
    try {
      ...
      return (T)
          unmarshaller.unmarshal(
              new ByteArrayInputStream(rootElem.asXML().getBytes(Charsets.UTF_8)));
  ...
  /**
   * Method left in place to discourage future attempts to use dom4j as a basis for Element to JAXB
   * - unless the underlying issue is understood and resolved. This sometimes fails badly whether e
   * is a JSONElement or an XMLElement - get: "javax.xml.bind.UnmarshalException: Namespace URIs and
   * local names to the unmarshaller needs to be interned." and that seems to make the unmarshaller
   * unstable from then on :-( Note that this method does NOT support Zimbra's greater flexibility
   * for Xml structure. Something similar to {@link fixupStructureForJaxb} would be needed to add
   * such support.
   */
  @Deprecated
  @SuppressWarnings("unchecked")
  ...
  /**
   * Manipulates a structure under {@link elem} which obeys Zimbra's SOAP XML structure rules to
   * comply with more stringent JAXB rules.
   *
   * <ol>
   *   <li>Zimbra allows attributes to be specified as elements.
   *       <p>One scenario where this happens - {@link XMLElement}'s {@code getAttribute(String key,
   *       String defaultValue)} will look for an attribute with "key" as the name. If it fails to
   *       find that, it looks for an element with "key" as the name and returns the elements text.
   ...
   * </pre>
   *       In JAXB, we typically use {@link XmlElement} for the associated field. Round tripping
   *       from XML will result in an element but round tripping from JSON will result in an
   *       attribute.
   ...
   * </pre>
   *       The element name "a" and the attribute name "n" are defaults - the actual expected values
   *       can be different - so we query the JAXB classes to see what they should be.
   * </ol>
   *
   * @param klass is the JAXB class for {@code elem} which must be under the "com.zimbra" package
   *     hierarchy.
   */
    ...
    for (int i = 0; i < numAttrs; i++) {
      ...
      if (!jaxbInfo.hasAttribute(attrName) && jaxbInfo.hasElement(attrName)) {
        ...
        org.w3c.dom.Element newElem =
            elem.getOwnerDocument().createElementNS(elem.getNamespaceURI(), attrName);
        ...
        if (jaxbInfo.hasWrapperElement(childName)) {
          ...
          for (int j = 0; j < wrappedList.getLength(); j++) {
            ...
            if (wSubnode.getNodeType() == Node.ELEMENT_NODE) {
              ...
              fixupStructureForJaxb(
                  wChild, jaxbInfo.getClassForWrappedElement(childName, wChild.getLocalName()));
            }
          }
          ...
          // Don't remove pre-existing child until later pass to avoid changing the list of child
          // elements
          if (orphans == null) {
          ...
          // This might be a keyvaluepair, the Element code doesn't have access to JAXB info, so
          // defaults
          // the element name to "a" and its attribute will be "n".  If this is what has happened,
          // replace
          // it with a corrected equivalent using the JAXB object for reference.
          JaxbInfo.KeyValuePairXmlRepresentationInfo kvpXmlRep =
              jaxbInfo.getKeyValuePairElementInfo();
          if (kvpXmlRep != null) {
            elem.getNamespaceURI();
            org.w3c.dom.Element newElem =
          ...
          LOG.debug(
              "JAXB class " + klass.getName() + " does NOT recognise element named:" + childName);
        }
      }
    }
  ...
  /**
   * @param elem represents a structure which may only match Zimbra's more relaxed rules rather than
   *     stringent JAXB rules. e.g. Zimbra allows attributes to be specified as elements.
   * @param klass is the JAXB class for {@link elem}
   * @return a JAXB object
   */
  @SuppressWarnings("unchecked")
  private static <T> T w3cDomDocToJaxb(
  ...
  /**
   * Return a JAXB object. This implementation uses a org.w3c.dom.Document as an intermediate
   * representation. This appears to be more reliable than using a DocumentSource based on
   * org.dom4j.Element
   *
   * @param klass is the JAXB class for {@link doc}
   * @param jaxbClassInContext is true if {@link klass} is the JAXB class for a request or response
   *     object that is in {@link JaxbUtil.MESSAGE_CLASSES}
   */
  @SuppressWarnings("unchecked")
  private static <T> T rawW3cDomDocToJaxb(
      org.w3c.dom.Document doc, Class<?> klass, boolean jaxbClassInContext)
      throws ServiceException {
    ...
    try {
      ...
      throw ServiceException.FAILURE(
          "Unable to unmarshal response for " + doc.getDocumentElement().getNodeName(), ex);
    }
  }
  ...
  /**
   * Return a JAXB object corresponding to {@link e} which is the Xml for a Request or Response.
   *
   * @param e MUST be a top level Request or Response element whose corresponding JAXB object is in
   *     {@link JaxbUtil.MESSAGE_CLASSES}
   */
  @SuppressWarnings("unchecked")
  ...
  public static CreateItemNotification getCreatedItemSOAP(BaseItemInfo mod)
      throws ServiceException {
    ...
    ImapMessageInfo messageInfo =
        new ImapMessageInfo(
            mod.getIdInMailbox(),
            mod.getImapUid(),
            mod.getMailItemType().toString(),
            mod.getFlagBitmask(),
            tags);
    return new CreateItemNotification(messageInfo);
  }

  public static ModifyItemNotification getModifiedItemSOAP(BaseItemInfo mod, int reason)
      throws ServiceException {
    String tags = mod.getTags() == null ? null : Joiner.on(",").join(mod.getTags());
    ImapMessageInfo messageInfo =
        new ImapMessageInfo(
  ...
  public static DeleteItemNotification getDeletedItemSOAP(int itemId, String type)
      throws ServiceException {
    return new DeleteItemNotification(itemId, type);
  }

  public static PendingFolderModifications getFolderMods(
      Integer folderId, HashMap<Integer, PendingFolderModifications> folderMap) {
