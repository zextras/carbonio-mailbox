...
/**
 * Used by {@link ZimbraBeanSerializerModifier} to handle some Zimbra JSON specific ways of
 * processing JAXB object fields.
 *
 * <p>{@link XmlElementWrapper} handling: Jackson with the JaxbAnnotationIntrospector does
 * understand @XmlElementWrapper BUT treats it as a property name, discarding the wrapped element
 * name. For Zimbra JSON we retain the wrapper and use the wrapped element name as the property name
 * for the wrapped property.
 *
 * <p>Element values wrapped in arrays Jackson with the JaxbAnnotationIntrospector knows which
 * fields are arrays and which are not and only uses arrays where necessary. Zimbra JSON was
 * originally designed as a serialization of a pure Element structure - and it typically isn't
 * possible to determine the difference between a list of elements with just one element and a
 * singleton. Hence, Zimbra JSON often treats fields as arrays even when they are not.
 *
 * <p>{@link XmlElements} handling:
 *
 * <pre>
 *     {@code @XmlElements({
 ...
 * </pre>
 *
 * Jackson with the JaxbAnnotationIntrospector wraps this in an array field called "alts". Zimbra
 * JSON does not.
 *
 * <p>Was some (non-functional) code here to handle Zimbra "Key Value pairs". Ended up deciding that
 * dedicated serializers were more appropriate.
 */
public class ZimbraBeanPropertyWriter extends BeanPropertyWriter {
  ...
  public ZimbraBeanPropertyWriter(
      BeanPropertyWriter wrapped, NameInfo nameInfo, JsonSerializer<Object> serializer) {
    super(wrapped);
  ...
  public void serializeAsField(Object bean, JsonGenerator jgen, SerializerProvider prov)
    ...
    if (wrappedName != null) {
      ...
      } else if (nameInfo.isAnyAttributeAllowed()
  ...
  private void startWrapping(JsonGenerator jgen) throws JsonGenerationException, IOException {
    if (wrapperName != null) {
  ...
  private void finishWrapping(JsonGenerator jgen) throws JsonGenerationException, IOException {
    if (wrapperName != null) {
      jgen.writeEndObject();
  ...
  /**
   * Serialize a field for which we have a specific wrappedName - typically associated with {@link
   * XmlElement} or {@link XmlElementRef}. Zimbra normally wraps values inside an array even if they
   * are not in an array. Assumes that any wrapping info has already been handled.
   */
  public void serializeInnerField(
      Object value, JsonGenerator jgen, SerializerProvider prov, JsonSerializer<Object> ser)
  ...
  /**
   * Zimbra JSON represents the text content of an element thus: [{ "_content": "text content" }]
   *
   * @throws IOException
   * @throws JsonGenerationException
   */
  private void serializeElementTextValue(
      Object value, JsonGenerator jgen, SerializerProvider prov, JsonSerializer<Object> ser)
      throws JsonGenerationException, IOException {
    if (!nameInfo.isTreatAsUniqueElement()) {
      jgen.writeStartArray();
   ...
   */
  private void serializeXmlElementArray(
      List<?> values, JsonGenerator jgen, SerializerProvider prov, JsonSerializer<Object> ser)
      throws JsonGenerationException, IOException {
    if (values.isEmpty()) {
  ...
  /**
   * Serialize Array associated with XmlElements or XmlElementRefs annotations Different names for
   * different objects. For default Jackson Xml serialization, the name associated with the array is
   * used as a wrapper but for Zimbra serialization, we don't use this wrapper. There MAY be another
   * wrapper but that will have been handled by "wrapperName" already
   */
  private void serializeXmlElementsArray(
      ArrayList<?> al, JsonGenerator jgen, SerializerProvider prov, Map<Class<?>, QName> nameMap)
      throws JsonGenerationException, IOException {
    for (Object obj : al) {
        ...
        if (org.w3c.dom.Element.class.isAssignableFrom(objClass)) {
          ...
          if (!nameInfo.isMixedAllowed()) {
            LOG.debug(
                "Unexpected '"
                    + objClass.getName()
                    + "' object in XmlElements(Refs) array - ignored");
          } else {
            jgen.writeFieldName(JSONElement.A_CONTENT /* "_content" */);
  ...
  private void serializeZimbraKeyValuePairs(
      List<?> pairs, JsonGenerator jgen, SerializerProvider prov)
      throws JsonGenerationException, IOException {
    if (pairs == null) {
      return;
    }
    ...
    for (Object obj : pairs) {
      ...
      if (KeyAndValue.class.isAssignableFrom(objClass)) {
        ...
        LOG.debug(
            "Unexpected '"
                + objClass.getName()
                + "' object in @ZimbraKeyValuePairs array - ignored");
      }
    }
    jgen.writeEndObject();
  }

  private void serializeXmlAnyAttributes(
      Map<?, ?> extraAttribs, JsonGenerator jgen, SerializerProvider prov)
      throws JsonGenerationException, IOException {
  ...
  private void serializeXmlAnyElementsArray(
      ArrayList<?> al, JsonGenerator jgen, SerializerProvider prov)
      throws JsonGenerationException, IOException {
    for (Object obj : al) {
      Class<?> objClass = obj.getClass();
      if (org.w3c.dom.Element.class.isAssignableFrom(objClass)) {
        ...
        LOG.debug(
            "Unexpected object of class '"
                + objClass.getName()
                + "' in XmlAnyElements array - ignored");
      }
    }
  }
