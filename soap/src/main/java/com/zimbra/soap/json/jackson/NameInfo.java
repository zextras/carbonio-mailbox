...
/**
 * Used to store field name information to be used in Zimbra-style JSON related to a JAXB object
 * field. A single field in a JAXB object may have a single field name e.g. from @XmlElement or
 * several possible names e.g. from @XmlElements In addition, an @XmlElementWrapper annotation
 * requires the resulting Zimbra-style JSON to add a wrapping field
 */
public class NameInfo {
  ...
  private void setWrappedInfo(
      AnnotationIntrospector ai, AnnotatedMember prop, String defaultWrappedName) {
    ...
    ZimbraJsonArrayForWrapper arrayForWrapperAnnot =
        prop.getAnnotation(ZimbraJsonArrayForWrapper.class);
  ...
  public boolean isAnyElementAllowed() {
    return anyElementAllowed;
  }

  public boolean isAnyAttributeAllowed() {
    return anyAttributeAllowed;
  }
  ...
  public boolean needSpecialHandling() {
    return haveSpecialWrappingInfo()
        || isMixedAllowed()
        || isAnyElementAllowed()
