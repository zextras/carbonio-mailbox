...
public final class JacksonUtil {
  ...
  /**
   * e.g. calData = new AppointmentData(uid, uid); ... JacksonUtil.jaxbToJSONElement(calData, new
   * QName(MailConstants.E_APPOINTMENT, MailConstants.NAMESPACE)); is roughly equivalent to Element
   * based code built on top of something like:
   * JSONElement.mFactory.createElement(MailConstants.E_APPOINTMENT); ...
   */
  public static Element jaxbToJSONElement(Object obj, org.dom4j.QName qn) throws ServiceException {
    try {
  ...
  /**
   * Note that currently does NOT use Jackson to go from JSON to JAXB to Element. {@code
   * SoapEngine.dispatch} uses Element.parseJSON(in) to parse a SOAP API command.
   *
  ...
  /**
   * Makes a best efforts guess to determine the name/namespace associated with the object and
   * produces a JSONElement tree from the supplied JSON string
   *
   * @param json - a String in JSON format
   * @param obj - a JAXB object associated with the {@code json} argument
   * @return a JSONElement corresponding to the {@code json} argument
   */
  public static Element jacksonJsonToElement(String json, Object obj) throws ServiceException {
    if (obj == null) return null;
    try {
  ...
  public static String jaxbToJsonString(ObjectMapper mapper, Object obj) throws ServiceException {
    try {
   ...
   * typically only classes with an {@link XmlRootElement} annotation have an authoritative
   * associated {@code QName}
   *
  ...
  private static org.dom4j.QName getElementName(Object obj) {
    ...
    if (root != null) {
      ...
      if (!root.namespace().equals(JaxbInfo.DEFAULT_MARKER)) ns = root.namespace();
    }
    if (Strings.isNullOrEmpty(ns)) {
      // Didn't get a valid namespace from XmlRootElement - see if there is one associated with the
      // package
  ...
  /**
   * We use our own annotation introspector which is based on a pair of annotation introspectors so
   * that we can handle both JAXB annotations and some Jackson annotations
   */
