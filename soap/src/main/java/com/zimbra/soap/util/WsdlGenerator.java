...
/**
 * This class represents a utility to generate the top level WSDL files for Zimbra SOAP interfaces
 *
 * @author gren
 */
public class WsdlGenerator {
  ...
  public static Document makeWsdlDoc(
      List<WsdlInfoForNamespace> nsInfos, String serviceName, String targetNamespace) {
    ...
    for (WsdlInfoForNamespace wsdlNsInfo : nsInfos) {
      ...
      for (String requestName : wsdlNsInfo.getRequests()) {
        ...
        addWsdlRequestAndResponseMessageElements(
            root, wsdlNsInfo, reqMsgName, respMsgName, requestName, responseName);

        addWsdlPortTypeOperationElements(
            portTypeElems.get(svcInfo), reqMsgName, respMsgName, reqOpName);
  ...
  private static void addWsdlTypesElement(
      Element root, QName svcTypes, List<WsdlInfoForNamespace> nsInfos) {
  ...
  private static void addWsdlRequestAndResponseMessageElements(
      Element root,
      WsdlInfoForNamespace nsInfo,
      String reqMsgName,
      String respMsgName,
      String requestName,
      String responseName) {
     ...
     * http://msdn.microsoft.com/en-us/magazine/cc188906.aspx Side Effects of "Parameters" Both
     * wsdl.exe and the WebMethod infrastructure exercise special behavior when a special part name,
     * "parameters," is used in a WSDL definition. The exact behavior is undocumented, but it's
     * critical to understand the convention if you happen to use it.
     */
  ...
  private static void addWsdlPortTypeOperationElements(
      Element portTypeElem, String reqMsgName, String respMsgName, String reqOpName) {
  ...
  private static void addWsdlBindingOperationElements(
      Element bindingElem, WsdlInfoForNamespace nsInfo, String reqOpName, String rootName) {
  ...
  public enum WsdlDefinition {
    ...
    ADMIN(
        "ZimbraAdminService.wsdl", "ZimbraAdminService", targetNsBase + "ZimbraAdminService.wsdl"),
  ...
  public static void createZimbraServiceWsdl(OutputStream out, String soapUrl, String adminSoapUrl)
    ...
    writeWsdl(
        out,
        WsdlDefinition.ALL.getTargetNamespace(),
        WsdlDefinition.ALL.getServiceName(),
        nsInfoList);
  }
  ...
  private static void addAdminNamespaceInfo(
