...
public class JaxbElementInfo implements JaxbNodeInfo {
  ...
  public JaxbElementInfo(XmlElement annot, String fieldName, Type defaultGenericType) {
    ...
    if (name == null) {
      throw new RuntimeException(
          String.format(
              "Ignoring element with annotation %s unable to determine name", annot.toString()));
    }
  ...
  /**
   * Note that unless the collection item type (for collection property) or property type (for
   * single valued property) is JAXBElement any name and namespace specified in {@link
   * XmlElementRef} are ignored - the actual values are taken from the {@link XmlRootElement}
   * annotation on the collection item type or property type.
   */
  ...
  public boolean isMultiElement() {
    return canHaveMultipleElements;
  }
