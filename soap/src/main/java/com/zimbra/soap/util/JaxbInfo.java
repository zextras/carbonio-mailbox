...
/**
 * Zimbra SOAP interfaces are more flexible than Jaxb in what is acceptable. In particular,
 * attributes can be represented as elements. This class provides a limited amount of Jaxb
 * information for a class to aid transforming Zimbra acceptable Xml into Jaxb acceptable Xml.
 *
 * @author gren
 *     <p>Treatment of annotations: XmlAccessType : Supported XmlAccessorType : Supported
 *     XmlAnyAttribute : Ignored XmlAnyElement : Ignored XmlAttribute : Supported XmlElement :
 *     Supported XmlElementRef : Supported XmlElementRefs : Supported XmlElementWrapper : Supported
 *     XmlElements : Supported XmlEnum : Ignored XmlEnumValue : Ignored XmlMixed : Ignored
 *     XmlRootElement : Supported XmlSeeAlso : Not supported - useful for finding subclasses
 *     XmlTransient : Supported XmlType : Useful for propOrder XmlValue : Ignored
 */
public final class JaxbInfo {
  ...
  public Class<?> getClassForWrappedElement(String wrapperName, String elementName) {
    ...
    if (info != null) {
      ...
      if (wKlass == null) {
        LOG.debug(
            "%s No class for wrapper element=%s sub-element=%s", stamp, wrapperName, elementName);
      }
      return wKlass;
    }
    LOG.debug(
        "%s Unknown wrapper element=%s (looking for sub-element=%s)",
        stamp, wrapperName, elementName);
    return null;
  }
  ...
  public List<String> getPropOrder() {
    ...
    if ((null == xmlType) || (null == xmlType.propOrder())) return propOrder;
  ...
  private org.dom4j.QName getQName(JaxbNodeInfo elemNode) {
    ...
    if (DEFAULT_MARKER.equals(elemNs)) {
      ...
      elemNs =
          XmlNsForm.QUALIFIED.equals(xmlSchemaAnnot.elementFormDefault())
              ? xmlSchemaAnnot.namespace()
              : "";
    }
  ...
  /**
   * If this object has keyvaluepairs for children, this returns information about them, otherwise
   * returns null. Note implicit assumption that there can only be one set of keyvaluepairs - this
   * is because the JSON representation would be unable to differentiate between them.
   */
