...
public class SearchDirectoryResponse {
  ...
  @XmlElements({
    @XmlElement(
        name = AccountConstants.E_CALENDAR_RESOURCE /* calresource */,
  ...
  public List<CalendarResourceInfo> getCalendarResources() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof CalendarResourceInfo) subset.add((CalendarResourceInfo) entry);
    }
  ...
  public List<DistributionListInfo> getDistributionLists() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof DistributionListInfo) subset.add((DistributionListInfo) entry);
    }
    return Collections.unmodifiableList(subset);
  }
  ...
  public List<AliasInfo> getAliases() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof AliasInfo) subset.add((AliasInfo) entry);
    }
    return Collections.unmodifiableList(subset);
  }
  ...
  public List<AccountInfo> getAccounts() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof AccountInfo) subset.add((AccountInfo) entry);
    }
    return Collections.unmodifiableList(subset);
  }
  ...
  public List<DomainInfo> getDomains() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof DomainInfo) subset.add((DomainInfo) entry);
    }
    return Collections.unmodifiableList(subset);
  }
  ...
  public List<CosInfo> getCOSes() {
    ...
    for (AdminObjectInterface entry : entries) {
      if (entry instanceof CosInfo) subset.add((CosInfo) entry);
    }
    return Collections.unmodifiableList(subset);
  }
}
