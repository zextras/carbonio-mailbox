...
public class GetAllRightsRequest {
   ...
   * @zm-api-field-description Target type on which a right is grantable <br>
   *     e.g. createAccount right is only grantable on domain entries and the globalgrant entry.
   *     <br>
   *     Don't confuse this with "whether a right is executable on a target type". <br>
   *     e.g. the renameAccount right is "executable" on account entries, but it is "grantable" on
   *     account, distribuiton list, domain, and globalgrant entries.
   */
   ...
   * @zm-api-field-description Flags whether to include all attribute names in the <b>&lt;attrs></b>
   *     elements in GetRightResponse if the right is meant for all attributes
   */
  ...
  public GetAllRightsRequest(String targetType, Boolean expandAllAttrs, String rightClass) {
  ...
  public Boolean isExpandAllAttrs() {
    return ZmBoolean.toBool(expandAllAttrs);
  }
