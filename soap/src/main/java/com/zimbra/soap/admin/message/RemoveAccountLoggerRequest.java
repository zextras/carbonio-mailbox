 ...
 * @zm-api-command-description Removes one or more custom loggers. If both the account and logger
 *     are specified, removes the given account logger if it exists. If only the account is
 *     specified or the category is "all", removes all custom loggers from that account. If only the
 *     logger is specified, removes that custom logger from all accounts. If neither element is
 *     specified, removes all custom loggers from all accounts on the server that receives the
 *     request.
 */
...
public class RemoveAccountLoggerRequest {
  ...
  public static RemoveAccountLoggerRequest createForAccountAndLogger(
      AccountSelector account, LoggerInfo logger) {
  ...
  public void setLogger(LoggerInfo logger) {
    this.logger = logger;
  }
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    return helper.add("id", id).add("account", account).add("logger", logger);
  }
