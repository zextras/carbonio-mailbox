...
public class ZimletStatus {
  ...
  public enum ZimletStatusSetting {
    ...
    public static ZimletStatusSetting fromString(String s) throws ServiceException {
      try {
        return ZimletStatusSetting.valueOf(s);
      } catch (IllegalArgumentException e) {
        throw ServiceException.INVALID_REQUEST(
            "invalid status setting: " + s + ", valid values: " + Arrays.asList(values()), null);
      }
    }
  }
  ...
  public ZimletStatus(
      String name, ZimletStatusSetting status, boolean extension, Integer priority) {
    this.name = name;
    this.status = status;
  ...
  public boolean getExtension() {
    return ZmBoolean.toBool(extension);
  }
