 ...
 * @zm-api-command-description <b>SearchMultiMailboxRequest</b> is a version of the standard
 *     <b>&lt;SearchRequest></b> API that allows the caller to specify one or more <b>&lt;mbx></b>
 *     targets to be searched in. Most parameters are identical to a normal search. <br>
 ...
 *     multiple mailboxes are specified. In general this API is intended to be used for text-based
 *     searches, or searches with system flags such as "unread".
 */
...
public class SearchMultiMailboxRequest implements SearchParameters {
   ...
   *     |attachAsc|attachDesc|flagAsc|flagDesc|priorityAsc|priorityDesc|idAsc|idDesc|readAsc|readDesc</b>
   *     If <b>{sort-by}</b> is "none" then cursors MUST NOT be used, and some searches are
   *     impossible (searches that require intersection of complex sub-ops). Server will throw an
   *     IllegalArgumentException if the search is invalid. <br>
