...
public class BackupSpec {
   ...
   * @zm-api-field-description Used for selecting backups to delete for method="delete". <br>
   *     Value is either a full backup label or a date or a time period. <br>
   *     If it is a date, the format is <b>YYYY/MM/DD[-hh:mm:ss]</b> If it is a time period, the
   *     format is <b>nn{d|m|y}</b> where d = day, m = month, y = year <br>
   *     e.g., before="7d" means delete backups older than 7 days. before="1y" means delete backups
   *     older than a year.
   */
   ...
   * @zm-api-field-description Option to include/exclude search index in a full backup (not
   *     applicable in incremental backup). Values: <b>include|exclude|config</b> <br>
   ...
   *     incremental backup). Values: <b>include|exclude|config</b> <br>
   *     Default value is "config", to use the configured value. "include" or "exclude" overrides
   *     the configuration.
   */
   ...
   * @zm-api-field-description Option to include/exclude secondary blobs in a full backup (not
   *     applicable in incremental backup). Values: <b>include|exclude|config</b> <br>
   *     Default value is "config", to use the configured value. "include" or "exclude" overrides
   ...
   * @zm-api-field-description Account selector - either one <b>&lt;account name="all"/></b> or a
   *     list of <b>&lt;account name="{account email addr}"/></b>
   */
  ...
  public void setZip(Boolean zip) {
    this.zip = ZmBoolean.fromBool(zip);
  }

  public void setZipStore(Boolean zipStore) {
    this.zipStore = ZmBoolean.fromBool(zipStore);
  }
  ...
  public Boolean getZip() {
    return ZmBoolean.toBool(zip);
  }

  public Boolean getZipStore() {
    return ZmBoolean.toBool(zipStore);
  }
