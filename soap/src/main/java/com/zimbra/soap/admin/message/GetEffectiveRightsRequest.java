 ...
 * @zm-api-command-description Returns <b>effective ADMIN</b> rights the authenticated admin has on
 *     the specified target entry. <br>
 *     <br>
 *     Effective rights are the rights the admin is actually allowed. It is the net result of
 *     applying ACL checking rules given the target and grantee. Specifically denied rights will
 *     <b>not</b> be returned. <br>
 *     <br>
 *     The result can help the admin console decide on what tabs to display after a target is
 *     selected. For example, after user1 is selected, if the admin does not have right to
 *     setPassword, it should probably hide or gray out the setPassword tab. <br>
 *     e.g.
 *     <pre>
...
public class GetEffectiveRightsRequest {
  ...
  @XmlTransient private Boolean expandSetAttrs;
  @XmlTransient private Boolean expandGetAttrs;
  ...
  /**
   * @zm-api-field-description Grantee. If <b>&lt;grantee></b> is omitted, the account identified by
   *     the auth token is regarded as the grantee.
   */
  ...
  private GetEffectiveRightsRequest() {
    this(
        (EffectiveRightsTargetSelector) null,
        (GranteeSelector) null,
        (Boolean) null,
        (Boolean) null);
  }
   ...
   * @zm-api-field-description Whether to include all attribute names in the
   *     <b>&lt;getAttrs>/&lt;setAttrs></b> elements in the response if all attributes of the target
   *     are gettable/settable Valid values are:
   *     <table>
  ...
  public String getExpandAllAttrs() {
    ...
    if ((expandSetAttrs != null) && expandSetAttrs) settings.add(EXPAND_SET_ATTRS);
    if ((expandGetAttrs != null) && expandGetAttrs) settings.add(EXPAND_GET_ATTRS);
