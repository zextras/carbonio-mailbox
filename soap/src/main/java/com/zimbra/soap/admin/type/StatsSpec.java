...
public class StatsSpec {
   ...
   * @zm-api-field-description if limit="true" is specified, attempt to reduce result set to under
   *     500 records
   */
   ...
   */
  @XmlElementRefs({@XmlElementRef(/* values */ type = StatsValueWrapper.class)})
  @XmlMixed
  ...
  public StatsSpec setStatValues(StatsValueWrapper statValues) {
    if (getContent() == null) setContent(Lists.newArrayList());
  ...
  public StatsValueWrapper getStatValues() {
    for (Object obj : getContent()) {
      if (obj instanceof StatsValueWrapper) return (StatsValueWrapper) obj;
    }
    return null;
  }
