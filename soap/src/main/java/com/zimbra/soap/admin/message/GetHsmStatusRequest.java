 ...
 * @zm-api-command-description Queries the status of the most recent HSM session. Status information
 *     for a given HSM session is available until the next time HSM runs or until the server is
 *     restarted. <br>
 *     Notes:
 *     <ul>
 ...
 *       <li>If &lt;GetHsmStatusRequest> is sent after a server restart but before an HSM session,
 *           the response will contain running="0" and no additional information.
 *       <li>Once HSM completes, the same &lt;GetHsmStatusResponse> will be returned until another
 *           HSM session or a server restart.
 *     </ul>
 */
