 ...
 * @zm-api-command-description Create a waitset to listen for changes on one or more accounts <br>
 *     Called once to initialize a WaitSet and to set its "default interest types"
 *     <p><b>WaitSet</b>: scalable mechanism for listening for changes to one or more accounts
 */
...
public class AdminCreateWaitSetRequest implements CreateWaitSetReq {
   ...
   * @zm-api-field-description If <b>{all-accounts}</b> is set, then all mailboxes on the system
   *     will be listened to, including any mailboxes which are created on the system while the
   *     WaitSet is in existence. Additionally:
   *     <ul>
   *       <li>&lt;add>, &lt;remove> and &lt;update> tags are IGNORED
   *       <li>The requesting authtoken must be an admin token
   *     </ul>
   *     <p>AllAccounts WaitSets are *semi-persistent*, that is, even if the server restarts, it is
   *     OK to call &lt;WaitSetRequest> passing in your previous sequence number. The server will
   *     attempt to resynchronize the waitset using the sequence number you provide (the server's
   *     ability to do this is limited by the RedoLogs that are available) Note: AllAccounts
   *     WaitSets are deprecated
   */
  ...
  public Boolean getAllAccounts() {
    return ZmBoolean.toBool(allAccounts, false);
  }
