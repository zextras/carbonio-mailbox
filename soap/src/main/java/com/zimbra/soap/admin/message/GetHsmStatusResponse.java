...
public class GetHsmStatusResponse {
  ...
  /**
   * @zm-api-field-description <b>1 (true)</b> if an HSM session is currently running, <b>0
   *     (false)</b> if the information returned applies to the last completed HSM session
   */
  ...
  public void setWasAborted(Boolean wasAborted) {
    this.wasAborted = ZmBoolean.fromBool(wasAborted);
  }

  public void setAborting(Boolean aborting) {
    this.aborting = ZmBoolean.fromBool(aborting);
  }
  ...
  public Boolean getWasAborted() {
    return ZmBoolean.toBool(wasAborted);
  }

  public Boolean getAborting() {
    return ZmBoolean.toBool(aborting);
  }
