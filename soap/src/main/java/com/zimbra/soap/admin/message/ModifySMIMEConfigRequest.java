 ...
 * @zm-api-command-description Modify a configuration for SMIME public key lookup via external LDAP
 *     on a domain or globalconfig. <br>
 *     Notes: if <b>&lt;domain></b> is present, modify the config on the domain, otherwise modify
 *     the config on globalconfig. <br>
 *     <br>
 *     Attributes:
 *     <table>
...
public class ModifySMIMEConfigRequest {
  ...
  public ModifySMIMEConfigRequest(SMIMEConfigModifications config, DomainSelector domain) {
    this.config = config;
    this.domain = domain;
  }
