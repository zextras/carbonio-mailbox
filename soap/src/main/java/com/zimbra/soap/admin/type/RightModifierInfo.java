...
public class RightModifierInfo {
   ...
   */
  @XmlAttribute(
      name = AdminConstants.A_DISINHERIT_SUB_GROUPS /* disinheritSubGroups */,
      required = false)
   ...
   * @zm-api-field-description Value is of the form : {right-name} | {inline-right} where <br>
   *     {right-name} = a system defined right name <br>
   *     {inline-right} = {op}.{target-type}.{attr-name} <br>
   *     {op} = set | get <br>
  ...
  public Boolean getCanDelegate() {
    return ZmBoolean.toBool(canDelegate);
  }

  public Boolean getDisinheritSubGroups() {
    return ZmBoolean.toBool(disinheritSubGroups);
  }

  public Boolean getSubDomain() {
    return ZmBoolean.toBool(subDomain);
  }
