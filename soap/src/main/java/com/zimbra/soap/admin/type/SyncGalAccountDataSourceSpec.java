...
public class SyncGalAccountDataSourceSpec {
   ...
   * @zm-api-field-description If fullSync is set to <b>0 (false)</b> or unset the default behavior
   *     is trickle sync which will pull in any new contacts or modified contacts since last sync.
   *     <br>
   *     If fullSync is set to <b>1 (true)</b>, then the server will go through all the contacts
   ...
   * @zm-api-field-description Reset flag. If set, then all the contacts will be populated again,
   *     regardless of the status since last sync. Reset needs to be done when there is a
   *     significant change in the configuration, such as filter, attribute map, or search base.
   */
  ...
  public void setFullSync(Boolean fullSync) {
    this.fullSync = ZmBoolean.fromBool(fullSync);
  }

  public void setReset(Boolean reset) {
    this.reset = ZmBoolean.fromBool(reset);
  }
  ...
  public Boolean getFullSync() {
    return ZmBoolean.toBool(fullSync);
  }

  public Boolean getReset() {
    return ZmBoolean.toBool(reset);
  }
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    return helper.add("by", by).add("fullSync", fullSync).add("reset", reset).add("value", value);
  }
