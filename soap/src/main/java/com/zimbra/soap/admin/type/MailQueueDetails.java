...
// soap-admin.txt implies there is a "stale" attribute (See GetMailQueueResponse/server/queue) but
// SOAP
...
public class MailQueueDetails {
   ...
   * @zm-api-field-description Indicates that the server has not completed scanning the MTA queue,
   ...
   * @zm-api-field-description Indicates that more qi's are available past the limit specified in
   *     the request.
   */
  ...
  /**
   * @zm-api-field-description Queue summary. The <b>&lt;qs></b> elements summarize the queue by
   *     various types of data (sender addresses, recipient domain, etc). Only the deferred queue
   *     has error summary type.
   */
  ...
  public MailQueueDetails(String name, long time, boolean stillScanning, int total, boolean more) {
    this.name = name;
    this.time = time;
  ...
  public boolean getStillScanning() {
    return ZmBoolean.toBool(stillScanning);
  }
