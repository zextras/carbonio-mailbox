 ...
 * @zm-api-command-description Exports the database data for the given items with SELECT INTO
 *     OUTFILE and deletes the items from the mailbox. Exported filenames follow the pattern
 *     {prefix}{table_name}.txt. The files are written to <b>sqlExportDir</b>. When sqlExportDir is
 *     not specified, data is not exported. Export is only supported for MySQL.
 */
...
public class ExportAndDeleteItemsRequest {
   ...
   */
  @XmlAttribute(
      name = AdminConstants.A_EXPORT_FILENAME_PREFIX /* exportFilenamePrefix */,
      required = false)
  ...
  public ExportAndDeleteItemsRequest(
      String exportDir, String exportFilenamePrefix, ExportAndDeleteMailboxSpec mailbox) {
