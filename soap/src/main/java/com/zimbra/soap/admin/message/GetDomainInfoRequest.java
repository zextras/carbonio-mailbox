 ...
 * @zm-api-command-admin-auth-required false - Only returns attributes that are pertinent to domain
 *     settings for cases when the user is not authenticated
 * @zm-api-command-description Get Domain information <br>
 *     This call does <b>not</b> require an auth token. It returns attributes that are pertinent to
 *     domain settings for cases when the user is not authenticated. For example, URL to direct the
 *     user to upon logging out or when auth token is expired. <br>
 *     <br>
...
public class GetDomainInfoRequest {
   ...
   * @zm-api-field-description If <b>{apply-config}</b> is <b>1 (true)</b>, then certain unset attrs
   *     on a domain will get their values from the global config. <br>
   *     if <b>{apply-config}</b> is <b>0 (false)</b>, then only attributes directly set on the
