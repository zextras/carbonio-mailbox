...
@XmlEnum
@GraphQLType(
    name = GqlConstants.CLASS_MESSAGE_CONTENT,
    description = "Message content the cient expects in response")
public enum MsgContent {
  ...
  public static MsgContent fromString(String msgContent) {
    try {
      if (msgContent != null) return MsgContent.valueOf(msgContent);
      else return null;
