 ...
 * @zm-api-command-description Directly set status of an entire appointment. This API is intended
 *     for mailbox Migration (ie migrating a mailbox onto this server) and is not used by normal
 *     mail clients. <br>
 *     <br>
 *     Need to specify folder for appointment <br>
 *     <br>
 *     Need way to add message WITHOUT processing it for calendar parts. Need to generate and
 *     patch-in the iCalendar for the <b>&lt;inv></b> but w/o actually processing the
...
public class SetAppointmentRequest {
   ...
   * @zm-api-field-description Set if all alarms have been dismissed; if this is set, nextAlarm
   *     should not be set
   */
   ...
   * @zm-api-field-description If specified, time when next alarm should go off. <br>
   *     If missing, two possibilities:
   *     <ol>
   *       <li>if noNextAlarm isn't set, keep current next alarm time (this is a backward
   *           compatibility case)
  ...
  /**
   * @zm-api-field-description List of replies received from attendees. If SetAppointmenRequest
   *     doesn't contain a <b>&lt;replies></b> block, existing replies will be kept. If
   *     <b>&lt;replies/></b> element is provided with no <b>&lt;reply></b> elements inside,
   *     existing replies will be removed, replaced with an empty set. If <b>&lt;replies></b>
   *     contains one or more <b>&lt;reply></b> elements, existing replies are replaced with the
   *     ones provided.
   */
  ...
  public Boolean getNoNextAlarm() {
    return ZmBoolean.toBool(noNextAlarm);
  }
