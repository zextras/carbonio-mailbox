...
public class Acl {
   ...
   * @zm-api-field-description Time when grants to internal grantees expire. If not specified in the
   *     request, defaults to the maximum allowed expiry for internal grants. If not specified in
   *     the response, defaults to 0. Value of 0 indicates that these grants never expire.
   */
  @XmlAttribute(
      name = MailConstants.A_INTERNAL_GRANT_EXPIRY /* internalGrantExpiry */,
      required = false)
  @GraphQLQuery(
      name = "internalGrantExpiry",
      description = "Time when grants to internal grantees expire.")
  private Long internalGrantExpiry;

  /**
   * @zm-api-field-tag millis-since-epoch
   * @zm-api-field-description Time when grants to guest grantees expire. If not specified in the
   *     request, defaults to the maximum allowed expiry for guest/external user grants. If not
   *     specified in the response, defaults to 0. Value of 0 indicates that these grants never
   *     expire.
   */
  @XmlAttribute(name = MailConstants.A_GUEST_GRANT_EXPIRY /* guestGrantExpiry */, required = false)
  @GraphQLQuery(
      name = "guestGrantExpiry",
      description = "Time when grants to guest grantees expire.")
