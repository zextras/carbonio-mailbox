 ...
 * @zm-api-command-description Get Folder <br>
 *     A {base-folder-id}, a {base-folder-uuid} or a {fully-qualified-path} can optionally be
 *     specified in the folder element; if none is present, the descent of the folder hierarchy
 *     begins at the mailbox's root folder (id 1). <br>
 *     If {fully-qualified-path} is present and {base-folder-id} or {base-folder-uuid} is also
 *     present, the path is treated as relative to the folder that was specified by id/uuid.
...
public class GetFolderRequest {
   ...
   *     <b>&lt;GetFolderResponse></b> <br>
   *     When you don't:
   *     <ul>
   *       <li>folders you can see appear normally,
   *       <li>folders you can't see (and can't see any subfolders) are omitted
   *       <li>folders you can't see (but *can* see >=1 subfolder) appear as &lt;folder id="{id}"
   *           name="{name}"> hierarchy placeholders
   *     </ul>
   */
   ...
   * @zm-api-field-description If set then grantee names are supplied in the <b>d</b> attribute in
   *     <b>&lt;grant></b>. Default: <b>unset</b>
   */
   ...
   * @zm-api-field-description If "view" is set then only the folders with matching view will be
   *     returned. Otherwise folders with any default views will be returned.
   */
   ...
   * @zm-api-field-description If "depth" is set to a non-negative number, we include that many
   *     levels of subfolders in the response. (so if depth="1", we'll include only the folder and
   *     its direct subfolders) If depth is missing or negative, the entire folder hierarchy is
   *     returned
   */
   ...
   * @zm-api-field-description If true, one level of mountpoints are traversed and the target
   *     folder's counts are applied to the local mountpoint. if the root folder as referenced by
   *     <b>{base-folder-id}</b> and/or <b>{fully-qualified-path}</b> is a mountpoint, "tr" is
   *     regarded as being automatically set. Mountpoints under mountpoints are not themselves
   *     expanded.
   */
  ...
  public Boolean isTraverseMountpoints() {
    return ZmBoolean.toBool(traverseMountpoints);
  }
