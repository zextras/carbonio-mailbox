...
public class CommonInstanceDataAttrs {
   ...
   * @zm-api-field-description <b>Your</b> iCalendar PTST (Participation status) <br>
   ...
   * @zm-api-field-description RECURRENCE-ID in "Z" (UTC) timezone, if this is an exception. <br>
   ...
   * @zm-api-field-description Offset from GMT in milliseconds for start time in the time zone of
   *     the instance; this is useful because the instance time zone may not be the same as the time
   *     zone of the requesting client; when rendering an all-day appointment, the client must shift
   *     the appointment by the difference between the instance time zone and its local time zone to
   *     determine the correct date to render the all-day block
   */
   ...
   *     (a.k.a. OutOfOffice) <br>
   *     While free-busy status is simply a property of an event that is set during creation/update,
   *     "actual" free-busy status is the true free-busy state that depends on appt/invite
   *     free-busy, event scheduling status (confirmed vs. tentative vs. cancel), and more
   *     importantly, the attendee's participation status. For example, actual free-busy is
   *     busy-Tentative for an event with Busy free-busy value until the attendee has acted on the
   *     invite.
   */
  ...
  // The hasEx attribute should be set at <appt>/<task> level only and should not be overridden at
  // <inst> level.
  // This is because the presence of exceptions instances is a property of the entire
  // appointment/task.
  /**
   ...
   * @zm-api-field-description Set if attendees have never been notified for this invite; for
   *     organizer only, used in SetAppointmentRequest only
   */
   ...
   * @zm-api-field-description Similar to the "tzo" attribute but for "dueDate". "tzoDue" can be
   *     different from "tzo" if start date and due date lie on different sides of a daylight
   *     savings transition
   */
  ...
  public void setIsRecurring(Boolean isRecurring) {
    this.isRecurring = ZmBoolean.fromBool(isRecurring);
  }

  public void setHasExceptions(Boolean hasExceptions) {
    this.hasExceptions = ZmBoolean.fromBool(hasExceptions);
  }
  ...
  public void setHasAlarm(Boolean hasAlarm) {
    this.hasAlarm = ZmBoolean.fromBool(hasAlarm);
  }
  ...
  public void setDraft(Boolean draft) {
    this.draft = ZmBoolean.fromBool(draft);
  }
  ...
  public Boolean getIsRecurring() {
    return ZmBoolean.toBool(isRecurring);
  }

  public Boolean getHasExceptions() {
    return ZmBoolean.toBool(hasExceptions);
  }
  ...
  public Boolean getHasOtherAttendees() {
    return ZmBoolean.toBool(hasOtherAttendees);
  }

  public Boolean getHasAlarm() {
    return ZmBoolean.toBool(hasAlarm);
  }
