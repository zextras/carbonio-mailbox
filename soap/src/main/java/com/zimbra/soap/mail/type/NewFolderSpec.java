...
@GraphQLType(
    name = GqlConstants.CLASS_NEW_FOLDER_SPEC,
    description = "Input for creating a new folder")
public class NewFolderSpec {

  /**
   * @zm-api-field-tag folder-name
   * @zm-api-field-description If "l" is unset, name is the full path of the new folder; otherwise,
   *     name may not contain the folder separator '/'
   */
  ...
  @GraphQLQuery(
      ...
      description =
          "If parentFolderId is unset, name is the full path of the new folder; otherwise, name may"
              + " not contain the folder separator '/'")
   ...
   * @zm-api-field-description If set, the server will fetch the folder if it already exists rather
   *     than throwing mail.ALREADY_EXISTS
   */
  @XmlAttribute(name = MailConstants.A_FETCH_IF_EXISTS /* fie */, required = false)
  @GraphQLInputField(
      name = GqlConstants.FETCH_IF_EXISTS,
      description =
          "If set, the server will fetch the folder if it already exists rather than throwing"
              + " mail.ALREADY_EXISTS")
  private ZmBoolean fetchIfExists;

  /**
   * @zm-api-field-tag sync-to-url
   * @zm-api-field-description If set (default) then if "url" is set, synchronize folder content on
   *     folder creation
   */
  ...
  public void setParentFolderId(@GraphQLNonNull String parentFolderId) {
  ...
  @GraphQLInputField(
      name = GqlConstants.SYNC_TO_URL,
      description =
          "If set (default) then if url is set, synchronize folder content on folder creation")
  public void setSyncToUrl(Boolean syncToUrl) {
    this.syncToUrl = ZmBoolean.fromBool(syncToUrl);
  }
  ...
  public Boolean getFetchIfExists() {
    return ZmBoolean.toBool(fetchIfExists);
  }

  public Boolean getSyncToUrl() {
    return ZmBoolean.toBool(syncToUrl);
  }
