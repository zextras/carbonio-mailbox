 ...
 * @zm-api-command-description Get information needed for Mini Calendar. <br>
 *     Date is returned if there is at least one appointment on that date. The date computation uses
 *     the requesting (authenticated) account's time zone, not the time zone of the account that
 *     owns the calendar folder.
 */
...
public class GetMiniCalRequest {
  ...
  /**
   * @zm-api-field-description Optional timezone specifier. References an existing server-known
   *     timezone by ID or the full specification of a custom timezone
   */
