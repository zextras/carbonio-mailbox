...
public class FolderActionSelector extends ActionSelector {
   ...
   * @zm-api-field-description For op="empty" - hard-delete all items in the folder (and all the
   *     folder's subfolders if "recursive" is set)
   */
  @XmlAttribute(name = MailConstants.A_RECURSIVE /* recursive */, required = false)
  @GraphQLQuery(
      name = "recursive",
      description = "For op=empty - hard-delete all items in the folder (and all the folder's")
   ...
   * @zm-api-field-description For <b>op="fb"</b> - set the excludeFreeBusy boolean for this folder
   *     (must specify <b>{exclude-free-busy-boolean}</b> for op="fb")
   */
  @XmlAttribute(name = MailConstants.A_EXCLUDE_FREEBUSY /* excludeFreeBusy */, required = false)
  @GraphQLQuery(
      name = "excludeFreeBusy",
      description =
          "For fb operation - set the excludeFreeBusy boolean for this folder (must specify for fb"
              + " operation)")
   ...
   * @zm-api-field-description Use with <b>op="update"</b> to change folder's default view (useful
   *     for migration)
   */
  @XmlAttribute(name = MailConstants.A_DEFAULT_VIEW /* view */, required = false)
  @GraphQLQuery(
      name = "view",
      description = "User with op=update to change folder's default view (usefor for migration)")
  private String view;
  ...
  @GraphQLQuery(
      name = "grants",
      description = "List of grants used with grant and !grant operations")
  ...
  public void setGrant(ActionGrantSelector grant) {
    this.grant = grant;
  }
  ...
  public void addGrant(ActionGrantSelector grant) {
    if (grants == null) this.grants = Lists.newArrayList();
  ...
  @GraphQLQuery(
      name = "excludeFreeBusy",
      description =
          "For op=fb - set the excludeFreeBusy boolean for this folder (must specify for op=fb)")
  public Boolean getExcludeFreebusy() {
    return ZmBoolean.toBool(excludeFreebusy);
  }
