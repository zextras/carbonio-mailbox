...
public class EditheaderTest {
   ...
   */
  @XmlAttribute(
      name = AdminConstants.A_RELATIONAL_COMPARATOR /* relationalComparator */,
      required = false)
  ...
  public void validateEditheaderTest() throws ServiceException {
    ...
    } else if (!CharsetUtil.US_ASCII.equals(
        CharsetUtil.checkCharset(headerName, CharsetUtil.US_ASCII))) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : headerName must be printable ASCII only", null);
    }
    if (comparator == null && headerValue != null && !headerValue.isEmpty()) {
      comparator = ComparatorNames.ASCII_CASEMAP_COMPARATOR;
    }
    if (comparator != null
        && !(comparator.equals(HeaderConstants.I_ASCII_NUMERIC)
            || comparator.equals(ComparatorNames.OCTET_COMPARATOR)
            || comparator.equals(ComparatorNames.ASCII_CASEMAP_COMPARATOR))) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : Invalid comparator type provided: " + comparator, null);
    }
    if (count != null && !count) {
      count = null;
    }
    ...
    if (count != null && count && value != null && value) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : :count and :value, both can not be received", null);
    }
    if (((count != null && count) || (value != null && value)) && relationalComparator == null) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : relational comparator not received", null);
    }
    if (matchType == null
        && count == null
        && value == null
    ...
    if (((count != null && count) || (value != null && value)) && matchType != null) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : :count or :value can not be used with matchType", null);
    }
    if ((count != null && count) && !comparator.equals(HeaderConstants.I_ASCII_NUMERIC)) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : :count can be used only with \""
              + HeaderConstants.I_ASCII_NUMERIC
              + "\"",
          null);
    }
    if ((count == null || !count)
        && (value == null || !value)
        && !StringUtil.isNullOrEmpty(relationalComparator)) {
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : relationalComparator \""
              + relationalComparator
              + "\" can only be used with :count or :value",
          null);
    }
    ...
    if (relationalComparator != null) {
      if (!(relationalComparator.equals(HeaderConstants.GT_OP)
        ...
        throw ServiceException.PARSE_ERROR(
            "EditheaderTest : Invalid relationalComparator provided : \""
                + relationalComparator
                + "\"",
            null);
      }
    ...
    if (comparator != null
      ...
      throw ServiceException.PARSE_ERROR(
          "EditheaderTest : No valid comparator (:value, :count or :is) found for numeric"
              + " operation.",
          null);
    }
  }
