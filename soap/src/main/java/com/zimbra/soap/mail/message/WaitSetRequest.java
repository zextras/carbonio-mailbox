 ...
 * @zm-api-command-description WaitSetRequest optionally modifies the wait set and checks for any
 *     notifications. If <b>block</b> is set and there are no notifications, then this API will
 ...
 *     <p>If the client sends a last known sync token then the notification is calculated by
 *     comparing the accounts current token with the client's last known.
 *     <p>If the client does not send a last known sync token, then notification is based on change
 *     since last Wait (or change since &lt;add> if this is the first time Wait has been called with
 *     the account)
 *     <p>The client may specify a custom timeout-length for their request if they know something
 *     about the particular underlying network. The server may or may not honor this request
 *     (depending on server configured max/min values). See LocalConfig values:
 *     <pre>
 ...
 * </pre>
 *     <p>WaitSet: scalable mechanism for listening for changes to one or more accounts
 */
...
public class WaitSetRequest implements WaitSetReq {
   ...
   * </table>
   *     <p>This is used if <b>types</b> isn't specified for an account
   */
   ...
   * @zm-api-field-description boolean flag. If true, WaitSetResponse will include details of
  ...
  public void setBlock(Boolean block) {
    this.block = ZmBoolean.fromBool(block);
  }
  ...
  public String getLastKnownSeqNo() {
    return lastKnownSeqNo;
  }

  @Override
  public Boolean getBlock() {
    return ZmBoolean.toBool(block, false);
  }
