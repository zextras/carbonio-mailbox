...
@GraphQLType(
    name = GqlConstants.CLASS_CALENDAR_TIME_ZONE_INFO,
    description = "Timezone specification")
public class CalTZInfo implements CalTZInfoInterface {

  /**
   * @zm-api-field-tag timezone-id
   * @zm-api-field-description Timezone ID. If this is the only detail present then this should be
   *     an existing server-known timezone's ID Otherwise, it must be present, although it will be
  ...
  @GraphQLNonNull
  @GraphQLQuery(
      name = GqlConstants.TIME_ZONE_STANDARD_OFFSET,
      description = "Standard Time's offset in minutes from UTC; local = UTC + offset")
  private final Integer tzStdOffset;

  /**
   * @zm-api-field-tag timezone-daylight-offset
   * @zm-api-field-description Daylight Saving Time's offset in minutes from UTC; present only if
   *     DST is used
   */
  @XmlAttribute(name = MailConstants.A_CAL_TZ_DAYOFFSET /* dayoff */, required = true)
  @GraphQLNonNull
  @GraphQLQuery(
      name = GqlConstants.TIME_ZONE_DAY_OFFSET,
      description =
  ...
  @GraphQLQuery(
      name = GqlConstants.STANDARD_TIME_ZONE_ONSET,
      description =
          "Time/rule for transitioning from daylight time to standard time. Either specify"
              + " week/wkday combo, or mday.")
  ...
  @GraphQLQuery(
      name = GqlConstants.DAYLIGHT_TIME_ZONE_ONSET,
      description =
          "Time/rule for transitioning from standard time to daylight time. See `standardTzOnset`"
              + " for more details.")
  ...
  @GraphQLQuery(
      name = GqlConstants.STANDARD_TIME_ZONE_NAME,
      description = "Standard Time component's timezone name")
  ...
  @GraphQLQuery(
      name = GqlConstants.DAYLIGHT_TIME_ZONE_NAME,
      description = "Daylight Saving Time component's timezone name")
  ...
  public TzOnsetInfo getStandardTzOnset() {
    return standardTzOnset;
  }

  @Override
  public TzOnsetInfo getDaylightTzOnset() {
    return daylightTzOnset;
  }
