 ...
 * @zm-api-command-description Applies one or more filter rules to messages specified by a
 *     comma-separated ID list, or returned by a search query. One or the other can be specified,
 *     but not both. Returns the list of ids of existing messages that were affected.
 */
...
public class ApplyOutgoingFilterRulesRequest {
  ...
  public ApplyOutgoingFilterRulesRequest addFilterRule(NamedElement filterRule) {
    this.filterRules.add(filterRule);
    return this;
  }

  public void setMsgIds(IdsAttr msgIds) {
    this.msgIds = msgIds;
  }
