...
public class Folder {
  ...
  public enum View {
    ...
    @XmlEnumValue("virtual conversation")
    VIRTUAL_CONVERSATION("virtual conversation"),
    @XmlEnumValue("remote folder")
  ...
  @GraphQLQuery(
      name = "name",
      description =
          "Name of folder; max length 128; whitespace is trimmed by server; Cannot contain ':',"
              + " '\"', '/', or any character below 0x20")
  private String name;
  ...
  @GraphQLQuery(
      ...
      description = "UUID of parent folder (absent for root folder)")
  ...
  @GraphQLQuery(
      name = "flags",
      description =
          "checked in UI (#), exclude free/(b)usy info, IMAP subscribed (*), does not (i)nherit"
              + " rights from parent, is a s(y)nc folder with external data source, sync is turned"
              + " on(~), folder does n(o)t allow inferiors / children")
  ...
  @GraphQLQuery(
      name = "rgb",
      description = "RGB color in format #rrggbb where r,g and b are hex digits")
  private String rgb;
   ...
   * @zm-api-field-description Number of unread messages with this tag, <b>including</b> those with
   *     the <b>IMAP \Deleted</b> flag set
   */
  @XmlAttribute(name = MailConstants.A_IMAP_UNREAD /* i4u */, required = false)
  @GraphQLQuery(
      name = "imapUnreadCount",
      description =
          "Number of unread messages with this tag, including those with the IMAP \\Deleted flag"
              + " set")
  private Integer imapUnreadCount;

  /**
   * @zm-api-field-tag default-type
   * @zm-api-field-description (optional) Default type for the folder; used by web client to decide
   *     which view to use; <br>
   *     possible values are the same as <b>&lt;SearchRequest></b>'s {types}:
   *     <b>conversation|message|contact|etc</b>
   */
  @XmlAttribute(name = MailConstants.A_DEFAULT_VIEW /* view */, required = false)
  @GraphQLQuery(
      name = "view",
      description = "Default type for the folder; used by web client to decide which view to use.")
   ...
   * @zm-api-field-description Number of non-subfolder items in folder, <b>including</b> those with
   *     the <b>IMAP \Deleted</b> flag set
   */
  ...
  @GraphQLQuery(
      name = "imapItemCount",
      description =
          "Number of non-subfolder items in folder, including those with the IMAP \\Deleted flag"
              + " set")
  ...
  @GraphQLQuery(
      name = "url",
      description = "URL (RSS, iCal, etc.) this folder syncs its contents to")
  private String url;
   ...
   * @zm-api-field-description Number of days for which web client would sync folder data for
   *     offline use
   */
  @XmlAttribute(
      name = MailConstants.A_WEB_OFFLINE_SYNC_DAYS /* webOfflineSyncDays */,
      required = false)
  @GraphQLQuery(
      name = "webOfflineSyncDays",
      description = "Number of days for which web client would sync folder data for offline use")
  private Integer webOfflineSyncDays;

  /**
   * @zm-api-field-tag effective-perms
   * @zm-api-field-description For remote folders, the access rights the authenticated user has on
   *     the folder - will contain the calculated (c)reate folder permission if the user has both
   *     (i)nsert and (r)ead access on the folder
   */
  @XmlAttribute(name = MailConstants.A_RIGHTS /* perm */, required = false)
  @GraphQLQuery(
      name = "rights",
      description =
          "For remote folders, the access rights the authenticated user has on the folder. c | i |"
              + " r")
  private String perm;
   ...
   * @zm-api-field-description URL to the folder in the REST interface for rest-enabled apps (such
   *     as notebook)
   */
  ...
  public Boolean isDeletable() {
    return ZmBoolean.toBool(deletable);
  }
  ...
  public Boolean getRecursive() {
    return ZmBoolean.toBool(recursive);
  }
  ...
  public void setDisableActiveSync(Boolean disableActiveSync) {
    this.recursive = ZmBoolean.fromBool(disableActiveSync);
  }

  public void setWebOfflineSyncDays(Integer webOfflineSyncDays) {
  ...
  public void setDeletable(Boolean deletable) {
    this.deletable = ZmBoolean.fromBool(deletable);
  }
  ...
  public void setRecursive(Boolean recursive) {
    this.recursive = ZmBoolean.fromBool(recursive);
  }
