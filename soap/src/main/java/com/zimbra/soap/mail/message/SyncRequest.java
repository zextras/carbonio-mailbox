 ...
 *           <ul>
 *             <li>If there are now no visible folders, you'll get an empty <b>&lt;folder/></b>
 *                 element
 *             <li>If there are any visible folders, you'll get the full visible folder hierarchy
 *           </ul>
 *       <li>If a {root-folder-id} other than the mailbox root (folder 1) is requested or if not all
 *           folders are visible when syncing to another user's mailbox, all changed items in other
 *           folders are presented as deletes
 *       <li>If the response is a mail.MUST_RESYNC fault, client has fallen too far out of date and
 *           must re-initial sync
 *     </ul>
 */
...
public class SyncRequest {
   ...
   * @zm-api-field-description Earliest Calendar date. If present, omit all appointments and tasks
   *     that don't have a recurrence ending after that time (specified in ms)
   */
   ...
   * @zm-api-field-description Earliest Message date. If present, omit all Messages and
   *     conversations that are older than time (specified in seconds) "Note:value in seconds,
   *     unlike calCutoff which is in milliseconds"
   */
   ...
   * @zm-api-field-description Root folder ID. If present, we start sync there rather than at folder
   *     11
   */
  ...
  public Boolean getTypedDeletes() {
    return ZmBoolean.toBool(typedDeletes);
  }
