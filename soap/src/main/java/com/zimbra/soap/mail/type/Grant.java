...
public class Grant {
   ...
   * @zm-api-field-description Rights - Some combination of (r)ead, (w)rite, (i)nsert, (d)elete,
   *     (a)dminister, workflow action (x), view (p)rivate, view (f)reebusy, (c)reate subfolder
   */
  ...
  @GraphQLQuery(
      name = "permissions",
      description =
          "Rights - Some combination of (r)ead, (w)rite, (i)nsert, (d)elete, (a)dminister, workflow"
              + " action (x), view (p)rivate, view (f)reebusy, (c)reate subfolder")
  private String perm;
   ...
   * @zm-api-field-description Time when this grant expires. For internal/guest grant: If this
   *     attribute is not specified, the expiry of the grant is derived from
   *     internalGrantExpiry/guestGrantExpiry of the ACL it is part of. If this attribute is
   *     specified (overridden), the expiry value can not be greater than the corresponding expiry
   *     value in the ACL. For public grant: If this attribute is not specified, defaults to the
   *     maximum allowed expiry for a public grant. If not specified in the response, defaults to 0.
   *     Value of 0 indicates that this grant never expires.
   */
   ...
   * @zm-api-field-description Name or email address of the principal being granted rights. optional
   *     if {grantee-type} is "all"/"guest"/"pub". When specified in a request, this can be just the
   *     username portion of the address in the default domain.
   */
  @XmlAttribute(name = MailConstants.A_DISPLAY /* d */, required = false)
  @GraphQLQuery(
      name = "granteeName",
      description = "Name or email address of the principal being granted rights.")
