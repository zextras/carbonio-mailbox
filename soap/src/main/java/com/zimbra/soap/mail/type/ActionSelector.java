...
public class ActionSelector {
   ...
   * @zm-api-field-description Comma separated list of item IDs to act on. Required except for
   *     TagActionRequest, where the tags items can be specified using their tag names as an
   *     alternative.
   */
  ...
  @GraphQLQuery(
      name = "ids",
      description =
          "Comma separated list of item IDs to act on.  Required except for op=tag, where the tags"
              + " items can be specified using their tag names as an alternative.")
  protected final String ids;

  /**
   * @zm-api-field-tag operation
   * @zm-api-field-description Operation <br>
   *     For ItemAction -
   *     delete|dumpsterdelete|recover|read|flag|priority|tag|move|trash|rename|update|color|lock|unlock|resetimapuid|copy
   *     <br>
   *     For MsgAction - delete|read|flag|tag|move|update|spam|trash <br>
   *     For ConvAction - delete|read|flag|priority|tag|move|spam|trash <br>
   *     For FolderAction -
   *     read|delete|rename|move|trash|empty|color|[!]grant|revokeorphangrants|url|import|sync|fb|[!]check|update|[!]syncon|retentionpolicy|[!]disableactivesync|webofflinesyncdays
   *     <br>
   *     For TagAction - read|rename|color|delete|update|retentionpolicy <br>
   *     For ContactAction - move|delete|flag|trash|tag|update <br>
  ...
  @GraphQLNonNull
  @GraphQLQuery(name = "operation", description = "The operation to perform")
  protected final String operation;

  /**
   * @zm-api-field-tag [-]constraint
   * @zm-api-field-description List of characters; constrains the set of affected items in a
   *     conversation
   *     <table>
   ...
   * @zm-api-field-description Tags - Comma separated list of integers. DEPRECATED - use "tn"
   *     instead
   */
  @Deprecated
  ...
  @GraphQLQuery(
      name = "nonExistentIds",
      description = "Flag to signify that any non-existent ids should be returned")
  protected ZmBoolean nonExistentIds;

  /**
   * @zm-api-field-tag newly-created-ids
   * @zm-api-field-description Flag to signify that ids of new items should be returned <br>
   *     applies to COPY action
   */
  @XmlAttribute(name = MailConstants.A_NEWLY_CREATED_IDS /* nci */, required = false)
  @GraphQLQuery(
      name = "newlyCreatedIds",
      description =
  ...
  public void setNonExistentIds(boolean r) {
    this.nonExistentIds = ZmBoolean.fromBool(r);
  }
  ;
  /** Use {@link ActionSelector#setTagNames(String)} instead. */
  @Deprecated
  ...
  public boolean getNonExistentIds() {
    return ZmBoolean.toBool(nonExistentIds);
  }
  ;

  public void setNewlyCreatedIds(boolean r) {
    this.newlyCreatedIds = ZmBoolean.fromBool(r);
  }
  ;

  public boolean getNewlyCreatedIds() {
    return ZmBoolean.toBool(newlyCreatedIds);
  }
  ;
