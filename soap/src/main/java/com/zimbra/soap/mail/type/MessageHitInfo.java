...
@GraphQLType(
    name = GqlConstants.CLASS_MESSAGE_HIT_INFO,
    description = "Message search result information containing a list of messages")
public class MessageHitInfo extends MessageInfo implements SearchHit {
  ...
  /**
   * @zm-api-field-description Hit Parts -- indicators that the named parts matched the search
   *     string
   */
  ...
  @GraphQLQuery(
      name = GqlConstants.CONTENT_MATCHED,
      description = "If the message matched the specified query string")
  public Boolean getContentMatched() {
    return ZmBoolean.toBool(contentMatched);
  }

  @GraphQLQuery(
      name = GqlConstants.MESSAGE_PART_HITS,
      description = "Hit Parts, indicators that the named parts matched the search string")
