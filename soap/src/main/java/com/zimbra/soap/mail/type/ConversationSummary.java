...
@GraphQLType(
    name = GqlConstants.CLASS_CONVERSATION_SUMMARY,
...
public class ConversationSummary {
   ...
   * @zm-api-field-description Total number of messages in conversation including those with the
   *     IMAP \Deleted flag set
   */
   ...
   * @zm-api-field-description Same flags as on <b>&lt;m></b> ("sarwfdxnu!?"), aggregated from all
   ...
   * @zm-api-field-description If elided is set, some participants are missing before the first
   *     returned <b>&lt;e></b> element
   */
  ...
  /**
   * @zm-api-field-description Email information for conversation participants if available (elided
   *     will be set if information for some participants is missing)
   */
  ...
  public void setElided(Boolean elided) {
    this.elided = ZmBoolean.fromBool(elided);
  }
  ...
  @GraphQLQuery(
      name = GqlConstants.NUM,
      description = "Number of messages in conversation without IMAP \\Deleted flag set")
  public Integer getNum() {
    return num;
  }

  @GraphQLQuery(
      name = GqlConstants.NUM_UNREAD,
  ...
  @GraphQLQuery(
      name = GqlConstants.TOTAL_SIZE,
      description =
          "Total number of messages in conversation including those with the IMAP \\Deleted flag")
  public Integer getTotalSize() {
    return totalSize;
  }

  @GraphQLQuery(
      name = GqlConstants.FLAGS,
      description =
          "Flags set on the conversation. (u)nread, (f)lagged, has (a)ttachment, (r)eplied, (s)ent"
              + " by me, for(w)arded, calendar in(v)ite, (d)raft, IMAP-\\Deleted (x),"
              + " (n)otification sent, urgent (!), low-priority (?), priority (+)")
  public String getFlags() {
    return flags;
  }

  @GraphQLQuery(
      name = GqlConstants.TAGS,
      description =
          "Tags - Comma separated list of integers.  DEPRECATED - use \"tagNames\" instead")
  ...
  @GraphQLQuery(
      name = GqlConstants.DATE,
      description = "Date (secs since epoch) of most recent message in the converstation")
  public Long getDate() {
    return date;
  }

  @GraphQLQuery(
      name = GqlConstants.ELIDED,
      description = "If elided is set, some participants are missing before the first returned")
  public Boolean getElided() {
    return ZmBoolean.toBool(elided);
  }
  ...
  @GraphQLQuery(
      name = GqlConstants.FRAGMENT,
      description = "First few bytes of the message (probably between 40 and 100 bytes)")
  public String getFragment() {
    return fragment;
  }

  @GraphQLQuery(
      name = GqlConstants.EMAILS,
      description = "Email information for conversation participants, if available")
