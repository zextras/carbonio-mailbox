...
public class EmailInfo implements EmailInfoInterface {
   ...
   * @zm-api-field-description Display name. If we have personal name, first word in "word1 word2"
   *     format, or last word in "word1, word2" format. If no personal name, take string before "@"
   *     in email-address.
   */
   ...
   * @zm-api-field-description Address type. <br>
   *     <b>{address-type}</b> = (f)rom, (t)o, (c)c, (b)cc, (r)eply-to, (s)ender, read-receipt
   *     (n)otification, (rf) resent-from <br>
   *     <br>
   *     Type is only sent when an individual message is returned. In the list of conversations, all
   *     the email addresseses returned for a conversation are a subset of the participants. In the
   *     list of messages in a converstation, the email addressses are the senders. <br>
   *     <br>
   *     Note that "rf" addresses can only be <b>returned</b> on a message; when sending a message,
   *     "rf" is ignored
   */
  ...
  public EmailInfo(String address, String display, String personal, String addressType) {
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.DISPLAY,
      description = "The email addresses display name. Example: \"Jane Doe\" <local@domain.com>")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.ADDRESS_TYPE,
      description =
          "Address type. Example: (f)rom, (t)o, (c)c, (b)cc, (r)eply-to, (s)ender, read-receipt"
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.IS_GROUP_MEMBERS_EXPANDABLE,
      description = "Denotes whether the group can be expanded showing its members")
