...
public class MailSearchParams implements SearchParameters {
   ...
   * @zm-api-field-description Set to <b>1 (true)</b> to include items with the \Deleted tag set in
   *     results
   */
  @XmlAttribute(
      name = MailConstants.A_INCLUDE_TAG_DELETED /* includeTagDeleted */,
      required = false)
  private ZmBoolean includeTagDeleted;

  /**
   * @zm-api-field-tag include-items-tagged-for-delete
   * @zm-api-field-description Set to <b>1 (true)</b> to include items with the \Muted tag set in
   *     results
   */
   ...
   */
  @XmlAttribute(
      name = MailConstants.A_ALLOWABLE_TASK_STATUS /* allowableTaskStatus */,
      required = false)
   ...
   * @zm-api-field-description Start time in milliseconds for the range to include instances for
   *     calendar items from. <br>
   *     If <b>calExpandInstStart</b> and <b>calExpandInstEnd</b> are specified, and the search
   *     types include calendar item types (e.g. appointment), then the search results include the
   *     instances for calendar items within that range in the form described in the description of
   *     the response. <br>
   *     <br>
   *     <b>***IMPORTANT NOTE</b>: Calendar Items that have no instances within that range are
   *     COMPLETELY EXCLUDED from the results (e.g. not even an <b>&lt;appt></b> element. Calendar
   *     Items with no data (such as Tasks with no date specified) are included, but with no
   *     instance information***
   */
  @XmlAttribute(
      name = MailConstants.A_CAL_EXPAND_INST_START /* calExpandInstStart */,
      required = false)
  private Long calItemExpandStart;

  /**
   * @zm-api-field-tag cal-item-expand-end-time-in-msec
   * @zm-api-field-description End time in milliseconds for the range to include instances for
   *     calendar items from.
   */
  ...
  /**
   * @zm-api-field-description Set this flat to <b>1 (true)</b> to search dumpster data instead of
   *     live data.
   */
   ...
   *     <b>NOTE</b>: only ONE of message, conversation may be set. If both are set, the first is
   *     used.
   */
  ...
  // Based on SearchParams.ExpandResults but allows "0" and "false" as synonyms for "none" + "1" for
  // "first"
  /**
   ...
   *     if fetch="{item-id-1,item-id-2,...,item-id-n}", messages with ids in the comma-separated
   *     list will be expanded <br>
   *     if fetch="all", all messages are expanded inline <br>
   *     if fetch="!", only the first message in the conversation will be expanded, whether it's a
   *     hit or not <br>
   *     if fetch="u" (or fetch="unread"), all unread hits are expanded <br>
   *     if fetch="u1" (or fetch="unread-first"), if there are any unread hits, they are expanded,
   *     otherwise the first hit is expanded. <br>
   *     if fetch="u1!", if there are any unread hits, they are expanded, otherwise the first hit
   *     and the first message are expanded (those may be the same) <br>
   *     if fetch="hits", all hits are expanded <br>
   *     if fetch="hits!", all hits are expanded if there are any, otherwise the first message is
   *     expanded <br>
   ...
   *     message hits <br>
   *     + if max="{max-inlined-length}" is specified, inlined body content in limited to the given
   *     length; <br>
   ...
   * @zm-api-field-description Set to <b>1 (true)</b> to cause inlined hits to return HTML parts if
   *     available
   */
   ...
   * @zm-api-field-description If 'needExp' is set in the request, two additional flags may be
   *     included in <b>&lt;e></b> elements for messages returned inline.
   *     <ul>
   *       <li>isGroup - set if the email address is a group
   *       <li>exp - present only when isGroup="1". <br>
   *           Set if the authed user can (has permission to) expand members in this group <br>
   ...
   * @zm-api-field-description Set to <b>0 (false)</b> to stop images in inlined HTML parts from
   *     being "neutered"
   */
   ...
   * @zm-api-field-description By default, only matching messages are included in conversation
   *     results.<br>
   *     Set to <b>1 (true)</b> to include all messages in the conversation, even if they don't
   *     match the search, including items in Trash and Junk folders.
   */
  @ZimbraJsonAttribute
   ...
   * @zm-api-field-description By default, text without an operator searches the CONTENT field. By
   *     setting the {default-field} value, you can control the default operator. Specify any of the
   *     text operators that are available in query.txt, e.g. 'content:' [the default] or
   *     'subject:', etc. The date operators (date, after, before) and the "item:" operator should
   *     not be specified as default fields because of quirks in the search grammar.
   */
  @XmlAttribute(name = MailConstants.A_FIELD /* field */, required = false)
  private String field;

  /**
   * @zm-api-field-description The maximum number of results to return. It defaults to 10 if not
   *     specified, and is capped by 1000
   */
  @XmlAttribute(name = MailConstants.A_QUERY_LIMIT /* limit */, required = false)
  private Integer limit;

  /**
   * @zm-api-field-description Specifies the 0-based offset into the results list to return as the
   *     first result for this search operation. <br>
  ...
  /**
   * @zm-api-field-description if <b>&lt;header></b>s are requested, any matching headers are
   *     included in inlined message hits
   */
   ...
   * @zm-api-field-description If set, Include the list of contact groups this contact is a member
   *     of. <br>
   *     <b>Note</b>: use sparingly, there is a performance penalty associated with computing this
   *     information
   */
  ...
  public void setFullConversation(ZmBoolean fullConversation) {
  ...
  public boolean getIncludeMemberOf() {
    return ZmBoolean.toBool(includeMemberOf, false);
  }
