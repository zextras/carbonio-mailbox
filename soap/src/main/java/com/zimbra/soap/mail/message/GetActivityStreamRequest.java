...
public class GetActivityStreamRequest {
   ...
   * @zm-api-field-description Item ID. If the id is for a Document, the response will include the
   *     activities for the requested Document. if it is for a Folder, the response will include the
   *     activities for all the Documents in the folder and subfolders.
   */
  ...
  /**
   * @zm-api-field-description Optionally <b>&lt;filter></b> can be used to filter the response
   *     based on the user that performed the activity, operation, or both. the server will cache
   *     previously established filter search results, and return the identifier in session
   *     attribute. The client is expected to reuse the session identifier in the subsequent filter
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    return helper.add("id", id).add("queryOffset", queryOffset).add("queryLimit", queryLimit);
  }
