...
public class ConvActionSelector extends ActionSelector {
   ...
   * @zm-api-field-description In case of "move" operation, this attr can also be used to specify
   *     the target folder, in terms of the relative path from the account / data source's root
   *     folder. The target account / data source is identified based on where the messages in this
   *     conversation already reside. If a conversation contains messages belonging of multiple
   *     accounts / data sources then it would not be affected by this operation.
   */
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    ...
    return helper.add("acctRelativePath", acctRelativePath);
  }
