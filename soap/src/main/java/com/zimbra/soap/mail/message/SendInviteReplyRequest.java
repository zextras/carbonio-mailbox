...
public class SendInviteReplyRequest {
   ...
   * @zm-api-field-description Verb - <b>ACCEPT, DECLINE, TENTATIVE</b>, COMPLETED, DELEGATED
   ...
   * @zm-api-field-description Update organizer. true by default. if false then only make the update
   *     locally. <br>
   *     Note that earlier documentation implied incorrectly that if this was false it would be
   *     ignored and treated as being true if an <b>&lt;m></b> element is present. <br>
   *     Also take a note that, if RSVP setting in original invite is not present or FALSE then
   *     <b>updateOrganizer</b> will be treated as FALSE.
   */
   ...
   * @zm-api-field-description If supplied then reply to just one instance of the specified Invite
   ...
   *     The client is responsible for setting the message recipient list in this case (which should
   *     include Organizer, if the client wants to tell the organizer about this response)
   */
  ...
  public void setTimezone(CalTZInfo timezone) {
  ...
  public Boolean getUpdateOrganizer() {
    return ZmBoolean.toBool(updateOrganizer);
  }
