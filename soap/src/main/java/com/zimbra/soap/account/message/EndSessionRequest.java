 ...
 * @zm-api-command-description End the current session, removing it from all caches. Called when the
 *     browser app (or other session-using app) shuts down. Has no effect if called in a
 *     &lt;nosession> context.
 */
...
public class EndSessionRequest {
  ...
  /**
   * @zm-api-field-description flag to clear all web sessions of the user default is 0 (false)
   */
  @XmlAttribute(name = AccountConstants.A_CLEAR_ALL_SOAP_SESSIONS /* all */, required = false)
  private ZmBoolean clearAllSoapSessions;

  /**
   * @zm-api-field-description flag to decide current session will be cleared or not default is 0
   *     (false)
   */
  @XmlAttribute(
      name = AccountConstants.A_EXCLUDE_CURRENT_SESSION /* excludeCurrent */,
      required = false)
