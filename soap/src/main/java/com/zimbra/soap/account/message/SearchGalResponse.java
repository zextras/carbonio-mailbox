...
public class SearchGalResponse {
   ...
   *       <li><b>0 (false)</b> - the underlying search does not support pagination. <b>limit</b>
   *           and <b>offset</b> in the request was not honored
   *     </ul>
   */
  @XmlAttribute(
      name = AccountConstants.A_PAGINATION_SUPPORTED /* paginationSupported */,
      required = false)
  ...
  @GraphQLQuery(
      name = GqlConstants.OFFSET,
      description =
          "The 0-based offset into the results list returned as the first result for this search"
              + " operation.")
  ...
  @GraphQLQuery(
      name = GqlConstants.QUERY_MORE,
      description = "Flags whether there are more results")
  public Boolean getMore() {
    return ZmBoolean.toBool(more);
  }

  @GraphQLQuery(
      name = GqlConstants.PAGINATION_SUPPORTED,
      description =
          "Flag whether the underlying search supported pagination. 1 (true) : limit and offset in"
              + " the request was honored. 0 (false) - the underlying search does not support"
              + " pagination.")
  public Boolean getPagingSupported() {
    return ZmBoolean.toBool(pagingSupported);
  }

  @GraphQLQuery(
      name = GqlConstants.TOKENIZE_KEY,
      description =
          "Valid values: and|or\n"
              + "      Not present if the search key was not tokenized.\n"
              + "      Some clients backtrack on GAL results assuming the results of a more"
              + " specific key is the subset of a more\n"
              + "      generic key, and it checks cached results instead of issuing another SOAP"
              + " request to the server.  \n"
              + "      If search key was tokenized and expanded with AND or OR, this cannot be"
              + " assumed.")
