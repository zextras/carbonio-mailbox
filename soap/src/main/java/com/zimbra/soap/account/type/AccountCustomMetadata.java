...
public class AccountCustomMetadata extends AccountKeyValuePairs implements CustomMetadataInterface {
   ...
   * @zm-api-field-description Metadata section key. <br>
   *     Must be no more than 36 characters long and must be in the format of
   *     <b>{namespace}:{section-name}</b>. Currently the only valid namespace is "zwc". <br>
   *     <br>
   *     Normally present but sometimes an empty element is created to show that custom metadata
   *     info is present but there are no sections to report on.
   */
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.SECTION,
      description =
          "Section. If absent this indicates that CustomMetadata info is present but there are no"
              + " sections to report on.")
  public String getSection() {
    return section;
  }

  public static List<AccountCustomMetadata> fromInterfaces(
      Iterable<CustomMetadataInterface> params) {
  ...
  public MoreObjects.ToStringHelper addToStringInfo(MoreObjects.ToStringHelper helper) {
    ...
    return helper.add("section", section);
  }
