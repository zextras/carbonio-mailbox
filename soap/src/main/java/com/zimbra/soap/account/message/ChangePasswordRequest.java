...
/**
 * <ChangePasswordRequest> <account by="name">...</account> <oldPassword>...</oldPassword>
 * <password>...</password> [<virtualHost>{virtual-host}</virtualHost>] </ChangePasswordRequest>
 *
 * @zm-api-command-auth-required false - This command can be sent before authenticating. The command
 *     handler will internally make sure the old password provided matches the current password of
 *     the account.
...
public class ChangePasswordRequest {
   ...
   * @zm-api-field-description if specified virtual-host is used to determine the domain of the
   *     account name, if it does not include a domain component. For example, if the domain foo.com
   *     has a zimbraVirtualHostname of "mail.foo.com", and an auth request comes in for "joe" with
   *     a virtualHost of "mail.foo.com", then the request will be equivalent to logging in with
   *     "joe@foo.com".
   */
  ...
  public ChangePasswordRequest(
      AccountSelector account, String oldPassword, String newPassword, boolean dryRun) {
    setAccount(account);
    setOldPassword(oldPassword);
    setPassword(newPassword);
    setDryRun(dryRun);
