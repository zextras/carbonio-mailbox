 ...
 * @zm-api-command-description Returns groups the user is either a member or an owner of. <br>
 *     Notes:
 *     <ul>
 ...
 *     </ul>
 *     For example, if isOwner="1" and isMember="none" on the request, and user is an owner and a
 *     member of a group, the returned entry for the group will have isOwner="1", but isMember will
 *     not be present.
 */
...
public class GetAccountDistributionListsRequest {
   ...
   * @zm-api-field-description Set to 1 if the response should include groups the user is an owner
   *     of. <br>
   ...
   * @zm-api-field-description comma-seperated attributes to return. Note: non-owner user can see
   *     only certain attributes of a group. If a specified attribute is not visible to the user, it
  ...
  public GetAccountDistributionListsRequest(
      Boolean owner, MemberOfSelector member, Iterable<String> attrs) {
    setOwnerOf(owner);
    setMemberOf(member);
    setAttrs(attrs);
  }

  public void setOwnerOf(Boolean owner) {
    this.ownerOf = ZmBoolean.fromBool(owner);
  }

  public Boolean getOwnerOf() {
    return ZmBoolean.toBool(ownerOf);
  }

  public void setMemberOf(MemberOfSelector member) {
