 ...
 * @zm-api-command-description Get a distribution list, optionally with ownership information an
 *     granted rights. <br>
 *     Notes:
 *     <ul>
 *       <li>If the authed account is one of the list owners, all (requested) attributes of the DL
 *           are returned in the response. Otherwise only attributes visible and useful to
 *           non-owners are returned.
 *       <li>Specified &lt;rights> are returned only if the authed account is one of the list
 *           owners.
 *       <li>Only grants on this group entry are returned, inherited grants on domain or globalgrant
 *           are not returned.
 *     </ul>
 */
...
public class GetDistributionListRequest extends AttrsImpl {
  ...
  /**
   * @zm-api-field-description return grants for the specified (comma-seperated) rights. <br>
  ...
  public GetDistributionListRequest(
      DistributionListSelector dl, Boolean needOwners, String needRights) {
    this.setDl(dl);
  ...
  public Boolean getNeedOwners() {
    return ZmBoolean.toBool(needOwners);
  }
  ...
  public void setDl(DistributionListSelector dl) {
    this.dl = dl;
  }
