...
public class AccountDataSource implements DataSource {
   ...
   * @zm-api-field-description Name for data source <br>
   *     e.g. "My IMAP Account"
   */
   ...
   * @zm-api-field-description indicates that this datasource is used for one way (incoming) import
   *     versus two-way sync
   */
   ...
   * @zm-api-field-description Name of server <br>
   *     e.g. "imap.myisp.com"
   */
   ...
   * @zm-api-field-description Port number of server <br>
   *     e.g. "143"
   */
  ...
  /**
   * @zm-api-field-description Which security layer to use for connection (cleartext, ssl, tls, or
   *     tls if available). If not set on data source, fallback to the value on global config.
   */
  ...
  /**
   * @zm-api-field-description Polling interval. For instance <b>"10m"</b> <br>
   *     The time interval between automated data imports for a data source. If unset or 0, the data
   *     source will not be scheduled for automated polling. Must be in valid duration format: <br>
   *     {digits}{time-unit}. digits: 0-9, time-unit: [hmsd]|ms. h - hours, m - * minutes, s -
   *     seconds, d - days, ms - milliseconds. <br>
  ...
  /**
   * @zm-api-field-description When forwarding or replying to messages sent to this data source,
   ...
   */
  @XmlAttribute(
      name = MailConstants.A_DS_USE_ADDRESS_FOR_FORWARD_REPLY /* useAddressForForwardReply */,
      required = false)
   ...
   */
  @XmlAttribute(
      name = MailConstants.A_DS_DEFAULT_SIGNATURE /* defaultSignature */,
      required = false)
   ...
   */
  @XmlAttribute(
      name = MailConstants.A_DS_FORWARD_REPLY_SIGNATURE /* forwardReplySignature */,
      required = false)
  ...
  public void setEnabled(Boolean enabled) {
    this.enabled = ZmBoolean.fromBool(enabled);
  }

  @Override
  public void setImportOnly(Boolean importOnly) {
    this.importOnly = ZmBoolean.fromBool(importOnly);
  }
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.ENABLED,
      description = "Flag whether or not the data source is enabled")
  public Boolean isEnabled() {
    return ZmBoolean.toBool(enabled);
  }

  @Override
  @GraphQLQuery(
      name = GqlConstants.IMPORT_ONLY,
      description = "indicates that this datasource is used for one way (incoming) import versus")
  public Boolean isImportOnly() {
    return ZmBoolean.toBool(importOnly);
  }
  ...
  @GraphQLQuery(
      name = GqlConstants.ADS_CONNECTION_TYPE,
      description =
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.USERNAME,
      description = "Login string on {data-source-server}, for example a user name")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.POLLING_INTERVAL,
      description =
          "The time interval between automated data imports for a data source. If unset or 0, the"
              + " data source will not be scheduled for automated polling. Must be in valid"
              + " duration format:\n"
              + "{digits}{time-unit}. digits: 0-9, time-unit: [hmsd]|ms. h - hours, m - * minutes,"
              + " s - seconds, d - days, ms - milliseconds.\n"
              + "If time unit is not specified, the default is s(seconds).")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.EMAIL_ADDRESS,
      description = "Email address for the data-source")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.USE_ADDRESS_FOR_FORWARD_REPLY,
      description =
          "When forwarding or replying to messages sent to this data source, this flags whether or"
              + " not to use the email address of the data source for the from address and the"
              + " designated signature/replyTo of the data source for the outgoing message.")
  public Boolean isUseAddressForForwardReply() {
    return ZmBoolean.toBool(useAddressForForwardReply);
  }
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.FORWARD_REPLY_SIGNATURE,
      description = "Forward / Reply Signature ID for data source")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.FROM_DISPLAY,
      description = "Personal part of email address to put in the from header")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.REPLY_TO_ADDRESS,
      description = "Email address to put in the reply-to header")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.REPLY_TO_DISPLAY,
      description = "Personal part of Email address to put in the reply-to header")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.IMPORT_CLASS,
      description = "Data import class used bt this data source")
  ...
  @Override
  @GraphQLQuery(
      name = GqlConstants.REFRESH_TOKEN,
      description = "refresh token for refreshing data source oauth token")
