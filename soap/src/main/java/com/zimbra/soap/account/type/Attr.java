...
/**
 * e.g. For element named "attr": <attr name="{name}" [pd="true"]>{value}</attr>
 *
 * <p>Note: where the attribute name is "n" rather than "name" use {@link KeyValuePair}
 */
...
public class Attr implements KeyAndValue {
  ...
  // If true, flags that the real value of this attribute has not been provided - i.e. value is set
  // to ""
  /**
   * @zm-api-field-tag attr-perm-denied
   * @zm-api-field-description Flags whether permission has been denied (optional). <br>
   *     If <b>1 (true)</b>, flags that the real value of this attribute has not been provided. <br>
   *     The value is set to ""
   */
  ...
  public Boolean getPermDenied() {
    return ZmBoolean.toBool(permDenied);
  }

  @GraphQLInputField(name = "permDenied", description = "Flags whether permission has been denied")
  public void setPermDenied(Boolean permDenied) {
    this.permDenied = ZmBoolean.fromBool(permDenied);
  }
