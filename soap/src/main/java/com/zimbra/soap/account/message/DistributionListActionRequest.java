 ...
 * @zm-api-command-description Perform an action on a Distribution List <br>
 *     Notes:
 *     <ul>
 *       <li>Authorized account must be one of the list owners
 *       <li>For owners/rights, only grants on the group itself will be modified, grants on domain
 *           and globalgrant (from which the right can be inherited) will not be touched. Only
 *           admins can modify grants on domains and globalgrant, owners of groups can only modify
 *           grants on the group entry.
 *     </ul>
 */
...
public class DistributionListActionRequest extends AttrsImpl {
  ...
  public DistributionListActionRequest(DistributionListSelector dl, DistributionListAction action) {
    this.setDl(dl);
    this.action = action;
  }
