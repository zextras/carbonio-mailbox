...
// MailConstants.A_SEARCH_TYPES (types) Used internally when calling SearchParams.parse, forced to
// be
// MailItem.Type.CONTACT
//
// soap.txt implies that MailConstants.A_GROUPBY (groupBy) is used but doesn't document it.
// Elsewhere, groupBy
...
// for GAL:
// MailConstants.A_FETCH (fetch), MailConstants.A_MAX_INLINED_LENGTH (max) MailConstants.A_WANT_HTML
// (html),
...
public class SearchGalRequest {
   ...
   * @zm-api-field-description Query string. Note: ignored if <b>{gal-search-ref-DN}</b> is
   *     specified
   */
   ...
   * @zm-api-field-description flag whether the <b>{exp}</b> flag is needed in the response for
   *     group entries. Default is unset.
   */
   ...
   * @zm-api-field-description Set this if the "isOwner" flag is needed in the response for group
   *     entries. Default is unset.
   */
   ...
   * @zm-api-field-description Specify if the "isMember" flag is needed in the response for group
   *     entries.
   *     <table>
   ...
   *     priorityAsc|priorityDesc</b> If <b>{sort-by}</b> is "none" then cursors MUST NOT be used,
   *     and some searches are impossible (searches that require intersection of complex sub-ops).
  ...
  public void setNeedIsOwner(Boolean needIsOwner) {
    this.needIsOwner = ZmBoolean.fromBool(needIsOwner);
  }

  public void setNeedIsMember(MemberOfSelector needIsMember) {
    this.needIsMember = needIsMember;
  }

  public void setNeedSMIMECerts(Boolean needSMIMECerts) {
    this.needSMIMECerts = ZmBoolean.fromBool(needSMIMECerts);
  }
  ...
  public Boolean getNeedIsOwner() {
    return ZmBoolean.toBool(needIsOwner);
  }
  ...
  public Boolean getNeedSMIMECerts() {
    return ZmBoolean.toBool(needSMIMECerts);
  }
