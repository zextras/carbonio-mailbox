...
public final class GetInfoResponse {
   ...
   */
  @XmlAttribute(
      name = AccountConstants.A_ATTACHMENT_SIZE_LIMIT /* attSizeLimit */,
      required = false)
  ...
  /**
   * @zm-api-field-description Server version: &lt;major>[.&lt;minor>[.&lt;maintenance>]][build]
   *     &lt;release> &lt;date>[&lt;type>]
   */
   ...
   * @zm-api-field-description 1 (true) if the auth token is a delegated auth token issued to an
   *     admin account
   */
   ...
   * @zm-api-field-description Mailbox quota used in bytes. <br>
   ...
   * @zm-api-field-description Time (in millis) of last write op from this session, or from *any*
   *     SOAP session if we don't have one <br>
   *     Returned only if the command successfully executes on the target user's home mail server
   */
   ...
   * @zm-api-field-description Time (in millis) of last write op from any SOAP session before this
   *     session was initiated, or same as {previous-SOAP-session-time} if we don't have one. <br>
   *     Returned only if the command successfully executes on the target user's home mail server
   */
   ...
   * @zm-api-field-description Number of messages received since the previous soap session, or since
   *     the last SOAP write op if we don't have a session. <br>
   *     Returned only if the command successfully executes on the target user's home mail server
   */
   ...
   *     needs. Only attributes listed in <b>zimbraAccountClientAttrs</b> will be returned.
   */
  @ZimbraKeyValuePairs
  ...
  @XmlElements({
    ...
    @XmlElement(
        ...
        type = AccountCaldavDataSource.class),
  ...
  public void setAttachmentSizeLimit(Long attachmentSizeLimit) {
    this.attachmentSizeLimit = attachmentSizeLimit;
  }

  public void setDocumentSizeLimit(Long documentSizeLimit) {
  ...
  public void setAdminDelegated(Boolean adminDelegated) {
    this.adminDelegated = ZmBoolean.fromBool(adminDelegated);
  }
  ...
  public void setPreviousSessionTime(Long previousSessionTime) {
    this.previousSessionTime = previousSessionTime;
  }

  public void setLastWriteAccessTime(Long lastWriteAccessTime) {
