...
public class SyncGalRequest {
  ...
  /**
   * @zm-api-field-description Flag whether only the ID attributes for matching contacts should be
   *     returned. <br>
   *     If the request has <b>idOnly</b> set to 1 (true), then the response will contain the id
   *     attribute without all the contact fields populated. The sync client then should make a
   *     batch request to the server to fetch the contact fields with
   *     <b>&lt;GetContactsRequest/></b>. <br>
   *     Note: <b>idOnly</b> only works when GAL sync account is configured/enabled. If
   *     <b>idOnly</b> is specified and GAL sync account is not enabled, <b>idOnly</b> will be
   *     ignored.
   */
   ...
   * @zm-api-field-description Flag whether count of remaining records should be returned in
   *     response or not. Note: <b>getCount</b> works only when idOnly is set to true and GAL sync
   *     account is configured/enabled.
   */
   ...
   * @zm-api-field-description Page size control for SyncGalRequest. The maximum entries that can be
   *     returned for every SyncGal Request can be controlled by specifying this limit.
   */
  ...
  public void setIdOnly(Boolean idOnly) {
    this.idOnly = ZmBoolean.fromBool(idOnly);
  }
  ...
  public Boolean getIdOnly() {
    return ZmBoolean.toBool(idOnly);
  }
